...
{
    public class AHCDataUtils : AnDataUtils
    {
        public AHCDataUtils() : base() { }

        public AHCDataUtils(GSTPContext dtContext) : base(dtContext) { }
        ...
        public override bool tongDatThuHoi(List<decimal> ids, string reason, string calledBy)
        {
            try
            {
                ...
                request.AllocatedList = GetAnHanhChinhThuHoi(ids).Select(x => x.ID).ToList();
                ...
                if ("SUCCESS".Equals(thuHoiResponseDTO.Status))
                {
                    ...
                    {
                        List<AHC_TONGDAT> thuhoiList = dt.AHC_TONGDAT.Where(p => thuHoiResponseDTO.AllocatedList.Contains(p.ID)).ToList();
                        foreach (AHC_TONGDAT p in thuhoiList)
                        {
                            // thu hồi thành công chuyển về chưa gửi
                            p.TRANGTHAI = 0;
                            p.NGAYTHUHOI = request.DateReall;
                            p.LYDOTHUHOI = request.Reason;
                            dt.AHC_TONGDAT_DOITUONG.Where(doiTuong => p.ID == (doiTuong.TONGDATID ?? 0)).ToList().ForEach(doiTuong => doiTuong.NGAYGUI = null);
                        }
                    }
                    // update status cho tong dat - thu hoi thanh cong ve chua gui
                    if (thuHoiResponseDTO.ExecutedList != null)
                    {
                        List<AHC_TONGDAT> executedList = dt.AHC_TONGDAT.Where(p => thuHoiResponseDTO.ExecutedList.Contains(p.ID)).ToList();
                        foreach (AHC_TONGDAT p in executedList)
                        {
        ...
        public override bool tongDat(List<decimal> ids)
        {
            try
            {
                List<TongDatDTO> anHanhChinh = GetAnHanhChinh(ids);
                string response = wrapperResult(anHanhChinh, DataUtils.END_POINT_VOFFICE);
                if ("SUCCESS".Equals(response))
                {
                    List<AHC_TONGDAT> sendSuccessList = dt.AHC_TONGDAT.Where(p => ids.Contains(p.ID)).ToList();
                    foreach (AHC_TONGDAT p in sendSuccessList)
                    {
                        // trang thai da gui
                        if (p.TRANGTHAI == 0) p.TRANGTHAI = 1;
                    }
                    foreach (TongDatDTO an in anHanhChinh)
                    {
                        List<decimal?> duongSuIdList = an.DoiTuong.Select(doiTuong => doiTuong.DuongSuId).ToList();
                        List<AHC_TONGDAT_DOITUONG> doiTuongSendSuccess = dt.AHC_TONGDAT_DOITUONG.Where(doiTuong => duongSuIdList.Contains(doiTuong.DUONGSUID) && doiTuong.TONGDATID == an.TongDatId).ToList();
        ...
        {
            List<AHC_TONGDAT> adsTongDats = dt.AHC_TONGDAT.Where(x => ids.Contains(x.ID) && (x.TRANGTHAI >= 0 && x.TRANGTHAI <= 2)).ToList();
            ...
            List<AHC_DON> allADSDon = dt.AHC_DON.Where(x => idADS_DON.Contains(x.ID)).ToList();
            Dictionary<decimal?, AHC_DON> mapAdsDon = new Dictionary<decimal?, AHC_DON>();
            ...
            foreach (AHC_DON adsDon in allADSDon)
            {
            ...
            string conditions = "(";
            foreach (AHC_TONGDAT tongDat in adsTongDats)
            {
            ...
            foreach (TongDatDTO dto in danhSachTongDat)
            {
                List<AHC_TONGDAT_DOITUONG> subList = allDoiTuongs.Where(x => dto.ID.Equals(x.TONGDATID) && x.NGAYGUI == null).ToList();
        ...
        {
            List<AHC_TONGDAT> adsTongDats = dt.AHC_TONGDAT.Where(x => ids.Contains(x.ID) && x.TRANGTHAI == 1).ToList();
            return adsTongDats;
        }

        private TongDatDTO ToDTO(AHC_TONGDAT model, Dictionary<decimal?, DM_BIEUMAU> mapBieuMau, Dictionary<string, decimal?> mapUser, Dictionary<decimal?, AHC_DON> mapADS_DON, Dictionary<decimal?, List<AHC_DON_DUONGSU>> mapDuongSu)
        {
            if (model == null)
                return null;
            ...
            {
                List<AHC_DON_XULY> aDS_DON_XULies = dt.AHC_DON_XULY.Where(x => x.DONID == entity.DONID).ToList();
            ...
            else if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "06-HC")
            {
                ...
                List<AHC_SOTHAM_THULY> aDS_DON_XULies = dt.AHC_SOTHAM_THULY.Where(x => x.DONID == entity.DONID).ToList();
                if (aDS_DON_XULies != null && aDS_DON_XULies.Any())
                {
                    dto.Sothongbao = Convert.ToString(aDS_DON_XULies[0].SOTHONGBAO);
                    dto.Ngaythongbao = Convert.ToDateTime(aDS_DON_XULies[0].NGAYTHONGBAO);
            ...
            {
                List<AHC_PHUCTHAM_THULY> aDS_DON_XULies = dt.AHC_PHUCTHAM_THULY.Where(x => x.DONID == entity.DONID).ToList();
                if (aDS_DON_XULies != null && aDS_DON_XULies.Any())
                {
                    dto.Sothongbao = Convert.ToString(aDS_DON_XULies[0].SOTHONGBAO);
                    dto.Ngaythongbao = Convert.ToDateTime(aDS_DON_XULies[0].NGAYTHONGBAO);
                    dto.Nguoiky = Convert.ToDecimal(aDS_DON_XULies[0].NGUOIKYID);

                }
            }
            else
            {
                List<AHC_FILE> aDS_FILEs = dt.AHC_FILE.Where(x => x.ID == entity.FILEID).ToList();
                if (aDS_FILEs != null && aDS_FILEs.Any())
                {
                    ...
                    {
                        List<AHC_SOTHAM_QUYETDINH> aDS_SOTHAM_QUYETDINHs = dt.AHC_SOTHAM_QUYETDINH.Where(x => x.QUYETDINHID == dM_BIEUMAU.QUYETDINHID && x.DONID == entity.DONID).ToList();
                    ...
                    {
                        List<AHC_PHUCTHAM_QUYETDINH> aDS_PHUCTHAM_QUYETDINHs = dt.AHC_PHUCTHAM_QUYETDINH.Where(x => x.QUYETDINHID == dM_BIEUMAU.QUYETDINHID && x.DONID == entity.DONID).ToList();
        ...
        private DoiTuongDTO ToDTO(TongDatDTO entityTongDat, AHC_TONGDAT_DOITUONG entity, AHC_DON_DUONGSU duongSu)
        {
