...
{
    public class ADSDataUtils : AnDataUtils
    {
        public ADSDataUtils() : base() { }

        public ADSDataUtils(GSTPContext dtContext) : base(dtContext) { }
        ...
        public override bool tongDatThuHoi(List<decimal> ids, string reason, string calledBy)
        {
            try
            {
                ...
                request.AllocatedList = GetAnDanSuThuHoi(ids).Select(x => x.ID).ToList();
                ...
                if ("SUCCESS".Equals(thuHoiResponseDTO.Status))
                {
                    ...
                    {
                        List<ADS_TONGDAT> thuhoiList = dt.ADS_TONGDAT.Where(p => thuHoiResponseDTO.AllocatedList.Contains(p.ID)).ToList();
                        foreach (ADS_TONGDAT p in thuhoiList)
                        {
                            // thu hồi thành công chuyển về chưa gửi
                            p.TRANGTHAI = 0;
                            p.NGAYTHUHOI = request.DateReall;
                            p.LYDOTHUHOI = request.Reason;
                            dt.ADS_TONGDAT_DOITUONG.Where(doiTuong => p.ID == (doiTuong.TONGDATID ?? 0)).ToList().ForEach(doiTuong => doiTuong.NGAYGUI = null);
                        }
                    }
                    // update status cho tong dat - thu hoi thanh cong ve chua gui
                    if (thuHoiResponseDTO.ExecutedList != null)
                    {
                        List<ADS_TONGDAT> executedList = dt.ADS_TONGDAT.Where(p => thuHoiResponseDTO.ExecutedList.Contains(p.ID)).ToList();
                        foreach (ADS_TONGDAT p in executedList)
                        {
        ...
        public override bool tongDat(List<decimal> ids)
        {
            try
            {
                List<TongDatDTO> anDanSu = GetAnDanSu(ids);
                string response = wrapperResult(anDanSu, DataUtils.END_POINT_VOFFICE);
                if ("SUCCESS".Equals(response))
                {
                    List<ADS_TONGDAT> sendSuccessList = dt.ADS_TONGDAT.Where(p => ids.Contains(p.ID)).ToList();
                    foreach (ADS_TONGDAT p in sendSuccessList)
                    {
                        // trang thai da gui
                    ...
                    foreach (TongDatDTO an in anDanSu)
                    {
                        ...
                        List<ADS_TONGDAT_DOITUONG> doiTuongSendSuccess = dt.ADS_TONGDAT_DOITUONG.Where(doiTuong => duongSuIdList.Contains(doiTuong.DUONGSUID) && doiTuong.TONGDATID == an.TongDatId).ToList();
        ...
        public List<TongDatDTO> GetAnDanSu(List<decimal> ids)
        {
            ...
            foreach (TongDatDTO dto in danhSachTongDat)
            {
                List<ADS_TONGDAT_DOITUONG> subList = allDoiTuongs.Where(x => dto.ID.Equals(x.TONGDATID) && x.NGAYGUI == null).ToList();
        ...
        {
            List<ADS_TONGDAT> adsTongDats = dt.ADS_TONGDAT.Where(x => ids.Contains(x.ID) && x.TRANGTHAI == 1).ToList();
            return adsTongDats;
        }

        private TongDatDTO ToDTO(ADS_TONGDAT model, Dictionary<decimal?, DM_BIEUMAU> mapBieuMau, Dictionary<string, decimal?> mapUser, Dictionary<decimal?, ADS_DON> mapADS_DON, Dictionary<decimal?, List<ADS_DON_DUONGSU>> mapDuongSu)
        {
            if (model == null)
                return null;
        ...
        private DoiTuongDTO ToDTO(TongDatDTO entityTongDat, ADS_TONGDAT_DOITUONG entity, ADS_DON_DUONGSU duongSu)
        {
