...
{
    public class APSDataUtils : AnDataUtils
    {
        public APSDataUtils() : base() { }

        public APSDataUtils(GSTPContext dtContext) : base(dtContext) { }
        ...
        public override bool tongDatThuHoi(List<decimal> ids, string reason, string calledBy)
        {
            try
            {
                ...
                request.AllocatedList = GetAnPhaSanThuHoi(ids).Select(x => x.ID).ToList();
                ...
                if ("SUCCESS".Equals(thuHoiResponseDTO.Status))
                {
                    ...
                    {
                        List<APS_TONGDAT> thuhoiList = dt.APS_TONGDAT.Where(p => thuHoiResponseDTO.AllocatedList.Contains(p.ID)).ToList();
                        foreach (APS_TONGDAT p in thuhoiList)
                        {
                            // thu hồi thành công chuyển về chưa gửi
                            p.TRANGTHAI = 0;
                            p.NGAYTHUHOI = request.DateReall;
                            p.LYDOTHUHOI = request.Reason;
                            dt.APS_TONGDAT_DOITUONG.Where(doiTuong => p.ID == (doiTuong.TONGDATID ?? 0)).ToList().ForEach(doiTuong => doiTuong.NGAYGUI = null);
                        }
                    }
                    // update status cho tong dat - thu hoi thanh cong ve chua gui
                    if (thuHoiResponseDTO.ExecutedList != null)
                    {
                        List<APS_TONGDAT> executedList = dt.APS_TONGDAT.Where(p => thuHoiResponseDTO.ExecutedList.Contains(p.ID)).ToList();
                        foreach (APS_TONGDAT p in executedList)
                        {
        ...
        public override bool tongDat(List<decimal> ids)
        {
            try
            {
                List<TongDatDTO> anPhaSan = GetAnPhaSan(ids);
                string response = wrapperResult(anPhaSan, DataUtils.END_POINT_VOFFICE);
                if ("SUCCESS".Equals(response))
                {
                    List<APS_TONGDAT> sendSuccessList = dt.APS_TONGDAT.Where(p => ids.Contains(p.ID)).ToList();
                    foreach (APS_TONGDAT p in sendSuccessList)
                    {
                        // trang thai da gui
                        if (p.TRANGTHAI == 0) p.TRANGTHAI = 1;
                    }
                    foreach (TongDatDTO an in anPhaSan)
                    {
                        List<decimal?> duongSuIdList = an.DoiTuong.Select(doiTuong => doiTuong.DuongSuId).ToList();
                        List<APS_TONGDAT_DOITUONG> doiTuongSendSuccess = dt.APS_TONGDAT_DOITUONG.Where(doiTuong => duongSuIdList.Contains(doiTuong.DUONGSUID) && doiTuong.TONGDATID == an.TongDatId).ToList();
        ...
        {
            List<APS_TONGDAT> adsTongDats = dt.APS_TONGDAT.Where(x => ids.Contains(x.ID) && (x.TRANGTHAI >= 0 && x.TRANGTHAI <= 2)).ToList();
            ...
            List<APS_DON> allADSDon = dt.APS_DON.Where(x => idADS_DON.Contains(x.ID)).ToList();
            Dictionary<decimal?, APS_DON> mapAdsDon = new Dictionary<decimal?, APS_DON>();
            ...
            foreach (APS_DON adsDon in allADSDon)
            {
            ...
            string conditions = "(";
            foreach (APS_TONGDAT tongDat in adsTongDats)
            {
            ...
            foreach (TongDatDTO dto in danhSachTongDat)
            {
                List<APS_TONGDAT_DOITUONG> subList = allDoiTuongs.Where(x => dto.ID.Equals(x.TONGDATID) && x.NGAYGUI == null).ToList();
        ...
        {
            List<APS_TONGDAT> adsTongDats = dt.APS_TONGDAT.Where(x => ids.Contains(x.ID) && x.TRANGTHAI == 1).ToList();
            return adsTongDats;
        }

        private TongDatDTO ToDTO(APS_TONGDAT model, Dictionary<decimal?, DM_BIEUMAU> mapBieuMau, Dictionary<string, decimal?> mapUser, Dictionary<decimal?, APS_DON> mapADS_DON, Dictionary<decimal?, List<APS_DON_DUONGSU>> mapDuongSu)
        {
            if (model == null)
                return null;
            ...
            {
                List<APS_DON_XULY> aDS_DON_XULies = dt.APS_DON_XULY.Where(x => x.DONID == entity.DONID).ToList();
            ...
            else if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "30-DS")
            {
                ...
                List<APS_SOTHAM_THULY> aDS_DON_XULies = dt.APS_SOTHAM_THULY.Where(x => x.DONID == entity.DONID).ToList();
                if (aDS_DON_XULies != null && aDS_DON_XULies.Any())
                {
                    dto.Sothongbao = Convert.ToString(aDS_DON_XULies[0].SOTHONGBAO);
                    dto.Ngaythongbao = Convert.ToDateTime(aDS_DON_XULies[0].NGAYTHONGBAO);
            ...
            {
                List<APS_PHUCTHAM_THULY> aDS_DON_XULies = dt.APS_PHUCTHAM_THULY.Where(x => x.DONID == entity.DONID).ToList();
                if (aDS_DON_XULies != null && aDS_DON_XULies.Any())
                {
                    dto.Sothongbao = Convert.ToString(aDS_DON_XULies[0].SOTHONGBAO);
                    dto.Ngaythongbao = Convert.ToDateTime(aDS_DON_XULies[0].NGAYTHONGBAO);
            ...
            {
                List<APS_FILE> aDS_FILEs = dt.APS_FILE.Where(x => x.ID == entity.FILEID).ToList();
                if (aDS_FILEs != null && aDS_FILEs.Any())
                {
                    ...
                    {
                        List<APS_SOTHAM_QUYETDINH> aDS_SOTHAM_QUYETDINHs = dt.APS_SOTHAM_QUYETDINH.Where(x => x.QUYETDINHID == dM_BIEUMAU.QUYETDINHID && x.DONID == entity.DONID).ToList();
                    ...
                    {
                        List<APS_PHUCTHAM_QUYETDINH> aDS_PHUCTHAM_QUYETDINHs = dt.APS_PHUCTHAM_QUYETDINH.Where(x => x.QUYETDINHID == dM_BIEUMAU.QUYETDINHID && x.DONID == entity.DONID).ToList();
        ...
        private DoiTuongDTO ToDTO(TongDatDTO entityTongDat, APS_TONGDAT_DOITUONG entity, APS_DON_DUONGSU duongSu)
        {
