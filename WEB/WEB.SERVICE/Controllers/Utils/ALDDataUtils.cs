...
{
    public class ALDDataUtils : AnDataUtils
    {
        public ALDDataUtils() : base() { }

        public ALDDataUtils(GSTPContext dtContext) : base(dtContext) { }
        ...
        public override bool tongDatThuHoi(List<decimal> ids, string reason, string calledBy)
        {
            try
            {
                ...
                request.AllocatedList = GetAnLaoDongThuHoi(ids).Select(x => x.ID).ToList();
                ...
                if ("SUCCESS".Equals(thuHoiResponseDTO.Status))
                {
                    ...
                    {
                        List<ALD_TONGDAT> thuhoiList = dt.ALD_TONGDAT.Where(p => thuHoiResponseDTO.AllocatedList.Contains(p.ID)).ToList();
                        foreach (ALD_TONGDAT p in thuhoiList)
                        {
                            // thu hồi thành công chuyển về chưa gửi
                            p.TRANGTHAI = 0;
                            p.NGAYTHUHOI = request.DateReall;
                            p.LYDOTHUHOI = request.Reason;
                            dt.ALD_TONGDAT_DOITUONG.Where(doiTuong => p.ID == (doiTuong.TONGDATID ?? 0)).ToList().ForEach(doiTuong => doiTuong.NGAYGUI = null);
                        }
                    }
                    // update status cho tong dat - thu hoi thanh cong ve chua gui
                    if (thuHoiResponseDTO.ExecutedList != null)
                    {
                        List<ALD_TONGDAT> executedList = dt.ALD_TONGDAT.Where(p => thuHoiResponseDTO.ExecutedList.Contains(p.ID)).ToList();
                        foreach (ALD_TONGDAT p in executedList)
                        {
        ...
        public override bool tongDat(List<decimal> ids)
        {
            try
            {
                List<TongDatDTO> anLaoDong = GetAnLaoDong(ids);
                string response = wrapperResult(anLaoDong, DataUtils.END_POINT_VOFFICE);
                if ("SUCCESS".Equals(response))
                {
                    List<ALD_TONGDAT> sendSuccessList = dt.ALD_TONGDAT.Where(p => ids.Contains(p.ID)).ToList();
                    foreach (ALD_TONGDAT p in sendSuccessList)
                    {
                        // trang thai da gui
                        if (p.TRANGTHAI == 0) p.TRANGTHAI = 1;
                    }
                    foreach (TongDatDTO an in anLaoDong)
                    {
                        List<decimal?> duongSuIdList = an.DoiTuong.Select(doiTuong => doiTuong.DuongSuId).ToList();
                        List<ALD_TONGDAT_DOITUONG> doiTuongSendSuccess = dt.ALD_TONGDAT_DOITUONG.Where(doiTuong => duongSuIdList.Contains(doiTuong.DUONGSUID) && doiTuong.TONGDATID == an.TongDatId).ToList();
        ...
        {
            List<ALD_TONGDAT> adsTongDats = dt.ALD_TONGDAT.Where(x => ids.Contains(x.ID) && (x.TRANGTHAI >= 0 && x.TRANGTHAI <= 2)).ToList();
            ...
            List<ALD_DON> allADSDon = dt.ALD_DON.Where(x => idADS_DON.Contains(x.ID)).ToList();
            Dictionary<decimal?, ALD_DON> mapAdsDon = new Dictionary<decimal?, ALD_DON>();
            ...
            foreach (ALD_DON adsDon in allADSDon)
            {
            ...
            string conditions = "(";
            foreach (ALD_TONGDAT tongDat in adsTongDats)
            {
            ...
            foreach (TongDatDTO dto in danhSachTongDat)
            {
                List<ALD_TONGDAT_DOITUONG> subList = allDoiTuongs.Where(x => dto.ID.Equals(x.TONGDATID) && x.NGAYGUI == null).ToList();
        ...
        {
            List<ALD_TONGDAT> adsTongDats = dt.ALD_TONGDAT.Where(x => ids.Contains(x.ID) && x.TRANGTHAI == 1).ToList();
            return adsTongDats;
        }

        private TongDatDTO ToDTO(ALD_TONGDAT model, Dictionary<decimal?, DM_BIEUMAU> mapBieuMau, Dictionary<string, decimal?> mapUser, Dictionary<decimal?, ALD_DON> mapADS_DON, Dictionary<decimal?, List<ALD_DON_DUONGSU>> mapDuongSu)
        {
            if (model == null)
                return null;
            ...
            {
                List<ALD_DON_XULY> aDS_DON_XULies = dt.ALD_DON_XULY.Where(x => x.DONID == entity.DONID).ToList();
            ...
            else if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "30-DS")
            {
                ...
                List<ALD_SOTHAM_THULY> aDS_DON_XULies = dt.ALD_SOTHAM_THULY.Where(x => x.DONID == entity.DONID).ToList();
                if (aDS_DON_XULies != null && aDS_DON_XULies.Any())
                {
                    dto.Sothongbao = Convert.ToString(aDS_DON_XULies[0].SOTHONGBAO);
                    dto.Ngaythongbao = Convert.ToDateTime(aDS_DON_XULies[0].NGAYTHONGBAO);
            ...
            {
                List<ALD_PHUCTHAM_THULY> aDS_DON_XULies = dt.ALD_PHUCTHAM_THULY.Where(x => x.DONID == entity.DONID).ToList();
                if (aDS_DON_XULies != null && aDS_DON_XULies.Any())
                {
                    dto.Sothongbao = Convert.ToString(aDS_DON_XULies[0].SOTHONGBAO);
                    dto.Ngaythongbao = Convert.ToDateTime(aDS_DON_XULies[0].NGAYTHONGBAO);
            ...
            {
                List<ALD_FILE> aDS_FILEs = dt.ALD_FILE.Where(x => x.ID == entity.FILEID).ToList();
                if (aDS_FILEs != null && aDS_FILEs.Any())
                {
                    ...
                    {
                        List<ALD_SOTHAM_QUYETDINH> aDS_SOTHAM_QUYETDINHs = dt.ALD_SOTHAM_QUYETDINH.Where(x => x.QUYETDINHID == dM_BIEUMAU.QUYETDINHID && x.DONID == entity.DONID).ToList();
                    ...
                    {
                        List<ALD_PHUCTHAM_QUYETDINH> aDS_PHUCTHAM_QUYETDINHs = dt.ALD_PHUCTHAM_QUYETDINH.Where(x => x.QUYETDINHID == dM_BIEUMAU.QUYETDINHID && x.DONID == entity.DONID).ToList();
        ...
        private DoiTuongDTO ToDTO(TongDatDTO entityTongDat, ALD_TONGDAT_DOITUONG entity, ALD_DON_DUONGSU duongSu)
        {
