...
{
    public partial class ChiTietDon : System.Web.UI.Page
    {
        ...
        void Load_DuongSu(DONKK_DON obj)
        {
            ...
            try
            {
                ...
                if (tblDS != null && tblDS.Rows.Count > 0)
                {
                    ...
                    tblDS.Columns.Add("HoKhauThuongTru", typeof(string));
        ...
        DataTable FillData(DataTable tbl, DataRow[] arr)
        {
            ...
            if (arr != null && arr.Length > 0)
            {
                foreach (DataRow row in arr)
                {
                    ...
                    //-------------------------
                    temp = "";
                    loaids = Convert.ToInt16(row["LoaiDuongSu"] + "");
                    if (loaids == 1)
                    {
                    ...
                    {
                        temp = (string.IsNullOrEmpty(row["NDD_DiaChiChiTiet"] + "")) ? "" : row["NDD_DiaChiChiTiet"].ToString();                        
                        temp += (string.IsNullOrEmpty(row["ToChuc_QuanHuyen"] + "")) ? "" : ((temp.Length > 0) ? ", " + row["ToChuc_QuanHuyen"].ToString() : row["ToChuc_QuanHuyen"].ToString());
                    }
