...
namespace WEB.DONKHOIKIEN.UserControl.Popup
{
    public partial class pDKGiaoDichDienTu : System.Web.UI.Page
    {
        DKKContextContainer dt = new DKKContextContainer();
        GSTPContext dtQLA = new GSTPContext();
        ...
        private void ThemMoiDK(String Group_DK, String MaDangKy, String temp, string ListVuViecDKNhanVB, Decimal ToaAnID, String MaLoaiVuViec, Decimal VuAnID, string TenVuViec, string MaVuViec, Decimal QLAn_DuongSuID)
        {
            ...
            obj.TRANGTHAI = 4; //trạng thái chờ xác nhận
        ...
        private void SendMail(String tenvuviec)
        {
        ...
        private void GhiLog(String ListMaVuViecDKNhanVB)
        {
            String CurrUserName = Session[ENUM_SESSION.SESSION_USERTEN] + "";
            decimal UserID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID] + "");
            String Detail_Action = CurrUserName + " - Đăng ký lại giao dịch.<br/>";
        ...
        private void LoadDropTuCachToTung()
        {
        ...
        private void LoadDropCapXetXu(string loai_toa)
        {
        ...
        private void LoadDSDonKienLienQuan(decimal DUONGSUID = 0, string MAVUVIEC = "", string capsx = "")
        {
        ...
        protected void cmdUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                //Nếu chưa đồng ý điều khoản
                lttThongBao.Text = "Vui lòng đọc và đồng ý quy định giao dịch điện tử";
                chkConfirm.Focus();
            }
            else
            {
                ...
                foreach (RepeaterItem item in rpt.Items)
                {
                    ...
                    if (chk.Checked)
                    {
                        ...
                        if (chkChonVuViec.Checked) //Đăng ký giao dịch có đăng ký nhận vb tống đạt
                        {
                            //Nếu đã chọn checkbox chọn vụ việc thì kiểm tra tiếp xem đã chọn vụ việc chưa
                            if (!CheckDK())
                            {
                            ...
                            if (MaLoaiVuViec == "AN_DANSU") //ADS
                            {
                                ADS_DON vuViec = dtQLA.ADS_DON.Where(x => x.MAVUVIEC == MaVuViec).FirstOrDefault();
                                ADS_DON_DUONGSU nguyenDon = dtQLA.ADS_DON_DUONGSU.Where(x => x.DONID == vuViec.ID && x.TUCACHTOTUNG_MA == "NGUYENDON").FirstOrDefault();
                                ADS_DON_DUONGSU biDon = dtQLA.ADS_DON_DUONGSU.Where(x => x.DONID == vuViec.ID && x.TUCACHTOTUNG_MA == "BIDON").FirstOrDefault();

                                TenVuViec = vuViec.TENVUVIEC + "<br/>Nguyên đơn: " + nguyenDon.TENDUONGSU + "<br/>Bị đơn: " + biDon.TENDUONGSU;
                            }
                            else if (MaLoaiVuViec == "AN_HNGD") //AHN
                            {
                                AHN_DON vuViec = dtQLA.AHN_DON.Where(x => x.MAVUVIEC == MaVuViec).FirstOrDefault();
                                AHN_DON_DUONGSU nguyenDon = dtQLA.AHN_DON_DUONGSU.Where(x => x.DONID == vuViec.ID && x.TUCACHTOTUNG_MA == "NGUYENDON").FirstOrDefault();
                                AHN_DON_DUONGSU biDon = dtQLA.AHN_DON_DUONGSU.Where(x => x.DONID == vuViec.ID && x.TUCACHTOTUNG_MA == "BIDON").FirstOrDefault();

                                TenVuViec = vuViec.TENVUVIEC + "<br/>Nguyên đơn: " + nguyenDon.TENDUONGSU + "<br/>Bị đơn: " + biDon.TENDUONGSU;
                            }
                            else if (MaLoaiVuViec == "AN_KDTM") //AKT
                            {
                                AKT_DON vuViec = dtQLA.AKT_DON.Where(x => x.MAVUVIEC == MaVuViec).FirstOrDefault();
                                AKT_DON_DUONGSU nguyenDon = dtQLA.AKT_DON_DUONGSU.Where(x => x.DONID == vuViec.ID && x.TUCACHTOTUNG_MA == "NGUYENDON").FirstOrDefault();
                                AKT_DON_DUONGSU biDon = dtQLA.AKT_DON_DUONGSU.Where(x => x.DONID == vuViec.ID && x.TUCACHTOTUNG_MA == "BIDON").FirstOrDefault();

                                TenVuViec = vuViec.TENVUVIEC + "<br/>Nguyên đơn: " + nguyenDon.TENDUONGSU + "<br/>Bị đơn: " + biDon.TENDUONGSU;
                            }
                            else if (MaLoaiVuViec == "AN_LAODONG") //ALD
                            {
                                ALD_DON vuViec = dtQLA.ALD_DON.Where(x => x.MAVUVIEC == MaVuViec).FirstOrDefault();
                                ALD_DON_DUONGSU nguyenDon = dtQLA.ALD_DON_DUONGSU.Where(x => x.DONID == vuViec.ID && x.TUCACHTOTUNG_MA == "NGUYENDON").FirstOrDefault();
                                ALD_DON_DUONGSU biDon = dtQLA.ALD_DON_DUONGSU.Where(x => x.DONID == vuViec.ID && x.TUCACHTOTUNG_MA == "BIDON").FirstOrDefault();

                                TenVuViec = vuViec.TENVUVIEC + "<br/>Nguyên đơn: " + nguyenDon.TENDUONGSU + "<br/>Bị đơn: " + biDon.TENDUONGSU;
                            }
                            else if (MaLoaiVuViec == "AN_HANHCHINH") //AHC
                            {
                                AHC_DON vuViec = dtQLA.AHC_DON.Where(x => x.MAVUVIEC == MaVuViec).FirstOrDefault();
                                AHC_DON_DUONGSU nguyenDon = dtQLA.AHC_DON_DUONGSU.Where(x => x.DONID == vuViec.ID && x.TUCACHTOTUNG_MA == "NGUYENDON").FirstOrDefault();
                                AHC_DON_DUONGSU biDon = dtQLA.AHC_DON_DUONGSU.Where(x => x.DONID == vuViec.ID && x.TUCACHTOTUNG_MA == "BIDON").FirstOrDefault();

                                TenVuViec = vuViec.TENVUVIEC + "<br/>Nguyên đơn: " + nguyenDon.TENDUONGSU + "<br/>Bị đơn: " + biDon.TENDUONGSU;
                            }
                            else if (MaLoaiVuViec == "AN_PHASAN") //APS
                            {
                                APS_DON vuViec = dtQLA.APS_DON.Where(x => x.MAVUVIEC == MaVuViec).FirstOrDefault();
                                APS_DON_DUONGSU nguyenDon = dtQLA.APS_DON_DUONGSU.Where(x => x.DONID == vuViec.ID && x.TUCACHTOTUNG_MA == "NGUYENDON").FirstOrDefault();
                                APS_DON_DUONGSU biDon = dtQLA.APS_DON_DUONGSU.Where(x => x.DONID == vuViec.ID && x.TUCACHTOTUNG_MA == "BIDON").FirstOrDefault();

                                TenVuViec = vuViec.TENVUVIEC + "<br/>Nguyên đơn: " + nguyenDon.TENDUONGSU + "<br/>Bị đơn: " + biDon.TENDUONGSU;
                            }
                        }
                        else //Đăng ký giao dịch không đăng ký nhận vb tống đạt
                        {
                            TenVuViec = "";
                        }
        ...
        protected void chkChonVuViec_CheckedChanged(object sender, EventArgs e)
        {
            if (chkChonVuViec.Checked)
            {
                pnDSVuViec.Visible = true;
            }
            else
            {
                pnDSVuViec.Visible = false;
            }
        }
