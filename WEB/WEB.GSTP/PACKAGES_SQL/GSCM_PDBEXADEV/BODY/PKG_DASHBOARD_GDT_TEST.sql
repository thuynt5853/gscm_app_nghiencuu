--------------------------------------------------------
--  DDL for Package Body PKG_DASHBOARD_GDT_TEST
--------------------------------------------------------

  CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "GSCM"."PKG_DASHBOARD_GDT_TEST" AS
...
--                                ,2,'<b>Rút Hồ sơ đoàn kiểm tra</b>',3,'<b>Chủ động GĐT qua Bản án</b>',NVL(v.ARRNGUOIKHIEUNAI,v.NGUOIKHIEUNAI)) NGUOIKHIEUNAI,
    ...
    --        AND ((v.PhongBanID = vPhongBanID) OR (vPhongBanID = 0 OR vPhongBanID IS NULL)) 
            ...
            --Kết quả xét xử 
             ...
             -- Thuộc án
                  and ( LoaiAnDB = 0                        
                        or (LoaiAnDB = 99 AND EXISTS(select 1 from GDTTT_DON don 
                                                        where don.LOAICONGVAN in(Select TEM.ID from DM_DATAITEM TEM 
            ...
            -- tìm theo cột
            AND (
            ----COLUMN_2-------cũ chuyển sang----------
                (vKetquathuly = 2 AND NVL(d.LOAIDON,0) NOT IN(4)
                                  AND d.ngaytao> to_Date('01/01/2024','dd/MM/yyyy')
                                  AND D.ISTHULY = 1  
                                  AND d.CD_LOAI = 0
                                  AND  d.TL_NGAY < vNgayThulyTu                            
                                                 and ((d.BAQD_LOAIAN != 1 AND (  EXISTS(SELECT 1 FROM GDTTT_VUAN_KETQUA_DON DKQ   
                                                                    LEFT JOIN GDTTT_VUAN_KETQUA KQ ON DKQ.VUAN_KETQUA_ID = KQ.ID
                                                                    WHERE DKQ.DONID = D.ID
                                                                            ...
                                                                            or
                                                                                not EXISTS( SELECT 1 FROM GDTTT_VUAN_KETQUA DKQ   
                ...
                (vKetquathuly = 3  AND NVL(d.LOAIDON,0) NOT IN(4)
                                      ...
                                      AND D.ISTHULY = 1
                                      AND d.CD_LOAI = 0
                                      AND d.TL_NGAY between vNgayThulyTu and vNgayThulyDen
                    ) 
    ----        ------COLUMN_4	                          
                OR 
                (vKetquathuly = 4  AND NVL(d.LOAIDON,0) NOT IN(4)
                                      AND d.ngaytao > to_Date('01/01/2024','dd/MM/yyyy')
                                      AND D.ISTHULY = 1
                                      AND d.CD_LOAI = 0
                                      AND( 
...
--        AND ((v.PhongBanID = vPhongBanID) OR (vPhongBanID = 0 OR vPhongBanID IS NULL)) 
        ...
        ----COLUMN_2-------Cũ chuyển sang----------
            (vKetquathuly = 2 AND NVL(d.LOAIDON,0) NOT IN(4)
              ...
              ----COLUMN_3-------Mới thụ lý----------
            OR (vKetquathuly = 3 AND NVL(d.LOAIDON,0) NOT IN(4)
                              AND d.ngaytao> to_Date('01/01/2024','dd/MM/yyyy')
                              AND D.ISTHULY = 1 
                              AND d.CD_LOAI = 0
                              AND d.TL_NGAY between vNgayThulyTu and vNgayThulyDen                          
...
--                          )  
                 ...
                 and ( 
                       ...
                       OR ( vKetquathuly = 7 AND( (v.LOAIAN != 1 and EXISTS(SELECT 'X' FROM GDTTT_VUAN_KETQUA KQ 
                                                                        WHERE KQ.VUANID = v.ID 
                                                                            AND KQ.GDQ_NGAY between vNgayThulyTu and vNgayThulyDen
                                                                            AND KQ.GQD_LOAIKETQUA in (2,3,4)
                                                                            )
                       ...
                       OR (vKetquathuly = 8 AND v.NGAYTAO <= vNgayThulyDen
                                             and  (v.LOAIAN != 1 and NOT EXISTS(SELECT 'X' FROM GDTTT_VUAN_KETQUA KQ WHERE KQ.VUANID = v.ID)) 
                       ...
                       OR (vKetquathuly = 11 AND (v.NGAYTHULYXXGDT between vNgayThulyTu and vNgayThulyDen
                                           OR  (v.NGAYTHULYXXGDT < vNgayThulyTu and (v.XXGDTTT_NGAYQD is null  OR to_char(v.XXGDTTT_NGAYQD,'dd/MM/yyyy') = '01/01/0001' ) )
                                           OR  (v.NGAYTHULYXXGDT < vNgayThulyTu and v.XXGDTTT_NGAYQD is not null and to_char(v.XXGDTTT_NGAYQD,'dd/MM/yyyy') != '01/01/0001' and v.XXGDTTT_NGAYQD >= vNgayThulyTu))     
                                  AND v.NGAYTHULYXXGDT is not null
                                  AND v.THAMQUYENXXGDT = 1
                       ...
                       OR (vKetquathuly = 12 AND ( v.XXGDTTT_NGAYQD between vNgayThulyTu and vNgayThulyDen 
                                            OR v.ISRUTKN = 1)
                                  AND v.NGAYTHULYXXGDT is not null
                                  AND v.THAMQUYENXXGDT = 1   
                           )           
                       OR (vKetquathuly = 13 AND ((v.NGAYTHULYXXGDT between vNgayThulyTu and vNgayThulyDen  and (v.XXGDTTT_NGAYQD is null  OR to_char(v.XXGDTTT_NGAYQD,'dd/MM/yyyy') = '01/01/0001' ))
                                           OR  (v.NGAYTHULYXXGDT < vNgayThulyTu and (v.XXGDTTT_NGAYQD is null OR to_char(v.XXGDTTT_NGAYQD,'dd/MM/yyyy') = '01/01/0001' ))
