--------------------------------------------------------
--  DDL for Package Body DLQGC06_AHC
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "GSCM"."DLQGC06_AHC" AS
...
---------------------------------------
                               ...
                               (
                                   ...
                                   FROM
                                       (
                                           ...
                                           FROM
                                               ahc_don              d
                                               LEFT JOIN ahc_don_duongsu      ndds ON d.id = ndds.donid
                                               LEFT JOIN dm_dataitem          tctt ON tctt.ma = ndds.tucachtotung_ma
                                               INNER JOIN ahc_sotham_banan     st ON st.donid = d.id
                                               ...
                                               LEFT JOIN dm_dataitem          ndqtn ON ndqtn.id = ndds.quoctichid
                                                                              AND ndqtn.groupid = 2
                                               LEFT JOIN ahc_sotham_thuly     tl ON tl.donid = d.id
                                               INNER JOIN ahc_don_thamphan     tp ON tp.donid = d.id
                                                                                 AND tp.mavaitro = 'VTTP_GIAIQUYETSOTHAM'
                                               LEFT JOIN dm_canbo             dc ON dc.id = tp.canboid
                                               LEFT JOIN c06_toaan_hanhchinh    db ON db.vuanid = d.id
                                                                                 AND db.duongsuid = ndds.id and db.LOAIBAQD = 1 and db.capxx = 'SO_THAM'
                                           WHERE
                                           ...
                                           FROM
                                               ...
                                               LEFT JOIN ahc_don_duongsu      ndds ON d.id = ndds.donid
                                               LEFT JOIN dm_dataitem          tctt ON tctt.ma = ndds.tucachtotung_ma
                                               INNER JOIN ahc_phuctham_banan   st ON st.donid = d.id
                                               INNER JOIN dm_toaan             dm ON dm.id = st.toaanid
                                                 -- nguyên đơn
                                               LEFT JOIN dm_hanhchinh         ndh1 ON ndh1.id = ndds.tamtruid
                                               LEFT JOIN dm_dataitem          ndqtn ON ndqtn.id = ndds.quoctichid
                                                                              AND ndqtn.groupid = 2
                                               LEFT JOIN c06_toaan_hanhchinh    db ON db.vuanid = d.id
                                                                                 AND db.duongsuid = ndds.id and db.LOAIBAQD = 1 and db.capxx = 'PHUC_THAM'
                                               LEFT JOIN ahc_phuctham_thuly   tl ON tl.donid = d.id
                                               INNER JOIN ahc_don_thamphan     tp ON tp.donid = d.id
                                           ...
                                           FROM
                                               ...
                                               LEFT JOIN ahc_don_duongsu        ndds ON d.id = ndds.donid
                                               LEFT JOIN dm_dataitem            tctt ON tctt.ma = ndds.tucachtotung_ma
                                               INNER JOIN ahc_sotham_quyetdinh   qd ON qd.donid = d.id
    ...
    -- nguyên đơn
                                               ...
                                               LEFT JOIN dm_dataitem            ndqtn ON ndqtn.id = ndds.quoctichid
                                                                              AND ndqtn.groupid = 2
                                               LEFT JOIN c06_toaan_hanhchinh      db ON db.vuanid = d.id
                                                                                 AND db.duongsuid = ndds.id and db.LOAIBAQD = 2 and db.capxx = 'SO_THAM'
                                               LEFT JOIN ahc_sotham_thuly     tl ON tl.donid = d.id
                                               INNER JOIN ahc_don_thamphan       tp ON tp.donid = d.id
                                                                                 AND tp.mavaitro = 'VTTP_GIAIQUYETSOTHAM'
                                               LEFT JOIN dm_canbo               dc ON dc.id = tp.canboid
...
-- TH4: lấy thông tin quyết định phúc thẩm thỏa mãn điều kiện đẩy đi
                                           ...
                                           FROM
                                               ...
                                               LEFT JOIN ahc_don_duongsu          ndds ON d.id = ndds.donid
                                               LEFT JOIN dm_dataitem              tctt ON tctt.ma = ndds.tucachtotung_ma
                                               INNER JOIN ahc_phuctham_quyetdinh   qd ON qd.donid = d.id
                                               INNER JOIN dm_toaan                 dm ON dm.id = qd.toaanid
    -- nguyên đơn
                                               LEFT JOIN dm_hanhchinh             ndh1 ON ndh1.id = ndds.tamtruid
                                               LEFT JOIN dm_dataitem              ndqtn ON ndqtn.id = ndds.quoctichid
                                                                              AND ndqtn.groupid = 2
                                               LEFT JOIN c06_toaan_hanhchinh        db ON db.vuanid = d.id 
                                                                                 AND db.duongsuid = ndds.id and db.LOAIBAQD = 2 and db.capxx = 'PHUC_THAM'
                                               LEFT JOIN ahc_phuctham_thuly       tl ON tl.donid = d.id
                                               INNER JOIN ahc_don_thamphan         tp ON tp.donid = d.id
                                                                                 AND tp.mavaitro = 'VTTP_GIAIQUYETPHUCTHAM'
                                               LEFT JOIN dm_canbo                 dc ON dc.id = tp.canboid
                                               INNER JOIN dm_qd_quyetdinh          qdts ON qd.quyetdinhid = qdts.id
                                           WHERE
                                               qdts.ma IN ('09-HC','40-HC','41-HC','42-HC','43-HC')
                                               AND qd.hieuluctu IS NOT NULL
                                               AND db.id IS NULL
                                               AND d.toaphucthamid = v_toaan_id
                                               AND EXISTS (
                                   ...
                                   FROM
                                       (
                                           SELECT
                                               ...
                                               d.nguoitao,
                                               c.trangthaijobshare,
                                               case WHEN c.capxx = 'SO_THAM' THEN
                                                       d.toaanid
                                                   ELSE
                                                       d.TOAPHUCTHAMID
                                               END AS toaan
                                           FROM
                                               c06_toaan_hanhchinh    c
                                               LEFT JOIN ahc_don              d ON c.vuanid = d.id
                                               LEFT JOIN ahc_don_duongsu      ndds ON ndds.id = c.duongsuid
                                               LEFT JOIN ahc_sotham_banan     st ON st.donid = d.id
                                               LEFT JOIN ahc_phuctham_banan   pt ON pt.donid = d.id
                                               LEFT JOIN dm_toaan             dm ON dm.id = d.toaanid
                                               LEFT JOIN ahc_don_thamphan     tpst ON tpst.donid = d.id
                                                                                  AND tpst.mavaitro = 'VTTP_GIAIQUYETSOTHAM'
                                               LEFT JOIN dm_canbo             dcst ON dcst.id = tpst.canboid
                                               LEFT JOIN ahc_don_thamphan     tppt ON tppt.donid = d.id
                                                                                  AND tppt.mavaitro = 'VTTP_GIAIQUYETPHUCTHAM'
                                               LEFT JOIN dm_canbo             dcpt ON dcpt.id = tppt.canboid
                                           WHERE
                                               c.trangthaiahc = 'HIEU_LUC'
                                               and c.status = '1'

                                       ) a
                                   WHERE a.toaan = v_toaan_id  and
                                       ( v_loaibaqd IS NULL
                                         OR a.loaibaqd = v_loaibaqd )
                                       AND ( v_loaian_id IS NULL
                                             OR a.loaian_id = v_loaian_id )
                           ...
                           FROM
                               (
                                   ...
                                   FROM
                                       ...
                                       LEFT JOIN ahc_don_duongsu      ndds ON d.id = ndds.donid
                                       LEFT JOIN dm_dataitem          tctt ON tctt.ma = ndds.tucachtotung_ma
                                       LEFT JOIN ahc_phuctham_banan   st ON st.donid = d.id
                                       LEFT JOIN dm_toaan             dm ON dm.id = st.toaanid
                                       LEFT JOIN dm_dataitem          ndqtn ON ndqtn.id = ndds.quoctichid
                                                                      AND ndqtn.groupid = 2
                                       LEFT JOIN dm_hanhchinh         dht ON dht.id = ndds.tamtrutinhid
                                       LEFT JOIN dm_hanhchinh         xht ON xht.id = ndds.tamtruid
                                       LEFT JOIN c06_toaan_hanhchinh    db ON db.vuanid = d.id
                                                                         AND db.duongsuid = ndds.id and db.LOAIBAQD = 1 and db.capxx = 'PHUC_THAM'
                                       LEFT JOIN ahc_phuctham_thuly   tl ON tl.donid = d.id
                                       LEFT JOIN ahc_don_thamphan     tp ON tp.donid = d.id
                                                                         AND tp.mavaitro = 'VTTP_GIAIQUYETPHUCTHAM'
                                       LEFT JOIN dm_canbo             dc ON dc.id = tp.canboid
                                   ...
                                   FROM
                                       ...
                                       LEFT JOIN c06_toaan_hanhchinh      db ON db.vuanid = d.id
                                                                         AND db.duongsuid = ndds.id and db.LOAIBAQD = 2 and db.capxx = 'SO_THAM'
                                       LEFT JOIN ahc_phuctham_thuly     tl ON tl.donid = d.id
                                       LEFT JOIN ahc_sotham_quyetdinh   qd ON qd.donid = d.id
                                       LEFT JOIN dm_toaan               dm ON dm.id = qd.toaanid
                                       LEFT JOIN ahc_don_thamphan       tp ON tp.donid = d.id
                                                                         AND tp.mavaitro = 'VTTP_GIAIQUYETSOTHAM'
                                       LEFT JOIN dm_canbo               dc ON dc.id = tp.canboid
                                       LEFT JOIN dm_hanhchinh           dht ON dht.id = ndds.tamtrutinhid
                                       LEFT JOIN dm_hanhchinh           xht ON xht.id = ndds.tamtruid
                                       LEFT JOIN dm_qhpl_tk             qhpl ON qhpl.id = d.qhpltkid
                                       LEFT JOIN dm_qd_quyetdinh          qdts ON qd.quyetdinhid = qdts.id
                                   WHERE
                                       db.id = v_id
                                       and qdts.ma IN (
                                                   '09-HC', '14-HC','15-HC'
...
-- TH4: lấy thông tin quyết định phúc thẩm thỏa mãn điều kiện đẩy đi
                                   ...
                                   FROM
                                       ...
                                       LEFT JOIN c06_toaan_hanhchinh        db ON db.vuanid = d.id
                                                                         AND db.duongsuid = ndds.id  and db.LOAIBAQD = 2 and db.capxx = 'PHUC_THAM'
                                       LEFT JOIN ahc_phuctham_thuly       tl ON tl.donid = d.id
                                       LEFT JOIN ahc_phuctham_quyetdinh   qd ON qd.donid = d.id
                                       LEFT JOIN dm_toaan                 dm ON dm.id = qd.toaanid
                                       LEFT JOIN ahc_don_thamphan         tp ON tp.donid = d.id
                                                                         AND tp.mavaitro = 'VTTP_GIAIQUYETPHUCTHAM'
                                       LEFT JOIN dm_canbo                 dc ON dc.id = tp.canboid
                                   ...
                                   WHERE
                                       db.id = v_id
                                       and qdts.ma IN ('09-HC','40-HC','41-HC','42-HC','43-HC')
                               ) a;
                           ...
                           FROM
                               (
                                   ...
                                   FROM
                                       ...
                                       LEFT JOIN ahc_don              d ON c.vuanid = d.id
                                       LEFT JOIN ahc_don_duongsu      ndds ON d.id = ndds.donid
                                       LEFT JOIN ahc_sotham_banan     st ON st.donid = c.vuanid
                                       LEFT JOIN ahc_phuctham_banan   pt ON pt.donid = c.vuanid
                                   WHERE
                                       c.duongsuid = v_duongsuid
                                       AND c.loaibaqd = v_loaiba_qd
                                       AND c.capxx = v_capxx
                                       AND c.trangthaiahc = 'THU_HOI'
                                   ORDER BY
                                       c.id DESC
                               ) a;
    ...
    BEGIN
        OPEN curreturn FOR SELECT
                           ...
                           FROM
                               ...
                               LEFT JOIN ahc_anphi         p ON p.duongsu_id = d.id
                           WHERE
                               d.isdon = 1
                               AND ( d.donid = vdonid
                                     OR d.donid IN (
    ...
    ---------------------------------------
        OPEN curreturn FOR SELECT
                               ...
                               b.status
                           FROM
                               c06_toaan_hanhchinh b
                           WHERE
                               b.duongsuid = v_duongsuid
                               AND b.trangthaiahc = 'THU_HOI'
                               AND b.id = (
                                   SELECT
                                       MAX(id)
                                   FROM
    ...
    BEGIN
        INSERT INTO c06_toaan_hanhchinh_history (
            lydo_thuhoi,
            noidung,
            dongboid,
            nguoithuhoi,
