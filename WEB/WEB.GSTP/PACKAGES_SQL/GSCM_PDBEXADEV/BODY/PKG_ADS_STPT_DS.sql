CREATE OR REPLACE PACKAGE BODY GSCM.PKG_ADS_STPT_DS AS
...
--      GN.TRUONGHOPGIAONHAN,
     ...
     INNER JOIN (SELECT G.* 
                 ...
                 WHERE (G.MAGIAIDOAN = 2 AND G.TOAANID = v_toaan_id) OR (G.MAGIAIDOAN = 3 AND G.TOAPHUCTHAMID = v_toaan_id)) GD ON A.ID=GD.DONID 
...
--toancau-anhnt-sửa check đã chuyển lại án sơ thẩm
           ...
           LEFT JOIN (SELECT CA.VUANID,i.TEN TRUONGHOPGIAONHAN, CA.NGUOITAO_PHUCTHAM, CA.NGAYTAO_PHUCTHAM FROM DM_DATAITEM i 
                  INNER JOIN ADS_CHUYEN_NHAN_AN CA ON CA.TRUONGHOPGIAONHANID=i.ID WHERE CA.TOANHANID=v_toaan_id 
                     and not exists (select 'x' from ads_don where id =nvl(CA.map_vuanid_new ,0) and magiaidoan = 7)
        ...
        (V_TEN_VU_AN IS NULL  OR ( FN_CONVERT_TO_VN(LOWER(A.TENVUVIEC)) LIKE  '%'||FN_CONVERT_TO_VN(LOWER(V_TEN_VU_AN))||'%' ) )--Tên vụ án
         ...
         AND (V_THAMPHAN_ID IS NULL 
          ...
          AND (v_thuky_id is null--Thư ký
           ...
           --Đơn mới
           OR(V_LOAIDON = 1
           AND (EXISTS( SELECT 'X' FROM ADS_DON_XULY DXL WHERE DXL.DONID = A.ID AND DXL.LOAIGIAIQUYET = 5 ))-- Thụ lý vụ việc
		   )
           --Đơn từ tòa khác chuyển đến
           OR(V_LOAIDON = 2 AND A.LOAIDON=2)
           --Đơn trùng
           OR(V_LOAIDON = 3
           AND (EXISTS( SELECT 'X' FROM ADS_DON_XULY DXL WHERE DXL.DONID = A.ID and DXL.LOAIGIAIQUYET = 6 --Đơn trùng
           )))
           --Đơn không thuộc thẩm quyền
           OR(V_LOAIDON = 4
           AND (EXISTS( SELECT 'X' FROM ADS_DON_XULY DXL WHERE DXL.DONID = A.ID and DXL.LOAIGIAIQUYET = 3 --Trả lại đơn
           )))
...
BEGIN
     ...
     SQL_STRING_VUAN := 'SELECT * FROM ADS_DON A WHERE (A.TOAANID = ' || V_TOAAN_ID || ' OR A.TOAPHUCTHAMID = '|| V_TOAAN_ID || ') AND A.MAGIAIDOAN != 7 ';
     ...
     '   WITH 
                ...
                V_TABLE_THAMPHAN_GIAIQUYET AS (
                               SELECT TP.DONID,
                                      MAX(TP.ID) KEEP (DENSE_RANK FIRST ORDER BY TP.NGAYPHANCONG DESC) AS ID,
                                      MAX(TP.CANBOID) KEEP (DENSE_RANK FIRST ORDER BY TP.NGAYPHANCONG DESC) AS CANBOID,
                                      TP.MAVAITRO,
                                      (CASE 
                                         WHEN TP.MAVAITRO IN (''VTTP_GIAIQUYETSOTHAM'',''VTTP_GIAIQUYETDON'') THEN 2
                                         WHEN TP.MAVAITRO = ''VTTP_GIAIQUYETPHUCTHAM'' THEN 3
                                       END) MAGIAIDOAN,
                                      MAX(NGAYPHANCONG) AS NGAYPHANCONG
                               FROM ADS_DON_THAMPHAN TP
                               JOIN V_TABLE_VUAN b ON TP.DONID = b.ID
                               GROUP BY DONID, MAVAITRO
                            ),
                ...
                V_TABLE_BC_KC AS (SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER 
                                  FROM (SELECT DS.ID,DS.DONID,DS.TENDUONGSU,DS.TUCACHTOTUNG_MA,ROW_NUMBER() OVER (PARTITION BY DS.DONID ORDER BY DS.ISDAIDIEN DESC,DS.TENDUONGSU) ROWNUMBER
                                        ...
                                        WHERE EXISTS(SELECT 1 FROM ADS_SOTHAM_KHANGCAO KC WHERE KC.DONID= b.ID AND KC.DUONGSUID=DS.ID AND KC.LOAIKHANGCAO !=2))BC 
                                  WHERE BC.ROWNUMBER <=3)
        ';
    ...
    '   SELECT DISTINCT A.ID,
                ...
                BA.ID BA_ID,
                QD.ID QD_ID,
				STBAKT.ID STBAKT_ID, -- VNPT HUYLQ 02/12/2025 update thụ lý lại ST
				STQDKT.ID STQDKT_ID
     ...
     '    
            INNER JOIN ( 
                SELECT * FROM ADS_DON_GIAIDOAN WHERE MAGIAIDOAN = 2 AND TOAANID = '|| V_TOAAN_ID ||'
                UNION ALL
                SELECT * FROM ADS_DON_GIAIDOAN WHERE MAGIAIDOAN = 3 AND TOAPHUCTHAMID = '|| V_TOAAN_ID ||'
            ) GD ON A.ID = GD.DONID     
          
            LEFT JOIN ADS_ANPHI AI ON A.ID=AI.DONID

            LEFT JOIN DVCQG_THANH_TOAN TT ON TT.ANPHI_ID=AI.ID AND TT.MALOAIVUVIEC=2

			-- VNPT HUYLQ 02/12/2025 lấy thông tin vụ án kết thúc ở ST
			LEFT JOIN ( SELECT STBA.ID, STBA.DONID FROM ADS_SOTHAM_BANAN STBA WHERE STBA.SOBANAN IS NOT NULL) STBAKT ON STBAKT.DONID = A.ID AND GD.MAGIAIDOAN=2
			LEFT JOIN ( SELECT STQD.ID, STQD.DONID FROM ADS_SOTHAM_QUYETDINH STQD LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID = STQD.QUYETDINHID WHERE QD.KET_THUC = 1 AND QD.ISSOTHAM = 1 AND QD.ISDANSU = 1) STQDKT ON STQDKT.DONID = A.ID AND GD.MAGIAIDOAN=2
            ...
            LEFT JOIN (SELECT CA.VUANID,I.TEN TRUONGHOPGIAONHAN, CA.NGUOITAO_PHUCTHAM, CA.NGAYTAO_PHUCTHAM 
                      ...
                      WHERE CA.TOANHANID='|| V_TOAAN_ID ||' AND NOT EXISTS (SELECT 1 FROM V_TABLE_VUAN WHERE ID =NVL(CA.MAP_VUANID_NEW ,0) AND MAGIAIDOAN = 7)                      )GN ON  GN.VUANID=A.ID
...
--        ;
     ...
     IF(NVL(LENGTH(V_QHPL),0) > 0) THEN
        ...
        SQL_STRING_WHERE := SQL_STRING_WHERE || 
            ' AND ( FN_CONVERT_TO_VN(LOWER(A.TENVUVIEC)) LIKE ''%'|| VV_QHPL ||'%''  
                  )'
        ;
     END IF;
     ...
     IF(NVL(LENGTH(V_SO_QD),0) > 0) THEN
        SQL_STRING_WHERE := SQL_STRING_WHERE ||
            ' AND ((EXISTS(SELECT 1 FROM ADS_SOTHAM_BANAN QSV WHERE UPPER(QSV.SOBANAN) LIKE ''%'|| V_SO_QD ||'%'' AND A.ID=QSV.DONID  )
                    OR EXISTS(SELECT 1 FROM ADS_SOTHAM_QUYETDINH QSV WHERE UPPER(QSV.SOQD) LIKE ''%'||V_SO_QD||'%'' AND A.ID=QSV.DONID  )
                    OR EXISTS(SELECT 1 FROM ADS_PHUCTHAM_BANAN QSV WHERE UPPER(QSV.SOBANAN) LIKE ''%'||V_SO_QD||'%'' AND A.ID=QSV.DONID  )
                    OR EXISTS(SELECT 1 FROM ADS_PHUCTHAM_QUYETDINH QSV WHERE UPPER(QSV.SOQD) LIKE ''%'||V_SO_QD||'%'' AND A.ID=QSV.DONID  )
                   )
                  )'
        ;
     END IF;    
     ...
     IF(NVL(LENGTH(V_NGAY_QD),0) > 0) THEN
        SQL_STRING_WHERE := SQL_STRING_WHERE ||
            ' AND ((EXISTS(SELECT 1 FROM ADS_SOTHAM_BANAN QSV WHERE TO_CHAR(QSV.NGAYMOPHIENTOA,''dd/MM/yyyy'') = '|| V_NGAY_QD ||' AND A.ID = QSV.DONID  )
                   OR EXISTS(SELECT 1 FROM ADS_SOTHAM_QUYETDINH QSV WHERE TO_CHAR(QSV.NGAYQD,''dd/MM/yyyy'') = '|| V_NGAY_QD ||' AND A.ID = QSV.DONID  )
                   OR EXISTS(SELECT 1 FROM ADS_PHUCTHAM_BANAN QSV WHERE TO_CHAR(QSV.NGAYMOPHIENTOA,''dd/MM/yyyy'') = '|| V_NGAY_QD ||' AND A.ID = QSV.DONID  )
                   OR EXISTS(SELECT 1 FROM ADS_PHUCTHAM_QUYETDINH QSV WHERE TO_CHAR(QSV.NGAYQD,''dd/MM/yyyy'') = '|| V_NGAY_QD ||' AND A.ID = QSV.DONID  )
                   )
                  )'
        ;
     END IF; 
            ...
            ' AND ( (TLS.DONID IS NOT NULL AND GD.MAGIAIDOAN = 2 AND ( TLS.NGAYTHULY > '|| VV_NGAYTHULY_DEN ||'))
                     OR (TLS.DONID IS NULL AND GD.MAGIAIDOAN = 2 AND ( A.NGAYNHANDON >= '|| VV_NGAYTHULY_TU ||' 
                         ...
                         OR (TLPT.DONID IS NULL AND GD.MAGIAIDOAN=3 AND EXISTS( SELECT 1 
                                                ...
                                                AND ( EXISTS( SELECT 1 FROM ADS_CHUYEN_NHAN_AN CNA
                                                ...
                                                AND ( XLD.NGAYGQ_YC <= '|| VV_NGAYTHULY_DEN ||' 
                                                                              OR EXISTS( SELECT 1 FROM ADS_DON_THAMPHAN PC 
                                                                                         WHERE PC.MAVAITRO = ''VTTP_GIAIQUYETDON''
                                                                                             AND XLD.NGAYGQ_YC IS NULL
                                                                                             AND PC.NGAYPHANCONG <= '|| VV_NGAYTHULY_DEN ||'
                                                                                             AND A.ID = PC.DONID
    ...
    IF(V_TINHTRANG_GIAIQUYET LIKE '1') THEN --Chưa giải quyết xong
        SQL_STRING_WHERE := SQL_STRING_WHERE ||
            '   
                AND 
                (
                    NOT EXISTS (SELECT 1 FROM ADS_DON_XULY XL WHERE XL.DONID = A.ID AND XL.LOAIGIAIQUYET IN (1,3))
                )
                AND 
                 (   
                     (   GD.MAGIAIDOAN = 2 -- SƠ THẨM
                         AND
                         (
                             ( EXISTS (SELECT 1 FROM ADS_SOTHAM_QUYETDINH QDVA INNER JOIN DM_QD_QUYETDINH QD ON QD.ID = QDVA.QUYETDINHID AND QD.KET_THUC = 1 
                                       WHERE A.ID = QDVA.DONID AND QDVA.NGAYQD >= '|| VV_DENNGAY ||')
                                       OR
                               EXISTS (SELECT 1 FROM ADS_SOTHAM_BANAN BA WHERE A.ID = BA.DONID AND BA.NGAYTUYENAN >= '|| VV_DENNGAY ||')

                             )
                             OR 
                             ( NOT EXISTS (SELECT 1 FROM ADS_SOTHAM_QUYETDINH QDVA INNER JOIN DM_QD_QUYETDINH QD ON QD.ID = QDVA.QUYETDINHID AND QD.KET_THUC = 1 WHERE A.ID = QDVA.DONID)
                               AND NOT EXISTS (SELECT 1 FROM ADS_SOTHAM_BANAN BA WHERE A.ID = BA.DONID)
                     ...
                     (   GD.MAGIAIDOAN = 3 -- PHÚC THẨM
                         AND
                         (
                             ( EXISTS (SELECT 1 FROM ADS_PHUCTHAM_QUYETDINH QDVA INNER JOIN DM_QD_QUYETDINH QD ON QD.ID = QDVA.QUYETDINHID AND QD.KET_THUC = 1 
                                       WHERE A.ID = QDVA.DONID AND QDVA.NGAYQD >= '|| VV_DENNGAY ||' )
                                       OR
                               EXISTS (SELECT 1 FROM ADS_PHUCTHAM_BANAN BA WHERE A.ID = BA.DONID AND BA.NGAYTUYENAN >= '|| VV_DENNGAY ||')

                             )
                             OR 
                             ( NOT EXISTS (SELECT 1 FROM ADS_PHUCTHAM_QUYETDINH QDVA INNER JOIN DM_QD_QUYETDINH QD ON QD.ID = QDVA.QUYETDINHID AND QD.KET_THUC = 1 WHERE A.ID = QDVA.DONID)
                               AND NOT EXISTS (SELECT 1 FROM ADS_PHUCTHAM_BANAN BA WHERE A.ID = BA.DONID)
    ...
    IF(V_TINHTRANG_GIAIQUYET LIKE '4') THEN --đã lên lịch xét xử
        SQL_STRING_WHERE := SQL_STRING_WHERE ||
            ' AND (EXISTS ( SELECT 1 
                        ...
                        OR EXISTS( SELECT 1 FROM ADS_PHUCTHAM_QUYETDINH PTQDVA
                                       LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID = PTQDVA.QUYETDINHID
                                       LEFT JOIN DM_QD_LOAI QDL ON QDL.ID = QD.LOAIID
    ...
    IF (V_AN_KET_THUC = 1) THEN
            SQL_STRING_WHERE := SQL_STRING_WHERE || '
                AND(EXISTS (SELECT 1 FROM ADS_DON_GIAIDOAN BC
                                        WHERE  BC.DONID = A.ID
                                          AND (
    ...
    ELSIF(V_AN_KET_THUC = 0) THEN
        SQL_STRING_WHERE := SQL_STRING_WHERE || '
            AND(NOT EXISTS (SELECT 1 FROM ADS_DON_GIAIDOAN BC
                                    WHERE BC.DONID = A.ID
                                      AND (
                                        (BC.MAGIAIDOAN = 2 AND BC.TOAANID = '|| V_TOAAN_ID ||') OR
                                        (BC.MAGIAIDOAN in (3,7) AND BC.TOAPHUCTHAMID = '|| V_TOAAN_ID ||')
    ...
    SQL_Finall:= SQL_STRING_WITH ||
               'SELECT  TTT.ID,
                ...
                PKG_STPT_ADS_GS.NOIDUNG_KHANGCAO_DANHSACH(A_ID) AS KHANGCAO_ST,
    ...
    INSERT INTO SQL_LOGS(log_time, sql_text) VALUES (SYSDATE, SQL_Finall);
    COMMIT;
    OPEN CURRETURN FOR SQL_Finall; 

END ADS_DON_SEARCH_TURNING;
