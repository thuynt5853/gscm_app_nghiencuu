--------------------------------------------------------
--  DDL for Package Body PKG_STPT_AHN_GS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "GSCM"."PKG_STPT_AHN_GS" AS
   ...
   ------------------dinh nghĩa bảng lay 1 ban ghi moi nhat với mục đích chỉ chạy 1 lần.
      --THAMPHAN --TOANCAU-12102023-TAMNC
		...
		FROM (
            ...
            FROM  AHN_KCKNQDK_PHUCTHAM_HDXX WHERE MAVAITRO IN('THAMPHAN','THAMPHANHDXX','THAMPHANDUKHUYET')) TP
        ...
        OR TP.MAVAITRO = 'THAMPHANDUKHUYET'));
         --AHN_KCKNQDK_PHUCTHAM_QUYETDINH
                ...
                FROM
                    (
                        SELECT
                            ...
                            QDL.MA
                        FROM
                            AHN_KCKNQDK_PHUCTHAM_QUYETDINH   PQD
...
--      GN.TRUONGHOPGIAONHAN,
                                      ...
                                      DECODE(XLD.LOAIGIAIQUYET, 1, '- Đã chuyển đơn',
                                             ...
                                             CASE
                                                 WHEN(A.VUANGOCID > 0
                                                      AND A.IS_TACHAN IS NULL) THEN
                                                     (
                                                         SELECT
                                                             '</br>- Đã nhập vụ án (Vụ án nhập: Thụ lý số:<b> '
                                                             || TO_CHAR(T.SOTHULY)
                                                             || '</b> ngày<b> '
                                                             || TO_CHAR(T.NGAYTHULY, 'dd/MM/yyyy')
                                                         FROM
                                                             AHN_DON            D
                                                 ...
                                                 WHEN(A.VUANGOCID > 0
                                                      AND A.IS_TACHAN = 1) THEN
                                                     (
                                                         SELECT
                                                             '</br>- Đã tách vụ án (Vụ án tách: Thụ lý số:<b> '
                                                             || TO_CHAR(T.SOTHULY)
                                                             || '</b> ngày<b> '
                                                             || TO_CHAR(T.NGAYTHULY, 'dd/MM/yyyy')
                                                         FROM
...
--      INNER JOIN ADS_DON_GIAIDOAN GD ON A.ID=GD.DONID
                                      ...
                                      LEFT JOIN AHN_CHUYEN_NHAN_AN   NA ON NA.MAP_VUANID_NEW = A.ID
...
----- BA Or QD----------------------------------------------
                                      ...
                                      LEFT JOIN (
                                          SELECT
                                              PTQDVA.*
                                          FROM
                                              AHN_KCKNQDK_PHUCTHAM_QUYETDINH   PTQDVA
                                      ...
                                      LEFT JOIN (
                                          SELECT
                                              ...
                                              || '</b>' QD_PT
                                          FROM
                                              AHN_KCKNQDK_PHUCTHAM_QUYETDINH PTQD
                                          WHERE
                                              QUYETDINHID = 213
         ...
         --- Lay ra trang thai giai quyet don
                                      ...
                                      LEFT JOIN (
                                          ...
                                          FROM
                                              AHN_KCKNQDK_PHUCTHAM_QUYETDINH PTQDVA --QUYẾT ĐỊNH 
             ...
             ------bị cáo kháng cáo lấy cho phúc thẩm    
                                          ...
                                          GROUP BY
                                              BC.DONID
                                               ...
                                               AND A.MAGIAIDOAN = 7   -- thêm mã giai đoạn =7
...
LEFT JOIN (
SELECT DSKC.DONID, '<br /><i>kháng cáo: </i> <br />'
                               || ' Tên đương sự: '
                               || DSKC.TENDUONGSU
                               || '<br/>'
                               ||
                LISTAGG(' - Ngày kháng cáo: '
                        || TO_CHAR(DSKC.NGAYKHANGCAO, 'dd/MM/yyyy'), '<br/>') WITHIN GROUP(
                    ORDER BY DSKC.TENDUONGSU
                )
            KHANGCAO_ST
            FROM ( SELECT DS.DONID, DS.TENDUONGSU
                                    || '-'
                                    || I.TEN TENDUONGSU,
                          KC.NGAYKHANGCAO,
                          KC.ID
                   FROM AHN_SOTHAM_KHANGCAO   KC
                   INNER JOIN AHN_DON_DUONGSU       DS ON KC.DUONGSUID = DS.ID
                                  ...
                                  WHERE
                                      ...
                                      AND ( V_UTTP IS NULL
                                          ...
                                          SELECT
                                              'X'
                                          FROM
                                              AHN_KCKNQDK_PHUCTHAM_THULY TLPT
                                          WHERE
                                      ...
                                      AND ( V_MA_VU_AN IS NULL
                                            OR ( LOWER(A.MAVUVIEC) LIKE LOWER(V_MA_VU_AN) ) )   --Mã vụ án
   ...
   exists( 
                                          ...
                                          FROM
                                              ...
                                              LEFT JOIN DM_CANBO         CB ON CB.ID = PCTP_GQ.CANBOID
                                          WHERE
                                              TP.MAVAITRO = 'VTTP_GIAIQUYETPHUCTHAM' and TP.DONID = A.ID and cbb.id=V_THAMPHAN_ID 
                                          GROUP BY
                                              TP.DONID
                                      ))                  
                --26/06/2023 tuyennh them tim kiem theo ten tham phan end--                    
         ...
         -----
                                            ...
                                            AND ( V_SOTHULY IS NULL
                                                  OR ( UPPER(TLPT.SOTHULY) = UPPER(V_SOTHULY) ) )--Số Thụ lý
         ...
         ----- 
            ...
            --GQ đơn;V_GQDON -- -- 
               ...
               --hieu check theo trạng thái vụ án hành chính        
                                            ...
                                            AND ( ( V_TRANGTHAIVUAN = 0
                                                    AND ( A.VUANGOCID = 0
                                                          OR A.VUANGOCID IS NULL ) )
                                                  OR ( V_TRANGTHAIVUAN = 1
                                                       AND A.VUANGOCID > 0
                                                       AND A.IS_TACHAN IS NULL )
           ...
           --------------
                                            ...
                                            AND ( V_SO_QD IS NULL--Số BA/QĐ
                                                  ...
                                                  OR ( EXISTS (
                                          SELECT 'X' FROM AHN_SOTHAM_BANAN QSV
                                          WHERE
                                              UPPER(QSV.SOBANAN) LIKE '%' || V_SO_QD || '%'
                                              AND NA.VUANID = QSV.DONID
                                      )
                                                       OR EXISTS (
                                          SELECT 'X' FROM AHN_SOTHAM_QUYETDINH QSV
                                          WHERE UPPER(QSV.SOQD) LIKE '%' || V_SO_QD || '%'
                                              AND NA.VUANID = QSV.DONID
                                      )
                                                       OR EXISTS (
                                          SELECT 'X' FROM AHN_KCKNQDK_PHUCTHAM_QUYETDINH QSV
                                          WHERE
                                              UPPER(QSV.SOQD) LIKE '%' || V_SO_QD || '%'
                                              AND A.ID = QSV.DONID
                                      ) ) )
                                          ...
                                          SELECT 'X' FROM AHN_SOTHAM_BANAN QSV
                                          WHERE
                                              TO_CHAR(QSV.NGAYMOPHIENTOA, 'dd/MM/yyyy') = V_NGAY_QD
                                              AND NA.VUANID = QSV.DONID
                                      )
                                                       OR EXISTS (
                                          SELECT 'X' FROM AHN_SOTHAM_QUYETDINH QSV
             ...
             AND (V_KETQUA IS NULL
                OR( V_KETQUA = 1
                    AND( EXISTS (
                        SELECT 'X' 
                        FROM AHN_KCKNQDK_PHUCTHAM_QUYETDINH   QSV 
                        LEFT JOIN DM_KETQUA_PHUCTHAM                KQPT ON KQPT.ID = QSV.KETQUAID
                        LEFT JOIN DM_QD_QUYETDINH                   QD ON QD.ID = QSV.QUYETDINHID 
                        LEFT JOIN DM_QD_LOAI                        QDL ON QDL.ID = QD.LOAIID
                        WHERE QSV.DONID = A.ID
                        AND KQPT.ID = 101 --Giữ nguyên quyết định của Tòa án cấp sơ thẩm
                        AND GD.MAGIAIDOAN = 7
                    )))
                OR( V_KETQUA = 2
                    AND( EXISTS (
                        ...
                        LEFT JOIN DM_QD_LOAI                        QDL ON QDL.ID = QD.LOAIID
                        WHERE QSV.DONID = A.ID
                        AND KQPT.ID = 103 --Hủy quyết định của Tòa án cấp sơ thẩm và chuyển hồ sơ vụ án cho Tòa án cấp sơ thẩm để tiếp tục giải quyết vụ án
                        AND GD.MAGIAIDOAN = 7
                    )))
                OR( V_KETQUA = 3
                    AND( EXISTS (
                        ...
                        LEFT JOIN DM_QD_LOAI                        QDL ON QDL.ID = QD.LOAIID
                        WHERE QSV.DONID = A.ID
                        AND KQPT.ID = 21 --Sửa toàn bộ bản án, quyết định sơ thẩm
                        AND GD.MAGIAIDOAN = 7
                    )))
                OR( V_KETQUA = 4
                    AND( EXISTS (
                        ...
                        LEFT JOIN DM_QD_LOAI                        QDL ON QDL.ID = QD.LOAIID
                        WHERE QSV.DONID = A.ID
                        AND KQPT.ID = 102 -- Sửa quyết định của Tòa án cấp sơ thẩm
                        AND GD.MAGIAIDOAN = 7
                    )))
             )
...
--                                              'X'
--                                          FROM
--                                              AHN_KCKNQDK_PHUCTHAM_THULY PTTL
--                                          WHERE
--                                              ( NOT EXISTS (
--                                                  SELECT
--                                                      'X'
                              ...
                              SELECT
                                ...
                                AND(
                                  ...
                                  (
                                    ...
                                    (
                                      V_TUNGAY IS NULL 
                                      OR TLPT.NGAYTHULY >= VV_TUNGAY
                                    ) 
                                    AND (
                                      V_DENNGAY IS NULL 
                                      OR TLPT.NGAYTHULY <= VV_DENNGAY
                                    ) 
                                    AND NOT EXISTS(
                                      SELECT 
                                        'x' 
                                  ...
                                  or(
                                    EXISTS(
                                      ...
                                      FROM 
                                        ...
                                        LEFT JOIN DM_QD_LOAI QDL ON QDL.ID = QD.LOAIID 
                                      WHERE 
                                        QDL.MA = 'DVARXX' --QDL.MA = 'DVARXX' == Đưa vụ án ra xét xử
                                        AND (
                                          V_TUNGAY IS NULL 
...
--                                                             OR A.NGAYTAO <= VV_DENNGAY )
                                               AND 
                                               --kiem tra phai co thu ly
                                                ( TLPT.DONID IS NOT NULL and TLPT.NGAYTHULY IS NOT NULL)
                                                and
                                                ( V_TUNGAY IS NULL
                                                OR TLPT.NGAYTHULY >= VV_TUNGAY )
                                                AND ( V_DENNGAY IS NULL
                                       ...
                                       --truong hop da co thong tin tham phan bat theo ngay phan cong tham phan
                                            NOT EXISTS (
                                          SELECT
                                              'x'
                                          FROM
...
--                                      ) 
                                EXISTS (
                                          ...
                                          FROM
                                              AHN_CHUYEN_NHAN_AN   
                                          WHERE
                                              TOACHUYENID = V_TOAAN_ID and VUANID = A.ID
                                            AND ( V_TUNGAY IS NULL
...
---------------------------------------------
    ...
    PROCEDURE AHN_PHUCTHAMQDK_THULY_GETMAXTT (
        VDONVIID    IN    NUMBER,
        VFROMDATE   IN    DATE,
        VTODATE     IN    DATE,
        CURRETURN   OUT   SYS_REFCURSOR
    ...
    BEGIN
        OPEN CURRETURN FOR SELECT
                           ...
                           FROM
                               ...
                               INNER JOIN AHN_DON                      D ON D.ID = T.DONID
                           WHERE
                               D.TOAANID = VDONVIID
                               AND T.NGAYTHULY BETWEEN VFROMDATE AND VTODATE;

    END AHN_PHUCTHAMQDK_THULY_GETMAXTT;

--Lấy ra list TGTT tamnc 1-4

   PROCEDURE AHN_PHUCTHAM_KCKN_TGTT_GETLIST (
        VDONID      IN    NUMBER,
        CURRETURN   OUT   SYS_REFCURSOR
    ) IS
    BEGIN
        ...
        --,h1.MA_TEN as Tamtru
                               ...
                               D.TOA_GIAIQUYET_ID
                           FROM
                               AHN_KCKNQDK_PHUCTHAM_THAMGIATOTUNG   D
    ...
    END AHN_PHUCTHAM_KCKN_TGTT_GETLIST;
  ...
  --Lấy ra list HDXX tamnc 1-4
    ...
    PROCEDURE AHN_KCKNQDK_PHUCTHAM_HDXX_GETLIST (
        VDONID      IN    NUMBER,
        CURRETURN   OUT   SYS_REFCURSOR
    ) IS
        CHECKBANANPT NUMBER;
    ...
    END AHN_KCKNQDK_PHUCTHAM_HDXX_GETLIST;

    --------------------------------------------

       --Lấy ra list Quyết định tamnc 1-4

    PROCEDURE AHN_KCKNQDK_PHUCTHAM_QUYETDINH_GETLIST (
        VDONID      IN    NUMBER,
        CURRETURN   OUT   SYS_REFCURSOR
    ) IS
    BEGIN
        OPEN CURRETURN FOR SELECT
                               ...
                               Q.TOA_GIAIQUYET_ID
                           FROM
                               AHN_KCKNQDK_PHUCTHAM_QUYETDINH   Q
    ...
    END AHN_KCKNQDK_PHUCTHAM_QUYETDINH_GETLIST;
---------------------------------------------------

    PROCEDURE AHN_KCKNQDK_PHUCTHAM_THULY_GETLIST (
        VDONID      IN    NUMBER,
        CURRETURN   OUT   SYS_REFCURSOR
    ) IS
    ...
    BEGIN
        ...
        OPEN CURRETURN FOR SELECT
                          ...
                          FROM
                              AHN_KCKNQDK_PHUCTHAM_THULY   T
                              LEFT JOIN AHN_FILE                     F ON F.ID = T.FILEID
                              LEFT JOIN (
                                  SELECT
                                      A.ID,
                                      A.TEN
    ...
    END AHN_KCKNQDK_PHUCTHAM_THULY_GETLIST;
--Lấy ra list bản án tamnc 29-3

    PROCEDURE AHN_PHUCTHAMQDK_BANANQUYETDINH_GETLIST (
        VDONID      IN    NUMBER,
        CURRETURN   OUT   SYS_REFCURSOR
    ) IS
        COUNTBANANST INT;
    ...
    END AHN_PHUCTHAMQDK_BANANQUYETDINH_GETLIST;
        ---------------------------------- Lấy ra quyết định số "" tamnc 1-4

    PROCEDURE AHN_DM_QUYETDINH_VUAN_PTQDK (
        CURRETURN OUT SYS_REFCURSOR
    ) AS
    BEGIN
        OPEN CURRETURN FOR SELECT
    ...
    END AHN_DM_QUYETDINH_VUAN_PTQDK;
    ...
    BEGIN
        FOR ITEMS IN (
            ...
            FROM
                ...
                LEFT JOIN AHN_SOTHAM_QUYETDINH   QD ON QD.ID = T2.SOQDBA
                                                     AND T2.LOAIKHANGCAO IN (
                    1,
                    2
                ) --> Kháng cáo quyết định --toancau-anhnt kc tđc

                LEFT JOIN AHN_SOTHAM_BANAN       BA ON BA.ID = T2.SOQDBA
            ...
            FROM
                AHN_SOTHAM_KHANGCAO     T2
                INNER JOIN AHN_DON_THAMGIATOTUNG   T3 ON T2.DUONGSUID = T3.ID
                                                       AND ( T3.DONID = T2.DONID )
  ...
  -- Tổng hợp kháng nghị
        ...
        FOR ITEMS IN (
            ...
            FROM
                ...
                LEFT JOIN AHN_SOTHAM_QUYETDINH   QD ON QD.ID = T2.BANANID
                                                     AND T2.LOAIKN IN (
                    1,
                    2
                ) --> Kháng cáo quyết định

                LEFT JOIN AHN_SOTHAM_BANAN       BA ON BA.ID = T2.BANANID
                                                 AND T2.LOAIKN = 0 --> Kháng cáo bản án

                LEFT JOIN AHN_SOTHAM_RUTKCKN     RUT ON T2.ID = RUT.IDKCKN
            WHERE
                T2.DONID = (
                    CASE
                        WHEN NOT EXISTS (
...
-----------------------------------------------------
  ...
  -- TOANCAU 01-04-2023 LẤY THÊM TRƯỜNG
            ...
            SELECT
                R_CHUYENAN_STPT_GS(V_STT => ROW_NUMBER() OVER(
                    ORDER BY
                        DXL.NGAYGQ_YC DESC, D.TENVUVIEC
                                   ...
                                   V_LYDOID => NULL, V_CHUYENNHANID => NULL)-- TOANCAU 01-04-2023 LẤY THÊM TRƯỜNG

            BULK COLLECT
            INTO V_ARRAY
            FROM
                AHN_DON        D
                JOIN AHN_DON_XULY   DXL ON DXL.DONID = D.ID
            WHERE
                ( 1 = (
                    CASE
                        WHEN ( VMAVUVIEC
                ...
                AND ( ( D.TOAANID = VTOAANID
                      ...
                      OR ( D.TOAPHUCTHAMID = VTOAANID
                           AND ( D.MAGIAIDOAN = 3
                                 OR D.MAGIAIDOAN = 7 ) --toancau-anhnt. thêm check pt tđc
...
----------------------
    ...
    -- TOANCAU 01-04-2023 LẤY THÊM TRƯỜNG
                ...
                R_CHUYENAN_STPT_GS(V_STT => ROW_NUMBER() OVER(
                    ORDER BY
                        CNA.NGAYGIAO DESC, D.TENVUVIEC
                                   ...
                                   V_NGAYCHUYEN => CNA.NGAYGIAO, V_NGAYNHAN => CNA.NGAYNHAN, V_NGAYTHULY => NULL, V_NOIDUNG => CNA.LYDOCHUYEN
                                   || CNA.NOIDUNG, V_TOANHAN => TA.TEN,
                                   V_LYDOID => CNA.TRUONGHOPGIAONHANID, V_CHUYENNHANID => CNA.ID)-- TOANCAU 01-04-2023 LẤY THÊM TRƯỜNG
        ...
        OPEN CURRETURN FOR SELECT
                              ...
                              PA.V_TOANHAN,
                              PA.V_LYDOID,
                              PA.V_CHUYENNHANID-- TOANCAU 01-04-2023 LẤY THÊM TRƯỜNG
    ...
    BEGIN
        SELECT
            R_NHANAN_STPT(V_STT => ROW_NUMBER() OVER(
                ORDER BY
                    A.NGAYGIAO DESC
        ...
        FROM
            ...
            JOIN AHN_DON_XULY         DXL ON DXL.DONID = A.VUANID
        WHERE
        ...
        PKG_STPT_DS_BC.FILL_HN_NHANAN(V_ARRAY);
        OPEN CURRETURN FOR SELECT
                               *
                           FROM
                               TABLE ( V_ARRAY ) A
   ...
   --toancau-tamnc 
    ...
    BEGIN
        BEGIN
            L_NOIDUNG_KC :='';
            FOR R_DS IN ( 
				SELECT DS.ID,DS.DONID,DS.TENDUONGSU || '-' || I.TEN TENDUONGSU  
				FROM AHN_DON_DUONGSU       DS
				LEFT JOIN DM_DATAITEM           I ON I.MA = DS.TUCACHTOTUNG_MA
				WHERE DS.DONID = V_DONID
                AND EXISTS (SELECT 'X' FROM AHN_SOTHAM_KHANGCAO WHERE DUONGSUID =DS.ID)
				)
			LOOP
				L_NGAYKHANGCAO :='';
				FOR R_KC IN (SELECT KC.* FROM AHN_SOTHAM_KHANGCAO   KC WHERE KC.DUONGSUID = R_DS.ID AND KC.DONID = V_DONID)
				LOOP
			...
			END LOOP;
            IF L_NOIDUNG_KC||' ' = ' ' THEN RETURN '';
            ELSE RETURN '<br /><i>Kháng cáo: </i>' || L_NOIDUNG_KC;
            END IF;
        EXCEPTION
