--------------------------------------------------------
--  DDL for Package Body PKG_STPT_ADS_GS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "GSCM"."PKG_STPT_ADS_GS" AS
   ...
   ------------------dinh nghĩa bảng lay 1 ban ghi moi nhat với mục đích chỉ chạy 1 lần.
		...
		FROM (
            ...
            FROM  ADS_KCKNQDK_PHUCTHAM_HDXX WHERE MAVAITRO IN('THAMPHAN','THAMPHANHDXX','THAMPHANDUKHUYET')) TP
...
------ - lấy thông tin số ngày kháng cáo + đương sự toancau
                                      LEFT JOIN (
                                          SELECT
                                                DSKC.DONID, '<br /><i>kháng cáo: </i> <br />' || LISTAGG(' Tên đương sự: ' || DSKC.TENDUONGSU ||DSKC.NGAYKHANGCAO, '<br/>') WITHIN GROUP( ORDER BY DSKC.TENDUONGSU ) KHANGCAO_ST
                                            FROM
                                                ( SELECT
...
--                    ) 
           ...
           --------------
                                      AND ( V_SO_QD IS NULL--Số BA/QĐ
                                            OR ( EXISTS (
                                          SELECT 'X' FROM ADS_KCKNQDK_PHUCTHAM_QUYETDINH QSV
                                          WHERE
             ...
             --5/7/2023 TOAN CAU - quyet(
             AND (V_KETQUA IS NULL
                OR( V_KETQUA = 1
                    AND( EXISTS (
                        SELECT 'X' 
                        FROM ADS_KCKNQDK_PHUCTHAM_QUYETDINH   QSV 
...
--                                          WHERE
--                                              (NOT EXISTS(SELECT 'X' FROM ADS_PHUCTHAM_BANAN PTBA WHERE PTBA.DONID=PTTL.DONID)
--                                            AND NOT EXISTS (
--                                                  SELECT
--                                                      'X'
--                                                  FROM
...
--                                                          '72-DS'
--                                                      )
----                                                      and QDL.MA not in ( 'DVARXX','CNTT','CVA','DC')
                                ...
                                --05/07/2023 tuyennh sua tim kiem theo "Da chuyen vu an"
                                      ...
                                      SELECT 'x'
                                          FROM
                                              ADS_CHUYEN_NHAN_AN  
                                          WHERE
                                              TOACHUYENID = V_TOAAN_ID AND VUANID = A.ID AND GD.MAGIAIDOAN = 7
                                              AND ( V_TUNGAY IS NULL
...
-------------------------------
    ...
    BEGIN
        OPEN CURRETURN FOR SELECT
                           ...
                           FROM
                               ...
                               LEFT JOIN DM_CANBO                         C ON C.ID = Q.NGUOIKYID
                           WHERE
                               Q.DONID = VDONID -- AND Q.TOA_GIAIQUYET_ID = Q.TOAANID
                           ORDER BY
                               Q.NGAYQD;
  ...
  -- Tổng hợp kháng nghị
    ...
    BEGIN
                ...
                R_CHUYENAN_STPT_GS(V_STT => ROW_NUMBER() OVER(
                    ORDER BY
                        DXL.NGAYGQ_YC DESC, D.TENVUVIEC
                                   ...
                                   V_NGAYCHUYEN => NULL, V_NGAYNHAN => NULL, V_NGAYTHULY => NULL, V_NOIDUNG => NULL, V_TOANHAN =>
                                   NULL,
                ...
                R_CHUYENAN_STPT_GS(V_STT => ROW_NUMBER() OVER(
                    ORDER BY
                        CNA.NGAYGIAO DESC, D.TENVUVIEC
                                   ...
                                   V_NGAYCHUYEN => CNA.NGAYGIAO, V_NGAYNHAN => CNA.NGAYNHAN, V_NGAYTHULY => NULL, V_NOIDUNG => CNA
                                   .LYDOCHUYEN || CNA.NOIDUNG, V_TOANHAN => TA.TEN,
        ...
        PKG_STPT_DS_BC.FILL_DS_CHUYENAN(V_ARRAY);
        OPEN CURRETURN FOR SELECT
                               PA.V_STT,
                               PA.V_VUANID,
                               PA.V_TOAANID,
    ...
    BEGIN
            ...
            R_NHANAN_STPT(V_STT => ROW_NUMBER() OVER(
                ORDER BY
                    A.NGAYGIAO DESC
                          ...
                          V_LOAIVUVIEC => 'Dân sự', V_LOAIVV => 1, V_TRUONGHOPGIAONHAN => I.TEN)
        BULK COLLECT
        INTO V_ARRAY
        FROM
            ADS_CHUYEN_NHAN_AN   A
        ...
        PKG_STPT_DS_BC.FILL_DS_NHANAN(V_ARRAY);
        OPEN CURRETURN FOR SELECT
                               *
                           FROM
                               TABLE ( V_ARRAY ) A
    ...
    BEGIN
        BEGIN
			...
			FOR R_DS IN ( 
                ...
                AND EXISTS (SELECT 'X' FROM ADS_SOTHAM_KHANGCAO WHERE DUONGSUID =DS.ID)
				)
			LOOP
				L_NGAYKHANGCAO :='';
				FOR R_KC IN (SELECT KC.* FROM ADS_SOTHAM_KHANGCAO   KC WHERE KC.DUONGSUID = R_DS.ID AND KC.DONID = V_DONID)
				LOOP
