...
BEGIN
SELECT COUNT(*) INTO V_COUNTS FROM AHC_SOTHAM_THULY TL  WHERE TL.DONID = vDONID AND TL.NGAYTHULY < TO_DATE('01/05/2022','dd/MM/yyyy');
--IF(V_COUNTS>=1)THEN 
--    OPEN curReturn FOR  
...
IF(V_COUNTS>=1)THEN 
    OPEN curReturn FOR  
        ...
        From AHC_DON_DUONGSU d 
        LEFT JOIN AHC_ANPHI x ON x.DUONGSU_IDS like '%' || d.ID || '%'
        LEFT JOIN DON_MIENANPHI dm ON x.ID = dm.ANPHI_ID AND dm.loaian = 6
        Where d.DONID=vDONID
