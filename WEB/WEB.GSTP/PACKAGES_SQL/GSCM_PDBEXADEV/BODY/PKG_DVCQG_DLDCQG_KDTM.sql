--------------------------------------------------------
--  DDL for Package Body PKG_DVCQG_DLDCQG_KDTM
--------------------------------------------------------

  CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "GSCM"."PKG_DVCQG_DLDCQG_KDTM" AS
...
BEGIN	
    ...
    TenLoaiAn := 'Kinh doanh, thương mại';
                ...
                A.* FROM (
                    --TH1: Bản án co hieu luc 
                          ...
                          SELECT 
                             ...
                             (SELECT COUNT(*)
                             FROM AKT_DON_DUONGSU DS where DS.DONID = B.DONID AND (
                                   (
                                      (DS.LOAIDUONGSU = 1 
                            ...
                            FROM 
                                ...
                                LEFT JOIN AKT_DON_DUONGSU NDS ON NDS.DONID = B.DONID AND NDS.TUCACHTOTUNG_MA = 'NGUYENDON' AND NDS.ISDAIDIEN = 1                         
                                AND (
                                  (NDS.LOAIDUONGSU = 1 
                               ...
                               OR (NDS.LOAIDUONGSU <> 1)
                                ...
                                LEFT JOIN AKT_DON_DUONGSU BDS ON BDS.DONID = B.DONID AND BDS.TUCACHTOTUNG_MA = 'BIDON' AND BDS.ISDAIDIEN = 1                        
                                AND (
                                  (BDS.LOAIDUONGSU = 1 
                               ...
                               OR (BDS.LOAIDUONGSU <> 1)
                                ...
                                LEFT JOIN AKT_DON D ON D.ID = B.DONID
                                LEFT JOIN AKT_PHUCTHAM_THULY TLPT ON TLPT.DONID = D.ID
                                ...
                                LEFT JOIN AKT_SOTHAM_THULY TL ON TL.DONID = B.DONID
                            WHERE 
                        ...
                        SELECT 
                            ...
                            FROM 
                               ...
                               OR (BDS.LOAIDUONGSU <> 1)
                                ...
                                AND KH.LINHVUC = LOAIAN AND KH.STATUS = 1) K ON B.DONID = K.DONID
                                LEFT JOIN AKT_PHUCTHAM_THULY TL ON TL.DONID = B.DONID
                            WHERE 
...
--               TH3:  Quyết định sơ thẩm về ly hôn  
                  SELECT 
                                ...
                                '' AS ghiChu,
                                 ...
                                 '' as SO_DUONGSU_DONGBO,
                             (SELECT COUNT(*)
                             FROM AKT_DON_DUONGSU DS where DS.DONID = Q.DONID AND (
                                   (
                                      (DS.LOAIDUONGSU = 1 
                                    AND (DS.QUOCTICHID <> 2 OR DS.XACTHUC_DLDCQG = 1))
                                 ...
                                 '-1' as TRANGTHAIBAQD
                            FROM 
                                AKT_SOTHAM_QUYETDINH Q
                                INNER JOIN DM_TOAAN DM ON DM.ID = Q.TOAANID
                                LEFT JOIN AKT_DON_DUONGSU NDS ON NDS.DONID = q.DONID AND NDS.TUCACHTOTUNG_MA = 'NGUYENDON' AND NDS.ISDAIDIEN = 1                         
                                AND (
                                  (NDS.LOAIDUONGSU = 1 
                                AND (NDS.QUOCTICHID <> 2 OR NDS.XACTHUC_DLDCQG = 1))
                               ...
                               OR (NDS.LOAIDUONGSU <> 1)
                                ...
                                left join DM_HANHCHINH h1 on h1.ID=NDS.TAMTRUID
                                LEFT JOIN AKT_DON_DUONGSU BDS ON BDS.DONID = q.DONID AND BDS.TUCACHTOTUNG_MA = 'BIDON' AND BDS.ISDAIDIEN = 1                        
                                AND (
                                  (BDS.LOAIDUONGSU = 1 
                                AND (BDS.QUOCTICHID <> 2 OR BDS.XACTHUC_DLDCQG = 1))
                               ...
                               OR (BDS.LOAIDUONGSU <> 1)
                                ...
                                LEFT JOIN DM_DATAITEM QTN ON QTN.ID = NDS.QUOCTICHID AND QTN.GROUPID = 2
                                LEFT JOIN DM_DATAITEM QTB ON QTB.ID = BDS.QUOCTICHID AND QTB.GROUPID = 2
                                LEFT JOIN AKT_DON D ON D.ID = Q.DONID
                                LEFT JOIN AKT_PHUCTHAM_THULY TLPT ON TLPT.DONID = D.ID
                                LEFT JOIN (SELECT KH.ID, KH.IDBAQD, KH.TAIKHOANTAO, KH.NGAYTAO, KH.DONID
                                FROM KHOBAQD KH
                                WHERE KH.CAPXX = CapXX_SO_THAM_NUMBER -- 2: sơ thẩm
                                AND KH.LINHVUC = LOAIAN AND KH.STATUS = 1) K ON Q.DONID = K.DONID
                                 LEFT JOIN AKT_SOTHAM_THULY TL ON TL.DONID = Q.DONID
  ...
  --               TH4: lấy thông tin Quyết định Phúc thẩm
                        SELECT 
                            ...
                            WHERE 
                                ...
                                AND (v_CheckNullKHOBAQD = 0 or K.id is null)
    --                              Đã đồng bộ  
                                AND Q.id is null  and lqdkt.ket_thuc = 1
                                AND q.toaanid = v_toaan_id       
                                AND (NDS.DONID IS NOT NULL OR BDS.DONID IS NOT NULL)
                                AND (v_DONID is null or Q.DONID = v_DONID)
                        )A
                     where (v_KHANGCAOQH is null OR  A.KHANGCAOQH = v_KHANGCAOQH)
                        ...
                        AND (v_thuky_id IS NULL 
                                    OR EXISTS(select 'X' from AKT_SoTham_HDXX tp where tp.CanBoID = v_thuky_id and tp.DONID=A.DONID) 
                                   OR EXISTS(select 'X' from AKT_PhucTham_HDXX tp where tp.CanBoID = v_thuky_id and tp.DONID=A.DONID)
                                    OR EXISTS(SELECT 'X' FROM AKT_DON_THAMPHAN TP WHERE  TP.THUKYID=v_thuky_id and TP.DONID=A.DONID)
                                )
                        
                        AND ( v_thamphan_id is null 
                                    OR (A.CAPXX =CapXX_SO_THAM AND  EXISTS(SELECT 'x' FROM AKT_SOTHAM_HDXX TP 
                                            WHERE TP.DONID = A.DONID AND TP.MAVAITRO='THAMPHAN' AND TP.CANBOID = v_thamphan_id ))
                                     OR (A.CAPXX =CapXX_PHUC_THAM AND  EXISTS(SELECT 'x' FROM AKT_PHUCTHAM_HDXX TP 
                                            WHERE TP.DONID = A.DONID AND TP.MAVAITRO='THAMPHAN' AND TP.CANBOID = v_thamphan_id ))        
                                            )
...
(  
    ...
    V_NGAYGUI_TU in varchar2,
    V_NGAYGUI_DEN in varchar2,
    V_TRANGTHAIDONGBO in varchar2, -- 2 là đã đồng bộ, 3 là thu hồi
            ...
            SELECT  ROW_NUMBER() OVER (ORDER BY A.NGAYGUI desc) STT, COUNT(*) OVER () as CountAll
                ...
                --TH1: Bản án không có kháng cao kháng nghi
                      SELECT b.id as KHOBAQDID,
                        ...
                        FROM KHOBAQD B
                        LEFT JOIN AKT_DON D ON B.DONID = D.ID                        
                        LEFT JOIN AKT_DON_DUONGSU NDS ON NDS.DONID = D.ID AND NDS.TUCACHTOTUNG_MA = 'NGUYENDON' AND NDS.ISDAIDIEN = 1     
                        AND (
                                  (NDS.LOAIDUONGSU = 1 
                                AND (NDS.QUOCTICHID <> 2 OR NDS.XACTHUC_DLDCQG = 1))
                        ...
                        LEFT JOIN AKT_DON_DUONGSU BDS ON BDS.DONID = D.ID AND BDS.TUCACHTOTUNG_MA = 'BIDON' AND BDS.ISDAIDIEN = 1    
                        AND (
                                  (BDS.LOAIDUONGSU = 1 
                                AND (BDS.QUOCTICHID <> 2 OR BDS.XACTHUC_DLDCQG = 1))
