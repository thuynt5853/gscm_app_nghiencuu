...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "GSCM"."PKG_STPT_DANHSACH" AS
     ...
     LOOP 
        ...
        ELSE
            ...
            SELECT COUNT('X')
                  INTO VV_CHECKCOUNT
            FROM AHS_VUAN A
                    --------------------------Quyết định gây kết thúc Sơ thẩm--------------------------            
                    INNER JOIN (SELECT VA.SOQUYETDINH, VA.NGAYQD, VA.VUANID, dmqd.TEN
            ...
            IF(VV_CHECKCOUNT > 0) THEN 
                SELECT QDVAST.SOQUYETDINH, QDVAST.NGAYQD
                    INTO VSOQDST, VNGAYQDST
                FROM AHS_VUAN A
                    --------------------------Quyết định gây kết thúc Sơ thẩm--------------------------            
                    INNER JOIN (SELECT VA.SOQUYETDINH, VA.NGAYQD, VA.VUANID, dmqd.TEN
                                    FROM AHS_SOTHAM_QUYETDINH_VUAN VA
        ...
        IF(item_hs.V_SOAN IS NOT NULL OR item_hs.V_NGAYXU IS NOT NULL) THEN
            item_hs.V_SOAN := item_hs.V_SOAN;
            item_hs.V_NGAYXU := item_hs.V_NGAYXU;
        ELSE
            VV_CHECKCOUNT := 0;
            VSOQUYETDINH := NULL;  VNGAYQD := NULL; VTENQD := NULL;

            SELECT COUNT('X')
                  INTO VV_CHECKCOUNT
            FROM AHS_VUAN A
                --------------------------Quyết định gây kết thúc Phúc thẩm--------------------------    
                INNER JOIN (SELECT VA.SOQUYETDINH, VA.NGAYQD, VA.VUANID, dmqd.TEN
            ...
            IF(VV_CHECKCOUNT > 0) THEN 
                SELECT QDVA.SOQUYETDINH, QDVA.NGAYQD, QDVA.TEN
                  INTO VSOQUYETDINH, VNGAYQD, VTENQD
                FROM AHS_VUAN A
                    --------------------------Quyết định gây kết thúc Phúc thẩm--------------------------    
                    INNER JOIN (SELECT VA.SOQUYETDINH, VA.NGAYQD, VA.VUANID, dmqd.TEN
                                    FROM AHS_PHUCTHAM_QUYETDINH_VUAN VA
