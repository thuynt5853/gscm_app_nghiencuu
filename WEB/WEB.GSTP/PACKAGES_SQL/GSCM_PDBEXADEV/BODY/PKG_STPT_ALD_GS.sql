--------------------------------------------------------
--  DDL for Package Body PKG_STPT_ALD_GS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "GSCM"."PKG_STPT_ALD_GS" AS
   ...
   ------------------------
           ...
           --THAMPHAN --TOANCAU-12102023-ANHNT
		...
		FROM (
            ...
            FROM  ALD_KCKNQDK_PHUCTHAM_HDXX WHERE MAVAITRO IN('THAMPHAN','THAMPHANHDXX','THAMPHANDUKHUYET')) TP
   ...
   ------------------dinh nghĩa bảng lay 1 ban ghi moi nhat với mục đích chỉ chạy 1 lần.
         --ALD_KCKNQDK_PHUCTHAM_QUYETDINH
        SELECT
                ...
                FROM
                    (
                        SELECT
                            ...
                            QDL.MA
                        FROM
                            ALD_KCKNQDK_PHUCTHAM_QUYETDINH PQD
...
--      GN.TRUONGHOPGIAONHAN,
                                  ...
                                  FROM
                                      ...
                                      LEFT JOIN ALD_CHUYEN_NHAN_AN NA ON NA.MAP_VUANID_NEW = A.ID
      ...
      ----- BA Or QD----------------------------------------------
                                    ...
                                    LEFT JOIN (
                                          SELECT
                                              ...
                                              || '</b>' QD_PT
                                          FROM
                                              ALD_KCKNQDK_PHUCTHAM_QUYETDINH PTQD
                                          WHERE
                                              QUYETDINHID = 213
         ...
         --- Lay ra trang thai giai quyet don
                                      ...
                                      LEFT JOIN (
                                          ...
                                          FROM
                                              ALD_KCKNQDK_PHUCTHAM_QUYETDINH PTQDVA --QUYẾT ĐỊNH 
                                          WHERE
                                              EXISTS (
                                                  SELECT
                                                      'X'
                                      ...
                                      LEFT JOIN (
                                          ...
                                          FROM
                                                   DM_DATAITEM I
                                              INNER JOIN ALD_CHUYEN_NHAN_AN CA ON CA.TRUONGHOPGIAONHANID = I.ID
                                          WHERE
                                      ...
                                      LEFT JOIN (
                                          SELECT
                                              ...
                                              LISTAGG(BC.TENDUONGSU
                                                      || ' '
                                                      || DECODE(BC.TUCACHTOTUNG_MA, 'NGUYENDON', '(Người khởi kiện)', 'BIDON', '(Người bị kiện)',
...
--                                          FROM
--                                              ALD_SOTHAM_KHANGCAO KC
--                                            INNER JOIN  ALD_DON_DUONGSU ds ON KC.DUONGSUID = DS.ID
                                  ...
                                  WHERE
                                      ...
                                      AND ( V_UTTP IS NULL
                                          ...
                                          SELECT
                                              'X'
                                          FROM
                                              ALD_KCKNQDK_PHUCTHAM_THULY TLPT
                                          WHERE
         ...
         --tuyennh comment code                          
           ...
           --------------
                                            AND ( V_SO_QD IS NULL--Số BA/QĐ
                                                  OR ( EXISTS (
                                          SELECT 'X' FROM ALD_SOTHAM_BANAN QSV
                                          WHERE
                                          ...
                                          WHERE
                                              ...
                                              AND NA.VUANID = QSV.DONID
                                                       ...
                                                       OR EXISTS (
                                          SELECT 'X' FROM ALD_KCKNQDK_PHUCTHAM_QUYETDINH QSV
                                          WHERE
                                              UPPER(QSV.SOQD) LIKE '%' || V_SO_QD || '%'
                                              AND A.ID = QSV.DONID
                                      ) ) )
                                            AND ( V_NGAY_QD IS NULL--Ngày BA/QĐ
                                                  OR ( EXISTS (
                                          SELECT 'X' FROM ALD_SOTHAM_BANAN QSV
                                          WHERE TO_CHAR(QSV.NGAYMOPHIENTOA, 'dd/MM/yyyy') = V_NGAY_QD
                                              AND NA.VUANID = QSV.DONID
                                      )
                                                       OR EXISTS (
                                          SELECT 'X' FROM ALD_SOTHAM_QUYETDINH QSV
                                          WHERE TO_CHAR(QSV.NGAYQD, 'dd/MM/yyyy') = V_NGAY_QD
                                              AND NA.VUANID = QSV.DONID
                                      )
                                                       OR EXISTS (
                                          SELECT 'X' FROM ALD_KCKNQDK_PHUCTHAM_QUYETDINH QSV
             ...
             AND (V_KETQUA IS NULL
                OR( V_KETQUA = 1
                    AND( EXISTS (
                        SELECT 'X' 
                        FROM ALD_KCKNQDK_PHUCTHAM_QUYETDINH   QSV 
...
--                                              'X'
--                                          FROM
--                                              ALD_KCKNQDK_PHUCTHAM_THULY PTTL
--                                          WHERE
--                                              ( NOT EXISTS (
--                                                  SELECT
--                                                      'X'
...
--                                              AND PTTL.DONID = A.ID
--                                      ) ) 
                                --12/07 -toancau - tuyennh - sua tinh trang chua giai quyet xong
                                --da thu ly
                   ...
                   OR ( V_TINHTRANG_GIAIQUYET = 7 --Đã giải quyết xong
                                    ...
                                    AND ( EXISTS (
                                    SELECT 'X' FROM ALD_KCKNQDK_PHUCTHAM_QUYETDINH PTQDVA 
                                    LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID=PTQDVA.QUYETDINHID
                                    LEFT JOIN DM_QD_LOAI QDL ON QDL.ID=QD.LOAIID 
                                    WHERE instr(',CNTT,',','||QDL.MA||',')>0
                                    AND (V_TUNGAY IS NULL OR PTQDVA.NGAYQD>=VV_TUNGAY)
                                    AND (V_DENNGAY IS NULL OR PTQDVA.NGAYQD<=VV_DENNGAY)
                                    AND PTQDVA.DONID=a.id  AND A.MAGIAIDOAN=7
                                     )))  
                                     -- 10/7 toancau quyet)
...
--                                      ) ) ) ) --là con của chưa giải quyết xong end 
    ...
    PROCEDURE ALD_KCKNQDK_PHUCTHAM_THULY_GETLIST (
        VDONID    IN NUMBER,
        CURRETURN OUT SYS_REFCURSOR
    ) IS
    ...
    END ALD_KCKNQDK_PHUCTHAM_THULY_GETLIST;

    PROCEDURE LD_CHUYENDON (
        VTOAANID       NUMBER,
    ...
    BEGIN
        IF VTRANGTHAI = 0 THEN
            ...
            FROM
                     ALD_DON D
                JOIN ALD_DON_XULY DXL ON DXL.DONID = D.ID
            WHERE
                ( 1 = (
                    CASE
                        WHEN ( VMAVUVIEC
...
----------------------
        ...
        PKG_STPT_DS_BC.FILL_LD_CHUYENAN(V_ARRAY);
        OPEN CURRETURN FOR SELECT
                               PA.V_STT,
                               PA.V_VUANID,
                               PA.V_TOAANID,
    ...
    BEGIN
        ...
        FROM
            ...
            JOIN ALD_DON_XULY DXL ON DXL.DONID = A.VUANID
        WHERE
                A.TOANHANID = VTOAANID
            AND 1 = (case when VTRANGTHAI = 0 and (A.TRANGTHAI = 0 or a.trangthai is null) then 1
            when VTRANGTHAI = 1 and a.trangthai = 1 then 1 else 0 end)
            AND 1 = (
                CASE
        ...
        PKG_STPT_DS_BC.FILL_LD_NHANAN(V_ARRAY);
        OPEN CURRETURN FOR SELECT
                               *
                           FROM
                               TABLE ( V_ARRAY ) A
    ...
    PROCEDURE ALD_PHUCTHAMQDK_THULY_GETMAXTT (
        VDONVIID  IN NUMBER,
        VFROMDATE IN DATE,
        VTODATE   IN DATE,
        CURRETURN OUT SYS_REFCURSOR
    ...
    BEGIN
        OPEN CURRETURN FOR SELECT
                           ...
                           FROM
                                    ALD_KCKNQDK_PHUCTHAM_THULY T
                               INNER JOIN ALD_DON D ON D.ID = T.DONID
                           WHERE
                                   D.TOAANID = VDONVIID
                               AND T.NGAYTHULY BETWEEN VFROMDATE AND VTODATE;

    END ALD_PHUCTHAMQDK_THULY_GETMAXTT;



    --Lấy ra list HDXX tamnc 29-3
    PROCEDURE ALD_KCKNQDK_PHUCTHAM_HDXX_GETLIST (
        VDONID    IN NUMBER,
        CURRETURN OUT SYS_REFCURSOR
    ) IS
        CHECKBANANPT NUMBER;
    ...
    END ALD_KCKNQDK_PHUCTHAM_HDXX_GETLIST;

--Lấy ra list TGTT tamnc 29-3
    PROCEDURE ALD_PHUCTHAM_KCKN_TGTT_GETLIST (
        VDONID    IN NUMBER,
        CURRETURN OUT SYS_REFCURSOR
    ) IS
    BEGIN
        ...
        --,h1.MA_TEN as Tamtru
                               ...
                               D.TOA_GIAIQUYET_ID
                           FROM
                               ALD_KCKNQDK_PHUCTHAM_THAMGIATOTUNG D
    ...
    END ALD_PHUCTHAM_KCKN_TGTT_GETLIST;

    --Lấy ra list Quyết định tamnc 29-3   
    PROCEDURE ALD_KCKNQDK_PHUCTHAM_QUYETDINH_GETLIST (
        VDONID    IN NUMBER,
        CURRETURN OUT SYS_REFCURSOR
    ) IS
    BEGIN
        OPEN CURRETURN FOR SELECT
                               ...
                               Q.TOA_GIAIQUYET_ID
                           FROM
                               ALD_KCKNQDK_PHUCTHAM_QUYETDINH Q
    ...
    END ALD_KCKNQDK_PHUCTHAM_QUYETDINH_GETLIST;


--Lấy ra list bản án tamnc 29-3
    PROCEDURE ALD_PHUCTHAMQDK_BANANQUYETDINH_GETLIST (
        VDONID    IN NUMBER,
        CURRETURN OUT SYS_REFCURSOR
    ) IS
        COUNTBANANST INT;
    ...
    END ALD_PHUCTHAMQDK_BANANQUYETDINH_GETLIST;

 ------------------------


    PROCEDURE SO_DK_KCKN_LD (
        V_DK_LD  OUT NUMBER,
    ...
    PROCEDURE ALD_DM_QUYETDINH_VUAN_PTQDK (
        CURRETURN OUT SYS_REFCURSOR
    ) AS
    BEGIN
    OPEN CURRETURN FOR SELECT
                           ...
                           WHERE
                                   ISPHUCTHAM = 1
                               AND ISLAODONG = 1
                               AND ( MA IN ('69-DS','70-DS','72-DS') )
                           ORDER BY
                               CASE WHEN MA='72-DS' THEN '1' ELSE MA END;
    END ALD_DM_QUYETDINH_VUAN_PTQDK;
    ...
    BEGIN
        FOR ITEMS IN (
            ...
            FROM
                ...
                LEFT JOIN ALD_SOTHAM_QUYETDINH QD ON QD.ID = T2.SOQDBA
                                                     AND T2.LOAIKHANGCAO IN ( 1, 2 ) --> Kháng cáo quyết định --toancau-anhnt kc tđc
                LEFT JOIN ALD_SOTHAM_BANAN     BA ON BA.ID = T2.SOQDBA
            ...
            FROM
                     ALD_SOTHAM_KHANGCAO T2
                INNER JOIN ALD_DON_THAMGIATOTUNG T3 ON T2.DUONGSUID = T3.ID
                                                       AND ( T3.DONID = T2.DONID )
  ...
  -- Tổng hợp kháng nghị
        FOR ITEMS IN (
            ...
            FROM
                ...
                LEFT JOIN ALD_SOTHAM_QUYETDINH QD ON QD.ID = T2.BANANID
                                                     AND T2.LOAIKN IN ( 1, 2 ) --> Kháng cáo quyết định
                LEFT JOIN ALD_SOTHAM_BANAN     BA ON BA.ID = T2.BANANID
                                                 AND T2.LOAIKN = 0 --> Kháng cáo bản án
                LEFT JOIN ALD_SOTHAM_RUTKCKN   RUT ON T2.ID = RUT.IDKCKN
            WHERE
                    T2.DONID = (
                        CASE
                            WHEN NOT EXISTS (
    ...
    BEGIN
        BEGIN
            ...
            FOR R_DS IN ( 
                ...
                AND EXISTS (SELECT 'X' FROM ALD_SOTHAM_KHANGCAO WHERE DUONGSUID =DS.ID)
				)
			LOOP
				L_NGAYKHANGCAO :='';
				FOR R_KC IN (SELECT KC.* FROM ALD_SOTHAM_KHANGCAO   KC WHERE KC.DUONGSUID = R_DS.ID AND KC.DONID = V_DONID)
				LOOP
