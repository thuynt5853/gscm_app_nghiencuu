--------------------------------------------------------
--  DDL for Package Body PKG_DASHBOARD_PT_APP
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "GSCM"."PKG_DASHBOARD_PT_APP" AS
...
BEGIN
    DELETE DASHBOARD_STPT WHERE LOAIAN=2 AND CAPXX=3;COMMIT;  
    ...
    ------
      FOR item in (
                    ...
                    LEFT JOIN(SELECT BA.DONID,BA.SOBANAN,BA.NGAYTUYENAN
                                            FROM ADS_PHUCTHAM_BANAN BA)STBA ON STBA.DONID=TL.DONID 
                    LEFT JOIN (
                             SELECT TT.DONID
                          ...
                          FROM(SELECT TS.DONID
                              ,LISTAGG(TS.TENDUONGSU,', ')WITHIN GROUP (ORDER BY TS.ISDAIDIEN DESC,TS.TENDUONGSU)TENDUONGSU
                                 FROM (  SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER,BC.ISDAIDIEN
                                            ...
                                            FROM ADS_DON_DUONGSU WHERE TUCACHTOTUNG_MA='NGUYENDON' AND ISPHUCTHAM=1--ISDAIDIEN=1                              
                                            ) BC 
                          ...
                          ,LISTAGG(TS.TENDUONGSU,', ')WITHIN GROUP (ORDER BY TS.ISDAIDIEN DESC,TS.TENDUONGSU)TENDUONGSU
                             FROM (
                                       SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER,BC.ISDAIDIEN
                                        FROM (SELECT ID,DONID,TENDUONGSU,TUCACHTOTUNG_MA,ISDAIDIEN,ROW_NUMBER()  OVER (PARTITION BY DONID ORDER BY ISDAIDIEN DESC,TENDUONGSU) ROWNUMBER
                                        FROM ADS_DON_DUONGSU WHERE TUCACHTOTUNG_MA='BIDON'AND ISPHUCTHAM=1--ISDAIDIEN=1                              
                                        ) BC 
                                       -- WHERE BC.ROWNUMBER <= 3
                                ) TS GROUP BY TS.DONID    
                         )TT
                   ...
                   LEFT JOIN(SELECT TT.DONID
                    ...
                    FROM ( 
                        ...
                        ,LISTAGG(dqd.TEN,',')WITHIN GROUP (ORDER BY QD.NGAYQD DESC)TEN
                                    FROM ADS_PHUCTHAM_QUYETDINH QD
                    ...
                    WHERE 
                     d.MAVUVIEC IS NOT NULL  AND TL.SOTHULY IS NOT NULL  AND TL.NGAYTHULY IS NOT NULL
      )
      LOOP
          insert into DASHBOARD_STPT
             ...
             COMMIT;          
      END LOOP;
END DASHBOARD_CREATE_DATA_ADS_PT;
PROCEDURE DASHBOARD_CREATE_DATA_AHN_PT
 AS    
    V_SYSDATE DATE;V_COUNTS NUMBER;
    V_KQLOAI NUMBER; V_SOTHULYXX VARCHAR2(512);V_NGAYTHULYXX DATE;
BEGIN
    DELETE DASHBOARD_STPT WHERE LOAIAN=3 AND CAPXX=3;COMMIT;  
    SELECT SYSDATE INTO V_SYSDATE FROM DUAL; 
    ------
      FOR item in (
                    SELECT TL.TOAANID
                    ...
                    LEFT JOIN(SELECT BA.DONID,BA.SOBANAN,BA.NGAYTUYENAN
                                            FROM AHN_PHUCTHAM_BANAN BA)STBA ON STBA.DONID=TL.DONID 
                    LEFT JOIN (
                             SELECT TT.DONID
                            ,DECODE(instr(TT.THAMPHANID,','),0,TT.THAMPHANID,SUBSTR(TT.THAMPHANID,0,instr(TT.THAMPHANID,',') - 1))THAMPHANID    
                             FROM ( SELECT hd.DONID                            
                                   ,LISTAGG(hd.CANBOID,',')WITHIN GROUP (ORDER BY hd.NGAYPHANCONG DESC)THAMPHANID      
                                    FROM AHN_PHUCTHAM_HDXX hd 
                                    WHERE hd.mavaitro='THAMPHAN' and hd.DONID!=0
                                    group by hd.DONID
                               )TT
                    )TP ON TP.DONID=TL.DONID                        
                         ...
                         SELECT TT.DONID,TT.TENDUONGSU  
                          FROM(SELECT TS.DONID
                              ,LISTAGG(TS.TENDUONGSU,', ')WITHIN GROUP (ORDER BY TS.ISDAIDIEN DESC,TS.TENDUONGSU)TENDUONGSU
                                 FROM (  SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER,BC.ISDAIDIEN
                                            FROM (SELECT ID,DONID,TENDUONGSU,TUCACHTOTUNG_MA,ISDAIDIEN,ROW_NUMBER()  OVER (PARTITION BY DONID ORDER BY ISDAIDIEN DESC,TENDUONGSU) ROWNUMBER
                                            FROM AHN_DON_DUONGSU WHERE TUCACHTOTUNG_MA='NGUYENDON' AND ISPHUCTHAM=1--ISDAIDIEN=1                              
                                            ) BC 
                                          --  WHERE BC.ROWNUMBER <= 3
                                    ) TS GROUP BY TS.DONID    
                           )TT
                        ...
                        FROM(SELECT COUNT(*), TS.DONID
                          ,LISTAGG(TS.TENDUONGSU,', ')WITHIN GROUP (ORDER BY TS.ISDAIDIEN DESC,TS.TENDUONGSU)TENDUONGSU
                             FROM (
                                       SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER,BC.ISDAIDIEN
                                        FROM (SELECT ID,DONID,TENDUONGSU,TUCACHTOTUNG_MA,ISDAIDIEN,ROW_NUMBER()  OVER (PARTITION BY DONID ORDER BY ISDAIDIEN DESC,TENDUONGSU) ROWNUMBER
                                        FROM AHN_DON_DUONGSU WHERE TUCACHTOTUNG_MA='BIDON'AND ISPHUCTHAM=1--ISDAIDIEN=1                              
                                        ) BC 
                                       -- WHERE BC.ROWNUMBER <= 3
                                ) TS GROUP BY TS.DONID    
                         )TT
                   ...
                   LEFT JOIN(SELECT TT.DONID
                    ...
                    FROM ( 
                        ...
                        ,LISTAGG(QD.NGAYQD,',')WITHIN GROUP (ORDER BY QD.NGAYQD DESC)NGAYQD
                        ,LISTAGG(dqd.TEN,',')WITHIN GROUP (ORDER BY QD.NGAYQD DESC)TEN
                                    FROM AHN_PHUCTHAM_QUYETDINH QD
                                    LEFT JOIN DM_QD_QUYETDINH DQD ON DQD.ID=QD.QUYETDINHID 
                                    LEFT JOIN DM_QD_LOAI QDL ON QDL.ID=DQD.LOAIID
                                    WHERE dqd.ket_thuc = 1
                          group by QD.DONID
      ...
      LOOP
          insert into DASHBOARD_STPT
             ...
             COMMIT;          
      END LOOP;
END DASHBOARD_CREATE_DATA_AHN_PT;
PROCEDURE DASHBOARD_CREATE_DATA_AKT_PT
 AS    
    V_SYSDATE DATE;V_COUNTS NUMBER;
    V_KQLOAI NUMBER; V_SOTHULYXX VARCHAR2(512);V_NGAYTHULYXX DATE;
BEGIN
    DELETE DASHBOARD_STPT WHERE LOAIAN=4 AND CAPXX=3;COMMIT;  
    SELECT SYSDATE INTO V_SYSDATE FROM DUAL; 
    ------
      FOR item in (
                    SELECT TL.TOAANID
                    ...
                    LEFT JOIN(SELECT BA.DONID,BA.SOBANAN,BA.NGAYTUYENAN
                                            FROM AKT_PHUCTHAM_BANAN BA)STBA ON STBA.DONID=TL.DONID 
                    LEFT JOIN (
                             SELECT TT.DONID
                            ,DECODE(instr(TT.THAMPHANID,','),0,TT.THAMPHANID,SUBSTR(TT.THAMPHANID,0,instr(TT.THAMPHANID,',') - 1))THAMPHANID    
                             FROM ( SELECT hd.DONID                            
                                   ,LISTAGG(hd.CANBOID,',')WITHIN GROUP (ORDER BY hd.NGAYPHANCONG DESC)THAMPHANID      
                                    FROM AKT_PHUCTHAM_HDXX hd 
                                    WHERE hd.mavaitro='THAMPHAN' and hd.DONID!=0
                                    group by hd.DONID
                               )TT
                    )TP ON TP.DONID=TL.DONID                        
                         ...
                         SELECT TT.DONID,TT.TENDUONGSU  
                          FROM(SELECT TS.DONID
                              ,LISTAGG(TS.TENDUONGSU,', ')WITHIN GROUP (ORDER BY TS.ISDAIDIEN DESC,TS.TENDUONGSU)TENDUONGSU
                                 FROM (  SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER,BC.ISDAIDIEN
                                            FROM (SELECT ID,DONID,TENDUONGSU,TUCACHTOTUNG_MA,ISDAIDIEN,ROW_NUMBER()  OVER (PARTITION BY DONID ORDER BY ISDAIDIEN DESC,TENDUONGSU) ROWNUMBER
                                            FROM AKT_DON_DUONGSU WHERE TUCACHTOTUNG_MA='NGUYENDON' AND ISPHUCTHAM=1--ISDAIDIEN=1                              
                                            ) BC 
                                          --  WHERE BC.ROWNUMBER <= 3
                                    ) TS GROUP BY TS.DONID    
                           )TT
                        ...
                        FROM(SELECT COUNT(*), TS.DONID
                          ,LISTAGG(TS.TENDUONGSU,', ')WITHIN GROUP (ORDER BY TS.ISDAIDIEN DESC,TS.TENDUONGSU)TENDUONGSU
                             FROM (
                                       SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER,BC.ISDAIDIEN
                                        FROM (SELECT ID,DONID,TENDUONGSU,TUCACHTOTUNG_MA,ISDAIDIEN,ROW_NUMBER()  OVER (PARTITION BY DONID ORDER BY ISDAIDIEN DESC,TENDUONGSU) ROWNUMBER
                                        FROM AKT_DON_DUONGSU WHERE TUCACHTOTUNG_MA='BIDON'AND ISPHUCTHAM=1--ISDAIDIEN=1                              
                                        ) BC 
                                       -- WHERE BC.ROWNUMBER <= 3
                                ) TS GROUP BY TS.DONID    
                         )TT
                   ...
                   LEFT JOIN(SELECT TT.DONID
                    ...
                    FROM ( 
                        ...
                        ,LISTAGG(QD.NGAYQD,',')WITHIN GROUP (ORDER BY QD.NGAYQD DESC)NGAYQD
                        ,LISTAGG(dqd.TEN,',')WITHIN GROUP (ORDER BY QD.NGAYQD DESC)TEN
                                    FROM AKT_PHUCTHAM_QUYETDINH QD
                                    LEFT JOIN DM_QD_QUYETDINH DQD ON DQD.ID=QD.QUYETDINHID 
                                    LEFT JOIN DM_QD_LOAI QDL ON QDL.ID=DQD.LOAIID
                                    WHERE dqd.ket_thuc = 1
                          group by QD.DONID
      ...
      LOOP
          insert into DASHBOARD_STPT
             ...
             COMMIT;          
      END LOOP;
END DASHBOARD_CREATE_DATA_AKT_PT;
PROCEDURE DASHBOARD_CREATE_DATA_ALD_PT
 AS    
    V_SYSDATE DATE;V_COUNTS NUMBER;
    V_KQLOAI NUMBER; V_SOTHULYXX VARCHAR2(512);V_NGAYTHULYXX DATE;
BEGIN
    DELETE DASHBOARD_STPT WHERE LOAIAN=5 AND CAPXX=3;COMMIT;  
    SELECT SYSDATE INTO V_SYSDATE FROM DUAL; 
    ------
      FOR item in (
                    SELECT TL.TOAANID
                    ...
                    LEFT JOIN(SELECT BA.DONID,BA.SOBANAN,BA.NGAYTUYENAN
                                            FROM ALD_PHUCTHAM_BANAN BA)STBA ON STBA.DONID=TL.DONID 
                    LEFT JOIN (
                             SELECT TT.DONID
                            ,DECODE(instr(TT.THAMPHANID,','),0,TT.THAMPHANID,SUBSTR(TT.THAMPHANID,0,instr(TT.THAMPHANID,',') - 1))THAMPHANID    
                             FROM ( SELECT hd.DONID                            
                                   ,LISTAGG(hd.CANBOID,',')WITHIN GROUP (ORDER BY hd.NGAYPHANCONG DESC)THAMPHANID      
                                    FROM ALD_PHUCTHAM_HDXX hd 
                                    WHERE hd.mavaitro='THAMPHAN' and hd.DONID!=0
                                    group by hd.DONID
                               )TT
                    )TP ON TP.DONID=TL.DONID                        
                         ...
                         SELECT TT.DONID,TT.TENDUONGSU  
                          FROM(SELECT TS.DONID
                              ,LISTAGG(TS.TENDUONGSU,', ')WITHIN GROUP (ORDER BY TS.ISDAIDIEN DESC,TS.TENDUONGSU)TENDUONGSU
                                 FROM (  SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER,BC.ISDAIDIEN
                                            FROM (SELECT ID,DONID,TENDUONGSU,TUCACHTOTUNG_MA,ISDAIDIEN,ROW_NUMBER()  OVER (PARTITION BY DONID ORDER BY ISDAIDIEN DESC,TENDUONGSU) ROWNUMBER
                                            FROM ALD_DON_DUONGSU WHERE TUCACHTOTUNG_MA='NGUYENDON' AND ISPHUCTHAM=1--ISDAIDIEN=1                              
                                            ) BC 
                                          --  WHERE BC.ROWNUMBER <= 3
                                    ) TS GROUP BY TS.DONID    
                           )TT
                        ...
                        FROM(SELECT COUNT(*), TS.DONID
                          ,LISTAGG(TS.TENDUONGSU,', ')WITHIN GROUP (ORDER BY TS.ISDAIDIEN DESC,TS.TENDUONGSU)TENDUONGSU
                             FROM (
                                       SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER,BC.ISDAIDIEN
                                        FROM (SELECT ID,DONID,TENDUONGSU,TUCACHTOTUNG_MA,ISDAIDIEN,ROW_NUMBER()  OVER (PARTITION BY DONID ORDER BY ISDAIDIEN DESC,TENDUONGSU) ROWNUMBER
                                        FROM ALD_DON_DUONGSU WHERE TUCACHTOTUNG_MA='BIDON'AND ISPHUCTHAM=1--ISDAIDIEN=1                              
                                        ) BC 
                                       -- WHERE BC.ROWNUMBER <= 3
                                ) TS GROUP BY TS.DONID    
                         )TT
                   ...
                   LEFT JOIN(SELECT TT.DONID
                    ...
                    FROM ( 
                        ...
                        ,LISTAGG(QD.NGAYQD,',')WITHIN GROUP (ORDER BY QD.NGAYQD DESC)NGAYQD
                        ,LISTAGG(dqd.TEN,',')WITHIN GROUP (ORDER BY QD.NGAYQD DESC)TEN
                                    FROM ALD_PHUCTHAM_QUYETDINH QD
                                    LEFT JOIN DM_QD_QUYETDINH DQD ON DQD.ID=QD.QUYETDINHID 
                                    LEFT JOIN DM_QD_LOAI QDL ON QDL.ID=DQD.LOAIID
                                    WHERE dqd.ket_thuc = 1
                          group by QD.DONID
      ...
      LOOP
          insert into DASHBOARD_STPT
             ...
             COMMIT;          
      END LOOP;
END DASHBOARD_CREATE_DATA_ALD_PT;
PROCEDURE DASHBOARD_CREATE_DATA_AHC_PT
 AS    
    V_SYSDATE DATE;V_COUNTS NUMBER;
    V_KQLOAI NUMBER; V_SOTHULYXX VARCHAR2(512);V_NGAYTHULYXX DATE;
BEGIN
    DELETE DASHBOARD_STPT WHERE LOAIAN=6 AND CAPXX=3;COMMIT;  
    SELECT SYSDATE INTO V_SYSDATE FROM DUAL; 
    ------
      FOR item in (
                    SELECT TL.TOAANID
                    ...
                    LEFT JOIN(SELECT BA.DONID,BA.SOBANAN,BA.NGAYTUYENAN
                                            FROM AHC_PHUCTHAM_BANAN BA)STBA ON STBA.DONID=TL.DONID 
                    LEFT JOIN (
                             SELECT TT.DONID
                            ,DECODE(instr(TT.THAMPHANID,','),0,TT.THAMPHANID,SUBSTR(TT.THAMPHANID,0,instr(TT.THAMPHANID,',') - 1))THAMPHANID    
                             FROM ( SELECT hd.DONID                            
                                   ,LISTAGG(hd.CANBOID,',')WITHIN GROUP (ORDER BY hd.NGAYPHANCONG DESC)THAMPHANID      
                                    FROM AHC_PHUCTHAM_HDXX hd 
                                    WHERE hd.mavaitro='THAMPHAN' and hd.DONID!=0
                                    group by hd.DONID
                               )TT
                    )TP ON TP.DONID=TL.DONID                        
                         ...
                         SELECT TT.DONID,TT.TENDUONGSU  
                          FROM(SELECT TS.DONID
                              ,LISTAGG(TS.TENDUONGSU,', ')WITHIN GROUP (ORDER BY TS.ISDAIDIEN DESC,TS.TENDUONGSU)TENDUONGSU
                                 FROM (  SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER,BC.ISDAIDIEN
                                            FROM (SELECT ID,DONID,TENDUONGSU,TUCACHTOTUNG_MA,ISDAIDIEN,ROW_NUMBER()  OVER (PARTITION BY DONID ORDER BY ISDAIDIEN DESC,TENDUONGSU) ROWNUMBER
                                            FROM AHC_DON_DUONGSU WHERE TUCACHTOTUNG_MA='NGUYENDON' AND ISPHUCTHAM=1--ISDAIDIEN=1                              
                                            ) BC 
                                          --  WHERE BC.ROWNUMBER <= 3
                                    ) TS GROUP BY TS.DONID    
                           )TT
                        ...
                        FROM(SELECT COUNT(*), TS.DONID
                          ,LISTAGG(TS.TENDUONGSU,', ')WITHIN GROUP (ORDER BY TS.ISDAIDIEN DESC,TS.TENDUONGSU)TENDUONGSU
                             FROM (
                                       SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER,BC.ISDAIDIEN
                                        FROM (SELECT ID,DONID,TENDUONGSU,TUCACHTOTUNG_MA,ISDAIDIEN,ROW_NUMBER()  OVER (PARTITION BY DONID ORDER BY ISDAIDIEN DESC,TENDUONGSU) ROWNUMBER
                                        FROM AHC_DON_DUONGSU WHERE TUCACHTOTUNG_MA='BIDON'AND ISPHUCTHAM=1--ISDAIDIEN=1                              
                                        ) BC 
                                       -- WHERE BC.ROWNUMBER <= 3
                                ) TS GROUP BY TS.DONID    
                         )TT
                   ...
                   LEFT JOIN(SELECT TT.DONID
                    ...
                    FROM ( 
                        ...
                        ,LISTAGG(QD.NGAYQD,',')WITHIN GROUP (ORDER BY QD.NGAYQD DESC)NGAYQD
                        ,LISTAGG(dqd.TEN,',')WITHIN GROUP (ORDER BY QD.NGAYQD DESC)TEN
                                    FROM AHC_PHUCTHAM_QUYETDINH QD
                                    LEFT JOIN DM_QD_QUYETDINH DQD ON DQD.ID=QD.QUYETDINHID 
                                    LEFT JOIN DM_QD_LOAI QDL ON QDL.ID=DQD.LOAIID
                                    WHERE dqd.ket_thuc = 1
                          group by QD.DONID
      ...
      LOOP
          insert into DASHBOARD_STPT
             ...
             COMMIT;          
      END LOOP;
END DASHBOARD_CREATE_DATA_AHC_PT;
PROCEDURE DASHBOARD_CREATE_DATA_APS_PT
 AS    
    V_SYSDATE DATE;V_COUNTS NUMBER;
    V_KQLOAI NUMBER; V_SOTHULYXX VARCHAR2(512);V_NGAYTHULYXX DATE;
BEGIN
    DELETE DASHBOARD_STPT WHERE LOAIAN=7 AND CAPXX=3;COMMIT;  
    SELECT SYSDATE INTO V_SYSDATE FROM DUAL; 
    ------
      FOR item in (
                    SELECT TL.TOAANID
                    ...
                    LEFT JOIN(SELECT BA.DONID,BA.SOBANAN,BA.NGAYTUYENAN
                                            FROM APS_PHUCTHAM_BANAN BA)STBA ON STBA.DONID=TL.DONID 
                    LEFT JOIN (
                             SELECT TT.DONID
                            ,DECODE(instr(TT.THAMPHANID,','),0,TT.THAMPHANID,SUBSTR(TT.THAMPHANID,0,instr(TT.THAMPHANID,',') - 1))THAMPHANID    
                             FROM ( SELECT hd.DONID                            
                                   ,LISTAGG(hd.CANBOID,',')WITHIN GROUP (ORDER BY hd.NGAYPHANCONG DESC)THAMPHANID      
                                    FROM APS_PHUCTHAM_HDXX hd 
                                    WHERE hd.mavaitro='THAMPHAN' and hd.DONID!=0
                                    group by hd.DONID
                               )TT
                    )TP ON TP.DONID=TL.DONID                        
                         ...
                         SELECT TT.DONID,TT.TENDUONGSU  
                          FROM(SELECT TS.DONID
                              ,LISTAGG(TS.TENDUONGSU,', ')WITHIN GROUP (ORDER BY TS.ISDAIDIEN DESC,TS.TENDUONGSU)TENDUONGSU
                                 FROM (  SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER,BC.ISDAIDIEN
                                            FROM (SELECT ID,DONID,TENDUONGSU,TUCACHTOTUNG_MA,ISDAIDIEN,ROW_NUMBER()  OVER (PARTITION BY DONID ORDER BY ISDAIDIEN DESC,TENDUONGSU) ROWNUMBER
                                            FROM APS_DON_DUONGSU WHERE TUCACHTOTUNG_MA='NGUYENDON' AND ISPHUCTHAM=1--ISDAIDIEN=1                              
                                            ) BC 
                                          --  WHERE BC.ROWNUMBER <= 3
                                    ) TS GROUP BY TS.DONID    
                           )TT
                        ...
                        FROM(SELECT COUNT(*), TS.DONID
                          ,LISTAGG(TS.TENDUONGSU,', ')WITHIN GROUP (ORDER BY TS.ISDAIDIEN DESC,TS.TENDUONGSU)TENDUONGSU
                             FROM (
                                       SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER,BC.ISDAIDIEN
                                        FROM (SELECT ID,DONID,TENDUONGSU,TUCACHTOTUNG_MA,ISDAIDIEN,ROW_NUMBER()  OVER (PARTITION BY DONID ORDER BY ISDAIDIEN DESC,TENDUONGSU) ROWNUMBER
                                        FROM APS_DON_DUONGSU WHERE TUCACHTOTUNG_MA='BIDON'AND ISPHUCTHAM=1--ISDAIDIEN=1                              
                                        ) BC 
                                       -- WHERE BC.ROWNUMBER <= 3
                                ) TS GROUP BY TS.DONID    
                         )TT
                   ...
                   LEFT JOIN(SELECT TT.DONID
                    ...
                    FROM ( 
                        ...
                        ,LISTAGG(QD.NGAYQD,',')WITHIN GROUP (ORDER BY QD.NGAYQD DESC)NGAYQD
                        ,LISTAGG(dqd.TEN,',')WITHIN GROUP (ORDER BY QD.NGAYQD DESC)TEN
                                    FROM APS_PHUCTHAM_QUYETDINH QD
                                    LEFT JOIN DM_QD_QUYETDINH DQD ON DQD.ID=QD.QUYETDINHID 
                                    LEFT JOIN DM_QD_LOAI QDL ON QDL.ID=DQD.LOAIID
                                    WHERE dqd.ket_thuc = 1
                          group by QD.DONID
      ...
      LOOP
          insert into DASHBOARD_STPT
             ...
             COMMIT;          
      END LOOP;
END DASHBOARD_CREATE_DATA_APS_PT;
