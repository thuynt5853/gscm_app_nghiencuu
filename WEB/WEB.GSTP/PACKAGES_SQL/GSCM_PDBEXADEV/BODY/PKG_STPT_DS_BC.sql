  ...
  -- Tổng hợp kháng cáo
    FOR ITEM IN (SELECT T1.* FROM TABLE(v_ARRAY) T1
       ...
       IF v_ARRAY(ITEM.v_STT).V_CHUYENNHANID > 0 THEN--TOANCAU - 01-04-2023 NẾU CÓ NỘI DUNG THÌ KHÔNG UPDATE
            SELECT LYDOCHUYEN ||  NOIDUNG INTO V_NOIDUNGNHANAN
            FROM ADS_CHUYEN_NHAN_AN
            WHERE ID = v_ARRAY(ITEM.v_STT).V_CHUYENNHANID;
        END IF;
        
        IF V_NOIDUNGNHANAN||' ' LIKE ' ' THEN
        --TOANCAU - 01-04-2023 NẾU CÓ NỘI DUNG THÌ KHÔNG UPDATE
...
(
  ...
  BEGIN
        v_tracedata := '(' || 
	    'vToaAnID => ' || vToaAnID || ',' ||
	    'vToaAnNhan_ten => ' || vToaAnNhan_ten || ',' ||
	    'vMavuviec => ' || vMavuviec || ',' ||
	    'vTenvuviec => ' || vTenvuviec || ',' ||
	    ' );';
      ...
      WHEN OTHERS THEN
          ...
          v_output := 'Finish procedure Due to Exception: Error:' || SQLCODE || ',' || SQLERRM;
		        PKG_TRACELOG.SP_INSERT_LOG_ERROR(
		            p_functionname => 'PKG_STPT_DS_BC.HC_CHUYENAN',
