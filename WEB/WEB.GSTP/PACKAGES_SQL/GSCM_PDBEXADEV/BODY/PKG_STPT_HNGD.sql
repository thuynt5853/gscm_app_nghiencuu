create or replace NONEDITIONABLE PACKAGE BODY        "PKG_STPT_HNGD" AS
      ...
      SELECT R_QUYETDINH_EXT(TTS.DONID,TTS.ID,TTS.MA)
        ...
        FROM(SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER FROM 
                    (  SELECT d.ID,d.DONID,d.TENDUONGSU,d.TUCACHTOTUNG_MA,ROW_NUMBER()  OVER (PARTITION BY d.DONID ORDER BY d.ISDAIDIEN DESC,d.TENDUONGSU) ROWNUMBER
                       FROM AHN_DON_DUONGSU d left join AHN_ANPHI p ON p.duongsu_id = d.ID
...
--      GN.TRUONGHOPGIAONHAN,
      ...
      FROM AHN_DON A
      INNER JOIN (SELECT G.* FROM AHN_DON_GIAIDOAN G WHERE (G.MAGIAIDOAN = 2 AND G.TOAANID = v_toaan_id) OR (G.MAGIAIDOAN = 3 AND G.TOAPHUCTHAMID = v_toaan_id)) GD ON A.ID=GD.DONID --Điều kiện để hiển thị 1 bản ghi duy nhất theo giai đoạn xét xử (tránh hiển thị 2 giai đoạn ở ST và PT ở tòa tỉnh) tuanvna
        ...
        WHERE   (V_TEN_VU_AN IS NULL  OR ( LOWER(A.TENVUVIEC) LIKE  '%'||LOWER(V_TEN_VU_AN)||'%' ) )--Tên vụ án
         ...
         -----
          AND (V_SOTHULY IS NULL 
                OR( Exists(Select 'X' from AHN_SOTHAM_THULY where donid = a.id and UPPER(SOTHULY)=UPPER(V_SOTHULY)) 
                    OR Exists(Select 'X' from AHN_PHUCTHAM_THULY where donid = a.id and UPPER(SOTHULY)=UPPER(V_SOTHULY)) 
                    ))--Số Thụ lý
         -----
          AND (V_THAMPHAN_ID IS NULL--Thẩm phán
             OR( EXISTS(SELECT 'x' FROM AHN_SOTHAM_HDXX PC WHERE   PC.CANBOID = V_THAMPHAN_ID AND MAVAITRO='THAMPHAN'  AND PC.DONID=A.ID))
             OR(EXISTS(SELECT 'x' FROM AHN_DON_THAMPHAN PC WHERE   PC.CANBOID = V_THAMPHAN_ID AND MAVAITRO='VTTP_GIAIQUYETSOTHAM' AND PC.DONID=A.ID) )
           )
            --GQ đơn;V_GQDON -- -- 
          AND (V_GQDON IS NULL 
...
--                  )GN ON  GN.VUANID=a.ID
        ...
        WHERE   (V_TEN_VU_AN IS NULL  OR ( LOWER(A.TENVUVIEC) LIKE  '%'||LOWER(V_TEN_VU_AN)||'%' ) )--Tên vụ án
          ...
          -----
          AND (V_SOTHULY IS NULL 
                OR(UPPER(TLS.SOTHULY)=UPPER(V_SOTHULY) OR Exists(Select 'X' from AHN_SOTHAM_THULY where donid = a.id and UPPER(SOTHULY)=UPPER(V_SOTHULY)) 
                    OR UPPER(TLPT.SOTHULY)=UPPER(V_SOTHULY) OR Exists(Select 'X' from AHN_PHUCTHAM_THULY where donid = a.id and UPPER(SOTHULY)=UPPER(V_SOTHULY)) 
                    ))--Số Thụ lý
         -----
         AND (V_THAMPHAN_ID IS NULL--Thẩm phán
             OR( EXISTS(SELECT 'x' FROM AHN_SOTHAM_HDXX PC WHERE   PC.CANBOID = V_THAMPHAN_ID AND MAVAITRO='THAMPHAN'  AND PC.DONID=A.ID))
    ...
    v_table.extend;
                ...
                item_ds.TINHTRANG_GQ,item_ds.CHECK_THULY,'3'
                );   
     END LOOP;
  ----------
  RETURN v_table;   
END DON_SEARCH_ITEM;
PROCEDURE  AHN_FILE_CHECKSTT
(   vdonviID in number,
    vMaGiaiDoan number,
    vNam number,
    vLoaiFile number,
...
END AHN_FILE_CHECKSTT;

PROCEDURE  AHN_FILE_CHECKSTT_ANPHI
(   vdonviID in number,
    vMaGiaiDoan number,
    vNam number,
    vLoaiFile number,
...
BEGIN
  ...
  checkXLD := 0;
  if(vID <> 0) then select count(id) into checkXLD from AHN_DON_XULY where ID = vID AND SOTHONGBAO = vSTT AND STB_PHU = vSTB_Phu; end if;
  if(vID = 0 OR checkXLD = 0) then 
    OPEN curReturn FOR   
    Select count(id)
    from (Select t.id From AHN_DON_XULY t 
    left join DON_YEUCAUBOSUNG y on y.DON_XULY_YCBS_ID = t.ID AND y.LOAIAN = 3
    ...
    Where t.LOAIAN = 3 AND t.TOAANID=vdonviID and t.SOTHONGBAO = vSTT and ((vSTB_Phu is null AND t.STB_PHU is null) OR (vSTB_Phu is not null AND t.STB_PHU = vSTB_Phu)) and (vID = 0 OR viD <> t.ID ) and EXTRACT(year FROM t.NGAYTHONGBAO) = vNam);
  else
    OPEN curReturn FOR   
    Select count(id)
    from (Select t.id From AHN_DON_XULY t 
    ...
    union all
    Select t.id From DON_YEUCAUBOSUNG t 
    Where t.LOAIAN = 3 AND t.TOAANID=vdonviID and t.SOTHONGBAO = vSTT and ((vSTB_Phu is null AND t.STB_PHU is null) OR (vSTB_Phu is not null AND t.STB_PHU = vSTB_Phu)) and t.DON_XULY_YCBS_ID <> t.ID and EXTRACT(year FROM t.NGAYTHONGBAO) = vNam);
  end if;
END AHN_FILE_CHECKSTT_ANPHI;
...
BEGIN
SELECT COUNT(*) INTO V_COUNTS FROM AHN_SOTHAM_THULY TL  WHERE TL.DONID = vDONID AND TL.NGAYTHULY < TO_DATE('01/05/2022','dd/MM/yyyy');
--IF(V_COUNTS>=1)THEN
--    OPEN curReturn FOR  
--      Select d.ID,d.TENDUONGSU
--      From AHN_DON_DUONGSU d 
--      LEFT JOIN AHN_ANPHI x ON x.DUONGSU_IDS like '%' || d.ID || '%'
--      Where d.DONID=vDONID And d.ISDON=1 AND d.TUCACHTOTUNG_MA='NGUYENDON' AND ((x.SOBIENLAI is not null AND x.NGAYTHONGBAO >= TO_DATE('01/05/2022','dd/MM/yyyy')) 
...
--      From AHN_DON_DUONGSU d 
--      LEFT JOIN AHN_ANPHI x ON x.DUONGSU_IDS like '%' || d.ID || '%'
--      Where d.DONID=vDONID And d.ISDON=1 AND d.TUCACHTOTUNG_MA='NGUYENDON' AND (x.SOBIENLAI is not null OR x.TINHTRANG=1)
--      ORder by d.ISDAIDIEN desc, d.TENDUONGSU;
--END IF;
IF(V_COUNTS>=1)THEN
    OPEN curReturn FOR  
      ...
      From AHN_DON_DUONGSU d 
      LEFT JOIN AHN_ANPHI x ON x.DUONGSU_IDS like '%' || d.ID || '%'
      LEFT JOIN DON_MIENANPHI dm ON x.ID = dm.ANPHI_ID AND dm.loaian = 3
      Where d.DONID=vDONID And d.ISDON=1 AND d.TUCACHTOTUNG_MA='NGUYENDON' AND (((x.SOBIENLAI is not null OR dm.ID IS NOT NULL)AND x.NGAYTHONGBAO >= TO_DATE('01/05/2022','dd/MM/yyyy')) 
        OR x.NGAYTHONGBAO < TO_DATE('01/05/2022','dd/MM/yyyy') OR x.NGAYTHONGBAO is null)
      ORder by d.ISDAIDIEN desc, d.TENDUONGSU;
ELSE
    OPEN curReturn FOR  
      ...
      LEFT JOIN AHN_ANPHI x ON x.DUONGSU_IDS like '%' || d.ID || '%'
      LEFT JOIN DON_MIENANPHI dm ON x.ID = dm.ANPHI_ID AND dm.loaian = 3
      Where d.DONID=vDONID And d.ISDON=1 AND d.TUCACHTOTUNG_MA='NGUYENDON' AND (x.SOBIENLAI is not NULL OR dm.ID IS NOT NULL OR x.TINHTRANG=1)
      ORder by d.ISDAIDIEN desc, d.TENDUONGSU;
END IF;
