...
--ITEM.V_NGAYTHULY ở dạng dd-mm-yy hoặc dd-mm-yyyy nên đang sai định dạng, thử đẩy sang dd/mm/yyyy xong gán xem ok 03/06/2025
                        -- if(ITEM.V_NGAYTHULY IS NOT NULL) then  VV_NGAYTHULY:=to_date(trim(ITEM.V_NGAYTHULY)||' 00:00:00','dd/MM/yyyy HH24:MI:SS');  end if;
                         if(ITEM.V_NGAYTHULY IS NOT NULL) then  VV_NGAYTHULY:=to_date(trim(TO_CHAR(ITEM.V_NGAYTHULY, 'dd/MM/yyyy'))||' 00:00:00','dd/MM/yyyy HH24:MI:SS');  end if;
            ...
            LOOP
                ...
                IF(CHECK_NUMBER > 0) THEN
                    ...
                    IF(HINHPHATID > 0) THEN
                            PKG_STPT_AHS_TONGHOPHINHPHAT.AHS_TONGHOPHINHPHAT_ST(0,ITEMS.ID,V_CURSOR);
    ...
    LOOP
        ...
        ELSE
            ...
            IF(CHECK_NUMBER > 0) THEN
                SELECT A.TEXT_REPORT INTO TEXT_REPORT
                FROM (SELECT QD.VUANID, LISTAGG(QD.SOQUYETDINH || CHR(10) || to_char(QD.NGAYQD,'dd/MM/yyyy')  || CHR(10) || DECODE(COALESCE(KQPT.TEN,DMQD.TEN),NULL,';',COALESCE(KQPT.TEN,DMQD.TEN)) || ';' || CHR(10) ,'')  WITHIN GROUP (ORDER BY QD.ID DESC) AS TEXT_REPORT
                      FROM AHS_PHUCTHAM_QUYETDINH_VUAN QD
                          INNER JOIN DM_QD_QUYETDINH DMQD ON DMQD.id = QUYETDINHID AND DMQD.KET_THUC = 1
                          LEFT JOIN DM_KETQUA_PHUCTHAM KQPT ON KQPT.ID = QD.KETQUAID AND KQPT.ID in (101,102,103)
                      WHERE QD.VUANID = ITEM.v_VUANID
                      GROUP BY QD.VUANID) A;
            END IF;
        END IF;          
        ...
        IF(CHECK_NUMBER > 0) THEN
            ...
            BEGIN
                IF(CHECK_NUMBER > 0) THEN
                    ...
                    select 
                            LISTAGG(  a.TEXT_REPORT, '; ' ) WITHIN GROUP (ORDER BY 'x')
                    
                            INTO TEXT_REPORT
                    From (
                    SELECT DECODE(COALESCE(KQPT.TEN,DMQD.TEN),NULL,';',COALESCE(KQPT.TEN,DMQD.TEN)) || ';' || CHR(10) as TEXT_REPORT, row_number() over (order by 'x') rn
                    FROM AHS_PHUCTHAM_QUYETDINH_VUAN QD
                        INNER JOIN DM_QD_QUYETDINH DMQD ON DMQD.id = QUYETDINHID AND DMQD.KET_THUC = 1
                        LEFT JOIN DM_KETQUA_PHUCTHAM KQPT ON KQPT.ID = QD.KETQUAID AND KQPT.ID in (101,102,103)
                    WHERE QD.VUANID = ITEM.v_VUANID
                ...
                END IF;
            EXCEPTION
                    WHEN NO_DATA_FOUND THEN TEXT_REPORT := '';
            END;
        ...
        IF(UPPER(TEXT_REPORT) LIKE '%SỬA%') THEN
            ...
            LOOP
                ...
                IF(CHECK_NUMBER > 0) THEN
                    ...
                    IF(HINHPHATID > 0) THEN
                            PKG_STPT_AHS_TONGHOPHINHPHAT.AHS_TONGHOPHINHPHAT_SOSANH(ITEM.v_VUANID,ITEMS.ID,V_CURSOR);
                            LOOP FETCH V_CURSOR INTO HINHPHAT;
                            EXIT WHEN V_CURSOR%NOTFOUND;
                            END LOOP;
                            CLOSE V_CURSOR;
...
--        NGAYTHULY BETWEEN TO_DATE(in_NGAYBATDAU,'DD/MM/YYYY') AND TO_DATE(in_NGAYKETTHUC,'DD/MM/YYYY');
    ...
    FROM (  SELECT TL.*
            ...
            FROM AHS_PHUCTHAM_THULY TL INNER JOIN TABLE(v_IDS) I ON I.v_ID=TL.TOAANID
            WHERE
                NGAYTHULY < TO_DATE(in_NGAYBATDAU,'DD/MM/YYYY') AND (NOT EXISTS (SELECT  STQD.* 
                                                                                    FROM AHS_PHUCTHAM_QUYETDINH_VUAN STQD 
                                                                                        INNER JOIN DM_QD_QUYETDINH DMQD ON DMQD.ID = STQD.QUYETDINHID AND DMQD.KET_THUC = 1 
                                                                                    WHERE TL.VUANID = STQD.VUANID)
                                                                        AND NOT EXISTS (SELECT BA.* 
                                                                                       FROM AHS_PHUCTHAM_BANAN BA 
            ...
            FROM AHS_PHUCTHAM_THULY TL INNER JOIN TABLE(v_IDS) I ON I.v_ID=TL.TOAANID
            WHERE
                NGAYTHULY < TO_DATE(in_NGAYBATDAU,'DD/MM/YYYY') AND ( EXISTS (SELECT  STQD.* 
                                                                                    FROM AHS_PHUCTHAM_QUYETDINH_VUAN STQD 
                                                                                        INNER JOIN DM_QD_QUYETDINH DMQD ON DMQD.ID = STQD.QUYETDINHID AND DMQD.KET_THUC = 1 
                                                                                    WHERE TL.VUANID = STQD.VUANID AND STQD.NGAYQD BETWEEN TO_DATE(in_NGAYBATDAU,'DD/MM/YYYY') AND TO_DATE(in_NGAYKETTHUC,'DD/MM/YYYY'))
                                                                        OR NOT EXISTS (SELECT BA.* 
                                                                                       FROM AHS_PHUCTHAM_BANAN BA 
                                                                                       WHERE TL.VUANID = BA.VUANID AND BA.NGAYBANAN BETWEEN TO_DATE(in_NGAYBATDAU,'DD/MM/YYYY') AND TO_DATE(in_NGAYKETTHUC,'DD/MM/YYYY')))                
            ) TL;
