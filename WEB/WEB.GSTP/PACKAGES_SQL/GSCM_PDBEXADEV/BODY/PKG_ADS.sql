create or replace PACKAGE BODY      PKG_ADS
AS
-- Package body
...
BEGIN	
    ...
    MaxIndex := PageIndex*PageSize ;
      ...
      --and a.magiaidoan = 2
    from ADS_ANPHI a where a.DonID =CurrDonID and ( V_TINHTRANG = 1 OR (V_TINHTRANG = 0 and  a.TINHTRANG = 0)) ;
		---------------------------------------------------
    OPEN curReturn FOR 
			select a.*, TotalItem as CountAll 
--			from (	select ROWNUM  stt,a.ID, a.DonID
--                --, d.tenduongsu || '-' || di.TEN as duongsu
-- VNPT Lê Bá Thọ 19/11/2025 sửa lấy lên tên đương sự 
               from (	select ROWNUM  stt,a.ID, a.DonID
                , (SELECT LISTAGG(d.TENDUONGSU || '-' || di.TEN, ', ') WITHIN GROUP (ORDER BY TENDUONGSU) "TENNGUOINOP"
                    FROM ADS_DON_DUONGSU d left join DM_DATAITEM di on di.MA=d.TUCACHTOTUNG_MA WHERE (d.DONID = CurrDonID OR d.DonID IN (SELECT ID FROM ADS_DON WHERE VUANGOCID=CurrDonID))  AND (',' || a.DUONGSU_IDS || ',') LIKE '%,' || d.ID || ',%') as duongsu
                ...
                , a.TOA_GIAIQUYET_ID
              from ADS_ANPHI a
              LEFT JOIN DON_MIENANPHI m ON a.ID = m.ANPHI_ID AND m.LOAIAN = 2
-- VNPT Lê Bá Thọ 19/11/2025 sửa lấy lên mã án phí 
--              left join ADS_DON_DUONGSU d on d.ID = a.DUONGSU_IDS         
--              left join DM_DATAITEM di on di.MA=d.TUCACHTOTUNG_MA
              left join dvcqg_thanh_toan t on t.ANPHI_ID=a.ID  and t.maloaivuviec = 2
...
--              ( V_TINHTRANG = 1 OR (V_TINHTRANG = 0 and  a.TINHTRANG = 0 and a.sobienlai is not null)) and 
              (a.DonID =CurrDonID or a.DonID IN (SELECT ID FROM ADS_DON WHERE VUANGOCID=CurrDonID AND IS_TACHAN IS NULL)) -- lay cua cac vu an duoc nhap
              order by a.NgayTao desc
				    ) a where a.stt>=MinIndex and a.stt<=MaxIndex;
END ADS_DON_ANPHI_GETBYDONID_V2;
...
BEGIN	
    ...
    MaxIndex := PageIndex*PageSize ;
      ...
      --and a.magiaidoan = 2
      --08/12/2025 vnpt le ba tho sua de luu an phi KCKN
    from ADS_ANPHI a where a.DonID =CurrDonID and ( V_TINHTRANG = 1 OR (V_TINHTRANG = 0 and  a.TINHTRANG = 0))and a.magiaidoan = P_GIAIDOAN  ;
		---------------------------------------------------
    OPEN curReturn FOR 
			select a.*, TotalItem as CountAll 
--			from (	select ROWNUM  stt,a.ID, a.DonID
...
END ADS_DON_ANPHI_GETBYDONID_KCKN;
...
BEGIN	
    ...
    MaxIndex := PageIndex*PageSize ;
		...
		--1 :den tong so ban ghi trong bang tbltintuc va luu vao trog bien total      
--	  select count (a.ID) into TotalItem 
--    from ADS_ANPHI_LICHSU a where a.DonID =CurrDonID and ( V_TINHTRANG = 1 OR (V_TINHTRANG = 0 and  a.TINHTRANG = 0)) AND a.A_ANPHI_ID = anPhiId;
		---------------------------------------------------
    OPEN curReturn FOR 
			select a.* 
		...
		(select  a.ID, 0 AS mienanphiId, a.DonID
               , (SELECT LISTAGG(d.TENDUONGSU || '-' || di.TEN, ', ') WITHIN GROUP (ORDER BY TENDUONGSU) "TENNGUOINOP"
                ...
                ,t.id DVCQG_TT_ID
              from ADS_ANPHI a
--              left join ADS_DON_DUONGSU d on d.ID = a.DUONGSU_ID
--              left join DM_DATAITEM di on di.MA=d.TUCACHTOTUNG_MA
              left join dvcqg_thanh_toan t on t.ANPHI_ID=a.ID  and t.maloaivuviec = 2
              left join tuphap_anphi tp on tp.dvcqg_tt_id=t.id
              left join tuphap_anphi_cn cn on cn.tuphap_anphi_id=tp.id
...
--              ( V_TINHTRANG = 1 OR (V_TINHTRANG = 0 and  a.TINHTRANG = 0 and a.sobienlai is not null)) and 
	...
	UNION ALL
			select  a.ID, m.id as mienanphiId ,a.DonID
                ...
                ,t.id DVCQG_TT_ID
              from ADS_ANPHI a
              INNER JOIN DON_MIENANPHI m ON a.ID = m.ANPHI_ID AND m.LOAIAN = 2
--              left join ADS_DON_DUONGSU d on d.ID = a.DUONGSU_ID
--              left join DM_DATAITEM di on di.MA=d.TUCACHTOTUNG_MA
              left join dvcqg_thanh_toan t on t.ANPHI_ID=a.ID  and t.maloaivuviec = 2
              left join tuphap_anphi tp on tp.dvcqg_tt_id=t.id
...
END ADS_ANPHI_LICHSU_GETBYDONID;
...
OPEN curReturn FOR  
      ...
      FROM ADS_ANPHI a      
      INNER JOIN ADS_DON_DUONGSU d ON d.ID = a.DUONGSU_IDS  -- VNPT- Lê BÁ Thọ sửa  lấy ra đương sự 19/11/2025
      LEFT JOIN DON_MIENANPHI dm ON dm.ANPHI_ID = a.ID AND dm.LOAIAN = 2
      left join DM_DATAITEM i on i.MA=d.TUCACHTOTUNG_MA
      WHERE a.DONID = vDONID AND a.TINHTRANG = 0 AND dm.ID IS NULL;
  END ADS_DON_DUONGSU_BIENLAI_V2;

-- 2. AHC_DON_ANPHI_GETBYDONID
END PKG_ADS;