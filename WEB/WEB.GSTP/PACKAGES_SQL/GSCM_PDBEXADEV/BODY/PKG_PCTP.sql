 ...
 FOR i IN (Select ID,'DS' as LOAIVV  from ADS_DON v where v.ToaAnID=vToaAnID and v.NGAYNHANDON between vFromDate and vToDate And CheckIsNotGQD(v.ID,'DS','VTTP_GIAIQUYETDON')=0
    ...
    /*Phân công ngẫu nhiên cho vụ việc*/
        ...
        if(vThamphanID>0) then
          ...
          If i.LOAIVV='DS' Then
            Insert Into ADS_DON_THAMPHAN(DONID,CANBOID,MAVAITRO,NGAYPHANCONG,NGAYNHANPHANCONG,NGUOIPHANCONGID,TOA_GIAIQUYET_ID) -- UPDATE toa_giaiquyet_id
            VALUES(i.ID,vThamphanID,'VTTP_GIAIQUYETDON',(SELECT SYSTIMESTAMP FROM DUAL),(SELECT SYSTIMESTAMP FROM DUAL),vChanhAnID,vToaAnID);
          ElsIf i.LOAIVV='HC' Then
            Insert Into AHC_DON_THAMPHAN(DONID,CANBOID,MAVAITRO,NGAYPHANCONG,NGAYNHANPHANCONG,NGUOIPHANCONGID, TOA_GIAIQUYET_ID)
            VALUES(i.ID,vThamphanID,'VTTP_GIAIQUYETDON',(SELECT SYSTIMESTAMP FROM DUAL),(SELECT SYSTIMESTAMP FROM DUAL),vChanhAnID,vToaAnID);
          ElsIf i.LOAIVV='HN' Then
            Insert Into AHN_DON_THAMPHAN(DONID,CANBOID,MAVAITRO,NGAYPHANCONG,NGAYNHANPHANCONG,NGUOIPHANCONGID, TOA_GIAIQUYET_ID)
            VALUES(i.ID,vThamphanID,'VTTP_GIAIQUYETDON',(SELECT SYSTIMESTAMP FROM DUAL),(SELECT SYSTIMESTAMP FROM DUAL),vChanhAnID, vToaAnID);
          ElsIf i.LOAIVV='KT' Then
            Insert Into AKT_DON_THAMPHAN(DONID,CANBOID,MAVAITRO,NGAYPHANCONG,NGAYNHANPHANCONG,NGUOIPHANCONGID,TOA_GIAIQUYET_ID) -- UPDATE toa_gq_id
            VALUES(i.ID,vThamphanID,'VTTP_GIAIQUYETDON',(SELECT SYSTIMESTAMP FROM DUAL),(SELECT SYSTIMESTAMP FROM DUAL),vChanhAnID,vToaAnID);  
          ElsIf i.LOAIVV='LD' Then
            Insert Into ALD_DON_THAMPHAN(DONID,CANBOID,MAVAITRO,NGAYPHANCONG,NGAYNHANPHANCONG,NGUOIPHANCONGID, TOA_GIAIQUYET_ID)
            VALUES(i.ID,vThamphanID,'VTTP_GIAIQUYETDON',(SELECT SYSTIMESTAMP FROM DUAL),(SELECT SYSTIMESTAMP FROM DUAL),vChanhAnID, vToaAnID);
