    ...
    -------------------------------------------------------------------------------------------------
        --Chưa phân công TTV
         ...
         and ( LoaiAnDB = 0 -- Thuộc án
                ...
                or (LoaiAnDB = 2 and NVL(v.IsAnChiDao,0)=1)
                or (LoaiAnDB = 3 and PKG_GDTTT_BAOCAO_APP.CHECK_ANTHOIHIEU(NVL(v.LoaiAn,0),DECODE(v.BAQD_CAPXETXU,4,v.NGAYQD,2,v.NGAYXUSOTHAM,
                                                                    v.NGAYXUPHUCTHAM), v.NGAYXUSOTHAM, NVL(v.ThuLyLai_VuAnId, 0),v.NGAYTHULYDON)<90)
         ) ; 
         --
              v_table.extend;
        ...
        and ( LoaiAnDB = 0 -- Thuộc án
                ...
                or (LoaiAnDB = 2 and NVL(v.IsAnChiDao,0)=1)
                or (LoaiAnDB = 3 and PKG_GDTTT_BAOCAO_APP.CHECK_ANTHOIHIEU(NVL(v.LoaiAn,0), DECODE(v.BAQD_CAPXETXU,4,v.NGAYQD,2,v.NGAYXUSOTHAM,v.NGAYXUPHUCTHAM), v.NGAYXUSOTHAM, NVL(v.ThuLyLai_VuAnId, 0),v.NGAYTHULYDON)<90)
         ) ; 
         --
              v_table.extend;
              v_table(v_table.count) := R_THONGKE_TONGHOP(
