create or replace PACKAGE BODY      PKG_ALD 
AS
-- Package body
...
--                , d.tenduongsu || '-' || di.TEN as duongsu
 , (SELECT LISTAGG(d.TENDUONGSU || '-' || di.TEN, ', ') WITHIN GROUP (ORDER BY TENDUONGSU) "TENNGUOINOP"
                ...
                ,a.TOA_GIAIQUYET_ID
              from ALD_ANPHI a
              LEFT JOIN DON_MIENANPHI m ON a.ID = m.ANPHI_ID AND m.LOAIAN = 5
--              left join ALD_DON_DUONGSU d on d.ID = a.DUONGSU_IDS
...
END ALD_DON_ANPHI_GETBYDONID_V2;
...
BEGIN	
    ...
    MaxIndex := PageIndex*PageSize ;
		...
		--1 :den tong so ban ghi trong bang tbltintuc va luu vao trog bien total      
--	  select count (a.ID) into TotalItem 
--    from ALD_ANPHI_LICHSU a where a.DonID =CurrDonID and ( V_TINHTRANG = 1 OR (V_TINHTRANG = 0 and  a.TINHTRANG = 0)) AND a.A_ANPHI_ID = anPhiId;
		---------------------------------------------------
    OPEN curReturn FOR 
			select a.*
...
--              ( V_TINHTRANG = 1 OR (V_TINHTRANG = 0 and  a.TINHTRANG = 0 and a.sobienlai is not null)) and 
        ...
        UNION ALL
        	select a.ID, m.id as mienanphiId, a.DonID
                ...
                ,t.id DVCQG_TT_ID
              from ALD_ANPHI a
              INNER JOIN DON_MIENANPHI m ON a.ID = m.ANPHI_ID AND m.LOAIAN = 5
              left join ALD_DON_DUONGSU d on d.ID = a.DUONGSU_ID
              left join DM_DATAITEM di on di.MA=d.TUCACHTOTUNG_MA
              left join dvcqg_thanh_toan t on t.ANPHI_ID=a.ID  and t.maloaivuviec = 5
              left join tuphap_anphi tp on tp.dvcqg_tt_id=t.id
...
END ALD_ANPHI_LICHSU_GETBYDONID;
...
OPEN curReturn FOR  
      ...
      FROM ALD_ANPHI a      
      INNER JOIN ALD_DON_DUONGSU d ON d.ID = a.DUONGSU_IDS -- VNPT- Lê BÁ Thọ sửa  lấy ra đương sự 22/11/2025
      LEFT JOIN DON_MIENANPHI dm ON dm.ANPHI_ID = a.ID AND dm.LOAIAN = 5
      left join DM_DATAITEM i on i.MA=d.TUCACHTOTUNG_MA
      WHERE a.DONID = vDONID AND a.TINHTRANG = 0 AND dm.ID IS NULL;
  END ALD_DON_DUONGSU_BIENLAI_V2;
