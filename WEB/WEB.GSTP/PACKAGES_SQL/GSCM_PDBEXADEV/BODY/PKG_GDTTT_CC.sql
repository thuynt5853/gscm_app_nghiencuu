...
--HS_ISKHIEUNAI=0 bo nguoi khieu nai khi them dơn trung 
        ...
        IF(V_COUNT_DS=0)THEN
             ...
             FROM GDTTT_DON_DUONGSU_CC WHERE DONID=V_DON_TRUNG and HS_ISKHIEUNAI=0;
        END IF;
         -------------
         IF(V_LOAIAN='01')THEN
         --24/09/2024 khong lấy 
...
BEGIN
  OPEN curReturn FOR
        ...
        where ds.DONID =VDONID        
            and (vTucachtotung is null 
                  or (isDauVu=2 and TuCachToTung = vTucachtotung)
                  or (isDauVu<2 and TuCachToTung = vTucachtotung and NVL(ds.hs_Bicandauvu,0)=isDauVu)
            )
        order by DONID;
...
PROCEDURE GDTTT_DS_TOIDANH_GETALL_KN
( 
   VDONID in number,
   vTucachtotung varchar2,
   isDauVu in number,
...
BEGIN
  OPEN curReturn FOR
        ...
        left join gdttt_don d on ds.DONID=d.id
        where ds.DONID =VDONID     
            and NVL(ds.HS_IsKhieuNai, 0)=0
            and (vTucachtotung is null 
                  or (isDauVu=2 and TuCachToTung = vTucachtotung)
                  or (isDauVu<2 and TuCachToTung = vTucachtotung and NVL(ds.hs_Bicandauvu,0)=isDauVu)
            )
        order by DONID;
end GDTTT_DS_TOIDANH_GETALL_KN;
...
BEGIN       
    V_SOTL:=0;
    SELECT nvl(Max(to_number(regexp_replace(D1.TL_SO, '[^[:digit:]]', ''))),0)INTO V_SOTL FROM 
    ( SELECT D.TL_SO FROM GDTTT_DON D
            WHERE  --EXTRACT(YEAR FROM  TO_DATE(D.TL_NGAY, 'DD-MON-RR'))= EXTRACT(YEAR FROM  TO_DATE(SYSDATE, 'DD-MON-RR')) -- Trường Date hệ thống sẽ tự hiểu nên không cần thêm TO_DATE
                   EXTRACT(YEAR FROM  D.TL_NGAY) = EXTRACT(YEAR FROM SYSDATE)
            AND( (V_LOAI_VB not in (4,8,10) AND D.LOAIDON not in (4,8,10) )  
      ...
      UNION
       SELECT xtl.SOTHULY as TL_SO FROM GDTTT_DON_XOATL xtl 
            WHERE 
                 EXTRACT(YEAR FROM  xtl.NGAYTHULY) = EXTRACT(YEAR FROM SYSDATE)
                  AND xtl.TOAANID=V_TOAANID 
                  AND xtl.LOAIAN=V_BAQD_LOAIAN                   
    ) D1
;
    V_SOTL := V_SOTL+1;   
END; 
...
BEGIN
/*   
     Sổ thụ lý chia theo mỗi loại án, mỗi loại án chia theo 3 loại sổ
     1. Hồ sơ Kháng nghị GĐT,TT - 4
     2. Đơn khiếu nại tư pháp - 8
        Đơn khiếu nại tư pháp kèm CV chuyển đơn - 10
     3. Văn bản hành chính,Tài liệu chung - 5
        Đơn đề nghị GĐT,TT - 1
        Đơn đề nghị GĐT,TT kèm theo CV chuyển đơn - 3
        CV kiến nghị GĐT,TT - 6
        CV kiến nghị GĐT,TT kèm Hồ sơ - 9
        Thông báo phát hiện vi phạm pháp luật - 7
*/
    vY:=vYear;
    OPEN curReturn FOR  
          Select d.ID
