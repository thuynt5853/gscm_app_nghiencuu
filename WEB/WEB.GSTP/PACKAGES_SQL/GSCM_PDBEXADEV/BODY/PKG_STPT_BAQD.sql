create or replace PACKAGE BODY      PKG_STPT_BAQD AS
...
END;
    ...
    OPEN curReturn FOR  
      Select q.ID,q.SOQD,q.NGAYQD,q.CHUCVU,q.QUYETDINHID
        ...
        else 0 end)as READONLY,q.TOA_GIAIQUYET_ID,
        (SELECT  -- 26/11/2025 VNPT Dinh Hoang Son - lay them ten duong su
         CASE 
             WHEN COUNT(dds.ID) = 0 THEN '' 
             ELSE '(' || LISTAGG(dds.TENDUONGSU, ', ') WITHIN GROUP (ORDER BY dds.TENDUONGSU) || ')'
         END
         FROM ADS_DON_DUONGSU dds
         WHERE ',' || q.DUONGSU_IDS || ',' LIKE '%,' || dds.ID || ',%'
        ) AS TENDUONGSU
      From ADS_SOTHAM_QUYETDINH q
      left join ADS_FILE f on q.FILEID=f.ID
      left join DM_QD_QUYETDINH_LYDO ld on ld.ID=q.LYDOID
