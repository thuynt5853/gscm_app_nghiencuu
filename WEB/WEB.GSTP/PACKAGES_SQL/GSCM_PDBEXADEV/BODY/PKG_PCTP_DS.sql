--------------------------------------------------------
--  DDL for Package Body PKG_PCTP_DS
--------------------------------------------------------

  CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "GSCM"."PKG_PCTP_DS" AS
    ...
    OPEN curReturn FOR
        SELECT tt.* from (
            SELECT  ROW_NUMBER() OVER (ORDER BY a.NGAYTAO desc) STT,
                ...
                DECODE(GD.MAGIAIDOAN,2, sotham_tp.HOTEN,3,phuctham_tp.HOTEN, '')THAMPHAN,
                DECODE(GD.MAGIAIDOAN,2, sotham_tk.HOTEN,3,phuctham_tk.HOTEN, '')THUKY,
                DECODE(GD.MAGIAIDOAN,2, sotham_qhpl.TEN,3,phuctham_qhpl.TEN, '')QUANHEPL,
                DECODE(GD.MAGIAIDOAN,2, sotham_thuly.MATHULY,3,phuctham_thuly.MATHULY, '')SOTHULY,
                DECODE(GD.MAGIAIDOAN,2,  TO_CHAR(sotham_thuly.NGAYTHULY,'dd/MM/yyyy') ,3,TO_CHAR(phuctham_thuly.NGAYTHULY,'dd/MM/yyyy'), '')NGAYTHULY,
                (select LISTAGG(ds.TENDUONGSU, ', ') WITHIN GROUP (ORDER BY  ds.TENDUONGSU) "TENDUONGSU" from ADS_DON_DUONGSU ds where ds.TUCACHTOTUNG_MA = 'NGUYENDON' and ds.DONID=a.ID  group by  ds.donid) TENNGUYENDON,
                (select LISTAGG(ds.TENDUONGSU, ', ') WITHIN GROUP (ORDER BY  ds.TENDUONGSU) "TENDUONGSU" from ADS_DON_DUONGSU ds where ds.TUCACHTOTUNG_MA = 'BIDON' and ds.DONID=a.ID  group by  ds.donid) TENBIDON,


                NVL(hs_stpt.TRANGTHAIID,0) TRANGTHAIHOSO,
                --1:Hố sơ đã lưu; 2:Hố sơ cho muon; 3:Hồ sơ đã chuyên
                DECODE(hs_stpt.TRANGTHAIID,1, 'Hồ sơ đã lưu',2,'Hồ sơ cho mượn', 3,'Hồ sơ đã chuyển','Chưa lưu') TENTRANGTHAIHOSO

              FROM ADS_DON A
              INNER JOIN ADS_DON_GIAIDOAN GD ON A.ID=GD.DONID
              --left join DM_DATAITEM di_qhpl on a.QUANHEPHAPLUATID=di_qhpl.ID
              LEFT JOIN DM_TOAAN T ON A.TOAANID=T.ID
                    LEFT JOIN (
                            SELECT BA.DONID,'</br>- Đã có bản án sơ thẩm' TINHTRANG_GQ, BA.SOBANAN, ba.NGAYTUYENAN FROM ADS_SOTHAM_BANAN BA
                     ...
                     LEFT JOIN ( 
                            SELECT PTBA.DONID,'</br>- Đã có bản án phúc thẩm' TINHTRANG_GQ,PTBA.SOBANAN, PTBA.NGAYTUYENAN  FROM ADS_PHUCTHAM_BANAN PTBA 
                  ...
                  --lấy thông tin thụ lý sơ thẩm   
                  left join (select thuly.donid,LISTAGG( thuly.MATHULY, ', ') WITHIN GROUP (ORDER BY  thuly.MATHULY) "MATHULY", thuly.NGAYTHULY from ADS_SOTHAM_THULY thuly  group by  thuly.donid,thuly.NGAYTHULY)sotham_thuly on sotham_thuly.donid = A.ID

                -- lấy thông tin thẩm phán sơ thẩm
                       left join (select  hdxx.donid,LISTAGG( cb.HOTEN, ', ') WITHIN GROUP (ORDER BY  cb.HOTEN) "HOTEN"
                       from ADS_SOTHAM_HDXX hdxx join DM_CANBO cb on hdxx.Canboid = cb.id and hdxx.MAVAITRO='THAMPHAN' group by  hdxx.donid ) sotham_tp on sotham_tp.donid = A.ID
                       -- lấy thông tin thư ký sơ thẩm
                        left join (select hdxx.donid,LISTAGG( cb.HOTEN, ', ') WITHIN GROUP (ORDER BY  cb.HOTEN) "HOTEN"from ADS_SOTHAM_HDXX hdxx join DM_CANBO cb on hdxx.Canboid = cb.id and hdxx.MAVAITRO='THUKY' group by  hdxx.donid)sotham_tk on sotham_tk.donid = A.ID
                      -- lấy thông tin quan hệ pháp luật sơ thẩm
                      left join (select st_banan.donid,LISTAGG( DECODE(di.TEN,null, st_banan.QUANHEPHAPLUAT_NAME, di.TEN)  , ', ') WITHIN GROUP (ORDER BY  di.TEN) "TEN"from ADS_SOTHAM_BANAN st_banan left join DM_DATAITEM di on st_banan.QUANHEPHAPLUATID = di.id   group by  st_banan.donid)sotham_qhpl on sotham_qhpl.donid = A.ID

                       -- lấy thông tin thẩm phán phúc thẩm
                       left join (select distinct hdxx.donid,LISTAGG( cb.HOTEN, ', ') WITHIN GROUP (ORDER BY  cb.HOTEN) "HOTEN" from ADS_PHUCTHAM_HDXX hdxx join DM_CANBO cb on hdxx.Canboid = cb.id and hdxx.MAVAITRO='THAMPHAN' group by  hdxx.donid) phuctham_tp on phuctham_tp.donid = A.ID
                       -- lấy thông tin thư ký phúc thẩm
                       left join (select distinct hdxx.donid,LISTAGG( cb.HOTEN, ', ') WITHIN GROUP (ORDER BY  cb.HOTEN) "HOTEN" from ADS_PHUCTHAM_HDXX hdxx join DM_CANBO cb on hdxx.Canboid = cb.id and hdxx.MAVAITRO='THUKY' group by  hdxx.donid)phuctham_tk on phuctham_tk.donid = A.ID

                  -- lấy thông tin quan hệ pháp luật phúc thẩm
                      left join (select phuctham_banan.donid,LISTAGG( DECODE(di.TEN,null, phuctham_banan.QUANHEPHAPLUAT_NAME, di.TEN)  , ', ') WITHIN GROUP (ORDER BY  di.TEN) "TEN"from ADS_PHUCTHAM_BANAN phuctham_banan left join DM_DATAITEM di on phuctham_banan.QUANHEPHAPLUATID = di.id   group by  phuctham_banan.donid)phuctham_qhpl on phuctham_qhpl.donid = A.ID
                        --lấy thông tin thụ lý phúc thẩm   
                         left join (select thuly.donid,LISTAGG( thuly.MATHULY, ', ') WITHIN GROUP (ORDER BY  thuly.MATHULY) "MATHULY",thuly.NGAYTHULY from ADS_PHUCTHAM_THULY thuly  group by  thuly.donid,thuly.NGAYTHULY)phuctham_thuly on phuctham_thuly.donid = A.ID
                ...
                WHERE 
                    ...
                    AND ( (GD.MAGIAIDOAN =2 AND  EXISTS(SELECT 1 FROM ADS_SOTHAM_BANAN QSV WHERE A.ID=QSV.DONID) OR EXISTS(SELECT 'X' FROM ADS_SOTHAM_QUYETDINH QSV join DM_QD_QUYETDINH dmqd on dmqd.id = qsv.LOAIQDID join DM_QD_LOAI dmlqd on dmlqd.id=dmqd.loaiid WHERE A.ID=QSV.DONID and dmlqd.ma='DC')  )
                    OR (GD.MAGIAIDOAN =3 AND  EXISTS(SELECT 1 FROM ADS_PHUCTHAM_BANAN QSV WHERE A.ID=QSV.DONID) OR EXISTS(SELECT 'X' FROM ADS_PHUCTHAM_QUYETDINH QSV join DM_QD_QUYETDINH dmqd on dmqd.id = qsv.LOAIQDID join DM_QD_LOAI dmlqd on dmlqd.id=dmqd.loaiid WHERE A.ID=QSV.DONID and dmlqd.ma='DC')  )
                    ...
                    AND (vNguyenDon is null OR     LOWER((select LISTAGG(ds.TENDUONGSU, ', ') WITHIN GROUP (ORDER BY  ds.TENDUONGSU) "TENDUONGSU" from ADS_DON_DUONGSU ds where ds.TUCACHTOTUNG_MA = 'NGUYENDON' and ds.DONID=a.ID  group by  ds.donid)) LIKE  '%'||LOWER(vNguyenDon)||'%'   )

                    AND (vBiDon is null OR     LOWER((select LISTAGG(ds.TENDUONGSU, ', ') WITHIN GROUP (ORDER BY  ds.TENDUONGSU) "TENDUONGSU" from ADS_DON_DUONGSU ds where ds.TUCACHTOTUNG_MA = 'NGUYENDON' and ds.DONID=a.ID  group by  ds.donid)) LIKE  '%'||LOWER(vBiDon)||'%'   )

                     AND (vMaVuViec is null OR   LOWER(DECODE(GD.MAGIAIDOAN,2, sotham_thuly.MATHULY,3,phuctham_thuly.MATHULY, '')) LIKE  '%'||LOWER(vMaVuViec)||'%'   )   
                   ...
                   --------------
--                    AND (vLoaian is null Or 
--                            ((EXISTS(SELECT 'X' FROM ADS_SOTHAM_BANAN QSV WHERE A.ID=QSV.DONID)
--                                OR EXISTS(SELECT 'X' FROM ADS_SOTHAM_QUYETDINH QSV WHERE A.ID=QSV.DONID)
--                                OR EXISTS(SELECT 'X' FROM ADS_PHUCTHAM_BANAN QSV WHERE A.ID=QSV.DONID)
--                                OR EXISTS(SELECT 'X' FROM ADS_PHUCTHAM_QUYETDINH QSV WHERE A.ID=QSV.DONID)
--                            )
--                            --Kiem tra them voi cac loai an khac
--                            )
--                        )
                   --Tình trạng GQ Đã giải quyết xong;
...
BEGIN 
    ...
    SELECT A.TOAANID INTO V_TOAANID FROM ADS_DON A WHERE A.ID=V_DONID;
    ----------
    IF(V_TRANGTHAI=2) THEN
       IF(V_NGAYPHANCONGTP IS NOT NULL AND  V_THAM_PHAN_ID IS NOT NULL AND V_NGAYPHANCONGLD IS NOT NULL AND V_PHANCONGLD_ID IS NOT NULL) THEN
         SELECT COUNT(*) INTO V_COUNT_CHECK FROM ADS_DON_THAMPHAN X 
          ...
          IF(V_COUNT_CHECK=0)THEN
             INSERT INTO ADS_DON_THAMPHAN
               (ID,DONID,CANBOID,MAVAITRO,NGAYPHANCONG,NGAYNHANPHANCONG,NGUOIPHANCONGID,
                NGAYTAO,NGUOITAO,NGAYSUA,NGUOISUA,TOA_GIAIQUYET_ID) -- UPDATE toa_giaiquyet_id
             VALUES (ADS_DON_THAMPHAN_SEQ.NEXTVAL,V_DONID,V_THAM_PHAN_ID,V_VAITRO,TO_DATE(V_NGAYPHANCONGLD,'dd/MM/yyyy'),TO_DATE(V_NGAYPHANCONGTP,'dd/MM/yyyy'),V_PHANCONGLD_ID,
   ...
   select tt.* from (
      ...
      LEFT JOIN DM_TOAAN T ON A.TOAANID=T.ID
        ------Trạng thái giải quyết trong danh sách
            LEFT JOIN (SELECT TL.DONID,TL.SOTHULY,TL.NGAYTHULY,'</br>- Đã thụ lý' TINHTRANG_GQ FROM ADS_SOTHAM_THULY TL 
            ...
            LEFT JOIN (SELECT TP.DONID,'</br>- Đã phân công Thẩm phán' TINHTRANG_GQ FROM ADS_DON_THAMPHAN TP WHERE TP.MAVAITRO='VTTP_GIAIQUYETSOTHAM' 
            ...
            LEFT JOIN (
                        SELECT QSV.DONID,'</br>- Đang hoãn phiên tòa sơ thẩm'TINHTRANG_GQ FROM ADS_SOTHAM_QUYETDINH QSV
             ...
             LEFT JOIN (
                 ...
                 LEFT JOIN (
                        SELECT QSV.DONID,'</br>- Đang tạm đình chỉ' TINHTRANG_GQ FROM ADS_SOTHAM_QUYETDINH QSV
                        INNER JOIN DM_QD_LOAI QDL ON QSV.LOAIQDID=QDL.ID 
                        LEFT JOIN ADS_SOTHAM_BANAN BA ON QSV.DONID =BA.DONID 
                ...
                LEFT JOIN (
                        SELECT BA.DONID,'</br>- Đã có bản án sơ thẩm' TINHTRANG_GQ FROM ADS_SOTHAM_BANAN BA
                        WHERE  BA.SOBANAN IS NOT NULL
              ...
              LEFT JOIN (
                       SELECT QSV.DONID,'</br>- Đã có QĐ đình chỉ' TINHTRANG_GQ FROM ADS_SOTHAM_QUYETDINH QSV 
                       INNER JOIN DM_QD_LOAI QDL ON QSV.LOAIQDID=QDL.ID
             ...
             LEFT JOIN (
                       SELECT QSV.DONID,'</br>- Đã có QĐ công nhận thỏa thuận' TINHTRANG_GQ FROM ADS_SOTHAM_QUYETDINH QSV 
                       INNER JOIN DM_QD_LOAI QDL ON QSV.LOAIQDID=QDL.ID
                       WHERE instr('CNTT',QDL.MA)>0
                       GROUP BY QSV.DONID,'</br>- Đã có QĐ công nhận thỏa thuận'
                       )CNTT ON  CNTT.DONID=a.id AND GD.MAGIAIDOAN=2
             LEFT JOIN (
                       SELECT QSV.DONID,'</br>- Đã có QĐ chuyển vụ án' TINHTRANG_GQ FROM ADS_SOTHAM_QUYETDINH QSV 
                       INNER JOIN DM_QD_LOAI QDL ON QSV.LOAIQDID=QDL.ID
                            ...
                            SELECT TT.DONID,RTRIM(SUBSTR(TT.HOTEN,0,INSTR(TT.HOTEN,',',1,DECODE(TT.COUNT_DS,1,1,2,2,3,3,3) )),',')HOTEN 
                                FROM (
                                      select COUNT(*)COUNT_DS,DS.DONID,
                                        ...
                                        FROM ADS_DON_DUONGSU DS WHERE DS.ISDAIDIEN=0
                                        GROUP BY DS.DONID
                                     )TT
                     )TS
                            ...
                            SELECT TT.DONID,RTRIM(SUBSTR(TT.HOTEN,0,INSTR(TT.HOTEN,',',1,DECODE(TT.COUNT_DS,1,1,2,2,3,3,3) )),',')HOTEN 
                                FROM (
                                      select COUNT(*)COUNT_DS,DS.DONID,
                                        ...
                                        INNER JOIN (SELECT DUONGSUID, LOAIKHANGCAO,DONID FROM ADS_SOTHAM_KHANGCAO ) KC ON KC.DUONGSUID = DS.ID AND KC.DONID=DS.DONID
        ...
        WHERE   (V_TEN_VU_AN IS NULL  OR ( LOWER(A.TENVUVIEC) LIKE  '%'||LOWER(V_TEN_VU_AN)||'%' ) )--Tên vụ án
            ...
            AND (V_TENDUONGSU IS NULL --Đương sự
                  OR( EXISTS (SELECT 'X' FROM ADS_DON_DUONGSU DS WHERE  UPPER(DS.TENDUONGSU) LIKE '%'||UPPER(V_TENDUONGSU)||'%' AND DS.DONID=A.ID)
                    )
            ) 
...
--         -----   
         ...
         AND (V_THAMPHAN_ID IS NULL
            ...
            --GQ đơn;V_GQDON -- -- 
          AND (V_GQDON IS NULL 
              OR(EXISTS ( SELECT 'X' FROM ADS_DON_XULY XL WHERE XL.LOAIGIAIQUYET=V_GQDON AND XL.DONID=A.ID) ) 
            )  
   ...
   FOR item in  (
      ...
      LEFT JOIN DM_TOAAN T ON A.TOAANID=T.ID
        ------Trạng thái giải quyết trong danh sách
          ...
          LEFT JOIN (
                 SELECT DS.TENDUONGSU,DS.DONID  FROM ADS_DON_DUONGSU DS WHERE DS.ISDAIDIEN=1 AND DS.TUCACHTOTUNG_MA='NGUYENDON'          
          )NDST ON NDST.DONID=A.ID AND GD.MAGIAIDOAN=2
         LEFT JOIN (
                SELECT DS.TENDUONGSU,DS.DONID  FROM ADS_DON_DUONGSU DS WHERE DS.ISDAIDIEN=1 AND DS.TUCACHTOTUNG_MA='NGUYENDON'          
          )NDPT ON NDPT.DONID=A.ID AND GD.MAGIAIDOAN=3
         LEFT JOIN (
                 SELECT DS.TENDUONGSU,DS.DONID  FROM ADS_DON_DUONGSU DS WHERE DS.ISDAIDIEN=1 AND DS.TUCACHTOTUNG_MA='BIDON'          
                            ...
                            FROM (
                                    ...
                                    FROM (   
                                          SELECT F.DON_ND FROM (
                                               select KC.DONID||';'
                                               ||decode(DS.TUCACHTOTUNG_MA,'BIDON','BĐ','NGUYENDON','NĐ','QUYENNVLQ','NLQ') ||' kc: '
                                ...
                                UNION ALL
                                    SELECT KC.DONID,'kc:'NOIDUNGKHANGCAO FROM ADS_SOTHAM_KHANGCAO  KC
                                    LEFT JOIN ADS_DON_DUONGSU DS ON DS.ID=KC.DUONGSUID
        ...
        LEFT JOIN (
                SELECT NDKN.DONID,LISTAGG(NDKN.NOIDUNGKN, '<br style="mso-data-placement:same-cell;" />')  WITHIN GROUP (ORDER BY NDKN.NOIDUNGKN)NOIDUNGKN
                            FROM (
                                    SELECT KC.DONID,' kn: ' ||KC.NOIDUNGKN NOIDUNGKN FROM ADS_SOTHAM_KHANGNGHI KC
                                    WHERE KC.NOIDUNGKN is not null
                                UNION ALL
                                    SELECT KC.DONID,'kn:'NOIDUNGKN FROM ADS_SOTHAM_KHANGNGHI KC
...
--           )PCTP_GQ ON PCTP_GQ.DONID = A.ID 
--          LEFT JOIN (
--                 SELECT DS.DONID,LISTAGG (DS.TENDUONGSU, ', ') WITHIN GROUP (ORDER BY DS.TENDUONGSU)TENDUONGSU  FROM ADS_DON_DUONGSU DS WHERE  DS.TUCACHTOTUNG_MA='NGUYENDON'          
--                 GROUP BY DS.DONID
--          )NDST ON NDST.DONID=A.ID AND GD.MAGIAIDOAN=2
--         LEFT JOIN (
--                SELECT DS.DONID,LISTAGG (DS.TENDUONGSU, ', ') WITHIN GROUP (ORDER BY DS.TENDUONGSU)TENDUONGSU FROM ADS_DON_DUONGSU DS WHERE DS.TUCACHTOTUNG_MA='NGUYENDON'     
...
--          )NDPT ON NDPT.DONID=A.ID AND GD.MAGIAIDOAN=3
--         LEFT JOIN (
--                 SELECT DS.DONID,LISTAGG (DS.TENDUONGSU, ', ') WITHIN GROUP (ORDER BY DS.TENDUONGSU)TENDUONGSU  FROM ADS_DON_DUONGSU DS WHERE DS.ISDAIDIEN=1 AND DS.TUCACHTOTUNG_MA='BIDON'          
--                 GROUP BY DS.DONID
--          )BDST ON BDST.DONID=A.ID AND GD.MAGIAIDOAN=2
--         LEFT JOIN (
--                SELECT DS.DONID,LISTAGG (DS.TENDUONGSU, ', ') WITHIN GROUP (ORDER BY DS.TENDUONGSU)TENDUONGSU  FROM ADS_DON_DUONGSU DS WHERE DS.ISDAIDIEN=1 AND DS.TUCACHTOTUNG_MA='BIDON'          
...
--                SELECT NDKN.DONID,LISTAGG(NDKN.NOIDUNGKN, '<br style="mso-data-placement:same-cell;" />')  WITHIN GROUP (ORDER BY NDKN.NOIDUNGKN)NOIDUNGKN
--                            FROM (
--                                    SELECT KC.DONID,decode(KC.DONVIKN,0,'CA','VKS')||' k/n' NOIDUNGKN FROM ADS_SOTHAM_KHANGNGHI KC
--                             )NDKN  GROUP BY NDKN.DONID
--               )NDKNS ON NDKNS.DONID=A.ID
--        ---------------------
--        WHERE   (V_TEN_VU_AN IS NULL  OR ( LOWER(A.TENVUVIEC) LIKE  '%'||LOWER(V_TEN_VU_AN)||'%' ) )--Tên vụ án
...
--            <tr>
--                <td colspan="9" style="line-height: 100%; font-size: 14pt; text-align: center;"><b>DANH SÁCH ÁN DÂN SỰ THỤ LÝ MỚI</b>
--                </td>
--            </tr>
--            <tr>
