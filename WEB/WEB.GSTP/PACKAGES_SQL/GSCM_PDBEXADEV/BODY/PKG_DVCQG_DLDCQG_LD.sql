--------------------------------------------------------
--  DDL for Package Body PKG_DVCQG_DLDCQG_LD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "GSCM"."PKG_DVCQG_DLDCQG_LD" AS
...
AS
    ...
    TenLoaiAn varchar2(100);
     CapXX_SO_THAM varchar2(100);
     CapXX_PHUC_THAM varchar2(100);
     LOAIAN_ID varchar2(100);
     LOAIAN varchar2(100);
     BAN_AN number;
     QUYET_DINH number;
     CHUA_DONG_BO varchar2(100);
     CapXX_SO_THAM_NUMBER number;
     CapXX_PHUC_THAM_NUMBER number;
BEGIN	
    ...
    TenLoaiAn := 'Lao động';
    CapXX_SO_THAM := 'Sơ thẩm';
    CapXX_SO_THAM_NUMBER := 2;
    CapXX_PHUC_THAM_NUMBER := 3;
    CapXX_PHUC_THAM := 'Phúc thẩm';
    LOAIAN_ID := '5';
    LOAIAN := 'ALD';
    BAN_AN := 0;
    QUYET_DINH := 1;
            ...
            SELECT  
                ...
                A.* FROM (
                    --TH1: Bản án co hieu luc 
                          ...
                          SELECT 
                                B.DONID,
                                LOAIAN_ID as LOAIAN_ID,
                                TenLoaiAn AS LOAI_AN_TEN,
                                BAN_AN AS LOAIBAQD,
                                B.id as BAQD_ID,
                                TO_CHAR(d.mavuviec)  AS MavuAn,
                                d.tenvuviec as TenVuAn, 
                                CapXX_SO_THAM as CAPXX,
                                ...
                                '' AS ghiChu,
                                 ...
                                 CHUA_DONG_BO AS TRANGTHAIBAQDNAME,
                                 ...
                                 '' as soGiayCNKH,
                                 D.QUANHEPHAPLUATID as QUANHEPHAPLUATID,
                                 '' as SO_DUONGSU_DONGBO,
                             (SELECT COUNT(*)
                             FROM ALD_DON_DUONGSU DS where DS.DONID = B.DONID AND (
                                   (
                                      (DS.LOAIDUONGSU = 1 
                            ...
                            FROM 
                                ...
                                LEFT JOIN ALD_DON_DUONGSU NDS ON NDS.DONID = B.DONID AND NDS.TUCACHTOTUNG_MA = 'NGUYENDON' AND NDS.ISDAIDIEN = 1                         
                                AND NDS.ISDAIDIEN = 1 AND (
                                  (NDS.LOAIDUONGSU = 1 
                               ...
                               OR (NDS.LOAIDUONGSU <> 1)
                                ...
                                LEFT JOIN ALD_DON_DUONGSU BDS ON BDS.DONID = B.DONID AND BDS.TUCACHTOTUNG_MA = 'BIDON' AND BDS.ISDAIDIEN = 1                        
                                AND BDS.ISDAIDIEN = 1 AND (
                                  (BDS.LOAIDUONGSU = 1 
                               ...
                               OR (BDS.LOAIDUONGSU <> 1)
                                ...
                                LEFT JOIN ALD_DON D ON D.ID = B.DONID
                                LEFT JOIN ALD_PHUCTHAM_THULY TLPT ON TLPT.DONID = D.ID
                                LEFT JOIN (SELECT KH.ID, KH.IDBAQD, KH.TAIKHOANTAO, KH.NGAYTAO, KH.DONID
                                FROM KHOBAQD KH
                                WHERE KH.CAPXX = CapXX_SO_THAM_NUMBER -- 2: sơ thẩm
                                AND KH.LINHVUC = LOAIAN AND KH.STATUS = 1) K ON B.DONID = K.DONID
                                LEFT JOIN ALD_SOTHAM_THULY TL ON TL.DONID = B.DONID
                            WHERE 
                               --Quoc tich la nguoi viet nam thi kiem tra cccd
                                 ((NDS.QUOCTICHID !=2 OR (NDS.QUOCTICHID = 2 AND LENGTH(trim(NDS.SO_CCCD))  = 12 )) OR (BDS.QUOCTICHID !=2 OR (BDS.QUOCTICHID = 2 AND LENGTH(trim(BDS.SO_CCCD))  = 12))) 
                                AND TLPT.ID IS NULL
                                -- Ngày hiệu lực của BA hoặc QĐ
                                AND B.ngayhieuluc is not null
                                -- Chua dong bo sang C06--
                                AND (v_CheckNullKHOBAQD = 0 or K.id is null)
                        ...
                        SELECT 
                            ...
                            FROM 
                               ...
                               OR (BDS.LOAIDUONGSU <> 1)
                                ...
                                LEFT JOIN (SELECT KH.ID, KH.IDBAQD, KH.TAIKHOANTAO, KH.NGAYTAO, KH.DONID
                                FROM KHOBAQD KH
                                WHERE KH.CAPXX = CapXX_PHUC_THAM_NUMBER -- 2: sơ thẩm
                                AND KH.LINHVUC = LOAIAN AND KH.STATUS = 1) K ON B.DONID = K.DONID
                                LEFT JOIN ALD_PHUCTHAM_THULY TL ON TL.DONID = B.DONID
                            WHERE 
                                (B.NGAYHIEULUC IS NOT NULL)
                                --Quoc tich la nguoi viet nam thi kiem tra cccd
                                AND ((NDS.QUOCTICHID !=2 OR (NDS.QUOCTICHID = 2 AND LENGTH(trim(NDS.SO_CCCD))  = 12 )) OR (BDS.QUOCTICHID !=2 OR (BDS.QUOCTICHID = 2 AND LENGTH(trim(BDS.SO_CCCD))  = 12))) 
                                 -- xet xu thi hieu luc ngay Ngày hiệu lực của BA hoặc QĐ 
...
--               TH3:  Quyết định sơ thẩm về ly hôn  
                  SELECT 
                                ...
                                '' AS ghiChu,
                                 ...
                                 '' as SO_DUONGSU_DONGBO,
                             (SELECT COUNT(*)
                             FROM ALD_DON_DUONGSU DS where DS.DONID = Q.DONID AND (
                                   (
                                      (DS.LOAIDUONGSU = 1 
                                    AND (DS.QUOCTICHID <> 2 OR DS.XACTHUC_DLDCQG = 1))
                                 ...
                                 '-1' as TRANGTHAIBAQD
                            FROM 
                                ALD_SOTHAM_QUYETDINH Q
                                INNER JOIN DM_TOAAN DM ON DM.ID = Q.TOAANID
                                LEFT JOIN ALD_DON_DUONGSU NDS ON NDS.DONID = q.DONID AND NDS.TUCACHTOTUNG_MA = 'NGUYENDON' AND NDS.ISDAIDIEN = 1                         
                                AND NDS.ISDAIDIEN = 1 AND (
                                  (NDS.LOAIDUONGSU = 1 
                                AND (NDS.QUOCTICHID <> 2 OR NDS.XACTHUC_DLDCQG = 1))
                               ...
                               OR (NDS.LOAIDUONGSU <> 1)
                                ...
                                left join DM_HANHCHINH h1 on h1.ID=NDS.TAMTRUID
                                LEFT JOIN ALD_DON_DUONGSU BDS ON BDS.DONID = q.DONID AND BDS.TUCACHTOTUNG_MA = 'BIDON' AND BDS.ISDAIDIEN = 1                        
                                AND BDS.ISDAIDIEN = 1 AND (
                                  (BDS.LOAIDUONGSU = 1 
                                AND (BDS.QUOCTICHID <> 2 OR BDS.XACTHUC_DLDCQG = 1))
                               ...
                               OR (BDS.LOAIDUONGSU <> 1)
                                ...
                                LEFT JOIN DM_DATAITEM QTN ON QTN.ID = NDS.QUOCTICHID AND QTN.GROUPID = 2
                                LEFT JOIN DM_DATAITEM QTB ON QTB.ID = BDS.QUOCTICHID AND QTB.GROUPID = 2
                                LEFT JOIN ALD_DON D ON D.ID = Q.DONID
                                LEFT JOIN ALD_PHUCTHAM_THULY TLPT ON TLPT.DONID = D.ID
                                LEFT JOIN (SELECT KH.ID, KH.IDBAQD, KH.TAIKHOANTAO, KH.NGAYTAO, KH.DONID
                                FROM KHOBAQD KH
                                WHERE KH.CAPXX = CapXX_SO_THAM_NUMBER -- 2: sơ thẩm
                                AND KH.LINHVUC = LOAIAN AND KH.STATUS = 1) K ON Q.DONID = K.DONID
                                 LEFT JOIN ALD_SOTHAM_THULY TL ON TL.DONID = Q.DONID
    ...
    --                          Chưa đồng bộ  
                                AND (v_CheckNullKHOBAQD = 0 or K.id is null) and lqdkt.ket_thuc = 1
                                AND TLPT.ID IS NULL
  ...
  --               TH4: lấy thông tin Quyết định Phúc thẩm
                        SELECT 
                             ...
                             FROM ALD_DON_DUONGSU DS where DS.DONID = Q.DONID AND (
                                 ...
                                 '-1' as TRANGTHAIBAQD
                            FROM 
                                 ALD_PHUCTHAM_QUYETDINH q
                                LEFT JOIN ALD_PHUCTHAM_QUYETDINH TK ON TK.QUYETDINHID = q.ID
                                INNER JOIN DM_TOAAN DM ON DM.ID = q.TOAANID
                                 LEFT JOIN ALD_DON_DUONGSU NDS ON NDS.DONID = q.DONID AND NDS.TUCACHTOTUNG_MA = 'NGUYENDON' AND NDS.ISDAIDIEN = 1                         
                                AND NDS.ISDAIDIEN = 1 AND (
                                  (NDS.LOAIDUONGSU = 1 
    ...
    --                              Đã đồng bộ  
                     ...
                     where (v_KHANGCAOQH is null OR  A.KHANGCAOQH = v_KHANGCAOQH)
                        ...
                        AND (v_Capxx is null OR (v_Capxx = 2 and  A.CAPXX = CapXX_SO_THAM) OR (v_Capxx = 3 and  A.CAPXX = CapXX_PHUC_THAM) )
                        ...
                        AND (v_thuky_id IS NULL 
                                    OR EXISTS(select 'X' from ALD_SoTham_HDXX tp where tp.CanBoID = v_thuky_id and tp.DONID=A.DONID) 
                                   OR EXISTS(select 'X' from ALD_PhucTham_HDXX tp where tp.CanBoID = v_thuky_id and tp.DONID=A.DONID)
                                    OR EXISTS(SELECT 'X' FROM ALD_DON_THAMPHAN TP WHERE  TP.THUKYID=v_thuky_id and TP.DONID=A.DONID)
                                )
                        
                        AND ( v_thamphan_id is null 
                                    OR (A.CAPXX =CapXX_SO_THAM AND  EXISTS(SELECT 'x' FROM ALD_SOTHAM_HDXX TP 
                                            WHERE TP.DONID = A.DONID AND TP.MAVAITRO='THAMPHAN' AND TP.CANBOID = v_thamphan_id ))
                                     OR (A.CAPXX =CapXX_PHUC_THAM AND  EXISTS(SELECT 'x' FROM ALD_PHUCTHAM_HDXX TP 
                                            WHERE TP.DONID = A.DONID AND TP.MAVAITRO='THAMPHAN' AND TP.CANBOID = v_thamphan_id ))        
                                            )
                )tt where tt.stt>=MinIndex and tt.stt<=MaxIndex
         ;
END SEARCH_PAGING_CHUADONGBO;
...
BEGIN	
    ...
    BAN_AN := 0;
    QUYET_DINH := 1;
    DA_DONG_BO := 'Đã đồng bộ';
    CHUA_DONG_BO := 'Đang đồng bộ';
            ...
            SELECT  ROW_NUMBER() OVER (ORDER BY A.NGAYGUI desc) STT, COUNT(*) OVER () as CountAll
                ...
                --TH1: Bản án không có kháng cao kháng nghi
                      SELECT b.id,
                            ...
                            D.TenVuViec AS TenVuAn, 
                            DECODE(B.CAPXX, 2, CapXX_SO_THAM, 3, CapXX_PHUC_THAM, '') AS CAPXX,
                            ...
                            TO_CHAR(NDS.NGAYTAO,'dd/MM/yyyy') AS NGAY_NHAN_NGUYEN_DON,
                            TO_CHAR(BDS.NGAYTAO,'dd/MM/yyyy') AS NGAY_NHAN_BI_DON,
                            -- nguyên đơn
                            NDS.ID AS NGUYENDON_ID,
                            NDS.LOAIDUONGSU as ND_LOAIDUONGSU,
                            NDS.TENDUONGSU AS HO_TEN_NGUYEN_DON,
                            NDS.SO_CCCD AS SO_GIAY_TO_NGUYEN_DON,
                            TO_CHAR(NDS.NGAYSINH,'dd/MM/yyyy') AS NGAY_SINH_NGUYEN_DON,
                            ...
                            TO_CHAR(BDS.NGAYSINH,'dd/MM/yyyy') AS NGAY_SINH_BI_DON,
                            BDS.GIOITINH  as GIOI_TINH_BI_DON,
                            BDS.SOCMND as SO_CMND_BI_DON,
                            QTB.MA AS QUOC_TICH_BI_DON,
                            QTB.ID as BD_QUOCTICHID, 
                            ...
                            (SELECT COUNT(*)
                             ...
                             WHERE DS.DONID = B.DONID and A.ID is null AND (
                                   ...
                                   OR (DS.LOAIDUONGSU <> 1 AND DS.QUOCTICHID <> 2)
                                  )
                                )) AS SO_DUONGSU_CHUADONGBO,
                             (SELECT COUNT(*)
                             FROM ALD_DON_DUONGSU DS where DS.DONID = B.DONID AND (
                        ...
                        FROM KHOBAQD B
                        LEFT JOIN ALD_DON D ON B.DONID = D.ID                        
                        LEFT JOIN ALD_DON_DUONGSU NDS ON NDS.DONID = D.ID AND NDS.TUCACHTOTUNG_MA = 'NGUYENDON' AND NDS.ISDAIDIEN = 1     
                        AND (
                                  (NDS.LOAIDUONGSU = 1 
                                AND (NDS.QUOCTICHID <> 2 OR NDS.XACTHUC_DLDCQG = 1))
                        ...
                        LEFT JOIN ALD_DON_DUONGSU BDS ON BDS.DONID = D.ID AND BDS.TUCACHTOTUNG_MA = 'BIDON' AND BDS.ISDAIDIEN = 1    
                        AND (
                                  (BDS.LOAIDUONGSU = 1 
                                AND (BDS.QUOCTICHID <> 2 OR BDS.XACTHUC_DLDCQG = 1))
