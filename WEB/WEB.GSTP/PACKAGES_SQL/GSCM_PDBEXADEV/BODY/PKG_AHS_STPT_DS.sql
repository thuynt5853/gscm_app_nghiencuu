CREATE OR REPLACE PACKAGE BODY GSCM.PKG_AHS_STPT_DS AS
...
--    VV_TOIDANH              VARCHAR(250);
--    VV_TENBICANBICAO        VARCHAR(250);
--
--BEGIN	 
--
...
--     END IF;
--
----     V_TABLE_TLST AS (SELECT DISTINCT VUANID, FIRST_VALUE(ID) OVER (PARTITION BY VUANID ORDER BY NGAYTHULY DESC, NGAYTAO DESC) AS ID
----                      FROM  AHS_SOTHAM_THULY
----                      ),
--
--     SQL_STRING_WITH := 
--     '  WITH          
--              V_TABLE_TLST AS (SELECT /*+ MATERIALIZE */ VUANID, ID
--							FROM (
--							SELECT VUANID, ID, ROW_NUMBER() OVER (PARTITION BY VUANID ORDER BY NGAYTHULY DESC, NGAYTAO DESC) AS rn
--							  FROM AHS_SOTHAM_THULY
--							)
--							WHERE rn = 1
--                      ),
--
--              V_TABLE_TLPT AS (SELECT DISTINCT VUANID, FIRST_VALUE(ID) OVER (PARTITION BY VUANID ORDER BY NGAYTHULY DESC,NGAYTAO DESC) AS ID
--                      FROM  AHS_PHUCTHAM_THULY
--                      ),  
--
--              V_TABLE_THAMPHAN_GIAIQUYET AS (SELECT TP.VUANID,TP.ID,TP.CANBOID,TP.MAVAITRO,TP.MAGIAIDOAN,TP.NGAYPHANCONG 
--                                   FROM (SELECT TP.MAVAITRO,TP.VUANID,TP.ID,TP.CANBOID, ROW_NUMBER()  OVER (PARTITION BY TP.VUANID,TP.MAVAITRO ORDER BY TP.NGAYPHANCONG DESC) ROWNUMBER,TP.NGAYPHANCONG,(CASE WHEN TP.MAVAITRO IN( ''VTTP_GIAIQUYETSOTHAM'',''VTTP_GIAIQUYETDON'') THEN 2 WHEN TP.MAVAITRO=''VTTP_GIAIQUYETPHUCTHAM'' THEN 3 END) MAGIAIDOAN
--                                          FROM AHS_THAMPHANGIAIQUYET TP) TP
--                                    WHERE TP.ROWNUMBER = 1
--                                    ),
--
--              V_TABLE_THAMPHAN_HDXX_ST AS (SELECT TP.VUANID,TP.ID,TP.CANBOID,TP.MAVAITRO,TP.MAGIAIDOAN,TP.NGAYPHANCONG 
--                                   FROM ( SELECT CAST(TP.MAVAITRO AS NVARCHAR2(20)) MAVAITRO,TP.VUANID,TP.ID,TP.CANBOID,ROW_NUMBER()  OVER (PARTITION BY TP.VUANID,TP.MAVAITRO ORDER BY TP.NGAYPHANCONG DESC) ROWNUMBER,TP.NGAYPHANCONG,2 MAGIAIDOAN
...
--                                    ),
--
--              V_TABLE_THAMPHAN_HDXX_PT AS (SELECT TP.VUANID,TP.ID,TP.CANBOID,TP.MAVAITRO,TP.MAGIAIDOAN,TP.NGAYPHANCONG 
--                                   FROM ( SELECT CAST(TP.MAVAITRO AS NVARCHAR2(20)) MAVAITRO,TP.VUANID,TP.ID,TP.CANBOID,ROW_NUMBER()  OVER (PARTITION BY TP.VUANID,TP.MAVAITRO ORDER BY TP.NGAYPHANCONG DESC) ROWNUMBER,TP.NGAYPHANCONG,3 MAGIAIDOAN
--                                          FROM  AHS_PHUCTHAM_HDXX TP
--                                          WHERE MAVAITRO IN(''THAMPHAN'',''THAMPHANHDXX'',''THAMPHANDUKHUYET'')) TP
--                                    WHERE TP.ROWNUMBER = 1 AND TP.MAVAITRO IN (''THAMPHAN'',''THAMPHANHDXX'')
--                                          OR TP.MAVAITRO = ''THAMPHANDUKHUYET''
--                                    ),
--
--              V_TABLE_BC AS (SELECT BC.ID,BC.VUANID,BC.HOTEN,C.TENTOIDANH,BC.BICANDAUVU,BC.ROWNUMBER 
--                             FROM (SELECT ID,VUANID,HOTEN,BICANDAUVU, ROW_NUMBER()  OVER (PARTITION BY VUANID ORDER BY BICANDAUVU DESC,NGAYTHAMGIA DESC) ROWNUMBER
...
--
--              V_TABLE_BC_KC AS (SELECT BC.ID,BC.VUANID,BC.HOTEN,C.TENTOIDANH,BC.BICANDAUVU,BC.ROWNUMBER 
...
--                                                    WHERE KC.NGUOIKCID=B.ID AND KC.VUANID=B.VUANID)
--                                      )BC 
--                                    LEFT JOIN (SELECT CD.BICANID,CD.TENTOIDANH,CD.ISMAIN FROM AHS_SOTHAM_CAOTRANG_DIEULUAT CD WHERE ISMAIN=1) C ON BC.ID = C.BICANID
--                                WHERE BC.ROWNUMBER <=3),
--
--              V_TABLE_TGTT_KC AS (SELECT TG.ID,TG.VUANID,TG.HOTEN,TG.TENTUCACHTGTT AS TENTOIDANH,TG.ROWNUMBER 
--                                  FROM (SELECT A.ID, A.VUANID, A.HOTEN,C.TEN TENTUCACHTGTT, ROW_NUMBER()  OVER (PARTITION BY A.VUANID ORDER BY A.HOTEN ASC) ROWNUMBER
...
--                                        WHERE EXISTS(SELECT 1 
--                                                     FROM AHS_SOTHAM_KHANGCAO KC 
--                                                     WHERE KC.NGUOIKCID=A.ID AND KC.VUANID=A.VUANID AND KC.NGUOIKCLOAI = 1)
--                                        )TG 
--                                  WHERE TG.ROWNUMBER <=3)
--    ';
--
--    SQL_STRING_SELECT := '
--         SELECT A.NGAYBANCAOTRANG,
...
--                DECODE(GD.MAGIAIDOAN,3,BC3.HOTEN || TG3.HOTEN,BC2.HOTEN) HOTENBICAN,
--                COUNT(*) OVER () AS COUNTALL,
--                ROW_NUMBER() OVER (ORDER BY A.NGAYTAO DESC) STT,
--                DECODE(GD.MAGIAIDOAN,2, TO_CHAR(A.NGAYTAO,''dd/MM/yyyy''),3, TO_CHAR(GN.NGAYTAO_PHUCTHAM,''dd/MM/yyyy''),'''') NGAY_TAO, 
--                DECODE(GD.MAGIAIDOAN,2, A.NGUOITAO,3,GN.NGUOITAO_PHUCTHAM,'''') NGUOITAO,
--                DECODE(GD.MAGIAIDOAN,2, TO_CHAR(A.NGAYTAO,''dd/MM/yyyy'')||''<br/>''||TO_CHAR(A.NGAYTAO,'' HH24:MI:SS''),3, TO_CHAR(GN.NGAYTAO_PHUCTHAM,''dd/MM/yyyy'')||''<br/>''||TO_CHAR(GN.NGAYTAO_PHUCTHAM,'' HH24:MI:SS''),'''') NGAYTAO, 
--                DECODE(GD.MAGIAIDOAN,3, ''</br><i>Tòa xét xử sơ thẩm: </i><b>''||B.TEN||''</b>'',NULL) TENTOASOTHAM, 
...
--                DECODE(GD.MAGIAIDOAN,2,BC3.HOTEN,'''') KHANGCAO_ST,
--                CASE WHEN (TLS.TINHTRANG_GQ || TLPT.TINHTRANG_GQ) IS NULL THEN ''- Chưa thụ lý''
--                     ELSE (TLS.TINHTRANG_GQ || TLPT.TINHTRANG_GQ) END  ||
--
...
--                         END
--
--                     ||QDST.TINHTRANG_GQ||QDPT.TINHTRANG_GQ
...
--    SQL_STRING_JOIN := '
--        FROM AHS_VUAN A
--            --Điều kiện để hiển thị 1 bản ghi duy nhất theo giai đoạn xét xử (tránh hiển thị 2 giai đoạn ở ST và PT ở tòa tỉnh)
--            INNER JOIN ( SELECT G.* 
...
--
--            LEFT JOIN DM_TOAAN B ON A.TOA_GIAIQUYET_ID = B.ID
--
...
--                           WHERE EXISTS(SELECT 1 FROM V_TABLE_TLST QDL WHERE QDL.ID=TL.ID)
...
--                          WHERE EXISTS(SELECT 1 FROM V_TABLE_TLPT QDL WHERE QDL.ID=TL.ID)
--                          GROUP BY TL.VUANID,''</br>- Thụ lý số:<b> ''|| TO_CHAR(TL.SOTHULY) ||''</b> ngày<b> ''||TO_CHAR(TL.NGAYTHULY,''dd/MM/yyyy'')||''</b>''
--                      )TLPT ON A.ID=TLPT.VUANID AND GD.MAGIAIDOAN=3
--
--          LEFT JOIN (SELECT TP.VUANID, ''</br>- Thẩm phán: <b>'' ||TO_CHAR(DECODE(CBHDXX.HOTEN,NULL,CB.HOTEN,CBHDXX.HOTEN)) || ''</b><i> (chủ tọa)</i>'' TINHTRANG_GQ
--                     FROM V_TABLE_THAMPHAN_GIAIQUYET TP
--                         LEFT JOIN (SELECT HDXX.CANBOID, HDXX.VUANID 
--                                    FROM V_TABLE_THAMPHAN_HDXX_ST HDXX
--                                    WHERE HDXX.MAVAITRO = ''THAMPHAN''
--                                    ) HDXX ON HDXX.VUANID = TP.VUANID 
...
--
--        LEFT JOIN (SELECT QSV.VUANID, LISTAGG(''</br>- QĐ ''|| DMQD.MAHIENTHI || '': số '' || QSV.SOQUYETDINH || '' ngày '' || TO_CHAR(QSV.NGAYQD,''dd/MM/yyyy'') , ''<br/>'') 
...
--                    GROUP BY QSV.VUANID
--                    ) QDST ON QDST.VUANID = A.ID AND GD.MAGIAIDOAN=2
--
--        LEFT JOIN (SELECT QSV.VUANID, LISTAGG(''</br>- QĐ ''|| DMQD.MAHIENTHI || '': số '' || QSV.SOQUYETDINH || '' ngày '' || TO_CHAR(QSV.NGAYQD,''dd/MM/yyyy'') , ''<br/>'') 
--                                     WITHIN GROUP (ORDER BY QSV.NGAYQD) TINHTRANG_GQ
...
--                       INNER JOIN (SELECT ID, LOAIID, MAHIENTHI FROM DM_QD_QUYETDINH WHERE KET_THUC = 1 ) DMQD ON DMQD.ID = QSV.QUYETDINHID
--                    GROUP BY QSV.VUANID
--                    ) QDPT ON QDPT.VUANID = A.ID AND GD.MAGIAIDOAN=3 
--
--
--             LEFT JOIN (
...
--                            ) CNA
--                        WHERE CNA.RN = 1
--                        AND NOT EXISTS (SELECT 1 FROM AHS_CHUYEN_NHAN_AN CN2 WHERE CN2.VUANID = CNA.MAP_VUANID_NEW AND CN2.TOANHANID = ' || V_TOAAN_ID || '   AND CN2.NGAYTAO > CNA.NGAYTAO)
...
--                        WITHIN GROUP (ORDER BY BC.ROWNUMBER)HOTEN FROM  V_TABLE_BC BC
--                        GROUP BY BC.VUANID   
--                 ) BC2 ON BC2.VUANID=A.ID      
--            ------lấy tội danh của bị cáo đầu vụ
--            LEFT JOIN( SELECT BC.VUANID,LISTAGG(BC.TENTOIDANH) 
--                        WITHIN GROUP (ORDER BY BC.ROWNUMBER)TENTOIDANH FROM  V_TABLE_BC BC WHERE BC.BICANDAUVU=1
--                        GROUP BY BC.VUANID   
--                 ) BC4 ON BC4.VUANID=A.ID 
--             ------bị cáo kháng cáo lấy cho phúc thẩm 
--            LEFT JOIN(SELECT BC.VUANID,''<br/><i>Bị cáo kháng cáo:</i> <br />''
--                        ||LISTAGG(DECODE(BC.BICANDAUVU,1,''<b>''||BC.HOTEN ||DECODE(BC.TENTOIDANH,NULL,NULL,'' - ''||BC.TENTOIDANH)||'' (đầu vụ)</b>'',BC.HOTEN||DECODE(BC.TENTOIDANH,NULL,NULL,'' - ''||BC.TENTOIDANH)),''<br/>'') 
--                        WITHIN GROUP (ORDER BY BC.ROWNUMBER) HOTEN FROM V_TABLE_BC_KC BC
--                        GROUP BY BC.VUANID 
--                 )BC3 ON BC3.VUANID=A.ID
--            ------NguoiTGTT cáo kháng cáo lấy cho phúc thẩm
...
--            ' AND ( FN_CONVERT_TO_VN(LOWER(A.TENVUAN)) LIKE ''%'|| VV_TEN_VU_AN ||'%''  
--            )'        
--            ;
--     END IF;
--
...
--     END IF;
--
--     --Tìm tội danh đã được gắn vào tên vụ án A.TENVUAN
--     IF(NVL(LENGTH(V_TOIDANH),0) > 0) THEN
--        --VV_TOIDANH := FN_CONVERT_TO_VN(LOWER(V_TOIDANH));
--        SQL_STRING_WHERE := SQL_STRING_WHERE || 
--            ' AND ( LOWER(BC4.TENTOIDANH) LIKE  ''%''|| LOWER('''|| V_TOIDANH ||''')||''%'' )'
--        ;
--     END IF;                            
--
--     --Mã vụ việc
...
--        SQL_STRING_WHERE := SQL_STRING_WHERE || 
--            ' AND ( LOWER(A.MAVUAN) LIKE '''|| LOWER(V_MA_VU_AN) ||'%'')'
--        ;
--     END IF; 
--
--     --Tên bị can/bị cáo
--     IF(NVL(LENGTH(V_BI_CAN),0) > 0) THEN
--        SQL_STRING_WHERE := SQL_STRING_WHERE || 
--            'AND (EXISTS (SELECT 1 
--                          FROM AHS_BICANBICAO BC
--                          WHERE FN_CONVERT_TO_VN(UPPER(BC.HOTEN)) LIKE ''%'' || FN_CONVERT_TO_VN(UPPER(''' || V_BI_CAN || '''))|| ''%'' AND BC.VUANID = A.ID)
--                    )'
--        ;
--     END IF;
--
...
--        SQL_STRING_WHERE := SQL_STRING_WHERE || ' 
--            AND (EXISTS(SELECT 1 FROM AHS_SOTHAM_THULY TL 
--                        WHERE TL.VUANID=A.ID AND UPPER(TL.SOTHULY) = UPPER('''|| V_SOTHULY ||''')  AND GD.MAGIAIDOAN = 2 )
--                 OR EXISTS(SELECT 1 FROM AHS_PHUCTHAM_THULY TL 
--                           WHERE TL.VUANID=A.ID AND UPPER(TL.SOTHULY) = UPPER('''|| V_SOTHULY ||''')  AND GD.MAGIAIDOAN = 3 )
--                 )'
--        ;
--     END IF;        
--
...
--        SQL_STRING_WHERE := SQL_STRING_WHERE ||
--            ' AND((GD.MAGIAIDOAN = 2 AND (A.NGAYBANCAOTRANG >= '|| VV_NGAYTHULY_TU ||') 
--                               AND (A.NGAYBANCAOTRANG <= '|| VV_NGAYTHULY_DEN ||') )
--                    OR EXISTS ( SELECT 1 
--                                FROM AHS_CHUYEN_NHAN_AN CNA 
--                                WHERE GD.MAGIAIDOAN = 3 AND (CNA.NGAYNHAN >=  '|| VV_NGAYTHULY_TU ||') 
...
--        SQL_STRING_WHERE := SQL_STRING_WHERE ||
--                '     AND ( EXISTS(SELECT 1 FROM AHS_SOTHAM_THULY TL WHERE TL.VUANID=A.ID AND GD.MAGIAIDOAN=2
--                                               AND (TL.NGAYTHULY>= '|| VV_NGAYTHULY_TU ||') 
--                                               AND (TL.NGAYTHULY<= '|| VV_NGAYTHULY_DEN ||') 
--                                           ) 
--                               OR EXISTS(SELECT 1 FROM AHS_PHUCTHAM_THULY TL WHERE TL.VUANID=A.ID AND GD.MAGIAIDOAN=3
--                                    AND (TL.NGAYTHULY>= '|| VV_NGAYTHULY_TU ||') 
--                                    AND (TL.NGAYTHULY<= '|| VV_NGAYTHULY_DEN ||') 
--                          )   )'
--        ;
...
--        SQL_STRING_WHERE := SQL_STRING_WHERE ||
--          '  AND (
--                     (( NOT EXISTS(SELECT 1 FROM AHS_SOTHAM_THULY TL WHERE TL.VUANID=A.ID AND GD.MAGIAIDOAN=2) 
--                           AND NOT EXISTS(SELECT 1 FROM AHS_PHUCTHAM_THULY TL WHERE TL.VUANID=A.ID AND GD.MAGIAIDOAN=3) 
--                        )
--                     AND (A.NGAYBANCAOTRANG>= '|| VV_NGAYTHULY_TU ||') 
--                     AND (A.NGAYBANCAOTRANG<= '|| VV_NGAYTHULY_DEN ||')      
--                    )
--                    OR
--                    (  EXISTS(SELECT 1 FROM AHS_SOTHAM_THULY TL 
--                        WHERE TL.VUANID=A.ID AND GD.MAGIAIDOAN=2
--                        AND (TL.NGAYTHULY> '|| VV_NGAYTHULY_DEN ||')
--                        ) 
--                           OR 
--                       EXISTS(SELECT 1 FROM AHS_PHUCTHAM_THULY TL 
...
--                        ' AND EXISTS(SELECT 1 
--                                     FROM V_TABLE_THAMPHAN_GIAIQUYET TP 
--                                     WHERE TP.VUANID = A.ID AND TP.CANBOID = '|| V_THAMPHAN_ID ||' 
...
--                        ' AND EXISTS (SELECT 1 
--                                      FROM V_TABLE_THAMPHAN_GIAIQUYET TP 
--                                      WHERE TP.VUANID = A.ID AND TP.MAVAITRO IN (''VTTP_GIAIQUYETSOTHAM'',''VTTP_GIAIQUYETPHUCTHAM'') 
...
--                         'AND EXISTS(SELECT 1 
--                                     FROM V_TABLE_THAMPHAN_GIAIQUYET TP 
--                                     WHERE TP.VUANID = A.ID AND TP.MAVAITRO = '''|| V_VAITRO_THAMPHAN ||''' 
--                                           AND TP.CANBOID = '|| V_THAMPHAN_ID ||'
--                                           AND TP.MAGIAIDOAN = GD.MAGIAIDOAN
--                                     )'
--                    ;
...
--        SQL_STRING_WHERE := SQL_STRING_WHERE ||
--            ' AND (    EXISTS(SELECT 1 FROM AHS_SOTHAM_BANAN QSV WHERE UPPER(QSV.SOBANAN) LIKE ''%'||V_SO_QD||'%'' AND A.ID=QSV.VUANID  )
--                       OR EXISTS(SELECT 1 FROM AHS_PHUCTHAM_BANAN QSV WHERE UPPER(QSV.SOBANAN) LIKE ''%'||V_SO_QD||'%'' AND A.ID=QSV.VUANID  )
--                       OR EXISTS(SELECT 1 FROM AHS_SOTHAM_QUYETDINH_VUAN QSV WHERE UPPER(QSV.SOQUYETDINH) LIKE ''%'||V_SO_QD||'%'' AND A.ID=QSV.VUANID  )
--                       OR EXISTS(SELECT 1 FROM AHS_PHUCTHAM_QUYETDINH_VUAN QSV WHERE UPPER(QSV.SOQUYETDINH) LIKE ''%'||V_SO_QD||'%'' AND A.ID=QSV.VUANID  )
--                       OR EXISTS(SELECT 1 FROM AHS_SOTHAM_QUYETDINH_BICAN QSV WHERE UPPER(QSV.SOQUYETDINH) LIKE ''%'||V_SO_QD||'%'' AND A.ID=QSV.VUANID  )
--                       OR EXISTS(SELECT 1 FROM AHS_PHUCTHAM_QUYETDINH_BICAN QSV WHERE UPPER(QSV.SOQUYETDINH) LIKE ''%'||V_SO_QD||'%'' AND A.ID=QSV.VUANID  )
--                     )'
--        ;
--     END IF;    
--
...
--        SQL_STRING_WHERE := SQL_STRING_WHERE ||
--            ' AND (    EXISTS(SELECT 1 FROM AHS_SOTHAM_BANAN QSV WHERE QSV.NGAYBANAN=TO_DATE('''||V_NGAY_QD ||''',''dd/MM/yyyy'') AND A.ID=QSV.VUANID  )
--                       OR EXISTS(SELECT 1 FROM AHS_PHUCTHAM_BANAN QSV WHERE QSV.NGAYBANAN=TO_DATE('''||V_NGAY_QD ||''',''dd/MM/yyyy'') AND A.ID=QSV.VUANID  )
--                       OR EXISTS(SELECT 1 FROM AHS_SOTHAM_QUYETDINH_VUAN QSV WHERE QSV.NGAYQD=TO_DATE('''||V_NGAY_QD ||''',''dd/MM/yyyy'') AND A.ID=QSV.VUANID  )
--                       OR EXISTS(SELECT 1 FROM AHS_PHUCTHAM_QUYETDINH_VUAN QSV WHERE QSV.NGAYQD=TO_DATE('''||V_NGAY_QD ||''',''dd/MM/yyyy'') AND A.ID=QSV.VUANID  )
--                       OR EXISTS(SELECT 1 FROM AHS_SOTHAM_QUYETDINH_BICAN QSV WHERE QSV.NGAYQD=TO_DATE('''||V_NGAY_QD ||''',''dd/MM/yyyy'') AND A.ID=QSV.VUANID  )
--                       OR EXISTS(SELECT 1 FROM AHS_PHUCTHAM_QUYETDINH_BICAN QSV WHERE QSV.NGAYQD=TO_DATE('''||V_NGAY_QD ||''',''dd/MM/yyyy'') AND A.ID=QSV.VUANID  )                    
--                  )'
--        ;
--     END IF;
--
...
--                        WHERE TP.VUANID=A.ID AND TP.THUKYID= '|| VCHECKTK || '  
...
--            'AND (EXISTS(SELECT 1 
--                         FROM AHS_SOTHAM_HDXX TP 
--                         WHERE TP.CANBOID = '|| V_THUKY_ID ||' AND TP.VUANID=A.ID) 
--                  OR EXISTS(SELECT 1 
--                            FROM AHS_PHUCTHAM_HDXX TP 
--                            WHERE TP.CANBOID = '|| V_THUKY_ID ||' AND TP.VUANID=A.ID)
--                  OR EXISTS(SELECT 1 
--                            FROM AHS_THAMPHANGIAIQUYET TP 
--                            WHERE TP.THUKYID = '|| V_THUKY_ID ||' AND TP.VUANID=A.ID) 
--                  )'
--        ;
--     END IF;     
--
...
--        SQL_STRING_WHERE := SQL_STRING_WHERE ||
--                            'AND EXISTS(
--                                SELECT 1 FROM AHS_PHUCTHAM_THULY PTTL 
...
--                                WHERE 
--                                --thêm mã quyết định giữ nguyên bản án sơ thẩm
...
--                           )';
--    ELSIF(V_KETQUA LIKE '2') THEN --Tăng hình phạt
--        SQL_STRING_WHERE := SQL_STRING_WHERE ||
--                   'AND ( EXISTS(
--                                SELECT 1 FROM  AHS_PHUCTHAM_THULY PTTL
...
--                             )
--                       )';
--    ELSIF(V_KETQUA LIKE '3') THEN --Giảm hình phạt
--        SQL_STRING_WHERE := SQL_STRING_WHERE ||
--                     'AND ( EXISTS (
--                                    SELECT 1 FROM AHS_PHUCTHAM_THULY PTTL
--                                    LEFT JOIN AHS_SOTHAM_BANAN STBA ON STBA.VUANID=PTTL.VUANID 
...
--                                    )
--                            )';
--    ELSIF(V_KETQUA LIKE '4') THEN --Hủy quyết định/bản án sơ thẩm để...
--        SQL_STRING_WHERE := SQL_STRING_WHERE ||
--                  'AND EXISTS(
--                                SELECT 1 FROM AHS_PHUCTHAM_THULY PTTL 
--                                LEFT JOIN AHS_PHUCTHAM_QUYETDINH_VUAN PTQDVA ON PTQDVA.VUANID=PTTL.VUANID --QUYẾT ĐỊNH 
...
--                                --thêm mã quyết định hủy bản án sơ thẩm
--                                KQPT.MA IN (''03'',''04'',''06'',''13'',''14'',''12'',''15'',''21'')
--                                AND (PTQDVA.NGAYQD>= '|| VV_TUNGAY ||')
--                                AND (PTQDVA.NGAYQD<= '|| VV_DENNGAY ||')
--                                AND PTTL.VUANID=A.ID AND GD.MAGIAIDOAN=3
--                           )';    
--    ELSIF(V_KETQUA LIKE '5') THEN --...Sửa toàn bộ bản án/QĐ sơ thẩm
--        SQL_STRING_WHERE := SQL_STRING_WHERE ||
...
--    END IF;
--
--    IF(V_QD_TAMGIAM = '1') THEN --BTG bắt tạm giam
--        SQL_STRING_WHERE := SQL_STRING_WHERE ||
--        'AND (  EXISTS( SELECT 1 
--                       FROM  AHS_VUAN VA
--                          INNER JOIN AHS_SOTHAM_THULY TL ON VA.ID=TL.VUANID 
--                          LEFT JOIN (SELECT QSV1.VUANID,DECODE(QSV1.HIEULUCDEN,NULL,SYSDATE,QSV1.HIEULUCDEN)HIEULUCDEN 
--                                     FROM ( SELECT QSV.* 
...
--                      WHERE (QSV2.HIEULUCDEN-SYSDATE) < 0 ANDBVA.ID = A.ID AND GD.MAGIAIDOAN = 2
--                    )
--               OR EXISTS( SELECT 1 
--                          FROM  AHS_VUAN VA
--                              INNER JOIN AHS_VUAN_GIAIDOAN VGD ON VA.ID=VGD.VUANID
--                              INNER JOIN AHS_PHUCTHAM_THULY TL ON VA.ID=TL.VUANID 
--                              INNER JOIN (SELECT QSV1.VUANID,DECODE(QSV1.HIEULUCDEN,NULL,SYSDATE,QSV1.HIEULUCDEN)HIEULUCDEN 
--                                          FROM ( SELECT QSV.* 
--                                                 FROM AHS_PHUCTHAM_QUYETDINH_BICAN QSV
--                                                     LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID=QSV.QUYETDINHID
--                                                     LEFT JOIN DM_QD_LOAI QDL ON QDL.ID=QD.LOAIID
--                                                 WHERE  INSTR('',BTG,'','',''||QDL.MA||'','')>0
--                                                 ORDER BY NGAYQD DESC
--                                               )QSV1 WHERE ROWNUM=1
--                                          )QSV2 ON VA.ID=QSV2.VUANID 
--                            WHERE (QSV2.HIEULUCDEN-SYSDATE)<0 ANDVA.ID=A.ID AND GD.MAGIAIDOAN=3
--                          )   
--            )';
--        ELSIF(V_QD_TAMGIAM = '2') THEN --Còn thời hạn dưới 10 ngày
--            SQL_STRING_WHERE := SQL_STRING_WHERE ||
--              'AND(  EXISTS( SELECT 1 
--                            FROM  AHS_VUAN VA
--                                INNER JOIN AHS_SOTHAM_THULY TL ON VA.ID=TL.VUANID 
...
--                                                )QSV1 WHERE ROWNUM=1
--                                          )QSV2 ON VA.ID=QSV2.VUANID 
--                            WHERE (QSV2.HIEULUCDEN-SYSDATE) < 10 AND (QSV2.HIEULUCDEN-SYSDATE) > 0 AND VA.ID = A.ID AND GD.MAGIAIDOAN = 2
--                            )
--                     OR EXISTS( SELECT 1 
--                                FROM  AHS_VUAN VA
--                                    INNER JOIN AHS_PHUCTHAM_THULY TL ON VA.ID=TL.VUANID 
...
--                                                     )QSV1 WHERE ROWNUM=1
--                                                )QSV2 ON VA.ID=QSV2.VUANID 
--                            WHERE (QSV2.HIEULUCDEN-SYSDATE) < 10  AND (QSV2.HIEULUCDEN-SYSDATE) > 0 AND VA.ID = A.ID AND GD.MAGIAIDOAN = 3
--                            )   
--                      )';
--    END IF;
--
...
--                                    )               
--                        )';
--        ELSIF(V_THOIHAN_GQ = '2') THEN
--            SQL_STRING_WHERE := SQL_STRING_WHERE ||
--                    'AND(
--                           --Sơ thẩm
--                            EXISTS(SELECT 1 FROM  AHS_VUAN VA
--                                    INNER JOIN AHS_SOTHAM_THULY TL ON VA.ID=TL.VUANID 
...
--    END IF;
--
--    IF(NVL(V_TINHTRANG_GIAIQUYET,'0') = '0') THEN
--            SQL_STRING_WHERE := SQL_STRING_WHERE ||'
--                    AND (A.NGAYTAO>=' || VV_TUNGAY || ') 
--                    AND (A.NGAYTAO<=' || VV_DENNGAY || ') ';
--        ELSIF(V_TINHTRANG_GIAIQUYET LIKE '1') THEN --Chưa giải quyết xong
--            SQL_STRING_WHERE := SQL_STRING_WHERE ||
--               ' 
--                    AND 
--                     (   
...
--                             AND
--                             (
--                                 ( EXISTS (SELECT 1 FROM AHS_SOTHAM_QUYETDINH_VUAN QDVA INNER JOIN DM_QD_QUYETDINH QD ON QD.ID = QDVA.QUYETDINHID AND QD.KET_THUC = 1 
--                                           WHERE A.ID = QDVA.VUANID AND QDVA.NGAYQD >= '|| VV_DENNGAY ||')
--                                           OR
--                                   EXISTS (SELECT 1 FROM AHS_SOTHAM_BANAN BA WHERE A.ID = BA.VUANID AND BA.NGAYBANAN >= '|| VV_DENNGAY ||')
--                                     
--                                 )
--                                 OR 
--                                 ( NOT EXISTS (SELECT 1 FROM AHS_SOTHAM_QUYETDINH_VUAN QDVA INNER JOIN DM_QD_QUYETDINH QD ON QD.ID = QDVA.QUYETDINHID AND QD.KET_THUC = 1 WHERE A.ID = QDVA.VUANID)
--                                   AND NOT EXISTS (SELECT 1 FROM AHS_SOTHAM_BANAN BA WHERE A.ID = BA.VUANID)
--                                     
--                                 )
--                             )
--                         )
...
--                             AND
--                             (
--                                 ( EXISTS (SELECT 1 FROM AHS_PHUCTHAM_QUYETDINH_VUAN QDVA INNER JOIN DM_QD_QUYETDINH QD ON QD.ID = QDVA.QUYETDINHID AND QD.KET_THUC = 1 
--                                           WHERE A.ID = QDVA.VUANID AND QDVA.NGAYQD >= '|| VV_DENNGAY ||' )
--                                           OR
--                                   EXISTS (SELECT 1 FROM AHS_PHUCTHAM_BANAN BA WHERE A.ID = BA.VUANID AND BA.NGAYBANAN >= '|| VV_DENNGAY ||')
--                                     
--                                 )
--                                 OR 
--                                 ( NOT EXISTS (SELECT 1 FROM AHS_PHUCTHAM_QUYETDINH_VUAN QDVA INNER JOIN DM_QD_QUYETDINH QD ON QD.ID = QDVA.QUYETDINHID AND QD.KET_THUC = 1 WHERE A.ID = QDVA.VUANID)
--                                   AND NOT EXISTS (SELECT 1 FROM AHS_PHUCTHAM_BANAN BA WHERE A.ID = BA.VUANID)
--                                     
--                                 )
--                             )
--                         )
--                     )'
--            ;
--        ELSIF(V_TINHTRANG_GIAIQUYET = '2') THEN --chưa phân công Thẩm phán
--            SQL_STRING_WHERE := SQL_STRING_WHERE ||'
--                AND ( EXISTS ( 
--                        SELECT 1 FROM AHS_SOTHAM_THULY STTL
--                        WHERE
--                        STTL.VUANID =  A.ID 
...
--
--                    AND NOT EXISTS (
--                                SELECT 1 FROM AHS_THAMPHANGIAIQUYET PC 
...
--                               )';
--        ELSIF(V_TINHTRANG_GIAIQUYET = '3') THEN --Đã phân công Thẩm phán
--            SQL_STRING_WHERE := SQL_STRING_WHERE ||'
--            AND  EXISTS ( SELECT 1 
...
--                                AND (PC.NGAYPHANCONG<=' || VV_DENNGAY || ')       
--                               )';
--        ELSIF(V_TINHTRANG_GIAIQUYET = '4') THEN --đã lên lịch xét xử
--            SQL_STRING_WHERE := SQL_STRING_WHERE ||'
--                                AND (
--                        EXISTS (
...
--
--                    )';
--       ELSIF(V_TINHTRANG_GIAIQUYET = '5') THEN --Đang hoãn  
--            SQL_STRING_WHERE := SQL_STRING_WHERE ||'
--                   AND (
--                     EXISTS (
--                                SELECT 1 FROM AHS_SOTHAM_QUYETDINH_VUAN QSV
...
--                      --Đang hoãn phuc tham                 
--                        OR EXISTS (
--                            SELECT 1 FROM  AHS_PHUCTHAM_QUYETDINH_VUAN PTQDVA --QUYẾT ĐỊNH 
...
--                        ) 
--                    )';
--        ELSIF(V_TINHTRANG_GIAIQUYET = '6') THEN --Đang tạm đình chỉ 
--            SQL_STRING_WHERE := SQL_STRING_WHERE ||'
...
--                        )
--                    )';
--        ELSIF(V_TINHTRANG_GIAIQUYET = '7') THEN --Đã giải quyết xong
--            SQL_STRING_WHERE := SQL_STRING_WHERE ||'
--                     AND (   
--                           EXISTS (
...
--                                    )         
--                        )';
--        ELSIF(V_TINHTRANG_GIAIQUYET = '8') THEN --Đã xét xử
--            SQL_STRING_WHERE := SQL_STRING_WHERE ||'
--                      AND ( EXISTS (
--                                    SELECT 1 FROM AHS_SOTHAM_BANAN BA
--                                    WHERE BA.SOBANAN IS NOT NULL
...
--                                        LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID = PTQDVA.QUYETDINHID
--                                        LEFT JOIN DM_QD_LOAI QDL ON QDL.ID=QD.LOAIID
--                                        WHERE (PTQDVA.NGAYQD>=' || VV_TUNGAY || ')
--                                    AND (PTQDVA.NGAYQD<=' || VV_DENNGAY || ')
--                                    AND PTQDVA.VUANID=A.ID  AND GD.MAGIAIDOAN=3
--                                    AND QD.KET_THUC =1
--                                    )        
--                            )';
--        ELSIF(V_TINHTRANG_GIAIQUYET = '9') THEN --Đình chỉ
--            SQL_STRING_WHERE := SQL_STRING_WHERE ||'
--                    AND ( EXISTS (
--                                    SELECT 1 FROM AHS_SOTHAM_QUYETDINH_VUAN QSV
--                                    LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID=QSV.QUYETDINHID
...
--                                )       
--                        )';
--        ELSIF(V_TINHTRANG_GIAIQUYET = '10') THEN --QĐ chuyển vụ án
--            SQL_STRING_WHERE := SQL_STRING_WHERE ||'
--                    AND ( EXISTS (
--                                    SELECT 1 FROM AHS_SOTHAM_QUYETDINH_VUAN QSV
--                                    LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID=QSV.QUYETDINHID
...
--                                    AND QSV.VUANID =A.ID AND GD.MAGIAIDOAN=2
--                                )
--                         OR EXISTS(SELECT 1 FROM AHS_CHUYEN_NHAN_AN CA WHERE CA.VUANID=A.ID AND CA.TOACHUYENID=' || V_TOAAN_ID || '    AND GD.MAGIAIDOAN=2)
--                         OR EXISTS (
--                                SELECT 1 FROM AHS_PHUCTHAM_QUYETDINH_VUAN PTQDVA 
--                                LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID=PTQDVA.QUYETDINHID 
--                                LEFT JOIN DM_QD_LOAI QDL ON QDL.ID=QD.LOAIID 
...
--                                )       
--                        )';
--        ELSIF(V_TINHTRANG_GIAIQUYET = '11') THEN --QĐ TRẢ HỒ SƠ
--            SQL_STRING_WHERE := SQL_STRING_WHERE ||'
--                    AND (EXISTS (SELECT 1 FROM AHS_PHUCTHAM_QUYETDINH_VUAN QD
--                                LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID=QD.QUYETDINHID 
--                                LEFT JOIN DM_QD_LOAI QDL ON QDL.ID=QD.LOAIID 
--                                WHERE QDL.MA=''TRAHS''
--                                AND (QD.NGAYQD>=' || VV_TUNGAY || ')
--                                AND (QD.NGAYQD<=' || VV_DENNGAY || ')
--                                AND QD.VUANID=A.ID AND GD.MAGIAIDOAN =3)
--                        OR EXISTS (SELECT 1 FROM AHS_SOTHAM_QUYETDINH_VUAN QD
--                                LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID=QD.QUYETDINHID 
--                                LEFT JOIN DM_QD_LOAI QDL ON QDL.ID=QD.LOAIID 
--                                WHERE QDL.MA=''TRAHS''
--                                AND (QD.NGAYQD>=' || VV_TUNGAY || ')
...
--    END IF;
--
--    IF(V_THANHNIEN = 1) THEN
--            SQL_STRING_WHERE := SQL_STRING_WHERE || '
--              AND ( EXISTS ( SELECT 1 FROM AHS_BICANBICAO BC
--                            WHERE BC.ISTREVITHANHNIEN = 1 AND BC.VUANID = A.ID)
--                       OR  
--                        EXISTS (SELECT 1 FROM AHS_NGUOITHAMGIATOTUNG NTT 
--                          WHERE NTT.ISTREVITHANHNIEN = 1 AND NTT.VUANID = A.ID)
--               )';
--        ELSIF(V_THANHNIEN = 2) THEN
--            SQL_STRING_WHERE := SQL_STRING_WHERE || '
...
--            SQL_STRING_WHERE := SQL_STRING_WHERE || '
--                AND ( A.GDTAOHS = 1)';
--        ELSIF(V_GDTAOHS = 2) THEN
--            SQL_STRING_WHERE := SQL_STRING_WHERE || '
--                AND ( A.GDTAOHS = 2)';
--        ELSIF(V_GDTAOHS = 3) THEN
--            SQL_STRING_WHERE := SQL_STRING_WHERE || '
--                AND ( A.GDTAOHS = 3)';
--        ELSIF(V_GDTAOHS = 4) THEN
--            SQL_STRING_WHERE := SQL_STRING_WHERE || '
--                AND ( A.GDTAOHS IS NULL)';
--    END IF;
--
--    --Hình thức xét xử
--    IF(V_HINHTHUCXX = 1) THEN
--            SQL_STRING_WHERE := SQL_STRING_WHERE || '
--                AND(EXISTS (SELECT 1 FROM AHS_SOTHAM_QUYETDINH_VUAN BC
--                                        WHERE BC.HINHTHUCXETXU = 1 AND BC.VUANID = A.ID
--                                        AND A.MAGIAIDOAN=2)
--                             OR
--                             EXISTS (SELECT 1 FROM AHS_PHUCTHAM_QUYETDINH_VUAN BC
--                                        WHERE BC.HINHTHUCXETXU = 1 AND BC.VUANID = A.ID
--                                        AND A.MAGIAIDOAN=3)          
--                             )';
--        ELSIF(V_HINHTHUCXX = 2) THEN
--            SQL_STRING_WHERE := SQL_STRING_WHERE || '
--                AND(EXISTS (SELECT 1 FROM AHS_SOTHAM_QUYETDINH_VUAN BC
...
--                                        AND A.MAGIAIDOAN=3)           
--                             )';
--        ELSIF(V_HINHTHUCXX = 5) THEN
--            SQL_STRING_WHERE := SQL_STRING_WHERE || '
--                AND((NOT EXISTS (SELECT 1 FROM AHS_SOTHAM_QUYETDINH_VUAN BC
--                                        WHERE BC.HINHTHUCXETXU = 1 AND BC.VUANID = A.ID)
--                                        AND
--                              NOT EXISTS(SELECT 1 FROM AHS_SOTHAM_QUYETDINH_VUAN BC
--                                        WHERE BC.HINHTHUCXETXU = 2 AND BC.VUANID = A.ID)
--                                        AND
--                              NOT EXISTS(SELECT 1 FROM AHS_SOTHAM_QUYETDINH_VUAN BC
--                                        WHERE BC.HINHTHUCXETXU = 3 AND BC.VUANID = A.ID)
...
--                             )
--                          OR
--                          (NOT EXISTS (SELECT 1 FROM AHS_PHUCTHAM_QUYETDINH_VUAN BC
--                                        WHERE BC.HINHTHUCXETXU = 1 AND BC.VUANID = A.ID)
--                                        AND
--                              NOT EXISTS(SELECT 1 FROM AHS_PHUCTHAM_QUYETDINH_VUAN BC
--                                        WHERE BC.HINHTHUCXETXU = 2 AND BC.VUANID = A.ID)
...
--        SQL_STRING_WHERE := SQL_STRING_WHERE || '
--            AND(EXISTS (SELECT 1 FROM AHS_VUAN_GIAIDOAN BC
--                                    WHERE  BC.VUANID = A.ID
--                                      AND (
...
--        SQL_STRING_WHERE := SQL_STRING_WHERE || '
--            AND(NOT EXISTS (SELECT 1 FROM AHS_VUAN_GIAIDOAN BC
--                                    WHERE BC.VUANID = A.ID
--                                      AND (
--                                        (BC.MAGIAIDOAN = 2 AND BC.TOAANID = '|| V_TOAAN_ID ||') OR
--                                        (BC.MAGIAIDOAN in (3,7) AND BC.TOAPHUCTHAMID = '|| V_TOAAN_ID ||')
...
--                                  || SQL_STRING_WHERE || ' ' 
--                                  || '
--               ) TT ' || CASE WHEN PAGE_INDEX = 0 AND PAGE_SIZE = 0 THEN ''
--                              ELSE 'WHERE TT.STT >= '|| MININDEX ||' AND TT.STT <= '|| MAXINDEX
--                               END;  
--
--END AHS_VUAN_SEARCH_TURNING;
...
--            a.TT, 
--            a.NgayBanCaoTrang,
--            DECODE(GD.MAGIAIDOAN,2, A.NGUOITAO,3,GN.NGUOITAO_PHUCTHAM,'') NGUOITAO, -- a.NguoiTao,
...
--            DECODE(GD.MAGIAIDOAN,3,GN.TruongHopGiaoNhan,Decode(A.TRUONGHOPGIAONHAN,270,'Xét xử lại cấp sơ thẩm',AA.TruongHopGiaoNhan))TruongHopGiaoNhan,
--            DECODE(GD.MAGIAIDOAN,2,BC3.hoten,'') KHANGCAO_ST--,STKC.KHANGCAO_ST
--             CASE  WHEN (TLS.TINHTRANG_GQ || TLPT.TINHTRANG_GQ) IS NULL THEN '- Chưa thụ lý' 
...
BEGIN	
    ...
    -----------------
     FOR item_hs IN (
      SELECT a.ID, 
             a.MaVuAn,
             '<i style="margin-right: 3px">Vụ án:</i><b>'||a.TenVuAn||'</b>' TenVuAn,
...
BEGIN	 
    ...
    --1 thẩm phán giải quyết và có ngày nhận phân công gần nhất làm thẩm phán chủ tọa
     ...
     -- gioi han data cua bang vu an
     SQL_STRING_VUAN := 'SELECT * FROM AHS_VUAN A WHERE (A.TOAANID = ' || V_TOAAN_ID || ' OR A.TOAPHUCTHAMID = '|| V_TOAAN_ID || ')';
     ...
     '  WITH
              ...
              V_TABLE_TLST AS (SELECT /*+ MATERIALIZE */ VUANID, ID
							FROM (
							SELECT tl.VUANID, tl.ID, ROW_NUMBER() OVER (PARTITION BY tl.VUANID ORDER BY NGAYTHULY DESC, tl.NGAYTAO DESC) AS rn
                                FROM AHS_SOTHAM_THULY tl 
                                JOIN V_TABLE_VUAN b ON tl.VUANID = b.ID 
							)
							WHERE rn = 1
                      ),

              V_TABLE_TLPT AS (SELECT DISTINCT tl.VUANID, FIRST_VALUE(tl.ID) OVER (PARTITION BY tl.VUANID ORDER BY NGAYTHULY DESC, tl.NGAYTAO DESC) AS ID
              ...
              V_TABLE_THAMPHAN_GIAIQUYET AS (SELECT TP.VUANID,TP.ID,TP.CANBOID,TP.MAVAITRO,TP.MAGIAIDOAN,TP.NGAYPHANCONG 
                                   FROM (SELECT TP.MAVAITRO,TP.VUANID,TP.ID,TP.CANBOID, ROW_NUMBER()  OVER (PARTITION BY TP.VUANID,TP.MAVAITRO ORDER BY TP.NGAYPHANCONG DESC) ROWNUMBER,TP.NGAYPHANCONG,(CASE WHEN TP.MAVAITRO IN( ''VTTP_GIAIQUYETSOTHAM'',''VTTP_GIAIQUYETDON'') THEN 2 WHEN TP.MAVAITRO=''VTTP_GIAIQUYETPHUCTHAM'' THEN 3 END) MAGIAIDOAN
                                          FROM AHS_THAMPHANGIAIQUYET TP 
                                          JOIN V_TABLE_VUAN b ON TP.VUANID = b.ID) TP
                                    WHERE TP.ROWNUMBER = 1
                                    ),
              ...
              V_TABLE_THAMPHAN_HDXX_ST AS (SELECT TP.VUANID,TP.ID,TP.CANBOID,TP.MAVAITRO,TP.MAGIAIDOAN,TP.NGAYPHANCONG 
                                   FROM ( SELECT CAST(TP.MAVAITRO AS NVARCHAR2(20)) MAVAITRO,TP.VUANID,TP.ID,TP.CANBOID,ROW_NUMBER()  OVER (PARTITION BY TP.VUANID,TP.MAVAITRO ORDER BY TP.NGAYPHANCONG DESC) ROWNUMBER,TP.NGAYPHANCONG,2 MAGIAIDOAN
                                          FROM  AHS_SOTHAM_HDXX TP 
                                          JOIN V_TABLE_VUAN b ON TP.VUANID = b.ID
                                          WHERE MAVAITRO IN (''THAMPHAN'',''THAMPHANHDXX'',''THAMPHANDUKHUYET'')) TP
                                    WHERE TP.ROWNUMBER = 1 AND TP.MAVAITRO IN (''THAMPHAN'',''THAMPHANHDXX'')
                                          OR TP.MAVAITRO = ''THAMPHANDUKHUYET''
                                    ),
              ...
              V_TABLE_BC AS (SELECT BC.ID,BC.VUANID,BC.HOTEN,C.TENTOIDANH,BC.BICANDAUVU,BC.ROWNUMBER 
                             FROM (SELECT ID,VUANID,HOTEN,BICANDAUVU, ROW_NUMBER()  OVER (PARTITION BY VUANID ORDER BY BICANDAUVU DESC,NGAYTHAMGIA DESC) ROWNUMBER
                                   FROM  AHS_BICANBICAO )BC 
                                   JOIN V_TABLE_VUAN b ON BC.VUANID = b.ID
                                   LEFT JOIN (SELECT CD.BICANID,CD.TENTOIDANH,CD.ISMAIN FROM AHS_SOTHAM_CAOTRANG_DIEULUAT CD WHERE ISMAIN=1) C ON BC.ID = C.BICANID
                             WHERE BC.ROWNUMBER <=3),
              ...
              V_TABLE_BC_KC AS (SELECT BC.ID,BC.VUANID,BC.HOTEN,C.TENTOIDANH,BC.BICANDAUVU,BC.ROWNUMBER 
                                FROM (SELECT B.ID,B.VUANID,B.HOTEN,B.BICANDAUVU, ROW_NUMBER()  OVER (PARTITION BY B.VUANID ORDER BY B.BICANDAUVU DESC,B.NGAYTHAMGIA DESC) ROWNUMBER
                                      FROM  AHS_BICANBICAO B 
                                      JOIN V_TABLE_VUAN v ON B.VUANID = v.ID
                                      WHERE EXISTS( SELECT 1 
                                                    FROM AHS_SOTHAM_KHANGCAO KC 
                                                    WHERE KC.NGUOIKCID=B.ID AND KC.VUANID=B.VUANID)
                                      )BC 
              ...
              V_TABLE_TGTT_KC AS (SELECT TG.ID,TG.VUANID,TG.HOTEN,TG.TENTUCACHTGTT AS TENTOIDANH,TG.ROWNUMBER 
                                  FROM (SELECT A.ID, A.VUANID, A.HOTEN,C.TEN TENTUCACHTGTT, ROW_NUMBER()  OVER (PARTITION BY A.VUANID ORDER BY A.HOTEN ASC) ROWNUMBER
                                        FROM AHS_NGUOITHAMGIATOTUNG A
                                            JOIN V_TABLE_VUAN b ON a.VUANID = b.ID
                                            INNER JOIN AHS_NGUOITHAMGIATOTUNG_TUCACH B ON A.ID = B.NGUOIID
                                            INNER JOIN DM_DATAITEM C ON B.TUCACHID = C.ID
                                        WHERE EXISTS(SELECT 1 
                                                     FROM AHS_SOTHAM_KHANGCAO KC 
    ...
    SQL_STRING_SELECT := '
         SELECT A.NGAYBANCAOTRANG,
                ...
                CASE WHEN (TLS.TINHTRANG_GQ || TLPT.TINHTRANG_GQ) IS NULL THEN ''- Chưa thụ lý''
                     ...
                     ||GNST.TINHTRANG_GQ
                TINHTRANG_GQ ,
                DECODE(BA.ID,NULL,DECODE(QD.ID,NULL,DECODE(STBA.ID,NULL,DECODE(STQD.ID,NULL,NULL,4),4),3),3) THULYXXLAI     
    ';
    ...
    SQL_STRING_JOIN := '
        FROM V_TABLE_VUAN A
			...
			-- lấy thông tin vụ án kết thục ở ST
			LEFT JOIN ( SELECT STBA.ID, STBA.VUANID FROM AHS_SOTHAM_BANAN STBA WHERE STBA.SOBANAN IS NOT NULL) STBA ON STBA.VUANID=A.ID AND GD.MAGIAIDOAN=2
			LEFT JOIN ( SELECT STQD.ID, STQD.VUANID FROM AHS_SOTHAM_QUYETDINH_VUAN STQD LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID = STQD.QUYETDINHID WHERE QD.KET_THUC = 1 AND QD.ISSOTHAM = 1 AND QD.ISHINHSU = 1) STQD ON STQD.VUANID=A.ID AND GD.MAGIAIDOAN=2			
        ...
        LEFT JOIN (SELECT QSV.VUANID, LISTAGG(''</br>- QĐ ''|| DMQD.MAHIENTHI || '': số '' || QSV.SOQUYETDINH || '' ngày '' || TO_CHAR(QSV.NGAYQD,''dd/MM/yyyy'') , ''<br/>'') 
           ...
           LEFT JOIN (
            ...
            -------trường hợp giao nhân dùng cho sơ thẩm với viện kiểm sát
            LEFT JOIN (SELECT A1.ID, DECODE(A1.TRUONGHOPGIAONHAN,1, ''VKS bàn giao hồ sơ sang Tòa án để xét xử sơ thẩm'',2,''Tòa án trả hồ sơ - VKS chấp nhận điều tra bổ sung'',3, ''Tòa án trả hồ sơ - VKS không chấp nhận điều tra bổ sung'', 2597,''Giám đốc thẩm hủy để xét xử lại sơ thẩm'',''''
                       ) TRUONGHOPGIAONHAN FROM V_TABLE_VUAN A1) AA ON AA.ID=A.ID
    ...
    IF(V_THOIHAN_GQ = '1') THEN
        ...
        ELSIF(V_THOIHAN_GQ = '2') THEN
            SQL_STRING_WHERE := SQL_STRING_WHERE ||
                    'AND(
                           --Sơ thẩm
                            EXISTS(SELECT 1 FROM  V_TABLE_VUAN VA
                                    INNER JOIN AHS_SOTHAM_THULY TL ON VA.ID=TL.VUANID 
                                    LEFT JOIN AHS_SOTHAM_BANAN BA ON BA.VUANID =VA.ID
                                    LEFT JOIN AHS_SOTHAM_CAOTRANG_DIEULUAT CT ON VA.ID=CT.VUANID
                                    LEFT JOIN AHS_SOTHAM_QUYETDINH_VUAN QSV ON VA.ID=QSV.VUANID 
