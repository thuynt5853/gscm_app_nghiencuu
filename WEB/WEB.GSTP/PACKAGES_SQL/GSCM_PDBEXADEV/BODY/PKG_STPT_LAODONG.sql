create or replace NONEDITIONABLE PACKAGE BODY      "PKG_STPT_LAODONG" AS
      ...
      SELECT R_QUYETDINH_EXT(TTS.DONID,TTS.ID,TTS.MA)
        ...
        FROM(SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER FROM 
                    (  SELECT d.ID,d.DONID,d.TENDUONGSU,d.TUCACHTOTUNG_MA,ROW_NUMBER()  OVER (PARTITION BY d.DONID ORDER BY d.ISDAIDIEN DESC,d.TENDUONGSU) ROWNUMBER
                       FROM ALD_DON_DUONGSU d left join ALD_ANPHI p ON p.duongsu_id = d.ID
...
--      GN.TRUONGHOPGIAONHAN,
    ...
    DECODE(A.HINHTHUCNHANDON,1,'<br/><i>TH giao nhận:</i> <b>Thụ lý mới</b>',
      ...
      INNER JOIN (SELECT G.* FROM ALD_DON_GIAIDOAN G WHERE (G.MAGIAIDOAN = 2 AND G.TOAANID = v_toaan_id) OR (G.MAGIAIDOAN = 3 AND G.TOAPHUCTHAMID = v_toaan_id)) GD ON A.ID=GD.DONID --Điều kiện để hiển thị 1 bản ghi duy nhất theo giai đoạn xét xử (tránh hiển thị 2 giai đoạn ở ST và PT ở tòa tỉnh) tuanvna
        ...
        WHERE   (V_TEN_VU_AN IS NULL  OR ( LOWER(A.TENVUVIEC) LIKE  '%'||LOWER(V_TEN_VU_AN)||'%' ) )--Tên vụ án
         ...
         -----
           AND (V_SOTHULY IS NULL 
                OR( Exists(Select 'X' from ALD_SOTHAM_THULY where donid = a.id and UPPER(SOTHULY)=UPPER(V_SOTHULY)) 
                    OR Exists(Select 'X' from ALD_SOTHAM_THULY where donid = a.id and UPPER(SOTHULY)=UPPER(V_SOTHULY)) 
                    ))--Số Thụ lý
         -----
        AND (V_THAMPHAN_ID IS NULL--Thẩm phán
             OR( EXISTS(SELECT 'x' FROM ALD_SOTHAM_HDXX PC WHERE   PC.CANBOID = V_THAMPHAN_ID AND MAVAITRO='THAMPHAN'  AND PC.DONID=A.ID))
             OR(EXISTS(SELECT 'x' FROM ALD_DON_THAMPHAN PC WHERE   PC.CANBOID = V_THAMPHAN_ID AND MAVAITRO='VTTP_GIAIQUYETSOTHAM' AND PC.DONID=A.ID) )
           )
            --GQ đơn;V_GQDON -- -- 
          AND (V_GQDON IS NULL 
...
--                  )GN ON  GN.VUANID=a.ID
    ...
    v_table.extend;
                ...
                item_ds.TINHTRANG_GQ,item_ds.CHECK_THULY,'5'
                );   
     END LOOP;
  ----------
  RETURN v_table;   
...
BEGIN
  ...
  checkXLD := 0;
  if(vID <> 0) then select count(id) into checkXLD from ALD_DON_XULY where ID = vID AND SOTHONGBAO = vSTT AND STB_PHU = vSTB_Phu; end if;
  if(vID = 0 OR checkXLD = 0) then 
    OPEN curReturn FOR   
    Select count(id)
    from (Select t.id From ALD_DON_XULY t 
    left join DON_YEUCAUBOSUNG y on y.DON_XULY_YCBS_ID = t.ID AND y.LOAIAN = 5
    ...
    Where t.LOAIAN = 5 AND t.TOAANID=vdonviID and t.SOTHONGBAO = vSTT and ((vSTB_Phu is null AND t.STB_PHU is null) OR (vSTB_Phu is not null AND t.STB_PHU = vSTB_Phu)) and (vID = 0 OR viD <> t.ID ) and EXTRACT(year FROM t.NGAYTHONGBAO) = vNam);
  else
    OPEN curReturn FOR   
    Select count(id)
    from (Select t.id From ALD_DON_XULY t 
    ...
    union all
    Select t.id From DON_YEUCAUBOSUNG t 
    Where t.LOAIAN = 5 AND t.TOAANID=vdonviID and t.SOTHONGBAO = vSTT and ((vSTB_Phu is null AND t.STB_PHU is null) OR (vSTB_Phu is not null AND t.STB_PHU = vSTB_Phu)) and t.DON_XULY_YCBS_ID <> t.ID and EXTRACT(year FROM t.NGAYTHONGBAO) = vNam);
  end if;
END ALD_FILE_CHECKSTT_ANPHI;
...
BEGIN
SELECT COUNT(*) INTO V_COUNTS FROM ALD_SOTHAM_THULY TL  WHERE TL.DONID = vDONID AND TL.NGAYTHULY < TO_DATE('01/05/2022','dd/MM/yyyy');
--IF(V_COUNTS>=1)THEN
--    OPEN curReturn FOR  
--      Select d.ID,d.TENDUONGSU
--      From ALD_DON_DUONGSU d 
--      LEFT JOIN ALD_ANPHI x ON x.DUONGSU_ID = d.ID
...
IF(V_COUNTS>=1)THEN
    OPEN curReturn FOR  
      Select d.ID,d.TENDUONGSU
      From ALD_DON_DUONGSU d 
      LEFT JOIN ALD_ANPHI x ON x.DUONGSU_IDS like '%' || d.ID || '%' -- VNPT Lê Bá Thọ check thụ lý theo DUONGSU_IDS 24/11/2025
      LEFT JOIN DON_MIENANPHI dm ON x.ID = dm.ANPHI_ID AND dm.loaian = 5
      Where d.DONID=vDONID
...
ELSE 
    OPEN curReturn FOR  
        Select d.ID,d.TENDUONGSU || ' - ' || i.TEN AS TENDUONGSU
        From ALD_DON_DUONGSU d 
        LEFT JOIN ALD_ANPHI x ON x.DUONGSU_IDS like '%' || d.ID || '%' -- VNPT Lê Bá Thọ check thụ lý theo DUONGSU_IDS 26/11/2025
        left join DM_DATAITEM i on i.MA=d.TUCACHTOTUNG_MA
        Where d.DONID=vDONID
        And d.ISDON=1 AND d.TUCACHTOTUNG_MA='NGUYENDON' AND (x.SOBIENLAI is not null OR x.TINHTRANG=1)  
        UNION 
