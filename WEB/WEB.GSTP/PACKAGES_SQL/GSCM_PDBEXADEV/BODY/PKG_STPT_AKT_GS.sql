--------------------------------------------------------
--  DDL for Package Body PKG_STPT_AKT_GS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "GSCM"."PKG_STPT_AKT_GS" AS
 ...
 PROCEDURE DON_SEARCH (
   ...
   ------------------------
     --THAMPHAN --TOANCAU-12102023-tamnc
		...
		FROM (
            ...
            FROM  AKT_KCKNQDK_PHUCTHAM_HDXX WHERE MAVAITRO IN('THAMPHAN','THAMPHANHDXX','THAMPHANDUKHUYET')) TP
   ...
   ------------------dinh nghĩa bảng lay 1 ban ghi moi nhat với mục đích chỉ chạy 1 lần.
         --AKT_KCKNQDK_PHUCTHAM_QUYETDINH
        SELECT
                ...
                FROM
                    (
                        SELECT
                            ...
                            QDL.MA
                        FROM
                            AKT_KCKNQDK_PHUCTHAM_QUYETDINH PQD
...
--      GN.TRUONGHOPGIAONHAN,
                                  ...
                                  FROM
                                      ...
                                      LEFT JOIN AKT_CHUYEN_NHAN_AN NA ON NA.MAP_VUANID_NEW = A.ID
      ...
      ----- BA Or QD----------------------------------------------
                                       LEFT JOIN (
                                          SELECT
                                              PTQDVA.*
                                          FROM
                                              AKT_KCKNQDK_PHUCTHAM_QUYETDINH PTQDVA
                                      ...
                                      LEFT JOIN (
                                          ...
                                          FROM
                                              AKT_KCKNQDK_PHUCTHAM_QUYETDINH PTQD
                                              LEFT JOIN DM_QD_QUYETDINH                QD ON QD.ID = PTQD.QUYETDINHID
                                          WHERE
                                              QD.MA = '72-DS'
                                      )                  BAPT ON BAPT.DONID = A.ID     
         --- Lay ra trang thai giai quyet don
                                      ...
                                      LEFT JOIN (
                                          ...
                                          FROM
                                              AKT_KCKNQDK_PHUCTHAM_QUYETDINH PTQDVA --QUYẾT ĐỊNH 
                                          WHERE
                                              EXISTS (
                                                  SELECT
                                                      'X'
                                      ...
                                      LEFT JOIN (
                                          ...
                                          FROM
                                                   DM_DATAITEM I
                                              INNER JOIN AKT_CHUYEN_NHAN_AN CA ON CA.TRUONGHOPGIAONHANID = I.ID
                                          WHERE
...
SELECT DSKC.DONID, '<br /><i>kháng cáo: </i> <br />'
            ...
            FROM ( SELECT DS.DONID, DS.TENDUONGSU
                   ...
                   INNER JOIN AKT_DON_DUONGSU       DS ON KC.DUONGSUID = DS.ID
                                  ...
                                  WHERE
                                      ...
                                      AND ( V_UTTP IS NULL
                                          ...
                                          SELECT
                                              'X'
                                          FROM
                                              AKT_KCKNQDK_PHUCTHAM_THULY TLPT
                                          WHERE
         ...
         -----
           ...
           --------------
                                            AND ( V_SO_QD IS NULL--Số BA/QĐ
                                                  OR ( EXISTS (
                                          SELECT 'X' FROM  AKT_SOTHAM_BANAN QSV
                                          WHERE
                                          ...
                                          WHERE
                                              ...
                                              AND NA.VUANID = QSV.DONID
                                                       ...
                                                       OR EXISTS (
                                          SELECT 'X' FROM
                                              AKT_KCKNQDK_PHUCTHAM_QUYETDINH QSV
                                          WHERE
                                              UPPER(QSV.SOQD) LIKE '%' || V_SO_QD || '%'
                                              AND A.ID = QSV.DONID
                                      ) ) )
                                          ...
                                          WHERE
                                              ...
                                              AND NA.VUANID = QSV.DONID
                                                       ...
                                                       OR EXISTS (
                                          SELECT 'X' FROM AKT_KCKNQDK_PHUCTHAM_QUYETDINH QSV
                                          WHERE
                                                  TO_CHAR(QSV.NGAYQD, 'dd/MM/yyyy') = V_NGAY_QD
                                              AND A.ID = QSV.DONID
                                      ) ) )
             ...
             AND (V_KETQUA IS NULL
                OR( V_KETQUA = 1
                    AND( EXISTS (
                        SELECT 'X' 
                        FROM AKT_KCKNQDK_PHUCTHAM_QUYETDINH   QSV 
...
--                                              'X'
--                                          FROM
--                                              AKT_KCKNQDK_PHUCTHAM_THULY PTTL
--                                          WHERE
--                                              ( NOT EXISTS (
--                                                  SELECT
--                                                      'X'
...
--                                      ) ) ) ) --là con của chưa giải quyết xong end 
 ...
 PROCEDURE AKT_PHUCTHAMQDK_THULY_GETMAXTT (
        VDONVIID  IN NUMBER,
        VFROMDATE IN DATE,
        VTODATE   IN DATE,
        CURRETURN OUT SYS_REFCURSOR
    ...
    BEGIN
        OPEN CURRETURN FOR SELECT
                           ...
                           FROM
                               ...
                               INNER JOIN AKT_DON D ON D.ID = T.DONID
                           WHERE
                                   D.TOAANID = VDONVIID
                               AND T.NGAYTHULY BETWEEN VFROMDATE AND VTODATE;

    END AKT_PHUCTHAMQDK_THULY_GETMAXTT;

--Lấy ra list TGTT tamnc 1-4
    PROCEDURE AKT_PHUCTHAM_KCKN_TGTT_GETLIST (
        VDONID    IN NUMBER,
        CURRETURN OUT SYS_REFCURSOR
    ) IS
    BEGIN
        ...
        --,h1.MA_TEN as Tamtru
                               ...
                               D.TOA_GIAIQUYET_ID
                           FROM
                               AKT_KCKNQDK_PHUCTHAM_THAMGIATOTUNG D
    ...
    END AKT_PHUCTHAM_KCKN_TGTT_GETLIST;
--Lấy ra list HDXX tamnc 1-4
    PROCEDURE AKT_KCKNQDK_PHUCTHAM_HDXX_GETLIST (
        VDONID    IN NUMBER,
        CURRETURN OUT SYS_REFCURSOR
    ) IS
        CHECKBANANPT NUMBER;
    ...
    END AKT_KCKNQDK_PHUCTHAM_HDXX_GETLIST;
     --------------------------------------------
       --Lấy ra list Quyết định tamnc 1-4
    PROCEDURE AKT_KCKNQDK_PHUCTHAM_QUYETDINH_GETLIST (
        VDONID    IN NUMBER,
        CURRETURN OUT SYS_REFCURSOR
    ) IS
    BEGIN
        OPEN CURRETURN FOR SELECT
                               ...
                               Q.TOA_GIAIQUYET_ID
                           FROM
                               AKT_KCKNQDK_PHUCTHAM_QUYETDINH Q
    ...
    END AKT_KCKNQDK_PHUCTHAM_QUYETDINH_GETLIST;

    ---------------------------------------------------

--Lấy ra list bản án tamnc 29-3
    PROCEDURE AKT_PHUCTHAMQDK_BANANQUYETDINH_GETLIST (
        VDONID    IN NUMBER,
        CURRETURN OUT SYS_REFCURSOR
    ) IS
        COUNTBANANST INT;
    ...
    END AKT_PHUCTHAMQDK_BANANQUYETDINH_GETLIST;


        ---------------------------------- Lấy ra quyết định số "" tamnc 1-4
    PROCEDURE AKT_DM_QUYETDINH_VUAN_PTQDK (
        CURRETURN OUT SYS_REFCURSOR
    ) AS
    BEGIN
        OPEN CURRETURN FOR SELECT
    ...
    END AKT_DM_QUYETDINH_VUAN_PTQDK;
  ...
  PROCEDURE KT_CHUYENDON (
    ...
    BEGIN
        IF VTRANGTHAI = 0 THEN
            ...
            FROM
                     AKT_DON D
                JOIN AKT_DON_XULY DXL ON DXL.DONID = D.ID
            WHERE
                ( 1 = (
                    CASE
                        WHEN ( VMAVUVIEC
...
----------------------
        ...
        PKG_STPT_DS_BC.FILL_KT_CHUYENAN(V_ARRAY);
        OPEN CURRETURN FOR SELECT
                               PA.V_STT,
                               PA.V_VUANID,
                               PA.V_TOAANID,
...
PROCEDURE KT_NHANDON (
    ...
    BEGIN
        ...
        FROM
            ...
            JOIN AKT_DON_XULY DXL ON DXL.DONID = A.VUANID
        WHERE
        ...
        PKG_STPT_DS_BC.FILL_KT_NHANAN(V_ARRAY);
        OPEN CURRETURN FOR SELECT
                               *
                           FROM
                               TABLE ( V_ARRAY ) A
    ...
    BEGIN
        FOR ITEMS IN (
            ...
            FROM
                ...
                LEFT JOIN AKT_SOTHAM_QUYETDINH QD ON QD.ID = T2.SOQDBA
                                                     AND T2.LOAIKHANGCAO IN ( 1, 2 ) --> Kháng cáo quyết định --toancau-anhnt kc tđc
                LEFT JOIN AKT_SOTHAM_BANAN     BA ON BA.ID = T2.SOQDBA
            ...
            FROM
                     AKT_SOTHAM_KHANGCAO T2
                INNER JOIN AKT_DON_THAMGIATOTUNG T3 ON T2.DUONGSUID = T3.ID
                                                       AND ( T3.DONID = T2.DONID )
  ...
  -- Tổng hợp kháng nghị
        FOR ITEMS IN (
            ...
            FROM
                ...
                LEFT JOIN AKT_SOTHAM_QUYETDINH QD ON QD.ID = T2.BANANID
                                                     AND T2.LOAIKN IN ( 1, 2 ) --> Kháng cáo quyết định
                LEFT JOIN AKT_SOTHAM_BANAN     BA ON BA.ID = T2.BANANID
    ...
    PROCEDURE AKT_KCKNQDK_PHUCTHAM_THULY_GETLIST (
        VDONID    IN NUMBER,
        CURRETURN OUT SYS_REFCURSOR
    ) IS
    ...
    BEGIN
        ...
        OPEN CURRETURN FOR SELECT
                              ...
                              T.NGAYTAO,
                              T.NGUOITAO,
                              T.TOA_GIAIQUYET_ID -- UPDATE toa_gq_id
                          FROM
                              AKT_KCKNQDK_PHUCTHAM_THULY T
                              LEFT JOIN AKT_FILE                   F ON F.ID = T.FILEID
                              LEFT JOIN (
                                  SELECT
                                      A.ID,
                                      A.TEN
    ...
    END AKT_KCKNQDK_PHUCTHAM_THULY_GETLIST;
    ...
    BEGIN
        BEGIN
            ...
            FOR R_DS IN ( 
                ...
                AND EXISTS (SELECT 'X' FROM AKT_SOTHAM_KHANGCAO WHERE DUONGSUID =DS.ID)
				)
			LOOP
				L_NGAYKHANGCAO :='';
				FOR R_KC IN (SELECT KC.* FROM AKT_SOTHAM_KHANGCAO   KC WHERE KC.DUONGSUID = R_DS.ID AND KC.DONID = V_DONID)
				LOOP
