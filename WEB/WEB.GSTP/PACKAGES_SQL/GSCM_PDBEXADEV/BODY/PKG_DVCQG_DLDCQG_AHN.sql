--------------------------------------------------------
--  DDL for Package Body PKG_DVCQG_DLDCQG_AHN
--------------------------------------------------------

  CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "GSCM"."PKG_DVCQG_DLDCQG_AHN" AS
            ...
            SELECT  ROW_NUMBER() OVER (ORDER BY A.NGAY_RA_BAN_AN desc) STT
                ...
                --TH1: Bản án không có kháng cao kháng nghi
                      SELECT b.id as KHOBAQDID,
                            ...
                            B.NGAYBAQD AS NGAY_RA_BAN_AN,
                            b.NGAYHIEULUCBAQD AS NGAY_HIEU_LUC_BA,
                            ...
                            NVL(D.HOAGIAI_TRANGTHAI, '') AS TRANG_THAI_TTHN,                            
                            ...
                            (SELECT COUNT(*)
                             ...
                             (SELECT COUNT(*)
                             FROM AHN_DON_DUONGSU DS where DS.DONID = B.DONID AND (
                                   (
                                      (DS.LOAIDUONGSU = 1 
                                    AND (DS.QUOCTICHID <> 2 OR DS.XACTHUC_DLDCQG = 1))
                        ...
                        FROM KHOBAQD B
                        LEFT JOIN AHN_DON D ON B.DONID = D.ID                        
                        LEFT JOIN AHN_DON_DUONGSU NDS ON NDS.DONID = D.ID AND NDS.TUCACHTOTUNG_MA = 'NGUYENDON' AND NDS.ISDAIDIEN = 1     
                        AND (
                                  (NDS.LOAIDUONGSU = 1 
                        ...
                        LEFT JOIN AHN_DON_DUONGSU BDS ON BDS.DONID = D.ID AND BDS.TUCACHTOTUNG_MA = 'BIDON' AND BDS.ISDAIDIEN = 1    
                        AND (
                                  (BDS.LOAIDUONGSU = 1 
                        ...
                        where B.LINHVUC = 'AHN' AND B.TOAANID = v_toaan_id  and
...
BEGIN
   ...
   OPEN CURRETURN FOR
    SELECT tt.* FROM(
            SELECT  
                ROW_NUMBER() OVER (ORDER BY A.LOAIBAQD desc) STT,
                COUNT(*) OVER () as CountAll,
                A.* FROM (
                    --TH1: Bản án co hieu luc
                          SELECT 
                            ...
                            AND KH.LINHVUC = 'AHN' AND KH.STATUS = 1) K ON B.DONID = K.DONID
                            LEFT JOIN AHN_SOTHAM_THULY TL ON TL.DONID = B.DONID
                    ...
                    -- TH2: lấy thông tin bản án Phúc thẩm thỏa mãn điều kiện đẩy đi               
                        SELECT 
                            ...
                            WHERE B.TK_KETQUA_CHITIET in (1,3)
                                --Quoc tich la nguoi viet nam thi kiem tra cccd
                                AND (NDS.QUOCTICHID !=2 OR (NDS.QUOCTICHID = 2 AND LENGTH(trim(NDS.SO_CCCD))  = 12 )) AND (BDS.QUOCTICHID !=2 OR (BDS.QUOCTICHID = 2 AND LENGTH(trim(BDS.SO_CCCD))  = 12))
                                -- Chua đồng bộ
               ...
               -- TH3:  Quyết định sơ thẩm về ly hôn
                  SELECT Q.DONID,
                        ...
                        '' as SO_DUONGSU_DONGBO,
                      (SELECT COUNT(*)
                             FROM AHN_DON_DUONGSU DS where DS.DONID = Q.DONID AND (
                                   (
                                      (DS.LOAIDUONGSU = 1 
                                    AND (DS.QUOCTICHID <> 2 OR DS.XACTHUC_DLDCQG = 1))
                    ...
                    LEFT JOIN (SELECT KH.ID, KH.IDBAQD, KH.TAIKHOANTAO, KH.NGAYTAO, KH.DONID
                            FROM KHOBAQD KH 
                            WHERE KH.CAPXX = 2 -- 2: sơ thẩm
                            AND KH.LINHVUC = 'AHN' AND KH.STATUS = 1) K ON Q.DONID = K.DONID
