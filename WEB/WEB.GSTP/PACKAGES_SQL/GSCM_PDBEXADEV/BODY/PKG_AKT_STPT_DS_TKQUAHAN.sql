--------------------------------------------------------
--  DDL for Package Body PKG_AKT_STPT_DS_TKQUAHAN
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "GSCM"."PKG_AKT_STPT_DS_TKQUAHAN" AS 
PROCEDURE AKT_DON_QUAHAN
( 
                     ...
                     (   GD.MAGIAIDOAN = 2 -- SƠ THẨM
                         ...
                         (
                             ( EXISTS (SELECT 1 FROM AKT_SOTHAM_QUYETDINH QDVA INNER JOIN DM_QD_QUYETDINH QD ON QD.ID = QDVA.QUYETDINHID AND QD.KET_THUC = 1 
                                       ...
                                       OR
                               EXISTS (SELECT 1 FROM AKT_SOTHAM_BANAN BA WHERE A.ID = BA.DONID)
                     ...
                     (   GD.MAGIAIDOAN = 3 -- PHÚC THẨM
                         ...
                         (
                             ( EXISTS (SELECT 1 FROM AKT_PHUCTHAM_QUYETDINH QDVA INNER JOIN DM_QD_QUYETDINH QD ON QD.ID = QDVA.QUYETDINHID AND QD.KET_THUC = 1 
                                       WHERE A.ID = QDVA.DONID)
                                       OR
                               EXISTS (SELECT 1 FROM AKT_PHUCTHAM_BANAN BA WHERE A.ID = BA.DONID)
...
PROCEDURE AKT_DON_QUAHAN_CHITIET
( 
    V_CAP_XET_XU_LOGIN      IN VARCHAR2,
    V_TOAAN_ID              IN VARCHAR2, 
...
BEGIN
    ...
    '    SELECT DISTINCT QH_CT.ID,
     ...
     '    INNER JOIN (SELECT G.* 
          ...
          INNER JOIN TK_QUAHAN QH ON QH.ID = QH_CT.TKQUAHANID AND QH.TRANGTHAI != 99 AND QH.LOAIAN = 4
...
END AKT_DON_QUAHAN_CHITIET;
END PKG_AKT_STPT_DS_TKQUAHAN;
