--------------------------------------------------------
--  DDL for Package Body PKG_DVCQG_DLDCQG_HC
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "GSCM"."PKG_DVCQG_DLDCQG_HC" AS
                ...
                A.* FROM (
                    --TH1: Bản án co hieu luc 
                          ...
                          SELECT 
                             ...
                             (SELECT COUNT(*)
                             FROM AHC_DON_DUONGSU DS where DS.DONID = B.DONID AND (
                                   (
                                      (DS.LOAIDUONGSU = 1 
                            ...
                            FROM 
                                ...
                                LEFT JOIN AHC_DON_DUONGSU NDS ON NDS.DONID = B.DONID AND NDS.TUCACHTOTUNG_MA = 'NGUYENDON' AND NDS.ISDAIDIEN = 1                         
                                 AND (
                                  (NDS.LOAIDUONGSU = 1 
                               ...
                               OR (NDS.LOAIDUONGSU <> 1)
                                ...
                                LEFT JOIN AHC_DON_DUONGSU BDS ON BDS.DONID = B.DONID AND BDS.TUCACHTOTUNG_MA = 'BIDON' AND BDS.ISDAIDIEN = 1                        
                                AND (
                                  (BDS.LOAIDUONGSU = 1 
                               ...
                               OR (BDS.LOAIDUONGSU <> 1)
                                ...
                                LEFT JOIN AHC_DON D ON D.ID = B.DONID
                                LEFT JOIN AHC_PHUCTHAM_THULY TLPT ON TLPT.DONID = D.ID
                                LEFT JOIN (SELECT KH.ID, KH.IDBAQD, KH.TAIKHOANTAO, KH.NGAYTAO, KH.DONID
                                FROM KHOBAQD KH 
                                WHERE KH.CAPXX = CapXX_SO_THAM_NUMBER
                                AND KH.LINHVUC = LOAIAN AND KH.STATUS = 1) K ON B.DONID = K.DONID
                                LEFT JOIN AHC_SOTHAM_THULY TL ON TL.DONID = B.DONID
                            WHERE 
                        ...
                        SELECT 
                            ...
                            FROM 
                               ...
                               OR (BDS.LOAIDUONGSU <> 1)
                                ...
                                LEFT JOIN (SELECT KH.ID, KH.IDBAQD, KH.TAIKHOANTAO, KH.NGAYTAO,KH.DONID
                                FROM KHOBAQD KH 
                                WHERE KH.CAPXX = CapXX_PHUC_THAM_NUMBER
                                AND KH.LINHVUC = LOAIAN AND KH.STATUS = 1) K ON B.DONID = K.DONID
                                LEFT JOIN AHC_PHUCTHAM_THULY TL ON TL.DONID = B.DONID
                            WHERE 
...
--               TH3:  Quyết định sơ thẩm về ly hôn  
                  SELECT 
                                ...
                                '' AS ghiChu,
                                 ...
                                 '' as SO_DUONGSU_DONGBO,
                             (SELECT COUNT(*)
                             FROM AHC_DON_DUONGSU DS where DS.DONID = Q.DONID AND (
                                   (
                                      (DS.LOAIDUONGSU = 1 
                                    AND (DS.QUOCTICHID <> 2 OR DS.XACTHUC_DLDCQG = 1))
                                 ...
                                 '-1' as TRANGTHAIBAQD
                            FROM 
                                AHC_SOTHAM_QUYETDINH Q
                                INNER JOIN DM_TOAAN DM ON DM.ID = Q.TOAANID
                                LEFT JOIN AHC_DON_DUONGSU NDS ON NDS.DONID = q.DONID AND NDS.TUCACHTOTUNG_MA = 'NGUYENDON' AND NDS.ISDAIDIEN = 1                         
                                AND (
                                  (NDS.LOAIDUONGSU = 1 
                                AND (NDS.QUOCTICHID <> 2 OR NDS.XACTHUC_DLDCQG = 1))
                               ...
                               OR (NDS.LOAIDUONGSU <> 1)
                                ...
                                left join DM_HANHCHINH h1 on h1.ID=NDS.TAMTRUID
                                LEFT JOIN AHC_DON_DUONGSU BDS ON BDS.DONID = q.DONID AND BDS.TUCACHTOTUNG_MA = 'BIDON' AND BDS.ISDAIDIEN = 1                        
                                AND (
                                  (BDS.LOAIDUONGSU = 1 
                                AND (BDS.QUOCTICHID <> 2 OR BDS.XACTHUC_DLDCQG = 1))
                               ...
                               OR (BDS.LOAIDUONGSU <> 1)
                                ...
                                LEFT JOIN DM_DATAITEM QTN ON QTN.ID = NDS.QUOCTICHID AND QTN.GROUPID = 2
                                LEFT JOIN DM_DATAITEM QTB ON QTB.ID = BDS.QUOCTICHID AND QTB.GROUPID = 2
                                LEFT JOIN AHC_DON D ON D.ID = Q.DONID
                                LEFT JOIN AHC_PHUCTHAM_THULY TLPT ON TLPT.DONID = D.ID
                                LEFT JOIN (SELECT KH.ID, KH.IDBAQD, KH.TAIKHOANTAO, KH.NGAYTAO,KH.DONID
                                FROM KHOBAQD KH 
                                WHERE KH.CAPXX = CapXX_SO_THAM_NUMBER
                                AND KH.LINHVUC = LOAIAN AND KH.STATUS = 1) K ON q.DONID = K.DONID
                                 LEFT JOIN AHC_SOTHAM_THULY TL ON TL.DONID = Q.DONID
    ...
    --                              Đã đồng bộ  
                     ...
                     where (v_KHANGCAOQH is null OR  A.KHANGCAOQH = v_KHANGCAOQH)
                        ...
                        AND (v_thuky_id IS NULL 
                                    OR EXISTS(select 'X' from AHC_SoTham_HDXX tp where tp.CanBoID = v_thuky_id and tp.DONID=A.DONID) 
                                   OR EXISTS(select 'X' from AHC_PhucTham_HDXX tp where tp.CanBoID = v_thuky_id and tp.DONID=A.DONID)
                                    OR EXISTS(SELECT 'X' FROM AHC_DON_THAMPHAN TP WHERE  TP.THUKYID=v_thuky_id and TP.DONID=A.DONID)
                                )
            ...
            SELECT  ROW_NUMBER() OVER (ORDER BY A.NGAYGUI desc) STT, COUNT(*) OVER () as CountAll
                ...
                --TH1: Bản án không có kháng cao kháng nghi
                      SELECT b.id as KHOBAQDID,
                        ...
                        FROM KHOBAQD B
                        LEFT JOIN AHC_DON D ON B.DONID = D.ID                        
                        LEFT JOIN AHC_DON_DUONGSU NDS ON NDS.DONID = D.ID AND NDS.TUCACHTOTUNG_MA = 'NGUYENDON' AND NDS.ISDAIDIEN = 1     
                        AND (
                                  (NDS.LOAIDUONGSU = 1 
                                AND (NDS.QUOCTICHID <> 2 OR NDS.XACTHUC_DLDCQG = 1))
                        ...
                        LEFT JOIN AHC_DON_DUONGSU BDS ON BDS.DONID = D.ID AND BDS.TUCACHTOTUNG_MA = 'BIDON' AND BDS.ISDAIDIEN = 1    
                        AND (
                                  (BDS.LOAIDUONGSU = 1 
                                AND (BDS.QUOCTICHID <> 2 OR BDS.XACTHUC_DLDCQG = 1))
