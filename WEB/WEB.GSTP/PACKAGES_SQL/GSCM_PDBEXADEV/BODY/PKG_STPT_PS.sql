--------------------------------------------------------
--  DDL for Package Body PKG_STPT_PS
--------------------------------------------------------

  CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "GSCM"."PKG_STPT_PS" AS
...
--    V_TABLE_HDXX_ST := T_QUYETDINH_EXT();
    ...
    SELECT R_QUYETDINH_EXT(TTS.DONID,TTS.ID,NULL)
		...
		SELECT R_THAMPHAN_EXT(TP.DONID,TP.ID,TP.CANBOID,TP.MAVAITRO,TP.MAGIAIDOAN,TP.NGAYTAO)
		BULK COLLECT INTO V_TABLE_THAMPHAN
		FROM (
			SELECT  MAVAITRO,DONID,ID,CANBOID, ROW_NUMBER()  OVER (PARTITION BY DONID,MAVAITRO ORDER BY NGAYTAO DESC) ROWNUMBER,NGAYTAO,
            ...
            UNION
            SELECT cast(MAVAITRO as nvarchar2(20)) MAVAITRO,DONID,ID,CANBOID,ROW_NUMBER()  OVER (PARTITION BY DONID,MAVAITRO ORDER BY NGAYTAO DESC) ROWNUMBER,NGAYTAO,2 MAGIAIDOAN
            FROM  APS_SOTHAM_HDXX WHERE MAVAITRO IN ('THAMPHAN','THAMPHANHDXX','THAMPHANDUKHUYET')
            UNION
            SELECT cast(MAVAITRO as nvarchar2(20)) MAVAITRO,DONID,ID,CANBOID,ROW_NUMBER()  OVER (PARTITION BY DONID,MAVAITRO ORDER BY NGAYTAO DESC) ROWNUMBER,NGAYTAO,3 MAGIAIDOAN
...
--                    LEFT JOIN (SELECT DONID,ID FROM TABLE(V_TABLE_THAMPHAN) )HD ON HD.DONID=TP.DONID
--                    LEFT JOIN ( SELECT GG.* FROM APS_DON_THAMPHAN GG
--                                 WHERE EXISTS (SELECT 'X' FROM TABLE(V_TABLE_THAMPHAN)TP WHERE TP.ID=GG.ID AND GG.MAVAITRO='VTTP_GIAIQUYETSOTHAM')
...
--      LEFT JOIN ( SELECT PTBA.DONID,'</br>- QĐ tuyên bố phá sản số: '||PTBA.SOBANAN||' ngày '||to_char(PTBA.NGAYTUYENAN,'dd/MM/yyyy') TINHTRANG_GQ FROM APS_PHUCTHAM_BANAN PTBA 
--            WHERE  PTBA.SOBANAN IS NOT NULL
--            GROUP BY PTBA.DONID,'</br>- QĐ tuyên bố phá sản số: '||PTBA.SOBANAN||' ngày '||to_char(PTBA.NGAYTUYENAN,'dd/MM/yyyy')
--           )BAPT ON  BAPT.DONID=d.id AND GD.MAGIAIDOAN=3
--      LEFT JOIN (
--            SELECT BA.donid,'</br>- QĐ tuyên bố phá sản số: '||BA.SOBANAN||' ngày '||to_char(BA.NGAYTUYENAN,'dd/MM/yyyy') TINHTRANG_GQ FROM APS_SOTHAM_BANAN BA
--            WHERE  BA.SOBANAN IS NOT NULL
--            GROUP BY BA.donid,'</br>- QĐ tuyên bố phá sản số: '||BA.SOBANAN||' ngày '||to_char(BA.NGAYTUYENAN,'dd/MM/yyyy')
--            )BAST ON  BAST.donid=d.id AND GD.MAGIAIDOAN=2
...
--          or UPPER(TPPC.idtp) like '%'||UPPER(vthamphan)||'%' or UPPER(TPPCPT.idtp) like '%'||UPPER(vthamphan)||'%'
    ...
    V_TABLE(V_TABLE.count) := R_STPT_6LOAIAN(
               r.ID,r.MAVUVIEC,r.TENVUVIEC,r.NGAYTAO,r.NGAY_TAO,r.NGUOITAO,
                r.MAGIAIDOAN,
                r.HOTENBICAN,r.TOASOTHAM,NULL,r.GIAIDOANVUVIEC,NULL,NULL,
                r.TINHTRANG_GQ,r.CHECK_THULY,'7'
                );     
    END LOOP;
    RETURN V_TABLE;
END DON_SEARCH_ITEM;
...
--          or UPPER(TPPC.idtp) like '%'||UPPER(vthamphan)||'%' or UPPER(TPPCPT.idtp) like '%'||UPPER(vthamphan)||'%'
    ...
    select ROW_NUMBER() OVER (ORDER BY a.NGAYNHANDON desc) STT,a.ID,a.MAVUVIEC,a.TENVUVIEC,a.SOTHUTU,a.NGAYNHANDON,a.NGUOITAO,a.NGAYTAO,a.QUANHEPL,a.MAGIAIDOAN,a.TOASOTHAM,
        a.HINHTHUCNHANDON,a.GiaiDoanVuViec,a.TenHinhThuc,a.TruongHopGiaoNhan,a.CHECK_THULY,a.THULYXXLAI,a.TINHTRANG_GQ,a.TenToaSoTham,a.BANAN_QD_ST,a.KHANGNGHI_ST,a.HoTenBiCan,A.QD_PT,(select Total from cte_Total) as CountAll
    from cte_Data a
  )
    select a.*
...
BEGIN 
  /*Select count(d.id)
    ...
    checkXLD := 0;
  if(vID <> 0) then select count(id) into checkXLD from APS_DON_XULY where ID = vID AND SOTHONGBAO = vSTT AND STB_PHU = vSTB_Phu; end if;
  if(vID = 0 OR checkXLD = 0) then 
    OPEN curReturn FOR   
    Select count(id)
    from (Select t.id From APS_DON_XULY t 
    left join DON_YEUCAUBOSUNG y on y.DON_XULY_YCBS_ID = t.ID AND y.LOAIAN = 7
    ...
    Where t.LOAIAN = 7 AND t.TOAANID=vdonviID and t.SOTHONGBAO = vSTT and ((vSTB_Phu is null AND t.STB_PHU is null) OR (vSTB_Phu is not null AND t.STB_PHU = vSTB_Phu)) and (vID = 0 OR viD <> t.ID ) and EXTRACT(year FROM t.NGAYTHONGBAO) = vNam);
  else
    OPEN curReturn FOR   
    Select count(id)
    from (Select t.id From APS_DON_XULY t 
    ...
    union all
    Select t.id From DON_YEUCAUBOSUNG t 
    Where t.LOAIAN = 7 AND t.TOAANID=vdonviID and t.SOTHONGBAO = vSTT and ((vSTB_Phu is null AND t.STB_PHU is null) OR (vSTB_Phu is not null AND t.STB_PHU = vSTB_Phu)) and t.DON_XULY_YCBS_ID <> t.ID and EXTRACT(year FROM t.NGAYTHONGBAO) = vNam);
  end if;
END APS_FILE_CHECKSTT;
...
BEGIN
SELECT COUNT(*) INTO V_COUNTS FROM APS_SOTHAM_THULY TL  WHERE TL.DONID = vDONID AND TL.NGAYTHULY < TO_DATE('01/05/2022','dd/MM/yyyy');
--IF(V_COUNTS>=1)THEN
--    OPEN curReturn FOR  
--    Select d.ID,d.TENDUONGSU
--    From APS_DON_DUONGSU d 
--    LEFT JOIN APS_ANPHI x ON x.DUONGSU_ID = d.ID
--    Where d.DONID=vDONID
--    And d.ISDON=1 AND d.TUCACHTOTUNG_MA='NGUOIYEUCAUPS' AND ((x.SOBIENLAI is not null AND x.NGAYTHONGBAO >= TO_DATE('01/05/2022','dd/MM/yyyy')) 
...
--    LEFT JOIN APS_ANPHI x ON x.DUONGSU_ID = d.ID
--    Where d.DONID=vDONID
--    And d.ISDON=1 AND d.TUCACHTOTUNG_MA='NGUOIYEUCAUPS' AND (x.SOBIENLAI is not null OR x.TINHTRANG=1)
--    ORder by d.ISDAIDIEN desc, d.TENDUONGSU;
--END IF;
IF(V_COUNTS>=1)THEN
    OPEN curReturn FOR  
    ...
    From APS_DON_DUONGSU d 
    LEFT JOIN APS_ANPHI x ON x.DUONGSU_ID = d.ID
    LEFT JOIN DON_MIENANPHI dm ON x.ID = dm.ANPHI_ID AND dm.loaian = 7
    Where d.DONID=vDONID
    And d.ISDON=1 AND d.TUCACHTOTUNG_MA='NGUOIYEUCAUPS' AND (((x.SOBIENLAI is not null OR dm.ID IS NOT NULL) AND x.NGAYTHONGBAO >= TO_DATE('01/05/2022','dd/MM/yyyy')) 
        OR x.NGAYTHONGBAO < TO_DATE('01/05/2022','dd/MM/yyyy') OR x.NGAYTHONGBAO is null)  
    ORder by d.ISDAIDIEN desc, d.TENDUONGSU;
ELSE
    OPEN curReturn FOR  
    ...
    LEFT JOIN DON_MIENANPHI dm ON x.ID = dm.ANPHI_ID AND dm.loaian = 7
    Where d.DONID=vDONID
    And d.ISDON=1 AND d.TUCACHTOTUNG_MA='NGUOIYEUCAUPS' AND (x.SOBIENLAI is not null OR dm.ID IS NOT NULL OR x.TINHTRANG=1)
    ORder by d.ISDAIDIEN desc, d.TENDUONGSU;
END IF;
...
IF(V_COUNTS>=1)THEN
    ...
    left join DM_DATAITEM i on i.MA=d.TUCACHTOTUNG_MA
    Where d.DONID=vDONID
    And d.ISDON=1 AND d.TUCACHTOTUNG_MA <> 'NGUOIYEUCAUPS'
    ORDER BY TENDUONGSU;
ELSE
    OPEN curReturn FOR  
    Select d.ID,d.TENDUONGSU || ' - ' || i.TEN AS TENDUONGSU
