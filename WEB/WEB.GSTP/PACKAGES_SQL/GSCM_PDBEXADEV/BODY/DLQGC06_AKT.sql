--------------------------------------------------------
--  DDL for Package Body DLQGC06_AKT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "GSCM"."DLQGC06_AKT" AS
                                   ...
                                   FROM
                                       (
                                           ...
                                           SELECT
                                               ...
                                               '4' AS loaian_id,
                                               'Kinh Doanh, Thương Mại' AS loai_an_ten,
                                           ...
                                           FROM
                                               akt_don              d
                                               LEFT JOIN akt_don_duongsu      ndds ON d.id = ndds.donid
                                               LEFT JOIN dm_dataitem          tctt ON tctt.ma = ndds.tucachtotung_ma
                                               INNER JOIN akt_sotham_banan     st ON st.donid = d.id
                                               ...
                                               LEFT JOIN dm_dataitem          ndqtn ON ndqtn.id = ndds.quoctichid
                                                                              AND ndqtn.groupid = 2
                                               LEFT JOIN akt_sotham_thuly     tl ON tl.donid = d.id
                                               INNER JOIN akt_don_thamphan     tp ON tp.donid = d.id
                                                                                 AND tp.mavaitro = 'VTTP_GIAIQUYETSOTHAM'
                                               LEFT JOIN dm_canbo             dc ON dc.id = tp.canboid
                                               LEFT JOIN c06_toaan_kdtm       db ON db.vuanid = d.id
                                           ...
                                           FROM
                                               ...
                                               LEFT JOIN akt_don_duongsu      ndds ON d.id = ndds.donid
                                               LEFT JOIN dm_dataitem          tctt ON tctt.ma = ndds.tucachtotung_ma
                                               INNER JOIN akt_phuctham_banan   st ON st.donid = d.id
                                               INNER JOIN dm_toaan             dm ON dm.id = st.toaanid
                                                 -- nguyên đơn
                                               LEFT JOIN dm_hanhchinh         ndh1 ON ndh1.id = ndds.tamtruid
                                               LEFT JOIN dm_dataitem          ndqtn ON ndqtn.id = ndds.quoctichid
                                               ...
                                               LEFT JOIN akt_phuctham_thuly   tl ON tl.donid = d.id
                                               INNER JOIN akt_don_thamphan     tp ON tp.donid = d.id
                                           ...
                                           FROM
                                               ...
                                               LEFT JOIN akt_don_duongsu        ndds ON d.id = ndds.donid
                                               LEFT JOIN dm_dataitem            tctt ON tctt.ma = ndds.tucachtotung_ma
                                               INNER JOIN akt_sotham_quyetdinh   qd ON qd.donid = d.id
...
-- TH4: lấy thông tin quyết định phúc thẩm thỏa mãn điều kiện đẩy đi
                                           ...
                                           FROM
                                               ...
                                               LEFT JOIN akt_don_duongsu          ndds ON d.id = ndds.donid
                                               LEFT JOIN dm_dataitem              tctt ON tctt.ma = ndds.tucachtotung_ma
                                               INNER JOIN akt_phuctham_quyetdinh   qd ON qd.donid = d.id
                                               INNER JOIN dm_toaan                 dm ON dm.id = qd.toaanid
    -- nguyên đơn
                                               LEFT JOIN dm_hanhchinh             ndh1 ON ndh1.id = ndds.tamtruid
                                               LEFT JOIN dm_dataitem              ndqtn ON ndqtn.id = ndds.quoctichid
                               ...
                               (
                                   ...
                                   FROM
                                       (
                                           ...
                                           FROM
                                               c06_toaan_kdtm       c
                                               LEFT JOIN akt_don              d ON c.vuanid = d.id
                                               LEFT JOIN akt_don_duongsu      ndds ON ndds.id = c.duongsuid
                                               LEFT JOIN akt_sotham_banan     st ON st.donid = d.id
                                               LEFT JOIN akt_phuctham_banan   pt ON pt.donid = d.id
                                               LEFT JOIN dm_toaan             dm ON dm.id = d.toaanid
                                               LEFT JOIN akt_don_thamphan     tpst ON tpst.donid = d.id
                                                                                  AND tpst.mavaitro = 'VTTP_GIAIQUYETSOTHAM'
                                               LEFT JOIN dm_canbo             dcst ON dcst.id = tpst.canboid
                                               LEFT JOIN akt_don_thamphan     tppt ON tppt.donid = d.id
                                                                                  AND tppt.mavaitro = 'VTTP_GIAIQUYETPHUCTHAM'
                                               LEFT JOIN dm_canbo             dcpt ON dcpt.id = tppt.canboid
                                           WHERE
                                               c.trangthaiakdtm = 'HIEU_LUC'
                                               and c.status = '1'
                                       ) a
                                   WHERE
                                       a.toaan = v_toaan_id
...
-- TH4: lấy thông tin quyết định phúc thẩm thỏa mãn điều kiện đẩy đi
                           ...
                           FROM
                               (
                                   ...
                                   FROM
                                       ...
                                       LEFT JOIN akt_don              d ON c.vuanid = d.id
                                       LEFT JOIN akt_don_duongsu      ndds ON d.id = ndds.donid
                                       LEFT JOIN akt_sotham_banan     st ON st.donid = c.vuanid
                                       LEFT JOIN akt_phuctham_banan   pt ON pt.donid = c.vuanid
                                   WHERE
                                       c.duongsuid = v_duongsuid
                                       AND c.loaibaqd = v_loaiba_qd
                                       AND c.capxx = v_capxx
                                       AND c.trangthaiakdtm = 'THU_HOI'
                                   ORDER BY
                                       c.id DESC
                               ) a;
    ...
    BEGIN
        INSERT INTO c06_toaan_kdtm_history (
            lydo_thuhoi,
            noidung,
            dongboid,
            nguoithuhoi,
