--------------------------------------------------------
--  DDL for Package Body DLQGC06_ADS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "GSCM"."DLQGC06_ADS" AS

    PROCEDURE ads_xacthuc_c06_update (
        vid        IN   INT,
        vxacthuc   IN   INT,
    ...
    END ads_xacthuc_c06_update;

    PROCEDURE c06_ads_search_chuadongbo (
                               ...
                               (
                                   ...
                                   FROM
                                       (
                                           ...
                                           FROM
                                               ads_don              d
                                               LEFT JOIN ads_don_duongsu      ndds ON d.id = ndds.donid
                                               LEFT JOIN dm_dataitem          tctt ON tctt.ma = ndds.tucachtotung_ma
                                               INNER JOIN ads_sotham_banan     st ON st.donid = d.id
                                               ...
                                               LEFT JOIN dm_dataitem          ndqtn ON ndqtn.id = ndds.quoctichid
                                                                              AND ndqtn.groupid = 2
                                               LEFT JOIN ads_sotham_thuly     tl ON tl.donid = d.id
                                               INNER JOIN ads_don_thamphan     tp ON tp.donid = d.id
                                                                                 AND tp.mavaitro = 'VTTP_GIAIQUYETSOTHAM'
                                               LEFT JOIN dm_canbo             dc ON dc.id = tp.canboid
                                               LEFT JOIN c06_toaan_dansu      db ON db.vuanid = d.id
                                           ...
                                           FROM
                                               ...
                                               LEFT JOIN ads_don_duongsu      ndds ON d.id = ndds.donid
                                               LEFT JOIN dm_dataitem          tctt ON tctt.ma = ndds.tucachtotung_ma
                                               INNER JOIN ads_phuctham_banan   st ON st.donid = d.id
                                               INNER JOIN dm_toaan             dm ON dm.id = st.toaanid
                                                 -- nguyên đơn
                                               LEFT JOIN dm_hanhchinh         ndh1 ON ndh1.id = ndds.tamtruid
                                               LEFT JOIN dm_dataitem          ndqtn ON ndqtn.id = ndds.quoctichid
                                               ...
                                               LEFT JOIN ads_phuctham_thuly   tl ON tl.donid = d.id
                                               INNER JOIN ads_don_thamphan     tp ON tp.donid = d.id
                                           ...
                                           FROM
                                               ...
                                               LEFT JOIN ads_don_duongsu        ndds ON d.id = ndds.donid
                                               LEFT JOIN dm_dataitem            tctt ON tctt.ma = ndds.tucachtotung_ma
                                               INNER JOIN ads_sotham_quyetdinh   qd ON qd.donid = d.id
...
-- TH4: lấy thông tin quyết định phúc thẩm thỏa mãn điều kiện đẩy đi
                                           ...
                                           FROM
                                               ...
                                               LEFT JOIN ads_don_duongsu          ndds ON d.id = ndds.donid
                                               LEFT JOIN dm_dataitem              tctt ON tctt.ma = ndds.tucachtotung_ma
                                               INNER JOIN ads_phuctham_quyetdinh   qd ON qd.donid = d.id
                                               INNER JOIN dm_toaan                 dm ON dm.id = qd.toaanid
    -- nguyên đơn
                                               LEFT JOIN dm_hanhchinh             ndh1 ON ndh1.id = ndds.tamtruid
                                               LEFT JOIN dm_dataitem              ndqtn ON ndqtn.id = ndds.quoctichid
    ...
    PROCEDURE c06_ads_search_dadongbo (
        v_loaian_id       IN    VARCHAR2,
        v_loaibaqd        IN    VARCHAR2,
        v_baqd_id         IN    VARCHAR2,
        v_khangcaoqh      IN    VARCHAR2,
...
---------------------------------------
                               ...
                               (
                                   ...
                                   FROM
                                       (
                                           ...
                                           FROM
                                               c06_toaan_dansu      c
                                               LEFT JOIN ads_don              d ON c.vuanid = d.id
                                               LEFT JOIN ads_don_duongsu      ndds ON ndds.id = c.duongsuid
                                               LEFT JOIN ads_sotham_banan     st ON st.donid = d.id
                                               LEFT JOIN ads_phuctham_banan   pt ON pt.donid = d.id
                                               LEFT JOIN dm_toaan             dm ON dm.id = d.toaanid
                                               LEFT JOIN ads_don_thamphan     tpst ON tpst.donid = d.id
                                                                                  AND tpst.mavaitro = 'VTTP_GIAIQUYETSOTHAM'
                                               LEFT JOIN dm_canbo             dcst ON dcst.id = tpst.canboid
                                               LEFT JOIN ads_don_thamphan     tppt ON tppt.donid = d.id
                                                                                  AND tppt.mavaitro = 'VTTP_GIAIQUYETPHUCTHAM'
                                               LEFT JOIN dm_canbo             dcpt ON dcpt.id = tppt.canboid
                                           WHERE
                                               c.trangthaiads = 'HIEU_LUC'
                                               and c.status = '1'
                                       ) a
                                   WHERE
                                       a.toaan = v_toaan_id
    ...
    PROCEDURE c06_ads_search_thuhoi (
        v_loaian_id       IN    VARCHAR2,
        v_loaibaqd        IN    VARCHAR2,
        v_baqd_id         IN    VARCHAR2,
        v_khangcaoqh      IN    VARCHAR2,
...
---------------------------------------
                                           ...
                                           SELECT
                                               ...
                                               CASE
                                                   ...
                                                   ELSE
                                                       to_char(pt.ngaytao, 'dd/MM/yyyy')
                                               END AS ngaytao,
                                               to_char(c.ngaythuhoi, 'dd/MM/yyyy') AS ngaygui,
                                               to_char(c.ngaydongbo, 'dd/MM/yyyy') AS ngaydongbo,
    ...
    PROCEDURE c06_ads_search_by_id (
        v_id        IN    NUMBER,
        curreturn   OUT   SYS_REFCURSOR
    ) AS
    BEGIN
    ...
    PROCEDURE c06_ads_duongsu_getbyid (
    ...
    PROCEDURE c06_ads_duongsu_cdb_getby (
        v_duongsuid     IN    NUMBER,
        v_capxx         IN    VARCHAR2,
...
-- TH4: lấy thông tin quyết định phúc thẩm thỏa mãn điều kiện đẩy đi
    ...
    PROCEDURE c06_ads_duongsu_guilai (
        v_id     IN    NUMBER,
        curreturn       OUT   SYS_REFCURSOR
    ) AS
    BEGIN
...
-- TH4: lấy thông tin quyết định phúc thẩm thỏa mãn điều kiện đẩy đi
    ...
    PROCEDURE c06_ads_duongsu_thuhoigannhat (
        v_duongsuid   IN    NUMBER,
        v_capxx       IN    VARCHAR2,
        v_loaiba_qd   IN    VARCHAR2,
        curreturn     OUT   SYS_REFCURSOR
                           ...
                           FROM
                               (
                                   ...
                                   FROM
                                       ...
                                       LEFT JOIN ads_don              d ON c.vuanid = d.id
                                       LEFT JOIN ads_don_duongsu      ndds ON d.id = ndds.donid
                                       LEFT JOIN ads_sotham_banan     st ON st.donid = c.vuanid
                                       LEFT JOIN ads_phuctham_banan   pt ON pt.donid = c.vuanid
                                   WHERE
                                       c.duongsuid = v_duongsuid
                                       AND c.loaibaqd = v_loaiba_qd
                                       AND c.capxx = v_capxx
                                       AND c.trangthaiads = 'THU_HOI'
                                   ORDER BY
                                       c.id DESC
                               ) a;
    ...
    PROCEDURE ads_don_dsduongsu_getby (
        vdonid      IN    NUMBER,
        curreturn   OUT   SYS_REFCURSOR
    ) AS
    BEGIN
    ...
    PROCEDURE c06_ads_history_by_id (
        v_id        IN    NUMBER,
        curreturn   OUT   SYS_REFCURSOR
    ) AS
    ...
    PROCEDURE ads_don_duongsu_notdaidien_doncha (
        vdonid      IN    NUMBER,
        curreturn   OUT   SYS_REFCURSOR
    ) IS
    BEGIN
