CREATE OR REPLACE PACKAGE BODY GSCM.PKG_AHN_STPT_DS AS

--PROCEDURE AHN_DON_SEARCH_TURNING
--( 
...
--
--          LEFT JOIN DVCQG_THANH_TOAN TT ON TT.ANPHI_ID=AI.ID AND TT.MALOAIVUVIEC=3
--
...
--
--        ----TOANCAU-1-11-2024
--        LEFT JOIN HOAGIAI_DON HGD ON HGD.VUVIECID = A.ID AND HGD.LOAIANID = 3                      
...
--        SQL_STRING_WHERE := SQL_STRING_WHERE ||
--            ' AND ((EXISTS(SELECT 1 FROM AHN_SOTHAM_BANAN QSV WHERE UPPER(QSV.SOBANAN) LIKE ''%'|| V_SO_QD ||'%'' AND A.ID=QSV.DONID  )
--                    OR EXISTS(SELECT 1 FROM AHN_SOTHAM_QUYETDINH QSV WHERE UPPER(QSV.SOQD) LIKE ''%'||V_SO_QD||'%'' AND A.ID=QSV.DONID  )
--                    OR EXISTS(SELECT 1 FROM AHN_PHUCTHAM_BANAN QSV WHERE UPPER(QSV.SOBANAN) LIKE ''%'||V_SO_QD||'%'' AND A.ID=QSV.DONID  )
--                    OR EXISTS(SELECT 1 FROM AHN_PHUCTHAM_QUYETDINH QSV WHERE UPPER(QSV.SOQD) LIKE ''%'||V_SO_QD||'%'' AND A.ID=QSV.DONID  )
--                   )
--                  )'
--        ;
--     END IF;    
...
--        SQL_STRING_WHERE := SQL_STRING_WHERE ||
--            ' AND ((EXISTS(SELECT 1 FROM AHN_SOTHAM_BANAN QSV WHERE TO_CHAR(QSV.NGAYMOPHIENTOA,''dd/MM/yyyy'') = '|| V_NGAY_QD ||' AND A.ID = QSV.DONID  )
--                   OR EXISTS(SELECT 1 FROM AHN_SOTHAM_QUYETDINH QSV WHERE TO_CHAR(QSV.NGAYQD,''dd/MM/yyyy'') = '|| V_NGAY_QD ||' AND A.ID = QSV.DONID  )
--                   OR EXISTS(SELECT 1 FROM AHN_PHUCTHAM_BANAN QSV WHERE TO_CHAR(QSV.NGAYMOPHIENTOA,''dd/MM/yyyy'') = '|| V_NGAY_QD ||' AND A.ID = QSV.DONID  )
--                   OR EXISTS(SELECT 1 FROM AHN_PHUCTHAM_QUYETDINH QSV WHERE TO_CHAR(QSV.NGAYQD,''dd/MM/yyyy'') = '|| V_NGAY_QD ||' AND A.ID = QSV.DONID  )
--                   )
--                  )'
--        ;
--     END IF; 
...
--                                                                                WHERE CNA.TRANGTHAI = 1 -- 1/ĐÃ NHẬN ÁN
--                                                                              )
--                                                AND ( EXISTS( SELECT 1 FROM AHN_CHUYEN_NHAN_AN CNA
...
--                                                     )
--                                                AND ( XLD.NGAYGQ_YC <= '|| VV_NGAYTHULY_DEN ||' 
--                                                                              OR EXISTS( SELECT 1 FROM AHN_DON_THAMPHAN PC 
--                                                                                         WHERE PC.MAVAITRO = ''VTTP_GIAIQUYETDON''
--                                                                                             AND XLD.NGAYGQ_YC IS NULL
--                                                                                             AND PC.NGAYPHANCONG <= '|| VV_NGAYTHULY_DEN ||'
--                                                                                             AND A.ID = PC.DONID
...
--                                       OR
--                               EXISTS (SELECT 1 FROM AHN_SOTHAM_BANAN BA WHERE A.ID = BA.DONID AND BA.NGAYTUYENAN >= '|| VV_DENNGAY ||')
--
--                             )
--                             OR 
...
--                                       WHERE A.ID = QDVA.DONID AND QDVA.NGAYQD >= '|| VV_DENNGAY ||' )
--                                       OR
--                               EXISTS (SELECT 1 FROM AHN_PHUCTHAM_BANAN BA WHERE A.ID = BA.DONID AND BA.NGAYTUYENAN >= '|| VV_DENNGAY ||')
--
--                             )
--                             OR 
...
--                           )
--                        OR EXISTS( SELECT 1 FROM AHN_PHUCTHAM_QUYETDINH PTQDVA
--                                       LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID = PTQDVA.QUYETDINHID
--                                       LEFT JOIN DM_QD_LOAI QDL ON QDL.ID = QD.LOAIID
...
--            END IF;                            
--
--            IF(V_THAMPHAN_ID NOT LIKE '') THEN
--                SQL_STRING_WHERE := SQL_STRING_WHERE || 
...
--            SQL_STRING_WHERE := SQL_STRING_WHERE || '
--                AND(EXISTS (SELECT 1 FROM AHN_DON_GIAIDOAN BC
--                                        WHERE  BC.DONID = A.ID
--                                          AND (
...
--        SQL_STRING_WHERE := SQL_STRING_WHERE || '
--            AND(NOT EXISTS (SELECT 1 FROM AHN_DON_GIAIDOAN BC
--                                    WHERE BC.DONID = A.ID
--                                      AND (
--                                        (BC.MAGIAIDOAN = 2 AND BC.TOAANID = '|| V_TOAAN_ID ||') OR
--                                        (BC.MAGIAIDOAN in (3,7) AND BC.TOAPHUCTHAMID = '|| V_TOAAN_ID ||')
...
BEGIN
    ...
    VV_TEN_VU_AN    := FN_CONVERT_TO_VN(LOWER(V_TEN_VU_AN));
    VV_QHPL         := FN_CONVERT_TO_VN(LOWER(V_QHPL));
    VV_TENDUONGSU   := FN_CONVERT_TO_VN(UPPER(V_TENDUONGSU));
    VV_MA_VU_AN     := FN_CONVERT_TO_VN(LOWER(V_MA_VU_AN));
    ...
    OPEN CURRETURN FOR
    SELECT TT.* FROM (
         WITH V_TABLE_TLST AS (SELECT TT.DONID,TT.ID 
                               FROM (SELECT DONID,FIRST_VALUE(ID) OVER (PARTITION BY DONID ORDER BY NGAYTHULY DESC,NGAYTAO DESC) ID
                                     FROM  AHN_SOTHAM_THULY ) TT GROUP BY TT.DONID,TT.ID
                               ),

              V_TABLE_TLPT AS (SELECT TT.DONID,TT.ID 
                               FROM (SELECT DONID,FIRST_VALUE(ID) OVER (PARTITION BY DONID ORDER BY NGAYTHULY DESC,NGAYTAO DESC) ID
                                     FROM  AHN_PHUCTHAM_THULY )TT GROUP BY TT.DONID,TT.ID
                               ),

              V_TABLE_THAMPHAN AS (SELECT TP.DONID,TP.ID,TP.CANBOID,TP.MAVAITRO,TP.MAGIAIDOAN,TP.NGAYPHANCONG 
                ...
                V_TABLE_ST AS (SELECT TT.DONID,TT.ID,TT.MA 
                               FROM (SELECT PQD.DONID,FIRST_VALUE(PQD.ID) OVER (PARTITION BY PQD.DONID,QDL.MA ORDER BY PQD.NGAYQD DESC,PQD.NGAYTAO DESC) ID, QDL.MA
                                     FROM  AHN_SOTHAM_QUYETDINH PQD
                                         LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID=PQD.QUYETDINHID 
                                         LEFT JOIN DM_QD_LOAI QDL ON QDL.ID=QD.LOAIID 
                                     )TT GROUP BY TT.DONID,TT.ID,TT.MA),

                V_TABLE_PT AS (SELECT TT.DONID,TT.ID,TT.MA 
                               FROM (SELECT PQD.DONID,FIRST_VALUE(PQD.ID) OVER (PARTITION BY PQD.DONID,QDL.MA ORDER BY PQD.NGAYQD DESC,PQD.NGAYTAO DESC) ID, QDL.MA
                ...
                V_TABLE_BC_KC AS (SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER 
                                  FROM (SELECT DS.ID,DS.DONID,DS.TENDUONGSU,DS.TUCACHTOTUNG_MA,ROW_NUMBER() OVER (PARTITION BY DS.DONID ORDER BY DS.ISDAIDIEN DESC,DS.TENDUONGSU) ROWNUMBER
                                        FROM AHN_DON_DUONGSU DS
                                        WHERE EXISTS(SELECT 'X' FROM AHN_SOTHAM_KHANGCAO KC WHERE KC.DUONGSUID=DS.ID AND KC.LOAIKHANGCAO !=2 AND KC.DONID=DS.DONID))BC 
                                  WHERE BC.ROWNUMBER <=3)
         ...
         SELECT ROW_NUMBER() OVER (ORDER BY A.NGAYTAO DESC) STT,
                ...
                A.HINHTHUCNHANDON,
                DECODE(A.HINHTHUCNHANDON,1,'<br/><i>TH giao nhận:</i> <b>Thụ lý mới</b>',270, '<br/><i>TH giao nhận:</i> <b>Xét xử lại cấp sơ thẩm</b>','<br/><i>TH giao nhận:</i> <b>'|| GN.TRUONGHOPGIAONHAN||'</b>') TRUONGHOPGIAONHAN,
                STBA.BANAN_QD_ST,
                '' AS QD_PT,
                STKN.KHANGNGHI_ST,
                PKG_STPT_AHN_GS.NOIDUNG_KHANGCAO_DANHSACH(A.ID) AS KHANGCAO_ST,--toancau-anhnt thêm trường QD_PT mặc định trống
                A.MAGIAIDOAN,
                (BC3.HOTEN||BC2.HOTEN) HOTENBICAN,
                DECODE(XLD.LOAIGIAIQUYET,1,'- Đã chuyển đơn',
                                                             CASE WHEN (TLS.TINHTRANG_GQ || TLPT.TINHTRANG_GQ) IS NULL 
                                                                  THEN '- Chưa thụ lý' 
                                                                  ELSE (TLS.TINHTRANG_GQ || TLPT.TINHTRANG_GQ) 
                                                             END  
                                                           ||CASE WHEN (TPPC.TINHTRANG_GQ || TPPCPT.TINHTRANG_GQ) IS NULL AND (TLS.TINHTRANG_GQ || TLPT.TINHTRANG_GQ) IS NOT NULL  
                                                                  THEN '</br>- Chưa phân công Thẩm phán' 
                                                           ...
                                                           ||GNST.TINHTRANG_GQ
                                                           --lanh thêm thông tin giải quyết của vụ án cha
                                                           ||CASE WHEN (A.VUANGOCID > 0 AND A.IS_TACHAN IS NULL) 
                                                                  THEN (SELECT '</br>- Đã nhập vụ án (Vụ án nhập: Thụ lý số:<b> '|| TO_CHAR(T.SOTHULY) ||'</b> ngày<b> '||TO_CHAR(T.NGAYTHULY,'dd/MM/yyyy')
                                                                                FROM AHN_DON D
                                                                                LEFT JOIN AHN_SOTHAM_THULY T ON D.ID = T.DONID
                                                                                WHERE D.ID = A.VUANGOCID)
                                                                  WHEN (A.VUANGOCID > 0 AND A.IS_TACHAN = 1) 
                                                                  THEN (SELECT '</br>- Đã tách vụ án (Vụ án tách: Thụ lý số:<b> '|| TO_CHAR(T.SOTHULY) ||'</b> ngày<b> '||TO_CHAR(T.NGAYTHULY,'dd/MM/yyyy')
                                                                            FROM AHN_DON D
                                                                            LEFT JOIN AHN_SOTHAM_THULY T ON D.ID = T.DONID
                                                                            WHERE D.ID = A.VUANGOCID)
      ...
      FROM AHN_DON A
          ...
          ------Trạng thái giải quyết trong danh sách
          LEFT JOIN (SELECT T2.DONID,T2.TOAANID,T2.NGAYTHULY,T2.SOTHULY,T2.TRUONGHOPTHULY,T2.QHPLTKID ,'</br>- Thụ lý số:<b> '|| TO_CHAR(T2.SOTHULY) ||'</b> ngày<b> '||TO_CHAR(T2.NGAYTHULY,'dd/MM/yyyy')||'</b>' TINHTRANG_GQ
          ...
          LEFT JOIN (SELECT T2.DONID,T2.NGAYTHULY,T2.SOTHULY,T2.TRUONGHOPTHULY,'</br>- Thụ lý số:<b> '|| TO_CHAR(T2.SOTHULY) ||'</b> ngày<b> '||TO_CHAR(T2.NGAYTHULY,'dd/MM/yyyy')||'</b>' TINHTRANG_GQ
          ...
          LEFT JOIN (SELECT TP.DONID,'</br>- Thẩm phán: <b>' ||TO_CHAR(DECODE(CBHDXX.HOTEN,NULL,CB.HOTEN,CBHDXX.HOTEN))||'</b><i> (chủ tọa)</i>' TINHTRANG_GQ
                     FROM (SELECT * FROM V_TABLE_THAMPHAN) TP
                         LEFT JOIN (SELECT * 
                                    FROM V_TABLE_THAMPHAN
                                    ) HDXX ON HDXX.DONID = TP.DONID AND HDXX.MAVAITRO = 'THAMPHAN' AND HDXX.MAGIAIDOAN = 2
					     LEFT JOIN DM_CANBO CB ON CB.ID=TP.CANBOID
                         LEFT JOIN DM_CANBO CBHDXX ON CBHDXX.ID=HDXX.CANBOID  
          ...
          LEFT JOIN (SELECT TP.DONID,'</br>- Thẩm phán: <b>' ||TO_CHAR(DECODE(CBHDXX.HOTEN,NULL,CB.HOTEN,CBHDXX.HOTEN))||'</b><i> (chủ tọa)</i>' TINHTRANG_GQ
                     FROM (SELECT * FROM V_TABLE_THAMPHAN) TP
                        LEFT JOIN (SELECT * 
                                   FROM V_TABLE_THAMPHAN
                                   ) HDXX ON HDXX.DONID = TP.DONID AND HDXX.MAVAITRO = 'THAMPHAN' AND HDXX.MAGIAIDOAN = 3
                        LEFT JOIN DM_CANBO CB ON CB.ID=TP.CANBOID
                        LEFT JOIN DM_CANBO CBHDXX ON CBHDXX.ID=HDXX.CANBOID  
          ...
          LEFT JOIN (SELECT QSV.DONID,'</br>- QĐ HPT số: '|| QSV.SOQD ||' ngày '||TO_CHAR(QSV.NGAYQD,'dd/MM/yyyy') TINHTRANG_GQ
                     ...
                     WHERE EXISTS (SELECT 'X' 
                                   FROM V_TABLE_ST QDL 
                                   WHERE QDL.ID=QSV.ID AND INSTR(',HPT,',','||QDL.MA||',')>0 ) 
          ...
          LEFT JOIN (SELECT QSV.DONID,'</br>- QĐ TĐC số: '|| QSV.SOQD ||' ngày '||TO_CHAR(QSV.NGAYQD,'dd/MM/yyyy') TINHTRANG_GQ
                     FROM AHN_SOTHAM_QUYETDINH QSV
                     WHERE EXISTS (SELECT 'X' 
                                   FROM V_TABLE_ST QDL 
                                   WHERE QDL.ID=QSV.ID AND INSTR(',TDC,',','||QDL.MA||',')>0 ) 
          ...
          LEFT JOIN (SELECT QSV.DONID,'</br>- QĐ ĐC số: '|| QSV.SOQD ||' ngày '||TO_CHAR(QSV.NGAYQD,'dd/MM/yyyy') TINHTRANG_GQ 
                   FROM AHN_SOTHAM_QUYETDINH QSV 
                   --chi lay 1 gia tri dau tien moi nhat dung
                   WHERE EXISTS (SELECT 'X' 
                                 FROM V_TABLE_ST QDL 
                                 WHERE QDL.ID=QSV.ID AND INSTR(',DC,',','||QDL.MA||',')>0  ) 
                   GROUP BY QSV.DONID,'</br>- QĐ ĐC số: '|| QSV.SOQD ||' ngày '||TO_CHAR(QSV.NGAYQD,'dd/MM/yyyy')
                   )DCST ON  DCST.DONID=A.ID AND GD.MAGIAIDOAN=2

          LEFT JOIN (SELECT PTQDVA.DONID,'</br>- QĐ ĐC số: '|| PTQDVA.SOQD ||' ngày '||TO_CHAR(PTQDVA.NGAYQD,'dd/MM/yyyy') TINHTRANG_GQ 
                     FROM AHN_PHUCTHAM_QUYETDINH PTQDVA 
                     WHERE EXISTS (SELECT 'X' FROM V_TABLE_PT QDL WHERE QDL.ID=PTQDVA.ID  AND INSTR(',DC,',','||QDL.MA||',')>0  )
        ...
        LEFT JOIN (SELECT QSV.DONID,'</br>- QĐ CNTT số: '|| QSV.SOQD ||' ngày '||TO_CHAR(QSV.NGAYQD,'dd/MM/yyyy') TINHTRANG_GQ
                   FROM AHN_SOTHAM_QUYETDINH QSV
                   WHERE EXISTS (SELECT 'X' 
                                 FROM V_TABLE_ST QDL 
                                 WHERE QDL.ID=QSV.ID AND INSTR(',CNTT,',','||QDL.MA||',')>0 ) 
                   GROUP BY QSV.DONID,'</br>- QĐ CNTT số: '|| QSV.SOQD ||' ngày '||TO_CHAR(QSV.NGAYQD,'dd/MM/yyyy')
                   )CNTT ON  CNTT.DONID=A.ID AND GD.MAGIAIDOAN=2

        LEFT JOIN (SELECT QSV.DONID,'</br>- QĐ CVA số: '|| QSV.SOQD ||' ngày '||TO_CHAR(QSV.NGAYQD,'dd/MM/yyyy') TINHTRANG_GQ 
                   FROM AHN_SOTHAM_QUYETDINH QSV 
                   WHERE EXISTS (SELECT 'X' 
                                 FROM V_TABLE_ST QDL 
                                 WHERE QDL.ID=QSV.ID  AND INSTR(',CVA,',','||QDL.MA||',')>0  )
                   GROUP BY QSV.DONID,'</br>- QĐ CVA số: '|| QSV.SOQD ||' ngày '||TO_CHAR(QSV.NGAYQD,'dd/MM/yyyy')
                   )CST ON  CST.DONID=A.ID AND GD.MAGIAIDOAN=2   

        LEFT JOIN (SELECT PTQDVA.DONID,'</br>- QĐ CVA số: '|| PTQDVA.SOQD ||' ngày '||TO_CHAR(PTQDVA.NGAYQD,'dd/MM/yyyy') TINHTRANG_GQ
                   FROM AHN_PHUCTHAM_QUYETDINH PTQDVA 
                   WHERE EXISTS (SELECT 'X' 
                                 FROM V_TABLE_PT QDL 
         ...
         --trường hợp giao nhận add vào cột trạng thái     --sửa check đã chuyển lại án sơ thẩm     
         LEFT JOIN (SELECT CNA.ID, CNA.VUANID, CNA.TOACHUYENID, CNA.TINHTRANG_GQ
                    FROM(SELECT CA.ID, CA.VUANID, CA.TOACHUYENID, '</br>- ' || I.TEN || '</br>- Đã chuyển vụ án' TINHTRANG_GQ,
                                ROW_NUMBER() OVER(PARTITION BY CA.VUANID, CA.TOACHUYENID ORDER BY CA.ID DESC ) RN
                            FROM AHN_CHUYEN_NHAN_AN CA
                                INNER JOIN DM_DATAITEM I ON CA.TRUONGHOPGIAONHANID = I.ID
                            WHERE CA.TOACHUYENID = V_TOAAN_ID) CNA
                    WHERE CNA.RN = 1 AND NOT EXISTS (SELECT 'X' 
                                                     FROM AHN_CHUYEN_NHAN_AN CN1
                                                         JOIN AHN_CHUYEN_NHAN_AN CN2 ON CN2.VUANID = CN1.MAP_VUANID_NEW
                                                     WHERE CN1.VUANID = CNA.VUANID AND CN2.TOANHANID = V_TOAAN_ID AND CN2.ID > CNA.ID )
                    )GNST ON  GNST.VUANID=A.ID AND GD.MAGIAIDOAN=2               
           ...
           LEFT JOIN (SELECT CA.VUANID,I.TEN TRUONGHOPGIAONHAN, CA.NGUOITAO_PHUCTHAM, CA.NGAYTAO_PHUCTHAM 
                      FROM DM_DATAITEM I 
                          INNER JOIN AHN_CHUYEN_NHAN_AN CA ON CA.TRUONGHOPGIAONHANID=I.ID 
                      WHERE CA.TOANHANID=V_TOAAN_ID AND NOT EXISTS (SELECT 'x' FROM AHN_DON WHERE ID =NVL(CA.MAP_VUANID_NEW ,0) AND MAGIAIDOAN = 7)
        ...
        WHERE A.MAGIAIDOAN != 7 --toancau 31-03-2023 bỏ án pt tđc
              AND (V_TEN_VU_AN IS NULL  OR ( FN_CONVERT_TO_VN(LOWER(A.TENVUVIEC)) LIKE  '%'||VV_TEN_VU_AN||'%' ) )--Tên vụ án

              AND (V_UTTP IS NULL OR (V_UTTP IS NOT NULL AND (EXISTS (SELECT 'X' 
                                                                      FROM AHN_SOTHAM_THULY TL  
                                                                      WHERE TL.UTTPDI = TO_NUMBER(V_UTTP) AND TL.DONID = A.ID AND GD.MAGIAIDOAN=2)    
              ...
              AND (V_QHPL IS NULL OR ( FN_CONVERT_TO_VN(LOWER(A.TENVUVIEC)) LIKE  '%'||VV_QHPL||'%' ) )--Quan hệ pháp luật tìm quan hệ pháp luật đã được gắn vào tên vụ án
            AND (V_MA_VU_AN IS NULL  OR ( FN_CONVERT_TO_VN(LOWER(A.MAVUVIEC)) LIKE VV_MA_VU_AN ) )   --Mã vụ án
            AND (V_TENDUONGSU IS NULL --Đương sự
                  OR( EXISTS (SELECT 'X' FROM AHN_DON_DUONGSU DS WHERE FN_CONVERT_TO_VN(UPPER(DS.TENDUONGSU)) LIKE '%'||VV_TENDUONGSU||'%' AND DS.DONID=A.ID)
                    ) ) 
         ...
         AND (V_THAMPHAN_ID IS NULL
           OR(V_VAITRO_THAMPHAN IS NULL 
                AND EXISTS(SELECT 'X' FROM V_TABLE_THAMPHAN TP WHERE TP.DONID = A.ID AND TP.CANBOID = V_THAMPHAN_ID AND TP.MAGIAIDOAN = GD.MAGIAIDOAN))
            OR (V_VAITRO_THAMPHAN = 'VTTP_GIAIQUYETVUVIEC'
                    AND EXISTS (SELECT 'X' FROM V_TABLE_THAMPHAN TP WHERE TP.DONID = A.ID AND TP.MAVAITRO IN ('VTTP_GIAIQUYETSOTHAM','VTTP_GIAIQUYETPHUCTHAM') AND TP.CANBOID = V_THAMPHAN_ID))
            OR(V_VAITRO_THAMPHAN = 'CHUTOAPHIENTOA'
                AND EXISTS(SELECT 'X' FROM V_TABLE_THAMPHAN TP WHERE TP.DONID = A.ID AND TP.MAVAITRO IN ('THAMPHAN','VTTP_GIAIQUYETSOTHAM','VTTP_GIAIQUYETPHUCTHAM') AND TP.CANBOID = V_THAMPHAN_ID AND TP.MAGIAIDOAN = GD.MAGIAIDOAN))
            OR(V_VAITRO_THAMPHAN IN ('VTTP_GIAIQUYETDON','THAMPHANHDXX','THAMPHANDUKHUYET')
                AND EXISTS(SELECT 'X' FROM V_TABLE_THAMPHAN TP WHERE TP.DONID = A.ID AND TP.MAVAITRO =V_VAITRO_THAMPHAN AND TP.CANBOID = V_THAMPHAN_ID AND TP.MAGIAIDOAN = GD.MAGIAIDOAN )))
...
BEGIN
     ...
     SQL_STRING_VUAN := 'SELECT * FROM AHN_DON A WHERE (A.TOAANID = ' || V_TOAAN_ID || ' OR A.TOAPHUCTHAMID = '|| V_TOAAN_ID || ') AND A.MAGIAIDOAN != 7 ';
     ...
     '   WITH 
                ...
                V_TABLE_BC_KC AS (SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER 
                                  FROM (SELECT DS.ID,DS.DONID,DS.TENDUONGSU,DS.TUCACHTOTUNG_MA,ROW_NUMBER() OVER (PARTITION BY DS.DONID ORDER BY DS.ISDAIDIEN DESC,DS.TENDUONGSU) ROWNUMBER
                                        ...
                                        WHERE EXISTS(SELECT 1 FROM AHN_SOTHAM_KHANGCAO KC WHERE KC.DONID = b.ID AND KC.DUONGSUID=DS.ID AND KC.LOAIKHANGCAO !=2))BC 
                                  WHERE BC.ROWNUMBER <=3)
    ...
    '    SELECT DISTINCT A.ID,
     ...
     '      INNER JOIN (SELECT G.* 
			...
			-- lấy thông tin vụ án kết thúc ở ST
			LEFT JOIN ( SELECT STBA.ID, STBA.DONID FROM AHN_SOTHAM_BANAN STBA WHERE STBA.SOBANAN IS NOT NULL) STBAKT ON STBAKT.DONID = A.ID AND GD.MAGIAIDOAN=2
			LEFT JOIN ( SELECT STQD.ID, STQD.DONID FROM AHN_SOTHAM_QUYETDINH STQD LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID = STQD.QUYETDINHID WHERE QD.KET_THUC = 1 AND QD.ISSOTHAM = 1 AND QD.ISHNGD = 1) STQDKT ON STQDKT.DONID = A.ID AND GD.MAGIAIDOAN=2
                ...
                DECODE(XLD_LOAIGIAIQUYET,1,''- Đã chuyển đơn'',
                                                           ...
                                                           ||CASE WHEN (A_VUANGOCID > 0 AND A_IS_TACHAN IS NULL) 
                                                                  ...
                                                                  THEN (SELECT ''</br>- Đã tách vụ án (Vụ án tách: Thụ lý số:<b> ''|| TO_CHAR(T.SOTHULY) ||''</b> ngày<b> ''||TO_CHAR(T.NGAYTHULY,''dd/MM/yyyy'')
                                                                            ...
                                                                            WHERE D.ID = A_VUANGOCID)
                                                              END) TINHTRANG_GQ,
                  DECODE(BA_ID,NULL,DECODE(QD_ID,NULL,DECODE(STBAKT_ID,NULL,DECODE(STQDKT_ID,NULL,NULL,4),4),3),3) THULYXXLAI, 
				  '''' THAMPHANHG
