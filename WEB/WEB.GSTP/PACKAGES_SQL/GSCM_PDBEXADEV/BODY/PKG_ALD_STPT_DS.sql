CREATE OR REPLACE PACKAGE BODY GSCM.PKG_ALD_STPT_DS AS

--PROCEDURE ALD_DON_SEARCH_TURNING
--( 
...
--    V_MA_THONG_BAO          IN NUMBER, -- 1 Tim theo ma vu an, 2 tism theo ma thong bao an phi
...
--                                     FROM ALD_DON_DUONGSU WHERE ISDAIDIEN=0) BC 
...
--                                        WHERE EXISTS(SELECT 1 FROM ALD_SOTHAM_KHANGCAO KC WHERE KC.DUONGSUID=DS.ID AND KC.LOAIKHANGCAO !=2 AND KC.DONID=DS.DONID))BC 
--                                  WHERE BC.ROWNUMBER <=3)
--        ';  
--
--       SQL_STRING_SELECT := 
...
--                      ) GD ON A.ID=GD.DONID 
--
--          LEFT JOIN ALD_ANPHI AI ON A.ID=AI.DONID
--
--          LEFT JOIN DVCQG_THANH_TOAN TT ON TT.ANPHI_ID=AI.ID AND TT.MALOAIVUVIEC=5
--
...
--                      WHERE CA.TOANHANID='|| V_TOAAN_ID ||' AND NOT EXISTS (SELECT 1 FROM ALD_DON WHERE ID =NVL(CA.MAP_VUANID_NEW ,0) AND MAGIAIDOAN = 7)                      )GN ON  GN.VUANID=A.ID
--
...
--
--        ----TOANCAU-1-11-2024
--        LEFT JOIN HOAGIAI_DON HGD ON HGD.VUVIECID = A.ID AND HGD.LOAIANID = 5                      
...
--        SQL_STRING_WHERE := SQL_STRING_WHERE ||
--            ' AND ((EXISTS(SELECT 1 FROM ALD_SOTHAM_BANAN QSV WHERE UPPER(QSV.SOBANAN) LIKE ''%'|| V_SO_QD ||'%'' AND A.ID=QSV.DONID  )
--                    OR EXISTS(SELECT 1 FROM ALD_SOTHAM_QUYETDINH QSV WHERE UPPER(QSV.SOQD) LIKE ''%'||V_SO_QD||'%'' AND A.ID=QSV.DONID  )
--                    OR EXISTS(SELECT 1 FROM ALD_PHUCTHAM_BANAN QSV WHERE UPPER(QSV.SOBANAN) LIKE ''%'||V_SO_QD||'%'' AND A.ID=QSV.DONID  )
--                    OR EXISTS(SELECT 1 FROM ALD_PHUCTHAM_QUYETDINH QSV WHERE UPPER(QSV.SOQD) LIKE ''%'||V_SO_QD||'%'' AND A.ID=QSV.DONID  )
--                   )
--                  )'
--        ;
--     END IF;    
...
--        SQL_STRING_WHERE := SQL_STRING_WHERE ||
--            ' AND ((EXISTS(SELECT 1 FROM ALD_SOTHAM_BANAN QSV WHERE TO_CHAR(QSV.NGAYMOPHIENTOA,''dd/MM/yyyy'') = '|| V_NGAY_QD ||' AND A.ID = QSV.DONID  )
--                   OR EXISTS(SELECT 1 FROM ALD_SOTHAM_QUYETDINH QSV WHERE TO_CHAR(QSV.NGAYQD,''dd/MM/yyyy'') = '|| V_NGAY_QD ||' AND A.ID = QSV.DONID  )
--                   OR EXISTS(SELECT 1 FROM ALD_PHUCTHAM_BANAN QSV WHERE TO_CHAR(QSV.NGAYMOPHIENTOA,''dd/MM/yyyy'') = '|| V_NGAY_QD ||' AND A.ID = QSV.DONID  )
--                   OR EXISTS(SELECT 1 FROM ALD_PHUCTHAM_QUYETDINH QSV WHERE TO_CHAR(QSV.NGAYQD,''dd/MM/yyyy'') = '|| V_NGAY_QD ||' AND A.ID = QSV.DONID  )
--                   )
--                  )'
--        ;
--     END IF; 
...
--                                                                                        FROM ALD_DON D
--                                                                                            LEFT JOIN ALD_DON_THAMPHAN PC ON PC.DONID = D.ID
...
--                                                                                WHERE CNA.TRANGTHAI = 1 -- 1/ĐÃ NHẬN ÁN
--                                                                              )
--                                                AND ( EXISTS( SELECT 1 FROM ALD_CHUYEN_NHAN_AN CNA
...
--                                                     )
--                                                AND ( XLD.NGAYGQ_YC <= '|| VV_NGAYTHULY_DEN ||' 
--                                                                              OR EXISTS( SELECT 1 FROM ALD_DON_THAMPHAN PC 
--                                                                                         WHERE PC.MAVAITRO = ''VTTP_GIAIQUYETDON''
--                                                                                             AND XLD.NGAYGQ_YC IS NULL
--                                                                                             AND PC.NGAYPHANCONG <= '|| VV_NGAYTHULY_DEN ||'
--                                                                                             AND A.ID = PC.DONID
...
--                AND 
--                (
--                    NOT EXISTS (SELECT 1 FROM ALD_DON_XULY XL WHERE XL.DONID = A.ID AND XL.LOAIGIAIQUYET IN (1,3))
--                )
--                AND 
--                 (   
...
--                         AND
--                         (
--                             ( EXISTS (SELECT 1 FROM ALD_SOTHAM_QUYETDINH QDVA INNER JOIN DM_QD_QUYETDINH QD ON QD.ID = QDVA.QUYETDINHID AND QD.KET_THUC = 1 
--                                       WHERE A.ID = QDVA.DONID AND QDVA.NGAYQD >= '|| VV_DENNGAY ||')
--                                       OR
--                               EXISTS (SELECT 1 FROM ALD_SOTHAM_BANAN BA WHERE A.ID = BA.DONID AND BA.NGAYTUYENAN >= '|| VV_DENNGAY ||')
--
--                             )
--                             OR 
--                             ( NOT EXISTS (SELECT 1 FROM ALD_SOTHAM_QUYETDINH QDVA INNER JOIN DM_QD_QUYETDINH QD ON QD.ID = QDVA.QUYETDINHID AND QD.KET_THUC = 1 WHERE A.ID = QDVA.DONID)
--                               AND NOT EXISTS (SELECT 1 FROM ALD_SOTHAM_BANAN BA WHERE A.ID = BA.DONID)
--
--                             )
--                         )
--                     )
...
--                         AND
--                         (
--                             ( EXISTS (SELECT 1 FROM ALD_PHUCTHAM_QUYETDINH QDVA INNER JOIN DM_QD_QUYETDINH QD ON QD.ID = QDVA.QUYETDINHID AND QD.KET_THUC = 1 
--                                       WHERE A.ID = QDVA.DONID AND QDVA.NGAYQD >= '|| VV_DENNGAY ||' )
--                                       OR
--                               EXISTS (SELECT 1 FROM ALD_PHUCTHAM_BANAN BA WHERE A.ID = BA.DONID AND BA.NGAYTUYENAN >= '|| VV_DENNGAY ||')
--
--                             )
--                             OR 
--                             ( NOT EXISTS (SELECT 1 FROM ALD_PHUCTHAM_QUYETDINH QDVA INNER JOIN DM_QD_QUYETDINH QD ON QD.ID = QDVA.QUYETDINHID AND QD.KET_THUC = 1 WHERE A.ID = QDVA.DONID)
--                               AND NOT EXISTS (SELECT 1 FROM ALD_PHUCTHAM_BANAN BA WHERE A.ID = BA.DONID)
--
--                             )
--                         )
--                     )
...
--                           )
--                        OR EXISTS( SELECT 1 FROM ALD_PHUCTHAM_QUYETDINH PTQDVA
--                                       LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID = PTQDVA.QUYETDINHID
--                                       LEFT JOIN DM_QD_LOAI QDL ON QDL.ID = QD.LOAIID
...
--            SQL_STRING_WHERE := SQL_STRING_WHERE || '
--                AND(EXISTS (SELECT 1 FROM ALD_DON_GIAIDOAN BC
--                                        WHERE  BC.DONID = A.ID
--                                          AND (
...
--        SQL_STRING_WHERE := SQL_STRING_WHERE || '
--            AND(NOT EXISTS (SELECT 1 FROM ALD_DON_GIAIDOAN BC
--                                    WHERE BC.DONID = A.ID
--                                      AND (
--                                        (BC.MAGIAIDOAN = 2 AND BC.TOAANID = '|| V_TOAAN_ID ||') OR
--                                        (BC.MAGIAIDOAN in (3,7) AND BC.TOAPHUCTHAMID = '|| V_TOAAN_ID ||')
...
--                PKG_STPT_ALD_GS.NOIDUNG_KHANGCAO_DANHSACH(A_ID) AS KHANGCAO_ST,
...
--      GN.TRUONGHOPGIAONHAN,
    ...
    DECODE(A.HINHTHUCNHANDON,1,'<br/><i>TH giao nhận:</i> <b>Thụ lý mới</b>',
      ...
      STBA.BANAN_QD_ST,STKN.KHANGNGHI_ST,'' QD_PT,PKG_STPT_ALD_GS.NOIDUNG_KHANGCAO_DANHSACH(a.ID) as KHANGCAO_ST, --toancau-tamnc thêm trường QD_PT,KHANGCAO_ST mặc định trống
...
--                  )GN ON  GN.VUANID=a.ID
        ...
        --------------------- TOANCAU lấy thêm thông tin kháng cáo  
...
BEGIN
     ...
     SQL_STRING_VUAN := 'SELECT * FROM ALD_DON A WHERE (A.TOAANID = ' || V_TOAAN_ID || ' OR A.TOAPHUCTHAMID = '|| V_TOAAN_ID || ') AND A.MAGIAIDOAN != 7 ';
     ...
     '   WITH 
                ...
                V_TABLE_BC_KC AS (SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER 
                                  FROM (SELECT DS.ID,DS.DONID,DS.TENDUONGSU,DS.TUCACHTOTUNG_MA,ROW_NUMBER() OVER (PARTITION BY DS.DONID ORDER BY DS.ISDAIDIEN DESC,DS.TENDUONGSU) ROWNUMBER
                                        ...
                                        WHERE EXISTS(SELECT 1 FROM ALD_SOTHAM_KHANGCAO KC WHERE KC.DONID = b.ID AND KC.DUONGSUID=DS.ID AND KC.LOAIKHANGCAO !=2))BC 
                                  WHERE BC.ROWNUMBER <=3)
        ';  
    ...
    '    SELECT DISTINCT A.ID,
                ...
                BA.ID BA_ID,
                QD.ID QD_ID,
                STBAKT.ID STBAKT_ID, -- VNPT QUANVV 03/12/2025 update thụ lý lại ST
				STQDKT.ID STQDKT_ID
     ...
     '      INNER JOIN (SELECT G.* 
			...
			-- VNPT QUANVV 02/12/2025 lấy thông tin vụ án kết thúc ở ST
			LEFT JOIN ( SELECT STBA.ID, STBA.DONID FROM ALD_SOTHAM_BANAN STBA WHERE STBA.SOBANAN IS NOT NULL) STBAKT ON STBAKT.DONID = A.ID AND GD.MAGIAIDOAN=2
			LEFT JOIN ( SELECT STQD.ID, STQD.DONID FROM ALD_SOTHAM_QUYETDINH STQD LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID = STQD.QUYETDINHID WHERE QD.KET_THUC = 1 AND QD.ISSOTHAM = 1 AND QD.ISLAODONG = 1) STQDKT ON STQDKT.DONID = A.ID AND GD.MAGIAIDOAN=2
        ...
        'SELECT  TTT.ID,
                ...
                DECODE(XLD_LOAIGIAIQUYET,1,''- Đã chuyển đơn'',
                                                           ...
                                                           ||CASE WHEN (A_VUANGOCID > 0 AND A_IS_TACHAN IS NULL) 
                                                              ...
                                                              END) TINHTRANG_GQ,
-- VNPT QUANVV
                  DECODE(BA_ID,NULL,DECODE(QD_ID,NULL, DECODE(STBAKT_ID,NULL,DECODE(STQDKT_ID,NULL,NULL,4),4),3),3) THULYXXLAI, '''' THAMPHANHG
