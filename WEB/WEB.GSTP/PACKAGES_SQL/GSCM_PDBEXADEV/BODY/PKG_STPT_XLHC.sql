CREATE OR REPLACE PACKAGE BODY GSCM.PKG_STPT_XLHC AS
...
-- VNPT - Lê Bá Thọ 16-10-2025 8:30
-- Tìm kiếm theo vai trò của thẩm phán
-- Thẩm phán HDXX Sơ thẩm
        SELECT CAST(MAVAITRO AS NVARCHAR2(20)) MAVAITRO,DONID,ID,CANBOID,
               ROW_NUMBER() OVER (PARTITION BY DONID,MAVAITRO ORDER BY NGAYPHANCONG DESC) ROWNUMBER,
               NGAYPHANCONG,
               2 MAGIAIDOAN
        ...
        -- Thẩm phán HDXX Kháng cáo/Kháng nghị quyết định kháng cáo
        SELECT CAST(MAVAITRO AS NVARCHAR2(20)) MAVAITRO,DONID,ID,CANBOID,
               ROW_NUMBER() OVER (PARTITION BY DONID,MAVAITRO ORDER BY NGAYPHANCONG DESC) ROWNUMBER,
               NGAYPHANCONG,
               7 MAGIAIDOAN
    ...
    WHERE (
        (TP.ROWNUMBER = 1 AND TP.MAVAITRO IN ('VTTP_GIAIQUYETDON','VTTP_GIAIQUYETSOTHAM','VTTP_GIAIQUYETPHUCTHAM','THAMPHAN','THAMPHANHDXX'))
        OR TP.MAVAITRO = 'THAMPHANDUKHUYET'
    );
------- END     
    ...
    WITH cte_Data AS(
      Select distinct(don.ID)
        ...
        ,STBA.BANAN_QD_ST
--        ,(BC3.HoTen||BC2.HoTen) HoTenBiCan 
-- VNPT - Lê Bá Thọ - 16-10-2025
-- lấy ra người bị đề nghị là đại diện
        ,(SELECT ds.HOTEN 
           FROM XLHC_DUONGSU ds 
           WHERE ds.DONID = don.ID 
             AND ds.BICANDAUVU = 1 
             AND ds.LOAIDOITUONG = 1 
             AND ROWNUM = 1) AS HoTenBiCan
--             
        ,STKN.KHANGNGHI_ST
        ,(TLS.TINHTRANG_GQ || TLPT.TINHTRANG_GQ) CHECK_THULY
        ,CASE  WHEN (TLS.TINHTRANG_GQ || TLPT.TINHTRANG_GQ) IS NULL THEN '- Chưa thụ lý'
...
--                )
          ...
          AND (vSoThuLy IS NULL OR(UPPER(TLS.SOTHULY)=UPPER(vSoThuLy) OR UPPER(TLPT.SOTHULY)=UPPER(vSoThuLy) ))

 
 ---Sửa ADS sang XLHC -- VNPT- Lê Bá Thọ - Chỉnh bộ lọc tìm kiếm theo tình trạng giải quyết - 17-9-2025 08:00
 --Tình trạng GQ;
    AND (vTinhTrangGQ IS NULL
                 OR  (( NVL(LENGTH(vTinhTrangGQ),0) = 0
             AND (
                    ( GD.MAGIAIDOAN = 2
                        AND (vTuNgayGQ IS NULL OR don.NGAYNHANDON>= VV_TUNGAY_GQ ) 
                        AND (vDenNgayGQ IS NULL OR don.NGAYNHANDON<=  VV_DENNGAY_GQ ) 
                    )
                    OR EXISTS ( SELECT 1 
                                FROM XLHC_CHUYEN_NHAN_AN CNA
                                WHERE GD.MAGIAIDOAN = 3
                                    AND (vtungaythuly IS NULL OR CNA.NGAYNHAN >=VV_NGAYTHULY_TU ) 
                                    AND (vDenNgayThuLy IS NULL OR CNA.NGAYNHAN <=  VV_NGAYTHULY_DEN )
                                )
                  )
        )
        OR (vTinhTrangGQ LIKE '1' --Chưa giải quyết xong
        
                AND 
                (
                    NOT EXISTS (SELECT 1 FROM XLHC_DON_XULY XL WHERE XL.DONID = don.ID AND XL.LOAIGIAIQUYET IN (1,3))
                )
                AND 
                 (   
                     (   GD.MAGIAIDOAN = 2 -- SƠ THẨM
                         AND
                         (
                             ( EXISTS (SELECT 1 FROM XLHC_SOTHAM_QUYETDINH QDVA INNER JOIN DM_QD_QUYETDINH QD ON QD.ID = QDVA.QUYETDINHID AND QD.KET_THUC = 1 
                                       WHERE don.ID = QDVA.DONID AND (vDenNgayGQ IS NULL OR QDVA.NGAYQD >= VV_DENNGAY_GQ ))
                                       OR
                               EXISTS (SELECT 1 FROM XLHC_SOTHAM_BANAN BA WHERE don.ID = BA.DONID AND (vDenNgayGQ IS NULL OR BA.NGAYTUYENAN >=  VV_DENNGAY_GQ ))

                             )
                             OR 
                             ( NOT EXISTS (SELECT 1 FROM XLHC_SOTHAM_QUYETDINH QDVA INNER JOIN DM_QD_QUYETDINH QD ON QD.ID = QDVA.QUYETDINHID AND QD.KET_THUC = 1 WHERE don.ID = QDVA.DONID)
                               AND NOT EXISTS (SELECT 1 FROM XLHC_SOTHAM_BANAN BA WHERE don.ID = BA.DONID)
                     ...
                     (   GD.MAGIAIDOAN = 3 -- PHÚC THẨM
                         AND
                         (
                             ( EXISTS (SELECT 1 FROM XLHC_PHUCTHAM_QUYETDINH QDVA INNER JOIN DM_QD_QUYETDINH QD ON QD.ID = QDVA.QUYETDINHID AND QD.KET_THUC = 1 
                                       WHERE don.ID = QDVA.DONID AND (vDenNgayGQ IS NULL OR QDVA.NGAYQD >=  VV_DENNGAY_GQ  ))
                                       OR
                               EXISTS (SELECT 1 FROM XLHC_PHUCTHAM_BANAN BA WHERE don.ID = BA.DONID AND (vDenNgayGQ IS NULL OR BA.NGAYTUYENAN >=  VV_DENNGAY_GQ ))

                             )
                             OR 
                             ( NOT EXISTS (SELECT 1 FROM XLHC_PHUCTHAM_QUYETDINH QDVA INNER JOIN DM_QD_QUYETDINH QD ON QD.ID = QDVA.QUYETDINHID AND QD.KET_THUC = 1 WHERE don.ID = QDVA.DONID)
                               AND NOT EXISTS (SELECT 1 FROM XLHC_PHUCTHAM_BANAN BA WHERE don.ID = BA.DONID)
                 ...
                 --- VNPT- Trần Công Hoan - Thêm điều kiện chưa có bản án - 19-9-2025 09:23
                 AND(
                   NOT EXISTS ( SELECT 1 
                                FROM XLHC_SOTHAM_BANAN STBA
                                WHERE (vTuNgayGQ IS NULL OR STBA.NGAYTUYENAN >=  VV_TUNGAY_GQ )
                                    AND (vDenNgayGQ IS NULL OR STBA.NGAYTUYENAN <=  VV_DENNGAY_GQ )
                                    AND STBA.DONID = don.ID  AND GD.MAGIAIDOAN = 2
                               )
                  AND NOT EXISTS ( SELECT 1 
                              FROM XLHC_PHUCTHAM_BANAN PTBA
                              WHERE (vTuNgayGQ IS NULL OR PTBA.NGAYTUYENAN >=  VV_TUNGAY_GQ )
                                  AND (vDenNgayGQ IS NULL OR PTBA.NGAYTUYENAN <=  VV_DENNGAY_GQ )
                                  AND PTBA.DONID = don.ID AND GD.MAGIAIDOAN = 3
                             )
                 )
         ...
         OR (vTinhTrangGQ LIKE '2'  --chưa phân công Thẩm phán
			AND (EXISTS(SELECT 1 
                          FROM XLHC_SOTHAM_THULY STTL
                          WHERE STTL.DONID =  don.ID 
                              AND ( vTuNgayGQ IS NULL OR STTL.NGAYTHULY >=  VV_TUNGAY_GQ )
                              AND (vDenNgayGQ IS NULL OR STTL.NGAYTHULY <=  VV_DENNGAY_GQ )
                          )
                  OR EXISTS(SELECT 1 
                            FROM XLHC_PHUCTHAM_THULY PTTL 
                            WHERE PTTL.DONID = don.ID
                                AND( vTuNgayGQ IS NULL OR PTTL.NGAYTHULY >=  VV_TUNGAY_GQ )
                                AND( vDenNgayGQ IS NULL OR PTTL.NGAYTHULY <=  VV_DENNGAY_GQ )
                            )
                  )
              AND (NOT EXISTS (SELECT 1 
                               ...
                               WHERE PC.DONID=don.ID
                                    AND ((PC.MAVAITRO =  SOTHAM  AND GD.MAGIAIDOAN = 2)--sơ thẩm
                                        OR (PC.MAVAITRO =  PHUCTHAM  AND GD.MAGIAIDOAN = 3)--phuc thẩm
                                        OR ( PC.MAVAITRO = PHUCTHAM  AND GD.MAGIAIDOAN = 7 )
                                        )
                                    AND ( vTuNgayGQ IS NULL OR PC.NGAYPHANCONG >=  VV_TUNGAY_GQ ) 
                                    AND ( vDenNgayGQ IS NULL OR PC.NGAYPHANCONG <=  VV_DENNGAY_GQ )  
                                )
                   )
            )
            OR (vTinhTrangGQ = 3  --đã phân công Thẩm phán
         AND EXISTS (SELECT 1 
            ...
            OR (vTinhTrangGQ LIKE '4' --đã lên lịch xét xử
               AND (EXISTS ( SELECT 1 
                                    FROM XLHC_SOTHAM_QUYETDINH QSV
                                        LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID = QSV.QUYETDINHID
                                        LEFT JOIN DM_QD_LOAI QDL ON QDL.ID = QD.LOAIID
                                ...
                                OR EXISTS( SELECT 1 FROM XLHC_PHUCTHAM_QUYETDINH PTQDVA
                                               LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID = PTQDVA.QUYETDINHID
                                               LEFT JOIN DM_QD_LOAI QDL ON QDL.ID = QD.LOAIID
            ...
            OR (vTinhTrangGQ LIKE '5' --Đang hoãn
               AND ( EXISTS (SELECT 1 
                                    FROM XLHC_SOTHAM_QUYETDINH QSV
                                        LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID = QSV.QUYETDINHID
                                        LEFT JOIN DM_QD_LOAI QDL ON QDL.ID = QD.LOAIID
                ...
                OR (vTinhTrangGQ LIKE '6'  --Đang tạm đình chỉ
           --so tham Đang tạm đình chỉ 
                      AND (EXISTS ( SELECT 1 
                                    FROM XLHC_SOTHAM_QUYETDINH QSV
                                        LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID = QSV.QUYETDINHID
            ...
            OR (vTinhTrangGQ LIKE '7'  --Đã giải quyết xong
              AND ( EXISTS (SELECT 1 
                            ...
                            OR EXISTS ( SELECT 1 
                                        FROM XLHC_SOTHAM_QUYETDINH QSV 
                                            LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID = QSV.QUYETDINHID 
                                        WHERE QD.KET_THUC = 1 AND QD.ISSOTHAM = 1 AND QD.ISXLHC = 1
                                            AND (vTuNgayGQ IS NULL OR QSV.NGAYQD >=  VV_TUNGAY_GQ )
                                            AND (vDenNgayGQ IS NULL OR QSV.NGAYQD <=  VV_DENNGAY_GQ )
                                            AND QSV.DONID = don.ID  AND GD.MAGIAIDOAN = 2
                                      )
                            ...
                            OR EXISTS ( SELECT 1 
                                        FROM XLHC_PHUCTHAM_QUYETDINH PTQDVA 
                                            LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID = PTQDVA.QUYETDINHID 
                                        WHERE QD.KET_THUC = 1 AND QD.ISPHUCTHAM = 1 AND QD.ISXLHC = 1
                                            AND (vTuNgayGQ IS NULL OR PTQDVA.NGAYQD >=  VV_TUNGAY_GQ )
                                            AND (vDenNgayGQ IS NULL OR PTQDVA.NGAYQD <=  VV_DENNGAY_GQ )
                                            AND PTQDVA.DONID = don.ID  AND GD.MAGIAIDOAN = 3
                                       )
                            OR EXISTS ( SELECT 1 
                                        ...
                                        WHERE ADX.LOAIGIAIQUYET IN (1, 3)
                                            AND (vTuNgayGQ IS NULL OR  ADX.NGAYGQ_YC >=  VV_TUNGAY_GQ )
                                            AND (vDenNgayGQ IS NULL OR ADX.NGAYGQ_YC <=  VV_DENNGAY_GQ )
                                            AND ADX.DONID = don.ID  AND GD.MAGIAIDOAN = 2
                                       )
                            --- Thêm điều kiện Đã giải quyết xong -- VNPT- Trần Công Hoan - ST Có kết quả ở màn 3.3 - 18-9-2025 18:30
                            OR EXISTS ( SELECT 1 
                                        FROM XLHC_SOTHAM_BANAN STBA
                                        WHERE (vTuNgayGQ IS NULL OR STBA.NGAYTUYENAN >=  VV_TUNGAY_GQ )
                                            AND (vDenNgayGQ IS NULL OR STBA.NGAYTUYENAN <=  VV_DENNGAY_GQ )
                           ...
                           --- Thêm điều kiện Đã giải quyết xong -- VNPT- Trần Công Hoan - PT Có kết quả ở màn 4.4 - 18-9-2025 18:30
                            OR EXISTS ( SELECT 1 
                                        FROM XLHC_PHUCTHAM_BANAN PTBA
                                        WHERE (vTuNgayGQ IS NULL OR PTBA.NGAYTUYENAN >=  VV_TUNGAY_GQ )
                                            AND (vDenNgayGQ IS NULL OR PTBA.NGAYTUYENAN <=  VV_DENNGAY_GQ )
            ...
            OR (vTinhTrangGQ LIKE '8'  --Đã xét xử
              AND ( EXISTS ( SELECT 1 
                                    FROM XLHC_SOTHAM_BANAN BA
                                    WHERE  BA.SOBANAN IS NOT NULL
                                        AND (vTuNgayGQ IS NULL OR BA.NGAYTUYENAN >=  VV_TUNGAY_GQ )
            ...
            OR (vTinhTrangGQ LIKE '9'  --Đình chỉ
               AND ( EXISTS ( SELECT 1 
                                    FROM XLHC_SOTHAM_QUYETDINH QSV 
                                        LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID=QSV.QUYETDINHID
                                        LEFT JOIN DM_QD_LOAI QDL ON QDL.ID = QD.LOAIID
            ...
            OR (vTinhTrangGQ LIKE '10'  --Công nhận thỏa thuận của đương sự
                 AND ( EXISTS ( SELECT 1 
                                    FROM XLHC_SOTHAM_QUYETDINH QSV 
                                        LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID=QSV.QUYETDINHID
                                        LEFT JOIN DM_QD_LOAI QDL ON QDL.ID=QD.LOAIID 
            ...
            OR(vTinhTrangGQ LIKE '11'  --QĐ chuyển vụ án
                AND ( EXISTS ( SELECT 1 
                                    FROM XLHC_SOTHAM_QUYETDINH QSV 
                                        LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID = QSV.QUYETDINHID
                                        LEFT JOIN DM_QD_LOAI QDL ON QDL.ID = QD.LOAIID
                           ...
                           OR EXISTS( SELECT 1 
                                      ...
                                      WHERE CA.VUANID=don.ID AND CA.TOACHUYENID =  vDonViID  AND GD.MAGIAIDOAN = 2)      
                           OR EXISTS( SELECT 1 
                                      FROM XLHC_PHUCTHAM_QUYETDINH PTQDVA 
                                          LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID = PTQDVA.QUYETDINHID
                                          LEFT JOIN DM_QD_LOAI QDL ON QDL.ID = QD.LOAIID 
...
--END lọc theo vai Trò của thẩm phán
--             and (vThamPhan IS NULL  
--                                            OR(vVaiTroThamPhan IS NULL 
--                                                AND EXISTS(SELECT 'X' FROM TABLE(V_TABLE_THAMPHAN) TP_temp WHERE TP_temp.DONID = don.ID AND TP_temp.CANBOID = vThamPhan AND TP_temp.MAGIAIDOAN = GD.MAGIAIDOAN))
--                                            OR (vVaiTroThamPhan = 'VTTP_GIAIQUYETVUVIEC'
--                                                    AND EXISTS (SELECT 'X' FROM TABLE(V_TABLE_THAMPHAN) TP_temp WHERE TP_temp.DONID = don.ID AND TP_temp.MAVAITRO IN ('VTTP_GIAIQUYETSOTHAM','VTTP_GIAIQUYETPHUCTHAM') AND TP_temp.CANBOID = vThamPhan))
--                                            OR(vVaiTroThamPhan = 'CHUTOAPHIENTOA'
--                                                AND EXISTS(SELECT 'X' FROM TABLE(V_TABLE_THAMPHAN) TP_temp WHERE TP_temp.DONID = don.ID AND TP_temp.MAVAITRO IN ('THAMPHAN','VTTP_GIAIQUYETSOTHAM','VTTP_GIAIQUYETPHUCTHAM') AND TP_temp.CANBOID = vThamPhan AND TP_temp.MAGIAIDOAN = GD.MAGIAIDOAN))
--                                            OR(vVaiTroThamPhan IN ('VTTP_GIAIQUYETDON','THAMPHANHDXX','THAMPHANDUKHUYET')
--                                             AND EXISTS(SELECT 'X' FROM TABLE(V_TABLE_THAMPHAN) TP_temp WHERE TP_temp.DONID = don.ID AND TP_temp.MAVAITRO =vVaiTroThamPhan AND TP_temp.CANBOID = vThamPhan AND TP_temp.MAGIAIDOAN = GD.MAGIAIDOAN )))

--VNPT- Lê Bá Thọ 16-10-2025 
-- Tìm kiếm theo vai trò của thẩm phán
            AND (vThamPhan IS NULL  
                --Không chọn vai trò - Lấy tất cả vai trò của thẩm phán trong giai đoạn
                OR (vVaiTroThamPhan IS NULL 
                    AND EXISTS(SELECT 'X' 
                               FROM TABLE(V_TABLE_THAMPHAN) TP_temp 
                               WHERE TP_temp.DONID = don.ID 
                               AND TP_temp.CANBOID = vThamPhan 
                               AND TP_temp.MAGIAIDOAN = GD.MAGIAIDOAN))
                
                --Thẩm phán giải quyết sơ thẩm
                OR (vVaiTroThamPhan = 'VTTP_GIAIQUYETSOTHAM'
                    AND EXISTS(SELECT 'X' 
                               FROM TABLE(V_TABLE_THAMPHAN) TP_temp 
                               WHERE TP_temp.DONID = don.ID 
                               AND TP_temp.MAVAITRO = 'VTTP_GIAIQUYETSOTHAM'
                               AND TP_temp.CANBOID = vThamPhan 
                               AND TP_temp.MAGIAIDOAN = GD.MAGIAIDOAN))
                               
                --Thẩm phán giải quyết phúc thẩm
                OR (vVaiTroThamPhan = 'VTTP_GIAIQUYETPHUCTHAM'
                    AND EXISTS(SELECT 'X' 
                               FROM TABLE(V_TABLE_THAMPHAN) TP_temp 
                               WHERE TP_temp.DONID = don.ID 
                               AND TP_temp.MAVAITRO = 'VTTP_GIAIQUYETPHUCTHAM'
                               AND TP_temp.CANBOID = vThamPhan 
                               AND TP_temp.MAGIAIDOAN = GD.MAGIAIDOAN))
               
                --Thẩm phán chủ tọa phiên tòa (bao gồm: THAMPHAN + VTTP_GIAIQUYET*)
                OR (vVaiTroThamPhan = 'CHUTOAPHIENTOA'
                    AND EXISTS(SELECT 'X' 
                               FROM TABLE(V_TABLE_THAMPHAN) TP_temp 
                               WHERE TP_temp.DONID = don.ID 
                               AND TP_temp.MAVAITRO IN ('THAMPHAN','VTTP_GIAIQUYETSOTHAM','VTTP_GIAIQUYETPHUCTHAM') 
                               AND TP_temp.CANBOID = vThamPhan 
                               AND TP_temp.MAGIAIDOAN = GD.MAGIAIDOAN))
                
                -- Các vai trò cụ thể khác (Xử lý hồ sơ, Thành viên HĐXX, Dự khuyết)
                OR (vVaiTroThamPhan IN ('VTTP_GIAIQUYETDON','THAMPHANHDXX','THAMPHANDUKHUYET')
                    AND EXISTS(SELECT 'X' 
                               FROM TABLE(V_TABLE_THAMPHAN) TP_temp 
                               WHERE TP_temp.DONID = don.ID 
                               AND TP_temp.MAVAITRO = vVaiTroThamPhan 
                               AND TP_temp.CANBOID = vThamPhan 
                               AND TP_temp.MAGIAIDOAN = GD.MAGIAIDOAN))
            )
...
---------------END
          ...
          AND (vPTRutKinhNghiem IS NULL
             ...
             -- VNPT- Lê Bá Thọ - check box án đã giải quyết xong từ tòa cũ - 15-9-2025 08:00
             AND ((V_AN_KET_THUC = 1
                                     AND  (    EXISTS(SELECT 'X' 
                                                         FROM XLHC_DON_GIAIDOAN AVGD 
                                                         WHERE don.ID=AVGD.DONID 
                                                                       AND (
                                                            (AVGD.MAGIAIDOAN = 2 AND AVGD.TOAANID = vToaXetXu) OR
                                                            (AVGD.MAGIAIDOAN in (3,7) AND AVGD.TOAPHUCTHAMID = vToaXetXu)
                                                          )
...
--                            D.TUCACHTOTUNG_MA,
                        ...
                        FROM
                            XLHC_DUONGSU   D
                            -- LEFT JOIN XLHC_ANPHI         P ON P.DUONGSU_ID = D.ID
--                        WHERE
--                            D.ISDAIDIEN = 0
...
--                                        tìm tên vụ án
    ...
    ---VNPT Lê Bá Thọ 11-10-2025 
    -- Thêm lọc theo vai Trò của thẩm phán
                                    and (vThamPhan IS NULL  
                                            OR(vVaiTroThamPhan IS NULL 
                                                AND EXISTS(SELECT 'X' FROM TABLE(V_TABLE_THAMPHAN) TP WHERE TP.DONID = A.ID AND TP.CANBOID = vThamPhan AND TP.MAGIAIDOAN = GD.MAGIAIDOAN))
                                            OR (vVaiTroThamPhan = 'VTTP_GIAIQUYETVUVIEC'
                                                    AND EXISTS (SELECT 'X' FROM TABLE(V_TABLE_THAMPHAN) TP WHERE TP.DONID = A.ID AND TP.MAVAITRO IN ('VTTP_GIAIQUYETSOTHAM','VTTP_GIAIQUYETPHUCTHAM') AND TP.CANBOID = vThamPhan))
                                            OR(vVaiTroThamPhan = 'CHUTOAPHIENTOA'
                                                AND EXISTS(SELECT 'X' FROM TABLE(V_TABLE_THAMPHAN) TP WHERE TP.DONID = A.ID AND TP.MAVAITRO IN ('THAMPHAN','VTTP_GIAIQUYETSOTHAM','VTTP_GIAIQUYETPHUCTHAM') AND TP.CANBOID = vThamPhan AND TP.MAGIAIDOAN = GD.MAGIAIDOAN))
                                            OR(vVaiTroThamPhan IN ('VTTP_GIAIQUYETDON','THAMPHANHDXX','THAMPHANDUKHUYET')
                                             AND EXISTS(SELECT 'X' FROM TABLE(V_TABLE_THAMPHAN) TP WHERE TP.DONID = A.ID AND TP.MAVAITRO =vVaiTroThamPhan AND TP.CANBOID = vThamPhan AND TP.MAGIAIDOAN = GD.MAGIAIDOAN )))
             --END lọc theo vai Trò của thẩm phán
                          ...
                          --Tình trạng GQ;
                                      AND ( ( vTinhTrangGQ IS NULL
                                              AND ( VV_TUNGAY IS NULL
                                                    OR A.NGAYTAO >= VV_TUNGAY )
                                              AND ( VV_DENNGAY IS NULL
                                ...
                                --VNPT Trần Công Hoan 19/09/2025 9:53 Thêm trường hợp chưa thụ lý
                                OR 
                                NOT EXISTS (SELECT 'X'
									                  FROM XLHC_KCKNQDK_PHUCTHAM_THULY PTTL
									                  WHERE ( vDenNgayGQ IS NULL OR PTTL.NGAYTHULY <= VV_DENNGAY )
									                    AND PTTL.DONID = A.ID
									                    AND GD.MAGIAIDOAN = 7)
                                
                                
									           ) )
                                      --toancau 04/07/2023 sua tieu chi chua phan cong tham phan
                                            OR ( vTinhTrangGQ = 2 --chưa phân công Thẩm phán  
                                            AND( TLPT.NGAYTHULY IS NOT NULL )
--                                            
                                            AND (  VV_TUNGAY IS NULL  OR TLPT.NGAYTHULY >= VV_TUNGAY  ) 
                                           AND ( VV_DENNGAY IS NULL  OR TLPT.NGAYTHULY <= VV_DENNGAY )
                                         AND ( 
                                            NOT EXISTS (
                                          SELECT
                                              'x'
                                          FROM
                                              XLHC_DON_THAMPHAN PC
                                          WHERE
                                              PC.DONID = A.ID
                                      ...
                                      OR( vTinhTrangGQ =4 --ĐÃ LÊN LỊCH XÉT XỬ
                                        AND ( EXISTS (
                                                    SELECT 'X' FROM XLHC_KCKNQDK_PHUCTHAM_QUYETDINH   PTQDVA --QUYẾT ĐỊNH 
                                                    LEFT JOIN DM_QD_QUYETDINH                  QD ON QD.ID = PTQDVA.QUYETDINHID
                                                    LEFT JOIN DM_QD_LOAI                       QDL ON QDL.ID = QD.LOAIID
                                                    WHERE QDL.MA = 'DVARXX' --QDL.MA = 'DVARXX' == Đưa vụ án ra xét xử
                                                    AND ( VV_TUNGAY IS NULL OR PTQDVA.NGAYQD >= VV_TUNGAY )
                                                    AND ( VV_DENNGAY IS NULL OR PTQDVA.NGAYQD <= VV_DENNGAY )
                   ...
                   ------------------------------
                                         OR ( vTinhTrangGQ = 6 --Đang tạm đình chỉ  
                                          ...
                                          SELECT
                                              'X'
                                          FROM
                                              XLHC_KCKNQDK_PHUCTHAM_QUYETDINH   PTQDVA
                                              LEFT JOIN DM_QD_QUYETDINH                  QD ON QD.ID = PTQDVA.QUYETDINHID
                                              LEFT JOIN DM_QD_LOAI                       QDL ON QDL.ID = QD.LOAIID
                                          WHERE
                                              QD.MA IN (
                                      ...
                                      SELECT
                                         ...
                                         OR EXISTS ( 
                  											...
                  											WHERE ADX.LOAIGIAIQUYET IN (1, 3)
                  												...
                  												AND ADX.DONID = A.ID  AND GD.MAGIAIDOAN = 7
                  											)
                                        --- Thêm điều kiện Đã giải quyết xong -- VNPT- Trần Công Hoan - PT Có kết quả ở màn 4.4 - 18-9-2025 18:30
                                        OR EXISTS ( SELECT 1 
                                          FROM XLHC_PHUCTHAM_BANAN PTBA
                                          WHERE (VV_TUNGAY IS NULL OR PTBA.NGAYTUYENAN >=  VV_TUNGAY )
                                              AND (VV_DENNGAY IS NULL OR PTBA.NGAYTUYENAN <=  VV_DENNGAY )
                                              AND PTBA.DONID = A.ID AND GD.MAGIAIDOAN = 7
                                         )
                                      ) )
...
--                                              AND GD.MAGIAIDOAN = 7
--                                      ) 
                                --05/07/2023 tuyennh sua tim kiem theo "Da chuyen vu an"
                                      EXISTS(
                                      SELECT 'x'
                                          FROM
                                              XLHC_CHUYEN_NHAN_AN  
                                          WHERE
                                              TOACHUYENID = vToaXetXu AND VUANID = A.ID AND GD.MAGIAIDOAN = 7
                                              AND ( VV_TUNGAY IS NULL
                                              OR NGAYGIAO >= VV_TUNGAY )
                                              AND ( VV_DENNGAY IS NULL
                                              OR NGAYGIAO <= VV_DENNGAY )  
                                      )
                                      --end
                                      ) ) )  
             -- END vTinhTrangGQ
   ...
   PROCEDURE XLHC_KCKN_DON_THAMPHAN_GETBY (
    ...
    BEGIN
        ...
        OPEN CURRETURN FOR SELECT
                              ...
                              D.TOA_GIAIQUYET_ID -- VNPT - Đinh Hoàng Sơn - thêm TOA_GIAIQUYET_ID - 15-9-2025
                          FROM
                              XLHC_DON_THAMPHAN   D
