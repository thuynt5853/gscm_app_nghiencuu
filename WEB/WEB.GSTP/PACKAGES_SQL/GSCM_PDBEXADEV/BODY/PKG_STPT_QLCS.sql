...
---------------------------QuyetdinhID
    if vLoaiAn='ADS' then 
      OPEN curReturn FOR  
          ...
          Where t.TOAANID = vdonviID and t.SoQD = vSoQD AND t.TOA_GIAIQUYET_ID = t.TOAANID 
...
---------------------------QuyetdinhID
        ...
        OPEN curReturn FOR  
            ...
            Where t.DONVIID = vdonviID and t.SoQuyetDinh = vSoQD AND t.TOA_GIAIQUYET_ID = t.DONVIID
              and t.NGAYQD between vFromDate and vToDate
    ...
    if vLoaiAn='ADS' then 
      OPEN curReturn FOR  
          ...
          inner join ADS_DON d on d.ID = t.DONID
              Where t.TOAANID = vdonviID AND t.TOAANID = t.TOA_GIAIQUYET_ID
                and t.NGayQD between vFromDate and vToDate
                and ((vLoaiQD in (62,63) and QuyetdinhID in (62,63))
                     or (vLoaiQD in (67,68) and QuyetdinhID in (67,68)) 
                     or (vLoaiQD in (41) and QuyetdinhID in (41))
...
---------------------------QuyetdinhID
        ...
        OPEN curReturn FOR  
            Select NVL(MAX(TO_NUMBER((regexp_replace( SUBSTR(t.SoQuyetDinh,1,DECODE(INSTR(t.SoQuyetDinh, '/'),0,LENGTH(t.SoQuyetDinh),INSTR(t.SoQuyetDinh, '/') - 1 )), '[^0-9]', '')))),0)
                    into vReturnID
            From AHS_SOTHAM_QUYETDINH_VUAN t inner join AHS_VUAN d on d.ID = t.VUANID
            Where t.DONVIID = vdonviID AND t.DONVIID = t.TOA_GIAIQUYET_ID
              and t.NGAYQD between vFromDate and vToDate
              and ((vLoaiQD in (77,78) and QuyetdinhID in (77,78))
                 or (vLoaiQD in (4,61) and QuyetdinhID in (4,61)) 
                 or (vLoaiQD in (79,80,128) and QuyetdinhID in (79,80,128)) 
    ...
    if vLoaiAn='ADS' then 
      ...
      OPEN curReturn FOR  
        ...
        OPEN curReturn FOR  
            ...
            Where d.TOAANID=vdonviID and t.SOBANAN = vSoBA AND t.TOAANID=t.TOA_GIAIQUYET_ID
              and t.NgayTuyenAn between vFromDate and vToDate;
     elsif vLoaiAn='XLHC_PT' then 
        OPEN curReturn FOR  
            Select t.ID into vReturnID
            From XLHC_PHUCTHAM_BANAN t inner join XLHC_DON d on d.ID=t.DONID
            Where d.TOAPHUCTHAMID=vdonviID and t.SOBANAN = vSoBA AND t.TOAANID=t.TOA_GIAIQUYET_ID
              and t.NgayTuyenAn between vFromDate and vToDate;  
  end if;      
...
BEGIN  
    if vLoaiAn='ADS' then 
      ...
      OPEN curReturn FOR  
          ...
          Where t.TOAANID=vdonviID AND t.TOAANID=t.TOA_GIAIQUYET_ID and t.NgayTuyenAn between vFromDate and vToDate;
...
BEGIN  
    if vLoaiAn='ADS' then 
      OPEN curReturn FOR  
          ...
          Where t.TOAANID=vdonviID and t.sothuly = vSoThuLy AND t.TOAANID = t.TOA_GIAIQUYET_ID
      ...
      OPEN curReturn FOR  
          ...
          Where t.TOAANID=vdonviID and t.sothuly = vSoThuLy AND t.TOAANID=t.TOA_GIAIQUYET_ID 
            AND t.TOAANID=t.TOA_GIAIQUYET_ID 
            and t.NGAYTHULY between vFromDate and vToDate;
     --toancau_linhnd
     elsif vLoaiAn='ADS_PTQDK' then 
      OPEN curReturn FOR  
          Select t.ID into vReturnID
          From ADS_KCKNQDK_PHUCTHAM_THULY t
          Where t.TOAANID=vdonviID and t.sothuly = vSoThuLy AND t.TOAANID=t.TOA_GIAIQUYET_ID 
            and t.NGAYTHULY between vFromDate and vToDate;       
        ...
        OPEN curReturn FOR  
            ...
            Where exists( select 'x' from AHS_VUAN d where d.ID=t.VUANID and d.TOAANID=vdonviID AND d.TOAANID = d.TOA_GIAIQUYET_ID) and t.sothuly = vSoThuLy
            and t.NGAYTHULY between vFromDate and vToDate;
...
--NVL(MAX(d.STT),0)
    if vLoaiAn='ADS' then 
      OPEN curReturn FOR  
          ...
          Where d.TOAANID=vdonviID and t.NGAYTHULY between vFromDate and vToDate AND d.TOAANID=d.TOA_GIAIQUYET_ID;
    elsif vLoaiAn='ADS_PT' then 
      OPEN curReturn FOR  
          Select  NVL(MAX(TO_NUMBER(regexp_replace(t.sothuly,'[^0-9]'))),0) into vReturnID
          From ADS_PHUCTHAM_THULY t 
          Where t.TOAANID=vdonviID AND t.TOAANID=t.TOA_GIAIQUYET_ID  and t.NGAYTHULY between vFromDate and vToDate;
     ...
     --toancau-anhnt
        ...
        OPEN curReturn FOR  
            Select  NVL(MAX(TO_NUMBER(regexp_replace(t.sothuly,'[^0-9]'))),0) into vReturnID
            From AHS_SOTHAM_THULY t 
            Where exists( select 'x' from AHS_VUAN d where d.ID=t.VUANID and d.TOAANID=vdonviID AND d.TOAANID=d.TOA_GIAIQUYET_ID) and t.NGAYTHULY between vFromDate and vToDate;
     elsif vLoaiAn='AHS' and v_thanhnien=1 then 
        OPEN curReturn FOR  
            Select  NVL(MAX(TO_NUMBER(regexp_replace(t.sothuly,'[^0-9]'))),0) into vReturnID
            From AHS_SOTHAM_THULY t 
      ...
      OPEN curReturn FOR  
          Select  NVL(MAX(TO_NUMBER(regexp_replace(t.sothuly,'[^0-9]'))),0)  into vReturnID
          From AHN_SOTHAM_THULY t
          Where t.TOAANID=vdonviID and t.NGAYTHULY between vFromDate and vToDate AND t.TOAANID=t.TOA_GIAIQUYET_ID;
     elsif vLoaiAn='AHN_PT' then 
      OPEN curReturn FOR  
          Select  NVL(MAX(TO_NUMBER(regexp_replace(t.sothuly,'[^0-9]'))),0)  into vReturnID
          From AHN_PHUCTHAM_THULY t
...
BEGIN  
    if vLoaiAn='ADS' then 
      OPEN curReturn FOR  
          Select t.ID into vReturnID
          From ADS_SOTHAM_THULY t
          Where t.TOAANID=vdonviID and t.sothongbao = vSothongbao AND t.TOAANID=t.TOA_GIAIQUYET_ID
            and t.NGAYTHULY between vFromDate and vToDate;
    elsif vLoaiAn='ADS_PT' then 
      OPEN curReturn FOR  
          Select t.ID into vReturnID
...
-- vnpt them dieu kien neu so thong bao qua lon (case string ve so bi loi) tra ve khong
...
--NVL(MAX(d.STT),0)
    if vLoaiAn='ADS' then 
          ...
          From ADS_SOTHAM_THULY t
          Where t.TOAANID=vdonviID and t.NGAYTHONGBAO between vFromDate and vToDate and t.TOAANID= t.TOA_GIAIQUYET_ID;
    elsif vLoaiAn='ADS_PT' then 
          Select  NVL(MAX(TO_NUMBER(regexp_replace(t.sothongbao,'[^0-9]'))),0) into vReturnID
          From ADS_PHUCTHAM_THULY t
          Where t.TOAANID=vdonviID AND t.TOAANID=t.TOA_GIAIQUYET_ID  and t.NGAYTHONGBAO between vFromDate and vToDate ;
    elsif vLoaiAn='AHN' then 
          Select  NVL(MAX(TO_NUMBER(regexp_replace(t.sothongbao,'[^0-9]'))),0) into vReturnID
          From AHN_SOTHAM_THULY t 
          Where t.TOAANID=vdonviID and t.NGAYTHONGBAO between vFromDate and vToDate and t.TOAANID= t.TOA_GIAIQUYET_ID;
    ...
    IF vReturnID > 10000000 THEN vReturnID := -1;
    END IF;

    OPEN curReturn FOR select vReturnID from dual;

--    v_output := 'finish: vReturnID=' || vReturnID;
--		        PKG_TRACELOG.SP_INSERT_LOG_INFO(
--		            p_functionname => 'PKG_STPT_QLCS.QLA_ST_PT_STBTL_GETMAXTT',
...
PROCEDURE   QLA_ST_PT_STB_XLDON_GETMAXTT_V2
    ...
    IS
      vReturnID number;
		v_Max_ADS_DON_XULY NUMBER;
		v_Max_DON_YEUCAUBOSUNG_DS NUMBER;
    BEGIN  
    --NVL(MAX(d.STT),0)
        if vLoaiAn='ADS' then 
        	Select NVL(MAX(TO_NUMBER(regexp_replace(t.SOTHONGBAO,'[^0-9]'))),0) into v_Max_ADS_DON_XULY
              From ADS_DON_XULY t
              Where 1=1 AND t.TOAANID=vdonviID and t.NGAYTHONGBAO between vFromDate and vToDate;

			Select NVL(MAX(TO_NUMBER(regexp_replace(t.SOTHONGBAO,'[^0-9]'))),0) into v_Max_DON_YEUCAUBOSUNG_DS
              From DON_YEUCAUBOSUNG t
              Where 1=1 AND t.LOAIAN=2 
			AND t.TOAANID=vdonviID and t.NGAYTHONGBAO between vFromDate and vToDate;
          OPEN curReturn FOR  
              Select Greatest(v_Max_ADS_DON_XULY, v_Max_DON_YEUCAUBOSUNG_DS) INTO vReturnID FROM DUAL;
        elsif vLoaiAn='AHN' then 
              Select  NVL(MAX(TO_NUMBER(regexp_replace(t.SOTHONGBAO,'[^0-9]'))),0) into v_Max_ADS_DON_XULY
              From AHN_DON_XULY t 
              Where t.TOAANID=vdonviID and t.NGAYTHONGBAO between vFromDate and vToDate;
...
END QLA_ST_PT_STB_XLDON_GETMAXTT_V2;
 -- BPXL HC
-- QLA_ST_PT_CheckSoThongBaoThuLy_V2
...
END QLA_ST_PT_CheckSoThongBaoThuLy_V2;
...
END QLA_ST_PT_STBTL_GETMAXTT_V2;
