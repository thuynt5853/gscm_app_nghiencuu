CREATE OR REPLACE PROCEDURE GSCM."ALD_SOTHAM_KCAOKNGHI_GETLIST" 
( vDonID in number,
...
--        ,s.TENDUONGSU as NguoiKCCapKN
  ...
  left join (select a.ID,a.TENDUONGSU||' -- '|| i.ten as TENDUONGSU from ALD_DON_DUONGSU a 
                                left join DM_DATAITEM i on i.MA=a.TUCACHTOTUNG_MA
                                    where a.DONID=vDonID) s on s.ID=d.DUONGSUID
  left join (select l.ID,l.HOTEN||' -- '|| i.ten as TENKC from ALD_DON_THAMGIATOTUNG l 
...
END ALD_SOTHAM_KCaoKNghi_GETLIST;

--create or replace NONEDITIONABLE PROCEDURE        "ALD_SOTHAM_KCAOKNGHI_GETLIST" 
--( vDonID in number,
--	curReturn OUT sys_refcursor
--)
--IS 
...
--                              WHEN 1 then 'Qua bưu điện' End) as HTNhanDonDonViKN
----        ,s.TENDUONGSU as NguoiKCCapKN
--        ,(decode(s.TENDUONGSU,null,tgtt.TENKC,s.TENDUONGSU) ||''|| decode(d.ISQUAHAN,1,decode(d.GQ_TINHTRANG,1,'<br>(Kháng cao quá hạn: Đã duyệt)','<br>(Kháng cao quá hạn: Chờ duyệt)'),'')) as NguoiKCCapKN
--        ,(CASE d.LOAIKHANGCAO WHEN 0 THEN 'Bản án'  WHEN 1 THEN 'Quyết định' ELSE 'Quyết định khác' END) as LoaiKCKN
--        ,d.NGAYKHANGCAO as NgayKCKN
--        ,(CASE d.LOAIKHANGCAO WHEN 0 THEN b.SOBANAN Else q.SOQD END) as SO_QDBA
--        ,d.NGAYQDBA as NGAYQDBA
...
--  left join (select l.ID,l.HOTEN||' -- '|| i.ten as TENKC from ALD_DON_THAMGIATOTUNG l 
--                            left join DM_DATAITEM i on i.MA=l.TUCACHTGTTID   
--                                    where l.DONID=vDonID) tgtt on tgtt.ID =d.DuongSuID 
--  left join (select e.ID,e.SOBANAN from ALD_SOTHAM_BANAN e where e.DONID=vDonID) b on b.ID=d.SOQDBA
--  left join (select f.ID,f.SOQD from ALD_SOTHAM_QUYETDINH f where f.DONID=vDonID) q on q.ID=d.SOQDBA
--  Where d.DONID=vDonID
--  union
