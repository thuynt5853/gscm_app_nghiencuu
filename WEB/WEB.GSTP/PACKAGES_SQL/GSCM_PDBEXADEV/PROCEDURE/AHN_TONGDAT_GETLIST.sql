create or replace NONEDITIONABLE PROCEDURE AHN_TONGDAT_GETLIST
( vDONID in number,
  vToaAnID in number,
...
BEGIN
select MAGIAIDOAN into vGiaiDoan from AHN_DON where ID=vDONID;
OPEN curReturn FOR  
  Select t.*,(b.MABM || '. ' || b.TENBM) TENBM,
    AHN_GET_VBTONGDAT_DUONGSU(vDONID,b.ID,t.ID) as TINHTRANGNHANVB
  From AHN_TONGDAT t   
...
--                             when vGiaiDoan=4 and a.ISGDTTT=1 then 1 else 0 end
  ...
  --inner join AHN_FILE f on t.FILEID=f.ID and f.MAGIAIDOAN=vGiaiDoan
  Where t.DONID=vDONID and t.TOAANID=vToaAnID
  ORder by b.THUTU;
END AHN_TONGDAT_GETLIST;