CREATE OR REPLACE PROCEDURE GSCM."ALD_PHUCTHAM_TGTT_GETLIST" 
( vDONID in number,
...
OPEN curReturn FOR  
  Select d.ID,d.HOTEN
      ...
      ,(cb.hoten|| '-'||d.chucvu_chucdanh) as chucvuchucdanh --duongph
        ...
        ,(select LISTAGG(TENDUONGSU, '<br/>') WITHIN GROUP (ORDER BY ID) AS description from ALD_DON_DUONGSU a where d.DUONGSUID like '%,'||a.ID||',%') TENDUONGSU
        , d.TOA_GIAIQUYET_ID
  From ALD_PHUCTHAM_THAMGIATOTUNG d 
