create or replace NONEDITIONABLE PROCEDURE        "AHN_SOTHAM_KCAOKNGHI_GETLIST" 
( vDonID in number,
...
--        ,s.TENDUONGSU as NguoiKCCapKN
  ...
  left join (select a.ID,a.TENDUONGSU||' -- '|| i.ten as TENDUONGSU from AHN_DON_DUONGSU a 
                                left join DM_DATAITEM i on i.MA=a.TUCACHTOTUNG_MA
                                    where a.DONID=vDonID) s on s.ID=d.DUONGSUID
  left join (select l.ID,l.HOTEN||' -- '|| i.ten as TENKC from AHN_DON_THAMGIATOTUNG l 
...
END AHN_SOTHAM_KCaoKNghi_GETLIST;
