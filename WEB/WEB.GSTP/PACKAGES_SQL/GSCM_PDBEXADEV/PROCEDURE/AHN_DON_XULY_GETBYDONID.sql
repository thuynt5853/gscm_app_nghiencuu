create or replace NONEDITIONABLE PROCEDURE        "AHN_DON_XULY_GETBYDONID" 
(
   CurrDonID in int,   
	 PageIndex	in	int,
	 PageSize	in	int,
...
BEGIN	
    ...
    from AHN_DON_XULY a where a.DonID =CurrDonID;
		---------------------------------------------------
    OPEN curReturn FOR 
			...
			from (select ROWNUM  stt,g.ID, g.DonID, g.DON_CHITIETID, g.DON_GOCID
                ...
                , g.NgaySua, g.NguoiSua,g.TENFILE,g.FILEID, g.LOAIDON, g.TOA_GIAIQUYET_ID
                    from (select a.ID, t.ID as DONID, a.DON_CHITIETID, t.ID as DON_GOCID
                        , a.LoaiGiaiQuyet   
                        , to_char('Tên nguyên đơn: ' || d.tenduongsu || '<br>Địa chỉ: ' ||decode(d.tamtruchitiet,null,'',d.tamtruchitiet || ',')|| (CASE d.LOAIDUONGSU WHEN 1 THEN h1.MA_TEN Else h2.MA_TEN END) 
                        ...
                        , t.TOA_GIAIQUYET_ID
                      from AHN_DON t
                        left join AHN_DON_XULY a on t.ID = a.DONID -- lay noi dung don
                        left join DM_ToaAn b on a.CDTN_ToaAnID = b.ID
                        left join AHN_FILE f on a.FILEID=f.ID
                        left join AHN_DON_DUONGSU d on d.DONID = t.ID and d.isdaidien=1 AND d.TUCACHTOTUNG_MA='NGUYENDON' -- lay thong tin duong su đơn gốc
                        left join DM_HANHCHINH h1 on h1.ID=d.TAMTRUID
                        left join DM_HANHCHINH h2 on h2.ID=d.NDD_DIACHIID

                        left join AHN_DON_THAMPHAN tp on tp.DONID = t.ID and tp.MAVAITRO = 'VTTP_GIAIQUYETDON' -- lay thong tin tham phan giai quyet
                        left join DM_CANBO c1 on c1.ID=tp.CANBOID
                      where t.ID =CurrDonID  
                      union all
                      select a.ID, a.DonID, c.ID as DON_CHITIETID, t.ID as DON_GOCID
                        , a.LoaiGiaiQuyet   
                        , to_char(DECODE(c.LOAIDON, 8, 'Người khởi kiện: ' || ds.TENNGUOINOP, 9, 'Người khởi kiện: ' || ds.TENNGUOINOP, nd.nguyendon)
                        ...
                        ||case when c.LOAIDON =1 then '<br>Loại đơn: <b>Đơn khởi kiện</b>'
                              ...
                              when c.LOAIDON =3 then '<br>Loại đơn: <b>Đơn từ Cơ quan quản lý nhà nước về gia đình</b>' 
                              when c.LOAIDON =4 then '<br>Loại đơn: <b>Đơn từ Cơ quan quản lý nhà nước về trẻ em</b>' 
                              when c.LOAIDON =5 then '<br>Loại đơn: <b>Đơn từ Hội liên hiệp phụ nữ</b>' 
                              when c.LOAIDON =6 then '<br>Loại đơn: <b>Đơn trùng</b>' 
                              when c.LOAIDON =7 then '<br>Loại đơn: <b>Đơn không thuộc thẩm quyền</b>' 
                              when c.LOAIDON =8 then '<br>Loại đơn: <b>Đơn có yêu cầu phản tố</b>' 
                              when c.LOAIDON =9 then '<br>Loại đơn: <b>Đơn có yêu cầu độc lập</b>' 
                         end)
                        as duongsu -- lay them duong su
                        , t.noidungkhoikien as NoiDung
                         ,(case when a.LoaiGiaiQuyet =1 then u'Chuy\1ec3n \0111\01a1n trong ng\00e0nh'
                              when a.LoaiGiaiQuyet =2 then u'Chuy\1ec3n \0111\01a1n ngo\00e0i ng\00e0nh' 
                              when a.LoaiGiaiQuyet =3 then u'Tr\1ea3 l\1ea1i \0111\01a1n' 
                        ...
                        , a.TraDon_CanCuID
                        , a.NgayTao, a.NguoiTao
                        , a.NgaySua, a.NguoiSua,f.TENFILE,a.FILEID, c.LOAIDON,c.TOA_GIAIQUYET_ID
                      from DON_CHITIET c
                        left join AHN_DON t on c.DONID = t.ID
                        left join AHN_DON_XULY a on c.ID = a.DON_CHITIETID --lay noi dung don
                        left join DM_ToaAn b on a.CDTN_ToaAnID = b.ID
                        left join AHN_FILE f on a.FILEID=f.ID                              
                        left join AHN_DON_THAMPHAN tp on tp.DONID = t.ID and tp.MAVAITRO = 'VTTP_GIAIQUYETDON' -- lay thong tin tham phan giai quyet
                        left join DM_CANBO c1 on c1.ID=tp.CANBOID
                        left join (select * from (select ct.donchitietid, 'Tên nguyên đơn: ' || d.TENDUONGSU || '<br>Địa chỉ: ' ||decode(d.tamtruchitiet,null,'',d.tamtruchitiet || ',')|| (CASE d.LOAIDUONGSU WHEN 1 THEN h1.MA_TEN Else h2.MA_TEN END) as nguyendon 
                            from AHN_DON_DUONGSU d
                            left join don_duongsu_chitiet ct on d.ID = ct.DUONGSUID and ct.loaian=3
                            left join DM_HANHCHINH h1 on h1.ID=d.TAMTRUID
                            left join DM_HANHCHINH h2 on h2.ID=d.NDD_DIACHIID
                        ...
                        left join (SELECT 
                            ...
                            LISTAGG(TENDUONGSU, ', ') WITHIN GROUP (ORDER BY TENDUONGSU) "TENNGUOINOP"
                                FROM (
                                    ...
                                    where a.donid = CurrDonID and c.LOAIANID = 3 and c.LOAIDON IN (8,9)
                                )
                            GROUP BY donchitietid  ) DS on ds.donchitietid = c.id
                      where c.DONID =CurrDonID AND c.LOAIANID = 3 
                      order by NgayTao desc) g
				    ) a where a.stt>=MinIndex and a.stt<=MaxIndex;
END AHN_DON_XULY_GETBYDONID;
