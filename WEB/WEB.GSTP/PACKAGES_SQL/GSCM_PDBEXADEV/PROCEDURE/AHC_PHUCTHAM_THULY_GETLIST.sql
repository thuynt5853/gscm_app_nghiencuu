CREATE OR REPLACE PROCEDURE GSCM."AHC_PHUCTHAM_THULY_GETLIST" 
( vDONID in number,
...
BEGIN
  select ID into vGroupTHGiaoNhan from DM_DATAGROUP where MA='TRUONGHOP_GIAONHAN';
   select count(ID) into CheckBanAnPT from AHC_PHUCTHAM_BANAN where DonID = vDONID; 
   select count(ID) into CheckPhanCongTP from AHC_DON_THAMPHAN where DonID = vDONID and mavaitro like 'VTTP_GIAIQUYETPHUCTHAM'; 
   select count(ID) into CheckNguoiTienHanhTT from AHC_PHUCTHAM_HDXX where DonID = vDONID;
OPEN curReturn FOR  
...
END AHC_PHUCTHAM_THULY_GETLIST;