CREATE OR REPLACE PROCEDURE GSCM."ALD_DON_DSDUONGSU_GETBY" 
( vDONID in number,
...
Where d.ISDON=1 and (d.DONID=vDONID OR d.DONID IN (SELECT ID FROM ALD_DON WHERE VUANGOCID=vDONID AND IS_TACHAN IS NULL))
ORder by d.ISDAIDIEN desc, d.TENDUONGSU;
END ALD_DON_DSDUONGSU_GETBY;