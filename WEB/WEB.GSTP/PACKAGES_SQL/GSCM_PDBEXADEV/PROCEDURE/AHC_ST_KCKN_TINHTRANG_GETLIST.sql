CREATE OR REPLACE PROCEDURE GSCM."AHC_ST_KCKN_TINHTRANG_GETLIST" 
( vDonID in number,
...
--        ,d.NGAYQDBA as NGAYQDBA
        ...
        ,r.TRANGTHAI
        ,r.NOIDUNGRUT
        ,D.TINHTRANG_GIAIQUYET --toancau lấy thêm trường
  ...
  left join (select g.ID,g.IDKCKN,g.NGAYRUT,g.TRANGTHAI,g.NOIDUNGRUT, g.TOA_GIAIQUYET_ID from AHC_SOTHAM_RUTKCKN g where g.DONID=vDonID and g.ISKCKN=vKhangCao) r on r.IDKCKN=d.ID
  left join (select a.ID,a.TENDUONGSU from AHC_DON_DUONGSU a where a.DONID=vDonID) s on s.ID=d.DUONGSUID
...
--        ,(case r.TRANGTHAI when 1 then 'Rút một phần' when 2 then 'Rút toàn bộ'  else 'Chưa rút' end) as TinhTrangName
  ...
  left join (select g.ID,g.IDKCKN,g.NGAYRUT,g.TRANGTHAI,g.NOIDUNGRUT, g.TOA_GIAIQUYET_ID from AHC_SOTHAM_RUTKCKN g where g.DONID=vDonID and g.ISKCKN=vKhangNghi) r on r.IDKCKN=d.ID
...
END AHC_ST_KCKN_TINHTRANG_GETLIST;