create or replace NONEDITIONABLE PROCEDURE        "AHN_DON_ANPHI_GETBYDONID" 
(
   CurrDonID in int,   
   V_TINHTRANG in int,   
	 PageIndex	in	int,
...
BEGIN	
    ...
    from AHN_ANPHI a where a.DonID =CurrDonID and ( V_TINHTRANG = 1 OR (V_TINHTRANG = 0 and  a.TINHTRANG = 0));
		---------------------------------------------------
    OPEN curReturn FOR 
...
END AHN_DON_ANPHI_GETBYDONID;

-- 5. AHN_DON_ANPHI_LICHSU_GETBYDONID