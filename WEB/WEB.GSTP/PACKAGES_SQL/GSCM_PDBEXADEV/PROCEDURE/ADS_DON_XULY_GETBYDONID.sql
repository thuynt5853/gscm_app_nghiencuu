CREATE OR REPLACE PROCEDURE GSCM."ADS_DON_XULY_GETBYDONID" 
(
   CurrDonID in int,   
	 PageIndex	in	int,
	 PageSize	in	int,
...
BEGIN	
    ...
    from ADS_DON_XULY a where a.DonID =CurrDonID;
		---------------------------------------------------
    OPEN curReturn FOR 
			...
			from (select ROWNUM  stt,g.ID, g.DonID, g.DON_CHITIETID, g.DON_GOCID
                ...
                , g.NgaySua, g.NguoiSua,g.TENFILE,g.FILEID, g.LOAIDON,g.TOA_GIAIQUYET_ID
                    from (select a.ID, t.ID as DONID, a.DON_CHITIETID, t.ID as DON_GOCID
                        ...
                        , a.NgaySua, a.NguoiSua,f.TENFILE,a.FILEID, 0 as LOAIDON,a.TOA_GIAIQUYET_ID
                      from ADS_DON t
                        left join ADS_DON_XULY a on t.ID = a.DONID --lanh lay noi dung don
                        left join DM_ToaAn b on a.CDTN_ToaAnID = b.ID
                        left join ADS_FILE f on a.FILEID=f.ID
                        left join ADS_DON_DUONGSU d on d.DONID = t.ID and d.isdaidien=1 AND d.TUCACHTOTUNG_MA='NGUYENDON' -- lanh lay thong tin duong su đơn gốc
                      ...
                      from DON_CHITIET c
                        left join ADS_DON t on c.DONID = t.ID
                        left join ADS_DON_XULY a on c.ID = a.DON_CHITIETID --lanh lay noi dung don
                        left join DM_ToaAn b on a.CDTN_ToaAnID = b.ID
                        left join ADS_FILE f on a.FILEID=f.ID                              
                        left join ADS_DON_THAMPHAN tp on tp.DONID = t.ID and tp.MAVAITRO = 'VTTP_GIAIQUYETDON' -- lanh lay thong tin tham phan giai quyet
                        left join DM_CANBO c1 on c1.ID=tp.CANBOID
                        left join (select * from (select ct.donchitietid, 'Tên nguyên đơn: ' || d.TENDUONGSU || '<br>Địa chỉ: ' ||decode(d.tamtruchitiet,null,'',d.tamtruchitiet || ',')|| (CASE d.LOAIDUONGSU WHEN 1 THEN h1.MA_TEN Else h2.MA_TEN END) as nguyendon 
                            ...
                            left join don_duongsu_chitiet ct on d.ID = ct.DUONGSUID and ct.loaian=2
                            left join DM_HANHCHINH h1 on h1.ID=d.TAMTRUID
                            left join DM_HANHCHINH h2 on h2.ID=d.NDD_DIACHIID
                        ...
                        left join (SELECT 
                            ...
                            LISTAGG(TENDUONGSU, ', ') WITHIN GROUP (ORDER BY TENDUONGSU) "TENNGUOINOP"
                                FROM (
                                    ...
                                    where a.donid = CurrDonID and c.LOAIANID = 2 and c.LOAIDON IN (5,6)
                                )
                            GROUP BY donchitietid  ) DS on ds.donchitietid = c.id
                      where c.DONID =CurrDonID AND c.LOAIANID = 2
                      order by NgayTao desc) g
				    ) a where a.stt>=MinIndex and a.stt<=MaxIndex order by NgayTao desc;
END ADS_DON_XULY_GETBYDONID;