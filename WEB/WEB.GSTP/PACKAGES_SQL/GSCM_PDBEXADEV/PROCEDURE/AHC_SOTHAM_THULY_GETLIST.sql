CREATE OR REPLACE PROCEDURE GSCM."AHC_SOTHAM_THULY_GETLIST" 
( vDONID in number,
...
BEGIN
   select count(ID) into CheckBanAnST from AHC_SOTHAM_BANAN where DonID = vDONID; 
   select count(ID) into CheckPhanCongTP from AHC_DON_THAMPHAN where DonID = vDONID and mavaitro like 'VTTP_GIAIQUYETSOTHAM'; 
   select count(ID) into CheckNguoiTienHanhTT from AHC_SOTHAM_HDXX where DonID = vDONID;
  OPEN curReturn FOR  
    ...
    left join AHC_FILE f on t.FILEID=f.ID
