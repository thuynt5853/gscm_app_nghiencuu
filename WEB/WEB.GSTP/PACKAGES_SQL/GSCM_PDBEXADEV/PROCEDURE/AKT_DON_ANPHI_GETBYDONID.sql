create or replace NONEDITIONABLE PROCEDURE "AKT_DON_ANPHI_GETBYDONID" 
(
   CurrDonID in int,   
   V_TINHTRANG in int,   
	 PageIndex	in	int,
...
--                , d.tenduongsu || '-' || di.TEN as duongsu
                ...
                , a.TOA_GIAIQUYET_ID
              from AKT_ANPHI a
--              left join A_DON_DUONGSU d on d.ID = a.DUONGSU_IDS   -- VNPT - Lê BÁ Thọ lấy ra đương sự thanh toán án phí 19/11/2025
--              left join DM_DATAITEM di on di.MA=d.TUCACHTOTUNG_MA
              ...
              a.sobienlai is not null and --vnpt le ba tho them dieu kien 11-12-2025
              --VNPT Lê Bá Thọ bỏ and ( a.sobienlai is not null or t.trangthaithanhtoan = 1) để hiển thị bảng án phí
                ( V_TINHTRANG = 1 OR (V_TINHTRANG = 0 and a.TINHTRANG = 0 ) )
