CREATE OR REPLACE PROCEDURE GSCM."ADS_SOTHAM_KCAOKNGHI_GETLIST" 
( vDonID in number,
...
--        ,s.TENDUONGSU as NguoiKCCapKN
  ...
  left join (select a.ID,a.TENDUONGSU||' -- '|| i.ten as TENDUONGSU from ADS_DON_DUONGSU a 
                                left join DM_DATAITEM i on i.MA=a.TUCACHTOTUNG_MA
                                    where a.DONID=vDonID) s on s.ID=d.DUONGSUID
  left join (select l.ID,l.HOTEN||' -- '|| i.ten as TENKC from ADS_DON_THAMGIATOTUNG l 
...
END ADS_SOTHAM_KCaoKNghi_GETLIST ;

-- Bản cũ trước khi toàn cầu sửa yêu cầu về tạm đình chỉ (xóa sau khi đã test xong) 
--create or replace NONEDITIONABLE PROCEDURE        "ADS_SOTHAM_KCAOKNGHI_GETLIST" 
--( vDonID in number,
--	curReturn OUT sys_refcursor
--)
--IS 
