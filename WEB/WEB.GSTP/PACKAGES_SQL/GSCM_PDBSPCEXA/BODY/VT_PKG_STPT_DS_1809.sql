--------------------------------------------------------
--  DDL for Package Body VT_PKG_STPT_DS_1809
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "GSCM"."VT_PKG_STPT_DS_1809" AS
...
--      GN.TRUONGHOPGIAONHAN,
        ...
        STBA.BANAN_QD_ST,'' as QD_PT,STKN.KHANGNGHI_ST, PKG_STPT_ADS_GS.NOIDUNG_KHANGCAO_DANHSACH(a.ID) as KHANGCAO_ST, --toancau 18092023-anhnt thêm trường QD_PT mặc định trống--STBA.BANAN_QD_ST,'' as QD_PT,STKN.KHANGNGHI_ST,STKC.KHANGCAO_ST, --toancau-anhnt thêm trường QD_PT mặc định trống
      ...
      INNER JOIN (SELECT G.* FROM ADS_DON_GIAIDOAN G WHERE (G.MAGIAIDOAN = 2 AND G.TOAANID = v_toaan_id) OR (G.MAGIAIDOAN = 3 AND G.TOAPHUCTHAMID = v_toaan_id)) GD ON A.ID=GD.DONID --toancau lay 1 giai doan duy nhat
...
------ - TOANCAU lấy thông tin số ngày kháng cáo + đương sự toancau
--        LEFT JOIN ( SELECT
...
--                  ) STKC ON STKC.DONID = A.ID
        ...
        (V_TEN_VU_AN IS NULL  OR ( LOWER(A.TENVUVIEC) LIKE  '%'||LOWER(V_TEN_VU_AN)||'%' ) )--Tên vụ án
...
--      GN.TRUONGHOPGIAONHAN,
      ...
      DECODE(A.HINHTHUCNHANDON,1,'<br/><i>TH giao nhận:</i> <b>Thụ lý mới</b>',
                                    270, '<br/><i>TH giao nhận:</i> <b>Xét xử lại cấp sơ thẩm</b>',
                                    '<br/><i>TH giao nhận:</i> <b>'|| GN.TruongHopGiaoNhan||'</b>') TRUONGHOPGIAONHAN,   
      STBA.BANAN_QD_ST,'' as QD_PT,STKN.KHANGNGHI_ST, PKG_STPT_AHN_GS.NOIDUNG_KHANGCAO_DANHSACH(a.ID) as KHANGCAO_ST,--toancau 18092023-anhnt thêm trường QD_PT mặc định trống--STBA.BANAN_QD_ST,STKN.KHANGNGHI_ST,'' QD_PT,STKC.KHANGCAO_ST,  --toancau-tamnc thêm trường QD_PT mặc định trống
      A.MAGIAIDOAN,(BC3.HoTen||BC2.HoTen)HOTENBICAN ,
      DECODE(XLD.LOAIGIAIQUYET,1,'- Đã chuyển đơn',CASE  WHEN (TLS.TINHTRANG_GQ || TLPT.TINHTRANG_GQ) IS NULL THEN '- Chưa thụ lý' 
                     ELSE (TLS.TINHTRANG_GQ || TLPT.TINHTRANG_GQ) 
             END  ||
            ...
            DECODE(BA.ID,null,DECODE(QD.ID,NULL,NULL,3),3) THULYXXLAI
      FROM AHN_DON A
      INNER JOIN (SELECT G.* FROM AHN_DON_GIAIDOAN G WHERE (G.MAGIAIDOAN = 2 AND G.TOAANID = v_toaan_id) OR (G.MAGIAIDOAN = 3 AND G.TOAPHUCTHAMID = v_toaan_id)) GD ON A.ID=GD.DONID --toancau lay 1 giai doan duy nhat
      left join DM_DATAITEM i on A.QUANHEPHAPLUATID=i.ID
      LEFT JOIN DM_TOAAN T ON A.TOAANID=T.ID
      ----- BA Or QD----------------------------------------------
...
--      GN.TRUONGHOPGIAONHAN,
      ...
      --STBA.BANAN_QD_ST,STKN.KHANGNGHI_ST,'' QD_PT,STKC.KHANGCAO_ST,
      A.MAGIAIDOAN,(BC3.HoTen||BC2.HoTen)HOTENBICAN ,
      DECODE(XLD.LOAIGIAIQUYET,1,'- Đã chuyển đơn',CASE  WHEN (TLS.TINHTRANG_GQ || TLPT.TINHTRANG_GQ) IS NULL THEN '- Chưa thụ lý' 
                                                ELSE (TLS.TINHTRANG_GQ || TLPT.TINHTRANG_GQ) 
                                                END  ||
      ...
      left join AKT_SOTHAM_THULY conTL ON A.ID = conTL.DONID
      left join DM_DATAITEM i on A.QUANHEPHAPLUATID=i.ID
      LEFT JOIN DM_TOAAN T ON A.TOAANID=T.ID
      ----- BA Or QD----------------------------------------------
...
--      STBA.BANAN_QD_ST,STKN.KHANGNGHI_ST,'' QD_PT,STKC.KHANGCAO_ST, --toancau-tamnc thêm trường QD_PT mặc định trống
      A.MAGIAIDOAN,(BC3.HoTen||BC2.HoTen)HOTENBICAN ,
      DECODE(XLD.LOAIGIAIQUYET,1,'- Đã chuyển đơn',CASE  WHEN (TLS.TINHTRANG_GQ || TLPT.TINHTRANG_GQ) IS NULL THEN '- Chưa thụ lý' 
                     ELSE (TLS.TINHTRANG_GQ || TLPT.TINHTRANG_GQ) 
             END  ||
...
--      GN.TRUONGHOPGIAONHAN,
        ...
        --STBA.BANAN_QD_ST,'' as QD_PT,STKN.KHANGNGHI_ST,STKC.KHANGCAO_ST, --toancau-anhnt thêm trường QD_PT mặc định trống
        A.MAGIAIDOAN,(BC3.HoTen||BC2.HoTen)HOTENBICAN ,
        DECODE(XLD.LOAIGIAIQUYET,1,'- Đã chuyển đơn',CASE  WHEN (TLS.TINHTRANG_GQ || TLPT.TINHTRANG_GQ) IS NULL THEN '- Chưa thụ lý' 
                     ELSE (TLS.TINHTRANG_GQ || TLPT.TINHTRANG_GQ) 
             END  ||
...
--                      )GN ON  GN.VUANID=a.ID
--TOANCAU 01-04-2023 CHỈ LẤY CHUYỂN ÁN CUỐI CÙNG
           LEFT JOIN (SELECT CA.VUANID,i.TEN,CA.ID TRUONGHOPGIAONHAN FROM DM_DATAITEM i 
                  INNER JOIN AHC_CHUYEN_NHAN_AN CA ON CA.TRUONGHOPGIAONHANID=i.ID WHERE CA.TOANHANID=v_toaan_id 
                  ORDER BY CA.ID DESC
...
IS 
  ...
  HOSO number DEFAULT 1;  SOTHAM number DEFAULT 2;  PHUCTHAM number DEFAULT 3;
  VV_NGAYTHULY_TU DATE;  VV_NGAYTHULY_DEN DATE;
  V_TABLE_TLST T_QUYETDINH_EXT;  V_TABLE_TLPT T_QUYETDINH_EXT;
  V_TABLE_TP T_QUYETDINH_EXT;
  V_TABLE_HDXX_PT T_QUYETDINH_EXT;  V_TABLE_HDXX_ST T_QUYETDINH_EXT;
  VV_TUNGAY_GQ date;  VV_DENNGAY_GQ date;
  V_TABLE_PT T_QUYETDINH;  V_TABLE_ST T_QUYETDINH;
  V_TABLE_BC T_BICANBICAO_EXT;  V_TABLE_BC_KC T_BICANBICAO_EXT;
BEGIN
    -- Giai đoạn vụ án/vụ việc
    -- HOSO = 1;
