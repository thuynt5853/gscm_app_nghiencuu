--------------------------------------------------------
--  DDL for Package Body PKG_STPT_HS
--------------------------------------------------------

  CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "GSCM"."PKG_STPT_HS" AS
...
(
    ...
    v_GDTaoHS in number,
	V_VAITRO_THAMPHAN IN VARCHAR2,--TOANCAU-03102023-ANHNT
    Page_Index in	int,
    Page_Size	in	int, 
...
--            DECODE(GD.MAGIAIDOAN,3,GN.TruongHopGiaoNhan,AA.TruongHopGiaoNhan)TruongHopGiaoNhan,
            ...
            DECODE(BA.ID,null,DECODE(QD.ID,NULL,NULL,3),3) THULYXXLAI, '' AS QD_PT
            FROM AHS_VUAN A
            ...
            LEFT JOIN ( 
--            SELECT TP.VUANID,'</br>- Thẩm phán: <b>'||TO_CHAR(DECODE(CBB.HOTEN,NULL,CB.HOTEN,CBB.HOTEN))||'</b><i> (chủ tọa)</i>' TINHTRANG_GQ
