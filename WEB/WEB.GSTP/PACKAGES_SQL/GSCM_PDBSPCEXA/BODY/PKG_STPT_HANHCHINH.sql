--------------------------------------------------------
--  DDL for Package Body PKG_STPT_HANHCHINH
--------------------------------------------------------

  CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "GSCM"."PKG_STPT_HANHCHINH" AS
      ...
      SELECT R_QUYETDINH_EXT(TTS.DONID,TTS.ID,TTS.MA)
        ...
        FROM(SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER FROM 
                    (  SELECT d.ID,d.DONID,d.TENDUONGSU,d.TUCACHTOTUNG_MA,ROW_NUMBER()  OVER (PARTITION BY d.DONID ORDER BY d.ISDAIDIEN DESC,d.TENDUONGSU) ROWNUMBER
                       FROM AHC_DON_DUONGSU d left join AHC_ANPHI p ON p.duongsu_ids like '%' || d.ID || '%'
...
--      INNER JOIN AHC_DON_GIAIDOAN GD ON A.ID=GD.DONID
    INNER JOIN (SELECT G.* FROM AHC_DON_GIAIDOAN G WHERE (G.MAGIAIDOAN = 2 AND G.TOAANID = v_toaan_id) OR (G.MAGIAIDOAN = 3 AND G.TOAPHUCTHAMID = v_toaan_id)) GD ON A.ID=GD.DONID --Điều kiện để hiển thị 1 bản ghi duy nhất theo giai đoạn xét xử (tránh hiển thị 2 giai đoạn ở ST và PT ở tòa tỉnh) tuanvna
        ...
        WHERE   (V_TEN_VU_AN IS NULL  OR ( LOWER(A.TENVUVIEC) LIKE  '%'||LOWER(V_TEN_VU_AN)||'%' ) )--Tên vụ án
          ...
          -----
           AND (V_SOTHULY IS NULL 
                OR( Exists(Select 'X' from AHC_SOTHAM_THULY where donid = a.id and UPPER(SOTHULY)=UPPER(V_SOTHULY)) 
                    OR Exists(Select 'X' from AHC_SOTHAM_THULY where donid = a.id and UPPER(SOTHULY)=UPPER(V_SOTHULY)) 
                    ))--Số Thụ lý
         -----
          AND (V_THAMPHAN_ID IS NULL--Thẩm phán
             OR( EXISTS(SELECT 'x' FROM AHC_SOTHAM_HDXX PC WHERE   PC.CANBOID = V_THAMPHAN_ID AND MAVAITRO='THAMPHAN'  AND PC.DONID=A.ID))
             OR(EXISTS(SELECT 'x' FROM AHC_DON_THAMPHAN PC WHERE   PC.CANBOID = V_THAMPHAN_ID AND MAVAITRO='VTTP_GIAIQUYETSOTHAM' AND PC.DONID=A.ID) )
           )
            --GQ đơn;V_GQDON -- -- 
          AND (V_GQDON IS NULL 
                                ...
                                SELECT 'X' FROM AHC_SOTHAM_THULY TL
                                  WHERE (NOT EXISTS(SELECT 'X' FROM AHC_SOTHAM_BANAN BA WHERE TL.DONID =BA.DONID)
                                        AND (NOT EXISTS(SELECT T1.DONID,T2.MA FROM AHC_SOTHAM_QUYETDINH T1 
                                                   ...
                                                   (instr(',DC,CVA,CNTT,TRAHS,',','||T2.MA||',')>0
 --                                   Manh them các loại QD gây kết thúc khác
                                                   OR QD.loaiid in (15,10,3) OR QD.id in (422,423,70,425)) AND TL.DONID=T1.DONID) )
                                                    )
                                                 AND (V_TUNGAY IS NULL OR   TL.NGAYTHULY>=VV_TUNGAY)
                                                 AND (V_DENNGAY IS NULL OR TL.NGAYTHULY<=VV_DENNGAY)
                                                 AND TL.DONID=a.id  AND GD.MAGIAIDOAN=2
...
--                  )GN ON  GN.VUANID=a.ID
    ...
    v_table.extend;
                ...
                item_ds.TINHTRANG_GQ,item_ds.CHECK_THULY,'6'
                );   
     END LOOP;
  ----------
  RETURN v_table;   
...
BEGIN
checkXLD := 0;
  if(vID <> 0) then select count(id) into checkXLD from AHC_DON_XULY where ID = vID AND SOTHONGBAO = vSTT AND STB_PHU = vSTB_Phu; end if;
  if(vID = 0 OR checkXLD = 0) then 
    OPEN curReturn FOR   
    Select count(id)
    from (Select t.id From AHC_DON_XULY t 
    left join DON_YEUCAUBOSUNG y on y.DON_XULY_YCBS_ID = t.ID AND y.LOAIAN = 6
    ...
    Where t.LOAIAN = 6 AND t.TOAANID=vdonviID and t.SOTHONGBAO = vSTT and ((vSTB_Phu is null AND t.STB_PHU is null) OR (vSTB_Phu is not null AND t.STB_PHU = vSTB_Phu)) and (vID = 0 OR viD <> t.ID ) and EXTRACT(year FROM t.NGAYTHONGBAO) = vNam);
  else
    OPEN curReturn FOR   
    Select count(id)
    from (Select t.id From AHC_DON_XULY t 
    ...
    union all
    Select t.id From DON_YEUCAUBOSUNG t 
    Where t.LOAIAN = 6 AND t.TOAANID=vdonviID and t.SOTHONGBAO = vSTT and ((vSTB_Phu is null AND t.STB_PHU is null) OR (vSTB_Phu is not null AND t.STB_PHU = vSTB_Phu)) and t.DON_XULY_YCBS_ID <> t.ID and EXTRACT(year FROM t.NGAYTHONGBAO) = vNam);
  end if;
  /*Select count(t.id)
          From AHC_DON_XULY t 
              Where t.TOAANID=vdonviID and t.SOTHONGBAO = vSTT and EXTRACT(year FROM t.NGAYTHONGBAO) = vNam;*/
END AHC_FILE_CHECKSTT_ANPHI;
...
BEGIN
SELECT COUNT(*) INTO V_COUNTS FROM AHC_SOTHAM_THULY TL  WHERE TL.DONID = vDONID AND TL.NGAYTHULY < TO_DATE('01/MAY/2022','dd/mon/yyyy');
IF(V_COUNTS>=1)THEN 
    OPEN curReturn FOR  
        Select d.ID,d.TENDUONGSU
        From AHC_DON_DUONGSU d 
        LEFT JOIN AHC_ANPHI x ON x.DUONGSU_IDS like '%' || d.ID || '%'
