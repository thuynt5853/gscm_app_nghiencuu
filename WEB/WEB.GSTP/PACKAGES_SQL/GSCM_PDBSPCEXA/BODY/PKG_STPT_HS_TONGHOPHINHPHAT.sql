--------------------------------------------------------
--  DDL for Package Body PKG_STPT_HS_TONGHOPHINHPHAT
--------------------------------------------------------

  CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "GSCM"."PKG_STPT_HS_TONGHOPHINHPHAT" AS
...
BEGIN   
        ...
        -------------------------------------- SƠ THẨM ---------------------------------------------------
        PKG_STPT_HS_TONGHOPHINHPHAT.AHS_SOTHAM_HINHPHAT_TH_CT_TG(VBICANID,V_CURSOR);
        LOOP 
        FETCH V_CURSOR 
        ...
        IF (CHECK_INTO > 0 ) THEN
        -------------------------------------- PHÚC THẨM -------------------------------------------------  
        PKG_STPT_HS_TONGHOPHINHPHAT.AHS_PHUCTHAM_HINHPHAT_TH_CT_TG(VBICANID,V_CURSOR);
        LOOP 
        FETCH V_CURSOR 
...
BEGIN
    ...
    IF(V_QD_DINHCHI_VUAN > 0 OR V_QD_DINHCHI_BICAO >0 OR V_QD_TAMDINHCHI_BICAO >0) THEN 
        ...
        ELSE
                ...
                PKG_STPT_HS_TONGHOPHINHPHAT.AHS_Y_AN_SO_THAM(VBICAOID,V_CURSOR); LOOP FETCH V_CURSOR INTO CHECK_Y_AN_SO_THAM; EXIT WHEN V_CURSOR%NOTFOUND; END LOOP; CLOSE V_CURSOR;
                                                        ...
                                                        IF(ITEM.HINHPHATID = 8 OR ITEM.HINHPHATID = 6 OR ITEM.HINHPHATID = 5 OR ITEM.HINHPHATID = 2 OR ITEM.HINHPHATID = 3 OR ITEM.HINHPHATID = 65) THEN
                                                            ...
                                                            IF(CHECK_TH_CT_TG = 0) THEN
                                                               IF (ITEM.HINHPHATID = 6 OR ITEM.HINHPHATID = 8 OR ITEM.HINHPHATID = 5) THEN          
                                                                    PKG_STPT_HS_TONGHOPHINHPHAT.AHS_KQXXPT(VBICAOID,V_CURSOR);
                                                                    LOOP 
                                                                    FETCH V_CURSOR 
                                                                        INTO  HP_TU;
                                                                        EXIT WHEN V_CURSOR%NOTFOUND;
                                                            ...
                                                            IF (ITEM.HINHPHATID = 3 OR ITEM.HINHPHATID = 65) THEN                        
                                                                PKG_STPT_HS_TONGHOPHINHPHAT.AHS_SOSANH_HINHPHAT_THOIGIAN(ITEM.HINHPHATID,VBICAOID,V_CURSOR);
                                                                LOOP 
                                                                FETCH V_CURSOR 
                                                                    INTO  HP_THOIGIAN;
                                                                    EXIT WHEN V_CURSOR%NOTFOUND;
                                                            ...
                                                            IF (ITEM.HINHPHATID = 2) THEN
                                                                PKG_STPT_HS_TONGHOPHINHPHAT.AHS_SOSANH_HINHPHAT_SOHOC(ITEM.HINHPHATID,VBICAOID,V_CURSOR);
                                                                LOOP 
                                                                FETCH V_CURSOR 
                                                                    INTO  HP_SOHOC;
                                                                    EXIT WHEN V_CURSOR%NOTFOUND;
...
BEGIN       
    ...
    select count('x') into V_RESULT_EXPORT_SOBC from ahs_bicanbicao bc where vuanid = VVUANID; 
    
    --Bị cáo kháng cáo
    select listagg(bc.id, ';') within group (order by '') --count('x')
    into BCKHANGCAO from ahs_bicanbicao bc inner join (select ID,nguoikcid from ahs_sotham_khangcao kc left join (select khangcaoid from ahs_sotham_rutkhangcao) rkc on rkc.khangcaoid = kc.id where NGUOIKCLOAI = 0 and rkc.khangcaoid is null) bckc on bckc.nguoikcid = bc.id where vuanid = VVUANID;
        
    --Người tham gia tố tụng kháng cáo 
    select count('x') into TGTTKHANGCAOVA from ahs_nguoithamgiatotung tgtt inner join (select ID,nguoikcid from ahs_sotham_khangcao kc left join (select khangcaoid from ahs_sotham_rutkhangcao) rkc on rkc.khangcaoid = kc.id where NGUOIKCLOAI = 1 and DSNGUOIBIKC is null and rkc.khangcaoid is null) bhkc on bhkc.nguoikcid = tgtt.id where vuanid = VVUANID;        
    select listagg(bhkc.DSNGUOIBIKC, ';') within group (order by '') --count('x') 
    into TGTTKHANGCAOBC from ahs_nguoithamgiatotung tgtt inner join (select ID,nguoikcid,DSNGUOIBIKC from ahs_sotham_khangcao kc left join (select khangcaoid from ahs_sotham_rutkhangcao) rkc on rkc.khangcaoid = kc.id where NGUOIKCLOAI = 1 and DSNGUOIBIKC is NOT null and rkc.khangcaoid is null) bhkc on bhkc.nguoikcid = tgtt.id where vuanid = VVUANID;
    
    -- Kháng nghị
    select count('x') into KHANGNGHIVA from ahs_sotham_khangnghi kn left join (select khangnghiid from ahs_sotham_rutkhangnghi where caprutkn = 3 and tinhtrang = 2) rkn on rkn.khangnghiid = kn.id where vuanid = vvuanid and DSNGUOIBIKN is null and rkn.khangnghiid is null;
    select listagg(DSNGUOIBIKN, ';') within group (order by '')--count('x') 
    into KHANGNGHIBC from ahs_sotham_khangnghi kn left join (select khangnghiid from ahs_sotham_rutkhangnghi where caprutkn = 3 and tinhtrang = 2) rkn on rkn.khangnghiid = kn.id where vuanid = vvuanid and DSNGUOIBIKN is NOT null and rkn.khangnghiid is null;    
    ...
    if(KHANGNGHIVA > 0 OR TGTTKHANGCAOVA > 0) then --Lấy hết tất cả bị cáo cùng tội danh
        ...
        SELECT COUNT('x') INTO V_QD_DINHCHI
        FROM AHS_PHUCTHAM_QUYETDINH_VUAN
        INNER JOIN (SELECT ID,TEN FROM dm_qd_quyetdinh 
                    WHERE TEN LIKE '%Quyết định đình chỉ%') dmqd ON dmqd.id = QUYETDINHID
        WHERE VUANID = VVUANID;
                                        ...
                                        FOR ITEM IN (select regexp_substr(DANHSACH_BC_CHAR, '[^;]+', 1, level) BCID FROM dual CONNECT BY LEVEL <= regexp_count(DANHSACH_BC_CHAR,  ';' ) + 1)
                                            LOOP
                                                IF(ITEM.BCID IS NOT NULL) THEN
                                                        ...
                                                        PKG_STPT_HS_TONGHOPHINHPHAT.AHS_TONGHOPHINHPHAT_PT(0,check_value,V_CURSOR);
                ...
                FOR ITEM IN (select regexp_substr(DANHSACH_BC_CHAR, '[^;]+', 1, level) BCID FROM dual CONNECT BY LEVEL <= regexp_count(DANHSACH_BC_CHAR,  ';' ) + 1)
                    LOOP
                        IF(ITEM.BCID IS NOT NULL) THEN
                                ...
                                PKG_STPT_HS_TONGHOPHINHPHAT.AHS_TONGHOPHINHPHAT_ST(0,check_value,V_CURSOR);
...
END PKG_STPT_HS_TONGHOPHINHPHAT;
