...
BEGIN 
      OPEN CURRETURN FOR
       SELECT TH.MA_TEN,TH.LOAITOA,TH.TEN,NS.* FROM TUPHAP_NGUOISUDUNG NS
       INNER JOIN DM_DONVITHIHANHAN TH ON NS.DONVITHA_ID=TH.ID
       WHERE LOWER(NS.USERNAME) = LOWER(V_USER_NAME) AND PASSWORD = (V_PASSWORD)      
       AND NS.HIEULUC =1 ;
