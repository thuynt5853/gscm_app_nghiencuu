--------------------------------------------------------
--  DDL for Package Body PKG_PCTP_KINHTE
--------------------------------------------------------

  CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "GSCM"."PKG_PCTP_KINHTE" AS
...
BEGIN 
    ...
    SELECT A.TOAANID INTO V_TOAANID FROM AKT_DON A WHERE A.ID=V_DONID;
    ----------
    IF(V_TRANGTHAI=2) THEN
       IF(V_NGAYPHANCONGTP IS NOT NULL AND  V_THAM_PHAN_ID IS NOT NULL AND V_NGAYPHANCONGLD IS NOT NULL AND V_PHANCONGLD_ID IS NOT NULL) THEN
         SELECT COUNT(*) INTO V_COUNT_CHECK FROM AKT_DON_THAMPHAN X 
         ...
         ----V_COUNT_CHECK
          IF(V_COUNT_CHECK=0)THEN
             ...
             VALUES (AKT_DON_THAMPHAN_SEQ.NEXTVAL,V_DONID,V_THAM_PHAN_ID,V_VAITRO,TO_DATE(V_NGAYPHANCONGLD,'dd/MM/yyyy'),TO_DATE(V_NGAYPHANCONGTP,'dd/MM/yyyy'),V_PHANCONGLD_ID,
   ...
   select tt.* from (
      ...
      LEFT JOIN DM_TOAAN T ON A.TOAANID=T.ID
        ------Trạng thái giải quyết trong danh sách
            ...
            LEFT JOIN (SELECT TP.DONID,'</br>- Đã phân công Thẩm phán' TINHTRANG_GQ FROM AKT_DON_THAMPHAN TP WHERE TP.MAVAITRO='VTTP_GIAIQUYETSOTHAM' 
            ...
            LEFT JOIN (
                        SELECT QSV.DONID,'</br>- Đang hoãn phiên tòa sơ thẩm'TINHTRANG_GQ FROM AKT_SOTHAM_QUYETDINH QSV
             ...
             LEFT JOIN (
                 ...
                 LEFT JOIN (
                        SELECT QSV.DONID,'</br>- Đang tạm đình chỉ' TINHTRANG_GQ FROM AKT_SOTHAM_QUYETDINH QSV
                        INNER JOIN DM_QD_LOAI QDL ON QSV.LOAIQDID=QDL.ID 
                        LEFT JOIN AKT_SOTHAM_BANAN BA ON QSV.DONID =BA.DONID 
                ...
                LEFT JOIN (
                        SELECT BA.DONID,'</br>- Đã có bản án sơ thẩm' TINHTRANG_GQ FROM AKT_SOTHAM_BANAN BA
              ...
              LEFT JOIN (
                       SELECT QSV.DONID,'</br>- Đã có QĐ đình chỉ' TINHTRANG_GQ FROM AKT_SOTHAM_QUYETDINH QSV 
                       INNER JOIN DM_QD_LOAI QDL ON QSV.LOAIQDID=QDL.ID
             ...
             LEFT JOIN (
                       SELECT QSV.DONID,'</br>- Đã có QĐ công nhận thỏa thuận' TINHTRANG_GQ FROM AKT_SOTHAM_QUYETDINH QSV 
                       INNER JOIN DM_QD_LOAI QDL ON QSV.LOAIQDID=QDL.ID
                       WHERE instr('CNTT',QDL.MA)>0
                       GROUP BY QSV.DONID,'</br>- Đã có QĐ công nhận thỏa thuận'
                       )CNTT ON  CNTT.DONID=a.id AND GD.MAGIAIDOAN=2
             LEFT JOIN (
                       SELECT QSV.DONID,'</br>- Đã có QĐ chuyển vụ án' TINHTRANG_GQ FROM AKT_SOTHAM_QUYETDINH QSV 
                       INNER JOIN DM_QD_LOAI QDL ON QSV.LOAIQDID=QDL.ID
                            ...
                            SELECT TT.DONID,RTRIM(SUBSTR(TT.HOTEN,0,INSTR(TT.HOTEN,',',1,DECODE(TT.COUNT_DS,1,1,2,2,3,3,3) )),',')HOTEN 
                                FROM (
                                      select COUNT(*)COUNT_DS,DS.DONID,
                                        ...
                                        FROM AKT_DON_DUONGSU DS WHERE DS.ISDAIDIEN=0
                                        GROUP BY DS.DONID
                                     )TT
                     )TS
                            ...
                            SELECT TT.DONID,RTRIM(SUBSTR(TT.HOTEN,0,INSTR(TT.HOTEN,',',1,DECODE(TT.COUNT_DS,1,1,2,2,3,3,3) )),',')HOTEN 
                                FROM (
                                      select COUNT(*)COUNT_DS,DS.DONID,
                                        ...
                                        INNER JOIN (SELECT DUONGSUID, LOAIKHANGCAO,DONID FROM AKT_SOTHAM_KHANGCAO ) KC ON KC.DUONGSUID = DS.ID AND KC.DONID=DS.DONID
        ...
        WHERE   (V_TEN_VU_AN IS NULL  OR ( LOWER(A.TENVUVIEC) LIKE  '%'||LOWER(V_TEN_VU_AN)||'%' ) )--Tên vụ án
            ...
            AND (V_TENDUONGSU IS NULL --Đương sự
                  OR( EXISTS (SELECT 'X' FROM AKT_DON_DUONGSU DS WHERE  UPPER(DS.TENDUONGSU) LIKE '%'||UPPER(V_TENDUONGSU)||'%' AND DS.DONID=A.ID)
                    )
            ) 
...
--         -----   
         ...
         AND (V_THAMPHAN_ID IS NULL
            ...
            --GQ đơn;V_GQDON -- -- 
          AND (V_GQDON IS NULL 
              OR(EXISTS ( SELECT 'X' FROM AKT_DON_XULY XL WHERE XL.LOAIGIAIQUYET=V_GQDON AND XL.DONID=A.ID) ) 
            )  
   ...
   FOR item in  (
      ...
      LEFT JOIN DM_TOAAN T ON A.TOAANID=T.ID
        ------Trạng thái giải quyết trong danh sách
          ...
          LEFT JOIN (
                 SELECT DS.TENDUONGSU,DS.DONID  FROM AKT_DON_DUONGSU DS WHERE DS.ISDAIDIEN=1 AND DS.TUCACHTOTUNG_MA='NGUYENDON'          
          )NDST ON NDST.DONID=A.ID AND GD.MAGIAIDOAN=2
         LEFT JOIN (
                SELECT DS.TENDUONGSU,DS.DONID  FROM AKT_DON_DUONGSU DS WHERE DS.ISDAIDIEN=1 AND DS.TUCACHTOTUNG_MA='NGUYENDON'          
          )NDPT ON NDPT.DONID=A.ID AND GD.MAGIAIDOAN=3
         LEFT JOIN (
                 SELECT DS.TENDUONGSU,DS.DONID  FROM AKT_DON_DUONGSU DS WHERE DS.ISDAIDIEN=1 AND DS.TUCACHTOTUNG_MA='BIDON'          
          ...
          LEFT JOIN (     
                        ...
                        -- REPLACE(ITEM.NDBD_NOIDUNG,'//BR','<br style="mso-data-placement:same-cell;" />') vì định dạng text
                            FROM (
                                ...
                                UNION ALL
                                    SELECT KC.DONID,'kc:'NOIDUNGKHANGCAO FROM AKT_SOTHAM_KHANGCAO  KC
                                    LEFT JOIN AKT_DON_DUONGSU DS ON DS.ID=KC.DUONGSUID
        ...
        LEFT JOIN (
                SELECT NDKN.DONID,LISTAGG(NDKN.NOIDUNGKN, '<br style="mso-data-placement:same-cell;" />')  WITHIN GROUP (ORDER BY NDKN.NOIDUNGKN)NOIDUNGKN
                            FROM (
                                    SELECT KC.DONID,' kn: ' ||KC.NOIDUNGKN NOIDUNGKN FROM AKT_SOTHAM_KHANGNGHI KC
                                    WHERE KC.NOIDUNGKN is not null
                                UNION ALL
                                    SELECT KC.DONID,'kn:'NOIDUNGKN FROM AKT_SOTHAM_KHANGNGHI KC
...
--         -----   
      ...
      LOOP             
           ...
           DBMS_LOB.APPEND(V_EXPORT_TEXT_ITEM,' 
                <td style="text-align: center; vertical-align: middle; border: 0.1pt solid #000000;">'||item.STT||'</td>
                <td style="text-align: center; vertical-align: middle; border: 0.1pt solid #000000;">KD,TM</td>
          ...
          <table cellpadding="1" cellspacing="1" style="font-family: times New Roman; font-size: 12pt; text-align: center; border-collapse: collapse;">
            ...
            <tr>
                <td colspan="15" style="line-height: 100%; font-size: 14pt; text-align: center;"><b>DANH SÁCH THẨM PHÁN '||v_TINHTRANG_GQ_TEN||' PHÂN CÔNG XÉT XỬ
                    <br />
                    '||V_CAPXX_TEN||' ÁN KINH DOANH THƯƠNG MẠI
                </b>
                    <br />
   ...
   FOR item in  (
      ...
      DBMS_LOB.APPEND(V_EXPORT_TEXT,'
           <table cellpadding="1" style="font-family: times New Roman; font-size: 12pt; text-align: center; border-collapse: collapse;">
            <tr>
                <td colspan="9" style="line-height: 100%; font-size: 14pt; text-align: center;"><b>DANH SÁCH ÁN KINH TẾ THỤ LÝ MỚI</b>
                </td>
            </tr>
            <tr>
