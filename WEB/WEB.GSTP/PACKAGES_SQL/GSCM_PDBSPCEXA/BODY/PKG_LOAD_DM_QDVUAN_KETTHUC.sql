--------------------------------------------------------
--  DDL for Package Body PKG_LOAD_DM_QDVUAN_KETTHUC
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "GSCM"."PKG_LOAD_DM_QDVUAN_KETTHUC" AS
...
BEGIN  
    ...
    ELSIF(VLOAIAN = '2') THEN -- Dân sự
        OPEN curReturn FOR    
            SELECT TEN, ID
            FROM DM_QD_QUYETDINH
            WHERE ISPHUCTHAM = 1 AND ISDANSU = 1 AND KET_THUC = 1
            ORDER BY TEN;
    ELSIF(VLOAIAN = '3') THEN -- Hôn nhân và gia đình 
        OPEN curReturn FOR    
            SELECT TEN,ID
