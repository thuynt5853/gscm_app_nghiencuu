create or replace NONEDITIONABLE PACKAGE BODY PKG_GDTTT_HCTP_BC AS
...
IS 
  ...
  V_CD_SOCV varchar2(250);V_CD_NGUOIKY varchar2(250);V_CD_NGAYCV date;V_LOAIDON8_1 clob;V_ID NUMBER;V_CAPCHAID NUMBER;
 ...
 --------
  ...
  FOR item IN (
          Select (case d.CD_LOAI when 0 then cast(pb.TENPHONGBAN as nvarchar2(250))
             ...
             ,D.ID,D.BAQD_LOAIAN,LA.LOAI_AN_TEN,D.CD_SOCV,D.CD_NGAYCV,D.CD_NGUOIKY,D.NGAYTAO  

             ,Decode(d.loaidon,4,SOTTXX.SOVB, Decode(SOTT.SOVB,null,SOTT_TLL.SOVB,SOTT.SOVB)) as CD_SOTOTRINH
              ,Decode(d.loaidon,4,SOTTXX.NGAYVB,Decode(SOTT.NGAYVB,null,SOTT_TLL.NGAYVB,SOTT.NGAYVB)) as CD_NGAYTOTRINH
...
--                      and 1=case when vNgayNhapDen is null then 1 when cv.NGAYTAO <= vNgayNhapDen then 1 else 0 end  
        ...
        LOOP
           ...
           V_CD_NGUOIKY_TEMP:=ITEM.CD_NGUOIKY;
             ---------    
                ...
                ITEM.ID,ITEM.NOICHUYEN,ITEM.BAQD_LOAIAN,ITEM.LOAI_AN_TEN,ITEM.CD_SOCV, ITEM.CD_NGAYCV,
                V_CD_NGUOIKY_TEMP,ITEM.NGAYTAO
    ...
    SELECT count(*)into V_COUNT FROM DM_PHONGBAN PB
     ...
     if(V_COUNT>0)then
         SELECT PA.CD_SOTOTRINH,PA.CD_NGAYTOTRINH,DECODE(V_BC_Nguoiky,NULL,PA.CD_NGUOIKY,V_BC_Nguoiky)NGUOIKY 
         INTO V_CD_SOCV,V_CD_NGAYCV,V_CD_NGUOIKY FROM TABLE(V_TABLE)PA  WHERE PA.CD_SOTOTRINH IS NOT NULL ORDER BY PA.NGAYTAO desc
...
IS 
  ...
  MININDEX	number;V_EXPORT_TEXT clob; VVNGAYNHAPTU varchar2(250);VVNGAYNHAPDEN varchar2(250);
  MAXINDEX	number; V_TABLE T_DON_SEARCH;V_BAQD_LOAIAN_NAME clob;V_NOICHUYEN clob;
  V_CD_SOCV varchar2(250);V_CD_NGUOIKY varchar2(250);V_CD_NGAYCV varchar2(250);V_COUNT_CHECK number;
  ...
  FOR item IN (
          Select (case d.CD_LOAI when 0 then cast(pb.TENPHONGBAN as nvarchar2(250))
             ...
             D.NGAYTAO,  
              ...
              Decode(d.loaidon,4,SOTTXX.SOVB , Decode(SOTT.SOVB,null,SOTT_TLL.SOVB,SOTT.SOVB)) as CD_SOTOTRINH,
               Decode(d.loaidon,4,SOTTXX.NGAYVB, Decode(SOTT.NGAYVB,null,SOTT_TLL.NGAYVB,SOTT.NGAYVB)) as CD_NGAYTOTRINH
           -- ,d.CD_SOTOTRINH,d.CD_NGAYTOTRINH 
        ...
        LOOP
             v_table.extend;
                 v_table(v_table.count) := R_DON_SEARCH(
                            ITEM.ID,ITEM.NOICHUYEN,ITEM.BAQD_LOAIAN,ITEM.LOAI_AN_TEN,ITEM.CD_SOCV, ITEM.CD_NGAYCV, ITEM.CD_NGUOIKY,ITEM.NGAYTAO
                            ,ITEM.CD_SOTOTRINH,ITEM.CD_NGAYTOTRINH);
        END LOOP;
     ...
     FROM (
      ...
      IF(V_BC_NGAYDK IS NOT NULL) THEN
       ...
       ELSIF(V_BC_Nguoiky IS NULL) THEN
           SELECT PA.CD_NGUOIKY INTO V_CD_NGUOIKY FROM TABLE(V_TABLE)PA
           ORDER BY PA.NGAYTAO desc FETCH FIRST 1 ROWS ONLY;
       END IF;
        -----
     SELECT UPPER(REPLACE(TA.TEN,'Tòa án nhân dân cấp cao','TANDCC')),DECODE(TA.LOAITOA,'TOICAO','TANDTC','CAPCAO','TANDCC'),TA.TEN
    ...
    --Insert số trang
      ...
      -------------------
       DBMS_LOB.APPEND(V_EXPORT_TEXT,'
            ...
            <p style="font-size: 14pt; text-align: justify; line-height: 120%;"><span style="color: white;">............</span> '||V_TENPHONGBANGUI||' chuyển các đơn đề nghị, kiến nghị, thông báo đến Quý vụ để xem xét, giải quyết theo thẩm quyền. Đề nghị Quý vụ ký xác nhận và chuyển phát danh sách đã ký nhận về phòng Hành chính tư pháp thuộc '||V_TENPHONGBANGUI||' '||V_TENDONVI_FULL||'./.</p>
             ');
            -----------
                DBMS_LOB.APPEND(V_EXPORT_TEXT,' 
                 <table cellpadding="0" cellspacing="1" style="font-family: times New Roman; font-size: 14pt; text-align: center; border-collapse: collapse;">
            ...
            END IF;
             ...
             if(item.BAQD_SO is not null)then
                    ...
                    if(item.BAQD_TOAANID is not null and item.BAQD_TOAANID !=0) then
                     V_NOIDUNG:=V_NOIDUNG||' '||item.TOAXX ||' đã có hiệu lực pháp luật.';
                     else
                      V_NOIDUNG:=V_NOIDUNG||' '||item.BAQD_TENTOA ||' đã có hiệu lực pháp luật.';
                    end if;
             ...
             -------
              ...
              ---
                ...
                ,null,null,null,null,null,null,null,to_char(item.CD_NGAYCV,'dd/MM/yyyy'),V_GIOITINHHOA,null
                ,null,null,to_char(item.CD_NGAYCV,'dd/MM/yyyy'),null);
        END LOOP;
                                                    ...
                                                    and 
                                                     1=case when vToaRaBAQD=0 then 1 when dtk.BAQD_TOAANID=vToaRaBAQD 
                                                                                        Or dtk.BAQD_TOAANID_PT=vToaRaBAQD 
                                                                                         Or dtk.BAQD_TOAANID_ST=vToaRaBAQD 
                                                                                          then 1 else 0 end  
...
IS 
  ...
  V_CD_NGAYCV date;V_LOAIDON8_1 NUMBER:=0;V_ID NUMBER;V_CAPCHAID NUMBER;
  V_CD_SOTB varchar2(250); v_dem NUMBER:=0;V_DIACHICOQUAN  varchar2(1000);V_NOIDUNG varchar2(2000);V_GIOITINH varchar2(250);V_GIOITINHHOA varchar2(250);
  V_LoaiCV_MA  varchar2(250);V_TOAAN_TEN varchar2(250);
  ...
  v_CAPXX  varchar2(500);v_LOAIAN_TEXT  varchar2(250); v_BIDANH  varchar2(250); v_loai_gdt  varchar2(250);
  v_TENDONVICC VARCHAR2(500);V_COUNT NUMBER;
...
--                    ,NVL(DONGKHIEUNAI.NGUOIGUI_DONGKHIEUNAI,'') AS NGUOIGUI_DONGKHIEUNAI
--                    ,NVL(DONGKHIEUNAI.NGUOIGUI_DONGKHIEUNAI_DIACHI,'') AS NGUOIGUI_DONGKHIEUNAI_DIACHI
            ...
            END IF; 
             ...
             if (item.BAQD_LOAIQDBA = 0)then
                ...
                v_table(v_table.count) := R_NOIBO_GIAYXACNHAN(
                 ...
                 v_loai_gdt,v_loaiBAQD,v_CAPXX,item.BAQD_SO, TO_char(item.BAQD_NGAYBA,'dd/MM/yyyy'),item.BAQD_TOAAN_NAME, V_NGUOIGUI_DONGKHIEUNAI, V_NGUOIGUI_DONGKHIEUNAI_DIACHI,
                 item.GXNSO,item.GXNNGAY,item.GXNNGUOIKY);
        END LOOP;
    OPEN curReturn FOR
       SELECT COUNT(*) OVER () as CountAll,
       PA.*
       --PA.LOAIDON ,PA.DUNGDONLA,PA.NGUOIGUI_GIOITINH,PA.HINHTHUCNHANDON,PA.NGAYTRENDON,PA.NGUOIGUI,PA.GIOITINH,PA.DIACHI,PA.NGAYNHANDON,PA.SOCV,PA.NGAYCV,PA.TAILIEU ,PA.TENCOQUAN,
       --PA.DIACHICOQUAN,PA.LOAIAN,PA.TENLOAIAN,PA.LOAIGDTT,PA.LOAIBAQD,PA.CAPXX ,PA.BA_SO ,PA.BA_NGAY ,PA.BA_TOAXX,PA.NGUOIGUI_DONGKHIEUNAI  
...
BEGIN
  ...
  FOR item IN (
           Select  d.CV_TENDONVI
                    ...
                    --,decode(d.BAQD_CAPXETXU,2,d.BAQD_TOAANID_ST,3,d.BAQD_TOAANID_PT,d.BAQD_TOAANID) BAQD_TOAANID
                    ,d.BAQD_TOAANID
                    ,decode(d.BAQD_CAPXETXU,2,TAS.TEN,3,TAP.TEN,TAG.TEN) BAQD_TOAAN_NAME
            ...
            from GDTTT_DON d
             --Thong tin Giay xac nhận
              LEFT JOIN(select so.*,sd.donid  from  SOPHATHANH_DON sd 
                                LEFT JOIN QUANLY_SOPHATHANH so on so.id = sd.SOPHATHANH_ID 
                                        where so.maso = 'SoGXN_DV'
                                                and so.trangthai = 1)sph on sph.donid = d.id

              LEFT JOIN (SELECT DONID,
                                LISTAGG(DECODE(GIOITINH,0, ' bà ', 1, ' ông ', ' ') || HOTEN, ',') WITHIN GROUP (ORDER BY ID DESC) NGUOIGUI_DONGKHIEUNAI,
                                LISTAGG(DECODE(GIOITINH,0, ' - Bà ', 1, ' - Ông ', ' - ') || HOTEN || ', ' || 'địa chỉ: ' || DIACHI, ';' || '<br/>') WITHIN GROUP (ORDER BY ID DESC) NGUOIGUI_DONGKHIEUNAI_DIACHI
                         FROM GDTTT_DON_NGUOIKN
                         GROUP BY DONID) DONGKHIEUNAI ON DONGKHIEUNAI.DONID = D.ID 
              ---
              LEFT JOIN (SELECT ID,TEN FROM DM_TOAAN) TAG ON TAG.ID = D.BAQD_TOAANID
              LEFT JOIN (SELECT ID,TEN FROM DM_TOAAN) TAP ON TAP.ID = D.BAQD_TOAANID_PT
              LEFT JOIN (SELECT ID,TEN FROM DM_TOAAN) TAS ON TAS.ID = D.BAQD_TOAANID_ST
              left join (select ID, GQD_LOAIKETQUA, GDQ_SO,GDQ_NGAY from GDTTT_VuAn) va on va.ID = d.VuViecID
              LEFT JOIN DM_VKS KS ON KS.ID=D.DONVICHUYEN_HSKN
                -----
             LEFT JOIN (
             ...
             -----------
                ...
                left join (select id, TEN from DM_DATAITEM) i on d.NGUOIKHANGNGHI=i.ID
                where d.TOAANID=vToaAnID and 1=(Case when vIsDonGoc=0 then 1  when vIsDonGoc=1 And NVL(d.DONTRUNGID,0)=0 then 1  Else 0 End)
                  and 1=(Case when vIsDonGoc=0 then 1  when vIsDonGoc=1 And NVL(d.DONTRUNGID,0)=0 then 1  Else 0 End)
                    And (vIsThuLy=-1 Or (vIsThuLy=1  and d.ISTHULY=1) 
                                   Or (vIsThuLy=2 and d.ISTHULY=2))       
               ...
               --anhvh 12/02/2020
                    ...
                    and (vTuNgay is null Or vTuNgay <= d.NGAYNHANDON)
                    and (vDenNgay is null Or d.NGAYNHANDON <= vDenNgay)
                    and (vHinhThucDon=0 Or d.LOAIDON=vHinhThucDon)
                    and (vSoHieuDon || ' '=' ' Or d.MADON =vSoHieuDon Or d.SOHIEUDON=vSoHieuDon)
                    ...
                    and (vSoCongVan || ' '=' ' Or (lower(d.CD_SOCV) =lower(vSoCongVan) And vNoiChuyen=2) 
                                             Or (lower(d.CD_SOCV) =lower(vSoCongVan) And vCD_TENDONVI='CVPC') 
                                             Or (lower(d.CD_SOTOTRINH) = lower(vSoCongVan) And vCD_TENDONVI='TTR' )) 
                    and (vNgayCongVan || ' '=' ' Or (to_char(d.CD_NGAYCV,'dd/MM/yyyy')=vNgayCongVan  And vNoiChuyen=2) 
                    ...
                    and (vCVPC_So || ' '=' ' Or lower(d.CV_SO) like '%' || lower(vCVPC_So) || '%')
                    and (vCVPC_Ngay || ' '=' ' Or to_char(d.CV_NGAY,'dd/MM/yyyy')=vCVPC_Ngay)
                    and (vCVPC_TenCQ || ' '=' ' Or lower(d.CV_TENDONVI) like '%' || lower(vCVPC_TenCQ) || '%')
                ...
                --anhvh 13/02/2020
                    ...
                    and (vTrangthai=-1 Or (vTrangthai=1 and d.CD_TRANGTHAI in (1,2)) 
                                       Or d.CD_TRANGTHAI=vTrangthai)
                    and  (vNoiChuyen=-1 OR vNoiChuyen=-2
                                        Or (vNoiChuyen=0 and (vCD_DONVIID=0 Or (vCD_DONVIID>0 And d.CD_TA_DONVIID=vCD_DONVIID))
                                                         and (vCD_TA_TRANGTHAI=-1 Or (vCD_TA_TRANGTHAI>=0 and CD_TA_TRANGTHAI=vCD_TA_TRANGTHAI))
                                            )
                ...
                and (vChidao=-1 Or (vChidao=0 and NVL(d.CHIDAO_COKHONG,0)>0)
                                Or (vChidao>0 and d.CHIDAO_LANHDAOID=vChidao)
                                )
                ...
                and (
                       ((vNgayNhapTu is null Or vNgayNhapTu <= d.NGAYTAO) and (vNgayNhapDen is null Or d.NGAYTAO <= vNgayNhapDen)
                        )
                    Or ((vNgayNhapTu is null Or vNgayNhapTu <= d.TL_NGAY)
                        and (vNgayNhapDen is null Or d.TL_NGAY <= vNgayNhapDen)
                        )
                    )
        ...
        LOOP
             ...
             if (item.BAQD_LOAIQDBA = 0)then
                ...
                v_table.extend;
                v_table(v_table.count) := R_NOIBO_GIAYXACNHAN(
                 v_Loaidon,'','',v_NguonDen,to_char(item.NGAYGHITRENDON,'dd/MM/yyyy'), item.DONGKHIEUNAI,V_GIOITINHHOA,item.Diachigui,to_char(item.NGAYNHANDON,'dd/MM/yyyy'),item.CV_SO,to_char(item.CV_NGAY,'dd/MM/yyyy'),
                 V_NOIDUNG,item.CV_TENDONVI,V_DIACHICOQUAN,v_LOAIAN_TEXT,item.BAQD_LOAIAN_NAME,
                 v_loai_gdt,v_loaiBAQD,v_CAPXX,item.BAQD_SO, TO_char(item.BAQD_NGAYBA,'dd/MM/yyyy'),item.BAQD_TOAAN_NAME,item.NGUOIGUI_DONGKHIEUNAI, item.NGUOIGUI_DONGKHIEUNAI_DIACHI,
                  item.GXNSO,item.GXNNGAY,item.GXNNGUOIKY);
        END LOOP;
  ...
  END DON_SEARCH_GIAYXACNHANCC_TOAKHAC;
            ...
            IF (retVal > 0) THEN 
              IF(V_BC_NGAYDK IS NOT NULL) THEN
                   ...
                   ELSE 
                      V_CD_NGAYCV:=SYSDATE;
                   END IF;
               select COUNT(*) INTO numYCBS from GDTTT_DON_YEUCAU_BOSUNG y Where y.DONID=item.ID AND y.SOTHONGBAO=V_BC_SoCV AND (V_CD_NGAYCV IS NOT NULL OR y.NGAYTHONGBAO=V_CD_NGAYCV);

               IF (retVal<2 AND vCD_TA_TRANGTHAI=4 AND numYCBS=0) THEN
                   v_dem:=v_dem+1;
                  IF(V_BC_SoCV IS NOT NULL) THEN
                       V_CD_SOTB:=to_char(V_BC_SoCV+v_dem-1);
                     ELSE
                  ...
                  SELECT TK.USERNAME INTO VV_NGUOITAO FROM QT_NGUOISUDUNG TK WHERE TK.ID = v_ID_USER;
                    ...
                    values (GDTTT_DON_YEUCAU_BOSUNG_SEQ.nextval,item.ID,2,V_BC_Nguoiky,V_BC_SoCV,V_CD_NGAYCV,item.CD_TA_LYDO_ISBAQD,item.CD_TA_LYDO_ISXACNHAN,item.CD_TA_LYDO_ISKHAC,item.CD_TA_LYDO_KHAC,0, sysdate,VV_NGUOITAO);           
               ELSE
                                                   ...
                                                   and
                                                    ...
                                                    and  (1=case when (vNoiChuyen=-1 OR vNoiChuyen=-2) then 1 
                                                        when (vNoiChuyen=0 and (vCD_DONVIID=0 Or (vCD_DONVIID>0 And dtk.CD_TA_DONVIID=vCD_DONVIID)) and (vCD_TA_TRANGTHAI=-1 Or (vCD_TA_TRANGTHAI>=0 and CD_TA_TRANGTHAI=vCD_TA_TRANGTHAI) 
                                                         OR (vCD_TA_TRANGTHAI=3 and NVL(CD_TA_TRANGTHAI,0) !=1 and EXISTS(SELECT 'X' FROM GDTTT_DON_YEUCAU_BOSUNG WHERE DONID=dtk.ID)) --lanhnt thêm trạng thái đơn
   ...
   LOOP
           ...
           SELECT COUNT(*) INTO retVal FROM GDTTT_DON_YEUCAU_BOSUNG WHERE DONID=item.ID;
            IF (retVal > 0) THEN 
               ...
               ELSE 
                 V_CD_NGAYCV:=SYSDATE;
               END IF;
               select COUNT(*) INTO numYCBS from GDTTT_DON_YEUCAU_BOSUNG y Where y.DONID=item.ID AND y.SOTHONGBAO=V_BC_SoCV AND (V_BC_NGAYDK IS NULL OR y.NGAYTHONGBAO=V_CD_NGAYCV);
               IF (retVal<2 AND vCD_TA_TRANGTHAI=4 AND numYCBS=0) THEN
                   v_dem:=v_dem+1;
                  IF(V_BC_SoCV IS NOT NULL) THEN
                       V_CD_SOTB:=to_char(V_BC_SoCV+v_dem-1);
           ...
           ELSE
              ...
              END IF;
                insert into GDTTT_DON_YEUCAU_BOSUNG 
                (id,DONID,LANTHU,NGUOIKY,SOTHONGBAO,NGAYTHONGBAO,CD_TA_LYDO_ISBAQD,CD_TA_LYDO_ISXACNHAN,CD_TA_LYDO_ISKHAC,NOIDUNG,KETQUA,ngaytao,NGUOITAO)
                values (GDTTT_DON_YEUCAU_BOSUNG_SEQ.nextval,item.ID,1,V_BC_Nguoiky,V_CD_SOTB,V_CD_NGAYCV,item.CD_TA_LYDO_ISBAQD,item.CD_TA_LYDO_ISXACNHAN,item.CD_TA_LYDO_ISKHAC,item.CD_TA_LYDO_KHAC,0, sysdate,VV_NGUOITAO);                                 
           END IF;
            /*IF(V_BC_SoCV IS NOT NULL) THEN
               V_CD_SOTB:=to_char(V_BC_SoCV+v_dem-1);
             ELSE
             ...
             -------
              V_DIACHICOQUAN:=item.CVDIACHI;
              ---
--              DECODE(item.LOAIDON,'7','thông báo','6','văn bản kiến nghị','9','văn bản kiến nghị','đơn đề nghị')
             if (item.LOAIDON in (7)) then
                v_Loaidon := 'văn bản thông báo';
             elsif (item.LOAIDON in (6,9)) then
                v_Loaidon := 'văn bản kiến nghị';
             else 
                v_Loaidon := 'đơn đề nghị';
             ...
             if (item.LOAIDON in (3,9)) then
              ...
              ---
              --DECODE(item.HINHTHUCNHANDON,'1','do tổ chức dịch vụ bưu chính chuyển đến','nộp trực tiếp')
              if (item.NGUON_DEN = 1) then
                    v_NguonDen := 'do tổ chức dịch vụ bưu chính chuyển đến';
              else
                    v_NguonDen := 'nộp trực tiếp';
              ...
              --
--              DECODE(item.CAPXX,'3','phúc thẩm','2','sơ thẩm','')
             if (item.BAQD_CAPXETXU = 3) then
                v_CAPXX := 'phúc thẩm';
             elsif (item.BAQD_CAPXETXU = 2) then
                v_CAPXX := 'sơ thẩm';
              ...
              --NGAYCV dung de luu tai lieu can bo sung--
...
--             END IF;
            ...
            ----
              ...
              ---
                ...
                v_table(v_table.count) := R_NOIBO_GIAYXACNHAN(
                 v_Loaidon,'','',v_NguonDen,to_char(item.NGAYGHITRENDON,'dd/MM/yyyy'), item.DONGKHIEUNAI,V_GIOITINHHOA,item.Diachigui,to_char(item.NGAYNHANDON,'dd/MM/yyyy'),
                 V_NOIDUNG,item.CV_SO,v_TailieuBS,item.CV_TENDONVI,V_DIACHICOQUAN,v_LOAIAN_TEXT,item.BAQD_LOAIAN_NAME,
                 v_loai_gdt,v_loaiBAQD,v_CAPXX,item.BAQD_SO, TO_char(item.BAQD_NGAYBA,'dd/MM/yyyy'),item.BAQD_TOAAN_NAME, 'item.NGUOIGUI_DONGKHIEUNAI', 'item.NGUOIGUI_DONGKHIEUNAI_DIACHI',
                 NULL,NULL,NULL);
        END LOOP;
