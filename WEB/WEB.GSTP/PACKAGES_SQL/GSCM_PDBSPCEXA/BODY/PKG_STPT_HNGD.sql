--------------------------------------------------------
--  DDL for Package Body PKG_STPT_HNGD
--------------------------------------------------------

  CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "GSCM"."PKG_STPT_HNGD" AS
...
BEGIN
SELECT COUNT(*) INTO V_COUNTS FROM AHN_SOTHAM_THULY TL  WHERE TL.DONID = vDONID AND TL.NGAYTHULY < TO_DATE('01/MAY/2022','dd/mon/yyyy');
IF(V_COUNTS>=1)THEN
    OPEN curReturn FOR  
      Select d.ID,d.TENDUONGSU
      From AHN_DON_DUONGSU d 
      LEFT JOIN AHN_ANPHI x ON x.DUONGSU_IDS like '%' || d.ID || '%'
      Where d.DONID=vDONID And d.ISDON=1 AND d.TUCACHTOTUNG_MA='NGUYENDON' AND ((x.SOBIENLAI is not null AND x.NGAYTHONGBAO >= TO_DATE('01/MAY/2022','dd/mon/yyyy')) 
