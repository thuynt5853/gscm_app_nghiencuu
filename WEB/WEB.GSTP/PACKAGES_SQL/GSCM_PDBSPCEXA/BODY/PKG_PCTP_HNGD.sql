--------------------------------------------------------
--  DDL for Package Body PKG_PCTP_HNGD
--------------------------------------------------------

  CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "GSCM"."PKG_PCTP_HNGD" AS
...
BEGIN 
    ...
    SELECT A.TOAANID INTO V_TOAANID FROM AHN_DON A WHERE A.ID=V_DONID;
    ----------
    IF(V_TRANGTHAI=2) THEN
       IF(V_NGAYPHANCONGTP IS NOT NULL AND  V_THAM_PHAN_ID IS NOT NULL AND V_NGAYPHANCONGLD IS NOT NULL AND V_PHANCONGLD_ID IS NOT NULL) THEN
         SELECT COUNT(*) INTO V_COUNT_CHECK FROM AHN_DON_THAMPHAN X 
         ...
         ----V_COUNT_CHECK
          IF(V_COUNT_CHECK=0)THEN
             ...
             VALUES (AHN_DON_THAMPHAN_SEQ.NEXTVAL,V_DONID,V_THAM_PHAN_ID,V_VAITRO,TO_DATE(V_NGAYPHANCONGLD,'dd/MM/yyyy'),TO_DATE(V_NGAYPHANCONGTP,'dd/MM/yyyy'),V_PHANCONGLD_ID,
   ...
   select tt.* from (
      ...
      LEFT JOIN DM_TOAAN T ON A.TOAANID=T.ID
        ------Trạng thái giải quyết trong danh sách
            ...
            LEFT JOIN (SELECT TP.DONID,'</br>- Đã phân công Thẩm phán' TINHTRANG_GQ FROM AHN_DON_THAMPHAN TP WHERE TP.MAVAITRO='VTTP_GIAIQUYETSOTHAM' 
            ...
            LEFT JOIN (
                        SELECT QSV.DONID,'</br>- Đang hoãn phiên tòa sơ thẩm'TINHTRANG_GQ FROM AHN_SOTHAM_QUYETDINH QSV
             ...
             LEFT JOIN (
                 ...
                 LEFT JOIN (
                        SELECT QSV.DONID,'</br>- Đang tạm đình chỉ' TINHTRANG_GQ FROM AHN_SOTHAM_QUYETDINH QSV
                        INNER JOIN DM_QD_LOAI QDL ON QSV.LOAIQDID=QDL.ID 
                        LEFT JOIN AHN_SOTHAM_BANAN BA ON QSV.DONID =BA.DONID 
                ...
                LEFT JOIN (
                        SELECT BA.DONID,'</br>- Đã có bản án sơ thẩm' TINHTRANG_GQ FROM AHN_SOTHAM_BANAN BA
              ...
              LEFT JOIN (
                       SELECT QSV.DONID,'</br>- Đã có QĐ đình chỉ' TINHTRANG_GQ FROM AHN_SOTHAM_QUYETDINH QSV 
                       INNER JOIN DM_QD_LOAI QDL ON QSV.LOAIQDID=QDL.ID
             ...
             LEFT JOIN (
                       SELECT QSV.DONID,'</br>- Đã có QĐ công nhận thỏa thuận' TINHTRANG_GQ FROM AHN_SOTHAM_QUYETDINH QSV 
                       INNER JOIN DM_QD_LOAI QDL ON QSV.LOAIQDID=QDL.ID
                       WHERE instr(',CNTT,TTLYHON,',','||QDL.MA||',')>0
                       GROUP BY QSV.DONID,'</br>- Đã có QĐ công nhận thỏa thuận'
                       )CNTT ON  CNTT.DONID=a.id AND GD.MAGIAIDOAN=2
             LEFT JOIN (
                       SELECT QSV.DONID,'</br>- Đã có QĐ chuyển vụ án' TINHTRANG_GQ FROM AHN_SOTHAM_QUYETDINH QSV 
                       INNER JOIN DM_QD_LOAI QDL ON QSV.LOAIQDID=QDL.ID
                            ...
                            SELECT TT.DONID,RTRIM(SUBSTR(TT.HOTEN,0,INSTR(TT.HOTEN,',',1,DECODE(TT.COUNT_DS,1,1,2,2,3,3,3) )),',')HOTEN 
                                FROM (
                                      select COUNT(*)COUNT_DS,DS.DONID,
                                        ...
                                        FROM AHN_DON_DUONGSU DS WHERE DS.ISDAIDIEN=0
                                        GROUP BY DS.DONID
                                     )TT
                     )TS
                            ...
                            SELECT TT.DONID,RTRIM(SUBSTR(TT.HOTEN,0,INSTR(TT.HOTEN,',',1,DECODE(TT.COUNT_DS,1,1,2,2,3,3,3) )),',')HOTEN 
                                FROM (
                                      select COUNT(*)COUNT_DS,DS.DONID,
                                        ...
                                        INNER JOIN (SELECT DUONGSUID, LOAIKHANGCAO,DONID FROM AHN_SOTHAM_KHANGCAO ) KC ON KC.DUONGSUID = DS.ID AND KC.DONID=DS.DONID
        ...
        WHERE   (V_TEN_VU_AN IS NULL  OR ( LOWER(A.TENVUVIEC) LIKE  '%'||LOWER(V_TEN_VU_AN)||'%' ) )--Tên vụ án
            ...
            AND (V_TENDUONGSU IS NULL --Đương sự
                  OR( EXISTS (SELECT 'X' FROM AHN_DON_DUONGSU DS WHERE  UPPER(DS.TENDUONGSU) LIKE '%'||UPPER(V_TENDUONGSU)||'%' AND DS.DONID=A.ID)
                    )
            ) 
...
--         -----   
         ...
         -----
          AND (V_THAMPHAN_ID IS NULL
            ...
            --GQ đơn;V_GQDON -- -- 
          AND (V_GQDON IS NULL 
              OR(EXISTS ( SELECT 'X' FROM AHN_DON_XULY XL WHERE XL.LOAIGIAIQUYET=V_GQDON AND XL.DONID=A.ID) ) 
            )  
...
IS 
    ...
    V_TUNGAY_BC             VARCHAR2(500);
    V_DENNGAY_BC            VARCHAR2(500);
    v_TINHTRANG_GQ_TEN      VARCHAR2(500);
    V_TOAAN_TEN             VARCHAR2(200);
    V_CAPXX_TEN             VARCHAR2(100);
    V_CAPTAND_TEN           VARCHAR2(100);
    V_HNGD                  VARCHAR2(20);
    V_EXPORT_TEXT           CLOB;
    V_EXPORT_TEXT_ITEM      CLOB;
    V_NOIDUNG_KHANGCAO      CLOB DEFAULT '';
    V_CURSOR                SYS_REFCURSOR;
BEGIN
     SELECT LA.TEN_VIET_TAT INTO V_HNGD FROM DM_LOAIAN LA WHERE ID=3;
   ...
   FOR item in  (
            SELECT ROW_NUMBER() OVER (ORDER BY DECODE(v_TINHTRANG_GIAIQUYET, 2, a.NGAYTAO, 3, PCTP_GQ.NGAYTAO) DESC) STT
                    ...
                    ,NDKNS.NOIDUNGKN NOIDUNG_KHANGNGHI
                    ,A.ID
            ...
            LEFT JOIN (SELECT DS.TENDUONGSU,DS.DONID  FROM AHN_DON_DUONGSU DS WHERE DS.ISDAIDIEN=1 AND DS.TUCACHTOTUNG_MA='NGUYENDON'          )NDST ON NDST.DONID=A.ID AND GD.MAGIAIDOAN=2
            LEFT JOIN (SELECT DS.TENDUONGSU,DS.DONID  FROM AHN_DON_DUONGSU DS WHERE DS.ISDAIDIEN=1 AND DS.TUCACHTOTUNG_MA='NGUYENDON'          )NDPT ON NDPT.DONID=A.ID AND GD.MAGIAIDOAN=3
            LEFT JOIN (SELECT DS.TENDUONGSU,DS.DONID  FROM AHN_DON_DUONGSU DS WHERE DS.ISDAIDIEN=1 AND DS.TUCACHTOTUNG_MA='BIDON'          )BDST ON BDST.DONID=A.ID AND GD.MAGIAIDOAN=2
            LEFT JOIN (SELECT DS.TENDUONGSU,DS.DONID  FROM AHN_DON_DUONGSU DS WHERE DS.ISDAIDIEN=1 AND DS.TUCACHTOTUNG_MA='BIDON'          )BDPT ON BDPT.DONID=A.ID AND GD.MAGIAIDOAN=3
             ----lấy 1 bản ghi mới nhất 27/12/2023
            LEFT JOIN (
                     SELECT FIRST_VALUE(TP.ID) OVER (PARTITION BY TP.DONID ORDER BY TP.NGAYNHANPHANCONG DESC,TP.NGAYTAO DESC) ID,
                     CB.HOTEN,TP.DONID,TP.NGAYTAO,TP.NGAYNHANPHANCONG,NPC.HOTEN HOTEN_LD,TP.NGUOIPHANCONGID,TP.CANBOID,TP.NGAYPHANCONG
            ...
            LEFT JOIN (SELECT NDKN.DONID, LISTAGG(NDKN.NOIDUNGKN, '<br style="mso-data-placement:same-cell;" />')  WITHIN GROUP (ORDER BY NDKN.NOIDUNGKN) NOIDUNGKN
                       FROM (SELECT KC.DONID, DECODE (KC.NOIDUNGKN, NULL,'kn:', 'kn:' ||KC.NOIDUNGKN ) NOIDUNGKN 
                             FROM AHN_SOTHAM_KHANGNGHI KC) NDKN  GROUP BY NDKN.DONID
            ...
            WHERE   (V_TEN_VU_AN IS NULL  OR ( LOWER(A.TENVUVIEC) LIKE  '%'||LOWER(V_TEN_VU_AN)||'%' ) )--Tên vụ án
                ...
                AND (V_SOTHULY IS NULL OR(UPPER(TLS.SOTHULY)=UPPER(V_SOTHULY) OR UPPER(TLPT.SOTHULY)=UPPER(V_SOTHULY) ))--Số Thụ lý
                AND (V_THAMPHAN_ID IS NULL
                        OR (EXISTS (SELECT 'x' FROM ADS_DON_THAMPHAN PC 
                                    WHERE   PC.CANBOID = V_THAMPHAN_ID  AND PC.DONID=A.ID
                                                                        AND ((v_Capxx=2 AND PC.MAVAITRO='VTTP_GIAIQUYETSOTHAM')
                                                                                OR(v_Capxx=3 AND PC.MAVAITRO='VTTP_GIAIQUYETPHUCTHAM')  )  
                                    )  
                            )--Thẩm phán
                    )
                --GQ đơn;V_GQDON -- -- 
                AND (V_GQDON IS NULL OR(EXISTS ( SELECT 'X' FROM AHN_DON_XULY XL WHERE XL.LOAIGIAIQUYET=V_GQDON AND XL.DONID=A.ID) ) )  
                AND (V_THUKY_ID is null--Thư ký
                        OR( EXISTS(select 'X' from AHN_SoTham_HDXX tp where tp.CanBoID = v_thuky_id and tp.DONID=a.ID) 
                        OR EXISTS(select 'X' from AHN_PhucTham_HDXX tp where tp.CanBoID = v_thuky_id and tp.DONID=a.ID)
                        )
                ...
                AND(v_TINHTRANG_GIAIQUYET IS NULL
                        ...
                        OR(v_TINHTRANG_GIAIQUYET=3 --đã phân công Thẩm phán
                            AND EXISTS (SELECT 'x' FROM AHN_DON_THAMPHAN PC 
                                        WHERE PC.DONID=A.ID
                ...
                AND ((instr('2,3,4,5,6',v_TINHTRANG_GIAIQUYET)=0 OR v_TINHTRANG_GIAIQUYET IS NULL)
                    OR (instr('2,3,4,5,6',v_TINHTRANG_GIAIQUYET)>0 
                        AND (EXISTS (SELECT 'X' FROM AHN_SOTHAM_THULY TL
                                    WHERE (NOT EXISTS(SELECT 'X' FROM AHN_SOTHAM_BANAN BA WHERE TL.DONID =BA.DONID)
                                            AND (NOT EXISTS(SELECT T1.DONID,T2.MA FROM AHN_SOTHAM_QUYETDINH T1 
                                            INNER JOIN DM_QD_LOAI T2 ON T1.LOAIQDID=T2.ID
                                            WHERE instr('DC,CVA,CNTT',T2.MA)>0 AND TL.DONID=T1.DONID) )
                             ...
                             OR EXISTS (SELECT 'X' FROM AHN_PHUCTHAM_THULY PTTL 
                                        WHERE (NOT EXISTS(SELECT 'X' FROM AHN_PHUCTHAM_BANAN PTBA 
                                                          WHERE PTBA.DONID=PTTL.DONID)
                                                              AND NOT EXISTS(SELECT 'X' FROM AHN_PHUCTHAM_QUYETDINH PTQDVA --QUYẾT ĐỊNH 
                                                                                INNER JOIN DM_QD_LOAI QDL ON QDL.ID=PTQDVA.LOAIQDID 
                                                                             WHERE instr('DC,CVA,CNTT',QDL.MA)>0 AND  PTTL.DONID=PTQDVA.DONID 
                                                                            )
        ...
        LOOP  
            ...
            V_NOIDUNG_KHANGCAO := '';
            FOR ITEM_NOIDUNGDKCKN IN (SELECT KC.NOIDUNGKHANGCAO, DECODE(DS.TUCACHTOTUNG_MA,'BIDON','BĐ','NGUYENDON','NĐ','QUYENNVLQ','NLQ') || ' kc: ' || KC.NOIDUNGKHANGCAO AS NDBD_NOIDUNG
                                      FROM AHN_SOTHAM_KHANGCAO KC
                                           LEFT JOIN AHN_DON_DUONGSU DS ON DS.ID=KC.DUONGSUID 
                                      WHERE KC.DONID = ITEM.ID)
            LOOP
                IF(ITEM_NOIDUNGDKCKN.NOIDUNGKHANGCAO IS NOT NULL) THEN
                        V_NOIDUNG_KHANGCAO := V_NOIDUNG_KHANGCAO || ITEM_NOIDUNGDKCKN.NDBD_NOIDUNG ||'<br style="mso-data-placement:same-cell;" />';
                    ELSE
                        V_NOIDUNG_KHANGCAO := 'kc: ' || '<br style="mso-data-placement:same-cell;" />' ;
                END IF;
            END LOOP;
            V_NOIDUNG_KHANGCAO := V_NOIDUNG_KHANGCAO || ITEM.NOIDUNG_KHANGNGHI;
           ...
           DBMS_LOB.APPEND(V_EXPORT_TEXT_ITEM,' 
                ...
                <td style="text-align: center; vertical-align: middle; border: 0.1pt solid #000000;">'||V_HNGD||'</td>
                ...
                <td style="text-align: left; vertical-align: middle; border: 0.1pt solid #000000;">'||V_NOIDUNG_KHANGCAO||'</td>
          ...
          <table cellpadding="1" cellspacing="1" style="font-family: times New Roman; font-size: 12pt; text-align: center; border-collapse: collapse;">
            ...
            <tr>
                <td colspan="15" style="line-height: 100%; font-size: 14pt; text-align: center;"><b>DANH SÁCH THẨM PHÁN  '||v_TINHTRANG_GQ_TEN||' PHÂN CÔNG XÉT XỬ
                    <br />
                    '||V_CAPXX_TEN||' ÁN HÔN NHÂN VÀ GIA ĐÌNH
                </b>
                    <br />
     ...
     SELECT  ROW_NUMBER() OVER (ORDER BY DECODE(v_Capxx,2,TLS.SOTHULY,3,to_number(regexp_replace(TLPT.SOTHULY, '[^[:digit:]]', '')) )ASC ) STT
      ...
      DBMS_LOB.APPEND(V_EXPORT_TEXT,'
           <table cellpadding="1" style="font-family: times New Roman; font-size: 12pt; text-align: center; border-collapse: collapse;">
            <tr>
                <td colspan="9" style="line-height: 100%; font-size: 14pt; text-align: center;"><b>DANH SÁCH ÁN HÔN NHÂN GIA ĐÌNH THỤ LÝ MỚI</b>
                </td>
            </tr>
            <tr>
