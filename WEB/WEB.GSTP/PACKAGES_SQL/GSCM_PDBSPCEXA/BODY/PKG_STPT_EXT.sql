--------------------------------------------------------
--  DDL for Package Body PKG_STPT_EXT
--------------------------------------------------------

  CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "GSCM"."PKG_STPT_EXT" AS
...
BEGIN
      open CurReturn for
        ...
        left JOIN AHS_PHUCTHAM_BICANBICAO PTBC ON PTBC.BICANID = BC.ID
...
BEGIN	
   ...
   SELECT DECODE(TA.LOAITOA,'CAPCAO',REPLACE(UPPER(TA.TEN),'TÒA ÁN NHÂN DÂN CẤP CAO',''),'CAPTINH',REPLACE(UPPER(TA.TEN),'TÒA ÁN NHÂN DÂN TỈNH'),'') INTO VTOAAN_TEN FROM DM_TOAAN TA WHERE TA.ID=VTOAANID;
   SELECT DECODE(TA.LOAITOA,'CAPCAO','TÒA ÁN NHÂN DÂN CẤP CAO','CAPTINH','TÒA ÁN NHÂN DÂN TỈNH')INTO VCAPXX_TEN FROM DM_TOAAN TA WHERE TA.ID=VTOAANID;
   --SELECT UPPER(TA.TEN) into VTOAAN_TEN FROM DM_TOAAN TA WHERE TA.ID=VTOAANID;
   SELECT COUNT(*) INTO VCOUNT_LS FROM AHS_NGUOITHAMGIATOTUNG WHERE INSTR(vArrSelectID,','||ID||',')>0; 
   ...
   --Insert số trang
     ...
     FOR item in (
                 ...
                 LEFT JOIN AHS_PHUCTHAM_THULY TL ON TL.VUANID=TG.VUANID  
                 where INSTR(vArrSelectID,','||tg.ID||',')>0
        )
     LOOP
        V_TT_LS:=V_TT_LS+1;
         ---------------
...
--            GROUP BY PTBC.VUANID;
        ...
        <p style="font-size: 14pt;text-align: justify;margin-top:3pt;margin-bottom:0pt;line-height: 115%;">
            ...
            , '||VTOAAN_TEN_FULL||' đã thụ lý vụ án hình sự phúc thẩm số '||item.sothuly||'/'|| to_char(sysdate,'yyyy') ||'/TLPT-HS. 
        </p>
        <p style="font-size: 14pt;text-align: justify;margin-top:3pt;margin-bottom:0pt;line-height: 115%;">
        ...
        <p style="font-size: 14pt;text-align: justify;margin-top:3pt;margin-bottom:0pt;line-height: 115%;">
            ...
            vụ án hình sự phúc thẩm thụ lý số '||item.sothuly||' ngày '||item.ngaythulys||'.
        </p>
   ...
   --Insert số trang
     ...
     LOOP
        ...
        ----------------
         ...
         GROUP BY NGUOI_TGTT_ID;
        ---------------
        SELECT decode(TD.TENTOIDANH,null,'',TD.TENTOIDANH) INTO V_TENTOIDANH FROM AHS_SOTHAM_CAOTRANG_DIEULUAT TD
        LEFT JOIN AHS_BICANBICAO A  ON A.ID = TD.BICANID 
        WHERE (TD.VUANID =ITEM.VUANID AND TD.ISMAIN=1) or (TD.VUANID =ITEM.VUANID AND ( A.BICANDAUVU =1 or A.BICANDAUVU is null)); -- duongph
        ...
        <p style="font-weight: bold; text-align: center; font-size: 16pt; line-height: 125%;">
            GIẤY XÁC NHẬN
            <br />
            <span style="font-weight: bold; text-align: center; font-size: 14pt;">Người bảo vệ quyền và lợi ích hợp pháp <br/> của đương sự tham gia tố tụng</span>
        </p>
        <p style="font-size: 14pt; line-height: 135%;margin-bottom:0pt;line-height: 115%;">
            <span style="color: #ffffff">......</span>Kính gửi: '||item.ONGBA||' '||item.HOTEN||', '||item.TEN||' '||item.TEN_VPLS
            ||' thuộc Đoàn luật sư '|| item.DOAN_LS||
        ...
        <p style="font-size: 14pt;text-align: justify;margin-top:3pt;margin-bottom:0pt;line-height: 115%;">
            ...
            trong vụ án '||item.TENVUAN||' phạm tội "'||( case when V_TENTOIDANH is null then '' else V_TENTOIDANH end) ||'"
        </p>
        <p style="font-size: 14pt;text-align: justify;margin-top:3pt;line-height: 115%;">
            <span style="color: #ffffff">......</span>2. '||item.ONGBA||' '||item.HOTEN||' thực hiện các quyền và nghĩa vụ của người
