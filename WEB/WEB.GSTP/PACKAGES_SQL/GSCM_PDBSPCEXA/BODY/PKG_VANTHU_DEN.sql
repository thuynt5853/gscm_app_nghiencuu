...
OPEN curReturn FOR
 SELECT a.* FROM ( 
       SELECT  COUNT(*) OVER () as CountAll, ROW_NUMBER() OVER (ORDER BY VT.NGAY_TAO DESC)STT,'<i>ĐV: </i>'||DECODE(P.ID,'13','Phòng hành chính tư pháp',P.TENPHONGBAN)||'<br/><i>NN: </i><b>'||CB.HOTEN||'</b>' DV_NHAN_NGUOI_NHAN
        ...
        ,VT.id,VT.LOAI_VB,VT.SODEN,to_char(VT.NGAY_DEN,'dd/MM/yyyy')NGAY_DEN,VT.NGUOI_GUI_BT,VT.DIACHI_GUI_BT
...
--           )
          (V_TRANGTHAICHUYEN IS NULL     AND(  V_PHONGBAN_ID IS NULL
             ...
             OR(V_TRANGTHAICHUYEN=0 AND NOT EXISTS(SELECT 'X' FROM VT_CHUYEN_NHAN CN WHERE CN.VANBANDEN_ID=VT.ID)) --đang lưu
             OR (V_TRANGTHAICHUYEN=1 AND  EXISTS(SELECT 'X' FROM VT_CHUYEN_NHAN CN WHERE CN.VANBANDEN_ID=VT.ID))--đã chuyển
             OR (V_TRANGTHAICHUYEN=2 AND  EXISTS(SELECT 'X' FROM VT_CHUYEN_NHAN CN WHERE CN.VANBANDEN_ID=VT.ID AND CN.CANBO_NHAN_ID IS NULL))--chua nhận
             OR (V_TRANGTHAICHUYEN=3 AND  EXISTS(SELECT 'X' FROM VT_CHUYEN_NHAN CN WHERE CN.VANBANDEN_ID=VT.ID AND CN.CANBO_NHAN_ID IS NOT NULL))--đã nhận
        )
