--------------------------------------------------------
--  DDL for Package Body PKG_STPT_DS
--------------------------------------------------------

  CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "GSCM"."PKG_STPT_DS" AS
...
( 
    ...
    V_UTTP IN VARCHAR2,
    vchecktk in number,
   -- V_TRANGTHAIVUAN in number, --anhvh  đóng lại vì trên code chưa có
    Page_Index in	int,
    Page_Size	in	int, 
      ...
      SELECT R_QUYETDINH_EXT(TTS.DONID,TTS.ID,TTS.MA)
        ...
        FROM(SELECT BC.ID,BC.DONID,BC.TENDUONGSU,BC.TUCACHTOTUNG_MA,BC.ROWNUMBER FROM 
                    (  SELECT d.ID,d.DONID,d.TENDUONGSU,d.TUCACHTOTUNG_MA,ROW_NUMBER()  OVER (PARTITION BY d.DONID ORDER BY d.ISDAIDIEN DESC,d.TENDUONGSU) ROWNUMBER
                       FROM ADS_DON_DUONGSU d left join ADS_ANPHI p ON p.duongsu_id = d.ID
...
--      GN.TRUONGHOPGIAONHAN,
      ...
      FROM ADS_DON A
      INNER JOIN (SELECT G.* FROM ADS_DON_GIAIDOAN G WHERE (G.MAGIAIDOAN = 2 AND G.TOAANID = v_toaan_id) OR (G.MAGIAIDOAN = 3 AND G.TOAPHUCTHAMID = v_toaan_id)) GD ON A.ID=GD.DONID--Điều kiện để hiển thị 1 bản ghi duy nhất theo giai đoạn xét xử (tránh hiển thị 2 giai đoạn ở ST và PT ở tòa tỉnh) tuanvna
...
--       MANHND THEM Quyết định gây kết thúc khác
        LEFT JOIN (
                       ...
                       )KT_QDST_KHAC ON  KT_QDST_KHAC.DONID=a.id AND GD.MAGIAIDOAN=2
             LEFT JOIN (
         ...
         AND( (v_TINHTRANG_GIAIQUYET IS NULL AND (V_TUNGAY IS NULL OR  a.NGAYTAO>=VV_TUNGAY) AND (V_DENNGAY IS NULL OR a.NGAYTAO<=VV_DENNGAY) )
             OR(v_TINHTRANG_GIAIQUYET=1 --Chưa giải quyết xong
                ...
                --                                   Manh them các loại QD gây kết thúc khác
                                                     OR QD.loaiid in (15,10,3) OR QD.id in (422,423,70,425)
                                                    )
                                ...
                                SELECT 'X' FROM ADS_SOTHAM_THULY TL
                                  WHERE (NOT EXISTS(SELECT 'X' FROM ADS_SOTHAM_BANAN BA WHERE TL.DONID =BA.DONID)
                                        AND (NOT EXISTS(SELECT T1.DONID,T2.MA FROM ADS_SOTHAM_QUYETDINH T1 
                                                   ...
                                                   WHERE 
                                                   (instr(',DC,CVA,CNTT,',','||T2.MA||',')>0 
                                                   -- Manh them các loại QD gây kết thúc khác
                                                    OR QD.loaiid in (15,10,3) OR QD.id in (422,423,70,425))
                                                   AND TL.DONID=T1.DONID) )
--                                                   instr(',DC,CVA,CNTT,',','||T2.MA||',')>0 AND TL.DONID=T1.DONID) )
...
BEGIN
SELECT COUNT(*) INTO V_COUNTS FROM ADS_SOTHAM_THULY TL  WHERE TL.DONID = vDONID AND TL.NGAYTHULY < TO_DATE('01/MAY/2022','dd/mon/yyyy');
...
IF(V_COUNTS>=1)THEN
    OPEN curReturn FOR  
        ...
        And d.ISDON=1 AND d.TUCACHTOTUNG_MA='NGUYENDON' AND ((x.SOBIENLAI is not null AND x.NGAYTHONGBAO >= TO_DATE('01/MAY/2022','dd/mon/yyyy')) 
        OR x.NGAYTHONGBAO < TO_DATE('01/MAY/2022','dd/mon/yyyy') OR x.NGAYTHONGBAO is null)
...
/* PROCEDURE  GET_DUONGSU_BIDON
(   
    vDONID in number,
    vND_ID in number,
    curReturn    OUT       sys_refcursor
)IS v_CheckDon number;
BEGIN
 SELECT COUNT(*) INTO v_CheckDon FROM ADS_DON_DUONGSU d WHERE d.ID = vND_ID AND NOT EXISTS(SELECT 'X' FROM DON_DUONGSU_CHITIET ct WHERE ct.DUONGSUID=vND_ID); 
  IF v_CheckDon = 1 THEN -- khong thuoc don chi tiet
  OPEN curReturn FOR  
  SELECT d.ID, d.TENDUONGSU FROM ADS_DON_DUONGSU d
  WHERE d.DONID = vDONID AND d.TUCACHTOTUNG_MA='BIDON' 
  AND NOT EXISTS(SELECT 'X' FROM DON_DUONGSU_CHITIET ctbd LEFT JOIN DON_CHITIET ct ON ctbd.DONCHITIETID = ct.ID WHERE d.ID = ctbd.DUONGSUID AND ct.LOAIDON IN (1,2,3,4))
  ORder by d.ISDAIDIEN desc, d.TENDUONGSU;
  ELSIF v_CheckDon = 0 THEN -- thuoc don chi tiet
  OPEN curReturn FOR 
  SELECT d.ID, d.TENDUONGSU FROM ADS_DON_DUONGSU d
  INNER JOIN DON_DUONGSU_CHITIET ct ON ct.DUONGSUID = d.ID
  WHERE d.DONID = vDONID AND d.TUCACHTOTUNG_MA='BIDON' 
  AND ct.DONCHITIETID IN (SELECT DONCHITIETID FROM DON_DUONGSU_CHITIET WHERE DUONGSUID = vND_ID)
  ORder by d.ISDAIDIEN desc, d.TENDUONGSU;
  END IF;
END GET_DUONGSU_BIDON; */
...
--            INNER JOIN ADS_DON_GIAIDOAN GD ON A.ID=GD.DONID
--            inner join DM_TOAAN ta on a.TOAANID=ta.ID
--            left join ADS_SOTHAM_BANAN ba on ba.donid = a.id
--          where  a.TOAANID = vDonViID 
--                and (vTenVuViec='' or LOWER(a.TENVUVIEC) LIKE  ('%' || LOWER(vTenVuViec) || '%'))
--                and ((vTrangThai = 1 and  Exists(select 'X' from QLHS_STPT hs where hs.vuviecid = a.id))
--                        OR (vTrangThai = 0 and Not Exists(select 'X' from QLHS_STPT hs where hs.vuviecid = a.id))
--                    )
--                and (   EXISTS (       
--                                    SELECT 'X' FROM ADS_SOTHAM_BANAN BA
--                                    WHERE  BA.SOBANAN IS NOT NULL
--                                    AND (vTuNgay IS NULL OR BA.NGAYTUYENAN>=vTuNgay)
--                                    AND (vDenNgay IS NULL OR BA.NGAYTUYENAN<=vDenNgay)
--                                    AND BA.DONID=a.id  AND GD.MAGIAIDOAN=2
--                                    AND (TRUNC(SYSDATE)- ba.NGAYTUYENAN) >=30
--                                    AND Not Exists(select 'X' from ADS_SOTHAM_KHANGCAO k where k.donid = a.id)
--                                 )
--                            OR  EXISTS (
--                                    SELECT 'X' FROM ADS_SOTHAM_QUYETDINH QSV 
--                                    LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID=QSV.QUYETDINHID                              LEFT JOIN DM_QD_LOAI QDL ON QDL.ID=QD.LOAIID
--                                    WHERE instr(',DC,CVA,CNTT,',','||QDL.MA||',')>0
--                                    AND (vTuNgay IS NULL OR QSV.NGAYQD>=vTuNgay)
--                                    AND (vDenNgay IS NULL OR QSV.NGAYQD<=vDenNgay)
--                                    AND QSV.DONID=a.id  AND GD.MAGIAIDOAN=2
--                                    AND (TRUNC(SYSDATE)- QSV.NGAYQD) >=30
--                                    AND Not Exists(select 'X' from ADS_SOTHAM_KHANGCAO k where k.donid = a.id)
--                                )
----                            OR EXISTS (
----                                    SELECT 'X' FROM ADS_PHUCTHAM_BANAN PTBA 
...
----                             OR EXISTS (
----                                    SELECT 'X' FROM ADS_PHUCTHAM_QUYETDINH PTQDVA 
----                                    LEFT JOIN DM_QD_QUYETDINH QD ON QD.ID=PTQDVA.QUYETDINHID                       LEFT JOIN DM_QD_LOAI QDL ON QDL.ID=QD.LOAIID 
----                                    WHERE  instr(',DC,',','||QDL.MA||',')>0
----                                    AND (V_TUNGAY IS NULL OR PTQDVA.NGAYQD>=VV_TUNGAY)
----                                    AND (V_DENNGAY IS NULL OR PTQDVA.NGAYQD<=VV_DENNGAY)
----                                    AND PTQDVA.DONID=a.id  AND GD.MAGIAIDOAN=3
...
--          --EXISTS(SELECT 'x' FROM ADS_DON_THAMPHAN PC WHERE   PC.CANBOID = V_THAMPHAN_ID  AND PC.DONID=A.ID )
--      open curReturn for
--        select a.ID,a.SOTHUTU,a.TENVUVIEC,a.TenToaAn,a.BAQD,a.KETQUA,a.Thamphan, vTotalItem as CountAll
--        from(
--          select ROW_NUMBER() OVER (ORDER BY a.SOTHUTU) as stt,a.ID,tl.SOTHULY||'<br/>'||to_char(tl.NGAYTHULY,'dd/MM/yyyy') as SOTHUTU,
--          'BA/QĐ:'||ba.SoBANAN||QSV.SOQD||'<br>'||to_char(ba.NGAYTUYENAN,'dd/MM/yyyy')||to_char(QSV.NGAYQD,'dd/MM/yyyy')  as BAQD ,a.TENVUVIEC,ta.TEN as TenToaAn,
--                 decode(hs.TRANGTHAIID,1,'Đã lưu',2,'Đã cho mươn',3,'Đã chuyển','') as KETQUA, cb.HOTEN as Thamphan
--          from ADS_DON a
--          INNER JOIN ADS_DON_GIAIDOAN GD ON A.ID=GD.DONID
--          inner join DM_TOAAN ta on a.TOAANID=ta.ID
--          left join (select donid,CANBOID from ADS_DON_THAMPHAN where MAVAITRO ='VTTP_GIAIQUYETSOTHAM') tp on tp.donid = a.id
--          left join DM_CANBO cb on cb.id =tp.CANBOID
--          left join ADS_SOTHAM_THULY tl on tl.donid = a.id
--          left join ADS_SOTHAM_BANAN ba on ba.donid = a.id
--          left join ADS_SOTHAM_QUYETDINH QSV on QSV.DONID=a.id
--          left join QLHS_STPT hs on hs.vuviecid = a.id
--          where a.TOAANID = vDonViID 
--                and ((vTrangThai = 1 and  Exists(select 'X' from QLHS_STPT hs where hs.vuviecid = a.id))
--                        OR (vTrangThai = 0 and Not Exists(select 'X' from QLHS_STPT hs where hs.vuviecid = a.id))
--                    )
...
--          inner join ADS_PHUCTHAM_BANAN ba on ba.donid = a.id
...
--             
--        --        and b.GQ_TINHTRANG=vTrangThai 
----                and ((vTrangThai = 1 and b.GQ_TINHTRANG = 1) or (vTrangThai = 0 and (b.GQ_TINHTRANG = 0 or b.GQ_TINHTRANG is null)))
----                and b.GQ_TOAANID=vDonViID
--                ;
--          -- Lấy dữ liệu theo PageIndex
--      open curReturn for
--        select a.ID,a.SOTHUTU,a.TENVUVIEC,a.TenToaAn,a.BAQD,a.KETQUA, vTotalItem as CountAll
--        from(
--          select ROW_NUMBER() OVER (ORDER BY a.SOTHUTU) as stt,a.ID,a.SOTHUTU,'BA/QĐ:'||ba.SoBANAN||'<br>'||to_char(ba.NGAYTUYENAN,'dd/MM/yyyy') as BAQD ,a.TENVUVIEC,ta.TEN as TenToaAn,
--                 null as KETQUA
--          from ADS_DON a
--          inner join DM_TOAAN ta on a.TOAANID=ta.ID
----          inner join ADS_SOTHAM_KHANGCAO b on a.ID=b.DONID
...
--            SELECT h.id INTO V_COUNT_CHECK FROM QLHS_STPT h WHERE h.VUVIECID=V_VUVIECID;
--            IF (V_COUNT_CHECK = 0) THEN
--                INSERT INTO QLHS_STPT 
--                    (ID, VUVIECID, LOAIAN, TRANGTHAIID, NGUOICHUYENID,TENNGUOICHUYEN,NGUOINHANID,
--                    TENNGUOINHAN,SOBUTLUC,GHICHU,NGAYGIAONHAN,NGUOITAO,NGAYTAO)
--                VALUES 
--                    (QLHS_STPT_SEQ.NEXTVAL, V_VUVIECID,V_LOAIAN,1,V_NGUOICHUYEN_ID,V_TENNGUOICHUYEN,V_NGUOINHAN_ID,
--                    V_TENNGUOINHAN,V_SOBUTLUC,V_GHICHU,V_NGAYGIAONHAN,V_NGUOITAO,SYSDATE);
--                V_COUNTS:=1;
--            ELSIF(V_COUNT_CHECK>0)THEN
--                 V_THONGBAO:='Hồ sơ vụ việc đã được lưu';
--            END IF;
--    ELSIF (V_TRANGTHAI = 2) THEN
--            SELECT h.id INTO V_COUNT_CHECK FROM QLHS_STPT h WHERE h.VUVIECID=V_VUVIECID;
--            IF (V_COUNT_CHECK = 1) THEN
--                UPDATE QLHS_STPT
--                    SET NGUOICHUYENID = V_NGUOICHUYEN_ID,
--                        TENNGUOICHUYEN = V_TENNGUOICHUYEN,
--                        NGUOINHANID     = V_NGUOINHAN_ID,
--                        TENNGUOINHAN    = V_TENNGUOINHAN,
--                        SOBUTLUC        = V_SOBUTLUC,
--                        GHICHU          = V_GHICHU,
--                        NGAYGIAONHAN    = V_NGAYGIAONHAN,
--                        NGUOISUA        = V_NGUOISUA,
--                        NGAYSUA         = SYSDATE
--                    WHERE VUVIECID = V_VUVIECID and LOAIAN = v_LOAIAN;
--                V_COUNTS:=1;
--            END IF;
--    END IF;
--END QLHOSO_INS_UP;
