create or replace NONEDITIONABLE PACKAGE BODY      PKG_GDTTT_HCTP_BC_APP AS
...
BEGIN
        ...
        FOR ITEM IN (
          SELECT ROW_NUMBER () OVER (ORDER BY D.NGAYTAO DESC) STT,
            ...
            D.LOAIDON,NVL (D.BAQD_LOAIQDBA, 0) BAQD_LOAIQDBA,D.NGUOITAO NGUOINHAP,D.DONGKHIEUNAI,
            D.ISNOTGDTTT,D.NGUOISUA,D.NGAYSUA,D.NGAYTAO NGAYNHAP,D.TL_NGAY,D.TL_SO,
            D.CD_SOCV,D.CD_NGAYCV,D.CD_NGUOIKY,D.ISSHOWFULL,
             CASE D.LOAIDON WHEN 1 THEN 'Đơn' WHEN 2 THEN 'Công văn' WHEN 3 THEN 'Đơn + Công văn'  END AS HINHTHUC             
            ...
            ,decode(SoTT.SOVB,null,SoTT_TLL.SOVB,SoTT.SOVB) CD_SOTOTRINH
            ,decode(SoTT.NGAYVB,null,SoTT_TLL.NGAYVB,SoTT.NGAYVB) CD_NGAYTOTRINH
            ,SoTT.SOVB||SoTT_TLL.SOVB || ' - '|| TO_CHAR (decode(SoTT.NGAYVB,null,SoTT_TLL.NGAYVB,SoTT.NGAYVB), 'dd/MM/yyyy')TOTRINH_SONGAY,D.THAMPHANID
            ,(CASE D.CD_LOAI WHEN 0 THEN 'block' ELSE 'none' END)ISSHOWNB
            ,(CASE D.CD_LOAI WHEN 0 THEN 'none' ELSE 'block' END)ISSHOWTK
            ,(CASE D.CD_TA_TRANGTHAI WHEN 0 THEN 'block' ELSE 'none' END)ISSHOWDDK
            ,(CASE D.CD_TA_TRANGTHAI WHEN 1 THEN 'block' ELSE 'none' END)ISSHOWCDDK
    ...
    IF(VTOAANID = 1) THEN
         V_TT_TP:=0;
          SELECT COUNT(*) INTO COUNT_TP FROM 
               ...
               --,PA.LOAIAN
        IF(COUNT_TP>0) THEN
            FOR ITEM_TP IN (SELECT  PA.TENTHAMPHAN,PA.LOAIAN, PA.SOTOTRINH, PA.NGAYTOTRINH FROM TABLE(V_TABLE)PA                              
                        ...
                        PA.SOTOTRINH, PA.NGAYTOTRINH  ORDER BY PA.TENTHAMPHAN)
            LOOP 
                     V_TT_TP:=V_TT_TP+1;
           ...
           DBMS_LOB.APPEND(V_EXPORT_TEXT,'
              ...
              <table cellpadding="2" cellspacing="1" style="font-family: times New Roman; font-size: 11pt; text-align: center; border-collapse: collapse;">
                    <tr><td colspan="11"></td></tr>
                    <tr align="center" style="text-align: center; font-weight: bold">
                        <td colspan="12" style="text-align: center; vertical-align: middle; font-size: 14pt;">Danh sách đơn vụ án '||V_LOAIAN ||' thụ lý '||VVNGAYNHAPTU||VVNGAYNHAPDEN||'
                        <br />
                   ...
                   DBMS_LOB.APPEND(V_EXPORT_TEXT,'
                    ...
                    <tr align="center" style="text-align: center; font-weight: bold">
                        <td colspan="8" style="vertical-align: top;"></td>
                        <td colspan="4" style="vertical-align: bottom;height:90px;">
                            <p style="font-size: 13pt;"><strong>'||V_CD_NGUOIKY||'</strong></p>
                        </td>
                    </tr>
                 <tr style="height: 0px;">
