--------------------------------------------------------
--  DDL for Package Body PKG_GDTTT_APP_CACC
--------------------------------------------------------

  CREATE OR REPLACE NONEDITIONABLE PACKAGE BODY "GSCM"."PKG_GDTTT_APP_CACC" AS
 ...
 -- array array_t := array_t(0); 
  ...
  BEGIN 
   ---------------
    ...
    WHERE  TOAANID=6 AND CREATE_DATE<=SYSDATE-2 ;
...
-- EXECUTE IMMEDIATE 'TRUNCATE TABLE GSCM.GDTTT_THONGKE_THAMPHAN'; 
    ...
    ------------------------
        ...
        FOR tp IN (SELECT c.ID,DECODE(d.MA,NULL,c.id,'PCA',0,'CA',0,c.ID) THAMPHANID,c.HOTEN,SUBSTR(c.HOTEN,INSTR(c.HOTEN,' ',-1)+ 1)ORDER_TEN,
                 ...
                 WHere c.TOAANID=6 and c.HieuLuc=1 and (c.MaDongBo is not null or Length(NVL(c.Madongbo,''))>0)
--                and c.id=9095
                 ------
--                 AND C.ID=20325--364 ----Nguyễn Thị Hoàng Anh -- AND C.ID=364 - Đặng Xuân đào
        ...
        LOOP
              FOR year_item IN (SELECT TT.NGAYTAO YEAR_ID,'Năm '||TT.NGAYTAO YEAR_TEN FROM (
                             ...
                             where a.TOAANID=6 AND a.NGAYTAO IS NOT NULL and ((a.ThamPhanID=tp.THAMPHANID AND tp.THAMPHANID!=0) OR tp.THAMPHANID=0)
                            ------------
--                             AND EXTRACT(year FROM a.NGAYTAO) IN (2019,2018,2017) ----lấy 3 năm để test
             ...
             FOR I IN 1..ARRAY.COUNT --trường hợp array(i)= 0 tất cả,1 án quốc hội, 3 án thời hiệu
                LOOP      
                    PKG_GDTTT_APP.TK_THAMPHAN_CREATE_DATA(6,tp.ID,NULL,NULL,array(I),year_item.YEAR_ID,V_CURSOR);
                    -- V_CURSOR:=V_CURSOR;
                    -----------------------
                   LOOP 
