          ...
          IF(V_COUNT_CHECK=0)THEN
           ...
           ----------
             ...
             VALUES (AHS_THAMPHANGIAIQUYET_SEQ.NEXTVAL,V_VUANID,V_THAM_PHAN_ID,V_VAITRO,TO_DATE(V_NGAYPHANCONGLD,'dd/MM/yyyy'),TO_DATE(V_NGAYPHANCONGTP,'dd/MM/yyyy'),V_PHANCONGLD_ID,
                sysdate,V_NGUOITAO,sysdate,V_NGUOITAO,V_THULY,V_FILE_ID,V_SOQD,TO_DATE(V_NGAYQD,'dd/MM/yyyy'));
              -----
             V_COUNTS:=1;
...
FUNCTION AHS_VUAN_GETALLPAGING_PRINT_OLD
(
    V_CAP_XET_XU_LOGIN  IN VARCHAR2,
    v_ten_vu_an in varchar2, 
    v_toidanh in varchar2, 
...
BEGIN	
    ...
    ------------------
     FOR item in (
           SELECT  ROW_NUMBER() OVER (ORDER BY DECODE(v_TINHTRANG_GIAIQUYET,2,a.NGAYTAO,3,PCTP_GQ.NGAYTAO) DESC) STT,COUNT(*) OVER () as CountAll,a.ID, a.MaVuAn, a.TenVuAn, a.TT, a.NgayBanCaoTrang,to_char(a.NgayTao,'dd/MM/yyyy')||'<br/>'||to_char(a.NgayTao,' HH24:MI:SS')NgayTao, a.NguoiTao,GD.MAGIAIDOAN, 
            DECODE(GD.MAGIAIDOAN,3,BC3.HoTen,BC2.HoTen) HoTenBiCan, decode(GD.MAGIAIDOAN,3,'</br><i>Tòa xét xử sơ thẩm: </i><b>'||b.Ten||'</b>',null) TenToaSoTham, 
            DECODE(GD.MAGIAIDOAN,3,GN.TruongHopGiaoNhan,AA.TruongHopGiaoNhan)TruongHopGiaoNhan,
            DECODE(GD.MAGIAIDOAN,2, 'Sơ thẩm'  , 3, 'Phúc thẩm' , 4, 'Thụ lý Giám đốc thẩm' ,'')  GiaiDoanVuViec,
            STBA.BANAN_QD_ST, STKN.KHANGNGHI_ST,
            ...
            ,PCTP_GQ.THULYID, 'Số QĐ: '||PCTP_GQ.SOQD SOQD,
            'Ngày QĐ: '||to_char(PCTP_GQ.NGAYQD,'dd/MM/yyyy')NGAYQD
            ,PCTP_GQ.ID THAMPHANGQ_ID
            ,'Trường hợp TL: '||PCTP_GQ.TRUONGHOPTHULY_TEN TRUONGHOPTHULY_TEN
            FROM AHS_VUAN A
            INNER JOIN AHS_VUAN_GIAIDOAN GD ON A.ID=GD.VUANID
            LEFT JOIN DM_TOAAN B ON A.TOAANID = B.ID
            ------Trạng thái giải quyết trong danh sách
            ...
            LEFT JOIN (SELECT TP.VUANID,'</br>- Đã phân công Thẩm phán' TINHTRANG_GQ FROM AHS_THAMPHANGIAIQUYET TP WHERE TP.MAVAITRO='VTTP_GIAIQUYETPHUCTHAM' 
                       GROUP BY TP.VUANID,'</br>- Đã phân công Thẩm phán')TPPCPT ON TPPCPT.VUANID=A.ID AND GD.MAGIAIDOAN=3 
            ----anhvh add 29/04/2020 hàm lấy 1 bản ghi mới nhất trong bảng AHS_THAMPHANGIAIQUYET      
            LEFT JOIN ( SELECT NNPC.HOTEN,NPC.HOTEN HOTEN_LD,DECODE(v_Capxx,2,THTL.TRUONGHOPTHULY_TEN,3,THTLPT.TRUONGHOPTHULY_TEN)TRUONGHOPTHULY_TEN
                        ,GG.* FROM AHS_THAMPHANGIAIQUYET GG
                        INNER JOIN (
                              ...
                              --   
                         LEFT JOIN (
                                     SELECT A.VUANID,A.ID, A.SOTHULY||' - '||C.TEN||' - '||TO_CHAR(A.NGAYTHULY,'dd/MM/yyyy')TRUONGHOPTHULY_TEN
                                     FROM AHS_SOTHAM_THULY A
                                     INNER JOIN (SELECT ID, TEN FROM DM_DATAITEM ) C  ON C.ID = A.TRUONGHOPTHULY
                                 )THTL ON THTL.VUANID=GG.VUANID 
                         LEFT JOIN (
                                     SELECT A.VUANID,A.ID, A.SOTHULY||' - '||C.TEN||' - '||TO_CHAR(A.NGAYTHULY,'dd/MM/yyyy')TRUONGHOPTHULY_TEN
                                     FROM AHS_PHUCTHAM_THULY A
                                     INNER JOIN (SELECT ID, TEN FROM DM_DATAITEM ) C  ON C.ID = A.TRUONGHOPTHULY
                                   )THTLPT ON THTLPT.VUANID=GG.VUANID 
           ...
           ------bị cáo lấy cho sơ thẩm
             ...
             ------bị cáo kháng cáo lấy cho phúc thẩm 
               LEFT JOIN(  SELECT TS.VUANID,'<br /><i>Bị cáo kháng cáo:</i> <br />'||REPLACE(REPLACE(REPLACE(REPLACE(TS.HOTEN,'_BOLD','<b>'),'_GHACH_NOI',' - '),'_DAUVU',' (đầu vụ)</b>'),';','<br/>')HOTEN FROM (
                                ...
                                FROM (                     
                                            SELECT COUNT(*)COUNT_BC,BC.VUANID,
                                            listagg (DECODE(BC.BICANDAUVU,1,'_BOLD'||BC.HOTEN||'_GHACH_NOI'||c.TenToiDanh||'_DAUVU',0,BC.HOTEN||'_GHACH_NOI'||c.TenToiDanh), ';') WITHIN GROUP (ORDER BY BC.BICANDAUVU DESC)||';' HOTEN
                                            FROM AHS_BICANBICAO BC  
                                            LEFT JOIN (SELECT CD.BICANID,CD.TENTOIDANH,CD.ISMAIN FROM AHS_SOTHAM_CAOTRANG_DIEULUAT CD WHERE ISMAIN=1) C ON BC.ID = C.BICANID
                                            INNER JOIN (SELECT NGUOIKCID, LOAIKHANGCAO,VUANID FROM AHS_SOTHAM_KHANGCAO) KC ON KC.NGUOIKCID = BC.ID AND KC.VUANID=BC.VUANID
                                            GROUP BY BC.VUANID
           ...
           DBMS_LOB.APPEND(V_EXPORT_TEXT_ITEM,' 
               <tr style="font-size: 12pt;padding:3pt;">
                ...
                <td style="text-align: left; vertical-align: middle; border: 0.1pt solid #000000;">
                                        <i style="margin-right: 3px;">Tên vụ án:</i>  <b>'||item.TenVuAn||'</b><br />
                                        <i style="margin-right: 3px;">Mã vụ án:</i>  <b>'||item.MavuAn||'</b><br />
                                        <i style="margin-right: 3px;">Cấp xét xử:</i>  <b>'||item.GiaiDoanVuViec||'</b><br />
                                        <i style="margin-right: 3px;">TH giao nhận:</i>  <b>'||item.TruongHopGiaoNhan||'</b>
                                        '||item.TenToaSoTham||item.BANAN_QD_ST||item.HoTenBiCan||item.KHANGNGHI_ST||'
                 </td>
                <td style="text-align: left; vertical-align: middle; border: 0.1pt solid #000000;">'||item.SOQD||'<br/>'||item.NGAYQD||'<br/>'||item.TRUONGHOPTHULY_TEN||'</td>
      ...
      DBMS_LOB.APPEND(V_EXPORT_TEXT,'
          <table cellpadding="1" cellspacing="1" style="font-family: times New Roman; font-size: 12pt; text-align: center; border-collapse: collapse;">
            <tr>
                <td colspan="6" style="height: 0pt;"></td>
            </tr>
            <tr>
                <td colspan="6" style="line-height: 100%; font-size: 14pt"><b>DANH SÁCH '||v_TINHTRANG_GQ_TEN||' THẨM PHÁN <br/>'||V_TOAAN_TEN||'</b>
                    <br /><i style="font-size: 12pt;">(Số liệu tính từ ngày '||V_TUNGAY_BC||' đến ngày '||V_DENNGAY_BC||')</i>
                </td>
            </tr>
            <tr>
                <td colspan="6" style="height: 15pt; text-align: left;"><b>Tổng số: '||CountAll_S||' bản ghi</b></td>
            </tr>
...
<td style="text-align: center; vertical-align: middle; border: 0.1pt solid #000000;">Ghi chú </td>
      ...
      ');
       OPEN V_CURSOR FOR
        ...
        RETURN V_CURSOR;       
END AHS_VUAN_GETALLPAGING_PRINT_OLD;
            ...
            CountAll_S:=item.CountAll;
               DBMS_LOB.APPEND(V_EXPORT_TEXT_ITEM,' 
                   ...
                   <td style="text-align: center; vertical-align: middle; border: 0.1pt solid #000000;">'||item.STT||'</td>
                    ...
                    <td style="text-align: center; vertical-align: middle; border: 0.1pt solid #000000;">'||item.THAMPHAN_TEN||'</td>
                    <td style="text-align: left; vertical-align: middle; border: 0.1pt solid #000000;"></td>
                    <td style="text-align: left; vertical-align: middle; border: 0.1pt solid #000000;"></td>
                    <td style="text-align: left; vertical-align: middle; border: 0.1pt solid #000000;"></td>
                    <td style="text-align: left; vertical-align: middle; border: 0.1pt solid #000000;"></td>
