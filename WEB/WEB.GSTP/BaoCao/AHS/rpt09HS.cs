...
{
    public partial class rpt09HS : DevExpress.XtraReports.UI.XtraReport
    {
        public rpt09HS()
        {
            InitializeComponent();
        ...
        {
            if (GetCurrentColumnValue("CANCU_DIEULUAT") + "" == "")
            {
                xrTable12.Rows[2].Visible = false;///
            }
            else
            {
                xrTable12.Rows[2].Visible = true;
            }
        }

        private void xrTableCell48_BeforePrint(object sender, CancelEventArgs e)
        {
            xrTableCell48.Text = xrTableCell48.Text.Replace("TAND", "Toà án nhân dân");
        }

        private void xrTableCell22_BeforePrint(object sender, CancelEventArgs e)
        {
            xrTableCell22.Text = xrTableCell22.Text.Replace("Trại tạm giam", "TTG").Replace("trại tạm giam", "TTG");
        }
    }
}
