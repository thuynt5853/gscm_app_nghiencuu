...
{
    public partial class DanSu : System.Web.UI.Page
    {
        ...
        private void LoadTreeGiaiDoan()
        {
            ...
            {
                DonID = Session[ENUM_LOAIAN.AN_DANSU] + "" == "" ? 0 : Convert.ToDecimal(Session[ENUM_LOAIAN.AN_DANSU] + "");
            }
            if (this.hsID != 0)
            {
                DonID = this.hsID;
            ...
            ADS_SOTHAM_THULY st = dt.ADS_SOTHAM_THULY.Where(x => x.DONID == DonID).FirstOrDefault<ADS_SOTHAM_THULY>();
            if (st != null)
            {
                XML = "<menuitem value='st' Text='Sơ thẩm " + ((DateTime)st.NGAYTHULY).ToString("dd/MM/yyyy") + "'></menuitem>";
            }
            ADS_PHUCTHAM_THULY pt = dt.ADS_PHUCTHAM_THULY.Where(x => x.DONID == DonID).FirstOrDefault<ADS_PHUCTHAM_THULY>();
            if (pt != null)
            {
        ...
        private void LoadData()
        {
            ...
            ADS_SOTHAM_BL oBLST = new ADS_SOTHAM_BL();
            ADS_PHUCTHAM_BL oBLPT = new ADS_PHUCTHAM_BL();
            DataTable oDT = null;
            ...
            if (pnThuLy.Visible)
            {
                ...
                {
                    oDT = oBLST.ADS_SOTHAM_THULY_GETLIST(DonID);
                }
                else if (hddGiaiDoan.Value == "pt")
                {
                    oDT = oBLPT.ADS_PHUCTHAM_THULY_GETLIST(DonID);
                }
            ...
            else if (pnHDXX.Visible)
            {
                if (hddGiaiDoan.Value == "st")
                {
                    oDT = oBLST.ADS_SOTHAM_HDXX_GETLIST(DonID);
                }
                else if (hddGiaiDoan.Value == "pt")
                {
                    oDT = oBLPT.ADS_PHUCTHAM_HDXX_GETLIST(DonID);
                }
                if (oDT != null && oDT.Rows.Count > 0)
                {
            ...
            {
                ADS_DON_DUONGSU_BL oBLDS = new ADS_DON_DUONGSU_BL();
                if (hddGiaiDoan.Value == "st")
                {
                    oDT = oBLDS.ADS_DON_DUONGSU_GETBY(DonID);
                }
                else if (hddGiaiDoan.Value == "pt")
                {
                    oDT = oBLDS.ADS_PHUCTHAM_DUONGSU_GETBY(DonID, 0);
                }
                if (oDT != null && oDT.Rows.Count > 0)
                {
                    total = oDT.Rows.Count; pageSize = dgListDuongSu.PageSize;
                    dgListDuongSu.CurrentPageIndex = Convert.ToInt16(hddPageIndex.Value) - 1;
                    if (hddGiaiDoan.Value == "st")// Sơ thẩm
                    {
                        // Ẩn cột Tham gia thụ lý Phúc thẩm
                        dgListDuongSu.Columns[1].Visible = false;
                    }
                    else if (hddGiaiDoan.Value == "pt")// Phúc thẩm
                    {
                        // Hiện cột Tham gia thụ lý Phúc thẩm
            ...
            else if (pnQuyetDinh.Visible)
            {
                if (hddGiaiDoan.Value == "st")
                {
                    oDT = oBLST.ADS_SOTHAM_QUYETDINH_GETLIST(DonID);
                }
                else if (hddGiaiDoan.Value == "pt")
                {
                    oDT = oBLPT.ADS_PHUCTHAM_QUYETDINH_GETLIST(DonID);
                }
                if (oDT != null && oDT.Rows.Count > 0)
                {
            ...
            else if (pnBanAn.Visible)
            {
                if (hddGiaiDoan.Value == "st")
                {
                    oDT = oBLST.ADS_SOTHAM_BANAN_CHITIET(DonID);
                }
                else if (hddGiaiDoan.Value == "pt")
                {
                    oDT = oBLPT.ADS_PHUCTHAM_BANAN_CHITIET(DonID);
                }
                if (oDT != null && oDT.Rows.Count > 0)
                {
            ...
            {
                oDT = oBLST.ADS_SOTHAM_KCaoKNghi_GETLIST(DonID);
