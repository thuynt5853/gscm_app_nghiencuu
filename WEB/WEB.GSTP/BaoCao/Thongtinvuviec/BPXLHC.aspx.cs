...
{
    public partial class BPXLHC : System.Web.UI.Page
    {
        ...
        private void LoadTreeGiaiDoan()
        {
            ...
            {
                DonID = Session[ENUM_LOAIAN.BPXLHC] + "" == "" ? 0 : Convert.ToDecimal(Session[ENUM_LOAIAN.BPXLHC] + "");
            }
            if (this.hsID != 0)
            {
                DonID = this.hsID;
            ...
            XLHC_SOTHAM_THULY st = dt.XLHC_SOTHAM_THULY.Where(x => x.DONID == DonID).FirstOrDefault<XLHC_SOTHAM_THULY>();
            if (st != null)
            {
                XML = "<menuitem value='st' Text='Sơ thẩm " + ((DateTime)st.NGAYTHULY).ToString("dd/MM/yyyy") + "'></menuitem>";
            }
            XLHC_PHUCTHAM_THULY pt = dt.XLHC_PHUCTHAM_THULY.Where(x => x.DONID == DonID).FirstOrDefault<XLHC_PHUCTHAM_THULY>();
            if (pt != null)
            {
        ...
        private void LoadData()
        {
            ...
            XLHC_SOTHAM_BL oBLST = new XLHC_SOTHAM_BL();
            XLHC_PHUCTHAM_BL oBLPT = new XLHC_PHUCTHAM_BL();
            DataTable oDT = null;
            ...
            if (pnThuLy.Visible)
            {
                ...
                {
                    oDT = oBLST.XLHC_SOTHAM_THULY_GETLIST(DonID);
                }
                else if (hddGiaiDoan.Value == "pt")
                {
                    oDT = oBLPT.XLHC_PHUCTHAM_THULY_GETLIST(DonID);
                }
            ...
            else if (pnHDXX.Visible)
            {
                if (hddGiaiDoan.Value == "st")
                {
                    oDT = oBLST.XLHC_SOTHAM_HDXX_GETLIST(DonID);
                }
                else if (hddGiaiDoan.Value == "pt")
                {
                    oDT = oBLPT.XLHC_PHUCTHAM_HDXX_GETLIST(DonID);
                }
                if (oDT != null && oDT.Rows.Count > 0)
                {
            ...
            {
                XLHC_DON_BL oDonBLST = new XLHC_DON_BL();
                XLHC_PHUCTHAM_BL oDonBLPT = new XLHC_PHUCTHAM_BL();
                if (hddGiaiDoan.Value == "st")
                {
                    oDT = oDonBLST.XLHC_DON_TGTT_GETLIST(DonID);
                }
                else if (hddGiaiDoan.Value == "pt")
                {
                    oDT = oDonBLPT.XLHC_PHUCTHAM_TGTT_GETLIST(DonID);
                }
                if (oDT != null && oDT.Rows.Count > 0)
                {
            ...
            else if (pnQuyetDinh.Visible)
            {
                if (hddGiaiDoan.Value == "st")
                {
                    oDT = oBLST.XLHC_SOTHAM_QUYETDINH_GETLIST(DonID);
                }
                else if (hddGiaiDoan.Value == "pt")
                {
                    oDT = oBLPT.XLHC_PHUCTHAM_QUYETDINH_GETLIST(DonID);
                }
                if (oDT != null && oDT.Rows.Count > 0)
                {
            ...
            else if (pnBanAn.Visible)
            {
                if (hddGiaiDoan.Value == "st")
                {
                    oDT = oBLST.XLHC_SOTHAM_BANAN_CHITIET(DonID);
                }
                else if (hddGiaiDoan.Value == "pt")
                {
                    oDT = oBLPT.XLHC_PHUCTHAM_BANAN_CHITIET(DonID);
                }
                if (oDT != null && oDT.Rows.Count > 0)
                {
                    ...
                    {
                        // ẩn cột Lý do
                        dgListBanAn.Columns[3].Visible = true;
                        dgListBanAn.Columns[5].Visible = false;
                    }
                    if (hddGiaiDoan.Value == "pt")
                    {
                        // ẩn cột Biện pháp
                        dgListBanAn.Columns[3].Visible = false;
                        dgListBanAn.Columns[5].Visible = true;
                    }
                }
            ...
            {
                oDT = oBLST.XLHC_SOTHAM_KCaoKNghi_GETLIST(DonID);
