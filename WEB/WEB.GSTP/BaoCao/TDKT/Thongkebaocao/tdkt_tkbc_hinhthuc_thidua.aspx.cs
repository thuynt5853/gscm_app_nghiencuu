...
{
    public partial class tdkt_tkbc_hinhthuc_thidua : System.Web.UI.Page
    {
        ...
        private void LoadDropHT_ThiDua()
        {
            string GroupName = ENUM_DANHMUC.DM_DANHHIEU_THIDUA;
            DM_DATAITEM_BL bl = new DM_DATAITEM_BL();
            DataTable tbl = bl.DM_DATAITEM_GETBYGROUPNAME(GroupName);
            if (tbl.Rows.Count > 0)
            {
                DropHT_ThiDua.DataSource = tbl;
                DropHT_ThiDua.DataTextField = "TEN";
                DropHT_ThiDua.DataValueField = "ID";
                DropHT_ThiDua.DataBind();
            }
            DropHT_ThiDua.Items.Insert(0, new ListItem("--- Chọn ---", "0"));
        }
        ...
        private void LoadReport()
        {
            ...
            decimal DonViID = Convert.ToDecimal(DropDonVi.SelectedValue)
                    , HinhThuc_ThiDuaID = Convert.ToDecimal(DropHT_ThiDua.SelectedValue)
