...
namespace WEB.GSTP.QLAN.ALD.Hoso
{
    public partial class Tailieu : System.Web.UI.Page
    {
        ...
        private void LoadDropNguoiBanGiao()
        {
            ...
            {
                List<ALD_DON_DUONGSU> lst = dt.ALD_DON_DUONGSU.Where(x => x.DONID == DonID).ToList();
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ALD_DON_TAILIEU donFile = new ALD_DON_TAILIEU();
                decimal TLID = (String.IsNullOrEmpty(hddID.Value)) ? 0 : Convert.ToDecimal(hddID.Value);
                if (TLID > 0)
                    donFile = dt.ALD_DON_TAILIEU.Where(x => x.ID == TLID).FirstOrDefault<ALD_DON_TAILIEU>();
                else
                    donFile = new ALD_DON_TAILIEU();
                try
                {
                ...
                if (hddID.Value == "" || hddID.Value == "0")
                {
                    ...
                    donFile.DONID = DonID;
                    dt.ALD_DON_TAILIEU.Add(donFile);
                }
                else
                {
        ...
        {
            lbthongbao.Text = "";
            decimal DonViID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_LAODONG] + "");
            int Total = 0;
            ALD_DON_BL donBL = new ALD_DON_BL();
            DataTable tbl = donBL.ALD_DON_BanGiaoTaiLieu_GETLIST(DonViID);
            if (tbl != null && tbl.Rows.Count > 0)
            {
        ...
        {
            ALD_DON_TAILIEU oT = dt.ALD_DON_TAILIEU.Where(x => x.ID == ID).FirstOrDefault();

            dt.ALD_DON_TAILIEU.Remove(oT);
            dt.SaveChanges();
            hddPageIndex.Value = "1";
            LoadGridNgayBanGiao();
            ResetControls();
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                switch (e.CommandName)
                {
                    ...
                    case "Sua":
                        hddFilePath.Value = "";
                        ALD_DON_TAILIEU bgtl = dt.ALD_DON_TAILIEU.Where(x => x.ID == ID).FirstOrDefault();
                        if (bgtl != null)
                        {
                        ...
                        break;
                    case "Download":
                        ALD_DON_TAILIEU oND = dt.ALD_DON_TAILIEU.Where(x => x.ID == ID).FirstOrDefault();
                        if (oND.TENFILE != "")
                        {
