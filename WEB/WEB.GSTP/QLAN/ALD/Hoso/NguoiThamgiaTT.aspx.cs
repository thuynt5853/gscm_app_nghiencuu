    ...
    public partial class NguoiThamgiaTT : System.Web.UI.Page
    {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ALD_DON_THAMGIATOTUNG oND;
                if (hddid.Value == "" || hddid.Value == "0")
                    oND = new ALD_DON_THAMGIATOTUNG();
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.ALD_DON_THAMGIATOTUNG.Where(x => x.ID == ID).FirstOrDefault();
                }
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    dt.ALD_DON_THAMGIATOTUNG.Add(oND);
                    dt.SaveChanges();
                }
                else
                {
        ...
        public void LoadGrid()
        {
            ...
            string current_id = Session[ENUM_LOAIAN.AN_LAODONG] + "";
            decimal ID = Convert.ToDecimal(current_id);
            DataTable oDT = oBL.ALD_DON_TGTT_GETLIST(ID);
        ...
        {
            ALD_DON_THAMGIATOTUNG oND = dt.ALD_DON_THAMGIATOTUNG.Where(x => x.ID == id).FirstOrDefault();
            dt.ALD_DON_THAMGIATOTUNG.Remove(oND);
            dt.SaveChanges();
            dgList.CurrentPageIndex = 0;
            LoadGrid();
            ResetControls();
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                decimal nguoiId = Convert.ToDecimal(rowView["ID"].ToString());
                ...
                //Comapre tòa id ra quyết định và tòa id hiện tại để check role
                //Cell 13 ứng với cột TOA_GIAIQUYET_ID
