...
checkHG.Checked == true ? txtHoaGiaiDenNgay.Text.Trim() : "", ddlQHPLTK.SelectedValue);
        ...
        protected void lbtimkiem_Click(object sender, EventArgs e)
        {
			...
			else
			{
                ...
                DataTable oDT = oBL.DON_NHAPTACH_SEARCH(Session["CAP_XET_XU"] + "", txtTenVuViec.Text.Trim(), txt_QHPL.Text.Trim(), txtMaVuViec.Text.Trim(), txtTENDUONGSU.Text.Trim(), dropCapxx.SelectedValue, DropToaAn.SelectedValue,
                                                DropTINHTRANG_THULY.SelectedValue, txtSOTHULY_THONGBAO.Text.Trim(), txt_NGAYTHULY_TU.Text, txt_NGAYTHULY_DEN.Text, ddlThamphan.SelectedValue, ddlHTND_Thuky.SelectedValue, Drop_Loaidon.SelectedValue, status, 5, pageindex, page_size);
				...
				if (oDT != null && oDT.Rows.Count > 0)
				{
					foreach (DataRow item in oDT.Rows)
					{
						...
						{
							ALD_DON dGoc = dt.ALD_DON.Where(x => x.ID == dNT.VUANGOCID).FirstOrDefault();
		...
		void Xoa_An_XxlaiST(MenuPermission oPer, decimal IDVuViec)
		{
			...
			ALD_DON oT = dt.ALD_DON.Where(x => x.ID == IDVuViec).FirstOrDefault();
			ALD_SOTHAM_THULY oT_TL = dt.ALD_SOTHAM_THULY.Where(x => x.DONID == IDVuViec).FirstOrDefault();
			...
			if (oT != null)
			{
				...
				{
					// xóa màn án phí
					List<ALD_ANPHI> lstAP = dt.ALD_ANPHI.Where(x => x.DONID == IDVuViec).ToList<ALD_ANPHI>();
					if (lstAP.Count > 0)
					{
						foreach (ALD_ANPHI ap in lstAP)
						{
							dt.ALD_ANPHI.Remove(ap);
						}
						dt.SaveChanges();
					}

					// xóa màn tài liệu
					List<ALD_DON_TAILIEU> lstTL = dt.ALD_DON_TAILIEU.Where(x => x.DONID == IDVuViec).ToList<ALD_DON_TAILIEU>();
					if (lstTL.Count > 0)
					{
						foreach (ALD_DON_TAILIEU tl in lstTL)
						{
							dt.ALD_DON_TAILIEU.Remove(tl);
						}
						dt.SaveChanges();
					}

					// xóa màn người tham gia tố tụng khác
					List<ALD_DON_THAMGIATOTUNG> lstTGTT = dt.ALD_DON_THAMGIATOTUNG.Where(x => x.DONID == IDVuViec).ToList<ALD_DON_THAMGIATOTUNG>();
					if (lstTL.Count > 0)
					{
						foreach (ALD_DON_THAMGIATOTUNG tgtt in lstTGTT)
						{
							dt.ALD_DON_THAMGIATOTUNG.Remove(tgtt);
						}
						dt.SaveChanges();
					}

					// xóa màn đơn khác
					List<DON_KHAC> lstDK = dt.DON_KHAC.Where(x => x.DONID == IDVuViec && x.LOAIANID == 5).ToList<DON_KHAC>();
					if (lstDK.Count > 0)
					{
					...
					List<ALD_DON_THAMPHAN> lstTP = dt.ALD_DON_THAMPHAN.Where(x => x.DONID == IDVuViec).ToList<ALD_DON_THAMPHAN>();
					if (lstTP.Count > 0)
					{
						foreach (ALD_DON_THAMPHAN tp in lstTP)
						{
							dt.ALD_DON_THAMPHAN.Remove(tp);
						}
						dt.SaveChanges();
					}
					// xóa đơn xử lý
					List<ALD_DON_XULY> lstDonXL = dt.ALD_DON_XULY.Where(x => x.DONID == IDVuViec).ToList<ALD_DON_XULY>();
					if (lstTP.Count > 0)
					{
						foreach (ALD_DON_XULY xuLy in lstDonXL)
						{
							dt.ALD_DON_XULY.Remove(xuLy);
						}
						dt.SaveChanges();
					}


					List<ALD_FILE> lstF = dt.ALD_FILE.Where(x => x.DONID == IDVuViec).ToList<ALD_FILE>();
					if (lstF.Count > 0)
					{
						foreach (ALD_FILE f in lstF)
						{
							dt.ALD_FILE.Remove(f);
						}
						dt.SaveChanges();
					}

					var delete_all_record_ALD_DON_DUONGSU_by_donid = dt.ALD_DON_DUONGSU.Where(x => x.DONID == IDVuViec);
					dt.ALD_DON_DUONGSU.RemoveRange(delete_all_record_ALD_DON_DUONGSU_by_donid);

					dt.ALD_DON.Remove(oT);
					dt.SaveChanges();

					//K: Nếu vụ việc tồn tại trong bảng DON_TIEPNHAN thì cập nhật trạng thái DON_GUINHAN = 4
					DON_TIEPNHAN dtn = dt.DON_TIEPNHAN.Where(x => x.DONID == IDVuViec && x.LOAIAN == 5).FirstOrDefault();
					if (dtn != null)
					{
					...
					GD.GIAIDOAN_DELETES("5", IDVuViec, 2);
					//---------------------------
		...
		void Xoa_An(MenuPermission oPer, decimal IDVuViec)
        {
            ...
            if (oT != null)
            {
                ...
                if (GiaiDoan == ENUM_GIAIDOANVUAN.SOTHAM)
                {
                    ...
                    // Kiểm tra Án phí
                    ALD_ANPHI anphi = dt.ALD_ANPHI.Where(x => x.DONID == IDVuViec).FirstOrDefault<ALD_ANPHI>();
                    if (anphi != null)
                    {
                    ...
                    ALD_DON_XULY xld = dt.ALD_DON_XULY.Where(x => x.DONID == IDVuViec).FirstOrDefault<ALD_DON_XULY>();
                    if (xld != null)
                    {
                    ...
                    ALD_DON_THAMPHAN gqd = dt.ALD_DON_THAMPHAN.Where(x => x.DONID == IDVuViec && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETDON).FirstOrDefault<ALD_DON_THAMPHAN>();
                    if (gqd != null)
                    {
                    ...
                    ALD_DON_TAILIEU tailieu = dt.ALD_DON_TAILIEU.Where(x => x.DONID == IDVuViec).FirstOrDefault<ALD_DON_TAILIEU>();
                    if (tailieu != null)
                    {
                    ...
                    ALD_DON_THAMGIATOTUNG tgtt = dt.ALD_DON_THAMGIATOTUNG.Where(x => x.DONID == IDVuViec).FirstOrDefault<ALD_DON_THAMGIATOTUNG>();
                    if (tgtt != null)
                    {
                    ...
                    //ALD_DON_DUONGSU ds = dt.ALD_DON_DUONGSU.Where(x => x.DONID == IDVuViec && x.ISDAIDIEN != 1).FirstOrDefault<ALD_DON_DUONGSU>();
                    //if (ds != null)
                    //{
                    ...
                    DON_CHITIET dct = dt.DON_CHITIET.Where(x => x.DONID == IDVuViec && x.LOAIANID == 5).FirstOrDefault();
                    DON_KHAC dk = dt.DON_KHAC.Where(x => x.DONID == IDVuViec && x.LOAIANID == 5).FirstOrDefault();
                    if (dct != null && dk != null)
                    {
		...
		{
			String MaLoaiVuAn = ENUM_LOAIAN.AN_LAODONG;
		...
		protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
		{
			...
			if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
			{
				...
				if (hddCHECK_THULY.Value == "" || Session[ENUM_SESSION.SESSION_NHOMNSDID] + "" == "1")
				{
					...
					else
					{
						...
						else
						{
							...
							if (VuAnID > 0)
							{
								List<ALD_DON_XULY> oDON_XLY = dt.ALD_DON_XULY.Where(x => x.DONID == VuAnID).ToList<ALD_DON_XULY>();
								...
								if ((dv["NGUOITAO"].ToString().ToLower() == strUserName.ToLower() || Session[ENUM_SESSION.SESSION_NHOMNSDID] + "" == "1" || Session[ENUM_SESSION.SESSION_NHOMNSDID] + "" == "541")
									...
									lbtXoa.Visible = false;
								ALD_DON_XULY oDXL_TL = dt.ALD_DON_XULY.Where(x => (x.DONID == VuAnID || x.DON_XULYID == VuAnID) && x.LOAIGIAIQUYET != 3).FirstOrDefault();
								if (oDON_XLY.Count > 0 && oDXL_TL == null)
								{
									e.Item.Cells[5].Text = "- Trả lại đơn";
								}
								var chuyenAn = dt.ALD_CHUYEN_NHAN_AN.Where(x => x.VUANID == VuAnID && x.TOACHUYENID == id_toaan).FirstOrDefault();
								ALD_DON_XULY oDXL_CD = dt.ALD_DON_XULY.Where(x => (x.DONID == VuAnID || x.DON_XULYID == VuAnID) && x.LOAIGIAIQUYET != 1).FirstOrDefault();
				...
				//    }
				//}
				string Result = new ALD_CHUYEN_NHAN_AN_BL().Check_NhanAn(VuAnID, "");
				if (Result != "")
				{
		...
		protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
		{
			...
			switch (e.CommandName)
			{
				...
				case "Select"://Lựa chọn vụ việc cần Lưu thông tin                  
					...
					List<ALD_DON_GIAIDOAN> gd = dt.ALD_DON_GIAIDOAN.AsNoTracking().Where(x => x.DONID == IDVuViec).ToList();
					...
					ALD_DON oDon = dt.ALD_DON.Where(x => x.ID == IDVuViec).FirstOrDefault();
				...
				case "Xoa":
					...
					if (Session[ENUM_SESSION.SESSION_NHOMNSDID] + "" == "1")
					{
						...
						if (oBL1.HISTORY_ALLDATA_BY_VUANID(IDVuViec, 5, Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Danh sách án Lao động", "Xóa", json) == false)
						{
					...
					else
					{
						...
						if (IDVuViec == VuAnID)
							Session[ENUM_LOAIAN.AN_LAODONG] = 0;
					}
		...
		private void createHoso_xetxulaiPhuctham(decimal vDonID)
		{
			...
			ALD_DON oDon = dt.ALD_DON.Where(x => x.ID == vDonID).FirstOrDefault();
			if (oDon != null)
			{
				...
				Session[ENUM_LOAIAN.AN_LAODONG] = oDON_new.ID;

				//them ma giai doan cap Phuc tham 
				GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
				GD.GAIDOAN_INSERT_UPDATE_XXLAI_PHUCTHAM("5", oDON_new.ID, 3, (decimal)oDON_new.TOAANID, LoginDonViID, 0, 0, 0);

				//Câp dương su vụ án
				List<ALD_DON_DUONGSU> lst = dt.ALD_DON_DUONGSU.Where(x => x.DONID == vDonID).ToList<ALD_DON_DUONGSU>();
				foreach (ALD_DON_DUONGSU vDuongsu_old in lst)
				{
					...
					//Ban giao tai lieu
					List<ALD_DON_TAILIEU> lstTaiLieu = dt.ALD_DON_TAILIEU.Where(x => x.DONID == vDonID && x.NGUOIBANGIAO == vDuongsu_old.ID).ToList<ALD_DON_TAILIEU>();
				...
				// Phan cong Tham phan
				List<ALD_DON_THAMPHAN> lstThamPhan = dt.ALD_DON_THAMPHAN.Where(x => x.DONID == vDonID && x.MAVAITRO == "VTTP_GIAIQUYETDON").ToList<ALD_DON_THAMPHAN>();
				...
				//Xu ly don
				List<ALD_DON_XULY> lstXulyDon = dt.ALD_DON_XULY.Where(x => x.DONID == vDonID && x.TOAANID == oDon.TOAANID).ToList<ALD_DON_XULY>();
				...
				//Tam ung an phi
				List<ALD_ANPHI> lstAP = dt.ALD_ANPHI.Where(x => x.DONID == vDonID).ToList();
				...
				ALD_SOTHAM_BANAN vBA_old = dt.ALD_SOTHAM_BANAN.Where(x => x.DONID == vDonID).FirstOrDefault();
				if (vBA_old != null)
				{
					ALD_SOTHAM_BANAN vBA_new = new ALD_SOTHAM_BANAN();
					...
					dt.ALD_SOTHAM_BANAN.Add(vBA_new);
					dt.SaveChanges();
				}
				List<ALD_SOTHAM_QUYETDINH> LstQD = dt.ALD_SOTHAM_QUYETDINH.Where(x => x.DONID == vDonID).ToList();
				if (LstQD.Count > 0)
				{
					foreach (ALD_SOTHAM_QUYETDINH vQD_old in LstQD)
					{
						ALD_SOTHAM_QUYETDINH vQD_new = new ALD_SOTHAM_QUYETDINH();
						...
						dt.ALD_SOTHAM_QUYETDINH.Add(vQD_new);
						dt.SaveChanges();
					}
				}
				//Chuyen nhan an
				List<ALD_CHUYEN_NHAN_AN> LstCN = dt.ALD_CHUYEN_NHAN_AN.Where(x => x.VUANID == vDonID).ToList();
				foreach (ALD_CHUYEN_NHAN_AN voND_old in LstCN)
				{
					ALD_CHUYEN_NHAN_AN oND = new ALD_CHUYEN_NHAN_AN();
		...
		protected void cmdTachan_Click(object sender, EventArgs e)
		{
			...
			ALD_SOTHAM_THULY oNSD = new ALD_SOTHAM_THULY();
			foreach (DataGridItem Item in dgList.Items)
			{
				...
				if (chkChon.Checked)
				{
					...
					oNSD = dt.ALD_SOTHAM_THULY.Where(x => x.DONID == vuAnGocId).FirstOrDefault();
				}
			}
			if (vuAnGocId == 0)
			{
			...
			{
				string link = "/QLAN/ALD/Hoso/Popup/pTachAn.aspx?DonID=" + vuAnGocId;
		...
		protected void cmdNhapan_Click(object sender, EventArgs e)
		{
			...
			{
				string link = "/QLAN/ALD/Hoso/Popup/pNhapAn.aspx?DonID=" + vuAnGocId;
				ScriptManager.RegisterStartupScript(this, this.GetType(), "onclick", "var Mleft = (screen.width/2)-(1200/2); var Mtop = (screen.height/2)-(750/2); javascript:window.open('" + link + "', '_blank', 'height=750px,width=1200px,top=\'+Mtop+\', left=\'+Mleft+\',resizable=yes,toolbar=yes,scrollbars=1'); ", true);
				Cls_Comon.CallFunctionJS(this, this.GetType(), "LoadModalDiv()");
			}
		}
        ...
        {
            string StrMsg = "PopupCenter('/QLAN/pDanhSachAnPhi.aspx?hsID=" + ENUM_LOAIVUVIEC_NUMBER.AN_LAODONG.ToString() + "');";
        ...
        {
            string link = "/QLAN/ALD/Hoso/Popup/pTKQuaHan.aspx";
            ScriptManager.RegisterStartupScript(this, this.GetType(), "onclick", "var Mleft = (screen.width/2)-(1200/2); var Mtop = (screen.height/2)-(750/2); javascript:window.open('" + link + "', '_blank', 'height=750px,width=1200px,top=\'+Mtop+\', left=\'+Mleft+\',resizable=yes,toolbar=yes,scrollbars=1'); ", true);
            Cls_Comon.CallFunctionJS(this, this.GetType(), "LoadModalDiv()");
        }
		...
		private void createHoso_xetxulaiSotham(decimal vDonID)
		{
			...
			if (oDon != null)
			{
				...
				//List<ALD_FILE> aDSFiles = dt.ALD_FILE.Where(x => x.DONID == vDonID).ToList<ALD_FILE>();
				//foreach (ALD_FILE aDSFile_OLD in aDSFiles)
				//{
				//	ALD_FILE ADS_FILE_NEW = new ALD_FILE()
				//	{
                ...
                List<ALD_FILE> oldFiles = dt.ALD_FILE
                ...
                Dictionary<ALD_FILE, ALD_FILE> tempMap = new Dictionary<ALD_FILE, ALD_FILE>();
                foreach (ALD_FILE oldFile in oldFiles)
                {
                    ...
                    dt.ALD_FILE.Add(newFile);
                    tempMap.Add(oldFile, newFile);

                }
                dt.SaveChanges();
                foreach (KeyValuePair<ALD_FILE, ALD_FILE> kv in tempMap)
                {
				...
				List<DON_CHITIET> dON_CHITIETs = dt.DON_CHITIET.Where(x => x.DONID == vDonID && x.LOAIANID == 5).ToList<DON_CHITIET>();
				...
				List<DON_DUONGSU_CHITIET> dON_DUONGSU_CHITIETs = dt.DON_DUONGSU_CHITIET.Where(x => x.DONID == vDonID && x.LOAIAN == 2).ToList<DON_DUONGSU_CHITIET>();
				...
				List<ALD_DON_THAMGIATOTUNG> aDS_DON_THAMGIATOTUNGs = dt.ALD_DON_THAMGIATOTUNG.Where(x => x.DONID == vDonID).ToList<ALD_DON_THAMGIATOTUNG>();
				foreach (ALD_DON_THAMGIATOTUNG ADS_DON_THAMGIATOTUNG_OLD in aDS_DON_THAMGIATOTUNGs)
				{
					ALD_DON_THAMGIATOTUNG ADS_DON_THAMGIATOTUNG_NEW = new ALD_DON_THAMGIATOTUNG();
					...
					dt.ALD_DON_THAMGIATOTUNG.Add(ADS_DON_THAMGIATOTUNG_NEW);
					dt.SaveChanges();
				}

				//Xu ly don
				List<ALD_DON_XULY> lstXulyDon = dt.ALD_DON_XULY.Where(x => (x.DONID == vDonID || x.DON_XULYID == vDonID) && x.TOAANID == oDon.TOAANID).ToList<ALD_DON_XULY>();
				foreach (ALD_DON_XULY vXulyDon_old in lstXulyDon)
				{
					ALD_DON_XULY vXulyDon_new = new ALD_DON_XULY();
					...
					DataTable donYCBSes = oBLYC.ALD_GETALL_DON_YCBS(vDonID, vXulyDon_old.ID, 1, 100);
				...
				foreach (ALD_ANPHI vAP_old in lstAP)
				{
					...
					{
						DAL.GSTP.DVCQG_THANH_TOAN dVCQG_THANH_TOAN = dt.DVCQG_THANH_TOAN.Where(x => x.ANPHI_ID == vAP_old.ID && x.MALOAIVUVIEC == "5").FirstOrDefault();
					...
					List<ALD_ANPHI_DUONGSU> aDS_ANPHI_DUONGSUs = DataExtensions.GetAllWithClause<ALD_ANPHI_DUONGSU>($" ANPHI_ID == {vAP_old.ID}");
					if (aDS_ANPHI_DUONGSUs != null && aDS_ANPHI_DUONGSUs.Count > 0)
					{
						foreach (ALD_ANPHI_DUONGSU aDS_ANPHI_DUONGSU_OLD in aDS_ANPHI_DUONGSUs)
						{
							ALD_ANPHI_DUONGSU aDS_ANPHI_DUONGSU_NEW = new ALD_ANPHI_DUONGSU();
							...
							DataExtensions.Insert<ALD_ANPHI_DUONGSU>(aDS_ANPHI_DUONGSU_NEW);
						}
					}

				}

				List<DON_MIENANPHI> dON_MIENANPHIs = DataExtensions.GetAllWithClause<DON_MIENANPHI>($" DONID = {vDonID} AND LOAIAN = 5");
				...
				List<DON_KHAC> dON_KHAC_OLDs = dt.DON_KHAC.Where(x => x.DONID == vDonID && x.LOAIANID == 5 && x.LOAIDON != 7).ToList<DON_KHAC>();
				...
				GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
				GD.GAIDOAN_INSERT_UPDATE_XXLAI_SOTHAM("5", oDON_new.ID, 2, (decimal)LoginDonViID, 0, 0, 0, 0);
			}
