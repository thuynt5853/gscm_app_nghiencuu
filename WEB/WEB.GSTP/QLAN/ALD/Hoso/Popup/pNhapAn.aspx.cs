...
namespace WEB.GSTP.QLAN.ALD.Hoso.Popup
{
    public partial class pNhapAn : System.Web.UI.Page
    {
                ...
                else
                {
                    ...
                    if (ds_vuAnConID.Count > 0)
                    {
                        foreach (decimal vuAnConID in ds_vuAnConID)
                        {
                            ...
                            try
                            {
                                ALD_DON donCon = dt.ALD_DON.Where(x => x.ID == vuAnConID).FirstOrDefault();

                                //Tạo bản ghi DON_NHAPTACH
                                DON_NHAPTACH donNhap = new DON_NHAPTACH();
                                donNhap.DONID = donCon.ID;
                                donNhap.LOAIANID = 5; //ALD
                                ...
                                List<ALD_DON_DUONGSU> dsDuongSu = dt.ALD_DON_DUONGSU.Where(x => x.DONID == vuAnConID).ToList();
                                string jsonDuongSu = ",";
                                ...
                                //Lấy ds thẩm phán
                                List<ALD_DON_THAMPHAN> dsThamPhan = dt.ALD_DON_THAMPHAN.Where(x => x.DONID == vuAnConID).ToList();
                                string jsonThamPhan = ",";
                                ...
                                //Lấy ds thư kí
                                List<ALD_SOTHAM_HDXX> dsSoThamHDXX = dt.ALD_SOTHAM_HDXX.Where(x => x.DONID == vuAnConID && x.MAVAITRO == "THUKY").ToList();
                                List<ALD_PHUCTHAM_HDXX> dsPhucThamHDXX = dt.ALD_PHUCTHAM_HDXX.Where(x => x.DONID == vuAnConID && x.MAVAITRO == "THUKY").ToList();
                                string jsonThuKy = ",";
                                ...
                                ALD_SOTHAM_THULY thuLy = dt.ALD_SOTHAM_THULY.Where(x => x.DONID == vuAnConID).FirstOrDefault();
                                int ttTL = 0;
                                string ngayTL = "", soTL = "";
                                if (thuLy != null)
                                {
                    ...
                    //Án lao động
                    string tenChucNang = "Nhập án Lao động từ " + donCon.MAVUVIEC + " đến " + donGoc.MAVUVIEC;
                    ...
                    string jsonString = JsonConvert.SerializeObject(objNoiDung);
                                ...
                                adsBL.HISTORY_ALLDATA_BY_VUANID(vuAnConID, 5, nguoiXoa, taiKhoanXoa, tenChucNang, hanhDong, jsonString);

                                //Cập nhật DONID của các bảng liên quan:
                                ALD_DON_BL update = new ALD_DON_BL();
                    ...
                    dt.SaveChanges();
                                ...
                                ALD_DON_XULY donXL = dt.ALD_DON_XULY.Where(x => x.DONID == vuAnConID).FirstOrDefault();
                                if (donXL != null)
                                {
                                    donXL.DONID = null;
        ...
        //            count++;
        //            vuAnConID = Convert.ToDecimal(Item.Cells[0].Text);
        //            oNSD = dt.ALD_SOTHAM_THULY.Where(x => x.DONID == vuAnConID).FirstOrDefault();
        //        }
        //    }
        //    if (vuAnConID == 0)
        //    {
