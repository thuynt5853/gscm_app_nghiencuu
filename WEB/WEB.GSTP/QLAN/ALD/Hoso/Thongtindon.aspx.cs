...
namespace WEB.GSTP.QLAN.ALD.Hoso
{
    public partial class Thongtindon : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    {
                        // hoangndh: update giong an DS 060725
                        SetTinhHuyenMacDinh();
                    ...
                    if (strtype != "new")
                    {
                        ...
                        if (!string.IsNullOrEmpty(current_id))
                        {
                            ...
                            List<ALD_DON_THAMPHAN> oDON_THAMPHAN = dt.ALD_DON_THAMPHAN.Where(x => x.DONID == Id).ToList<ALD_DON_THAMPHAN>();
                            ...
                            List<ALD_DON_XULY> oDON_XLY = dt.ALD_DON_XULY.Where(x => x.DONID == Id).ToList<ALD_DON_XULY>();
                            if (oDON_XLY.Count > 0)
                            {
            ...
            {
                lstMsgT.Text = lstMsgB.Text = "Lỗi: " + ex;
            }
        ...
        private void LoadInfo(decimal ID)
        {
            ...
            KHOBAQD_BL aldDon = new KHOBAQD_BL();
            ...
            if (lstNguyendon.Count > 0)
            {
                ...
                if (hdTrangThaiXacThucND.Value == "1")
                {
                    cmdGet037ND.Enabled = chkBoxCMNDND.Enabled = chkKhongLamSachND.Enabled = false;
                }
                else
                {
                    cmdGet037ND.Enabled = chkBoxCMNDND.Enabled = chkKhongLamSachND.Enabled = true;
                }
            }
            #endregion
            //Load Bị đơn
            ...
            if (lstBidon.Count > 0)
            {
                ...
                if (ddlLoaiBidon.SelectedValue == "1")
                {
                    ...
                    chkBD_IsNguoitantat.Visible = true;
                    if (oBD.ISNGUOITANTAT == 1) chkBD_IsNguoitantat.Checked = true;
                }
                else
                {
                    pnBD_Canhan.Visible = false;
                    pnBD_Tochuc.Visible = true;
                    chkBD_IsNguoitantat.Visible = false;
                }
                ...
                if (hdTrangThaiXacThucBD.Value == "1")
                {
                    cmdGet037BD.Enabled = chkBoxCMNDBD.Enabled = chkKhongLamSachBD.Enabled = false;
                }
                else
                {
                    cmdGet037BD.Enabled = chkBoxCMNDBD.Enabled = chkKhongLamSachBD.Enabled = true;
                }
            }
            else
            {
        ...
        private bool CheckValid()
        {
            ...
            if (ddlLoaiNguyendon.SelectedValue == "1" && ddlND_Gioitinh.SelectedValue == "")
            {
                lstMsgT.Text = "Bạn chưa chọn giới tính";
                return false;
            }
            
            if (ddlLoaiBidon.SelectedValue == "1" && ddlBD_Gioitinh.SelectedValue == "")
            {
                lstMsgT.Text = "Bạn chưa chọn giới tính";
                return false;
            }
            ...
            {
                decimal ID = Convert.ToDecimal(hddID.Value);
                var oT = dt.ALD_DON.Where(x => x.ID == ID).FirstOrDefault();
                if (ckbTienHanhHG.Checked == false)
                {
                    if (oT.HOAGIAI_TRANGTHAI.GetValueOrDefault(0) > 0)
                    {
                        HOAGIAI_DON hgdon = DataExtensions.GetAllWithClause<HOAGIAI_DON>($"VUVIECID = {oT.ID} AND LOAIANID = {ENUM_LOAIVUVIEC_NUMBER.AN_LAODONG.toNumber()}").FirstOrDefault();
                        if (hgdon != null)
                        {
        ...
        private bool SaveData()
        {
            try
            {
                ...
                if (hddID.Value == "" || hddID.Value == "0")
                {
                   ...
                   // oT.MAVUVIEC = ENUM_LOAIVUVIEC.AN_LAODONG + Session[ENUM_SESSION.SESSION_MADONVI] + oT.TT.ToString();
                    ...
                    //Sinh ma vu viec
                    oT.MAVUVIEC = ENUM_LOAIVUVIEC.AN_LAODONG + "." + Session[ENUM_SESSION.SESSION_MADONVI] + "." + oT.ID.ToString();
                    dt.SaveChanges();
                ...
                if (ckbTienHanhHG.Checked)
                {
                    ...
                    HOAGIAI_DON donHG = DataExtensions.GetAllWithClause<HOAGIAI_DON>($"VUVIECID = {oT.ID} AND LOAIANID = {ENUM_LOAIVUVIEC_NUMBER.AN_LAODONG.toNumber()}").FirstOrDefault();
                    if (donHG == null)
                    {
                        donHG = new HOAGIAI_DON()
                        {
                            LOAIANID = ENUM_LOAIVUVIEC_NUMBER.AN_LAODONG.toNumber(),
                ...
                if (Convert.ToDecimal(ddlLoaiNguyendon.SelectedValue) == 1) { oND.GIOITINH = Convert.ToDecimal(ddlND_Gioitinh.SelectedValue); };
                ...
                if (chkKhongCoNYC2.Checked == false)
                {
                    ...
                    if (chkBD_IsNguoitantat.Visible)
                        oBD.ISNGUOITANTAT = chkBD_IsNguoitantat.Checked ? 1 : 0;
                    else
                        oBD.ISNGUOITANTAT = 0;
                    ...
                    if (Convert.ToDecimal(ddlLoaiBidon.SelectedValue) == 1)
                    {
        ...
        protected void ddlLoaiBidon_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ddlLoaiBidon.SelectedValue == "1")
            {
                ...
                pnBD_Tochuc.Visible = false;
                chkBD_IsNguoitantat.Visible = true;
                Cls_Comon.SetFocus(this, this.GetType(), chkBD_IsNguoitantat.ClientID);
            }
            else
            {
                pnBD_Canhan.Visible = false;
        ...
        protected void ddlLoaiQuanhe_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                lbl_ttnguyendon.Text = "Tên nguyên đơn";
        ...
        protected void ddlLoaiNguyendon_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ddlLoaiNguyendon.SelectedValue == "1")
            {
                ...
                chkISBVQLNK.Visible = false;
                chkND_IsNguoitantat.Visible = true;
                Cls_Comon.SetFocus(this, this.GetType(), chkND_IsNguoitantat.ClientID);
            }
            else
            {
                pnNDCanhan.Visible = false;
        ...
        private void setToaGiaiQuyetId()
        {
            ...
            if (current_id == "" || current_id == "0")
            {
                current_id = Session[ENUM_LOAIAN.AN_LAODONG].ToString();
            }
        ...
        protected void btnGet037BD_Click(object sender, EventArgs e)
        {
            ...
            if (quocTich == "2")
            {
                ...
                else
                {
                    ...
                    Session.Remove("CongDan");
                    Session["CongDan"] = CongDan;
                    string StrMsg = "PopupCenter('/QLAN/ALD/Hoso/Popup/pGetDuongSu037.aspx?caller=DuongSuBD','Thông tin công dân từ hệ thống Cơ sở dữ liệu quốc gia về dân cư',850,700);";
                    ScriptManager.RegisterStartupScript(this, this.GetType(), Guid.NewGuid().ToString(), StrMsg, true);
                }
            }
            else
