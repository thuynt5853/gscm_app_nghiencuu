...
namespace WEB.GSTP.QLAN.ALD
{
    public partial class Sauxetxu : System.Web.UI.Page
    {
        ...
        private void LoadGrid()
        {
            ...
            DataTable oDT = oBL.ALD_RUTKN(vDonViID, txtMaVuViec.Text, txtTenVuViec.Text, txtSoQDBA.Text, txtTenDuongSu.Text, dFrom, dTo, Convert.ToDecimal(rdbTrangthai.SelectedValue), pageIndex, pageSize);
            int Total = 0;
        ...
        protected void cmdCapnhat_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    decimal vuViecID = 0; ALD_SAUXETXU obj; bool isNew = false; ALD_DON don;
                    foreach (DataRow row in dsSelected.Rows)
                    {
                        vuViecID = Convert.ToDecimal(row["ID"].ToString());
                        obj = dt.ALD_SAUXETXU.Where(x => x.VUANID == vuViecID).FirstOrDefault<ALD_SAUXETXU>();
                        if (obj == null)
                        {
                            isNew = true;
                            obj = new ALD_SAUXETXU();
                        ...
                        obj.VUANID = vuViecID;
                        don = dt.ALD_DON.Where(x => x.ID == vuViecID).FirstOrDefault<ALD_DON>();
                        if (don != null)
                        {
                            if (don.MAGIAIDOAN == ENUM_GIAIDOANVUAN.SOTHAM)// lưu sơ thẩm
                            {
                                obj.ST_ISRUTKN = 1;
                ...
                {
                    lbthongBaoUpdate.Text = "Không có vụ việc cần rút kinh nghiệm!";
                }
            }
        ...
        private DataTable CreateTableSelected()
        {
            ...
            dsSelected.Columns.Add("MAVUVIEC", typeof(string));
            dsSelected.Columns.Add("TENVUVIEC", typeof(string));
            return dsSelected;
        }
        private void LoadGridSelected()
        {
            ...
            {
                // load thông tin
                decimal DonID = Convert.ToDecimal(dsSelected.Rows[0]["ID"].ToString());
                ALD_SAUXETXU sx = dt.ALD_SAUXETXU.Where(x => x.VUANID == DonID).FirstOrDefault<ALD_SAUXETXU>();
                if (sx != null)
                {
                    ALD_DON don = dt.ALD_DON.Where(x => x.ID == DonID).FirstOrDefault<ALD_DON>();
                    if (don != null)
                    {
                        if (don.TOAANID == don.TOAPHUCTHAMID)// lưu sơ thẩm
                        {
        ...
        protected void cmdTTSauXetXu_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridItem Item in dgList.Items)
            {
                ...
                if (chkChon.Checked)
                {
                    ...
                    row["MAVUVIEC"] = Item.Cells[3].Text;
                    row["TENVUVIEC"] = Item.Cells[4].Text;
        ...
        public void loadedit(decimal ID, String MAGIAIDOAN)
        {
            ...
            if (MAGIAIDOAN == "Sơ thẩm")
                oND = dt.ALD_SAUXETXU.Where(x => x.VUANID == ID && x.ST_ISRUTKN == 1).FirstOrDefault();
            else
                oND = dt.ALD_SAUXETXU.Where(x => x.VUANID == ID && x.PT_ISRUTKN == 1).FirstOrDefault();

            if (oND != null)
            {
                if (dsSelected != null) dsSelected.Clear();
                ALD_DON oVA = dt.ALD_DON.Where(x => x.ID == oND.VUANID).FirstOrDefault();

                DataRow row = dsSelected.NewRow();
                row["ID"] = oVA.ID;
                row["MAVUVIEC"] = oVA.MAVUVIEC;
                row["TENVUVIEC"] = oVA.TENVUVIEC;
                dsSelected.Rows.Add(row);
