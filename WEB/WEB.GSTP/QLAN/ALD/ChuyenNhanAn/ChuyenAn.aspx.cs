...
namespace WEB.GSTP.QLAN.ALD.ChuyenNhanAn
{
    public partial class ChuyenAn : System.Web.UI.Page
    {
        ...
        private void LoadGrid()
        {
            ...
            DataTable oDT = oBL.LD_CHUYENAN(vDonViID, txtTenToa.Text.Trim(), txtMaVuViec.Text.Trim(), txtTenVuViec.Text.Trim(), txtSoquyetdinh.Text.Trim(), txtSobanan.Text.Trim(), dFrom, dTo, txtTenduongsu.Text.Trim(), Convert.ToDecimal(rdbTrangthai.SelectedValue), Convert.ToDecimal(rdbLoai.SelectedValue));
        ...
        protected void cmdNhanan_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridItem Item in dgList.Items)
            {
                ...
                if (chkChon.Checked)
                {
                    ...
                    {
                        ALD_DON_XULY donXuLy = dt.ALD_DON_XULY.Where(x => x.DONID == IDVUAN).FirstOrDefault();
                    ...
                    else
                    {
                        ...
                        else if (hddLydo.Value == "05")
                        {
                            ...
                            //decimal IDVUAN = Convert.ToDecimal(input.VuViecID);
                            ALD_DON oDon = dt.ALD_DON.Where(x => x.ID == IDVUAN).FirstOrDefault();
                            if (oDon.MAGIAIDOAN == ENUM_GIAIDOANVUAN.PHUCTHAM_QDK)
                            {
                                ALD_CHUYEN_NHAN_AN_BL aLD_CHUYEN_NHAN_AN_BL = new ALD_CHUYEN_NHAN_AN_BL();
                                decimal donIdOld = aLD_CHUYEN_NHAN_AN_BL.getDonIdOld(oDon.ID);
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                try
                {
                    ...
                    ALD_DON oDon = dt.ALD_DON.Where(x => x.ID == ID).FirstOrDefault();
                    if (oDon != null)
                    {
                    ...
                    if (rdbLoai.SelectedValue == "0")//Chuyển nhận đơn
                    {
                        ...
                        {
                            ALD_DON_XULY dxl = dt.ALD_DON_XULY.Where(x => x.DONID == ID).OrderByDescending(x => x.ID).FirstOrDefault();
                    ...
                    else
                    {
                        ...
                        {
                            ALD_SOTHAM_THULY oTL = dt.ALD_SOTHAM_THULY.Where(x => x.DONID == ID).OrderByDescending(y => y.NGAYTHULY).Take(1).FirstOrDefault();
                            if (oTL != null) lstNgaythuly.Text = ((DateTime)oTL.NGAYTHULY).ToString("dd/MM/yyyy");
                            if (rdbTrangthai.SelectedValue == "0")
                            {
                                bool flag = false;

                                List<ALD_SOTHAM_KHANGCAO> lstkc = dt.ALD_SOTHAM_KHANGCAO.Where(x => x.DONID == ID && (x.TINHTRANG_GIAIQUYET == 0 || x.TINHTRANG_GIAIQUYET == null)).OrderBy(y => y.ISQUAHAN).ToList();
                                List<ALD_SOTHAM_KHANGNGHI> lstkn = dt.ALD_SOTHAM_KHANGNGHI.Where(x => x.DONID == ID && (x.TINHTRANG_GIAIQUYET == 0 || x.TINHTRANG_GIAIQUYET == null)).ToList();
                                ALD_SOTHAM_KHANGCAO okc = dt.ALD_SOTHAM_KHANGCAO.Where(x => x.DONID == ID && (x.TINHTRANG_GIAIQUYET == 0 || x.TINHTRANG_GIAIQUYET == null)).OrderByDescending(y => y.ISQUAHAN).FirstOrDefault();
                                ...
                                if (flag == false)
                                {
                                    ...
                                    if (oDMQD != null)
                                    {
                                        ...
                                        ALD_SOTHAM_QUYETDINH oQD = dt.ALD_SOTHAM_QUYETDINH.Where(x => x.DONID == ID && x.LOAIQDID == oDMQD.ID && x.TOAANID == IDToa).OrderByDescending(y => y.NGAYQD).FirstOrDefault();
                                        if (oQD != null)
                                        {
                            ...
                            else
                            {
                                ...
                                {
                                    ALD_DON_XULY dxl = dt.ALD_DON_XULY.Where(x => x.DONID == ID).FirstOrDefault();
                                    strNoiDung = dxl.LYDO;
                                }
                            }
                        }
                        ...
                        {
                            ALD_PHUCTHAM_THULY oTL = dt.ALD_PHUCTHAM_THULY.Where(x => x.DONID == ID).OrderByDescending(y => y.NGAYTHULY).Take(1).FirstOrDefault();
                            if (oTL != null) lstNgaythuly.Text = ((DateTime)oTL.NGAYTHULY).ToString("dd/MM/yyyy");
                            if (rdbTrangthai.SelectedValue == "0")
                            {
                                //DM_KETQUA_PHUCTHAM oKQPT = dt.DM_KETQUA_PHUCTHAM.Where(x => x.MA == "06").FirstOrDefault();
                                //List<ALD_PHUCTHAM_BANAN> lstba = dt.ALD_PHUCTHAM_BANAN.Where(x => x.DONID == ID && x.KETQUAPHUCTHAMID == oKQPT.ID).ToList();
                                //DM_KETQUA_PHUCTHAM oKQPT2 = dt.DM_KETQUA_PHUCTHAM.Where(x => x.MA == "04").FirstOrDefault();
                                //List<ALD_PHUCTHAM_BANAN> lstba2 = dt.ALD_PHUCTHAM_BANAN.Where(x => x.DONID == ID && x.KETQUAPHUCTHAMID == oKQPT2.ID).ToList();
                                //List<ALD_PHUCTHAM_QUYETDINH> lstQD = dt.ALD_PHUCTHAM_QUYETDINH.Where(x => x.DONID == ID && x.KETQUAID == 103).ToList();
                                ...
                                List<ALD_PHUCTHAM_BANAN> lstba = dt.ALD_PHUCTHAM_BANAN.Where(x => x.DONID == ID && (x.KETQUAPHUCTHAMID == 4 || x.KETQUAPHUCTHAMID == 22)).ToList();
                                List<ALD_PHUCTHAM_QUYETDINH> lstqd = dt.ALD_PHUCTHAM_QUYETDINH.Where(x => x.DONID == ID && x.KETQUAID == 103).ToList();
                        ...
                        {
                            //ALD_KCKNQDK_PHUCTHAM_THULY oTL = dt.ALD_KCKNQDK_PHUCTHAM_THULY.Where(x => x.DONID == ID).OrderByDescending(y => y.NGAYTHULY).Take(1).FirstOrDefault();
                            ALD_KCKNQDK_PHUCTHAM_THULY oTL = DataExtensions.GetAllByDonId<ALD_KCKNQDK_PHUCTHAM_THULY>(ID).OrderByDescending(y => y.NGAYTHULY).Take(1).FirstOrDefault();
                            if (oTL != null) lstNgaythuly.Text = ((DateTime)oTL.NGAYTHULY).ToString("dd/MM/yyyy");
                            if (rdbTrangthai.SelectedValue == "0")
                            {

                                //List<ALD_KCKNQDK_PHUCTHAM_QUYETDINH> lstqd = dt.ALD_KCKNQDK_PHUCTHAM_QUYETDINH.Where(x => x.DONID == ID && (x.KETQUAID == 101 || x.KETQUAID == 102 || x.KETQUAID == 103)).ToList();
                                List<ALD_KCKNQDK_PHUCTHAM_QUYETDINH> lstqd = DataExtensions.GetAllWithClause<ALD_KCKNQDK_PHUCTHAM_QUYETDINH>($"DONID = {ID} AND KETQUAID IN (101,102,103)");
                                if (lstqd.Count > 0)
                                {
        ...
        private string genNoiDung(decimal MaGiaiDoan, decimal ID, string strNoiDung, bool isAn = true)
        {
            if (isAn)
            {
                if (MaGiaiDoan == ENUM_GIAIDOANVUAN.SOTHAM || MaGiaiDoan == ENUM_GIAIDOANVUAN.HOSO)
                {
                    ...
                    List<ALD_SOTHAM_KHANGCAO> lstkc = dt.ALD_SOTHAM_KHANGCAO.Where(x => x.DONID == ID && (x.TINHTRANG_GIAIQUYET == 0 || x.TINHTRANG_GIAIQUYET == null)).OrderBy(y => y.ISQUAHAN).ToList();
                    List<ALD_SOTHAM_KHANGNGHI> lstkn = dt.ALD_SOTHAM_KHANGNGHI.Where(x => x.DONID == ID && (x.TINHTRANG_GIAIQUYET == 0 || x.TINHTRANG_GIAIQUYET == null)).ToList();
                    ALD_SOTHAM_KHANGCAO okc = dt.ALD_SOTHAM_KHANGCAO.Where(x => x.DONID == ID).OrderByDescending(y => y.ISQUAHAN).FirstOrDefault();
                    if (lstkc.Count > 0 && lstkn.Count > 0)
                    {
                        if (okc.ISQUAHAN == 1)
                            strNoiDung = "Do có kháng cáo và kháng nghị phúc thẩm <b><span style='color: red'>(KC quá hạn)</span></b>";
        ...
        {
            ALD_CHUYEN_NHAN_AN_BL Bl = new ALD_CHUYEN_NHAN_AN_BL();
            string nd = "";
            for (int i = 0; i < lstChuyenAnID.Count; i++)
            {
                string Msg_Ex = "Không được phép hủy chuyển án.";
                var ChuyenAnId = lstChuyenAnID[i];
                ALD_CHUYEN_NHAN_AN oChuyenAn = dt.ALD_CHUYEN_NHAN_AN.Where(x => x.ID == ChuyenAnId).FirstOrDefault();
                ALD_DON_XULY dxl = dt.ALD_DON_XULY.Where(x => x.DONID == oChuyenAn.VUANID).FirstOrDefault();
                if (oChuyenAn != null)
                {
                    ...
                    if (oChuyenAn.TRANGTHAI != 1) // Án chưa nhận
                    {
                        ...
                        dt.ALD_CHUYEN_NHAN_AN.Remove(oChuyenAn);
                        dt.SaveChanges();
                        dgList.CurrentPageIndex = 0;
                        hddPageIndex.Value = "1";
                        //10092021 insert quản lý hồ sơ trạng thái kháng nghị kháng cáo
                        new QLHS_BL().DeleteHoSoLuTruCoChuyenAn(DonID, ENUM_LOAIVUVIEC_NUMBER.AN_LAODONG);

                        // cap nhat lai MAGIAIDON sau khi huy chuyen
                        ALD_DON oDon = dt.ALD_DON.Where(X => X.ID == DonID).FirstOrDefault();
                        ALD_DON_GIAIDOAN oGD = dt.ALD_DON_GIAIDOAN.Where(x => x.DONID == DonID && x.TOAANID == oChuyenAn.TOACHUYENID).FirstOrDefault();
                        if (oGD == null)
                        {
                            oGD = dt.ALD_DON_GIAIDOAN.Where(x => x.DONID == DonID && x.TOAPHUCTHAMID == oChuyenAn.TOACHUYENID).FirstOrDefault();
                        }
                        ...
                        /*                        if (oDon != null && oDon.MAGIAIDOAN != null && oDon.MAGIAIDOAN == ENUM_GIAIDOANVUAN.SOTHAM)
                                                {
                                                    oGD = dt.ALD_DON_GIAIDOAN.Where(x => x.DONID == DonID && x.TOAANID == oChuyenAn.TOACHUYENID).FirstOrDefault();
                                                    oDon.MAGIAIDOAN = oGD.MAGIAIDOAN;
                                                    dt.SaveChanges();
                                                }
            ...
            //    {
            //        ALD_CHUYEN_NHAN_AN ObjChuyenNhan = dt.ALD_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonID).FirstOrDefault();
            //        if (ObjChuyenNhan != null)
            //        {
            //            dt.ALD_CHUYEN_NHAN_AN.Remove(ObjChuyenNhan);
            //            dt.SaveChanges();
            //            dgList.CurrentPageIndex = 0;
            //            hddPageIndex.Value = "1";
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (toacapchaID == 0)
                {
                    ...
                    /*                    BL.GSTP.Danhmuc.DM_TOAAN_TACH_NHAP_MAPPING_BL bl = new BL.GSTP.Danhmuc.DM_TOAAN_TACH_NHAP_MAPPING_BL();
                                        DataTable dtSapNhap = bl.GETS_BY_TOAANTID(toaanST);
                                        ...
                                        if (dtSapNhap != null)
                                        {
                                            for (int i = 0; i < dtSapNhap.Rows.Count; i++)
                                            {
                                                if (dtSapNhap.Rows[i]["HIEULUC"].ToString() == "1")
                                                    if ((decimal)dtSapNhap.Rows[i]["TOAANID"] == toaanST)
                                                        selectList.Items.Add(new ListItem(dtSapNhap.Rows[i]["TOTOAANTEN"].ToString(), dtSapNhap.Rows[i]["TOTOAANID"].ToString()));
                                                    else
                                                        selectList.Items.Add(new ListItem(dtSapNhap.Rows[i]["TOAANTEN"].ToString(), dtSapNhap.Rows[i]["TOAANID"].ToString()));
                                            }
                                        }*/
                ...
                /*                if (!string.IsNullOrEmpty(hddTN_ID.Value))
                                {
                                    ...
                                    //BL.GSTP.Danhmuc.DM_TOAAN_TACH_NHAP_MAPPING_BL bl = new BL.GSTP.Danhmuc.DM_TOAAN_TACH_NHAP_MAPPING_BL();
                                    //DataTable dtSapNhap = bl.GETS_BY_TOAANTID(toacapchaID);
                                    ...
                                    //    {
                                    //        if (dtSapNhap.Rows[i]["HIEULUC"].ToString() == "1")
                                    //            if ((decimal)dtSapNhap.Rows[i]["TOAANID"] == toacapchaID)
                                    //                selectList.Items.Add(new ListItem(dtSapNhap.Rows[i]["TOTOAANTEN"].ToString(), dtSapNhap.Rows[i]["TOTOAANID"].ToString()));
                                    //            else
                                    //                selectList.Items.Add(new ListItem(dtSapNhap.Rows[i]["TOAANTEN"].ToString(), dtSapNhap.Rows[i]["TOAANID"].ToString()));
                                    //    }
