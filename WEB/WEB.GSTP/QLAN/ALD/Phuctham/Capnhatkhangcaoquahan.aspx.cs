...
namespace WEB.GSTP.QLAN.ALD.Phuctham
{
    public partial class Capnhatkhangcaoquahan : System.Web.UI.Page
    {
        ...
        {
            ALD_SOTHAM_KHANGCAO oND = dt.ALD_SOTHAM_KHANGCAO.Where(x => x.DONID == id).FirstOrDefault();
            if (oND != null)
            {
                oND.GQ_NGAY = null;
        ...
        private void LoadGrid()
        {
            ...
            DataTable oDT = oBL.ALD_PT_KCQUAHAN(vDonViID, txtMaVuViec.Text, txtTenVuViec.Text, dFrom, dTo, Convert.ToDecimal(rdbTrangthai.SelectedValue), pageIndex, pageSize);
            int Total = 0;
        ...
        protected void cmdGiaiQuyet_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                ALD_DON don = dt.ALD_DON.Where(x => x.ID == ID).FirstOrDefault<ALD_DON>();
                if (don != null)
                {
