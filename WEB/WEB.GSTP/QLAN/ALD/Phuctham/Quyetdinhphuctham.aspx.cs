...
namespace WEB.GSTP.QLAN.ALD.Phuctham
{
    public partial class Quyetdinhphuctham : System.Web.UI.Page
    {
        ...
        public void LoadGrid()
        {
            ...
            DM_QUYETDINH_VUAN oBL = new DM_QUYETDINH_VUAN();
            DataTable oDT = oBL.DGLIST_QUYETDINH_PT(ENUM_LOAIVUVIEC_NUMBER.AN_LAODONG, ID);
            if (oDT != null)
            {
        ...
        public void xoa(decimal id)
        {
            ...
            ALD_TONGDAT oTD = dt.ALD_TONGDAT.Where(x => x.DONID == oND.DONID && x.MAPID == oND.ID && x.MAP_TABLE == ENUM_MAP_TABLE.ALD_PHUCTHAM_QUYETDINH).FirstOrDefault();
            if (oTD != null)
            {
            ...
            if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.DONID), 5, Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Quyết định Phúc thẩm án Lao động", "Xóa", json) == false)
            {
            ...
            try
            {
                ...
                switch (e.CommandName)
                {
                    case "Download":
                        ...
                        break;
                    case "Sua":
                        ALD_PHUCTHAM_QUYETDINH oND1 = dt.ALD_PHUCTHAM_QUYETDINH.Where(x => x.ID == ND_id).FirstOrDefault();
                        // khong duoc xoa khi da Tong dat - 29/11/2025 vnpt check
                        ALD_TONGDAT oTD = dt.ALD_TONGDAT.Where(x => x.DONID == oND1.DONID && x.MAPID == oND1.ID && x.MAP_TABLE == ENUM_MAP_TABLE.ALD_PHUCTHAM_QUYETDINH).FirstOrDefault();
                        if (oTD != null)
                        {
        ...
        protected void ddlQuyetdinh_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (oT != null)
            {
                ...
                //Load số quyêt định với các loại QD ALD_PT sau
