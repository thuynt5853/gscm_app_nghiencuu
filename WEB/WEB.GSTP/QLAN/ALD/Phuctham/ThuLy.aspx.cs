...
namespace WEB.GSTP.QLAN.ALD.Phuctham
{
    public partial class ThuLy : System.Web.UI.Page
    {
            ...
            try
            {
                if (!IsPostBack)
                {
                    ...
                    else
                    {
                        ...
                        DataTable oDT = oBL.ALD_PHUCTHAM_THULY_GETLIST(ID);
                        // hoangndh 160725: khong load id cua thu y cu khi sang toa moi
        ...
        private decimal SetNewSoThuLy()
        {
            ...
            string sothulymoi = oSTBL.GET_STL_NEW(DonViID, "ALD_PT", ngaythuly).ToString();
        ...
        void SetNewSoThongbao()
        {
            ...
            txtSothongbao.Text = oSTBL.GET_STBTL_NEW(DonViID, "ALD_PT", ngaythongbao).ToString();
        ...
        private void CheckShowCommand(decimal DonID)
        {
            ...
            if (oT != null)
            {
                ...
                ALD_SOTHAM_BL objST = new ALD_SOTHAM_BL();
                if (oT.MAGIAIDOAN == ENUM_GIAIDOANVUAN.SOTHAM && objST.ALD_SOTHAM_KCaoKNghi_GETLIST(DonID).Rows.Count == 0)
                {
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                int CheckBanAnPT = (string.IsNullOrEmpty(rowView["CheckBanAnPT"] + "")) ? 0 : Convert.ToInt16(rowView["CheckBanAnPT"] + "");
                ...
                if (CheckBanAnPT > 0 || CheckPhanCongTP > 0 || CheckNguoiTienHanhTT > 0 || hddIsShowCommand.Value == "False")
                {
                    lblSua.Text = "Chi tiết";
                    lbtXoa.Visible = false;
                    lbtXoaSothulyKhongSuDungLai.Visible = false;
        ...
        private bool CheckValid()
        {
            ...
            //    Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
            //    ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
            //    Decimal CheckID = oSTBL.CheckSoTLTheoLoaiAn(DonViID, "ALD_PT", sothuly, ngaythuly);
            //    if (CheckID > 0)
            //    {
            //        Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
            //        String strMsg = "";
            //        String STTNew = oSTBL.GET_STL_NEW(DonViID, "ALD_PT", ngaythuly).ToString();
            ...
            if (!String.IsNullOrEmpty(txtNgaythuly.Text))
            {
                ...
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal CheckID = oSTBL.CheckSoTBTLTheoLoaiAn(DonViID, "ALD_PT", sothongbao, ngaythuly);
                if (CheckID > 0)
                {
                    Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
                    String strMsg = "";
                    String STTNew = oSTBL.GET_STBTL_NEW(DonViID, "ALD_PT", ngaythuly).ToString();
                    if (CheckID != CurrThuLyID)
                    {
                        //lbthongbao.Text = "Số thụ lý này đã có!";
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                decimal FileID = 0;
                ALD_PHUCTHAM_THULY oND;
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    oND = new ALD_PHUCTHAM_THULY();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.ALD_PHUCTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
                    if (oND.FILEID != null) FileID = (decimal)oND.FILEID;
                    // - vnpt 1/12/2025  check tong dat
                    ALD_TONGDAT oTD = dt.ALD_TONGDAT.Where(x => x.DONID == oND.DONID && x.MAPID == oND.ID && x.MAP_TABLE == ENUM_MAP_TABLE.ALD_PHUCTHAM_THULY).FirstOrDefault();
                    if (oTD != null)
                    {
                        lbthongbao.Text = "Bạn không thể lưu khi đã tống đạt!";
                        return;
                    }
                }
                oND.DONID = DONID;
                ...
                oQLSTL.update_STPT_QUANLY_SOTHULY(3, 5, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), ((DateTime)oND.NGAYTHULY).ToString("dd/MM/yyyy", cul), Regex.Match(oND.SOTHULY, @"\d+").Value);

                ALD_DON_BL objDBL = new ALD_DON_BL();
                ALD_PHUCTHAM_BL oBL = new ALD_PHUCTHAM_BL();
                DataTable oDT = oBL.ALD_PHUCTHAM_THULY_GETLIST(DONID);
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    {
                        ALD_PHUCTHAM_BL oPTBL = new ALD_PHUCTHAM_BL();
                        oND.TT = oPTBL.THULY_GETNEWTT(Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]));
                        oND.MATHULY = "P" + ENUM_LOAIVUVIEC.AN_LAODONG + Session[ENUM_SESSION.SESSION_MADONVI] +
                                      oND.TT.ToString();
                        ...
                        GD.GAIDOAN_INSERT_UPDATE("5", DONID, 3, 0,
                            Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), 0, 0, 0);
                    }
                }
                else
                {
        ...
        {
            ALD_PHUCTHAM_THULY oND = dt.ALD_PHUCTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
            if (oND != null)
            {
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                switch (e.CommandName)
                {
                    ...
                    case "Sua":
                        lbthongbao.Text = "";
                        ALD_PHUCTHAM_THULY oND1 = dt.ALD_PHUCTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
                        // khong duoc SỬA khi da Tong dat - 29/11/2025 vnpt check
                        ALD_TONGDAT oTD = dt.ALD_TONGDAT.Where(x => x.DONID == oND1.DONID && x.MAPID == oND1.ID && x.MAP_TABLE == ENUM_MAP_TABLE.ALD_PHUCTHAM_THULY).FirstOrDefault();
                        if (oTD != null)
                        {
                    ...
                    case "Xoa":
                        ALD_PHUCTHAM_THULY oND2 = dt.ALD_PHUCTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
                        // khong duoc xoa khi da Tong dat - 29/11/2025 vnpt check
                        ALD_TONGDAT oTD1 = dt.ALD_TONGDAT.Where(x => x.DONID == oND2.DONID && x.MAPID == oND2.ID && x.MAP_TABLE == ENUM_MAP_TABLE.ALD_PHUCTHAM_THULY).FirstOrDefault();
                        if (oTD1 != null)
                        {
        ...
        protected void txtNgayThuLy_TextChanged(object sender, EventArgs e)
        {
            ...
            //    STPT_QUANLY_SOTHULY oQLSTL = new STPT_QUANLY_SOTHULY();
            //    DataTable dtStl = oQLSTL.get_STPT_QUANLY_SOTHULY(3, 5, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtNgaythuly.Text);
            //    if (dtStl != null)
            //    {
            ...
            //    string check_ngaythulycuoi_trongnam = oQLSTL.get_LATEST_DATE_IN_SOTHULY(3, 5, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtNgaythuly.Text);
        ...
        protected void btnSaveLydoXoa_Insert(object sender, EventArgs e)
        {
            ...
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            ALD_PHUCTHAM_THULY oND = dt.ALD_PHUCTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
            ...
            if (oND != null)
            {
                ...
                ALD_DON_THAMPHAN ALD_DON_THAMPHAN = dt.ALD_DON_THAMPHAN.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM).FirstOrDefault<ALD_DON_THAMPHAN>();
                if (ALD_DON_THAMPHAN != null)
                {
                    lbthongbao.Text = "Xóa không thành công! Do đang tồn tại thông tin phân công thẩm phán giải quyết.";
                    return;
                }
                ALD_PHUCTHAM_THAMGIATOTUNG ALD_PHUCTHAM_THAMGIATOTUNG = dt.ALD_PHUCTHAM_THAMGIATOTUNG.Where(x => x.DONID == DONID).FirstOrDefault<ALD_PHUCTHAM_THAMGIATOTUNG>();
                if (ALD_PHUCTHAM_THAMGIATOTUNG != null)
                {
                    lbthongbao.Text = "Xóa không thành công! Do đang tồn tại người tham gia tố tụng.";
                    return;
                }
                ALD_PHUCTHAM_HDXX ALD_PHUCTHAM_HDXX = dt.ALD_PHUCTHAM_HDXX.Where(x => x.DONID == DONID).FirstOrDefault<ALD_PHUCTHAM_HDXX>();
                if (ALD_PHUCTHAM_HDXX != null)
                {
                    lbthongbao.Text = "Xóa không thành công! Do đang tồn tại thông tin người tiến hành tố tụng.";
                    return;
                }
                ALD_PHUCTHAM_HOAGIAI ALD_PHUCTHAM_HOAGIAI = dt.ALD_PHUCTHAM_HOAGIAI.Where(x => x.DONID == DONID).FirstOrDefault<ALD_PHUCTHAM_HOAGIAI>();
                if (ALD_PHUCTHAM_HOAGIAI != null)
                {
                    lbthongbao.Text = "Xóa không thành công! Do đang tồn tại thông tin hòa giải.";
                    return;
                }
                ALD_PHUCTHAM_QUYETDINH ALD_PHUCTHAM_QUYETDINH = dt.ALD_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DONID).FirstOrDefault<ALD_PHUCTHAM_QUYETDINH>();
                ...
                dt.ALD_PHUCTHAM_THULY.Remove(oND);
                dt.SaveChanges();
                if (FileID > 0)
                {
                    try
