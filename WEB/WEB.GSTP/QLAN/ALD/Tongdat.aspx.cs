...
namespace WEB.GSTP.QLAN.ALD
{
    public partial class Tongdat : System.Web.UI.Page
    {
        ...
        ALD_TONGDAT_BL TONGDAT_BL = new ALD_TONGDAT_BL();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                string current_id = Session[ENUM_LOAIAN.AN_LAODONG] + "", TOAANID = "0";
                ...
                List<DONKK_USER_DKNHANVB> lstDKVB = dkk.DONKK_USER_DKNHANVB.Where(x => x.VUVIECID == ID && x.MALOAIVUVIEC == ENUM_LOAIAN.AN_LAODONG && x.TRANGTHAI == 1).ToList();
                if (lstDKVB.Count == 0)
        ...
        private void LoadCombobox()
        {
            ...
            //Decimal VuAnId = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_LAODONG]);
            //DataTable tbl = oBL.ALD_FILE_TONGDAT(ID);
            //ALD_SOTHAM_BANAN oND = dt.ALD_SOTHAM_BANAN.Where(x => x.DONID == VuAnId && x.TOAANID == TOAANID).FirstOrDefault();
            //DM_BIEUMAU bm52 = dt.DM_BIEUMAU.Where(bm => bm.ID == 230).FirstOrDefault();
            //ALD_FILE af = dt.ALD_FILE.Where(x => x.BIEUMAUID == 230 && x.DONID == VuAnId).FirstOrDefault();
            //ALD_TONGDAT td = dt.ALD_TONGDAT.Where(x => x.DONID == VuAnId && x.BIEUMAUID == 230).FirstOrDefault();
            //if (oND != null && td == null)
            //{
            ...
            //    tbl.Rows.Add(dr);
            //}
            //ALD_PHUCTHAM_BANAN oND2 = dt.ALD_PHUCTHAM_BANAN.Where(x => x.DONID == VuAnId && x.TOAANID == TOAANID).FirstOrDefault();
            //DM_BIEUMAU bm75 = dt.DM_BIEUMAU.Where(bm => bm.ID == 261).FirstOrDefault();
            //af = dt.ALD_FILE.Where(x => x.BIEUMAUID == 261 && x.DONID == VuAnId).FirstOrDefault();
            //td = dt.ALD_TONGDAT.Where(x => x.DONID == VuAnId && x.BIEUMAUID == 261).FirstOrDefault();
            ...
            ALD_DON objDon = dt.ALD_DON.Where(x => x.ID == ID).FirstOrDefault();
            decimal giai_doan = Convert.ToDecimal(objDon.MAGIAIDOAN);
            TONGDAT_BL oTongDatBL = new TONGDAT_BL();
            DataTable oDT = oTongDatBL.GET_BM_TONGDAT(ID, 5, giai_doan);
            ...
            //dv.Sort = "THUTU asc";
            //oDT = dv.ToTable();
            // VNPT -- locbx -- 01/12/2025 sắp xếp theo ngày tạo 
        ...
        private List<ALD_TONGDAT_GS> convertToALD_TONGDATFromDataTable(DataTable dataTable)
        {
            List<ALD_TONGDAT_GS> aLD_TONGDATs = new List<ALD_TONGDAT_GS>();
            foreach (DataRow row in dataTable.Rows)
            {
                ALD_TONGDAT_GS tONGDAT = new ALD_TONGDAT_GS();
                ...
                aLD_TONGDATs.Add(tONGDAT);
            }
            return aLD_TONGDATs;
        }

        private List<ALD_TONGDAT_NGUOINHAN_GS> convertToALD_TONGDATDOITUONGFromDataTable(DataTable dataTable)
        {
            List<ALD_TONGDAT_NGUOINHAN_GS> aLD_TONGDATs = new List<ALD_TONGDAT_NGUOINHAN_GS>();
            foreach (DataRow row in dataTable.Rows)
            {
                ALD_TONGDAT_NGUOINHAN_GS tONGDAT = new ALD_TONGDAT_NGUOINHAN_GS();
                ...
                // VNPT VŨ VĂN QUÂN 15:00 03/11/2025
        ...
        {
            BL.GSTP.ALD.ALD_TONGDAT_BL Bl = new BL.GSTP.ALD.ALD_TONGDAT_BL();
            //lbthongbao.Text = "";
            ...
            if (CheckValid() == true)
            {
                ...
                ALD_TONGDAT adTongDat = null;

                if (ANPHI_ID_DROPDOWN != 0)
                {
                    adTongDat = dt.ALD_TONGDAT.Where(x => x.DONID == DONID && x.BIEUMAUID == bieuMauID && x.MAPID == ANPHI_ID_DROPDOWN).FirstOrDefault();
                }
                else
                {
                    adTongDat = dt.ALD_TONGDAT.Where(x => x.DONID == DONID && x.BIEUMAUID == bieuMauID).FirstOrDefault();
                }

                ALD_TONGDAT_GS oND = null;

                if (TongDatID != 0)
                {
                    oND = convertToALD_TONGDATFromDataTable(Bl.ALD_TONGDAT_GETBYID(TongDatID)).FirstOrDefault();
                }

                bool isNew = false;

                bool isUpdate = false;
                ALD_TONGDAT td = new ALD_TONGDAT();
                List<ALD_TONGDAT_DOITUONG> td_dt = new List<ALD_TONGDAT_DOITUONG>();
                ...
                else
                {
                    ...
                    td = dt.ALD_TONGDAT.Where(x => x.ID == TongDatID).FirstOrDefault();
                    td_dt = dt.ALD_TONGDAT_DOITUONG.Where(x => x.TONGDATID == td.ID && x.TRANGTHAI == 1).ToList();

                    CONTENT_JSON_OLD = JsonConvert.SerializeObject(td, Formatting.Indented);
                }
                if (dM_BIEUMAU.MABM == "100-DS")
                {
                    oND.MAP_TABLE = ENUM_MAP_TABLE.ALD_ANPHI;
                ...
                if (isNew)
                {
                    ...
                    //---------01/12/2025---- locbx vnpt - Thêm biến FILEID để lọc riêng từng văn bản trùng
                    if (ddlBieumau.SelectedValue.Split('_').Length > 2)
                    {
                        decimal FILEID = Convert.ToDecimal(ddlBieumau.SelectedValue.Split('_')[2]);
                        oF = dt.ALD_FILE.Where(x => x.DONID == DONID && x.BIEUMAUID == bieu_mau_id && x.ID== FILEID).FirstOrDefault();
                    }
                    else
                    {
                        oF = dt.ALD_FILE.Where(x => x.DONID == DONID && x.BIEUMAUID == bieu_mau_id).FirstOrDefault();
                    }
                    //---------01/12/2025----  vnpt chỉnh lấy thêm mapid maptable
                    if (oF != null)
                    {
                        oND.FILEID = oF.ID;
                        var donxuly = dt.ALD_DON_XULY.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);
                        if (donxuly != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.ALD_DON_XULY;
                            oND.MAPID = donxuly.ID;
                        }

                        var donxulydct =
                                        (from dx in dt.ALD_DON_XULY
                        ...
                        var anphi = dt.ALD_ANPHI.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);
                        ...
                        var sothamthuly = dt.ALD_SOTHAM_THULY.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (sothamthuly != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.ALD_SOTHAM_THULY;
                            oND.MAPID = sothamthuly.ID;
                        }

                        var sothamquyetdinh = dt.ALD_SOTHAM_QUYETDINH.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (sothamquyetdinh != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.ALD_SOTHAM_QUYETDINH;
                            oND.MAPID = sothamquyetdinh.ID;
                        }

                        var phucthamthuly = dt.ALD_PHUCTHAM_THULY.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (phucthamthuly != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.ALD_PHUCTHAM_THULY;
                            oND.MAPID = phucthamthuly.ID;
                        }

                        var phucthamquyetdinh = dt.ALD_PHUCTHAM_QUYETDINH.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (phucthamquyetdinh != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.ALD_PHUCTHAM_QUYETDINH;
                            oND.MAPID = phucthamquyetdinh.ID;
                        }

                        var banansotham = dt.ALD_SOTHAM_BANAN.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (banansotham != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.ALD_SOTHAM_BANAN;
                            oND.MAPID = banansotham.ID;
                        }

                        var bananphuctham = dt.ALD_PHUCTHAM_BANAN.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (bananphuctham != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.ALD_PHUCTHAM_BANAN;
                            oND.MAPID = bananphuctham.ID;
                        }
                    }
                    oND.BIEUMAUID = bieu_mau_id;
                    oND.TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                    oND.ID = Bl.ALD_TONGDAT_UPIN(oND);
                    ...
                    log.InsertLog("ALD_TONGDAT", oND.ID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Insert", "", CONTENT_JSON, "");
                }
                else
                {
                ...
                if (!isNew)
                {
                    ...
                    CONTENT_JSON = JsonConvert.SerializeObject(oND, Formatting.Indented);
                    LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                    log.InsertLog("ALD_TONGDAT", oND.ID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Update", lbthongbao.Text, CONTENT_JSON, CONTENT_JSON_OLD);
                }
                if (trDuongsu.Visible)
                {
                    decimal TongdatID = oND.ID;
                    List<decimal> IDs = new List<decimal>();
                    foreach (DataRow row in TONGDAT_BL.ALD_TONGDATDOITUONG_GETBYTONGDATID(TongDatID).Rows)
                    {
                    ...
                    foreach (DataGridItem Item in dgTructiep.Items)
                    {
                        ...
                        decimal? ID;
                        List<ALD_TONGDAT_NGUOINHAN_GS> lstCheck;
                        try
                        {
                            if (Item.Cells[7].Text == "&nbsp;")
                            {
                                ID = null;
                                lstCheck = new List<ALD_TONGDAT_NGUOINHAN_GS>();
                            }
                            else
                            {
                                ID = Convert.ToDecimal(Item.Cells[7].Text);
                                lstCheck = convertToALD_TONGDATDOITUONGFromDataTable(Bl.ALD_TONGDATDOITUONG_GETBYID(ID.Value));
                            }
                        }
                        catch
                        {
                        ...
                        ALD_TONGDAT_NGUOINHAN_GS oTD = null;
                        if (lstCheck.Count > 0)
                        {
                            oTD = lstCheck[0];
                        ...
                        {
                            oTD = new ALD_TONGDAT_NGUOINHAN_GS();
                        }
                        ...
                        if (chkIsSend.Checked)
                        {
                            ...
                            Bl.ALD_TONGDATDOITUONG_UPIN(oTD);
                            dt.SaveChanges();
                            ...
                            if (lstCheck.Count > 0)
                            {
                                ...
                                LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                                log.InsertLog("ALD_TONGDAT_DOITUONG", TongDatID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Update", lbthongbao.Text, CONTENT_JSON, CONTENT_JSON_OLD);
                            }
                            else
                            {
                                //AnhPN log
                                CONTENT_JSON = JsonConvert.SerializeObject(oTD, Formatting.Indented);
                                LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                                log.InsertLog("ALD_TONGDAT_DOITUONG", oTD.TONGDATID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Insert", lbthongbao.Text, CONTENT_JSON, "");
                            }
                            ...
                            {
                                CONTENT_JSON = JsonConvert.SerializeObject(oTD, Formatting.Indented);
                                obj.DVCQG_THANH_TOAN_UP_TONGDAT(oND.ID, (decimal)DuongsuID, "5");
                    ...
                    if (IDs.Count > 0)
                    {
                        foreach (decimal i in IDs)
                            TONGDAT_BL.ALD_TONGDATDOITUONG_REMOVEBYID(i);
                    }
                }
                if (trTructuyen.Visible)
                {
                    ...
                    foreach (DataGridItem Item in dgTructuyen.Items)
                    {
                        ...
                        List<ALD_TONGDAT_DOITUONG> lstCheck = dt.ALD_TONGDAT_DOITUONG.Where(x => x.TONGDATID == TongdatID && x.DUONGSUID == DuongsuID).ToList();
                        ALD_TONGDAT_DOITUONG oTD = null;
                        if (lstCheck.Count > 0)
                        {
                            oTD = lstCheck[0];
                        }
                        ...
                        if (chkIsSendTT.Checked)
                        {
                            ...
                            {
                                oTD.TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                                dt.ALD_TONGDAT_DOITUONG.Add(oTD);
                            }
                            dt.SaveChanges();
        ...
        {
            ALD_TONGDAT td = dt.ALD_TONGDAT.Where(x => x.DONID == DonID && x.TOAANID == ToaAnID && x.BIEUMAUID == BieuMauID && (MAPID == 0 || x.MAPID == MAPID)).FirstOrDefault();
            List<ALD_TONGDAT_DOITUONG> td_dt = dt.ALD_TONGDAT_DOITUONG.Where(x => x.TONGDATID == td.ID).ToList();
            foreach (var item in td_dt)
            {
                ALD_TONGDAT_NGUOINHAN_GS oTD = new ALD_TONGDAT_NGUOINHAN_GS();
        ...
        private void ReUpdateStatusUpdate(List<ALD_TONGDAT_DOITUONG> td_dt, decimal TongDatId)
        {
            ...
            List<ALD_TONGDAT_DOITUONG> list_detail = dt.ALD_TONGDAT_DOITUONG.Where(x => x.TONGDATID == TongDatId).ToList();
        ...
        {
            ALD_DON_BL oBL = new ALD_DON_BL();
            BL.GSTP.ALD.ALD_TONGDAT_BL TongDatBL = new BL.GSTP.ALD.ALD_TONGDAT_BL();
            string current_id = Session[ENUM_LOAIAN.AN_LAODONG] + "";
            decimal ID = Convert.ToDecimal(current_id), ToaAnID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);

            DataTable oDT = oBL.ALD_TONGDAT_GETLIST(ID, ToaAnID);
            var lstTongDat = convertToALD_TONGDATFromDataTable(oDT);
            List<ALD_TONGDAT_NGUOINHAN_GS> lstNguoiNhan = new List<ALD_TONGDAT_NGUOINHAN_GS>();
            //  -- VNPT 06122025
            List<ALD_TONGDAT_GS> lstHaventDoituongtongdat = new List<ALD_TONGDAT_GS>();
            foreach (var tongdat in lstTongDat)
            {
                DataTable doiTuongDataTable = TongDatBL.ALD_TONGDATDOITUONG_GETBYTONGDATID(tongdat.ID);
                if (doiTuongDataTable.Rows.Count == 0)
                {
                    lstHaventDoituongtongdat.Add(tongdat);
                }
                lstNguoiNhan.AddRange(convertToALD_TONGDATDOITUONGFromDataTable(doiTuongDataTable));
            }
            if (oDT != null && lstNguoiNhan.Count == 0)
            {
                if (lstNguoiNhan.Count == 0)
                {
                    ...
                    obj.TONGDAT_DELETE_ERROR(ID.ToString(), "5");
                }
            }
        ...
        public void xoa(decimal id)
        {
            ...
            List<ALD_TONGDAT_DOITUONG> lst = dt.ALD_TONGDAT_DOITUONG.Where(x => x.TONGDATID == id && (x.TRANGTHAI == 0 || x.TRANGTHAI == 2 || ((x.TRANGTHAI == 5 || x.TRANGTHAI == 6) && x.NGAYPHATHANH == null))).ToList();
            if (lst.Count > 0)
            {
                dt.ALD_TONGDAT_DOITUONG.RemoveRange(lst);
                dt.SaveChanges();
            }
            //--
            List<ALD_TONGDAT_DOITUONG> lst_obj = dt.ALD_TONGDAT_DOITUONG.Where(x => x.TONGDATID == id).ToList();
            ...
            if (oND != null)
            {
                if (lst_obj.Count == 0)
                {
                    obj.DVCQG_TT_REMOVE_TONGDAT(id, "5", ref _VALUE);
                    if (_VALUE == 1)
                    {
                        dt.ALD_TONGDAT.Remove(oND);
                        dt.SaveChanges();
                    }
                    else
                    {
        ...
        public void loadSua(decimal ID)
        {
            ...
            if (oBM != null)
            {
                ...
                {
                    ALD_ANPHI aDS_ANPHI = dt.ALD_ANPHI.Where(x => x.ID == MapID).FirstOrDefault();
                    ...
                    if (aDS_ANPHI != null)
                    {
                        DataTable DON_MIENANPHI = dON_MIENANPHI.GET_DON_MIENANPHI_BY_ANPHI_ID(aDS_ANPHI.ID, 5);
            ...
            {
                nguoiNhans = TONGDATBL.ALD_TONGDATDOITUONG_ANPHI_GETBYTONGDATID(ID);
            }
            else
            {
                nguoiNhans = TONGDATBL.ALD_TONGDATDOITUONG_GETBYTONGDATID(ID);
            }
            ...
            {
                string name = TONGDATBL.ALD_TONGDATDOITUONG_GETTENDUONGSU(Convert.ToDecimal(row["ID"]));
        ...
        public void loadPhatHanhLai(decimal ID, decimal vID)
        {
            ...
            cmdLammoi.Visible = true;
            BL.GSTP.ALD.ALD_TONGDAT_BL TONGDATBL = new BL.GSTP.ALD.ALD_TONGDAT_BL();
            DataTable nguoiNhans = TONGDATBL.ALD_TONGDATDOITUONG_GETBYTONGDATID(ID);
            DataTable nguoiNhan = TONGDATBL.ALD_TONGDATDOITUONG_GETBYID(vID);
            ...
            string nguoiNhanName = TONGDATBL.ALD_TONGDATDOITUONG_GETTENDUONGSU(Convert.ToDecimal(nguoiNhanRow["ID"]));
        ...
        //    if (FILEID > 0)
        //    {
        //        //ALD_FILE oF = dt.ALD_FILE.Where(x => x.ID == FILEID).FirstOrDefault();
        //        //if (oF != null) {
        //        //    if (oF.TENFILE !=null)
        //        //        lbtDownload.Visible = true;
        //        //    else
        ...
        //    //List<ALD_TONGDAT_DOITUONG> lst = dt.ALD_TONGDAT_DOITUONG.Where(x => x.TONGDATID == ID).ToList();
        //    //int vCheck = 0;
        //    //if (lst.Count > 0)
        //    //{
        ...
        protected void loadAction(string nDID)
        {
            ...
            //    //Call package
            //    BL.GSTP.ALD.ALD_TONGDAT_BL tongDatBL = new BL.GSTP.ALD.ALD_TONGDAT_BL();
            //    tongDatBL.ALD_TONGDAT_THUHOI_DOITUONG(ID, TongdatID, LyDoThuHoi, NgayThuHoi, NguoiSua);
            ...
            if (TrangThai == 2) // Xem lý do thu hồi
            {
                ...
                DataTable objLyDo = oBL.GET_LYDO_THUHOI(5, TongdatID);
                if (objLyDo != null)
                {
            ...
            {
                BL.GSTP.ALD.ALD_TONGDAT_BL tongDatBL = new BL.GSTP.ALD.ALD_TONGDAT_BL();
                tongDatBL.ALD_TONGDAT_VBDH_DOITUONG(ID);
                LoadGrid();
            }
            else if (TrangThai == 5)
            {
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "Download":
                    ...
                    ALD_TONGDAT ads = dt.ALD_TONGDAT.Where(x => x.ID == tongDatId).FirstOrDefault();
                    byte[] conten = null;
                    if (ads != null)
                    {
                        ...
                        {
                            ALD_FILE oF = dt.ALD_FILE.Where(x => x.ID == ads.FILEID).FirstOrDefault();
                            if (oF != null)
                            {
                                if (oF.TENFILE != null)
                                {
                ...
                case "UpdateNgayNhan":
                    ...
                    if (txtNGAYNHANTONGDAT.Text != "")
                    {
                        ...
                        if (obj.TONGDAT_DOITUONG_VNID(Id_obj, "5", txtNGAYNHANTONGDAT.Text) == true)
                        {
                    ...
                    else if (txtNGAYNHANTONGDAT.Text == "")
                    {
                        ...
                        if (obj.TONGDAT_DOITUONG_VNID(Id_obj, "5", null) == true)
                        {
                            strMsg = "Bạn đã cập nhật thành công";
                            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('" + strMsg + "')", true);
                            LoadGrid();
        ...
        private void LoadDoituong(bool IsShowVKS, decimal BieuMauID)
        {
            ...
            if (IS_TD_DUONGSU == 1)
            {
                if (oBM.MABM == "100-DS")
                {
                    oDTDS = TONGDAT_BL.ALD_TONGDATDOITUONG_ANPHI_GETBY(DONID, TOAANID, BMID, 0, ANPHI_ID);
                }
                else
                {
                    //---------01/12/2025----  vnpt chỉnh lấy thêm lọc FILEID
                    oDTDS = TONGDAT_BL.ALD_TONGDATDOITUONG_GETBYFILEID(DONID, TOAANID, BMID, 0, FILEID);
                }
            ...
            else if (IS_TD_NGUYENDON == 1 && IS_TD_DUONGSU != 1)
            {
                if (oBM.MABM == "100-DS")
                {
                    oDTDS = TONGDAT_BL.ALD_TONGDATDOITUONG_ANPHI_GETBY(DONID, TOAANID, BMID, 1, ANPHI_ID);
                }
                else
                {
                    //---------01/12/2025----  vnpt chỉnh lấy thêm lọc checkVNEID
                    oDTDS = TONGDAT_BL.ALD_TONGDATDOITUONG_GETBYFILEID(DONID, TOAANID, BMID, 1, FILEID);
                }
                trDuongsu.Visible = true;
            }
            if (trDuongsu.Visible)
        ...
        protected void ddlBieumau_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (ddlBieumau.SelectedValue != "0")
            {
                ...
                decimal bieu_mau_id = Convert.ToDecimal(ddlBieumau.SelectedValue.Split('_')[0]);
                //ALD_FILE oF = dt.ALD_FILE.Where(x => x.DONID == DONID && x.BIEUMAUID == bieu_mau_id).FirstOrDefault();
        ...
        {
            BL.GSTP.ALD.ALD_TONGDAT_BL aLD = new BL.GSTP.ALD.ALD_TONGDAT_BL();
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                string TenBM = aLD.ALD_TENBM_GETBYTONGDATID(tongdatID);
                Label lbTenBM = (Label)e.Item.FindControl("lbTenBM");
                lbTenBM.Text = TenBM;
                lbNoiNhan.Text = aLD.ALD_TONGDATDOITUONG_GETTENDUONGSU(id);
                if (duongsu_id != null)
                {
                }
                ...
                BL.GSTP.ALD.ALD_TONGDAT_BL Bl = new BL.GSTP.ALD.ALD_TONGDAT_BL();
                ALD_TONGDAT_GS oND = convertToALD_TONGDATFromDataTable(Bl.ALD_TONGDAT_GETBYID(tongdatID)).FirstOrDefault();
                ...
                int rowspan = aLD.ALD_TONGDATDOITUONG_GETBYTONGDATID(tongdatID).Rows.Count;
        ...
        protected void dgTructiep_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            BL.GSTP.ALD.ALD_TONGDAT_BL _BL = new BL.GSTP.ALD.ALD_TONGDAT_BL();
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                DataRowView rowView = (DataRowView)e.Item.DataItem;
                ...
                if (ID == null) // nếu là hàng theem 
                {
                    ...
                    dt.ALD_DON_DUONGSU.Where(x => x.DONID == DONID && x.TUCACHTOTUNG_MA == ENUM_LAODONG_TUCACHTOTUNG.QUYENNVLQ).ToList()
                        .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.TENDUONGSU, "QUYENNVLQ1" + "," + x.ID)));
                    //if (oT.MAGIAIDOAN == 2)
                    //{
                    //    dt.ALD_DON_THAMGIATOTUNG.Where(x => x.DONID == DONID)
                    //       .ToList()
                    //       .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.HOTEN, x.TUCACHTGTTID + "," + x.ID)));
                    //}
                    //else if (oT.MAGIAIDOAN == 3)
                    //{
                    //    dt.ALD_PHUCTHAM_THAMGIATOTUNG.Where(x => x.DONID == DONID)
                    //       .ToList()
                    //       .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.HOTEN, x.TUCACHTGTTID + "," + x.ID)));
                    //}
                    ...
                    var atddtisSENT = dt.ALD_TONGDAT_DOITUONG.Where(x => x.TONGDATID == tdId).ToList();
                    if (atddtisSENT.Count > 0)
                    {
                        var tddt = atddtisSENT.Select(x => x.DUONGSUID).Where(x => x != null).ToList();
                        dt.ALD_DON_DUONGSU.Where(x => x.DONID == DONID && x.TUCACHTOTUNG_MA != ENUM_LAODONG_TUCACHTOTUNG.QUYENNVLQ && !tddt.Contains(x.ID)).ToList()
        ...
        protected void btn_insert_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (oBM.MABM == "100-DS") // 100-DS. Thông báo về việc miễn tạm ứng án phí  
                {
                    nguoiNhans = TONGDATBL.ALD_TONGDATDOITUONG_ANPHI_GETBYTONGDATID(TongDatID);
                }
                else
                {
                    nguoiNhans = TONGDATBL.ALD_TONGDATDOITUONG_GETBYTONGDATID(TongDatID);
                }
                nguoiNhans.Columns.Add("TUCACHTOTUNG_MA");
                nguoiNhans.Columns.Add("TONGDAT_DOITUONG");
                nguoiNhans.Columns.Add("TENDUONGSU");
        ...
        protected void ddlNoiNhan_SelectedIndexChanged(object sender, EventArgs e)
        {
            foreach (DataGridItem item in dgTructiep.Items)
            {
                ...
                Label lbDiaChi = item.FindControl("lbDiaChi") as Label;
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        ALD_DON_DUONGSU tgtt = dt.ALD_DON_DUONGSU.FirstOrDefault(x => x.ID == nguoiID);
                        if (tgtt != null)
                        {
                            string diaChi = "";
        ...
        {
            BL.GSTP.ALD.ALD_TONGDAT_BL tONGDAT_BL = new BL.GSTP.ALD.ALD_TONGDAT_BL();
            ...
            try
            {
                ...
                foreach (DataGridItem item in dgList.Items)
                {
                    ...
                    if (cb_thuhoi.Checked == true)
                    {
                        ...
                        //AnhPN log
                        LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                        var data = DataExtensions.FindById<ALD_TONGDAT>(v_id);
                        var CONTENT_JSON_OLD = JsonConvert.SerializeObject(data, Formatting.Indented);
                        log.InsertLog("ALD_TONGDAT", v_id.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Thu hồi", LyDoThuHoi, "", CONTENT_JSON_OLD);

                        //AnhPN log
                        List<ALD_TONGDAT_DOITUONG> lst = dt.ALD_TONGDAT_DOITUONG.Where(x => x.TONGDATID == v_id).ToList();
                        CONTENT_JSON_OLD = JsonConvert.SerializeObject(lst.FirstOrDefault(), Formatting.Indented);
                        log.InsertLog("ALD_TONGDAT_DOITUONG", v_id.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Thu hồi", LyDoThuHoi, "", CONTENT_JSON_OLD);

                        tONGDAT_BL.ALD_TONGDAT_THUHOI(v_id, NgayThuHoi, LyDoThuHoi, NguoiSua);
                    }
                }

                // Call API
                string result = CallApiThuhoi("5", tongDatIds, LyDoThuHoi, NgayThuHoi, NguoiSua);
                JToken jObject = JToken.Parse(result);
                if ((string)jObject["status"] == "SUCCESS")
                {
                    lbThongBaoThuHoi.Text = "Thu hồi thành công !";
        ...
        public List<TongDatDTO> GetAnLaoDong(List<decimal> ids)
        {
            List<ALD_TONGDAT> aldTongDats = dt.ALD_TONGDAT.Where(x => ids.Contains(x.ID) && (x.TRANGTHAI >= 0 && x.TRANGTHAI <= 2)).ToList();
            HashSet<decimal?> idBieuMaus = new HashSet<decimal?>(aldTongDats.Select(x => x.BIEUMAUID).ToList());
            ...
            HashSet<decimal?> idALD_DON = new HashSet<decimal?>(aldTongDats.Select(x => x.DONID).ToList());
            List<ALD_DON> allALDDon = dt.ALD_DON.Where(x => idALD_DON.Contains(x.ID)).ToList();
            Dictionary<decimal?, ALD_DON> mapAldDon = new Dictionary<decimal?, ALD_DON>();
            Dictionary<decimal?, List<ALD_DON_DUONGSU>> mapDuongSu = dt.ALD_DON_DUONGSU
                .Where(x => idALD_DON.Contains(x.DONID))
                .GroupBy(x => x.DONID, x => x)
                .ToDictionary(x => x.Key, x => x.ToList());
            // bo xung thong tin van ban di
            foreach (ALD_DON aldDon in allALDDon)
            {
                mapAldDon.Add(aldDon.ID, aldDon);
            }
            ...
            HashSet<string> usernames = new HashSet<string>(aldTongDats.Where(x => x.NGUOITAO != null).ToList().Select(x => x.NGUOITAO).ToList());
            ...
            string conditions = "(";
            foreach (ALD_TONGDAT tongDat in aldTongDats)
            {
            ...
            List<TongDatDTO> danhSachTongDat = aldTongDats.Select(x => ToDTO(x, mapBieuMau, mapUser, mapAldDon, mapDuongSu)).ToList();
            ...
            List<ALD_TONGDAT_DOITUONG> allDoiTuongs = dt.ALD_TONGDAT_DOITUONG
            ...
            Dictionary<decimal, ALD_DON_DUONGSU> donDuongSu = dt.ALD_DON_DUONGSU
            ...
            foreach (TongDatDTO dto in danhSachTongDat)
            {
                List<ALD_TONGDAT_DOITUONG> subList = allDoiTuongs.Where(x => dto.ID.Equals(x.TONGDATID) && x.TRANGTHAI == 1).ToList();
        ...
        public List<ALD_TONGDAT> GetAnLaoDongThuHoi(List<decimal> ids)
        {
            List<ALD_TONGDAT> aldTongDats = dt.ALD_TONGDAT.Where(x => ids.Contains(x.ID)).ToList();
            return aldTongDats;
        }

        private TongDatDTO ToDTO<T>(T model, Dictionary<decimal?, DM_BIEUMAU> mapBieuMau, Dictionary<string, decimal?> mapUser, Dictionary<decimal?, ALD_DON> mapALD_DON, Dictionary<decimal?, List<ALD_DON_DUONGSU>> mapDuongSu)
        {
            if (model == null)
                return null;
            ...
            if (model.GetType() == typeof(ALD_TONGDAT))
            {
                ...
                dto.Mavuviec = dto.DonId == null ? null : mapALD_DON[dto.DonId].MAVUVIEC;
                dto.Tenvuviec = dto.DonId == null ? null : mapALD_DON[dto.DonId].TENVUVIEC;
                dto.Magiaidoan = dto.DonId == null ? null : mapALD_DON[dto.DonId].MAGIAIDOAN;
                dto.qhpl = dto.DonId == null ? null : mapALD_DON[dto.DonId].QUANHEPHAPLUAT_NAME;
                ...
                {
                    List<ALD_DON_XULY> aLD_DON_XULies = dt.ALD_DON_XULY.Where(x => x.DONID == entity.DONID).ToList();
                    if (aLD_DON_XULies != null && aLD_DON_XULies.Any())
                    {
                        dto.Sothongbao = Convert.ToString(aLD_DON_XULies[0].SOTHONGBAO);
                        dto.Ngaythongbao = Convert.ToDateTime(aLD_DON_XULies[0].NGAYTHONGBAO);
                    }
                    // lay thong tin can bo
                    decimal? canboid = dt.ALD_DON_THAMPHAN.Where(x => x.DONID == entity.DONID && x.MAVAITRO == "VTTP_GIAIQUYETDON").Select(x => x.CANBOID).Take(1).toNumber();
                    dto.Nguoiky = canboid;
                }
                else if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "100-DS") // 100-DS. Thông báo về việc miễn tạm ứng án phí 
                {
                    ALD_ANPHI aDS_ANPHI = dt.ALD_ANPHI.Where(x => x.ID == entity.MAPID).FirstOrDefault();
                    DON_MIENANPHI_BL dON_MIENANPHI = new DON_MIENANPHI_BL();
                    ...
                    if (aDS_ANPHI != null)
                    {
                        ...
                        //dto.mathongbao = dVCQG_THANH_TOAN.DVCQG_THANH_TOAN_SEARCH(2, entity.DONID.Value, aDS_ANPHI.ID, "5");
                    }

                }
                else if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "30-DS")
                {
                    List<ALD_SOTHAM_THULY> aLD_DON_XULies = dt.ALD_SOTHAM_THULY.Where(x => x.DONID == entity.DONID).ToList();
                    if (aLD_DON_XULies != null && aLD_DON_XULies.Any())
                    {
                        dto.Sothongbao = Convert.ToString(aLD_DON_XULies[0].SOTHONGBAO);
                        dto.Ngaythongbao = Convert.ToDateTime(aLD_DON_XULies[0].NGAYTHONGBAO);
                ...
                {
                    List<ALD_PHUCTHAM_THULY> aLD_DON_XULies = dt.ALD_PHUCTHAM_THULY.Where(x => x.DONID == entity.DONID).ToList();
                    if (aLD_DON_XULies != null && aLD_DON_XULies.Any())
                    {
                        dto.Sothongbao = Convert.ToString(aLD_DON_XULies[0].SOTHONGBAO);
                        dto.Ngaythongbao = Convert.ToDateTime(aLD_DON_XULies[0].NGAYTHONGBAO);
                        dto.Nguoiky = aLD_DON_XULies[0].NGUOIKYID;
                    }
                }
                else if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "52-DS") // 52-DS. Bản án dân sự sơ thẩm vnpt Lê Bá Thọ 28/11/2025 lấy số cho vbdh
                {
                    ALD_SOTHAM_BANAN ba = dt.ALD_SOTHAM_BANAN.Where(x => x.DONID == entity.DONID).FirstOrDefault();
                    if (ba != null)
                    {
                        ...
                        ALD_SOTHAM_HDXX oHD = dt.ALD_SOTHAM_HDXX.Where(x => x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN && x.DONID == entity.DONID).OrderByDescending(x => x.NGAYPHANCONG).FirstOrDefault();
                        if (oHD != null)
                        {
                ...
                {
                    ALD_PHUCTHAM_BANAN ba = dt.ALD_PHUCTHAM_BANAN.Where(x => x.DONID == entity.DONID).FirstOrDefault();
                    if (ba != null)
                    {
                        dto.Sothongbao = Convert.ToString(ba.SOBANAN);
                        dto.Ngaythongbao = Convert.ToDateTime(ba.NGAYTUYENAN);
                        //load người kí - mặc định chủ tọa
                        ALD_PHUCTHAM_HDXX oHD = dt.ALD_PHUCTHAM_HDXX.Where(x => x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN && x.DONID == entity.DONID).OrderByDescending(x => x.NGAYPHANCONG).FirstOrDefault();
                        if (oHD != null)
                        {
                            DM_CANBO oTPCT = dt.DM_CANBO.Where(x => x.ID == oHD.CANBOID).FirstOrDefault();
                            if (oTPCT != null)
                ...
                {
                    List<ALD_FILE> aLD_FILEs = dt.ALD_FILE.Where(x => x.ID == entity.FILEID).ToList();
                    if (aLD_FILEs != null && aLD_FILEs.Any())
                    {
                        decimal? maGiaiDoan = aLD_FILEs[0].MAGIAIDOAN;
                        // so tham
                        if (maGiaiDoan != null && maGiaiDoan == 2)
                        {
                            List<ALD_SOTHAM_QUYETDINH> aLD_SOTHAM_QUYETDINHs = dt.ALD_SOTHAM_QUYETDINH.Where(x => x.QUYETDINHID == dM_BIEUMAU.QUYETDINHID && x.DONID == entity.DONID).ToList();
                            if (aLD_SOTHAM_QUYETDINHs != null && aLD_SOTHAM_QUYETDINHs.Any())
                            {
                                dto.Sothongbao = Convert.ToString(aLD_SOTHAM_QUYETDINHs[0].SOQD);
                                dto.Ngaythongbao = Convert.ToDateTime(aLD_SOTHAM_QUYETDINHs[0].NGAYQD);
                                dto.Nguoiky = Convert.ToDecimal(aLD_SOTHAM_QUYETDINHs[0].NGUOIKYID);
                            }
                        }
                        // phuc tham
                        else if (maGiaiDoan != null && maGiaiDoan == 3)
                        {
                            List<ALD_PHUCTHAM_QUYETDINH> aLD_PHUCTHAM_QUYETDINHs = dt.ALD_PHUCTHAM_QUYETDINH.Where(x => x.QUYETDINHID == dM_BIEUMAU.QUYETDINHID && x.DONID == entity.DONID).ToList();
                            if (aLD_PHUCTHAM_QUYETDINHs != null && aLD_PHUCTHAM_QUYETDINHs.Any())
                            {
                                dto.Sothongbao = Convert.ToString(aLD_PHUCTHAM_QUYETDINHs[0].SOQD);
                                dto.Ngaythongbao = Convert.ToDateTime(aLD_PHUCTHAM_QUYETDINHs[0].NGAYQD);
                                dto.Nguoiky = Convert.ToDecimal(aLD_PHUCTHAM_QUYETDINHs[0].NGUOIKYID);
                            }
                        }
                    }
                }
            ...
            else
            {
                try
                {
                    ...
                    if (status == "SUCCESS")
                    {
                        ...
                        //AnhPN log
                        LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                        log.InsertLog("CallApi_ALD", TongDatID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Gửi sang VBĐH", "Gửi thành công", "", "");
                    }
                    else
                    {
                        ...
                        //AnhPN log
                        LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                        log.InsertLog("CallApi_ALD", TongDatID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Gửi sang VBĐH", "Gửi thất bại", "", "");
                    }
                }
                catch (Exception)
                {
        ...
        protected void ddlTCTT_SelectedIndexChanged(object sender, EventArgs e)
        {
            foreach (DataGridItem item in dgTructiep.Items)
            {
                ...
                if (oT.MAGIAIDOAN == 2)
                {
                    ...
                    {
                        dt.ALD_DON_THAMGIATOTUNG.Where(x => x.DONID == DONID && x.TUCACHTGTTID == ddlTCTT.SelectedValue)
                                                .ToList()
                                                .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.HOTEN, x.TUCACHTGTTID + "," + x.ID)));
                    }
                }
                else if (oT.MAGIAIDOAN == 3)
                {
                    ...
                    {
                        dt.ALD_PHUCTHAM_THAMGIATOTUNG.Where(x => x.DONID == DONID && x.TUCACHTGTTID == ddlTCTT.SelectedValue)
                                                    .ToList()
                                                    .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.HOTEN, x.TUCACHTGTTID + "," + x.ID)));
                    }
                }
        ...
        private string GetSoThongBao(decimal bieuMauID, decimal donID, decimal duongsuID)
        {
            ...
            {
                ALD_ANPHI anPhi = dt.ALD_ANPHI.Where(x => x.DONID == donID && x.DUONGSU_ID == duongsuID).FirstOrDefault();
                if (anPhi != null)
                {
            ...
            else if (bieu_mau.MABM == "100-DS") // 100-DS. Thông báo về việc miễn tạm ứng án phí 
            {
                ...
                DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(anPhi.ID, 5);
                if (anPhi != null)
                {
            ...
            {
                ALD_SOTHAM_THULY thuly = dt.ALD_SOTHAM_THULY.Where(x => x.DONID == donID).FirstOrDefault();
                if (thuly != null)
                {
            ...
            else if (bieu_mau.QUYETDINHID != null) // Các biểu mẫu quyết định vụ việc
            {
                ...
                {
                    ALD_SOTHAM_QUYETDINH qdinh = dt.ALD_SOTHAM_QUYETDINH.Where(x => x.DONID == donID).FirstOrDefault();
                    if (qdinh != null)
                    {
                        STB = qdinh.SOQD;
                    }
                ...
                {
                    ALD_PHUCTHAM_QUYETDINH qdinh = dt.ALD_PHUCTHAM_QUYETDINH.Where(x => x.DONID == donID).FirstOrDefault();
                    if (qdinh != null)
                    {
                        STB = qdinh.SOQD;
                    }
