...
namespace WEB.GSTP.QLAN.ALD.PhucthamQDK
{
    public partial class Bananphuctham : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    hddDonID.Value = Session[ENUM_LOAIAN.AN_LAODONG] + "";
        ...
        {
            decimal DONID = Convert.ToDecimal(hddDonID.Value);
            string Result = new ALD_CHUYEN_NHAN_AN_BL().Check_ChuyenNhanAn(DONID, "Không được sửa đổi thông tin.", Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]));
            if (Result != "")
            {
            ...
            //vnpt 16072025: bo sung check theo toa giai quyet
            string donviID = Session[ENUM_SESSION.SESSION_DONVIID]?.ToString();
            List<ALD_KCKNQDK_PHUCTHAM_QUYETDINH> lstQD = DataExtensions.GetAllWithClause<ALD_KCKNQDK_PHUCTHAM_QUYETDINH>("DONID = " + DONID + "AND TOA_GIAIQUYET_ID  = " + donviID + " AND (LOAIQDID IN (10,3) OR QUYETDINHID IN (213,145,146))");
            if (lstQD.Count >= 1)
            {
                pnQDVV.Visible = true;
            ...
            ALD_TONGDAT td = dt.ALD_TONGDAT.Where(x => x.DONID == DONID && (x.BIEUMAUID == 230 || x.BIEUMAUID == 261)).FirstOrDefault();
            if (td != null)
            {
            ...
            ALD_KCKNQDK_PHUCTHAM_QUYETDINH qd64DS = DataExtensions.GetAllWithClause<ALD_KCKNQDK_PHUCTHAM_QUYETDINH>($"DONID = {DONID} AND QUYETDINHID = 64").FirstOrDefault();
            if (qd64DS == null)
            {
        ...
        protected void cmdUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (oND == null)
                {
                    oND = new ALD_PHUCTHAM_BANAN(); isNew = true;
        ...
        private string getQHPL_NAME_THULY()
        {
            ...
            ALD_KCKNQDK_PHUCTHAM_THULY oT = DataExtensions.GetAllWithClause<ALD_KCKNQDK_PHUCTHAM_THULY>("DONID = " + ID).FirstOrDefault();
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.ALD_PHUCTHAMKCKNQDK_BANANQUYETDINH_GETLIST(ID);
        ...
        private void LoadNguoiKyInfo()
        {
            ...
            ALD_KCKNQDK_PHUCTHAM_HDXX oND = DataExtensions.GetAllWithClause<ALD_KCKNQDK_PHUCTHAM_HDXX>("DONID = " + DonID + " AND MAVAITRO ='" + ENUM_NGUOITIENHANHTOTUNG.THAMPHAN + "'").FirstOrDefault<ALD_KCKNQDK_PHUCTHAM_HDXX>();
            if (oND != null)
            {
        ...
        {
            Decimal ID = Convert.ToDecimal(hddDonID.Value);
            //List<ALD_PHUCTHAM_QUYETDINH> lstQD = dt.ALD_PHUCTHAM_QUYETDINH.Where(x => x.DONID == ID && (x.LOAIQDID == 10 || x.QUYETDINHID == 423 || x.QUYETDINHID == 422 || x.LOAIQDID == 3 || x.LOAIQDID == 23)).ToList();
            List<ALD_KCKNQDK_PHUCTHAM_QUYETDINH> lstQD = DataExtensions.GetAllWithClause<ALD_KCKNQDK_PHUCTHAM_QUYETDINH>
