...
namespace WEB.GSTP.QLAN.ALD.PhucthamQDK
{
    public partial class ThuLy : System.Web.UI.Page
    {
        ...
        private void LoadQD()
        {
            ...
            decimal donOld = aLD_CHUYEN_NHAN_AN_BL.getDonIdOld(idAn);

            ALD_SOTHAM_KHANGCAO aLD_SOTHAM_KHANGCAO = dt.ALD_SOTHAM_KHANGCAO.Where(x => x.DONID == donOld && x.LOAIKHANGCAO == 2 && x.TINHTRANG_GIAIQUYET != 1).FirstOrDefault();
            ALD_SOTHAM_KHANGNGHI oKN = dt.ALD_SOTHAM_KHANGNGHI.Where(x => x.DONID == donOld && x.LOAIKN == 2 && x.TINHTRANG_GIAIQUYET != 1).FirstOrDefault();
            if (aLD_SOTHAM_KHANGCAO != null)
            {
                ALD_SOTHAM_QUYETDINH aLD_SOTHAM_QUYETDINH = dt.ALD_SOTHAM_QUYETDINH.Where(x => x.ID == aLD_SOTHAM_KHANGCAO.SOQDBA).FirstOrDefault();
                if (aLD_SOTHAM_QUYETDINH != null)
                {
                    DM_QD_QUYETDINH dM_QD_QUYETDINH = dt.DM_QD_QUYETDINH.Where(x => x.ID == aLD_SOTHAM_QUYETDINH.QUYETDINHID).FirstOrDefault();
                    txtQDST.Text = "Số " + aLD_SOTHAM_QUYETDINH.SOQD + " - " + dM_QD_QUYETDINH.TEN;
                }
            }
            else if (oKN != null)
            {
                ALD_SOTHAM_QUYETDINH aLD_SOTHAM_QUYETDINH = dt.ALD_SOTHAM_QUYETDINH.Where(x => x.ID == oKN.BANANID).FirstOrDefault();
                if (aLD_SOTHAM_QUYETDINH != null)
                {
                    DM_QD_QUYETDINH dM_QD_QUYETDINH = dt.DM_QD_QUYETDINH.Where(x => x.ID == aLD_SOTHAM_QUYETDINH.QUYETDINHID).FirstOrDefault();
                    txtQDST.Text = "Số " + aLD_SOTHAM_QUYETDINH.SOQD + " - " + dM_QD_QUYETDINH.TEN;
        ...
        private void CheckShowCommand(decimal DonID)
        {
            ...
            if (oT != null)
            {
                ...
                ALD_DON_THAMPHAN_BL oBL = new ALD_DON_THAMPHAN_BL();
                DataTable oDT = oBL.ALD_DON_THAMPHAN_GETBY(DonID, ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM);
        ...
        private void SetNewSoThuLy()
        {
            ...
            txtSoThuly.Text = oSTBL.GET_STL_NEW(DonViID, "ALD_PTQDK", ngaythuly).ToString();
            SetNewSoThongbao();
        }
        ...
        private void SetNewSoThongbao()
        {
            ...
            txtSothongbao.Text = oSTBL.GET_STBTL_NEW(DonViID, "ALD_PTQDK", ngaythongbao).ToString();
        }
        ...
        private void LoadCombobox()
        {
            ...
            string current_id = Session[ENUM_LOAIAN.AN_LAODONG] + "";
            decimal idAn = Convert.ToDecimal(current_id);
            List<ALD_CHUYEN_NHAN_AN> listcna = dt.ALD_CHUYEN_NHAN_AN.Where(x => x.MAP_VUANID_NEW == idAn && x.TOANHANID == toaanId).ToList();
        ...
        private bool CheckValid()
        {
            ...
            if (!String.IsNullOrEmpty(txtNgaythuly.Text))
            {
                ...
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal CheckID = oSTBL.CheckSoTLTheoLoaiAn(DonViID, "ALD_PTQDK", sothuly, ngaythuly);
                if (CheckID > 0)
                {
                    Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
                    String strMsg = "";
                    String STTNew = oSTBL.GET_STL_NEW(DonViID, "ALD_PTQDK", ngaythuly).ToString();
            ...
            if (!String.IsNullOrEmpty(txtNgaythuly.Text))
            {
                ...
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal CheckID = oSTBL.CheckSoTBTLTheoLoaiAn(DonViID, "ALD_PTQDK", sothongbao, ngaythuly);
                if (CheckID > 0)
                {
                    Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
                    String strMsg = "";
                    String STTNew = oSTBL.GET_STBTL_NEW(DonViID, "ALD_PTQDK", ngaythuly).ToString();
                    if (CheckID != CurrThuLyID)
                    {
                        //lbthongbao.Text = "Số thụ lý này đã có!";
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                decimal FileID = 0;
                ALD_KCKNQDK_PHUCTHAM_THULY oND;
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    oND = new ALD_KCKNQDK_PHUCTHAM_THULY();
                ...
                else
                {
                    ...
                    oND = DataExtensions.FindById<ALD_KCKNQDK_PHUCTHAM_THULY>(ID);
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    {
                        ALD_KCKN_PHUCTHAM_BL oPTBL = new ALD_KCKN_PHUCTHAM_BL();
                        ...
                        GD.GAIDOAN_INSERT_UPDATE("3", DONID, 3, 0,
        ...
        //    List<AHN_PHUCTHAM_BANAN_FILE> lst = dt.A _PHUCTHAM_BANAN_FILE.Where(x => x.BANANID == DonID).ToList();
        ...
        {
            ALD_KCKNQDK_PHUCTHAM_THULY oND = DataExtensions.FindById<ALD_KCKNQDK_PHUCTHAM_THULY>(id); //  ALD_PHUCTHAM_THULY oND = dt.ALD_PHUCTHAM_THULY.Where(x => x.ID == id).FirstOrDefault();
            if (oND != null)
            {
                ...
                if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.DONID), Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_LAODONG), Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Thụ lý Phúc thẩm án Hôn nhân", "Xóa", json) == false)
                {
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                {
                    case "Download":
                        var oND = DataExtensions.FindById<ALD_KCKNQDK_PHUCTHAM_THULY>(ND_id);
                        if (oND.TENFILE != "")
                        {
        ...
        private void txtQuanhephapluat_name(ALD_KCKNQDK_PHUCTHAM_THULY oT)
        {
            if (oT.QUANHEPHAPLUAT_NAME != null)
            {
                txtQuanhephapluat.Text = oT.QUANHEPHAPLUAT_NAME;
