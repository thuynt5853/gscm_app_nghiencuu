    ...
    public partial class GiaiquyetPhuctham : System.Web.UI.Page
    {
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                ALD_PHUCTHAM_HDXX hdxx = dt.ALD_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == DonviID).FirstOrDefault();
                ALD_PHUCTHAM_QUYETDINH qd = dt.ALD_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == DonviID).FirstOrDefault();
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string current_id = Session[ENUM_LOAIAN.AN_LAODONG] + "", MaVaiTro = ddlVaitro.SelectedValue;
                decimal DONID = Convert.ToDecimal(current_id), CanBoID = Convert.ToDecimal(ddlThamphan.SelectedValue);

                ALD_DON_THAMPHAN oND;
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    // Kiểm tra nếu thẩm phán đã được phân công rồi thì không phân lại nữa. Chọn thẩm phán khác
                    oND = dt.ALD_DON_THAMPHAN.Where(x => x.DONID == DONID && x.CANBOID == CanBoID && x.MAVAITRO == MaVaiTro).FirstOrDefault();
                    if (oND != null)
                    {
                    ...
                    oND = new ALD_DON_THAMPHAN();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.ALD_DON_THAMPHAN.Where(x => x.ID == ID).FirstOrDefault();
                }
                oND.DONID = DONID;
                oND.CANBOID = CanBoID;
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.ALD_DON_THAMPHAN_GETBY(ID, ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM);
        ...
        public void xoa(decimal id)
        {
            ...
            ALD_DON_THAMPHAN oND = dt.ALD_DON_THAMPHAN.Where(x => x.ID == id).FirstOrDefault();
            ...
            if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.DONID), 5, Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Thẩm phán Phúc thẩm án Lao động", "Xóa", json) == false)
            {
            ...
            ALD_PHUCTHAM_THAMGIATOTUNG ALD_PHUCTHAM_THAMGIATOTUNG = dt.ALD_PHUCTHAM_THAMGIATOTUNG.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == DonviID).FirstOrDefault<ALD_PHUCTHAM_THAMGIATOTUNG>();
            if (ALD_PHUCTHAM_THAMGIATOTUNG != null)
            {
                lbthongbao.Text = "Xóa không thành công! Do đang tồn tại người tham gia tố tụng.";
                return;
            }
            ALD_PHUCTHAM_HDXX ALD_PHUCTHAM_HDXX = dt.ALD_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == DonviID).FirstOrDefault<ALD_PHUCTHAM_HDXX>();
            if (ALD_PHUCTHAM_HDXX != null)
            {
                lbthongbao.Text = "Xóa không thành công! Do đang tồn tại thông tin người tiến hành tố tụng.";
                return;
            }
            ALD_PHUCTHAM_HOAGIAI ALD_PHUCTHAM_HOAGIAI = dt.ALD_PHUCTHAM_HOAGIAI.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == DonviID).FirstOrDefault<ALD_PHUCTHAM_HOAGIAI>();
            if (ALD_PHUCTHAM_HOAGIAI != null)
            {
                lbthongbao.Text = "Xóa không thành công! Do đang tồn tại thông tin hòa giải.";
                return;
            }
            ALD_PHUCTHAM_QUYETDINH ALD_PHUCTHAM_QUYETDINH = dt.ALD_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == DonviID).FirstOrDefault<ALD_PHUCTHAM_QUYETDINH>();
            ...
            dt.ALD_DON_THAMPHAN.Remove(oND);
            dt.SaveChanges();
            dgList.CurrentPageIndex = 0;
            LoadGrid();
            ResetControls();
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            //ALD_PHUCTHAM_HDXX hdxx = dt.ALD_PHUCTHAM_HDXX.Where(x => x.DONID == DonID).FirstOrDefault();
            //ALD_PHUCTHAM_QUYETDINH qd = dt.ALD_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DonID).FirstOrDefault();
        ...
        private bool CheckQDofThamPhan(decimal DonID, decimal TPID)
        {
            ...
            ALD_PHUCTHAM_QUYETDINH obj_QD = dt.ALD_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.NGUOIKYID == TPID).FirstOrDefault<ALD_PHUCTHAM_QUYETDINH>();
            if (obj_QD != null)
            {
