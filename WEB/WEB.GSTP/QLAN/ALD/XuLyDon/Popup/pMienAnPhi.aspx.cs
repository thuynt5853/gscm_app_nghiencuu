    ...
    public partial class pMienAnPhi : System.Web.UI.Page
    {
        ...
        protected void cmdCapNhatAP_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(APId, 5);
                ...
                try
                {
                    ...
                    if ((dataTable != null && dataTable.Rows.Count > 0 && hddHistoryId.Value != "") || hddHistoryId.Value == "")
                    {
                        ...
                        decimal Mienanphi_ID = dON_MIENANPHI_BL.UPSERT_DON_MIENANPHI(ID_MIENANPHI, APId, DONID, 5, v_lydo, nguoitao, ngaytao, nguoisua, ngaysua, v_sothongbao, d_ngaythongbao, v_stb_phu);
        ...
        private bool CheckValidAP()
        {
            ...
            ALD_ANPHI objAnPhi = dt.ALD_ANPHI.Where(x => x.ID == A_ANPHI_ID).FirstOrDefault();
            ...
            DON_MIENANPHI_BL dON_MIENANPHI_BL = new DON_MIENANPHI_BL();
            DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(A_ANPHI_ID, 5);
            ...
            if (hddHistoryId.Value != "")
            {
                ...
                if (dS_TONGDAT_BL.ALD_TONGDATDOITUONG_ANPHI_GETBYANPHIID_TONGDATID(objAnPhi.DONID, objAnPhi.ID).Rows.Count > 0)
                {
            ...
            switch (e.CommandName)
            {
                ...
                case "XoaAP":
                    try
                    {
                        ...
                        obj.CHECK_DELETE_ANPHI(CurrID, "5", ref _check_out);//anhvh check delete 06/09/2023
                        if (_check_out == 1)
                        {
                            ALD_ANPHI objAP = dt.ALD_ANPHI.Where(x => x.ID == CurrID).FirstOrDefault();
                            ALD_TONGDAT oTD = dt.ALD_TONGDAT.Where(x => x.DONID == objAP.DONID).FirstOrDefault();
                            ALD_TONGDAT_BL dS_TONGDAT_BL = new ALD_TONGDAT_BL();
                            if (dS_TONGDAT_BL.ALD_TONGDATDOITUONG_ANPHI_GETBYANPHIID_TONGDATID(objAP.DONID, objAP.ID).Rows.Count > 0)
                            ...
                            else
                            {
                                ...
                                LOG_THONGTINANPHI_QLTA log = new LOG_THONGTINANPHI_QLTA();
                                log.InsertLog(Session[ENUM_LOAIAN.AN_LAODONG].ToString(), CurrID.ToString(), MATHONGBAO, Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Update", CONTENT_JSON, CONTENT_JSON_OLD);
                                dt.SaveChanges();
        ...
        protected void rptAP_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                ALD_DON_XULY tralaiDonXuly = dt.ALD_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 3).FirstOrDefault();

                DON_MIENANPHI_BL dON_MIENANPHI_BL1 = new DON_MIENANPHI_BL();
                DataTable dataTable1 = dON_MIENANPHI_BL1.GET_DON_MIENANPHI_BY_ANPHI_ID(vAPID, 5);
                ...
                {
                    ALD_ANPHI objAP = dt.ALD_ANPHI.Where(x => x.ID == vAPID).FirstOrDefault();
        ...
        void LoadInfo_AnPhiDaMien(Decimal CurrID)
        {
            ...
            hddHistoryId.Value = CurrID + "";
            ALD_ANPHI aDSAnphi = dt.ALD_ANPHI.Where(x => x.ID == CurrID).FirstOrDefault();
            if (aDSAnphi != null)
            {
                chkNopAnPhi.Checked = true;
        ...
        private void LoadGrid_AnPhi()
        {
            ...
            int pageindex = 1;
            decimal A_ANPHI_ID = Convert.ToDecimal(Request.QueryString["ID"].ToString());
            DataTable oDT = oBL.ALD_ANPHI_LICHSU_GETBYDONID(DONID, A_ANPHI_ID, 1, pageindex, page_size);
            //if (oDT.Rows.Count > 0)
            //{
