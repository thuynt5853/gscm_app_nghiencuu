...
namespace WEB.GSTP.QLAN.ALD.Sotham
{
    public partial class NguoiTienhanhTT : System.Web.UI.Page
    {
        ...
        private void CheckQuyen(decimal ID)
        {
            ...
            ALD_DON_THAMPHAN obj = dt.ALD_DON_THAMPHAN.Where(x => x.DONID == ID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).OrderByDescending(x => x.NGAYNHANPHANCONG).FirstOrDefault();
            if (obj != null)
            {
            ...
            List<ALD_DON_THAMPHAN> lst = dt.ALD_DON_THAMPHAN.Where(x => x.DONID == ID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).ToList<ALD_DON_THAMPHAN>();
            ...
            ALD_SOTHAM_BANAN oBA = dt.ALD_SOTHAM_BANAN.Where(x => x.DONID == ID).FirstOrDefault();
            if (oBA != null)
            {
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                ALD_SOTHAM_HDXX obj = dt.ALD_SOTHAM_HDXX.Where(x => x.ID == ID).FirstOrDefault();
                if (obj != null)
                {

                    ALD_CHUYEN_NHAN_AN chuyenan = dt.ALD_CHUYEN_NHAN_AN.Where(x => x.VUANID == DONID).OrderByDescending(x => x.ID).FirstOrDefault();
                    if (chuyenan != null)
                    {
                        if (obj.NGAYTAO <= chuyenan.NGAYTAO)
                        {
                            List<ALD_SOTHAM_QUYETDINH> qd = dt.ALD_SOTHAM_QUYETDINH.Where(x => x.DONID == DONID).ToList();
                        ...
                        {
                            List<ALD_SOTHAM_QUYETDINH> qd = dt.ALD_SOTHAM_QUYETDINH.Where(x => x.DONID == DONID && x.NGAYTAO > chuyenan.NGAYTAO).ToList();
                            if ((qd != null && qd.Count > 0))
                            {
                                lblSua.Visible = false;
                                lbtXoa.Visible = false;
                    ...
                    else
                    {
                        ...
                        List<ALD_SOTHAM_QUYETDINH> qd = dt.ALD_SOTHAM_QUYETDINH.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == Donvi_ID).ToList();
                        if ((qd != null && qd.Count > 0))
                        {
                            lblSua.Visible = false;
                            lbtXoa.Visible = false;
                ...
                // hoangndh -vnpt kiem tra quyen toa an sua xoa
                ...
                //ALD_SOTHAM_QUYETDINH qd = dt.ALD_SOTHAM_QUYETDINH.Where(x => x.DONID == DONID).FirstOrDefault();
                //if (qd != null)
                //{
                //    lblSua.Visible = false;
                //    lbtXoa.Visible = false;
        ...
        private void LoadCombobox()
        {
            ...
            if (loaiQD != null)
            {
                ALD_SOTHAM_QUYETDINH qd = dt.ALD_SOTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.LOAIQDID == loaiQD.ID).FirstOrDefault<ALD_SOTHAM_QUYETDINH>();
                if (qd != null)
                {
                    ALD_DON_THAMPHAN tpgq = dt.ALD_DON_THAMPHAN.Where(x => x.DONID == DonID && x.MAVAITRO == "VTTP_GIAIQUYETSOTHAM").OrderByDescending(x => x.NGAYTAO).FirstOrDefault<ALD_DON_THAMPHAN>();
                    if (tpgq != null)
                    {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                List<ALD_DON_THAMPHAN> countTPGQ = dt.ALD_DON_THAMPHAN.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).ToList();
                List<ALD_SOTHAM_HDXX> countChuToa = dt.ALD_SOTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).ToList();
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    if (tucach == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN)
                    {
                        ...
                        oND = dt.ALD_SOTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).FirstOrDefault<ALD_SOTHAM_HDXX>();
                    ...
                    oND = new ALD_SOTHAM_HDXX();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.ALD_SOTHAM_HDXX.Where(x => x.ID == ID).FirstOrDefault();
                }
                oND.DONID = DONID;
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    ALD_SOTHAM_HDXX CheckCanBo = dt.ALD_SOTHAM_HDXX.Where(x => x.DONID == DONID && x.CANBOID == oND.CANBOID).FirstOrDefault();
        ...
        public void LoadGrid()
        {
            ...
            string current_id = Session[ENUM_LOAIAN.AN_LAODONG] + "";
            decimal ID = Convert.ToDecimal(current_id);
            DataTable oDT = oBL.ALD_SOTHAM_HDXX_GETLIST(ID);
        ...
        {
            ALD_SOTHAM_HDXX oND = dt.ALD_SOTHAM_HDXX.Where(x => x.ID == id).FirstOrDefault();
            dt.ALD_SOTHAM_HDXX.Remove(oND);
            dt.SaveChanges();
            dgList.CurrentPageIndex = 0;
            LoadGrid();
            ResetControls();
