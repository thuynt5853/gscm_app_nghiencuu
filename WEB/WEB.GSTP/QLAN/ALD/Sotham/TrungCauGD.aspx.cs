...
{
    public partial class TrungCauGD1 : System.Web.UI.Page
    {
        GSTPContext dt = new GSTPContext();
        CultureInfo cul = new CultureInfo("vi-VN");
        ALD_TRUNGCAU_GIAMDINH obj = new ALD_TRUNGCAU_GIAMDINH();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                VuAnID = (String.IsNullOrEmpty(Session[ENUM_LOAIAN.AN_LAODONG] + "")) ? 0 : Convert.ToDecimal(Session[ENUM_LOAIAN.AN_LAODONG] + "");
                hddMaGiaiDoan.Value = MaGiaiDoan + "";
                hddVuAnID.Value = VuAnID + "";
                ALD_TRUNGCAU_GIAMDINH oT = dt.ALD_TRUNGCAU_GIAMDINH.Where(x => x.VUANID == VuAnID && x.MAGIAIDOAN == MaGiaiDoan).SingleOrDefault();
                if (oT != null)
                {
                    LoadInfo();
                }
                ...
                ALD_DON oDT = dt.ALD_DON.Where(x => x.ID == VuAnID).FirstOrDefault();
                ...
                List<ALD_SOTHAM_THULY> lstCount = dt.ALD_SOTHAM_THULY.Where(x => x.DONID == VuAnID).ToList();
                if (lstCount.Count == 0)
                {
                ...
                List<ALD_DON_THAMPHAN> lstTP = dt.ALD_DON_THAMPHAN.Where(x => x.DONID == VuAnID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).ToList();
                if (lstTP.Count == 0)
                {
                ...
                string Result = new ALD_CHUYEN_NHAN_AN_BL().Check_NhanAn(VuAnID, StrMsg);
                if (Result != "")
                {
                ...
                //    int countItem = dt.ALD_DON_THAMPHAN.Count(s => s.THUKYID == CurrentUserId && s.DONID == VuAnID && s.MAVAITRO == "VTTP_GIAIQUYETSOTHAM");
                //    if (countItem > 0)
                //    {
                //        //được gán 
                //    }
        ...
        protected void cmdUpdateBottom_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                obj = dt.ALD_TRUNGCAU_GIAMDINH.Where(x => x.VUANID == mavuan && x.MAGIAIDOAN == MaGiaidoan).SingleOrDefault();
            }
            catch (Exception ex)
            {
            ...
            else
            {
                ...
                dt.ALD_TRUNGCAU_GIAMDINH.Add(obj);
                dt.SaveChanges();
            }
