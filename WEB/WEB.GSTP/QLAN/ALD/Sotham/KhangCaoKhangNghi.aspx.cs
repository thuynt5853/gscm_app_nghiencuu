...
namespace WEB.GSTP.QLAN.ALD.Sotham
{
    public partial class KhangCaoKhangNghi : System.Web.UI.Page
    {
        ...
        private void CheckQuyen(decimal ID)
        {
            ...
            List<ALD_DON_THAMPHAN> lstTP = dt.ALD_DON_THAMPHAN.Where(x => x.DONID == ID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).ToList();
            if (lstTP.Count == 0)
            {
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (IsKhangCao == 1)
                {
                    ALD_SOTHAM_KHANGCAO oKC = dt.ALD_SOTHAM_KHANGCAO.Where(x => x.ID == idKCKN).FirstOrDefault();
                    if (oKC != null)
                    {
                        ...
                        string Result = new ALD_CHUYEN_NHAN_AN_BL().Check_ChuyenNhanAn(DONID, StrMsg, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]));
                        if (Result == "")
                        {
                ...
                {
                    ALD_SOTHAM_KHANGNGHI oKN = dt.ALD_SOTHAM_KHANGNGHI.Where(x => x.ID == idKCKN).FirstOrDefault();
                    if (oKN != null)
                    {
        ...
        private void LoadComboboxKhangCao()
        {
            ...
            //load lấy người tham gia tố tụng();
            string hoten = "";
            List<ALD_DON_THAMGIATOTUNG> lst = dt.ALD_DON_THAMGIATOTUNG.Where(x => x.DONID == DonID).ToList();
        ...
        void LoadDrop_ST_BanAn(DropDownList drop, Decimal DonID)
        {
            ...
            DataTable oDT = oBL.DANHSACH_KHANGCAO_BANAN(ENUM_LOAIVUVIEC_NUMBER.AN_LAODONG, DonID);
            if (oDT != null)
            {
                drop.DataSource = oDT;
        ...
        void LoadDrop_ST_QuyetDinh(DropDownList drop, Decimal DonID, Decimal loaiKC)
        {
            ...
            {
                PAGE_KHANGCAO_ST oBL = new PAGE_KHANGCAO_ST();
                DataTable oDT = oBL.DANHSACH_KHANGCAO_QUYETDINH(ENUM_LOAIVUVIEC_NUMBER.AN_LAODONG, DonID);
                if (oDT != null)
                {
                    drop.DataSource = oDT;
                    drop.DataTextField = "TEN";
            ...
            {
                PAGE_KHANGCAO_ST oBL = new PAGE_KHANGCAO_ST();
                DataTable oDT = oBL.DANHSACH_KHANGCAO_QUYETDINH_KHAC(ENUM_LOAIVUVIEC_NUMBER.AN_LAODONG, DonID);
                if (oDT != null)
                {
                    drop.DataSource = oDT;
                    drop.DataTextField = "TEN";
            ...
            try
            {
                ...
                switch (e.CommandName)
                {
                    case "Download":
                        ...
                        if (IsKhangCao == 1)
                        {
                            ALD_SOTHAM_KHANGCAO oKN = dt.ALD_SOTHAM_KHANGCAO.Where(x => x.ID == ID).FirstOrDefault();
                            TENFILE = oKN.TENFILE;
                    ...
                    case "Xoa":
                        ...
                        ALD_SOTHAM_KHANGCAO oKC = dt.ALD_SOTHAM_KHANGCAO.Where(x => x.DONID == DonID && x.ID == ID).FirstOrDefault();
                        if (Result != "")
                        {
                            if (oKC.ISQUAHAN == 1)
                            {
            ...
            try
            {
                ...
                if (IsKhangCao == KHANGCAO)// Kháng cáo
                {
                    ...
                    if (oND != null)
                    {
                        ...
                        ALD_ANPHI oAP = dt.ALD_ANPHI.Where(x => x.DONID == oND.DONID && x.DUONGSU_ID == oND.DUONGSUID && x.MAGIAIDOAN == 3).FirstOrDefault();
                        if (oAP != null)
                        {
        ...
        public void xoa(decimal ID, decimal IsKhangCao)
        {
            if (IsKhangCao == KHANGCAO)// Kháng cáo
            {
                ...
                ALD_ANPHI objAP = dt.ALD_ANPHI.Where(x => x.DONID == DonID && x.DUONGSU_ID == oND.DUONGSUID && x.MAGIAIDOAN == 3).FirstOrDefault();
                if (objAP != null)
                {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                decimal ID = 0, IsKhangCao = 0, DonID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_LAODONG] + "");
                ALD_DON oDon = dt.ALD_DON.Where(x => x.ID == DonID).FirstOrDefault();
                if (rdbPanelKC.SelectedValue == KHANGCAO.ToString() && rdbPanelKN.SelectedValue == KHANGCAO.ToString())// Kháng cáo
                {
                    ...
                    {
                        ALD_ANPHI lstAPnew = new ALD_ANPHI();
        ...
        protected void rdbPanelKC_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else // Kháng nghị
            {
                ...
                DON_KHAC oD = dt.DON_KHAC.Where(x => x.DONID == DonID && x.LOAIANID == 5 && x.LOAIKCKN == 2).FirstOrDefault();
        ...
        protected void cmdLoadDonKhac_Click(object sender, EventArgs e)
        {
            ...
            DON_KHAC oD = dt.DON_KHAC.Where(x => x.ID == ID && x.LOAIANID == 5 && x.LOAIKCKN == 1).FirstOrDefault();
            if (oD != null)
            {
                pnKhangCao.Visible = true;
                pnKhangNghi.Visible = false;
        ...
        void CheckNgayKCQuaHan()
        {
            ...
            if (banan_qd_id > 0)
            {
                if (loai_ba_qd == 0)
                {
                    ...
                    ALD_SOTHAM_BANAN obj = dt.ALD_SOTHAM_BANAN.Where(x => x.ID == banan_qd_id).FirstOrDefault();
                    if (obj != null)
                ...
                else
                {
                    ...
                    ALD_SOTHAM_QUYETDINH obj = dt.ALD_SOTHAM_QUYETDINH.Where(x => x.ID == banan_qd_id).FirstOrDefault();
                    if (obj != null)
                    {
