...
namespace WEB.GSTP.QLAN.ALD.Sotham
{
    public partial class ThuLy : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    DataTable oDT = oBL.ALD_SOTHAM_THULY_GETLIST(ID);
        ...
        private void LoadNguyenDon(decimal donID)
        {
            ...
            DataTable tb = oBL.ALD_THULY_NGUYENDON(donID);
            if (tb.Rows.Count > 0)
            {
        ...
        private void LoadBiDon(decimal donID)
        {
            ...
            //Load bị đơn 
            List<ALD_DON_DUONGSU> tb = dt.ALD_DON_DUONGSU.Where(x => x.DONID == donID && x.TUCACHTOTUNG_MA == "BIDON").OrderByDescending(x => x.ISDAIDIEN).ToList();
            if (tb.Count > 0)
            {
        ...
        private void LoadTHThuyLy(decimal vid)
        {
            ...
            ALD_DON obj = dt.ALD_DON.Where(x => x.ID == vid).FirstOrDefault();
        ...
        private void CheckShowCommand(decimal DonID)
        {
            ...
            ALD_SOTHAM_QUYETDINH qd = dt.ALD_SOTHAM_QUYETDINH.Where(x => x.DONID == DonID && (x.LOAIQDID == 3 || x.LOAIQDID == 10 || x.LOAIQDID == 11 || x.LOAIQDID == 21)).FirstOrDefault();
            if (qd != null)
            {
        ...
        private decimal SetNewSoThuLy()
        {
            ...
            string sothulymoi = oSTBL.GET_STL_NEW(DonViID, "ALD", ngaythuly).ToString();
        ...
        void SetNewSoThongbao()
        {
            ...
            txtSothongbao.Text = oSTBL.GET_STBTL_NEW(DonViID, "ALD", ngaythongbao).ToString();
        ...
        private void ResetControls()
        {
            ...
            LoadNguyenDon(Convert.ToDecimal(Session[ENUM_LOAIAN.AN_LAODONG] + ""));
            LoadBiDon(Convert.ToDecimal(Session[ENUM_LOAIAN.AN_LAODONG] + ""));
            decimal donID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_LAODONG] + "");
            ALD_SOTHAM_THULY tl = dt.ALD_SOTHAM_THULY.Where(x => x.DONID == donID).FirstOrDefault();
            if (tl != null)
            {
        ...
        private bool CheckValid()
        {
            ...
            decimal DONID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_LAODONG]);
            List<ALD_DON_XULY> lstGQD = dt.ALD_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 5).ToList();
            if (lstGQD.Count > 0)
            {
                ALD_DON_XULY oDXL = lstGQD[0];
            ...
            //    Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
            //    ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
            //    Decimal CheckID = oSTBL.CheckSoTLTheoLoaiAn(DonViID, "ALD", sothuly, ngaythuly);
            //    if (CheckID > 0)
            //    {
            //        Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
            //        String strMsg = "";
            //        String STTNew = oSTBL.GET_STL_NEW(DonViID, "ALD", ngaythuly).ToString();
            ...
            if (!String.IsNullOrEmpty(txtNgaythuly.Text))
            {
                ...
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal CheckID = oSTBL.CheckSoTBTLTheoLoaiAn(DonViID, "ALD", sothongbao, ngaythuly);
                if (CheckID > 0)
                {
                    Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
                    String strMsg = "";
                    String STTNew = oSTBL.GET_STBTL_NEW(DonViID, "ALD", ngaythuly).ToString();
                    if (CheckID != CurrThuLyID)
                    {
                        //lbthongbao.Text = "Số thụ lý này đã có!";
        ...
        private void LoadDuongSu()
        {
            decimal DONID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_LAODONG] + "");
            ALD_DON_DUONGSU_BL oBL = new ALD_DON_DUONGSU_BL();
            dgDuongSu.DataSource = oBL.ALD_DON_DUONGSU_GETBY(DONID);
            dgDuongSu.DataBind();
        ...
        protected void cmdDuongSu_Click(object sander, EventArgs e)
        {
            try
            {
                ...
                foreach (DataGridItem oItem in dgDuongSu.Items)
                {
                    ...
                    ALD_DON_DUONGSU oT = dt.ALD_DON_DUONGSU.Where(x => x.ID == DSID).FirstOrDefault();
                    if (oT != null)
                    {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                decimal FileID = 0;
                ALD_SOTHAM_THULY oND;
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    oND = new ALD_SOTHAM_THULY();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.ALD_SOTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
                ...
                ALD_SOTHAM_BL oBL = new ALD_SOTHAM_BL();
                DataTable oDT = oBL.ALD_SOTHAM_THULY_GETLIST(DONID);

                STPT_QUANLY_SOTHULY oQLSTL = new STPT_QUANLY_SOTHULY();
                if (oQLSTL.update_STPT_QUANLY_SOTHULY(2, 5, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), ((DateTime)oND.NGAYTHULY).ToString("dd/MM/yyyy", cul), Regex.Match(oND.SOTHULY, @"\d+").Value) == false)
                {
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    else
                    {
                        ...
                        oND.MATHULY = "S" + ENUM_LOAIVUVIEC.AN_LAODONG + Session[ENUM_SESSION.SESSION_MADONVI] + oND.TT.ToString();
                        oND.NGAYTAO = DateTime.Now;
                        oND.NGUOITAO = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                        dt.ALD_SOTHAM_THULY.Add(oND);
                        dt.SaveChanges();
                ...
                ALD_DON_DUONGSU ndcu = dt.ALD_DON_DUONGSU.Where(x => x.DONID == DONID && x.ISDAIDIEN == 1 && x.TUCACHTOTUNG_MA == "NGUYENDON").FirstOrDefault();
                ALD_DON_DUONGSU bdcu = dt.ALD_DON_DUONGSU.Where(x => x.DONID == DONID && x.ISDAIDIEN == 1 && x.TUCACHTOTUNG_MA == "BIDON").FirstOrDefault();
                ALD_DON_DUONGSU ndmoi = dt.ALD_DON_DUONGSU.Where(x => x.ID == idNDmoi).FirstOrDefault();
                ALD_DON_DUONGSU bdmoi = dt.ALD_DON_DUONGSU.Where(x => x.ID == idBDmoi).FirstOrDefault();
                if (idNDmoi != ndcu.ID)
                {
                    ndcu.ISDAIDIEN = 0;
                    ndmoi.ISDAIDIEN = 1;
                ...
                oBLs.CHECK_ENABLE_THULY(DONID, "5");
            }
            catch (Exception ex)
            {
        ...
        {
            ALD_SOTHAM_THULY oND = dt.ALD_SOTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            {
                case "Download":
                    var oND = dt.ALD_FILE.Where(x => x.ID == ND_id).FirstOrDefault();
                    if (oND.TENFILE != "")
                    {
                ...
                case "Sua":
                    lbthongbao.Text = "";
                    ALD_SOTHAM_THULY oND1 = dt.ALD_SOTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
                    // khong duoc SỬA khi da Tong dat - 29/11/2025 vnpt check
                    ALD_TONGDAT oTD = dt.ALD_TONGDAT.Where(x => x.DONID == oND1.DONID && x.MAPID == oND1.ID && x.MAP_TABLE == ENUM_MAP_TABLE.ALD_SOTHAM_THULY).FirstOrDefault();
                    if (oTD != null)
                    {
                ...
                case "Xoa":
                    ALD_SOTHAM_THULY oND2 = dt.ALD_SOTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
                    // khong duoc xoa khi da Tong dat - 29/11/2025 vnpt check
                    ALD_TONGDAT oTD1 = dt.ALD_TONGDAT.Where(x => x.DONID == oND2.DONID && x.MAPID == oND2.ID && x.MAP_TABLE == ENUM_MAP_TABLE.ALD_SOTHAM_THULY).FirstOrDefault();
                    if (oTD1 != null)
                    {
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                ALD_CHUYEN_NHAN_AN_BL _chuyenNhanBl = new ALD_CHUYEN_NHAN_AN_BL();
                ...
                List<ALD_SOTHAM_THULY> check_button_xoa = dt.ALD_SOTHAM_THULY.Where(x => x.DONID == DONID).ToList();
        ...
        private decimal UploadFileID(ALD_DON oDon, decimal FileID, string strMaBieumau, string STT)
        {
        ...
        protected void txtNgayThuLy_TextChanged(object sender, EventArgs e)
        {
            ...
            //    STPT_QUANLY_SOTHULY oQLSTL = new STPT_QUANLY_SOTHULY();
            //    DataTable dtStl = oQLSTL.get_STPT_QUANLY_SOTHULY(2, 5, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtNgaythuly.Text);
            //    if (dtStl != null)
            //    {
            ...
            //    string check_ngaythulycuoi_trongnam = oQLSTL.get_LATEST_DATE_IN_SOTHULY(2, 5, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtNgaythuly.Text);
        ...
        protected void btnSaveLydoXoa_Insert(object sender, EventArgs e)
        {
            ...
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            ALD_SOTHAM_THULY oND = dt.ALD_SOTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
            decimal DonID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_LAODONG] + "");
            ...
            if (Convert.ToDecimal(Regex.Match(oND.SOTHULY, @"\d+").Value) == SetNewSoThuLy() - 1)
            {
            ...
            if (check_button_xoa.Count < 2 && oND != null)
            {
                ...
                ALD_SOTHAM_BANAN ba = dt.ALD_SOTHAM_BANAN.Where(x => x.DONID == DonID).FirstOrDefault<ALD_SOTHAM_BANAN>();
                if (ba != null)
                {
                ...
                ALD_SOTHAM_QUYETDINH qd = dt.ALD_SOTHAM_QUYETDINH.Where(x => x.DONID == DonID).FirstOrDefault<ALD_SOTHAM_QUYETDINH>();
                if (qd != null)
                {
                ...
                ALD_SOTHAM_HDXX hdxx = dt.ALD_SOTHAM_HDXX.Where(x => x.DONID == DonID).FirstOrDefault<ALD_SOTHAM_HDXX>();
                if (hdxx != null)
                {
                ...
                // Kiểm tra hòa giải
                ALD_SOTHAM_HOAGIAI hg = dt.ALD_SOTHAM_HOAGIAI.Where(x => x.DONID == DonID).FirstOrDefault<ALD_SOTHAM_HOAGIAI>();
                if (hg != null)
                {
                ...
                ALD_DON_THAMPHAN gqst = dt.ALD_DON_THAMPHAN.Where(x => x.DONID == DonID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).FirstOrDefault<ALD_DON_THAMPHAN>();
                if (gqst != null)
                {
            ...
            //Xoa Thụ lý Sơ thẩm
            dt.ALD_SOTHAM_THULY.Remove(oND);
            dt.SaveChanges();
            if (FileID > 0)
            {
