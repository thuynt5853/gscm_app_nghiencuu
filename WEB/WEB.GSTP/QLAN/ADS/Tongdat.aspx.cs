...
namespace WEB.GSTP.QLAN.ADS
{
    public partial class Tongdat : System.Web.UI.Page
    {
        ...
        ADS_TONGDAT_BL TONGDAT_BL = new ADS_TONGDAT_BL();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                string current_id = Session[ENUM_LOAIAN.AN_DANSU] + "", TOAANID = "0";
        ...
        private void LoadCombobox()
        {
            ...
            //Decimal VuAnId = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_DANSU]);
            //DataTable tbl = oBL.ADS_FILE_TONGDAT(ID);
            //ADS_SOTHAM_BANAN oND = dt.ADS_SOTHAM_BANAN.Where(x => x.DONID == VuAnId && x.TOAANID == TOAANID).FirstOrDefault();
            //DM_BIEUMAU bm52 = dt.DM_BIEUMAU.Where(bm => bm.ID == 230).FirstOrDefault();
            //ADS_FILE af = dt.ADS_FILE.Where(x => x.BIEUMAUID == 230 && x.DONID == VuAnId).FirstOrDefault();
            //ADS_TONGDAT td = dt.ADS_TONGDAT.Where(x => x.DONID == VuAnId && x.BIEUMAUID == 230).FirstOrDefault();
            //if (oND != null && td == null)
            //{
            ...
            //    tbl.Rows.Add(dr);
            //}
            //ADS_PHUCTHAM_BANAN oND2 = dt.ADS_PHUCTHAM_BANAN.Where(x => x.DONID == VuAnId && x.TOAANID == TOAANID).FirstOrDefault();
            //DM_BIEUMAU bm75 = dt.DM_BIEUMAU.Where(bm => bm.ID == 261).FirstOrDefault();
            //af = dt.ADS_FILE.Where(x => x.BIEUMAUID == 261 && x.DONID == VuAnId).FirstOrDefault();
            //td = dt.ADS_TONGDAT.Where(x => x.DONID == VuAnId && x.BIEUMAUID == 261).FirstOrDefault();
            ...
            ADS_DON objDon = dt.ADS_DON.Where(x => x.ID == ID).FirstOrDefault();
            decimal giai_doan = Convert.ToDecimal(objDon.MAGIAIDOAN);
            TONGDAT_BL oTongDatBL = new TONGDAT_BL();
            DataTable oDT = oTongDatBL.GET_BM_TONGDAT(ID, 2, giai_doan);
        ...
        private List<ADS_TONGDAT_GS> convertToADS_TONGDATFromDataTable(DataTable dataTable)
        {
            List<ADS_TONGDAT_GS> aDS_TONGDATs = new List<ADS_TONGDAT_GS>();
            foreach (DataRow row in dataTable.Rows)
            {
                ADS_TONGDAT_GS tONGDAT = new ADS_TONGDAT_GS();
                ...
                aDS_TONGDATs.Add(tONGDAT);
            }
            return aDS_TONGDATs;
        }

        private List<ADS_TONGDAT_NGUOINHAN_GS> convertToADS_TONGDATDOITUONGFromDataTable(DataTable dataTable)
        {
            List<ADS_TONGDAT_NGUOINHAN_GS> aDS_TONGDATs = new List<ADS_TONGDAT_NGUOINHAN_GS>();
            foreach (DataRow row in dataTable.Rows)
            {
                ADS_TONGDAT_NGUOINHAN_GS tONGDAT = new ADS_TONGDAT_NGUOINHAN_GS();
                ...
                // VNPT Nguyễn Đăng Huy Hoàng 15:00 29/10/2025
        ...
        {
            BL.GSTP.ADS.ADS_TONGDAT_BL Bl = new BL.GSTP.ADS.ADS_TONGDAT_BL();            
            //lbthongbao.Text = "";
            ...
            if (CheckValid() == true)
            {
                ...
                decimal ANPHI_ID_DROPDOWN =0;
                if(ddlBieumau.SelectedValue.Split('_')[1]!="")
                {
                    ANPHI_ID_DROPDOWN = Convert.ToDecimal(ddlBieumau.SelectedValue.Split('_')[1]);
                }
                ...
                ADS_TONGDAT adTongDat = null;

                if (ANPHI_ID_DROPDOWN != 0)
                {
                    adTongDat = dt.ADS_TONGDAT.Where(x => x.DONID == DONID && x.BIEUMAUID == bieuMauID && x.MAPID == ANPHI_ID_DROPDOWN).FirstOrDefault();
                }
                else
                {
                    adTongDat = dt.ADS_TONGDAT.Where(x => x.DONID == DONID && x.BIEUMAUID == bieuMauID).FirstOrDefault();
                }

                ADS_TONGDAT_GS oND = null;

                if (TongDatID != 0)
                {
                    oND = convertToADS_TONGDATFromDataTable(Bl.ADS_TONGDAT_GETBYID(TongDatID)).FirstOrDefault();
                }
                
                bool isNew = false;

                bool isUpdate = false;
                ADS_TONGDAT td = new ADS_TONGDAT();
                List<ADS_TONGDAT_DOITUONG> td_dt = new List<ADS_TONGDAT_DOITUONG>();
                ...
                else
                {
                    ...
                    td = dt.ADS_TONGDAT.Where(x => x.ID == TongDatID).FirstOrDefault();
                    td_dt = dt.ADS_TONGDAT_DOITUONG.Where(x => x.TONGDATID == td.ID && x.TRANGTHAI == 1).ToList();

                    CONTENT_JSON_OLD = JsonConvert.SerializeObject(td, Formatting.Indented);
                }
                if (dM_BIEUMAU.MABM == "100-DS")
                {
                    oND.MAP_TABLE = ENUM_MAP_TABLE.ADS_ANPHI;
                ...
                if (isNew)
                {
                    ...
                    if (ddlBieumau.SelectedValue.Split('_').Length > 2)
                    {
                        ...
                        oF = dt.ADS_FILE.Where(x => x.DONID == DONID && x.BIEUMAUID == bieu_mau_id && x.ID == FILEID).FirstOrDefault();
                    }else
                    {
                        oF = dt.ADS_FILE.Where(x => x.DONID == DONID && x.BIEUMAUID == bieu_mau_id).FirstOrDefault();
                    }
                    //---------21/11/2025----  vnpt chỉnh lấy thêm mapid maptable
                    if (oF != null)
                    {
                        oND.FILEID = oF.ID;
                        var donxuly = dt.ADS_DON_XULY.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (donxuly != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.ADS_DON_XULY;
                            oND.MAPID = donxuly.ID;
                        }

                        var donxulydct =
                                        (from dx in dt.ADS_DON_XULY
                        ...
                        var anphi = dt.ADS_ANPHI.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);
                        ...
                        var sothamthuly = dt.ADS_SOTHAM_THULY.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (sothamthuly != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.ADS_SOTHAM_THULY;
                            oND.MAPID = sothamthuly.ID;
                        }
                        
                        var sothamquyetdinh = dt.ADS_SOTHAM_QUYETDINH.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (sothamquyetdinh != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.ADS_SOTHAM_QUYETDINH;
                            oND.MAPID = sothamquyetdinh.ID;
                        }
                        
                        var phucthamthuly = dt.ADS_PHUCTHAM_THULY.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (phucthamthuly != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.ADS_PHUCTHAM_THULY;
                            oND.MAPID = phucthamthuly.ID;
                        }
                        
                        var phucthamquyetdinh = dt.ADS_PHUCTHAM_QUYETDINH.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (phucthamquyetdinh != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.ADS_PHUCTHAM_QUYETDINH;
                            oND.MAPID = phucthamquyetdinh.ID;
                        }

                        var banansotham = dt.ADS_SOTHAM_BANAN.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (banansotham != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.ADS_SOTHAM_BANAN;
                            oND.MAPID = banansotham.ID;
                        }
                        
                        var bananphuctham = dt.ADS_PHUCTHAM_BANAN.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (bananphuctham != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.ADS_PHUCTHAM_BANAN;
                    ...
                    oND.ID = Bl.ADS_TONGDAT_UPIN(oND);
                    ...
                    log.InsertLog("ADS_TONGDAT", oND.ID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Insert", "", CONTENT_JSON, "");
                }
                else
                {
                ...
                if (!isNew)
                {
                    ...
                    CONTENT_JSON = JsonConvert.SerializeObject(oND, Formatting.Indented);
                    LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                    log.InsertLog("ADS_TONGDAT", oND.ID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Update", lbthongbao.Text, CONTENT_JSON, CONTENT_JSON_OLD);
                }
                if (trDuongsu.Visible)
                {
                    decimal TongdatID = oND.ID;
                    List<decimal> IDs = new List<decimal>();
                    foreach (DataRow row in TONGDAT_BL.ADS_TONGDATDOITUONG_GETBYTONGDATID(TongDatID).Rows)
                    {
                    ...
                    foreach (DataGridItem Item in dgTructiep.Items)
                    {
                        ...
                        decimal? ID;
                        List<ADS_TONGDAT_NGUOINHAN_GS> lstCheck;
                        try
                        {
                            if (Item.Cells[7].Text == "&nbsp;")
                            {
                                ID = null;
                                lstCheck = new List<ADS_TONGDAT_NGUOINHAN_GS>();
                            }
                            else
                            {
                                ID = Convert.ToDecimal(Item.Cells[7].Text);
                                lstCheck = convertToADS_TONGDATDOITUONGFromDataTable(Bl.ADS_TONGDATDOITUONG_GETBYID(ID.Value));
                            }
                        }
                        catch
                        {
                        ...
                        ADS_TONGDAT_NGUOINHAN_GS oTD = null;
                        if (lstCheck.Count > 0)
                        {
                            oTD = lstCheck[0];
                        ...
                        {
                            oTD = new ADS_TONGDAT_NGUOINHAN_GS();
                        }
                        ...
                        if (chkIsSend.Checked)
                        {
                            ...
                            Bl.ADS_TONGDATDOITUONG_UPIN(oTD);
                            dt.SaveChanges();
                            ...
                            if (lstCheck.Count > 0)
                            {
                                ...
                                LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                                log.InsertLog("ADS_TONGDAT_DOITUONG", TongDatID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Update", lbthongbao.Text, CONTENT_JSON, CONTENT_JSON_OLD);
                            }
                            else
                            {
                                //AnhPN log
                                CONTENT_JSON = JsonConvert.SerializeObject(oTD, Formatting.Indented);
                                LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                                log.InsertLog("ADS_TONGDAT_DOITUONG", oTD.TONGDATID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Insert", lbthongbao.Text, CONTENT_JSON, "");
                            }
                            ...
                            {
                                CONTENT_JSON = JsonConvert.SerializeObject(oTD, Formatting.Indented);
                                obj.DVCQG_THANH_TOAN_UP_TONGDAT(oND.ID, (decimal)DuongsuID, "2");
                    ...
                    if (IDs.Count > 0)
                    {
                        foreach (decimal i in IDs)
                            TONGDAT_BL.ADS_TONGDATDOITUONG_REMOVEBYID(i);
                    }
                }
                if (trTructuyen.Visible)
                {
                    ...
                    foreach (DataGridItem Item in dgTructuyen.Items)
                    {
                        ...
                        if (chkIsSendTT.Checked)
                        {
                            ...
                            if (lstCheck.Count == 0)
                            {
                                if (oTD.TOA_GIAIQUYET_ID == null) oTD.TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                                dt.ADS_TONGDAT_DOITUONG.Add(oTD);
                            }
        ...
        {
            ADS_TONGDAT td = dt.ADS_TONGDAT.Where(x => x.DONID == DonID && x.TOAANID == ToaAnID && x.BIEUMAUID == BieuMauID && (MAPID == 0 || x.MAPID == MAPID)).FirstOrDefault();
            List<ADS_TONGDAT_DOITUONG> td_dt = dt.ADS_TONGDAT_DOITUONG.Where(x => x.TONGDATID == td.ID).ToList();
            foreach (var item in td_dt)
            {
                ADS_TONGDAT_NGUOINHAN_GS oTD = new ADS_TONGDAT_NGUOINHAN_GS();
        ...
        private void ReUpdateStatusUpdate(List<ADS_TONGDAT_DOITUONG> td_dt, decimal TongDatId)
        {
            ...
            List<ADS_TONGDAT_DOITUONG> list_detail = dt.ADS_TONGDAT_DOITUONG.Where(x => x.TONGDATID == TongDatId).ToList();
        ...
        {
            ADS_DON_BL oBL = new ADS_DON_BL();
            BL.GSTP.ADS.ADS_TONGDAT_BL TongDatBL = new BL.GSTP.ADS.ADS_TONGDAT_BL();
            string current_id = Session[ENUM_LOAIAN.AN_DANSU] + "";
            decimal ID = Convert.ToDecimal(current_id), ToaAnID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);

            DataTable oDT = oBL.ADS_TONGDAT_GETLIST(ID, ToaAnID);
            var lstTongDat = convertToADS_TONGDATFromDataTable(oDT);
            List<ADS_TONGDAT_NGUOINHAN_GS> lstNguoiNhan = new List<ADS_TONGDAT_NGUOINHAN_GS>();
            List<ADS_TONGDAT_GS> lstHaventDoituongtongdat = new List<ADS_TONGDAT_GS>();
            foreach (var tongdat in lstTongDat)
            {
                DataTable doiTuongDataTable = TongDatBL.ADS_TONGDATDOITUONG_GETBYTONGDATID(tongdat.ID);
                if (doiTuongDataTable.Rows.Count == 0)
                {
                    lstHaventDoituongtongdat.Add(tongdat);
                }
                lstNguoiNhan.AddRange(convertToADS_TONGDATDOITUONGFromDataTable(doiTuongDataTable));
            }
            if (oDT != null && lstNguoiNhan.Count == 0)
            {
                if (lstNguoiNhan.Count == 0)
                {
                    ...
                    obj.TONGDAT_DELETE_ERROR(ID.ToString(), "2");
                }
            }
        ...
        public void xoa(decimal id)
        {
            ...
            List<ADS_TONGDAT_DOITUONG> lst = dt.ADS_TONGDAT_DOITUONG.Where(x => x.TONGDATID == id && (x.TRANGTHAI == 0 || x.TRANGTHAI == 2 || ((x.TRANGTHAI == 5 || x.TRANGTHAI == 6) && x.NGAYPHATHANH == null))).ToList();
            if (lst.Count > 0)
            {
                //AnhPN log
                var CONTENT_JSON_OLD = JsonConvert.SerializeObject(lst, Formatting.Indented);
                LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                log.InsertLog("ADS_TONGDAT_DOITUONG", id.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Delete", "", "", CONTENT_JSON_OLD);

                dt.ADS_TONGDAT_DOITUONG.RemoveRange(lst);
                dt.SaveChanges();
            }
            //--
            List<ADS_TONGDAT_DOITUONG> lst_obj = dt.ADS_TONGDAT_DOITUONG.Where(x => x.TONGDATID == id).ToList();
            ...
            if (oND != null)
            {
                if (lst_obj.Count == 0)
                {
                    ...
                    {
                        //AnhPN log
                        var CONTENT_JSON_OLD = JsonConvert.SerializeObject(oND, Formatting.Indented);
                        LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                        log.InsertLog("ADS_TONGDAT", id.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Delete", "", "", CONTENT_JSON_OLD);
        ...
        public void loadSua(decimal ID)
        {
            ...
            trThemFile.Visible = false;//28/11/2024
            ...
            if (oBM != null)
            {
                ...
                {
                    ADS_ANPHI aDS_ANPHI = dt.ADS_ANPHI.Where(x => x.ID == MapID).FirstOrDefault();
                    ...
                    if (aDS_ANPHI != null)
                    {
                        DataTable DON_MIENANPHI = dON_MIENANPHI.GET_DON_MIENANPHI_BY_ANPHI_ID(aDS_ANPHI.ID, 2);
            ...
            {
                nguoiNhans = TONGDATBL.ADS_TONGDATDOITUONG_ANPHI_GETBYTONGDATID(ID);
            } else
            {
                nguoiNhans = TONGDATBL.ADS_TONGDATDOITUONG_GETBYTONGDATID(ID);
            }
            ...
            {
                string name = TONGDATBL.ADS_TONGDATDOITUONG_GETTENDUONGSU(Convert.ToDecimal(row["ID"]));
        ...
        public void loadPhatHanhLai(decimal ID, decimal vID)
        {
            ...
            cmdLammoi.Visible = true;
            BL.GSTP.ADS.ADS_TONGDAT_BL TONGDATBL = new BL.GSTP.ADS.ADS_TONGDAT_BL();
            DataTable nguoiNhans = TONGDATBL.ADS_TONGDATDOITUONG_GETBYTONGDATID(ID);
            DataTable nguoiNhan = TONGDATBL.ADS_TONGDATDOITUONG_GETBYID(vID);
            ...
            string nguoiNhanName = TONGDATBL.ADS_TONGDATDOITUONG_GETTENDUONGSU(Convert.ToDecimal(nguoiNhanRow["ID"]));
        ...
        // VNPT 24/06/2025 comment do không sử dụng
        ...
        protected void loadAction(string nDID)
        {
            ...
            //    //Call package
            //    BL.GSTP.ADS.ADS_TONGDAT_BL tongDatBL = new BL.GSTP.ADS.ADS_TONGDAT_BL();
            //    tongDatBL.ADS_TONGDAT_THUHOI_DOITUONG(ID, TongdatID, LyDoThuHoi, NgayThuHoi, NguoiSua);
            ...
            if (TrangThai == 2) // Xem lý do thu hồi
            {
                ...
                DataTable objLyDo = oBL.GET_LYDO_THUHOI(2, TongdatID);
                if (objLyDo != null)
                {
            ...
            {
                BL.GSTP.ADS.ADS_TONGDAT_BL tongDatBL = new BL.GSTP.ADS.ADS_TONGDAT_BL();
                tongDatBL.ADS_TONGDAT_VBDH_DOITUONG(ID);
                LoadGrid();
            }
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "Download":
                    ...
                    ADS_TONGDAT ads = dt.ADS_TONGDAT.Where(x => x.ID == tongDatId).FirstOrDefault();
                    byte[] conten=null;
                    if (ads != null)
                    {
                        ...
                        {
                            ADS_FILE oF = dt.ADS_FILE.Where(x => x.ID == ads.FILEID).FirstOrDefault();
                            if (oF != null)
                            {
                                if (oF.TENFILE != null)
                                {
                ...
                case "UpdateNgayNhan":
                    ...
                    if (txtNGAYNHANTONGDAT.Text != "")
                    {
                        ...
                        if (obj.TONGDAT_DOITUONG_VNID(Id_obj, "2", txtNGAYNHANTONGDAT.Text) == true)
                        {
                    ...
                    else if (txtNGAYNHANTONGDAT.Text == "")
                    {
                        ...
                        if (obj.TONGDAT_DOITUONG_VNID(Id_obj, "2", null) == true)
                        {
                            strMsg = "Bạn đã cập nhật thành công";
                            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('" + strMsg + "')", true);
                            LoadGrid();
        ...
        private void LoadDoituong(bool IsShowVKS, decimal BieuMauID)
        {
            ...
            if (IS_TD_DUONGSU == 1)
            {
                if (oBM.MABM == "100-DS")
                {
                    oDTDS = TONGDAT_BL.ADS_TONGDATDOITUONG_ANPHI_GETBY(DONID, TOAANID, BMID, 0, ANPHI_ID);
                }
                else
                {
                    //---------21/11/2025----  vnpt chỉnh lấy thêm lọc FILEID
                    oDTDS = TONGDAT_BL.ADS_TONGDATDOITUONG_GETBYFILEID(DONID, TOAANID, BMID, 0, FILEID);
                }
            ...
            else if (IS_TD_NGUYENDON == 1 && IS_TD_DUONGSU != 1)
            {
                if (oBM.MABM == "100-DS")
                {
                    oDTDS = TONGDAT_BL.ADS_TONGDATDOITUONG_ANPHI_GETBY(DONID, TOAANID, BMID, 1, ANPHI_ID);
                }
                else
                {
                    //---------21/11/2025----  vnpt chỉnh lấy thêm lọc checkVNEID
                    oDTDS = TONGDAT_BL.ADS_TONGDATDOITUONG_GETBYFILEID(DONID, TOAANID, BMID, 1, FILEID);
                }
        ...
        {
            BL.GSTP.ADS.ADS_TONGDAT_BL aDS = new BL.GSTP.ADS.ADS_TONGDAT_BL();
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                string TenBM = aDS.ADS_TENBM_GETBYTONGDATID(tongdatID);
                Label lbTenBM = (Label)e.Item.FindControl("lbTenBM");
                lbTenBM.Text = TenBM;
                lbNoiNhan.Text = aDS.ADS_TONGDATDOITUONG_GETTENDUONGSU(id);
                if (duongsu_id != null)
                {
                }
                ...
                BL.GSTP.ADS.ADS_TONGDAT_BL Bl = new BL.GSTP.ADS.ADS_TONGDAT_BL();
                ADS_TONGDAT_GS oND = convertToADS_TONGDATFromDataTable(Bl.ADS_TONGDAT_GETBYID(tongdatID)).FirstOrDefault();
                ...
                int rowspan = aDS.ADS_TONGDATDOITUONG_GETBYTONGDATID(tongdatID).Rows.Count;
        ...
        protected void dgTructiep_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            BL.GSTP.ADS.ADS_TONGDAT_BL _BL = new BL.GSTP.ADS.ADS_TONGDAT_BL();
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                DataRowView rowView = (DataRowView)e.Item.DataItem;
               ...
               // vHINHTHUCGUI = ((DropDownList)e.Item.FindControl("ddlHinhthuc")).SelectedValue;
                ...
                if (ID == null) // nếu là hàng theem 
                {
                    ...
                    var atddtisSENT = dt.ADS_TONGDAT_DOITUONG.Where(x => x.TONGDATID == tdId).ToList();
                    if (atddtisSENT.Count > 0)
                    {
                        ...
                        dt.ADS_DON_DUONGSU.Where(x => x.DONID == DONID && !tddt.Contains(x.ID)).ToList()
        ...
        protected void btn_insert_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (oBM.MABM == "100-DS") // 100-DS. Thông báo về việc miễn tạm ứng án phí  
                {
                    nguoiNhans = TONGDATBL.ADS_TONGDATDOITUONG_ANPHI_GETBYTONGDATID(TongDatID);
                }
                else
                {
                    nguoiNhans = TONGDATBL.ADS_TONGDATDOITUONG_GETBYTONGDATID(TongDatID);
                }
        ...
        protected void btn_remove_Click(object sender, EventArgs e)
        {
            if (cmdLammoi.Visible == false)
            {
                ...
                //ADS_FILE oF = dt.ADS_FILE.Where(x => x.DONID == DONID && x.BIEUMAUID == bieu_mau_id).FirstOrDefault();
                //if (oF != null) BMID = oF.BIEUMAUID + "" == "" ? 0 : (decimal)oF.BIEUMAUID;
                DM_BIEUMAU oBM = dt.DM_BIEUMAU.Where(x => x.ID == bieu_mau_id).FirstOrDefault();
                if (oBM != null)
                {
            ...
            foreach (DataGridItem item in dgTructiep.Items)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        // vnpt 03/12/2025
                        //ADS_DON_THAMGIATOTUNG tgtt = dt.ADS_DON_THAMGIATOTUNG.FirstOrDefault(x => x.ID == nguoiID);
                        ADS_DON_DUONGSU tgtt = dt.ADS_DON_DUONGSU.FirstOrDefault(x => x.ID == nguoiID);
                        if (tgtt != null)
                        {
                            string diaChi = "";
        ...
        {
            BL.GSTP.ADS.ADS_TONGDAT_BL tONGDAT_BL = new BL.GSTP.ADS.ADS_TONGDAT_BL();
            ...
            try
            {
                ...
                // Call API
                string result = CallApiThuhoi("2",tongDatIds, LyDoThuHoi, NgayThuHoi, NguoiSua);
                JToken jObject = JToken.Parse(result);
                if ((string)jObject["status"] == "SUCCESS")
                {
                    lbThongBaoThuHoi.Text = "Thu hồi thành công !";
                    ...
                    foreach (DataGridItem item in dgList.Items)
                    {
                        ...
                        if (cb_thuhoi.Checked == true)
                        {
                            //AnhPN log
                            LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                            var data = DataExtensions.FindById<ADS_TONGDAT>(v_id);
                            var CONTENT_JSON_OLD = JsonConvert.SerializeObject(data, Formatting.Indented);                            
                            log.InsertLog("ADS_TONGDAT", v_id.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Thu hồi", LyDoThuHoi, "", CONTENT_JSON_OLD);

                            //AnhPN log
                            List<ADS_TONGDAT_DOITUONG> lst = dt.ADS_TONGDAT_DOITUONG.Where(x => x.TONGDATID == v_id).ToList();
                            CONTENT_JSON_OLD = JsonConvert.SerializeObject(lst.FirstOrDefault(), Formatting.Indented);
                            log.InsertLog("ADS_TONGDAT_DOITUONG", v_id.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Thu hồi", LyDoThuHoi, "", CONTENT_JSON_OLD);

                            tONGDAT_BL.ADS_TONGDAT_THUHOI(v_id, NgayThuHoi, LyDoThuHoi, NguoiSua);
                        }
                    }
                }
                else
        ...
        protected void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridItem item in dgList.Items)
            {
                ...
                String[] ND_id_arr = Hi_column_value.Value.Split(';');
                string V_NGAYGUI = ND_id_arr[0] + "";
                string V_NGAYPHATHANH = ND_id_arr[1] + "";//13/01/2025
                ...
                if (curTongDatID == TongDatID)
                {
                    ...
                    if ((TrangThai == 5 || TrangThai == 6) && V_NGAYPHATHANH == "")//13/01/2025
                    {
                        canDelete = true;
                    }
                }
        ...
        public List<TongDatDTO> GetAnDanSu(List<decimal> ids)
        {
            List<ADS_TONGDAT> adsTongDats = dt.ADS_TONGDAT.Where(x => ids.Contains(x.ID) && (x.TRANGTHAI >= 0 && x.TRANGTHAI <= 2)).ToList();
            ...
            List<ADS_DON> allADSDon = dt.ADS_DON.Where(x => idADS_DON.Contains(x.ID)).ToList();
            Dictionary<decimal?, ADS_DON> mapAdsDon = new Dictionary<decimal?, ADS_DON>();
            Dictionary<decimal?, List<ADS_DON_DUONGSU>> mapDuongSu = dt.ADS_DON_DUONGSU
                .Where(x => idADS_DON.Contains(x.DONID))
                .GroupBy(x => x.DONID, x => x)
                .ToDictionary(x => x.Key, x => x.ToList());
            // sonnv: bo xung thong tin van ban di
            foreach (ADS_DON adsDon in allADSDon)
            {
            ...
            string conditions = "(";
            foreach (ADS_TONGDAT tongDat in adsTongDats)
            {
            ...
            List<ADS_TONGDAT_DOITUONG> allDoiTuongs = dt.ADS_TONGDAT_DOITUONG
            ...
            Dictionary<decimal, ADS_DON_DUONGSU> donDuongSu = dt.ADS_DON_DUONGSU
            ...
            foreach (TongDatDTO dto in danhSachTongDat)
            {
                List<ADS_TONGDAT_DOITUONG> subList = allDoiTuongs.Where(x => dto.ID.Equals(x.TONGDATID) && x.TRANGTHAI == 1).ToList();
        ...
        public List<ADS_TONGDAT> GetAnDanSuThuHoi(List<decimal> ids)
        {
            List<ADS_TONGDAT> adsTongDats = dt.ADS_TONGDAT.Where(x => ids.Contains(x.ID)).ToList();
            return adsTongDats;
        }

        private TongDatDTO ToDTO<T>(T model, Dictionary<decimal?, DM_BIEUMAU> mapBieuMau, Dictionary<string, decimal?> mapUser, Dictionary<decimal?, ADS_DON> mapADS_DON, Dictionary<decimal?, List<ADS_DON_DUONGSU>> mapDuongSu)
        {
            if (model == null) return null;

            // mapping án dân sự
            if (model.GetType() == typeof(ADS_TONGDAT))
            {
                ADS_TONGDAT entity = model as ADS_TONGDAT;
                ...
                {
                    List<ADS_DON_XULY> aDS_DON_XULies = dt.ADS_DON_XULY.Where(x => x.DONID == entity.DONID).ToList();
                    ...
                    decimal? canboid = dt.ADS_DON_THAMPHAN.Where(x => x.DONID == entity.DONID && x.MAVAITRO == "VTTP_GIAIQUYETDON").Select(x => x.CANBOID).Take(1).toNumber();
                    dto.Nguoiky = canboid;
                }
                else if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "100-DS") // 100-DS. Thông báo về việc miễn tạm ứng án phí 
                {
                    ADS_ANPHI aDS_ANPHI = dt.ADS_ANPHI.Where(x => x.ID == entity.MAPID).FirstOrDefault();
                    DON_MIENANPHI_BL dON_MIENANPHI = new DON_MIENANPHI_BL();
                    ...
                    if (aDS_ANPHI != null)
                    {
                        ...
                        //dto.mathongbao = dVCQG_THANH_TOAN.DVCQG_THANH_TOAN_SEARCH(2, entity.DONID.Value, aDS_ANPHI.ID, "2");
                    }

                }
                else if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "30-DS")
                {
                    List<ADS_SOTHAM_THULY> aDS_DON_XULies = dt.ADS_SOTHAM_THULY.Where(x => x.DONID == entity.DONID).ToList();
                    if (aDS_DON_XULies != null && aDS_DON_XULies.Any())
                    {
                        dto.Sothongbao = Convert.ToString(aDS_DON_XULies[0].SOTHONGBAO);
                        dto.Ngaythongbao = Convert.ToDateTime(aDS_DON_XULies[0].NGAYTHONGBAO);
                ...
                {
                    List<ADS_PHUCTHAM_THULY> aDS_DON_XULies = dt.ADS_PHUCTHAM_THULY.Where(x => x.DONID == entity.DONID).ToList();
                    if (aDS_DON_XULies != null && aDS_DON_XULies.Any())
                    {
                        dto.Sothongbao = Convert.ToString(aDS_DON_XULies[0].SOTHONGBAO);
                        dto.Ngaythongbao = Convert.ToDateTime(aDS_DON_XULies[0].NGAYTHONGBAO);
                ...
                {
                    ADS_SOTHAM_BANAN ba = dt.ADS_SOTHAM_BANAN.Where(x => x.DONID == entity.DONID).FirstOrDefault();
                    if (ba != null)
                    {
                        ...
                        ADS_SOTHAM_HDXX oHD = dt.ADS_SOTHAM_HDXX.Where(x => x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN && x.DONID == entity.DONID).OrderByDescending(x => x.NGAYPHANCONG).FirstOrDefault();
                        if (oHD != null)
                        {
                ...
                {
                    ADS_PHUCTHAM_BANAN ba = dt.ADS_PHUCTHAM_BANAN.Where(x => x.DONID == entity.DONID).FirstOrDefault();
                    if (ba != null)
                    {
                        dto.Sothongbao = Convert.ToString(ba.SOBANAN);
                        dto.Ngaythongbao = Convert.ToDateTime(ba.NGAYTUYENAN);
                        //load người kí - mặc định chủ tọa
                        ADS_PHUCTHAM_HDXX oHD = dt.ADS_PHUCTHAM_HDXX.Where(x => x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN && x.DONID == entity.DONID).OrderByDescending(x => x.NGAYPHANCONG).FirstOrDefault();
                        if (oHD != null)
                        {
                            DM_CANBO oTPCT = dt.DM_CANBO.Where(x => x.ID == oHD.CANBOID).FirstOrDefault();
                            if (oTPCT != null)
                ...
                // VNPT Nguyễn Đăng Huy Hoàng 16:42:00 14/11/2025
                // tống đạt BM màn màn quyết định vụ việc Sơ thẩm Dân sự
                else if (dM_BIEUMAU != null && ENUM_PHATHANH_BIEUMAU_DS.IsValid_QDVUVIEC_ST_ADS(dM_BIEUMAU.MABM)) // biểu mẫu tống đạt màn quyết định vụ việc Sơ thẩm Dân sự
                {
                    List<ADS_SOTHAM_QUYETDINH> adsSothamQD = dt.ADS_SOTHAM_QUYETDINH.Where(x => x.DONID == entity.DONID).ToList();
                    if (adsSothamQD != null && adsSothamQD.Any())
                    {
                        dto.Sothongbao = Convert.ToString(adsSothamQD[0].SOQD);
                        dto.Ngaythongbao = Convert.ToDateTime(adsSothamQD[0].NGAYQD);
                        dto.Nguoiky = adsSothamQD[0].NGUOIKYID;
                    }
                }
                // VNPT Nguyễn Đăng Huy Hoàng 13:22:00 17/11/2025
                // tống đạt BM màn màn quyết định vụ việc phúc thẩm Dân sự
                else if (dM_BIEUMAU != null && ENUM_PHATHANH_BIEUMAU_DS.IsValid_QDVUVIEC_PT_ADS(dM_BIEUMAU.MABM)) // biểu mẫu tống đạt màn quyết định vụ việc Sơ thẩm Dân sự
                {
                    List<ADS_PHUCTHAM_QUYETDINH> adsPhucthamQD = dt.ADS_PHUCTHAM_QUYETDINH.Where(x => x.DONID == entity.DONID).ToList();
                    if (adsPhucthamQD != null && adsPhucthamQD.Any())
                    {
                        dto.Sothongbao = Convert.ToString(adsPhucthamQD[0].SOQD);
                        dto.Ngaythongbao = Convert.ToDateTime(adsPhucthamQD[0].NGAYQD);
                        dto.Nguoiky = adsPhucthamQD[0].NGUOIKYID;
                    }
                }
                else
                {
                    ...
                    if (aDS_FILEs != null && aDS_FILEs.Any())
                    {
                        ...
                        {
                            List<ADS_SOTHAM_QUYETDINH> aDS_SOTHAM_QUYETDINHs = dt.ADS_SOTHAM_QUYETDINH.Where(x => x.QUYETDINHID == dM_BIEUMAU.QUYETDINHID && x.DONID == entity.DONID).ToList();
                        ...
                        {
                            List<ADS_PHUCTHAM_QUYETDINH> aDS_PHUCTHAM_QUYETDINHs = dt.ADS_PHUCTHAM_QUYETDINH.Where(x => x.QUYETDINHID == dM_BIEUMAU.QUYETDINHID && x.DONID == entity.DONID).ToList();
        ...
        protected string CallApi(List<decimal> ids)
        {
            ...
            //string apiUrl = "http://daotao-vbdh.toaan.gov.vn:8082/rest/dataMigration/tong-dat";
            string apiUrl = ConfigurationManager.AppSettings["IpTongDat"] + "tong-dat ";
            var input = GetAnDanSu(ids);
        ...
        {
            WebClient client = new WebClient();
            //string apiUrl = "http://daotao-vbdh.toaan.gov.vn:8082/rest/dataMigration/tong-dat-thu-hoi";
            ...
            else
            {
                try
                {
                    ...
                    if (status == "SUCCESS")
                    {
                        ...
                        //AnhPN log
                        LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                        log.InsertLog("CallApi_ADS", TongDatID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Gửi sang VBĐH", "Gửi thành công", "", "");
                    }
                    else
                    {
                        ...
                        //AnhPN log
                        LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                        log.InsertLog("CallApi_ADS", TongDatID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Gửi sang VBĐH", "Gửi thất bại", "", "");
                    }
                }
                catch (Exception ex)
                {
        ...
        protected void ddlTCTT_SelectedIndexChanged(object sender, EventArgs e)
        {
            foreach (DataGridItem item in dgTructiep.Items)
            {
                ...
                if (oT.MAGIAIDOAN == 2)
                {
                    ...
                    {
                        dt.ADS_DON_THAMGIATOTUNG.Where(x => x.DONID == DONID && x.TUCACHTGTTID == ddlTCTT.SelectedValue)
                                                .ToList()
                                                .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.HOTEN, x.TUCACHTGTTID + "," + x.ID)));
                    }
                }
                else if (oT.MAGIAIDOAN == 3)
                {
                    ...
                    {
                        dt.ADS_PHUCTHAM_THAMGIATOTUNG.Where(x => x.DONID == DONID && x.TUCACHTGTTID == ddlTCTT.SelectedValue)
                                                    .ToList()
                                                    .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.HOTEN, x.TUCACHTGTTID + "," + x.ID)));
                    }
                }
        ...
        private string GetSoThongBao(decimal bieuMauID, decimal donID, decimal duongsuID)
        {
            ...
            {
                ADS_ANPHI anPhi = dt.ADS_ANPHI.Where(x => x.DONID == donID && x.DUONGSU_ID == duongsuID).FirstOrDefault();
                if (anPhi != null)
                {
            ...
            else if (bieu_mau.MABM == "100-DS") // 100-DS. Thông báo về việc miễn tạm ứng án phí 
            {
                ...
                DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(anPhi.ID, 2);
                if (anPhi != null)
                {
            ...
            {
                ADS_SOTHAM_THULY thuly = dt.ADS_SOTHAM_THULY.Where(x => x.DONID == donID).FirstOrDefault();
                if (thuly != null)
                {
            ...
            {
                ADS_PHUCTHAM_BANAN ba = dt.ADS_PHUCTHAM_BANAN.Where(x => x.DONID == donID).FirstOrDefault();
                if (ba != null)
                {
                    STB = ba.SOBANAN;
                }
            ...
            // VNPT Nguyễn Đăng Huy Hoàng 16:42:00 14/11/2025
            // tống đạt BM màn màn quyết định vụ việc Sơ thẩm Dân sự
            else if (bieu_mau != null && ENUM_PHATHANH_BIEUMAU_DS.IsValid_QDVUVIEC_ST_ADS(bieu_mau.MABM)) // biểu mẫu tống đạt màn quyết định vụ việc Sơ thẩm Dân sự
            {
                List<ADS_SOTHAM_QUYETDINH> adsSothamQD = dt.ADS_SOTHAM_QUYETDINH.Where(x => x.DONID == donID).ToList();
                if (adsSothamQD != null && adsSothamQD.Any())
                {
                    STB = Convert.ToString(adsSothamQD[0].SOQD);
                }
            }
            // VNPT Nguyễn Đăng Huy Hoàng 13:22:00 17/11/2025
            // tống đạt BM màn màn quyết định vụ việc phúc thẩm Dân sự
            else if (bieu_mau != null && ENUM_PHATHANH_BIEUMAU_DS.IsValid_QDVUVIEC_PT_ADS(bieu_mau.MABM)) // biểu mẫu tống đạt màn quyết định vụ việc Sơ thẩm Dân sự
            {
                List<ADS_PHUCTHAM_QUYETDINH> adsPhucThamQD = dt.ADS_PHUCTHAM_QUYETDINH.Where(x => x.DONID == donID).ToList();
                if (adsPhucThamQD != null && adsPhucThamQD.Any())
                {
                    STB = Convert.ToString(adsPhucThamQD[0].SOQD);
                }
            }
            else if (bieu_mau.QUYETDINHID != null) // Các biểu mẫu quyết định vụ việc
            {
                ...
                {
                    ADS_SOTHAM_QUYETDINH qdinh = dt.ADS_SOTHAM_QUYETDINH.Where(x => x.DONID == donID).FirstOrDefault();
                    if (qdinh != null)
                    {
                        STB = qdinh.SOQD;
                    }
                ...
                {
                    ADS_PHUCTHAM_QUYETDINH qdinh = dt.ADS_PHUCTHAM_QUYETDINH.Where(x => x.DONID == donID).FirstOrDefault();
                    if (qdinh != null)
                    {
                        STB = qdinh.SOQD;
                    }
