...
namespace WEB.GSTP.QLAN.ADS.Thamphan
{
    public partial class GiaiquyetPhucthamKCKNQDK : System.Web.UI.Page
    {
        ...
        void CheckQuyen(decimal DONID)
        {
            ...
            DataTable oDT = oBL.ADS_PHUCTHAM_KCKN_TGTT_GETLIST(DONID);
            ...
            ADS_PHUCTHAM_BL BL = new ADS_PHUCTHAM_BL();
            DataTable DT = BL.ADS_KCKNQDK_PHUCTHAM_HDXX_GETLIST(DONID);
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                ADS_DON oT = dt.ADS_DON.Where(x => x.ID == DONID).FirstOrDefault();
                if (oT.MAGIAIDOAN == ENUM_GIAIDOANVUAN.THULYGDT || oT.MAGIAIDOAN == ENUM_GIAIDOANVUAN.SOTHAM && oT.TOA_GIAIQUYET_ID == donvi_ID)
                {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    oND = dt.ADS_DON_THAMPHAN.Where(x => x.DONID == DONID && x.CANBOID == CanBoID && x.MAVAITRO == MaVaiTro && x.TOA_GIAIQUYET_ID == currenttoaid).FirstOrDefault();
                    if (oND != null)
                    {
                ...
                //tamnc insert người phân công vào người tiến hành TT 
                //ADS_KCKNQDK_PHUCTHAM_HDXX oTHTT = dt.ADS_KCKNQDK_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).FirstOrDefault();
                ADS_KCKNQDK_PHUCTHAM_HDXX oTHTT = DataExtensions.GetAllWithClause<ADS_KCKNQDK_PHUCTHAM_HDXX>($"DONID = {DONID} AND MAVAITRO = '{ENUM_NGUOITIENHANHTOTUNG.THAMPHAN}' AND TOA_GIAIQUYET_ID ={Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID])}" ).FirstOrDefault();
                if (oTHTT == null)
                {
                    oTHTT = new ADS_KCKNQDK_PHUCTHAM_HDXX();
                    oTHTT.DONID = DONID;
        ...
        public void LoadGrid()
        {
            ...
            string current_id = Session[ENUM_LOAIAN.AN_DANSU] + "";
            decimal ID = Convert.ToDecimal(current_id);
            DataTable oDT = oBL.ADS_KCKN_DON_THAMPHAN_GETBY(ID, ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM);
        ...
        public void xoa(decimal id)
        {
            ...
            if (oND != null)
            {
                ...
                if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.DONID), 2, Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Phân công thẩm phán giải quyết Phúc thẩm kc/kn QĐ TDC và QĐ khác Dân sự", "Xóa", json) == false)
                {
        ...
        private bool CheckQDofThamPhan(decimal DonID,decimal TPID)
        {
            ...
            //ADS_KCKNQDK_PHUCTHAM_QUYETDINH obj_QD = dt.ADS_KCKNQDK_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.NGUOIKYID == TPID).FirstOrDefault<ADS_KCKNQDK_PHUCTHAM_QUYETDINH>();
            ADS_KCKNQDK_PHUCTHAM_QUYETDINH obj_QD = DataExtensions.GetAllWithClause<ADS_KCKNQDK_PHUCTHAM_QUYETDINH>($"DONID = {DonID} AND NGUOIKYID = {TPID}").FirstOrDefault();
            if (obj_QD != null)
            {
