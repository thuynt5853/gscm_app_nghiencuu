...
namespace WEB.GSTP.QLAN.ADS.Thamphan
{
    public partial class GiaiquyetSotham : System.Web.UI.Page
    {
        ...
        private void CheckQuyen(decimal DONID)
        {
            ...
            List<ADS_SOTHAM_THULY> lstCount = dt.ADS_SOTHAM_THULY.Where(x => x.DONID == DONID).ToList();
            if (lstCount.Count == 0)
            {
            ...
            ADS_SOTHAM_BANAN oBA = dt.ADS_SOTHAM_BANAN.Where(x => x.DONID == DONID).FirstOrDefault();
            if (oBA != null)
            {
            ...
            string Result = new ADS_CHUYEN_NHAN_AN_BL().Check_NhanAn(DONID, StrMsg, toaAnId);
            if (Result != "")
            {
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (current_id == "") Response.Redirect(Cls_Comon.GetRootURL() + "/QLAN/ADS/Hoso/Danhsach.aspx");
            decimal DONID = Convert.ToDecimal(current_id);
            //var qdCount = dt.ADS_SOTHAM_QUYETDINH.Where(x => x.DONID == DONID).Count();
            //var bdCount = dt.ADS_SOTHAM_BANAN.Where(x => x.DONID == DONID).Count();
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                ADS_DON_THAMPHAN obj = dt.ADS_DON_THAMPHAN.Where(x => x.ID == ID).FirstOrDefault();
                if (obj != null)
                {

                    ADS_CHUYEN_NHAN_AN chuyenan = dt.ADS_CHUYEN_NHAN_AN.Where(x => x.VUANID == DONID).OrderByDescending(x => x.ID).FirstOrDefault();
                    if (chuyenan != null)
                    {
                        if (obj.NGAYTAO <= chuyenan.NGAYTAO)
                        {
                            List<ADS_SOTHAM_HDXX> NTHTT = dt.ADS_SOTHAM_HDXX.Where(x => x.DONID == DONID).ToList();
                            List<ADS_SOTHAM_QUYETDINH> qd = dt.ADS_SOTHAM_QUYETDINH.Where(x => x.DONID == DONID).ToList();
                        ...
                        {
                            List<ADS_SOTHAM_HDXX> NTHTT = dt.ADS_SOTHAM_HDXX.Where(x => x.DONID == DONID && x.NGAYTAO > chuyenan.NGAYTAO).ToList();
                            List<ADS_SOTHAM_QUYETDINH> qd = dt.ADS_SOTHAM_QUYETDINH.Where(x => x.DONID == DONID && x.NGAYTAO > chuyenan.NGAYTAO).ToList();
                            if ((NTHTT != null && NTHTT.Count > 0) || (qd != null && qd.Count > 0))
                            {
                                lblSua.Visible = false;
                                lbtXoa.Visible = false;
                    ...
                    else
                    {
                        ...
                        List<ADS_SOTHAM_HDXX> NTHTT = dt.ADS_SOTHAM_HDXX.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == DonviID).ToList();
                        List<ADS_SOTHAM_QUYETDINH> qd = dt.ADS_SOTHAM_QUYETDINH.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == DonviID).ToList();
                        if ((NTHTT != null && NTHTT.Count > 0) || (qd != null && qd.Count > 0))
                        {
                            lblSua.Visible = false;
                            lbtXoa.Visible = false;
                ...
                //ADS_SOTHAM_HDXX hdxx = dt.ADS_SOTHAM_HDXX.Where(x => x.DONID == DONID).FirstOrDefault();
                //ADS_SOTHAM_QUYETDINH qd = dt.ADS_SOTHAM_QUYETDINH.Where(x => x.DONID == DONID).FirstOrDefault();
                //if(hdxx != null || qd != null)
                //{
                //    lblSua.Visible = false;
                //    lbtXoa.Visible = false;
                //}
        ...
        private void LoadCombobox()
        {
            ...
            // Mặc định là thẩm phán giải quyết đơn
            int DonID = Session[ENUM_LOAIAN.AN_DANSU] + "" == "" ? 0 : Convert.ToInt32(Session[ENUM_LOAIAN.AN_DANSU]);
            ADS_DON_THAMPHAN obj = dt.ADS_DON_THAMPHAN.Where(x => x.DONID == DonID).FirstOrDefault();
            if (obj != null)
            {
                ddlThamphan.SelectedValue = obj.CANBOID.ToString();

            }
            // Load Người phân công
        ...
        private bool CheckValid()
        {
            ...
            if (!Cls_Comon.IsValidDate(txtNgayphancong.Text))
            {
                lbthongbao.Text = "Bạn chưa nhập ngày phân công theo định dạng (dd/MM/yyyy)!";
            ...
            if (!Cls_Comon.IsValidDate(txtNhanphancong.Text))
            {
                lbthongbao.Text = "Bạn chưa nhập ngày nhận phân công theo định dạng (dd/MM/yyyy)!";
            ...
            if (txtNgayketthuc.Text.Trim() != "")
            {
                ...
                {
                    lbthongbao.Text = "Bạn chưa nhập ngày kết thúc theo định dạng (dd/MM/yyyy)!";
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string current_id = Session[ENUM_LOAIAN.AN_DANSU] + "", MaVaiTro = ddlVaitro.SelectedValue;
                decimal DONID = Convert.ToDecimal(current_id), CanBoID = Convert.ToDecimal(ddlThamphan.SelectedValue);

                ADS_DON_THAMPHAN oND;
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    // Kiểm tra nếu thẩm phán đã được phân công rồi thì không phân lại nữa. Chọn thẩm phán khác
                    oND = dt.ADS_DON_THAMPHAN.Where(x => x.DONID == DONID && x.CANBOID == CanBoID && x.MAVAITRO == MaVaiTro).FirstOrDefault();
                    if (oND != null)
                    {
                    ...
                    oND = new ADS_DON_THAMPHAN();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.ADS_DON_THAMPHAN.Where(x => x.ID == ID).FirstOrDefault();
                }
                oND.DONID = DONID;
                oND.CANBOID = CanBoID;
                ...
                ADS_SOTHAM_HDXX oTHTT = dt.ADS_SOTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).FirstOrDefault();
        ...
        {
            ADS_DON_THAMPHAN_BL oBL = new ADS_DON_THAMPHAN_BL();
            string current_id = Session[ENUM_LOAIAN.AN_DANSU] + "";
            decimal ID = Convert.ToDecimal(current_id);
            DataTable oDT = oBL.ADS_DON_THAMPHAN_GETBY(ID, ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM);
        ...
        public void xoa(decimal id)
        {
            ...
            ADS_DON_THAMPHAN oND = dt.ADS_DON_THAMPHAN.Where(x => x.ID == id).FirstOrDefault();
            if (oND != null)
            {
                //Luu thong tin truoc khi xoa Phan cong thẩm phán
                ...
                if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.DONID), 2, Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Phân công thẩm phán giải quyết Sơ thẩm Dân sự", "Xóa", json) == false)
                {
                    lbthongbao.Text = "Xóa không thành công!";
                    return;
                }//Ket thuc 

                //Xoa Phan cong thẩm phán
                dt.ADS_DON_THAMPHAN.Remove(oND);
                dt.SaveChanges();
            }
            dgList.CurrentPageIndex = 0;
            LoadGrid();
        ...
        private bool CheckQDofThamPhan(decimal DonID, decimal TPID)
        {
            ...
            ADS_SOTHAM_QUYETDINH obj_QD = dt.ADS_SOTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.NGUOIKYID == TPID).FirstOrDefault<ADS_SOTHAM_QUYETDINH>();
            if (obj_QD != null)
            {
