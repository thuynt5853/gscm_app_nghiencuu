...
namespace WEB.GSTP.QLAN.ADS.Phuctham
{
    public partial class Quyetdinhphuctham : System.Web.UI.Page
    {
        ...
        private void LoadNguoiKyInfo()
        {
            ...
            ADS_PHUCTHAM_HDXX oND = dt.ADS_PHUCTHAM_HDXX.Where(x => x.DONID == DonID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).OrderByDescending(x => x.ID).FirstOrDefault<ADS_PHUCTHAM_HDXX>();
            if (oND != null)
            {
        ...
        public void LoadGrid()
        {
            ...
            DM_QUYETDINH_VUAN oBL = new DM_QUYETDINH_VUAN();
            DataTable oDT = oBL.DGLIST_QUYETDINH_PT(ENUM_LOAIVUVIEC_NUMBER.AN_DANSU, ID);
            if (oDT != null)
            {
        ...
        public void xoa(decimal id)
        {
            ...
            ADS_TONGDAT oTD = dt.ADS_TONGDAT.Where(x => x.DONID == oND.DONID && x.MAPID == oND.ID && x.MAP_TABLE == ENUM_MAP_TABLE.ADS_PHUCTHAM_QUYETDINH).FirstOrDefault();
            if (oTD != null)
            {
            ...
            ADS_DON_BL oBL = new ADS_DON_BL();
            if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.DONID), 2, Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Quyet dinh Phúc thẩm án Dân sự", "Xóa", json) == false)
            {
            ...
            //var list = dt.ADS_PHUCTHAM_BANAN.Where(x => x.DONID == DONID).ToList();
            //var ADS_PHUCTHAM_BANAN = list.FirstOrDefault(x => x.TOA_GIAIQUYET_ID == currenttoaid);
            ADS_PHUCTHAM_BANAN PHUCTHAM_BANAN = dt.ADS_PHUCTHAM_BANAN.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == currenttoaid).FirstOrDefault<ADS_PHUCTHAM_BANAN>();
            if (PHUCTHAM_BANAN != null)
            {
            ...
            try
            {
                ...
                switch (e.CommandName)
                {
                    case "Download":
                        ...
                        break;
                    case "Sua":
                        ADS_PHUCTHAM_QUYETDINH oND1 = dt.ADS_PHUCTHAM_QUYETDINH.Where(x => x.ID == ND_id).FirstOrDefault();
                        // khong duoc xoa khi da Tong dat - 27/11/2025 vnpt check
                        ADS_TONGDAT oTD = dt.ADS_TONGDAT.Where(x => x.DONID == oND1.DONID && x.MAPID == oND1.ID && x.MAP_TABLE == ENUM_MAP_TABLE.ADS_PHUCTHAM_QUYETDINH).FirstOrDefault();
                        if (oTD != null)
                        {
        ...
        {
            decimal ID = Convert.ToDecimal(hddid.Value);
            ADS_PHUCTHAM_QUYETDINH oQD = dt.ADS_PHUCTHAM_QUYETDINH.Where(x => x.ID == ID).FirstOrDefault();
