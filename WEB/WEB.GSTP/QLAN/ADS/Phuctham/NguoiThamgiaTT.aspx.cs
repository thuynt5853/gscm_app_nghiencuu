    ...
    public partial class NguoiThamgiaTT : System.Web.UI.Page
    {
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                ADS_DON oT = dt.ADS_DON.Where(x => x.ID == DONID && x.TOA_GIAIQUYET_ID == DonviID).FirstOrDefault();
                if (oT != null && (oT.MAGIAIDOAN == ENUM_GIAIDOANVUAN.THULYGDT || oT.MAGIAIDOAN == ENUM_GIAIDOANVUAN.SOTHAM))
                {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string current_id = Session[ENUM_LOAIAN.AN_DANSU] + "";
                decimal DONID = Convert.ToDecimal(current_id);
                ADS_PHUCTHAM_THAMGIATOTUNG oND = null;
                if (hddid.Value == "" || hddid.Value == "0")
                    oND = new ADS_PHUCTHAM_THAMGIATOTUNG();
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.ADS_PHUCTHAM_THAMGIATOTUNG.Where(x => x.ID == ID).FirstOrDefault();
                }
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    dt.ADS_PHUCTHAM_THAMGIATOTUNG.Add(oND);
                }
                else
                {
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.ADS_PHUCTHAM_TGTT_GETLIST(ID);
        ...
        {
            ADS_PHUCTHAM_THAMGIATOTUNG oND = dt.ADS_PHUCTHAM_THAMGIATOTUNG.Where(x => x.ID == id).FirstOrDefault();
            if (oND != null)
            {
                //Anhpn 
                var currenttoaid = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                decimal DONID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_DANSU] + "");
                ADS_PHUCTHAM_HDXX ADS_PHUCTHAM_HDXX = dt.ADS_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == currenttoaid).FirstOrDefault<ADS_PHUCTHAM_HDXX>();
                ...
                dt.ADS_PHUCTHAM_THAMGIATOTUNG.Remove(oND);
                dt.SaveChanges();
                dgList.CurrentPageIndex = 0;
                LoadGrid();
                ResetControls();
            ...
            foreach (ListItem oItem in chkListTGTT.Items)
            {
                if (oItem.Selected)
                {
                    ...
                    if (dt.ADS_PHUCTHAM_THAMGIATOTUNG.Where(x => x.NTGTT_DONKK_ID == NID).ToList().Count == 0)
                    {
                        ...
                        ADS_PHUCTHAM_THAMGIATOTUNG oS = new ADS_PHUCTHAM_THAMGIATOTUNG();
                        oS.DONID = oD.DONID;
                        oS.HOTEN = oD.HOTEN;
                        ...
                        dt.ADS_PHUCTHAM_THAMGIATOTUNG.Add(oS);
                        dt.SaveChanges();
                    }
                }
            }
        ...
        private void load_so_dk(string v_stpt)
        {
            ...
            decimal sodk_ = 0;
            oBL.SO_DK_RETURN(v_stpt, ref sodk_, Session[ENUM_SESSION.SESSION_DONVIID] + "");
