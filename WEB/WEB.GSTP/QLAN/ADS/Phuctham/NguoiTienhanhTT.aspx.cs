...
namespace WEB.GSTP.QLAN.ADS.Phuctham
{
    public partial class NguoiTienhanhTT : System.Web.UI.Page
    {
        ...
        void CheckQuyen()
        {
            ...
            decimal DONID = (String.IsNullOrEmpty(Session[ENUM_LOAIAN.AN_DANSU] + ""))?0:Convert.ToDecimal(Session[ENUM_LOAIAN.AN_DANSU] + "");
            decimal Donvi_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
            ADS_DON oT = dt.ADS_DON.Where(x => x.ID == DONID && x.TOA_GIAIQUYET_ID == Donvi_ID).FirstOrDefault();
            List<ADS_PHUCTHAM_THULY> lstCount = dt.ADS_PHUCTHAM_THULY.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == Donvi_ID).ToList();
        ...
        private void LoadCombobox()
        {
            ...
            if (loaiQD != null)
            {
                ADS_PHUCTHAM_QUYETDINH qd = dt.ADS_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.LOAIQDID == loaiQD.ID).FirstOrDefault<ADS_PHUCTHAM_QUYETDINH>();
                if (qd != null)
                {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ADS_PHUCTHAM_HDXX oND;

                // Lấy count thụ lý và count chủ tọa để cho nhập thêm thẩm phán chủ tọa
                List<ADS_DON_THAMPHAN> countTPGQ = dt.ADS_DON_THAMPHAN.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM).ToList();
                List<ADS_PHUCTHAM_HDXX> countChuToa = dt.ADS_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).ToList();
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    if (tucach == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN)
                    {
                        ...
                        oND = dt.ADS_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN && x.TOA_GIAIQUYET_ID == currenttoaid).FirstOrDefault<ADS_PHUCTHAM_HDXX>();
                    ...
                    oND = new ADS_PHUCTHAM_HDXX();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.ADS_PHUCTHAM_HDXX.Where(x => x.ID == ID).FirstOrDefault();
                }
                oND.DONID = DONID;
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    ADS_PHUCTHAM_HDXX CheckCanBo = dt.ADS_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.CANBOID == oND.CANBOID).FirstOrDefault();
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.ADS_PHUCTHAM_HDXX_GETLIST(ID);
        ...
        {
            ADS_PHUCTHAM_HDXX oND = dt.ADS_PHUCTHAM_HDXX.Where(x => x.ID == id).FirstOrDefault();

            //Anhpn 
            var currenttoaid = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
            decimal DONID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_DANSU] + "");
            ADS_PHUCTHAM_HOAGIAI ADS_PHUCTHAM_HOAGIAI = dt.ADS_PHUCTHAM_HOAGIAI.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == currenttoaid).FirstOrDefault<ADS_PHUCTHAM_HOAGIAI>();
            ...
            dt.ADS_PHUCTHAM_HDXX.Remove(oND);
            dt.SaveChanges();
            hddPageIndex.Value = "1";
            dgList.CurrentPageIndex = 0;
            LoadGrid();
