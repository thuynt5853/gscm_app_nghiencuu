...
namespace WEB.GSTP.QLAN.ADS.Phuctham
{
    public partial class ThuLy : System.Web.UI.Page
    {
        ...
        private decimal SetNewSoThuLy()
        {
            ...
            string sothulymoi = oSTBL.GET_STL_NEW(DonViID, "ADS_PT", ngaythuly).ToString();
        ...
        void SetNewSoThongbao()
        {
            ...
            txtSothongbao.Text = oSTBL.GET_STBTL_NEW(DonViID, "ADS_PT", ngaythongbao).ToString();
        ...
        private bool CheckValid()
        {
            ...
            //    Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
            //    ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
            //    Decimal CheckID = oSTBL.CheckSoTLTheoLoaiAn(DonViID, "ADS_PT", sothuly, ngaythuly);
            //    if (CheckID > 0)
            //    {
            //        Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);

            //        String strMsg = "";
            //        String STTNew = oSTBL.GET_STL_NEW(DonViID, "ADS_PT", ngaythuly).ToString();
            ...
            if (!String.IsNullOrEmpty(txtNgaythuly.Text))
            {
                ...
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal CheckID = oSTBL.CheckSoTBTLTheoLoaiAn(DonViID, "ADS_PT", sothongbao, ngaythuly);
                if (CheckID > 0)
                {
                    Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
                    String strMsg = "";
                    String STTNew = oSTBL.GET_STBTL_NEW(DonViID, "ADS_PT", ngaythuly).ToString();
                    if (CheckID != CurrThuLyID)
                    {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                decimal FileID = 0;
                ADS_PHUCTHAM_BL oPTBL = new ADS_PHUCTHAM_BL();
                ADS_PHUCTHAM_THULY oND;
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    oND = new ADS_PHUCTHAM_THULY();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.ADS_PHUCTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
                    if (oND.FILEID != null) FileID = (decimal)oND.FILEID;
                    // - vnpt 1/12/2025  check tong dat
                    ADS_TONGDAT oTD = dt.ADS_TONGDAT.Where(x => x.DONID == oND.DONID && x.MAPID == oND.ID && x.MAP_TABLE == ENUM_MAP_TABLE.ADS_PHUCTHAM_THULY).FirstOrDefault();
                    if (oTD != null)
                    {
                ...
                oQLSTL.update_STPT_QUANLY_SOTHULY(3, 2, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), ((DateTime)oND.NGAYTHULY).ToString("dd/MM/yyyy", cul), Regex.Match(oND.SOTHULY, @"\d+").Value);
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    GD.GAIDOAN_INSERT_UPDATE("2", DONID, 3,0, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), 0, 0, 0);
                }
                else
                {
        ...
        {
            ADS_PHUCTHAM_THULY oND = dt.ADS_PHUCTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
            if (oND != null)
            {
                try
                {
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                switch (e.CommandName)
                {
                    ...
                    case "Sua":
                        lbthongbao.Text = "";
                        ADS_PHUCTHAM_THULY oND1 = dt.ADS_PHUCTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
                        // khong duoc SỬA khi da Tong dat - 29/11/2025 vnpt check
                        ADS_TONGDAT oTD = dt.ADS_TONGDAT.Where(x => x.DONID == oND1.DONID && x.MAPID == oND1.ID && x.MAP_TABLE == ENUM_MAP_TABLE.ADS_PHUCTHAM_THULY).FirstOrDefault();
                        if (oTD != null)
                        {
                            lbthongbao.Text = "Bạn không thể sửa khi đã tống đạt!";
                            return;
                    ...
                    case "Xoa":
                        ADS_PHUCTHAM_THULY oND2 = dt.ADS_PHUCTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
                        // khong duoc xoa khi da Tong dat - 29/11/2025 vnpt check
                        ADS_TONGDAT oTD1 = dt.ADS_TONGDAT.Where(x => x.DONID == oND2.DONID && x.MAPID == oND2.ID && x.MAP_TABLE == ENUM_MAP_TABLE.ADS_PHUCTHAM_THULY).FirstOrDefault();
                        if (oTD1 != null)
                        {
        ...
        protected void txtNgayThuLy_TextChanged(object sender, EventArgs e)
        {
            ...
            //    STPT_QUANLY_SOTHULY oQLSTL = new STPT_QUANLY_SOTHULY();
            //    DataTable dtStl = oQLSTL.get_STPT_QUANLY_SOTHULY(3, 2, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtNgaythuly.Text);
            //    if (dtStl != null)
            //    {
            ...
            //    string check_ngaythulycuoi_trongnam = oQLSTL.get_LATEST_DATE_IN_SOTHULY(3, 2, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtNgaythuly.Text);
        ...
        protected void btnSaveLydoXoa_Insert(object sender, EventArgs e)
        {
            ...
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            ADS_PHUCTHAM_THULY oND = dt.ADS_PHUCTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
            ...
            if (oND != null)
            {
                ...
                ADS_DON_THAMPHAN ADS_DON_THAMPHAN = dt.ADS_DON_THAMPHAN.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM).FirstOrDefault<ADS_DON_THAMPHAN>();
                if (ADS_DON_THAMPHAN != null)
                {
                    lbthongbao.Text = "Xóa không thành công! Do đang tồn tại thông tin phân công thẩm phán giải quyết.";
                    return;
                }
                ADS_PHUCTHAM_THAMGIATOTUNG ADS_PHUCTHAM_THAMGIATOTUNG = dt.ADS_PHUCTHAM_THAMGIATOTUNG.Where(x => x.DONID == DONID).FirstOrDefault<ADS_PHUCTHAM_THAMGIATOTUNG>();
                if (ADS_PHUCTHAM_THAMGIATOTUNG != null)
                {
                    lbthongbao.Text = "Xóa không thành công! Do đang tồn tại người tham gia tố tụng.";
                    return;
                }
                ADS_PHUCTHAM_HDXX ADS_PHUCTHAM_HDXX = dt.ADS_PHUCTHAM_HDXX.Where(x => x.DONID == DONID).FirstOrDefault<ADS_PHUCTHAM_HDXX>();
                if (ADS_PHUCTHAM_HDXX != null)
                {
                    lbthongbao.Text = "Xóa không thành công! Do đang tồn tại thông tin người tiến hành tố tụng.";
                    return;
                }
                ADS_PHUCTHAM_HOAGIAI ADS_PHUCTHAM_HOAGIAI = dt.ADS_PHUCTHAM_HOAGIAI.Where(x => x.DONID == DONID).FirstOrDefault<ADS_PHUCTHAM_HOAGIAI>();
                if (ADS_PHUCTHAM_HOAGIAI != null)
                {
                    lbthongbao.Text = "Xóa không thành công! Do đang tồn tại thông tin hòa giải.";
                    return;
                }
                ADS_PHUCTHAM_QUYETDINH ADS_PHUCTHAM_QUYETDINH = dt.ADS_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DONID).FirstOrDefault<ADS_PHUCTHAM_QUYETDINH>();
                ...
                dt.ADS_PHUCTHAM_THULY.Remove(oND);
                dt.SaveChanges();
                if (FileID > 0)
                {
                    try
