...
namespace WEB.GSTP.QLAN.ADS.TaoHS.Popup
{
    public partial class uDSNguoiThamGiaToTung : System.Web.UI.UserControl
    {
        ...
        public void LoadGrid()
        {
            ...
            //dgList.DataSource = oBL.ADS_DON_TGTT_GETLIST(DONID);
            //dgList.DataBind();
            int page_size = 20;
            int pageindex = Convert.ToInt32(hddPageIndex.Value);
            //AHS_SOTHAM_BL objBL = new AHS_SOTHAM_BL();
            ADS_DON_BL oBL = new ADS_DON_BL();
            DataTable tbl = oBL.ADS_DON_TGTT_GETLIST(DONID);
        ...
        protected void rpt_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                HiddenField hidToaGiaiQuyetId = (HiddenField)e.Item.FindControl("hidToaGiaiQuyetId");

                string toagiaiquyetID = hidToaGiaiQuyetId?.Value?.Trim();
                string donviID = Session[ENUM_SESSION.SESSION_DONVIID]?.ToString()?.Trim();
                ...
                //    lbtXoa.Visible = false;
                //string Result = new ADS_CHUYEN_NHAN_AN_BL().Check_NhanAn(donid, "");
                //if (Result != "")
                //{
                //  lttSua.Text = "<a href='javascript:;' onclick='popup_form_edit(" + rv["ID"].ToString() + ");'>Chi tiết</a>";
                //  Cls_Comon.SetLinkButton(lbtXoa, false);
