<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="uDSNguoiThamGiaToTung.ascx.cs" Inherits="WEB.GSTP.QLAN.ADS.TaoHS.Popup.uDSNguoiThamGiaToTung" %>
        ...
        <ItemTemplate>
            <tr>
                ...
                <td>
                    ...
                    <asp:HiddenField ID="hidToaGiaiQuyetId" runat="server" Value='<%# Eval("TOA_GIAIQUYET_ID") %>' />
                </td>
            </tr>
        </ItemTemplate>
    </asp:Repeater>
...
<script>
    function popup_form_edit(curr_id) {
        var link = "";
        if (curr_id > 0)
            link = "/QLAN/ADS/TaoHS/Popup/pNguoiThamGiaTT.aspx?hsID=<%=Donid%>&uID=" + curr_id;
        else
            link = "/QLAN/ADS/TaoHS/Popup/pNguoiThamGiaTT.aspx?hsID=<%=Donid%>";
        var width = 950;
        var height = 350;
