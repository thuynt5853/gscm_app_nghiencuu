...
namespace WEB.GSTP.QLAN.ADS.Sotham
{
    public partial class Hoagiai : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (dgList.Items.Count > 0)
                {
                    ...
                    DataTable oDT = oBL.ADS_SOTHAM_HOAGIAI_GETLIST(ID);
                    loadedit(Convert.ToDecimal(oDT.Rows[0]["ID"]));
                }
                ADS_DON_XULY obj = dt.ADS_DON_XULY.Where(x => x.DONID == ID).FirstOrDefault();
                if (obj != null)
                {
                    hddNgayGiaiQuyetDon.Value = obj.NGAYGQ_YC + "" == "" ? "" : ((DateTime)obj.NGAYGQ_YC).ToString("dd/MM/yyyy");
                }
            }
        }
        ...
        private void LoadCombobox()
        {
            ...
            string current_id = Session[ENUM_LOAIAN.AN_DANSU] + "";
            decimal DONID = Convert.ToDecimal(current_id);
            List<ADS_DON_THAMPHAN> lstTP = dt.ADS_DON_THAMPHAN.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).OrderByDescending(x => x.NGAYNHANPHANCONG).ToList();
            if (lstTP.Count > 0)
            {
                if (ddlNguoiky.Items.FindByValue(lstTP[0].CANBOID.ToString()) != null)
                    ddlNguoiky.SelectedValue = lstTP[0].CANBOID.ToString();
            }
        ...
        private bool CheckValid()
        {
            ...
            {
                lbthongbao.Text = "Bạn chưa nhập ngày thông báo theo định dạng (dd/MM/yyyy) !";
                txtNgaythongbao.Focus();
                return false;
            }
            DateTime NgayTB = DateTime.Parse(txtNgaythongbao.Text, cul, DateTimeStyles.NoCurrentDateDefault);
            if (NgayTB > DateTime.Now)
            {
                lbthongbao.Text = "Ngày thông báo phải nhỏ hơn ngày hiện tại !";
                txtNgaythongbao.Focus();
                return false;
            }
            if (hddNgayGiaiQuyetDon.Value != "")
            {
                DateTime NgayGiaiQuyetDon = DateTime.Parse(hddNgayGiaiQuyetDon.Value, cul, DateTimeStyles.NoCurrentDateDefault);
                if (NgayTB < NgayGiaiQuyetDon)
                {
                    lbthongbao.Text = "Ngày thông báo phải lớn hơn ngày giải quyết đơn " + hddNgayGiaiQuyetDon.Value + " !";
                    txtNgaythongbao.Focus();
                    return false;
                }
            }
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ADS_SOTHAM_HOAGIAI oND;
                if (hddid.Value == "" || hddid.Value == "0")
                    oND = new ADS_SOTHAM_HOAGIAI();
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.ADS_SOTHAM_HOAGIAI.Where(x => x.ID == ID).FirstOrDefault();
                }
                oND.DONID = DONID;
                //oND.MATHULY = "";
        ...
        public void xoa(decimal id)
        {
            ...
            ADS_SOTHAM_HOAGIAI oND = dt.ADS_SOTHAM_HOAGIAI.Where(x => x.ID == id).FirstOrDefault();

            dt.ADS_SOTHAM_HOAGIAI.Remove(oND);
            dt.SaveChanges();
            dgList.CurrentPageIndex = 0;
            LoadGrid();
            ResetControls();
        ...
        public void loadedit(decimal ID)
        {
            ...
            if (oND.NGAY != null) txtNgaythongbao.Text = ((DateTime)oND.NGAY).ToString("dd/MM/yyyy", cul);
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "Xoa":
                    ...
                    break;
                case "Download":
                    ADS_SOTHAM_HOAGIAI oND = dt.ADS_SOTHAM_HOAGIAI.Where(x => x.ID == ND_id).FirstOrDefault();
                    if (oND.TENFILE != "")
                    {
