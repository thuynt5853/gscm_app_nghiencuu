...
namespace WEB.GSTP.QLAN.ADS.Sotham
{
    public partial class KhangCaoKhangNghi : System.Web.UI.Page
    {
        ...
        void CheckQuyen()
        {
            ...
            List<ADS_DON_THAMPHAN> lstTP = dt.ADS_DON_THAMPHAN.Where(x => x.DONID == DonID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).ToList();
            if (lstTP.Count == 0)
            {
            ...
            //ADS_SOTHAM_QUYETDINH QDS = dt.ADS_SOTHAM_QUYETDINH.Where(x => x.DONID == ID && x.QUYETDINHID == 46).OrderByDescending(x => x.ID).FirstOrDefault();
            //if (QDS.QUYETDINHID != 46)
            //{
            //    Cls_Comon.SetButton(btnUpdate, false);
            //    Cls_Comon.SetButton(btnLammoi, false);
            //    hddShowCommand.Value = "False";
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (IsKhangCao == 1)
                {
                    ADS_SOTHAM_KHANGCAO oKC = dt.ADS_SOTHAM_KHANGCAO.Where(x => x.ID == idKCKN).FirstOrDefault();
                    if (oKC != null)
                    {
                        ...
                        string Result = new ADS_CHUYEN_NHAN_AN_BL().Check_ChuyenNhanAn(DONID, StrMsg, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]));
                        if (Result == "")
                        {
                ...
                {
                    ADS_SOTHAM_KHANGNGHI oKN = dt.ADS_SOTHAM_KHANGNGHI.Where(x => x.ID == idKCKN).FirstOrDefault();
                    if (oKN != null)
                    {
        ...
        private void LoadComboboxKhangCao()
        {
            ...
            ddlNguoikhangcao.DataSource = oDSBL.ADS_DUONGSU_KHANGCAO(DonID);
        ...
        void LoadDrop_ST_BanAn(DropDownList drop, Decimal DonID)
        {
            ...
            DataTable oDT = oBL.DANHSACH_KHANGCAO_BANAN(ENUM_LOAIVUVIEC_NUMBER.AN_DANSU, DonID);
            if (oDT != null)
            {
                drop.DataSource = oDT;
        ...
        void LoadDrop_ST_QuyetDinh(DropDownList drop, Decimal DonID, Decimal loaiKC)
        {
            ...
            {
                PAGE_KHANGCAO_ST oBL = new PAGE_KHANGCAO_ST();
                DataTable oDT = oBL.DANHSACH_KHANGCAO_QUYETDINH(ENUM_LOAIVUVIEC_NUMBER.AN_DANSU, DonID);
                if (oDT != null)
                {
                    drop.DataSource = oDT;
                    drop.DataTextField = "TEN";
            ...
            {
                PAGE_KHANGCAO_ST oBL = new PAGE_KHANGCAO_ST();
                DataTable oDT = oBL.DANHSACH_KHANGCAO_QUYETDINH_KHAC(ENUM_LOAIVUVIEC_NUMBER.AN_DANSU, DonID);
                if (oDT != null)
                {
                    drop.DataSource = oDT;
                    drop.DataTextField = "TEN";
            ...
            try
            {
                ...
                switch (e.CommandName)
                {
                    case "Download":
                        ...
                        if (IsKhangCao == 1)
                        {
                            ADS_SOTHAM_KHANGCAO oKN = dt.ADS_SOTHAM_KHANGCAO.Where(x => x.ID == ID).FirstOrDefault();
                            TENFILE = oKN.TENFILE;
                    ...
                    case "Xoa":
                        ...
                        ADS_SOTHAM_KHANGCAO oKC = dt.ADS_SOTHAM_KHANGCAO.Where(x => x.DONID == DonID && x.ID == ID).FirstOrDefault();
                        if (Result != "")
                        {
                            if (oKC.ISQUAHAN == 1)
                            {
            ...
            try
            {
                ...
                if (IsKhangCao == KHANGCAO)// Kháng cáo
                {
                    ...
                    if (oND != null)
                    {
                        ...
                        ADS_ANPHI oAP = dt.ADS_ANPHI.Where(x => x.DONID == oND.DONID && x.DUONGSU_ID == oND.DUONGSUID && x.MAGIAIDOAN == 3).FirstOrDefault();
                        if (oAP != null)
                        {
        ...
        public void xoa(decimal ID, decimal IsKhangCao)
        {
            if (IsKhangCao == KHANGCAO)// Kháng cáo
            {
                ...
                ADS_ANPHI objAP = dt.ADS_ANPHI.Where(x => x.DONID == DonID && x.DUONGSU_ID == oND.DUONGSUID && x.MAGIAIDOAN == 3).FirstOrDefault();
                if (objAP != null)
                {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                decimal ID = 0, IsKhangCao = 0, DonID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_DANSU] + "");
                ADS_DON oDon = dt.ADS_DON.Where(x => x.ID == DonID).FirstOrDefault();
                if (rdbPanelKC.SelectedValue == KHANGCAO.ToString() && rdbPanelKN.SelectedValue == KHANGCAO.ToString())// Kháng cáo
                {
                    ...
                    {
                        ADS_ANPHI lstAPnew = new ADS_ANPHI();
        ...
        private bool CheckValid(decimal isKhangCao)
        {
            if (isKhangCao == KHANGCAO)// Kháng cáo
            {
                ...
                {
                    lbthongbao.Text = "Bạn chưa chọn miễn án phí. Hãy chọn lại!";
                    return false;
                }
        ...
        protected void rdbPanelKC_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else // Kháng nghị
            {
                ...
                DON_KHAC oD = dt.DON_KHAC.Where(x => x.DONID == DonID && x.LOAIANID == 2 && x.LOAIKCKN == 2).FirstOrDefault();
        ...
        protected void cmdLoadDonKhac_Click(object sender, EventArgs e)
        {
            ...
            DON_KHAC oD = dt.DON_KHAC.Where(x => x.ID == ID && x.LOAIANID == 2 && x.LOAIKCKN == 1).FirstOrDefault();
            if (oD != null)
            {
                pnKhangCao.Visible = true;
                pnKhangNghi.Visible = false;
        ...
        void CheckNgayKCQuaHan()
        {
            ...
            if (banan_qd_id > 0)
            {
                if (loai_ba_qd == 0)
                {
                    ...
                    ADS_SOTHAM_BANAN obj = dt.ADS_SOTHAM_BANAN.Where(x => x.ID == banan_qd_id).FirstOrDefault();
                    if (obj != null)
                ...
                else
                {
                    ...
                    ADS_SOTHAM_QUYETDINH obj = dt.ADS_SOTHAM_QUYETDINH.Where(x => x.ID == banan_qd_id).FirstOrDefault();
                    if (obj != null)
                    {
