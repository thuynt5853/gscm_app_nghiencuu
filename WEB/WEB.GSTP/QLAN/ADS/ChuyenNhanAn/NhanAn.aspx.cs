...
namespace WEB.GSTP.QLAN.ADS.ChuyenNhanAn
{
    public partial class NhanAn : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    //string sql = "select * from HOSO where TOA_AN_XET_XU='" + ToaAnID.ToString() + "' and LOAI_AN='" + ENUM_LOAIAN.AN_DANSU + "' and TRANG_THAI=1";
                    ...
                    //        #region Update Vụ việc ADS_DON
                    //        ADS_DON vDON = dt.ADS_DON.Where(x => x.MAVUVIEC == MaVuViec && ((x.MAGIAIDOAN == ENUM_GIAIDOANVUAN.SOTHAM && x.TOAANID == ToaAnID) || (x.MAGIAIDOAN == ENUM_GIAIDOANVUAN.PHUCTHAM && x.TOAPHUCTHAMID == ToaAnID))).FirstOrDefault();
                    //        if (vDON == null)
                    //        {
                    //            IsNew = true;
                    //            vDON = new ADS_DON();
                    ...
                    //        VuAnID = vDON.ID;
                    //        #endregion
                    //        #region Update Table ADS_CHUYEN_NHAN_AN
                    //        IsNew = false;
                    //        ADS_CHUYEN_NHAN_AN chuyen_nhan_an = dt.ADS_CHUYEN_NHAN_AN.Where(x => x.VUANID == VuAnID && x.TOACHUYENID == Toa_Chuyen_ID && x.TOANHANID == Toa_Nhan_ID).FirstOrDefault();
                    //        if (chuyen_nhan_an == null)
                    //        {
                    //            IsNew = true;
                    //            chuyen_nhan_an = new ADS_CHUYEN_NHAN_AN();
                    //        }
                    ...
                    //        if (IsNew)
                    //        {
                    //            dt.ADS_CHUYEN_NHAN_AN.Add(chuyen_nhan_an);
                    //        }
                    //        dt.SaveChanges();
                    //        #endregion
                    //    }
        ...
        private void LoadGrid()
        {
            ...
            DataTable oDT = oBL.DS_NHANAN(txt_NG_KC.Text.Trim(), txtSoQD.Text.Trim(), txt_NgayQD.Text.Trim(), vDonViID, txtMaVuViec.Text, txtTenVuViec.Text, txtTenToa.Text, Convert.ToDecimal(ddlTHGN.SelectedValue), dFrom, dTo, Convert.ToDecimal(rdbTrangthai.SelectedValue), Convert.ToDecimal(rdbLoai.SelectedValue));
        ...
        protected void cmdNhanan_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridItem Item in dgList.Items)
            {
                ...
                if (chkChon.Checked)
                {
                    ...
                    input.NgayNhan = DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss");
                    data.Add(input);
                }
            }
        ...
        protected void cmdLuu_Click(object sender, EventArgs e)
        {
            ...
            foreach (RepeaterItem item in rptCapNhat.Items)
            {
                ...
                ADS_CHUYEN_NHAN_AN oT = dt.ADS_CHUYEN_NHAN_AN.Where(x => x.ID == ID).FirstOrDefault();
                if (oT != null)
                {
                    ...
                    if (oIT.MA == ENUM_TRUONGHOP_GIAONHAN.KHONGTHUOC_THAMQUYEN_XETXU)
                    {
                        ...
                        else
                        {
                            if (rdbLoai.SelectedValue == "0")
                            {
                                ...
                                GD.GAIDOAN_INSERT_UPDATE("2", oDonMoi.ID, 2, oDonMoi.TOAANID.Value, 0, 0, 0, 0);
                            ...
                            else
                            {
                                ...
                                */
                                string UserName = Session[ENUM_SESSION.SESSION_USERNAME] + "", LoaiVuViec = ENUM_LOAIVUVIEC.AN_DANSU, MaToaAn = Session[ENUM_SESSION.SESSION_MADONVI] + "";
                                ...
                                GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                                GD.GAIDOAN_INSERT_UPDATE("2", DonID_New, 2, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), 0, 0, 0, 0);
                            }
                    ...
                    else if (oIT.MA == ENUM_TRUONGHOP_GIAONHAN.XETXULAI_CAPSOTHAM)
                    {
                        ...
                        if (oDon.MAGIAIDOAN == ENUM_GIAIDOANVUAN.PHUCTHAM_QDK)
                        {
                            //oDon.MAGIAIDOAN = ENUM_GIAIDOANVUAN.SOTHAM;
                            //dt.SaveChanges();
                            ADS_CHUYEN_NHAN_AN_BL _caBL = new ADS_CHUYEN_NHAN_AN_BL();
                            decimal donIdOld = _caBL.getDonIdOld(oDon.ID);
                            GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                            ADS_DON oDonOld = dt.ADS_DON.Where(x => x.ID == donIdOld).FirstOrDefault();
                            ...
                            List<ADS_SOTHAM_KHANGCAO> khangCaos = dt.ADS_SOTHAM_KHANGCAO.Where(x => x.DONID == donIdOld && (x.TINHTRANG_GIAIQUYET == null || x.TINHTRANG_GIAIQUYET == 0) && x.LOAIKHANGCAO == 2).ToList();
                            ...
                            List<ADS_SOTHAM_KHANGNGHI> khangNghis = dt.ADS_SOTHAM_KHANGNGHI.Where(x => x.DONID == donIdOld && (x.TINHTRANG_GIAIQUYET == null || x.TINHTRANG_GIAIQUYET == 0) && x.LOAIKN == 2).ToList();
                        ...
                        else
                        {
                            ...
                            //update 14082025
                            oDON_new.TOA_GIAIQUYET_ID = Convert.ToDecimal(oDON_new.TOAANID);
                            oDON_new.TOA_PHUCTHAM_GIAIQUYET_ID = oDON_new.TOAPHUCTHAMID;
                            dt.ADS_DON.Add(oDON_new);
                            dt.SaveChanges();

                            GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                            GD.GAIDOAN_INSERT_UPDATE("2", oDON_new.ID, 2, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), 0, 0, 0, 0);
                            ...
                            //Câp dương su vụ án
                            List<ADS_DON_DUONGSU> lst = dt.ADS_DON_DUONGSU.Where(x => x.DONID == DonID).ToList<ADS_DON_DUONGSU>();
                            foreach (ADS_DON_DUONGSU vDuongsu_old in lst)
                            {
                                ...
                                //Ban giao tai lieu
                                List<ADS_DON_TAILIEU> lstTaiLieu = dt.ADS_DON_TAILIEU.Where(x => x.DONID == DonID && x.NGUOIBANGIAO == vDuongsu_old.ID).ToList<ADS_DON_TAILIEU>();
                            ...
                            //// Phan cong Tham phan
                            List<ADS_DON_THAMPHAN> lstThamPhan = dt.ADS_DON_THAMPHAN.Where(x => x.DONID == DonID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETDON).ToList<ADS_DON_THAMPHAN>();
                            foreach (ADS_DON_THAMPHAN vThamphan_old in lstThamPhan)
                            {
                                ...
                                //update 14082025
                                if(vThamphan_new.TOA_GIAIQUYET_ID == null) vThamphan_new.TOA_GIAIQUYET_ID = vThamphan_old.TOA_GIAIQUYET_ID;
                                dt.ADS_DON_THAMPHAN.Add(vThamphan_new);
                                dt.SaveChanges();
                            }
                            ////Xu ly don
                            List<ADS_DON_XULY> lstXulyDon = dt.ADS_DON_XULY.Where(x => x.DONID == DonID).ToList<ADS_DON_XULY>();
                            foreach (ADS_DON_XULY vXulyDon_old in lstXulyDon)
                            {
                                ...
                                //update 14082025
                                if(vXulyDon_new.TOA_GIAIQUYET_ID == null) vXulyDon_new.TOA_GIAIQUYET_ID = vXulyDon_old.TOA_GIAIQUYET_ID;
                                dt.ADS_DON_XULY.Add(vXulyDon_new);
                                dt.SaveChanges();
                            }
                            //Tam ung an phi
                            List<ADS_ANPHI> lstAP = dt.ADS_ANPHI.Where(x => x.DONID == DonID && x.MAGIAIDOAN == 2).ToList();
                            foreach (ADS_ANPHI vAP_old in lstAP)
                            {
                                ...
                                ////vAP_new.DUONGSU_ID = vAP_old.DUONGSU_ID; Sử dụng PKG để INSERT Sử dụng PKG để INSERT
                                vAP_new.NGUOINOP = vAP_old.NGUOINOP;
                                //update 14082025
                                if (vAP_new.TOA_GIAIQUYET_ID == null)
                                    vAP_new.TOA_GIAIQUYET_ID = vAP_old.TOA_GIAIQUYET_ID;
                                dt.ADS_ANPHI.Add(vAP_new);
                                dt.SaveChanges();

                                var CNA = new CHUYEN_NHAN_AN();
                                CNA.INSERT_ANPHI_DUONGSUID(2, DonID, oDON_new.ID);
                    ...
                    {
                        oDon.TOAPHUCTHAMID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                        ADS_SOTHAM_BL aDS_SOTHAM_BL = new ADS_SOTHAM_BL();
                        ADS_CHUYEN_NHAN_AN_BL _chuyenNhanAnBl = new ADS_CHUYEN_NHAN_AN_BL();

                        //var soThamKhangCaos = dt.ADS_SOTHAM_KHANGCAO.Where(x => x.DONID == oDon.ID && x.LOAIKHANGCAO == 2 && x.TINHTRANG_GIAIQUYET != 1);
                        //var soThamKhangNghis = dt.ADS_SOTHAM_KHANGNGHI.Where(x => x.DONID == oDon.ID && x.LOAIKN == 2 && x.TINHTRANG_GIAIQUYET != 1);
                        ...
                        if (_chuyenNhanAnBl.CheckCoKcKnTamDinhChi(DonID))
                        {
                            ...
                            #region toancau-anhnt Kiểm tra xem có phải kháng cáo kháng nghị quyết định khác
                            ...
                            GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                            GD.GAIDOAN_INSERT_UPDATE("2", oDON_new.ID, ENUM_GIAIDOANVUAN.PHUCTHAM_QDK, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), 0, 0, 0);
                            ...
                            //Câp dương su vụ án
                            List<ADS_DON_DUONGSU> lst = dt.ADS_DON_DUONGSU.Where(x => x.DONID == DonID).OrderByDescending(x => x.ID).ToList<ADS_DON_DUONGSU>();
                            foreach (ADS_DON_DUONGSU vDuongsu_old in lst)
                            {
                                ADS_DON_DUONGSU vDuongsu_new = new ADS_DON_DUONGSU();
                                vDuongsu_new.DONID = oDON_new.ID;
                                ...
                                vDuongsu_new.ISDAIDIEN_DONCHITIET = vDuongsu_old.ISDAIDIEN_DONCHITIET;
                                //update 14082025
                                if (vDuongsu_new.TOA_GIAIQUYET_ID == null)
                                    vDuongsu_new.TOA_GIAIQUYET_ID = vDuongsu_old.TOA_GIAIQUYET_ID;
                                // vnpt 06/12/2025
                                vDuongsu_new.XACTHUC_DLDCQG = vDuongsu_old.XACTHUC_DLDCQG;
                                dt.ADS_DON_DUONGSU.Add(vDuongsu_new);
                                ...
                                foreach (ADS_DON_TAILIEU vdonFile_old in lstTaiLieu)
                                {
                                    ...
                                    donFile_new.NGUOINHANID = vdonFile_old.NGUOINHANID;
                                    //update 14082025
                                    if (donFile_new.TOA_GIAIQUYET_ID == null)
                                        donFile_new.TOA_GIAIQUYET_ID = vdonFile_old.TOA_GIAIQUYET_ID;
                                    dt.ADS_DON_TAILIEU.Add(donFile_new);
                                    dt.SaveChanges();
                                }
                            ...
                            #endregion toancau-anhnt Kiểm tra xem có phải kháng cáo kháng nghị quyết định khác
                        }
                        else
                        {
                            oDon.MAGIAIDOAN = ENUM_GIAIDOANVUAN.PHUCTHAM;
                            ...
                            //anhvh add 26/06/2020
                            GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                            GD.GAIDOAN_INSERT_UPDATE("2", DonID, oDon.MAGIAIDOAN.Value, 0, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), 0, 0, 0);
        ...
        private void UpdateNguyenDonBiDon(List<NGUOI_THAM_GIA_AK> List_NguyeDon_BiDon, decimal DonID)
        {
            if (List_NguyeDon_BiDon.Count > 0)
            {
                ...
                {
                    IsNew = false;
                    ADS_DON_DUONGSU DuongSu = dt.ADS_DON_DUONGSU.Where(x => x.DONID == DonID && x.ID_DUONGSU_TACC == item.Guid).FirstOrDefault();
                    if (DuongSu == null)
                    {
                        IsNew = true;
                        DuongSu = new ADS_DON_DUONGSU();
                    }
                    ...
                    if (IsNew)
                    {
                        ...
                        dt.ADS_DON_DUONGSU.Add(DuongSu);
                    }
                    dt.SaveChanges();
                }
            }
        ...
        private void UpdateNguoiTGTT(List<NGUOI_THAM_GIA_CHUNG> ListNguoiTGTT, decimal DonID)
        {
            if (ListNguoiTGTT.Count > 0)
            {
                ...
                {
                    ADS_DON_THAMGIATOTUNG NguoiTGTT = dt.ADS_DON_THAMGIATOTUNG.Where(x => x.DONID == DonID && x.ID_DUONGSU_TACC == item.Guid).FirstOrDefault();
                    if (NguoiTGTT != null)
                    {
                        IsNew = true;
                        NguoiTGTT = new ADS_DON_THAMGIATOTUNG();
                    ...
                    if (IsNew)
                    {
                        ...
                        dt.ADS_DON_THAMGIATOTUNG.Add(NguoiTGTT);
                    }
                    dt.SaveChanges();
                }
            }
        ...
        private void UpdateThuLy(THU_LY ThuLy, decimal ToaAnID, decimal DonID, string CapXetXu)
        {
            if (CapXetXu == "SOTHAM")
            {
                ...
                bool IsNew = false;
                ADS_SOTHAM_THULY STThuLy = dt.ADS_SOTHAM_THULY.Where(x => x.DONID == DonID && x.SOTHULY == ThuLy.SOTHULY.ToString()).FirstOrDefault();
                if (STThuLy != null)
                {
                    IsNew = true;
                    STThuLy = new ADS_SOTHAM_THULY();
                ...
                if (IsNew)
                {
                    dt.ADS_SOTHAM_THULY.Add(STThuLy);
                }
                dt.SaveChanges();
            }
            else
            {
                ...
                bool IsNew = false;
                ADS_PHUCTHAM_THULY PTThuLy = dt.ADS_PHUCTHAM_THULY.Where(x => x.DONID == DonID && x.SOTHULY == ThuLy.SOTHULY.ToString()).FirstOrDefault();
                if (PTThuLy != null)
                {
                    IsNew = true;
                    PTThuLy = new ADS_PHUCTHAM_THULY();
                ...
                if (IsNew)
                {
                    dt.ADS_PHUCTHAM_THULY.Add(PTThuLy);
                }
                dt.SaveChanges();
            }
        }
        private void UpdateNguoiTHTT(List<NGUOI_TIEN_HANH_TO_TUNG> ListNguoiTHTT, decimal DonID, string CapXetXu)
        {
            if (ListNguoiTHTT.Count > 0)
            {
                ...
                foreach (NGUOI_TIEN_HANH_TO_TUNG item in ListNguoiTHTT)
                {
                    ...
                    if (CapXetXu == "SOTHAM")
                    {
                        ADS_SOTHAM_HDXX Hdxx = dt.ADS_SOTHAM_HDXX.Where(x => x.DONID == DonID && x.CANBOID == CanBoID).FirstOrDefault();
                        if (Hdxx == null)
                        {
                            IsNew = true;
                            Hdxx = new ADS_SOTHAM_HDXX();
                    ...
                    {
                        ADS_PHUCTHAM_HDXX Hdxx = dt.ADS_PHUCTHAM_HDXX.Where(x => x.DONID == DonID && x.CANBOID == CanBoID).FirstOrDefault();
                        if (Hdxx == null)
                        {
                            IsNew = true;
                            Hdxx = new ADS_PHUCTHAM_HDXX();
                            Hdxx.NGAYTAO = DateTime.Now;
                        }
                        else
                        {
                        ...
                        {
                            //update 14082025
                            if (Hdxx.TOA_GIAIQUYET_ID == null) Hdxx.TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                            dt.ADS_PHUCTHAM_HDXX.Add(Hdxx);
                        }
                    }
                    dt.SaveChanges();
                }
        ...
        private void UpdateQDVuViec(List<QUYET_DINH> ListQD, decimal ToaAnID, decimal DonID, string CapXetXu)
        {
            if (ListQD.Count > 0)
            {
                ...
                foreach (QUYET_DINH item in ListQD)
                {
                    ...
                    if (CapXetXu == "SOTHAM")
                    {
                        ADS_SOTHAM_QUYETDINH QdVuAn = dt.ADS_SOTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.TOAANID == ToaAnID).FirstOrDefault();
                        if (QdVuAn == null)
                        {
                            IsNew = false;
                            QdVuAn = new ADS_SOTHAM_QUYETDINH()
                            {
                        ...
                        if (IsNew)
                        {
                            ...
                            dt.ADS_SOTHAM_QUYETDINH.Add(QdVuAn);
                        }
                        dt.SaveChanges();
                    }
                    // Phúc thẩm
                    ...
                    {
                        ADS_PHUCTHAM_QUYETDINH QdVuAn = dt.ADS_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.TOAANID == ToaAnID).FirstOrDefault();
                        if (QdVuAn == null)
                        {
                            IsNew = false;
                            QdVuAn = new ADS_PHUCTHAM_QUYETDINH()
                            {
                                NGAYTAO = DateTime.Now
                            };
                        }
                        ...
                        if (IsNew)
                        {
                            dt.ADS_PHUCTHAM_QUYETDINH.Add(QdVuAn);
                        }
                        dt.SaveChanges();
                    }
                }
        ...
        private void UpdateBanAn(BAN_AN bAn, LICH_XET_XU LichXetXu, decimal ToaAnID, decimal DonID, string CapXetXu)
        {
            ...
            if (CapXetXu == "SOTHAM")
            {
                ADS_SOTHAM_BANAN STBanAn = dt.ADS_SOTHAM_BANAN.Where(x => x.DONID == DonID && x.TOAANID == ToaAnID).FirstOrDefault();
                if (STBanAn == null)
                {
                    IsNew = true;
                    STBanAn = new ADS_SOTHAM_BANAN();
                ...
                if (IsNew)
                {
                    ...
                    dt.ADS_SOTHAM_BANAN.Add(STBanAn);
                }
                dt.SaveChanges();
            }
            // Phúc thẩm
            ...
            {
                ADS_PHUCTHAM_BANAN PTBanAn = dt.ADS_PHUCTHAM_BANAN.Where(x => x.DONID == DonID && x.TOAANID == ToaAnID).FirstOrDefault();
                if (PTBanAn == null)
                {
                    IsNew = true;
                    PTBanAn = new ADS_PHUCTHAM_BANAN();
                ...
                DM_KETQUA_PHUCTHAM KQPT = dt.DM_KETQUA_PHUCTHAM.Where(x => x.ISADS == 1 && x.MA == bAn.KETQUA).FirstOrDefault();
                if (KQPT != null)
                {
                ...
                if (IsNew)
                {
                    ...
                    dt.ADS_PHUCTHAM_BANAN.Add(PTBanAn);
                }
                dt.SaveChanges();
            }
        }
        private void UpdateKhangCao(List<KHANG_CAO> ListKhangCao, decimal ToaAnID, decimal DonID)
        {
            if (ListKhangCao.Count > 0)
            {
                ...
                foreach (KHANG_CAO item in ListKhangCao)
                {
                    ...
                    decimal NguoiKCID = 0;
                    ADS_DON_DUONGSU DuongSu = dt.ADS_DON_DUONGSU.Where(x => x.DONID == DonID && x.ID_DUONGSU_TACC == Guid_NguoiKC).FirstOrDefault();
                    if (DuongSu != null)
                    {
                        NguoiKCID = DuongSu.ID;
                    }
                    ADS_SOTHAM_KHANGCAO kc = dt.ADS_SOTHAM_KHANGCAO.Where(x => x.DONID == DonID && x.DUONGSUID == NguoiKCID && x.TOAANRAQDID == ToaAnID).FirstOrDefault();
                    if (kc != null)
                    {
                        IsNew = true;
                        kc = new ADS_SOTHAM_KHANGCAO();
        ...
        private decimal Action_ChuyenAn_KhongThuoc_ThamQuyen(decimal DonID, string LoaiVuViec, decimal ToaNhanID, string MaToaNhan, string NguoiTao)
        {
            ...
            DataTable tbl = Cls_Comon.GetTableByProcedurePaging("ADS_CVA_KHONG_THAM_QUYEN", parameters);
            decimal Result = 0;
            if (tbl.Rows.Count > 0)
        ...
        private void HuyNhan(List<decimal> lstDonID)
        {
            ...
            //            ADS_DON oT = dt.ADS_DON.Where(x => x.ID == DonID_New).FirstOrDefault();
            ...
            for (int i = 0; i < lstDonID.Count; i++)
            {
                ...
                if (ObjChuyenAn != null)
                {
                    ...
                    if (ObjChuyenAn.MAP_VUANID_NEW.GetValueOrDefault(0) > 0)
                    {
                        ...
                        if (rdbLoai.SelectedValue == "0")
                        {
                            ADS_DON_THAMPHAN thamPhan = dt.ADS_DON_THAMPHAN.Where(x => x.DONID == DonID_New && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETDON).FirstOrDefault();
                            if (thamPhan != null)
                            {
                                IsThuLy = true;
