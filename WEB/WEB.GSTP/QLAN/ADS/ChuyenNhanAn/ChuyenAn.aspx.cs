...
namespace WEB.GSTP.QLAN.ADS.ChuyenNhanAn
{
    public partial class ChuyenAn : System.Web.UI.Page
    {
        ...
        private void LoadGrid()
        {
            ...
            DataTable oDT = oBL.DS_CHUYENAN(vDonViID, txtTenToa.Text.Trim(), txtMaVuViec.Text.Trim(), txtTenVuViec.Text.Trim(), txtSoquyetdinh.Text.Trim(), txtSobanan.Text.Trim(), dFrom, dTo, txtTenduongsu.Text.Trim(), Convert.ToDecimal(rdbTrangthai.SelectedValue), Convert.ToDecimal(rdbLoai.SelectedValue));
        ...
        protected void cmdLuu_Click(object sender, EventArgs e)
        {
            ...
            foreach (RepeaterItem item in rptCapNhat.Items)
            {
                ...
                try
                {
                    ...
                    ////update MAGIAIDOAN de khong cho phuc tham sua du lieu
                    //if (oIT != null && oIT.MA == ENUM_TRUONGHOP_GIAONHAN.XETXULAI_CAPSOTHAM && oDon.MAGIAIDOAN == ENUM_GIAIDOANVUAN.PHUCTHAM)
                    //{
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                try
                {
                    ...
                    if (rdbLoai.SelectedValue == "0")//Chuyển nhận đơn
                    {
                        ...
                        {
                            ADS_DON_XULY dxl = dt.ADS_DON_XULY.Where(x => x.DONID == ID).OrderByDescending(x => x.ID).FirstOrDefault();
                    ...
                    else
                    {
                        ...
                        {
                            ADS_SOTHAM_THULY oTL = dt.ADS_SOTHAM_THULY.Where(x => x.DONID == ID).OrderByDescending(y => y.NGAYTHULY).Take(1).FirstOrDefault();
                            if (oTL != null) lstNgaythuly.Text = ((DateTime)oTL.NGAYTHULY).ToString("dd/MM/yyyy");
                            if (rdbTrangthai.SelectedValue == "0")
                            {
                                bool flag = false;

                                List<ADS_SOTHAM_KHANGCAO> lstkc = dt.ADS_SOTHAM_KHANGCAO.Where(x => x.DONID == ID && (x.TINHTRANG_GIAIQUYET == 0 || x.TINHTRANG_GIAIQUYET == null)).OrderBy(y => y.ISQUAHAN).ToList();
                                List<ADS_SOTHAM_KHANGNGHI> lstkn = dt.ADS_SOTHAM_KHANGNGHI.Where(x => x.DONID == ID && (x.TINHTRANG_GIAIQUYET == 0 || x.TINHTRANG_GIAIQUYET == null)).ToList();
                                ADS_SOTHAM_KHANGCAO okc = dt.ADS_SOTHAM_KHANGCAO.Where(x => x.DONID == ID && (x.TINHTRANG_GIAIQUYET == 0 || x.TINHTRANG_GIAIQUYET == null)).OrderByDescending(y => y.ISQUAHAN).FirstOrDefault();
                                ...
                                if (flag == false)
                                {
                                    ...
                                    if (oDMQD != null)
                                    {
                                        ...
                                        ADS_SOTHAM_QUYETDINH oQD = dt.ADS_SOTHAM_QUYETDINH.Where(x => x.DONID == ID && x.LOAIQDID == oDMQD.ID && x.TOAANID == IDToa).OrderByDescending(y => y.NGAYQD).FirstOrDefault();
                                        if (oQD != null)
                                        {
                            ...
                            else
                            {
                                ...
                                {
                                    ADS_DON_XULY dxl = dt.ADS_DON_XULY.Where(x => x.DONID == ID).FirstOrDefault();
                                    strNoiDung = dxl.LYDO;
                                }
                            }
                        }
                        ...
                        {
                            ADS_PHUCTHAM_THULY oTL = dt.ADS_PHUCTHAM_THULY.Where(x => x.DONID == ID).OrderByDescending(y => y.NGAYTHULY).Take(1).FirstOrDefault();
                            if (oTL != null) lstNgaythuly.Text = ((DateTime)oTL.NGAYTHULY).ToString("dd/MM/yyyy");
                            if (rdbTrangthai.SelectedValue == "0")
                            {
                                //DM_KETQUA_PHUCTHAM oKQPT = dt.DM_KETQUA_PHUCTHAM.Where(x => x.MA == "06").FirstOrDefault();
                                //List<ADS_PHUCTHAM_BANAN> lstba = dt.ADS_PHUCTHAM_BANAN.Where(x => x.DONID == ID && x.KETQUAPHUCTHAMID == oKQPT.ID).ToList();
                                //DM_KETQUA_PHUCTHAM oKQPT2 = dt.DM_KETQUA_PHUCTHAM.Where(x => x.MA == "04").FirstOrDefault();
                                //List<ADS_PHUCTHAM_BANAN> lstba2 = dt.ADS_PHUCTHAM_BANAN.Where(x => x.DONID == ID && x.KETQUAPHUCTHAMID == oKQPT2.ID).ToList();
                                //List<ADS_PHUCTHAM_QUYETDINH> lstQD = dt.ADS_PHUCTHAM_QUYETDINH.Where(x => x.DONID == ID && x.KETQUAID == 103).ToList();
                                ...
                                List<ADS_PHUCTHAM_BANAN> lstba = dt.ADS_PHUCTHAM_BANAN.Where(x => x.DONID == ID && (x.KETQUAPHUCTHAMID == 4 || x.KETQUAPHUCTHAMID == 22)).ToList();
                                List<ADS_PHUCTHAM_QUYETDINH> lstqd = dt.ADS_PHUCTHAM_QUYETDINH.Where(x => x.DONID == ID && x.KETQUAID == 103).ToList();
                        ...
                        {
                            //ADS_KCKNQDK_PHUCTHAM_THULY oTL = dt.ADS_KCKNQDK_PHUCTHAM_THULY.Where(x => x.DONID == ID).OrderByDescending(y => y.NGAYTHULY).Take(1).FirstOrDefault();
                            ADS_KCKNQDK_PHUCTHAM_THULY oTL = DataExtensions.GetAllByDonId<ADS_KCKNQDK_PHUCTHAM_THULY>(ID).OrderByDescending(y => y.NGAYTHULY).Take(1).FirstOrDefault();
                            if (oTL != null) lstNgaythuly.Text = ((DateTime)oTL.NGAYTHULY).ToString("dd/MM/yyyy");
                            if (rdbTrangthai.SelectedValue == "0")
                            {
                                //List<ADS_KCKNQDK_PHUCTHAM_QUYETDINH> lstqd = dt.ADS_KCKNQDK_PHUCTHAM_QUYETDINH.Where(x => x.DONID == ID && (x.KETQUAID == 101 || x.KETQUAID == 102 || x.KETQUAID == 103)).ToList();
                                List<ADS_KCKNQDK_PHUCTHAM_QUYETDINH> lstqd = DataExtensions.GetAllWithClause<ADS_KCKNQDK_PHUCTHAM_QUYETDINH>($"DONID = {ID} AND KETQUAID IN (101,102,103)");
                                if (lstqd.Count > 0)
                                {
            ...
            for (int i = 0; i < lstChuyenAnID.Count; i++)
            {
                ...
                if (oChuyenAn != null)
                {
                    ...
                    if (oChuyenAn.TRANGTHAI != 1) // Án chưa nhận
                    {
                        ...
                        if (oGD == null)
                        {
                            oGD = dt.ADS_DON_GIAIDOAN.Where(x => x.DONID == DonID && x.TOAPHUCTHAMID == oChuyenAn.TOACHUYENID).FirstOrDefault();
                        }
        ...
        protected void rptCapNhat_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (hddTHGN.Value == "05")
                {
                    ...
                    {
                        ADS_DON oDonST = dt.ADS_DON.Where(x => x.MAVUVIEC == mavuviec && x.TOAANID != toaanST).FirstOrDefault();
