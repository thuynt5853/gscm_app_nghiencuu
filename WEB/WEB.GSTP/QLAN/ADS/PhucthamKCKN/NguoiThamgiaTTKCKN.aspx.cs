...
{
    public partial class NguoiThamgiaTTKCKN : System.Web.UI.Page
    {
        ...
        private void CheckQuyen(decimal DONID)
        {
            ...
            //ADS_PHUCTHAM_BL oBL = new ADS_PHUCTHAM_BL();
            //DataTable oDT = oBL.ADS_KCKNQDK_PHUCTHAM_HDXX_GETLIST(DONID);
            //if (oDT != null && oDT.Rows.Count > 0)
            //{
        ...
        public void GetDuongSu(string strlstId = "")
        {
            ...
            if (string.IsNullOrEmpty(strlstId))
            {
                ...
                var lstDuongSu = (from a in dt.ADS_DON_DUONGSU.Where(s => s.DONID == donId)
        ...
        private bool CheckValid()
        {
            ...
            {
                lbthongbao.Text = "Bạn phải nhập ngày tham gia theo mẫu như sau (ngày/tháng/năm).";
            ...
            {
                lbthongbao.Text = "Bạn phải nhập ngày sinh theo mẫu như sau (ngày/tháng/năm).";
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string current_id = Session[ENUM_LOAIAN.AN_DANSU] + "";
                decimal DONID = Convert.ToDecimal(current_id);
                ADS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG oND = null;
                if (hddid.Value == "" || hddid.Value == "0")
                    oND = new ADS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG();
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    //oND = dt.ADS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG.Where(x => x.ID == ID).FirstOrDefault();
                    oND = DataExtensions.FindById<ADS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG>(ID);
                    // oND = listtgtt.Where(x => x.ID == ID).FirstOrDefault();
                }
                ...
                var data = (from s in dt.ADS_DON_DUONGSU
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    //dt.ADS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG.Add(oND);
        ...
        public void LoadGrid()
        {
            ...
            string current_id = Session[ENUM_LOAIAN.AN_DANSU] + "";
            decimal ID = Convert.ToDecimal(current_id);
            DataTable oDT = oBL.ADS_PHUCTHAM_KCKN_TGTT_GETLIST(ID);
        ...
        {
            //ADS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG oND = dt.ADS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG.Where(x => x.ID == id).FirstOrDefault();
            ADS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG oND = DataExtensions.FindById<ADS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG>(id);
            //  ADS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG oND = listtgtt.Where(x => x.ID == id).FirstOrDefault();
            if (oND != null)
            {
                DataExtensions.Delete(oND);
                //dt.ADS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG.Remove(oND);
                ////listtgtt.Remove(oND);
                //dt.SaveChanges();
                dgList.CurrentPageIndex = 0;
                LoadGrid();
        ...
        protected void cmdChonTGTT_Click(object sender, EventArgs e)
        {
            ...
            foreach (ListItem oItem in chkListTGTT.Items)
            {
                if (oItem.Selected)
                {
                    ...
                    if (dt.ADS_PHUCTHAM_THAMGIATOTUNG.Where(x => x.NTGTT_DONKK_ID == NID).ToList().Count == 0)
                    {
                        ADS_DON_THAMGIATOTUNG oD = dt.ADS_DON_THAMGIATOTUNG.Where(x => x.ID == NID).FirstOrDefault();
                        ADS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG oS = new ADS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG();
                        oS.DONID = oD.DONID;
                        oS.HOTEN = oD.HOTEN;
        ...
        {
            //------------------
            ADS_NGUOITHAMGIATOTUNG_BL oBL = new ADS_NGUOITHAMGIATOTUNG_BL();
            decimal sodk_ = 0;
            oBL.SO_DK_PHUCTHAMKCKN_RETURN(v_stpt, ref sodk_, Session[ENUM_SESSION.SESSION_DONVIID] + "");
        ...
        protected void btn_GXN_NBC_Click(object sender, EventArgs e)
        {
            ...
            {
                //--------------
                ADS_NGUOITHAMGIATOTUNG_BL objBL = new ADS_NGUOITHAMGIATOTUNG_BL();
