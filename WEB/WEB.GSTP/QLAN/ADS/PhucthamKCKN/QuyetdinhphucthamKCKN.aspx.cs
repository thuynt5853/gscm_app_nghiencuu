...
{
    public partial class QuyetdinhphucthamKCKN : System.Web.UI.Page
    {
        ...
        private void CheckQuyen(decimal DONID)
        {
            ...
            DataTable oDT = oBL.ADS_PHUCTHAMKCKNQDK_BANANQUYETDINH_GETLIST(DONID);
            ...
            //if (oQD != null)
            //{
            //    lbthongbao.Text = "Vụ việc đã có đã có quyết định và kết quả , không được sửa đổi !";
            //    Cls_Comon.SetButton(cmdUpdate, false);
            //}
            //ADS_KCKNQDK_PHUCTHAM_QUYETDINH oQD66 = DataExtensions.GetAllWithClause<ADS_KCKNQDK_PHUCTHAM_QUYETDINH>($"DONID = {DONID} AND QUYETDINHID = 64").FirstOrDefault();
            //if (oQD66 != null)
            //{
        ...
        private void SetNewSoQD()
        {
            ...
            if (!String.IsNullOrEmpty(txtNgayQD.Text))
                ...
                ngayBD = DateTime.Now;
            txtSoQD.Text = oSTBL.GET_SQD_NEW(DonViID, "ADS_PT", ngayBD, LoaiQD).ToString();
        }
        ...
        private void LoadQD()
        {
            ...
            DataTable oDT = oBL.ADS_DM_QUYETDINH_VUAN_PT();

            //toancau_linhnd_sửa lại list quyết định
            List<string> idsToDelete = new List<string>() { "145", "146", "212", "213" };
            int[] idsToAdd = new int[] { 270, 271 };

            List<ListItem> listqd = new List<ListItem>();
            foreach (DataRow row in oDT.Rows)
            {
                if (row["ID"].ToString() != "64")
                {
                    listqd.Add(new ListItem()
                    {
            ...
            listqd = listqd.Where(x => !idsToDelete.Contains(x.Value)).ToList();
            var listAdd = dt.DM_QD_QUYETDINH.Where(x => x.ID == 270 || x.ID == 271).ToList();
            listqd = listqd.Concat(listAdd.Select(x => new ListItem() { Value = x.ID.ToString(), Text = x.TEN })).OrderBy(x => x.Text).ToList();

            //ddlQuyetdinh = new DropDownList();
            ddlQuyetdinh.Items.Clear();
            ddlQuyetdinh.Items.AddRange(listqd.ToArray());
        ...
        private void LoadNguoiKyInfo()
        {
            ...
            //ADS_KCKNQDK_PHUCTHAM_HDXX oND = dt.ADS_KCKNQDK_PHUCTHAM_HDXX.Where(x => x.DONID == DonID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).OrderByDescending(x => x.ID).FirstOrDefault<ADS_KCKNQDK_PHUCTHAM_HDXX>();
            ADS_KCKNQDK_PHUCTHAM_HDXX oND = DataExtensions.GetAllWithClause<ADS_KCKNQDK_PHUCTHAM_HDXX>($"DONID ={DonID} AND MAVAITRO ='{ENUM_NGUOITIENHANHTOTUNG.THAMPHAN}'").OrderByDescending(x => x.ID).FirstOrDefault();
            if (oND != null)
            {
        ...
        private bool CheckValid()
        {
            ...
            if (!String.IsNullOrEmpty(txtNgayQD.Text))
            {
                ...
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal LoaiQD = Convert.ToDecimal(ddlQuyetdinh.SelectedValue);
                Decimal CheckID = oSTBL.CHECK_SQDTheoLoaiAn(DonViID, "ADS_PT", so, ngay, LoaiQD);
                if (CheckID > 0)
                {
                    String strMsg = "";
                    String STTNew = oSTBL.GET_SQD_NEW(DonViID, "ADS_PT", ngay, LoaiQD).ToString();
            ...
            if (hddFilePath.Value != "")
            {
                try
                {
                    ...
                    using (FileStream fs = File.OpenRead(strFilePath))
                    {
                        ...
                        objFile.TENFILE = Cls_Comon.ChuyenTenFileUpload(strTenBM);
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    soqd = STTQD.ToString() + "/" + DateTime.Now.Year.ToString();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    //oND = dt.ADS_KCKNQDK_PHUCTHAM_QUYETDINH.Where(x => x.ID == ID).FirstOrDefault();
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.ADS_KCKNQDK_PHUCTHAM_QUYETDINH_GETLIST(ID);
        ...
        public void xoa(decimal id)
        {
            ...
            //Luu thong tin truoc khi xoa Quyet dinh Phuc tham
            string strUserName = Session[ENUM_SESSION.SESSION_USERNAME] + "";
            var json = new JavaScriptSerializer().Serialize(oND);
            ADS_DON_BL oBL = new ADS_DON_BL();
            if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.DONID), Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_DANSU), Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Quyet dinh Phúc thẩm án Dân sự", "Xóa", json) == false)
            {
                lbthongbao.Text = "Xóa Quyet dinh Phúc thẩm không thành công!";
                return;
            }//Ket thuc
            //Xoa quyet dinh Phuc tham
        ...
        {
            decimal ID = Convert.ToDecimal(hddid.Value);
            //ADS_KCKNQDK_PHUCTHAM_QUYETDINH oQD = dt.ADS_KCKNQDK_PHUCTHAM_QUYETDINH.Where(x => x.ID == ID).FirstOrDefault();
            ADS_KCKNQDK_PHUCTHAM_QUYETDINH oQD = DataExtensions.FindById<ADS_KCKNQDK_PHUCTHAM_QUYETDINH>(ID);
            ...
            try
            {
                ...
                if (oT != null)
                {
                    ...
                    if (ID == 143 || ID == 144 || ID == 145 || ID == 146 || ID == 61)
                    {
                        ...
                        ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                        String STTNew = oSTBL.GET_SQD_NEW(DonViID, "ADS_PT", ngayQD, ID).ToString();
                        txtSoQD.Text = STTNew;
                    }
                }
                LoadLydo();
