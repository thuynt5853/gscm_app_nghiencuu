...
{
    public partial class ThuLyKCKN : System.Web.UI.Page
    {
        private GSTPContext dt = new GSTPContext();
        private CultureInfo cul = new CultureInfo("vi-VN");

        // List<ADS_KCKNQDK_PHUCTHAM_THULY> list = new List<ADS_KCKNQDK_PHUCTHAM_THULY>();
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    //ddlQuyetDinh.Visible = false;
                    //quyetdinhId.SelectedValue = "0";
                    LoadNguoiKyDdlInfo();
                    LoadQD();
                    LoadGrid();
                    //LoadQuyetDinhInfo();
                    ...
                    {
                        ADS_KCKN_PHUCTHAM_BL oBL = new ADS_KCKN_PHUCTHAM_BL();
                        DataTable oDT = oBL.ADS_PHUCTHAM_THULY_GETLIST(ID);
        ...
        private void CheckShowCommand(decimal DonID)
        {
            ...
            if (oT != null)
            {
                ...
                ADS_KCKN_PHUCTHAM_BL oBL = new ADS_KCKN_PHUCTHAM_BL();
                DataTable oDT = oBL.ADS_KCKN_DON_THAMPHAN_GETBY(DonID, ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM);
                if (oDT != null && oDT.Rows.Count > 0)
                {
                    bool exists = false;
                ...
                ADS_SOTHAM_BL objST = new ADS_SOTHAM_BL();
                if (oT.MAGIAIDOAN == ENUM_GIAIDOANVUAN.SOTHAM && objST.ADS_SOTHAM_KCaoKNghi_GETLIST(DonID).Rows.Count == 0)
                {
                ...
                //có quyết định ko thể sửa đổi
                //ADS_KCKNQDK_PHUCTHAM_QUYETDINH ba = dt.ADS_KCKNQDK_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DonID).FirstOrDefault();
                ADS_KCKNQDK_PHUCTHAM_QUYETDINH ba = DataExtensions.GetAllByDonId<ADS_KCKNQDK_PHUCTHAM_QUYETDINH>(DonID).FirstOrDefault();
                if (ba != null && oDT.AsEnumerable()
               .Any(row => row["TOA_GIAIQUYET_ID"].ToString() == currenttoaid.ToString()))
                {
                    lbthongbao.Text = "Đã có kết quả giải quyết. Không được sửa đổi !";
                    Cls_Comon.SetButton(cmdUpdate, false);
                    //Cls_Comon.SetButton(cmdLammoi, false);
                    hddIsShowCommand.Value = "False";
                    return;
        ...
        private void LoadCombobox()
        {
            ...
            List<ADS_CHUYEN_NHAN_AN> listcna = dt.ADS_CHUYEN_NHAN_AN.Where(x => x.MAP_VUANID_NEW == idAn && x.TOANHANID == toaanId).ToList();
            if (listcna.Count > 0 )
            {
        ...
        private void SetNewSoThuLy()
        {
            ...
            //toancau_linhnd_sửa lỗi số thụ lý bị trùng
            txtSoThuly.Text = oSTBL.GET_STL_NEW(DonViID, "ADS_PTQDK", ngaythuly).ToString();
            SetNewSoThongbao();
        }
        ...
        private void SetNewSoThongbao()
        {
            ...
            //toancau_linhnd_sửa lỗi số thông báo bị trùng
            txtSothongbao.Text = oSTBL.GET_STBTL_NEW(DonViID, "ADS_PTQDK", ngaythongbao).ToString();
        }
        ...
        private void ResetControls()
        {
            ...
            try
            {
                ddlLoaiQuanhe.SelectedIndex = ddlLoaiThuLy.SelectedIndex = 0;
                ddlQHPLTK.SelectedIndex = ddlQuanhephapluat.SelectedIndex = 0;
            }
            catch (Exception ex) { }
        ...
        private bool CheckValid()
        {
            ...
            decimal TOAANID = Convert.ToDecimal(current_id);
            bool check = true;
            ...
            if (!String.IsNullOrEmpty(txtNgaythuly.Text))
            {
                ...
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal CheckID = oSTBL.CheckSoTLTheoLoaiAn(DonViID, "ADS_PTQDK", sothuly, ngaythuly);
                //List<ADS_KCKNQDK_PHUCTHAM_THULY> list = new List<ADS_KCKNQDK_PHUCTHAM_THULY>();
                //list = dt.ADS_KCKNQDK_PHUCTHAM_THULY.Where(x => x.TOAANID == TOAANID).ToList();
                //var max = list.Max(x => x.SOTHULY).toNumber();
                ...
                if (CheckID > 0)
                {
                    ...
                    String strMsg = "";
                    String STTNew = oSTBL.GET_STL_NEW(DonViID, "ADS_PTQDK", ngaythuly).ToString();
                ...
                //var stl = txtSoThuly.Text.toNumber();
                //foreach (var item in list)
                //{
                //    if (stl == item.SOTHULY.toNumber())
                //    {
                //        String strMsg = "";
                //        strMsg = "Số thụ lý " + txtSoThuly.Text + " đã có trong hệ thống. Bạn có thể dùng số " + (max + 1).ToString();
                //        txtSoThuly.Text = (max + 1).ToString();
                //        txtSothongbao.Text = (max + 1).ToString();
                //        ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('" + strMsg + "')", true);
                //        txtSoThuly.Focus();
                //        check = true;
                //        return false;
            ...
            if (!String.IsNullOrEmpty(txtNgaythuly.Text))
            {
                ...
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal CheckID = oSTBL.CheckSoTBTLTheoLoaiAn(DonViID, "ADS_PTQDK", sothongbao, ngaythuly);
                //List<ADS_KCKNQDK_PHUCTHAM_THULY> list = new List<ADS_KCKNQDK_PHUCTHAM_THULY>();
                //list = dt.ADS_KCKNQDK_PHUCTHAM_THULY.Where(x => x.TOAANID == TOAANID).ToList();
                //var max = list.Max(x => x.SOTHONGBAO).toNumber();
                if (CheckID > 0)
                {
                    Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
                    String strMsg = "";
                    String STTNew = oSTBL.GET_STBTL_NEW(DonViID, "ADS_PTQDK", ngaythuly).ToString();
                    if (CheckID != CurrThuLyID)
                    {
                        //lbthongbao.Text = "Số thụ lý này đã có!";
                        strMsg = "Số Thông báo thụ lý" + txtSothongbao.Text + " đã có trong hệ thống. Bạn có thể dùng số " + STTNew.ToString();
                ...
                //if (check == true) txtSothongbao.Text = (max + 1).ToString();
                //var stb = txtSothongbao.Text.toNumber();
                //foreach (var item in list)
                //{
                //    if (stb == item.SOTHONGBAO.toNumber())
                //    {
                //        String strMsg = "";
                //        strMsg = "Số Thông báo thụ lý " + txtSothongbao.Text + " đã có trong hệ thống. Bạn có thể dùng số " + (max + 1).ToString();
                //        txtSothongbao.Text = (max + 1).ToString();
                //        txtSoThuly.Text = (max + 1).ToString();
                //        ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('" + strMsg + "')", true);
                //        txtSothongbao.Focus();
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                decimal FileID = 0;
                ADS_KCKN_PHUCTHAM_BL oPTBL = new ADS_KCKN_PHUCTHAM_BL();
                ADS_KCKNQDK_PHUCTHAM_THULY oND;
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    oND = new ADS_KCKNQDK_PHUCTHAM_THULY();
                    //oND.SOTHONGBAO = objDBL.GETFILENEWTT((decimal)oDon.TOAANID, (decimal)ENUM_GIAIDOANVUAN.PHUCTHAM, DateTime.Now.Year, 0).ToString();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    //oND = dt.ADS_KCKNQDK_PHUCTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
                    oND = DataExtensions.FindById<ADS_KCKNQDK_PHUCTHAM_THULY>(ID);
                ...
                //quyetdinhId.SelectedValue = (string.IsNullOrEmpty(oND.TK_QUYETDINH + "")) ? "0" : oND.TK_QUYETDINH.ToString();
                //if (quyetdinhId.SelectedValue == "1")
                //    ddlQuyetDinh.Visible = true;
                //else
                //    ddlQuyetDinh.Visible = false;
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    oND.TT = oPTBL.THULY_GETNEWTT(CurrDonViID);
                    oND.MATHULY = "P" + ENUM_LOAIVUVIEC.AN_DANSU + Session[ENUM_SESSION.SESSION_MADONVI] + oND.TT.ToString();
                    oND.NGAYTAO = DateTime.Now;
                    oND.NGUOITAO = CurrUserName + "";
                    if (CurrDonViID > 0)
                        oND.TOAANID = CurrDonViID;
                    if (oND.TOA_GIAIQUYET_ID == null)
                    {
                        oND.TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                    }
                    DataExtensions.Insert(oND);
                    //dt.ADS_KCKNQDK_PHUCTHAM_THULY.Add(oND);
                    //dt.SaveChanges();
        ...
        {
            //ADS_KCKNQDK_PHUCTHAM_THULY oND = dt.ADS_KCKNQDK_PHUCTHAM_THULY.Where(x => x.ID == id).FirstOrDefault();
            ADS_KCKNQDK_PHUCTHAM_THULY oND = DataExtensions.FindById<ADS_KCKNQDK_PHUCTHAM_THULY>(id);
            if (oND != null)
            {
                decimal FileID = 0;
                if (oND.FILEID != null) FileID = (decimal)oND.FILEID;

                //Luu thong tin truoc khi xoa Thu ly Phuc tham
                ...
                if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.DONID), Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_DANSU), Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Thu lý Phúc thẩm án Dân sự", "Xóa", json) == false)
                {
                    lbthongbao.Text = "Xóa Thụ lý Phúc thẩm không thành công!";
                    return;
                }//Ket thuc

                //Xoa Thu ly Phuc Tham
                DataExtensions.Delete(oND);
                //dt.ADS_KCKNQDK_PHUCTHAM_THULY.Remove(oND);
                //dt.SaveChanges();
                if (FileID > 0)
                {
                    try
        ...
        public void loadedit(decimal ID)
        {
            //ADS_KCKNQDK_PHUCTHAM_THULY oND = dt.ADS_KCKNQDK_PHUCTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
            ADS_KCKNQDK_PHUCTHAM_THULY oND = DataExtensions.FindById<ADS_KCKNQDK_PHUCTHAM_THULY>(ID);
            //ADS_KCKNQDK_PHUCTHAM_THULY oND = list.Where(x => x.ID == ID).FirstOrDefault();
            if (oND != null)
            {
                try
                {
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                {
                    case "Download":
                        var oND = DataExtensions.FindById<ADS_KCKNQDK_PHUCTHAM_THULY>(ND_id);
                        if (oND.TENFILE != "")
                        {
        ...
        //protected void quyetdinhId_SelectedIndexChanged(object sender, EventArgs e)
        //{
        //    if (quyetdinhId.SelectedValue == "1")
        //    {
        //        ddlQuyetDinh.Visible = true;
        ...
        //toancau_linhnd_load quyết định
        private void LoadQD()
        {
            string current_id = Session[ENUM_LOAIAN.AN_DANSU] + "";
            decimal idAn = Convert.ToDecimal(current_id);
            ADS_CHUYEN_NHAN_AN_BL aDS_CHUYEN_NHAN_AN_BL = new ADS_CHUYEN_NHAN_AN_BL();
            decimal donOld = aDS_CHUYEN_NHAN_AN_BL.getDonIdOld(idAn);

            ADS_SOTHAM_KHANGCAO aDS_SOTHAM_KHANGCAO = dt.ADS_SOTHAM_KHANGCAO.Where(x => x.DONID == donOld && x.LOAIKHANGCAO == 2 && x.TINHTRANG_GIAIQUYET != 1).FirstOrDefault();
            ADS_SOTHAM_KHANGNGHI oKN = dt.ADS_SOTHAM_KHANGNGHI.Where(x => x.DONID == donOld && x.LOAIKN == 2 && x.TINHTRANG_GIAIQUYET != 1).FirstOrDefault();
            if (aDS_SOTHAM_KHANGCAO != null)
            {
                ADS_SOTHAM_QUYETDINH aDS_SOTHAM_QUYETDINH = dt.ADS_SOTHAM_QUYETDINH.Where(x => x.ID == aDS_SOTHAM_KHANGCAO.SOQDBA).FirstOrDefault();
                if (aDS_SOTHAM_QUYETDINH != null)
                {
                    DM_QD_QUYETDINH dM_QD_QUYETDINH = dt.DM_QD_QUYETDINH.Where(x => x.ID == aDS_SOTHAM_QUYETDINH.QUYETDINHID).FirstOrDefault();
                    txtQDST.Text = "Số " + aDS_SOTHAM_QUYETDINH.SOQD + " - " + dM_QD_QUYETDINH.TEN;
                }
            }
            else if (oKN != null)
            {
                ADS_SOTHAM_QUYETDINH aDS_SOTHAM_QUYETDINH = dt.ADS_SOTHAM_QUYETDINH.Where(x => x.ID == oKN.BANANID).FirstOrDefault();
                if (aDS_SOTHAM_QUYETDINH != null)
                {
                    DM_QD_QUYETDINH dM_QD_QUYETDINH = dt.DM_QD_QUYETDINH.Where(x => x.ID == aDS_SOTHAM_QUYETDINH.QUYETDINHID).FirstOrDefault();
                    txtQDST.Text = "Số " + aDS_SOTHAM_QUYETDINH.SOQD + " - " + dM_QD_QUYETDINH.TEN;
            ...
            //List<ADS_PHUCTHAM_QUYETDINH> idPT = new List<ADS_PHUCTHAM_QUYETDINH>();
            //idPT = dt.ADS_PHUCTHAM_QUYETDINH.Where(x => x.DONID == idAn).ToList();
            //List<ADS_SOTHAM_QUYETDINH> idST = new List<ADS_SOTHAM_QUYETDINH>();
            //idST = dt.ADS_SOTHAM_QUYETDINH.Where(x => x.DONID == idAn).ToList();

            //if (idPT.Count() != 0)
            //{
            //    List<DM_QD_QUYETDINH> qd = new List<DM_QD_QUYETDINH>();
            //    var count = 0;
            //    foreach (var item in idPT)
            //    {
            //        qd = dt.DM_QD_QUYETDINH.Where(x => x.ID == item.QUYETDINHID).ToList();
            //        ddlQuyetDinh.Items.Insert(count, new ListItem(qd.FirstOrDefault().TEN));
            //        ddlQuyetDinh.SelectedIndex = count;
            //        count++;
            //    }
            //}
            //else if (idST.Count() != 0)
            //{
            //    List<DM_QD_QUYETDINH> qd = new List<DM_QD_QUYETDINH>();
            //    var count = 0;
            ...
            //    List<decimal> checkQD = new List<decimal>() { 42, 45 };
            //    if (idST != null && idST.Count() > 0)
            //    {
            //        var countQDTDC = idST.Where(x => x.QUYETDINHID == 42 || x.QUYETDINHID == 45).Count();
            //        if (countQDTDC == idST.Count())
            //        {
            //            //tdc
            //            quyetdinhId.SelectedValue = "0";
            //            khac.Enabled = false;
            //            dinhchi.Enabled = true;
            //            ddlQuyetDinh.Visible = false;
            //        }
            //        else if (countQDTDC == 0)
            //        {
            //            //disable tdc
            //            quyetdinhId.SelectedValue = "1";
            //            khac.Enabled = true;
            //            dinhchi.Enabled = false;
            //            ddlQuyetDinh.Visible = true;
            //        }
            //        else if (countQDTDC < idST.Count())
            //        {
            //            //hiện cả 2
            //            quyetdinhId.SelectedValue = "0";
            //        }
