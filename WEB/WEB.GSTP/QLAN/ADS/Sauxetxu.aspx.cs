...
namespace WEB.GSTP.QLAN.ADS
{
    public partial class Sauxetxu : System.Web.UI.Page
    {
        ...
        private void LoadGrid()
        {
            ...
            DataTable oDT = oBL.ADS_RUTKN(vDonViID, txtMaVuViec.Text, txtTenVuViec.Text, txtSoQDBA.Text, txtTenDuongSu.Text, dFrom, dTo, Convert.ToDecimal(rdbTrangthai.SelectedValue), pageIndex, pageSize);
            int Total = 0;
        ...
        protected void cmdCapnhat_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    decimal vuViecID = 0; ADS_SAUXETXU obj; bool isNew = false; ADS_DON don;
                    foreach (DataRow row in dsSelected.Rows)
                    {
                        vuViecID = Convert.ToDecimal(row["ID"].ToString());
                        obj = dt.ADS_SAUXETXU.Where(x => x.VUANID == vuViecID).FirstOrDefault<ADS_SAUXETXU>();
                        if (obj == null)
                        {
                            isNew = true;
                            obj = new ADS_SAUXETXU();
                        ...
                        obj.VUANID = vuViecID;
                        don = dt.ADS_DON.Where(x => x.ID == vuViecID).FirstOrDefault<ADS_DON>();
                        if (don != null)
                        {
        ...
        private void LoadGridSelected()
        {
            ...
            if (dsSelected != null && dsSelected.Rows.Count > 0)
            {
                ...
                ADS_SAUXETXU sx = dt.ADS_SAUXETXU.Where(x => x.VUANID == DonID).FirstOrDefault<ADS_SAUXETXU>();
                if (sx != null)
                {
        ...
        public void loadedit(decimal ID, String MAGIAIDOAN)
        {
            ...
            if (MAGIAIDOAN == "Sơ thẩm")
                oND = dt.ADS_SAUXETXU.Where(x => x.VUANID == ID && x.ST_ISRUTKN == 1).FirstOrDefault();
            else
                oND = dt.ADS_SAUXETXU.Where(x => x.VUANID == ID && x.PT_ISRUTKN == 1).FirstOrDefault();

            if (oND != null)
            {
                if (dsSelected != null) dsSelected.Clear();
                ADS_DON oVA = dt.ADS_DON.Where(x => x.ID == oND.VUANID).FirstOrDefault();
