...
namespace WEB.GSTP.QLAN.ADS.XuLyDon
{
    public partial class Danhsach : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                try
                {
                    ...
                    ADS_DON_XULY oDXuly = dt.ADS_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 1).FirstOrDefault();
                    if (oDXuly != null)
                    {
                        ADS_CHUYEN_NHAN_AN ca = dt.ADS_CHUYEN_NHAN_AN.Where(x => x.VUANID == DONID).FirstOrDefault();
                        if (ca != null)
                        {
                    ...
                    ADS_CHUYEN_NHAN_AN objCN = dt.ADS_CHUYEN_NHAN_AN.Where(x => x.MAP_VUANID_NEW == DONID && x.TRUONGHOPGIAONHANID == 266).FirstOrDefault();
                    if (objCN != null)
        ...
        void SetNewSoTB()
        {
            ...
            String STTNew = oSTBL.GET_STB_XLDon_NEW_V2(DonViID, "ADS", ngayTB).ToString();
        ...
        {
            List<ADS_DON_XULY> lstXL = dt.ADS_DON_XULY.Where(x => x.DONID == ID && x.LOAIGIAIQUYET == 5).ToList();
            if (lstXL.Count == 0)
            {
                Cls_Comon.SetButton(cmdThemmoiAP, false);
                Cls_Comon.SetButton(cmdCapNhatAP, false);
        ...
        void LoadDropDuongSu(decimal DONID)
        {
            ...
            //Danh mục lý do tra đơn
            DataTable obj = oDSBL.ADS_DON_DUONGSU_ANPHI(DONID);
        ...
        protected void rpt_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "Them":
                    ...
                    if (DonID > 0)
                    {
                        ADS_DON_DUONGSU dds = dt.ADS_DON_DUONGSU.Where(x => x.DONID == DonID && x.ISDAIDIEN == 1 && x.TUCACHTOTUNG_MA == "NGUYENDON").FirstOrDefault();
                        txtDuongSu.Text = dds.TENDUONGSU;
                    }
                    else
                    {
                        var a = (from d in dt.DON_DUONGSU_CHITIET
                                 join s in dt.ADS_DON_DUONGSU on d.DUONGSUID equals s.ID
                ...
                case "Xoa":
                    ...
                    ADS_TONGDAT oTD = dt.ADS_TONGDAT.Where(x => x.DONID == DONID).FirstOrDefault();
                    if (oTD != null)
                    {
                    ...
                    string MATHONGBAO = "";
                    var DVCQG_THANH_TOAN = DataExtensions.GetAllWithClause<BL.GSTP.BANGSETGET.DVCQG_THANH_TOAN>("ANPHI_ID = " + CurrID + " AND MALOAIVUVIEC = '2'");
                    ...
                    if (_VALUE == 1)
                    {
                        ...
                        if (oAnphi != null)
                        {
                            ...
                            log.InsertLog(Session[ENUM_LOAIAN.AN_DANSU].ToString(), CurrID.ToString(), MATHONGBAO, Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Delete", CONTENT_JSON, CONTENT_JSON_OLD);
                        ...
                        {
                            ADS_DON_XULY_BL oBL = new ADS_DON_XULY_BL();
                            DataTable oYC = oBL.ADS_GETALL_DON_YCBS(DonID, CurrID, 1, 5);
                            if (oYC.Rows.Count > 0)
                            {
                                oBL.DEL_DON_YCBS_GETBYDONID(Convert.ToDecimal(oYC.Rows[0]["ID"].ToString()));
                            }
                        }
                ...
                case "BoSung":
                    ...
                    string link = "/QLAN/ADS/XuLyDon/Popup/pBoSungTaiLieu.aspx?ID=" + CurrID;
                ...
                case "TraLaiDon":
                    CurrID = Convert.ToDecimal(e.CommandArgument.ToString());
                    Cls_Comon.CallFunctionJS(this, this.GetType(), "LoadModalDiv();");
                    string linkPop = "/QLAN/ADS/XuLyDon/Popup/pTraLaiDon.aspx?ID=" + CurrID;
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                else
                {
                    ...
                    if (rowView["ID"] + "" != "")
                    {
                        ...
                        ADS_DON_XULY oXL = dt.ADS_DON_XULY.Where(x => x.ID == idXuLy).FirstOrDefault();
                        decimal? DONCHITIETID = oXL.DON_CHITIETID;
                        decimal ISDONCHITIET = DON_ID == 0 ? 1 : 0;
                        ADS_DON_XULY_BL oBL = new ADS_DON_XULY_BL();
                        DataTable oDT = oBL.CHECK_ADS_DON_DUONGSU_ANPHI_V2(DONID, DONCHITIETID, ISDONCHITIET);
                        if (oDT.Rows.Count > 0)
                        {
                            lblSua.Visible = false;
                            lbtXoa.Visible = false;
                    ...
                    {
                        decimal idXuLy = Convert.ToDecimal((rowView["ID"] + "").ToString());
                        ADS_DON_XULY obj = dt.ADS_DON_XULY.Where(x => x.ID == idXuLy).FirstOrDefault();
                        ADS_DON_XULY_BL oBL = new ADS_DON_XULY_BL();
                        DataTable oYC = oBL.ADS_GETALL_DON_YCBS(DONID, idXuLy, 1, 5);
                    ...
                    if (rowView["ID"] + "" != "")
                    {
                        ...
                        ADS_DON_XULY obj = dt.ADS_DON_XULY.Where(x => x.ID == idXuLy && x.LOAIGIAIQUYET == adsThuLy).FirstOrDefault();
                        //ADS_DON_XULY_BL oBL = new ADS_DON_XULY_BL();
                        //DataTable oYC = oBL.ADS_GETALL_DON_YCBS(DONID, idXuLy, 1, 5);
                    ...
                    ADS_SOTHAM_THULY oTLST = dt.ADS_SOTHAM_THULY.Where(x => x.DONID == DONID).FirstOrDefault();
                    if (oT.MAGIAIDOAN == ENUM_GIAIDOANVUAN.PHUCTHAM || oTLST != null)
                    {
                        lblSua.Visible = false;
                        lbtXoa.Visible = false;
                        lbtTraLaiDon.Visible = false;
                        //them theo nghiep vu ban giao an
                        //lbtThem.Visible = false;
                    }
                ...
                {
                    var objADS_DON_XULY = dt.ADS_DON_XULY
                        .FirstOrDefault(x => x.DONID == donId);

                    if (toagiaiquyetID != donviID) // TH: khác tòa
                    {
        ...
        private decimal UploadFileID(ADS_DON oDon, decimal FileID, string strMaBieumau, string STT, string stb_Phu)
        {
        ...
        protected void cmdCapNhat_Click(object sender, EventArgs e)
        {
            ...
            ADS_CHUYEN_NHAN_AN cna = new ADS_CHUYEN_NHAN_AN();
            decimal FileID = 0;
            ...
            else
            {
                ...
                if (txtSothongbao.Text != "")
                {
                    ...
                    decimal check = oBL.CHECKSTT_ADS((decimal)oDon.TOAANID, (decimal)oDon.MAGIAIDOAN, vNam, 0, Convert.ToDecimal(txtSothongbao.Text), ddlStbPhu.SelectedValue, obj.ID);
                    DateTime ngaythongbao = DateTime.Parse(this.txtNgaythongbao.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
                    String strMsg = "";
                    ADS_DON_BL oBL_STB = new ADS_DON_BL();
                    String STTNew = oBL_STB.GET_STB_XLDon_NEW_V2((decimal)oDon.TOAANID, "ADS", ngaythongbao).ToString();
            ...
            else if (bienphap == ENUM_ADS_BIENPHAPGQ.ADS_ChuyenDonTrongNganh)
            {
                ...
                //cna.VUANID = oDon.DONID_TOACU.HasValue ? oDon.DONID_TOACU.Value : oDon.ID;
                //cna.TOACHUYENID = oDon.TOAANID;
                //cna.TOANHANID = Convert.ToInt16(hddToaAn.Value);
                //cna.NGAYGIAO = DateTime.Parse(txtNgayGQ.Text, cul, DateTimeStyles.NoCurrentDateDefault);
                //cna.TRUONGHOPGIAONHANID = 266;
                //cna.NGUOIGIAOID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID] + "");
                //cna.TRANGTHAI = 0;
                //dt.ADS_CHUYEN_NHAN_AN.Add(cna);
                oDon.LOAIDON = 2;
                dt.SaveChanges();
        ...
        private void ChuyenDonSangToaAnMoi(ADS_DON oDon, decimal DonXyLyID, ADS_DON_XULY obj, decimal ToaAnCuEdit)
        {
            if (DonXyLyID == 0)
            {
                ...
                //update 14082025
                oDonMoi.TOA_GIAIQUYET_ID = Convert.ToDecimal(oDonMoi.TOAANID);
                ...
                foreach (var item in lstoDS)
                {
                    ...
                    if (oDS.TOA_GIAIQUYET_ID == null)
                        oDS.TOA_GIAIQUYET_ID = Convert.ToDecimal(Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]));
        ...
        private void LoadGrid_AnPhi()
        {
            ...
            DataTable oDT = oBL.ADS_ANPHI_GETBYDONID_V2(DONID, 1, pageindex, page_size);
            if (oDT.Rows.Count > 0)
            {
            ...
            switch (e.CommandName)
            {
                ...
                case "XoaAP":
                    try
                    {
                        ...
                        obj.CHECK_DELETE_ANPHI(CurrID, "2", ref _check_out);//anhvh check delete 06/09/2023
                        if (_check_out == 1)
                        {
                            ADS_ANPHI objAP = dt.ADS_ANPHI.Where(x => x.ID == CurrID).FirstOrDefault();
                            ADS_TONGDAT oTD = dt.ADS_TONGDAT.Where(x => x.DONID == objAP.DONID).FirstOrDefault();
                            DON_MIENANPHI_BL dON = new DON_MIENANPHI_BL();
                            DataTable dataTable = dON.GET_DON_MIENANPHI_BY_ANPHI_ID(objAP.ID, 2);
                            ADS_TONGDAT_BL dS_TONGDAT_BL = new ADS_TONGDAT_BL();
                            ...
                            if (oTD != null && objAP.TINHTRANG != 1 && dataTable.Rows.Count == 0)
                            {
                                ...
                                && dS_TONGDAT_BL.ADS_TONGDATDOITUONG_ANPHI_GETBYANPHIID_TONGDATID(objAP.DONID, objAP.ID).Rows.Count > 0)
                            {
                                lbThongbaoAP.Text = "Bạn không thể xóa khi đã tống đạt!";
                                return;
                            }
                            else
                            {
                                ...
                                string MATHONGBAO = "";
                                var DVCQG_THANH_TOAN = DataExtensions.GetAllWithClause<BL.GSTP.BANGSETGET.DVCQG_THANH_TOAN>("ANPHI_ID = " + objAP.ID + " AND MALOAIVUVIEC = '2'");
                                if (DVCQG_THANH_TOAN.Count() > 0)
                                {
                                    MATHONGBAO = DVCQG_THANH_TOAN.FirstOrDefault().MA_THONGBAO;
                                }
                                ...
                                dt.ADS_ANPHI.Remove(objAP);
                                dt.SaveChanges();
                                lbThongbaoAP.Text = "Xóa thành công!";
                                //DELETE DVCQG_THANH_TOAN-------------
                                decimal _VALUE = 0;
                                obj.DVCQG_THANH_TOAN_DELETE_ANPHI(CurrID, "2", ref _VALUE);
                            }
                        }
                        else
                        {
                ...
                case "LSTBAnPhi":
                    try
                    {
                        CurrID = Convert.ToDecimal(e.CommandArgument.ToString());
                        Cls_Comon.CallFunctionJS(this, this.GetType(), "LoadModalDiv();");
                        string linkPop = "/QLAN/ADS/XuLyDon/Popup/pMienAnPhi.aspx?ID=" + CurrID;
        ...
        void LoadInfo_AnPhi(Decimal CurrID)
        {
            ...
            List<ADS_ANPHI> lst = dt.ADS_ANPHI.Where(x => x.ID == CurrID).ToList<ADS_ANPHI>();
        ...
        protected void rptAP_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                DAL.GSTP.DVCQG_THANH_TOAN oAP = dt.DVCQG_THANH_TOAN.Where(x => x.ANPHI_ID == vAPID && x.MALOAIVUVIEC == "2").FirstOrDefault();
                ADS_SOTHAM_THULY oTLST = dt.ADS_SOTHAM_THULY.Where(x => x.DONID == DONID).FirstOrDefault();

                // Kiểm tra xem đơn đã được trả lại (LOAIGIAIQUYET == 3) hay chưa
                bool isDonTraLai = dt.ADS_DON_XULY.Any(x => x.DONID == DONID && x.LOAIGIAIQUYET == 3);

                ADS_ANPHI aDS_ANPHI = dt.ADS_ANPHI.Where(x => x.ID == vAPID).FirstOrDefault();

                DON_MIENANPHI_BL dON_MIENANPHI_BL = new DON_MIENANPHI_BL();
                DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(vAPID, 2);
        ...
        protected void cmdCapNhatAP_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    try
                    {
                        objAP = dt.ADS_ANPHI.Where(x => x.ID == APId).FirstOrDefault();
                        if (APId != 0)
                        {
                            objAP.NGAYSUA = DateTime.Now;
                            objAP.NGUOISUA = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                    ...
                    catch (Exception ex) { objAP = new ADS_ANPHI(); }
                    ...
                    {
                        decimal donID = Convert.ToDecimal(current_id);
                        ADS_DON_XULY donXL = dt.ADS_DON_XULY.Where(x => (x.DONID == DONID || x.DON_XULYID == DONID) && x.LOAIGIAIQUYET == 5).FirstOrDefault();
                        DVCQG_THANH_TOAN_BL obj_anphi = new DVCQG_THANH_TOAN_BL();
                        obj_anphi.DVCQG_THANH_TOAN_INSERT_ANPHI(2, donID, donXL.ID, objAP.ID, "2", Convert.ToString(objAP.DUONGSU_ID));
                    }
                    else if (APId != 0)
                    {
                        ///UPDATE vào bảng DVCQG_THANH_TOAN
                        DVCQG_THANH_TOAN_BL obj = new DVCQG_THANH_TOAN_BL();
                        obj.DVCQG_THANH_TOAN_UP_ANPHI(objAP.ID, "2");
                    }
                    ...
                    if (chkNopAnPhi.Checked == false)
                    {
                        ...
                        ADS_DON_XULY donXLS = dt.ADS_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 5).FirstOrDefault();
                        if (donXLS.LOAIGIAIQUYET == 5)
                        {
                            DataTable oDT = obj_anphiS.CHECK_THANH_TOAN_IN_ANPHI(2, donIDS, donXLS.ID, objAP.ID, "2", objAP.DUONGSU_ID + "");
                            if (oDT.Rows.Count == 0)
                            {
                                obj_anphiS.DVCQG_THANH_TOAN_INSERT_ANPHI(2, donIDS, donXLS.ID, objAP.ID, "2", objAP.DUONGSU_ID + "");
                            }
                        }
                    }
                    else
                    {
                        ...
                        ADS_DON aDS_DON = dt.ADS_DON.Where(x => x.ID == DONID).FirstOrDefault();
                        DM_BIEUMAU bmMienAnPhi = dt.DM_BIEUMAU.Where(x => x.MABM == "100-DS").FirstOrDefault();
                        ADS_FILE objFile = dt.ADS_FILE.Where(x => x.DONID == DONID && x.BIEUMAUID == bmMienAnPhi.ID).FirstOrDefault();
                        if (objFile != null)
                        {
        ...
        private bool CheckValidAP()
        {
            ...
            else
            {
                ...
                if (hddCurrAPID.Value != "" && hddCurrAPID.Value != "0")
                {
                    ...
                    ///-----------------
                    ADS_ANPHI objAP = dt.ADS_ANPHI.Where(x => x.ID == APId).FirstOrDefault();
                    ADS_TONGDAT oTD = dt.ADS_TONGDAT.Where(x => x.DONID == objAP.DONID).FirstOrDefault();
                    ...
                    {
                        ADS_TONGDAT_BL dS_TONGDAT_BL = new ADS_TONGDAT_BL();
                        DataTable anphiTable = dS_TONGDAT_BL.ADS_TONGDATDOITUONG_ANPHI_GETBYANPHIID_TONGDATID(objAP.DONID, objAP.ID);
                ...
                ADS_ANPHI ap = dt.ADS_ANPHI.Where(x => x.DUONGSU_ID == dsID && x.ID != APId && APId != 0).FirstOrDefault();
                if (ap != null)
                {
        ...
        void SetNewSoTBAP()
        {
            ...
            String STTNew = oSTBL.GET_STB_ANPHI_NEW(DonViID, "ADS", ngayTB).ToString();
