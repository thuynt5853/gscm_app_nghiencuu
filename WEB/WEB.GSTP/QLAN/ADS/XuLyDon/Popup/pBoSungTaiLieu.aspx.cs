...
namespace WEB.GSTP.QLAN.ADS.XuLyDon.Popup
{
    public partial class pBoSungTaiLieu : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                try
                {
                    ...
                    DataTable obj = aDXL.ADS_GETALL_DON_YCBS(Convert.ToDecimal(DONID), xuLyID, 1, 20);
                    ...
                    ADS_DON_XULY donXL = dt.ADS_DON_XULY.Where(x => x.ID == xuLyID).FirstOrDefault();
        ...
        protected void cmdCapNhat_Click(object sender, EventArgs e)
        {
            ...
            decimal xuLyID = (String.IsNullOrEmpty(Request["ID"] + "")) ? 0 : Convert.ToDecimal(Request["ID"] + "");
            ADS_DON_XULY oXuLy = dt.ADS_DON_XULY.Where(x => x.ID == xuLyID).FirstOrDefault();
            string bienphap = dropBienPhapGQ.SelectedValue;
            ADS_DON oDon = dt.ADS_DON.Where(x => x.ID == DONID).FirstOrDefault();
            ...
            DataTable checkGQ = oBLYC.CHECK_NGAYGQ_DON_YCBS(obj.ID, xuLyID, 2, txtNgayGQ.Text);
            ...
            ADS_DON_XULY objXuLy = dt.ADS_DON_XULY.Where(x => x.ID == obj.DON_XULY_YCBS_ID).FirstOrDefault();
            ...
            DataTable objYCBS = oBLYC.ADS_GETALL_DON_YCBS(Convert.ToDecimal(DONID), obj.DON_XULY_YCBS_ID.Value, 1, 20);
        ...
        protected void Reset()
        {
            ...
            DataTable donYCBS = oBLYC1.ADS_GETALL_DON_YCBS(Convert.ToDecimal(DONID), xuLyID, 1, 20);
        ...
        protected void dgDS_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "Xoa":
                    ...
                    //ADS_DON_XULY_BL oBL = new ADS_DON_XULY_BL();
                    //DataTable oYC = oBL.ADS_GETALL_DON_YCBS(DONID, ID, 1, 5);

                    //foreach (DataRow donYCBS in oYC.Rows)
                    //{
                    ...
                    DataTable obj = oBLDel.ADS_GETALL_DON_YCBS(Convert.ToDecimal(DONID), ID_DON_YCBS, 1, 20);
                    if (e.Item.ItemIndex == 0 && obj.Rows.Count > 0)
                    {
                        ADS_DON_XULY objXuLy = dt.ADS_DON_XULY.Where(x => x.ID == ID_DON_YCBS).FirstOrDefault();
                    ...
                    {
                        ADS_DON_XULY objXuLy = dt.ADS_DON_XULY.Where(x => x.ID == ID_DON_YCBS).FirstOrDefault();
                        dt.ADS_DON_XULY.Remove(objXuLy);
                        dt.SaveChanges();
                    }
                    LoadDSTL();
                    Reset();
        ...
        private void validateEnableButton(decimal DONID, decimal DON_YCBSID)
        {
            decimal xuLyID = (String.IsNullOrEmpty(Request["ID"] + "")) ? 0 : Convert.ToDecimal(Request["ID"] + "");
            ADS_DON_XULY_BL aDXL = new ADS_DON_XULY_BL();
            DataTable objYCBS = aDXL.ADS_GETALL_DON_YCBS(Convert.ToDecimal(DONID), xuLyID, 1, 20);
        ...
        protected void dgDS_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                decimal ID_DON_YCBS = Convert.ToDecimal(Request.QueryString["ID"].ToString());
                ADS_DON_XULY oXL = dt.ADS_DON_XULY.Where(x => x.ID == ID_DON_YCBS).FirstOrDefault();
                ...
                if (e.Item.ItemIndex == 0)
                {
                    ...
                    DataTable anphiList = oBL.CHECK_ADS_DON_DUONGSU_ANPHI_V2(DONID, DONCHITIETID, ISDONCHITIET);
                ...
                //DataTable obj = oBL.ADS_GETALL_DON_YCBS(Convert.ToDecimal(DONID), ID_DON_YCBS, 1, 20);
                ...
                ADS_DON oT = dt.ADS_DON.Where(x => x.ID == DONID).FirstOrDefault();
                ADS_SOTHAM_THULY oTLST = dt.ADS_SOTHAM_THULY.Where(x => x.DONID == DONIDHandle).FirstOrDefault();
                if (oT.MAGIAIDOAN == ENUM_GIAIDOANVUAN.PHUCTHAM || oTLST != null)
                {
                    cmdEdit.Visible = false;
                    cmdXoa.Visible = false;
        ...
        private void LoadDSTL()
        {
            ...
            DataTable oDT = oBL.ADS_GETALL_DON_YCBS(DONID, ID_DON_YCBS, pageindex, page_size);
            dgDS.DataSource = oDT;
            dgDS.DataBind();
        }
    }
