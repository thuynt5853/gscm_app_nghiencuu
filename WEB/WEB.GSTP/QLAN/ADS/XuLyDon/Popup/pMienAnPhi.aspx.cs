...
namespace WEB.GSTP.QLAN.ADS.XuLyDon.Popup
{
    public partial class pMienAnPhi : System.Web.UI.Page
    {
        ...
        protected void cmdCapNhatAP_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(APId, 2);
                ...
                try
                {
                    ...
                    if ((dataTable != null && dataTable.Rows.Count > 0 && hddHistoryId.Value != "") || hddHistoryId.Value == "")
                    {
                        ...
                        decimal Mienanphi_ID = dON_MIENANPHI_BL.UPSERT_DON_MIENANPHI(ID_MIENANPHI, APId, DONID, 2, v_lydo, nguoitao, ngaytao, nguoisua, ngaysua, v_sothongbao, d_ngaythongbao, v_stb_phu);
                    ...
                    string MATHONGBAO = "";
                    var DVCQG_THANH_TOAN = DataExtensions.GetAllWithClause<BL.GSTP.BANGSETGET.DVCQG_THANH_TOAN>("ANPHI_ID = " + APId + " AND MALOAIVUVIEC = '2'");
        ...
        private decimal UploadFileID(ADS_DON oDon, decimal FileID, string strMaBieumau, string STT, string stb_Phu)
        {
            ...
            try
            {
                ...
                if (hddFilePath.Value != "")
                {
                    ...
                    {
                        string strFilePath = "";
                        // VNPT - Mục án phí ở màn DS đang mặc định là chkKySo = true
                        //if (chkKySo.Checked)
                        //{
        ...
        {
            decimal A_ANPHI_ID = 0;

            // Ưu tiên lấy từ hidden field (sau khi bấm sửa hoặc lưu)
            if (!string.IsNullOrEmpty(hddCurrAPID.Value))
            {
                A_ANPHI_ID = Convert.ToDecimal(hddCurrAPID.Value);
            }
            // Lần đầu mở popup → lấy từ querystring
            else if (Request.QueryString["ID"] != null)
            {
                A_ANPHI_ID = Convert.ToDecimal(Request.QueryString["ID"]);
                hddCurrAPID.Value = A_ANPHI_ID.ToString(); // đồng bộ luôn
            }
            else
            {
                lbThongbaoAP.Text = "Không xác định được ID án phí!";
                return false;
            }

            // Lấy thông tin án phí
            ADS_ANPHI objAnPhi = dt.ADS_ANPHI.Where(x => x.ID == A_ANPHI_ID).FirstOrDefault();
            ...
            DON_MIENANPHI_BL dON_MIENANPHI_BL = new DON_MIENANPHI_BL();
            DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(A_ANPHI_ID, 2);

            //---- Kiểm tra bắt buộc nhập ngày thông báo
            if (string.IsNullOrEmpty(txtNgaythongbaoAP.Text))
            {
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(),
                    "alertMessage", "alert('Chưa nhập Ngày thông báo');", true);
                txtNgaythongbaoAP.Focus();
                return false;
            }

            //---- Kiểm tra bắt buộc nhập số thông báo
            if (string.IsNullOrEmpty(txtSothongbaoAP.Text))
            {
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(),
                    "alertMessage", "alert('Chưa nhập Số thông báo');", true);
                txtSothongbaoAP.Focus();
                return false;
            }

            // Nếu đang sửa và đã tống đạt → không được sửa
            if (!string.IsNullOrEmpty(hddHistoryId.Value))
            {
                ADS_TONGDAT_BL dS_TONGDAT_BL = new ADS_TONGDAT_BL();
                if (dS_TONGDAT_BL.ADS_TONGDATDOITUONG_ANPHI_GETBYANPHIID_TONGDATID(objAnPhi.DONID, objAnPhi.ID).Rows.Count > 0)
                {
            ...
            // Nếu thêm mới → kiểm tra đã miễn chưa
            if (string.IsNullOrEmpty(hddHistoryId.Value))
            {
                if (objAnPhi.TINHTRANG == 1 || dataTable.Rows.Count > 0)
                {
        ...
        protected void rptAP_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "DownloadAP":
                    ...
                    break;
                case "SuaAP":
                    ADS_ANPHI oND1 = dt.ADS_ANPHI.Where(x => x.ID == CurrID).FirstOrDefault();
                    // khong duoc xoa khi da Tong dat - 27/11/2025 vnpt check
                    if (oND1 != null)
                    {
                        ADS_TONGDAT oTD = dt.ADS_TONGDAT.Where(x => x.DONID == oND1.DONID && x.MAPID == oND1.ID && x.MAP_TABLE == ENUM_MAP_TABLE.ADS_ANPHI).FirstOrDefault();
                        if (oTD != null)
                        {
                            lbThongbaoAP.Text = "Bạn không thể sửa khi đã tống đạt!";
                            return;
                ...
                case "XoaAP":
                    try
                    {
                        ...
                        ADS_ANPHI oND2 = dt.ADS_ANPHI.Where(x => x.ID == CurrID).FirstOrDefault();
                        if(oND2 != null)
                        {
                            ADS_TONGDAT oTD1 = dt.ADS_TONGDAT.Where(x => x.DONID == oND2.DONID && x.MAPID == oND2.ID && x.MAP_TABLE == ENUM_MAP_TABLE.ADS_ANPHI).FirstOrDefault();
                            if (oTD1 != null)
                            {
                                lbThongbaoAP.Text = "Bạn không thể xoá khi đã tống đạt!";
                                return;
                            }
                        }
                        // khong duoc xoa khi da Tong dat - 27/11/2025 vnpt check
                        ...
                        if (_check_out == 1)
                        {
                            ...
                            ADS_TONGDAT_BL dS_TONGDAT_BL = new ADS_TONGDAT_BL();
                            if (dS_TONGDAT_BL.ADS_TONGDATDOITUONG_ANPHI_GETBYANPHIID_TONGDATID(objAP.DONID, objAP.ID).Rows.Count > 0)
                            {
                            ...
                            else
                            {
                                ...
                                LOG_THONGTINANPHI_QLTA log = new LOG_THONGTINANPHI_QLTA();
                                log.InsertLog(Session[ENUM_LOAIAN.AN_DANSU].ToString(), CurrID.ToString(), MATHONGBAO, Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Update", CONTENT_JSON, CONTENT_JSON_OLD);
        ...
        protected void rptAP_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                ADS_DON_XULY tralaiDonXuly = dt.ADS_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 3).FirstOrDefault();

                DON_MIENANPHI_BL dON_MIENANPHI_BL1 = new DON_MIENANPHI_BL();
                DataTable dataTable1 = dON_MIENANPHI_BL1.GET_DON_MIENANPHI_BY_ANPHI_ID(vAPID, 2);
                ...
                {
                    ADS_ANPHI objAP = dt.ADS_ANPHI.Where(x => x.ID == vAPID).FirstOrDefault();
        ...
        void LoadInfo_AnPhiDaMien(Decimal CurrID)
        {
            ...
            hddHistoryId.Value = CurrID + "";
            ADS_ANPHI aDSAnphi = dt.ADS_ANPHI.Where(x => x.ID == CurrID).FirstOrDefault();
            if (aDSAnphi != null)
            {
                chkNopAnPhi.Checked = true;
        ...
        private void LoadGrid_AnPhi()
        {
            ...
            DataTable oDT = oBL.ADS_ANPHI_LICHSU_GETBYDONID(DONID, A_ANPHI_ID, 1, pageindex, page_size);
            //if (oDT.Rows.Count > 0)
            //{
