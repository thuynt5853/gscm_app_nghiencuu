    ...
    public partial class pNhapAn : System.Web.UI.Page
    {
        ...
        private void createHoso_xetxulaiPhuctham(decimal vDonID)
        {
            ...
            if (oDon != null)
            {
                ...
                //update 14082025
                oDON_new.TOA_GIAIQUYET_ID = Convert.ToDecimal(oDon.TOAANID);
                ...
                if (vBA_old != null)
                {
                    ...
                    // update 110825
                    if(vBA_new.TOA_GIAIQUYET_ID == null) vBA_new.TOA_GIAIQUYET_ID = vBA_old.TOA_GIAIQUYET_ID;
                ...
                if (LstQD.Count > 0)
                {
                    foreach (ADS_SOTHAM_QUYETDINH vQD_old in LstQD)
                    {
                        ...
                        //update 14082025
                        if (vQD_new.TOA_GIAIQUYET_ID == null) vQD_new.TOA_GIAIQUYET_ID = vQD_old.TOA_GIAIQUYET_ID;
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (ds_vuAnConID.Count > 0)
                    {
                        ...
                        {
                            //ADS_SOTHAM_THULY oNSD = dt.ADS_SOTHAM_THULY.Where(x => x.DONID == vuAnConID).FirstOrDefault();

                            try
                            {
                                ADS_DON donCon = dt.ADS_DON.Where(x => x.ID == vuAnConID).FirstOrDefault();
                                ...
                                List<ADS_DON_DUONGSU> dsDuongSu = dt.ADS_DON_DUONGSU.Where(x => x.DONID == vuAnConID).ToList();
                                string jsonDuongSu = ",";
                                ...
                                //Lấy ds thẩm phán
                                List<ADS_DON_THAMPHAN> dsThamPhan = dt.ADS_DON_THAMPHAN.Where(x => x.DONID == vuAnConID).ToList();
                                string jsonThamPhan = ",";
                                ...
                                //Lấy ds thư kí
                                List<ADS_SOTHAM_HDXX> dsSoThamHDXX = dt.ADS_SOTHAM_HDXX.Where(x => x.DONID == vuAnConID && x.MAVAITRO == "THUKY").ToList();
                                List<ADS_PHUCTHAM_HDXX> dsPhucThamHDXX = dt.ADS_PHUCTHAM_HDXX.Where(x => x.DONID == vuAnConID && x.MAVAITRO == "THUKY").ToList();
                                string jsonThuKy = ",";
                                ...
                                ADS_SOTHAM_THULY thuLy = dt.ADS_SOTHAM_THULY.Where(x => x.DONID == vuAnConID).FirstOrDefault();
                                int ttTL = 0;
                                string ngayTL = "", soTL = "";
                                if (thuLy != null)
                                {
                    ...
                    //Án dân sự
                    string tenChucNang = "Nhập án Dân sự từ " + donCon.MAVUVIEC + " đến " + donGoc.MAVUVIEC;
                    ...
                    string jsonString = JsonConvert.SerializeObject(objNoiDung);
                                ...
                                adsBL.HISTORY_ALLDATA_BY_VUANID(vuAnConID, 2, nguoiXoa, taiKhoanXoa, tenChucNang, hanhDong, jsonString);

                                //Cập nhật DONID của các bảng liên quan:
                                ADS_DON_BL update = new ADS_DON_BL();
                    ...
                    donCT.THONGTINTHEM = donCon.THONGTINTHEM;
                    donCT.DONKKID = null;
                    donCT.GHICHU = "";
                    donCT.TTGQ = null;
                    donCT.NOIDUNGTTGQ = "";
                    ...
                    dt.SaveChanges();
                                ...
                                ADS_DON_XULY donXL = dt.ADS_DON_XULY.Where(x => x.DONID == vuAnConID).FirstOrDefault();
                                if (donXL != null)
                                {
                                    donXL.DONID = null;
