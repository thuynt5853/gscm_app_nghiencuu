    ...
    public partial class NguoiThamgiaTT : System.Web.UI.Page
    {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ADS_DON_THAMGIATOTUNG oND;
                if (hddid.Value == "" || hddid.Value == "0")
                    oND = new ADS_DON_THAMGIATOTUNG();
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.ADS_DON_THAMGIATOTUNG.Where(x => x.ID == ID).FirstOrDefault();
                }
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    dt.ADS_DON_THAMGIATOTUNG.Add(oND);
                    dt.SaveChanges();
                }
                else
                {
        ...
        public void LoadGrid()
        {
            ...
            string current_id = Session[ENUM_LOAIAN.AN_DANSU] + "";
            decimal ID = Convert.ToDecimal(current_id);
            DataTable oDT = oBL.ADS_DON_TGTT_GETLIST(ID);
        ...
        {
            ADS_DON_THAMGIATOTUNG oND = dt.ADS_DON_THAMGIATOTUNG.Where(x => x.ID == id).FirstOrDefault();
            dt.ADS_DON_THAMGIATOTUNG.Remove(oND);
            dt.SaveChanges();
            dgList.CurrentPageIndex = 0;
            LoadGrid();
            ResetControls();
        ...
        protected void btn_GXN_NBC_Click(object sender, EventArgs e)
        {
            ...
            if (checkTCTGTT)
            {
                ...
                //--------------------------
                Response.Clear();
                Response.AddHeader("content-disposition", "attachment;filename=xacnhanbihaisotham.doc");
