...
namespace WEB.GSTP.QLAN.ADS.Hoso
{
    public partial class Tailieu : System.Web.UI.Page
    {
        ...
        private void LoadDropNguoiBanGiao()
        {
            ...
            {
                List<ADS_DON_DUONGSU> lst = dt.ADS_DON_DUONGSU.Where(x => x.DONID == DonID).ToList();
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ADS_DON_TAILIEU donFile = new ADS_DON_TAILIEU();
                decimal TLID = (String.IsNullOrEmpty(hddID.Value)) ? 0 : Convert.ToDecimal(hddID.Value);
                if (TLID > 0)
                    donFile = dt.ADS_DON_TAILIEU.Where(x => x.ID == TLID).FirstOrDefault<ADS_DON_TAILIEU>();
                else
                    donFile = new ADS_DON_TAILIEU();
                try
                {
                ...
                if (hddID.Value == "" || hddID.Value == "0")
                {
                    ...
                    dt.ADS_DON_TAILIEU.Add(donFile);
                }
                else
                {
        ...
        {
            decimal DonViID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_DANSU] + "");
            int Total = 0;
            ADS_DON_BL donBL = new ADS_DON_BL();
            DataTable tbl = donBL.ADS_DON_BanGiaoTaiLieu_GETLIST(DonViID);
            if (tbl != null && tbl.Rows.Count > 0)
            {
        ...
        {
            ADS_DON_TAILIEU oT = dt.ADS_DON_TAILIEU.Where(x => x.ID == ID).FirstOrDefault();
            dt.ADS_DON_TAILIEU.Remove(oT);
            dt.SaveChanges();
            hddPageIndex.Value = "1";
            LoadGridNgayBanGiao();
            ResetControls();
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                switch (e.CommandName)
                {
                    ...
                    case "Sua":
                        hddFilePath.Value = "";
                        ADS_DON_TAILIEU bgtl = dt.ADS_DON_TAILIEU.Where(x => x.ID == ID).FirstOrDefault();
                        if (bgtl != null)
                        {
                        ...
                        break;
                    case "Download":
                        ADS_DON_TAILIEU oND = dt.ADS_DON_TAILIEU.Where(x => x.ID == ID).FirstOrDefault();
                        if (oND.TENFILE != "")
                        {
