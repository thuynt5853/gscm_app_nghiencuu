    ...
    public partial class Danhsach : System.Web.UI.Page
    {
        ...
        private void Load_Data()
        {
            ...
            DataTable oDT = oBL.DON_SEARCH(Session["CAP_XET_XU"] + "", txtTenVuViec.Text.Trim(), txt_QHPL.Text.Trim(), txtMaVuViec.Text.Trim(), txtTENDUONGSU.Text.Trim(), dropCapxx.SelectedValue, DropToaAn.SelectedValue,
                                            DropTINHTRANG_THULY.SelectedValue, txtSOTHULY_THONGBAO.Text.Trim(), txt_NGAYTHULY_TU.Text, txt_NGAYTHULY_DEN.Text, ddlThamphan.SelectedValue, ddlVaiTroThamPhan.SelectedValue, DropTINHTRANG_GIAIQUYET.SelectedValue, txtTuNgay.Text.Trim(), txtDenNgay.Text.Trim(), Drop_KETQUA.SelectedValue, txtSoQD.Text.Trim(),
                                            txt_NgayQD.Text.Trim(), ddlHTND_Thuky.SelectedValue, DropTHOIHAN_GQ.SelectedValue, Drop_Loaidon.SelectedValue, Drop_PT_RKINHNGHIEM.SelectedValue, drop_BIENPHAPGQ.SelectedValue, dropUTTP.SelectedValue, vchecktk, trangThaiVuAn, ck_GQTDC_QDK.Checked == true ? 1 : 0,
                                            ...
                                            checkHG.Checked == true ? txtHoaGiaiDenNgay.Text.Trim() : "",
                                            ddlQHPLTK.SelectedValue
                                            );
        ...
        protected void lbtimkiem_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                DataTable oDT = oBL.DON_NHAPTACH_SEARCH(Session["CAP_XET_XU"] + "", txtTenVuViec.Text.Trim(), txt_QHPL.Text.Trim(), txtMaVuViec.Text.Trim(), txtTENDUONGSU.Text.Trim(), dropCapxx.SelectedValue, DropToaAn.SelectedValue,
                                                DropTINHTRANG_THULY.SelectedValue, txtSOTHULY_THONGBAO.Text.Trim(), txt_NGAYTHULY_TU.Text, txt_NGAYTHULY_DEN.Text, ddlThamphan.SelectedValue, ddlHTND_Thuky.SelectedValue, Drop_Loaidon.SelectedValue, status, 2, pageindex, page_size);
                ...
                if (oDT != null && oDT.Rows.Count > 0)
                {
                    foreach (DataRow item in oDT.Rows)
                    {
                        ...
                        {
                            ADS_DON dGoc = dt.ADS_DON.Where(x => x.ID == dNT.VUANGOCID).FirstOrDefault();
        ...
        void Xoa_An_XxlaiST(MenuPermission oPer, decimal IDVuViec)
        {
            ...
            ADS_DON oT = dt.ADS_DON.Where(x => x.ID == IDVuViec).FirstOrDefault();
            ADS_SOTHAM_THULY oT_TL = dt.ADS_SOTHAM_THULY.Where(x => x.DONID == IDVuViec).FirstOrDefault();
            ...
            if (oT != null)
            {
                ...
                {
                    // xóa màn án phí
                    List<ADS_ANPHI> lstAP = dt.ADS_ANPHI.Where(x => x.DONID == IDVuViec).ToList<ADS_ANPHI>();
                    if (lstAP.Count > 0)
                    {
                        foreach (ADS_ANPHI ap in lstAP)
                        {
                            dt.ADS_ANPHI.Remove(ap);
                        }
                        dt.SaveChanges();
                    }

                    // xóa màn tài liệu
                    List<ADS_DON_TAILIEU> lstTL = dt.ADS_DON_TAILIEU.Where(x => x.DONID == IDVuViec).ToList<ADS_DON_TAILIEU>();
                    if (lstTL.Count > 0)
                    {
                        foreach (ADS_DON_TAILIEU tl in lstTL)
                        {
                            dt.ADS_DON_TAILIEU.Remove(tl);
                        }
                        dt.SaveChanges();
                    }

                    // xóa màn người tham gia tố tụng khác
                    List<ADS_DON_THAMGIATOTUNG> lstTGTT = dt.ADS_DON_THAMGIATOTUNG.Where(x => x.DONID == IDVuViec).ToList<ADS_DON_THAMGIATOTUNG>();
                    if (lstTL.Count > 0)
                    {
                        foreach (ADS_DON_THAMGIATOTUNG tgtt in lstTGTT)
                        {
                            dt.ADS_DON_THAMGIATOTUNG.Remove(tgtt);
                        }
                        dt.SaveChanges();
                    }

                    // xóa màn đơn khác
                    List<DON_KHAC> lstDK = dt.DON_KHAC.Where(x => x.DONID == IDVuViec && x.LOAIANID == 2).ToList<DON_KHAC>();
                    if (lstDK.Count > 0)
                    {
                    ...
                    List<ADS_DON_THAMPHAN> lstTP = dt.ADS_DON_THAMPHAN.Where(x => x.DONID == IDVuViec).ToList<ADS_DON_THAMPHAN>();
                    if (lstTP.Count > 0)
                    {
                        foreach (ADS_DON_THAMPHAN tp in lstTP)
                        {
                            dt.ADS_DON_THAMPHAN.Remove(tp);
                        }
                        dt.SaveChanges();
                    }
                    // xóa đơn xử lý
                    List<ADS_DON_XULY> lstDonXL = dt.ADS_DON_XULY.Where(x => x.DONID == IDVuViec).ToList<ADS_DON_XULY>();
                    if (lstTP.Count > 0)
                    {
                        foreach (ADS_DON_XULY xuLy in lstDonXL)
                        {
                            dt.ADS_DON_XULY.Remove(xuLy);
                        }
                        dt.SaveChanges();
                    }


                    List<ADS_FILE> lstF = dt.ADS_FILE.Where(x => x.DONID == IDVuViec).ToList<ADS_FILE>();
                    if (lstF.Count > 0)
                    {
                        foreach (ADS_FILE f in lstF)
                        {
                            dt.ADS_FILE.Remove(f);
                        }
                        dt.SaveChanges();
                    }

                    var delete_all_record_ADS_DON_DUONGSU_by_donid = dt.ADS_DON_DUONGSU.Where(x => x.DONID == IDVuViec);
                    dt.ADS_DON_DUONGSU.RemoveRange(delete_all_record_ADS_DON_DUONGSU_by_donid);

                    dt.ADS_DON.Remove(oT);
                    dt.SaveChanges();

                    //K: Nếu vụ việc tồn tại trong bảng DON_TIEPNHAN thì cập nhật trạng thái DON_GUINHAN = 4
                    DON_TIEPNHAN dtn = dt.DON_TIEPNHAN.Where(x => x.DONID == IDVuViec && x.LOAIAN == 2).FirstOrDefault();
                    if (dtn != null)
                    {
                    ...
                    GD.GIAIDOAN_DELETES("2", IDVuViec, 2);
                    //---------------------------
        ...
        void Xoa_An(MenuPermission oPer, decimal IDVuViec)
        {
            ...
            if (oT != null)
            {
                ...
                if (oT_TL == null)//chưa thụ lý
                {
                    ...
                    // Kiểm tra Án phí
                    ADS_ANPHI anphi = dt.ADS_ANPHI.Where(x => x.DONID == IDVuViec).FirstOrDefault<ADS_ANPHI>();
                    if (anphi != null)
                    {
                    ...
                    ADS_DON_XULY xld = dt.ADS_DON_XULY.Where(x => x.DONID == IDVuViec).FirstOrDefault<ADS_DON_XULY>();
                    if (xld != null)
                    {
                    ...
                    ADS_DON_THAMPHAN gqd = dt.ADS_DON_THAMPHAN.Where(x => x.DONID == IDVuViec && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETDON).FirstOrDefault<ADS_DON_THAMPHAN>();
                    if (gqd != null)
                    {
                    ...
                    ADS_DON_TAILIEU tailieu = dt.ADS_DON_TAILIEU.Where(x => x.DONID == IDVuViec).FirstOrDefault<ADS_DON_TAILIEU>();
                    if (tailieu != null)
                    {
                    ...
                    ADS_DON_THAMGIATOTUNG tgtt = dt.ADS_DON_THAMGIATOTUNG.Where(x => x.DONID == IDVuViec).FirstOrDefault<ADS_DON_THAMGIATOTUNG>();
                    if (tgtt != null)
                    {
                    ...
                    //ADS_DON_DUONGSU ds = dt.ADS_DON_DUONGSU.Where(x => x.DONID == IDVuViec && x.ISDAIDIEN != 1).FirstOrDefault<ADS_DON_DUONGSU>();
                    //if (ds != null)
                    //{
                    ...
                    DON_CHITIET dct = dt.DON_CHITIET.Where(x => x.DONID == IDVuViec && x.LOAIANID == 2).FirstOrDefault();
                    DON_KHAC dk = dt.DON_KHAC.Where(x => x.DONID == IDVuViec && x.LOAIANID == 2).FirstOrDefault();
                    if (dct != null && dk != null)
                    {
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (hddCHECK_THULY.Value == "" || Session[ENUM_SESSION.SESSION_NHOMNSDID] + "" == "1")//Session[ENUM_SESSION.SESSION_NHOMNSDID]+""=="1" // nhóm Quản trị hệ thống 
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (VuAnID > 0)
                            {
                                List<ADS_DON_XULY> oDON_XLY = dt.ADS_DON_XULY.Where(x => (x.DONID == VuAnID || x.DON_XULYID == VuAnID)).ToList<ADS_DON_XULY>();
                                ...
                                if ((dv["NGUOITAO"].ToString().ToLower() == strUserName.ToLower() || Session[ENUM_SESSION.SESSION_NHOMNSDID] + "" == "1" || Session[ENUM_SESSION.SESSION_NHOMNSDID] + "" == "541")
                                    ...
                                    lbtXoa.Visible = false;
                                ADS_DON_XULY oDXL_TL = dt.ADS_DON_XULY.Where(x => (x.DONID == VuAnID || x.DON_XULYID == VuAnID) && x.LOAIGIAIQUYET != 3).FirstOrDefault();
                                if (oDON_XLY.Count > 0 && oDXL_TL == null)
                                {
                                    e.Item.Cells[5].Text = "-Trả lại đơn";
                                }
                                var chuyenAn = dt.ADS_CHUYEN_NHAN_AN.Where(x => x.VUANID == VuAnID && x.TOACHUYENID == id_toaan).FirstOrDefault();
                                ADS_DON_XULY oDXL_CD = dt.ADS_DON_XULY.Where(x => (x.DONID == VuAnID || x.DON_XULYID == VuAnID) && x.LOAIGIAIQUYET != 1).FirstOrDefault();
                ...
                else if ((oPer.CAPNHAT == true || Session[ENUM_SESSION.SESSION_NHOMNSDID] + "" == "1" || dv["NGUOITAO"].ToString().ToLower() == strUserName.ToLower()) && LoginDonViID == id_toaan)
                {
                    lblSua.Visible = true;
                    if (Session["CAP_XET_XU"] + "" == "CAPCAO")
                    {
                ...
                string Result = new ADS_CHUYEN_NHAN_AN_BL().Check_NhanAn(VuAnID, "", Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]));
                if (Result != "")
                {
                ...
                {
                    ADS_SOTHAM_BANAN obj = dt.ADS_SOTHAM_BANAN.Where(x => x.DONID == VuAnID).FirstOrDefault();
                    var objQD = (from a in dt.ADS_SOTHAM_QUYETDINH
                ...
                {
                    ADS_DON_GIAIDOAN gd = dt.ADS_DON_GIAIDOAN.Where(x => x.DONID == VuAnID && x.MAGIAIDOAN == 3).FirstOrDefault();
                    if (gd != null && gd.TOAPHUCTHAMID == LoginDonViID)
                    {
                        ADS_PHUCTHAM_BANAN obj = dt.ADS_PHUCTHAM_BANAN.Where(x => x.DONID == VuAnID).FirstOrDefault();
                        var objQD = (from a in dt.ADS_PHUCTHAM_QUYETDINH
                                     join d in dt.DM_QD_QUYETDINH on a.QUYETDINHID equals d.ID
                                     join c in dt.DM_QD_LOAI on d.LOAIID equals c.ID
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "Select"://Lựa chọn vụ việc cần Lưu thông tin                  
                    ...
                    List<ADS_DON_GIAIDOAN> gd = dt.ADS_DON_GIAIDOAN.AsNoTracking().Where(x => x.DONID == IDVuViec).ToList();
                    ...
                    //Session[ENUM_SESSION.SESSION_DONVIID] = oDon.MAGIAIDOAN;
                ...
                case "view":
                    Session[ENUM_LOAIAN.AN_DANSU] = IDVuViec;
                    string StrKetqua = "PopupReport('/BaoCao/Thongtinvuviec/DanSu.aspx','Thông tin án Dân sự',1000,700);";
                ...
                case "Xoa":
                    ...
                    if (Session[ENUM_SESSION.SESSION_NHOMNSDID] + "" == "1")
                    {
                        ...
                        if (oBL.HISTORY_ALLDATA_BY_VUANID(IDVuViec, 2, Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Danh sách án Dân sự", "Xóa", json) == false)
                        {
        ...
        private void createHoso_xetxulaiPhuctham(decimal vDonID)
        {
            ...
            if (oDon != null)
            {
                ...
                foreach (ADS_DON_THAMPHAN vThamphan_old in lstThamPhan)
                {
                    ...
                    //update 14082025
                    if (vThamphan_new.TOA_GIAIQUYET_ID == null) vThamphan_new.TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ...
                foreach (ADS_DON_XULY vXulyDon_old in lstXulyDon)
                {
                    ...
                    //update 14082025
                    if (vXulyDon_new.TOA_GIAIQUYET_ID == null) vXulyDon_new.TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ...
                if (vBA_old != null)
                {
                    ...
                    //update 14082025
                    if (vBA_new.TOA_GIAIQUYET_ID == null) vBA_new.TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ...
                if (LstQD.Count > 0)
                {
                    foreach (ADS_SOTHAM_QUYETDINH vQD_old in LstQD)
                    {
                        ...
                        //update 14082025
                        if (vQD_new.TOA_GIAIQUYET_ID == null) vQD_new.TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
        ...
        private void createHoso_xetxulaiSotham(decimal vDonID)
        {
            ...
            if (oDon != null)
            {
                ...
                //List<ADS_FILE> aDSFiles = dt.ADS_FILE.Where(x => x.DONID == vDonID).ToList<ADS_FILE>();
                //foreach (ADS_FILE aDSFile_OLD in aDSFiles)
                //            {
                //	ADS_FILE ADS_FILE_NEW = new ADS_FILE()
                //	{
                ...
                List<ADS_FILE> oldFiles = dt.ADS_FILE
                ...
                Dictionary<ADS_FILE, ADS_FILE> tempMap = new Dictionary<ADS_FILE, ADS_FILE>();
                foreach (ADS_FILE oldFile in oldFiles)
                {
                    ...
                    dt.ADS_FILE.Add(newFile);
                    tempMap.Add(oldFile, newFile);

                }
                dt.SaveChanges();
                foreach (KeyValuePair<ADS_FILE, ADS_FILE> kv in tempMap)
                {
                ...
                List<DON_CHITIET> dON_CHITIETs = dt.DON_CHITIET.Where(x => x.DONID == vDonID && x.LOAIANID == 2).ToList<DON_CHITIET>();
                ...
                List<ADS_DON_THAMGIATOTUNG> aDS_DON_THAMGIATOTUNGs = dt.ADS_DON_THAMGIATOTUNG.Where(x => x.DONID == vDonID).ToList<ADS_DON_THAMGIATOTUNG>();
                foreach (ADS_DON_THAMGIATOTUNG ADS_DON_THAMGIATOTUNG_OLD in aDS_DON_THAMGIATOTUNGs)
                {
                    ADS_DON_THAMGIATOTUNG ADS_DON_THAMGIATOTUNG_NEW = new ADS_DON_THAMGIATOTUNG();
                    ...
                    dt.ADS_DON_THAMGIATOTUNG.Add(ADS_DON_THAMGIATOTUNG_NEW);
                    dt.SaveChanges();
                }

                //Xu ly don
                List<ADS_DON_XULY> lstXulyDon = dt.ADS_DON_XULY.Where(x => (x.DONID == vDonID || x.DON_XULYID == vDonID) && x.TOAANID == oDon.TOAANID).ToList<ADS_DON_XULY>();
                foreach (ADS_DON_XULY vXulyDon_old in lstXulyDon)
                {
                    ADS_DON_XULY vXulyDon_new = new ADS_DON_XULY();
                    ...
                    DataTable donYCBSes = oBLYC.ADS_GETALL_DON_YCBS(vDonID, vXulyDon_old.ID, 1, 100);
                ...
                foreach (ADS_ANPHI vAP_old in lstAP)
                {
                    ...
                    {
                        DAL.GSTP.DVCQG_THANH_TOAN dVCQG_THANH_TOAN = dt.DVCQG_THANH_TOAN.Where(x => x.ANPHI_ID == vAP_old.ID && x.MALOAIVUVIEC == "2").FirstOrDefault();
                    ...
                    List<ADS_ANPHI_DUONGSU> aDS_ANPHI_DUONGSUs = DataExtensions.GetAllWithClause<ADS_ANPHI_DUONGSU>($" ANPHI_ID == {vAP_old.ID}");
                    if (aDS_ANPHI_DUONGSUs != null && aDS_ANPHI_DUONGSUs.Count > 0)
                    {
                        foreach (ADS_ANPHI_DUONGSU aDS_ANPHI_DUONGSU_OLD in aDS_ANPHI_DUONGSUs)
                        {
                            ADS_ANPHI_DUONGSU aDS_ANPHI_DUONGSU_NEW = new ADS_ANPHI_DUONGSU();
                            ...
                            DataExtensions.Insert<ADS_ANPHI_DUONGSU>(aDS_ANPHI_DUONGSU_NEW);
                        }
                    }

                }

                List<DON_MIENANPHI> dON_MIENANPHIs = DataExtensions.GetAllWithClause<DON_MIENANPHI>($" DONID = {vDonID} AND LOAIAN = 2");
                ...
                List<DON_KHAC> dON_KHAC_OLDs = dt.DON_KHAC.Where(x => x.DONID == vDonID && x.LOAIANID == 2 && x.LOAIDON != 7).ToList<DON_KHAC>();
                ...
                GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                GD.GAIDOAN_INSERT_UPDATE_XXLAI_SOTHAM("2", oDON_new.ID, 2, (decimal)LoginDonViID, 0, 0, 0, 0);
            }
        ...
        protected void cmdTachan_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridItem Item in dgList.Items)
            {
                ...
                if (chkChon.Checked)
                {
                    ...
                    oNSD = dt.ADS_SOTHAM_THULY.Where(x => x.DONID == vuAnGocId).FirstOrDefault();
                }
            }
            if (vuAnGocId == 0)
            {
            ...
            {
                string link = "/QLAN/ADS/Hoso/Popup/pTachAn.aspx?DonID=" + vuAnGocId;
        ...
        protected void cmdNhapan_Click(object sender, EventArgs e)
        {
            ...
            {
                string link = "/QLAN/ADS/Hoso/Popup/pNhapAn.aspx?DonID=" + vuAnGocId;
                ScriptManager.RegisterStartupScript(this, this.GetType(), "onclick", "var Mleft = (screen.width/2)-(1200/2); var Mtop = (screen.height/2)-(750/2); javascript:window.open('" + link + "', '_blank', 'height=750px,width=1200px,top=\'+Mtop+\', left=\'+Mleft+\',resizable=yes,toolbar=yes,scrollbars=1'); ", true);
                Cls_Comon.CallFunctionJS(this, this.GetType(), "LoadModalDiv()");
            }
        }
        ...
        {
            string StrMsg = "PopupCenter('/QLAN/pDanhSachAnPhi.aspx?hsID=" + ENUM_LOAIVUVIEC_NUMBER.AN_DANSU.ToString() + "');";
        ...
        {
            string link = "/QLAN/ADS/Hoso/Popup/pTKQuaHan.aspx";
            ScriptManager.RegisterStartupScript(this, this.GetType(), "onclick", "var Mleft = (screen.width/2)-(1200/2); var Mtop = (screen.height/2)-(750/2); javascript:window.open('" + link + "', '_blank', 'height=750px,width=1200px,top=\'+Mtop+\', left=\'+Mleft+\',resizable=yes,toolbar=yes,scrollbars=1'); ", true);
            Cls_Comon.CallFunctionJS(this, this.GetType(), "LoadModalDiv()");
        }
    }
