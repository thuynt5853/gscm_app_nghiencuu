...
namespace WEB.GSTP.QLAN.ADS.AnPhi
{
    public partial class Danhsach : System.Web.UI.Page
    {
        ...
        private void CheckQuyen()
        {
            ...
            if (oT.MAGIAIDOAN == ENUM_GIAIDOANVUAN.PHUCTHAM || oT.MAGIAIDOAN == ENUM_GIAIDOANVUAN.PHUCTHAM_QDK || oT.MAGIAIDOAN == ENUM_GIAIDOANVUAN.THULYGDT)
            {
                lbtthongbao.Text = "Vụ việc đã được chuyển lên tòa cấp trên, không được sửa đổi !";
                Cls_Comon.SetButton(cmdCapNhat, false);
                return;
            }
            ADS_DON_XULY donxl = dt.ADS_DON_XULY.Where(x => x.DONID == ID).FirstOrDefault();
            if (donxl != null)
            {
            ...
            List<ADS_DON_XULY> lts_donxl = dt.ADS_DON_XULY.Where(x => x.DONID == ID).ToList();
            ...
            string Result = new ADS_CHUYEN_NHAN_AN_BL().Check_NhanAn(ID, StrMsg, donvi);
            if (Result != "")
            {
        ...
        private void LoadDropDuongSu()
        {
            ...
            ddlDuongSu.DataSource = oDSBL.ADS_DON_DUONGSU_BIENLAI_V2(DONID); ;
        ...
        private void LoadGrid_AnPhi()
        {
            ...
            DataTable oDT = oBL.ADS_ANPHI_GETBYDONID(DONID, 0, pageindex, page_size);
            if (oDT.Rows.Count > 0)
            {
                ADS_ANPHI ap = dt.ADS_ANPHI.Where(x => x.DONID == DONID && x.SOBIENLAI != null && x.TINHTRANG == 0).FirstOrDefault();
                if (ap != null)
                {
        ...
        protected void rpt_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "ENABLE":
                    ...
                    {
                        oBLs.CHECK_TRANGTHAI_THULY(CurrID, "2");
                ...
                case "Sua":
                    ...
                    DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(CurrID, 2);
                ...
                case "Xoa":
                    try
                    {
                        ...
                        DataTable dataTableXoa = dON_MIENANPHI_BLs.GET_DON_MIENANPHI_BY_ANPHI_ID(CurrID, 2);
                ...
                case "DownloadAP_THA":
                    ...
                    //--------------------
                    byte[] V_FILE_DATA = oBL.File_Attach_Anphi_Return("2", CurrID, ref V_FILE_NAME);
                    string V_FILE_TYLE = "";
        ...
        private void LoadInfo_AnPhi(Decimal CurrID)
        {
            string current_id = Session[ENUM_LOAIAN.AN_DANSU] + "";
            decimal DONID = Convert.ToDecimal(current_id);
        ...
        {
            Response.Redirect(Cls_Comon.GetRootURL() + "/QLAN/ADS/Sotham/ThuLy.aspx");
        }
        ...
        protected void rpt_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                //tbl =obj.GET_THANH_TOAN_BY_MA(V_MA_THONGBAO);
                //DataRow row = row = tbl.Rows[0];
        ...
        protected void ddlDuongSu_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (idDS != 0)
            {
                string current_id = Session[ENUM_LOAIAN.AN_DANSU] + "";
                decimal DONID = Convert.ToDecimal(current_id);
                decimal idAP = dt.ADS_ANPHI.Where(x => x.DONID == DONID && x.DUONGSU_ID == idDS).FirstOrDefault().ID;
