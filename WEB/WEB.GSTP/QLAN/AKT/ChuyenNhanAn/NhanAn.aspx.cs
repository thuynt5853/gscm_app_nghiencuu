...
namespace WEB.GSTP.QLAN.AKT.ChuyenNhanAn
{
    public partial class NhanAn : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    #region Quét các vụ việc được chuyển từ Tòa án cấp cao và lưu vào AKT_DON và AKT_CHUYEN_NHAN_AN
                    ...
                    //string sql = "select * from HOSO where TOA_AN_XET_XU='" + ToaAnID.ToString() + "' and LOAI_AN='" + ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI + "' and TRANG_THAI=1";
                    ...
                    //        #region Update Vụ việc AKT_DON
                    //        AKT_DON vDON = dt.AKT_DON.Where(x => x.MAVUVIEC == MaVuViec && ((x.MAGIAIDOAN == ENUM_GIAIDOANVUAN.SOTHAM && x.TOAANID == ToaAnID) || (x.MAGIAIDOAN == ENUM_GIAIDOANVUAN.PHUCTHAM && x.TOAPHUCTHAMID == ToaAnID))).FirstOrDefault();
                    //        if (vDON == null)
                    //        {
                    //            IsNew = true;
                    //            vDON = new AKT_DON();
                    ...
                    //        VuAnID = vDON.ID;
                    //        #endregion
                    //        #region Update Table AKT_CHUYEN_NHAN_AN
                    //        IsNew = false;
                    //        AKT_CHUYEN_NHAN_AN chuyen_nhan_an = dt.AKT_CHUYEN_NHAN_AN.Where(x => x.VUANID == VuAnID && x.TOACHUYENID == Toa_Chuyen_ID && x.TOANHANID == Toa_Nhan_ID).FirstOrDefault();
                    //        if (chuyen_nhan_an == null)
                    //        {
                    //            IsNew = true;
                    //            chuyen_nhan_an = new AKT_CHUYEN_NHAN_AN();
                    //        }
                    ...
                    //        if (IsNew)
                    //        {
                    //            dt.AKT_CHUYEN_NHAN_AN.Add(chuyen_nhan_an);
                    //        }
                    //        dt.SaveChanges();
                    //        #endregion
                    //    }
        ...
        private void LoadGrid()
        {
            ...
            DataTable oDT = oBL.KT_NHANAN(txt_NG_KC.Text.Trim(), txtSoQD.Text.Trim(), txt_NgayQD.Text.Trim(), vDonViID, txtMaVuViec.Text, txtTenVuViec.Text, txtTenToa.Text, Convert.ToDecimal(ddlTHGN.SelectedValue), dFrom, dTo, Convert.ToDecimal(rdbTrangthai.SelectedValue), Convert.ToDecimal(rdbLoai.SelectedValue));
        ...
        private void UpdateDuongSuIdTGTT(AKT_DON_THAMGIATOTUNG oTTOLD, AKT_DON_THAMGIATOTUNG oTTNEW, List<DuongSuTemp> lstDuongSuTemp)
        {
        ...
        protected void cmdLuu_Click(object sender, EventArgs e)
        {
            ...
            foreach (RepeaterItem item in rptCapNhat.Items)
            {
                ...
                AKT_CHUYEN_NHAN_AN oT = dt.AKT_CHUYEN_NHAN_AN.Where(x => x.ID == ID).FirstOrDefault();
                if (oT != null)
                {
                    ...
                    if (oIT.MA == ENUM_TRUONGHOP_GIAONHAN.KHONGTHUOC_THAMQUYEN_XETXU)
                    {
                        ...
                        else
                        {
                            if (rdbLoai.SelectedValue == "0")
                            {
                                ...
                                oDonMoi.MAVUVIEC = ENUM_LOAIVUVIEC.AN_KINHDOANH_THUONGMAI + oTA.MA + oDonMoi.TT.ToString();
                                ...
                                if (oDonMoi.TOA_GIAIQUYET_ID == null && oDon.TOA_GIAIQUYET_ID != null)
                                {
                                    oDonMoi.TOA_GIAIQUYET_ID = oDon.TOA_GIAIQUYET_ID;
                                }
                                dt.AKT_DON.Add(oDonMoi);
                                dt.SaveChanges();

                                GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                                GD.GAIDOAN_INSERT_UPDATE("4", oDonMoi.ID, 2, oDonMoi.TOAANID.Value, 0, 0, 0, 0);
                                List<DuongSuTemp> lstDuongSuTemp = new List<DuongSuTemp>();


                                decimal DonChiTietID = 0;
                                ...
                                foreach (var dct_data in lstDON_CHITIET)
                                {
                                    DON_CHITIET DCT = new DON_CHITIET();

                                    DCT.DONID = oDonMoi.ID;
                                    DCT.LOAIANID = dct_data.LOAIANID;
                                    DCT.TOAANID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                                    DCT.HINHTHUCNHANDON = dct_data.HINHTHUCNHANDON;
                                    DCT.NGAYVIETDON = dct_data.NGAYVIETDON;
                                    DCT.NGAYNHANDON = dct_data.NGAYNHANDON;
                                    DCT.CANBONHANDONID = dct_data.CANBONHANDONID;
                                    DCT.THAMPHANKYNHANDON = dct_data.THAMPHANKYNHANDON;
                                    DCT.YEUTONUOCNGOAI = dct_data.YEUTONUOCNGOAI;
                                    DCT.LOAIDON = dct_data.LOAIDON;
                                    DCT.USERTT_EMAIL = dct_data.USERTT_EMAIL;
                                    DCT.USERTT_ID = dct_data.USERTT_ID;
                                    DCT.USERTT_NGAYTAO = dct_data.USERTT_NGAYTAO;
                                    DCT.USERTT_NGAYGUI = dct_data.USERTT_NGAYGUI;
                                    DCT.USERTT_NGAYBOSUNG = dct_data.USERTT_NGAYBOSUNG;
                                    DCT.NGUOITAO = dct_data.NGUOITAO;
                                    DCT.NGAYTAO = dct_data.NGAYTAO;
                                    DCT.NGUOISUA = dct_data.NGUOISUA;
                                    DCT.NGAYSUA = dct_data.NGAYSUA;
                                    DCT.NOIDUNGKHOIKIEN = dct_data.NOIDUNGKHOIKIEN;
                                    DCT.THONGTINTHEM = dct_data.THONGTINTHEM;
                                    DCT.DONKKID = dct_data.DONKKID;
                                    DCT.GHICHU = dct_data.GHICHU;
                                    DCT.TTGQ = dct_data.TTGQ;
                                    DCT.NOIDUNGTTGQ = dct_data.NOIDUNGTTGQ;
                                    DCT.DONGUINHANID = dct_data.DONGUINHANID;
                                    DCT.IS_NHAPAN = dct_data.IS_NHAPAN;
                                    if (DCT.TOA_GIAIQUYET_ID == null && dct_data.TOA_GIAIQUYET_ID != null)
                                    {
                                        DCT.TOA_GIAIQUYET_ID = dct_data.TOA_GIAIQUYET_ID;
                                    }
                                    dt.DON_CHITIET.Add(DCT);
                                    dt.SaveChanges();
                                    listDOnCT.Add(new KeyValuePair<decimal, decimal>(dct_data.ID, DCT.ID));
                                }
                                //lấy danh sách đương sự
                                IQueryable<AKT_DON_DUONGSU> lstoDS = dt.AKT_DON_DUONGSU.Where(s => s.DONID == oDon.ID);
                                foreach (var it in lstoDS)
                                {
                                    AKT_DON_DUONGSU oDS = new AKT_DON_DUONGSU();
                                    ...
                                    if (oDS.TOA_GIAIQUYET_ID == null && it.TOA_GIAIQUYET_ID != null)
                                    {
                                        oDS.TOA_GIAIQUYET_ID = it.TOA_GIAIQUYET_ID;
                                    }
                                    dt.AKT_DON_DUONGSU.Add(oDS);
                                    dt.SaveChanges();
                                    ...
                                    foreach (var ddsct_data in lstDON_DUONGSU_CHITIET)
                                    {
                                        ...
                                        ddsct.DONID = oDonMoi.ID;
                                        ddsct.DUONGSUID = oDS.ID;
                                        ...
                                        if (ddsct.TOA_GIAIQUYET_ID == null && ddsct_data.TOA_GIAIQUYET_ID != null)
                                        {
                                ...
                                IQueryable<AKT_DON_THAMGIATOTUNG> lstoTT = dt.AKT_DON_THAMGIATOTUNG.Where(s => s.DONID == oDon.ID);
                                foreach (var it in lstoTT)
                                {
                                    //lưu thông tin người tham gia tố tụng
                                    AKT_DON_THAMGIATOTUNG oTT = new AKT_DON_THAMGIATOTUNG();
                                    ...
                                    // insert toa_gq_id
                                    if (oTT.TOA_GIAIQUYET_ID == null && it.TOA_GIAIQUYET_ID != null)
                                    {
                                        oTT.TOA_GIAIQUYET_ID = it.TOA_GIAIQUYET_ID;
                                    }
                                    dt.AKT_DON_THAMGIATOTUNG.Add(oTT);
                                    dt.SaveChanges();
                                }

                                IQueryable<AKT_DON_TAILIEU> lstTailieu = dt.AKT_DON_TAILIEU.Where(s => s.DONID == oDon.ID);
                                foreach (var it in lstTailieu)
                                {
                                    ...
                                    // insert toa_gq_id
                                    if (objtl.TOA_GIAIQUYET_ID == null && it.TOA_GIAIQUYET_ID != null)
                                    {
                                        objtl.TOA_GIAIQUYET_ID = it.TOA_GIAIQUYET_ID;
                                    }
                                    dt.AKT_DON_TAILIEU.Add(objtl);
                                    dt.SaveChanges();
                                }
                            ...
                            else
                            {
                                ...
                                */
                                string UserName = Session[ENUM_SESSION.SESSION_USERNAME] + "", LoaiVuViec = ENUM_LOAIVUVIEC.AN_KINHDOANH_THUONGMAI, MaToaAn = Session[ENUM_SESSION.SESSION_MADONVI] + "";
                                ...
                                GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                                GD.GAIDOAN_INSERT_UPDATE("4", DonID_New, 2, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), 0, 0, 0, 0);
                            }
                        }
                    }
                    else if (oIT.MA == ENUM_TRUONGHOP_GIAONHAN.XETXULAI_CAPSOTHAM)
                    {
                        ...
                        {
                            AKT_CHUYEN_NHAN_AN_BL _caBL = new AKT_CHUYEN_NHAN_AN_BL();
                            decimal donIdOld = _caBL.getDonIdOld(oDon.ID);
                            GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                            AKT_DON oDonOld = dt.AKT_DON.Where(x => x.ID == donIdOld).FirstOrDefault();
                            ...
                            List<AKT_SOTHAM_KHANGCAO> khangCaos = dt.AKT_SOTHAM_KHANGCAO.Where(x => x.DONID == donIdOld && (x.TINHTRANG_GIAIQUYET == null || x.TINHTRANG_GIAIQUYET == 0) && x.LOAIKHANGCAO == 2).ToList();
                            if (khangCaos != null && khangCaos.Count > 0)
                            {
                                foreach (AKT_SOTHAM_KHANGCAO kc in khangCaos)
                                {
                                    kc.TINHTRANG_GIAIQUYET = 1;
                                    dt.SaveChanges();
                                }
                            }
                            List<AKT_SOTHAM_KHANGNGHI> khangNghis = dt.AKT_SOTHAM_KHANGNGHI.Where(x => x.DONID == donIdOld && (x.TINHTRANG_GIAIQUYET == null || x.TINHTRANG_GIAIQUYET == 0) && x.LOAIKN == 2).ToList();
                            if (khangNghis != null && khangNghis.Count > 0)
                            {
                                foreach (AKT_SOTHAM_KHANGNGHI kn in khangNghis)
                                {
                        ...
                        else
                        {
                            ...
                            //GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                            //GD.GAIDOAN_INSERT_UPDATE("4", DonID, 2, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), 0, 0, 0, 0);
                            AKT_DON oDON_new = new AKT_DON();
                            ...
                            if (oDON_new.TOA_GIAIQUYET_ID == null && oDon.TOA_GIAIQUYET_ID != null)
                            {
                                oDON_new.TOA_GIAIQUYET_ID = oDon.TOA_GIAIQUYET_ID;
                            }
                            // insert toa_pt_qd_id
                            if (oDON_new.TOA_PHUCTHAM_GIAIQUYET_ID == null && oDon.TOA_PHUCTHAM_GIAIQUYET_ID != null)
                            {
                                oDON_new.TOA_PHUCTHAM_GIAIQUYET_ID = oDon.TOA_PHUCTHAM_GIAIQUYET_ID;
                            }
                            dt.AKT_DON.Add(oDON_new);
                            dt.SaveChanges();

                            GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                            GD.GAIDOAN_INSERT_UPDATE("4", oDON_new.ID, 2, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), 0, 0, 0, 0);
                            ...
                            foreach (var dct_data in lstDON_CHITIET)
                            {
                                DON_CHITIET dct_dt = new DON_CHITIET();

                                dct_dt.DONID = oDON_new.ID;
                                dct_dt.LOAIANID = dct_data.LOAIANID;
                                dct_dt.TOAANID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                                dct_dt.HINHTHUCNHANDON = dct_data.HINHTHUCNHANDON;
                                dct_dt.NGAYVIETDON = dct_data.NGAYVIETDON;
                                dct_dt.NGAYNHANDON = dct_data.NGAYNHANDON;
                                dct_dt.CANBONHANDONID = dct_data.CANBONHANDONID;
                                dct_dt.THAMPHANKYNHANDON = dct_data.THAMPHANKYNHANDON;
                                dct_dt.YEUTONUOCNGOAI = dct_data.YEUTONUOCNGOAI;
                                dct_dt.LOAIDON = dct_data.LOAIDON;
                                dct_dt.USERTT_EMAIL = dct_data.USERTT_EMAIL;
                                dct_dt.USERTT_ID = dct_data.USERTT_ID;
                                dct_dt.USERTT_NGAYTAO = dct_data.USERTT_NGAYTAO;
                                dct_dt.USERTT_NGAYGUI = dct_data.USERTT_NGAYGUI;
                                dct_dt.USERTT_NGAYBOSUNG = dct_data.USERTT_NGAYBOSUNG;
                                dct_dt.NGUOITAO = dct_data.NGUOITAO;
                                dct_dt.NGAYTAO = dct_data.NGAYTAO;
                                dct_dt.NGUOISUA = dct_data.NGUOISUA;
                                dct_dt.NGAYSUA = dct_data.NGAYSUA;
                                dct_dt.NOIDUNGKHOIKIEN = dct_data.NOIDUNGKHOIKIEN;
                                dct_dt.THONGTINTHEM = dct_data.THONGTINTHEM;
                                dct_dt.DONKKID = dct_data.DONKKID;
                                dct_dt.GHICHU = dct_data.GHICHU;
                                dct_dt.TTGQ = dct_data.TTGQ;
                                dct_dt.NOIDUNGTTGQ = dct_data.NOIDUNGTTGQ;
                                dct_dt.DONGUINHANID = dct_data.DONGUINHANID;
                                dct_dt.IS_NHAPAN = dct_data.IS_NHAPAN;
                                if (dct_dt.TOA_GIAIQUYET_ID == null && dct_data.TOA_GIAIQUYET_ID != null)
                                {
                                    dct_dt.TOA_GIAIQUYET_ID = dct_data.TOA_GIAIQUYET_ID;
                                }
                                dt.DON_CHITIET.Add(dct_dt);
                                dt.SaveChanges();
                                listDOnCT.Add(new KeyValuePair<decimal, decimal>(dct_data.ID, dct_dt.ID));
                            }

                            //Câp dương su vụ án
                            List<AKT_DON_DUONGSU> lst = dt.AKT_DON_DUONGSU.Where(x => x.DONID == DonID).ToList<AKT_DON_DUONGSU>();
                            foreach (AKT_DON_DUONGSU vDuongsu_old in lst)
                            {
                                AKT_DON_DUONGSU vDuongsu_new = new AKT_DON_DUONGSU();
                                ...
                                //Ban giao tai lieu
                                List<AKT_DON_TAILIEU> lstTaiLieu = dt.AKT_DON_TAILIEU.Where(x => x.DONID == DonID && x.NGUOIBANGIAO == vDuongsu_old.ID).ToList<AKT_DON_TAILIEU>();
                                foreach (AKT_DON_TAILIEU vdonFile_old in lstTaiLieu)
                                {
                                    AKT_DON_TAILIEU donFile_new = new AKT_DON_TAILIEU();
                                    ...
                                    // insert toa_gq_id
                                    if (donFile_new.TOA_GIAIQUYET_ID == null && vdonFile_old.TOA_GIAIQUYET_ID != null)
                                    {
                                        donFile_new.TOA_GIAIQUYET_ID = vdonFile_old.TOA_GIAIQUYET_ID;
                                    }
                                    dt.AKT_DON_TAILIEU.Add(donFile_new);
                                    dt.SaveChanges();
                                }

                            }
                            // Phan cong Tham phan
                            List<AKT_DON_THAMPHAN> lstThamPhan = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == DonID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETDON).ToList<AKT_DON_THAMPHAN>();
                            foreach (AKT_DON_THAMPHAN vThamphan_old in lstThamPhan)
                            {
                                AKT_DON_THAMPHAN vThamphan_new = new AKT_DON_THAMPHAN();
                                ...
                                // insert toa_gq_id
                                if (vThamphan_new.TOA_GIAIQUYET_ID == null && vThamphan_old.TOA_GIAIQUYET_ID != null)
                                {
                                    vThamphan_new.TOA_GIAIQUYET_ID = vThamphan_old.TOA_GIAIQUYET_ID;
                                }
                                dt.AKT_DON_THAMPHAN.Add(vThamphan_new);
                                dt.SaveChanges();
                            }
                            //Xu ly don
                            List<AKT_DON_XULY> lstXulyDon = dt.AKT_DON_XULY.Where(x => x.DONID == DonID).ToList<AKT_DON_XULY>();
                            foreach (AKT_DON_XULY vXulyDon_old in lstXulyDon)
                            {
                                AKT_DON_XULY vXulyDon_new = new AKT_DON_XULY();
                                ...
                                dt.AKT_DON_XULY.Add(vXulyDon_new);
                                dt.SaveChanges();
                            }
                            //Tam ung an phi
                            List<AKT_ANPHI> lstAP = dt.AKT_ANPHI.Where(x => x.DONID == DonID && x.MAGIAIDOAN == 2).ToList();
                            foreach (AKT_ANPHI vAP_old in lstAP)
                            {
                                AKT_ANPHI vAP_new = new AKT_ANPHI();
                                ...
                                dt.AKT_ANPHI.Add(vAP_new);
                                dt.SaveChanges();

                                var CNA = new CHUYEN_NHAN_AN();
                                CNA.INSERT_ANPHI_DUONGSUID(4, DonID, oDON_new.ID);
                            }
                    ...
                    else
                    {
                        ...
                        //var soThamKhangCaos = dt.AKT_SOTHAM_KHANGCAO.Where(x => x.DONID == oDon.ID && x.LOAIKHANGCAO == 2 && (x.TINHTRANG_GIAIQUYET == null || x.TINHTRANG_GIAIQUYET == 0));
                        //var soThamKhangNghis = dt.AKT_SOTHAM_KHANGNGHI.Where(x => x.DONID == oDon.ID && x.LOAIKN == 2 && (x.TINHTRANG_GIAIQUYET == null || x.TINHTRANG_GIAIQUYET == 0));

                        //if ((soThamKhangCaos != null && soThamKhangCaos.Count() > 0) || (soThamKhangNghis != null && soThamKhangNghis.Count() > 0))
                        AKT_CHUYEN_NHAN_AN_BL _chuyenNhanAnBl = new AKT_CHUYEN_NHAN_AN_BL();
                        if (_chuyenNhanAnBl.CheckCoKcKnTamDinhChi(DonID))
                        {
                            ...
                            dt.AKT_DON.Add(oDON_new);
                            dt.SaveChanges();
                            GD.GAIDOAN_INSERT_UPDATE("4", oDON_new.ID, ENUM_GIAIDOANVUAN.PHUCTHAM_QDK, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), 0, 0, 0);
                        ...
                        else
                        {
                            ...
                            GD.GAIDOAN_INSERT_UPDATE("4", DonID, 3, 0, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), 0, 0, 0);
                        }
                    }
        ...
        private void UpdateNguyenDonBiDon(List<NGUOI_THAM_GIA_AK> List_NguyeDon_BiDon, decimal DonID)
        {
            if (List_NguyeDon_BiDon.Count > 0)
            {
                ...
                {
                    IsNew = false;
                    AKT_DON_DUONGSU DuongSu = dt.AKT_DON_DUONGSU.Where(x => x.DONID == DonID && x.ID_DUONGSU_TACC == item.Guid).FirstOrDefault();
                    if (DuongSu == null)
                    {
                        IsNew = true;
                        DuongSu = new AKT_DON_DUONGSU();
                    }
                    ...
                    if (IsNew)
                    {
                        ...
                        dt.AKT_DON_DUONGSU.Add(DuongSu);
                    }
                    dt.SaveChanges();
                }
            }
        ...
        private void UpdateNguoiTGTT(List<NGUOI_THAM_GIA_CHUNG> ListNguoiTGTT, decimal DonID)
        {
            if (ListNguoiTGTT.Count > 0)
            {
                ...
                {
                    AKT_DON_THAMGIATOTUNG NguoiTGTT = dt.AKT_DON_THAMGIATOTUNG.Where(x => x.DONID == DonID && x.ID_DUONGSU_TACC == item.Guid).FirstOrDefault();
                    if (NguoiTGTT != null)
                    {
                        IsNew = true;
                        NguoiTGTT = new AKT_DON_THAMGIATOTUNG();
                    ...
                    if (IsNew)
                    {
                        ...
                        dt.AKT_DON_THAMGIATOTUNG.Add(NguoiTGTT);
                    }
                    dt.SaveChanges();
                }
            }
        ...
        private void UpdateThuLy(THU_LY ThuLy, decimal ToaAnID, decimal DonID, string CapXetXu)
        {
            if (CapXetXu == "SOTHAM")
            {
                ...
                bool IsNew = false;
                AKT_SOTHAM_THULY STThuLy = dt.AKT_SOTHAM_THULY.Where(x => x.DONID == DonID && x.SOTHULY == ThuLy.SOTHULY.ToString()).FirstOrDefault();
                if (STThuLy != null)
                {
                    IsNew = true;
                    STThuLy = new AKT_SOTHAM_THULY();
                ...
                if (IsNew)
                {
                    ...
                    dt.AKT_SOTHAM_THULY.Add(STThuLy);
                }
                dt.SaveChanges();
            }
            else
            {
                ...
                bool IsNew = false;
                AKT_PHUCTHAM_THULY PTThuLy = dt.AKT_PHUCTHAM_THULY.Where(x => x.DONID == DonID && x.SOTHULY == ThuLy.SOTHULY.ToString()).FirstOrDefault();
                if (PTThuLy != null)
                {
                    IsNew = true;
                    PTThuLy = new AKT_PHUCTHAM_THULY();
                ...
                if (IsNew)
                {
                    ...
                    dt.AKT_PHUCTHAM_THULY.Add(PTThuLy);
                }
                dt.SaveChanges();
            }
        }
        private void UpdateNguoiTHTT(List<NGUOI_TIEN_HANH_TO_TUNG> ListNguoiTHTT, decimal DonID, string CapXetXu)
        {
            if (ListNguoiTHTT.Count > 0)
            {
                ...
                foreach (NGUOI_TIEN_HANH_TO_TUNG item in ListNguoiTHTT)
                {
                    ...
                    if (CapXetXu == "SOTHAM")
                    {
                        AKT_SOTHAM_HDXX Hdxx = dt.AKT_SOTHAM_HDXX.Where(x => x.DONID == DonID && x.CANBOID == CanBoID).FirstOrDefault();
                        if (Hdxx == null)
                        {
                            IsNew = true;
                            Hdxx = new AKT_SOTHAM_HDXX();
                        ...
                        if (IsNew)
                        {
                            ...
                            dt.AKT_SOTHAM_HDXX.Add(Hdxx);
                        }
                    }
                    // Phúc thẩm
                    else
                    {
                        AKT_PHUCTHAM_HDXX Hdxx = dt.AKT_PHUCTHAM_HDXX.Where(x => x.DONID == DonID && x.CANBOID == CanBoID).FirstOrDefault();
                        if (Hdxx == null)
                        {
                            IsNew = true;
                            Hdxx = new AKT_PHUCTHAM_HDXX();
                            Hdxx.NGAYTAO = DateTime.Now;
                        }
                        else
                        {
        ...
        private void UpdateQDVuViec(List<QUYET_DINH> ListQD, decimal ToaAnID, decimal DonID, string CapXetXu)
        {
            if (ListQD.Count > 0)
            {
                ...
                foreach (QUYET_DINH item in ListQD)
                {
                    ...
                    if (CapXetXu == "SOTHAM")
                    {
                        AKT_SOTHAM_QUYETDINH QdVuAn = dt.AKT_SOTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.TOAANID == ToaAnID).FirstOrDefault();
                        if (QdVuAn == null)
                        {
                            IsNew = false;
                            QdVuAn = new AKT_SOTHAM_QUYETDINH()
                            {
                        ...
                        if (IsNew)
                        {
                            ...
                            dt.AKT_SOTHAM_QUYETDINH.Add(QdVuAn);
                        }
                        dt.SaveChanges();
                    }
                    // Phúc thẩm
                    ...
                    {
                        AKT_PHUCTHAM_QUYETDINH QdVuAn = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.TOAANID == ToaAnID).FirstOrDefault();
                        if (QdVuAn == null)
                        {
                            IsNew = false;
                            QdVuAn = new AKT_PHUCTHAM_QUYETDINH()
                            {
                                NGAYTAO = DateTime.Now
                            };
                        }
                        ...
                        if (IsNew)
                        {
                            ...
                            dt.AKT_PHUCTHAM_QUYETDINH.Add(QdVuAn);
                        }
                        dt.SaveChanges();
                    }
                }
        ...
        private void UpdateBanAn(BAN_AN bAn, LICH_XET_XU LichXetXu, decimal ToaAnID, decimal DonID, string CapXetXu)
        {
            ...
            if (CapXetXu == "SOTHAM")
            {
                AKT_SOTHAM_BANAN STBanAn = dt.AKT_SOTHAM_BANAN.Where(x => x.DONID == DonID && x.TOAANID == ToaAnID).FirstOrDefault();
                if (STBanAn == null)
                {
                    IsNew = true;
                    STBanAn = new AKT_SOTHAM_BANAN();
                ...
                if (IsNew)
                {
                    ...
                    dt.AKT_SOTHAM_BANAN.Add(STBanAn);
                }
                dt.SaveChanges();
            }
            // Phúc thẩm
            ...
            {
                AKT_PHUCTHAM_BANAN PTBanAn = dt.AKT_PHUCTHAM_BANAN.Where(x => x.DONID == DonID && x.TOAANID == ToaAnID).FirstOrDefault();
                if (PTBanAn == null)
                {
                    IsNew = true;
                    PTBanAn = new AKT_PHUCTHAM_BANAN();
                ...
                DM_KETQUA_PHUCTHAM KQPT = dt.DM_KETQUA_PHUCTHAM.Where(x => x.ISAKT == 1 && x.MA == bAn.KETQUA).FirstOrDefault();
                if (KQPT != null)
                {
                ...
                if (IsNew)
                {
                    ...
                    dt.AKT_PHUCTHAM_BANAN.Add(PTBanAn);
                }
                dt.SaveChanges();
            }
        }
        private void UpdateKhangCao(List<KHANG_CAO> ListKhangCao, decimal ToaAnID, decimal DonID)
        {
            if (ListKhangCao.Count > 0)
            {
                ...
                foreach (KHANG_CAO item in ListKhangCao)
                {
                    ...
                    decimal NguoiKCID = 0;
                    AKT_DON_DUONGSU DuongSu = dt.AKT_DON_DUONGSU.Where(x => x.DONID == DonID && x.ID_DUONGSU_TACC == Guid_NguoiKC).FirstOrDefault();
                    if (DuongSu != null)
                    {
                        NguoiKCID = DuongSu.ID;
                    }
                    AKT_SOTHAM_KHANGCAO kc = dt.AKT_SOTHAM_KHANGCAO.Where(x => x.DONID == DonID && x.DUONGSUID == NguoiKCID && x.TOAANRAQDID == ToaAnID).FirstOrDefault();
                    if (kc != null)
                    {
                        IsNew = true;
                        kc = new AKT_SOTHAM_KHANGCAO();
        ...
        private decimal Action_ChuyenAn_KhongThuoc_ThamQuyen(decimal DonID, string LoaiVuViec, decimal ToaNhanID, string MaToaNhan, string NguoiTao)
        {
            ...
            DataTable tbl = Cls_Comon.GetTableByProcedurePaging("AKT_CVA_KHONG_THAM_QUYEN", parameters);
            decimal Result = 0;
            if (tbl.Rows.Count > 0)
        ...
        {
            AKT_DON_BL Bl = new AKT_DON_BL();
            //for (int i = 0; i < lstDonID.Count; i++)
            //{
            //    var ChuyenNhanID = lstDonID[i];
            //    decimal DonID_New = 0;
            //    AKT_CHUYEN_NHAN_AN ObjChuyenAn = dt.AKT_CHUYEN_NHAN_AN.Where(x => x.ID == ChuyenNhanID).FirstOrDefault();
            //    if (ObjChuyenAn != null)
            //    {
            ...
            for (int i = 0; i < lstDonID.Count; i++)
            {
                var ChuyenNhanID = lstDonID[i];
                decimal DonID_New = 0;
                AKT_CHUYEN_NHAN_AN ObjNhanAn = dt.AKT_CHUYEN_NHAN_AN.Where(x => x.ID == ChuyenNhanID).FirstOrDefault();
                if (ObjNhanAn != null)
                {
                    int LoaiVuViec = Convert.ToInt32(ENUM_LOAIVUVIEC.AN_KINHDOANH_THUONGMAI);
                    if (ObjNhanAn.MAP_VUANID_NEW.GetValueOrDefault(0) > 0)
                    {
                        ...
                        if (rdbLoai.SelectedValue == "0")
                        {
                            AKT_DON_THAMPHAN thamPhan = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == DonID_New && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETDON).FirstOrDefault();
                            if (thamPhan != null)
                            {
                                IsThuLy = true;
                        ...
                        if (!IsThuLy)
                        {
                            ...
                            AKT_DON oDonNew = dt.AKT_DON.Where(x => x.ID == DonID_New).FirstOrDefault();
                            var json = new JavaScriptSerializer().Serialize(oDonNew);
                            ADS_DON_BL oBL = new ADS_DON_BL();
                            if (oBL.HISTORY_ALLDATA_BY_VUANID(DonID_New, Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI), Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Huỷ nhận án hành chính", "Xóa", json) == false)
                            {
                                break;
                            }
                            //Ket thuc 
                            ...
                            AKT_DON oVuan = dt.AKT_DON.Where(x => x.ID == ObjNhanAn.VUANID).FirstOrDefault();
                    ...
                    else
                    {
                        ...
                        if (oVuan.MAGIAIDOAN == ENUM_GIAIDOANVUAN.PHUCTHAM_QDK)
                        {
                            ...
                            AKT_CHUYEN_NHAN_AN chuyenAnSTlenPTTDC = dt.AKT_CHUYEN_NHAN_AN.Where(x => x.MAP_VUANID_NEW == oVuan.ID).FirstOrDefault();
                            List<AKT_SOTHAM_THULY> oThuLyST = dt.AKT_SOTHAM_THULY.Where(x => x.DONID == oldVuAnId && x.NGAYTAO >= chuyenAnSTlenPTTDC.NGAYTAO).ToList();
                            List<AKT_SOTHAM_HDXX> oSTNguoiTienHanhToTung = dt.AKT_SOTHAM_HDXX.Where(x => x.DONID == oldVuAnId && x.NGAYTAO >= chuyenAnSTlenPTTDC.NGAYTAO).ToList();
                            List<AKT_DON_THAMPHAN> oThamPhanST = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == oldVuAnId && x.NGAYTAO >= chuyenAnSTlenPTTDC.NGAYTAO).ToList();
                            List<AKT_SOTHAM_BANAN> oBanAnST = dt.AKT_SOTHAM_BANAN.Where(x => x.DONID == oldVuAnId && x.NGAYTAO >= chuyenAnSTlenPTTDC.NGAYTAO).ToList();
                            List<AKT_SOTHAM_QUYETDINH> oQDST = dt.AKT_SOTHAM_QUYETDINH.Where(x => x.DONID == oldVuAnId && x.NGAYTAO >= chuyenAnSTlenPTTDC.NGAYTAO).ToList();
                            List<AKT_SOTHAM_KHANGCAO> oKCST = dt.AKT_SOTHAM_KHANGCAO.Where(x => x.DONID == oldVuAnId && x.NGAYTAO >= chuyenAnSTlenPTTDC.NGAYTAO).ToList();
                            List<AKT_SOTHAM_KHANGNGHI> oKNST = dt.AKT_SOTHAM_KHANGNGHI.Where(x => x.DONID == oldVuAnId && x.NGAYTAO >= chuyenAnSTlenPTTDC.NGAYTAO).ToList();
                            ...
                            {
                                AKT_DON oVuAnOld = dt.AKT_DON.Where(x => x.ID == oldVuAnId).FirstOrDefault();
                                AKT_CHUYEN_NHAN_AN ObjNhanAnLanCuoi = dt.AKT_CHUYEN_NHAN_AN.Where(x => x.VUANID == oldVuAnId && x.TOACHUYENID == oVuAnOld.TOAANID).OrderByDescending(x => x.ID).FirstOrDefault();
                                AKT_CHUYEN_NHAN_AN ObjNhanAnLanGanCuoi = dt.AKT_CHUYEN_NHAN_AN.Where(x => x.VUANID == oldVuAnId && x.TOACHUYENID == oVuAnOld.TOAANID && x.ID < ObjNhanAnLanCuoi.ID).OrderByDescending(x => x.ID).FirstOrDefault();
                                if (ObjNhanAnLanGanCuoi == null)
                                {
                                    #region toancau - anhnt update lại kết quả kháng cáo/kháng nghị
                                    List<AKT_SOTHAM_KHANGCAO> khangCaos = dt.AKT_SOTHAM_KHANGCAO.Where(x => x.DONID == oldVuAnId && x.TINHTRANG_GIAIQUYET == 1 && x.LOAIKHANGCAO == 2).ToList();
                                    if (khangCaos != null && khangCaos.Count > 0)
                                    {
                                        foreach (AKT_SOTHAM_KHANGCAO kc in khangCaos)
                                        {
                                    ...
                                    List<AKT_SOTHAM_KHANGNGHI> khangNghis = dt.AKT_SOTHAM_KHANGNGHI.Where(x => x.DONID == oldVuAnId && x.TINHTRANG_GIAIQUYET == 1 && x.LOAIKN == 2).ToList();
                                    if (khangNghis != null && khangNghis.Count > 0)
                                    {
                                        foreach (AKT_SOTHAM_KHANGNGHI kn in khangNghis)
                                        {
                                ...
                                {
                                    #region toancau - anhnt update lại kết quả kháng cáo/kháng nghị
                                    List<AKT_SOTHAM_KHANGCAO> khangCaos = dt.AKT_SOTHAM_KHANGCAO.Where(x => x.DONID == oldVuAnId && x.TINHTRANG_GIAIQUYET == 1 && x.LOAIKHANGCAO == 2 && x.NGAYTAO >= ObjNhanAnLanGanCuoi.NGAYTAO && x.NGAYTAO <= ObjNhanAnLanCuoi.NGAYTAO).ToList();
                                    if (khangCaos != null && khangCaos.Count > 0)
                                    {
                                        foreach (AKT_SOTHAM_KHANGCAO kc in khangCaos)
                                        {
                                    ...
                                    List<AKT_SOTHAM_KHANGNGHI> khangNghis = dt.AKT_SOTHAM_KHANGNGHI.Where(x => x.DONID == oldVuAnId && x.TINHTRANG_GIAIQUYET == 1 && x.LOAIKN == 2 && x.NGAYTAO >= ObjNhanAnLanGanCuoi.NGAYTAO && x.NGAYTAO <= ObjNhanAnLanCuoi.NGAYTAO).ToList();
                                    if (khangNghis != null && khangNghis.Count > 0)
                                    {
                                        foreach (AKT_SOTHAM_KHANGNGHI kn in khangNghis)
                                        {
