...
namespace WEB.GSTP.QLAN.AKT.ChuyenNhanAn
{
    public partial class ChuyenAn : System.Web.UI.Page
    {
        ...
        private void LoadGrid()
        {
            ...
            DataTable oDT = oBL.KT_CHUYENAN(vDonViID, txtTenToa.Text.Trim(), txtMaVuViec.Text.Trim(), txtTenVuViec.Text.Trim(), txtSoquyetdinh.Text.Trim(), txtSobanan.Text.Trim(), dFrom, dTo, txtTenduongsu.Text.Trim(), Convert.ToDecimal(rdbTrangthai.SelectedValue), Convert.ToDecimal(rdbLoai.SelectedValue));
        ...
        protected void cmdNhanan_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridItem Item in dgList.Items)
            {
                ...
                if (chkChon.Checked)
                {
                    ...
                    {
                        AKT_DON_XULY donXuLy = dt.AKT_DON_XULY.Where(x => x.DONID == IDVUAN).FirstOrDefault();
                    ...
                    else
                    {
                        ...
                        else if (hddLydo.Value == "05")
                        {
                            ...
                            AKT_DON oDon = dt.AKT_DON.Where(x => x.ID == IDVUAN).FirstOrDefault();
                            if (oDon.MAGIAIDOAN == ENUM_GIAIDOANVUAN.PHUCTHAM_QDK)
                            {
                                AKT_CHUYEN_NHAN_AN_BL chuyenNhanBL = new AKT_CHUYEN_NHAN_AN_BL();
       ...
       private string genNoiDung(decimal MaGiaiDoan, decimal ID, string strNoiDung, bool isAn = true)
        {
            if (isAn)
            {
                if (MaGiaiDoan == ENUM_GIAIDOANVUAN.SOTHAM || MaGiaiDoan == ENUM_GIAIDOANVUAN.HOSO)
                {
                    ...
                    List<AKT_SOTHAM_KHANGCAO> lstkc = dt.AKT_SOTHAM_KHANGCAO.Where(x => x.DONID == ID && (x.TINHTRANG_GIAIQUYET == 0 || x.TINHTRANG_GIAIQUYET == null)).OrderBy(y => y.ISQUAHAN).ToList();
                    List<AKT_SOTHAM_KHANGNGHI> lstkn = dt.AKT_SOTHAM_KHANGNGHI.Where(x => x.DONID == ID && (x.TINHTRANG_GIAIQUYET == 0 || x.TINHTRANG_GIAIQUYET == null)).ToList();
                    AKT_SOTHAM_KHANGCAO okc = dt.AKT_SOTHAM_KHANGCAO.Where(x => x.DONID == ID).OrderByDescending(y => y.ISQUAHAN).FirstOrDefault();
                    ...
                    if (flag == false)
                    {
                        ...
                        if (oDMQD != null)
                        {
                            ...
                            AKT_SOTHAM_QUYETDINH oQD = dt.AKT_SOTHAM_QUYETDINH.Where(x => x.DONID == ID && x.LOAIQDID == oDMQD.ID && x.TOAANID == IDToa).OrderByDescending(y => y.NGAYQD).FirstOrDefault();
                            if (oQD != null)
                            {
                ...
                {
                    List<AKT_PHUCTHAM_BANAN> lstba = dt.AKT_PHUCTHAM_BANAN.Where(x => x.DONID == ID && (x.KETQUAPHUCTHAMID == 4 || x.KETQUAPHUCTHAMID == 22)).ToList();
                    List<AKT_PHUCTHAM_QUYETDINH> lstqd = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.DONID == ID && x.KETQUAID == 103).ToList();
                ...
                {
                    // AKT_KCKNQDK_PHUCTHAM_THULY oTL = DataExtensions.GetAllByDonId<AKT_KCKNQDK_PHUCTHAM_THULY>(ID).OrderByDescending(y => y.NGAYTHULY).Take(1).FirstOrDefault();

                    List<AKT_KCKNQDK_PHUCTHAM_QUYETDINH> lstqd = DataExtensions.GetAllWithClause<AKT_KCKNQDK_PHUCTHAM_QUYETDINH>($"DONID = {ID} AND KETQUAID IN (101,102,103)");
                    if (lstqd.Count > 0)
                    {
            ...
            {
                AKT_DON_XULY dxl = dt.AKT_DON_XULY.Where(x => x.DONID == ID).OrderByDescending(x => x.ID).FirstOrDefault();
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                try
                {
                    ...
                    AKT_DON oDon = dt.AKT_DON.Where(x => x.ID == ID).FirstOrDefault();
                    if (oDon != null)
                    {
                    ...
                    else
                    {
                        if (MaGiaiDoan == ENUM_GIAIDOANVUAN.SOTHAM || MaGiaiDoan == ENUM_GIAIDOANVUAN.HOSO)
                        {
                            AKT_SOTHAM_THULY oTL = dt.AKT_SOTHAM_THULY.Where(x => x.DONID == ID).OrderByDescending(y => y.NGAYTHULY).Take(1).FirstOrDefault();
                            ...
                            else
                            {
                                ...
                                {
                                    AKT_DON_XULY dxl = dt.AKT_DON_XULY.Where(x => x.DONID == ID).FirstOrDefault();
                                    strNoiDung = dxl.LYDO;
                                }
                            }
                        }
                        ...
                        {
                            AKT_PHUCTHAM_THULY oTL = dt.AKT_PHUCTHAM_THULY.Where(x => x.DONID == ID).OrderByDescending(y => y.NGAYTHULY).Take(1).FirstOrDefault();
                            if (oTL != null) lstNgaythuly.Text = ((DateTime)oTL.NGAYTHULY).ToString("dd/MM/yyyy");
                            if (rdbTrangthai.SelectedValue == "0")
                            {
                                //DM_KETQUA_PHUCTHAM oKQPT = dt.DM_KETQUA_PHUCTHAM.Where(x => x.MA == "06").FirstOrDefault();
                                //List<AKT_PHUCTHAM_BANAN> lstba = dt.AKT_PHUCTHAM_BANAN.Where(x => x.DONID == ID && x.KETQUAPHUCTHAMID == oKQPT.ID).ToList();
                                //DM_KETQUA_PHUCTHAM oKQPT2 = dt.DM_KETQUA_PHUCTHAM.Where(x => x.MA == "04").FirstOrDefault();
                                //List<AKT_PHUCTHAM_BANAN> lstba2 = dt.AKT_PHUCTHAM_BANAN.Where(x => x.DONID == ID && x.KETQUAPHUCTHAMID == oKQPT2.ID).ToList();
                                //List<AKT_PHUCTHAM_QUYETDINH> lstQD = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.DONID == ID && x.KETQUAID == 103).ToList();
        ...
        {
            AKT_CHUYEN_NHAN_AN_BL Bl = new AKT_CHUYEN_NHAN_AN_BL();
            ...
            //    {
            //        AKT_CHUYEN_NHAN_AN ObjChuyenNhan = dt.AKT_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonID).FirstOrDefault();
            //        if (ObjChuyenNhan != null)
            //        {
            //            dt.AKT_CHUYEN_NHAN_AN.Remove(ObjChuyenNhan);
            //            dt.SaveChanges();
            //            dgList.CurrentPageIndex = 0;
            //            hddPageIndex.Value = "1";

            //            //10092021 insert quản lý hồ sơ trạng thái kháng nghị kháng cáo
            //            new QLHS_BL().DeleteHoSoLuTruCoChuyenAn(DonID, ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI);

            //            //manhnd 10/12/2021 cap nhat lai MAGIAIDON sau khi huy chuyen
            //            AKT_DON_GIAIDOAN oGD = dt.AKT_DON_GIAIDOAN.Where(x => x.DONID == DonID && x.TOAANID == ObjChuyenNhan.TOACHUYENID).FirstOrDefault();
            //            AKT_DON oDon = dt.AKT_DON.Where(X => X.ID == DonID).FirstOrDefault();
            ...
            for (int i = 0; i < lstChuyenAnID.Count; i++)
            {
                ...
                AKT_CHUYEN_NHAN_AN oChuyenAn = dt.AKT_CHUYEN_NHAN_AN.Where(x => x.ID == ChuyenAnId).FirstOrDefault();
                AKT_DON_XULY dxl = dt.AKT_DON_XULY.Where(x => x.DONID == oChuyenAn.VUANID).FirstOrDefault();
                if (oChuyenAn != null)
                {
                    ...
                    {
                        dt.AKT_CHUYEN_NHAN_AN.Remove(oChuyenAn);
                        dt.SaveChanges();
                        dgList.CurrentPageIndex = 0;
                        hddPageIndex.Value = "1";
                        //10092021 insert quản lý hồ sơ trạng thái kháng nghị kháng cáo
                        ...
                        //manhnd 10/12/2021 cap nhat lai MAGIAIDON sau khi huy chuyen
                        AKT_DON_GIAIDOAN oGD = dt.AKT_DON_GIAIDOAN.Where(x => x.DONID == DonID && x.TOAANID == oChuyenAn.TOACHUYENID).FirstOrDefault();

                        AKT_DON oDon = dt.AKT_DON.Where(X => X.ID == DonID).FirstOrDefault();
                        if (oGD == null)
                        {
                            oGD = dt.AKT_DON_GIAIDOAN.Where(x => x.DONID == DonID && x.TOAPHUCTHAMID == oChuyenAn.TOACHUYENID).FirstOrDefault();
                        }
                        oDon.MAGIAIDOAN = oGD.MAGIAIDOAN;
                        dt.SaveChanges();
                    }
        ...
        protected void rptCapNhat_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (hddTHGN.Value == "05")
                {
                    ...
                    {
                        AKT_DON oDonST = dt.AKT_DON.Where(x => x.MAVUVIEC == mavuviec && x.TOAANID != toaanST).FirstOrDefault();
