...
namespace WEB.GSTP.QLAN.AKT.XuLyDon
{
    public partial class Danhsach : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                sDONID = Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + "";
                try
                {
                    SetDonGhep();
                    ...
                    AKT_DON_XULY oDXuly = dt.AKT_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 1).FirstOrDefault();
                    if (oDXuly != null)
                    {
                        AKT_CHUYEN_NHAN_AN ca = dt.AKT_CHUYEN_NHAN_AN.Where(x => x.VUANID == DONID).FirstOrDefault();
                        if (ca != null)
                        {
                    ...
                    AKT_CHUYEN_NHAN_AN objCN = dt.AKT_CHUYEN_NHAN_AN.Where(x => x.MAP_VUANID_NEW == DONID && x.TRUONGHOPGIAONHANID == 266).FirstOrDefault();
                    if (objCN != null)
                    {//nếu là đơn chuyển trong hệ thống toà án thì không check
                        ADS_DON_XULY oldDonXuLy = dt.ADS_DON_XULY.Where(x => x.DONID == objCN.VUANID && x.LOAIGIAIQUYET == 1).FirstOrDefault();
                        if (oldDonXuLy == null)
                        {
        ...
        void SetNewSoTB()
        {
            ...
            AKT_DON_BL oSTBL = new AKT_DON_BL();
            //Số Thông báo mới
            DateTime ngayTB;
            ...
            String STTNew = oSTBL.GET_STB_XLDon_NEW_V2(DonViID, "AKT", ngayTB).ToString();
        ...
        private void CheckQuyen(decimal DONID)
        {
            ...
            List<AKT_DON_THAMPHAN> lstCount = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETDON).ToList();
            if (lstCount.Count == 0)
            {
            ...
            //List<AKT_SOTHAM_THULY> lstTL = dt.AKT_SOTHAM_THULY.Where(x => x.DONID == DONID).ToList();
            //if (lstTL.Count > 0)
            //{
        ...
        {
            List<AKT_DON_XULY> lstXL = dt.AKT_DON_XULY.Where(x => x.DONID == ID && x.LOAIGIAIQUYET == 5).ToList();
            if (lstXL.Count == 0)
            {
                Cls_Comon.SetButton(cmdThemmoiAP, false);
                Cls_Comon.SetButton(cmdCapNhatAP, false);
        ...
        void LoadDropDuongSu(decimal DONID)
        {
            ...
            DataTable obj = oDSBL.AKT_DON_DUONGSU_ANPHI(DONID);
        ...
        {
            AKT_DON_XULY_BL oBL = new AKT_DON_XULY_BL();
            string current_id = Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + "";
        ...
        protected void rpt_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "Them":
                    ...
                    if (DonID > 0)
                    {
                        AKT_DON_DUONGSU dds = dt.AKT_DON_DUONGSU.Where(x => x.DONID == DonID && x.ISDAIDIEN == 1 && x.TUCACHTOTUNG_MA == "NGUYENDON").FirstOrDefault();
                        txtDuongSu.Text = dds.TENDUONGSU;
                    }
                    else
                    {
                        var a = (from d in dt.DON_DUONGSU_CHITIET
                                 join s in dt.AKT_DON_DUONGSU on d.DUONGSUID equals s.ID
                ...
                case "Xoa":
                    ...
                    AKT_DON_XULY oT = dt.AKT_DON_XULY.Where(x => x.ID == CurrID).FirstOrDefault();
                    ...
                    AKT_TONGDAT oTD = dt.AKT_TONGDAT.Where(x => x.DONID == DONID).FirstOrDefault();
                    if (oTD != null)
                    {
                    ...
                    string MATHONGBAO = "";
                    var DVCQG_THANH_TOAN = DataExtensions.GetAllWithClause<BL.GSTP.BANGSETGET.DVCQG_THANH_TOAN>("ANPHI_ID = " + CurrID + " AND MALOAIVUVIEC = '4'");
                    ...
                    decimal _VALUE = 0;
                    obj.DVCQG_THANH_TOAN_DELETE_XULY(CurrID, "4", ref _VALUE);
                    if (_VALUE == 1)
                    {
                        ...
                        AKT_ANPHI oAnphi = dt.AKT_ANPHI.Where(x => x.DONID == oT.DONID).FirstOrDefault();
                        if (oAnphi != null)
                        {
                            ...
                            log.InsertLog(Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI].ToString(), CurrID.ToString(), MATHONGBAO, Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Delete", CONTENT_JSON, CONTENT_JSON_OLD);

                            dt.AKT_ANPHI.Remove(oAnphi);
                        }

                        //-----

                        dt.AKT_DON_XULY.Remove(oT);
                        ...
                        if (oT.LOAIGIAIQUYET == 4 || oT.LOAIGIAIQUYET == 5)
                        {
                            ...
                            DataTable oYC = oBL.AKT_GETALL_DON_YCBS(DonID, CurrID, 1, 5);
                ...
                case "Download":
                    CurrID = Convert.ToDecimal(e.CommandArgument.ToString());
                    var oND = dt.AKT_FILE.Where(x => x.ID == CurrID).FirstOrDefault();
                    if (oND.TENFILE != "")
                    {
                ...
                case "BoSung":
                    ...
                    string link = "/QLAN/AKT/XuLyDon/Popup/pBoSungTaiLieu.aspx?ID=" + CurrID;
                ...
                case "TraLaiDon":
                    CurrID = Convert.ToDecimal(e.CommandArgument.ToString());
                    Cls_Comon.CallFunctionJS(this, this.GetType(), "LoadModalDiv();");
                    string linkPop = "/QLAN/AKT/XuLyDon/Popup/pTraLaiDon.aspx?ID=" + CurrID;
        ...
        {
            //DONGHEP.Visible = false;
            AKT_DON_XULY obj = dt.AKT_DON_XULY.Where(x => x.ID == CurrID).Single<AKT_DON_XULY>();
            if (obj != null)
            {
                ...
                {
                    AKT_FILE objFile = dt.AKT_FILE.Where(x => x.ID == obj.FILEID).FirstOrDefault();
                ...
                switch (bienphap)
                {
                    ...
                    default:
                        ...
                        pnThongbao.Visible = false;
                        //Lanh chuyen sang load danh sach an phi
                        //List<AKT_ANPHI> lstAP = dt.AKT_ANPHI.Where(x => x.DONID == obj.DONID).ToList();
                        //if (lstAP.Count > 0)
                        //{
                        //    AKT_ANPHI objAP = lstAP[0];
        ...
        protected void rpt_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                else
                {
                    ...
                    AKT_FILE oF = dt.AKT_FILE.Where(x => x.ID == vFILEID).FirstOrDefault();
                    if (oF != null)
                    {
                        if (oF.TENFILE != null)
                        {
                    ...
                    if (rowView["ID"] + "" != "")
                    {
                        ...
                        AKT_DON_XULY oXL = dt.AKT_DON_XULY.Where(x => x.ID == idXuLy).FirstOrDefault();
                        decimal? DONCHITIETID = oXL.DON_CHITIETID;
                        decimal ISDONCHITIET = DON_ID == 0 ? 1 : 0;
                        AKT_DON_XULY_BL oBL = new AKT_DON_XULY_BL();
                        DataTable oDT = oBL.CHECK_AKT_DON_DUONGSU_ANPHI_V2(DONID, DONCHITIETID, ISDONCHITIET);
                        if (oDT.Rows.Count > 0)
                        {
                            lblSua.Visible = false;
                            lbtXoa.Visible = false;
                    ...
                    {
                        decimal idXuLy = Convert.ToDecimal((rowView["ID"] + "").ToString());
                        AKT_DON_XULY obj = dt.AKT_DON_XULY.Where(x => x.ID == idXuLy).FirstOrDefault();
                        AKT_DON_XULY_BL oBL = new AKT_DON_XULY_BL();
                        DataTable oYC = oBL.AKT_GETALL_DON_YCBS(DONID, idXuLy, 1, 5);
                        bool existsYCBS = false;
                        decimal adsYCBoSungDon = Convert.ToDecimal(ENUM_AKT_BIENPHAPGQ.AKT_YCBoSungDon);
                        decimal adsTraLaiDon = Convert.ToDecimal(ENUM_AKT_BIENPHAPGQ.AKT_TraLaiDon);
                    ...
                    {
                        decimal idXuLy = Convert.ToDecimal((rowView["ID"] + "").ToString());
                        decimal adsThuLy = Convert.ToDecimal(ENUM_AKT_BIENPHAPGQ.AKT_ThuLy);

                        AKT_DON_XULY obj = dt.AKT_DON_XULY.Where(x => x.ID == idXuLy && x.LOAIGIAIQUYET == adsThuLy).FirstOrDefault();
                        //ADS_DON_XULY_BL oBL = new ADS_DON_XULY_BL();
                        //DataTable oYC = oBL.AKT_GETALL_DON_YCBS(DONID, idXuLy, 1, 5);
                    ...
                    AKT_SOTHAM_THULY oTLST = dt.AKT_SOTHAM_THULY.Where(x => x.DONID == DONID).FirstOrDefault();
        ...
        private decimal UploadFileID(AKT_DON oDon, decimal FileID, string strMaBieumau, string STT, string stb_Phu)
        {
        ...
        protected void cmdCapNhat_Click(object sender, EventArgs e)
        {
            ...
            if (hddCurrID.Value != "" && hddCurrID.Value != "0")
            {
                ...
                obj = dt.AKT_DON_XULY.Where(x => x.ID == DonXyLyID).FirstOrDefault();
                ...
                else obj = new AKT_DON_XULY();
                if (obj.LOAIGIAIQUYET == 4 && bienphap != "4")
                {
                    ADS_DON_XULY_BL oBL = new ADS_DON_XULY_BL();
                    DataTable oYC = oBL.AKT_GETALL_DON_YCBS(DONID, obj.ID, 1, 5);
                    if (oYC.Rows.Count > 0) oBL.DEL_DON_YCBS_GETBYDONID(Convert.ToDecimal(oYC.Rows[0]["ID"].ToString()));
                }
            }
            else
            {
                ...
                //if (dt.AKT_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 5).ToList().Count > 0)
                //{
                //    lbtthongbao.Text = "Đơn đã được thụ lý, không được phép thêm mới !";
                //    return;
                //}
                //if (dt.AKT_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 3).ToList().Count > 0)
                //{
                //    lbtthongbao.Text = "Đơn đã trả lại, không được phép thêm mới !";
                //    return;
                //}
                obj = new AKT_DON_XULY();
                AKT_DON_BL oBL = new AKT_DON_BL();
                ...
                if (txtSothongbao.Text != "")
                {
                    ...
                    decimal check = oBL.CHECKSTT_AKT((decimal)oDon.TOAANID, (decimal)oDon.MAGIAIDOAN, vNam, 0, Convert.ToDecimal(txtSothongbao.Text), ddlStbPhu.SelectedValue, obj.ID);
                    DateTime ngaythongbao = DateTime.Parse(this.txtNgaythongbao.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
                    String strMsg = "";
                    AKT_DON_BL oBL_STB = new AKT_DON_BL();
                    String STTNew = oBL_STB.GET_STB_XLDon_NEW_V2((decimal)oDon.TOAANID, "AKT", ngaythongbao).ToString();
                    //Đã tồn tại số thông báo
                    if (check > 0)
                    {
            ...
            {
                //List<AKT_ANPHI> lstAP = dt.AKT_ANPHI.Where(x => x.DONID == DONID).ToList();
                //AKT_ANPHI objAP = new AKT_ANPHI();
                //if (lstAP.Count > 0)
                //{
                //    objAP = lstAP[0];
                ...
                //    objAP.NGAYTAO = DateTime.Now;
                //    objAP.NGUOITAO = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                //    dt.AKT_ANPHI.Add(objAP);
                //    dt.SaveChanges();
                //}
                /////insert vào bảng DVCQG_THANH_TOAN
                //DVCQG_THANH_TOAN_BL obj_anphi = new DVCQG_THANH_TOAN_BL();
                //obj_anphi.DVCQG_THANH_TOAN_INSERT_ANPHI(2, DONID, obj.ID, objAP.ID, "4");

                AKT_DON_XULY_BL oBLYC = new AKT_DON_XULY_BL();
            ...
            else if (bienphap == ENUM_ADS_BIENPHAPGQ.ADS_ChuyenDonTrongNganh)
            {
                ...
                //    oNSD.IDANDANSU = 0;
                //    dt.SaveChanges();
                //    Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] = 0;
                //    UpdateTrangThaiDonKK();
                //    //Cls_Comon.ShowMessageAndRedirect(this, this.GetType(), "MsgXLDON", "Hoàn thành chuyển đơn đến tòa án khác, bạn hãy chọn đơn khác để xử lý tiếp !", Cls_Comon.GetRootURL() + "/QLAN/AKT/Hoso/Danhsach.aspx");                    
                //}
            }
            ...
            else
            {
                ...
                //    Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] = 0;
                //    UpdateTrangThaiDonKK();
                //    Cls_Comon.ShowMessageAndRedirect(this, this.GetType(), "MsgXLDON", "Hoàn thành giải quyết đơn, bạn hãy chọn đơn khác để xử lý tiếp !", Cls_Comon.GetRootURL() + "/QLAN/AKT/Hoso/Danhsach.aspx");
                //}
            }
            #endregion
        ...
        private void ChuyenDonSangToaAnMoi(AKT_DON oDon, decimal DonXyLyID, AKT_DON_XULY obj, decimal ToaAnCuEdit)
        {
            if (DonXyLyID == 0)
            {
                ...
                GD.GAIDOAN_INSERT_UPDATE("4", oDonMoi.ID, 2, obj.CDTN_TOAANID.Value, 0, 0, 0, 0);
                ...
                foreach (var item in lstoTT)
                {
                    ...
                    if (oTT.TOA_GIAIQUYET_ID == null && item.TOA_GIAIQUYET_ID != null)
                    {
                ...
                foreach (var item in lstTailieu)
                {
                    ...
                    // insert toa_gq_id
                    if (objtl.TOA_GIAIQUYET_ID == null && item.TOA_GIAIQUYET_ID != null)
                    {
                        objtl.TOA_GIAIQUYET_ID = item.TOA_GIAIQUYET_ID;
                    }
            ...
            else
            {
                ...
                AKT_DON oDonNew = dt.AKT_DON.FirstOrDefault(s => s.DONID_TOACU == obj.DONID && s.TOAANID == ToaAnCuEdit);
                if (oDonNew != null)
                {
                    if (oDonNew.TOAANID.Value != obj.CDTN_TOAANID)
                    {
                        ...
                        oDonNew.MAVUVIEC = ENUM_LOAIVUVIEC.AN_KINHDOANH_THUONGMAI + oTA.MA + oDonNew.TT.ToString();
                        dt.SaveChanges();

                        GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                        GD.GAIDOAN_UPDATE("4", oDonNew.ID, 2, Convert.ToDecimal(oDonNew.TOAANID), 0, 0, 0, 0);
                    }
                }
        ...
        private void DeleteDonChuyenToaAnKhac(AKT_DON_XULY obj)
        {
            AKT_DON oDon = dt.AKT_DON.FirstOrDefault(s => s.DONID_TOACU == obj.DONID && s.TOAANID == obj.CDTN_TOAANID);
            if (oDon != null)
            {
                if (dt.AKT_DON_THAMGIATOTUNG.Count(s => s.DONID == oDon.ID) > 0)
                {
                    List<AKT_DON_THAMGIATOTUNG> lstTGTT = dt.AKT_DON_THAMGIATOTUNG.Where(s => s.DONID == oDon.ID).ToList();
                    dt.AKT_DON_THAMGIATOTUNG.RemoveRange(lstTGTT);
                }
                if (dt.AKT_DON_TAILIEU.Count(s => s.DONID == oDon.ID) > 0)
                {
                    List<AKT_DON_TAILIEU> lstTGTT = dt.AKT_DON_TAILIEU.Where(s => s.DONID == oDon.ID).ToList();
                    dt.AKT_DON_TAILIEU.RemoveRange(lstTGTT);
                }
                List<AKT_DON_DUONGSU> lstDS = dt.AKT_DON_DUONGSU.Where(s => s.DONID == oDon.ID).ToList();
                GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                GD.GIAIDOAN_DELETES("4", oDon.ID, 2);

                dt.AKT_DON.Remove(oDon);
                dt.AKT_DON_DUONGSU.RemoveRange(lstDS);
                dt.SaveChanges();
            }
        ...
        void UpdateTrangThaiDonKK()
        {
            ...
            string loai_an = ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI + "";
            decimal vuviecid = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + "");
            string yeucau = "";
            switch (bienphap_gd)
            {
                case 1:
        ...
        protected void lbtDownload_Click(object sender, EventArgs e)
        {
            ...
            AKT_FILE oND = dt.AKT_FILE.Where(x => x.ID == FileID).FirstOrDefault();
            if (oND.TENFILE != "")
            {
                var cacheKey = Guid.NewGuid().ToString("N");
                Context.Cache.Insert(key: cacheKey, value: oND.NOIDUNG, dependencies: null, absoluteExpiration: DateTime.Now.AddSeconds(30), slidingExpiration: System.Web.Caching.Cache.NoSlidingExpiration);
        ...
        protected void TinhTamUngAnPhi()
        {
            ...
            {
                txtTamUng = txtGTTC * 5 / 100;
            }
            else if (txtGTTC <= 800000000)
            {
                txtTamUng = 20000000 + (txtGTTC - 400000000) * 4 / 100;
            }
            else if (txtGTTC <= 2000000000)
            {
                txtTamUng = 36000000 + (txtGTTC - 800000000) * 3 / 100;
            }
            else if (txtGTTC <= 4000000000)
            {
                txtTamUng = 72000000 + (txtGTTC - 2000000000) * 2 / 100;
            }
            else
            {
                txtTamUng = 112000000 + (txtGTTC - 4000000000) * 1 / 1000;
            }
        ...
        private void LoadGrid_AnPhi()
        {
            ...
            DataTable oDT = oBL.AKT_ANPHI_GETBYDONID_V2(DONID, 1, pageindex, page_size);
            if (oDT.Rows.Count > 0)
            {
            ...
            switch (e.CommandName)
            {
                ...
                case "XoaAP":
                    try
                    {
                        ...
                        obj.CHECK_DELETE_ANPHI(CurrID, "4", ref _check_out);
                        if (_check_out == 1)
                        {
                            AKT_ANPHI objAP = dt.AKT_ANPHI.Where(x => x.ID == CurrID).FirstOrDefault();
                            AKT_TONGDAT oTD = dt.AKT_TONGDAT.Where(x => x.DONID == objAP.DONID).FirstOrDefault();

                            DON_MIENANPHI_BL dON = new DON_MIENANPHI_BL();
                            DataTable dataTable = dON.GET_DON_MIENANPHI_BY_ANPHI_ID(objAP.ID, 4);
                            AKT_TONGDAT_BL dS_TONGDAT_BL = new AKT_TONGDAT_BL();
                            ...
                            if (oTD != null && objAP.TINHTRANG != 1 && dataTable.Rows.Count == 0)
                            {
                                ...
                                && dS_TONGDAT_BL.AKT_TONGDATDOITUONG_ANPHI_GETBYANPHIID_TONGDATID(objAP.DONID, objAP.ID).Rows.Count > 0)
                            {
                                lbThongbaoAP.Text = "Bạn không thể xóa khi đã tống đạt!";
                                return;
                            }
                            else
                            {
                                ...
                                dt.AKT_ANPHI.Remove(objAP);

                                dt.SaveChanges();
                                lbThongbaoAP.Text = "Xóa thành công!";
                                //DELETE DVCQG_THANH_TOAN-------------
                                decimal _VALUE = 0;
                                obj.DVCQG_THANH_TOAN_DELETE_ANPHI(CurrID, "4", ref _VALUE);
                            }
                        }
                        else
                        {
                ...
                case "LSTBAnPhi":
                    try
                    {
                        CurrID = Convert.ToDecimal(e.CommandArgument.ToString());
                        Cls_Comon.CallFunctionJS(this, this.GetType(), "LoadModalDiv();");
                        string linkPop = "/QLAN/AKT/XuLyDon/Popup/pMienAnPhi.aspx?ID=" + CurrID;
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "onclick", "var Mleft = (screen.width/2)-(950/2); var Mtop = (screen.height/2)-(600/2); javascript:window.open('" + linkPop + "', '_blank', 'height=600px,width=950px,top=\'+Mtop+\', left=\'+Mleft+\',resizable=yes,toolbar=no,scrollbars=0'); ", true);
                    }
                    catch (Exception ex) { lbThongbaoAP.Text = ex.Message; }
                    break;
        ...
        void LoadInfo_AnPhi(Decimal CurrID)
        {
            ...
            List<AKT_ANPHI> lst = dt.AKT_ANPHI.Where(x => x.ID == CurrID).ToList<AKT_ANPHI>();
        ...
        protected void rptAP_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                DAL.GSTP.DVCQG_THANH_TOAN oAP = dt.DVCQG_THANH_TOAN.Where(x => x.ANPHI_ID == vAPID && x.MALOAIVUVIEC == "4").FirstOrDefault();
                AKT_SOTHAM_THULY oTLST = dt.AKT_SOTHAM_THULY.Where(x => x.DONID == DONID).FirstOrDefault();

                // Kiểm tra xem đơn đã được trả lại (LOAIGIAIQUYET == 3) hay chưa
                bool isDonTraLai = dt.AKT_DON_XULY.Any(x => x.DONID == DONID && x.LOAIGIAIQUYET == 3);

                // án phí
                DON_MIENANPHI_BL dON_MIENANPHI_BL = new DON_MIENANPHI_BL();
                DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(vAPID, 4);

                AKT_ANPHI aDS_ANPHI = dt.AKT_ANPHI.Where(x => x.ID == vAPID).FirstOrDefault();
        ...
        protected void cmdCapNhatAP_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    try
                    {
                        objAP = dt.AKT_ANPHI.Where(x => x.ID == APId).FirstOrDefault();
                        if (APId != 0)
                        {
                            objAP.NGAYSUA = DateTime.Now;
                            objAP.NGUOISUA = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                    ...
                    catch (Exception ex) { objAP = new AKT_ANPHI(); }
                    objAP.DONID = DONID;
                    objAP.TINHTRANG = chkNopAnPhi.Checked == true ? 1 : 0;
                    objAP.GIATRITRANHCHAP = (String.IsNullOrEmpty(txtGiaTriTranhChap.Text.Trim())) ? 0 : Convert.ToDecimal(txtGiaTriTranhChap.Text.Trim(), cul);
                    objAP.MUCGIAMANPHI = (String.IsNullOrEmpty(txtMucGiamAnPhi.Text.Trim())) ? 0 : Convert.ToDecimal(txtMucGiamAnPhi.Text.Trim(), cul);
                    ...
                    if (APId == 0)
                    {
                        ...
                        // insert toa_gq_id
                        if (objAP.TOA_GIAIQUYET_ID == null)
                        {
                    ...
                    {
                        decimal donID = Convert.ToDecimal(current_id);
                        AKT_DON_XULY donXL = dt.AKT_DON_XULY.Where(x => (x.DONID == DONID || x.DON_XULYID == DONID) && x.LOAIGIAIQUYET == 5).FirstOrDefault();
                        DVCQG_THANH_TOAN_BL obj_anphi = new DVCQG_THANH_TOAN_BL();

                        // lưu ý
                        obj_anphi.DVCQG_THANH_TOAN_INSERT_ANPHI(2, donID, donXL.ID, objAP.ID, ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI, Convert.ToString(objAP.DUONGSU_ID));
                    }
                    else if (APId != 0)
                    {
                        ...
                        // lưu ý
                        obj.DVCQG_THANH_TOAN_UP_ANPHI(objAP.ID, ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI);
                    }
                    //anhpn add log án phí 08/01/2025
                    string MATHONGBAO = "";

                    // lưu ý
                    var DVCQG_THANH_TOAN = DataExtensions.GetAllWithClause<BL.GSTP.BANGSETGET.DVCQG_THANH_TOAN>("ANPHI_ID = " + objAP.ID + " AND MALOAIVUVIEC = '4'");
                    if (DVCQG_THANH_TOAN.Count() > 0)
                    {
                        MATHONGBAO = DVCQG_THANH_TOAN.FirstOrDefault().MA_THONGBAO;
                    }
                    ...
                    if (chkNopAnPhi.Checked == false)
                    {
                        ...
                        AKT_DON_XULY donXLS = dt.AKT_DON_XULY.Where(x => (x.DONID == DONID || x.DON_XULYID == DONID) && x.LOAIGIAIQUYET == 5).FirstOrDefault();
                        if (donXLS != null && donXLS.LOAIGIAIQUYET == 5)
                        {
                            DataTable oDT = obj_anphiS.CHECK_THANH_TOAN_IN_ANPHI(2, donIDS, donXLS.ID, objAP.ID, "4", objAP.DUONGSU_ID + "");
                            if (oDT.Rows.Count == 0)
                            {
                                obj_anphiS.DVCQG_THANH_TOAN_INSERT_ANPHI(2, donIDS, donXLS.ID, objAP.ID, "4", objAP.DUONGSU_ID + "");
                            }
                        }
                    }
                    else
                    {
                        ...
                        AKT_DON aDS_DON = dt.AKT_DON.Where(x => x.ID == DONID).FirstOrDefault();
                        DM_BIEUMAU bmMienAnPhi = dt.DM_BIEUMAU.Where(x => x.MABM == "100-DS").FirstOrDefault();
                        AKT_FILE objFile = dt.AKT_FILE.Where(x => x.DONID == DONID && x.BIEUMAUID == bmMienAnPhi.ID).FirstOrDefault();
                        if (objFile != null)
                        {
        ...
        private bool CheckValidAP()
        {
            ...
            else
            {
                ...
                if (hddCurrAPID.Value != "" && hddCurrAPID.Value != "0")
                {
                    ...
                    AKT_ANPHI objAP = dt.AKT_ANPHI.Where(x => x.ID == APId).FirstOrDefault();
                    AKT_TONGDAT oTD = dt.AKT_TONGDAT.Where(x => x.DONID == objAP.DONID).FirstOrDefault();
                    if (oTD != null)
                    {
                ...
                AKT_ANPHI ap = dt.AKT_ANPHI.Where(x => x.DUONGSU_ID == dsID && x.ID != APId && APId != 0).FirstOrDefault();
                if (ap != null)
                {
        ...
        void SetNewSoTBAP()
        {
            ...
            String STTNew = oSTBL.GET_STB_ANPHI_NEW(DonViID, "AKT", ngayTB).ToString();
