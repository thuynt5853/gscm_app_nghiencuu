    ...
    public partial class pMienAnPhi : System.Web.UI.Page
    {
        ...
        protected void cmdCapNhatAP_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(APId, 4);
                ...
                try
                {
                    ...
                    if ((dataTable != null && dataTable.Rows.Count > 0 && hddHistoryId.Value != "") || hddHistoryId.Value == "")
                    {
                        ...
                        decimal Mienanphi_ID = dON_MIENANPHI_BL.UPSERT_DON_MIENANPHI(ID_MIENANPHI, APId, DONID, 4, v_lydo, nguoitao, ngaytao, nguoisua, ngaysua, v_sothongbao, d_ngaythongbao, v_stb_phu);
                    ...
                    // lưu ý
                    var DVCQG_THANH_TOAN = DataExtensions.GetAllWithClause<BL.GSTP.BANGSETGET.DVCQG_THANH_TOAN>("ANPHI_ID = " + APId + " AND MALOAIVUVIEC = '4'");
        ...
        private bool CheckValidAP()
        {
            ...
            AKT_ANPHI objAnPhi = dt.AKT_ANPHI.Where(x => x.ID == A_ANPHI_ID).FirstOrDefault();
            ...
            DON_MIENANPHI_BL dON_MIENANPHI_BL = new DON_MIENANPHI_BL();
            DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(A_ANPHI_ID, 4);

           // decimal dsID = objAnPhi.DUONGSU_IDS.Value;
            ...
            if (hddHistoryId.Value != "")
            {
                ...
                if (dS_TONGDAT_BL.AKT_TONGDATDOITUONG_ANPHI_GETBYANPHIID_TONGDATID(objAnPhi.DONID, objAnPhi.ID).Rows.Count > 0)
                {
            ...
            // {
            //     lbThongbaoAP.Text = "Bạn không thể xóa khi thụ lý sơ thẩm!";
            //     return false;
            // }
        ...
        protected void rptAP_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "DownloadAP":
                    ...
                    break;
                case "SuaAP":
                    AKT_ANPHI oND1 = dt.AKT_ANPHI.Where(x => x.ID == CurrID).FirstOrDefault();
                    // khong duoc xoa khi da Tong dat - 27/11/2025 vnpt check
                    if (oND1 != null)
                    {
                        AKT_TONGDAT oTD = dt.AKT_TONGDAT.Where(x => x.DONID == oND1.DONID && x.MAPID == oND1.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AKT_ANPHI).FirstOrDefault();
                        if (oTD != null)
                        {
                            lbThongbaoAP.Text = "Bạn không thể sửa khi đã tống đạt!";
                            return;
                ...
                case "XoaAP":
                    try
                    {
                        ...
                        AKT_ANPHI oND2 = dt.AKT_ANPHI.Where(x => x.ID == CurrID).FirstOrDefault();
                        // khong duoc xoa khi da Tong dat - 27/11/2025 vnpt check
                        if (oND2 != null)
                        {
                            AKT_TONGDAT oTD1 = dt.AKT_TONGDAT.Where(x => x.DONID == oND2.DONID && x.MAPID == oND2.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AKT_ANPHI).FirstOrDefault();
                            if (oTD1 != null)
                            {
                                lbThongbaoAP.Text = "Bạn không thể sửa khi đã tống đạt!";
                                return;
                        ...
                        obj.CHECK_DELETE_ANPHI(CurrID, "4", ref _check_out);//anhvh check delete 06/09/2023
                        if (_check_out == 1)
                        {
                            ...
                            AKT_TONGDAT_BL dS_TONGDAT_BL = new AKT_TONGDAT_BL();
                            if (dS_TONGDAT_BL.AKT_TONGDATDOITUONG_ANPHI_GETBYANPHIID_TONGDATID(objAP.DONID, objAP.ID).Rows.Count > 0)
                            {
                            ...
                            else
                            {
                                ...
                                LOG_THONGTINANPHI_QLTA log = new LOG_THONGTINANPHI_QLTA();
                                log.InsertLog(Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI].ToString(), CurrID.ToString(), MATHONGBAO, Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Update", CONTENT_JSON, CONTENT_JSON_OLD);
        ...
        protected void rptAP_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                AKT_DON_XULY tralaiDonXuly = dt.AKT_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 3).FirstOrDefault();

                DON_MIENANPHI_BL dON_MIENANPHI_BL1 = new DON_MIENANPHI_BL();
                DataTable dataTable1 = dON_MIENANPHI_BL1.GET_DON_MIENANPHI_BY_ANPHI_ID(vAPID, 4);
                ...
                {
                    AKT_ANPHI objAP = dt.AKT_ANPHI.Where(x => x.ID == vAPID).FirstOrDefault();
                    DON_MIENANPHI_BL dON_MIENANPHI_BL = new DON_MIENANPHI_BL();
        ...
        void LoadInfo_AnPhiDaMien(Decimal CurrID)
        {
            ...
            hddHistoryId.Value = CurrID + "";
            AKT_ANPHI aDSAnphi = dt.AKT_ANPHI.Where(x => x.ID == CurrID).FirstOrDefault();
            if (aDSAnphi != null)
            {
                chkNopAnPhi.Checked = true;
        ...
        private void LoadGrid_AnPhi()
        {
            ...
            decimal A_ANPHI_ID = Convert.ToDecimal(Request.QueryString["ID"].ToString());
            DataTable oDT = oBL.AKT_ANPHI_LICHSU_GETBYDONID(DONID, A_ANPHI_ID, 1, pageindex, page_size);
            //if (oDT.Rows.Count > 0)
            //{
