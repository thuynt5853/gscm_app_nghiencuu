...
namespace WEB.GSTP.QLAN.AKT.XuLyDon.Popup
{
    public partial class pBoSungTaiLieu : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                string current_id = Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + "", bienphap = dropBienPhapGQ.SelectedValue;
                ...
                try
                {
                    ...
                    DataTable obj = aDXL.AKT_GETALL_DON_YCBS(Convert.ToDecimal(DONID), xuLyID, 1, 20);
                    ...
                    AKT_DON_XULY donXL = dt.AKT_DON_XULY.Where(x => x.ID == xuLyID).FirstOrDefault();
        ...
        protected void cmdCapNhat_Click(object sender, EventArgs e)
        {
            ...
            decimal xuLyID = (String.IsNullOrEmpty(Request["ID"] + "")) ? 0 : Convert.ToDecimal(Request["ID"] + "");
            AKT_DON_XULY oXuLy = dt.AKT_DON_XULY.Where(x => x.ID == xuLyID).FirstOrDefault();
            AKT_DON oDon = dt.AKT_DON.Where(x => x.ID == DONID).FirstOrDefault();
            ...
            DataTable checkGQ = oBLYC.CHECK_NGAYGQ_DON_YCBS(obj.ID, xuLyID, 4, txtNgayGQ.Text);
            ...
            AKT_DON_XULY objXuLy = dt.AKT_DON_XULY.Where(x => x.ID == obj.DON_XULY_YCBS_ID).FirstOrDefault();
            ...
            DataTable oDT = oBLYC.AKT_GETALL_DON_YCBS(DONID, objXuLy.ID, 1, 1);
            ...
            DataTable objYCBS = oBLYC.AKT_GETALL_DON_YCBS(Convert.ToDecimal(DONID), obj.DON_XULY_YCBS_ID.Value, 1, 20);
        ...
        protected void Reset()
        {
            ...
            DataTable donYCBS = oBLYC1.AKT_GETALL_DON_YCBS(Convert.ToDecimal(DONID), xuLyID, 1, 20);
        ...
        protected void dgDS_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "Xoa":
                    ...
                    DataTable donYCBSHandle = oBLDel.GET_DON_YCBS_GETBYDONID(ID);
                    
                    if (donYCBSHandle.Rows[0]["FILEID"] != null)
                    {

                        decimal FileID = donYCBSHandle.Rows[0]["FILEID"] != null ? donYCBSHandle.Rows[0]["FILEID"].toNumber() : 0;
                        if (FileID > 0)
                        {
                            try
                            {
                    ...
                    DataTable obj = oBLDel.AKT_GETALL_DON_YCBS(Convert.ToDecimal(DONID), ID_DON_YCBS, 1, 20);
                    if (e.Item.ItemIndex == 0 && obj.Rows.Count > 0)
                    {
                        AKT_DON_XULY objXuLy = dt.AKT_DON_XULY.Where(x => x.ID == ID_DON_YCBS).FirstOrDefault();
                    ...
                    {
                        AKT_DON_XULY objXuLy = dt.AKT_DON_XULY.Where(x => x.ID == ID_DON_YCBS).FirstOrDefault();
                        dt.AKT_DON_XULY.Remove(objXuLy);
                        dt.SaveChanges();
                    }
        ...
        private void validateEnableButton(decimal DONID, decimal DON_YCBSID)
        {
            decimal xuLyID = (String.IsNullOrEmpty(Request["ID"] + "")) ? 0 : Convert.ToDecimal(Request["ID"] + "");
            ADS_DON_XULY_BL aDXL = new ADS_DON_XULY_BL();
            DataTable objYCBS = aDXL.AKT_GETALL_DON_YCBS(Convert.ToDecimal(DONID), xuLyID, 1, 20);
        ...
        protected void dgDS_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                decimal ID_DON_YCBS = Convert.ToDecimal(Request.QueryString["ID"].ToString());
                AKT_DON_XULY oXL = dt.AKT_DON_XULY.Where(x => x.ID == ID_DON_YCBS).FirstOrDefault();
                ...
                if (e.Item.ItemIndex == 0)
                {
                    ...
                    DataTable anphiList = oBL.CHECK_AKT_DON_DUONGSU_ANPHI_V2(DONID, DONCHITIETID, ISDONCHITIET);
                ...
                // DataTable obj = oB.AKT_GETALL_DON_YCBS(Convert.ToDecimal(DONID), ID_DON_YCBS, 1, 20);
        ...
        private void LoadDSTL()
        {
            ...
            DataTable oDT = oBL.AKT_GETALL_DON_YCBS(DONID, ID_DON_YCBS, pageindex, page_size);
            dgDS.DataSource = oDT;
            dgDS.DataBind();
        }
    }
