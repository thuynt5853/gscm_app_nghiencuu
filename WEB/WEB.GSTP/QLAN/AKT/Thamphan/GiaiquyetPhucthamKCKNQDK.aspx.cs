...
namespace WEB.GSTP.QLAN.AKT.Thamphan
{
    public partial class GiaiquyetPhucthamKCKNQDK : System.Web.UI.Page
    {
        ...
        void check_Quyen(decimal DONID)
        {
            ...
            DataTable oDT = oBL.AKT_PHUCTHAM_KCKN_TGTT_GETLIST(DONID);
            ...
            AKT_KCKN_PHUCTHAM_BL BL = new AKT_KCKN_PHUCTHAM_BL();
            DataTable DT = BL.AKT_KCKNQDK_PHUCTHAM_HDXX_GETLIST(DONID);
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                AKT_KCKNQDK_PHUCTHAM_HDXX oTHTT = DataExtensions.GetAllWithClause<AKT_KCKNQDK_PHUCTHAM_HDXX>($"DONID = {DONID} AND MAVAITRO = '{ENUM_NGUOITIENHANHTOTUNG.THAMPHAN}'AND TOA_GIAIQUYET_ID ={Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID])}").FirstOrDefault();
                if (oTHTT == null)
                {
                    oTHTT = new AKT_KCKNQDK_PHUCTHAM_HDXX();
                    oTHTT.DONID = DONID;
        ...
        public void xoa(decimal id)
        {
            ...
            if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.DONID), 4, Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Thẩm phán giải quyết Phúc thẩm án Kinh tế", "Xóa", json) == false)
            {
