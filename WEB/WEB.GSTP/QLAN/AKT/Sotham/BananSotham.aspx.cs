...
namespace WEB.GSTP.QLAN.AKT.Sotham
{
    public partial class BananSotham : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                DSID = (String.IsNullOrEmpty(Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + "")) ? 0 : Convert.ToDecimal(Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI]);
                hddURLKS.Value = Cls_Comon.GetRootURL() + "/FileUploadHandler.aspx";
                hddDonID.Value = Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + "" == "" ? "0" : Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + "";
                if (hddDonID.Value == "0") Response.Redirect(Cls_Comon.GetRootURL() + "/QLAN/AKT/Hoso/Danhsach.aspx");
        ...
        void SetNewSoQD()
        {
            ...
            AKT_SOTHAM_BL oSTBL = new AKT_SOTHAM_BL();
        ...
        bool CheckCongbo(decimal ID)
        {
            var list = DataExtensions.GetAllWithClause<BAQD_CONGBO>(
                $"VUVIECID = {ID} AND LOAIANID = {ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI} AND CAPXETXU = 2 AND TRANGTHAI IN (2,3)"
            );
        ...
        private void CheckQuyen(decimal ID)
        {
            ...
            List<AKT_DON_THAMPHAN> lstTPGQ = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == ID && x.MAVAITRO == "VTTP_GIAIQUYETSOTHAM").ToList();
            if (lstTPGQ.Count == 0)
            {
            ...
            List<AKT_SOTHAM_HDXX> lstTPCT = dt.AKT_SOTHAM_HDXX.Where(x => x.DONID == ID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).ToList();
            if (lstTPCT.Count == 0)
            {
                lstErr.Text = lbthongbaoQD.Text = "Chưa cập nhật thông tin hội đồng xét xử !";
                DisableButtonsBanan();
            ...
            //List<AKT_SOTHAM_QUYETDINH> lstQDXX = dt.AKT_SOTHAM_QUYETDINH.Where(x => x.DONID == ID && x.LOAIQDID == IDLQD).ToList();
            //if (lstQDXX.Count == 0)
            //{
            ...
            //Decimal soDuongSuChuaXacThuc = dt.AKT_DON_DUONGSU.Count(x => x.DONID == ID && x.XACTHUC_DLDCQG == 0 && x.QUOCTICHID == 2);
        ...
        private bool CheckQuyenSua(decimal ID)
        {
            ...
            AKT_SOTHAM_BANAN ba = dt.AKT_SOTHAM_BANAN.Where(x => x.DONID == ID).FirstOrDefault();
            if (ba != null)
            {
            ...
            List<decimal> dmQDIds = dt.DM_QD_QUYETDINH.Where(x => x.ISSOTHAM == 1 && x.ISKDTM == 1 && x.KET_THUC == 1).Select(x => x.ID).ToList();
            List<AKT_SOTHAM_QUYETDINH> qdKetThuc = dt.AKT_SOTHAM_QUYETDINH.Where(x => x.DONID == ID && dmQDIds.Contains(x.QUYETDINHID.Value)).ToList();
            if (ba != null && qdKetThuc != null)
            {
                AKT_SOTHAM_KHANGCAO kc2 = dt.AKT_SOTHAM_KHANGCAO.Where(x => x.DONID == ID).FirstOrDefault();
                if (kc2 != null)
                {
                ...
                AKT_SOTHAM_KHANGNGHI kn2 = dt.AKT_SOTHAM_KHANGNGHI.Where(x => x.DONID == ID).FirstOrDefault();
                if (kn2 != null)
                {
            ...
            {
                AKT_SOTHAM_KHANGCAO kc = dt.AKT_SOTHAM_KHANGCAO.Where(x => x.DONID == ID && x.TINHTRANG_GIAIQUYET == 1).OrderByDescending(x => x.ID).FirstOrDefault();
                AKT_SOTHAM_KHANGNGHI kn = dt.AKT_SOTHAM_KHANGNGHI.Where(x => x.DONID == ID && x.TINHTRANG_GIAIQUYET == 1).OrderByDescending(x => x.ID).FirstOrDefault();
                if (kc != null)
                {
                    var kcChuaGiaiQuyet = dt.AKT_SOTHAM_KHANGCAO.Where(x => x.DONID == ID && x.ID > kc.ID && x.TINHTRANG_GIAIQUYET != 2).ToList();
                ...
                if (kn != null)
                {
                    var knChuaGiaiQuyet = dt.AKT_SOTHAM_KHANGNGHI.Where(x => x.DONID == ID && x.ID > kn.ID && x.TINHTRANG_GIAIQUYET != 3).ToList();
            ...
            AKT_TONGDAT td = dt.AKT_TONGDAT.Where(x => x.DONID == ID && (x.BIEUMAUID == 230 || x.BIEUMAUID == 261)).FirstOrDefault();
            if (td != null)
            {
        ...
        private void LoadDuongSuYC()
        {
            ...
            decimal DonID = Convert.ToDecimal(hddDonID.Value);
            List<AKT_DON_DUONGSU> lstDS = dt.AKT_DON_DUONGSU.Where(x => x.DONID == DonID).OrderBy(x => x.TENDUONGSU).ToList<AKT_DON_DUONGSU>();
        ...
        private void LoadQD()
        {
            ...
            DM_QUYETDINH_VUAN_KETTHUC oBL = new DM_QUYETDINH_VUAN_KETTHUC();
            DataTable oDT = oBL.DM_QUYETDINH_VUAN_SOTHAM_KETTHUC(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI);
        ...
        private void LoadNguoiKyInfo()
        {
            ...
            AKT_SOTHAM_HDXX oND = dt.AKT_SOTHAM_HDXX.Where(x => x.DONID == DonID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).OrderByDescending(x => x.NGAYPHANCONG).FirstOrDefault<AKT_SOTHAM_HDXX>();
            if (oND != null)
            {
            ...
            {
                AKT_DON_THAMPHAN oTP = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == DonID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).OrderByDescending(x => x.NGAYPHANCONG).FirstOrDefault();
                if (oTP != null)
                {
        ...
        {
            Decimal ID = Convert.ToDecimal(hddDonID.Value);
            List<AKT_SOTHAM_QUYETDINH> lstQD = dt.AKT_SOTHAM_QUYETDINH.Where(x => x.DONID == ID && (x.LOAIQDID == 10 || x.QUYETDINHID == 423 || x.QUYETDINHID == 422 || x.QUYETDINHID == 70 || x.QUYETDINHID == 425 || x.LOAIQDID == 3)).ToList();
            if (lstQD.Count >= 1)
            {
        ...
        {
            AKT_SOTHAM_QUYETDINH oND = dt.AKT_SOTHAM_QUYETDINH.Where(x => x.ID == id).FirstOrDefault();
            if (oND.NOIDUNG == null && oND.QT_FILE_ID != null)
            {
                ...
                {
                    qtFileDelete.DESCRIPTION = "Tài khoản " + Session[ENUM_SESSION.SESSION_USERNAME] + " đã xóa file tại form BanAnSoTham " + ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI + ".";
                    QT_FILE_BL fileH = new QT_FILE_BL();
                    fileH.DeleteFileLogic(qtFileDelete);
                }
            }
            TK_SOTHAM_QUYETDINH TK = DataExtensions.GetAllWithClause<TK_SOTHAM_QUYETDINH>($"QUYETDINHID = {id} AND LOAIAN = {Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI)}").FirstOrDefault();
            if (TK != null)
            {
            ...
            var list = DataExtensions.GetAllWithClause<BAQD_CONGBO>($"  VUVIECID = {oND.DONID.Value} " +
                                                                    $"  AND LOAIANID = {Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI)} " +
                                                                    $"  AND CAPXETXU = {2} ");
            ...
            if (CheckQuyenSua(Convert.ToDecimal(oND.DONID + "")) == true)
            {
                if (oND != null)
                {
                    ...
                    dt.AKT_SOTHAM_QUYETDINH.Remove(oND);
                    ...
                    if (FileID > 0)
                    {
                        try
                        {
                            AKT_FILE objf = dt.AKT_FILE.Where(x => x.ID == FileID).FirstOrDefault();
                            dt.AKT_FILE.Remove(objf);
                            dt.SaveChanges();
                        }
                        catch (Exception ex) { }
                    }
        ...
        {   
            try
            {
                ...
                KHOBAQD_BL ald = new KHOBAQD_BL();
                bool isExist = ald.IsExistKHOBADQ(1, DONID, 2,ENUM_LOAIVUVIEC_TEXT.AN_KINHDOANH_THUONGMAI);
                if (isExist)
                {
                ...
                else
                {
                    ...
                    TK = DataExtensions.GetAllWithClause<TK_SOTHAM_QUYETDINH>($"QUYETDINHID= {ID} AND LOAIAN =  {Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI)}").FirstOrDefault();
                    oND = dt.AKT_SOTHAM_QUYETDINH.Where(x => x.ID == ID).FirstOrDefault();
                    if (oND.TOAANID.ToString() != Session[ENUM_SESSION.SESSION_DONVIID].ToString())
                    {
                ...
                try
                {
                    if (hddFilePathQD.Value != "")
                    {
                        ...
                        QT_FILE qtFile = fileHelper.InsertFile_Minio_Banan(strFilePath, Convert.ToInt32(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI), "BANANSOTHAM");
                        if (qtFile == null)
                        {
                ...
                if (pnLyDo.Visible == true) oND.LYDOID = Convert.ToDecimal(ddlLydo.SelectedValue + "");


                if (!ddlQuyetdinh.SelectedItem.Text.Contains("19-VDS") && ddlQuyetdinh.SelectedValue != "4")
                {
                    TK.DONID = DONID;
                    TK.LOAIAN = Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI);
                ...
                if (oQDT.ISCONGBO == 1 && oND.HIEULUCTU != null)
                {
                    ...
                    temp_congbo.LOAIANID = Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI);
        ...
        private decimal UploadFileID(AKT_DON oDon, decimal FileID, string strMaBieumau, decimal STT)
        {
            AKT_DON_BL oBL = new AKT_DON_BL();
            decimal IDFIle = 0;
            decimal IDBM = 0;
            ...
            if (FileID > 0)
                objFile = dt.AKT_FILE.Where(x => x.ID == FileID).FirstOrDefault();
            ...
            if (FileID == 0)
                dt.AKT_FILE.Add(objFile);
            dt.SaveChanges();
            IDFIle = objFile.ID;
            return IDFIle;
        }
        private decimal UploadFileIDQD(AKT_DON oDon, decimal FileID, string strMaBieumau, decimal STT)
        {
            AKT_DON_BL oBL = new AKT_DON_BL();
            decimal IDFIle = 0;
            decimal IDBM = 0;
        ...
        {
            AKT_DON oDon = dt.AKT_DON.FirstOrDefault(s => s.ID == DONID);
            DONKK_USER_DKNHANVB obj = dkk.DONKK_USER_DKNHANVB.FirstOrDefault(s => s.MAVUVIEC == oDon.MAVUVIEC && s.VUVIECID == oDon.ID && s.MALOAIVUVIEC == ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI && s.TRANGTHAI == 1);
            if (obj != null)
            {
        ...
        private void SetEnable_data_QD(AKT_SOTHAM_QUYETDINH QD)
        {
            if (QD != null)
            {
                ...
                try
                {
                    ...
                    bool isExist = oDonBL.IsExistKHOBADQ(1, QD.DONID ?? 0, 2, ENUM_LOAIVUVIEC_TEXT.AN_KINHDOANH_THUONGMAI);
                    if (isExist)
                    {
                ...
                AKT_TONGDAT oTD = dt.AKT_TONGDAT.Where(x => x.DONID == QD.DONID && x.MAPID == QD.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AKT_SOTHAM_QUYETDINH).FirstOrDefault();
                if (oTD != null)
                {
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            {
                case "Download":
                    var oND = dt.AKT_SOTHAM_QUYETDINH.Where(x => x.DONID == ID && x.FILEID == ND_id).FirstOrDefault();
                    ...
                    else
                    {
                        ...
                        string _pathStore = QT_FILE_BL.ToPathFolderStore(qT_FILE.DATE_CREATED.Value, Convert.ToInt32(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI)) + "\\BANANSOTHAM";
                ...
                case "Sua":
                    hddFilePathQD.Value = "";
                    lbthongbao.Text = "";
                    AKT_SOTHAM_QUYETDINH oND1 = dt.AKT_SOTHAM_QUYETDINH.Where(x => x.ID == ND_id).FirstOrDefault();
                    // khong duoc xoa khi da Tong dat - 28/11/2025 vnpt check
                    //AKT_TONGDAT oTD = dt.AKT_TONGDAT.Where(x => x.DONID == oND1.DONID && x.MAPID == oND1.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AKT_SOTHAM_QUYETDINH).FirstOrDefault();
                    //if (oTD != null)
                    //{
                ...
                case "Xoa":
                    ...
                    AKT_SOTHAM_QUYETDINH oND2 = dt.AKT_SOTHAM_QUYETDINH.Where(x => x.ID == ND_id).FirstOrDefault();
                    // khong duoc xoa khi da Tong dat - 27/11/2025 vnpt check
                    AKT_TONGDAT oTD1 = dt.AKT_TONGDAT.Where(x => x.DONID == oND2.DONID && x.MAPID == oND2.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AKT_SOTHAM_QUYETDINH).FirstOrDefault();
                    if (oTD1 != null)
                    {
                    ...
                    //GTEL-HUNGQ 22-09-2025 Dữ liệu Quyết định đã được chia sẻ không được xóa
                    KHOBAQD_BL ald = new KHOBAQD_BL();
                    bool isExist = ald.IsExistKHOBADQ(1, DonID, 2,ENUM_LOAIVUVIEC_TEXT.AN_KINHDOANH_THUONGMAI);
                    if (isExist)
                    {
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                AKT_SOTHAM_KHANGCAO oTKC = dt.AKT_SOTHAM_KHANGCAO.Where(x => x.DONID == DONID).FirstOrDefault();
                AKT_SOTHAM_KHANGNGHI oTKN = dt.AKT_SOTHAM_KHANGNGHI.Where(x => x.DONID == DONID).FirstOrDefault();
        ...
        protected void ddlQuyetdinh_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (btnUpdate.Enabled == true)
            {
                ...
                //--Kiểm tra đã phân công thẩm phán chưa
                AKT_DON_THAMPHAN oTP = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == IDD && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).FirstOrDefault();
                if (oTP == null)
                {
                ...
                {
                    //Kiểm tra xem đã có quyết định đưa vụ việc ra xét xử hay không
                    AKT_SOTHAM_QUYETDINH QDST = dt.AKT_SOTHAM_QUYETDINH.Where(x => x.DONID == IDD && x.LOAIQDID == 5 && x.QUYETDINHID != 147 && x.QUYETDINHID != 148 /*Quyết định chuyển vụ án giải quyết theo thủ tục rút gọn sang giải quyết theo thủ tục thông thường*/).OrderByDescending(x => x.NGAYQD).FirstOrDefault();
                    if (QDST == null)
                    {
            ...
            if (oT != null)
            {
                ...
                if (ID == 62 || ID == 63 || ID == 67 || ID == 68 || ID == 41 || ID == 42 || ID == 45 || ID == 147 || ID == 4 || ID == 61)
                {
                    ...
                    String STTNew = oSTBL.GET_SQD_NEW(DonViID, "AKT", ngayQD, ID).ToString();
                    txtSoQD.Text = STTNew;
                }
            }
            LoadLydo();
        ...
        protected void ddlNguoiYC_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            List<AKT_DON_DUONGSU> lstDS = dt.AKT_DON_DUONGSU.Where(x => x.DONID == DonID && x.ID != NguoiYC).OrderBy(x => x.TENDUONGSU).ToList<AKT_DON_DUONGSU>();
        ...
        private void LoadAnPhi()
        {
            ...
            dgAnPhi.DataSource = oBL.AKT_SOTHAM_BANAN_ANPHI_GET(DONID);
            dgAnPhi.DataBind();
        ...
        private void LoadTGTT()
        {
            ...
            decimal DONID = Convert.ToDecimal(current_id);
            AKT_SOTHAM_BL oBL = new AKT_SOTHAM_BL();
            DataTable dtTGTT = oBL.AKT_SOTHAM_BANAN_TGTT_GET(DONID);
        ...
        {
            List<DM_BOLUAT> lst = dt.DM_BOLUAT.Where(x => x.LOAI == ENUM_LOAIVUVIEC.AN_KINHDOANH_THUONGMAI && x.HIEULUC == 1).OrderByDescending(y => y.NGAYBANHANH).ToList();
            if (lst != null && lst.Count > 0)
            {
        ...
        {
            decimal ID = Convert.ToDecimal(hddDonID.Value);
            dgFile.DataSource = dt.AKT_SOTHAM_BANAN_FILE.Where(x => x.DONID == ID).ToList();
            dgFile.DataBind();
            string current_id = Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + "";
            decimal DONID = Convert.ToDecimal(current_id);
            AKT_DON oT = dt.AKT_DON.Where(x => x.ID == DONID).FirstOrDefault();
        ...
        private void LoadDieuLuat()
        {
            ...
            decimal DONID = Convert.ToDecimal(current_id);
            AKT_SOTHAM_BL oBL = new AKT_SOTHAM_BL();
            dgDieuLuat.DataSource = oBL.AKT_SOTHAM_BANAN_DIEULUAT_GET(DONID);
            dgDieuLuat.DataBind();
        }
        private void LoadBanAnInfo(decimal DonID)
        {
            List<AKT_SOTHAM_BANAN> lst = dt.AKT_SOTHAM_BANAN.Where(x => x.DONID == DonID).ToList();
            if (lst.Count > 0)
            {
            ...
            else
            {
                ...
                AKT_SOTHAM_THULY tl = dt.AKT_SOTHAM_THULY.Where(x => x.DONID == DonID).OrderByDescending(x => x.NGAYTHULY).FirstOrDefault();
                if (tl != null)
                {
                    if (tl.QHPLTKID != null)
                    {
        ...
        private bool CheckValidQDVV()
        {
            ...
            if (!String.IsNullOrEmpty(txtNgayQD.Text) && !String.IsNullOrEmpty(txtSoQD.Text))
            {
                ...
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal LoaiQD = Convert.ToDecimal(ddlQuyetdinh.SelectedValue);
                Decimal CheckID = oSTBL.CHECK_SQDTheoLoaiAn(DonViID, "AKT", so, ngay, LoaiQD);
                if (CheckID > 0)
                {
                    String strMsg = "";
                    String STTNew = oSTBL.GET_SQD_NEW(DonViID, "AKT", ngay, LoaiQD).ToString();
            ...
            decimal ID = Convert.ToDecimal(hddID.Value);
            DM_QUYETDINH_VUAN_KETTHUC oBL = new DM_QUYETDINH_VUAN_KETTHUC();
            DataTable oDT = oBL.DGLIST_BAQD_QUYETDINH_KETTHUC_ST(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI, DONID);
            ...
            //if (oND != null)
            //{
            //    AKT_TONGDAT oTD = dt.AKT_TONGDAT.Where(x => x.DONID == oND.DONID && x.MAPID == oND.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AKT_SOTHAM_QUYETDINH).FirstOrDefault();
            //    if (oTD != null)
            //    {
        ...
        private bool CheckValid()
        {
            ...
            if (!String.IsNullOrEmpty(txtNgaytuyenan.Text))
            {
                ...
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal CheckID = oSTBL.CheckSoBATheoLoaiAn(DonViID, "AKT", so, ngayBA);
                if (CheckID > 0)
                {
                    Decimal CurrBanAnId = (string.IsNullOrEmpty(hddBanAnID.Value)) ? 0 : Convert.ToDecimal(hddBanAnID.Value);
                    String strMsg = "";
                    String STTNew = oSTBL.GETSoBANEWTheoLoaiAn(DonViID, "AKT", ngayBA).ToString();
        ...
        private void LoadCombobox()
        {
            ...
            ddlQHPLQDVV.DataSource = dt.DM_QHPL_TK.Where(x => x.STYLES == ENUM_QHPLTK.KINHDOANH_THUONGMAI && x.ENABLE == 1).OrderBy(y => y.ARRTHUTU).ToList();
            ...
            //decimal ID = Convert.ToDecimal(ddlQuyetdinh.SelectedValue);
            ddlLoaiQD.DataSource = dt.DM_QD_LOAI.Where(x => x.HIEULUC == 1 && x.ISKDTM == 1).OrderBy(y => y.THUTU).ToList();
            ...
            string current_id = Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + "";
            decimal DONID = Convert.ToDecimal(current_id);
            AKT_SOTHAM_HDXX oHD = dt.AKT_SOTHAM_HDXX.Where(x => x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN && x.DONID == DONID).OrderByDescending(x => x.NGAYPHANCONG).FirstOrDefault();
            if (oHD != null)
            {
        ...
        protected void lbtDownload_Click(object sender, EventArgs e)
        {
            ...
            {
                decimal DONID = Convert.ToDecimal(hddDonID.Value);
                AKT_FILE oND = dt.AKT_FILE.Where(x => x.DONID == DONID).FirstOrDefault();
                if (oND.TENFILE != "")
                {
                    var cacheKey = Guid.NewGuid().ToString("N");
        ...
        protected void cmdUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                AKT_SOTHAM_BANAN_FILE oTF = new AKT_SOTHAM_BANAN_FILE();
                //GTEL-HUNGQ 22-09-2025 thêm check có bị can chưa xác thực thì không cho Lưu
                //Decimal soDuongSuChuaXacThuc = dt.AKT_DON_DUONGSU.Count(x => x.DONID == DONID && x.XACTHUC_DLDCQG == 0 && x.QUOCTICHID == 2);
                //if (soDuongSuChuaXacThuc > 0)
                //{
                //    lstErr.Text = lbthongbaoQD.Text = "Có đương sự chưa xác thực thông tin. Đề nghị xác thực tại màn hình Danh sách đương sự.";
                //    return;
                ...
                KHOBAQD_BL ald = new KHOBAQD_BL();
                bool isExist = ald.IsExistKHOBADQ(0, DONID, 2,ENUM_LOAIVUVIEC_TEXT.AN_KINHDOANH_THUONGMAI);
                if (isExist)
                {
                    lstErr.Text = lbthongbaoQD.Text = "Vụ việc đã được đồng bộ. Phải thu hồi đồng bộ trước khi chỉnh sửa/xóa";
                    return;
                ...
                if (lst.Count == 0)
                {
                    ...
                    oND = new AKT_SOTHAM_BANAN();
                }
                else
                {
                    oND = lst[0];
                    // khong duoc xoa khi da Tong dat - 27/11/2025 vnpt check
                    AKT_TONGDAT oTD2 = dt.AKT_TONGDAT.Where(x => x.DONID == oND.DONID && x.MAPID == oND.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AKT_SOTHAM_BANAN).FirstOrDefault();
                    if (oTD2 != null)
                    {
                ...
                try
                {
                    if (hddFilePath.Value != "")
                    {
                        ...
                        dt.AKT_SOTHAM_BANAN_FILE.Add(oTF);
                            dt.SaveChanges();
                        //}
                        #endregion
                        //File.Delete(strFilePath);
                ...
                /* 25.04.2025 Gọi hàm UploadFileID, lưu vào bảng AKT_FILE là có Bản án
                 * File bản án nếu đính kèm sẽ lưu vào bảng AKT_SOTHAM_BANAN_FILE
        ...
        {
            AKT_DON oDon = dt.AKT_DON.FirstOrDefault(s => s.ID == DONID);
            DONKK_USER_DKNHANVB obj = dkk.DONKK_USER_DKNHANVB.FirstOrDefault(s => s.MAVUVIEC == oDon.MAVUVIEC && s.VUVIECID == oDon.ID && s.MALOAIVUVIEC == ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI && s.TRANGTHAI == 3);
            if (obj != null)
            {
        ...
        protected void dgFile_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "Xoa":
                    ...
                    AKT_SOTHAM_BANAN_FILE oT = dt.AKT_SOTHAM_BANAN_FILE.Where(x => x.ID == ND_id).FirstOrDefault();
                    if (oT.NOIDUNG == null)
                    {
                    ...
                    AKT_FILE oDsF = dt.AKT_FILE.Where(x => x.DONID == oT.DONID && x.TENFILE == oT.TENFILE).FirstOrDefault();
                    if (oDsF != null)
                    {
                        dt.AKT_FILE.Remove(oDsF);
                        dt.SaveChanges();
                    }
                    dt.AKT_SOTHAM_BANAN_FILE.Remove(oT);
                    dt.SaveChanges();
                    LoadFile();
                    break;
                case "Download":
                    AKT_SOTHAM_BANAN_FILE oND = dt.AKT_SOTHAM_BANAN_FILE.Where(x => x.ID == ND_id).FirstOrDefault();
                    if (oND.NOIDUNG != null && oND.QT_FILE_ID == null)
                    {
                        var cacheKey = Guid.NewGuid().ToString("N");
        ...
        protected void cmdLuatUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dt.AKT_SOTHAM_BANAN_DIEULUAT.Where(x => x.DONID == DONID && x.DIEULUATID == DIEUID).ToList().Count > 0)
                {
                    lstMsgDieuluat.Text = "Đã tồn tại điều luật này trong danh sách !";
                    return;
                }

                AKT_SOTHAM_BANAN_DIEULUAT oT = new AKT_SOTHAM_BANAN_DIEULUAT();
                oT.DONID = DONID;
                oT.DIEULUATID = DIEUID;
                oT.NGAYTAO = DateTime.Now;
                oT.NGUOITAO = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                dt.AKT_SOTHAM_BANAN_DIEULUAT.Add(oT);
                dt.SaveChanges();
                LoadDieuLuat();
        ...
        protected void dgDieuLuat_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "Xoa":
                    ...
                    AKT_SOTHAM_BANAN_DIEULUAT oT = dt.AKT_SOTHAM_BANAN_DIEULUAT.Where(x => x.ID == ND_id).FirstOrDefault();
                    dt.AKT_SOTHAM_BANAN_DIEULUAT.Remove(oT);
                    dt.SaveChanges();
                    LoadDieuLuat();
                    break;
            }
        ...
        protected void cmdAnphi_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (DataGridItem oItem in dgAnPhi.Items)
                {
                    ...
                    TextBox txtNgaynhanbanan = (TextBox)oItem.FindControl("txtNgaynhanbanan");
                    List<AKT_SOTHAM_BANAN_ANPHI> lst = dt.AKT_SOTHAM_BANAN_ANPHI.Where(x => x.DONID == DONID && x.DUONGSU == DSID).ToList();
                    if (lst.Count == 0)
                    {
                        AKT_SOTHAM_BANAN_ANPHI oT = new AKT_SOTHAM_BANAN_ANPHI();
                        oT.DONID = DONID;
                        oT.DUONGSU = DSID;
                        ...
                        oT.NGAYTAO = DateTime.Now;
                        oT.NGUOITAO = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                        dt.AKT_SOTHAM_BANAN_ANPHI.Add(oT);
                        dt.SaveChanges();
                    }
                    else
                    {
                        AKT_SOTHAM_BANAN_ANPHI oT = lst[0];
                        oT.DONID = DONID;
                        oT.DUONGSU = DSID;
                        oT.MIENANPHI = chkMien.Checked == true ? 1 : 0;
                        oT.ANPHI = txtAnphi.Text == "" ? 0 : Convert.ToDecimal(txtAnphi.Text.Replace(".", ""));
        ...
        protected void cmdTGTT_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (DataGridItem oItem in dgTGTT.Items)
                {
                    ...
                    List<AKT_SOTHAM_BANAN_TGTT> lst = dt.AKT_SOTHAM_BANAN_TGTT.Where(x => x.DONID == DONID && x.THAMGIATOTUNGID == DSID).ToList();
                    if (lst.Count == 0)
                    {
                        AKT_SOTHAM_BANAN_TGTT oT = new AKT_SOTHAM_BANAN_TGTT();
                        oT.DONID = DONID;
                        ...
                        dt.AKT_SOTHAM_BANAN_TGTT.Add(oT);
                        dt.SaveChanges();
                    }
                    else
                    {
                        AKT_SOTHAM_BANAN_TGTT oT = lst[0];
                        oT.DONID = DONID;
                        oT.THAMGIATOTUNGID = DSID;
                        oT.ISTHAMGIA = chkThamgiaTGTT.Checked == true ? 1 : 0;
                        oT.NGAYNHANBANAN = (String.IsNullOrEmpty(txtNgayTGTT.Text.Trim())) ? (DateTime?)null : DateTime.Parse(txtNgayTGTT.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
        ...
        protected void cmdHuyBanAn_Click(object sender, EventArgs e)
        {
            ...
            List<AKT_SOTHAM_BANAN> banans = dt.AKT_SOTHAM_BANAN.Where(x => x.DONID == DonID).ToList();
            if (banans.Count > 0)
            {
                //GTEL-HUNGQ 07-10-2025 Dữ liệu Bản án đã được chia sẻ không được xóa
                KHOBAQD_BL ald = new KHOBAQD_BL();
                ...
                if (oBL.HISTORY_ALLDATA_BY_VUANID(DonID, 4, Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Bản án Sơ thẩm án Kinh tế", "Xóa", json) == false)
                {
                    lbthongbao.Text = "Xóa không thành công!";
                    return;
                }//Ket thuc
                 //Xoa Bản án Sơ thẩm
                dt.AKT_SOTHAM_BANAN.RemoveRange(banans);
            }
            // Xóa thông tin file đính kèm
            List<AKT_SOTHAM_BANAN_FILE> files = dt.AKT_SOTHAM_BANAN_FILE.Where(x => x.DONID == DonID).ToList();
            if (files.Count > 0)
            {
                ...
                dt.AKT_SOTHAM_BANAN_FILE.RemoveRange(files);
            }
            // Xóa điều luật áp dụng, tội danh
            List<AKT_SOTHAM_BANAN_DIEULUAT> dieuLuats = dt.AKT_SOTHAM_BANAN_DIEULUAT.Where(x => x.DONID == DonID).ToList();
            if (dieuLuats.Count > 0)
            {
                dt.AKT_SOTHAM_BANAN_DIEULUAT.RemoveRange(dieuLuats);
            }
            // Xóa thông tin án phí
            List<AKT_SOTHAM_BANAN_ANPHI> anPhis = dt.AKT_SOTHAM_BANAN_ANPHI.Where(x => x.DONID == DonID).ToList();
            if (anPhis.Count > 0)
            {
                dt.AKT_SOTHAM_BANAN_ANPHI.RemoveRange(anPhis);
            }
            // Xóa thông tin người tham gia tố tụng
            List<AKT_SOTHAM_BANAN_TGTT> tGTTs = dt.AKT_SOTHAM_BANAN_TGTT.Where(x => x.DONID == DonID).ToList();
            if (tGTTs.Count > 0)
            {
                dt.AKT_SOTHAM_BANAN_TGTT.RemoveRange(tGTTs);
            }
            ...
            AKT_FILE file = dt.AKT_FILE.Where(x => x.DONID == DonID && x.BIEUMAUID == BieuMauID && x.MAGIAIDOAN == ENUM_GIAIDOANVUAN.SOTHAM).FirstOrDefault();
            if (file != null)
            {
                dt.AKT_FILE.Remove(file);
            }
        ...
        private void UploadFileID(AKT_DON oDon, string strMaBieumau, AKT_SOTHAM_BANAN_FILE fileDinhKem)
        {
            AKT_DON_BL oBL = new AKT_DON_BL();
            decimal IDBM = 0;
            string strTenBM = "";
            ...
            AKT_FILE objFile = dt.AKT_FILE.Where(x => x.DONID == oDon.ID && x.TOAANID == oDon.TOAANID && x.MAGIAIDOAN == oDon.MAGIAIDOAN && x.BIEUMAUID == IDBM).FirstOrDefault();
            if (objFile == null)
            {
                isNew = true;
                objFile = new AKT_FILE();
            }
            objFile.DONID = oDon.ID;
            objFile.TOAANID = oDon.TOAANID;
            objFile.MAGIAIDOAN = oDon.MAGIAIDOAN;
        ...
        {
            decimal ID = Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + "" == "" ? 0 : Convert.ToDecimal(Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI]);
            AKT_SOTHAM_THULY oT = dt.AKT_SOTHAM_THULY.Where(x => x.DONID == ID).FirstOrDefault();
