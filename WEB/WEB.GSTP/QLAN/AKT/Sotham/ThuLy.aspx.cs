...
namespace WEB.GSTP.QLAN.AKT.Sotham
{
    public partial class ThuLy : System.Web.UI.Page
    {
        ...
        private decimal loaian = Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI);
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    DataTable oDT = oBL.AKT_SOTHAM_THULY_GETLIST(ID);
        ...
        private void LoadBiDon(decimal donID)
        {
            ...
            //Load bị đơn 
            List<AKT_DON_DUONGSU> tb = dt.AKT_DON_DUONGSU.Where(x => x.DONID == donID && x.TUCACHTOTUNG_MA == "BIDON").OrderByDescending(x => x.ISDAIDIEN).ToList();
            if (tb.Count > 0)
            {
        ...
        private void LoadTHThuyLy(decimal vid)
        {
            ...
            AKT_DON obj = dt.AKT_DON.Where(x => x.ID == vid).FirstOrDefault();
        ...
        private decimal SetNewSoThuLy()
        {
            ...
            string sothulymoi = oSTBL.GET_STL_NEW(DonViID, "AKT", ngaythuly).ToString();
        ...
        void SetNewSoThongbao()
        {
            ...
            txtSothongbao.Text = oSTBL.GET_STBTL_NEW(DonViID, "AKT", ngaythongbao).ToString();
        ...
        private void CheckShowCommand(decimal DonID)
        {
            ...
            AKT_SOTHAM_QUYETDINH qd = dt.AKT_SOTHAM_QUYETDINH.Where(x => x.DONID == DonID && (x.LOAIQDID == 3 || x.LOAIQDID == 10 || x.LOAIQDID == 11 || x.LOAIQDID == 21)).FirstOrDefault();
            if (qd != null)
            {
        ...
        private void ResetControls()
        {
            ...
            LoadNguyenDon(Convert.ToDecimal(Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + ""));
            LoadBiDon(Convert.ToDecimal(Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + ""));
            decimal donID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + "");
            AKT_SOTHAM_THULY tl = dt.AKT_SOTHAM_THULY.Where(x => x.DONID == donID).FirstOrDefault();
            if (tl != null)
            {
        ...
        private bool CheckValid()
        {
            ...
            decimal DONID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI]);
            List<AKT_DON_XULY> lstGQD = dt.AKT_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 5).ToList();
            if (lstGQD.Count > 0)
            {
                AKT_DON_XULY oDXL = lstGQD[0];
            ...
            //    Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
            //    ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
            //    Decimal CheckID = oSTBL.CheckSoTLTheoLoaiAn(DonViID, "AKT", sothuly, ngaythuly);
            //    if (CheckID > 0)
            //    {
            //        Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
            //        String strMsg = "";
            //        String STTNew = oSTBL.GET_STL_NEW(DonViID, "AKT", ngaythuly).ToString();
            ...
            if (!String.IsNullOrEmpty(txtNgaythuly.Text))
            {
                ...
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal CheckID = oSTBL.CheckSoTBTLTheoLoaiAn(DonViID, "AKT", sothongbao, ngaythuly);
                if (CheckID > 0)
                {
                    Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
                    String strMsg = "";
                    String STTNew = oSTBL.GET_STBTL_NEW(DonViID, "AKT", ngaythuly).ToString();
                    if (CheckID != CurrThuLyID)
                    {
                        //lbthongbao.Text = "Số thụ lý này đã có!";
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                decimal FileID = 0;
                AKT_SOTHAM_THULY oND;
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    oND = new AKT_SOTHAM_THULY();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.AKT_SOTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
                ...
                AKT_SOTHAM_BL oBL = new AKT_SOTHAM_BL();
                DataTable oDT = oBL.AKT_SOTHAM_THULY_GETLIST(DONID);

                STPT_QUANLY_SOTHULY oQLSTL = new STPT_QUANLY_SOTHULY();
                if (oQLSTL.update_STPT_QUANLY_SOTHULY(2, 4, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), ((DateTime)oND.NGAYTHULY).ToString("dd/MM/yyyy", cul), Regex.Match(oND.SOTHULY, @"\d+").Value) == false)
                {
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    else
                    {
                        ...
                        oND.MATHULY = "S" + ENUM_LOAIVUVIEC.AN_KINHDOANH_THUONGMAI + Session[ENUM_SESSION.SESSION_MADONVI] + oND.TT.ToString();
                ...
                AKT_DON_DUONGSU ndcu = dt.AKT_DON_DUONGSU.Where(x => x.DONID == DONID && x.ISDAIDIEN == 1 && x.TUCACHTOTUNG_MA == "NGUYENDON").FirstOrDefault();
                AKT_DON_DUONGSU bdcu = dt.AKT_DON_DUONGSU.Where(x => x.DONID == DONID && x.ISDAIDIEN == 1 && x.TUCACHTOTUNG_MA == "BIDON").FirstOrDefault();
                AKT_DON_DUONGSU ndmoi = dt.AKT_DON_DUONGSU.Where(x => x.ID == idNDmoi).FirstOrDefault();
                AKT_DON_DUONGSU bdmoi = dt.AKT_DON_DUONGSU.Where(x => x.ID == idBDmoi).FirstOrDefault();
                if (idNDmoi != ndcu.ID)
                {
                    ndcu.ISDAIDIEN = 0;
                    ndmoi.ISDAIDIEN = 1;
                }
                if (bdcu != null && idBDmoi != bdcu.ID)
                {
                    bdcu.ISDAIDIEN = 0;
                    bdmoi.ISDAIDIEN = 1;
                }
                if (idNDmoi != ndcu.ID || (bdcu != null && idBDmoi != bdcu.ID))
                {
                ...
                oBLs.CHECK_ENABLE_THULY(DONID, "4");
            }
            catch (Exception ex)
            {
        ...
        {
            AKT_SOTHAM_THULY oND = dt.AKT_SOTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "Sua":
                    lbthongbao.Text = "";
                    AKT_SOTHAM_THULY oND1 = dt.AKT_SOTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
                    // khong duoc SỬA khi da Tong dat - 29/11/2025 vnpt check
                    AKT_TONGDAT oTD = dt.AKT_TONGDAT.Where(x => x.DONID == oND1.DONID && x.MAPID == oND1.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AKT_SOTHAM_THULY).FirstOrDefault();
                    if (oTD != null)
                    {
                ...
                case "Xoa":
                    AKT_SOTHAM_THULY oND2 = dt.AKT_SOTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
                    // khong duoc SỬA khi da Tong dat - 29/11/2025 vnpt check
                    AKT_TONGDAT oTD1 = dt.AKT_TONGDAT.Where(x => x.DONID == oND2.DONID && x.MAPID == oND2.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AKT_SOTHAM_THULY).FirstOrDefault();
                    if (oTD1 != null)
                    {
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                List<AKT_SOTHAM_THULY> check_button_xoa = dt.AKT_SOTHAM_THULY.Where(x => x.DONID == DONID).ToList();
        ...
        private decimal UploadFileID(AKT_DON oDon, decimal FileID, string strMaBieumau, string STT)
        {
        ...
        private void txtQuanhephapluat_name(AKT_SOTHAM_THULY oT)
        {
            if (oT.QUANHEPHAPLUAT_NAME != null)
            {
                txtQuanhephapluat.Text = oT.QUANHEPHAPLUAT_NAME;
        ...
        protected void txtNgayThuLy_TextChanged(object sender, EventArgs e)
        {
            ...
            //    STPT_QUANLY_SOTHULY oQLSTL = new STPT_QUANLY_SOTHULY();
            //    DataTable dtStl = oQLSTL.get_STPT_QUANLY_SOTHULY(2, 4, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtNgaythuly.Text);
            //    if (dtStl != null)
            //    {
            ...
            //    string check_ngaythulycuoi_trongnam = oQLSTL.get_LATEST_DATE_IN_SOTHULY(2, 4, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtNgaythuly.Text);
        ...
        protected void btnSaveLydoXoa_Insert(object sender, EventArgs e)
        {
            ...
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            AKT_SOTHAM_THULY oND = dt.AKT_SOTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
            decimal DonID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + "");
            ...
            if (check_button_xoa.Count < 2 && oND != null)
            {
                ...
                AKT_SOTHAM_BANAN ba = dt.AKT_SOTHAM_BANAN.Where(x => x.DONID == DonID).FirstOrDefault<AKT_SOTHAM_BANAN>();
                if (ba != null)
                {
                ...
                AKT_SOTHAM_QUYETDINH qd = dt.AKT_SOTHAM_QUYETDINH.Where(x => x.DONID == DonID).FirstOrDefault<AKT_SOTHAM_QUYETDINH>();
                if (qd != null)
                {
                ...
                AKT_SOTHAM_HDXX hdxx = dt.AKT_SOTHAM_HDXX.Where(x => x.DONID == DonID).FirstOrDefault<AKT_SOTHAM_HDXX>();
                if (hdxx != null)
                {
                ...
                // Kiểm tra hòa giải
                AKT_SOTHAM_HOAGIAI hg = dt.AKT_SOTHAM_HOAGIAI.Where(x => x.DONID == DonID).FirstOrDefault<AKT_SOTHAM_HOAGIAI>();
                if (hg != null)
                {
                ...
                AKT_DON_THAMPHAN gqst = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == DonID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).FirstOrDefault<AKT_DON_THAMPHAN>();
                if (gqst != null)
                {
            ...
            //Xoa Thụ lý Sơ thẩm
            dt.AKT_SOTHAM_THULY.Remove(oND);
            dt.SaveChanges();
            if (FileID > 0)
            {
