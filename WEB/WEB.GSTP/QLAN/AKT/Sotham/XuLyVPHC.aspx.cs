...
namespace WEB.GSTP.QLAN.AKT.Sotham
{
    public partial class XuLyVPHC : System.Web.UI.Page
    {
        ...
        AKT_XULY_VIPHAMHC obj = new AKT_XULY_VIPHAMHC();
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                AKT_XULY_VIPHAMHC oT = dt.AKT_XULY_VIPHAMHC.Where(x => x.VUANID == VuAnID && x.MAGIAIDOAN == MaGiaiDoan).SingleOrDefault();
                if (oT != null)
                {
                    loadinfro();
                }
                AKT_DON oDT = dt.AKT_DON.Where(x => x.ID == VuAnID).FirstOrDefault();
                ...
                List<AKT_SOTHAM_THULY> lstCount = dt.AKT_SOTHAM_THULY.Where(x => x.DONID == VuAnID).ToList();
                if (lstCount.Count == 0)
                {
                ...
                List<AKT_DON_THAMPHAN> lstTP = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == VuAnID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).ToList();
                if (lstTP.Count == 0)
                {
                ...
                string Result = new AKT_CHUYEN_NHAN_AN_BL().Check_NhanAn(VuAnID, StrMsg, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]));
                if (Result != "")
                {
                ...
                //    int countItem = dt.AKT_DON_THAMPHAN.Count(s => s.THUKYID == CurrentUserId && s.DONID == VuAnID && s.MAVAITRO == "VTTP_GIAIQUYETSOTHAM");
                //    if (countItem > 0)
                //    {
                //        //được gán 
                //    }
        ...
        protected void cmdUpdateBottom_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                obj = dt.AKT_XULY_VIPHAMHC.Where(x => x.VUANID == mavuan && x.MAGIAIDOAN == MaGiaidoan).SingleOrDefault();
            }
            catch (Exception ex)
            {
