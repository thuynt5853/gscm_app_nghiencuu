...
namespace WEB.GSTP.QLAN.AKT.PhucthamQDK
{
    public partial class Quyetdinhphuctham : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    //    int countItem = dt.AKT_DON_THAMPHAN.Count(s => s.THUKYID == CurrentUserId && s.DONID == DONID && s.MAVAITRO == "VTTP_GIAIQUYETPHUCTHAM");
                    //    if (countItem > 0)
                    //    {
                    //        //được gán
                    //    }
        ...
        private void check_Quyen(decimal DONID)
        {
            ...
            DataTable oDT = oBL.AKT_PHUCTHAMKCKNQDK_BANANQUYETDINH_GETLIST(DONID);
        ...
        private void SetNewSoQD()
        {
            ...
            if (!String.IsNullOrEmpty(txtNgayQD.Text))
                ...
                ngayBD = DateTime.Now;
            txtSoQD.Text = oSTBL.GET_SQD_NEW(DonViID, "AKT_PT", ngayBD, LoaiQD).ToString();
        }
        ...
        private void LoadCombobox()
        {
            ...
            decimal DonID = Convert.ToDecimal(hddDonID.Value);
            AKT_PHUCTHAM_THULY tl = dt.AKT_PHUCTHAM_THULY.Where(x => x.DONID == DonID).OrderByDescending(x => x.NGAYTHULY).FirstOrDefault();
            if (tl != null)
            {
        ...
        private void LoadQD()
        {
            ...
            DataTable oDT = oBL.AKT_DM_QUYETDINH_VUAN_PT();
        ...
        private void LoadNguoiKyInfo()
        {
            ...
            AKT_KCKNQDK_PHUCTHAM_HDXX oND = DataExtensions.GetAllWithClause<AKT_KCKNQDK_PHUCTHAM_HDXX>($"DONID ={DonID} AND MAVAITRO ='{ENUM_NGUOITIENHANHTOTUNG.THAMPHAN}'").OrderByDescending(x => x.ID).FirstOrDefault();
            //  AKT_PHUCTHAM_HDXX oND = dt.AKT_PHUCTHAM_HDXX.Where(x => x.DONID == DonID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).OrderByDescending(x => x.ID).FirstOrDefault<AKT_PHUCTHAM_HDXX>();
            if (oND != null)
            {
        ...
        private bool CheckValid()
        {
            ...
            if (!String.IsNullOrEmpty(txtNgayQD.Text) && !String.IsNullOrEmpty(txtSoQD.Text))
            {
                ...
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal LoaiQD = Convert.ToDecimal(ddlQuyetdinh.SelectedValue);
                Decimal CheckID = oSTBL.CHECK_SQDTheoLoaiAn(DonViID, "AKT_PT", so, ngay, LoaiQD);
                if (CheckID > 0)
                {
                    Decimal CurrID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);

                    String strMsg = "";
                    String STTNew = oSTBL.GET_SQD_NEW(DonViID, "AKT_PT", ngay, LoaiQD).ToString();
                    if (CheckID != CurrID)
                    {
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.AKT_KCKNQDK_PHUCTHAM_QUYETDINH_GETLIST(ID);
        ...
        public void xoa(decimal id)
        {
            ...
            if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.DONID), Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI), Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Quyết định Phúc thẩm án Hôn nhân", "Xóa", json) == false)
            {
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                {
                    case "Download":
                        var oND = DataExtensions.FindById<AKT_KCKNQDK_PHUCTHAM_QUYETDINH>(ND_id);
                        if (oND.TENFILE != "")
                        {
        ...
        {
            decimal ID = Convert.ToDecimal(hddid.Value);
            AKT_KCKNQDK_PHUCTHAM_QUYETDINH oQD = DataExtensions.FindById<AKT_KCKNQDK_PHUCTHAM_QUYETDINH>(ID);
            // AKT_PHUCTHAM_QUYETDINH oQD = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.ID == ID).FirstOrDefault();
        ...
        protected void ddlQuyetdinh_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (oT != null)
            {
                ...
                if (ID == 143 || ID == 144 || ID == 145 || ID == 146 || ID == 61)
                {
                    ...
                    ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                    String STTNew = oSTBL.GET_SQD_NEW(DonViID, "AKT_PT", ngayQD, ID).ToString();
                    txtSoQD.Text = STTNew;
                }
            }
            LoadLydo();
