...
namespace WEB.GSTP.QLAN.AKT.PhucthamQDK
{
    public partial class NguoiTienhanhTT : System.Web.UI.Page
    {
        ...
        private void check_Quyen(decimal DONID)
        {
            ...
            DataTable oDT = oBL.AKT_KCKNQDK_PHUCTHAM_QUYETDINH_GETLIST(DONID);
        ...
        private void LoadCombobox()
        {
            ...
            if (loaiQD != null)
            {
                //AKT_PHUCTHAM_QUYETDINH qd = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.LOAIQDID == loaiQD.ID).FirstOrDefault<AKT_PHUCTHAM_QUYETDINH>();
                AKT_KCKNQDK_PHUCTHAM_QUYETDINH qd = DataExtensions.GetAllWithClause<AKT_KCKNQDK_PHUCTHAM_QUYETDINH>
                        ("DONID = " + DonID + " and LOAIQDID = " + loaiQD.ID).FirstOrDefault<AKT_KCKNQDK_PHUCTHAM_QUYETDINH>();
                if (qd != null)
                {
                    AKT_DON_THAMPHAN tpgq = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == DonID && x.MAVAITRO == "VTTP_GIAIQUYETPHUCTHAM").OrderByDescending(x => x.NGAYTAO).FirstOrDefault<AKT_DON_THAMPHAN>();
                    if (tpgq != null)
                    {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                List<AKT_DON_THAMPHAN> countTPGQ = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM && x.TOA_GIAIQUYET_ID == donvi_ID).ToList();
                //List<AKT_PHUCTHAM_HDXX> countChuToa = dt.AKT_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).ToList();
                List<AKT_KCKNQDK_PHUCTHAM_HDXX> countChuToa = DataExtensions.GetAllWithClause<AKT_KCKNQDK_PHUCTHAM_HDXX>($"DONID = {DONID} AND MAVAITRO = '{ENUM_NGUOITIENHANHTOTUNG.THAMPHAN}' AND TOA_GIAIQUYET_ID = {donvi_ID}") ;
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    if (tucach == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN)
                    {
                        ...
                        //oND = dt.AKT_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).FirstOrDefault<AKT_PHUCTHAM_HDXX>();
                        oND = DataExtensions.GetAllWithClause<AKT_KCKNQDK_PHUCTHAM_HDXX>($"DONID = {DONID} AND MAVAITRO = '{ENUM_NGUOITIENHANHTOTUNG.THAMPHAN}' AND TOA_GIAIQUYET_ID = {donvi_ID} ").FirstOrDefault();
                    ...
                    oND = new AKT_KCKNQDK_PHUCTHAM_HDXX();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    //oND = dt.AKT_PHUCTHAM_HDXX.Where(x => x.ID == ID).FirstOrDefault();
                    oND = DataExtensions.GetAllWithClause<AKT_KCKNQDK_PHUCTHAM_HDXX>("ID = " + ID).FirstOrDefault();
                }
                oND.DONID = DONID;
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    //AKT_PHUCTHAM_HDXX CheckCanBo = dt.AKT_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.CANBOID == oND.CANBOID).FirstOrDefault();
                    AKT_KCKNQDK_PHUCTHAM_HDXX CheckCanBo = DataExtensions.GetAllWithClause<AKT_KCKNQDK_PHUCTHAM_HDXX>
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.AKT_KCKNQDK_PHUCTHAM_HDXX_GETLIST(ID);
        ...
        {
            //AKT_PHUCTHAM_HDXX oND = dt.AKT_PHUCTHAM_HDXX.Where(x => x.ID == id).FirstOrDefault();
            AKT_KCKNQDK_PHUCTHAM_HDXX oND = DataExtensions.FindById<AKT_KCKNQDK_PHUCTHAM_HDXX>(id);
            //dt.AKT_PHUCTHAM_HDXX.Remove(oND);
