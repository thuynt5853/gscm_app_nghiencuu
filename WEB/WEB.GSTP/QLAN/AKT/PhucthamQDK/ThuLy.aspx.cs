    ...
    public partial class ThuLy : System.Web.UI.Page
    {
        ...
        private void CheckShowCommand(decimal DonID)
        {
            ...
            if (oT != null)
            {
                ...
                DataTable oDT = oBL.AKT_DON_THAMPHAN_GETBY(DonID, ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM);
            ...
            AKT_PHUCTHAM_BANAN ba = dt.AKT_PHUCTHAM_BANAN.Where(x => x.DONID == DonID && x.TOA_GIAIQUYET_ID == currenttoaid).FirstOrDefault();
            if (ba != null)
            {
        ...
        private void SetNewSoThuLy()
        {
            ...
            txtSoThuly.Text = oSTBL.GET_STL_NEW(DonViID, "AKT_PTQDK", ngaythuly).ToString();
            SetNewSoThongbao();
        }
        ...
        private void SetNewSoThongbao()
        {
            ...
            txtSothongbao.Text = oSTBL.GET_STBTL_NEW(DonViID, "AKT_PTQDK", ngaythongbao).ToString();
        }
        ...
        private void LoadCombobox()
        {
            ...
            List<AKT_CHUYEN_NHAN_AN> listcna = dt.AKT_CHUYEN_NHAN_AN.Where(x => x.MAP_VUANID_NEW == idAn && x.TOANHANID == toaanId).ToList();
        ...
        private bool CheckValid()
        {
            ...
            if (!String.IsNullOrEmpty(txtNgaythuly.Text))
            {
                ...
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal CheckID = oSTBL.CheckSoTLTheoLoaiAn(DonViID, "AKT_PTQDK", sothuly, ngaythuly);
                if (CheckID > 0)
                {
                    Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
                    String strMsg = "";
                    String STTNew = oSTBL.GET_STL_NEW(DonViID, "AKT_PTQDK", ngaythuly).ToString();
            ...
            if (!String.IsNullOrEmpty(txtNgaythuly.Text))
            {
                ...
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal CheckID = oSTBL.CheckSoTBTLTheoLoaiAn(DonViID, "AKT_PTQDK", sothongbao, ngaythuly);
                if (CheckID > 0)
                {
                    Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
                    String strMsg = "";
                    String STTNew = oSTBL.GET_STBTL_NEW(DonViID, "AKT_PTQDK", ngaythuly).ToString();
                    if (CheckID != CurrThuLyID)
                    {
                        //lbthongbao.Text = "Số thụ lý này đã có!";
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                decimal FileID = 0;
                AKT_KCKNQDK_PHUCTHAM_THULY oND;
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    oND = new AKT_KCKNQDK_PHUCTHAM_THULY();
                ...
                else
                {
                    ...
                    oND = DataExtensions.FindById<AKT_KCKNQDK_PHUCTHAM_THULY>(ID);
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    AKT_KCKN_PHUCTHAM_BL oPTBL = new AKT_KCKN_PHUCTHAM_BL();
        ...
        {
            AKT_KCKNQDK_PHUCTHAM_THULY oND = DataExtensions.FindById<AKT_KCKNQDK_PHUCTHAM_THULY>(id); //  AKT_PHUCTHAM_THULY oND = dt.AKT_PHUCTHAM_THULY.Where(x => x.ID == id).FirstOrDefault();
            if (oND != null)
            {
                ...
                if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.DONID), Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI), Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Thụ lý Phúc thẩm án Kinh Tế", "Xóa", json) == false)
                {
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                {
                    case "Download":
                        var oND = DataExtensions.FindById<AKT_KCKNQDK_PHUCTHAM_THULY>(ND_id);
                        if (oND.TENFILE != "")
                        {
        ...
        private void LoadQD()
        {
            string current_id = Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + "";
            decimal idAn = Convert.ToDecimal(current_id);
            AKT_CHUYEN_NHAN_AN_BL AKT_CHUYEN_NHAN_AN_BL = new AKT_CHUYEN_NHAN_AN_BL();
            decimal donOld = AKT_CHUYEN_NHAN_AN_BL.getDonIdOld(idAn);

            AKT_SOTHAM_KHANGCAO AKT_SOTHAM_KHANGCAO = dt.AKT_SOTHAM_KHANGCAO.Where(x => x.DONID == donOld && x.LOAIKHANGCAO == 2 && x.TINHTRANG_GIAIQUYET != 1).FirstOrDefault();
            AKT_SOTHAM_KHANGNGHI oKN = dt.AKT_SOTHAM_KHANGNGHI.Where(x => x.DONID == donOld && x.LOAIKN == 2 && x.TINHTRANG_GIAIQUYET != 1).FirstOrDefault();
            if (AKT_SOTHAM_KHANGCAO != null)
            {
                AKT_SOTHAM_QUYETDINH AKT_SOTHAM_QUYETDINH = dt.AKT_SOTHAM_QUYETDINH.Where(x => x.ID == AKT_SOTHAM_KHANGCAO.SOQDBA).FirstOrDefault();
                if (AKT_SOTHAM_QUYETDINH != null)
                {
                    DM_QD_QUYETDINH dM_QD_QUYETDINH = dt.DM_QD_QUYETDINH.Where(x => x.ID == AKT_SOTHAM_QUYETDINH.QUYETDINHID).FirstOrDefault();
                    txtQDST.Text = "Số " + AKT_SOTHAM_QUYETDINH.SOQD + " - " + dM_QD_QUYETDINH.TEN;
                }
            }
            else if (oKN != null)
            {
                AKT_SOTHAM_QUYETDINH AKT_SOTHAM_QUYETDINH = dt.AKT_SOTHAM_QUYETDINH.Where(x => x.ID == oKN.BANANID).FirstOrDefault();
                if (AKT_SOTHAM_QUYETDINH != null)
                {
                    DM_QD_QUYETDINH dM_QD_QUYETDINH = dt.DM_QD_QUYETDINH.Where(x => x.ID == AKT_SOTHAM_QUYETDINH.QUYETDINHID).FirstOrDefault();
                    txtQDST.Text = "Số " + AKT_SOTHAM_QUYETDINH.SOQD + " - " + dM_QD_QUYETDINH.TEN;
