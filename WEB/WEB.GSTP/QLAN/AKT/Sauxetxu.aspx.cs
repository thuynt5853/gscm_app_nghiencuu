...
namespace WEB.GSTP.QLAN.AKT
{
    public partial class Sauxetxu : System.Web.UI.Page
    {
        ...
        private void LoadGrid()
        {
            ...
            DataTable oDT = oBL.AKT_RUTKN(vDonViID, txtMaVuViec.Text, txtTenVuViec.Text, txtSoQDBA.Text, txtTenDuongSu.Text, dFrom, dTo, Convert.ToDecimal(rdbTrangthai.SelectedValue), pageIndex, pageSize);
            int Total = 0;
        ...
        protected void cmdCapnhat_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    decimal vuViecID = 0; AKT_SAUXETXU obj; bool isNew = false; AKT_DON don;
                    foreach (DataRow row in dsSelected.Rows)
                    {
                        vuViecID = Convert.ToDecimal(row["ID"].ToString());
                        obj = dt.AKT_SAUXETXU.Where(x => x.VUANID == vuViecID).FirstOrDefault<AKT_SAUXETXU>();
                        if (obj == null)
                        {
                            isNew = true;
                            obj = new AKT_SAUXETXU();
                        ...
                        obj.VUANID = vuViecID;
                        don = dt.AKT_DON.Where(x => x.ID == vuViecID).FirstOrDefault<AKT_DON>();
                        if (don != null)
                        {
        ...
        private void LoadGridSelected()
        {
            ...
            if (dsSelected != null && dsSelected.Rows.Count > 0)
            {
                ...
                AKT_SAUXETXU sx = dt.AKT_SAUXETXU.Where(x => x.VUANID == DonID).FirstOrDefault<AKT_SAUXETXU>();
                if (sx != null)
                {
        ...
        public void loadedit(decimal ID, String MAGIAIDOAN)
        {
            ...
            if (MAGIAIDOAN == "Sơ thẩm")
                oND = dt.AKT_SAUXETXU.Where(x => x.VUANID == ID && x.ST_ISRUTKN == 1).FirstOrDefault();
            else
                oND = dt.AKT_SAUXETXU.Where(x => x.VUANID == ID && x.PT_ISRUTKN == 1).FirstOrDefault();

            if (oND != null)
            {
                if (dsSelected != null) dsSelected.Clear();
                AKT_DON oVA = dt.AKT_DON.Where(x => x.ID == oND.VUANID).FirstOrDefault();
