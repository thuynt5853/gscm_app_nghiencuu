...
namespace WEB.GSTP.QLAN.AKT.AnPhi
{
    public partial class Danhsach : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                try
                {
                    ...
                    // txtHanNopAnPhi.Text = txtNgayNopAnPhi.Text =
        ...
        void CheckQuyen()
        {
            ...
            AKT_DON_XULY donxl = dt.AKT_DON_XULY.Where(x => x.DONID == ID).FirstOrDefault();
            if (donxl != null)
            {
                hddNgayGQYC.Value =
                    donxl.NGAYGQ_YC + "" == "" ? "" : ((DateTime)donxl.NGAYGQ_YC).ToString("dd/MM/yyyy");
            }
            //Kiểm tra Đơn đã trả lại đơn chưa

            List<AKT_DON_XULY> lts_donxl = dt.AKT_DON_XULY.Where(x => x.DONID == ID).ToList();
            ...
            string Result = new AKT_CHUYEN_NHAN_AN_BL().Check_NhanAn(ID, StrMsg,
            ...
            if (anKetThuc)
            {
                lbtthongbao.Text = "Vụ việc đã kết thúc tại tòa cũ, không thể chỉnh sửa tại tòa mới !";
                Cls_Comon.SetButton(cmdCapNhat, false);
        ...
        void LoadDropDuongSu()
        {
            ...
            ddlDuongSu.DataSource = oDSBL.AKT_DON_DUONGSU_BIENLAI_V2(DONID);
            ;
        ...
        private void LoadGrid_AnPhi()
        {
            ...
            DataTable oDT = oBL.AKT_ANPHI_GETBYDONID(DONID, 0, pageindex, page_size);
            if (oDT.Rows.Count > 0)
            {
                AKT_ANPHI ap = dt.AKT_ANPHI.Where(x => x.DONID == DONID && x.SOBIENLAI != null && x.TINHTRANG == 0)
                    .FirstOrDefault();
                if (ap != null)
                {
        ...
        protected void rpt_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ScriptManager.GetCurrent(this.Page)
                .RegisterPostBackControl((ImageButton)e.Item.FindControl("lblDownloadAP_THA"));
            ScriptManager.GetCurrent(this.Page)
                .RegisterPostBackControl((ImageButton)e.Item.FindControl("lblDownloadAP"));
            ...
            switch (e.CommandName)
            {
                case "ENABLE":
                    ...
                    {
                        oBLs.CHECK_TRANGTHAI_THULY(CurrID, "4");
                        cmdENABLE.ImageUrl = "/UI/img/Manager/Unlocked_Icon.png";
                        cmdENABLE.ToolTip =
                            "Trạng thái mở khóa, Vụ việc chưa được thụ lý, có thể sửa được thông tin nộp án phí ở phần mềm án phí";
                    }
                    else
                    {
                        oBLs.CHECK_TRANGTHAI_THULY(CurrID, "4");
                        ...
                        cmdENABLE.ToolTip =
                            "Trạng thái bị khóa, vụ việc đã được thụ lý, không thể sửa được thông tin nộp án phí ở phần mềm án phí";
                    }

                    break;
                case "Sua":
                    DON_MIENANPHI_BL dON_MIENANPHI_BL = new DON_MIENANPHI_BL();
                    DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(CurrID, 4);
                ...
                case "Xoa":
                    try
                    {
                        ...
                        DataTable dataTableXoa = dON_MIENANPHI_BLs.GET_DON_MIENANPHI_BY_ANPHI_ID(CurrID, 4);
                ...
                case "DownloadAP_THA":
                    ...
                    //--------------------
                    byte[] V_FILE_DATA = oBL.File_Attach_Anphi_Return("4", CurrID, ref V_FILE_NAME);
                    string V_FILE_TYLE = "";
            ...
            switch (fileEx)
            {
                ...
                case ".docx":
                    Response.ContentType =
                        "application/vnd.openxmlformats-officeFileAttach.wordprocessingml.FileAttach";
                    break;
                case ".xls":
                    Response.ContentType = "application/vnd.ms-excel";
                    break;
                case ".xlsx":
                    Response.ContentType = "application/vnd.openxmlformats-officeFileAttach.spreadsheetml.sheet";
                    break;
                case ".gif":
                ...
                default:
                    ...
                    break; //getMimeType(sExtention, oConnection);  
            }
        ...
        void LoadInfo_AnPhi(Decimal CurrID)
        {
            ...
            if (lst != null && lst.Count > 0)
            {
                ...
                txtNgayNopAnPhi.Text = (obj.NGAYNOPANPHI == null)
                    ? ""
                    : ((DateTime)obj.NGAYNOPANPHI).ToString("dd/MM/yyyy", cul);
                txtNgayNopBL.Text = (obj.NGAYNOPBIENLAI == null)
                    ? ""
                    : ((DateTime)obj.NGAYNOPBIENLAI).ToString("dd/MM/yyyy", cul);
                ...
                if (((DateTime)obj.NGAYTHONGBAO).ToString("dd/MM/yyyy") != "01/01/0001")
                    txtNgayThongBao.Text = (obj.NGAYTHONGBAO == null)
                        ? ""
                        : ((DateTime)obj.NGAYTHONGBAO).ToString("dd/MM/yyyy", cul);
        ...
        protected void cmdCapNhat_Click(object sender, EventArgs e)
        {
            try
            {
                if (hddCurrID.Value == "")
                {
                    lbtthongbao.Text = "Bạn chưa chọn biên lai tạm ứng cần cập nhật!";
                    return;
                }
                ...
                {
                    if (!DateTime.TryParseExact(txtNgayNopAnPhi.Text, "dd/MM/yyyy", cul,
                            DateTimeStyles.NoCurrentDateDefault, out NgayNopAnPhi))
                    {
                    ...
                    if (hddNgayGQYC.Value != "")
                    {
                        DateTime NgayGQYC = DateTime.Parse(hddNgayGQYC.Value, cul, DateTimeStyles.NoCurrentDateDefault);
                        if (NgayNopAnPhi < NgayGQYC)
                        {
                            lbtthongbao.Text = "Bạn phải nhập ngày nộp tạm ứng án phí lớn hơn ngày GQ/YC " +
                                               hddNgayGQYC.Value;
                            txtNgayNopAnPhi.Focus();
                            return;
                        }
                    ...
                    {
                        if (!DateTime.TryParseExact(txtNgayNopBL.Text, "dd/MM/yyyy", cul,
                                DateTimeStyles.NoCurrentDateDefault, out NgayNopBienLai))
                        {
                ...
                try
                {
                    ...
                    objAP.NGAYNOPANPHI = (String.IsNullOrEmpty(txtNgayNopAnPhi.Text.Trim()))
                        ? DateTime.MinValue
                        : DateTime.Parse(this.txtNgayNopAnPhi.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
                    objAP.NGAYNOPBIENLAI = (String.IsNullOrEmpty(txtNgayNopBL.Text.Trim()))
                        ? DateTime.MinValue
                        : DateTime.Parse(this.txtNgayNopBL.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
                    objAP.NGAYSUA = DateTime.Now;
                    objAP.NGUOISUA = Session[ENUM_SESSION.SESSION_USERNAME] + "";
        ...
        {
            Response.Redirect(Cls_Comon.GetRootURL() + "/QLAN/AKT/Sotham/ThuLy.aspx");
        }
        ...
        protected void rpt_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                string V_MA_THONGBAO = rowView["MA_THONGBAO"] + "";
                //decimal vAPID = Convert.ToDecimal(rowView.Row[0]);
                DAL.GSTP.DVCQG_THANH_TOAN oAP = dt.DVCQG_THANH_TOAN.Where(x => x.MA_THONGBAO == V_MA_THONGBAO)
                    .FirstOrDefault();
        ...
        protected void ddlDuongSu_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (idDS != 0)
            {
                string current_id = Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + "";
                decimal DONID = Convert.ToDecimal(current_id);
                decimal idAP = dt.AKT_ANPHI.Where(x => x.DONID == DONID && x.DUONGSU_ID == idDS).FirstOrDefault().ID;
        ...
        //        string current_id = Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + "";
        //        decimal DONID = Convert.ToDecimal(current_id);
        //        AKT_ANPHI objAP = dt.AKT_ANPHI.Where(x => x.DONID == DONID).FirstOrDefault();
        //        dt.AKT_ANPHI.Remove(objAP);
        //        dt.SaveChanges();
        //        lbtthongbao.Text = "Xóa thành công!";
        //        resertInforAnPhi();
