...
namespace WEB.GSTP.QLAN.AKT.Hoso
{
    public partial class DuongSu : System.Web.UI.Page
    {
        ...
        private void CheckQuyen(decimal ID)
        {
            ...
            bool daCoBanAn = dt.AKT_SOTHAM_BANAN.Any(x => x.DONID == ID);
            if (daCoBanAn)
            {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //check nếu vụ án đã có bản án thì không cho sửa thông tin đương sự
                bool daCoBanAn = dt.AKT_SOTHAM_BANAN.Any(x => x.DONID == DONID);
                if (daCoBanAn)
                {
                    lbthongbao.Text = "Đã có bản án, không được sửa đổi!";
                    Cls_Comon.SetButton(cmdUpdate, false);
                ...
                if (chkISDNNN.Visible)
                    oND.ISDNNN = chkISDNNN.Checked ? 1 : 0;
                else
                oND.ISDNNN = 0;
                oND.DONID = DONID;
                oND.ISSOTHAM = 1;
        ...
        public void LoadGrid()
        {
            ...
            string current_id = Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + "";
            decimal ID = Convert.ToDecimal(current_id);
            DataTable oDT = oBL.AKT_DON_DSDUONGSU_GETBY(ID);
        ...
        public void xoa(decimal id)
        {
            ...
            //manhnd 17/12/2020 cho xoa duong su để xóa vụ an
   
            AKT_ANPHI oAnphi = dt.AKT_ANPHI.Where(x => x.DONID == DONID).FirstOrDefault();
            if (oND.ISDAIDIEN == 1 && oAnphi != null)
            {
                lbthongbao.Text = "Giải quyết đơn đã có dữ liệu, không được phép xóa Đương sự đại diện!.";
                return;
            }
            AKT_DON_THAMPHAN oTPThuly = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == DONID).FirstOrDefault();
        ...
        public void loadedit(decimal ID)
        {
            ...
            else
            {
                ...
                chkISDNNN.Visible = true;
                pnNuocNgoai.Visible = true;
                if (oND.ISDNNN == 1) chkISDNNN.Checked = true;
            }
            ...
            if (hdTrangThaiXacThuc.Value == "1")
            {
                cmdGet037.Enabled = false;
                chkKhongLamSach.Enabled = false;
                chkBoxCMNDND.Enabled = false;
            }
            else
            {
                cmdGet037.Enabled = true;
                chkKhongLamSach.Enabled = true;
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
               ...
               // Cls_Comon.SetLinkButton(lblXem, oPer.CAPNHAT);
                ...
                AKT_ANPHI oAP = dt.AKT_ANPHI.Where(x => x.DONID == oT.ID && x.TOA_GIAIQUYET_ID == donvi_ID).FirstOrDefault();
        ...
        /*
            1.Người sửa: GTEL-HUNGNQ:  
            2.Nội Dung: thêm hàm check CCCD với dữ liệu quốc gia C06 
            3.Ngày tạo: 27-09-2025
        */
        #region check c06
        protected void btnGet037_Click(object sender, EventArgs e)
        {
            ...
            if (quocTich == "2")
            {
                ...
                else
                {
                    ...
                    string StrMsg = "PopupCenter('/QLAN/AKT/Hoso/Popup/pGetDuongSu037.aspx?caller=DuongSu','Thông tin công dân từ hệ thống Cơ sở dữ liệu quốc gia về dân cư',850,700);";
