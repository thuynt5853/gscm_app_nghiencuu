...
namespace WEB.GSTP.QLAN.AKT.Hoso.Popup
{
    public partial class pTachAn : System.Web.UI.Page
    {
        ...
        {
            AKT_DON_BL oBL = new AKT_DON_BL(); //KHAI- AKT

            AKT_DON donGoc = dt.AKT_DON.Where(x => x.ID == DonGocID).FirstOrDefault();
        ...
        private void Load_Grid_DonChiTiet()
        {
            ...
            DataTable oDT = oBL.GetDonGhep(vDonViID, DonGocID, 4); //KHAI- AKT
        ...
        private void Load_Grid_DonKhac()
        {
            ...
            int page_size = 10,
                pageindex = Convert.ToInt32(hddPageIndex.Value);
            DONGHEP_BL oBL = new DONGHEP_BL();
            DataTable oDT = oBL.GetDonGhepDonKC(vDonViID, DonGocID, 4); //KHAI- AKT
        ...
        protected void cmdTachAn_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridItem item in dgList.Items)
            {
                ...
                else
                {
                    ...
                    List<DON_DUONGSU_CHITIET> dsCT = dt.DON_DUONGSU_CHITIET
                        .Where(x => x.DONID == DonGocID && x.LOAIAN == 4 && x.DONCHITIETID == donID_DonTach)
                        .ToList(); //KHAI- AKT
            ...
            if (!myCheck)
            {
                ...
                try
                {
                    AKT_DON objDonGoc = dt.AKT_DON.Where(x => x.ID == DonGocID).FirstOrDefault(); //KHAI- AKT
                    AKT_DON objDonMoi = objDonGoc;
                    ...
                    AKT_DON_BL dsBL = new AKT_DON_BL(); //KHAI- AKT
                    objDonMoi.TT = dsBL.GETNEWTT((decimal)objDonMoi.TOAANID);
                    objDonMoi.MAVUVIEC = Session[ENUM_SESSION.SESSION_MADONVI] + "." + ENUM_LOAIVUVIEC.AN_KINHDOANH_THUONGMAI + "." + objDonMoi.TT.ToString(); //KHAI- AKT
                    ...
                    List<DON_DUONGSU_CHITIET> duongSuCT = dt.DON_DUONGSU_CHITIET
                        .Where(x => x.DONID == DonGocID && x.LOAIAN == 4 && x.DONCHITIETID == DonDaiDienID)
                        .ToList(); //KHAI- AKT

                    AKT_DON_DUONGSU nguyenDonDaiDien = new AKT_DON_DUONGSU(); //KHAI- AKT
                    AKT_DON_DUONGSU biDonDaiDien = new AKT_DON_DUONGSU();

                    foreach (var item in duongSuCT)
                    {
                        AKT_DON_DUONGSU objDS = dt.AKT_DON_DUONGSU.Where(x => x.ID == item.DUONGSUID).FirstOrDefault(); //KHAI- AKT
                    ...
                    if (nguyenDonDaiDien.ID != 0 && biDonDaiDien.ID != 0)
                    {
                        ...
                        dt.AKT_DON.Add(objDonMoi); //KHAI- AKT
                        dt.SaveChanges();

                        //Thêm thông tin vụ việc vào _DON_GIAIDOAN
                        GIAI_DOAN_BL donGD = new GIAI_DOAN_BL();
                        donGD.GAIDOAN_INSERT_UPDATE("4", objDonMoi.ID, 2, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), 0, 0, 0, 0); //KHAI- AKT

                        //Update bảng _DON_XULY
                        AKT_DON_XULY donXuLy = dt.AKT_DON_XULY.Where(x => x.DON_CHITIETID == DonDaiDienID && x.DON_XULYID == DonGocID).FirstOrDefault(); //KHAI- AKT
                        if (donXuLy != null)
                        {
                    ...
                    if (!myCheck)
                    {
                        ...
                        List<AKT_DON_DUONGSU> listDS = dt.AKT_DON_DUONGSU.Where(x => x.DONID == DonGocID).ToList(); //KHAI- AKT
                        ...
                        foreach (DataGridItem item in dgList.Items)
                        {
                            ...
                            if (chonVuTach.Checked)
                            {
                                ...
                                if (donID_DonTach != DonDaiDienID)
                                {
                                    ...
                                    //Update DON_XULYID
                                    AKT_DON_XULY donXuLy = dt.AKT_DON_XULY.Where(x => x.DON_CHITIETID == donID_DonTach && x.DON_XULYID == DonGocID).FirstOrDefault(); //KHAI- AKT
                                    if (donXuLy != null)
                                    {
                                    ...
                                    foreach (var itemDS in dsDuongSu)
                                    {
                                        ...
                                        if (!strDuongSuDonTachID.Contains(tempDuongSuID)) //Nếu đương sự không tồn tại trong danh sách DS của đơn khởi kiện mà không chọn để tách và ds đương dự của vụ gốc
                                        {
                                            ...
                                            AKT_DON_DUONGSU donDuongSu = dt.AKT_DON_DUONGSU.Where(x => x.ID == itemDS.DUONGSUID).FirstOrDefault(); //KHAI- AKT
                                            if (donDuongSu != null)
                                            {
                                            ...
                                            //Cập nhật án phí
                                            AKT_ANPHI anPhi = dt.AKT_ANPHI.Where(x => x.DONID == DonGocID && x.DUONGSU_ID == itemDS.DUONGSUID).FirstOrDefault(); //KHAI- AKT
                                            if (anPhi != null)
                                            {
                                            ...
                                            AKT_DON_TAILIEU taiLieu = dt.AKT_DON_TAILIEU
                                                .Where(x => x.DONID == DonGocID && x.NGUOIBANGIAO == itemDS.DUONGSUID)
                                                .FirstOrDefault(); //KHAI- AKT
                                        ...
                                        else
                                        {
                                            ...
                                            AKT_DON_DUONGSU donDuongSu = dt.AKT_DON_DUONGSU.Where(x => x.ID == itemDS.DUONGSUID).FirstOrDefault(); //KHAI- AKT
                                            AKT_DON_DUONGSU objDS = donDuongSu;
                                            ...
                                            if (taiLieu != null)
                                            {
                                                AKT_DON_TAILIEU taiLieuCopy = taiLieu; //KHAI- AKT
                                                taiLieuCopy.DONID = objDonMoi.ID;
                                                taiLieuCopy.NGUOIBANGIAO = objDS.ID;
                                                dt.AKT_DON_TAILIEU.Add(taiLieuCopy);
                                            }
                        ...
                        foreach (DataGridItem itemKhac in dgListKhac.Items)
                        {
                            ...
                            if (chonDonKhac.Checked)
                            {
                                ...
                                {
                                    AKT_DON_DUONGSU donKhacDS = dt.AKT_DON_DUONGSU
                                ...
                                {
                                    AKT_DON_THAMGIATOTUNG donKhacTGTT = dt.AKT_DON_THAMGIATOTUNG
                                        .Where(x => x.DONID == DonGocID && x.ID == donKhac.DUONGSUID)
                                        .FirstOrDefault(); //KHAI- AKT
                        ...
                        //Copy sang đơn mới
                        List<AKT_SOTHAM_HDXX> listHDXX = dt.AKT_SOTHAM_HDXX.Where(x => x.DONID == DonGocID).ToList();
                        AKT_SOTHAM_HDXX hdxx = new AKT_SOTHAM_HDXX(); //KHAI- AKT
                        if (listHDXX != null)
                        {
                        ...
                        AKT_SOTHAM_THULY thuly = dt.AKT_SOTHAM_THULY.Where(x => x.DONID == DonGocID).FirstOrDefault();
                        if (thuly != null)
                        {
                            AKT_SOTHAM_THULY thuLyDonMoi = thuly; //KHAI- AKT
                            thuLyDonMoi.DONID = objDonMoi.ID;
                            dt.AKT_SOTHAM_THULY.Add(thuLyDonMoi);
                            dt.SaveChanges();
                        }

                        List<AKT_DON_THAMPHAN> listThamPhan = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == DonGocID).ToList();
                        AKT_DON_THAMPHAN thamPhan = new AKT_DON_THAMPHAN(); //KHAI- AKT
                        if (listThamPhan != null)
                        {
                            foreach (var item in listThamPhan)
                            {
                                ...
                                dt.AKT_DON_THAMPHAN.Add(thamPhan);
                                dt.SaveChanges();
                            }
                        }

                        List<AKT_SOTHAM_QUYETDINH> listQD = dt.AKT_SOTHAM_QUYETDINH.Where(x => x.DONID == DonGocID).ToList();
                        AKT_SOTHAM_QUYETDINH quyetDinh = new AKT_SOTHAM_QUYETDINH(); //KHAI- AKT
                        if (listQD != null)
                        {
                            foreach (var item in listQD)
                            {
                                ...
                                dt.AKT_SOTHAM_QUYETDINH.Add(quyetDinh);
                                dt.SaveChanges();
                            }
                        }
                        ...
                        List<AKT_DON_DUONGSU> dsDuongSuDonNhap = dt.AKT_DON_DUONGSU.Where(x => x.DONID == objDonMoi.ID).ToList(); //KHAI- AKT
                        string jsonDuongSu = ",";
                        ...
                        //Lấy ds thẩm phán
                        List<AKT_DON_THAMPHAN> dsThamPhan = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == objDonMoi.ID).ToList(); //KHAI- AKT
                        string jsonThamPhan = ",";
                        ...
                        //Lấy ds thư kí
                        List<AKT_SOTHAM_HDXX> dsSoThamHDXX = dt.AKT_SOTHAM_HDXX.Where(x => x.DONID == objDonMoi.ID && x.MAVAITRO == "THUKY").ToList();
                        List<AKT_PHUCTHAM_HDXX> dsPhucThamHDXX = dt.AKT_PHUCTHAM_HDXX.Where(x => x.DONID == objDonMoi.ID && x.MAVAITRO == "THUKY").ToList();
                        string jsonThuKy = ",";
                        ...
                        AKT_SOTHAM_THULY thuLy = dt.AKT_SOTHAM_THULY.Where(x => x.DONID == objDonMoi.ID).FirstOrDefault(); //KHAI- AKT
                        int ttTL = 0;
                        string ngayTL = "", soTL = "";
                        if (thuLy != null)
                        {
        ...
        protected void chkChon_CheckedChanged(object sender, EventArgs e)
        {
            foreach (DataGridItem item in dgList.Items)
            {
                ...
                if (chonVuTach.Checked)
                {
                    ...
                    {
                        AKT_DON aktdon = dt.AKT_DON.Where(x => x.ID == idDCT).FirstOrDefault();
                        if (aktdon.LOAIDON == 1)
                        {
                            chonDaiDien.Visible = true;
                        }
