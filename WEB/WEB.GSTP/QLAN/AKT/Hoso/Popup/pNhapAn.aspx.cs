...
namespace WEB.GSTP.QLAN.AKT.Hoso.Popup
{
    public partial class pNhapAn : System.Web.UI.Page
    {
                ...
                else
                {
                    ...
                    if (ds_vuAnConID.Count > 0)
                    {
                        foreach (decimal vuAnConID in ds_vuAnConID)
                        {
                            ...
                            try
                            {
                                AKT_DON donCon = dt.AKT_DON.Where(x => x.ID == vuAnConID).FirstOrDefault();

                    //Tạo bản ghi DON_NHAPTACH
                    DON_NHAPTACH donNhap = new DON_NHAPTACH();
                    donNhap.DONID = donCon.ID;
                    donNhap.LOAIANID = 4; //AKT
                    ...
                    List<AKT_DON_DUONGSU> dsDuongSu = dt.AKT_DON_DUONGSU.Where(x => x.DONID == vuAnConID).ToList();
                    string jsonDuongSu = ",";
                    ...
                    //Lấy ds thẩm phán
                    List<AKT_DON_THAMPHAN> dsThamPhan = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == vuAnConID).ToList();
                    string jsonThamPhan = ",";
                    ...
                    //Lấy ds thư kí
                    List<AKT_SOTHAM_HDXX> dsSoThamHDXX = dt.AKT_SOTHAM_HDXX.Where(x => x.DONID == vuAnConID && x.MAVAITRO == "THUKY").ToList();
                    List<AKT_PHUCTHAM_HDXX> dsPhucThamHDXX = dt.AKT_PHUCTHAM_HDXX.Where(x => x.DONID == vuAnConID && x.MAVAITRO == "THUKY").ToList();
                    string jsonThuKy = ",";
                    ...
                    AKT_SOTHAM_THULY thuLy = dt.AKT_SOTHAM_THULY.Where(x => x.DONID == vuAnConID).FirstOrDefault();
                    int ttTL = 0;
                    string ngayTL = "", soTL = "";
                    if (thuLy != null)
                    {
                    ...
                    string tenChucNang = "Nhập án Kinh doanh, thương mại  từ " + donCon.MAVUVIEC + " đến " + donGoc.MAVUVIEC;
                    ...
                    string jsonString = JsonConvert.SerializeObject(objNoiDung);
                                ...
                                adsBL.HISTORY_ALLDATA_BY_VUANID(vuAnConID, 4, nguoiXoa, taiKhoanXoa, tenChucNang, hanhDong, jsonString);

                                //Cập nhật DONID của các bảng liên quan:
                                AKT_DON_BL update = new AKT_DON_BL();
                    ...
                    if (donCT.TOA_GIAIQUYET_ID == null)
                    {
                        donCT.TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                                ...
                                AKT_DON_XULY donXL = dt.AKT_DON_XULY.Where(x => x.DONID == vuAnConID).FirstOrDefault();
                                if (donXL != null)
                                {
                                    donXL.DONID = null;
        ...
        //            count++;
        //            vuAnConID = Convert.ToDecimal(Item.Cells[0].Text);
        //            oNSD = dt.AKT_SOTHAM_THULY.Where(x => x.DONID == vuAnConID).FirstOrDefault();
        //        }
        //    }
        //    if (vuAnConID == 0)
        //    {
