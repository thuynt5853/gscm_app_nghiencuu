...
namespace WEB.GSTP.QLAN.AKT.Phuctham
{
    public partial class Quyetdinhphuctham : System.Web.UI.Page
    {
        ...
        public void LoadGrid()
        {
            ...
            DM_QUYETDINH_VUAN oBL = new DM_QUYETDINH_VUAN();
            DataTable oDT = oBL.DGLIST_QUYETDINH_PT(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI, ID);
            if (oDT != null)
            {
        ...
        public void xoa(decimal id)
        {
            ...
            if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.DONID), 4, Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Quyết định Phúc thẩm án Kinh tế", "Xóa", json) == false)
            {
            ...
            //Anhpn 
            AKT_PHUCTHAM_BANAN AKT_PHUCTHAM_BANAN = dt.AKT_PHUCTHAM_BANAN.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == DONVI_ID).FirstOrDefault<AKT_PHUCTHAM_BANAN>();
            ...
            try
            {
                ...
                switch (e.CommandName)
                {
                    case "Download":
                        ...
                        break;
                    case "Sua":
                        AKT_PHUCTHAM_QUYETDINH oND1 = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.ID == ND_id).FirstOrDefault();
                        // khong duoc xoa khi da Tong dat - 27/11/2025 vnpt check
                        AKT_TONGDAT oTD = dt.AKT_TONGDAT.Where(x => x.DONID == oND1.DONID && x.MAPID == oND1.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AKT_PHUCTHAM_QUYETDINH).FirstOrDefault();
                        if (oTD != null)
                        {
                    ...
                    case "Xoa":
                        ...
                        if (oTD1 != null)
                        {
                            lbthongbao.Text = "Bạn không thể xoá khi đã tống đạt!";
                            return;
                        }
