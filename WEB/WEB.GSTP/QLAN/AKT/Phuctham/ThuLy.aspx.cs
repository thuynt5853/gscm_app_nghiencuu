    ...
    public partial class ThuLy : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    {
                        AKT_PHUCTHAM_BL oBL = new AKT_PHUCTHAM_BL();
                        DataTable oDT = oBL.AKT_PHUCTHAM_THULY_GETLIST(ID);
        ...
        private void CheckShowCommand(decimal DonID)
        {
            ...
            if (oT != null)
            {
                ...
                AKT_SOTHAM_BL objST = new AKT_SOTHAM_BL();
                if (oT.MAGIAIDOAN == ENUM_GIAIDOANVUAN.SOTHAM && objST.AKT_SOTHAM_KCaoKNghi_GETLIST(DonID).Rows.Count == 0)
                {
        ...
        private decimal SetNewSoThuLy()
        {
            ...
            string sothulymoi = oSTBL.GET_STL_NEW(DonViID, "AKT_PT", ngaythuly).ToString();
        ...
        void SetNewSoThongbao()
        {
            ...
            txtSothongbao.Text = oSTBL.GET_STBTL_NEW(DonViID, "AKT_PT", ngaythongbao).ToString();
        ...
        private bool CheckValid()
        {
            ...
            //    Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
            //    ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
            //    Decimal CheckID = oSTBL.CheckSoTLTheoLoaiAn(DonViID, "AKT_PT", sothuly, ngaythuly);
            //    if (CheckID > 0)
            //    {
            //        Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
            //        String strMsg = "";
            //        String STTNew = oSTBL.GET_STL_NEW(DonViID, "AKT_PT", ngaythuly).ToString();
            ...
            if (!String.IsNullOrEmpty(txtNgaythuly.Text))
            {
                ...
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal CheckID = oSTBL.CheckSoTBTLTheoLoaiAn(DonViID, "AKT_PT", sothongbao, ngaythuly);
                if (CheckID > 0)
                {
                    Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
                    String strMsg = "";
                    String STTNew = oSTBL.GET_STBTL_NEW(DonViID, "AKT_PT", ngaythuly).ToString();
                    if (CheckID != CurrThuLyID)
                    {
                        //lbthongbao.Text = "Số thụ lý này đã có!";
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                decimal FileID = 0;
                AKT_PHUCTHAM_THULY oND;
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    oND = new AKT_PHUCTHAM_THULY();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.AKT_PHUCTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
                    ...
                    {
                        lbthongbao.Text = "Không có quyền sửa thụ lý này! Vui lòng làm mới trước khi lưu";
                        return;
                    }
                    // - vnpt 1/12/2025  check tong dat
                    AKT_TONGDAT oTD = dt.AKT_TONGDAT.Where(x => x.DONID == oND.DONID && x.MAPID == oND.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AKT_PHUCTHAM_THULY).FirstOrDefault();
                    if (oTD != null)
                    {
                ...
                oQLSTL.update_STPT_QUANLY_SOTHULY(3, 4, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), ((DateTime)oND.NGAYTHULY).ToString("dd/MM/yyyy", cul), Regex.Match(oND.SOTHULY, @"\d+").Value);

                AKT_DON_BL objDBL = new AKT_DON_BL();
                decimal rFileID = UploadFileID(oDon, FileID, "65-DS", oND.SOTHONGBAO);
                if (rFileID > 0) oND.FILEID = rFileID;
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    AKT_PHUCTHAM_BL oPTBL = new AKT_PHUCTHAM_BL();
                    oND.TT = oPTBL.THULY_GETNEWTT(Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]));
                    oND.MATHULY = "P" + ENUM_LOAIVUVIEC.AN_KINHDOANH_THUONGMAI + Session[ENUM_SESSION.SESSION_MADONVI] + oND.TT.ToString();
        ...
        {
            AKT_PHUCTHAM_THULY oND = dt.AKT_PHUCTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
            if (oND != null)
            {
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                switch (e.CommandName)
                {
                    ...
                    case "Sua":
                        lbthongbao.Text = "";
                        AKT_PHUCTHAM_THULY oND1 = dt.AKT_PHUCTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
                        // khong duoc SỬA khi da Tong dat - 29/11/2025 vnpt check
                        AKT_TONGDAT oTD = dt.AKT_TONGDAT.Where(x => x.DONID == oND1.DONID && x.MAPID == oND1.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AKT_PHUCTHAM_THULY).FirstOrDefault();
                        if (oTD != null)
                        {
                            lbthongbao.Text = "Bạn không thể sửa khi đã tống đạt!";
                            return;
                    ...
                    case "Xoa":
                        AKT_PHUCTHAM_THULY oND2 = dt.AKT_PHUCTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
                        // khong duoc xoa khi da Tong dat - 29/11/2025 vnpt check
                        AKT_TONGDAT oTD1 = dt.AKT_TONGDAT.Where(x => x.DONID == oND2.DONID && x.MAPID == oND2.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AKT_PHUCTHAM_THULY).FirstOrDefault();
                        if (oTD1 != null)
                        {
        ...
        protected void txtNgayThuLy_TextChanged(object sender, EventArgs e)
        {
            ...
            //    STPT_QUANLY_SOTHULY oQLSTL = new STPT_QUANLY_SOTHULY();
            //    DataTable dtStl = oQLSTL.get_STPT_QUANLY_SOTHULY(3, 4, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtNgaythuly.Text);
            //    if (oQLSTL != null)
            //    {
            ...
            //    string check_ngaythulycuoi_trongnam = oQLSTL.get_LATEST_DATE_IN_SOTHULY(3, 4, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtNgaythuly.Text);
        ...
        protected void btnSaveLydoXoa_Insert(object sender, EventArgs e)
        {
            ...
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            AKT_PHUCTHAM_THULY oND = dt.AKT_PHUCTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
            ...
            if (oND != null)
            {
                ...
                AKT_DON_THAMPHAN AKT_DON_THAMPHAN = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM && x.TOA_GIAIQUYET_ID == DONVIID).FirstOrDefault<AKT_DON_THAMPHAN>();
                if (AKT_DON_THAMPHAN != null)
                {
                    lbthongbao.Text = "Xóa không thành công! Do đang tồn tại thông tin phân công thẩm phán giải quyết.";
                    return;
                }
                AKT_PHUCTHAM_THAMGIATOTUNG AKT_PHUCTHAM_THAMGIATOTUNG = dt.AKT_PHUCTHAM_THAMGIATOTUNG.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == DONVIID).FirstOrDefault<AKT_PHUCTHAM_THAMGIATOTUNG>();
                if (AKT_PHUCTHAM_THAMGIATOTUNG != null)
                {
                    lbthongbao.Text = "Xóa không thành công! Do đang tồn tại người tham gia tố tụng.";
                    return;
                }
                AKT_PHUCTHAM_HDXX AKT_PHUCTHAM_HDXX = dt.AKT_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == DONVIID).FirstOrDefault<AKT_PHUCTHAM_HDXX>();
                if (AKT_PHUCTHAM_HDXX != null)
                {
                    lbthongbao.Text = "Xóa không thành công! Do đang tồn tại thông tin người tiến hành tố tụng.";
                    return;
                }
                AKT_PHUCTHAM_HOAGIAI AKT_PHUCTHAM_HOAGIAI = dt.AKT_PHUCTHAM_HOAGIAI.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == DONVIID).FirstOrDefault<AKT_PHUCTHAM_HOAGIAI>();
                if (AKT_PHUCTHAM_HOAGIAI != null)
                {
                    lbthongbao.Text = "Xóa không thành công! Do đang tồn tại thông tin hòa giải.";
                    return;
                }
                AKT_PHUCTHAM_QUYETDINH AKT_PHUCTHAM_QUYETDINH = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == DONVIID).FirstOrDefault<AKT_PHUCTHAM_QUYETDINH>();
                ...
                dt.AKT_PHUCTHAM_THULY.Remove(oND);
                dt.SaveChanges();
                if (FileID > 0)
                {
                    try
