    ...
    public partial class NguoiTienhanhTT : System.Web.UI.Page
    {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                AKT_PHUCTHAM_HDXX oND;

                // Lấy count thụ lý và count chủ tọa để cho nhập thêm thẩm phán chủ tọa
                List<AKT_DON_THAMPHAN> countTPGQ = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM).ToList();
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    if (tucach == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN)
                    {
                        ...
                        oND = dt.AKT_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN && x.TOA_GIAIQUYET_ID == currenttoaid).FirstOrDefault<AKT_PHUCTHAM_HDXX>();
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.AKT_PHUCTHAM_HDXX_GETLIST(ID);
        ...
        public void xoa(decimal id)
        {
            ...
            AKT_PHUCTHAM_HOAGIAI AKT_PHUCTHAM_HOAGIAI = dt.AKT_PHUCTHAM_HOAGIAI.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == DONVI_ID).FirstOrDefault<AKT_PHUCTHAM_HOAGIAI>();
            if (AKT_PHUCTHAM_HOAGIAI != null)
            {
                lbthongbao.Text = "Xóa không thành công! Do đang tồn tại thông tin hòa giải.";
                return;
            }
            AKT_PHUCTHAM_QUYETDINH AKT_PHUCTHAM_QUYETDINH = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == DONVI_ID).FirstOrDefault<AKT_PHUCTHAM_QUYETDINH>();
