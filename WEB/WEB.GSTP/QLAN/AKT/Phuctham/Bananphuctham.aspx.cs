    ...
    public partial class Bananphuctham : System.Web.UI.Page
    {
        ...
        bool CheckCongbo(decimal ID)
        {
            var list = DataExtensions.GetAllWithClause<BAQD_CONGBO>(
                $"VUVIECID = {ID} AND LOAIANID = {ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI} AND CAPXETXU = 3 AND TRANGTHAI IN (2,3)"
            );
        ...
        void CheckQuyen()
        {
            ...
            List<AKT_PHUCTHAM_QUYETDINH> lstQD = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DONID && (x.LOAIQDID == 10 || x.QUYETDINHID == 423 || x.QUYETDINHID == 422 || x.LOAIQDID == 3 || x.QUYETDINHID == 145
            || x.QUYETDINHID == 212 || x.QUYETDINHID == 213)).ToList();
            if (lstQD.Count >= 1)
            {
                pnQDVV.Visible = true;
            ...
            List<AKT_PHUCTHAM_BANAN> lstBA = dt.AKT_PHUCTHAM_BANAN.Where(x => x.DONID == DONID).ToList();
            if (lstBA.Count >= 1)
            {
                pnBAPT.Visible = true;
            ...
            List<AKT_DON_THAMPHAN> lstTP = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM).ToList();
            if (lstTP.Count == 0)
            {
            ...
            List<AKT_PHUCTHAM_HDXX> lstTPCT = dt.AKT_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).ToList();
            if (lstTPCT.Count == 0)
            {
                lstErr.Text = lbthongbaoQD.Text = "Chưa phân công thẩm phán giải quyết !";
                Cls_Comon.SetButton(cmdUpdate, false);
            ...
            AKT_CHUYEN_NHAN_AN oChuyenAn = dt.AKT_CHUYEN_NHAN_AN.Where(x => x.VUANID == DONID).OrderByDescending(x => x.NGAYTAO).FirstOrDefault();
            DM_DATAITEM oIT = dt.DM_DATAITEM.Where(x => x.ID == oChuyenAn.TRUONGHOPGIAONHANID).FirstOrDefault();
            if (oIT != null && oIT.MA == ENUM_TRUONGHOP_GIAONHAN.XETXULAI_CAPSOTHAM)
            {
            ...
            AKT_TONGDAT td = dt.AKT_TONGDAT.Where(x => x.DONID == DONID && ( x.BIEUMAUID == 261)).FirstOrDefault();// vnpt -- xoá check biểu mẫu sơ thẩm đã tống đạt trên phúc thẩm
            if (td != null)
            {
            ...
            AKT_PHUCTHAM_QUYETDINH oQD = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DONID && x.LOAIQDID == 5
            ...
            bool isExist = ads.IsExistKHOBADQ(0, DONID, 3,ENUM_LOAIVUVIEC_TEXT.AN_KINHDOANH_THUONGMAI);
            if (isExist)
            {
            ...
            isExist = ads.IsExistKHOBADQ(1, DONID, 3, ENUM_LOAIVUVIEC_TEXT.AN_KINHDOANH_THUONGMAI);
            if (isExist)
            {
                lstErr.Text = lbthongbaoQD.Text = "Vụ việc đã được đồng bộ. Phải thu hồi đồng bộ trước khi chỉnh sửa/xóa";
                Cls_Comon.SetButton(btnUpdate, false);
        ...
        private void LoadFile()
        {
            ...
            List<AKT_PHUCTHAM_BANAN_FILE> lst = dt.AKT_PHUCTHAM_BANAN_FILE.Where(x => x.BANANID == ID).ToList();
        ...
        {
            AKT_PHUCTHAM_BANAN oT = dt.AKT_PHUCTHAM_BANAN.Where(x => x.DONID == DonID).FirstOrDefault<AKT_PHUCTHAM_BANAN>();
            if (oT != null)
            {
            ...
            else
            {
                ...
                AKT_PHUCTHAM_THULY tlpt = dt.AKT_PHUCTHAM_THULY.Where(x => x.DONID == DonID).OrderByDescending(x => x.NGAYTHULY).FirstOrDefault();
                if (tlpt != null)
                {
        ...
        private bool CheckValid()
        {
            ...
            if (!String.IsNullOrEmpty(txtNgaytuyenan.Text))
            {
                ...
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal CheckID = oSTBL.CheckSoBATheoLoaiAn(DonViID, "AKT_PT", so, ngayBA);
                if (CheckID > 0)
                {
                    Decimal CurrBanAnId = (string.IsNullOrEmpty(hddBanAnID.Value)) ? 0 : Convert.ToDecimal(hddBanAnID.Value);
                    String strMsg = "";
                    String STTNew = oSTBL.GETSoBANEWTheoLoaiAn(DonViID, "AKT_PT", ngayBA).ToString();
        ...
        private void LoadDropKetQuaPhucTham()
        {
            ...
            ddlKetQuaPhucTham.DataSource = dt.DM_KETQUA_PHUCTHAM.Where(x => x.ISAKT == 1 && x.ISBANAN == 1).OrderBy(y => y.THUTU).ToList();
        ...
        protected void cmdUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                AKT_PHUCTHAM_BANAN_FILE oTF = new AKT_PHUCTHAM_BANAN_FILE();
                bool isNew = false;
                AKT_PHUCTHAM_BANAN oND = dt.AKT_PHUCTHAM_BANAN.Where(x => x.DONID == DONID).FirstOrDefault<AKT_PHUCTHAM_BANAN>();
                if (oND == null)
                {
                    oND = new AKT_PHUCTHAM_BANAN(); isNew = true;
                ...
                else
                {
                    ...
                    AKT_TONGDAT oTD1 = dt.AKT_TONGDAT.Where(x => x.DONID == oND.DONID && x.MAPID == oND.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AKT_PHUCTHAM_BANAN).FirstOrDefault();
                    if (oTD1 != null)
                    {
                ...
                bool isExist = ald.IsExistKHOBADQ(0, DONID, 3,ENUM_LOAIVUVIEC_TEXT.AN_KINHDOANH_THUONGMAI);
                if (isExist)
                {
                ...
                try
                {
                    if (hddFilePath.Value != "")
                    {
                        ...
                        using (FileStream fs = File.OpenRead(strFilePath))
                        {
                            ...
                            dt.AKT_PHUCTHAM_BANAN_FILE.Add(oTF);
                            dt.SaveChanges();
                        }
                        #endregion
        ...
        protected void dgFile_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "Xoa":
                    ...
                    AKT_PHUCTHAM_BANAN_FILE oT = dt.AKT_PHUCTHAM_BANAN_FILE.Where(x => x.ID == ND_id).FirstOrDefault();
                    dt.AKT_PHUCTHAM_BANAN_FILE.Remove(oT);
                    dt.SaveChanges();
                    LoadFile();
                    break;
                case "Download":
                    AKT_PHUCTHAM_BANAN_FILE oND = dt.AKT_PHUCTHAM_BANAN_FILE.Where(x => x.ID == ND_id).FirstOrDefault();
                    if (oND.TENFILE != "")
                    {
        ...
        protected void cmdHuyBanAn_Click(object sender, EventArgs e)
        {
            ...
            AKT_PHUCTHAM_BANAN banan = dt.AKT_PHUCTHAM_BANAN.Where(x => x.DONID == DONID).FirstOrDefault();
            if (banan != null)
            {
                // khong duoc xoa khi da Tong dat - 27/11/2025 vnpt check
                AKT_TONGDAT oTD2 = dt.AKT_TONGDAT.Where(x => x.DONID == banan.DONID && x.MAPID == banan.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AKT_PHUCTHAM_BANAN).FirstOrDefault();
                if (oTD2 != null)
                {
                    lstErr.Text = "Bạn không thể xóa khi đã tống đạt!";
                    return;
                }
                //GTEL-HUNGNQ 08-10-2025 Dữ liệu Bản án đã được chia sẻ không được xóa
                KHOBAQD_BL ald = new KHOBAQD_BL();
                bool isExist = ald.IsExistKHOBADQ(0, DONID, 3,ENUM_LOAIVUVIEC_TEXT.AN_KINHDOANH_THUONGMAI);
                if (isExist)
                {
                ...
                if (oBL.HISTORY_ALLDATA_BY_VUANID(DONID, 4, Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Bản án Phúc thẩm án Kinh tế", "Xóa", json) == false)
                {
                    return;
                }//Ket thuc
                 //Xoa Bản án Phúc thẩm
                dt.AKT_PHUCTHAM_BANAN.Remove(banan);
            }
            // Xóa thông tin file đính kèm
            List<AKT_PHUCTHAM_BANAN_FILE> files = dt.AKT_PHUCTHAM_BANAN_FILE.Where(x => x.BANANID == DONID).ToList();
            if (files.Count > 0)
            {
                dt.AKT_PHUCTHAM_BANAN_FILE.RemoveRange(files);
            }
            // Xóa thông tin người tham gia tố tụng
            List<AKT_PHUCTHAM_BANAN_TGTT> tGTTs = dt.AKT_PHUCTHAM_BANAN_TGTT.Where(x => x.DONID == DONID).ToList();
            if (tGTTs.Count > 0)
            {
                dt.AKT_PHUCTHAM_BANAN_TGTT.RemoveRange(tGTTs);
            }
            ...
            AKT_FILE file = dt.AKT_FILE.Where(x => x.DONID == DONID && x.BIEUMAUID == BieuMauID && x.MAGIAIDOAN == ENUM_GIAIDOANVUAN.PHUCTHAM).FirstOrDefault();
            if (file != null)
            {
        ...
        private string getQHPL_NAME_THULY()
        {
            ...
            AKT_PHUCTHAM_THULY oT = dt.AKT_PHUCTHAM_THULY.Where(x => x.DONID == ID).FirstOrDefault();
        ...
        private void txtQuanhephapluat_name(AKT_PHUCTHAM_BANAN oT)
        {
            ...
            {
                AKT_PHUCTHAM_BANAN oTT = dt.AKT_PHUCTHAM_BANAN.Where(x => x.DONID == oT.DONID).FirstOrDefault();
        ...
        private void txtQuanhephapluat_name(AKT_PHUCTHAM_THULY oT)
        {
            if (oT.QUANHEPHAPLUAT_NAME != null)
            {
                txtQuanhephapluat.Text = oT.QUANHEPHAPLUAT_NAME;
            ...
            {
                AKT_DON oTT = dt.AKT_DON.Where(x => x.ID == oT.DONID).FirstOrDefault();
                if (oTT.QUANHEPHAPLUAT_NAME != null)
                {
                    txtQuanhephapluat.Text = oTT.QUANHEPHAPLUAT_NAME;
                }
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.DGLIST_BAQD_QUYETDINH_KETTHUC_PT(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI, ID);
            if (oDT != null)
            {
        ...
        {
            DM_QUYETDINH_VUAN_KETTHUC oBL = new DM_QUYETDINH_VUAN_KETTHUC();
            DataTable oDT = oBL.DM_QUYETDINH_VUAN_PHUCTHAM_KETTHUC(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI);
        ...
        private void LoadNguoiKyInfo()
        {
            ...
            AKT_PHUCTHAM_HDXX oND = dt.AKT_PHUCTHAM_HDXX.Where(x => x.DONID == DonID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).FirstOrDefault<AKT_PHUCTHAM_HDXX>();
            if (oND != null)
            {
            ...
            {
                AKT_DON_THAMPHAN oTP = dt.AKT_DON_THAMPHAN.Where(x => x.DONID == DonID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM).FirstOrDefault();
                if (oTP != null)
                {
        ...
        {
            Decimal ID = Convert.ToDecimal(hddDonID.Value);
            List<AKT_PHUCTHAM_QUYETDINH> lstQD = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.DONID == ID && (x.LOAIQDID == 10 || x.QUYETDINHID == 423 || x.QUYETDINHID == 422 || x.LOAIQDID == 3)).ToList();
            if (lstQD.Count >= 1)
            {
                Cls_Comon.SetButton(btnUpdate, false);
            }
        ...
        {
            AKT_PHUCTHAM_QUYETDINH oND = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.ID == id).FirstOrDefault();
            TK_PHUCTHAM_QUYETDINH TK = DataExtensions.GetAllWithClause<TK_PHUCTHAM_QUYETDINH>($"QUYETDINHID = {id} AND LOAIAN = {Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI)}").FirstOrDefault();
            if (TK != null)
            {
            ...
            if (oND != null)
            {
                ...
                dt.AKT_PHUCTHAM_QUYETDINH.Remove(oND);
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //GTEL-HUNGNQ 08-10-2025 Dữ liệu Bản án đã được chia sẻ không được xóa
                KHOBAQD_BL ald = new KHOBAQD_BL();
                bool isExist = ald.IsExistKHOBADQ(1, DONID, 3,ENUM_LOAIVUVIEC_TEXT.AN_KINHDOANH_THUONGMAI);
                if (isExist)
                {
                ...
                AKT_PHUCTHAM_QUYETDINH oND;
                TK_PHUCTHAM_QUYETDINH TK = new TK_PHUCTHAM_QUYETDINH();

                decimal STTQD = 0;
                if ((hddID.Value == "" || hddID.Value == "0"))
                {
                    oND = new AKT_PHUCTHAM_QUYETDINH();
                    AKT_DON_BL oBL = new AKT_DON_BL();
                ...
                else
                {
                    ...
                    TK = DataExtensions.GetAllWithClause<TK_PHUCTHAM_QUYETDINH>($"QUYETDINHID= {ID} AND LOAIAN =  {Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI)}").FirstOrDefault();

                    oND = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.ID == ID).FirstOrDefault();
                ...
                if (hddID.Value == "" || hddID.Value == "0")
                {
                    ...
                    oND.NGUOITAO = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                    oND.TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                    dt.AKT_PHUCTHAM_QUYETDINH.Add(oND);
                    dt.SaveChanges();
                    TK.ID = oND.ID;
        ...
        {
            //AKT_PHUCTHAM_QUYETDINH oND = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.ID == ID && (x.LOAIQDID == 15 || x.LOAIQDID == 3 || x.LOAIQDID == 10 || x.QUYETDINHID == 422 || x.QUYETDINHID == 423 || x.QUYETDINHID == 212 || x.QUYETDINHID == 213)).FirstOrDefault();
            AKT_PHUCTHAM_QUYETDINH oND = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.ID == ID).FirstOrDefault();
            TK_PHUCTHAM_QUYETDINH TK = DataExtensions.GetAllWithClause<TK_PHUCTHAM_QUYETDINH>($"QUYETDINHID = {ID} AND LOAIAN = {Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI)}").FirstOrDefault();
        ...
        private void SetEnable_data_QD(AKT_PHUCTHAM_QUYETDINH QD)
        {
            if (QD != null)
            {
                // khong duoc sửa khi da Tong dat nhưng cho phép nhập thêm những trường trống -27 / 11 / 2025 vnpt check
                AKT_TONGDAT oTD = dt.AKT_TONGDAT.Where(x => x.DONID == QD.DONID && x.MAPID == QD.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AKT_PHUCTHAM_BANAN).FirstOrDefault();
                if (oTD != null)
                {
                    if (QD.QHPLTKID != null)
                    {
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            {
                case "Download":
                    var oND = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.DONID == ID).FirstOrDefault();
                    if (oND.TENFILE != "")
                    {
                        var cacheKey = Guid.NewGuid().ToString("N");
                ...
                case "Sua":
                    hddFilePathQD.Value = "";
                    lbthongbao.Text = "";
                    //AKT_PHUCTHAM_QUYETDINH oND3 = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.ID == ND_id).FirstOrDefault();
                    // khong duoc xoa khi da Tong dat - 01/12/2025 vnpt Lê Bá Thọ check
                    //AKT_TONGDAT oTD2 = dt.AKT_TONGDAT.Where(x => x.DONID == oND3.DONID && x.MAPID == oND3.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AKT_PHUCTHAM_QUYETDINH).FirstOrDefault();
                    //if (oTD2 != null)
                    //{
                ...
                case "Xoa":
                    ...
                    AKT_PHUCTHAM_QUYETDINH oND2 = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.ID == ND_id).FirstOrDefault();
                    // khong duoc xoa khi da Tong dat - 01/12/2025 vnpt Lê Bá Thọ vnpt check
                    AKT_TONGDAT oTD1 = dt.AKT_TONGDAT.Where(x => x.DONID == oND2.DONID && x.MAPID == oND2.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AKT_PHUCTHAM_QUYETDINH).FirstOrDefault();
                    if (oTD1 != null)
                    {
                        lbthongbaoQD.Text = "Bạn không thể xoá khi đã tống đạt!";
                        return;
                    }
                    //GTEL-HUNGNQ 08-10-2025 Dữ liệu Bản án đã được chia sẻ không được xóa
                    KHOBAQD_BL ald = new KHOBAQD_BL();
                    bool isExist = ald.IsExistKHOBADQ(1, ID, 3,ENUM_LOAIVUVIEC_TEXT.AN_KINHDOANH_THUONGMAI);
                    if (isExist)
                    {
                        lbthongbao.Text = "Vụ việc đã được chia sẻ dữ liệu. Không được xóa.";
                        return;
                    }
                    //END GTEL-HUNGNQ 08-10-2025
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                decimal DONID = Convert.ToDecimal(hddDonID.Value);
                AKT_PHUCTHAM_QUYETDINH oT = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.ID == DONID).FirstOrDefault();
        ...
        protected void ddlQuyetdinh_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (btnUpdate.Enabled == true)
            {
                ...
                {
                    AKT_PHUCTHAM_QUYETDINH oQD = dt.AKT_PHUCTHAM_QUYETDINH.Where(x => x.DONID == IDD && x.LOAIQDID == 5 && x.QUYETDINHID != 147 && x.QUYETDINHID != 148 /*Quyết định chuyển vụ án giải quyết theo thủ tục rút gọn sang giải quyết theo thủ tục thông thường*/).FirstOrDefault();
                    if (oQD == null)
                    {
        ...
        protected void set_valueLydo(AKT_PHUCTHAM_QUYETDINH oND)
        {
        ...
        private void LoadDropKetQuaQuyetdinhPhuctham()
        {
            if (ddlQuyetdinh.SelectedItem.Text.Contains("72-DS"))
            {
                ...
                ddlKetquaQuyetdinh.DataSource = dt.DM_KETQUA_PHUCTHAM.Where(x => x.ISAKT == 1 && x.ISQUYETDINH == 1).OrderBy(y => y.THUTU).ToList();
            ...
            {
                ddlKetquaQuyetdinh.Items.Clear();
                ddlKetquaQuyetdinh.DataSource = dt.DM_KETQUA_PHUCTHAM.Where(x => x.ISAKT == 1 && x.ISBANAN == 1 && x.MA != "13" && x.MA != "14" && x.MA != "18").OrderBy(y => y.THUTU).ToList();
                ddlKetquaQuyetdinh.DataTextField = "TEN";
                ddlKetquaQuyetdinh.DataValueField = "ID";
                ddlKetquaQuyetdinh.DataBind();
                ddlKetquaQuyetdinh.Items.Insert(0, new ListItem("--- Chọn ---", "0"));
