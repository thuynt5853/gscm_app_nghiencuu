...
namespace WEB.GSTP.QLAN.AKT.Phuctham
{
    public partial class Capnhatkhangcaoquahan : System.Web.UI.Page
    {
        ...
        {
            AKT_SOTHAM_KHANGCAO oND = dt.AKT_SOTHAM_KHANGCAO.Where(x => x.DONID == id).FirstOrDefault();
            if (oND != null)
            {
                oND.GQ_NGAY = null;
        ...
        private void LoadGrid()
        {
            ...
            DataTable oDT = oBL.AKT_PT_KCQUAHAN(vDonViID, txtMaVuViec.Text, txtTenVuViec.Text, dFrom, dTo, Convert.ToDecimal(rdbTrangthai.SelectedValue), pageIndex, pageSize);
            int Total = 0;
        ...
        protected void cmdGiaiQuyet_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                AKT_DON don = dt.AKT_DON.Where(x => x.ID == ID).FirstOrDefault<AKT_DON>();
                if (don != null)
                {
