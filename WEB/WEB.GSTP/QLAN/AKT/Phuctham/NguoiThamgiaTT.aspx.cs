    ...
    public partial class NguoiThamgiaTT : System.Web.UI.Page
    {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string current_id = Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI] + "";
                decimal DONID = Convert.ToDecimal(current_id);
                AKT_PHUCTHAM_THAMGIATOTUNG oND;
                if (hddid.Value == "" || hddid.Value == "0")
                    oND = new AKT_PHUCTHAM_THAMGIATOTUNG();
                else
                {
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.AKT_PHUCTHAM_TGTT_GETLIST(ID);
        ...
        public void xoa(decimal id)
        {
            ...
            if (oND != null)
            {
                ...
                AKT_PHUCTHAM_BANAN AKT_PHUCTHAM_BANAN = dt.AKT_PHUCTHAM_BANAN.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == currenttoaid).FirstOrDefault<AKT_PHUCTHAM_BANAN>();
            ...
            foreach (ListItem oItem in chkListTGTT.Items)
            {
                if (oItem.Selected)
                {
                    ...
                    if (dt.AKT_PHUCTHAM_THAMGIATOTUNG.Where(x => x.NTGTT_DONKK_ID == NID).ToList().Count == 0)
                    {
                        ...
                        AKT_PHUCTHAM_THAMGIATOTUNG oS = new AKT_PHUCTHAM_THAMGIATOTUNG();
                        oS.DONID = oD.DONID;
                        oS.HOTEN = oD.HOTEN;
