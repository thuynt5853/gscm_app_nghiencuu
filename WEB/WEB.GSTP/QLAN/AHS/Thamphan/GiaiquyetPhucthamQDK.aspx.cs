...
{
    public partial class GiaiquyetPhucthamQDK : System.Web.UI.Page
    {
        ...
        private void LoadCombobox()
        {
            ...
            DM_CANBO_BL oDMCBBL = new DM_CANBO_BL();
            DataTable oCBDT = oDMCBBL.DM_CANBO_GETBYDONVI_CHUCDANH(CurrDonViID, ENUM_CHUCDANH.CHUCDANH_THAMPHAN);
            ...
            DataTable tbl = oDMCBBL.DM_CANBO_GETBYDONVI_2CHUCVU(CurrDonViID
                                                                , ENUM_CHUCVU.CHUCVU_CA, ENUM_CHUCVU.CHUCVU_PCA);
            if (tbl != null && tbl.Rows.Count > 0)
            {
        ...
        private bool CheckValid()
        {
            ...
            else
            {
                ...
                if (DateTime.Compare(NgayQD, DNgayThuLy) < 0)
                {
                    lbthongbao.Text = "Ngày quyết định phải lớn hơn hoặc bằng ngày thụ lý sơ thẩm (" + DNgayThuLy.ToString("dd/MM/yyyy") + "). Hãy nhập lại";
                    txtNgayQD.Focus();
                    return false;
                }
                if (DateTime.Compare(NgayQD, DateTime.Today) > 0)
                {
            ...
            else
            {
                ...
                if (DateTime.Compare(NgayPhanCong, NgayQD) < 0)
                {
                    lbthongbao.Text = "Ngày phân công phải lớn hơn hoặc bằng ngày quyết định. Hãy nhập lại";
                    txtNgayphancong.Focus();
                    return false;
                }
            ...
            else
            {
                ...
                if (DateTime.Compare(NgayNhanPhanCong, DateTime.Today) > 0)
                {
                    lbthongbao.Text = "Ngày nhận phân công phải nhỏ hơn hoặc bằng ngày hiện tại. Hãy nhập lại.";
                    txtNhanphancong.Focus();
                    return false;
                }
            }
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string current_id = Session[ENUM_LOAIAN.AN_HINHSU] + "", MaVaiTro = ddlVaitro.SelectedValue;
                decimal VUANID = Convert.ToDecimal(current_id), CanBoID = Convert.ToDecimal(ddlThamphan.SelectedValue);

                AHS_THAMPHANGIAIQUYET oND;
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    // Kiểm tra nếu thẩm phán đã được phân công rồi thì không phân lại nữa. Chọn thẩm phán khác
                    oND = dt.AHS_THAMPHANGIAIQUYET.Where(x => x.VUANID == VUANID && x.CANBOID == CanBoID && x.MAVAITRO == MaVaiTro).FirstOrDefault();
                    if (oND != null)
                    {
                ...
                List<AHS_KCKNQDK_PHUCTHAM_HDXX> lstHDXX = DataExtensions.GetAllWithClause<AHS_KCKNQDK_PHUCTHAM_HDXX>($"VUANID = {VUANID} AND MAVAITRO='{ENUM_NGUOITIENHANHTOTUNG.THAMPHAN}' AND TOA_GIAIQUYET_ID='{donviId}'").ToList();
                if(lstHDXX==null || (lstHDXX!=null && lstHDXX.Count==0))
                {
                    AHS_KCKNQDK_PHUCTHAM_HDXX tpct = new AHS_KCKNQDK_PHUCTHAM_HDXX()
                    {
                        VUANID = VUANID,
                        MAVAITRO = ENUM_NGUOITIENHANHTOTUNG.THAMPHAN,
                        CANBOID=oND.CANBOID,
                        NGAYPHANCONG = oND.NGAYPHANCONG,
                        NGAYNHANPHANCONG = oND.NGAYNHANPHANCONG,
                        NGAYTHAMGIA = oND.NGAYTHAMGIA,
                        NGAYKETTHUC = oND.NGAYKETTHUC,
                        NGUOIPHANCONGID= oND.NGUOIPHANCONGID,
                        NGAYTAO = DateTime.Now,
                        NGAYSUA = DateTime.Now,
                        SOQD = oND.SOQD,
                        NGAYQD = oND.NGAYQD,
                        NGUOITAO = oND.NGUOITAO,
                        NGUOISUA = oND.NGUOISUA,
                        TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID])
                };
                    DataExtensions.Insert(tpct);
                }
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.AHS_THAMPHANGIAIQUYET_GETBY(ID, ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM);
        ...
        public void xoa(decimal id)
        {
            ...
            if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.VUANID), 1, Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Thẩm phán Phúc thẩm Hình sự", "Xóa", json) == false)
            {
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            //AHS_KCKNQDK_PHUCTHAM_HDXX hdxx = dt.AHS_KCKNQDK_PHUCTHAM_HDXX.Where(x => x.VUANID == VUANID).FirstOrDefault();
            //AHS_KCKNQDK_PHUCTHAM_QUYETDINH_BICAN qdBC = dt.AHS_KCKNQDK_PHUCTHAM_QUYETDINH_BICAN.Where(x => x.VUANID == VUANID).FirstOrDefault();
            //AHS_KCKNQDK_PHUCTHAM_QUYETDINH_VUAN qdVA = dt.AHS_KCKNQDK_PHUCTHAM_QUYETDINH_VUAN.Where(x => x.VUANID == VUANID).FirstOrDefault();

            switch (e.CommandName)
            {
                case "Sua":
                    //if (hdxx != null) { lbthongbao.Text = "Đã có người tiến hành tố tụng, không được sửa !"; return; }
                    //if (qdBC != null || qdVA != null) { lbthongbao.Text = "Đã có quyết định, không được sửa !"; return; }
                ...
                case "Xoa":
                    ...
                    //if (qdBC != null || qdVA != null) { lbthongbao.Text = "Đã có quyết định, không được xóa !"; return; }
                    ...
                    if (!CheckQDofThamPhan(VUANID, TPID))
                    {
                        return;
                    }
                    xoa(ND_id);
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                AHS_KCKNQDK_PHUCTHAM_HDXX hdxx = DataExtensions.GetAllWithClause<AHS_KCKNQDK_PHUCTHAM_HDXX>($"VUANID = {VUANID} AND TOA_GIAIQUYET_ID = {Donvi_ID}").FirstOrDefault();
                AHS_KCKNQDK_PHUCTHAM_QUYETDINH_BICAN qdBC = DataExtensions.GetAllWithClause<AHS_KCKNQDK_PHUCTHAM_QUYETDINH_BICAN>($"VUANID = {VUANID} AND TOA_GIAIQUYET_ID = {Donvi_ID}").FirstOrDefault();
                AHS_KCKNQDK_PHUCTHAM_QUYETDINH_VUAN qdVA = DataExtensions.GetAllWithClause<AHS_KCKNQDK_PHUCTHAM_QUYETDINH_VUAN>($"VUANID = {VUANID} AND TOA_GIAIQUYET_ID = {Donvi_ID}").FirstOrDefault();
        ...
        private decimal UploadFileID(decimal VuAnID, string strMaBieumau)
        {
            ...
            if (oVuAn != null)
            {
                ...
                bool isNew = false;
                AHS_FILE objFile = dt.AHS_FILE.Where(x => x.VUANID == VuAnID && x.MAGIAIDOAN == ENUM_GIAIDOANVUAN.PHUCTHAM_QDK && x.BIEUMAUID == IDBM).FirstOrDefault();
                if (objFile == null)
                {
                    isNew = true;
                ...
                objFile.MAGIAIDOAN = ENUM_GIAIDOANVUAN.PHUCTHAM_QDK;
                objFile.LOAIFILE = 0;
        ...
        private bool CheckQDofThamPhan(decimal VuAnID, decimal TPID)
        {
            // kiểm tra các quyết định mà thẩm phán đã ký trong vụ án (nếu có)
            AHS_KCKNQDK_PHUCTHAM_QUYETDINH_BICAN obj_QDBiCan_BiCao = DataExtensions.GetAllWithClause< AHS_KCKNQDK_PHUCTHAM_QUYETDINH_BICAN>($"VUANID = {VuAnID} AND NGUOIKYID = {TPID}").FirstOrDefault();
            if (obj_QDBiCan_BiCao != null)
            {
                lbthongbao.Text = "Thẩm phán đã ký quyết định liên quan tới bị can, bị cáo. Không được xóa!";
                return false;
            }
            AHS_KCKNQDK_PHUCTHAM_QUYETDINH_VUAN obj_QD_VuAn = DataExtensions.GetAllWithClause<AHS_KCKNQDK_PHUCTHAM_QUYETDINH_VUAN>($"VUANID = {VuAnID} AND NGUOIKYID = {TPID}").FirstOrDefault();
            if (obj_QD_VuAn != null)
            {
