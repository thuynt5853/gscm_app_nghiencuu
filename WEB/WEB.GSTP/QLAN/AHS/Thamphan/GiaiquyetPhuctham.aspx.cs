    ...
    public partial class GiaiquyetPhuctham : System.Web.UI.Page
    {
        ...
        public void xoa(decimal id)
        {
            ...
            if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.VUANID), 1, Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Thẩm phán Phúc thẩm Hình sự", "Xóa", json) == false)
            {
             ...
             //Anhpn 
            decimal DONID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HINHSU] + "");
            AHS_NGUOITHAMGIATOTUNG AHS_NGUOITHAMGIATOTUNG = dt.AHS_NGUOITHAMGIATOTUNG.Where(x => x.VUANID == DONID && x.ISPHUCTHAM == 1 && x.TOA_GIAIQUYET_ID == Donvi_ID).FirstOrDefault<AHS_NGUOITHAMGIATOTUNG>();
            if (AHS_NGUOITHAMGIATOTUNG != null)
            {
                lbthongbao.Text = "Xóa không thành công! Do đang tồn tại người tham gia tố tụng.";
                return;
            }
            AHS_PHUCTHAM_HDXX AHS_PHUCTHAM_HDXX = dt.AHS_PHUCTHAM_HDXX.Where(x => x.VUANID == DONID && x.TOA_GIAIQUYET_ID == Donvi_ID).FirstOrDefault<AHS_PHUCTHAM_HDXX>();
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            //AHS_PHUCTHAM_HDXX hdxx = dt.AHS_PHUCTHAM_HDXX.Where(x => x.VUANID == VUANID).FirstOrDefault();
            //AHS_PHUCTHAM_QUYETDINH_BICAN qdBC = dt.AHS_PHUCTHAM_QUYETDINH_BICAN.Where(x => x.VUANID == VUANID).FirstOrDefault();
            //AHS_PHUCTHAM_QUYETDINH_VUAN qdVA = dt.AHS_PHUCTHAM_QUYETDINH_VUAN.Where(x => x.VUANID == VUANID).FirstOrDefault();
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                decimal Donvi_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                AHS_PHUCTHAM_HDXX hdxx = dt.AHS_PHUCTHAM_HDXX.Where(x => x.VUANID == VUANID && x.TOA_GIAIQUYET_ID == Donvi_ID).FirstOrDefault();
                AHS_PHUCTHAM_QUYETDINH_BICAN qdBC = dt.AHS_PHUCTHAM_QUYETDINH_BICAN.Where(x => x.VUANID == VUANID && x.TOA_GIAIQUYET_ID == Donvi_ID).FirstOrDefault();
                AHS_PHUCTHAM_QUYETDINH_VUAN qdVA = dt.AHS_PHUCTHAM_QUYETDINH_VUAN.Where(x => x.VUANID == VUANID && x.TOA_GIAIQUYET_ID == Donvi_ID).FirstOrDefault();
        ...
        private bool CheckQDofThamPhan(decimal VuAnID, decimal TPID)
        {
            ...
            AHS_PHUCTHAM_QUYETDINH_BICAN obj_QDBiCan_BiCao = dt.AHS_PHUCTHAM_QUYETDINH_BICAN.Where(x => x.VUANID == VuAnID && x.NGUOIKYID == TPID).FirstOrDefault<AHS_PHUCTHAM_QUYETDINH_BICAN>();
            if (obj_QDBiCan_BiCao != null)
            {
                lbthongbao.Text = "Thẩm phán đã ký quyết định liên quan tới bị can, bị cáo. Không được xóa!";
                return false;
            }
            AHS_PHUCTHAM_QUYETDINH_VUAN obj_QD_VuAn = dt.AHS_PHUCTHAM_QUYETDINH_VUAN.Where(x => x.VUANID == VuAnID && x.NGUOIKYID == TPID).FirstOrDefault<AHS_PHUCTHAM_QUYETDINH_VUAN>();
            if (obj_QD_VuAn != null)
            {
                lbthongbao.Text = "Thẩm phán đã ký quyết định liên quan tới vụ án. Không được xóa!";
                return false;
            }
            AHS_PHUCTHAM_BANAN obj_BanAn = dt.AHS_PHUCTHAM_BANAN.Where(x => x.VUANID == VuAnID && x.NGUOIKY == TPID).FirstOrDefault<AHS_PHUCTHAM_BANAN>();
            if (obj_BanAn != null)
            {
                lbthongbao.Text = "Thẩm phán đã ký bản án phúc thẩm của vụ án. Không được xóa!";
                return false;
            }
            return true;
        }
