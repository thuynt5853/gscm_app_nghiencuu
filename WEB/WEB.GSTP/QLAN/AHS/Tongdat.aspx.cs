...
namespace WEB.GSTP.QLAN.AHS
{
    public partial class Tongdat : System.Web.UI.Page
    {
        ...
        AHS_TONGDAT_BL TONGDAT_BL = new AHS_TONGDAT_BL();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                string current_id = Session[ENUM_LOAIAN.AN_HINHSU] + "", TOAANID = "0";
                ...
                else if (HinhThucNHanDon == 3)
                {
                    ...
                    isTructuyen = true;
                    //List<AHS_BICANBICAO> lstDSTT = dt.AHS_BICANBICAO.Where(x => x.VUANID == ID && x.ISDAIDIEN == 1 && x.TUCACHTOTUNG_MA == ENUM_DANSU_TUCACHTOTUNG.NGUYENDON).ToList();
                    //if (lstDSTT.Count > 0)
        ...
        private void LoadCombobox()
        {
            ...
            //DataTable tbl = oBL.AHS_FILE_TONGDAT(ID);
            //AHS_SOTHAM_BANAN oND = dt.AHS_SOTHAM_BANAN.Where(x => x.VUANID == VuAnId && x.TOAANID == TOAANID).FirstOrDefault();
            //DM_BIEUMAU bm52 = dt.DM_BIEUMAU.Where(bm => bm.ID == 230).FirstOrDefault();
            //AHS_FILE af = dt.AHS_FILE.Where(x => x.BIEUMAUID == 230 && x.VUANID == VuAnId).FirstOrDefault();
            //AHS_TONGDAT td = dt.AHS_TONGDAT.Where(x => x.VUANID == VuAnId && x.BIEUMAUID == 230).FirstOrDefault();
            //if (oND != null && td == null)
            //{
            ...
            //    tbl.Rows.Add(dr);
            //}
            //AHS_PHUCTHAM_BANAN oND2 = dt.AHS_PHUCTHAM_BANAN.Where(x => x.VUANID == VuAnId && x.TOAANID == TOAANID).FirstOrDefault();
            //DM_BIEUMAU bm75 = dt.DM_BIEUMAU.Where(bm => bm.ID == 261).FirstOrDefault();
            //af = dt.AHS_FILE.Where(x => x.BIEUMAUID == 261 && x.VUANID == VuAnId).FirstOrDefault();
            //td = dt.AHS_TONGDAT.Where(x => x.VUANID == VuAnId && x.BIEUMAUID == 261).FirstOrDefault();
            ...
            AHS_VUAN objDon = dt.AHS_VUAN.Where(x => x.ID == ID).FirstOrDefault();
            decimal giai_doan = Convert.ToDecimal(objDon.MAGIAIDOAN);
            TONGDAT_BL oTongDatBL = new TONGDAT_BL();
            DataTable oDT = oTongDatBL.GET_BM_TONGDAT(ID, 1, giai_doan);
            foreach (DataRow row in oDT.Rows)
            {
                row["TENBM"] = row["MABM"] + ". " + row["TENBM"];
        ...
        private List<AHS_TONGDAT_GS> convertToAHS_TONGDATFromDataTable(DataTable dataTable)
        {
            List<AHS_TONGDAT_GS> aHS_TONGDATs = new List<AHS_TONGDAT_GS>();
            foreach (DataRow row in dataTable.Rows)
            {
                AHS_TONGDAT_GS tONGDAT = new AHS_TONGDAT_GS();
                ...
                try
                {
                    tONGDAT.VUANID = Convert.ToDecimal(row["DONID"]);
                }
                catch
                {
                    tONGDAT.VUANID = null;
                ...
                aHS_TONGDATs.Add(tONGDAT);
            }
            return aHS_TONGDATs;
        }

        private List<AHS_TONGDAT_NGUOINHAN_GS> convertToAHS_TONGDATDOITUONGFromDataTable(DataTable dataTable)
        {
            List<AHS_TONGDAT_NGUOINHAN_GS> aHS_TONGDATs = new List<AHS_TONGDAT_NGUOINHAN_GS>();
            foreach (DataRow row in dataTable.Rows)
            {
                AHS_TONGDAT_NGUOINHAN_GS tONGDAT = new AHS_TONGDAT_NGUOINHAN_GS();
                ...
                // VNPT HuyLQ thêm DS ngày gửi VNeID thành công và ngày xem trên VNeID 19/11/2025 10:08:00
                try
                {
                ...
                //---------21/11/2025---- vnpt - Thêm  XACTHUC_DLDCQG
                try
                {
        ...
        {
            BL.GSTP.AHS.AHS_TONGDAT_BL Bl = new BL.GSTP.AHS.AHS_TONGDAT_BL();
            string CONTENT_JSON = "";
            lbthongbao.Text = "";
            if (CheckValid() == true)
            {
                ...
                AHS_TONGDAT_GS oND = convertToAHS_TONGDATFromDataTable(Bl.AHS_TONGDAT_GETBYID(TongDatID)).FirstOrDefault();
                bool isNew = false;

                bool isUpdate = false;
                AHS_TONGDAT td = new AHS_TONGDAT();
                List<AHS_TONGDAT_DOITUONG> td_dt = new List<AHS_TONGDAT_DOITUONG>();
                ...
                else
                {
                    ...
                    isUpdate = true;
                    td = dt.AHS_TONGDAT.Where(x => x.ID == TongDatID).FirstOrDefault();
                    td_dt = dt.AHS_TONGDAT_DOITUONG.Where(x => x.TONGDATID == td.ID && x.TRANGTHAI == 1).ToList();
                }
                oND.VUANID = DONID;
                oND.TOAANID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                if (isNew)
                {
                    decimal bieu_mau_id = 0;
                    //---------21/11/2025---- Đinh Hoàng Sơn vnpt - Thêm biến FILEID để lọc riêng từng văn bản trùng
                    if (ddlBieumau.SelectedValue.Split('_').Length > 1)
                    {
                        bieu_mau_id = Convert.ToDecimal(ddlBieumau.SelectedValue.Split('_')[0]);
                        decimal FILEID = Convert.ToDecimal(ddlBieumau.SelectedValue.Split('_')[1]);
                        oF = dt.AHS_FILE.Where(x => x.VUANID == DONID && x.BIEUMAUID == bieu_mau_id && x.ID == FILEID).FirstOrDefault();
                    }
                    else
                    {
                        bieu_mau_id = Convert.ToDecimal(ddlBieumau.SelectedValue);
                        oF = dt.AHS_FILE.Where(x => x.VUANID == DONID && x.BIEUMAUID == bieu_mau_id).FirstOrDefault();
                    }
                    if (oF != null)
                    {
                        oND.FILEID = oF.ID;
                        var sothamquyetdinh = dt.AHS_SOTHAM_QUYETDINH_VUAN.FirstOrDefault(x => x.VUANID == DONID && x.FILEID == oF.ID);

                        if (sothamquyetdinh != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.AHS_SOTHAM_QUYETDINH_VUAN;
                            oND.MAPID = sothamquyetdinh.ID;
                        }
                        
                        var phucthamquyetdinh = dt.AHS_PHUCTHAM_QUYETDINH_VUAN.FirstOrDefault(x => x.VUANID == DONID && x.FILEID == oF.ID);

                        if (phucthamquyetdinh != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.AHS_PHUCTHAM_QUYETDINH_VUAN;
                            oND.MAPID = phucthamquyetdinh.ID;
                        }
                        
                        var banansotham = dt.AHS_SOTHAM_BANAN.FirstOrDefault(x => x.VUANID == DONID && x.FILEID == oF.ID);

                        if (banansotham != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.AHS_SOTHAM_BANAN;
                            oND.MAPID = banansotham.ID;
                        }
                        
                        var bananphuctham = dt.AHS_PHUCTHAM_BANAN.FirstOrDefault(x => x.VUANID == DONID && x.FILEID == oF.ID);

                        if (bananphuctham != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.AHS_PHUCTHAM_BANAN;
                    ...
                    oND.ID = Bl.AHS_TONGDAT_UPIN(oND);
                }
                else
                {
                ...
                if (trDuongsu.Visible)
                {
                    ...
                    foreach (DataRow row in TONGDAT_BL.AHS_TONGDATDOITUONG_GETBYTONGDATID(TongDatID).Rows)
                    {
                    ...
                    foreach (DataGridItem Item in dgTructiep.Items)
                    {
                        ...
                        decimal? ID;
                        List<AHS_TONGDAT_NGUOINHAN_GS> lstCheck;
                        try
                        {
                            if (Item.Cells[7].Text == "&nbsp;")
                            {
                                ID = null;
                                lstCheck = new List<AHS_TONGDAT_NGUOINHAN_GS>();
                            }
                            else
                            {
                                ID = Convert.ToDecimal(Item.Cells[7].Text);
                                lstCheck = convertToAHS_TONGDATDOITUONGFromDataTable(Bl.AHS_TONGDATDOITUONG_GETBYID(ID.Value));
                            }
                        }
                        catch
                        {
                        ...
                        AHS_TONGDAT_NGUOINHAN_GS oTD = null;
                        if (lstCheck.Count > 0)
                        {
                            oTD = lstCheck[0];
                        }
                        ...
                        {
                            oTD = new AHS_TONGDAT_NGUOINHAN_GS();
                        }
                        ...
                        if (chkIsSend.Checked)
                        {
                            ...
                            if (DuongsuID != null)
                            {
                                ...
                                obj.DVCQG_THANH_TOAN_UP_TONGDAT(oND.ID, (decimal)DuongsuID, "1");
                    ...
                    if (IDs.Count > 0)
                    {
                        foreach (decimal i in IDs)
                            TONGDAT_BL.AHS_TONGDATDOITUONG_REMOVEBYID(i);
                    }
                }
                if (trTructuyen.Visible)
                {
                    ...
                    foreach (DataGridItem Item in dgTructuyen.Items)
                    {
                        ...
                        List<AHS_TONGDAT_DOITUONG> lstCheck = dt.AHS_TONGDAT_DOITUONG.Where(x => x.TONGDATID == TongdatID && x.DUONGSUID == DuongsuID).ToList();
                        AHS_TONGDAT_DOITUONG oTD = null;
                        if (lstCheck.Count > 0)
                        {
                            oTD = lstCheck[0];
                        }
                        ...
                        if (chkIsSendTT.Checked)
                        {
                            ...
                            if (lstCheck.Count == 0)
                            {
                                ...
                                dt.AHS_TONGDAT_DOITUONG.Add(oTD);
                                dt.SaveChanges();
                ...
                {
                    lst.Add(oND.ID);
                    // vnpt 6/12/2025 chinh luu so vbdh
                    status = CallApi(lst, GetSoThongBao(oND.ID, Convert.ToDecimal(oND.BIEUMAUID), DONID));
                    if (status == "SUCCESS")
                    {
                    ...
                    else
                    {
                        ...
                        {
                            decimal BieuMauID = Convert.ToDecimal(ddlBieumau.SelectedValue);
                            ReUpdateStatusAdd(DONID, ToaAnID, BieuMauID);
                        }
                    }
                }
                catch (Exception)
        ...
        private void ReUpdateStatusAdd(decimal DonID, decimal ToaAnID, decimal BieuMauID)
        {
            AHS_TONGDAT td = dt.AHS_TONGDAT.Where(x => x.VUANID == DonID && x.TOAANID == ToaAnID && x.BIEUMAUID == BieuMauID).FirstOrDefault();
            List<AHS_TONGDAT_DOITUONG> td_dt = dt.AHS_TONGDAT_DOITUONG.Where(x => x.TONGDATID == td.ID).ToList();
            foreach (var item in td_dt)
            {
                AHS_TONGDAT_NGUOINHAN_GS oTD = new AHS_TONGDAT_NGUOINHAN_GS();
        ...
        private void ReUpdateStatusUpdate(List<AHS_TONGDAT_DOITUONG> td_dt, decimal TongDatId)
        {
            ...
            List<AHS_TONGDAT_DOITUONG> list_detail = dt.AHS_TONGDAT_DOITUONG.Where(x => x.TONGDATID == TongDatId).ToList();
        ...
        {
            AHS_VUAN_BL oBL = new AHS_VUAN_BL();
            BL.GSTP.AHS.AHS_TONGDAT_BL TongDatBL = new BL.GSTP.AHS.AHS_TONGDAT_BL();
            string current_id = Session[ENUM_LOAIAN.AN_HINHSU] + "";
            decimal ID = Convert.ToDecimal(current_id), ToaAnID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);

            DataTable oDT = oBL.AHS_TONGDAT_GETLIST(ID, ToaAnID);
            var lstTongDat = convertToAHS_TONGDATFromDataTable(oDT);
            List<AHS_TONGDAT_NGUOINHAN_GS> lstNguoiNhan = new List<AHS_TONGDAT_NGUOINHAN_GS>();
            //  -- VNPT 06122025
            List<AHS_TONGDAT_GS> lstHaventDoituongtongdat = new List<AHS_TONGDAT_GS>();
            foreach (var tongdat in lstTongDat)
            {
                DataTable doiTuongDataTable = TongDatBL.AHS_TONGDATDOITUONG_GETBYTONGDATID(tongdat.ID);
                if (doiTuongDataTable.Rows.Count == 0)
                {
                    lstHaventDoituongtongdat.Add(tongdat);
                }
                lstNguoiNhan.AddRange(convertToAHS_TONGDATDOITUONGFromDataTable(doiTuongDataTable));
            }
            if (oDT != null && lstNguoiNhan.Count == 0)
            {
                if (lstNguoiNhan.Count == 0)
                {
                    ...
                    obj.TONGDAT_DELETE_ERROR(ID.ToString(), "1");
                }
            }
        ...
        public void xoa(decimal id)
        {
            ...
            List<AHS_TONGDAT_DOITUONG> lst = dt.AHS_TONGDAT_DOITUONG.Where(x => x.TONGDATID == id && (x.TRANGTHAI == 0 || x.TRANGTHAI == 2 || ((x.TRANGTHAI == 5 || x.TRANGTHAI == 6) && x.NGAYPHATHANH == null))).ToList();
            if (lst.Count > 0)
            {
                dt.AHS_TONGDAT_DOITUONG.RemoveRange(lst);
                dt.SaveChanges();
            }
            //--
            List<AHS_TONGDAT_DOITUONG> lst_obj = dt.AHS_TONGDAT_DOITUONG.Where(x => x.TONGDATID == id).ToList();
            AHS_TONGDAT oND = dt.AHS_TONGDAT.Where(x => x.ID == id).FirstOrDefault();
            if (oND != null)
            {
                if (lst_obj.Count == 0)
                {
                    obj.DVCQG_TT_REMOVE_TONGDAT(id, "1", ref _VALUE);
                    if (_VALUE == 1)
                    {
                        dt.AHS_TONGDAT.Remove(oND);
                        dt.SaveChanges();
                    }
                    else
                    {
        ...
        public void loadSua(decimal ID)
        {
            ...
            cmdLammoi.Visible = true;
            BL.GSTP.AHS.AHS_TONGDAT_BL TONGDATBL = new BL.GSTP.AHS.AHS_TONGDAT_BL();
            DataTable nguoiNhans = TONGDATBL.AHS_TONGDATDOITUONG_GETBYTONGDATID(ID);
            ...
            {
                string name = TONGDATBL.AHS_TONGDATDOITUONG_GETTENDUONGSU(Convert.ToDecimal(row["ID"]));
        ...
        public void loadPhatHanhLai(decimal ID, decimal vID)
        {
            ...
            BL.GSTP.AHS.AHS_TONGDAT_BL TONGDATBL = new BL.GSTP.AHS.AHS_TONGDAT_BL();
            DataTable nguoiNhans = TONGDATBL.AHS_TONGDATDOITUONG_GETBYTONGDATID(ID);
            DataTable nguoiNhan = TONGDATBL.AHS_TONGDATDOITUONG_GETBYID(vID);
            ...
            string nguoiNhanName = TONGDATBL.AHS_TONGDATDOITUONG_GETTENDUONGSU(Convert.ToDecimal(nguoiNhanRow["ID"]));
        ...
        public void loadedit(decimal ID)
        {
            ...
            if (FILEID > 0)
            {
                //AHS_FILE oF = dt.AHS_FILE.Where(x => x.ID == FILEID).FirstOrDefault();
                //if (oF != null) {
                //    if (oF.TENFILE !=null)
                //        lbtDownload.Visible = true;
                //    else
            ...
            //List<AHS_TONGDAT_DOITUONG> lst = dt.AHS_TONGDAT_DOITUONG.Where(x => x.TONGDATID == ID).ToList();
            //int vCheck = 0;
            //if (lst.Count > 0)
            //{
        ...
        protected void loadAction(string nDID)
        {
            ...
            //    //Call package
            //    BL.GSTP.AHS.AHS_TONGDAT_BL tongDatBL = new BL.GSTP.AHS.AHS_TONGDAT_BL();
            //    tongDatBL.AHS_TONGDAT_THUHOI_DOITUONG(ID, TongdatID, LyDoThuHoi, NgayThuHoi, NguoiSua);
            ...
            if (TrangThai == 2) // Xem lý do thu hồi
            {
                ...
                DataTable objLyDo = oBL.GET_LYDO_THUHOI(1, TongdatID);
                if (objLyDo != null)
                {
            ...
            {
                BL.GSTP.AHS.AHS_TONGDAT_BL tongDatBL = new BL.GSTP.AHS.AHS_TONGDAT_BL();
                tongDatBL.AHS_TONGDAT_VBDH_DOITUONG(ID);
                LoadGrid();
            }
            else if (TrangThai == 5)
            {
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "Download":
                    ...
                    AHS_TONGDAT ads = dt.AHS_TONGDAT.Where(x => x.ID == tongDatId).FirstOrDefault();
                    byte[] conten=null;
                    if (ads != null)
                    {
                        ...
                        {
                            AHS_FILE oF = dt.AHS_FILE.Where(x => x.ID == ads.FILEID).FirstOrDefault();
                            if (oF != null)
                            {
                                if (oF.TENFILE != null)
                                {
                ...
                case "UpdateNgayNhan":
                    ...
                    if (txtNGAYNHANTONGDAT.Text != "")
                    {
                        ...
                        if (obj.TONGDAT_DOITUONG_VNID(Id_obj, "1", txtNGAYNHANTONGDAT.Text) == true)
                        {
                    ...
                    else if (txtNGAYNHANTONGDAT.Text == "")
                    {
                        ...
                        if (obj.TONGDAT_DOITUONG_VNID(Id_obj, "1", null) == true)
                        {
                            strMsg = "Bạn đã cập nhật thành công";
                            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('" + strMsg + "')", true);
                            LoadGrid();
        ...
        private void LoadDoituong(bool IsShowVKS, decimal BieuMauID)
        {
            ...
            if (BieuMauID == 0)
            {
                ...
                if (ddlBieumau.SelectedValue.Split('_').Length > 1)
                {
                    ...
                    FILEID = Convert.ToDecimal(ddlBieumau.SelectedValue.Split('_')[1]);
                } else
                {
                    BMID = Convert.ToDecimal(ddlBieumau.SelectedValue);
                }
            }
            else
            {
            ...
            {
                //---------21/11/2025---- vnpt - Thêm biến FILEID để lọc riêng từng văn bản trùng
                oDTDS = TONGDAT_BL.AHS_TONGDATDOITUONG_GETBYFILEID(DONID, TOAANID, BMID, 0, FILEID);
                trDuongsu.Visible = true;
            }
            else if (IS_TD_NGUYENDON == 1 && IS_TD_DUONGSU != 1)
            {
                //---------21/11/2025---- vnpt - Thêm biến FILEID để lọc riêng từng văn bản trùng
                oDTDS = TONGDAT_BL.AHS_TONGDATDOITUONG_GETBYFILEID(DONID, TOAANID, BMID, 1, FILEID);
                trDuongsu.Visible = true;
            }
            if (trDuongsu.Visible)
            {
        ...
        {
            BL.GSTP.AHS.AHS_TONGDAT_BL aHS = new BL.GSTP.AHS.AHS_TONGDAT_BL();
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                string TenBM = aHS.AHS_TENBM_GETBYTONGDATID(tongdatID);
                Label lbTenBM = (Label)e.Item.FindControl("lbTenBM");
                lbTenBM.Text = TenBM;
                lbNoiNhan.Text = aHS.AHS_TONGDATDOITUONG_GETTENDUONGSU(id);
                if (duongsu_id != null)
                {
                }
                ...
                int rowspan = aHS.AHS_TONGDATDOITUONG_GETBYTONGDATID(tongdatID).Rows.Count;
        ...
        //private void Update_AHS_SOTHAM_BANAN_ANPHI(decimal DonID, decimal DuongSuID, DateTime? NgayNhan)
        //{
        //    AHS_SOTHAM_BANAN ba = dt.AHS_SOTHAM_BANAN.Where(x => x.VUANID == DonID).FirstOrDefault();
        //    if (ba != null)
        //    {
        //        bool isNew = false;
        //        AHS_SOTHAM_BANAN_ANPHI ba_ap = dt.AHS_SOTHAM_BANAN_ANPHI.Where(x => x.DONID == DonID && x.DUONGSU == DuongSuID).FirstOrDefault();
        //        if (ba_ap == null)
        //        {
        //            isNew = true;
        //            ba_ap = new AHS_SOTHAM_BANAN_ANPHI();
        //        }
        //        ba_ap.DONID = DonID;
        ...
        //        if (isNew)
        //        {
        //            dt.AHS_SOTHAM_BANAN_ANPHI.Add(ba_ap);
        //        }
        //        dt.SaveChanges();
        //    }
        //}

        private string GetTuCachToTung(decimal ID_DuongSu)
        {
            string ketQua = "";
            var tcttID = dt.AHS_NGUOITHAMGIATOTUNG_TUCACH.FirstOrDefault(x => x.NGUOIID == ID_DuongSu);
            var tenTCTT = dt.DM_DATAITEM.FirstOrDefault(x => x.ID == tcttID.TUCACHID);
            if (tenTCTT != null)
            {
                ketQua = tenTCTT.MA;
            }
        ...
        protected void dgTructiep_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            BL.GSTP.AHS.AHS_TONGDAT_BL _BL = new BL.GSTP.AHS.AHS_TONGDAT_BL();
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                DataRowView rowView = (DataRowView)e.Item.DataItem;
                ...
                //lbTenTCTT
                if (e.Item.Cells[1].Text + "" == "BICANDAUVU")
                {
                    lbTenTCTT.Text = "Bị can đầu vụ";
                }
                else if (e.Item.Cells[1].Text + "" == "BICAN")
                {
                    lbTenTCTT.Text = "Bị can";
                }
                else
                {
                ...
                if (ID == null) // nếu là hàng theem 
                {
                    ...
                    if (oT.MAGIAIDOAN == 2)
                    {
                        dt.AHS_NGUOITHAMGIATOTUNG.Where(x => x.VUANID == DONID)
                           .ToList()
                           .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.HOTEN, GetTuCachToTung(x.ID) + "," + x.ID)));
                    }
                    ...
                    var atddtisSENT = dt.AHS_TONGDAT_DOITUONG.Where(x => x.TONGDATID == tdId).ToList();
                    if (atddtisSENT.Count > 0)
                    {
                        ...
                        dt.AHS_NGUOITHAMGIATOTUNG.Where(x => x.VUANID == DONID && !tddt.Contains(x.ID)).ToList()
                        .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.HOTEN, GetTuCachToTung(x.ID) + "," + x.ID)));
                        dt.AHS_BICANBICAO.Where(x => x.VUANID == DONID && !tddt.Contains(x.ID)).ToList()
                        .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.HOTEN, (x.BICANDAUVU == 1 ? "BICANDAUVU" : "BICAN") + "," + x.ID)));
                    }
                    lstTGTT.Add(new KeyValuePair<string, string>("--Chọn nơi nhận--", "KHÁC"));
                    ...
                    DM_DATAITEM_BL oBL = new DM_DATAITEM_BL();
                    ddlTCTT.DataSource = oBL.DM_DATAITEM_GETBYGROUPNAME(ENUM_DANHMUC.TUCACHTGTTHS);
        ...
        protected void btn_insert_Click(object sender, EventArgs e)
        {
            if (cmdLammoi.Visible == false)
            {
                ...
                //AHS_FILE oF = dt.AHS_FILE.Where(x => x.VUANID == DONID && x.BIEUMAUID == bieu_mau_id).FirstOrDefault();
                //if (oF != null) BMID = oF.BIEUMAUID + "" == "" ? 0 : (decimal)oF.BIEUMAUID;
            ...
            else
            {
                ...
                BL.GSTP.AHS.AHS_TONGDAT_BL TONGDATBL = new BL.GSTP.AHS.AHS_TONGDAT_BL();
                DataTable nguoiNhans = TONGDATBL.AHS_TONGDATDOITUONG_GETBYTONGDATID(TongDatID);
                nguoiNhans.Columns.Add("TUCACHTOTUNG_MA");
                nguoiNhans.Columns.Add("TONGDAT_DOITUONG");
                nguoiNhans.Columns.Add("TENDUONGSU");
                nguoiNhans.Columns.Add("TENTCTT");
            ...
            foreach (DataGridItem item in dgTructiep.Items)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        decimal ID = Convert.ToDecimal(current_id);
                        AHS_VUAN objDon = dt.AHS_VUAN.Where(x => x.ID == ID).FirstOrDefault();
                        chkIsXACTHUC_DLDCQG.Checked = false;
                        if (!string.IsNullOrEmpty(strSplit[0]))
                        {
                            if (strSplit[0] == "BICANDAUVU" || strSplit[0] == "BICAN")
                            {
                                AHS_BICANBICAO bican = dt.AHS_BICANBICAO.FirstOrDefault(x => x.VUANID == objDon.ID && x.ID == nguoiID);
                                if (bican != null)
                                {
                                    string diaChi = "";
                                    item.Cells[0].Text = bican.ID.ToString();
                                    string tamTruChiTiet = bican.TAMTRUCHITIET + "";
                                    string maTen = dt.DM_HANHCHINH.FirstOrDefault(x => x.ID == bican.TAMTRU_HUYEN)?.MA_TEN ?? "" + "";

                                    if (!string.IsNullOrWhiteSpace(tamTruChiTiet))
                                    {
                                        diaChi = tamTruChiTiet;

                                        if (bican.TAMTRU_HUYEN != null && bican.TAMTRU_HUYEN != 0 && !string.IsNullOrWhiteSpace(maTen))
                                        {
                                            diaChi += ", " + maTen;
                                        }
                                    }
                                    ...
                                    {
                                        if (!string.IsNullOrWhiteSpace(maTen))
                                        {
                                            diaChi = maTen;
                                        }
                                    }
                                    ...
                                    chkIsXACTHUC_DLDCQG.Checked = bican.XACTHUC_DLDCQG == "1";
                                }
                            } else
                            {
                                AHS_NGUOITHAMGIATOTUNG tgtt = dt.AHS_NGUOITHAMGIATOTUNG.FirstOrDefault(x => x.VUANID == objDon.ID && x.ID == nguoiID);
                                if (tgtt != null)
                                {
                                    txtDiachi.Text = tgtt.DIACHICHITIET;
                                    lbDiaChi.Text = tgtt.DIACHICHITIET;
                                    item.Cells[0].Text = tgtt.ID.ToString();
                                    if (!string.IsNullOrWhiteSpace(tgtt.DIACHICHITIET))
                                    {
                                        txtDiachi.Visible = false;
                                        lbDiaChi.Visible = true;
                                    }
        ...
        {
            BL.GSTP.AHS.AHS_TONGDAT_BL tONGDAT_BL = new BL.GSTP.AHS.AHS_TONGDAT_BL();
            ...
            try
            {
                ...
                // Call API
                string result = CallApiThuhoi("1", tongDatIds, LyDoThuHoi, NgayThuHoi, NguoiSua);
                JToken jObject = JToken.Parse(result);
                if ((string)jObject["status"] == "SUCCESS")
                {
                    lbThongBaoThuHoi.Text = "Thu hồi thành công !";
                    ...
                    foreach (DataGridItem item in dgList.Items)
                    {
                        ...
                        if (cb_thuhoi.Checked == true)
                        {
                            tONGDAT_BL.AHS_TONGDAT_THUHOI(v_id, NgayThuHoi, LyDoThuHoi, NguoiSua);
                        }
                    }
                }
                else
        ...
        public List<TongDatDTO> GetAnHinhSu(List<decimal> ids, string soTB)
        {
            List<AHS_TONGDAT> ahsTongDats = dt.AHS_TONGDAT.Where(x => ids.Contains(x.ID) && (x.TRANGTHAI >= 0 && x.TRANGTHAI <= 2)).ToList();
            HashSet<decimal?> idBieuMaus = new HashSet<decimal?>(ahsTongDats.Select(x => x.BIEUMAUID).ToList());
            ...
            HashSet<decimal?> idAHS_DON = new HashSet<decimal?>(ahsTongDats.Select(x => x.VUANID).ToList());
            List<AHS_VUAN> allAHSDon = dt.AHS_VUAN.Where(x => idAHS_DON.Contains(x.ID)).ToList();
            Dictionary<decimal?, AHS_VUAN> mapAhsDon = new Dictionary<decimal?, AHS_VUAN>();
            Dictionary<decimal?, List<AHS_BICANBICAO>> mapDuongSu = dt.AHS_BICANBICAO
                .Where(x => idAHS_DON.Contains(x.VUANID))
                .GroupBy(x => x.VUANID, x => x)
                .ToDictionary(x => x.Key, x => x.ToList());
            // bo xung thong tin van ban di
            foreach (AHS_VUAN ahsDon in allAHSDon)
            {
                mapAhsDon.Add(ahsDon.ID, ahsDon);
            }
            ...
            HashSet<string> usernames = new HashSet<string>(ahsTongDats.Where(x => x.NGUOITAO != null).ToList().Select(x => x.NGUOITAO).ToList());
            ...
            string conditions = "(";
            foreach (AHS_TONGDAT tongDat in ahsTongDats)
            {
            ...
            List<TongDatDTO> danhSachTongDat = ahsTongDats.Select(x => ToDTO(x, mapBieuMau, mapUser, mapAhsDon, mapDuongSu)).ToList();
            ...
            HashSet<decimal?> duongSuID = new HashSet<decimal?>(allDoiTuongs.Where(x => x.MATUCACH == "BICAN" || x.MATUCACH == "BICANDAUVU").Select(x => x.DUONGSUID).ToList());
            Dictionary<decimal, AHS_BICANBICAO> donDuongSu = dt.AHS_BICANBICAO
                .Where(x => duongSuID.Contains(x.ID))
                .ToDictionary(x => x.ID, x => x);

            AHS_BICANBICAO defaultDonDuongSu;
            foreach (TongDatDTO dto in danhSachTongDat)
            {
                List<AHS_TONGDAT_DOITUONG> subList = allDoiTuongs.Where(x => dto.ID.Equals(x.TONGDATID) && x.TRANGTHAI == 1).ToList();
        ...
        {
            List<AHS_TONGDAT> ahsTongDats = dt.AHS_TONGDAT.Where(x => ids.Contains(x.ID)).ToList();
            return ahsTongDats;
        }

        private TongDatDTO ToDTO<T>(T model, Dictionary<decimal?, DM_BIEUMAU> mapBieuMau, Dictionary<string, decimal?> mapUser, Dictionary<decimal?, AHS_VUAN> mapAHS_DON, Dictionary<decimal?, List<AHS_BICANBICAO>> mapDuongSu)
        {
            if (model == null)
                return null;
            ...
            if (model.GetType() == typeof(AHS_TONGDAT))
            {
                ...
                dto.Mavuviec = dto.DonId == null ? null : mapAHS_DON[dto.DonId].MAVUAN;
                dto.Tenvuviec = dto.DonId == null ? null : mapAHS_DON[dto.DonId].TENVUAN;
                dto.Magiaidoan = dto.DonId == null ? null : mapAHS_DON[dto.DonId].MAGIAIDOAN;
                //dto.qhpl = dto.DonId == null ? null : mapAHS_DON[dto.DonId].QUANHEPHAPLUAT_NAME;
                ...
                //// bo xung AHS_DON_XULY
                //DM_BIEUMAU dM_BIEUMAU = mapBieuMau[entity.BIEUMAUID];
                //if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "29-DS")
                //{
                //    List<AHS_DON_XULY> aHS_DON_XULies = dt.AHS_DON_XULY.Where(x => x.DONID == entity.DONID).ToList();
                //    if (aHS_DON_XULies != null && aHS_DON_XULies.Any())
                //    {
                //        dto.Sothongbao = Convert.ToString(aHS_DON_XULies[0].SOTHONGBAO);
                //        dto.Ngaythongbao = Convert.ToDateTime(aHS_DON_XULies[0].NGAYTHONGBAO);
                //    }
                //    // lay thong tin can bo
                //    decimal? canboid = dt.AHS_DON_THAMPHAN.Where(x => x.DONID == entity.DONID && x.MAVAITRO == "VTTP_GIAIQUYETDON").Select(x => x.CANBOID).Take(1).toNumber();
                //    dto.Nguoiky = canboid;
                //}
                //else if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "30-DS")
                //{
                //    List<AHS_SOTHAM_THULY> aHS_DON_XULies = dt.AHS_SOTHAM_THULY.Where(x => x.DONID == entity.DONID).ToList();
                //    if (aHS_DON_XULies != null && aHS_DON_XULies.Any())
                //    {
                //        dto.Sothongbao = Convert.ToString(aHS_DON_XULies[0].SOTHONGBAO);
                //        dto.Ngaythongbao = Convert.ToDateTime(aHS_DON_XULies[0].NGAYTHONGBAO);
                ...
                //{
                //    List<AHS_PHUCTHAM_THULY> aHS_DON_XULies = dt.AHS_PHUCTHAM_THULY.Where(x => x.DONID == entity.DONID).ToList();
                //    if (aHS_DON_XULies != null && aHS_DON_XULies.Any())
                //    {
                //        dto.Sothongbao = Convert.ToString(aHS_DON_XULies[0].SOTHONGBAO);
                //        dto.Ngaythongbao = Convert.ToDateTime(aHS_DON_XULies[0].NGAYTHONGBAO);
                //        dto.Nguoiky = aHS_DON_XULies[0].NGUOIKYID;
                //    }
                //}
                //else
                //{
                //    List<AHS_FILE> aHS_FILEs = dt.AHS_FILE.Where(x => x.ID == entity.FILEID).ToList();
                //    if (aHS_FILEs != null && aHS_FILEs.Any())
                //    {
                //        decimal? maGiaiDoan = aHS_FILEs[0].MAGIAIDOAN;
                //        // so tham
                //        if (maGiaiDoan != null && maGiaiDoan == 2)
                //        {
                //            List<AHS_SOTHAM_QUYETDINH> aHS_SOTHAM_QUYETDINHs = dt.AHS_SOTHAM_QUYETDINH.Where(x => x.QUYETDINHID == dM_BIEUMAU.QUYETDINHID && x.DONID == entity.DONID).ToList();
                //            if (aHS_SOTHAM_QUYETDINHs != null && aHS_SOTHAM_QUYETDINHs.Any())
                //            {
                //                dto.Sothongbao = Convert.ToString(aHS_SOTHAM_QUYETDINHs[0].SOQD);
                //                dto.Ngaythongbao = Convert.ToDateTime(aHS_SOTHAM_QUYETDINHs[0].NGAYQD);
                //                dto.Nguoiky = Convert.ToDecimal(aHS_SOTHAM_QUYETDINHs[0].NGUOIKYID);
                //            }
                //        }
                //        // phuc tham
                //        else if (maGiaiDoan != null && maGiaiDoan == 3)
                //        {
                //            List<AHS_PHUCTHAM_QUYETDINH> aHS_PHUCTHAM_QUYETDINHs = dt.AHS_PHUCTHAM_QUYETDINH.Where(x => x.QUYETDINHID == dM_BIEUMAU.QUYETDINHID && x.DONID == entity.DONID).ToList();
                //            if (aHS_PHUCTHAM_QUYETDINHs != null && aHS_PHUCTHAM_QUYETDINHs.Any())
                //            {
                //                dto.Sothongbao = Convert.ToString(aHS_PHUCTHAM_QUYETDINHs[0].SOQD);
                //                dto.Ngaythongbao = Convert.ToDateTime(aHS_PHUCTHAM_QUYETDINHs[0].NGAYQD);
                //                dto.Nguoiky = Convert.ToDecimal(aHS_PHUCTHAM_QUYETDINHs[0].NGUOIKYID);
                //            }
                //        }
                //    }
                //}
                ...
                if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "27-HS") // Bản án sơ thẩm vnpt 9/12/2025 lấy số cho vbdh
                {
                    AHS_SOTHAM_BANAN ba = dt.AHS_SOTHAM_BANAN.Where(x => x.ID == entity.MAPID).FirstOrDefault();
                    if (ba != null)
                    {
                        //dto.Sothongbao = Convert.ToString(ba.SOBANAN);
                        //dto.Ngaythongbao = Convert.ToDateTime(ba.NGAYBANAN);
                        dto.Nguoiky = ba.NGUOIKY;
                    }
                } else if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "28-HS") // Bản án phúc thẩm vnpt 9/12/2025 lấy số cho vbdh
                {
                    AHS_PHUCTHAM_BANAN ba = dt.AHS_PHUCTHAM_BANAN.Where(x => x.ID == entity.MAPID).FirstOrDefault();
                    if (ba != null)
                    {
                        //dto.Sothongbao = Convert.ToString(ba.SOBANAN);
                        //dto.Ngaythongbao = Convert.ToDateTime(ba.NGAYBANAN);
                ...
                } else
                {
                    ...
                    if (aHS_FILEs != null && aHS_FILEs.Any())
                    {
                        ...
                        if (maGiaiDoan != null && maGiaiDoan == 2)
                        {
                            List<AHS_SOTHAM_QUYETDINH_VUAN> aDS_SOTHAM_QUYETDINHs = dt.AHS_SOTHAM_QUYETDINH_VUAN.Where(x => x.QUYETDINHID == dM_BIEUMAU.QUYETDINHID && x.ID == entity.MAPID).ToList();
                        ...
                        {
                            List<AHS_PHUCTHAM_QUYETDINH_VUAN> aDS_PHUCTHAM_QUYETDINHs = dt.AHS_PHUCTHAM_QUYETDINH_VUAN.Where(x => x.QUYETDINHID == dM_BIEUMAU.QUYETDINHID && x.ID == entity.MAPID).ToList();
        ...
        private DoiTuongDTO ToDTO<T, E>(TongDatDTO entityTongDat, T model, E duongSuModel)
        {
            ...
            if (model.GetType() == typeof(AHS_TONGDAT_DOITUONG))
            {
                ...
                dto.DuongSuId = (entity.MATUCACH == "BICAN" || entity.MATUCACH == "BICANDAUVU") ? entity.DUONGSUID : null;
        ...
        protected string CallApi(List<decimal> ids, string soTB)
        {
            ...
            var input = GetAnHinhSu(ids, soTB);
        ...
        protected void VBDH_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    AHS_TONGDAT td = dt.AHS_TONGDAT.Where(x => x.ID == TongDatID).FirstOrDefault();
                    string stb = "";
                    if (td != null)
                    {
                        // vnpt 6/12/2025 chinh luu so vbdh
                        stb = GetSoThongBao(td.ID, Convert.ToDecimal(td.BIEUMAUID), Convert.ToDecimal(td.VUANID));
                    }
                    string status = CallApi(lst, stb);
                    if (status == "SUCCESS")
                    {
        ...
        private string GetSoThongBao(decimal tongdatID, decimal bieuMauID, decimal donID)
        {
            string STB = "";
            decimal bmID = bieuMauID;
            DM_BIEUMAU bieu_mau = dt.DM_BIEUMAU.Where(x => x.ID == bmID).FirstOrDefault();
            // vnpt 6/12/2025 chinh lay so vbdh
            AHS_TONGDAT entity = dt.AHS_TONGDAT.FirstOrDefault(x => x.ID == tongdatID);
            if (entity != null)
            {
                if (bieu_mau.QUYETDINHID != null) // Các biểu mẫu quyết định vụ việc
                {
                    ...
                    if (aDS_FILEs != null && aDS_FILEs.Any())
                    {
                        ...
                        if (maGiaiDoan != null && maGiaiDoan == 2)
                        {
                            AHS_SOTHAM_QUYETDINH_VUAN qdinh = dt.AHS_SOTHAM_QUYETDINH_VUAN.Where(x => x.VUANID == donID && entity.MAPID == x.ID).FirstOrDefault();
                            if (qdinh != null)
                            {
                                STB = qdinh.SOQUYETDINH;
                            }
                        }
                        else if (maGiaiDoan != null && maGiaiDoan == 3)
                        {
                            AHS_PHUCTHAM_QUYETDINH_VUAN qdinh = dt.AHS_PHUCTHAM_QUYETDINH_VUAN.Where(x => x.VUANID == donID && entity.MAPID == x.ID).FirstOrDefault();
                            if (qdinh != null)
                            {
                                STB = qdinh.SOQUYETDINH;
                            }
                ...
                if (bieu_mau != null && bieu_mau.MABM == "27-HS") //  Bản án sơ thẩm vnpt Lê Bá Thọ 3/12/2025 lấy số cho vbdh
                {
                    AHS_SOTHAM_BANAN ba = dt.AHS_SOTHAM_BANAN.Where(x => x.VUANID == donID).FirstOrDefault();
                    if (ba != null)
                    {
                        STB = Convert.ToString(ba.SOBANAN);
                    }
                }
                else if (bieu_mau != null && bieu_mau.MABM == "28-HS") // Bản án phúc thẩm vnpt /2025 lấy số cho vbdh
                {
