    ...
    public partial class pChonToiDanh : System.Web.UI.Page
    {
        ...
        protected void dropBiCao_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string link = "/QLAN/AHS/PhucTham/BanAn/Popup/pChonToiDanh.aspx?aID=" + BanAnID + "&bID=" + dropBiCao.SelectedValue;
        ...
        void XoaToiDanh(string DelItem)
        {
            ...
            foreach (String item in arr)
            {
               if (item.Length > 0)
               {
                    ...
                    List<AHS_PHUCTHAM_BANAN_DIEU_CT> lst = dt.AHS_PHUCTHAM_BANAN_DIEU_CT.Where(x => x.BICANID == BiCanID
                                                                                                      && x.BANANID == ban_an_id
        ...
        void GetOldChange()
        {
            ...
            lst = dt.AHS_PHUCTHAM_BANAN_DIEU_CT.Where(x => x.BICANID == BiCanID
                                                          && x.BANANID == BanAnID).ToList<AHS_PHUCTHAM_BANAN_DIEU_CT>();
            if (lst != null && lst.Count > 0)
            {
                foreach (AHS_PHUCTHAM_BANAN_DIEU_CT obj in lst)
            ...
            else
            {
                ...
                List<AHS_PHUCTHAM_BANAN_DIEU_CT> lstToiDanhCT = null;
                lstToiDanhCT = dt.AHS_PHUCTHAM_BANAN_DIEU_CT.Where(x => x.BANANID == BanAnID 
                                                                        && x.BICANID == BiCanID).ToList<AHS_PHUCTHAM_BANAN_DIEU_CT>();
                if (lstToiDanhCT != null && lstToiDanhCT.Count>0)
                {
                    foreach (AHS_PHUCTHAM_BANAN_DIEU_CT obj in lstToiDanhCT)
                        OldChange += obj.DIEULUATID + ";" + obj.TOIDANHID + "|";
                    hddOld.Value = OldChange;
                }
            }
