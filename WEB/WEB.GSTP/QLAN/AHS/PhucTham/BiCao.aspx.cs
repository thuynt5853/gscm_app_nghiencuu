...
namespace WEB.GSTP.QLAN.AHS.PhucTham
{
    public partial class BiCao : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                if (VuAnID > 0)
                {
                    ...
                    //check vụ án có bản án thì không cho sửa
                    bool daCoBanAn = dt.AHS_PHUCTHAM_BANAN.Any(x => x.VUANID == VuAnID);

                    if (anKetThuc || daCoBanAn)
                    {
                        lstMsgT.Text = daCoBanAn 
                            ? "Vụ việc đã có bản án, không được sửa đổi!"
                            : "Vụ việc đã kết thúc tại tòa cũ, không thể chỉnh sửa tại tòa mới !";
        ...
        protected void cmdUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (daCoBanAn)
            {
                lstMsgT.Text = "Vụ việc đã có bản án, không được sửa đổi!";
                Cls_Comon.SetButton(cmdUpdate, false);
                Cls_Comon.SetButton(cmdUpdateBottom, false);
                return;
            }
