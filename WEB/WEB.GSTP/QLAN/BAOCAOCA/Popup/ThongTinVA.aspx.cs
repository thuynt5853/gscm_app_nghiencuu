...
namespace WEB.GSTP.QLAN.BAOCAOCA.Popup
{
    public partial class ThongTinVA : System.Web.UI.Page
    {
        ...
        /// <summary>
        /// Load thông tin trong trường hợp đơn chưa gắn vụ án
        /// </summary>
        #region Thông tin từ bảng đơn
        void LoadThongTinVuAn(GDTTT_DON obj)
        {
            ...
            if (obj != null)
            {
                ...
                #region loai an
                temp_str = "";
                temp_id = String.IsNullOrEmpty(obj.BAQD_LOAIAN + "") ? 0 : (decimal)obj.BAQD_LOAIAN;
                if (temp_id > 0)
                {
                    if (temp_id < 10)
                        temp_str = "0" + temp_id + "";
                ...
                try
                {
                    txtVuAn_SoThuLy.Text = (String.IsNullOrEmpty(obj.TL_SO + "") ? "" : ("Số <b>" + obj.TL_SO + "</b>"))
                                            + ((String.IsNullOrEmpty(obj.TL_NGAY + "") || (obj.TL_NGAY == DateTime.MinValue)) ? "" : (" ngày <b>" + ((DateTime)obj.TL_NGAY).ToString("dd/MM/yyyy", cul) + "</b>"));
                }
                ...
                else if (obj.BAQD_CAPXETXU == 2)
                {
                    ...
                    catch (Exception ex) { lttToaXuDenghi.Text = ex.ToString(); }
