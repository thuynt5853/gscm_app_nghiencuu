...
namespace WEB.GSTP.QLAN.BAOCAOCA.Popup
{
    public partial class ThongTinVAHS : System.Web.UI.Page
    {
            ...
            else
            {
                if (!IsPostBack)
                {
                    ...
                    else
                    {
                        if (DonID > 0)
                        {
                            GDTTT_DON obj = dt.GDTTT_DON.Where(x => x.ID == DonID).FirstOrDefault();
                            LoadThongTinVuAn(obj);
                            Load_ThongTinThuLy(obj);
        ...
        /// <summary>
        /// Load thông tin đơn trường hợp chưa gắn với vụ án id
        /// </summary>
        #region Load thông tin đơn trường hợp chưa gắn với vụ án id
        void LoadThongTinVuAn(GDTTT_DON obj)
        {
            string temp = "";
            string temp_str = "";
            decimal temp_id = 0;
            if (obj != null)
            {
                ...
                if (obj.BAQD_CAPXETXU == 4)
                {
                    ...
                    try
                    {
                        temp = (String.IsNullOrEmpty(obj.BAQD_SO + "")) ? "" : ("Số <b>" + obj.BAQD_SO + "</b>");
                        temp += (String.IsNullOrEmpty(obj.BAQD_NGAYBA + "") || (obj.BAQD_NGAYBA == DateTime.MinValue)) ? "" : "  ngày <b>" + ((DateTime)obj.BAQD_NGAYBA).ToString("dd/MM/yyyy", cul) + "</b>";
                        lttBanAnDenghi.Text = temp;
                    }
                    catch (Exception ex) { lttBanAnDenghi.Text = ex.ToString(); }

                    DM_TOAAN objTA = null;
                    temp_id = String.IsNullOrEmpty(obj.BAQD_TOAANID + "") ? 0 : (decimal)obj.BAQD_TOAANID;
                    if (temp_id > 0)
                    {
                        objTA = dt.DM_TOAAN.Where(x => x.ID == obj.BAQD_TOAANID).Single();
                        lttToaXuDenghi.Text = objTA.MA_TEN;
                    }
                    //--BA PT neu co
                    string toa_PT = "", soPT = "", ngaypt = "";
                    decimal toaptID = 0;
                    ngaypt = (String.IsNullOrEmpty(obj.BAQD_NGAYBA_PT + "") || (obj.BAQD_NGAYBA_PT == DateTime.MinValue)) ? "" : "  ngày <b>" + ((DateTime)obj.BAQD_NGAYBA_PT).ToString("dd/MM/yyyy", cul) + "</b>";
                    soPT = (String.IsNullOrEmpty(obj.BAQD_SO_PT + "")) ? "" : ("Số <b>" + obj.BAQD_SO_PT + "</b>");
                    toaptID = String.IsNullOrEmpty(obj.BAQD_TOAANID_PT + "") ? 0 : (decimal)obj.BAQD_TOAANID_PT;
                    if (toaptID > 0)
                    {
                        objTA = dt.DM_TOAAN.Where(x => x.ID == obj.BAQD_TOAANID_PT).Single();
                        toa_PT = " tại " + objTA.MA_TEN;
                    }
                    lttBanAnPT.Text = soPT + ngaypt + toa_PT;
                    if (String.IsNullOrEmpty(lttBanAnPT.Text.Trim()))
                    ...
                    string toa_ST = "", soST = "", ngayST = "";
                    decimal toaST_id = 0;
                    ngayST = (String.IsNullOrEmpty(obj.BAQD_NGAYBA_ST + "") || (obj.BAQD_NGAYBA_ST == DateTime.MinValue)) ? "" : "  ngày <b>" + ((DateTime)obj.BAQD_NGAYBA_ST).ToString("dd/MM/yyyy", cul) + "</b>";
                    soST = (String.IsNullOrEmpty(obj.BAQD_SO_ST + "")) ? "" : ("Số <b>" + obj.BAQD_SO_ST + "</b>");
                    toaST_id = String.IsNullOrEmpty(obj.BAQD_TOAANID_ST + "") ? 0 : (decimal)obj.BAQD_TOAANID_ST;
                    if (toaST_id > 0)
                    {
                        objTA = dt.DM_TOAAN.Where(x => x.ID == obj.BAQD_TOAANID_ST).Single();
                        toa_ST = " tại " + objTA.MA_TEN;
                    }
                    lttBanAnST.Text = soST + ngayST + toa_ST;
                    if (String.IsNullOrEmpty(lttBanAnST.Text.Trim()))
                ...
                else if (obj.BAQD_CAPXETXU == 2)
                {
                    ...
                    try
                    {
                        temp = (String.IsNullOrEmpty(obj.BAQD_SO_ST + "")) ? "" : ("Số <b>" + obj.BAQD_SO_ST + "</b>");
                        temp += (String.IsNullOrEmpty(obj.BAQD_NGAYBA_ST + "") || (obj.BAQD_NGAYBA_ST == DateTime.MinValue)) ? "" : "  ngày <b>" + ((DateTime)obj.BAQD_NGAYBA_ST).ToString("dd/MM/yyyy", cul) + "</b>";
                        lttBanAnDenghi.Text = temp;
                    }
                    catch (Exception ex) { }

                    DM_TOAAN objTA = null;
                    temp_id = String.IsNullOrEmpty(obj.BAQD_TOAANID_ST + "") ? 0 : (decimal)obj.BAQD_TOAANID_ST;
                    if (temp_id > 0)
                    {
                        objTA = dt.DM_TOAAN.Where(x => x.ID == obj.BAQD_TOAANID_ST).Single();
                        lttToaXuDenghi.Text = objTA.MA_TEN;
                ...
                else
                {
                    try
                    {
                        temp = (String.IsNullOrEmpty(obj.BAQD_SO_PT + "")) ? "" : ("Số <b>" + obj.BAQD_SO_PT + "</b>");
                        temp += (String.IsNullOrEmpty(obj.BAQD_NGAYBA_PT + "") || (obj.BAQD_NGAYBA_PT == DateTime.MinValue)) ? "" : "  ngày <b>" + ((DateTime)obj.BAQD_NGAYBA_PT).ToString("dd/MM/yyyy", cul) + "</b>";
                        lttBanAnDenghi.Text = temp;
                    }
                    catch (Exception ex) { }

                    DM_TOAAN objTA = null;
                    temp_id = String.IsNullOrEmpty(obj.BAQD_TOAANID_PT + "") ? 0 : (decimal)obj.BAQD_TOAANID_PT;
                    if (temp_id > 0)
                    {
                        objTA = dt.DM_TOAAN.Where(x => x.ID == obj.BAQD_TOAANID_PT).Single();
                        lttToaXuDenghi.Text = objTA.MA_TEN;
                ...
                #endregion
                lttVuAn_NguoiKhieuNai.Text = obj.NGUOIGUI_HOTEN;
                //------------------------
                lttGhiChu.Text = obj.GHICHU + "";
            }
        }
        void Load_ThongTinThuLy(GDTTT_DON objDon)
        {
            string Str = "";
            if (string.IsNullOrEmpty(lttIsTuHinh.Text))
                lttIsTuHinh.Text = (string.IsNullOrEmpty(objDon.ISANTUHINH + "") || (objDon.ISANTUHINH == 0)) ? "" : "Có";

            Str += "<div class='row_info'>";
            Str += "Thụ lý lần <b>" + 1 + "</b>";
            Str += "<span style='margin-left: 5px;'>";
            Str += (String.IsNullOrEmpty(objDon.TL_SO + "") ? "" : "số " + objDon.TL_SO);
            Str += (String.IsNullOrEmpty(objDon.TL_NGAY + "") ? "" : (" ngày " + ((DateTime)objDon.TL_NGAY).ToString("dd/MM/yyyy", cul) + ""));
            Str += "</span>";
            Str += String.IsNullOrEmpty(objDon.NGUOIGUI_HOTEN + "") ? "" : " của " + objDon.NGUOIGUI_HOTEN;
            Str += "</div>";
            txtVuAn_SoThuLy.Text = Str;
        ...
        void Load_TTV_LanhDao(GDTTT_DON obj)
        {
            decimal temp_id = 0;
            #region Thông tin TTV/lanh dao
            DM_CANBO objCB = null;
