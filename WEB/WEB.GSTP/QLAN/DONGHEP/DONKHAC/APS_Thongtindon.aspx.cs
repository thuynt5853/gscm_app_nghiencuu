...
namespace WEB.GSTP.QLAN.DONGHEP.DONKHAC
{
    public partial class APS_Thongtindon : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        APS_DON oT = dt.APS_DON.Where(x => x.ID == IdDon).FirstOrDefault();
        ...
        private void LoadInfo(decimal ID)
        {
            ...
            DON_KHAC oD = dt.DON_KHAC.Where(x => x.ID == ID && x.LOAIANID == 7).FirstOrDefault();
            APS_DON APS = dt.APS_DON.FirstOrDefault(s => s.ID == oD.DONID);
            txtMaVuViec.Text = APS.MAVUVIEC;
            txtTenVuViec.Text = APS.TENVUVIEC;
            ...
            ddlLoaiQuanhe.SelectedValue = APS.LOAIQUANHE.ToString();
            APS_DON APSoD = dt.APS_DON.Where(x => x.ID == oD.DONID).FirstOrDefault();
            txtQuanhephapluat_name(APSoD);
            if (APS.QHPLTKID != null)
                ddlQHPLTK.SelectedValue = APS.QHPLTKID.ToString();
            ...
            if (oD.LOAIDON == 8)
            {
                if (oD.ISDUONGSU == 0)
                {
                    APS_DON_THAMGIATOTUNG oTT = dt.APS_DON_THAMGIATOTUNG.Where(x => x.ID == oD.DUONGSUID).FirstOrDefault();
                ...
                if (oD.ISDUONGSU == 1)
                {
                    APS_DON_DUONGSU oDonDS = dt.APS_DON_DUONGSU.Where(x => x.ID == oD.DUONGSUID).FirstOrDefault();
        ...
        private void LoadCombobox()
        {
            ...
            APS_DON_DUONGSU_BL oBL_DS_TGTT = new APS_DON_DUONGSU_BL();
            ddlNguoidungdon.DataSource = oBL_DS_TGTT.APS_DON_DUONGSU_THAMGIATOTUNG(Convert.ToDecimal(donid));
            ...
            //Load đương sự
            ddlNguoiKC.DataSource = oBL_DS_TGTT.APS_DON_DUONGSU_THAMGIATOTUNG(Convert.ToDecimal(donid));
        ...
        private bool SaveData()
        {
            try
            {
                ...
                if (loaiDon == "7")
                {
                    ...
                    {
                        oKC = dt.DON_KHAC.Where(x => x.ID == ID && x.LOAIANID == 7).FirstOrDefault();
                    }
                    ...
                    oKC.DONID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_PHASAN]);
                    oKC.LOAIANID = Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_PHASAN);
                    ...
                    APS_DON oDS = dt.APS_DON.Where(x => x.ID == oKC.DONID).FirstOrDefault();
                ...
                else if (loaiDon == "8")
                {
                    ...
                    {
                        oDK = dt.DON_KHAC.Where(x => x.ID == ID && x.LOAIANID == 7).FirstOrDefault();
                    }
                    DateTime dNgayNhanKC;
                    DateTime dNgayGhiTrenDon;
                    dNgayNhanKC = (String.IsNullOrEmpty(txtNgayNhan.Text.Trim())) ? DateTime.MinValue : DateTime.Parse(this.txtNgayNhan.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
                    ...
                    oDK.DONID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_PHASAN]);
                    oDK.LOAIANID = Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_PHASAN);
                    ...
                    {
                        APS_DON_DUONGSU APS_DuongSu = null;

                        if (idDuongSuDK == 0)
                        {
                            APS_DuongSu = new APS_DON_DUONGSU();
                        }
                        else
                        {
                            APS_DuongSu = dt.APS_DON_DUONGSU.Where(x => x.ID == idDuongSuDK).FirstOrDefault();
                        }
                        decimal donid = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_PHASAN]);
                        APS_DuongSu.DONID = donid;
                        APS_DuongSu.TENDUONGSU = Cls_Comon.FormatTenRieng(txtHoTen_DK.Text);
                        if (APS_DuongSu.ISDAIDIEN == 1)
                        {
                            APS_DuongSu.ISDAIDIEN = 1;
                        }
                        else
                        {
                            APS_DuongSu.ISDAIDIEN = 0;
                        }
                        APS_DuongSu.TUCACHTOTUNG_MA = TCTT.MA;
                        APS_DuongSu.LOAIDUONGSU = Convert.ToDecimal(ddlLoaidungdon.SelectedValue);
                        //if (chkISBVQLNK.Visible)
                        //    APS_DuongSu.ISBVQLNGUOIKHAC = chkISBVQLNK.Checked ? 1 : 0;
                        //else
                        //    APS_DuongSu.ISBVQLNGUOIKHAC = 0;
                        APS_DuongSu.SOCMND = txtCMND_Dk.Text;
                        APS_DuongSu.TAMTRUTINHID = Convert.ToDecimal(ddlTamtru_Tinh_DK.SelectedValue);
                        APS_DuongSu.TAMTRUID = Convert.ToDecimal(ddlTamtru_Huyen_DK.SelectedValue);
                        APS_DuongSu.TAMTRUCHITIET = txtDiaChiCT_DK.Text;
                        ...
                        APS_DuongSu.NGAYSINH = dNDNgaysinh;
                        APS_DuongSu.NAMSINH = txtNamsinh_DK.Text == "" ? 0 : Convert.ToDecimal(txtNamsinh_DK.Text);
                        APS_DuongSu.GIOITINH = Convert.ToDecimal(ddlGioiTinh_DK.SelectedValue);
                        APS_DuongSu.EMAIL = txtEmail_DK.Text;
                        APS_DuongSu.DIENTHOAI = txtTel_DK.Text;
                        //if (pnNDTochuc.Visible)
                        //{
                        //    APS_DuongSu.NDD_DIACHIID = Convert.ToDecimal(ddlNDD_Huyen_NguyenDon.SelectedValue);
                        //    APS_DuongSu.NDD_DIACHICHITIET = txtND_NDD_Diachichitiet.Text;
                        //}
                        APS_DuongSu.ISSOTHAM = 1;
                        APS_DuongSu.ISDON = 1;
                        if (idDuongSuDK != 0)
                        {
                            APS_DuongSu.NGAYSUA = DateTime.Now;
                            APS_DuongSu.NGUOISUA = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                            dt.SaveChanges();
                        }
                        else
                        {
                            APS_DuongSu.NGAYTAO = DateTime.Now;
                            APS_DuongSu.NGUOITAO = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                            dt.APS_DON_DUONGSU.Add(APS_DuongSu);
                            dt.SaveChanges();
                        }
                        oDK.ISDUONGSU = 1;
                        oDK.DUONGSUID = APS_DuongSu.ID;
                    }
                    if (TCTT != null && TCTT.GROUPID == 16)
                    {
                        APS_DON_THAMGIATOTUNG APS_TGTT = null;

                        if (idDuongSuDK == 0)
                        {
                            APS_TGTT = new APS_DON_THAMGIATOTUNG();
                        }
                        else
                        {
                            APS_TGTT = dt.APS_DON_THAMGIATOTUNG.Where(x => x.ID == idDuongSuDK).FirstOrDefault();
                        }
                        decimal donid = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_PHASAN]);
                        APS_TGTT.DONID = donid;
                        APS_TGTT.HOTEN = Cls_Comon.FormatTenRieng(txtHoTen_DK.Text);
                        //if (APS_TGTT.ISDAIDIEN == 1)
                        //{
                        //    APS_TGTT.ISDAIDIEN = 1;
                        //}
                        //else
                        //{
                        //    APS_TGTT.ISDAIDIEN = 0;
                        //}
                        APS_TGTT.TUCACHTGTTID = TCTT.MA;
                        //////APS_TGTT. = Convert.ToDecimal(ddlLoaidungdon.SelectedValue);
                        //if (chkISBVQLNK.Visible)
                        //    APS_TGTT.ISBVQLNGUOIKHAC = chkISBVQLNK.Checked ? 1 : 0;
                        //else
                        //    APS_TGTT.ISBVQLNGUOIKHAC = 0;
                        APS_TGTT.SOCMND = txtCMND_Dk.Text;
                        APS_TGTT.TAMTRUTINHID = Convert.ToDecimal(ddlTamtru_Tinh_DK.SelectedValue);
                        APS_TGTT.TAMTRUID = Convert.ToDecimal(ddlTamtru_Huyen_DK.SelectedValue);
                        APS_TGTT.TAMTRUCHITIET = txtDiaChiCT_DK.Text;
                        DateTime dNDNgaysinh;
                        dNDNgaysinh = (String.IsNullOrEmpty(txtNgaysinh_DK.Text.Trim())) ? DateTime.MinValue : DateTime.Parse(this.txtNgaysinh_DK.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
                        APS_TGTT.NGAYSINH = dNDNgaysinh;
                        APS_TGTT.NAMSINH = txtNamsinh_DK.Text == "" ? 0 : Convert.ToDecimal(txtNamsinh_DK.Text);
                        APS_TGTT.GIOITINH = Convert.ToDecimal(ddlGioiTinh_DK.SelectedValue);
                        APS_TGTT.EMAIL = txtEmail_DK.Text;
                        APS_TGTT.DIENTHOAI = txtTel_DK.Text;
                        //if (pnNDTochuc.Visible)
                        //{
                        //    APS_TGTT.NDD_DIACHIID = Convert.ToDecimal(ddlNDD_Huyen_NguyenDon.SelectedValue);
                        //    APS_TGTT.NDD_DIACHICHITIET = txtND_NDD_Diachichitiet.Text;
                        //}
                        //APS_TGTT.ISSOTHAM = 1;
                        //APS_TGTT.ISDON = 1;
                        if (idDuongSuDK != 0)
                        {
                            APS_TGTT.NGAYSUA = DateTime.Now;
                            APS_TGTT.NGUOISUA = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                            dt.SaveChanges();
                        }
                        else
                        {
                            APS_TGTT.NGAYTAO = DateTime.Now;
                            APS_TGTT.NGUOITAO = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                            dt.APS_DON_THAMGIATOTUNG.Add(APS_TGTT);
                            dt.SaveChanges();
                        }
                        oDK.ISDUONGSU = 0;
                        oDK.DUONGSUID = APS_TGTT.ID;
                    }
                    ...
                    APS_DON oDS = dt.APS_DON.Where(x => x.ID == oDK.DONID).FirstOrDefault();
                    oDS.HINHTHUCNHANDON = Convert.ToDecimal(ddlHinhthucnhandon.SelectedValue);
                    dt.SaveChanges();
                }
                else
        ...
        protected void ddlNguoidungdon_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (isDuongSu == 0)
                {
                    APS_DON_THAMGIATOTUNG oDSTT = dt.APS_DON_THAMGIATOTUNG.Where(x => x.DONID == donid && x.ID == idDuongSu).FirstOrDefault();
                    ddlTuCachToTung_DK.Enabled = false;
                    if (oDSTT.HOTEN != null)
                    {
                        txtHoTen_DK.Enabled = false;
            ...
            else
            {
                ...
                if (isDuongSu == 0)
                {
                    APS_DON_THAMGIATOTUNG oTGTT = dt.APS_DON_THAMGIATOTUNG.Where(x => x.DONID == donid && x.ID == idDuongSu).FirstOrDefault();
        ...
        private void LoadQD_BA_InfoKhangCao()
        {
            ...
            decimal DonIDID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_PHASAN] + "");
            APS_DON oDon = dt.APS_DON.Where(x => x.ID == DonIDID).FirstOrDefault();
            if (oDon != null)
            {
