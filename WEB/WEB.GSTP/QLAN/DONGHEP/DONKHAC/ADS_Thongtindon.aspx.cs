...
{
    public partial class ADS_Thongtindon : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        ADS_DON oT = dt.ADS_DON.Where(x => x.ID == IdDon).FirstOrDefault();
        ...
        private void LoadInfo(decimal ID)
        {
            ...
            DON_KHAC oD = dt.DON_KHAC.Where(x => x.ID == ID && x.LOAIANID == 2).FirstOrDefault();
            ADS_DON ADS = dt.ADS_DON.FirstOrDefault(s => s.ID == oD.DONID);
            txtMaVuViec.Text = ADS.MAVUVIEC;
            txtTenVuViec.Text = ADS.TENVUVIEC;
            ...
            ddlLoaiQuanhe.SelectedValue = ADS.LOAIQUANHE.ToString();
            ADS_DON ADSoD = dt.ADS_DON.Where(x => x.ID == oD.DONID).FirstOrDefault();
            txtQuanhephapluat_name(ADSoD);
            if (ADS.QHPLTKID != null)
                ddlQHPLTK.SelectedValue = ADS.QHPLTKID.ToString();
            ...
            if (oD.LOAIDON == 8)
            {
                if (oD.ISDUONGSU == 0)
                {
                    ADS_DON_THAMGIATOTUNG oTT = dt.ADS_DON_THAMGIATOTUNG.Where(x => x.ID == oD.DUONGSUID).FirstOrDefault();
                ...
                if (oD.ISDUONGSU == 1)
                {
                    ADS_DON_DUONGSU oDonDS = dt.ADS_DON_DUONGSU.Where(x => x.ID == oD.DUONGSUID).FirstOrDefault();
        ...
        private void ResetControls()
        {
            ...
            txtNDKC.Text = "";
            txtToaQDBA.Text = "";
            ddlLoaidungdon.SelectedIndex = 0;
        ...
        private void LoadCombobox()
        {
            ...
            ADS_DON_DUONGSU_BL oBL_DS_TGTT = new ADS_DON_DUONGSU_BL();
            ddlNguoidungdon.DataSource = oBL_DS_TGTT.ADS_DON_DUONGSU_THAMGIATOTUNG(Convert.ToDecimal(donid));
            ...
            //Load đương sự
            ddlNguoiKC.DataSource = oBL_DS_TGTT.ADS_DON_DUONGSU_THAMGIATOTUNG(Convert.ToDecimal(donid));
        ...
        //    decimal donid = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_DANSU]);
        //    string hoten = "";
        //    List<ADS_DON_THAMGIATOTUNG> lst = dt.ADS_DON_THAMGIATOTUNG.Where(x => x.DONID == donid).ToList();
        ...
        //            ddlNguoiKC.Items.Add(new ListItem(hoten, item.ID.ToString()));
        //        }
        ...
        private bool SaveData()
        {
            try
            {
                ...
                if (loaiDon == "7")
                {
                    ...
                    {
                        oKC = dt.DON_KHAC.Where(x => x.ID == ID && x.LOAIANID == 2).FirstOrDefault();
                    }
                    ...
                    oKC.DONID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_DANSU]);
                    oKC.LOAIANID = Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_DANSU);
                    ...
                    ADS_DON oDS = dt.ADS_DON.Where(x => x.ID == oKC.DONID).FirstOrDefault();
                ...
                else if (loaiDon == "8")
                {
                    ...
                    {
                        oDK = dt.DON_KHAC.Where(x => x.ID == ID && x.LOAIANID == 2).FirstOrDefault();
                    }
                    ...
                    oDK.DONID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_DANSU]);
                    oDK.LOAIANID = Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_DANSU);
                    ...
                    {
                        ADS_DON_DUONGSU ADS_DuongSu = null;

                        if (idDuongSuDK == 0)
                        {
                            ADS_DuongSu = new ADS_DON_DUONGSU();
                        }
                        else
                        {
                            ADS_DuongSu = dt.ADS_DON_DUONGSU.Where(x => x.ID == idDuongSuDK).FirstOrDefault();
                        }
                        decimal donid = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_DANSU]);
                        ADS_DuongSu.DONID = donid;
                        ADS_DuongSu.TENDUONGSU = Cls_Comon.FormatTenRieng(txtHoTen_DK.Text);
                        if (ADS_DuongSu.ISDAIDIEN == 1)
                        {
                            ADS_DuongSu.ISDAIDIEN = 1;
                        }
                        else
                        {
                            ADS_DuongSu.ISDAIDIEN = 0;
                        }
                        ADS_DuongSu.TUCACHTOTUNG_MA = TCTT.MA;
                        ADS_DuongSu.LOAIDUONGSU = Convert.ToDecimal(ddlLoaidungdon.SelectedValue);
                        //if (chkISBVQLNK.Visible)
                        //    ADS_DuongSu.ISBVQLNGUOIKHAC = chkISBVQLNK.Checked ? 1 : 0;
                        //else
                        //    ADS_DuongSu.ISBVQLNGUOIKHAC = 0;
                        ADS_DuongSu.SOCMND = txtCMND_Dk.Text;
                        ADS_DuongSu.TAMTRUTINHID = Convert.ToDecimal(ddlTamtru_Tinh_DK.SelectedValue);
                        ADS_DuongSu.TAMTRUID = Convert.ToDecimal(ddlTamtru_Huyen_DK.SelectedValue);
                        ADS_DuongSu.TAMTRUCHITIET = txtDiaChiCT_DK.Text;
                        ...
                        ADS_DuongSu.NGAYSINH = dNDNgaysinh;
                        ADS_DuongSu.NAMSINH = txtNamsinh_DK.Text == "" ? 0 : Convert.ToDecimal(txtNamsinh_DK.Text);
                        ADS_DuongSu.GIOITINH = Convert.ToDecimal(ddlGioiTinh_DK.SelectedValue);
                        ADS_DuongSu.EMAIL = txtEmail_DK.Text;
                        ADS_DuongSu.DIENTHOAI = txtTel_DK.Text;
                        ...
                        //}
                        ADS_DuongSu.ISSOTHAM = 1;
                        ADS_DuongSu.ISDON = 1;
                        if (idDuongSuDK != 0)
                        {
                            ADS_DuongSu.NGAYSUA = DateTime.Now;
                            ADS_DuongSu.NGUOISUA = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                            dt.SaveChanges();
                        }
                        else
                        {
                            ADS_DuongSu.NGAYTAO = DateTime.Now;
                            ADS_DuongSu.NGUOITAO = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                            //update 14082025
                            ADS_DuongSu.TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                            dt.ADS_DON_DUONGSU.Add(ADS_DuongSu);
                            dt.SaveChanges();
                        }
                        oDK.ISDUONGSU = 1;
                        oDK.DUONGSUID = ADS_DuongSu.ID;
                    }
                    if (TCTT != null && TCTT.GROUPID == 16)
                    {
                        ADS_DON_THAMGIATOTUNG ADS_TGTT = null;

                        if (idDuongSuDK == 0)
                        {
                            ADS_TGTT = new ADS_DON_THAMGIATOTUNG();
                        }
                        else
                        {
                            ADS_TGTT = dt.ADS_DON_THAMGIATOTUNG.Where(x => x.ID == idDuongSuDK).FirstOrDefault();
                        }
                        decimal donid = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_DANSU]);
                        ADS_TGTT.DONID = donid;
                        ADS_TGTT.HOTEN = Cls_Comon.FormatTenRieng(txtHoTen_DK.Text);
                        ...
                        ADS_TGTT.SOCMND = txtCMND_Dk.Text;
                        ADS_TGTT.TAMTRUTINHID = Convert.ToDecimal(ddlTamtru_Tinh_DK.SelectedValue);
                        ADS_TGTT.TAMTRUID = Convert.ToDecimal(ddlTamtru_Huyen_DK.SelectedValue);
                        ADS_TGTT.TAMTRUCHITIET = txtDiaChiCT_DK.Text;
                        DateTime dNDNgaysinh;
                        dNDNgaysinh = (String.IsNullOrEmpty(txtNgaysinh_DK.Text.Trim())) ? DateTime.MinValue : DateTime.Parse(this.txtNgaysinh_DK.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
                        ADS_TGTT.NGAYSINH = dNDNgaysinh;
                        ADS_TGTT.NAMSINH = txtNamsinh_DK.Text == "" ? 0 : Convert.ToDecimal(txtNamsinh_DK.Text);
                        ADS_TGTT.GIOITINH = Convert.ToDecimal(ddlGioiTinh_DK.SelectedValue);
                        ADS_TGTT.EMAIL = txtEmail_DK.Text;
                        ADS_TGTT.DIENTHOAI = txtTel_DK.Text;
                        //if (pnNDTochuc.Visible)
                        //{
                        ...
                        if (idDuongSuDK != 0)
                        {
                            ADS_TGTT.NGAYSUA = DateTime.Now;
                            ADS_TGTT.NGUOISUA = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                            dt.SaveChanges();
                        }
                        else
                        {
                            ADS_TGTT.NGAYTAO = DateTime.Now;
                            ADS_TGTT.NGUOITAO = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                            //update 14082025
                            ADS_TGTT.TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                            dt.ADS_DON_THAMGIATOTUNG.Add(ADS_TGTT);
                            dt.SaveChanges();
                        }
                        oDK.ISDUONGSU = 0;
                        oDK.DUONGSUID = ADS_TGTT.ID;
                    }
                    ...
                    ADS_DON oDS = dt.ADS_DON.Where(x => x.ID == oDK.DONID).FirstOrDefault();
                    oDS.HINHTHUCNHANDON = Convert.ToDecimal(ddlHinhthucnhandon.SelectedValue);
                    dt.SaveChanges();
        ...
        protected void ddlNguoidungdon_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                if (isDuongSu == 1)
                {
                    ADS_DON_DUONGSU oDS = dt.ADS_DON_DUONGSU.Where(x => x.DONID == donid && x.ID == idDuongSu).FirstOrDefault();
                    ddlTuCachToTung_DK.Enabled = false;
                ...
                if (isDuongSu == 0)
                {
                    ADS_DON_THAMGIATOTUNG oDSTT = dt.ADS_DON_THAMGIATOTUNG.Where(x => x.DONID == donid && x.ID == idDuongSu).FirstOrDefault();
                    ddlTuCachToTung_DK.Enabled = false;
                    if (oDSTT.HOTEN != null)
                    {
                        txtHoTen_DK.Enabled = false;
            ...
            else
            {
                ...
                if (isDuongSu == 0)
                {
                    ADS_DON_THAMGIATOTUNG oTGTT = dt.ADS_DON_THAMGIATOTUNG.Where(x => x.DONID == donid && x.ID == idDuongSu).FirstOrDefault();
        ...
        private void LoadQD_BA_InfoKhangCao()
        {
            ...
            decimal DonIDID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_DANSU] + "");
            ADS_DON oDon = dt.ADS_DON.Where(x => x.ID == DonIDID).FirstOrDefault();
            if (oDon != null)
            {
