...
namespace WEB.GSTP.QLAN.DONGHEP
{
    public partial class ALD_Thongtindon : System.Web.UI.Page
    {
        ...
        {
            int DONID = Convert.ToInt32(Session[ENUM_LOAIAN.AN_LAODONG]);
            txtNguoiNop.Items.Clear();
            txtNguoiNop.Items.Clear();
            List<ALD_DON_DUONGSU> listycPhanto = dt.ALD_DON_DUONGSU.Where(x => x.DONID == DONID && x.TUCACHTOTUNG_MA == "BIDON").OrderBy(x => x.TENDUONGSU).ToList<ALD_DON_DUONGSU>();
            List<ALD_DON_DUONGSU> listycDocLap = dt.ALD_DON_DUONGSU.Where(x => x.DONID == DONID && x.TUCACHTOTUNG_MA == "QUYENNVLQ").OrderBy(x => x.TENDUONGSU).ToList<ALD_DON_DUONGSU>();
        ...
        {
            DON_CHITIET oT = dt.DON_CHITIET.Where(x => x.ID == ID && x.LOAIANID == 5).FirstOrDefault();
            ALD_DON obj = dt.ALD_DON.FirstOrDefault(s => s.ID == oT.DONID);
            ...
            //List<DON_CHITIET_DUONGSU> lstNguyendon = dt.DON_CHITIET_DUONGSU.Where(x => x.DONID == oT.ID && x.LOAIANID == 5 && x.ISDAIDIEN == 1 && x.TUCACHTOTUNG_MA == ENUM_DANSU_TUCACHTOTUNG.NGUYENDON).ToList();
        ...
        private bool CheckValid()
        {
            ...
            if (ddlND_Gioitinh.SelectedValue == "")
            {
                lstMsgB.Text = "Bạn chưa chọn giới tính";
                return false;
            }

            if (ddlBD_Gioitinh.SelectedValue == "")
            {
                lstMsgB.Text = "Bạn chưa chọn giới tính";
                return false;
            }
        ...
        private bool SaveData()
        {
            try
            {
                ...
                {
                    oT = dt.DON_CHITIET.Where(x => x.ID == ID && x.LOAIANID == 5).FirstOrDefault();
                }
                oT.DONID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_LAODONG]);
                oT.LOAIANID = Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_LAODONG);
                ...
                if (ddlLoaidon.SelectedValue != "6" && ddlLoaidon.SelectedValue != "5")
                {
                    ...
                    {
                        oND = dt.ALD_DON_DUONGSU.Where(x => x.ID == idDuongSuND).FirstOrDefault();
                    }
                    ...
                    if (chkKhongCoNYC2.Checked == false)
                    {
                        ...
                        if (idDuongSuBD == 0)
                        {
                            oBD = new ALD_DON_DUONGSU();
                        }
                        else
                        {
                            oBD = dt.ALD_DON_DUONGSU.Where(x => x.ID == idDuongSuBD).FirstOrDefault();
                        }
                        oBD.DONID = donid;
                        ...
                        ALD_DON_DUONGSU oDuongSu_BD = null;
                        for (int i = 0; i < oListDsBD.Count; i++)
                        {
                            ...
                            if (Ods != null)
                            {
                                if (Ods.TUCACHTOTUNG_MA == "BIDON" && Ods.ISDAIDIEN_DONCHITIET == 1)
                                {
                                    oDuongSu_BD = dt.ALD_DON_DUONGSU.Where(x => x.ID == IDduongSu).FirstOrDefault();
                                }
                            }
                        }
                    ...
                    List<DON_DUONGSU_CHITIET> oListDs = dt.DON_DUONGSU_CHITIET.Where(x => x.DONCHITIETID == ID).ToList();
                    ALD_DON_DUONGSU oDuongSu_ND = null;
                    for (int i = 0; i < oListDs.Count; i++)
                    {
                        int IDduongSu = Convert.ToInt32(oListDs[i].DUONGSUID);
                        ALD_DON_DUONGSU Ods = dt.ALD_DON_DUONGSU.Where(x => x.ID == IDduongSu).FirstOrDefault();
                        if (Ods != null)
                        {
                            if (Ods.TUCACHTOTUNG_MA == "NGUYENDON" && Ods.ISDAIDIEN_DONCHITIET == 1)
                            {
                                oDuongSu_ND = dt.ALD_DON_DUONGSU.Where(x => x.ID == IDduongSu).FirstOrDefault();
                            }
                        }
        ...
        protected void lkThemDuongSuKhac_Click(object sender, EventArgs e)
        {
            if (SaveData())
            {
                ...
                {
                    string StrMsg = "PopupCenter('/QLAN/ALD/Hoso/Popup/pDuongsuChiTiet.aspx?hsID=" + DonID + "&bID=0','Thêm đương sự',950,450);";
        ...
        {
            decimal donidBD = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_LAODONG]);
            int idDuongSuBD = Convert.ToInt32(DropDownListDuongSu.SelectedValue);
            ...
            {
                ALD_DON_DUONGSU oDS = dt.ALD_DON_DUONGSU.Where(x => x.DONID == donidBD && x.ID == idDuongSuBD).FirstOrDefault();
            ...
            else
            {
                ...
                ALD_DON_DUONGSU oDuongSu = dt.ALD_DON_DUONGSU.Where(x => x.DONID == donidBD && x.ID == idDuongSuBD).FirstOrDefault();
                if (oDuongSu.LOAIDUONGSU == 1)
                {
                    ddlLoaiBidon.SelectedValue = "1";
                    pnBD_Canhan.Visible = true;
        ...
        void LoaddsDuongSu()
        {
            ...
            decimal donid = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_LAODONG]);
            ALD_DON_DUONGSU_BL ADS_BL = new ALD_DON_DUONGSU_BL();
            DataTable DuongSu = ADS_BL.ALD_DON_DUONGSU_Getall(DonCT, donid);
            ...
            DataTable DuongSuBD = ADS_BL.ALD_DON_DUONGSU_Getall_BiDon(DonCT, donid);
