...
namespace WEB.GSTP.QLAN.DONGHEP
{
    public partial class AKT_Thongtindon : System.Web.UI.Page
    {
        ...
        {
            int DONID = Convert.ToInt32(Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI]);
            txtNguoiNop.Items.Clear();
            txtNguoiNop.Items.Clear();
            List<AKT_DON_DUONGSU> listycPhanto = dt.AKT_DON_DUONGSU.Where(x => x.DONID == DONID && x.TUCACHTOTUNG_MA == "BIDON").OrderBy(x => x.TENDUONGSU).ToList<AKT_DON_DUONGSU>();
            List<AKT_DON_DUONGSU> listycDocLap = dt.AKT_DON_DUONGSU.Where(x => x.DONID == DONID && x.TUCACHTOTUNG_MA == "QUYENNVLQ").OrderBy(x => x.TENDUONGSU).ToList<AKT_DON_DUONGSU>();
        ...
        {
            DON_CHITIET oT = dt.DON_CHITIET.Where(x => x.ID == ID && x.LOAIANID == 4).FirstOrDefault();
            AKT_DON obj = dt.AKT_DON.FirstOrDefault(s => s.ID == oT.DONID);
            ...
            //List<DON_CHITIET_DUONGSU> lstNguyendon = dt.DON_CHITIET_DUONGSU.Where(x => x.DONID == oT.ID && x.LOAIANID == 4 && x.ISDAIDIEN == 1 && x.TUCACHTOTUNG_MA == ENUM_DANSU_TUCACHTOTUNG.NGUYENDON).ToList();
            ...
            if (oT.LOAIDON != 5 && oT.LOAIDON != 6)
            {
                ...
                if (oListDs.Count > 0)
                {
                    ...
                    if (ddlLoaiNguyendon.SelectedValue != "1")
                    {
                        if (oND.ISDNNN != null) chkND_ISDNNN.Checked = oND.ISDNNN == 1 ? true : false;
                    ...
                    else
                    {
                        ...
                        if (ddlLoaiBidon.SelectedValue != "1")
                        {
                            if (oBD.ISDNNN != null) chkBD_ISDNNN.Checked = oBD.ISDNNN == 1 ? true : false;
        ...
        private bool SaveData()
        {
            try
            {
                ...
                {
                    oT = dt.DON_CHITIET.Where(x => x.ID == ID && x.LOAIANID == 4).FirstOrDefault();
                }
                oT.DONID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI]);
                oT.LOAIANID = Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_KINHDOANH_THUONGMAI);
                ...
                if (ddlLoaidon.SelectedValue != "6" && ddlLoaidon.SelectedValue != "5")
                {
                    ...
                    {
                        oND = dt.AKT_DON_DUONGSU.Where(x => x.ID == idDuongSuND).FirstOrDefault();
                    }
                    ...
                    if (chkKhongCoNYC2.Checked == false)
                    {
                        ...
                        if (idDuongSuBD == 0)
                        {
                            oBD = new AKT_DON_DUONGSU();
                        }
                        else
                        {
                            oBD = dt.AKT_DON_DUONGSU.Where(x => x.ID == idDuongSuBD).FirstOrDefault();
                        }
                        oBD.DONID = donid;
                        ...
                        AKT_DON_DUONGSU oDuongSu_BD = null;
                        for (int i = 0; i < oListDsBD.Count; i++)
                        {
                            ...
                            if(Ods != null)
                            {
                                if (Ods.TUCACHTOTUNG_MA == "BIDON" && Ods.ISDAIDIEN_DONCHITIET == 1)
                                {
                                    oDuongSu_BD = dt.AKT_DON_DUONGSU.Where(x => x.ID == IDduongSu).FirstOrDefault();
                                }
                            }
                        }
                    ...
                    List<DON_DUONGSU_CHITIET> oListDs = dt.DON_DUONGSU_CHITIET.Where(x => x.DONCHITIETID == ID).ToList();
                    AKT_DON_DUONGSU oDuongSu_ND = null;
                    for (int i = 0; i < oListDs.Count; i++)
                    {
                        int IDduongSu = Convert.ToInt32(oListDs[i].DUONGSUID);
                        AKT_DON_DUONGSU Ods = dt.AKT_DON_DUONGSU.Where(x => x.ID == IDduongSu).FirstOrDefault();
                        if (Ods != null)
                        {
                            if (Ods.TUCACHTOTUNG_MA == "NGUYENDON" && Ods.ISDAIDIEN_DONCHITIET == 1)
                            {
                                oDuongSu_ND = dt.AKT_DON_DUONGSU.Where(x => x.ID == IDduongSu).FirstOrDefault();
                            }
                        }
                    }
                    if (hddID.Value == "" || hddID.Value == "0")
        ...
        protected void lkThemDuongSuKhac_Click(object sender, EventArgs e)
        {
            if (SaveData())
            {
                ...
                {
                    string StrMsg = "PopupCenter('/QLAN/AKT/Hoso/Popup/pDuongsuChiTiet.aspx?hsID=" + DonID + "&bID=0','Thêm đương sự',950,450);";
        ...
        {
            decimal donidBD = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI]);
            int idDuongSuBD = Convert.ToInt32(DropDownListDuongSu.SelectedValue);
            ...
            {
                AKT_DON_DUONGSU oDS = dt.AKT_DON_DUONGSU.Where(x => x.DONID == donidBD && x.ID == idDuongSuBD).FirstOrDefault();
            ...
            else
            {
                ...
                AKT_DON_DUONGSU oDuongSu = dt.AKT_DON_DUONGSU.Where(x => x.DONID == donidBD && x.ID == idDuongSuBD).FirstOrDefault();
                if (oDuongSu.LOAIDUONGSU == 1)
                {
                    ddlLoaiBidon.SelectedValue = "1";
                    pnBD_Canhan.Visible = true;
        ...
        void LoaddsDuongSu()
        {
            ...
            decimal donid = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_KINHDOANH_THUONGMAI]);
            AKT_DON_DUONGSU_BL ADS_BL = new AKT_DON_DUONGSU_BL();
            DataTable DuongSu = ADS_BL.AKT_DON_DUONGSU_Getall(DonCT, donid);
            ...
            DataTable DuongSuBD = ADS_BL.AKT_DON_DUONGSU_Getall_BiDon(DonCT, donid);
