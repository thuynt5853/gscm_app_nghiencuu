...
namespace WEB.GSTP.QLAN.DONGHEP
{
    public partial class AHC_Thongtindon : System.Web.UI.Page
    {
        ...
        {
            int DONID = Convert.ToInt32(Session[ENUM_LOAIAN.AN_HANHCHINH]);
            txtNguoiNop.Items.Clear();
            txtNguoiNop.Items.Clear();
            List<AHC_DON_DUONGSU> listycPhanto = dt.AHC_DON_DUONGSU.Where(x => x.DONID == DONID && x.TUCACHTOTUNG_MA == "BIDON").OrderBy(x => x.TENDUONGSU).ToList<AHC_DON_DUONGSU>();
            List<AHC_DON_DUONGSU> listycDocLap = dt.AHC_DON_DUONGSU.Where(x => x.DONID == DONID && x.TUCACHTOTUNG_MA == "QUYENNVLQ").OrderBy(x => x.TENDUONGSU).ToList<AHC_DON_DUONGSU>();
        ...
        {
            DON_CHITIET oT = dt.DON_CHITIET.Where(x => x.ID == ID && x.LOAIANID == 6).FirstOrDefault();
            AHC_DON obj = dt.AHC_DON.FirstOrDefault(s => s.ID == oT.DONID);
            ...
            txtQDSO.Text = obj.SOQD + "";
            txtQDHanhvi.Text = obj.HANHVIHC + "";
            txtQDTen.Text = obj.TENQD + "";
            if (obj.NGAYQD != DateTime.MinValue && obj.NGAYQD != null) txtQDNgay.Text = ((DateTime)obj.NGAYQD).ToString("dd/MM/yyyy", cul);
            ...
            //List<DON_CHITIET_DUONGSU> lstNguyendon = dt.DON_CHITIET_DUONGSU.Where(x => x.DONID == oT.ID && x.LOAIANID == 6 && x.ISDAIDIEN == 1 && x.TUCACHTOTUNG_MA == ENUM_DANSU_TUCACHTOTUNG.NGUYENDON).ToList();
        ...
        private bool CheckValid()
        {
            ...
            if (ddlLoaidon.SelectedValue != "5" && ddlLoaidon.SelectedValue != "6")
            {
                ...
                {
                    lstMsgB.Text = "Chưa nhập tên người khởi kiện";
                ...
                if (ddlLoaiNguyendon.SelectedValue == "1")
                {
                    ...
                    {
                        lstMsgB.Text = "Chưa nhập năm sinh người khởi kiện";
                        txtND_Namsinh.Focus();
                        return false;
                    }
                    if (lblND_Batbuoc2.Text != "")
                    {
                        ...
                        {
                            lstMsgB.Text = "Chưa chọn nơi sinh sống của người khởi kiện!";
                ...
                if (ddlLoaiBidon.SelectedValue == "1")
                {
                    ...
                    //{
                    //    lstMsgB.Text = "Chưa nhập năm sinh bị đơn";
                    //    txtBD_Namsinh.Focus();
                    //    return false;
                    //}
                ...
                if (txtBD_Ten.Text == "")
                {
                    lstMsgB.Text = "Chưa nhập tên người bị kiện";
                    txtBD_Ten.Focus();
                    return false;
                }
            }
        ...
        private bool SaveData()
        {
            try
            {
                ...
                {
                    oT = dt.DON_CHITIET.Where(x => x.ID == ID && x.LOAIANID == 6).FirstOrDefault();
                }
                oT.DONID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HANHCHINH]);
                oT.LOAIANID = Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_HANHCHINH);
                ...
                if (ddlLoaidon.SelectedValue != "5" && ddlLoaidon.SelectedValue != "6")
                {
                    ...
                    {
                        oND = dt.AHC_DON_DUONGSU.Where(x => x.ID == idDuongSuND).FirstOrDefault();
                    }
                    ...
                    if (idDuongSuBD == 0)
                    {
                        oBD = new AHC_DON_DUONGSU();
                    }
                    else
                    {
                        oBD = dt.AHC_DON_DUONGSU.Where(x => x.ID == idDuongSuBD).FirstOrDefault();
                    }
                    oBD.DONID = donid;
                    ...
                    List<DON_DUONGSU_CHITIET> oListDs = dt.DON_DUONGSU_CHITIET.Where(x => x.DONCHITIETID == ID).ToList();
                    AHC_DON_DUONGSU oDuongSu_ND = null;
                    AHC_DON_DUONGSU oDuongSu_BD = null;
                    for (int i = 0; i < oListDs.Count; i++)
                    {
                        int IDduongSu = Convert.ToInt32(oListDs[i].DUONGSUID);
                        AHC_DON_DUONGSU Ods = dt.AHC_DON_DUONGSU.Where(x => x.ID == IDduongSu).FirstOrDefault();
                        if (Ods != null)
                        {
                            if (Ods.TUCACHTOTUNG_MA == "NGUYENDON" && Ods.ISDAIDIEN_DONCHITIET == 1)
                            {
                                oDuongSu_ND = dt.AHC_DON_DUONGSU.Where(x => x.ID == IDduongSu).FirstOrDefault();
                            }
                            if (Ods.TUCACHTOTUNG_MA == "BIDON" && Ods.ISDAIDIEN_DONCHITIET == 1)
                            {
                                oDuongSu_BD = dt.AHC_DON_DUONGSU.Where(x => x.ID == IDduongSu).FirstOrDefault();
                            }
                        }
        ...
        protected void lkThemDuongSuKhac_Click(object sender, EventArgs e)
        {
            if (SaveData())
            {
                ...
                {
                    string StrMsg = "PopupCenter('/QLAN/AHC/Hoso/Popup/pDuongsuChiTiet.aspx?hsID=" + DonID + "&bID=0','Thêm đương sự',950,450);";
        ...
        void LoaddsDuongSu()
        {
            ...
            decimal donid = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HANHCHINH]);
            AHC_DON_DUONGSU_BL ADS_BL = new AHC_DON_DUONGSU_BL();
            DataTable DuongSu = ADS_BL.AHC_DON_DUONGSU_Getall(DonCT, donid);
            ...
            DataTable DuongSuBD = ADS_BL.AHC_DON_DUONGSU_Getall_BiDon(DonCT, donid);
