...
namespace WEB.GSTP.QLAN.THA.HoSo
    {
        public partial class ChonToiDanh : System.Web.UI.Page
        {
            ...
            void LoadDrop()
            {
                List<THA_BIAN> lstBC = dt.THA_BIAN.Where(x => x.VUANID == VuAnID).ToList<THA_BIAN>();
                if (lstBC != null && lstBC.Count > 0)
                {
                    dropBiCao.Items.Clear();
                    foreach (THA_BIAN obj in lstBC)
            ...
            void GetOldChange()
            {
                ...
                lst = dt.THA_SOTHAM_CAOTRANG_DIEULUAT.Where(x => x.BICANID == bicanid
                                                              && x.VUANID == VuAnID).ToList<THA_SOTHAM_CAOTRANG_DIEULUAT>();
                if (lst != null && lst.Count > 0)
                {
            ...
            void XoaItem(string DelItem)
            {
                ...
                foreach (String item in arr)
                {
                    if (item.Length > 0)
                    {
                        ...
                        if (lst.Count > 0)
                        {
                            obj = lst.Single<THA_SOTHAM_CAOTRANG_DIEULUAT>();
