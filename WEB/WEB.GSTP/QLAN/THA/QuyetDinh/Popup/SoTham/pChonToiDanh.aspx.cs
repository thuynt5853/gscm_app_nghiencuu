    ...
    public partial class pChonToiDanh : System.Web.UI.Page
    {
        ...
        protected void dropBiCao_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string link = "/QLAN/THA/QuyetDinh/Popup/SoTham/pChonToiDanh.aspx?aID=" + BanAnID;
        ...
        void XoaToiDanh(string DelItem)
        {
            ...
            foreach (String item in arr)
            {
               if (item.Length > 0)
               {
                    ...
                    List<THA_SOTHAM_BANAN_DIEU_CHITIET> lst = DataExtensions.GetAllWithClause<THA_SOTHAM_BANAN_DIEU_CHITIET>($"BICANID = {BiCanID} AND VUANID = {VuAnID} AND DIEULUATID = {boluatid} AND TOIDANHID = {toidanhid}");
                    if (lst.Count > 0)
                    {
        ...
        void GetOldChange()
        {
            ...
            lst = DataExtensions.GetAllWithClause<THA_SOTHAM_BANAN_DIEU_CHITIET>($"BICANID = {BiCanID} AND VUANID = {VuAnID}");
            ...
            else
            {
                ...
                List<THA_SOTHAM_BANAN_DIEU_CHITIET> lstToiDanhCT = null;
                lstToiDanhCT = DataExtensions.GetAllWithClause<THA_SOTHAM_BANAN_DIEU_CHITIET>($"BICANID = {BiCanID} AND VUANID = {VuAnID}");
                if (lstToiDanhCT != null && lstToiDanhCT.Count>0)
                {
                    foreach (THA_SOTHAM_BANAN_DIEU_CHITIET obj in lstToiDanhCT)
                        OldChange += obj.DIEULUATID + ";" + obj.TOIDANHID + "|";
                    hddOld.Value = OldChange;
                }
            }
