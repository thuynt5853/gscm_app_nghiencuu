    ...
    public partial class Thongtindon : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                DsHNDuongSu1.Visible = false;
                DsHNDuongSu1.DonID = 0;
                LoadLoaiDon(false);
                ...
                if (strtype == "new")
                {
                    ...
                    if (strDonID != "")
                    {
                        ...
                        LoadInfo(ID);
                        DsHNDuongSu1.Visible = true;
                        DsHNDuongSu1.DonID = ID;
                        DsHNDuongSu1.ReLoad();
                    }
                }
                ...
                if (strtype != "new")
                {
                    ...
                    if (!string.IsNullOrEmpty(current_id))
                    {
                        ...
                        List<AHN_DON_THAMPHAN> oDON_THAMPHAN = dt.AHN_DON_THAMPHAN.Where(x => x.DONID == Id).ToList<AHN_DON_THAMPHAN>();
                        ...
                        List<AHN_DON_XULY> oDON_XLY = dt.AHN_DON_XULY.Where(x => x.DONID == Id).ToList<AHN_DON_XULY>();
                        if (oDON_XLY.Count > 0)
                        {
            ...
            DsHNDuongSu1.DonID = Convert.ToDecimal(hddID.Value);
        }
        ...
        private void LoadInfo(decimal ID)
        {
            ...
            chkKhongCoNYC2.Visible = oT.LOAIQUANHE == 1 ? false : true;
            ...
            #endregion Hòa giải trạng thái
            //Kiểm tra ẩn/hiện lý do ly hôn
            //trLydoLyhon.Visible = false;

            //DM_QHPL_TK oDMTCT = dt.DM_QHPL_TK.Where(x => x.ID == oT.QHPLTKID).FirstOrDefault();
            //if (oDMTCT != null)
            //    if (oDMTCT.ARRSAPXEP.Contains("0/1042"))
            //    {
            //        trLydoLyhon.Visible = true;
            //        txtSoconchuathanhnien.Text = oT.SOCONCHUATHANHNIEN == null ? "" : oT.SOCONCHUATHANHNIEN.ToString();
            //        txtSoconduoi7tuoi.Text = oT.SOCONDUOI7TUOI == null ? "" : oT.SOCONDUOI7TUOI.ToString();
            //    }
            ...
            List<AHN_DON_DUONGSU> lstNguyendon = dt.AHN_DON_DUONGSU.Where(x => x.DONID == oT.ID && x.ISDAIDIEN == 1 && x.TUCACHTOTUNG_MA == ENUM_DANSU_TUCACHTOTUNG.NGUYENDON).ToList();
            ...
            if (lstNguyendon.Count > 0)
            {
                ...
                {
                    chkKhongLamSachND.Checked = true; // ngươi dùng xác nhận không làm sạch được
                }
                else
                {
            ...
            List<AHN_DON_DUONGSU> lstBidon = dt.AHN_DON_DUONGSU.Where(x => x.DONID == oT.ID && x.ISDAIDIEN == 1 && x.TUCACHTOTUNG_MA == ENUM_DANSU_TUCACHTOTUNG.BIDON).ToList();
            AHN_DON_DUONGSU oBD = new AHN_DON_DUONGSU();
            if (lstBidon.Count > 0)
            {
                oBD = lstBidon[0];
                ...
                if (oBD.XACTHUC_DLDCQG == 3)
                {
                    chkKhongLamSachBD.Checked = true; // ngươi dùng xác nhận không làm sạch được
                }
                else
                {
        ...
        {
            //string quocTichND = ddlND_Quoctich.SelectedValue;
            //if (hdTrangThaiXacThucND.Value == "0" && quocTichND == "2")
            //{
            //    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Bạn cần xác thực thông tin Nguyên đơn!');", true);
            //    return false;
            //}
            //string quocTichBD = ddlBD_Quoctich.SelectedValue;

            //if (hdTrangThaiXacThucBD.Value == "0" && quocTichBD == "2")
            //{
            //    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Bạn cần xác thực thông tin Bị đơn!');", true);
            //    return false;
            //}
            ...
            //    {
            //        lstMsgT.Text = lstMsgB.Text = "Chưa nhập số con dưới 7 tuổi";
            //        txtSoconduoi7tuoi.Focus();
            //        return false;
            //    }
            //    if (txtSoconchuathanhnien.Text == "")
            //    {
            //        lstMsgT.Text = lstMsgB.Text = "Chưa nhập số con từ 7 đến dưới 18 tuổi";
            ...
            if (!chkBoxCMNDND.Checked)
            {
                ...
                if (!string.IsNullOrEmpty(txtND_CMND.Text) && txtND_CMND.Text.Length != 9 && txtND_CMND.Text.Length != 12)
                {
                    lstMsgT.Text = lstMsgB.Text = "Số CMND Nguyên đơn chưa đúng định dạng!";
                    txtND_CMND.Focus();
                    return false;
                }
                if (!string.IsNullOrEmpty(txtND_CCCD.Text) && txtND_CCCD.Text.Length != 12)
                {
                    lstMsgT.Text = lstMsgB.Text = "Số CCCD Nguyên đơn chưa đúng định dạng!";
                    txtND_CCCD.Focus();
                    return false;
                }
            ...
            if (!chkBoxCMNDBD.Checked)
            {
                ...
                if (!string.IsNullOrEmpty(txtBD_CMND.Text) && txtBD_CMND.Text.Length != 9 && txtBD_CMND.Text.Length != 12)
                {
                    lstMsgT.Text = lstMsgB.Text = "Số CMND của Bị đơn chưa đúng định dạng! phải 9 số hoặc 12 số";
                    txtBD_CMND.Focus();
                    return false;
                }
                if (!string.IsNullOrEmpty(txtBD_CCCD.Text) && txtBD_CCCD.Text.Length != 12)
                {
                    lstMsgT.Text = lstMsgB.Text = "Số CCCD của Bị đơn chưa đúng định dạng! phải 12 số";
                    txtBD_CCCD.Focus();
                    return false;
                }
                ...
                if (ddlBD_Quoctich.SelectedValue == "2")
                {
                    ...
                    {
                        lstMsgT.Text = lstMsgB.Text = "Số hộ chiếu của Bị đơn chưa đúng định dạng!";
                        txtBD_HoChieu.Focus();
                        return false;
                    }
                }
            ...
            if (ddlLoaiNguyendon.SelectedValue == "1")
            {
                //Không bắt nhập thông tin năm sinh theo yêu cầu của Khánh P3
                ...
                //{
                //    lstMsgT.Text = lstMsgB.Text = "Bạn chưa nhập ngày sinh.";
                //    txtND_Ngaysinh.Focus();
                //    return false;
                //}

                if (!string.IsNullOrEmpty(txtND_Ngaysinh.Text) && Cls_Comon.IsValidDate(txtND_Ngaysinh.Text) == false)
                {
                    lstMsgT.Text = lstMsgB.Text = "Bạn phải nhập ngày sinh theo định dạng dd/MM/yyyy.";
                    txtND_Ngaysinh.Focus();
                    return false;
                }
            ...
            if (chkKhongCoNYC2.Checked == false)
            {
                if (ddlLoaiBidon.SelectedValue == "1")
                {
                    ...
                    //{
                    //    lstMsgT.Text = lstMsgB.Text = "Chưa nhập năm sinh bị đơn";
                    //    txtBD_Namsinh.Focus();
                    //    return false;
                    //}
                    ...
                    if (!string.IsNullOrEmpty(txtBD_Ngaysinh.Text) && Cls_Comon.IsValidDate(txtBD_Ngaysinh.Text) == false)
                    {
                        lstMsgT.Text = lstMsgB.Text = "Bạn phải nhập ngày sinh theo định dạng dd/MM/yyyy.";
                        txtBD_Ngaysinh.Focus();
                        return false;
                ...
                if (ddlBD_Gioitinh.SelectedValue == "0")
                {
                    lstMsgT.Text = lstMsgB.Text = "Bạn chưa chọn Giới tính Bị đơn";
                    ddlBD_Gioitinh.Focus();
                    return false;
                }
            }
            ...
            {
                decimal ID = Convert.ToDecimal(hddID.Value);
                var oT = dt.AHN_DON.Where(x => x.ID == ID).FirstOrDefault();
                if (ckbTienHanhHG.Checked == false)
                {
                    if (oT.HOAGIAI_TRANGTHAI.GetValueOrDefault(0) > 0)
                    {
                        HOAGIAI_DON hgdon = DataExtensions.GetAllWithClause<HOAGIAI_DON>($"VUVIECID = {oT.ID} AND LOAIANID = {ENUM_LOAIVUVIEC_NUMBER.AN_HONNHAN_GIADINH.toNumber()}").FirstOrDefault();
                        if (hgdon != null)
                        {
        ...
        private bool SaveData()
        {
            try
            {
                ...
                {
                    decimal ID = Convert.ToDecimal(hddID.Value);
                    oT = dt.AHN_DON.Where(x => x.ID == ID).FirstOrDefault();
                }
                ...
                oT.TENVUVIEC = Cls_Comon.FormatTenRieng(txtTennguyendon.Text) + " - " + Cls_Comon.FormatTenRieng(txtBD_Ten.Text) + " - " + txtQuanhephapluat.Text;
                ...
                if (hddID.Value == "" || hddID.Value == "0")
                {
                    ...
                    //Sinh ma vu viec
                    oT.MAVUVIEC = ENUM_LOAIVUVIEC.AN_HONNHAN_GIADINH + "." + Session[ENUM_SESSION.SESSION_MADONVI] + "." + oT.ID.ToString();
                    dt.SaveChanges();
                ...
                if (ckbTienHanhHG.Checked)
                {
                    ...
                    HOAGIAI_DON donHG = DataExtensions.GetAllWithClause<HOAGIAI_DON>($"VUVIECID = {oT.ID} AND LOAIANID = {ENUM_LOAIVUVIEC_NUMBER.AN_HONNHAN_GIADINH.toNumber()}").FirstOrDefault();
                    if (donHG == null)
                    {
                        donHG = new HOAGIAI_DON()
                        {
                            LOAIANID = ENUM_LOAIVUVIEC_NUMBER.AN_HONNHAN_GIADINH.toNumber(),
                ...
                {
                    if (hdTrangThaiXacThucND.Value == null)
                ...
                if (chkKhongCoNYC2.Checked == false)
                {
                    ...
                    {
                        if (hdTrangThaiXacThucBD.Value == null)
        ...
        protected void btnGet037ND_Click(object sender, EventArgs e)
        {
            ...
            if (quocTich == "2")
            {
                ...
                if (CongDan == null)
                {
                    hdTrangThaiXacThucND.Value = "2"; //Đã xác thực nhưng không có kết quả
                ...
                else
                {
                    ...
                    txtTennguyendon.Enabled = txtND_CCCD.Enabled = txtND_Namsinh.Enabled = txtND_Ngaysinh.Enabled = false;
        ...
        protected void btnGet037BD_Click(object sender, EventArgs e)
        {
            ...
            if (quocTich == "2")
            {
                ...
                if (CongDan == null)
                {
                    hdTrangThaiXacThucBD.Value = "2"; //Đã xác thực nhưng không có kết quả
                    string strMsg = "Không tồn tại dữ liệu về Đương sự!";
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('" + strMsg + "')", true);
                    return;
                }
                else
                {
                    ...
                    txtBD_Ten.Enabled = txtBD_CCCD.Enabled = txtBD_Namsinh.Enabled = txtBD_Ngaysinh.Enabled = false;
                    ddlBD_Quoctich.Enabled = false;
                    hdTrangThaiXacThucBD.Value = "1";
                    chkKhongLamSachBD.Checked = false;
                    Session.Remove("CongDan");
                    Session["CongDan"] = CongDan;
                    string StrMsg = "PopupCenter('/QLAN/AHN/Hoso/Popup/pGetDuongSu037.aspx?caller=DuongSuBD','Thông tin công dân từ hệ thống Cơ sở dữ liệu quốc gia về dân cư',850,700);";
                    ScriptManager.RegisterStartupScript(this, this.GetType(), Guid.NewGuid().ToString(), StrMsg, true);
                }
            }
            else
