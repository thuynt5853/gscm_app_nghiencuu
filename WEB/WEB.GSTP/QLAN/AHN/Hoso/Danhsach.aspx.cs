...
namespace WEB.GSTP.QLAN.AHN.Hoso
{
    public partial class Danhsach : System.Web.UI.Page
    {
        ...
        protected void lbtimkiem_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                DataTable oDT = oBL.DON_NHAPTACH_SEARCH(Session["CAP_XET_XU"] + "", txtTenVuViec.Text.Trim(), txt_QHPL.Text.Trim(), txtMaVuViec.Text.Trim(), txtTENDUONGSU.Text.Trim(), dropCapxx.SelectedValue, DropToaAn.SelectedValue,
                                                DropTINHTRANG_THULY.SelectedValue, txtSOTHULY_THONGBAO.Text.Trim(), txt_NGAYTHULY_TU.Text, txt_NGAYTHULY_DEN.Text, ddlThamphan.SelectedValue, ddlHTND_Thuky.SelectedValue, Drop_Loaidon.SelectedValue, status, 3, pageindex, page_size);
                ...
                if (oDT != null && oDT.Rows.Count > 0)
                {
                    foreach (DataRow item in oDT.Rows)
                    {
                        ...
                        {
                            AHN_DON dGoc = dt.AHN_DON.Where(x => x.ID == dNT.VUANGOCID).FirstOrDefault();
        ...
        void Xoa_An_XxlaiST(MenuPermission oPer, decimal IDVuViec)
        {
            ...
            AHN_DON oT = dt.AHN_DON.Where(x => x.ID == IDVuViec).FirstOrDefault();
            AHN_SOTHAM_THULY oT_TL = dt.AHN_SOTHAM_THULY.Where(x => x.DONID == IDVuViec).FirstOrDefault();
            ...
            if (oT != null)
            {
                ...
                {
                    // xóa màn án phí
                    List<AHN_ANPHI> lstAP = dt.AHN_ANPHI.Where(x => x.DONID == IDVuViec).ToList<AHN_ANPHI>();
                    if (lstAP.Count > 0)
                    {
                        foreach (AHN_ANPHI ap in lstAP)
                        {
                            dt.AHN_ANPHI.Remove(ap);
                        }
                        dt.SaveChanges();
                    }

                    // xóa màn tài liệu
                    List<AHN_DON_TAILIEU> lstTL = dt.AHN_DON_TAILIEU.Where(x => x.DONID == IDVuViec).ToList<AHN_DON_TAILIEU>();
                    if (lstTL.Count > 0)
                    {
                        foreach (AHN_DON_TAILIEU tl in lstTL)
                        {
                            dt.AHN_DON_TAILIEU.Remove(tl);
                        }
                        dt.SaveChanges();
                    }

                    // xóa màn người tham gia tố tụng khác
                    List<AHN_DON_THAMGIATOTUNG> lstTGTT = dt.AHN_DON_THAMGIATOTUNG.Where(x => x.DONID == IDVuViec).ToList<AHN_DON_THAMGIATOTUNG>();
                    if (lstTL.Count > 0)
                    {
                        foreach (AHN_DON_THAMGIATOTUNG tgtt in lstTGTT)
                        {
                            dt.AHN_DON_THAMGIATOTUNG.Remove(tgtt);
                        }
                        dt.SaveChanges();
                    }

                    // xóa màn đơn khác
                    List<DON_KHAC> lstDK = dt.DON_KHAC.Where(x => x.DONID == IDVuViec && x.LOAIANID == 3).ToList<DON_KHAC>();
                    if (lstDK.Count > 0)
                    {
                    ...
                    List<AHN_DON_THAMPHAN> lstTP = dt.AHN_DON_THAMPHAN.Where(x => x.DONID == IDVuViec).ToList<AHN_DON_THAMPHAN>();
                    if (lstTP.Count > 0)
                    {
                        foreach (AHN_DON_THAMPHAN tp in lstTP)
                        {
                            dt.AHN_DON_THAMPHAN.Remove(tp);
                        }
                        dt.SaveChanges();
                    }
                    // xóa đơn xử lý
                    List<AHN_DON_XULY> lstDonXL = dt.AHN_DON_XULY.Where(x => x.DONID == IDVuViec).ToList<AHN_DON_XULY>();
                    if (lstTP.Count > 0)
                    {
                        foreach (AHN_DON_XULY xuLy in lstDonXL)
                        {
                            dt.AHN_DON_XULY.Remove(xuLy);
                        }
                        dt.SaveChanges();
                    }


                    List<AHN_FILE> lstF = dt.AHN_FILE.Where(x => x.DONID == IDVuViec).ToList<AHN_FILE>();
                    if (lstF.Count > 0)
                    {
                        foreach (AHN_FILE f in lstF)
                        {
                            dt.AHN_FILE.Remove(f);
                        }
                        dt.SaveChanges();
                    }

                    var delete_all_record_AHN_DON_DUONGSU_by_donid = dt.AHN_DON_DUONGSU.Where(x => x.DONID == IDVuViec);
                    dt.AHN_DON_DUONGSU.RemoveRange(delete_all_record_AHN_DON_DUONGSU_by_donid);

                    dt.AHN_DON.Remove(oT);
                    dt.SaveChanges();

                    //K: Nếu vụ việc tồn tại trong bảng DON_TIEPNHAN thì cập nhật trạng thái DON_GUINHAN = 4
                    DON_TIEPNHAN dtn = dt.DON_TIEPNHAN.Where(x => x.DONID == IDVuViec && x.LOAIAN == 3).FirstOrDefault();
                    if (dtn != null)
                    {
                    ...
                    GD.GIAIDOAN_DELETES("3", IDVuViec, 2);
                    //---------------------------
        ...
        void Xoa_An(MenuPermission oPer, decimal IDVuViec)
        {
            ...
            if (oT != null)
            {
                ...
                if (GiaiDoan == ENUM_GIAIDOANVUAN.SOTHAM)
                {
                    ...
                    // Kiểm tra Án phí
                    AHN_ANPHI anphi = dt.AHN_ANPHI.Where(x => x.DONID == IDVuViec).FirstOrDefault<AHN_ANPHI>();
                    if (anphi != null)
                    {
                    ...
                    AHN_DON_XULY xld = dt.AHN_DON_XULY.Where(x => x.DONID == IDVuViec).FirstOrDefault<AHN_DON_XULY>();
                    if (xld != null)
                    {
                    ...
                    AHN_DON_THAMPHAN gqd = dt.AHN_DON_THAMPHAN.Where(x => x.DONID == IDVuViec && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETDON).FirstOrDefault<AHN_DON_THAMPHAN>();
                    if (gqd != null)
                    {
                    ...
                    AHN_DON_TAILIEU tailieu = dt.AHN_DON_TAILIEU.Where(x => x.DONID == IDVuViec).FirstOrDefault<AHN_DON_TAILIEU>();
                    if (tailieu != null)
                    {
                    ...
                    AHN_DON_THAMGIATOTUNG tgtt = dt.AHN_DON_THAMGIATOTUNG.Where(x => x.DONID == IDVuViec).FirstOrDefault<AHN_DON_THAMGIATOTUNG>();
                    if (tgtt != null)
                    {
                    ...
                    //AHN_DON_DUONGSU ds = dt.AHN_DON_DUONGSU.Where(x => x.DONID == IDVuViec && x.ISDAIDIEN != 1).FirstOrDefault<AHN_DON_DUONGSU>();
                    //if (ds != null)
                    //{
                    ...
                    DON_CHITIET dct = dt.DON_CHITIET.Where(x => x.DONID == IDVuViec && x.LOAIANID == 3).FirstOrDefault();
                    DON_KHAC dk = dt.DON_KHAC.Where(x => x.DONID == IDVuViec && x.LOAIANID == 3).FirstOrDefault();
                    if (dct != null && dk != null)
                    {
        ...
        {
            String MaLoaiVuAn = ENUM_LOAIAN.AN_HONNHAN_GIADINH;
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (hddCHECK_THULY.Value == "" || Session[ENUM_SESSION.SESSION_NHOMNSDID] + "" == "1")
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (VuAnID > 0)
                            {
                                List<AHN_DON_XULY> oDON_XLY = dt.AHN_DON_XULY.Where(x => x.DONID == VuAnID).ToList<AHN_DON_XULY>();
                                ...
                                if ((dv["NGUOITAO"].ToString().ToLower() == strUserName.ToLower() || Session[ENUM_SESSION.SESSION_NHOMNSDID] + "" == "1" || Session[ENUM_SESSION.SESSION_NHOMNSDID] + "" == "541")
                                    ...
                                    lbtXoa.Visible = false;
                                AHN_DON_XULY oDXL_TL = dt.AHN_DON_XULY.Where(x => (x.DONID == VuAnID || x.DON_XULYID == VuAnID) && x.LOAIGIAIQUYET != 3).FirstOrDefault();
                                if (oDON_XLY.Count > 0 && oDXL_TL == null)
                                {
                                    e.Item.Cells[5].Text = "- Trả lại đơn";
                                }
                                var chuyenAn = dt.AHN_CHUYEN_NHAN_AN.Where(x => x.VUANID == VuAnID && x.TOACHUYENID == id_toaan).FirstOrDefault();
                                AHN_DON_XULY oDXL_CD = dt.AHN_DON_XULY.Where(x => (x.DONID == VuAnID || x.DON_XULYID == VuAnID) && x.LOAIGIAIQUYET != 1).FirstOrDefault();
                ...
                string Result = new AHN_CHUYEN_NHAN_AN_BL().Check_NhanAn(VuAnID, "");
                if (Result != "")
                {
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "Select"://Lựa chọn vụ việc cần Lưu thông tin                  
                    ...
                    AHN_DON oDon = dt.AHN_DON.Where(x => x.ID == IDVuViec).FirstOrDefault();

                    //lưu seccsion thông tin kết thúc vụ án
                    decimal Donvi_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                    List<AHN_DON_GIAIDOAN> gd = dt.AHN_DON_GIAIDOAN.AsNoTracking().Where(x => x.DONID == IDVuViec).ToList();
                ...
                case "Xoa":
                    ...
                    if (Session[ENUM_SESSION.SESSION_NHOMNSDID] + "" == "1")
                    {
                        ...
                        if (oBL1.HISTORY_ALLDATA_BY_VUANID(IDVuViec, 3, Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Danh sách án Hôn nhân và gia đình", "Xóa", json) == false)
                        {
        ...
        private void createHoso_xetxulaiPhuctham(decimal vDonID)
        {
            ...
            AHN_DON oDon = dt.AHN_DON.Where(x => x.ID == vDonID).FirstOrDefault();
            if (oDon != null)
            {
                ...
                // an hn
                if (oDON_new.TOA_GIAIQUYET_ID == null) 
                ...
                Session[ENUM_LOAIAN.AN_HONNHAN_GIADINH] = oDON_new.ID;

                //them ma giai doan cap Phuc tham 
                GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                GD.GAIDOAN_INSERT_UPDATE_XXLAI_PHUCTHAM("3", oDON_new.ID, 3, (decimal)oDON_new.TOAANID, LoginDonViID, 0, 0, 0);

                //Câp dương su vụ án
                List<AHN_DON_DUONGSU> lst = dt.AHN_DON_DUONGSU.Where(x => x.DONID == vDonID).ToList<AHN_DON_DUONGSU>();
                foreach (AHN_DON_DUONGSU vDuongsu_old in lst)
                {
                    ...
                    //Ban giao tai lieu
                    List<AHN_DON_TAILIEU> lstTaiLieu = dt.AHN_DON_TAILIEU.Where(x => x.DONID == vDonID && x.NGUOIBANGIAO == vDuongsu_old.ID).ToList<AHN_DON_TAILIEU>();
                ...
                // Phan cong Tham phan
                List<AHN_DON_THAMPHAN> lstThamPhan = dt.AHN_DON_THAMPHAN.Where(x => x.DONID == vDonID && x.MAVAITRO == "VTTP_GIAIQUYETDON").ToList<AHN_DON_THAMPHAN>();
                foreach (AHN_DON_THAMPHAN vThamphan_old in lstThamPhan)
                {
                    ...
                    if (vThamphan_new.TOA_GIAIQUYET_ID == null)
                ...
                //Xu ly don
                List<AHN_DON_XULY> lstXulyDon = dt.AHN_DON_XULY.Where(x => x.DONID == vDonID && x.TOAANID == oDon.TOAANID).ToList<AHN_DON_XULY>();
                ...
                //Tam ung an phi
                List<AHN_ANPHI> lstAP = dt.AHN_ANPHI.Where(x => x.DONID == vDonID).ToList();
                ...
                AHN_SOTHAM_BANAN vBA_old = dt.AHN_SOTHAM_BANAN.Where(x => x.DONID == vDonID).FirstOrDefault();
                if (vBA_old != null)
                {
                    AHN_SOTHAM_BANAN vBA_new = new AHN_SOTHAM_BANAN();
                    ...
                    dt.AHN_SOTHAM_BANAN.Add(vBA_new);
                    dt.SaveChanges();
                }
                List<AHN_SOTHAM_QUYETDINH> LstQD = dt.AHN_SOTHAM_QUYETDINH.Where(x => x.DONID == vDonID).ToList();
                if (LstQD.Count > 0)
                {
                    foreach (AHN_SOTHAM_QUYETDINH vQD_old in LstQD)
                    {
                        AHN_SOTHAM_QUYETDINH vQD_new = new AHN_SOTHAM_QUYETDINH();
                        ...
                        // an hn
                        vQD_new.TOA_GIAIQUYET_ID = vBA_old.TOA_GIAIQUYET_ID;
                        
                        dt.AHN_SOTHAM_QUYETDINH.Add(vQD_new);
                        dt.SaveChanges();
                    }
                }
                //Chuyen nhan an
                List<AHN_CHUYEN_NHAN_AN> LstCN = dt.AHN_CHUYEN_NHAN_AN.Where(x => x.VUANID == vDonID).ToList();
                foreach (AHN_CHUYEN_NHAN_AN voND_old in LstCN)
                {
                    AHN_CHUYEN_NHAN_AN oND = new AHN_CHUYEN_NHAN_AN();
        ...
        private void createHoso_xetxulaiSotham(decimal vDonID)
        {
            ...
            if (oDon != null)
            {
                ...
                //List<AHN_FILE> aDSFiles = dt.AHN_FILE.Where(x => x.DONID == vDonID).ToList<AHN_FILE>();
                //foreach (AHN_FILE aDSFile_OLD in aDSFiles)
                //{
                //    AHN_FILE ADS_FILE_NEW = new AHN_FILE()
                //    {
...
               
                List<AHN_FILE> oldFiles = dt.AHN_FILE
...
                
                Dictionary<AHN_FILE, AHN_FILE> tempMap = new Dictionary<AHN_FILE, AHN_FILE>();
                foreach (AHN_FILE oldFile in oldFiles)
                {
                    ...
                    dt.AHN_FILE.Add(newFile);
                    tempMap.Add(oldFile, newFile);

                }
                dt.SaveChanges();
                foreach (KeyValuePair<AHN_FILE, AHN_FILE> kv in tempMap)
                {
                ...
                List<DON_CHITIET> dON_CHITIETs = dt.DON_CHITIET.Where(x => x.DONID == vDonID && x.LOAIANID == 3).ToList<DON_CHITIET>();
                ...
                List<DON_DUONGSU_CHITIET> dON_DUONGSU_CHITIETs = dt.DON_DUONGSU_CHITIET.Where(x => x.DONID == vDonID && x.LOAIAN == 3).ToList<DON_DUONGSU_CHITIET>();
                ...
                List<AHN_DON_THAMGIATOTUNG> aDS_DON_THAMGIATOTUNGs = dt.AHN_DON_THAMGIATOTUNG.Where(x => x.DONID == vDonID).ToList<AHN_DON_THAMGIATOTUNG>();
                foreach (AHN_DON_THAMGIATOTUNG ADS_DON_THAMGIATOTUNG_OLD in aDS_DON_THAMGIATOTUNGs)
                {
                    AHN_DON_THAMGIATOTUNG ADS_DON_THAMGIATOTUNG_NEW = new AHN_DON_THAMGIATOTUNG();
                    ...
                    dt.AHN_DON_THAMGIATOTUNG.Add(ADS_DON_THAMGIATOTUNG_NEW);
                    dt.SaveChanges();
                }

                //Xu ly don
                List<AHN_DON_XULY> lstXulyDon = dt.AHN_DON_XULY.Where(x => (x.DONID == vDonID || x.DON_XULYID == vDonID) && x.TOAANID == oDon.TOAANID).ToList<AHN_DON_XULY>();
                foreach (AHN_DON_XULY vXulyDon_old in lstXulyDon)
                {
                    AHN_DON_XULY vXulyDon_new = new AHN_DON_XULY();
                    ...
                    DataTable donYCBSes = oBLYC.AHN_GETALL_DON_YCBS(vDonID, vXulyDon_old.ID, 1, 100);
                ...
                foreach (AHN_ANPHI vAP_old in lstAP)
                {
                    ...
                    {
                        DAL.GSTP.DVCQG_THANH_TOAN dVCQG_THANH_TOAN = dt.DVCQG_THANH_TOAN.Where(x => x.ANPHI_ID == vAP_old.ID && x.MALOAIVUVIEC == "3").FirstOrDefault();
                    ...
                    List<AHN_ANPHI_DUONGSU> aDS_ANPHI_DUONGSUs = DataExtensions.GetAllWithClause<AHN_ANPHI_DUONGSU>($" ANPHI_ID == {vAP_old.ID}");
                    if (aDS_ANPHI_DUONGSUs != null && aDS_ANPHI_DUONGSUs.Count > 0)
                    {
                        foreach (AHN_ANPHI_DUONGSU aDS_ANPHI_DUONGSU_OLD in aDS_ANPHI_DUONGSUs)
                        {
                            AHN_ANPHI_DUONGSU aDS_ANPHI_DUONGSU_NEW = new AHN_ANPHI_DUONGSU();
                            ...
                            DataExtensions.Insert<AHN_ANPHI_DUONGSU>(aDS_ANPHI_DUONGSU_NEW);
                        }
                    }

                }

                List<DON_MIENANPHI> dON_MIENANPHIs = DataExtensions.GetAllWithClause<DON_MIENANPHI>($" DONID = {vDonID} AND LOAIAN = 3");
                ...
                List<DON_KHAC> dON_KHAC_OLDs = dt.DON_KHAC.Where(x => x.DONID == vDonID && x.LOAIANID == 3 && x.LOAIDON != 7).ToList<DON_KHAC>();
                ...
                GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                GD.GAIDOAN_INSERT_UPDATE_XXLAI_SOTHAM("3", oDON_new.ID, 2, (decimal)LoginDonViID, 0, 0, 0, 0);
            }
        ...
        protected void cmdTachan_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridItem Item in dgList.Items)
            {
                ...
                if (chkChon.Checked)
                {
                    ...
                    oNSD = dt.AHN_SOTHAM_THULY.Where(x => x.DONID == vuAnGocId).FirstOrDefault();
                }
            }
            if (vuAnGocId == 0)
            {
            ...
            {
                string link = "/QLAN/AHN/Hoso/Popup/pTachAn.aspx?DonID=" + vuAnGocId;
        ...
        protected void cmdNhapan_Click(object sender, EventArgs e)
        {
            ...
            {
                string link = "/QLAN/AHN/Hoso/Popup/pNhapAn.aspx?DonID=" + vuAnGocId;
                ScriptManager.RegisterStartupScript(this, this.GetType(), "onclick", "var Mleft = (screen.width/2)-(1200/2); var Mtop = (screen.height/2)-(750/2); javascript:window.open('" + link + "', '_blank', 'height=750px,width=1200px,top=\'+Mtop+\', left=\'+Mleft+\',resizable=yes,toolbar=yes,scrollbars=1'); ", true);
                Cls_Comon.CallFunctionJS(this, this.GetType(), "LoadModalDiv()");
            }
        }
        ...
        {
            string StrMsg = "PopupCenter('/QLAN/pDanhSachAnPhi.aspx?hsID=" + ENUM_LOAIVUVIEC_NUMBER.AN_HONNHAN_GIADINH.ToString() + "');";
        ...
        {
            string link = "/QLAN/AHN/Hoso/Popup/pTKQuaHan.aspx";
            ScriptManager.RegisterStartupScript(this, this.GetType(), "onclick", "var Mleft = (screen.width/2)-(1200/2); var Mtop = (screen.height/2)-(750/2); javascript:window.open('" + link + "', '_blank', 'height=750px,width=1200px,top=\'+Mtop+\', left=\'+Mleft+\',resizable=yes,toolbar=yes,scrollbars=1'); ", true);
            Cls_Comon.CallFunctionJS(this, this.GetType(), "LoadModalDiv()");
        }
    }
