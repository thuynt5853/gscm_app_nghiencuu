<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPages/GSTP.Master" AutoEventWireup="true" CodeBehind="NguoiThamgiaTT.aspx.cs" Inherits="WEB.GSTP.QLAN.AHN.Hoso.NguoiThamgiaTT" %>
        ...
        function ValidDataInput() {
            ...
            if (!$('#<%=chkBoxCMNDBD.ClientID%>').is(':checked')) {
                ...
                if (txtBD_CMND.value.trim().length == 0 && txtBD_CCCD.value.trim().length == 0 && txtBD_HoChieu.value.trim().length == 0) {
                    alert('Bạn chưa nhập 1 trong 3 nội dung Số CMND/ Thẻ căn cước/ Hộ chiếu. Hãy chọn lại! Nếu không có hãy tích chọn "Không có"');
                    txtBD_CMND.focus();
                    return false;
                }
            }
