...
namespace WEB.GSTP.QLAN.AHN.Hoso.Popup
{
    public partial class pTachAn : System.Web.UI.Page
    {
        ...
        {
            AHN_DON_BL oBL = new AHN_DON_BL(); //KHAI- AHN
        ...
        private void Load_Grid_DonChiTiet()
        {
            ...
            DataTable oDT = oBL.GetDonGhep(vDonViID, DonGocID, 3); //KHAI- AHN
        ...
        private void Load_Grid_DonKhac()
        {
            ...
            int page_size = 10,
                pageindex = Convert.ToInt32(hddPageIndex.Value);
            DONGHEP_BL oBL = new DONGHEP_BL();
            DataTable oDT = oBL.GetDonGhepDonKC(vDonViID, DonGocID, 3); //KHAI- AHN
        ...
        protected void cmdTachAn_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridItem item in dgList.Items)
            {
                ...
                else
                {
                    ...
                    List<DON_DUONGSU_CHITIET> dsCT = dt.DON_DUONGSU_CHITIET
                        .Where(x => x.DONID == DonGocID && x.LOAIAN == 3 && x.DONCHITIETID == donID_DonTach)
                        .ToList(); //KHAI- AHN
            ...
            if (!myCheck)
            {
                ...
                try
                {
                    AHN_DON objDonGoc = dt.AHN_DON.Where(x => x.ID == DonGocID).FirstOrDefault(); //KHAI- AHN
                    AHN_DON objDonMoi = objDonGoc;
                    ...
                    AHN_DON_BL dsBL = new AHN_DON_BL(); //KHAI- AHN
                    objDonMoi.TT = dsBL.GETNEWTT((decimal)objDonMoi.TOAANID);
                    objDonMoi.MAVUVIEC = Session[ENUM_SESSION.SESSION_MADONVI] + "." + ENUM_LOAIVUVIEC.AN_HONNHAN_GIADINH + "." + objDonMoi.TT.ToString(); //KHAI- AHN
                    ...
                    List<DON_DUONGSU_CHITIET> duongSuCT = dt.DON_DUONGSU_CHITIET
                        .Where(x => x.DONID == DonGocID && x.LOAIAN == 3 && x.DONCHITIETID == DonDaiDienID)
                        .ToList(); //KHAI- AHN

                    AHN_DON_DUONGSU nguyenDonDaiDien = new AHN_DON_DUONGSU(); //KHAI- AHN
                    AHN_DON_DUONGSU biDonDaiDien = new AHN_DON_DUONGSU();

                    foreach (var item in duongSuCT)
                    {
                        AHN_DON_DUONGSU objDS = dt.AHN_DON_DUONGSU.Where(x => x.ID == item.DUONGSUID).FirstOrDefault(); //KHAI- AHN
                    ...
                    if (nguyenDonDaiDien.ID != 0 && biDonDaiDien.ID != 0)
                    {
                        ...
                        dt.AHN_DON.Add(objDonMoi); //KHAI- AHN
                        dt.SaveChanges();

                        //Thêm thông tin vụ việc vào _DON_GIAIDOAN
                        GIAI_DOAN_BL donGD = new GIAI_DOAN_BL();
                        donGD.GAIDOAN_INSERT_UPDATE("3", objDonMoi.ID, 2, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), 0, 0, 0, 0); //KHAI- AHN

                        //Update bảng _DON_XULY
                        AHN_DON_XULY donXuLy = dt.AHN_DON_XULY.Where(x => x.DON_CHITIETID == DonDaiDienID && x.DON_XULYID == DonGocID).FirstOrDefault(); //KHAI- AHN
                        if (donXuLy != null)
                        {
                    ...
                    if (!myCheck)
                    {
                        ...
                        List<AHN_DON_DUONGSU> listDS = dt.AHN_DON_DUONGSU.Where(x => x.DONID == DonGocID).ToList(); //KHAI- AHN
                        ...
                        foreach (DataGridItem item in dgList.Items)
                        {
                            ...
                            if (chonVuTach.Checked)
                            {
                                ...
                                if (donID_DonTach != DonDaiDienID)
                                {
                                    ...
                                    //Update DON_XULYID
                                    AHN_DON_XULY donXuLy = dt.AHN_DON_XULY.Where(x => x.DON_CHITIETID == donID_DonTach && x.DON_XULYID == DonGocID).FirstOrDefault(); //KHAI- AHN
                                    if (donXuLy != null)
                                    {
                                    ...
                                    foreach (var itemDS in dsDuongSu)
                                    {
                                        ...
                                        if (!strDuongSuDonTachID.Contains(tempDuongSuID)) //Nếu đương sự không tồn tại trong danh sách DS của đơn khởi kiện mà không chọn để tách và ds đương dự của vụ gốc
                                        {
                                            ...
                                            AHN_DON_DUONGSU donDuongSu = dt.AHN_DON_DUONGSU.Where(x => x.ID == itemDS.DUONGSUID).FirstOrDefault(); //KHAI- AHN
                                            if (donDuongSu != null)
                                            {
                                            ...
                                            //Cập nhật án phí
                                            AHN_ANPHI anPhi = dt.AHN_ANPHI.Where(x => x.DONID == DonGocID && x.DUONGSU_ID == itemDS.DUONGSUID).FirstOrDefault(); //KHAI- AHN
                                            if (anPhi != null)
                                            {
                                            ...
                                            AHN_DON_TAILIEU taiLieu = dt.AHN_DON_TAILIEU
                                                .Where(x => x.DONID == DonGocID && x.NGUOIBANGIAO == itemDS.DUONGSUID)
                                                .FirstOrDefault(); //KHAI- AHN
                                        ...
                                        else
                                        {
                                            ...
                                            AHN_DON_DUONGSU donDuongSu = dt.AHN_DON_DUONGSU.Where(x => x.ID == itemDS.DUONGSUID).FirstOrDefault(); //KHAI- AHN
                                            AHN_DON_DUONGSU objDS = donDuongSu;
                                            ...
                                            if (taiLieu != null)
                                            {
                                                AHN_DON_TAILIEU taiLieuCopy = taiLieu; //KHAI- AHN
                                                taiLieuCopy.DONID = objDonMoi.ID;
                                                taiLieuCopy.NGUOIBANGIAO = objDS.ID;
                                                dt.AHN_DON_TAILIEU.Add(taiLieuCopy);
                                            }
                        ...
                        foreach (DataGridItem itemKhac in dgListKhac.Items)
                        {
                            ...
                            if (chonDonKhac.Checked)
                            {
                                ...
                                {
                                    AHN_DON_DUONGSU donKhacDS = dt.AHN_DON_DUONGSU
                                ...
                                {
                                    AHN_DON_THAMGIATOTUNG donKhacTGTT = dt.AHN_DON_THAMGIATOTUNG
                                        .Where(x => x.DONID == DonGocID && x.ID == donKhac.DUONGSUID)
                                        .FirstOrDefault(); //KHAI- AHN
                        ...
                        //Copy sang đơn mới
                        List<AHN_SOTHAM_HDXX> listHDXX = dt.AHN_SOTHAM_HDXX.Where(x => x.DONID == DonGocID).ToList();
                        AHN_SOTHAM_HDXX hdxx = new AHN_SOTHAM_HDXX(); //KHAI- AHN
                        if (listHDXX != null)
                        {
                        ...
                        AHN_SOTHAM_THULY thuly = dt.AHN_SOTHAM_THULY.Where(x => x.DONID == DonGocID).FirstOrDefault();
                        if (thuly != null)
                        {
                            AHN_SOTHAM_THULY thuLyDonMoi = thuly; //KHAI- AHN
                            thuLyDonMoi.DONID = objDonMoi.ID;
                            dt.AHN_SOTHAM_THULY.Add(thuLyDonMoi);
                            dt.SaveChanges();
                        }

                        List<AHN_DON_THAMPHAN> listThamPhan = dt.AHN_DON_THAMPHAN.Where(x => x.DONID == DonGocID).ToList();
                        AHN_DON_THAMPHAN thamPhan = new AHN_DON_THAMPHAN(); //KHAI- AHN
                        if (listThamPhan != null)
                        {
                            foreach (var item in listThamPhan)
                            {
                                ...
                                dt.AHN_DON_THAMPHAN.Add(thamPhan);
                                dt.SaveChanges();
                            }
                        }

                        List<AHN_SOTHAM_QUYETDINH> listQD = dt.AHN_SOTHAM_QUYETDINH.Where(x => x.DONID == DonGocID).ToList();
                        AHN_SOTHAM_QUYETDINH quyetDinh = new AHN_SOTHAM_QUYETDINH(); //KHAI- AHN
                        if (listQD != null)
                        {
                            foreach (var item in listQD)
                            {
                                ...
                                dt.AHN_SOTHAM_QUYETDINH.Add(quyetDinh);
                                dt.SaveChanges();
                            }
                        }
                        ...
                        List<AHN_DON_DUONGSU> dsDuongSuDonNhap = dt.AHN_DON_DUONGSU.Where(x => x.DONID == objDonMoi.ID).ToList(); //KHAI- AHN
                        string jsonDuongSu = ",";
                        ...
                        //Lấy ds thẩm phán
                        List<AHN_DON_THAMPHAN> dsThamPhan = dt.AHN_DON_THAMPHAN.Where(x => x.DONID == objDonMoi.ID).ToList(); //KHAI- AHN
                        string jsonThamPhan = ",";
                        ...
                        //Lấy ds thư kí
                        List<AHN_SOTHAM_HDXX> dsSoThamHDXX = dt.AHN_SOTHAM_HDXX.Where(x => x.DONID == objDonMoi.ID && x.MAVAITRO == "THUKY").ToList();
                        List<AHN_PHUCTHAM_HDXX> dsPhucThamHDXX = dt.AHN_PHUCTHAM_HDXX.Where(x => x.DONID == objDonMoi.ID && x.MAVAITRO == "THUKY").ToList();
                        string jsonThuKy = ",";
                        ...
                        AHN_SOTHAM_THULY thuLy = dt.AHN_SOTHAM_THULY.Where(x => x.DONID == objDonMoi.ID).FirstOrDefault(); //KHAI- AHN
                        int ttTL = 0;
                        string ngayTL = "", soTL = "";
                        if (thuLy != null)
                        {
        ...
        protected void chkChon_CheckedChanged(object sender, EventArgs e)
        {
            foreach (DataGridItem item in dgList.Items)
            {
                ...
                if (chonVuTach.Checked)
                {
                    ...
                    {
                        AHN_DON ahndon = dt.AHN_DON.Where(x => x.ID == idDCT).FirstOrDefault();
                        if (ahndon.LOAIDON == 1)
                        {
                            chonDaiDien.Visible = true;
                        }
