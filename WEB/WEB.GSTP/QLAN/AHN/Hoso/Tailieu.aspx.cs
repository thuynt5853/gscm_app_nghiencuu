...
namespace WEB.GSTP.QLAN.AHN.Hoso
{
    public partial class Tailieu : System.Web.UI.Page
    {
        ...
        private void LoadDropNguoiBanGiao()
        {
            ...
            if (rdbTuCachToTung.SelectedValue == "0") // Người khởi kiện
            {
                List<AHN_DON_DUONGSU> lst = dt.AHN_DON_DUONGSU.Where(x => x.DONID == DonID).ToList();
            ...
            else if (rdbTuCachToTung.SelectedValue == "1") // Người được ủy quyền nguyên đơn
            {
                List<AHN_DON_THAMGIATOTUNG> lst = dt.AHN_DON_THAMGIATOTUNG.Where(x => x.DONID == DonID).ToList();
                if (lst != null && lst.Count > 0)
                {
                    ddlNguoiBanGiao.DataSource = lst;
        ...
        private void LoadDropNguoiNhan()
        {
            ...
            ddlNguoiNhan.Items.Add(new ListItem("--- Chọn ---", "0"));
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                AHN_DON_TAILIEU donFile = new AHN_DON_TAILIEU();
                decimal TLID = (String.IsNullOrEmpty(hddID.Value)) ? 0 : Convert.ToDecimal(hddID.Value);
                if (TLID > 0)
                    donFile = dt.AHN_DON_TAILIEU.Where(x => x.ID == TLID).FirstOrDefault<AHN_DON_TAILIEU>();
                else
                    donFile = new AHN_DON_TAILIEU();
                try
                {
                ...
                if (hddID.Value == "" || hddID.Value == "0")
                {
                    ...
                    dt.AHN_DON_TAILIEU.Add(donFile);
                }
                else
                {
        ...
        {
            lbthongbao.Text = "";
            decimal DonViID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HONNHAN_GIADINH] + "");
            int Total = 0;
            AHN_DON_BL donBL = new AHN_DON_BL();
            DataTable tbl = donBL.AHN_DON_BanGiaoTaiLieu_GETLIST(DonViID);
            if (tbl != null && tbl.Rows.Count > 0)
            {
        ...
        {
            AHN_DON_TAILIEU oT = dt.AHN_DON_TAILIEU.Where(x => x.ID == ID).FirstOrDefault();
            dt.AHN_DON_TAILIEU.Remove(oT);
            dt.SaveChanges();
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                switch (e.CommandName)
                {
                    ...
                    case "Sua":
                        hddFilePath.Value = "";
                        AHN_DON_TAILIEU bgtl = dt.AHN_DON_TAILIEU.Where(x => x.ID == ID).FirstOrDefault();
                        if (bgtl != null)
                        {
                            ...
                            try
                            {
                                ddlNguoiBanGiao.SelectedValue = bgtl.NGUOIBANGIAO + "" == "" ? "0" : bgtl.NGUOIBANGIAO.ToString();
                            }
                            catch { }
                            try
                            {
                        ...
                        break;
                    case "Download":
                        AHN_DON_TAILIEU oND = dt.AHN_DON_TAILIEU.Where(x => x.ID == ID).FirstOrDefault();
                        if (oND.TENFILE != "")
                        {
