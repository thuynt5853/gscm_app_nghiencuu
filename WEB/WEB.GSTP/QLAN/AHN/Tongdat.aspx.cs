...
namespace WEB.GSTP.QLAN.AHN
{
    public partial class Tongdat : System.Web.UI.Page
    {
        ...
        AHN_TONGDAT_BL TONGDAT_BL = new AHN_TONGDAT_BL();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                string current_id = Session[ENUM_LOAIAN.AN_HONNHAN_GIADINH] + "", TOAANID = "0";
                ...
                List<DONKK_USER_DKNHANVB> lstDKVB = dkk.DONKK_USER_DKNHANVB.Where(x => x.VUVIECID == ID && x.MALOAIVUVIEC == ENUM_LOAIAN.AN_HONNHAN_GIADINH && x.TRANGTHAI == 1).ToList();
                if (lstDKVB.Count == 0)
        ...
        private void LoadCombobox()
        {
            ...
            //Decimal VuAnId = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HONNHAN_GIADINH]);
            //DataTable tbl = oBL.AHN_FILE_TONGDAT(ID);
            //AHN_SOTHAM_BANAN oND = dt.AHN_SOTHAM_BANAN.Where(x => x.DONID == VuAnId && x.TOAANID == TOAANID).FirstOrDefault();
            //DM_BIEUMAU bm52 = dt.DM_BIEUMAU.Where(bm => bm.ID == 230).FirstOrDefault();
            //AHN_FILE af = dt.AHN_FILE.Where(x => x.BIEUMAUID == 230 && x.DONID == VuAnId).FirstOrDefault();
            //AHN_TONGDAT td = dt.AHN_TONGDAT.Where(x => x.DONID == VuAnId && x.BIEUMAUID == 230).FirstOrDefault();
            //if (oND != null && td == null)
            //{
            ...
            //    tbl.Rows.Add(dr);
            //}
            //AHN_PHUCTHAM_BANAN oND2 = dt.AHN_PHUCTHAM_BANAN.Where(x => x.DONID == VuAnId && x.TOAANID == TOAANID).FirstOrDefault();
            //DM_BIEUMAU bm75 = dt.DM_BIEUMAU.Where(bm => bm.ID == 261).FirstOrDefault();
            //af = dt.AHN_FILE.Where(x => x.BIEUMAUID == 261 && x.DONID == VuAnId).FirstOrDefault();
            //td = dt.AHN_TONGDAT.Where(x => x.DONID == VuAnId && x.BIEUMAUID == 261).FirstOrDefault();
            ...
            AHN_DON objDon = dt.AHN_DON.Where(x => x.ID == ID).FirstOrDefault();
            decimal giai_doan = Convert.ToDecimal(objDon.MAGIAIDOAN);
            TONGDAT_BL oTongDatBL = new TONGDAT_BL();
            DataTable oDT = oTongDatBL.GET_BM_TONGDAT(ID, 3, giai_doan);
        ...
        private List<AHN_TONGDAT_GS> convertToAHN_TONGDATFromDataTable(DataTable dataTable)
        {
            List<AHN_TONGDAT_GS> aHN_TONGDATs = new List<AHN_TONGDAT_GS>();
            foreach (DataRow row in dataTable.Rows)
            {
                AHN_TONGDAT_GS tONGDAT = new AHN_TONGDAT_GS();
                ...
                aHN_TONGDATs.Add(tONGDAT);
            }
            return aHN_TONGDATs;
        }

        private List<AHN_TONGDAT_NGUOINHAN_GS> convertToAHN_TONGDATDOITUONGFromDataTable(DataTable dataTable)
        {
            List<AHN_TONGDAT_NGUOINHAN_GS> aHN_TONGDATs = new List<AHN_TONGDAT_NGUOINHAN_GS>();
            foreach (DataRow row in dataTable.Rows)
            {
                AHN_TONGDAT_NGUOINHAN_GS tONGDAT = new AHN_TONGDAT_NGUOINHAN_GS();
        ...
        {
            BL.GSTP.AHN.AHN_TONGDAT_BL Bl = new BL.GSTP.AHN.AHN_TONGDAT_BL();
            //lbthongbao.Text = "";
            ...
            if (CheckValid() == true)
            {
                ...
                AHN_TONGDAT adTongDat = null;
                if (ANPHI_ID_DROPDOWN != 0)
                {
                    adTongDat = dt.AHN_TONGDAT.Where(x => x.DONID == DONID && x.BIEUMAUID == bieuMauID && x.MAPID == ANPHI_ID_DROPDOWN).FirstOrDefault();
                }
                else
                {
                    adTongDat = dt.AHN_TONGDAT.Where(x => x.DONID == DONID && x.BIEUMAUID == bieuMauID).FirstOrDefault();
                }

                AHN_TONGDAT_GS oND = null;

                if (TongDatID != 0)
                {
                    oND = convertToAHN_TONGDATFromDataTable(Bl.AHN_TONGDAT_GETBYID(TongDatID)).FirstOrDefault();
                }

                bool isNew = false;
                bool isUpdate = false;
                AHN_TONGDAT td = new AHN_TONGDAT();
                List<AHN_TONGDAT_DOITUONG> td_dt = new List<AHN_TONGDAT_DOITUONG>();
                ...
                else
                {
                    ...
                    td = dt.AHN_TONGDAT.Where(x => x.ID == TongDatID).FirstOrDefault();
                    try
                    {
                        td_dt = dt.AHN_TONGDAT_DOITUONG
                                  .Where(x => x.TONGDATID == td.ID && x.TRANGTHAI == 1)
                                  .ToList();
                    }
                    catch (Exception ex)
                    {
                ...
                {
                    oND.MAP_TABLE = ENUM_MAP_TABLE.AHN_ANPHI;
                ...
                if (isNew)
                {
                    ...
                    //---------27/11/2025---- Đinh Hoàng Sơn vnpt - Thêm biến FILEID để lọc riêng từng văn bản trùng
                    if (ddlBieumau.SelectedValue.Split('_').Length > 2)
                    {
                        decimal FILEID = Convert.ToDecimal(ddlBieumau.SelectedValue.Split('_')[2]);
                        oF = dt.AHN_FILE.Where(x => x.DONID == DONID && x.BIEUMAUID == bieu_mau_id && x.ID == FILEID).FirstOrDefault();
                    }
                    else
                    {
                        oF = dt.AHN_FILE.Where(x => x.DONID == DONID && x.BIEUMAUID == bieu_mau_id).FirstOrDefault();
                    }
                    //---------29/11/2025----  vnpt chỉnh lấy thêm mapid maptable
                    if (oF != null)
                    {
                        oND.FILEID = oF.ID;
                        var donxuly = dt.AHN_DON_XULY.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (donxuly != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.AHN_DON_XULY;
                            oND.MAPID = donxuly.ID;
                        }

                        var donxulydct =
                                        (from dx in dt.AHN_DON_XULY
                        ...
                        var anphi = dt.AHN_ANPHI.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);
                        ...
                        var sothamthuly = dt.AHN_SOTHAM_THULY.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (sothamthuly != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.AHN_SOTHAM_THULY;
                            oND.MAPID = sothamthuly.ID;
                        }

                        var sothamquyetdinh = dt.AHN_SOTHAM_QUYETDINH.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (sothamquyetdinh != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.AHN_SOTHAM_QUYETDINH;
                            oND.MAPID = sothamquyetdinh.ID;
                        }

                        var phucthamthuly = dt.AHN_PHUCTHAM_THULY.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (phucthamthuly != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.AHN_PHUCTHAM_THULY;
                            oND.MAPID = phucthamthuly.ID;
                        }

                        var phucthamquyetdinh = dt.AHN_PHUCTHAM_QUYETDINH.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (phucthamquyetdinh != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.AHN_PHUCTHAM_QUYETDINH;
                            oND.MAPID = phucthamquyetdinh.ID;
                        }

                        var banansotham = dt.AHN_SOTHAM_BANAN.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (banansotham != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.AHN_SOTHAM_BANAN;
                            oND.MAPID = banansotham.ID;
                        }

                        var bananphuctham = dt.AHN_PHUCTHAM_BANAN.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                        if (bananphuctham != null)
                        {
                            oND.MAP_TABLE = ENUM_MAP_TABLE.AHN_PHUCTHAM_BANAN;
                    ...
                    oND.ID = Bl.AHN_TONGDAT_UPIN(oND);
                    ...
                    log.InsertLog("AHN_TONGDAT", oND.ID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Insert", "", CONTENT_JSON, "");
                    Hi_TongdatID.Value = oND.ID.ToString();
                }
                else
                {
                ...
                if (!isNew)
                {
                    ...
                    CONTENT_JSON = JsonConvert.SerializeObject(oND, Formatting.Indented);
                    LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                    log.InsertLog("AHN_TONGDAT", oND.ID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Update", lbthongbao.Text, CONTENT_JSON, CONTENT_JSON_OLD);
                    Hi_TongdatID.Value = oND.ID.ToString();
                }
                ...
                if (trDuongsu.Visible)
                {
                    ...
                    foreach (DataRow row in TONGDAT_BL.AHN_TONGDATDOITUONG_GETBYTONGDATID(TongDatID).Rows)
                    {
                    ...
                    foreach (DataGridItem Item in dgTructiep.Items)
                    {
                        ...
                        decimal? ID;
                        List<AHN_TONGDAT_NGUOINHAN_GS> lstCheck;
                        try
                        {
                            if (Item.Cells[7].Text == "&nbsp;")
                            {
                                ID = null;
                                lstCheck = new List<AHN_TONGDAT_NGUOINHAN_GS>();
                            }
                            else
                            {
                                ID = Convert.ToDecimal(Item.Cells[7].Text);
                                lstCheck = convertToAHN_TONGDATDOITUONGFromDataTable(Bl.AHN_TONGDATDOITUONG_GETBYID(ID.Value));
                            }
                        }
                        catch
                        {
                        ...
                        AHN_TONGDAT_NGUOINHAN_GS oTD = null;
                        if (lstCheck.Count > 0)
                        {
                            oTD = lstCheck[0];
                        ...
                        {
                            oTD = new AHN_TONGDAT_NGUOINHAN_GS();
                        }
                        ...
                        if (chkIsSend.Checked)
                        {
                            ...
                            Bl.AHN_TONGDATDOITUONG_UPIN(oTD);
                            dt.SaveChanges();
                            ...
                            if (lstCheck.Count > 0)
                            {
                                ...
                                LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                                log.InsertLog("AHN_TONGDAT_DOITUONG", TongDatID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Update", lbthongbao.Text, CONTENT_JSON, CONTENT_JSON_OLD);
                            }
                            else
                            {
                                //AnhPN log
                                CONTENT_JSON = JsonConvert.SerializeObject(oTD, Formatting.Indented);
                                LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                                log.InsertLog("AHN_TONGDAT_DOITUONG", oTD.TONGDATID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Insert", lbthongbao.Text, CONTENT_JSON, "");
                            }
                            ...
                            {
                                CONTENT_JSON = JsonConvert.SerializeObject(oTD, Formatting.Indented);
                                obj.DVCQG_THANH_TOAN_UP_TONGDAT(oND.ID, (decimal)DuongsuID, "3");
                    ...
                    if (IDs.Count > 0)
                    {
                        foreach (decimal i in IDs)
                            TONGDAT_BL.AHN_TONGDATDOITUONG_REMOVEBYID(i);
                    }
                }
                if (trTructuyen.Visible)
                {
                    ...
                    foreach (DataGridItem Item in dgTructuyen.Items)
                    {
                        ...
                        List<AHN_TONGDAT_DOITUONG> lstCheck = dt.AHN_TONGDAT_DOITUONG.Where(x => x.TONGDATID == TongdatID && x.DUONGSUID == DuongsuID).ToList();
                        AHN_TONGDAT_DOITUONG oTD = null;
                        if (lstCheck.Count > 0)
                        {
                            oTD = lstCheck[0];
                        }
                        ...
                        if (chkIsSendTT.Checked)
                        {
                            ...
                            {
                                oTD.TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                                dt.AHN_TONGDAT_DOITUONG.Add(oTD);
                            }
                            dt.SaveChanges();
                ...
                //-------check xem đã thống đạt sang DVCQG_THANH_TOAN chưa nếu chưa thì update
                //if (Hi_TongdatID.Value != "")
                //{
                //    DVCQG_THANH_TOAN_BL obj_anphiS = new DVCQG_THANH_TOAN_BL();
                //    obj_anphiS.DVCQG_THANH_TOAN_UP_TONGDAT_RE(Hi_TongdatID.Value,DONID, "3");
                //}
                //-------check xem đã thống đạt sang dịch vụ công chưa
        ...
        {
            AHN_TONGDAT td = dt.AHN_TONGDAT.Where(x => x.DONID == DonID && x.TOAANID == ToaAnID && x.BIEUMAUID == BieuMauID && (MAPID == 0 || x.MAPID == MAPID)).FirstOrDefault();
            List<AHN_TONGDAT_DOITUONG> td_dt = dt.AHN_TONGDAT_DOITUONG.Where(x => x.TONGDATID == td.ID).ToList();
            foreach (var item in td_dt)
            {
                AHN_TONGDAT_NGUOINHAN_GS oTD = new AHN_TONGDAT_NGUOINHAN_GS();
        ...
        private void ReUpdateStatusUpdate(List<AHN_TONGDAT_DOITUONG> td_dt, decimal TongDatId)
        {
            ...
            List<AHN_TONGDAT_DOITUONG> list_detail = dt.AHN_TONGDAT_DOITUONG.Where(x => x.TONGDATID == TongDatId).ToList();
        ...
        {
            AHN_DON_BL oBL = new AHN_DON_BL();
            BL.GSTP.AHN.AHN_TONGDAT_BL TongDatBL = new BL.GSTP.AHN.AHN_TONGDAT_BL();
            string current_id = Session[ENUM_LOAIAN.AN_HONNHAN_GIADINH] + "";
            decimal ID = Convert.ToDecimal(current_id), ToaAnID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);

            DataTable oDT = oBL.AHN_TONGDAT_GETLIST(ID, ToaAnID);
            var lstTongDat = convertToAHN_TONGDATFromDataTable(oDT);
            List<AHN_TONGDAT_NGUOINHAN_GS> lstNguoiNhan = new List<AHN_TONGDAT_NGUOINHAN_GS>();
            //  -- VNPT 06122025
            List<AHN_TONGDAT_GS> lstHaventDoituongtongdat = new List<AHN_TONGDAT_GS>();
            foreach (var tongdat in lstTongDat)
            {
                DataTable doiTuongDataTable = TongDatBL.AHN_TONGDATDOITUONG_GETBYTONGDATID(tongdat.ID);
                if (doiTuongDataTable.Rows.Count == 0)
                {
                    lstHaventDoituongtongdat.Add(tongdat);
                }
                lstNguoiNhan.AddRange(convertToAHN_TONGDATDOITUONGFromDataTable(doiTuongDataTable));
            }
            if (oDT != null && lstNguoiNhan.Count == 0)//08/04/2025 trường hợp đã tống đạt nhưng không tồn tại đối tượng tống đạt
            {
                if (lstNguoiNhan.Count == 0)
                {
                    TONGDAT_BL obj = new TONGDAT_BL();
                    obj.TONGDAT_DELETE_ERROR(ID.ToString(), "3");
                }
            }
        ...
        public void xoa(decimal id)
        {
            ...
            //---------25/04/2025 
            List<AHN_TONGDAT_DOITUONG> lst = dt.AHN_TONGDAT_DOITUONG.Where(x => x.TONGDATID == id && (x.TRANGTHAI == 0 || x.TRANGTHAI == 2 || ((x.TRANGTHAI == 5 || x.TRANGTHAI == 6) && x.NGAYPHATHANH == null))).ToList();
            if (lst.Count > 0)
            {
                dt.AHN_TONGDAT_DOITUONG.RemoveRange(lst);
                dt.SaveChanges();
            }
            //--
            List<AHN_TONGDAT_DOITUONG> lst_obj = dt.AHN_TONGDAT_DOITUONG.Where(x => x.TONGDATID == id).ToList();
            AHN_TONGDAT oND = dt.AHN_TONGDAT.Where(x => x.ID == id).FirstOrDefault();
            if (oND != null)
            {
                if (lst_obj.Count == 0)
                {
                    obj.DVCQG_TT_REMOVE_TONGDAT(id, "3", ref _VALUE);
                    if (_VALUE == 1)
                    {
                        dt.AHN_TONGDAT.Remove(oND);
                        dt.SaveChanges();
                    }
                    else
                    {
        ...
        public void loadSua(decimal ID)
        {
            ...
            if (oBM != null)
            {
                ...
                {
                    AHN_ANPHI aDS_ANPHI = dt.AHN_ANPHI.Where(x => x.ID == MapID).FirstOrDefault();
                    DON_MIENANPHI_BL dON_MIENANPHI = new DON_MIENANPHI_BL();

                    if (aDS_ANPHI != null)
                    {
                        DataTable DON_MIENANPHI = dON_MIENANPHI.GET_DON_MIENANPHI_BY_ANPHI_ID(aDS_ANPHI.ID, 3);
            ...
            {
                nguoiNhans = TONGDATBL.AHN_TONGDATDOITUONG_ANPHI_GETBYTONGDATID(ID);
            }
            else
            {
                nguoiNhans = TONGDATBL.AHN_TONGDATDOITUONG_GETBYTONGDATID(ID);
            }
            ...
            {
                string name = TONGDATBL.AHN_TONGDATDOITUONG_GETTENDUONGSU(Convert.ToDecimal(row["ID"]));
        ...
        public void loadPhatHanhLai(decimal ID, decimal vID)
        {
            ...
            cmdLammoi.Visible = true;
            BL.GSTP.AHN.AHN_TONGDAT_BL TONGDATBL = new BL.GSTP.AHN.AHN_TONGDAT_BL();
            DataTable nguoiNhans = TONGDATBL.AHN_TONGDATDOITUONG_GETBYTONGDATID(ID);
            DataTable nguoiNhan = TONGDATBL.AHN_TONGDATDOITUONG_GETBYID(vID);
            ...
            string nguoiNhanName = TONGDATBL.AHN_TONGDATDOITUONG_GETTENDUONGSU(Convert.ToDecimal(nguoiNhanRow["ID"]));
        ...
        //    if (FILEID > 0)
        //    {
        //        //AHN_FILE oF = dt.AHN_FILE.Where(x => x.ID == FILEID).FirstOrDefault();
        //        //if (oF != null) {
        //        //    if (oF.TENFILE !=null)
        //        //        lbtDownload.Visible = true;
        //        //    else
        ...
        //    //List<AHN_TONGDAT_DOITUONG> lst = dt.AHN_TONGDAT_DOITUONG.Where(x => x.TONGDATID == ID).ToList();
        //    //int vCheck = 0;
        //    //if (lst.Count > 0)
        //    //{
        ...
        protected void loadAction(string nDID)
        {
            ...
            //    //Call package
            //    BL.GSTP.AHN.AHN_TONGDAT_BL tongDatBL = new BL.GSTP.AHN.AHN_TONGDAT_BL();
            //    tongDatBL.AHN_TONGDAT_THUHOI_DOITUONG(ID, TongdatID, LyDoThuHoi, NgayThuHoi, NguoiSua);
            ...
            if (TrangThai == 2) // Xem lý do thu hồi
            {
                ...
                DataTable objLyDo = oBL.GET_LYDO_THUHOI(3, TongdatID);
                if (objLyDo != null)
                {
            ...
            {
                BL.GSTP.AHN.AHN_TONGDAT_BL tongDatBL = new BL.GSTP.AHN.AHN_TONGDAT_BL();
                tongDatBL.AHN_TONGDAT_VBDH_DOITUONG(ID);
                LoadGrid();
            }
            else if (TrangThai == 5)
            {
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "Download":
                    ...
                    AHN_TONGDAT ads = dt.AHN_TONGDAT.Where(x => x.ID == tongDatId).FirstOrDefault();
                    byte[] conten = null;
                    if (ads != null)
                    {
                        ...
                        {
                            AHN_FILE oF = dt.AHN_FILE.Where(x => x.ID == ads.FILEID).FirstOrDefault();
                            if (oF != null)
                            {
                                if (oF.TENFILE != null)
                                {
                ...
                case "UpdateNgayNhan":
                    ...
                    // AHN_TONGDAT_DOITUONG oT = dt.AHN_TONGDAT_DOITUONG.Where(x => x.ID == Id_obj).FirstOrDefault();
                    //if(oT.NGAYPHATHANH < INPUT_NGAYNHANTONGDAT) 25/04/2025 nhóm triển khai chốt là không cần check phải có ngày tống đạt rồi mới được nhập ngày nhận tống đạt
                    ...
                    if (txtNGAYNHANTONGDAT.Text != "")
                    {
                        ...
                        if (obj.TONGDAT_DOITUONG_VNID(Id_obj, "3", txtNGAYNHANTONGDAT.Text) == true)
                        {
                    ...
                    else if (txtNGAYNHANTONGDAT.Text == "")
                    {
                        ...
                        if (obj.TONGDAT_DOITUONG_VNID(Id_obj, "3", null) == true)
                        {
                            strMsg = "Bạn đã cập nhật thành công";
                            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('" + strMsg + "')", true);
                            LoadGrid();
        ...
        private void LoadDoituong(bool IsShowVKS, decimal BieuMauID)
        {
            ...
            if (IS_TD_DUONGSU == 1)
            {
                if (oBM.MABM == "100-DS")
                {
                    oDTDS = TONGDAT_BL.AHN_TONGDATDOITUONG_ANPHI_GETBY(DONID, TOAANID, BMID, 0, ANPHI_ID);
                    trDuongsu.Visible = true;
                }
                else
                {
                    //---------21/11/2025----  vnpt chỉnh lấy thêm FILEID
                    oDTDS = TONGDAT_BL.AHN_TONGDATDOITUONG_GETBYFILEID(DONID, TOAANID, BMID, 0, FILEID);
                }
                trDuongsu.Visible = true;
            }
            else if (IS_TD_NGUYENDON == 1 && IS_TD_DUONGSU != 1)
            {
                if (oBM.MABM == "100-DS")
                {
                    oDTDS = TONGDAT_BL.AHN_TONGDATDOITUONG_ANPHI_GETBY(DONID, TOAANID, BMID, 1, ANPHI_ID);
                    trDuongsu.Visible = true;
                }
                else
                {
                    //---------21/11/2025----  vnpt chỉnh lấy thêm FILEID
                    oDTDS = TONGDAT_BL.AHN_TONGDATDOITUONG_GETBYFILEID(DONID, TOAANID, BMID, 1, FILEID);
                }
        ...
        {
            BL.GSTP.AHN.AHN_TONGDAT_BL aHN = new BL.GSTP.AHN.AHN_TONGDAT_BL();
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                string TenBM = aHN.AHN_TENBM_GETBYTONGDATID(tongdatID);
                Label lbTenBM = (Label)e.Item.FindControl("lbTenBM");
                lbTenBM.Text = TenBM;
                lbNoiNhan.Text = aHN.AHN_TONGDATDOITUONG_GETTENDUONGSU(id);
                if (duongsu_id != null)
                {
                }
                ...
                BL.GSTP.AHN.AHN_TONGDAT_BL Bl = new BL.GSTP.AHN.AHN_TONGDAT_BL();
                AHN_TONGDAT_GS oND = convertToAHN_TONGDATFromDataTable(Bl.AHN_TONGDAT_GETBYID(tongdatID)).FirstOrDefault();
                ...
                int rowspan = aHN.AHN_TONGDATDOITUONG_GETBYTONGDATID(tongdatID).Rows.Count;
        ...
        protected void dgTructiep_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            BL.GSTP.AHN.AHN_TONGDAT_BL _BL = new BL.GSTP.AHN.AHN_TONGDAT_BL();
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                DataRowView rowView = (DataRowView)e.Item.DataItem;
                ...
                string vHINHTHUCGUI = rowView["HINHTHUCGUI"].ToString();
                ...
                if (ID == null) // nếu là hàng theem 
                {
                    ...
                    dt.AHN_DON_DUONGSU.Where(x => x.DONID == DONID && x.TUCACHTOTUNG_MA == ENUM_DANSU_TUCACHTOTUNG.QUYENNVLQ).ToList()
                        .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.TENDUONGSU, "QUYENNVLQ1" + "," + x.ID)));
                    //if (oT.MAGIAIDOAN == 2)
                    //{
                    //    dt.AHN_DON_THAMGIATOTUNG.Where(x => x.DONID == DONID)
                    //       .ToList()
                    //       .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.HOTEN, x.TUCACHTGTTID + "," + x.ID)));
                    //}
                    //else if (oT.MAGIAIDOAN == 3)
                    //{
                    //    dt.AHN_PHUCTHAM_THAMGIATOTUNG.Where(x => x.DONID == DONID)
                    //       .ToList()
                    //       .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.HOTEN, x.TUCACHTGTTID + "," + x.ID)));
                    //}
                    ...
                    var atddtisSENT = dt.AHN_TONGDAT_DOITUONG.Where(x => x.TONGDATID == tdId).ToList();
                    if (atddtisSENT.Count > 0)
                    {
                        var tddt = atddtisSENT.Select(x => x.DUONGSUID).Where(x => x != null).ToList();
                        dt.AHN_DON_DUONGSU.Where(x => x.DONID == DONID && x.TUCACHTOTUNG_MA != ENUM_DANSU_TUCACHTOTUNG.QUYENNVLQ && !tddt.Contains(x.ID)).ToList()
        ...
        protected void btn_insert_Click(object sender, EventArgs e)
        {
            if (cmdLammoi.Visible == false)
            {
                ...
                decimal bieu_mau_id = Convert.ToDecimal(ddlBieumau.SelectedValue.Split('_')[0]);
                ANPHI_ID = Convert.ToDecimal(ddlBieumau.SelectedValue.Split('_')[1]);
                //AHN_FILE oF = dt.AHN_FILE.Where(x => x.DONID == DONID && x.BIEUMAUID == bieu_mau_id).FirstOrDefault();
                //---------27/11/2025---- Đinh Hoàng Sơn vnpt - Thêm biến FILEID để lọc riêng từng văn bản trùng\
                if (ddlBieumau.SelectedValue.Split('_').Length > 2)
                {
                    FILEID = Convert.ToDecimal(ddlBieumau.SelectedValue.Split('_')[2]);
            ...
            else
            {
                ...
                if (oBM.MABM == "100-DS") // 100-DS. Thông báo về việc miễn tạm ứng án phí  
                {
                    nguoiNhans = TONGDATBL.AHN_TONGDATDOITUONG_ANPHI_GETBYTONGDATID(TongDatID);
                }
                else
                {
                    nguoiNhans = TONGDATBL.AHN_TONGDATDOITUONG_GETBYTONGDATID(TongDatID);
                }
                nguoiNhans.Columns.Add("TUCACHTOTUNG_MA");
                nguoiNhans.Columns.Add("TONGDAT_DOITUONG");
                nguoiNhans.Columns.Add("TENDUONGSU");
        ...
        protected void ddlNoiNhan_SelectedIndexChanged(object sender, EventArgs e)
        {
            foreach (DataGridItem item in dgTructiep.Items)
            {
                ...
                {
                    //lNoiNhan.Items[0].Attributes["disabled"] = "disabled";
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        // AHN_DON_THAMGIATOTUNG tgtt = dt.AHN_DON_THAMGIATOTUNG.FirstOrDefault(x => x.ID == nguoiID);
                        AHN_DON_DUONGSU tgtt = dt.AHN_DON_DUONGSU.FirstOrDefault(x => x.ID == nguoiID);
                        if (tgtt != null)
                        {
                            string diaChi = "";
        ...
        {
            BL.GSTP.AHN.AHN_TONGDAT_BL tONGDAT_BL = new BL.GSTP.AHN.AHN_TONGDAT_BL();
            ...
            try
            {
                ...
                // Call API
                string result = CallApiThuhoi("3", tongDatIds, LyDoThuHoi, NgayThuHoi, NguoiSua);
                JToken jObject = JToken.Parse(result);
                if ((string)jObject["status"] == "SUCCESS")
                {
                    lbThongBaoThuHoi.Text = "Thu hồi thành công !";
                    ...
                    foreach (DataGridItem item in dgList.Items)
                    {
                        ...
                        if (cb_thuhoi.Checked == true)
                        {
                            //AnhPN log
                            LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                            var data = DataExtensions.FindById<AHN_TONGDAT>(v_id);
                            var CONTENT_JSON_OLD = JsonConvert.SerializeObject(data, Formatting.Indented);
                            log.InsertLog("AHN_TONGDAT", v_id.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Thu hồi", LyDoThuHoi, "", CONTENT_JSON_OLD);

                            //AnhPN log
                            List<AHN_TONGDAT_DOITUONG> lst = dt.AHN_TONGDAT_DOITUONG.Where(x => x.TONGDATID == v_id).ToList();
                            CONTENT_JSON_OLD = JsonConvert.SerializeObject(lst.FirstOrDefault(), Formatting.Indented);
                            log.InsertLog("AHN_TONGDAT_DOITUONG", v_id.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Thu hồi", LyDoThuHoi, "", CONTENT_JSON_OLD);

                            tONGDAT_BL.AHN_TONGDAT_THUHOI(v_id, NgayThuHoi, LyDoThuHoi, NguoiSua);
                        }
                    }
                }
                else
        ...
        {
            List<AHN_TONGDAT> ahnTongDats = dt.AHN_TONGDAT.Where(x => ids.Contains(x.ID) && (x.TRANGTHAI >= 0 && x.TRANGTHAI <= 2)).ToList();
            HashSet<decimal?> idBieuMaus = new HashSet<decimal?>(ahnTongDats.Select(x => x.BIEUMAUID).ToList());
            ...
            HashSet<decimal?> idAHN_DON = new HashSet<decimal?>(ahnTongDats.Select(x => x.DONID).ToList());
            List<AHN_DON> allAHNDon = dt.AHN_DON.Where(x => idAHN_DON.Contains(x.ID)).ToList();
            Dictionary<decimal?, AHN_DON> mapAhnDon = new Dictionary<decimal?, AHN_DON>();
            Dictionary<decimal?, List<AHN_DON_DUONGSU>> mapDuongSu = dt.AHN_DON_DUONGSU
                .Where(x => idAHN_DON.Contains(x.DONID))
                .GroupBy(x => x.DONID, x => x)
                .ToDictionary(x => x.Key, x => x.ToList());
            // bo xung thong tin van ban di
            foreach (AHN_DON ahnDon in allAHNDon)
            {
                mapAhnDon.Add(ahnDon.ID, ahnDon);
            }
            ...
            HashSet<string> usernames = new HashSet<string>(ahnTongDats.Where(x => x.NGUOITAO != null).ToList().Select(x => x.NGUOITAO).ToList());
            ...
            string conditions = "(";
            foreach (AHN_TONGDAT tongDat in ahnTongDats)
            {
            ...
            List<TongDatDTO> danhSachTongDat = ahnTongDats.Select(x => ToDTO(x, mapBieuMau, mapUser, mapAhnDon, mapDuongSu)).ToList();
            ...
            try
            {
                ...
                foreach (TongDatDTO dto in danhSachTongDat)
                {
                    List<AHN_TONGDAT_DOITUONG> subList = allDoiTuongs.Where(x => dto.ID.Equals(x.TONGDATID) && x.TRANGTHAI == 1).ToList();
        ...
        public List<AHN_TONGDAT> GetAnHonNhanGiaDinhThuHoi(List<decimal> ids)
        {
            List<AHN_TONGDAT> ahnTongDats = dt.AHN_TONGDAT.Where(x => ids.Contains(x.ID)).ToList();
            return ahnTongDats;
        }

        private TongDatDTO ToDTO<T>(T model, Dictionary<decimal?, DM_BIEUMAU> mapBieuMau, Dictionary<string, decimal?> mapUser, Dictionary<decimal?, AHN_DON> mapAHN_DON, Dictionary<decimal?, List<AHN_DON_DUONGSU>> mapDuongSu)
        {
            if (model == null)
                return null;
            ...
            if (model.GetType() == typeof(AHN_TONGDAT))
            {
                ...
                dto.Mavuviec = dto.DonId == null ? null : mapAHN_DON[dto.DonId].MAVUVIEC;
                dto.Tenvuviec = dto.DonId == null ? null : mapAHN_DON[dto.DonId].TENVUVIEC;
                dto.Magiaidoan = dto.DonId == null ? null : mapAHN_DON[dto.DonId].MAGIAIDOAN;
                dto.qhpl = dto.DonId == null ? null : mapAHN_DON[dto.DonId].QUANHEPHAPLUAT_NAME;
                //dto.mapID = dto.mapID;
                ...
                {
                    List<AHN_DON_XULY> aHN_DON_XULies = dt.AHN_DON_XULY.Where(x => x.DONID == entity.DONID).ToList();
                    if (aHN_DON_XULies != null && aHN_DON_XULies.Any())
                    {
                        dto.Sothongbao = Convert.ToString(aHN_DON_XULies[0].SOTHONGBAO);
                        dto.Ngaythongbao = Convert.ToDateTime(aHN_DON_XULies[0].NGAYTHONGBAO);
                    }
                ...
                {
                    AHN_ANPHI aDS_ANPHI = dt.AHN_ANPHI.Where(x => x.ID == entity.MAPID).FirstOrDefault();
                    DON_MIENANPHI_BL dON_MIENANPHI = new DON_MIENANPHI_BL();
                    ...
                    if (aDS_ANPHI != null)
                    {
                        ...
                        //dto.mathongbao = dVCQG_THANH_TOAN.DVCQG_THANH_TOAN_SEARCH(2, entity.DONID.Value, aDS_ANPHI.ID, "3");
                    }

                }
                else if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "30-DS")
                {
                    List<AHN_SOTHAM_THULY> aHN_DON_XULies = dt.AHN_SOTHAM_THULY.Where(x => x.DONID == entity.DONID).ToList();
                    if (aHN_DON_XULies != null && aHN_DON_XULies.Any())
                    {
                        dto.Sothongbao = Convert.ToString(aHN_DON_XULies[0].SOTHONGBAO);
                        dto.Ngaythongbao = Convert.ToDateTime(aHN_DON_XULies[0].NGAYTHONGBAO);
                ...
                {
                    List<AHN_PHUCTHAM_THULY> aHN_DON_XULies = dt.AHN_PHUCTHAM_THULY.Where(x => x.DONID == entity.DONID).ToList();
                    if (aHN_DON_XULies != null && aHN_DON_XULies.Any())
                    {
                        dto.Sothongbao = Convert.ToString(aHN_DON_XULies[0].SOTHONGBAO);
                        dto.Ngaythongbao = Convert.ToDateTime(aHN_DON_XULies[0].NGAYTHONGBAO);
                        dto.Nguoiky = aHN_DON_XULies[0].NGUOIKYID;
                    }
                } else if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "52-DS") // 52-DS. Bản án dân sự sơ thẩm vnpt Lê Bá Thọ 28/11/2025 lấy số cho vbdh
                {
                    AHN_SOTHAM_BANAN ba = dt.AHN_SOTHAM_BANAN.Where(x => x.DONID == entity.DONID).FirstOrDefault();
                    if (ba != null)
                    {
                        ...
                        AHN_SOTHAM_HDXX oHD = dt.AHN_SOTHAM_HDXX.Where(x => x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN && x.DONID == entity.DONID).OrderByDescending(x => x.NGAYPHANCONG).FirstOrDefault();
                        if (oHD != null)
                        {
                ...
                {
                    AHN_PHUCTHAM_BANAN ba = dt.AHN_PHUCTHAM_BANAN.Where(x => x.DONID == entity.DONID).FirstOrDefault();
                    if (ba != null)
                    {
                        dto.Sothongbao = Convert.ToString(ba.SOBANAN);
                        dto.Ngaythongbao = Convert.ToDateTime(ba.NGAYTUYENAN);
                        //load người kí - mặc định chủ tọa
                        AHN_PHUCTHAM_HDXX oHD = dt.AHN_PHUCTHAM_HDXX.Where(x => x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN && x.DONID == entity.DONID).OrderByDescending(x => x.NGAYPHANCONG).FirstOrDefault();
                        if (oHD != null)
                        {
                            DM_CANBO oTPCT = dt.DM_CANBO.Where(x => x.ID == oHD.CANBOID).FirstOrDefault();
                            if (oTPCT != null)
                ...
                {
                    List<AHN_FILE> aHN_FILEs = dt.AHN_FILE.Where(x => x.ID == entity.FILEID).ToList();
                    if (aHN_FILEs != null && aHN_FILEs.Any())
                    {
                        decimal? maGiaiDoan = aHN_FILEs[0].MAGIAIDOAN;
                        // so tham
                        if (maGiaiDoan != null && maGiaiDoan == 2)
                        {
                            List<AHN_SOTHAM_QUYETDINH> aHN_SOTHAM_QUYETDINHs = dt.AHN_SOTHAM_QUYETDINH.Where(x => x.QUYETDINHID == dM_BIEUMAU.QUYETDINHID && x.DONID == entity.DONID).ToList();
                            if (aHN_SOTHAM_QUYETDINHs != null && aHN_SOTHAM_QUYETDINHs.Any())
                            {
                                dto.Sothongbao = Convert.ToString(aHN_SOTHAM_QUYETDINHs[0].SOQD);
                                dto.Ngaythongbao = Convert.ToDateTime(aHN_SOTHAM_QUYETDINHs[0].NGAYQD);
                                dto.Nguoiky = Convert.ToDecimal(aHN_SOTHAM_QUYETDINHs[0].NGUOIKYID);
                            }
                        }
                        // phuc tham
                        else if (maGiaiDoan != null && maGiaiDoan == 3)
                        {
                            List<AHN_PHUCTHAM_QUYETDINH> aHN_PHUCTHAM_QUYETDINHs = dt.AHN_PHUCTHAM_QUYETDINH.Where(x => x.QUYETDINHID == dM_BIEUMAU.QUYETDINHID && x.DONID == entity.DONID).ToList();
                            if (aHN_PHUCTHAM_QUYETDINHs != null && aHN_PHUCTHAM_QUYETDINHs.Any())
                            {
                                dto.Sothongbao = Convert.ToString(aHN_PHUCTHAM_QUYETDINHs[0].SOQD);
                                dto.Ngaythongbao = Convert.ToDateTime(aHN_PHUCTHAM_QUYETDINHs[0].NGAYQD);
                                dto.Nguoiky = Convert.ToDecimal(aHN_PHUCTHAM_QUYETDINHs[0].NGUOIKYID);
                            }
                        }
                    }
                }
        ...
        protected string CallApi(List<decimal> ids)
        {
            ...
            var input = GetAnHonNhanGiaDinh(ids);
            ...
            else
            {
                try
                {
                    ...
                    if (status == "SUCCESS")
                    {
                        ...
                        //AnhPN log
                        LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                        log.InsertLog("CallApi_AHN", TongDatID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Gửi sang VBĐH", "Gửi thành công", "", "");
                    }
                    else
                    {
                        ...
                        //AnhPN log
                        LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                        log.InsertLog("CallApi_AHN", TongDatID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Gửi sang VBĐH", "Gửi thất bại", "", "");
                    }
                }
                catch (Exception ex)
                {
        ...
        protected void ddlTCTT_SelectedIndexChanged(object sender, EventArgs e)
        {
            foreach (DataGridItem item in dgTructiep.Items)
            {
                ...
                if (oT.MAGIAIDOAN == 2)
                {
                    ...
                    {
                        dt.AHN_DON_THAMGIATOTUNG.Where(x => x.DONID == DONID && x.TUCACHTGTTID == ddlTCTT.SelectedValue)
                                                .ToList()
                                                .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.HOTEN, x.TUCACHTGTTID + "," + x.ID)));
                    }
                }
                else if (oT.MAGIAIDOAN == 3)
                {
                    ...
                    {
                        dt.AHN_PHUCTHAM_THAMGIATOTUNG.Where(x => x.DONID == DONID && x.TUCACHTGTTID == ddlTCTT.SelectedValue)
                                                    .ToList()
                                                    .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.HOTEN, x.TUCACHTGTTID + "," + x.ID)));
                    }
                }
        ...
        private string GetSoThongBao(decimal bieuMauID, decimal donID, decimal duongsuID)
        {
            ...
            {
                AHN_ANPHI anPhi = dt.AHN_ANPHI.Where(x => x.DONID == donID && x.DUONGSU_ID == duongsuID).FirstOrDefault();
                if (anPhi != null)
                {
            ...
            else if (bieu_mau.MABM == "100-DS") // 100-DS. Thông báo về việc miễn tạm ứng án phí 
            {
                ...
                DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(anPhi.ID, 3);
                if (anPhi != null)
                {
            ...
            {
                AHN_SOTHAM_THULY thuly = dt.AHN_SOTHAM_THULY.Where(x => x.DONID == donID).FirstOrDefault();
                if (thuly != null)
                {
            ...
            else if (bieu_mau.QUYETDINHID != null) // Các biểu mẫu quyết định vụ việc
            {
                ...
                {
                    AHN_SOTHAM_QUYETDINH qdinh = dt.AHN_SOTHAM_QUYETDINH.Where(x => x.DONID == donID).FirstOrDefault();
                    if (qdinh != null)
                    {
                        STB = qdinh.SOQD;
                    }
                ...
                {
                    AHN_PHUCTHAM_QUYETDINH qdinh = dt.AHN_PHUCTHAM_QUYETDINH.Where(x => x.DONID == donID).FirstOrDefault();
                    if (qdinh != null)
                    {
                        STB = qdinh.SOQD;
                    }
