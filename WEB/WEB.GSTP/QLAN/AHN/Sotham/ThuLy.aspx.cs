...
namespace WEB.GSTP.QLAN.AHN.Sotham
{
    public partial class ThuLy : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    DataTable oDT = oBL.AHN_SOTHAM_THULY_GETLIST(ID);
        ...
        private void LoadNguyenDon(decimal donID)
        {
            ...
            {
                AHN_ANPHI ap = dt.AHN_ANPHI.Where(x => x.DONID == donID && (x.TINHTRANG == 1 || x.SOBIENLAI != null)).FirstOrDefault();
                if (ap != null)
                {
                    List<AHN_DON_DUONGSU> tb = dt.AHN_DON_DUONGSU.Where(x => x.DONID == donID && x.TUCACHTOTUNG_MA == "NGUYENDON").OrderByDescending(x => x.ISDAIDIEN).ToList();
                    if (tb.Count > 0)
                    {
                        ddlNguyenDon.DataSource = tb;
                        ddlNguyenDon.DataTextField = "TENDUONGSU";
        ...
        private void LoadBiDon(decimal donID)
        {
            ...
            //Load bị đơn 
            List<AHN_DON_DUONGSU> tb = dt.AHN_DON_DUONGSU.Where(x => x.DONID == donID && x.TUCACHTOTUNG_MA == "BIDON").OrderByDescending(x => x.ISDAIDIEN).ToList();
            if (tb.Count > 0)
            {
        ...
        private void CheckShowCommand(decimal DonID)
        {
            ...
            AHN_SOTHAM_QUYETDINH qd = dt.AHN_SOTHAM_QUYETDINH.Where(x => x.DONID == DonID && (x.LOAIQDID == 3 || x.LOAIQDID == 10 || x.LOAIQDID == 11 | x.LOAIQDID == 23 || x.LOAIQDID == 21)).FirstOrDefault();
            if (qd != null)
            {
        ...
        private decimal SetNewSoThuLy()
        {
            ...
            string sothulymoi = oSTBL.GET_STL_NEW(DonViID, "AHN", ngaythuly).ToString();
        ...
        void SetNewSoThongbao()
        {
            ...
            txtSothongbao.Text = oSTBL.GET_STBTL_NEW(DonViID, "AHN", ngaythongbao).ToString();
        ...
        private void ResetControls()
        {
            ...
            LoadNguyenDon(Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HONNHAN_GIADINH] + ""));
            LoadBiDon(Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HONNHAN_GIADINH] + ""));
            decimal donID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HONNHAN_GIADINH] + "");
            AHN_SOTHAM_THULY tl = dt.AHN_SOTHAM_THULY.Where(x => x.DONID == donID).FirstOrDefault();
            if (tl != null)
            {
        ...
        private bool CheckValid()
        {
            ...
            decimal DONID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HONNHAN_GIADINH]);
            List<AHN_DON_XULY> lstGQD = dt.AHN_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 5).ToList();
            if (lstGQD.Count > 0)
            {
                AHN_DON_XULY oDXL = lstGQD[0];
            ...
            //    Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
            //    ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
            //    Decimal CheckID = oSTBL.CheckSoTLTheoLoaiAn(DonViID, "AHN", sothuly, ngaythuly);
            //    if (CheckID > 0)
            //    {
            //        Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
            //        String strMsg = "";
            //        String STTNew = oSTBL.GET_STL_NEW(DonViID, "AHN", ngaythuly).ToString();
            ...
            if (!String.IsNullOrEmpty(txtNgaythuly.Text))
            {
                ...
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal CheckID = oSTBL.CheckSoTBTLTheoLoaiAn(DonViID, "AHN", sothongbao, ngaythuly);
                if (CheckID > 0)
                {
                    Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
                    String strMsg = "";
                    String STTNew = oSTBL.GET_STBTL_NEW(DonViID, "AHN", ngaythuly).ToString();
                    if (CheckID != CurrThuLyID)
                    {
                        //lbthongbao.Text = "Số thụ lý này đã có!";
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                decimal FileID = 0;
                AHN_SOTHAM_THULY oND;
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    oND = new AHN_SOTHAM_THULY();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.AHN_SOTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
                ...
                AHN_SOTHAM_BL oBL = new AHN_SOTHAM_BL();
                DataTable oDT = oBL.AHN_SOTHAM_THULY_GETLIST(DONID);

                STPT_QUANLY_SOTHULY oQLSTL = new STPT_QUANLY_SOTHULY();
                if (oQLSTL.update_STPT_QUANLY_SOTHULY(2, 3, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), ((DateTime)oND.NGAYTHULY).ToString("dd/MM/yyyy", cul), Regex.Match(oND.SOTHULY, @"\d+").Value) == false)
                {
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    else
                    {
                        ...
                        oND.MATHULY = "S" + ENUM_LOAIVUVIEC.AN_HONNHAN_GIADINH + Session[ENUM_SESSION.SESSION_MADONVI] + oND.TT.ToString();
                        oND.NGAYTAO = DateTime.Now;
                        oND.NGUOITAO = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                        oND.TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                        dt.AHN_SOTHAM_THULY.Add(oND);
                ...
                AHN_DON_DUONGSU ndcu = dt.AHN_DON_DUONGSU.Where(x => x.DONID == DONID && x.ISDAIDIEN == 1 && x.TUCACHTOTUNG_MA == "NGUYENDON").FirstOrDefault();
                AHN_DON_DUONGSU bdcu = dt.AHN_DON_DUONGSU.Where(x => x.DONID == DONID && x.ISDAIDIEN == 1 && x.TUCACHTOTUNG_MA == "BIDON").FirstOrDefault();
                AHN_DON_DUONGSU ndmoi = dt.AHN_DON_DUONGSU.Where(x => x.ID == idNDmoi).FirstOrDefault();
                AHN_DON_DUONGSU bdmoi = dt.AHN_DON_DUONGSU.Where(x => x.ID == idBDmoi).FirstOrDefault();
                if (idNDmoi != ndcu.ID)
                {
                    ndcu.ISDAIDIEN = 0;
                    ndmoi.ISDAIDIEN = 1;
                ...
                oBLs.CHECK_ENABLE_THULY(DONID, "3");
            }
            catch (Exception ex)
            {
        ...
        {
            AHN_SOTHAM_THULY oND = dt.AHN_SOTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "Sua":
                    AHN_SOTHAM_THULY oND1 = dt.AHN_SOTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
                    if (oND1 != null)
                    {
                        // khong duoc xoa khi da Tong dat - 27/11/2025 vnpt check
                        AHN_TONGDAT oTD1 = dt.AHN_TONGDAT.Where(x => x.DONID == oND1.DONID && x.MAPID == oND1.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AHN_SOTHAM_THULY).FirstOrDefault();
                        if (oTD1 != null)
                        {
                ...
                case "Xoa":
                    AHN_SOTHAM_THULY oND2 = dt.AHN_SOTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
                    if (oND2 != null)
                    {
                        // khong duoc xoa khi da Tong dat - 27/11/2025 vnpt check
                        AHN_TONGDAT oTD2 = dt.AHN_TONGDAT.Where(x => x.DONID == oND2.DONID && x.MAPID == oND2.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AHN_SOTHAM_THULY).FirstOrDefault();
                        if (oTD2 != null)
                        {
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                decimal vFILEID = (string.IsNullOrEmpty(rowView["FILEID"] + "")) ? 0 : Convert.ToDecimal(rowView["FILEID"] + "");
                ...
                List<AHN_SOTHAM_THULY> check_button_xoa = dt.AHN_SOTHAM_THULY.Where(x => x.DONID == DONID).ToList();
        ...
        private void txtQuanhephapluat_name(AHN_SOTHAM_THULY oT)
        {
            if (oT.QUANHEPHAPLUAT_NAME != null)
            {
                txtQuanhephapluat.Text = oT.QUANHEPHAPLUAT_NAME;
        ...
        protected void txtNgayThuLy_TextChanged(object sender, EventArgs e)
        {
            ...
            //    STPT_QUANLY_SOTHULY oQLSTL = new STPT_QUANLY_SOTHULY();
            //    DataTable dtStl = oQLSTL.get_STPT_QUANLY_SOTHULY(2, 3, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtNgaythuly.Text);
            //    if (dtStl != null)
            //    {
            ...
            //    string check_ngaythulycuoi_trongnam = oQLSTL.get_LATEST_DATE_IN_SOTHULY(2, 3, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtNgaythuly.Text);
        ...
        protected void btnSaveLydoXoa_Insert(object sender, EventArgs e)
        {
            ...
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            AHN_SOTHAM_THULY oND = dt.AHN_SOTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
            decimal DonID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HONNHAN_GIADINH] + "");
            ...
            if (check_button_xoa.Count < 2 && oND != null)
            {
                ...
                AHN_SOTHAM_BANAN ba = dt.AHN_SOTHAM_BANAN.Where(x => x.DONID == DonID).FirstOrDefault<AHN_SOTHAM_BANAN>();
                if (ba != null)
                {
                ...
                AHN_SOTHAM_QUYETDINH qd = dt.AHN_SOTHAM_QUYETDINH.Where(x => x.DONID == DonID).FirstOrDefault<AHN_SOTHAM_QUYETDINH>();
                if (qd != null)
                {
                ...
                AHN_SOTHAM_HDXX hdxx = dt.AHN_SOTHAM_HDXX.Where(x => x.DONID == DonID).FirstOrDefault<AHN_SOTHAM_HDXX>();
                if (hdxx != null)
                {
                ...
                // Kiểm tra hòa giải
                AHN_SOTHAM_HOAGIAI hg = dt.AHN_SOTHAM_HOAGIAI.Where(x => x.DONID == DonID).FirstOrDefault<AHN_SOTHAM_HOAGIAI>();
                if (hg != null)
                {
                ...
                AHN_DON_THAMPHAN gqst = dt.AHN_DON_THAMPHAN.Where(x => x.DONID == DonID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).FirstOrDefault<AHN_DON_THAMPHAN>();
                if (gqst != null)
                {
            ...
            //Xoa Thụ lý Sơ thẩm 
            dt.AHN_SOTHAM_THULY.Remove(oND);
            dt.SaveChanges();
            if (FileID > 0)
            {
