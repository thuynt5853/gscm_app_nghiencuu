...
namespace WEB.GSTP.QLAN.AHN.AnPhi
{
    public partial class Danhsach : System.Web.UI.Page
    {
        ...
        void CheckQuyen()
        {
            ...
            AHN_DON_XULY donxl = dt.AHN_DON_XULY.Where(x => x.DONID == ID).FirstOrDefault();
            if (donxl != null)
            {
            ...
            List<AHN_DON_XULY> lts_donxl = dt.AHN_DON_XULY.Where(x => x.DONID == ID).ToList();
            ...
            List<AHN_SOTHAM_THULY> lstTL = dt.AHN_SOTHAM_THULY.Where(x => x.DONID == ID).ToList();
            if (lstTL.Count > 0)
            {
        ...
        void LoadDropDuongSu()
        {
            ...
            {
                DataTable obj = oDSBL.AHN_DON_DUONGSU_ANPHI(DONID);
                if (obj != null)
                {
                    foreach (DataRow row in obj.Rows)
                    {
                        string id = row["ID"].ToString();
                        AHN_ANPHI ap = dt.AHN_ANPHI.Where(x => x.DUONGSU_IDS == id && x.DONID == DONID && x.TINHTRANG == 0).FirstOrDefault();
                        if (ap != null)
                        {
                            DON_MIENANPHI_BL dON_MIENANPHI_BL = new DON_MIENANPHI_BL();
                            DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(ap.ID, 3);
                            if (dataTable == null || dataTable.Rows.Count == 0)
                            {
                                ddlDuongSu.Items.Add(new ListItem(row["TENDUONGSU"].ToString(), row["ID"].ToString()));
                            }
                        }
                    }
                }
            ...
            {
                DataTable obj = oDSBL.AHN_DON_DUONGSU_GETLIST(DONID);
                if (obj != null)
                {
                    foreach (DataRow row in obj.Rows)
                    {
                        ...
                        if (ap != null)
                        {
                            ...
                            if (dataTable == null || dataTable.Rows.Count == 0)
                            {
                                ddlDuongSu.Items.Add(new ListItem(row["DUONGSU"].ToString(), row["ID"].ToString()));
                            }
                        }
                    }
                }
                string ids = "";// = obj.Rows[0]["ID"].ToString() + "," + obj.Rows[1]["ID"].ToString();
                foreach (DataRow row in obj.Rows)
                {
                    ids += row["ID"].ToString() + ",";
                }
                AHN_ANPHI ap2 = dt.AHN_ANPHI.Where(x => x.DUONGSU_IDS == ids.Remove(ids.Length - 1) && x.DONID == DONID && x.TINHTRANG == 0).FirstOrDefault();
                if (ap2 != null)
                {
                    DON_MIENANPHI_BL dON_MIENANPHI_BL = new DON_MIENANPHI_BL();
                    DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(ap2.ID, 3);
                    if (dataTable == null || dataTable.Rows.Count == 0)
                    {
                        ddlDuongSu.Items.Add(new ListItem("Cả nguyên đơn và bị đơn", ap2.DUONGSU_IDS));
                    }
                }
        ...
        private void LoadGrid_AnPhi()
        {
            ...
            DataTable oDT = oBL.AHN_ANPHI_GETBYDONID(DONID, 0, pageindex, page_size);
            if (oDT.Rows.Count > 0)
            {
                AHN_ANPHI ap = dt.AHN_ANPHI.Where(x => x.DONID == DONID && x.SOBIENLAI != null && x.TINHTRANG == 0).FirstOrDefault();
                if (ap != null)
                {
        ...
        protected void rpt_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "ENABLE":
                    ...
                    {
                        oBLs.CHECK_TRANGTHAI_THULY(CurrID, "3");
                ...
                case "Sua":
                    DON_MIENANPHI_BL dON_MIENANPHI_BL = new DON_MIENANPHI_BL();
                    DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(CurrID, 3);
                    if (dataTable != null && dataTable.Rows.Count > 0)
                    {
                ...
                case "Xoa":
                    try
                    {
                        ...
                        DataTable dataTableXoa = dON_MIENANPHI_BLs.GET_DON_MIENANPHI_BY_ANPHI_ID(CurrID, 3);
                ...
                case "DownloadAP_THA":
                    ...
                    //--------------------
                    byte[] V_FILE_DATA = oBL.File_Attach_Anphi_Return("3", CurrID, ref V_FILE_NAME);
                    string V_FILE_TYLE = "";
        ...
        protected void ddlDuongSu_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (idDS != "0")
            {
                string current_id = Session[ENUM_LOAIAN.AN_HONNHAN_GIADINH] + "";
                decimal DONID = Convert.ToDecimal(current_id);
                decimal idAP = dt.AHN_ANPHI.Where(x => x.DONID == DONID && x.DUONGSU_IDS == idDS).FirstOrDefault().ID;
