    ...
    public partial class pMienAnPhi : System.Web.UI.Page
    {
        ...
        protected void cmdCapNhatAP_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(APId, 3);
                ...
                try
                {
                    ...
                    if ((dataTable != null && dataTable.Rows.Count > 0 && hddHistoryId.Value != "") || hddHistoryId.Value == "")
                    {
                        ...
                        decimal Mienanphi_ID = dON_MIENANPHI_BL.UPSERT_DON_MIENANPHI(ID_MIENANPHI, APId, DONID, 3, v_lydo, nguoitao, ngaytao, nguoisua, ngaysua, v_sothongbao, d_ngaythongbao, v_stb_phu);
        ...
        private bool CheckValidAP()
        {
            ...
            AHN_ANPHI objAnPhi = dt.AHN_ANPHI.Where(x => x.ID == A_ANPHI_ID).FirstOrDefault();
            ...
            DON_MIENANPHI_BL dON_MIENANPHI_BL = new DON_MIENANPHI_BL();
            DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(A_ANPHI_ID, 3);
            ...
            if (hddHistoryId.Value != "")
            {
                ...
                if (dS_TONGDAT_BL.AHN_TONGDATDOITUONG_ANPHI_GETBYANPHIID_TONGDATID(objAnPhi.DONID, objAnPhi.ID).Rows.Count > 0)
                {
            ...
            switch (e.CommandName)
            {
                ...
                case "XoaAP":
                    try
                    {
                        ...
                        obj.CHECK_DELETE_ANPHI(CurrID, "3", ref _check_out);//anhvh check delete 06/09/2023
                        if (_check_out == 1)
                        {
                            AHN_ANPHI objAP = dt.AHN_ANPHI.Where(x => x.ID == CurrID).FirstOrDefault();
                            AHN_TONGDAT oTD = dt.AHN_TONGDAT.Where(x => x.DONID == objAP.DONID).FirstOrDefault();
                            AHN_TONGDAT_BL dS_TONGDAT_BL = new AHN_TONGDAT_BL();
                            if (dS_TONGDAT_BL.AHN_TONGDATDOITUONG_ANPHI_GETBYANPHIID_TONGDATID(objAP.DONID, objAP.ID).Rows.Count > 0)
                            {
                            ...
                            else
                            {
                                ...
                                LOG_THONGTINANPHI_QLTA log = new LOG_THONGTINANPHI_QLTA();
                                log.InsertLog(Session[ENUM_LOAIAN.AN_HONNHAN_GIADINH].ToString(), CurrID.ToString(), MATHONGBAO, Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Update", CONTENT_JSON, CONTENT_JSON_OLD);
        ...
        protected void rptAP_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                AHN_DON_XULY tralaiDonXuly = dt.AHN_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 3).FirstOrDefault();

                DON_MIENANPHI_BL dON_MIENANPHI_BL1 = new DON_MIENANPHI_BL();
                DataTable dataTable1 = dON_MIENANPHI_BL1.GET_DON_MIENANPHI_BY_ANPHI_ID(vAPID, 3);
                ...
                {
                    AHN_ANPHI objAP = dt.AHN_ANPHI.Where(x => x.ID == vAPID).FirstOrDefault();
        ...
        void LoadInfo_AnPhiDaMien(Decimal CurrID)
        {
            ...
            hddHistoryId.Value = CurrID + "";
            AHN_ANPHI aDSAnphi = dt.AHN_ANPHI.Where(x => x.ID == CurrID).FirstOrDefault();
            if (aDSAnphi != null)
            {
                chkNopAnPhi.Checked = true;
        ...
        private void LoadGrid_AnPhi()
        {
            ...
            decimal A_ANPHI_ID = Convert.ToDecimal(Request.QueryString["ID"].ToString());
            DataTable oDT = oBL.AHN_ANPHI_LICHSU_GETBYDONID(DONID, A_ANPHI_ID, 1, pageindex, page_size);
            //if (oDT.Rows.Count > 0)
            //{
