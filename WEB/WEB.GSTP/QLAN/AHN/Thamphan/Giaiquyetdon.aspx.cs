    ...
    public partial class Giaiquyetdon : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                AHN_CHUYEN_NHAN_AN objCN = dt.AHN_CHUYEN_NHAN_AN.Where(x => x.MAP_VUANID_NEW == ID && x.TRUONGHOPGIAONHANID == 266).FirstOrDefault();
                if (objCN != null)
                {
                    AHN_DON_XULY oldDonXuLy = dt.AHN_DON_XULY.Where(x => x.DONID == objCN.VUANID && x.LOAIGIAIQUYET == 1).FirstOrDefault();
                    if (oldDonXuLy == null)
                    {
        ...
        public void LoadGrid()
        {
            ...
            string current_id = Session[ENUM_LOAIAN.AN_HONNHAN_GIADINH] + "";
            decimal ID = Convert.ToDecimal(current_id);
            DataTable oDT = oBL.AHN_DON_THAMPHAN_GETBY(ID, ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETDON);
        ...
        public void xoa(decimal id)
        {
            ...
            AHN_DON_XULY xld = dt.AHN_DON_XULY.Where(x => x.DONID == oND.DONID).FirstOrDefault();
            ...
            //Luu thong tin Thẩm phán giải quyến đơn trước khi xoa
            ...
            if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.DONID), 3, Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Thẩm phán giải quyết đơn án Hôn nhân", "Xóa", json) == false)
            {
                lbthongbao.Text = "Xóa không thành công!";
                return;
            }//Ket thuc
            //Xoa Thẩm phán giải quyến đơn
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                //if (anKetThuc)
                //{
                //    lbthongbao.Text = "Vụ việc đã kết thúc tại tòa cũ, k    hông thể chỉnh sửa tại tòa mới !";
                //    lbtXoa.Visible = false;
                //    lblSua.Visible = false;
                //    Cls_Comon.SetButton(cmdUpdate, false);
                //    Cls_Comon.SetButton(cmdLammoi, false);
