...
namespace WEB.GSTP.QLAN.AHN
{
    public partial class Sauxetxu : System.Web.UI.Page
    {
        ...
        private void LoadGrid()
        {
            ...
            DataTable oDT = oBL.AHN_RUTKN(vDonViID, txtMaVuViec.Text, txtTenVuViec.Text, txtSoQDBA.Text, txtTenDuongSu.Text, dFrom, dTo, Convert.ToDecimal(rdbTrangthai.SelectedValue), pageIndex, pageSize);
            int Total = 0;
        ...
        protected void cmdCapnhat_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    decimal vuViecID = 0; AHN_SAUXETXU obj; bool isNew = false; AHN_DON don;
                    foreach (DataRow row in dsSelected.Rows)
                    {
                        vuViecID = Convert.ToDecimal(row["ID"].ToString());
                        obj = dt.AHN_SAUXETXU.Where(x => x.VUANID == vuViecID).FirstOrDefault<AHN_SAUXETXU>();
                        if (obj == null)
                        {
                            isNew = true;
                            obj = new AHN_SAUXETXU();
                        ...
                        obj.VUANID = vuViecID;
                        don = dt.AHN_DON.Where(x => x.ID == vuViecID).FirstOrDefault<AHN_DON>();
                        if (don != null)
                        {
        ...
        private void LoadGridSelected()
        {
            ...
            if (dsSelected != null && dsSelected.Rows.Count > 0)
            {
                ...
                AHN_SAUXETXU sx = dt.AHN_SAUXETXU.Where(x => x.VUANID == DonID).FirstOrDefault<AHN_SAUXETXU>();
                if (sx != null)
                {
        ...
        public void loadedit(decimal ID, String MAGIAIDOAN)
        {
            ...
            if (MAGIAIDOAN == "Sơ thẩm")
                oND = dt.AHN_SAUXETXU.Where(x => x.VUANID == ID && x.ST_ISRUTKN == 1).FirstOrDefault();
            else
                oND = dt.AHN_SAUXETXU.Where(x => x.VUANID == ID && x.PT_ISRUTKN == 1).FirstOrDefault();

            if (oND != null)
            {
                if (dsSelected != null) dsSelected.Clear();
                AHN_DON oVA = dt.AHN_DON.Where(x => x.ID == oND.VUANID).FirstOrDefault();
