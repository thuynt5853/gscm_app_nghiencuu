    ...
    public partial class NguoiThamgiaTT : System.Web.UI.Page
    {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string current_id = Session[ENUM_LOAIAN.AN_HONNHAN_GIADINH] + "";
                decimal DONID = Convert.ToDecimal(current_id);
                AHN_PHUCTHAM_THAMGIATOTUNG oND;
                if (hddid.Value == "" || hddid.Value == "0")
                    oND = new AHN_PHUCTHAM_THAMGIATOTUNG();
                else
                {
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.AHN_PHUCTHAM_TGTT_GETLIST(ID);
            ...
            foreach (ListItem oItem in chkListTGTT.Items)
            {
                if (oItem.Selected)
                {
                    ...
                    if (dt.AHN_PHUCTHAM_THAMGIATOTUNG.Where(x => x.NTGTT_DONKK_ID == NID).ToList().Count == 0)
                    {
                        ...
                        AHN_PHUCTHAM_THAMGIATOTUNG oS = new AHN_PHUCTHAM_THAMGIATOTUNG();
                        oS.DONID = oD.DONID;
                        oS.HOTEN = oD.HOTEN;
