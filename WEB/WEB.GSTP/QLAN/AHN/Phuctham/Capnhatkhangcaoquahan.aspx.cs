...
namespace WEB.GSTP.QLAN.AHN.Phuctham
{
    public partial class Capnhatkhangcaoquahan : System.Web.UI.Page
    {
        ...
        {
            AHN_SOTHAM_KHANGCAO oND = dt.AHN_SOTHAM_KHANGCAO.Where(x => x.DONID == id).FirstOrDefault();
            if (oND != null)
            {
                oND.GQ_NGAY = null;
        ...
        private void LoadGrid()
        {
            ...
            DataTable oDT = oBL.AHN_PT_KCQUAHAN(vDonViID, txtMaVuViec.Text, txtTenVuViec.Text, dFrom, dTo, Convert.ToDecimal(rdbTrangthai.SelectedValue), pageIndex, pageSize);
            int Total = 0;
        ...
        protected void cmdGiaiQuyet_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                AHN_DON don = dt.AHN_DON.Where(x => x.ID == ID).FirstOrDefault<AHN_DON>();
                if (don != null)
                {
