...
namespace WEB.GSTP.QLAN.AHN.Phuctham
{
    public partial class Quyetdinhphuctham : System.Web.UI.Page
    {
        ...
        public void LoadGrid()
        {
            ...
            DM_QUYETDINH_VUAN oBL = new DM_QUYETDINH_VUAN();
            DataTable oDT = oBL.DGLIST_QUYETDINH_PT(ENUM_LOAIVUVIEC_NUMBER.AN_HONNHAN_GIADINH, ID);
            if (oDT != null)
            {
        ...
        public void xoa(decimal id)
        {
            ...
            AHN_TONGDAT oTD = dt.AHN_TONGDAT.Where(x => x.DONID == oND.DONID && x.MAPID == oND.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AHN_PHUCTHAM_QUYETDINH).FirstOrDefault();
            if (oTD != null)
            {
            ...
            if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.DONID), 3, Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Quyết định Phúc thẩm án Hôn nhân", "Xóa", json) == false)
            {
            ...
            try
            {
                ...
                switch (e.CommandName)
                {
                    case "Download":
                        ...
                        break;
                    case "Sua":
                        AHN_PHUCTHAM_QUYETDINH oND1 = dt.AHN_PHUCTHAM_QUYETDINH.Where(x => x.ID == ND_id).FirstOrDefault();
                        if (oND1 != null)
                        {
                            // khong duoc xoa khi da Tong dat - 27/11/2025 vnpt check
                            AHN_TONGDAT oTD = dt.AHN_TONGDAT.Where(x => x.DONID == oND1.DONID && x.MAPID == oND1.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AHN_PHUCTHAM_QUYETDINH).FirstOrDefault();
                            if (oTD != null)
                            {
                                lbthongbao.Text = "Bạn không thể sửa khi đã tống đạt!";
                                return;
        ...
        {
            decimal ID = Convert.ToDecimal(hddid.Value);
            AHN_PHUCTHAM_QUYETDINH oQD = dt.AHN_PHUCTHAM_QUYETDINH.Where(x => x.ID == ID).FirstOrDefault();
