...
namespace WEB.GSTP.QLAN.AHN.PhucthamQDK
{
    public partial class NguoiTienhanhTT : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    //    int countItem = dt.AHN_DON_THAMPHAN.Count(s => s.THUKYID == CurrentUserId && s.DONID == DONID && s.MAVAITRO == "VTTP_GIAIQUYETPHUCTHAM");
                    //    if (countItem > 0)
                    //    {
                    //        //được gán
                    //    }
        ...
        private void check_Quyen(decimal DONID)
        {
            ...
            DataTable oDT = oBL.AHN_KCKNQDK_PHUCTHAM_QUYETDINH_GETLIST(DONID);
        ...
        private void LoadCombobox()
        {
            ...
            if (loaiQD != null)
            {
                //AHN_PHUCTHAM_QUYETDINH qd = dt.AHN_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.LOAIQDID == loaiQD.ID).FirstOrDefault<AHN_PHUCTHAM_QUYETDINH>();
                AHN_PHUCTHAM_QUYETDINH qd = DataExtensions.GetAllWithClause<AHN_PHUCTHAM_QUYETDINH>
                        ("DONID = " + DonID + " and LOAIQDID = " + loaiQD.ID).FirstOrDefault<AHN_PHUCTHAM_QUYETDINH>();
                if (qd != null)
                {
                    AHN_DON_THAMPHAN tpgq = dt.AHN_DON_THAMPHAN.Where(x => x.DONID == DonID && x.MAVAITRO == "VTTP_GIAIQUYETPHUCTHAM").OrderByDescending(x => x.NGAYTAO).FirstOrDefault<AHN_DON_THAMPHAN>();
                    if (tpgq != null)
                    {
        ...
        private bool CheckValid()
        {
            ...
            if (tucach == ENUM_CHUCDANH.CHUCDANH_HTND)
            {
                ...
                {
                    lbthongbao.Text = "Chưa chọn Hội thẩm nhân dân !"; Cls_Comon.SetFocus(this, this.GetType(), ddlHTND_Thuky.ClientID);
                    return false;
                }
            }
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                AHN_KCKNQDK_PHUCTHAM_HDXX oND;

                // Lấy count thụ lý và count chủ tọa để cho nhập thêm thẩm phán chủ tọa
                List<AHN_DON_THAMPHAN> countTPGQ = dt.AHN_DON_THAMPHAN.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM && x.TOA_GIAIQUYET_ID == Donvi_id).ToList();
                //List<AHN_PHUCTHAM_HDXX> countChuToa = dt.AHN_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).ToList();
                List<AHN_KCKNQDK_PHUCTHAM_HDXX> countChuToa = DataExtensions.GetAllWithClause<AHN_KCKNQDK_PHUCTHAM_HDXX>($"DONID = {DONID} AND MAVAITRO = '{ENUM_NGUOITIENHANHTOTUNG.THAMPHAN}' AND TOA_GIAIQUYET_ID = {Donvi_id}");
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    if (tucach == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN)
                    {
                        ...
                        //oND = dt.AHN_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).FirstOrDefault<AHN_PHUCTHAM_HDXX>();
                        oND = DataExtensions.GetAllWithClause<AHN_KCKNQDK_PHUCTHAM_HDXX>($"DONID = {DONID} AND MAVAITRO = '{ENUM_NGUOITIENHANHTOTUNG.THAMPHAN}' AND TOA_GIAIQUYET_ID = {Donvi_id}").FirstOrDefault();
                    ...
                    oND = new AHN_KCKNQDK_PHUCTHAM_HDXX();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    //oND = dt.AHN_PHUCTHAM_HDXX.Where(x => x.ID == ID).FirstOrDefault();
                    oND = DataExtensions.GetAllWithClause<AHN_KCKNQDK_PHUCTHAM_HDXX>("ID = " + ID).FirstOrDefault();
                }
                oND.DONID = DONID;
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    //AHN_PHUCTHAM_HDXX CheckCanBo = dt.AHN_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.CANBOID == oND.CANBOID).FirstOrDefault();
                    AHN_KCKNQDK_PHUCTHAM_HDXX CheckCanBo = DataExtensions.GetAllWithClause<AHN_KCKNQDK_PHUCTHAM_HDXX>
                        ("DONID = " + DONID + " and CANBOID = " + oND.CANBOID).FirstOrDefault();
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.AHN_KCKNQDK_PHUCTHAM_HDXX_GETLIST(ID);
            if (oDT != null && oDT.Rows.Count > 0)
            {
        ...
        {
            //AHN_PHUCTHAM_HDXX oND = dt.AHN_PHUCTHAM_HDXX.Where(x => x.ID == id).FirstOrDefault();
            AHN_KCKNQDK_PHUCTHAM_HDXX oND = DataExtensions.FindById<AHN_KCKNQDK_PHUCTHAM_HDXX>(id);
            //dt.AHN_PHUCTHAM_HDXX.Remove(oND);
