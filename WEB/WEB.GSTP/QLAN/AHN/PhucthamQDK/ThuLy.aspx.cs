    ...
    public partial class ThuLy : System.Web.UI.Page
    {
            ...
            try
            {
                if (!IsPostBack)
                {
                    ...
                    else
                    {
                        ...
                        DataTable oDT = oBL.AHN_KCKNQDK_PHUCTHAM_THULY_GETLIST(ID);
        ...
        private void CheckShowCommand(decimal DonID)
        {
            ...
            if (oT != null)
            {
                ...
                DataTable oDT = oBL.AHN_DON_THAMPHAN_GETBY(DonID, ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM);
                if (oDT != null && oDT.Rows.Count > 0)
                {
                    bool exists = false;
        ...
        private void SetNewSoThuLy()
        {
            ...
            txtSoThuly.Text = oSTBL.GET_STL_NEW(DonViID, "AHN_PTQDK", ngaythuly).ToString();
            SetNewSoThongbao();
        }
        ...
        private void SetNewSoThongbao()
        {
            ...
            txtSothongbao.Text = oSTBL.GET_STBTL_NEW(DonViID, "AHN_PTQDK", ngaythongbao).ToString();
        }
        ...
        private void LoadCombobox()
        {
            ...
            List<AHN_CHUYEN_NHAN_AN> listcna = dt.AHN_CHUYEN_NHAN_AN.Where(x => x.MAP_VUANID_NEW == idAn && x.TOANHANID == toaanId).ToList();
        ...
        private void LoadQD()
        {
            string current_id = Session[ENUM_LOAIAN.AN_HONNHAN_GIADINH] + "";
            decimal idAn = Convert.ToDecimal(current_id);
            AHN_CHUYEN_NHAN_AN_BL aHC_CHUYEN_NHAN_AN_BL = new AHN_CHUYEN_NHAN_AN_BL();
            decimal donOld = aHC_CHUYEN_NHAN_AN_BL.getDonIdOld(idAn);

            AHN_SOTHAM_KHANGCAO aHN_SOTHAM_KHANGCAO = dt.AHN_SOTHAM_KHANGCAO.Where(x => x.DONID == donOld && x.LOAIKHANGCAO == 2 && x.TINHTRANG_GIAIQUYET != 1).FirstOrDefault();
            AHN_SOTHAM_KHANGNGHI oKN = dt.AHN_SOTHAM_KHANGNGHI.Where(x => x.DONID == donOld && x.LOAIKN == 2 && x.TINHTRANG_GIAIQUYET != 1).FirstOrDefault();
            if (aHN_SOTHAM_KHANGCAO != null)
            {
                AHN_SOTHAM_QUYETDINH aHN_SOTHAM_QUYETDINH = dt.AHN_SOTHAM_QUYETDINH.Where(x => x.ID == aHN_SOTHAM_KHANGCAO.SOQDBA).FirstOrDefault();
                if (aHN_SOTHAM_QUYETDINH != null)
                {
                    DM_QD_QUYETDINH dM_QD_QUYETDINH = dt.DM_QD_QUYETDINH.Where(x => x.ID == aHN_SOTHAM_QUYETDINH.QUYETDINHID).FirstOrDefault();
                    txtQDST.Text = "Số " + aHN_SOTHAM_QUYETDINH.SOQD + " - " + dM_QD_QUYETDINH.TEN;
                }
            }
            else if (oKN != null)
            {
                AHN_SOTHAM_QUYETDINH aHN_SOTHAM_QUYETDINH = dt.AHN_SOTHAM_QUYETDINH.Where(x => x.ID == oKN.BANANID).FirstOrDefault();
                if (aHN_SOTHAM_QUYETDINH != null)
                {
                    DM_QD_QUYETDINH dM_QD_QUYETDINH = dt.DM_QD_QUYETDINH.Where(x => x.ID == aHN_SOTHAM_QUYETDINH.QUYETDINHID).FirstOrDefault();
                    txtQDST.Text = "Số " + aHN_SOTHAM_QUYETDINH.SOQD + " - " + dM_QD_QUYETDINH.TEN;
        ...
        private bool CheckValid()
        {
            ...
            if (!String.IsNullOrEmpty(txtNgaythuly.Text))
            {
                ...
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal CheckID = oSTBL.CheckSoTLTheoLoaiAn(DonViID, "AHN_PTQDK", sothuly, ngaythuly);
                if (CheckID > 0)
                {
                    Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
                    String strMsg = "";
                    String STTNew = oSTBL.GET_STL_NEW(DonViID, "AHN_PTQDK", ngaythuly).ToString();
            ...
            if (!String.IsNullOrEmpty(txtNgaythuly.Text))
            {
                ...
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal CheckID = oSTBL.CheckSoTBTLTheoLoaiAn(DonViID, "AHN_PTQDK", sothongbao, ngaythuly);
                if (CheckID > 0)
                {
                    Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
                    String strMsg = "";
                    String STTNew = oSTBL.GET_STBTL_NEW(DonViID, "AHN_PTQDK", ngaythuly).ToString();
                    if (CheckID != CurrThuLyID)
                    {
                        //lbthongbao.Text = "Số thụ lý này đã có!";
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                decimal FileID = 0;
                AHN_KCKNQDK_PHUCTHAM_THULY oND;
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    oND = new AHN_KCKNQDK_PHUCTHAM_THULY();
                ...
                else
                {
                    ...
                    oND = DataExtensions.FindById<AHN_KCKNQDK_PHUCTHAM_THULY>(ID);
        ...
        {
            AHN_KCKNQDK_PHUCTHAM_THULY oND = DataExtensions.FindById<AHN_KCKNQDK_PHUCTHAM_THULY>(id); //  AHN_PHUCTHAM_THULY oND = dt.AHN_PHUCTHAM_THULY.Where(x => x.ID == id).FirstOrDefault();
            if (oND != null)
            {
                ...
                if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.DONID), Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_HONNHAN_GIADINH), Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Thụ lý Phúc thẩm án Hôn nhân", "Xóa", json) == false)
                {
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                {
                    case "Download":
                        var oND = DataExtensions.FindById<AHN_KCKNQDK_PHUCTHAM_THULY>(ND_id);
                        if (oND.TENFILE != "")
                        {
