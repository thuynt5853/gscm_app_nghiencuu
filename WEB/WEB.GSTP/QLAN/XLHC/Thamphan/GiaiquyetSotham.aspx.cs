...
namespace WEB.GSTP.QLAN.XLHC.Thamphan
{
    public partial class GiaiquyetSotham : System.Web.UI.Page
    {
        ...
        private void CheckQuyen_Original(decimal DONID)
        {
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            Cls_Comon.SetButton(cmdUpdate, oPer.CAPNHAT);

            List<XLHC_SOTHAM_THULY> lstCount = dt.XLHC_SOTHAM_THULY.Where(x => x.DONID == DONID).ToList();
            if (lstCount.Count == 0)
            {
        ...
        /*
         * Trang này là bước B. Giải quyết hồ sơ - 2. Phân công thẩm phán giải quyết
         */
        private void CheckQuyen(decimal DONID)
        {
            ...
            //Check bước trước đó gần nhất B. Giải quyết hồ sơ - 1. Thụ lý
            //Chưa có thụ lý thì không cho làm gì
            List<XLHC_SOTHAM_THULY> lstCount = dt.XLHC_SOTHAM_THULY.Where(x => x.DONID == DONID).ToList();
            if (lstCount.Count == 0)
            {
                lbthongbao.Text = "Chưa cập nhật thông tin thụ lý sơ thẩm !";
            ...
            //Check xem vụ án đã có giai đoạn nào ở sau chưa
            //Check nếu có bước B3.1(Người tiến hành tố tụng) thì không cho phép sửa bản ghi thông tin thẩm phán (Kể cả bản ghi được thêm mới)
            //CRUD=0123
            //hddActionType.Value=2: Update/Sửa
            //hddActionType.Value=3: Delete/Xoá
            //Button Làm mới vẫn enable để cho phép thêm
            /*if (!String.IsNullOrEmpty(hddid.Value) && ("2" == hddActionType.Value || "3" == hddActionType.Value))
            {
                decimal DON_TP_ID = Convert.ToDecimal(hddid.Value);
                //Kiểm tra đã có Người tiến hành tố tụng chưa
                XLHC_SOTHAM_HDXX oNguoiTHTT;
                oNguoiTHTT = dt.XLHC_SOTHAM_HDXX.Where(x => x.DONID == DONID).FirstOrDefault();
                //Đã có người tiến hành tố tụng
                if (oNguoiTHTT != null)
                {
                    //Đã có người THTT, nếu tìm thấy đơn nghĩa là Update/Delete thì không cho thực hiện
                    XLHC_DON_THAMPHAN oDonThamPham = dt.XLHC_DON_THAMPHAN.Where(x => x.ID == DON_TP_ID && x.DONID == DONID).FirstOrDefault();

                    if (oDonThamPham != null)
                    {
                        lbthongbao.Text = "Vụ việc đã có Thông tin người tiến hành tố tụng, không được sửa đổi thông tin thẩm phán giải quyết!";
                        Cls_Comon.SetButton(cmdUpdate, false);
                        //Cls_Comon.SetButton(cmdLammoi, false);
                        return;
                    }
            ...
            //List<XLHC_SOTHAM_QUYETDINH> qdKetThuc = dt.XLHC_SOTHAM_QUYETDINH.Where(x => x.DONID == DONID && dmQDIds.Contains(x.QUYETDINHID.Value)).ToList();
            //if (qdKetThuc.Count != 0)
            //{
            //    XLHC_SOTHAM_KHANGCAO kc = dt.XLHC_SOTHAM_KHANGCAO.Where(x => x.DONID == DONID).FirstOrDefault();
            //    if (kc != null)
            //    {
            ...
            XLHC_SOTHAM_BANAN banAn = dt.XLHC_SOTHAM_BANAN.Where(x => x.DONID == DONID).FirstOrDefault();
            if (banAn != null)
            {
            ...
            DataTable oDT = oBL.DGLIST_BAQD_QUYETDINH_KETTHUC_ST(ENUM_LOAIVUVIEC_NUMBER.BPXLHC, DONID);
            if (oDT.Rows.Count > 0)
            {
        ...
        //return :
        //0: check OK được quyền action
        //!0: check NOT OK, không được action tiếp
        private int CheckQuyen(decimal DONID, out int retCode, out string retDesc)
        {
            int result = 1; //mặc định check quyền khác 0 là không được phép Action
            retCode = 0;
            retDesc = "";
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            Cls_Comon.SetButton(cmdUpdate, oPer.CAPNHAT);
            ...
            if (!String.IsNullOrEmpty(hddid.Value) && ("2" == hddActionType.Value || "3" == hddActionType.Value))
            {
                ...
                oNguoiTHTT = dt.XLHC_SOTHAM_HDXX.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == DonViID).FirstOrDefault();
                //Đã có người tiến hành tố tụng
                if (oNguoiTHTT != null)
                {
                    //Đã có người THTT, nếu tìm thấy đơn nghĩa là Update/Delete thì không cho thực hiện
                    ...
                    if (oDonThamPham != null)
                    {
                        lbthongbao.Text = "Vụ việc đã có Thông tin người tiến hành tố tụng, không được sửa đổi!";
                        Cls_Comon.SetButton(cmdUpdate, false);
                        //Cls_Comon.SetButton(cmdLammoi, false);
                        return result;
                    }
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                XLHC_DON_THAMPHAN obj = dt.XLHC_DON_THAMPHAN.Where(x => x.ID == ID).FirstOrDefault();
                if (obj != null)
                {

                    XLHC_CHUYEN_NHAN_AN chuyenan = dt.XLHC_CHUYEN_NHAN_AN.Where(x => x.VUANID == DONID).OrderByDescending(x => x.ID).FirstOrDefault();
                    if (chuyenan != null)
                    {
                        if (obj.NGAYTAO <= chuyenan.NGAYTAO)
                        {
                            List<XLHC_SOTHAM_HDXX> NTHTT = dt.XLHC_SOTHAM_HDXX.Where(x => x.DONID == DONID).ToList();
                            List<XLHC_SOTHAM_QUYETDINH> qd = dt.XLHC_SOTHAM_QUYETDINH.Where(x => x.DONID == DONID).ToList();
                        ...
                        {
                            List<XLHC_SOTHAM_HDXX> NTHTT = dt.XLHC_SOTHAM_HDXX.Where(x => x.DONID == DONID && x.NGAYTAO > chuyenan.NGAYTAO).ToList();
                            List<XLHC_SOTHAM_QUYETDINH> qd = dt.XLHC_SOTHAM_QUYETDINH.Where(x => x.DONID == DONID && x.NGAYTAO > chuyenan.NGAYTAO).ToList();
                            if ((NTHTT != null && NTHTT.Count > 0) || (qd != null && qd.Count > 0))
                            {
                                lblSua.Visible = false;
                                lbtXoa.Visible = false;
                    ...
                    {
                        decimal DonviID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                        List<XLHC_SOTHAM_HDXX> NTHTT = dt.XLHC_SOTHAM_HDXX.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == DonviID).ToList();
                        List<XLHC_SOTHAM_QUYETDINH> qd = dt.XLHC_SOTHAM_QUYETDINH.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == DonviID).ToList();
                        if ((NTHTT != null && NTHTT.Count > 0) || (qd != null && qd.Count > 0))
                        {
                            lblSua.Visible = false;
                            lbtXoa.Visible = false;
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.XLHC_DON_THAMPHAN_GETBY(ID, ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM);
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            //Gán ID của record vào hidden id = id trong bảng XLHC_DON_THAMPHAN
            hddid.Value = ND_id + "";
            ...
            switch (e.CommandName)
            {
                ...
                case "Xoa":
                    ...
                    //CRUD | 0123
                    hddActionType.Value = "3";  //Xoá 
                    result = CheckQuyen(DONID, out retCode, out retDesc);
                    if (0 != result)
                    {
                        return;
                    }
        ...
        private bool CheckQDofThamPhan(decimal DonID, decimal TPID)
        {
            ...
            XLHC_SOTHAM_QUYETDINH obj_QD = dt.XLHC_SOTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.NGUOIKYID == TPID).FirstOrDefault<XLHC_SOTHAM_QUYETDINH>();
            if (obj_QD != null)
            {
