...
namespace WEB.GSTP.QLAN.XLHC
{
    public partial class DangVanBanTongDat_CongThongTin : System.Web.UI.Page
    {
        ...
        private const string CAPXX_ST = "1", CAPXX_PT = "2", CAPXX_GDT = "3", CAPXX_TT = "4", LINHVUC_XLHC = "34";
        ...
        private void LoadVuViec()
        {
            ...
            else
            {
                ...
                decimal GiaiDoan = 0;
                XLHC_DON don = dt.XLHC_DON.Where(x => x.ID == DonID).FirstOrDefault();
                if (don != null)
                {
        ...
        private void LoadDanhSach()
        {
            ...
            DataTable tbl = Cls_Comon.GetTableByProcedurePaging("XLHC_TONGDAT_GetVB_CTTDT", prm);
        ...
        protected void BtnGuiCongThongTinDienTu_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string prm_key = "dGFuZHRjL3NlcnZpY2UvZ3NvbG90aW9uL2FucHg=",
                    prm_linhVuc = LINHVUC_XLHC, StrLinhVuc = "xử lý hành chính", BMKhongThanhCong = "";
                ...
                foreach (DataGridItem item in dgListChuaDangTai.Items)
                {
                    ...
                    if (chkIsSend.Checked)
                    {
                        ...
                        XLHC_TONGDAT tdat = dt.XLHC_TONGDAT.Where(x => x.ID == TongDatID).FirstOrDefault();
                        if (tdat != null)
                        {
                            int prm_type = 0;
                            ...
                            if (bm != null)
                            {
                                ...
                                if (bm.GIAIDOAN == ENUM_GIAIDOANVUAN.SOTHAM.ToString())// sơ thẩm
                                {
                                    ...
                                    XLHC_SOTHAM_THULY ThuLyST = dt.XLHC_SOTHAM_THULY.Where(x => x.DONID == DONID).OrderByDescending(x => x.NGAYTHULY).FirstOrDefault();
                                    if (ThuLyST != null)
                                    {
                                ...
                                else if (bm.GIAIDOAN == ENUM_GIAIDOANVUAN.PHUCTHAM.ToString())
                                {
                                    ...
                                    XLHC_PHUCTHAM_THULY ThuLyPT = dt.XLHC_PHUCTHAM_THULY.Where(x => x.DONID == DONID).OrderByDescending(x => x.NGAYTHULY).FirstOrDefault();
                                    if (ThuLyPT != null)
                                    {
                                ...
                                XLHC_DUONGSU NguoiKhoiKien = dt.XLHC_DUONGSU.Where(x => x.DONID == DONID && x.BICANDAUVU == 1).FirstOrDefault();
                                if (NguoiKhoiKien != null)
                                {
                                    if (NguoiKhoiKien.GIOITINH == 1)
                                    {
                                        prm_nguoiKhoiKien = "Ông " + NguoiKhoiKien.HOTEN;
                                    }
                                    else
                                    {
                                        prm_nguoiKhoiKien = "Bà " + NguoiKhoiKien.HOTEN;
                                    }
                                    decimal HuyenID = NguoiKhoiKien.TAMTRU + "" == "" ? 0 : (decimal)NguoiKhoiKien.TAMTRU;
                                ...
                                //XLHC_DON_DUONGSU NguoiBiKien = dt.XLHC_DON_DUONGSU.Where(x => x.DONID == DONID && x.TUCACHTOTUNG_MA == ENUM_DANSU_TUCACHTOTUNG.BIDON).FirstOrDefault();
                                //if (NguoiBiKien != null)
                                //{
