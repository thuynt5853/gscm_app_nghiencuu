...
{
    public partial class BananPhucThamKCKN : System.Web.UI.Page
    {
        ...
        void CheckQuyen()
        {
            ...
            XLHC_KCKNQDK_PHUCTHAM_QUYETDINH qd66DS = DataExtensions.GetAllWithClause<XLHC_KCKNQDK_PHUCTHAM_QUYETDINH>($"DONID = {DONID}").FirstOrDefault();
            if (qd66DS == null)
            {
                lstErr.Text = "Chưa cập nhật thông tin quyết định vụ việc !";
                Cls_Comon.SetButton(cmdUpdate, false);
                return;
            }
            ...
            //List<XLHC_KCKNQDK_PHUCTHAM_QUYETDINH> lstQD = DataExtensions.GetAllWithClause<XLHC_KCKNQDK_PHUCTHAM_QUYETDINH>("DONID = " + DONID + " AND (LOAIQDID IN (10,3) OR QUYETDINHID IN (213,145,146))");
            //if (lstQD.Count >= 1)
            //{
            //    pnQDVV.Visible = true;
            ...
            //List<XLHC_PHUCTHAM_BANAN> lstBA = dt.XLHC_PHUCTHAM_BANAN.Where(x => x.DONID == DONID).ToList();
            //if (lstBA.Count >= 1)
            //{
            //    pnBAPT.Visible = true;
        ...
        private void LoadBanAnInfo(decimal DonID)
        {
            ...
            {
                //XLHC_KCKNQDK_PHUCTHAM_THULY tlpt = DataExtensions.GetAllByDonId<XLHC_KCKNQDK_PHUCTHAM_THULY>(DonID).OrderByDescending(x => x.NGAYTHULY).FirstOrDefault();
                //if (tlpt != null)
                //{
        ...
        private bool CheckValid()
        {
            ...
            {
                DateTime NgayMPH = DateTime.Parse(txtNgaymophientoa.Text, cul, DateTimeStyles.NoCurrentDateDefault);

                if (NgayMPH.Date > DateTime.Now.Date)
                {
            ...
            {
                DateTime NgayQD = DateTime.Parse(txtNgaytuyenan.Text, cul, DateTimeStyles.NoCurrentDateDefault);
        ...
        private void LoadDropKetQuaPhucTham()
        {
            try
            {
                ...
                ddlKetQuaPhucTham.DataSource = dt.DM_KETQUA_PHUCTHAM
                    .Where(x => x.ISXLHC == 1  && listQĐ_muc_3_2.Contains(x.MA))
                    .OrderBy(y => y.THUTU)
                    .ToList();
