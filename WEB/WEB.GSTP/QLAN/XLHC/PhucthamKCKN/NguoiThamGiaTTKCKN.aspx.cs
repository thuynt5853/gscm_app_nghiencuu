...
namespace WEB.GSTP.QLAN.XLHC.PhucthamKCKN
{
    public partial class NguoiThamGiaTTKCKN : System.Web.UI.Page
    {
        ...
        private void LoadTGTTFromDonKK()
        {
            ...
            decimal donIdOld = xLHC_CHUYEN_NHAN_AN_BL.getDonIdOld(DONID);
            chkListTGTT.DataSource = oBL.XLHC_DON_TGTT_GETLIST(donIdOld);
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string current_id = Session[ENUM_LOAIAN.BPXLHC] + "";
                decimal DONID = Convert.ToDecimal(current_id);
                XLHC_KCKNQDK_PHUCTHAM_THAMGIATOTUNG oND;
                if (hddid.Value == "" || hddid.Value == "0")
                    oND = new XLHC_KCKNQDK_PHUCTHAM_THAMGIATOTUNG();
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = DataExtensions.FindById<XLHC_KCKNQDK_PHUCTHAM_THAMGIATOTUNG>(ID);
                }
                oND.DONID = DONID;
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.XLHC_PHUCTHAM_KCKN_TGTT_GETLIST(ID);
        ...
        {
            XLHC_KCKNQDK_PHUCTHAM_THAMGIATOTUNG oND = DataExtensions.FindById<XLHC_KCKNQDK_PHUCTHAM_THAMGIATOTUNG>(id);
            if (oND != null)
            {
            ...
            foreach (ListItem oItem in chkListTGTT.Items)
            {
                if (oItem.Selected)
                {
                    ...
                    if (dt.XLHC_PHUCTHAM_THAMGIATOTUNG.Where(x => x.NTGTT_DONKK_ID == NID).ToList().Count == 0)
                    {
                        ...
                        XLHC_KCKNQDK_PHUCTHAM_THAMGIATOTUNG oS = new XLHC_KCKNQDK_PHUCTHAM_THAMGIATOTUNG();
                        oS.DONID = DONID;
                        oS.HOTEN = oD.HOTEN;
