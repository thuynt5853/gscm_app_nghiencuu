...
namespace WEB.GSTP.QLAN.XLHC
{
    public partial class Sauxetxu : System.Web.UI.Page
    {
        ...
        private void LoadGrid()
        {
            ...
            DataTable oDT = oBL.XLHC_RUTKN(vDonViID, txtMaVuViec.Text, txtTenVuViec.Text, txtSoQDBA.Text, txtTenDuongSu.Text, dFrom, dTo, Convert.ToDecimal(rdbTrangthai.SelectedValue), pageIndex, pageSize);
            int Total = 0;
        ...
        protected void cmdCapnhat_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    decimal vuViecID = 0; XLHC_SAUXETXU obj; bool isNew = false; XLHC_DON don;
                    foreach (DataRow row in dsSelected.Rows)
                    {
                        vuViecID = Convert.ToDecimal(row["ID"].ToString());
                        obj = dt.XLHC_SAUXETXU.Where(x => x.VUANID == vuViecID).FirstOrDefault<XLHC_SAUXETXU>();
                        if (obj == null)
                        {
                            isNew = true;
                            obj = new XLHC_SAUXETXU();
                        ...
                        obj.VUANID = vuViecID;
                        don = dt.XLHC_DON.Where(x => x.ID == vuViecID).FirstOrDefault<XLHC_DON>();
                        if (don != null)
                        {
        ...
        private void LoadGridSelected()
        {
            ...
            if (dsSelected != null && dsSelected.Rows.Count > 0)
            {
                ...
                XLHC_SAUXETXU sx = dt.XLHC_SAUXETXU.Where(x => x.VUANID == DonID).FirstOrDefault<XLHC_SAUXETXU>();
                if (sx != null)
                {
