...
{
    public partial class CapnhatKNquahan : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // ✅ Luôn gọi LoadGrid() để cập nhật lại dữ liệu (kể cả khi phân trang)
                LoadGrid();
            }
        ...
        private void LoadGrid()
        {
            ...
            STPT_KHANGNGHIQUAHAN oBL = new STPT_KHANGNGHIQUAHAN();
            DataTable oDT = oBL.ADS_PT_KCQUAHAN(LOAIAN, vDonViID, txtMaVuViec.Text, txtTenVuViec.Text, txtTimkiem_SoBAQD.Text, txtTimkiem_NgayBAQD.Text, txtTimkiem_NguoiKC.Text, txtTuNgay.Text, txtDenNgay.Text,
        ...
        protected void cmdTimkiem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    lbthongbao.Text = "Bạn phải nhập KN từ ngày theo định dạng (dd/MM/yyyy)!";
                    txtTuNgay.Focus();
                    return;
                }
                if (txtDenNgay.Text != "" && Cls_Comon.IsValidDate(txtDenNgay.Text) == false)
                {
                    lbthongbao.Text = "Bạn phải nhập KN đến ngày theo định dạng (dd/MM/yyyy)!";
                    txtDenNgay.Focus();
                    return;
                }
                if (txtTuNgay.Text != "" && txtDenNgay.Text != "")
                {
                    ...
                    {
                        lbthongbao.Text = "Bạn phải nhập ngày KN từ ngày phải nhỏ hơn đến ngày!";
                        txtDenNgay.Focus();
                        return;
                    }
                }
        ...
        {
            Cls_Comon.SetButton(cmdInQuyetdinh, false);
            try
            {
                decimal SOTHAM_KHANGNGHI_ID = Convert.ToDecimal(hddKhangcaoID.Value);
                XLHC_DON don = dt.XLHC_DON.Where(x => x.ID == DONID).FirstOrDefault<XLHC_DON>();
                XLHC_SOTHAM_KHANGCAO kn = dt.XLHC_SOTHAM_KHANGCAO.Where(x => x.ID == SOTHAM_KHANGNGHI_ID).FirstOrDefault<XLHC_SOTHAM_KHANGCAO>();
                DM_DATAITEM tucachtotung;

                /*                XLHC_DUONGSU duongsu = dt.XLHC_DUONGSU.Where(x => x.ID == kn.DUONGSUID && x.DONID == don.ID).FirstOrDefault<XLHC_DUONGSU>();
                                DM_DATAITEM tucachtotung;
                                if (duongsu == null)
                                {
                                    XLHC_DON_THAMGIATOTUNG tgtt = dt.XLHC_DON_THAMGIATOTUNG.Where(x => x.ID == kn.DUONGSUID && x.DONID == don.ID).FirstOrDefault<XLHC_DON_THAMGIATOTUNG>();
                                    tucachtotung = dt.DM_DATAITEM.Where(x => x.MA == tgtt.TUCACHTGTTID).FirstOrDefault<DM_DATAITEM>();
                                    lblNguoiKN.InnerText = tgtt.HOTEN + " - " + tucachtotung.TEN;
                                }*/
                /*                else
                                {
                                    tucachtotung = dt.DM_DATAITEM.Where(x => x.MA == duongsu.TUCACHTOTUNG_MA).FirstOrDefault<DM_DATAITEM>();
                                    lblNguoiKN.InnerText = duongsu.HOTEN + " - " + tucachtotung.TEN;
                                }*/
                ...
                XLHC_DONXIN_HOAN_MIEN donHoanMien = dt.XLHC_DONXIN_HOAN_MIEN.Where(y => y.DONID == kn.DONID && y.DM_QUYETDINH_ID == kn.SOQDBA).FirstOrDefault<XLHC_DONXIN_HOAN_MIEN>();
                XLHC_SOTHAM_BANAN quyetdinhsotham = dt.XLHC_SOTHAM_BANAN.Where(x => x.DONID == kn.DONID && x.QUYETDINHID == kn.SOQDBA).FirstOrDefault<XLHC_SOTHAM_BANAN>();
                if (donHoanMien != null)
                {
                    lblQd.InnerText = "Số " + donHoanMien.SO_QUYETDINH + " ngày " + (string.IsNullOrEmpty(donHoanMien.NGAYTAO + "") ? "" : ((DateTime)donHoanMien.NGAYTAO).ToString("dd/MM/yyyy", cul));
                }
                else if (quyetdinhsotham != null)
                {
                    lblQd.InnerText = "Số " + quyetdinhsotham.SOBANAN + " ngày " + (string.IsNullOrEmpty(quyetdinhsotham.NGAYTUYENAN + "") ? "" : ((DateTime)quyetdinhsotham.NGAYTUYENAN).ToString("dd/MM/yyyy", cul));
                } else
                {
                    XLHC_SOTHAM_QUYETDINH qd = dt.XLHC_SOTHAM_QUYETDINH.Where(x => x.DONID == kn.DONID && x.QUYETDINHID == kn.SOQDBA).FirstOrDefault<XLHC_SOTHAM_QUYETDINH>();
                    lblQd.InnerText = "Số " + qd.SOQD + " ngày " + (string.IsNullOrEmpty(qd.NGAYQD + "") ? "" : ((DateTime)qd.NGAYQD).ToString("dd/MM/yyyy", cul));
                }
                if (kn.TYPE == 1) // khieu nai
                {
                    XLHC_DUONGSU duongsu = dt.XLHC_DUONGSU.Where(x => x.ID == kn.DUONGSUID && x.DONID == don.ID).FirstOrDefault<XLHC_DUONGSU>();
                    if(duongsu == null)
                    {
                    ...
                    {
                        lblNguoiKN.InnerText = duongsu.HOTEN;
                    }
                }
                else if (kn.TYPE == 2) // kien nghi
                {
                    XLHC_DON donKN = dt.XLHC_DON.Where(x => x.ID == kn.DUONGSUID).FirstOrDefault<XLHC_DON>();
                    lblNguoiKN.InnerText = donKN.CQDN_TEN;
                }
                else if (kn.TYPE == 3) // khang nghi
                {
                    DM_VKS vks = dt.DM_VKS.Where(x => x.ID == kn.DUONGSUID).FirstOrDefault<DM_VKS>();
                    lblNguoiKN.InnerText = vks.TEN;
                }
                lblTenVuAn.InnerText = don.TENVUVIEC;
                lblNgayKhangcao.InnerText = string.IsNullOrEmpty(kn.NGAYKHANGCAO + "") ? "" : ((DateTime)kn.NGAYKHANGCAO).ToString("dd/MM/yyyy", cul);
                lblNoidungKN.InnerText = kn.NOIDUNGKHANGCAO;
            }
            catch (Exception ex)
            {
        ...
        protected void cmdSuggestSoThuly_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    XLHC_SOTHAM_KHANGCAO oKhangcao = dt.XLHC_SOTHAM_KHANGCAO.Where(x => x.ID == SOTHAM_KHANGCAO_ID).FirstOrDefault<XLHC_SOTHAM_KHANGCAO>();
                    ...
                    decimal VLOAIKHANGCAO = Convert.ToDecimal(oKhangcao.TYPE);
        ...
        protected void cmdCapnhatThuly_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    lbthongBaoUpdateThuly.Text = "Đã có kết quả KN quá han! Không được sửa!";
                    return;
                }
                ...
                XLHC_SOTHAM_KHANGCAO oKhangcao = dt.XLHC_SOTHAM_KHANGCAO.Where(x => x.ID == oThuly.KHANGCAOID).FirstOrDefault<XLHC_SOTHAM_KHANGCAO>();
                ...
                STPT_KHANGNGHIQUAHAN oBL = new STPT_KHANGNGHIQUAHAN();
                decimal SOTHAM_KHANGCAO_ID = Convert.ToDecimal(hddKhangcaoID.Value);
                if (oBL.SOTHULY_GETMAXTT_CHECK(CurrDonViID, (decimal)oKhangcao.TYPE, txtSothuly.Text, txtNgaythuly.Text))
                {
        ...
        protected void cmdXoaThuly_Click(object sender, EventArgs e)
        {
            ...
            {
                lbthongBaoUpdateThuly.Text = "Đã có Hội đồng xét xử hoặc Kết quả KN quá hạn, không được xóa!";
                return;
            }
        }
        ...
        protected void cmdCapnhaHDXX_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (hddKetqua.Value != "" && hddKetqua.Value != "0")
                {
                    lbthongBaoUpdateHDXX.Text = "Đã có kết quả KN quá hạn! Không được sửa!";
                    return;
                }
        ...
        private void cmdSua_HDXX_Click(decimal ID)
        {
            try
            {
                ...
                if (oDT.Rows[0]["NGAYPHANCONG"] != DBNull.Value && !string.IsNullOrEmpty(oDT.Rows[0]["NGAYPHANCONG"].ToString()))
                    txtNgayphancong.Text = ((DateTime)oDT.Rows[0]["NGAYPHANCONG"]).ToString("dd/MM/yyyy", cul);

                if (oDT.Rows[0]["NGAYNHANPHANCONG"] != DBNull.Value && !string.IsNullOrEmpty(oDT.Rows[0]["NGAYNHANPHANCONG"].ToString()))
        ...
        protected void cmdXoaHDXX_Click(decimal ID)
        {
            ...
            {
                lbthongBaoUpdateHDXX.Text = "Đã có kết quả KN quá hạn, không được xóa!";
            }
        }
        ...
        protected void cmdCapnhatKetqua_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dgList_HDXX.Items.Count == 0)
                {
                    lbthongBaoUpdateKetqua.Text = "Bạn chưa nhập hội đồng xét xử!";
                    return;
                }
        ...
        protected void cmdCapnhatKetqua_old()
        {
            decimal VuViecID = Convert.ToDecimal(hddVuViecID.Value),
                    KhangNghiID = Convert.ToDecimal(hddKhangcaoID.Value),
                    DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);

            XLHC_SOTHAM_KHANGCAO oT = dt.XLHC_SOTHAM_KHANGCAO.Where(x => x.ID == KhangNghiID).FirstOrDefault();
            if (oT != null)
            {
                //Cập nhật lại thông tin giải quyết kháng nghị quá hạn
                ...
                if (thamphan_1 != null)
                {
                    oT.GQ_THAMPHANID = Convert.ToDecimal(thamphan_1.CANBOID);
/*                    KHANGCAOQUAHAN_HDXX thamphan_2 = DataExtensions.GetAllWithClause<KHANGCAOQUAHAN_HDXX>("THULYID = " + hddThulyid.Value + "AND MAVAITRO LIKE 'THAMPHANHDXX' AND CANBOID != " + thamphan_1.CANBOID + " ORDER BY NGAYNHANPHANCONG").FirstOrDefault();
            ...
            {
                lbthongbao.Text = "Không tìm thấy KN cần giải quyết!";
            }
        ...
        protected void cmdXoaKetqua_old()
        {
            ...
            XLHC_SOTHAM_KHANGCAO oND = dt.XLHC_SOTHAM_KHANGCAO.Where(x => x.ID == KHANGCAOID).FirstOrDefault();
            if (oND != null)
            {
                oND.GQ_NGAY = null;
        ...
        protected void cmdInDanhsach_Click(DataTable tbl)
        {
            ...
            string saveAs = TemplateWordSTPT + "rptDanhsachKNquahan.xlsx";
            string fileNameSave = "rptDanhsachKNquahan.xlsx";

            //Đường dẫn vào thư mục file Template.
            string dataDir = TemplateWordSTPT + "rptDanhsachinKNquahan2.xlsx";

            //Open Template
            FileStream fstream = new FileStream(dataDir, FileMode.Open);
            Workbook workbook = new Workbook(fstream);
            Worksheet worksheet = workbook.Worksheets["rptDanhsachKNquahan"];
            ...
            try
            {
                if (tbl != null && tbl.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow row in tbl.Rows)
                    {
                        ...
                        if ((row["NGUOIKCCAPKN"] + "") != "") worksheet.Cells['G' + Convert.ToString(rowcount)].Value = row["NGUOIKCCAPKN"] + "";
                        if ((row["NGAYKN"] + "") != "") worksheet.Cells['H' + Convert.ToString(rowcount)].Value = row["NGAYKN"] + "";
        ...
        protected void cmdInDanhsach_Click(object sender, EventArgs e)
        {
            ...
            tbl = oBL.ADS_PT_KCQUAHAN(LOAIAN, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtMaVuViec.Text, txtTenVuViec.Text, txtTimkiem_SoBAQD.Text, txtTimkiem_NgayBAQD.Text, txtTimkiem_NguoiKC.Text, txtTuNgay.Text, txtDenNgay.Text,
                                                txtTimkiem_SoTL.Text, txtTimkiem_TuNgayTL.Text, txtTimkiem_DenNgayTL.Text, Convert.ToDecimal(ddlTimkiem_Trangthai.SelectedValue + ""), txtTrangthai_tungay.Text, txtTrangthai_denngay.Text, Convert.ToDecimal(ddlTimkiem_Thamphan.SelectedValue + ""), Convert.ToDecimal(ddlTimkiem_Thuky.SelectedValue + ""),
                                                CHECKEDLIST, 0, 0);
        ...
        protected void cmdInQuyetDinh_Click(object sender, EventArgs e)
        {
            try
            {
                if (hddKetqua.Value == "" || hddKetqua.Value == "0")
                {
                    lbthongBaoUpdateKetqua.Text = "Bạn chưa nhập kết quả KN quá hạn!";
                    return;
                }
        ...
        protected void cmdInQuyetDinh_KhongChapnhan()
        {
            try
            {
                ...
                fileName = TemplateWordSTPT + "rpt60DS.doc";
                saveAs = TemplateWordSTPT + "rpt60DS" + Session[ENUM_SESSION.SESSION_USERID] + ".doc";

                fileNameSave = "rptQD_KCN_KCQH_" + tbl.Rows[0]["ITEM_HOTENNGUOIKC"].ToString().Replace(" ", "_") + ".doc"; //+strNguoiNhan      
