...
namespace WEB.GSTP.QLAN.XLHC.Sotham
{
    public partial class NguoiTienhanhTT : System.Web.UI.Page
    {
        ...
        void CheckQuyen()
        {
            ...
            decimal ID = (String.IsNullOrEmpty(Session[ENUM_LOAIAN.BPXLHC] + "")) ? 0 : Convert.ToDecimal(Session[ENUM_LOAIAN.BPXLHC] + "");
            XLHC_DON oT = dt.XLHC_DON.Where(x => x.ID == ID).FirstOrDefault();

            ////////////////////////////////////////////////////////////////////////////////////////////////
            ///2. Check các điều kiện phía trước bước này
            ...
            XLHC_DON_THAMPHAN obj = dt.XLHC_DON_THAMPHAN.Where(x => x.DONID == ID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).OrderByDescending(x => x.NGAYNHANPHANCONG).FirstOrDefault();
            if (obj != null)
            {
            ...
            List<XLHC_DON_THAMPHAN> lst = dt.XLHC_DON_THAMPHAN.Where(x => x.DONID == ID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).ToList<XLHC_DON_THAMPHAN>();
            ...
            //3. Check đã có bước sau chưa
            //XLHC_SOTHAM_QUYETDINH
            //CRUD=0123
            ...
            {
                List<XLHC_SOTHAM_QUYETDINH> objSothamQuyetdinh = dt.XLHC_SOTHAM_QUYETDINH.Where(x => x.DONID == ID).ToList();
                if (objSothamQuyetdinh.Count > 0)
                {
                    lbthongbao.Text = "Vụ việc đã có quyết định giải quyết, không được sửa đổi!";
                    Cls_Comon.SetButton(cmdUpdate, false);
                    //Cls_Comon.SetButton(cmdLammoi, false);
                    hddShowCommand.Value = "False";
                    return;
        ...
        private int CheckQuyen(decimal DONID, out int retCode, out string retDesc)
        {
            ...
            DataTable oDT = oBL.DGLIST_BAQD_QUYETDINH_KETTHUC_ST(ENUM_LOAIVUVIEC_NUMBER.BPXLHC, ID);
            if (oDT.Rows.Count > 0)
            {
            ...
            if (!String.IsNullOrEmpty(hddid.Value) && ("2" == hddActionType.Value || "3" == hddActionType.Value))
            {
                ...
                List<XLHC_SOTHAM_QUYETDINH> objSothamQuyetdinh = dt.XLHC_SOTHAM_QUYETDINH.Where(x => x.DONID == ID && x.TOA_GIAIQUYET_ID == DonviID).ToList();
                if (objSothamQuyetdinh.Count > 0)
                {
                    lbthongbao.Text = "Vụ việc đã có quyết định giải quyết, không được sửa đổi!";
                    Cls_Comon.SetButton(cmdUpdate, false);
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                XLHC_SOTHAM_HDXX obj = dt.XLHC_SOTHAM_HDXX.Where(x => x.ID == ID).FirstOrDefault();
                if (obj != null)
                {
        ...
        private void LoadCombobox()
        {
            ...
            if (loaiQD != null)
            {
                XLHC_SOTHAM_QUYETDINH qd = dt.XLHC_SOTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.LOAIQDID == loaiQD.ID).FirstOrDefault<XLHC_SOTHAM_QUYETDINH>();
                if (qd != null)
                {
                    XLHC_DON_THAMPHAN tpgq = dt.XLHC_DON_THAMPHAN.Where(x => x.DONID == DonID && x.MAVAITRO == "VTTP_GIAIQUYETSOTHAM").OrderByDescending(x => x.NGAYTAO).FirstOrDefault<XLHC_DON_THAMPHAN>();
                    if (tpgq != null)
                    {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                XLHC_SOTHAM_HDXX oND;

                // Lấy count thụ lý và count chủ tọa để cho nhập thêm thẩm phán chủ tọa
                List<XLHC_DON_THAMPHAN> countTPGQ = dt.XLHC_DON_THAMPHAN.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).ToList();
                List<XLHC_SOTHAM_HDXX> countChuToa = dt.XLHC_SOTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).ToList();
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    if (tucach == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN)
                    {
                        ...
                        oND = dt.XLHC_SOTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).FirstOrDefault<XLHC_SOTHAM_HDXX>();
                    ...
                    oND = new XLHC_SOTHAM_HDXX();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.XLHC_SOTHAM_HDXX.Where(x => x.ID == ID).FirstOrDefault();
                }
                oND.DONID = DONID;
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    XLHC_SOTHAM_HDXX CheckCanBo = dt.XLHC_SOTHAM_HDXX.Where(x => x.DONID == DONID && x.CANBOID == oND.CANBOID).FirstOrDefault();
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.XLHC_SOTHAM_HDXX_GETLIST(ID);
        ...
        {
            XLHC_SOTHAM_HDXX oND = dt.XLHC_SOTHAM_HDXX.Where(x => x.ID == id).FirstOrDefault();
            dt.XLHC_SOTHAM_HDXX.Remove(oND);
            dt.SaveChanges();
            dgList.CurrentPageIndex = 0;
            LoadGrid();
            ResetControls();
