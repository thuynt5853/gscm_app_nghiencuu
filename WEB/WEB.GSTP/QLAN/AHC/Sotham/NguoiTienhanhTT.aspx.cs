...
namespace WEB.GSTP.QLAN.AHC.Sotham
{
    public partial class NguoiTienhanhTT : System.Web.UI.Page
    {
        ...
        private void CheckQuyen(decimal ID)
        {
            ...
            AHC_DON_THAMPHAN obj = dt.AHC_DON_THAMPHAN.Where(x => x.DONID == ID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).OrderByDescending(x => x.NGAYNHANPHANCONG).FirstOrDefault();
            if (obj != null)
            {
            ...
            List<AHC_DON_THAMPHAN> lst = dt.AHC_DON_THAMPHAN.Where(x => x.DONID == ID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).ToList<AHC_DON_THAMPHAN>();
            ...
            AHC_SOTHAM_BANAN oBA = dt.AHC_SOTHAM_BANAN.Where(x => x.DONID == ID).FirstOrDefault();
            if (oBA != null)
            {
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                bool isReadOnly = _chuyenNhanBl.CheckIsReadOnlyNguoiTTTTST(Convert.ToDecimal(rowView["ID"].ToString()), DONID, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]));


                if (oT.MAGIAIDOAN == ENUM_GIAIDOANVUAN.PHUCTHAM || oT.MAGIAIDOAN == ENUM_GIAIDOANVUAN.THULYGDT || isReadOnly)
                {
                ...
                AHC_SOTHAM_HDXX obj = dt.AHC_SOTHAM_HDXX.Where(x => x.ID == ID).FirstOrDefault();
                ...
                if (obj != null)
                {
                    ...
                    AHC_CHUYEN_NHAN_AN chuyenan = dt.AHC_CHUYEN_NHAN_AN.Where(x => x.VUANID == DONID).OrderByDescending(x => x.ID).FirstOrDefault();
                    if (chuyenan != null)
                    {
                        if (obj.NGAYTAO <= chuyenan.NGAYTAO)
                        {
                            List<AHC_SOTHAM_QUYETDINH> qd = dt.AHC_SOTHAM_QUYETDINH.Where(x => x.DONID == DONID).ToList();
                        ...
                        {
                            List<AHC_SOTHAM_QUYETDINH> qd = dt.AHC_SOTHAM_QUYETDINH.Where(x => x.DONID == DONID && x.NGAYTAO > chuyenan.NGAYTAO).ToList();
                            if ((qd != null && qd.Count > 0))
                            {
                                lblSua.Visible = false;
                                lbtXoa.Visible = false;
                    ...
                    else
                    {
                        ...
                        List<AHC_SOTHAM_QUYETDINH> qd = dt.AHC_SOTHAM_QUYETDINH.Where(x => x.DONID == DONID && x.TOA_GIAIQUYET_ID == Donvi_ID).ToList();
                        if ((qd != null && qd.Count > 0))
                        {
                            lblSua.Visible = false;
                            lbtXoa.Visible = false;
        ...
        private void LoadCombobox()
        {
            ...
            if (loaiQD != null)
            {
                AHC_SOTHAM_QUYETDINH qd = dt.AHC_SOTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.LOAIQDID == loaiQD.ID).FirstOrDefault<AHC_SOTHAM_QUYETDINH>();
                if (qd != null)
                {
                    AHC_DON_THAMPHAN tpgq = dt.AHC_DON_THAMPHAN.Where(x => x.DONID == DonID && x.MAVAITRO == "VTTP_GIAIQUYETSOTHAM").OrderByDescending(x => x.NGAYTAO).FirstOrDefault<AHC_DON_THAMPHAN>();
                    if (tpgq != null)
                    {
        ...
        private bool CheckValid()
        {
            ...
            {
                if (ddlHTND_Thuky.SelectedValue == "0")
                {
                    lbthongbao.Text = "Bạn chưa chọn Hội thẩm nhân dân !";
                    ddlHTND_Thuky.Focus();
                    return false;
                }
            }
            else if (tucach == ENUM_NGUOITIENHANHTOTUNG.THUKY || tucach == ENUM_NGUOITIENHANHTOTUNG.THUKYDUKHUYET)
            {
                if (ddlHTND_Thuky.SelectedValue == "0")
                {
                    lbthongbao.Text = "Bạn chưa chọn Thư ký !";
                    ddlHTND_Thuky.Focus();
                    return false;
                }
            }
            else
            if (tucach == ENUM_CHUCDANH.CHUCDANH_KSV)
            {
                if (ddlKSV_Nguoi.SelectedValue == "0")
                {
                    lbthongbao.Text = "Bạn chưa chọn Kiểm sát viên !";
                    ddlKSV_Nguoi.Focus();
                    return false;
                }
            }
            ...
            if (txtNgayphancong.Text.Trim() != "")
            {
                ...
                DateTime NgayPC = DateTime.Parse(txtNgayphancong.Text, cul, DateTimeStyles.NoCurrentDateDefault);
                if (hdfNgayPCTPGQ.Value != "")
                {
                    DateTime NgayPCTPGQ = DateTime.Parse(hdfNgayPCTPGQ.Value, cul, DateTimeStyles.NoCurrentDateDefault);
                    if (NgayPC < NgayPCTPGQ)
                    {
                        lbthongbao.Text = "Bạn phải nhập ngày phân công lớn hơn ngày phân công thẩm phán giải quyết " + hdfNgayPCTPGQ.Value + " !";
                        txtNgayphancong.Focus();
                        return false;
                    }
                }
                if (NgayPC > DateTime.Now)
                {
                    lbthongbao.Text = "Bạn phải nhập ngày phân công nhỏ hơn ngày hiện tại !";
                    txtNgayphancong.Focus();
                    return false;
                }
            }
            if (txtNhanphancong.Text.Trim() != "")
            {
                if (txtNgayphancong.Text.Trim() == "")
                {
                    lbthongbao.Text = "Bạn chưa nhập ngày phân công !";
                    txtNgayphancong.Focus();
                    return false;
                }
                if (Cls_Comon.IsValidDate(txtNgayphancong.Text) == false)
                ...
                DateTime DayPC = DateTime.Parse(txtNgayphancong.Text, cul, DateTimeStyles.NoCurrentDateDefault);
                DateTime NgayNhanPC = DateTime.Parse(txtNhanphancong.Text, cul, DateTimeStyles.NoCurrentDateDefault);
                if (NgayNhanPC < DayPC)
                {
                    lbthongbao.Text = "Bạn phải nhập ngày nhận phân công lớn hơn ngày phân công !";
                    txtNhanphancong.Focus();
                    return false;
                }
                if (NgayNhanPC > DateTime.Now)
                {
                    lbthongbao.Text = "Bạn phải nhập ngày nhận phân công nhỏ hơn ngày hiện tại !";
                    txtNhanphancong.Focus();
                    return false;
                }
            }
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                List<AHC_DON_THAMPHAN> countTPGQ = dt.AHC_DON_THAMPHAN.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).ToList();
                List<AHC_SOTHAM_HDXX> countChuToa = dt.AHC_SOTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).ToList();
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    if (tucach == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN)
                    {
                        ...
                        oND = dt.AHC_SOTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).FirstOrDefault<AHC_SOTHAM_HDXX>();
                    ...
                    oND = new AHC_SOTHAM_HDXX();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.AHC_SOTHAM_HDXX.Where(x => x.ID == ID).FirstOrDefault();
                }
                oND.DONID = DONID;
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    AHC_SOTHAM_HDXX CheckCanBo = dt.AHC_SOTHAM_HDXX.Where(x => x.DONID == DONID && x.CANBOID == oND.CANBOID).FirstOrDefault();
        ...
        public void LoadGrid()
        {
            ...
            string current_id = Session[ENUM_LOAIAN.AN_HANHCHINH] + "";
            decimal ID = Convert.ToDecimal(current_id);
            DataTable oDT = oBL.AHC_SOTHAM_HDXX_GETLIST(ID);
        ...
        {
            AHC_SOTHAM_HDXX oND = dt.AHC_SOTHAM_HDXX.Where(x => x.ID == id).FirstOrDefault();
            dt.AHC_SOTHAM_HDXX.Remove(oND);
            dt.SaveChanges();
            dgList.CurrentPageIndex = 0;
            LoadGrid();
            ResetControls();
        ...
        public void loadedit(decimal ID)
        {
            ...
            txtNgayketthuc.Text = oND.NGAYKETTHUC + "" == "" ? "" : ((DateTime)oND.NGAYKETTHUC).ToString("dd/MM/yyyy", cul);
        }
