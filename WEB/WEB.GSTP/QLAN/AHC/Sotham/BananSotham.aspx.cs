...
namespace WEB.GSTP.QLAN.AHC.Sotham
{
    public partial class BananSotham : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                DSID = (String.IsNullOrEmpty(Session[ENUM_LOAIAN.AN_HANHCHINH] + ""))
                    ? 0
                    : Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HANHCHINH]);
                hddURLKS.Value = Cls_Comon.GetRootURL() + "/FileUploadHandler.aspx";
                hddDonID.Value = Session[ENUM_LOAIAN.AN_HANHCHINH] + "" == ""
                    ? "0"
                    : Session[ENUM_LOAIAN.AN_HANHCHINH] + "";
        ...
        bool CheckCongbo(decimal ID)
        {
            var list = DataExtensions.GetAllWithClause<BAQD_CONGBO>(
                $"VUVIECID = {ID} AND LOAIANID = {ENUM_LOAIVUVIEC_NUMBER.AN_HANHCHINH} AND CAPXETXU = 2 AND TRANGTHAI IN (2,3)"
            );
        ...
        private void CheckQuyen(decimal ID)
        {
            ...
            DataTable oDT = oBL.DGLIST_BAQD_QUYETDINH_KETTHUC_ST(ENUM_LOAIVUVIEC_NUMBER.AN_HANHCHINH, ID);
            if (oDT.Rows.Count > 0)
            {
                pnQDVV.Visible = true;
            ...
            {
                hddNgayThuLy.Value = lstCount[0].NGAYTHULY + "" == ""
                    ? ""
                    : ((DateTime)lstCount[0].NGAYTHULY).ToString("dd/MM/yyyy");
            }

            //Kiểm tra đã phân công thẩm phán chưa
            List<AHC_DON_THAMPHAN> lstTPGQ = dt.AHC_DON_THAMPHAN
                .Where(x => x.DONID == ID && x.MAVAITRO == "VTTP_GIAIQUYETSOTHAM").ToList();
            if (lstTPGQ.Count == 0)
            {
            ...
            List<AHC_SOTHAM_HDXX> lstTPCT = dt.AHC_SOTHAM_HDXX
                .Where(x => x.DONID == ID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).ToList();
            if (lstTPCT.Count == 0)
            {
                lstErr.Text = lbthongbaoQD.Text = "Chưa cập nhật thông tin hội đồng xét xử !";
                DisableButtonsBanan();
            ...
            //List<AHC_SOTHAM_QUYETDINH> lstQDXX =
            //    dt.AHC_SOTHAM_QUYETDINH.Where(x => x.DONID == ID && x.LOAIQDID == IDLQD).ToList();
            //if (lstQDXX.Count == 0)
            //{
            ...
            if (anKetThuc)
            {
                lstErr.Text = lbthongbao.Text = "Vụ việc đã kết thúc tại tòa cũ, không thể chỉnh sửa tại tòa mới !";
                DisableButtonsBanan();
                DisableButtonsQuyetdinh();
            ...
            //Decimal soDuongSuChuaXacThuc = dt.AHC_DON_DUONGSU.Count(x => x.DONID == ID && x.XACTHUC_DLDCQG == 0 && x.QUOCTICHID == 2);
        ...
        private bool CheckQuyenSua(decimal ID)
        {
            ...
            AHC_SOTHAM_BANAN ba = dt.AHC_SOTHAM_BANAN.Where(x => x.DONID == ID).FirstOrDefault();
            if (ba != null)
            {
            ...
            List<decimal> dmQDIds = dt.DM_QD_QUYETDINH
                .Where(x => x.ISSOTHAM == 1 && x.ISHANHCHINH == 1 && x.KET_THUC == 1).Select(x => x.ID).ToList();
            List<AHC_SOTHAM_QUYETDINH> qdKetThuc = dt.AHC_SOTHAM_QUYETDINH
                .Where(x => x.DONID == ID && dmQDIds.Contains(x.QUYETDINHID.Value)).ToList();
            if (ba != null && qdKetThuc != null)
            {
                AHC_SOTHAM_KHANGCAO kc2 = dt.AHC_SOTHAM_KHANGCAO.Where(x => x.DONID == ID).FirstOrDefault();
                if (kc2 != null)
                {
                ...
                AHC_SOTHAM_KHANGNGHI kn2 = dt.AHC_SOTHAM_KHANGNGHI.Where(x => x.DONID == ID).FirstOrDefault();
                if (kn2 != null)
                {
            ...
            {
                AHC_SOTHAM_KHANGCAO kc = dt.AHC_SOTHAM_KHANGCAO.Where(x => x.DONID == ID && x.TINHTRANG_GIAIQUYET == 1)
                    .OrderByDescending(x => x.ID).FirstOrDefault();
                AHC_SOTHAM_KHANGNGHI kn = dt.AHC_SOTHAM_KHANGNGHI
                    .Where(x => x.DONID == ID && x.TINHTRANG_GIAIQUYET == 1).OrderByDescending(x => x.ID)
                    .FirstOrDefault();
                if (kc != null)
                {
                    var kcChuaGiaiQuyet = dt.AHC_SOTHAM_KHANGCAO
                        .Where(x => x.DONID == ID && x.ID > kc.ID && x.TINHTRANG_GIAIQUYET != 2).ToList();
                ...
                if (kn != null)
                {
                    var knChuaGiaiQuyet = dt.AHC_SOTHAM_KHANGNGHI
                        .Where(x => x.DONID == ID && x.ID > kn.ID && x.TINHTRANG_GIAIQUYET != 3).ToList();
            ...
            string Result = new AHC_CHUYEN_NHAN_AN_BL().Check_NhanAn(ID, StrMsg,
                Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]));
            if (Result != "")
            {
                lstErr.Text = lbthongbaoQD.Text = Result;
                lbthongbaoA.Text = "Chưa nhập bản án sơ thẩm";
                DisableButtonsBanan();
                DisableButtonsQuyetdinh();
                return false;
            }
            ...
            AHC_TONGDAT td = dt.AHC_TONGDAT.Where(x => x.DONID == ID && (x.BIEUMAUID == 284 || x.BIEUMAUID == 257))
                .FirstOrDefault();
            if (td != null)
            {
            ...
            if (ba != null)
            {
                KHOBAQD_BL ahc = new KHOBAQD_BL();
                bool isExist = ahc.IsExistKHOBADQ(0, ID, 2,ENUM_LOAIVUVIEC_TEXT.AN_HANHCHINH);
                if (isExist)
                {
        ...
        public void LoadGrid()
        {
            ...
            if (oDT != null && oDT.Rows.Count > 0)
            {
                ...
                hddTotalPage.Value = Cls_Comon.GetTotalPage(Convert.ToInt32(oDT.Rows.Count), Convert.ToInt32(20))
                    .ToString();
                lstSobanghiT.Text = lstSobanghiB.Text = "Có <b>" + oDT.Rows.Count.ToString() +
                                                        " </b> bản ghi trong <b>" + hddTotalPage.Value + "</b> trang";
                Cls_Comon.SetPageButton(hddTotalPage, hddPageIndex, lbTFirst, lbBFirst, lbTLast, lbBLast, lbTNext,
                    lbBNext, lbTBack, lbBBack, lbTStep1, lbBStep1, lbTStep2,
        ...
        {
            ddlNguoiYC.Items.Clear();
            ddlNguoiBiYC.Items.Clear();
            decimal DonID = Convert.ToDecimal(hddDonID.Value);
            List<AHC_DON_DUONGSU> lstDS = dt.AHC_DON_DUONGSU.Where(x => x.DONID == DonID).OrderBy(x => x.TENDUONGSU)
                .ToList<AHC_DON_DUONGSU>();
        ...
        private void LoadQD()
        {
            ...
            DM_QUYETDINH_VUAN_KETTHUC oBL = new DM_QUYETDINH_VUAN_KETTHUC();
            DataTable oDT = oBL.DM_QUYETDINH_VUAN_SOTHAM_KETTHUC(ENUM_LOAIVUVIEC_NUMBER.AN_HANHCHINH);
        ...
        private void LoadLydo()
        {
            if (ddlQuyetdinh.Items.Count > 0 && ddlQuyetdinh.SelectedValue != "0")
            {
                ...
                List<DM_QD_QUYETDINH_LYDO> lst = dt.DM_QD_QUYETDINH_LYDO.Where(x => x.QDID == ID & x.HIEULUC == 1)
                    .OrderBy(y => y.THUTU).ToList();
        ...
        private void LoadNguoiKyInfo()
        {
            ...
            AHC_SOTHAM_HDXX oND = dt.AHC_SOTHAM_HDXX
                .Where(x => x.DONID == DonID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN)
                .OrderByDescending(x => x.NGAYPHANCONG).FirstOrDefault<AHC_SOTHAM_HDXX>();
            if (oND != null)
            {
            ...
            {
                AHC_DON_THAMPHAN oTP = dt.AHC_DON_THAMPHAN
                    .Where(x => x.DONID == DonID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM)
                    .OrderByDescending(x => x.NGAYPHANCONG).FirstOrDefault();
                if (oTP != null)
                {
        ...
        {
            decimal ID = Convert.ToDecimal(hddDonID.Value);
            List<AHC_SOTHAM_QUYETDINH> lstQD = dt.AHC_SOTHAM_QUYETDINH.Where(x =>
                x.DONID == ID && (x.LOAIQDID == 10 || x.QUYETDINHID == 423 || x.QUYETDINHID == 422 ||
                                  x.QUYETDINHID == 70 || x.QUYETDINHID == 425 || x.LOAIQDID == 3)).ToList();
            if (lstQD.Count >= 1)
            {
        ...
        public void xoa(decimal id)
        {
            ...
            if (oND.NOIDUNG == null && oND.QT_FILE_ID != null)
            {
                ...
                {
                    qtFileDelete.DESCRIPTION = "Tài khoản " + Session[ENUM_SESSION.SESSION_USERNAME] + " đã xóa file tại form BanAnSoTham " + ENUM_LOAIAN.AN_HANHCHINH + ".";
                    QT_FILE_BL fileH = new QT_FILE_BL();
                    fileH.DeleteFileLogic(qtFileDelete);
                }
            }
            TK_SOTHAM_QUYETDINH TK = DataExtensions
                .GetAllWithClause<TK_SOTHAM_QUYETDINH>(
                    $"QUYETDINHID = {id} AND LOAIAN =  {Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_HANHCHINH)}")
                .FirstOrDefault();
            if (TK != null)
            {
            ...
            var list = DataExtensions.GetAllWithClause<BAQD_CONGBO>($"  VUVIECID = {oND.DONID.Value} " +
                                                                    $"  AND LOAIANID = {Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_HANHCHINH)} " +
                                                                    $"  AND CAPXETXU = {2} ");
        ...
        protected void btnUpdate_Quyetdinh_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //Decimal soDuongSuChuaXacThuc = dt.AHC_DON_DUONGSU.Count(x => x.DONID == DONID && x.XACTHUC_DLDCQG == 0 && x.QUOCTICHID == 2);
                //if (soDuongSuChuaXacThuc > 0)
                //{
                //    lstErr.Text = "Có đương sự chưa xác thực thông tin. Đề nghị xác thực tại màn hình Danh sách đương sự.";
                //    return;
                //}

                KHOBAQD_BL ahc = new KHOBAQD_BL();
                bool isExist = ahc.IsExistKHOBADQ(1, DONID, 2,ENUM_LOAIVUVIEC_TEXT.AN_HANHCHINH);
                if (isExist)
                {
                ...
                if ((hddid.Value == "" || hddid.Value == "0"))
                {
                    ...
                    if (soDuongSuChuaXacThuc > 0)
                    {
                        lbthongbao.Text = lstErr.Text = "Có đương sự chưa xác thực thông tin. Đề nghị xác thực tại màn hình Danh sách đương sự.";
                        return;
                    }
                    ...
                    if (!String.IsNullOrEmpty(txtNgayQD.Text.Trim()))
                        STTQD = oBL.GETFILENEWTT(Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]),
                            ENUM_GIAIDOANVUAN.SOTHAM, NgayQD.Year, 1);
                    else
                    {
                        Decimal? a = null;
                        STTQD = oBL.GETFILENEWTT(Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]),
                            ENUM_GIAIDOANVUAN.SOTHAM, Convert.ToDecimal(a), 1);
                    }
                }
                else
                {
                    ...
                    {
                        lbthongbao.Text =
                            "Quyết định đang chọn thuộc thẩm quyền của tòa án khác, không được phép thay đổi !";
                        return;
                    }
                ...
                try
                {
                    if (hddFilePathQD.Value != "")
                    {
                        ...
                        QT_FILE qtFile = fileHelper.InsertFile_Minio_Banan(strFilePath, Convert.ToInt32(ENUM_LOAIVUVIEC_NUMBER.AN_HANHCHINH), "BANANSOTHAM");
                        if (qtFile == null)
                        {
                ...
                if (ddlQuyetdinh.SelectedItem.Text.Contains("15-HC"))
                {
                    TK.DONID = DONID;
                    TK.LOAIAN = Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_HANHCHINH);
                    TK.APDUNGANLE = ddlCBBA_AnleQD.SelectedValue == "0" ? 0 : 1;
                    TK.SOANLE = ddlCBBA_AnleQD.SelectedValue;
                    TK.YEUTONUOCNGOAI = ddlYeutonuocngoai_QD.SelectedValue == ""
                        ? 0
                        : Convert.ToDecimal(ddlYeutonuocngoai_QD.SelectedValue);
                    TK.ISVKSTHAMGIA = rdbVKSThamgia_QD.SelectedValue == ""
                        ? 0
                        : Convert.ToDecimal(rdbVKSThamgia_QD.SelectedValue);
                    TK.TK_ISKIENCOQUANNN = rdbKiennghiCQNN_QD.SelectedValue == ""
                        ? 0
                        : Convert.ToDecimal(rdbKiennghiCQNN_QD.SelectedValue);
                    TK.TK_ISDOITHOAI = rdbDoithoai_QD.SelectedValue == ""
                        ? 0
                        : Convert.ToDecimal(rdbDoithoai_QD.SelectedValue);
                    TK.TK_ISBOITHUONG = rdbBoithuong_QD.SelectedValue == ""
                        ? 0
                        : Convert.ToDecimal(rdbBoithuong_QD.SelectedValue);
                    TK.TK_SOTIEN = (String.IsNullOrEmpty(txtSotien_QD.Text + ""))
                        ? 0
                        : Convert.ToDecimal(txtSotien_QD.Text.Replace(".", ""));

                    if (rdbYeucauKK_QD.SelectedValue == "0")
                    {
                        TK.TK_BACYEUCAUKK = 1;
                        TK.TK_CHAPNHAN1PHAN = 0;
                        TK.TK_CHAPNHANTOANBO = 0;
                    }

                    if (rdbYeucauKK_QD.SelectedValue == "1")
                    {
                        TK.TK_CHAPNHAN1PHAN = 1;
                        TK.TK_BACYEUCAUKK = 0;
                        TK.TK_CHAPNHANTOANBO = 0;
                    }

                    if (rdbYeucauKK_QD.SelectedValue == "2")
                    {
                        TK.TK_CHAPNHANTOANBO = 1;
                        TK.TK_BACYEUCAUKK = 0;
                        TK.TK_CHAPNHAN1PHAN = 0;
                    }
                }
                ...
                oND.NGAYQD = (String.IsNullOrEmpty(txtNgayQD.Text.Trim()))
                    ? (DateTime?)null
                    : DateTime.Parse(this.txtNgayQD.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);

                oND.NGAYMOPT = (String.IsNullOrEmpty(txtNgayMoPhienToaQD.Text.Trim()))
                    ? (DateTime?)null
                    : DateTime.Parse(this.txtNgayMoPhienToaQD.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
                ...
                oND.HIEULUCTU = (String.IsNullOrEmpty(txtHieuLucTuNgay.Text.Trim()))
                    ? (DateTime?)null
                    : DateTime.Parse(this.txtHieuLucTuNgay.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
                oND.HIEULUCDEN = (String.IsNullOrEmpty(txtHieuLucDenNgay.Text.Trim()))
                    ? (DateTime?)null
                    : DateTime.Parse(this.txtHieuLucDenNgay.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
                ...
                if (oQDT.ISCONGBO == 1 && oND.HIEULUCTU != null)
                {
                    ...
                    temp_congbo.LOAIANID = Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_HANHCHINH);
                ...
                if (AsyncFileUpLoadQD.HasFile && dgFile.Items.Count < 1)
                {
                    ...
                    if (extension == ".doc" || extension == ".docx" || extension == ".pdf")
                    {
                        ...
                        ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "filePath",
                            "top.$get(\"" + hddFilePathQD.ClientID + "\").value = '" + path + "';", true);
                    }
        ...
        private decimal UploadFileIDQD(AHC_DON oDon, decimal FileID, string strMaBieumau, decimal STT)
        {
            AHC_DON_BL oBL = new AHC_DON_BL();
            decimal IDFIle = 0;
            decimal IDBM = 0;
        ...
        private void TamNgungDONKK_USER_DKNHANVB(decimal DONID, string TenQuyetDinh)
        {
            ...
            DONKK_USER_DKNHANVB obj = dkk.DONKK_USER_DKNHANVB.FirstOrDefault(s =>
                s.MAVUVIEC == oDon.MAVUVIEC && s.VUVIECID == oDon.ID && s.MALOAIVUVIEC == ENUM_LOAIAN.AN_HANHCHINH &&
                s.TRANGTHAI == 1);
            if (obj != null)
            {

                if (TenQuyetDinh.StartsWith("09-HC.") || TenQuyetDinh.StartsWith("45-DS.") ||
                    TenQuyetDinh.StartsWith("46-DS.") || TenQuyetDinh.StartsWith("38-DS.") ||
                    TenQuyetDinh.StartsWith("39-DS."))
                {
        ...
        public void loadedit(decimal ID)
        {
            ...
            if (ddlQuyetdinh.SelectedItem.Text.Contains("15-HC"))
            {
                ...
                if (TK.TK_ISKIENCOQUANNN != null) rdbKiennghiCQNN_QD.SelectedValue = TK.TK_ISKIENCOQUANNN.ToString();
                if (TK.TK_ISDOITHOAI != null) rdbDoithoai_QD.SelectedValue = TK.TK_ISDOITHOAI.ToString();
                if (TK.TK_ISBOITHUONG != null) rdbBoithuong_QD.SelectedValue = TK.TK_ISBOITHUONG.ToString();
                //if (TK.TK_SOTIEN != null) TK.TK_SOTIEN = (String.IsNullOrEmpty(txtSotien_QD.Text + "")) ? 0 : Convert.ToDecimal(txtSotien_QD.Text.Replace(".", ""));
                if (TK.TK_SOTIEN != null)
                    txtSotien_QD.Text =
                        TK.TK_SOTIEN + "" == "0" ? "" : ((decimal)TK.TK_SOTIEN).ToString("#,0.###", cul);
                if (TK.YEUTONUOCNGOAI != null) ddlYeutonuocngoai_QD.SelectedValue = TK.YEUTONUOCNGOAI.ToString();

                if ((TK.TK_BACYEUCAUKK + "") == "1")
                    rdbYeucauKK_QD.SelectedValue = "0";
                if ((TK.TK_CHAPNHAN1PHAN + "") == "1")
                    rdbYeucauKK_QD.SelectedValue = "1";
                if ((TK.TK_CHAPNHANTOANBO + "") == "1")
                    rdbYeucauKK_QD.SelectedValue = "2";
        ...
        private void SetEnable_data_QD(AHC_SOTHAM_QUYETDINH QD)
        {
            if (QD != null)
            {
                ...
                AHC_TONGDAT oTD = dt.AHC_TONGDAT.Where(x => x.DONID == QD.DONID && x.MAPID == QD.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AHC_SOTHAM_QUYETDINH).FirstOrDefault();
                if (oTD != null)
                {
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            {
                case "Download":
                    var oND = dt.AHC_SOTHAM_QUYETDINH.Where(x => x.DONID == ID && x.FILEID == ND_id).FirstOrDefault();
                    if (oND.NOIDUNGFILE != null)
                    {
                        if (oND.NOIDUNGFILE.Length != 0 && oND.QT_FILE_ID == null)
                        {
                            var cacheKey = Guid.NewGuid().ToString("N");
                            Context.Cache.Insert(key: cacheKey, value: oND.NOIDUNGFILE, dependencies: null,
                                absoluteExpiration: DateTime.Now.AddSeconds(30),
                                slidingExpiration: System.Web.Caching.Cache.NoSlidingExpiration);
                            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "Download",
                                "window.location='" + Cls_Comon.GetRootURL_HTTPS() + "/DownloadFile.aspx?cacheKey=" +
                                cacheKey + "&FileName=" + oND.TENFILE + "&Extension=" + oND.KIEUFILE + "';", true);
                        }
                    }
                    else
                    {
                        ...
                        string _pathStore = QT_FILE_BL.ToPathFolderStore(qT_FILE.DATE_CREATED.Value, Convert.ToInt32(ENUM_LOAIVUVIEC_NUMBER.AN_HANHCHINH)) + "\\BANANSOTHAM";
                ...
                case "Xoa":
                    ...
                    string StrMsg = "Không được sửa đổi thông tin.";
                    string Result = new AHC_CHUYEN_NHAN_AN_BL().Check_NhanAn(DonID, StrMsg,
                        Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]));
                    if (Result != "")
                    {
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                AHC_SOTHAM_KHANGCAO oTKC = dt.AHC_SOTHAM_KHANGCAO.Where(x => x.DONID == DONID).FirstOrDefault();
                AHC_SOTHAM_KHANGNGHI oTKN = dt.AHC_SOTHAM_KHANGNGHI.Where(x => x.DONID == DONID).FirstOrDefault();
        ...
        protected void ddlQuyetdinh_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (btnUpdate.Enabled == true)
            {
                ...
                //--Kiểm tra đã phân công thẩm phán chưa
                AHC_DON_THAMPHAN oTP = dt.AHC_DON_THAMPHAN.Where(x => x.DONID == IDD && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).FirstOrDefault();
                if (oTP == null)
                {
                ...
                {
                    //Kiểm tra xem đã có quyết định đưa vụ việc ra xét xử hay không
                    AHC_SOTHAM_QUYETDINH QDST = dt.AHC_SOTHAM_QUYETDINH
                        .Where(x => x.DONID == IDD && x.LOAIQDID == 5 && x.QUYETDINHID != 147 &&
                                    x.QUYETDINHID !=
                                    148 /*Quyết định chuyển vụ án giải quyết theo thủ tục rút gọn sang giải quyết theo thủ tục thông thường*/)
                        .OrderByDescending(x => x.NGAYQD).FirstOrDefault();
                    if (QDST == null)
                    {
            ...
            if (oT != null)
            {
                ...
                if (ID == 62 || ID == 63 || ID == 67 || ID == 68 || ID == 41 || ID == 42 || ID == 45 || ID == 147 ||
                    ID == 4 || ID == 61)
                {
                    // lấy số mới nhất 
                    Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
        ...
        protected void ddlNguoiYC_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            List<AHC_DON_DUONGSU> lstDS = dt.AHC_DON_DUONGSU.Where(x => x.DONID == DonID && x.ID != NguoiYC)
                .OrderBy(x => x.TENDUONGSU).ToList<AHC_DON_DUONGSU>();
        ...
        protected void lbtDownload_Click(object sender, EventArgs e)
        {
            ...
            {
                decimal DONID = Convert.ToDecimal(hddDonID.Value);
                AHC_FILE oND = dt.AHC_FILE.Where(x => x.DONID == DONID).FirstOrDefault();
                if (oND.TENFILE != "")
                {
                    var cacheKey = Guid.NewGuid().ToString("N");
                    Context.Cache.Insert(key: cacheKey, value: oND.URL, dependencies: null,
                        absoluteExpiration: DateTime.Now.AddSeconds(30),
                        slidingExpiration: System.Web.Caching.Cache.NoSlidingExpiration);
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "Download",
                        "window.location='" + Cls_Comon.GetRootURL_HTTPS() + "/DownloadFile.aspx?cacheKey=" + cacheKey +
        ...
        private void LoadAnPhi()
        {
            ...
            dgAnPhi.DataSource = oBL.AHC_SOTHAM_BANAN_ANPHI_GET(DONID);
            dgAnPhi.DataBind();
            //Kiểm tra xem có miễn án phí không
        ...
        private void LoadTGTT()
        {
            ...
            decimal DONID = Convert.ToDecimal(current_id);
            AHC_SOTHAM_BL oBL = new AHC_SOTHAM_BL();
            DataTable dtTGTT = oBL.AHC_SOTHAM_BANAN_TGTT_GET(DONID);
        ...
        {
            List<DM_BOLUAT> lst = dt.DM_BOLUAT.Where(x => x.LOAI == ENUM_LOAIVUVIEC.AN_HANHCHINH && x.HIEULUC == 1)
        ...
        {
            decimal ID = Convert.ToDecimal(hddDonID.Value);
            dgFile.DataSource = dt.AHC_SOTHAM_BANAN_FILE.Where(x => x.DONID == ID).ToList();
            dgFile.DataBind();
        ...
        private void LoadDieuLuat()
        {
            ...
            decimal DONID = Convert.ToDecimal(current_id);
            AHC_SOTHAM_BL oBL = new AHC_SOTHAM_BL();
            dgDieuLuat.DataSource = oBL.AHC_SOTHAM_BANAN_DIEULUAT_GET(DONID);
            dgDieuLuat.DataBind();
        }

        private void LoadBanAnInfo(decimal DonID)
        {
            List<AHC_SOTHAM_BANAN> lst = dt.AHC_SOTHAM_BANAN.Where(x => x.DONID == DonID).ToList();
            if (lst.Count > 0)
            {
                ...
                if (oT.NGAYTUYENAN != null)
                    txtNgaytuyenan.Text = ((DateTime)oT.NGAYTUYENAN).ToString("dd/MM/yyyy", cul);
                if (oT.NGAYHIEULUC != null)
                    txtNgayhieuluc.Text = ((DateTime)oT.NGAYHIEULUC).ToString("dd/MM/yyyy", cul);

                if (oT.ISVKSTHAMGIA != null)
                    rdbVKSThamgia.SelectedValue = oT.ISVKSTHAMGIA.ToString();

                if ((oT.TK_ISANLE == null || oT.TK_ISANLE == 1) && oT.SOANLE == null)
                {
                ...
                rdVuAnQuaHan.SelectedValue =
                    (string.IsNullOrEmpty(oT.TK_ISQUAHAN + "")) ? "0" : oT.TK_ISQUAHAN.ToString();
                rdNNChuQuan.SelectedValue = (string.IsNullOrEmpty(oT.TK_QUAHAN_CHUQUAN + ""))
                    ? "0"
                    : oT.TK_QUAHAN_CHUQUAN.ToString();
                rdNNKhachQuan.SelectedValue = (string.IsNullOrEmpty(oT.TK_QUAHAN_KHACHQUAN + ""))
                    ? "0"
                    : oT.TK_QUAHAN_KHACHQUAN.ToString();
                rdbKiennghiCQNN.SelectedValue = (string.IsNullOrEmpty(oT.TK_ISKIENCOQUANNN + ""))
                    ? "0"
                    : oT.TK_ISKIENCOQUANNN.ToString();
                rdbDoithoai.SelectedValue =
                    (string.IsNullOrEmpty(oT.TK_ISDOITHOAI + "")) ? "0" : oT.TK_ISDOITHOAI.ToString();
                rdbBoithuong.SelectedValue =
                    (string.IsNullOrEmpty(oT.TK_ISBOITHUONG + "")) ? "0" : oT.TK_ISBOITHUONG.ToString();
                if (rdbBoithuong.SelectedValue == "0")
                {
                ...
                else
                {
                    ...
                    txtSotien.Text = oT.TK_SOTIEN + "" == "0" ? "" : ((decimal)oT.TK_SOTIEN).ToString("#,0.###", cul);
                }

                if ((oT.TK_BACYEUCAUKK + "") == "1")
                    rdbYeucauKK.SelectedValue = "0";
                if ((oT.TK_CHAPNHAN1PHAN + "") == "1")
                    rdbYeucauKK.SelectedValue = "1";
                if ((oT.TK_CHAPNHANTOANBO + "") == "1")
                    rdbYeucauKK.SelectedValue = "2";
            ...
            else
            {
                ...
                AHC_SOTHAM_THULY tl = dt.AHC_SOTHAM_THULY.Where(x => x.DONID == DonID)
                    .OrderByDescending(x => x.NGAYTHULY).FirstOrDefault();
                if (tl != null)
                {
                    if (tl.QHPLTKID != null)
                    {
        ...
        private bool CheckValidQDVV()
        {
            ...
            else
            {
                ...
                {
                    DateTime NgayNhanPC = DateTime.Parse(hddNgayNhanPhanCong.Value, cul,
                        DateTimeStyles.NoCurrentDateDefault);
                    if (NgayQD < NgayNhanPC)
                    {
                        lbthongbaoQD.Text = "ngày mở phiên toà phải lớn hơn ngày phân công thẩm phán giải quyết " +
                                            hddNgayNhanPhanCong.Value + " !";
                        txtNgayMoPhienToaQD.Focus();
                        return false;
                    }
                }
            ...
            if (ddlQuyetdinh.SelectedItem.Text.Contains("15-HC"))
            {
                //if (rdbAnle_QD.SelectedValue == "")
                //{
                //    lbthongbaoQD.Text = "Bạn chưa chọn \"Áp dụng án lệ ?\"";
                //    return false;
                //}

                if (rdbVKSThamgia_QD.SelectedValue == "")
                {
                    lbthongbaoQD.Text = "Bạn chưa chọn \"Có VKS tham gia ?\"";
                    return false;
                }

                if (rdbYeucauKK_QD.SelectedValue == "")
                {
                    lbthongbaoQD.Text = "Bạn chưa chọn \"Xử lý yêu cầu khởi kiện ?\"";
                    return false;
                }

                if (rdbKiennghiCQNN_QD.SelectedValue == "")
                {
                    lbthongbaoQD.Text =
                        "Bạn chưa chọn \"Kiến nghị với CQNN, người đứng đầu CQNN xem xét trách nhiệm của CQNN, người có thẩm quyền của CQNN ?\"";
                    return false;
                }

                if (rdbDoithoai_QD.SelectedValue == "")
                {
                    lbthongbaoQD.Text = "Bạn chưa chọn \"Có đối thoại ?\"";
                    return false;
                }

                if (rdbBoithuong_QD.SelectedValue == "")
                {
                    lbthongbaoQD.Text = "Bạn chưa chọn \"Giải quyết yêu cầu bồi thường thiệt hại ?\"";
                    return false;
                }
            }
            ...
            {
                DateTime ngayBA = DateTime.Parse(this.txtNgaytuyenan.Text.Trim(), cul,
                    DateTimeStyles.NoCurrentDateDefault);
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                ...
                if (CheckID > 0)
                {
                    Decimal CurrBanAnId = (string.IsNullOrEmpty(hddBanAnID.Value))
                        ? 0
                        : Convert.ToDecimal(hddBanAnID.Value);
            ...
            {
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage",
                    "alert('" + " Chưa chọn số án lệ áp dụng. Hãy kiểm tra lại. " + "')", true);
            ...
            decimal ID = Convert.ToDecimal(hddid.Value);
            DM_QUYETDINH_VUAN_KETTHUC oBL = new DM_QUYETDINH_VUAN_KETTHUC();
            DataTable oDT = oBL.DGLIST_BAQD_QUYETDINH_KETTHUC_ST(ENUM_LOAIVUVIEC_NUMBER.AN_HANHCHINH, DONID);
            ...
            //if (oND != null)
            //{
            //    AHC_TONGDAT oTD = dt.AHC_TONGDAT.Where(x => x.DONID == oND.DONID && x.MAPID == oND.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AHC_SOTHAM_QUYETDINH).FirstOrDefault();
            //    if (oTD != null)
            //    {
        ...
        private bool CheckValid()
        {
            ...
            DateTime dNgayMoPhienToa = (String.IsNullOrEmpty(txtNgaymophientoa.Text.Trim()))
                ? DateTime.MinValue
                : DateTime.Parse(this.txtNgaymophientoa.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
            ...
            DateTime NgayThuLy = hddNgayThuLy.Value == ""
                ? DateTime.MinValue
                : DateTime.Parse(hddNgayThuLy.Value, cul, DateTimeStyles.NoCurrentDateDefault);
            ...
            DateTime dNgayTA = (String.IsNullOrEmpty(txtNgaytuyenan.Text.Trim()))
                ? DateTime.MinValue
                : DateTime.Parse(this.txtNgaytuyenan.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
            ...
            if (txtNgayhieuluc.Text.Trim() != "")
            {
                ...
                DateTime dNgayHieuLuc = (String.IsNullOrEmpty(txtNgayhieuluc.Text.Trim()))
                    ? DateTime.MinValue
                    : DateTime.Parse(this.txtNgayhieuluc.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
            ...
            if (rdbYeucauKK.SelectedValue == "")
            {
                lstErr.Text = "Bạn chưa chọn \"Xử lý yêu cầu khởi kiện ?\"";
                return false;
            }

            if (rdbKiennghiCQNN.SelectedValue == "")
            {
                lstErr.Text =
                    "Bạn chưa chọn \"Kiến nghị với CQNN, người đứng đầu CQNN xem xét trách nhiệm của CQNN, người có thẩm quyền của CQNN ?\"";
                return false;
            ...
            if (rdbDoithoai.SelectedValue == "")
            {
                lstErr.Text = "Bạn chưa chọn \"Có đối thoại ?\"";
                return false;
            }

            if (rdbBoithuong.SelectedValue == "")
            {
                lstErr.Text = "Bạn chưa chọn \"Giải quyết yêu cầu bồi thường thiệt hại ?\"";
                return false;
            }
        ...
        private void LoadCombobox()
        {
            ...
            //Load QHPL Thống kê.
            ddlQHPLTK.DataSource = dt.DM_QHPL_TK.Where(x => x.STYLES == ENUM_QHPLTK.HANHCHINH && x.ENABLE == 1)
                .OrderBy(y => y.ARRTHUTU).ToList();
            ...
            ddlQHPLQDVV.DataSource = dt.DM_QHPL_TK.Where(x => x.STYLES == ENUM_QHPLTK.HANHCHINH && x.ENABLE == 1)
                .OrderBy(y => y.ARRTHUTU).ToList();
            ...
            //decimal ID = Convert.ToDecimal(ddlQuyetdinh.SelectedValue);
            ddlLoaiQD.DataSource = dt.DM_QD_LOAI.Where(x => x.HIEULUC == 1 && x.ISHANHCHINH == 1).OrderBy(y => y.THUTU)
                .ToList();
            ...
            decimal DonID = Convert.ToDecimal(hddDonID.Value);
            AHC_SOTHAM_THULY tl = dt.AHC_SOTHAM_THULY.Where(x => x.DONID == DonID).OrderByDescending(x => x.NGAYTHULY)
                .FirstOrDefault();
            if (tl != null)
            {
                try
            ...
            string current_id = Session[ENUM_LOAIAN.AN_HANHCHINH] + "";
            decimal DONID = Convert.ToDecimal(current_id);
            AHC_SOTHAM_HDXX oHD = dt.AHC_SOTHAM_HDXX
                .Where(x => x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN && x.DONID == DONID)
                .OrderByDescending(x => x.NGAYPHANCONG).FirstOrDefault();
            if (oHD != null)
            {
        ...
        protected void cmdUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                KHOBAQD_BL ahc = new KHOBAQD_BL();
                bool isExist = ahc.IsExistKHOBADQ(0, DONID, 2,ENUM_LOAIVUVIEC_TEXT.AN_HANHCHINH);
                if (isExist)
                {
                    lstErr.Text = "Vụ việc đã được đồng bộ. Phải thu hồi đồng bộ trước khi chỉnh sửa/xóa";
                    return;
                ...
                AHC_SOTHAM_BANAN_FILE oTF = new AHC_SOTHAM_BANAN_FILE();
                List<AHC_SOTHAM_BANAN> lst = dt.AHC_SOTHAM_BANAN.Where(x => x.DONID == DONID).ToList();
                AHC_SOTHAM_BANAN oND;
                if (lst.Count == 0)
                {
                    Decimal soDuongSuChuaXacThuc = dt.AHC_DON_DUONGSU.Count(x => x.DONID == DONID && x.XACTHUC_DLDCQG == 0 && x.QUOCTICHID == 2);
                    if (soDuongSuChuaXacThuc > 0)
                    ...
                    oND = new AHC_SOTHAM_BANAN();
                }
                else
                {
                    oND = lst[0];
                    // khong duoc xoa khi da Tong dat - 29/11/2025 vnpt check
                    AHC_TONGDAT oTD2 = dt.AHC_TONGDAT.Where(x => x.DONID == oND.DONID && x.MAPID == oND.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AHC_SOTHAM_BANAN).FirstOrDefault();
                    if (oTD2 != null)
                    {
                ...
                oND.NGAYMOPHIENTOA = (String.IsNullOrEmpty(txtNgaymophientoa.Text.Trim()))
                    ? (DateTime?)null
                    : DateTime.Parse(this.txtNgaymophientoa.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
                oND.NGAYTUYENAN = (String.IsNullOrEmpty(txtNgaytuyenan.Text.Trim()))
                    ? (DateTime?)null
                    : DateTime.Parse(this.txtNgaytuyenan.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
                oND.NGAYHIEULUC = (String.IsNullOrEmpty(txtNgayhieuluc.Text.Trim()))
                    ? (DateTime?)null
                    : DateTime.Parse(this.txtNgayhieuluc.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
                ...
                oND.ISVKSTHAMGIA = rdbVKSThamgia.SelectedValue == ""
                    ? 0
                    : Convert.ToDecimal(rdbVKSThamgia.SelectedValue);
                oND.TK_ISANLE = ddlCBBA_Anle.SelectedValue == "0" ? 0 : 1;
                ...
                oND.TK_QUAHAN_CHUQUAN =
                    rdNNChuQuan.SelectedValue == "" ? 0 : Convert.ToDecimal(rdNNChuQuan.SelectedValue);
                oND.TK_QUAHAN_KHACHQUAN = rdNNKhachQuan.SelectedValue == ""
                    ? 0
                    : Convert.ToDecimal(rdNNKhachQuan.SelectedValue);

                oND.TK_ISKIENCOQUANNN = rdbKiennghiCQNN.SelectedValue == ""
                    ? 0
                    : Convert.ToDecimal(rdbKiennghiCQNN.SelectedValue);
                oND.TK_ISDOITHOAI = rdbDoithoai.SelectedValue == "" ? 0 : Convert.ToDecimal(rdbDoithoai.SelectedValue);
                oND.TK_ISBOITHUONG =
                    rdbBoithuong.SelectedValue == "" ? 0 : Convert.ToDecimal(rdbBoithuong.SelectedValue);
                oND.TK_SOTIEN = (String.IsNullOrEmpty(txtSotien.Text + ""))
                    ? 0
                    : Convert.ToDecimal(txtSotien.Text.Replace(".", ""));
                try
                {
                    if (hddFilePath.Value != "")
                    {
                        ...
                        dt.AHC_SOTHAM_BANAN_FILE.Add(oTF);
                        dt.SaveChanges();
                        //}
                ...
                if (rdbYeucauKK.SelectedValue == "0")
                {
                    oND.TK_BACYEUCAUKK = 1;
                    oND.TK_CHAPNHAN1PHAN = 0;
                    oND.TK_CHAPNHANTOANBO = 0;
                }

                if (rdbYeucauKK.SelectedValue == "1")
                {
                    oND.TK_CHAPNHAN1PHAN = 1;
                    oND.TK_BACYEUCAUKK = 0;
                    oND.TK_CHAPNHANTOANBO = 0;
                }

                if (rdbYeucauKK.SelectedValue == "2")
                {
                    oND.TK_CHAPNHANTOANBO = 1;
                    oND.TK_BACYEUCAUKK = 0;
                    oND.TK_CHAPNHAN1PHAN = 0;
                }
                ...
                if (!String.IsNullOrEmpty(txtNgayQD.Text.Trim()))
                    STTQD = oBL.GETFILENEWTT(Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]),
                        ENUM_GIAIDOANVUAN.SOTHAM, NgayBA.Year, 1);
                else
                {
                    Decimal? a = null;
                    STTQD = oBL.GETFILENEWTT(Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]),
                ...
                AHC_DON oDon = dt.AHC_DON.Where(x => x.ID == DONID).FirstOrDefault();
                //UploadFileID(oDon, FileID, "22-HC", STTQD);
                if (lst.Count == 0)
                {
                    FileID = 0;
                }
                ...
                var rFileID = UploadFileID(oDon, FileID, "22-HC", STTQD);
                if (rFileID > 0)
                {
                    oND.FILEID = rFileID;
                    dt.SaveChanges();
                ...
                if(oND.NGAYHIEULUC != null)
                {
                    ...
                    temp_congbo.MAVUAN = oDon.MAVUVIEC;
                    temp_congbo.VUVIECID = oND.DONID;
        ...
        protected void dgFile_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "Xoa":
                    ...
                    AHC_SOTHAM_BANAN_FILE oT = dt.AHC_SOTHAM_BANAN_FILE.Where(x => x.ID == ND_id).FirstOrDefault();
                    if (oT.NOIDUNG == null)
                    {
                    ...
                    AHC_FILE ohcF = dt.AHC_FILE.Where(x => x.DONID == oT.DONID && x.TENFILE == oT.TENFILE).FirstOrDefault();
                    if (ohcF != null)
                    {
                        dt.AHC_FILE.Remove(ohcF);
                        dt.SaveChanges();
                    }
                    dt.AHC_SOTHAM_BANAN_FILE.Remove(oT);

                    dt.SaveChanges();
                    LoadFile();
                    break;
                case "Download":
                    AHC_SOTHAM_BANAN_FILE oND = dt.AHC_SOTHAM_BANAN_FILE.Where(x => x.ID == ND_id).FirstOrDefault();
                    if (oND.NOIDUNG != null && oND.QT_FILE_ID == null)
                    {
                        var cacheKey = Guid.NewGuid().ToString("N");
        ...
        protected void cmdLuatUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dt.AHC_SOTHAM_BANAN_DIEULUAT.Where(x => x.DONID == DONID && x.DIEULUATID == DIEUID).ToList().Count >
                    0)
                {
                ...
                AHC_SOTHAM_BANAN_DIEULUAT oT = new AHC_SOTHAM_BANAN_DIEULUAT();
                oT.DONID = DONID;
                ...
                dt.AHC_SOTHAM_BANAN_DIEULUAT.Add(oT);
                dt.SaveChanges();
                LoadDieuLuat();
        ...
        protected void dgDieuLuat_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "Xoa":
                    ...
                    AHC_SOTHAM_BANAN_DIEULUAT oT = dt.AHC_SOTHAM_BANAN_DIEULUAT.Where(x => x.ID == ND_id)
                        .FirstOrDefault();
                    dt.AHC_SOTHAM_BANAN_DIEULUAT.Remove(oT);
                    dt.SaveChanges();
                    LoadDieuLuat();
                    break;
            }
            ...
            try
            {
                ...
                if (dgAnPhi.Items.Count > 0)
                {
                    foreach (DataGridItem oItem in dgAnPhi.Items)
                    {
                        ...
                        {
                            DateTime NgayNhanBA;
                            if (DateTime.TryParse(txtNgaynhanbanan.Text, cul, DateTimeStyles.NoCurrentDateDefault,
                                    out NgayNhanBA))
                            {
                ...
                foreach (DataGridItem oItem in dgAnPhi.Items)
                {
                    ...
                    TextBox txtNgaynhanbanan = (TextBox)oItem.FindControl("txtNgaynhanbanan");
                    List<AHC_SOTHAM_BANAN_ANPHI> lst = dt.AHC_SOTHAM_BANAN_ANPHI
                        .Where(x => x.DONID == DONID && x.DUONGSU == DSID).ToList();
                    if (lst.Count == 0)
                    {
                        AHC_SOTHAM_BANAN_ANPHI oT = new AHC_SOTHAM_BANAN_ANPHI();
                        oT.DONID = DONID;
                        oT.DUONGSU = DSID;
                        ...
                        oT.NGAYNHANAN = (String.IsNullOrEmpty(txtNgaynhanbanan.Text.Trim()))
                            ? (DateTime?)null
                            : DateTime.Parse(txtNgaynhanbanan.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
                        oT.NGAYTAO = DateTime.Now;
                        oT.NGUOITAO = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                        oT.TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                        dt.AHC_SOTHAM_BANAN_ANPHI.Add(oT);
                        dt.SaveChanges();
                    }
                    else
                    {
                        AHC_SOTHAM_BANAN_ANPHI oT = lst[0];
                        oT.DONID = DONID;
                        oT.DUONGSU = DSID;
                        oT.MIENANPHI = chkMien.Checked == true ? 1 : 0;
                        oT.ANPHI = txtAnphi.Text == "" ? 0 : Convert.ToDecimal(txtAnphi.Text.Replace(".", ""));
        ...
        protected void cmdTGTT_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (DataGridItem oItem in dgTGTT.Items)
                {
                    ...
                    List<AHC_SOTHAM_BANAN_TGTT> lst = dt.AHC_SOTHAM_BANAN_TGTT
                        .Where(x => x.DONID == DONID && x.THAMGIATOTUNGID == DSID).ToList();
                    if (lst.Count == 0)
                    {
                        AHC_SOTHAM_BANAN_TGTT oT = new AHC_SOTHAM_BANAN_TGTT();
                        oT.DONID = DONID;
                        oT.THAMGIATOTUNGID = DSID;
                        oT.ISTHAMGIA = chkThamgiaTGTT.Checked == true ? 1 : 0;
                        oT.NGAYNHANBANAN = (String.IsNullOrEmpty(txtNgayTGTT.Text.Trim()))
                            ? (DateTime?)null
                            : DateTime.Parse(txtNgayTGTT.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
                        dt.AHC_SOTHAM_BANAN_TGTT.Add(oT);
                        dt.SaveChanges();
                    }
                    else
                    {
                        AHC_SOTHAM_BANAN_TGTT oT = lst[0];
                        oT.DONID = DONID;
                        oT.THAMGIATOTUNGID = DSID;
                        oT.ISTHAMGIA = chkThamgiaTGTT.Checked == true ? 1 : 0;
                        oT.NGAYNHANBANAN = (String.IsNullOrEmpty(txtNgayTGTT.Text.Trim()))
        ...
        protected void rdbBoithuong_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (rdbBoithuong.SelectedValue == "0" || rdbBoithuong_QD.SelectedValue == "0")
            {
                txtSotien.Enabled = false;
                txtSotien_QD.Enabled = false;
            }
            else
            {
                txtSotien.Enabled = true;
                txtSotien_QD.Enabled = true;
            }
        ...
        protected void cmdHuyBanAn_Click(object sender, EventArgs e)
        {
            ...
            decimal DonID = Session[ENUM_LOAIAN.AN_HANHCHINH] + "" == ""
                ? 0
                : Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HANHCHINH] + "");
            //reset_TENVUVIEC(DonID);

            List<AHC_SOTHAM_BANAN> banans = dt.AHC_SOTHAM_BANAN.Where(x => x.DONID == DonID).ToList();
            if (banans.Count > 0)
            {
                ...
                if (oBL.HISTORY_ALLDATA_BY_VUANID(DonID, 6, Session[ENUM_SESSION.SESSION_USERID] + "", strUserName,
                        "Bản án Sơ thẩm an Hành chính", "Xóa", json) == false)
                {
                ...
                //Xoa Bản án So thẩm
                dt.AHC_SOTHAM_BANAN.RemoveRange(banans);
            }

            // Xóa thông tin file đính kèm
            List<AHC_SOTHAM_BANAN_FILE> files = dt.AHC_SOTHAM_BANAN_FILE.Where(x => x.DONID == DonID).ToList();
            if (files.Count > 0)
            {
                ...
                dt.AHC_SOTHAM_BANAN_FILE.RemoveRange(files);
            }

            // Xóa điều luật áp dụng, tội danh
            List<AHC_SOTHAM_BANAN_DIEULUAT> dieuLuats =
                dt.AHC_SOTHAM_BANAN_DIEULUAT.Where(x => x.DONID == DonID).ToList();
            if (dieuLuats.Count > 0)
            {
                dt.AHC_SOTHAM_BANAN_DIEULUAT.RemoveRange(dieuLuats);
            }

            // Xóa thông tin án phí
            List<AHC_SOTHAM_BANAN_ANPHI> anPhis = dt.AHC_SOTHAM_BANAN_ANPHI.Where(x => x.DONID == DonID).ToList();
            if (anPhis.Count > 0)
            {
                dt.AHC_SOTHAM_BANAN_ANPHI.RemoveRange(anPhis);
            }

            // Xóa thông tin người tham gia tố tụng
            List<AHC_SOTHAM_BANAN_TGTT> tGTTs = dt.AHC_SOTHAM_BANAN_TGTT.Where(x => x.DONID == DonID).ToList();
            if (tGTTs.Count > 0)
            {
                dt.AHC_SOTHAM_BANAN_TGTT.RemoveRange(tGTTs);
            }

            // Xóa file tống đạt bản án
            decimal BieuMauID = 0;
            DM_BIEUMAU bm = dt.DM_BIEUMAU.Where(x => x.MABM == "22-HC").FirstOrDefault();
            if (bm != null)
            {
                BieuMauID = bm.ID;
            }

            AHC_FILE file = dt.AHC_FILE.Where(x =>
                    x.DONID == DonID && x.BIEUMAUID == BieuMauID && x.MAGIAIDOAN == ENUM_GIAIDOANVUAN.SOTHAM)
                .FirstOrDefault();
            if (file != null)
            {
                dt.AHC_FILE.Remove(file);
            }
        ...
        private void ResetControl_Banan()
        {
            ...
            rdbYeucauKK.ClearSelection();
            ddlYeutonuocngoai.SelectedIndex = 0;
            rdbKiennghiCQNN.ClearSelection();
            rdbDoithoai.ClearSelection();
            rdbBoithuong.ClearSelection();
            txtSotien.Text = "";
            //txtNguoiKy.Text = "";
        ...
        private void UploadFileID(AHC_DON oDon, string strMaBieumau, AHC_SOTHAM_BANAN_FILE fileDinhKem)
        {
            AHC_DON_BL oBL = new AHC_DON_BL();
            decimal IDBM = 0;
            string strTenBM = "";
            ...
            AHC_FILE objFile = dt.AHC_FILE.Where(x =>
                x.DONID == oDon.ID && x.TOAANID == oDon.TOAANID && x.MAGIAIDOAN == oDon.MAGIAIDOAN &&
                x.BIEUMAUID == IDBM).FirstOrDefault();
            if (objFile == null)
            {
                isNew = true;
                objFile = new AHC_FILE();
            }
        ...
        {
            decimal ID = Session[ENUM_LOAIAN.AN_HANHCHINH] + "" == ""
                ? 0
                : Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HANHCHINH]);
            AHC_SOTHAM_THULY oT = dt.AHC_SOTHAM_THULY.Where(x => x.DONID == ID).FirstOrDefault();
