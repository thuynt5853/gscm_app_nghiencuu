...
namespace WEB.GSTP.QLAN.AHC.Sotham
{
    public partial class TrungCauGD : System.Web.UI.Page
    {
        ...
        AHC_TRUNGCAU_GIAMDINH obj = new AHC_TRUNGCAU_GIAMDINH();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                VuAnID = (String.IsNullOrEmpty(Session[ENUM_LOAIAN.AN_HANHCHINH] + "")) ? 0 : Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HANHCHINH] + "");
                hddMaGiaiDoan.Value = MaGiaiDoan + "";
                hddVuAnID.Value = VuAnID + "";
                AHC_TRUNGCAU_GIAMDINH oT = dt.AHC_TRUNGCAU_GIAMDINH.Where(x => x.VUANID == VuAnID && x.MAGIAIDOAN == MaGiaiDoan).SingleOrDefault();
                if (oT != null)
                {
                    LoadInfo();
                }
                ...
                AHC_DON oDT = dt.AHC_DON.Where(x => x.ID == VuAnID).FirstOrDefault();
                ...
                List<AHC_SOTHAM_THULY> lstCount = dt.AHC_SOTHAM_THULY.Where(x => x.DONID == VuAnID).ToList();
                if (lstCount.Count == 0)
                {
                ...
                List<AHC_DON_THAMPHAN> lstTP = dt.AHC_DON_THAMPHAN.Where(x => x.DONID == VuAnID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).ToList();
                if (lstTP.Count == 0)
                {
                ...
                string Result = new AHC_CHUYEN_NHAN_AN_BL().Check_NhanAn(VuAnID, StrMsg, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]));
                if (Result != "")
                {
                ...
                //    int countItem = dt.AHC_DON_THAMPHAN.Count(s => s.THUKYID == CurrentUserId && s.DONID == VuAnID && s.MAVAITRO == "VTTP_GIAIQUYETSOTHAM");
                //    if (countItem > 0)
                //    {
                //        //được gán 
                //    }
        ...
        protected void cmdUpdateBottom_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                obj = dt.AHC_TRUNGCAU_GIAMDINH.Where(x => x.VUANID == mavuan && x.MAGIAIDOAN == MaGiaidoan).SingleOrDefault();
            }
            catch (Exception ex)
            {
