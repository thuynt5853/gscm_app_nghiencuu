...
namespace WEB.GSTP.QLAN.AHC.Sotham
{
    public partial class ThuLy : System.Web.UI.Page
    {
        ...
        private decimal loaian = Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_HANHCHINH);
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    DataTable oDT = oBL.AHC_SOTHAM_THULY_GETLIST(ID);
        ...
        private void LoadBiDon(decimal donID)
        {
            ...
            //Load bị đơn 
            List<AHC_DON_DUONGSU> tb = dt.AHC_DON_DUONGSU.Where(x => x.DONID == donID && x.TUCACHTOTUNG_MA == "BIDON").OrderByDescending(x => x.ISDAIDIEN).ToList();
            if (tb.Count > 0)
            {
        ...
        private void CheckShowCommand(decimal DonID)
        {
            ...
            AHC_SOTHAM_QUYETDINH qd = dt.AHC_SOTHAM_QUYETDINH.Where(x => x.DONID == DonID && (x.LOAIQDID == 3 || x.LOAIQDID == 10 || x.LOAIQDID == 11 || x.LOAIQDID == 21)).FirstOrDefault();
            if (qd != null)
            {
        ...
        private decimal SetNewSoThuLy()
        {
            ...
            string sothulymoi = oSTBL.GET_STL_NEW(DonViID, "AHC", ngaythuly).ToString();
        ...
        {
            Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
            ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();//So thong bao
            ...
            txtSothongbao.Text = oSTBL.GET_STBTL_NEW(DonViID, "AHC", ngaythongbao).ToString();
        }
        private void ResetControls()
        {
            ...
            LoadNguyenDon(Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HANHCHINH] + ""));
            LoadBiDon(Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HANHCHINH] + ""));
            decimal donID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HANHCHINH] + "");
            AHC_SOTHAM_THULY tl = dt.AHC_SOTHAM_THULY.Where(x => x.DONID == donID).FirstOrDefault();
            if (tl != null)
            {
        ...
        private bool CheckValid()
        {
            ...
            decimal DONID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HANHCHINH]);
            List<AHC_DON_XULY> lstGQD = dt.AHC_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 5).ToList();
            if (lstGQD.Count > 0)
            {
                AHC_DON_XULY oDXL = lstGQD[0];
            ...
            //    Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
            //    ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
            //    Decimal CheckID = oSTBL.CheckSoTLTheoLoaiAn(DonViID, "AHC", sothuly, ngaythuly);
            //    if (CheckID > 0)
            //    {
            //        Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);

            //        String strMsg = "";
            //        String STTNew = oSTBL.GET_STL_NEW(DonViID, "AHC", ngaythuly).ToString();
            ...
            if (!String.IsNullOrEmpty(txtNgaythuly.Text))
            {
                ...
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal CheckID = oSTBL.CheckSoTBTLTheoLoaiAn(DonViID, "AHC", sothongbao, ngaythuly);
                if (CheckID > 0)
                {
                    Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
                    String strMsg = "";
                    String STTNew = oSTBL.GET_STBTL_NEW(DonViID, "AHC", ngaythuly).ToString();
                    if (CheckID != CurrThuLyID)
                    {
                        //lbthongbao.Text = "Số thụ lý này đã có!";
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                AHC_SOTHAM_THULY oND;
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    oND = new AHC_SOTHAM_THULY();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.AHC_SOTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
                ...
                AHC_DON_BL objDBL = new AHC_DON_BL();
                
                AHC_SOTHAM_BL oBL = new AHC_SOTHAM_BL();
                DataTable oDT = oBL.AHC_SOTHAM_THULY_GETLIST(DONID);

                STPT_QUANLY_SOTHULY oQLSTL = new STPT_QUANLY_SOTHULY();
                if (oQLSTL.update_STPT_QUANLY_SOTHULY(2, 6, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), ((DateTime)oND.NGAYTHULY).ToString("dd/MM/yyyy", cul), Regex.Match(oND.SOTHULY, @"\d+").Value) == false)
                {
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    else
                    {
                        ...
                        oND.MATHULY = "S" + ENUM_LOAIVUVIEC.AN_HANHCHINH + Session[ENUM_SESSION.SESSION_MADONVI] + oND.TT.ToString();
                        oND.NGAYTAO = DateTime.Now;
                        oND.NGUOITAO = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                        // update 130825
                        oND.TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                        dt.AHC_SOTHAM_THULY.Add(oND);
                        dt.SaveChanges();
                        //Cập nhật lại trạng thái vụ việc

                        oDon.MAGIAIDOAN = ENUM_GIAIDOANVUAN.SOTHAM;
                        decimal rFileID = UploadFileID(oDon, FileID, "06-HC", oND.SOTHONGBAO);
                ...
                AHC_DON_DUONGSU ndcu = dt.AHC_DON_DUONGSU.Where(x => x.DONID == DONID && x.ISDAIDIEN == 1 && x.TUCACHTOTUNG_MA == "NGUYENDON").FirstOrDefault();
                AHC_DON_DUONGSU bdcu = dt.AHC_DON_DUONGSU.Where(x => x.DONID == DONID && x.ISDAIDIEN == 1 && x.TUCACHTOTUNG_MA == "BIDON").FirstOrDefault();
                AHC_DON_DUONGSU ndmoi = dt.AHC_DON_DUONGSU.Where(x => x.ID == idNDmoi).FirstOrDefault();
                AHC_DON_DUONGSU bdmoi = dt.AHC_DON_DUONGSU.Where(x => x.ID == idBDmoi).FirstOrDefault();
                if (idNDmoi != ndcu.ID)
                {
                    ndcu.ISDAIDIEN = 0;
                    ndmoi.ISDAIDIEN = 1;
                ...
                oBLs.CHECK_ENABLE_THULY(DONID, "6");
            }
            catch (Exception ex)
            {
        ...
        {
            AHC_SOTHAM_THULY oND = dt.AHC_SOTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "Sua":
                    lbthongbao.Text = "";
                    AHC_SOTHAM_THULY oND1 = dt.AHC_SOTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
                    // khong duoc SỬA khi da Tong dat - 29/11/2025 vnpt check
                    AHC_TONGDAT oTD = dt.AHC_TONGDAT.Where(x => x.DONID == oND1.DONID && x.MAPID == oND1.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AHC_SOTHAM_THULY).FirstOrDefault();
                    if (oTD != null)
                    {
                ...
                case "Xoa":
                    AHC_SOTHAM_THULY oND2 = dt.AHC_SOTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
                    // khong duoc xoa khi da Tong dat - 29/11/2025 vnpt check
                    AHC_TONGDAT oTD1 = dt.AHC_TONGDAT.Where(x => x.DONID == oND2.DONID && x.MAPID == oND2.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AHC_SOTHAM_THULY).FirstOrDefault();
                    if (oTD1 != null)
                    {
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                AHC_CHUYEN_NHAN_AN_BL _chuyenNhanBl = new AHC_CHUYEN_NHAN_AN_BL();
                ...
                List<AHC_SOTHAM_THULY> check_button_xoa = dt.AHC_SOTHAM_THULY.Where(x => x.DONID == DONID).ToList();
        ...
        private void txtQuanhephapluat_name(AHC_SOTHAM_THULY oT)
        {
            if (oT.QUANHEPHAPLUAT_NAME != null)
            {
                txtQuanhephapluat.Text = oT.QUANHEPHAPLUAT_NAME;
        ...
        protected void txtNgayThuLy_TextChanged(object sender, EventArgs e)
        {
            ...
            //    STPT_QUANLY_SOTHULY oQLSTL = new STPT_QUANLY_SOTHULY();
            //    DataTable dtStl = oQLSTL.get_STPT_QUANLY_SOTHULY(2, 6, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtNgaythuly.Text);
            //    if (dtStl != null)
            //    {
            ...
            //    string check_ngaythulycuoi_trongnam = oQLSTL.get_LATEST_DATE_IN_SOTHULY(2, 6, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtNgaythuly.Text);
        ...
        protected void btnSaveLydoXoa_Insert(object sender, EventArgs e)
        {
            ...
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            AHC_SOTHAM_THULY oND = dt.AHC_SOTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
            decimal DonID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HANHCHINH] + "");
            ...
            if (check_button_xoa.Count < 2 && oND != null)
            {
                ...
                AHC_SOTHAM_BANAN ba = dt.AHC_SOTHAM_BANAN.Where(x => x.DONID == DonID).FirstOrDefault<AHC_SOTHAM_BANAN>();
                if (ba != null)
                {
                ...
                AHC_SOTHAM_QUYETDINH qd = dt.AHC_SOTHAM_QUYETDINH.Where(x => x.DONID == DonID).FirstOrDefault<AHC_SOTHAM_QUYETDINH>();
                if (qd != null)
                {
                ...
                AHC_SOTHAM_HDXX hdxx = dt.AHC_SOTHAM_HDXX.Where(x => x.DONID == DonID).FirstOrDefault<AHC_SOTHAM_HDXX>();
                if (hdxx != null)
                {
                    lbthongbao.Text = "Vụ việc đã có thông tin người tiến hành tố tụng. Không được xóa.";
                    return;
                }
                // Kiểm tra Thông báo về việc tiếp cận công khai chứng cứ và đối thoại
                AHC_SOTHAM_HOAGIAI hg = dt.AHC_SOTHAM_HOAGIAI.Where(x => x.DONID == DonID).FirstOrDefault<AHC_SOTHAM_HOAGIAI>();
                if (hg != null)
                {
                    lbthongbao.Text = "Vụ việc đã có thông báo về việc tiếp cận công khai chứng cứ và đối thoại. Không được xóa.";
                    return;
                }
                // Kiểm tra phân công thẩm phán giải quyết
                AHC_DON_THAMPHAN gqst = dt.AHC_DON_THAMPHAN.Where(x => x.DONID == DonID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).FirstOrDefault<AHC_DON_THAMPHAN>();
                if (gqst != null)
                {
            ...
            //Xoa Thụ lý Sơ thẩm
            dt.AHC_SOTHAM_THULY.Remove(oND);
            dt.SaveChanges();
            if (FileID > 0)
            {
