...
namespace WEB.GSTP.QLAN.AHC
{
    public partial class Sauxetxu : System.Web.UI.Page
    {
        ...
        private void LoadGrid()
        {
            ...
            DataTable oDT = oBL.AHC_RUTKN(vDonViID, txtMaVuViec.Text, txtTenVuViec.Text, txtSoQDBA.Text, txtTenDuongSu.Text, dFrom, dTo, Convert.ToDecimal(rdbTrangthai.SelectedValue), pageIndex, pageSize);
            int Total = 0;
        ...
        protected void cmdCapnhat_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    decimal vuViecID = 0; AHC_SAUXETXU obj; bool isNew = false; AHC_DON don;
                    foreach (DataRow row in dsSelected.Rows)
                    {
                        vuViecID = Convert.ToDecimal(row["ID"].ToString());
                        obj = dt.AHC_SAUXETXU.Where(x => x.VUANID == vuViecID).FirstOrDefault<AHC_SAUXETXU>();
                        if (obj == null)
                        {
                            isNew = true;
                            obj = new AHC_SAUXETXU();
                        ...
                        obj.VUANID = vuViecID;
                        don = dt.AHC_DON.Where(x => x.ID == vuViecID).FirstOrDefault<AHC_DON>();
                        if (don != null)
                        {
        ...
        private void LoadGridSelected()
        {
            ...
            if (dsSelected != null && dsSelected.Rows.Count > 0)
            {
                ...
                AHC_SAUXETXU sx = dt.AHC_SAUXETXU.Where(x => x.VUANID == DonID).FirstOrDefault<AHC_SAUXETXU>();
                if (sx != null)
                {
        ...
        public void loadedit(decimal ID, String MAGIAIDOAN)
        {
            ...
            if (MAGIAIDOAN == "Sơ thẩm")
                oND = dt.AHC_SAUXETXU.Where(x => x.VUANID == ID && x.ST_ISRUTKN == 1).FirstOrDefault();
            else
                oND = dt.AHC_SAUXETXU.Where(x => x.VUANID == ID && x.PT_ISRUTKN == 1).FirstOrDefault();

            if (oND != null)
            {
                if (dsSelected != null) dsSelected.Clear();
                AHC_DON oVA = dt.AHC_DON.Where(x => x.ID == oND.VUANID).FirstOrDefault();
