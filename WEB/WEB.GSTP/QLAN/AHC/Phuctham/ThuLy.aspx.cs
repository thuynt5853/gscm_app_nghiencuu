...
namespace WEB.GSTP.QLAN.AHC.Phuctham
{
    public partial class ThuLy : System.Web.UI.Page
    {
            ...
            try
            {
                if (!IsPostBack)
                {
                    ...
                    else
                    {
                        ...
                        DataTable oDT = oBL.AHC_PHUCTHAM_THULY_GETLIST(ID);
        ...
        private decimal SetNewSoThuLy()
        {
            ...
            string sothulymoi = oSTBL.GET_STL_NEW(DonViID, "AHC_PT", ngaythuly).ToString();
        ...
        void SetNewSoThongbao()
        {
            ...
            txtSothongbao.Text = oSTBL.GET_STBTL_NEW(DonViID, "AHC_PT", ngaythongbao).ToString();
        }
        ...
        private void LoadCombobox()
        {
            ...
            List<DM_DATAITEM> lstTHGN = dt.DM_DATAITEM.Where(x => x.GROUPID == oGTHTL.ID && (x.MA == "02" || x.MA == "03" || x.MA == "04" || x.MA == "10"  || x.MA == "AHS_PT_ThuLyLai")).ToList();
        ...
        private bool CheckValid()
        {
            ...
            //    Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
            //    ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
            //    Decimal CheckID = oSTBL.CheckSoTLTheoLoaiAn(DonViID, "AHC_PT", sothuly, ngaythuly);
            //    if (CheckID > 0)
            //    {
            //        Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);

            //        String strMsg = "";
            //        String STTNew = oSTBL.GET_STL_NEW(DonViID, "AHC_PT", ngaythuly).ToString();
            ...
            if (!String.IsNullOrEmpty(txtNgaythuly.Text))
            {
                ...
                Decimal DonViID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal CheckID = oSTBL.CheckSoTBTLTheoLoaiAn(DonViID, "AHC_PT", sothongbao, ngaythuly);
                if (CheckID > 0)
                {
                    Decimal CurrThuLyID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);
                    String strMsg = "";
                    String STTNew = oSTBL.GET_STBTL_NEW(DonViID, "AHC_PT", ngaythuly).ToString();
                    if (CheckID != CurrThuLyID)
                    {
                        //lbthongbao.Text = "Số thụ lý này đã có!";
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                AHC_PHUCTHAM_THULY oND;
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    oND = new AHC_PHUCTHAM_THULY();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.AHC_PHUCTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
                    if (oND.FILEID != null) FileID = (decimal)oND.FILEID;
                    // - vnpt 1/12/2025  check tong dat
                    AHC_TONGDAT oTD = dt.AHC_TONGDAT.Where(x => x.DONID == oND.DONID && x.MAPID == oND.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AHC_PHUCTHAM_THULY).FirstOrDefault();
                    if (oTD != null)
                    {
                ...
                oND.SOTHULY = ddlSothuly.SelectedValue + (ddlStlPhu != null ? ddlStlPhu.SelectedValue : "");
                ...
                oQLSTL.update_STPT_QUANLY_SOTHULY(3, 6, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), ((DateTime)oND.NGAYTHULY).ToString("dd/MM/yyyy", cul), Regex.Match(oND.SOTHULY, @"\d+").Value);

                
                AHC_PHUCTHAM_BL oBL = new AHC_PHUCTHAM_BL();
                DataTable oDT = oBL.AHC_PHUCTHAM_THULY_GETLIST(DONID);
        ...
        {
            AHC_PHUCTHAM_THULY oND = dt.AHC_PHUCTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
            if (oND != null)
            {
                ...
                if (stlphu.Length != 0 && ddlStlPhu != null)
                {
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                switch (e.CommandName)
                {
                    ...
                    case "Sua":
                        lbthongbao.Text = "";
                        AHC_PHUCTHAM_THULY oND1 = dt.AHC_PHUCTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
                        // khong duoc SỬA khi da Tong dat - 29/11/2025 vnpt check
                        AHC_TONGDAT oTD = dt.AHC_TONGDAT.Where(x => x.DONID == oND1.DONID && x.MAPID == oND1.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AHC_PHUCTHAM_THULY).FirstOrDefault();
                        if (oTD != null)
                        {
                    ...
                    case "Xoa":
                        AHC_PHUCTHAM_THULY oND2 = dt.AHC_PHUCTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
                        // khong duoc xoa khi da Tong dat - 29/11/2025 vnpt check
                        AHC_TONGDAT oTD1 = dt.AHC_TONGDAT.Where(x => x.DONID == oND2.DONID && x.MAPID == oND2.ID && x.MAP_TABLE == ENUM_MAP_TABLE.AHC_PHUCTHAM_THULY).FirstOrDefault();
                        if (oTD1 != null)
                        {
        ...
        protected void txtNgayThuLy_TextChanged(object sender, EventArgs e)
        {
            ...
            //    STPT_QUANLY_SOTHULY oQLSTL = new STPT_QUANLY_SOTHULY();
            //    DataTable dtStl = oQLSTL.get_STPT_QUANLY_SOTHULY(3, 6, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtNgaythuly.Text);
            //    if (dtStl != null)
            //    {
            ...
            //    string check_ngaythulycuoi_trongnam = oQLSTL.get_LATEST_DATE_IN_SOTHULY(3, 6, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtNgaythuly.Text);
        ...
        protected void btnSaveLydoXoa_Insert(object sender, EventArgs e)
        {
            ...
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            AHC_PHUCTHAM_THULY oND = dt.AHC_PHUCTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
            ...
            if (oND != null)
            {
                ...
                AHC_DON_THAMPHAN AHC_DON_THAMPHAN = dt.AHC_DON_THAMPHAN.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM).FirstOrDefault<AHC_DON_THAMPHAN>();
                if (AHC_DON_THAMPHAN != null)
                {
                    lbthongbao.Text = "Xóa không thành công! Do đang tồn tại thông tin phân công thẩm phán giải quyết.";
                    return;
                }
                AHC_PHUCTHAM_THAMGIATOTUNG AHC_PHUCTHAM_THAMGIATOTUNG = dt.AHC_PHUCTHAM_THAMGIATOTUNG.Where(x => x.DONID == DONID).FirstOrDefault<AHC_PHUCTHAM_THAMGIATOTUNG>();
                if (AHC_PHUCTHAM_THAMGIATOTUNG != null)
                {
                    lbthongbao.Text = "Xóa không thành công! Do đang tồn tại người tham gia tố tụng.";
                    return;
                }
                AHC_PHUCTHAM_HDXX AHC_PHUCTHAM_HDXX = dt.AHC_PHUCTHAM_HDXX.Where(x => x.DONID == DONID).FirstOrDefault<AHC_PHUCTHAM_HDXX>();
                if (AHC_PHUCTHAM_HDXX != null)
                {
                    lbthongbao.Text = "Xóa không thành công! Do đang tồn tại thông tin người tiến hành tố tụng.";
                    return;
                }
                AHC_PHUCTHAM_HOAGIAI AHC_PHUCTHAM_HOAGIAI = dt.AHC_PHUCTHAM_HOAGIAI.Where(x => x.DONID == DONID).FirstOrDefault<AHC_PHUCTHAM_HOAGIAI>();
                if (AHC_PHUCTHAM_HOAGIAI != null)
                {
                    lbthongbao.Text = "Xóa không thành công! Do đang tồn tại thông tin hòa giải.";
                    return;
                }
                AHC_PHUCTHAM_QUYETDINH AHC_PHUCTHAM_QUYETDINH = dt.AHC_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DONID).FirstOrDefault<AHC_PHUCTHAM_QUYETDINH>();
                if (AHC_PHUCTHAM_QUYETDINH != null)
                {
                ...
                dt.AHC_PHUCTHAM_THULY.Remove(oND);
                dt.SaveChanges();
                if (FileID > 0)
                {
                    try
