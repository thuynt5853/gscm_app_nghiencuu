    ...
    public partial class Danhsach : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                sDONID = Session[ENUM_LOAIAN.AN_HANHCHINH] + "";
                try
                {
                    SetDonGhep();
                    ...
                    AHC_DON_XULY oDXuly = dt.AHC_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 1).FirstOrDefault();
                    if (oDXuly != null)
                    {
                        AHC_CHUYEN_NHAN_AN ca = dt.AHC_CHUYEN_NHAN_AN.Where(x => x.VUANID == DONID).FirstOrDefault();
                        if (ca != null)
                        {
        ...
        void SetNewSoTB()
        {
            ...
            String STTNew = oSTBL.GET_STB_XLDon_NEW_V2(DonViID, "AHC", ngayTB).ToString();
        ...
        private void CheckQuyen(decimal DONID)
        {
            ...
            List<AHC_DON_THAMPHAN> lstCount = dt.AHC_DON_THAMPHAN.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETDON).ToList();
            if (lstCount.Count == 0)
            {
            ...
            //List<AHC_SOTHAM_THULY> lstTL = dt.AHC_SOTHAM_THULY.Where(x => x.DONID == DONID).ToList();
            //if (lstTL.Count > 0)
            //{
        ...
        {
            List<AHC_DON_XULY> lstXL = dt.AHC_DON_XULY.Where(x => x.DONID == ID && x.LOAIGIAIQUYET == 5).ToList();
            if (lstXL.Count == 0)
            {
                Cls_Comon.SetButton(cmdThemmoiAP, false);
                Cls_Comon.SetButton(cmdCapNhatAP, false);
        ...
        void LoadDropDuongSu(decimal DONID)
        {
            ...
            DataTable obj = oDSBL.AHC_DON_DUONGSU_ANPHI(DONID);
        ...
        {
            AHC_DON_XULY_BL oBL = new AHC_DON_XULY_BL();
        ...
        protected void rpt_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "Download":
                    ...
                    AHC_FILE oND = dt.AHC_FILE.Where(x => x.ID == CurrID).FirstOrDefault();
                    if (oND.TENFILE != "")
                    {
                ...
                case "Them":
                    ...
                    if (DonID > 0)
                    {
                        AHC_DON_DUONGSU dds = dt.AHC_DON_DUONGSU.Where(x => x.DONID == DonID && x.ISDAIDIEN == 1 && x.TUCACHTOTUNG_MA == "NGUYENDON").FirstOrDefault();
                        txtDuongSu.Text = dds.TENDUONGSU;
                    }
                    else
                    {
                        var a = (from d in dt.DON_DUONGSU_CHITIET
                                 join s in dt.AHC_DON_DUONGSU on d.DUONGSUID equals s.ID
                ...
                case "Xoa":
                    ...
                    AHC_DON_XULY oT = dt.AHC_DON_XULY.Where(x => x.ID == CurrID).FirstOrDefault();
                    ...
                    AHC_TONGDAT oTD = dt.AHC_TONGDAT.Where(x => x.DONID == DONID).FirstOrDefault();
                    if (oTD != null)
                    {
                    ...
                    decimal _VALUE = 0;
                    obj.DVCQG_THANH_TOAN_DELETE_XULY(CurrID, "6", ref _VALUE);
                    ...
                    if (_VALUE == 1)
                    {
                        ...
                        AHC_ANPHI oAnphi = dt.AHC_ANPHI.Where(x => x.DONID == oT.DONID).FirstOrDefault();
                        if (oAnphi != null)
                        {
                            ...
                            log.InsertLog(Session[ENUM_LOAIAN.AN_HANHCHINH].ToString(), CurrID.ToString(), MATHONGBAO, Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Delete", CONTENT_JSON, CONTENT_JSON_OLD);

                            dt.AHC_ANPHI.Remove(oAnphi);
                            //-----
                        }

                        dt.AHC_DON_XULY.Remove(oT);
                        ...
                        {
                            AHC_DON_XULY_BL oBL = new AHC_DON_XULY_BL();
                            DataTable oYC = oBL.AHC_GETALL_DON_YCBS(DonID, CurrID, 1, 5);
                            // update 080825: check rong
                ...
                case "BoSung":
                    ...
                    string link = "/QLAN/AHC/XuLyDon/Popup/pBoSungTaiLieu.aspx?ID=" + CurrID;
                ...
                case "TraLaiDon":
                    CurrID = Convert.ToDecimal(e.CommandArgument.ToString());
                    Cls_Comon.CallFunctionJS(this, this.GetType(), "LoadModalDiv();");
                    string linkPop = "/QLAN/AHC/XuLyDon/Popup/pTraLaiDon.aspx?ID=" + CurrID;
        ...
        {
            //DONGHEP.Visible = false;
            AHC_DON_XULY obj = dt.AHC_DON_XULY.Where(x => x.ID == CurrID).Single<AHC_DON_XULY>();
            if (obj != null)
            {
                ...
                {
                    AHC_FILE objFile = dt.AHC_FILE.Where(x => x.ID == obj.FILEID).FirstOrDefault();
                ...
                switch (bienphap)
                {
                    ...
                    default:
                        ...
                        //List<AHC_ANPHI> lstAP = dt.AHC_ANPHI.Where(x => x.DONID == obj.DONID).ToList();
                        //if (lstAP.Count > 0)
                        //{
                        //    AHC_ANPHI objAP = lstAP[0];
        ...
        protected void rpt_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            decimal DonID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HANHCHINH] + "");
            AHC_DON_XULY oND = dt.AHC_DON_XULY.FirstOrDefault(x => x.DONID == DonID);
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                // check quyền sửa xoá toà mới
                string toagiaiquyetID = e.Item.Cells[9].Text.Trim();
                string nguoiTao = e.Item.Cells[10].Text.Trim();
                string donviID = Session[ENUM_SESSION.SESSION_DONVIID]?.ToString();

                if (!String.IsNullOrEmpty(toagiaiquyetID) && !toagiaiquyetID.Equals(donviID)) // TH: khác toà
                {
                    if (donviID == oND?.TOA_GIAIQUYET_ID?.ToString()) // TH: toà đang đăng nhập là toà xử lý đơn
                    {
                        lbtXoa.Visible = true;
                        lblSua.Visible = true;
                    }
                ...
                //if (toagiaiquyetID != donviID && String.IsNullOrEmpty(nguoiTao))
                //{
                //    lbtXoa.Visible = false;
                //    lblSua.Visible = false;
                //}
                ...
                if (rowView["ID"] + "" != "")
                {
                    ...
                    AHC_DON_XULY oXL = dt.AHC_DON_XULY.Where(x => x.ID == idXuLy).FirstOrDefault();
                    decimal? DONCHITIETID = oXL.DON_CHITIETID;
                    decimal ISDONCHITIET = DON_ID == 0 ? 1 : 0;
                    AHC_DON_XULY_BL oBL = new AHC_DON_XULY_BL();
                    DataTable oDT = oBL.CHECK_AHC_DON_DUONGSU_ANPHI_V2(DONID, DONCHITIETID, ISDONCHITIET);
                    if (oDT.Rows.Count > 0)
                    {
                        lblSua.Visible = false;
                        lbtXoa.Visible = false;
                ...
                {
                    decimal idXuLy = Convert.ToDecimal((rowView["ID"] + "").ToString());
                    AHC_DON_XULY obj = dt.AHC_DON_XULY.Where(x => x.ID == idXuLy).FirstOrDefault();
                    AHC_DON_XULY_BL oBL = new AHC_DON_XULY_BL();
                    DataTable oYC = oBL.AHC_GETALL_DON_YCBS(DONID, idXuLy, 1, 5);
                    ...
                    //    DataTable oDT = oBL.CHECK_LOAIGIAIQUYET_DON_YCBS(idXuLy, 6);
                    //    if (oDT.Rows.Count > 0)
                    //    {
                ...
                if (rowView["ID"] + "" != "")
                {
                    ...
                    AHC_DON_XULY obj = dt.AHC_DON_XULY.Where(x => x.ID == idXuLy && x.LOAIGIAIQUYET == adsThuLy).FirstOrDefault();
                ...
                //        break;
                //}
                if (!String.IsNullOrEmpty(toagiaiquyetID) && !toagiaiquyetID.Equals(donviID) && String.IsNullOrEmpty(nguoiTao))
                {
                    lbtXoa.Visible = false;
                    lblSua.Visible = false;
                }
        ...
        protected void cmdCapNhat_Click(object sender, EventArgs e)
        {
            ...
            AHC_CHUYEN_NHAN_AN cna = new AHC_CHUYEN_NHAN_AN();
            decimal FileID = 0;
            ...
            if (hddChiTietID.Value != "")
            {
                ...
                //    var data=dt.AHC_DON_XULY.Where(x => x.LOAIGIAIQUYET == 3 && x.DON_CHITIETID.HasValue);
            ...
            {
                DonXyLyID = Convert.ToInt32(hddCurrID.Value);
                obj = dt.AHC_DON_XULY.Where(x => x.ID == DonXyLyID).FirstOrDefault();
                ...
                else obj = new AHC_DON_XULY();
                if (obj.LOAIGIAIQUYET == 4 && bienphap != "4")
                {
                    AHC_DON_XULY_BL oBL = new AHC_DON_XULY_BL();
                    DataTable oYC = oBL.AHC_GETALL_DON_YCBS(DONID, obj.ID, 1, 5);
                    if (oYC.Rows.Count > 0) oBL.DEL_DON_YCBS_GETBYDONID(Convert.ToDecimal(oYC.Rows[0]["ID"].ToString()));
                }
            }
            else
            {
                ...
                //if (dt.AHC_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 5).ToList().Count > 0)
                //{
                //    lbtthongbao.Text = "Đơn đã được thụ lý, không được phép thêm mới !";
                //    return;
                //}
                //if (dt.AHC_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 3).ToList().Count > 0)
                //{
                //    lbtthongbao.Text = "Đơn đã trả lại, không được phép thêm mới !";
                //    return;
                //}
                obj = new AHC_DON_XULY();
                AHC_DON_BL oBL = new AHC_DON_BL();
                ...
                if (txtSothongbao.Text != "")
                {
                    ...
                    decimal check = oBL.CHECKSTT_AHC((decimal)oDon.TOAANID, (decimal)oDon.MAGIAIDOAN, vNam, 0, Convert.ToDecimal(txtSothongbao.Text), ddlStbPhu.SelectedValue, obj.ID);
                    DateTime ngaythongbao = DateTime.Parse(this.txtNgaythongbao.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
                    String strMsg = "";
                    AHC_DON_BL oBL_STB = new AHC_DON_BL();
                    String STTNew = oBL_STB.GET_STB_XLDon_NEW_V2((decimal)oDon.TOAANID, "AHC", ngaythongbao).ToString();
                    if (check > 0)
                    {
            ...
            switch (bienphap)
            {
                ...
                default://Thụ lý
                    ...
                    //if (chkNopAnPhi.Checked == false)
                    //{
                    rFileID = UploadFileID(oDon, FileID, "04-HC", obj.SOTHONGBAO, obj.STB_PHU);
                    if (rFileID > 0) obj.FILEID = rFileID;
                    //}
            ...
            {
                //List<AHC_ANPHI> lstAP = dt.AHC_ANPHI.Where(x => x.DONID == DONID).ToList();
                ...
                AHC_DON_XULY_BL oBLYC = new AHC_DON_XULY_BL();
            ...
            else if (bienphap == ENUM_ADS_BIENPHAPGQ.ADS_ChuyenDonTrongNganh)
            {
                ...
                //    //Cls_Comon.ShowMessageAndRedirect(this, this.GetType(), "MsgXLDON", "Hoàn thành chuyển đơn đến tòa án khác, bạn hãy chọn đơn khác để xử lý tiếp !", Cls_Comon.GetRootURL() + "/QLAN/AHC/Hoso/Danhsach.aspx");                    
                //}
            }
            ...
            else
            {
                ...
                //    Session[ENUM_LOAIAN.AN_DANSU] = 0;
                //    UpdateTrangThaiDonKK();
                //    Cls_Comon.ShowMessageAndRedirect(this, this.GetType(), "MsgXLDON", "Hoàn thành giải quyết đơn, bạn hãy chọn đơn khác để xử lý tiếp !", Cls_Comon.GetRootURL() + "/QLAN/AHC/Hoso/Danhsach.aspx");
                //}
            }
            #endregion
            #endregion
        ...
        private void ChuyenDonSangToaAnMoi(AHC_DON oDon, decimal DonXyLyID, AHC_DON_XULY obj, decimal ToaAnCuEdit)
        {
            if (DonXyLyID == 0)
            {
                ...
                //update 14082025
                if (oDonMoi.TOA_GIAIQUYET_ID == null) oDonMoi.TOA_GIAIQUYET_ID = oDonMoi.TOAANID;
                if (oDonMoi.TOA_PHUCTHAM_GIAIQUYET_ID == null)
                    oDonMoi.TOA_PHUCTHAM_GIAIQUYET_ID = oDonMoi.TOAPHUCTHAMID;

                dt.AHC_DON.Add(oDonMoi);
                dt.SaveChanges();

                GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                GD.GAIDOAN_INSERT_UPDATE("6", oDonMoi.ID, 2, obj.CDTN_TOAANID.Value, 0, 0, 0, 0);
            ...
            else
            {
                ...
                AHC_DON oDonNew = dt.AHC_DON.FirstOrDefault(s => s.DONID_TOACU == obj.DONID && s.TOAANID == ToaAnCuEdit);
                if (oDonNew != null)
                {
                    if (oDonNew.TOAANID.Value != obj.CDTN_TOAANID)
                    {
                        ...
                        oDonNew.MAVUVIEC = ENUM_LOAIVUVIEC.AN_HANHCHINH + oTA.MA + oDonNew.TT.ToString();
                        dt.SaveChanges();
        ...
        private void DeleteDonChuyenToaAnKhac(AHC_DON_XULY obj)
        {
            AHC_DON oDon = dt.AHC_DON.FirstOrDefault(s => s.DONID_TOACU == obj.DONID && s.TOAANID == obj.CDTN_TOAANID);
            if (oDon != null)
            {
                if (dt.AHC_DON_THAMGIATOTUNG.Count(s => s.DONID == oDon.ID) > 0)
                {
                    List<AHC_DON_THAMGIATOTUNG> lstTGTT = dt.AHC_DON_THAMGIATOTUNG.Where(s => s.DONID == oDon.ID).ToList();
                    dt.AHC_DON_THAMGIATOTUNG.RemoveRange(lstTGTT);
                }

                List<AHC_DON_DUONGSU> lstDS = dt.AHC_DON_DUONGSU.Where(s => s.DONID == oDon.ID).ToList();

                if (dt.AHC_DON_TAILIEU.Count(s => s.DONID == oDon.ID) > 0)
                {
                    List<AHC_DON_TAILIEU> lstTGTT = dt.AHC_DON_TAILIEU.Where(s => s.DONID == oDon.ID).ToList();
                    dt.AHC_DON_TAILIEU.RemoveRange(lstTGTT);
                }

                GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                GD.GIAIDOAN_DELETES("6", oDon.ID, 2);

                dt.AHC_DON.Remove(oDon);

                dt.AHC_DON_DUONGSU.RemoveRange(lstDS);
                dt.SaveChanges();
            }
        ...
        void UpdateTrangThaiDonKK()
        {
            ...
            string loai_an = ENUM_LOAIAN.AN_HANHCHINH + "";
            decimal vuviecid = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HANHCHINH] + "");
            string yeucau = "";
            switch (bienphap_gd)
            {
                case 1:
        ...
        //{
        //string url = Cls_Comon.GetRootURL() + "/BaoCao/AHC/ViewReport.aspx?vgq = " + dropBienPhapGQ.SelectedValue + "&vdID = " + Session[ENUM_SESSION.SESSION_DONVIID];
        ...
        protected void changeValue()
        {
            ...
            if (txtGTTC <= 12000000)
            {
                txtTamUng = 300000 - txtMGAP;
                txtTamUngAnPhi.Text = txtTamUng.ToString();
            }
            else if (txtGTTC <= 400000000)
            {
                txtTamUng = txtGTTC * 5 / 200 - txtMGAP;
                txtTamUngAnPhi.Text = txtTamUng.ToString();
            }
            else if (txtGTTC <= 800000000)
            {
                txtTamUng = 10000000 + (txtGTTC - 400000000) * 4 / 200 - txtMGAP;
                txtTamUngAnPhi.Text = txtTamUng.ToString();
            }
        }
        ...
        private void LoadGrid_AnPhi()
        {
            ...
            DataTable oDT = oBL.AHC_ANPHI_GETBYDONID_V2(DONID, 1, pageindex, page_size);
            if (oDT.Rows.Count > 0)
            {
            ...
            switch (e.CommandName)
            {
                ...
                case "XoaAP":
                    try
                    {
                        ...
                        obj.CHECK_DELETE_ANPHI(CurrID, "6", ref _check_out);
                        if (_check_out == 1)
                        {
                            ...
                            DataTable dataTable = dON.GET_DON_MIENANPHI_BY_ANPHI_ID(objAP.ID, 6);
                            ...
                            if (oTD != null && objAP.TINHTRANG != 1 && dataTable.Rows.Count == 0)
                            {
                                ...
                                && dS_TONGDAT_BL.AHC_TONGDATDOITUONG_ANPHI_GETBYANPHIID_TONGDATID(objAP.DONID, objAP.ID).Rows.Count > 0)
                            {
                                lbThongbaoAP.Text = "Bạn không thể xóa khi đã tống đạt!";
                                return;
                            }
                            else
                            {
                                ...
                                decimal _VALUE = 0;
                                obj.DELETE_DATA_DUONGSU_AHC_FORM(CurrID);//DELETE TABLE AHN_ANPHI_DUONGSU
                ...
                case "LSTBAnPhi":
                    try
                    {
                        CurrID = Convert.ToDecimal(e.CommandArgument.ToString());
                        Cls_Comon.CallFunctionJS(this, this.GetType(), "LoadModalDiv();");
                        string linkPop = "/QLAN/AHC/XuLyDon/Popup/pMienAnPhi.aspx?ID=" + CurrID;
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "onclick", "var Mleft = (screen.width/2)-(950/2); var Mtop = (screen.height/2)-(600/2); javascript:window.open('" + linkPop + "', '_blank', 'height=600px,width=950px,top=\'+Mtop+\', left=\'+Mleft+\',resizable=yes,toolbar=no,scrollbars=0'); ", true);
                    }
                    catch (Exception ex) { lbThongbaoAP.Text = ex.Message; }
                    break;
        ...
        void LoadInfo_AnPhi(Decimal CurrID)
        {
            ...
            List<AHC_ANPHI> lst = dt.AHC_ANPHI.Where(x => x.ID == CurrID).ToList<AHC_ANPHI>();
            if (lst != null && lst.Count > 0)
            {
                ...
                string[] dsDuongSu = obj.DUONGSU_IDS.Split(',');
        ...
        protected void rptAP_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                bool isDonTraLai = dt.AHC_DON_XULY.Any(x => x.DONID == DONID && x.LOAIGIAIQUYET == 3);

                AHC_ANPHI aDS_ANPHI = dt.AHC_ANPHI.Where(x => x.ID == vAPID).FirstOrDefault();
        ...
        protected void cmdCapNhatAP_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    catch (Exception ex) { objAP = new AHC_ANPHI(); }
                    objAP.TINHTRANG = chkNopAnPhi.Checked == true ? 1 : 0;
                    objAP.GIATRITRANHCHAP = (String.IsNullOrEmpty(txtGiaTriTranhChap.Text.Trim())) ? 0 : Convert.ToDecimal(txtGiaTriTranhChap.Text.Trim(), cul);
                    objAP.MUCGIAMANPHI = (String.IsNullOrEmpty(txtMucGiamAnPhi.Text.Trim())) ? 0 : Convert.ToDecimal(txtMucGiamAnPhi.Text.Trim(), cul);
                    objAP.TAMUNGANPHI = (String.IsNullOrEmpty(txtTamUngAnPhi.Text.Trim())) ? 0 : Convert.ToDecimal(txtTamUngAnPhi.Text.Trim(), cul);
                    string listDuongSu = "";
                    if (lbxDuongSu.Items.Count > 0)
                    {
                        for (int i = 0; i < lbxDuongSu.Items.Count; i++)
                        {
                            if (lbxDuongSu.Items[i].Selected)
                            {
                                listDuongSu = listDuongSu + lbxDuongSu.Items[i].Value + ",";
                            }
                        }
                    }
                    objAP.DUONGSU_IDS = listDuongSu.Remove(listDuongSu.Length - 1);
                    objAP.DONID = DONID;
                    objAP.SONGAYGIAHAN = (String.IsNullOrEmpty(txtSoNgayGiaHan.Text.Trim())) ? 0 : Convert.ToDecimal(txtSoNgayGiaHan.Text.Trim());
                    objAP.HANNOP_SONGAY = (String.IsNullOrEmpty(txtHanNopAnPhi.Text.Trim())) ? 0 : Convert.ToInt32(txtHanNopAnPhi.Text);
                    ...
                    {
                        decimal donID = Convert.ToDecimal(current_id);
                        AHC_DON_XULY donXL = dt.AHC_DON_XULY.Where(x => (x.DONID == DONID || x.DON_XULYID == DONID) && x.LOAIGIAIQUYET == 5).FirstOrDefault();
                        DVCQG_THANH_TOAN_BL obj_anphi = new DVCQG_THANH_TOAN_BL();
                        obj_anphi.DVCQG_THANH_TOAN_INSERT_ANPHI(2, donID, donXL.ID, objAP.ID, "6", objAP.DUONGSU_IDS);
                        obj_anphi.INSERT_DATA_DUONGSU_AHC_FORM(objAP.ID);//--dung tách duongsu_ids,.. thành bảng AHN_ANPHI_DUONGSU
                    }
                    else if (APId != 0)
                    {
                        ///UPDATE vào bảng DVCQG_THANH_TOAN
                        DVCQG_THANH_TOAN_BL obj = new DVCQG_THANH_TOAN_BL();
                        obj.DVCQG_THANH_TOAN_UP_ANPHI(objAP.ID, "6");
                        //----------------------
                        obj.DELETE_DATA_DUONGSU_AHC_FORM(objAP.ID);//DELETE TABLE AHN_ANPHI_DUONGSU                       
                        obj.INSERT_DATA_DUONGSU_AHC_FORM(objAP.ID);//--dung tách duongsu_ids,.. thành bảng AHN_ANPHI_DUONGSU
                    }
                    //anhpn add log án phí 08/01/2025
                    string MATHONGBAO = "";
                    var DVCQG_THANH_TOAN = DataExtensions.GetAllWithClause<BL.GSTP.BANGSETGET.DVCQG_THANH_TOAN>("ANPHI_ID = " + objAP.ID + " AND MALOAIVUVIEC = '6'");
                    if (DVCQG_THANH_TOAN.Count() > 0)
                    {
                        MATHONGBAO = DVCQG_THANH_TOAN.FirstOrDefault().MA_THONGBAO;
                    }
                    ...
                    if (chkNopAnPhi.Checked == false)
                    {
                        ...
                        AHC_DON_XULY donXLS = dt.AHC_DON_XULY.Where(x => (x.DONID == DONID || x.DON_XULYID == DONID) && x.LOAIGIAIQUYET == 5).FirstOrDefault();
                        if (donXLS != null && donXLS.LOAIGIAIQUYET == 5)
                        {
                            DataTable oDT = obj_anphiS.CHECK_THANH_TOAN_IN_ANPHI(2, donIDS, donXLS.ID, objAP.ID, "6", objAP.DUONGSU_IDS);
                            if (oDT.Rows.Count == 0)
                            {
                                obj_anphiS.DVCQG_THANH_TOAN_INSERT_ANPHI(2, donIDS, donXLS.ID, objAP.ID, "6", objAP.DUONGSU_IDS);
                                obj_anphiS.INSERT_DATA_DUONGSU_AHN_FORM(objAP.ID);//--dung tách duongsu_ids,.. thành bảng AHC_ANPHI_DUONGSU
                            }
                        }
                    }
                    else
        ...
        private bool CheckValidAP()
        {
            ...
            {
                string[] sDS = lbxDuongSu.SelectedValue.Split(',');
                for (int i = 0; i < sDS.Count(); i++)
                {
                    ...
                    if (hddCurrAPID.Value != "" && hddCurrAPID.Value != "0")
                    {
                        ...
                        {
                            AHC_TONGDAT_BL dS_TONGDAT_BL = new AHC_TONGDAT_BL();
                            DataTable anphiTable = dS_TONGDAT_BL.AHC_TONGDATDOITUONG_ANPHI_GETBYANPHIID_TONGDATID(objAP.DONID, objAP.ID);
                    ...
                    List<AHC_ANPHI> lsan = dt.AHC_ANPHI.Where(x => x.DUONGSU_IDS.Contains(idDS) && x.ID != APId && APId != 0).ToList();
                    ...
                    //        if (lbxDuongSu.Items[i].Selected)
                    //        {
                    //            string dsID = lbxDuongSu.Items[i].Value;
                    //            decimal APId = 0;
                    //            if (hddCurrAPID.Value != "" && hddCurrAPID.Value != "0")
                    //            {
                    //                APId = Convert.ToDecimal(hddCurrAPID.Value);
                    //            }
                    //            AHC_ANPHI ap = dt.AHC_ANPHI.Where(x => x.DUONGSU_IDS.Contains(dsID) && x.ID != APId).FirstOrDefault();
                    //            if (ap != null)
                    //            {
                    //                string strMsg = "Đương sự này đã có thông tin biên lai án phí. Bạn hãy chọn đương sự khác!";
                    //                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('" + strMsg + "')", true);
