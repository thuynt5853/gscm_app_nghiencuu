...
namespace WEB.GSTP.QLAN.AHC.Hoso
{
    public partial class Tailieu : System.Web.UI.Page
    {
        ...
        private void LoadDropNguoiBanGiao()
        {
            ...
            {
                List<AHC_DON_DUONGSU> lst = dt.AHC_DON_DUONGSU.Where(x => x.DONID == DonID).ToList();
        ...
        private void LoadDropNguoiNhan()
        {
            ...
            if (tbl != null && tbl.Rows.Count > 0)
            {
                ...
                foreach (DataRow row in tbl.Rows)
                {
                    ...
                    ddlNguoiNhan.Items.Add(new ListItem(temp, row["ID"].ToString()));
                }
            }
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                AHC_DON_TAILIEU donFile = new AHC_DON_TAILIEU();
                decimal TLID = (String.IsNullOrEmpty(hddID.Value)) ? 0 : Convert.ToDecimal(hddID.Value);
                if (TLID > 0)
                    donFile = dt.AHC_DON_TAILIEU.Where(x => x.ID == TLID).FirstOrDefault<AHC_DON_TAILIEU>();
                else
                    donFile = new AHC_DON_TAILIEU();
                try
                {
                ...
                if (hddID.Value == "" || hddID.Value == "0")
                {
                    ...
                    dt.AHC_DON_TAILIEU.Add(donFile);
                }
                else
                {
        ...
        {
            lbthongbao.Text = "";
            decimal DonViID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_HANHCHINH] + "");
            int Total = 0;
            AHC_DON_BL donBL = new AHC_DON_BL();
            DataTable tbl = donBL.AHC_DON_BanGiaoTaiLieu_GETLIST(DonViID);
            if (tbl != null && tbl.Rows.Count > 0)
            {
        ...
        {
            AHC_DON_TAILIEU oT = dt.AHC_DON_TAILIEU.Where(x => x.ID == ID).FirstOrDefault();

            dt.AHC_DON_TAILIEU.Remove(oT);
            dt.SaveChanges();
            hddPageIndex.Value = "1";
            LoadGridNgayBanGiao();
            ResetControls();
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                switch (e.CommandName)
                {
                    ...
                    case "Sua":
                        hddFilePath.Value = "";
                        AHC_DON_TAILIEU bgtl = dt.AHC_DON_TAILIEU.Where(x => x.ID == ID).FirstOrDefault();
                        if (bgtl != null)
                        {
                        ...
                        break;
                    case "Download":
                        AHC_DON_TAILIEU oND = dt.AHC_DON_TAILIEU.Where(x => x.ID == ID).FirstOrDefault();
                        if (oND.TENFILE != "")
                        {
