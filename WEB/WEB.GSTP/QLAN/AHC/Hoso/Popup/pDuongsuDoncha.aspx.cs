    ...
    public partial class pDuongsuDoncha : System.Web.UI.Page
    {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (DonID == 0)
                {
                    DonID = String.IsNullOrEmpty(current_id) ? 0 : Convert.ToDecimal(current_id);
                }
                ...
                if (chkKhongLamSach.Checked)
                {
                    xacThuc = 3; // ngươi dùng xác nhận không làm sạch được
                }
                else
                {
                    if (hdTrangThaiXacThuc.Value == null)
                        xacThuc = 0;
                    else
                        xacThuc = Convert.ToInt16(hdTrangThaiXacThuc.Value);
                }
            ...
            if (quocTich == "2")
            {
                ...
                else
                {
                    ...
                    if (DuongSuID > 0)
                    {
                        ...
                        AHC_DON_DUONGSU ds = dt.AHC_DON_DUONGSU.Where(x => x.ID == DuongSuID).First();
                        if (ds != null)
                        {
