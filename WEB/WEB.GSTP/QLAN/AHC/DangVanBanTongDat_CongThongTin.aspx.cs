...
namespace WEB.GSTP.QLAN.AHC
{
    public partial class DangVanBanTongDat_CongThongTin : System.Web.UI.Page
    {
         ...
         */
        /*
         * Mã biểu mẫu thông báo phá sản: TBPS
         * Mã biểu mẫu Thông báo kết quả ủy thác tư pháp: TBKQUTTP
         * Mã biểu mẫu Thông báo tìm người vắng mặt: TBVM
         */
        GSTPContext dt = new GSTPContext();
        private const string CAPXX_ST = "1", CAPXX_PT = "2", CAPXX_GDT = "3", CAPXX_TT = "4", LINHVUC_HANHCHINH = "49";
        ...
        private void LoadDanhSach()
        {
            ...
            DataTable tbl = Cls_Comon.GetTableByProcedurePaging("AHC_TONGDAT_GetVB_CTTDT", prm);
        ...
        protected void BtnGuiCongThongTinDienTu_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string prm_key = "dGFuZHRjL3NlcnZpY2UvZ3NvbG90aW9uL2FucHg=",
                    prm_linhVuc = LINHVUC_HANHCHINH, StrLinhVuc = "hành chính", BMKhongThanhCong = "";
                ...
                foreach (DataGridItem item in dgListChuaDangTai.Items)
                {
                    ...
                    if (chkIsSend.Checked)
                    {
                        ...
                        AHC_TONGDAT tdat = dt.AHC_TONGDAT.Where(x => x.ID == TongDatID).FirstOrDefault();
                        if (tdat != null)
                        {
                            int prm_type = 0;
                            ...
                            if (bm != null)
                            {
                                ...
                                if (bm.GIAIDOAN == ENUM_GIAIDOANVUAN.SOTHAM.ToString())// sơ thẩm
                                {
                                    ...
                                    AHC_SOTHAM_THULY ThuLyST = dt.AHC_SOTHAM_THULY.Where(x => x.DONID == DONID).OrderByDescending(x => x.NGAYTHULY).FirstOrDefault();
                                    if (ThuLyST != null)
                                    {
                                ...
                                else if (bm.GIAIDOAN == ENUM_GIAIDOANVUAN.PHUCTHAM.ToString())
                                {
                                    ...
                                    AHC_PHUCTHAM_THULY ThuLyPT = dt.AHC_PHUCTHAM_THULY.Where(x => x.DONID == DONID).OrderByDescending(x => x.NGAYTHULY).FirstOrDefault();
                                    if (ThuLyPT != null)
                                    {
                                ...
                                AHC_DON_DUONGSU NguoiKhoiKien = dt.AHC_DON_DUONGSU.Where(x => x.DONID == DONID && x.TUCACHTOTUNG_MA == ENUM_DANSU_TUCACHTOTUNG.NGUYENDON).FirstOrDefault();
                                ...
                                AHC_DON_DUONGSU NguoiBiKien = dt.AHC_DON_DUONGSU.Where(x => x.DONID == DONID && x.TUCACHTOTUNG_MA == ENUM_DANSU_TUCACHTOTUNG.BIDON).FirstOrDefault();
                                if (NguoiBiKien != null)
                                {
                                ...
                                if (MaBM == "06-HC" || MaBM == "35-HC")// Thông báo thụ lý hành chính
                                {
                                    prm_title = "Ngày " + strNgayHieuLuc + ", " + TenToaThuLy + " đã thụ lý vụ án " + StrLinhVuc + " số " + prm_soThongBao + " về việc: " + prm_tenVuAn + ", giữa người khởi kiện: " + prm_nguoiKhoiKien + " và người bị kiện: " + prm_nguoiBiKien + ", địa chỉ: " + prm_diaChi + ".";
                                    prm_type = TBTHULY_HC;
                                }
