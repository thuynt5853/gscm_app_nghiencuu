...
namespace WEB.GSTP.QLAN.AHC.AnPhi
{
    public partial class Danhsach : System.Web.UI.Page
    {
        ...
        void CheckQuyen()
        {
            ...
            AHC_DON_XULY donxl = dt.AHC_DON_XULY.Where(x => x.DONID == ID).FirstOrDefault();
            if (donxl != null)
            {
            ...
            List<AHC_DON_XULY> lts_donxl = dt.AHC_DON_XULY.Where(x => x.DONID == ID).ToList();
            ...
            //List<AHC_SOTHAM_THULY> lstTL = dt.AHC_SOTHAM_THULY.Where(x => x.DONID == ID).ToList();
            //if (lstTL.Count > 0)
            //{
        ...
        void LoadDropDuongSu()
        {
            ...
            List<AHC_ANPHI> obj = dt.AHC_ANPHI.Where(x => x.DONID == DONID && x.TINHTRANG == 0).ToList();
            if (obj != null)
            {
                foreach (AHC_ANPHI ap in obj)
                {
                    DON_MIENANPHI_BL dmb = new DON_MIENANPHI_BL();
                    DataTable dataTable = dmb.GET_DON_MIENANPHI_BY_ANPHI_ID(ap.ID, 6);

                    if (dataTable == null || dataTable.Rows.Count == 0)
                    {
                        string tenDuongSu = "";
                        string[] idDS = ap.DUONGSU_IDS.Split(',');
                        foreach (string id in idDS)
                        {
                            if (!String.IsNullOrEmpty(id))
                            {
                                decimal idd = Convert.ToDecimal(id);
                                tenDuongSu += dt.AHC_DON_DUONGSU.Where(x => x.ID == idd).FirstOrDefault().TENDUONGSU + ", ";
                            }
                        }
                        tenDuongSu = tenDuongSu.Remove(tenDuongSu.Length - 2);
                        ddlDuongSu.Items.Add(new ListItem(tenDuongSu, ap.DUONGSU_IDS));
                    }
                }
            }
        ...
        private void LoadGrid_AnPhi()
        {
            ...
            DataTable oDT = oBL.AHC_ANPHI_GETBYDONID(DONID, 0, pageindex, page_size);
            if (oDT.Rows.Count > 0)
            {
                AHC_ANPHI ap = dt.AHC_ANPHI.Where(x => x.DONID == DONID && x.SOBIENLAI != null && x.TINHTRANG == 0).FirstOrDefault();
                if (ap != null)
                {
        ...
        protected void rpt_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "ENABLE":
                    ...
                    {
                        oBLs.CHECK_TRANGTHAI_THULY(CurrID, "6");
                ...
                case "Sua":
                    ...
                    DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(CurrID, 6);
                    if (dataTable != null && dataTable.Rows.Count > 0)
                    {
                ...
                case "Xoa":
                    try
                    {
                        ...
                        DataTable dataTableXoa = dON_MIENANPHI_BLs.GET_DON_MIENANPHI_BY_ANPHI_ID(CurrID, 6);
                ...
                case "DownloadAP_THA":
                    ...
                    //--------------------
                    byte[] V_FILE_DATA = oBL.File_Attach_Anphi_Return("6", CurrID, ref V_FILE_NAME);
                    string V_FILE_TYLE = "";
        ...
        {
            //txtGiaTriTranhChap.Text = null;
            //txtMucGiamAnPhi.Text = null;
            //txtAnPhi.Text = null;
            //txtTamUngAnPhi.Text = null;
            //txtHanNopAnPhi.Text = null;
            //txtSoNgayGiaHan.Text = null;
        ...
        protected void cmdCapNhat_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    lbtthongbao.Text = "Bạn chưa nhập ngày nộp tạm ứng án phí !";
                ...
                else
                {
                    ...
                    if (hddNgayGQYC.Value != "")
                    {
                        ...
                        {
                            lbtthongbao.Text = "Bạn phải nhập ngày nộp tạm ứng án phí lớn hơn ngày GQ/YC " + hddNgayGQYC.Value;
                            txtNgayNopAnPhi.Focus();
                            return;
                        }
                    }
                ...
                {
                    lbtthongbao.Text = "Bạn chưa chọn cán bộ nhận biên lai !";
                    return;
                }
                if (txtSoBienLai.Text == "")
                {
                    lbtthongbao.Text = "Bạn chưa nhập số biên lai !";
                    return;
                }
                if (txtNgayNopBL.Text == "")
                {
                    lbtthongbao.Text = "Bạn chưa nhập ngày nộp biên lai !";
                    return;
                }
                else
                {
                    ...
                    {
                        lbtthongbao.Text = "Ngày nộp biên lai phải nhỏ hơn ngày hiện tại!";
                        txtNgayNopBL.Focus();
                        return;
                    }
                    if (NgayNopBienLai < NgayNopAnPhi)
                    {
                        lbtthongbao.Text = "Bạn phải nhập ngày nộp biên lai lớn hơn ngày nộp tạm ứng án phí!";
                        txtNgayNopBL.Focus();
                        return;
                    }
                }
        ...
        {
            Response.Redirect(Cls_Comon.GetRootURL() + "/QLAN/AHC/Sotham/ThuLy.aspx");
        }
        ...
        protected void rpt_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                DAL.GSTP.DVCQG_THANH_TOAN oAP = dt.DVCQG_THANH_TOAN.Where(x => x.MA_THONGBAO == V_MA_THONGBAO).FirstOrDefault();
        ...
        {
            string idDS = ddlDuongSu.SelectedValue + "";
            if (idDS != "0")
            {
                string current_id = Session[ENUM_LOAIAN.AN_HANHCHINH] + "";
                decimal DONID = Convert.ToDecimal(current_id);
                decimal idAP = dt.AHC_ANPHI.Where(x => x.DONID == DONID && x.DUONGSU_IDS == idDS).FirstOrDefault().ID;
        ...
        //        decimal DONID = Convert.ToDecimal(current_id);
        //        ADS_ANPHI objAP = dt.ADS_ANPHI.Where(x => x.DONID == DONID).FirstOrDefault();
        ...
        //    txtGiaTriTranhChap.Enabled = status == true ? false : true;
        //    txtMucGiamAnPhi.Enabled = status == true ? false : true;
        //    txtAnPhi.Enabled = status == true ? false : true;
        //    txtTamUngAnPhi.Enabled = status == true ? false : true;
        //    txtHanNopAnPhi.Enabled = status == true ? false : true;
        //    txtSoNgayGiaHan.Enabled = status == true ? false : true;
