...
namespace WEB.GSTP.QLAN.AHC
{
    public partial class Tongdat : System.Web.UI.Page
    {
        ...
        AHC_TONGDAT_BL TONGDAT_BL = new AHC_TONGDAT_BL();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                string current_id = Session[ENUM_LOAIAN.AN_HANHCHINH] + "", TOAANID = "0";
                ...
                List<DONKK_USER_DKNHANVB> lstDKVB = dkk.DONKK_USER_DKNHANVB.Where(x => x.VUVIECID == ID && x.MALOAIVUVIEC == ENUM_LOAIAN.AN_HANHCHINH && x.TRANGTHAI == 1).ToList();
                if (lstDKVB.Count == 0)
        ...
        private void LoadCombobox()
        {
            ...
            //DataTable tbl = oBL.AHC_FILE_TONGDAT(ID);
            //AHC_SOTHAM_BANAN oND = dt.AHC_SOTHAM_BANAN.Where(x => x.DONID == VuAnId && x.TOAANID == TOAANID).FirstOrDefault();
            //DM_BIEUMAU bm52 = dt.DM_BIEUMAU.Where(bm => bm.ID == 230).FirstOrDefault();
            //AHC_FILE af = dt.AHC_FILE.Where(x => x.BIEUMAUID == 230 && x.DONID == VuAnId).FirstOrDefault();
            //AHC_TONGDAT td = dt.AHC_TONGDAT.Where(x => x.DONID == VuAnId && x.BIEUMAUID == 230).FirstOrDefault();
            //if (oND != null && td == null)
            //{
            ...
            //    tbl.Rows.Add(dr);
            //}
            //AHC_PHUCTHAM_BANAN oND2 = dt.AHC_PHUCTHAM_BANAN.Where(x => x.DONID == VuAnId && x.TOAANID == TOAANID).FirstOrDefault();
            //DM_BIEUMAU bm75 = dt.DM_BIEUMAU.Where(bm => bm.ID == 261).FirstOrDefault();
            //af = dt.AHC_FILE.Where(x => x.BIEUMAUID == 261 && x.DONID == VuAnId).FirstOrDefault();
            //td = dt.AHC_TONGDAT.Where(x => x.DONID == VuAnId && x.BIEUMAUID == 261).FirstOrDefault();
            ...
            AHC_DON objDon = dt.AHC_DON.Where(x => x.ID == ID).FirstOrDefault();
            decimal giai_doan = Convert.ToDecimal(objDon.MAGIAIDOAN);
            TONGDAT_BL oTongDatBL = new TONGDAT_BL();
            DataTable oDT = oTongDatBL.GET_BM_TONGDAT(ID, 6, giai_doan);
        ...
        private List<AHC_TONGDAT_GS> convertToAHC_TONGDATFromDataTable(DataTable dataTable)
        {
            List<AHC_TONGDAT_GS> aHC_TONGDATs = new List<AHC_TONGDAT_GS>();
            foreach (DataRow row in dataTable.Rows)
            {
                AHC_TONGDAT_GS tONGDAT = new AHC_TONGDAT_GS();
                ...
                aHC_TONGDATs.Add(tONGDAT);
            }
            return aHC_TONGDATs;
        }

        private List<AHC_TONGDAT_NGUOINHAN_GS> convertToAHC_TONGDATDOITUONGFromDataTable(DataTable dataTable)
        {
            List<AHC_TONGDAT_NGUOINHAN_GS> aHC_TONGDATs = new List<AHC_TONGDAT_NGUOINHAN_GS>();
            foreach (DataRow row in dataTable.Rows)
            {
                AHC_TONGDAT_NGUOINHAN_GS tONGDAT = new AHC_TONGDAT_NGUOINHAN_GS();
                ...
                //---------29/11/2025----  vnpt chỉnh lấy thêm cột checkVNEID
                try
                {
        ...
        {
            BL.GSTP.AHC.AHC_TONGDAT_BL Bl = new BL.GSTP.AHC.AHC_TONGDAT_BL();
            //lbthongbao.Text = "";
            ...
            AHC_TONGDAT adTongDat = null;

            if (ANPHI_ID_DROPDOWN != 0)
            {
                adTongDat = dt.AHC_TONGDAT.Where(x => x.DONID == DONID && x.BIEUMAUID == bieuMauID && x.MAPID == ANPHI_ID_DROPDOWN).FirstOrDefault();
            }
            else
            {
                adTongDat = dt.AHC_TONGDAT.Where(x => x.DONID == DONID && x.BIEUMAUID == bieuMauID).FirstOrDefault();
            }

            AHC_TONGDAT_GS oND = null;

            if (TongDatID != 0)
            {
                oND = convertToAHC_TONGDATFromDataTable(Bl.AHC_TONGDAT_GETBYID(TongDatID)).FirstOrDefault();
            }

            bool isNew = false;

            bool isUpdate = false;
            AHC_TONGDAT td = new AHC_TONGDAT();
            List<AHC_TONGDAT_DOITUONG> td_dt = new List<AHC_TONGDAT_DOITUONG>();
            ...
            else
            {
                ...
                td = dt.AHC_TONGDAT.Where(x => x.ID == TongDatID).FirstOrDefault();
                td_dt = dt.AHC_TONGDAT_DOITUONG.Where(x => x.TONGDATID == td.ID && x.TRANGTHAI == 1).ToList();

                CONTENT_JSON_OLD = JsonConvert.SerializeObject(td, Formatting.Indented);
            }
            if (dM_BIEUMAU.MABM == "100-DS")
            {
                oND.MAP_TABLE = ENUM_MAP_TABLE.AHC_ANPHI;
            ...
            if (isNew)
            {
                ...
                //---------29/11/2025---- vnpt - Thêm biến FILEID để lọc riêng từng văn bản trùng
                if (ddlBieumau.SelectedValue.Split('_').Length > 2)
                {
                    decimal FILEID = Convert.ToDecimal(ddlBieumau.SelectedValue.Split('_')[2]);
                    oF = dt.AHC_FILE.Where(x => x.DONID == DONID && x.BIEUMAUID == bieu_mau_id && x.ID == FILEID).FirstOrDefault();
                }
                else
                {
                    oF = dt.AHC_FILE.Where(x => x.DONID == DONID && x.BIEUMAUID == bieu_mau_id).FirstOrDefault();
                }
                //---------29/11/2025----  vnpt chỉnh lấy thêm mapid maptable
                if (oF != null)
                {
                    oND.FILEID = oF.ID;
                    var donxuly = dt.AHC_DON_XULY.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                    if (donxuly != null)
                    {
                        oND.MAP_TABLE = ENUM_MAP_TABLE.AHC_DON_XULY;
                        oND.MAPID = donxuly.ID;
                    }

                    var donxulydct =
                                    (from dx in dt.AHC_DON_XULY
                    ...
                    var anphi = dt.AHC_ANPHI.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);
                    ...
                    var sothamthuly = dt.AHC_SOTHAM_THULY.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                    if (sothamthuly != null)
                    {
                        oND.MAP_TABLE = ENUM_MAP_TABLE.AHC_SOTHAM_THULY;
                        oND.MAPID = sothamthuly.ID;
                    }

                    var sothamquyetdinh = dt.AHC_SOTHAM_QUYETDINH.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                    if (sothamquyetdinh != null)
                    {
                        oND.MAP_TABLE = ENUM_MAP_TABLE.AHC_SOTHAM_QUYETDINH;
                        oND.MAPID = sothamquyetdinh.ID;
                    }

                    var phucthamthuly = dt.AHC_PHUCTHAM_THULY.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                    if (phucthamthuly != null)
                    {
                        oND.MAP_TABLE = ENUM_MAP_TABLE.AHC_PHUCTHAM_THULY;
                        oND.MAPID = phucthamthuly.ID;
                    }

                    var phucthamquyetdinh = dt.AHC_PHUCTHAM_QUYETDINH.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                    if (phucthamquyetdinh != null)
                    {
                        oND.MAP_TABLE = ENUM_MAP_TABLE.AHC_PHUCTHAM_QUYETDINH;
                        oND.MAPID = phucthamquyetdinh.ID;
                    }

                    var phucthamquyetdinhTDC = DataExtensions.GetAllByclause("AHC_KCKNQDK_PHUCTHAM_QUYETDINH", $"DONID = '{DONID}' and FILEID = '{oF.ID}'").FirstOrDefault();

                    if (phucthamquyetdinhTDC != null)
                    {
                        oND.MAP_TABLE = ENUM_MAP_TABLE.AHC_KCKNQDK_PHUCTHAM_QUYETDINH;
                        oND.MAPID = phucthamquyetdinhTDC.ID;
                    }

                    var banansotham = dt.AHC_SOTHAM_BANAN.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                    if (banansotham != null)
                    {
                        oND.MAP_TABLE = ENUM_MAP_TABLE.AHC_SOTHAM_BANAN;
                        oND.MAPID = banansotham.ID;
                    }

                    var bananphuctham = dt.AHC_PHUCTHAM_BANAN.FirstOrDefault(x => x.DONID == DONID && x.FILEID == oF.ID);

                    if (bananphuctham != null)
                    {
                        oND.MAP_TABLE = ENUM_MAP_TABLE.AHC_PHUCTHAM_BANAN;
                ...
                oND.ID = Bl.AHC_TONGDAT_UPIN(oND);
                ...
                log.InsertLog("AHC_TONGDAT", oND.ID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Insert", "", CONTENT_JSON, "");
            }
            else
            {
            ...
            if (!isNew)
            {
                ...
                CONTENT_JSON = JsonConvert.SerializeObject(oND, Formatting.Indented);
                LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                log.InsertLog("AHC_TONGDAT", oND.ID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Update", lbthongbao.Text, CONTENT_JSON, CONTENT_JSON_OLD);
            }
            if (trDuongsu.Visible)
            {
                decimal TongdatID = oND.ID;
                List<decimal> IDs = new List<decimal>();
                foreach (DataRow row in TONGDAT_BL.AHC_TONGDATDOITUONG_GETBYTONGDATID(TongDatID).Rows)
                {
                ...
                foreach (DataGridItem Item in dgTructiep.Items)
                {
                    ...
                    decimal? ID;
                    List<AHC_TONGDAT_NGUOINHAN_GS> lstCheck;
                    try
                    {
                        if (Item.Cells[7].Text == "&nbsp;")
                        {
                            ID = null;
                            lstCheck = new List<AHC_TONGDAT_NGUOINHAN_GS>();
                        }
                        else
                        {
                            ID = Convert.ToDecimal(Item.Cells[7].Text);
                            lstCheck = convertToAHC_TONGDATDOITUONGFromDataTable(Bl.AHC_TONGDATDOITUONG_GETBYID(ID.Value));
                        }
                    }
                    catch
                    {
                    ...
                    AHC_TONGDAT_NGUOINHAN_GS oTD = null;
                    if (lstCheck.Count > 0)
                    {
                        oTD = lstCheck[0];
                    ...
                    {
                        oTD = new AHC_TONGDAT_NGUOINHAN_GS();
                    }
                    ...
                    if (chkIsSend.Checked)
                    {
                        ...
                        Bl.AHC_TONGDATDOITUONG_UPIN(oTD);
                        dt.SaveChanges();
                        ...
                        if (lstCheck.Count > 0)
                        {
                            ...
                            LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                            log.InsertLog("AHC_TONGDAT_DOITUONG", TongDatID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Update", lbthongbao.Text, CONTENT_JSON, CONTENT_JSON_OLD);
                        }
                        else
                        {
                            //AnhPN log
                            CONTENT_JSON = JsonConvert.SerializeObject(oTD, Formatting.Indented);
                            LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                            log.InsertLog("AHC_TONGDAT_DOITUONG", oTD.TONGDATID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Insert", lbthongbao.Text, CONTENT_JSON, "");
                        }
                        ...
                        {
                            CONTENT_JSON = JsonConvert.SerializeObject(oTD, Formatting.Indented);
                            obj.DVCQG_THANH_TOAN_UP_TONGDAT(oND.ID, (decimal)DuongsuID, "6");
                ...
                if (IDs.Count > 0)
                {
                    foreach (decimal i in IDs)
                        TONGDAT_BL.AHC_TONGDATDOITUONG_REMOVEBYID(i);
                }
            }
            if (trTructuyen.Visible)
            {
                ...
                foreach (DataGridItem Item in dgTructuyen.Items)
                {
                    ...
                    List<AHC_TONGDAT_DOITUONG> lstCheck = dt.AHC_TONGDAT_DOITUONG.Where(x => x.TONGDATID == TongdatID && x.DUONGSUID == DuongsuID).ToList();
                    AHC_TONGDAT_DOITUONG oTD = null;
                    if (lstCheck.Count > 0)
                    {
                        oTD = lstCheck[0];
                    }
                    ...
                    if (chkIsSendTT.Checked)
                    {
                        ...
                        {
                            oTD.TOA_GIAIQUYET_ID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);
                            dt.AHC_TONGDAT_DOITUONG.Add(oTD);
                        }
        ...
        {
            AHC_TONGDAT td = dt.AHC_TONGDAT.Where(x => x.DONID == DonID && x.TOAANID == ToaAnID && x.BIEUMAUID == BieuMauID && (MAPID == 0 || x.MAPID == MAPID)).FirstOrDefault();
            List<AHC_TONGDAT_DOITUONG> td_dt = dt.AHC_TONGDAT_DOITUONG.Where(x => x.TONGDATID == td.ID).ToList();
            foreach (var item in td_dt)
            {
                AHC_TONGDAT_NGUOINHAN_GS oTD = new AHC_TONGDAT_NGUOINHAN_GS();
        ...
        private void ReUpdateStatusUpdate(List<AHC_TONGDAT_DOITUONG> td_dt, decimal TongDatId)
        {
            ...
            List<AHC_TONGDAT_DOITUONG> list_detail = dt.AHC_TONGDAT_DOITUONG.Where(x => x.TONGDATID == TongDatId).ToList();
        ...
        {
            AHC_DON_BL oBL = new AHC_DON_BL();
            BL.GSTP.AHC.AHC_TONGDAT_BL TongDatBL = new BL.GSTP.AHC.AHC_TONGDAT_BL();
            string current_id = Session[ENUM_LOAIAN.AN_HANHCHINH] + "";
            decimal ID = Convert.ToDecimal(current_id), ToaAnID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);

            DataTable oDT = oBL.AHC_TONGDAT_GETLIST(ID, ToaAnID);
            var lstTongDat = convertToAHC_TONGDATFromDataTable(oDT);
            List<AHC_TONGDAT_NGUOINHAN_GS> lstNguoiNhan = new List<AHC_TONGDAT_NGUOINHAN_GS>();
            //  -- VNPT 06122025
            List<AHC_TONGDAT_GS> lstHaventDoituongtongdat = new List<AHC_TONGDAT_GS>();
            foreach (var tongdat in lstTongDat)
            {
                DataTable doiTuongDataTable = TongDatBL.AHC_TONGDATDOITUONG_GETBYTONGDATID(tongdat.ID);
                if (doiTuongDataTable.Rows.Count == 0)
                {
                    lstHaventDoituongtongdat.Add(tongdat);
                }
                lstNguoiNhan.AddRange(convertToAHC_TONGDATDOITUONGFromDataTable(doiTuongDataTable));
            }
            if (oDT != null && lstNguoiNhan.Count == 0)
            {
                if (lstNguoiNhan.Count == 0)
                {
                    ...
                    obj.TONGDAT_DELETE_ERROR(ID.ToString(), "6");
                }
            }
        ...
        public void xoa(decimal id)
        {
            ...
            List<AHC_TONGDAT_DOITUONG> lst = dt.AHC_TONGDAT_DOITUONG.Where(x => x.TONGDATID == id && (x.TRANGTHAI == 0 || x.TRANGTHAI == 2 || ((x.TRANGTHAI == 5 || x.TRANGTHAI == 6) && x.NGAYPHATHANH == null))).ToList();
            if (lst.Count > 0)
            {
                dt.AHC_TONGDAT_DOITUONG.RemoveRange(lst);
                dt.SaveChanges();
            }
            //--
            List<AHC_TONGDAT_DOITUONG> lst_obj = dt.AHC_TONGDAT_DOITUONG.Where(x => x.TONGDATID == id).ToList();
            AHC_TONGDAT oND = dt.AHC_TONGDAT.Where(x => x.ID == id).FirstOrDefault();
            if (oND != null)
            {
                if (lst_obj.Count == 0)
                {
                    obj.DVCQG_TT_REMOVE_TONGDAT(id, "6", ref _VALUE);
                    if (_VALUE == 1)
                    {
                        dt.AHC_TONGDAT.Remove(oND);
                        dt.SaveChanges();
                    }
                    else
                    {
        ...
        public void loadSua(decimal ID)
        {
            ...
            if (oND != null)
            {
                ...
                MapID = oND.MAPID != null ? oND.MAPID : 0;
            }
            ...
            if (oBM != null)
            {
                ...
                {
                    AHC_ANPHI aDS_ANPHI = dt.AHC_ANPHI.Where(x => x.ID == MapID).FirstOrDefault();
                    DON_MIENANPHI_BL dON_MIENANPHI = new DON_MIENANPHI_BL();

                    if (aDS_ANPHI != null)
                    {
                        DataTable DON_MIENANPHI = dON_MIENANPHI.GET_DON_MIENANPHI_BY_ANPHI_ID(aDS_ANPHI.ID, 6);
            ...
            {
                nguoiNhans = TONGDATBL.AHC_TONGDATDOITUONG_ANPHI_GETBYTONGDATID(ID);
            }
            else
            {
                nguoiNhans = TONGDATBL.AHC_TONGDATDOITUONG_GETBYTONGDATID(ID);
            }
            ...
            {
                string name = TONGDATBL.AHC_TONGDATDOITUONG_GETTENDUONGSU(Convert.ToDecimal(row["ID"]));
        ...
        public void loadPhatHanhLai(decimal ID, decimal vID)
        {
            ...
            cmdLammoi.Visible = true;
            BL.GSTP.AHC.AHC_TONGDAT_BL TONGDATBL = new BL.GSTP.AHC.AHC_TONGDAT_BL();
            DataTable nguoiNhans = TONGDATBL.AHC_TONGDATDOITUONG_GETBYTONGDATID(ID);
            DataTable nguoiNhan = TONGDATBL.AHC_TONGDATDOITUONG_GETBYID(vID);
            ...
            string nguoiNhanName = TONGDATBL.AHC_TONGDATDOITUONG_GETTENDUONGSU(Convert.ToDecimal(nguoiNhanRow["ID"]));
        ...
        private void loadBangSua(int cnt)
        {
            ...
            {
                DataGridItem Item = dgTructiep.Items[i];
                //---------21/11/2025----  vnpt chỉnh lấy thêm cột checkVNEID
        ...
        //    if (FILEID > 0)
        //    {
        //        //AHC_FILE oF = dt.AHC_FILE.Where(x => x.ID == FILEID).FirstOrDefault();
        //        //if (oF != null) {
        //        //    if (oF.TENFILE !=null)
        //        //        lbtDownload.Visible = true;
        //        //    else
        ...
        //    //List<AHC_TONGDAT_DOITUONG> lst = dt.AHC_TONGDAT_DOITUONG.Where(x => x.TONGDATID == ID).ToList();
        //    //int vCheck = 0;
        //    //if (lst.Count > 0)
        //    //{
        ...
        protected void loadAction(string nDID)
        {
            ...
            //    //Call package
            //    BL.GSTP.AHC.AHC_TONGDAT_BL tongDatBL = new BL.GSTP.AHC.AHC_TONGDAT_BL();
            //    tongDatBL.AHC_TONGDAT_THUHOI_DOITUONG(ID, TongdatID, LyDoThuHoi, NgayThuHoi, NguoiSua);
            ...
            if (TrangThai == 2) // Xem lý do thu hồi
            {
                ...
                DataTable objLyDo = oBL.GET_LYDO_THUHOI(6, TongdatID);
                if (objLyDo != null)
                {
            ...
            {
                BL.GSTP.AHC.AHC_TONGDAT_BL tongDatBL = new BL.GSTP.AHC.AHC_TONGDAT_BL();
                tongDatBL.AHC_TONGDAT_VBDH_DOITUONG(ID);
                LoadGrid();
            }
            else if (TrangThai == 5)
            {
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "Download":
                    ...
                    AHC_TONGDAT ads = dt.AHC_TONGDAT.Where(x => x.ID == tongDatId).FirstOrDefault();
                    byte[] conten = null;
                    if (ads != null)
                    {
                        if (ads.URL_FILE != null)
                        {
                            string vfileNam = Path.GetFileName(ads.URL_FILE).Replace(" ", "_");
                        ...
                        {
                            AHC_FILE oF = dt.AHC_FILE.Where(x => x.ID == ads.FILEID).FirstOrDefault();
                            if (oF != null)
                            {
                                if (oF.TENFILE != null)
                                {
                ...
                case "UpdateNgayNhan":
                    ...
                    if (txtNGAYNHANTONGDAT.Text != "")
                    {
                        ...
                        if (obj.TONGDAT_DOITUONG_VNID(Id_obj, "6", txtNGAYNHANTONGDAT.Text) == true)
                        {
                    ...
                    else if (txtNGAYNHANTONGDAT.Text == "")
                    {
                        ...
                        if (obj.TONGDAT_DOITUONG_VNID(Id_obj, "6", null) == true)
                        {
                            strMsg = "Bạn đã cập nhật thành công";
                            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('" + strMsg + "')", true);
                            LoadGrid();
        ...
        private void LoadDoituong(bool IsShowVKS, decimal BieuMauID)
        {
            ...
            decimal ANPHI_ID = 0;
            //---------21/11/2025----  vnpt chỉnh lấy thêm FILEID
            decimal FILEID = -1;
            if (BieuMauID == 0)
            {
                ...
                //---------21/11/2025----  vnpt - Thêm biến FILEID để lọc riêng từng văn bản trùng
                if (ddlBieumau.SelectedValue.Split('_').Length > 2)
                {
            ...
            if (IS_TD_DUONGSU == 1)
            {
                if (oBM.MABM == "100-DS")
                {
                    oDTDS = TONGDAT_BL.AHC_TONGDATDOITUONG_ANPHI_GETBY(DONID, TOAANID, BMID, 0, ANPHI_ID);
                }
                else
                {
                    //---------21/11/2025----  vnpt chỉnh lấy thêm lọc FILEID
                    oDTDS = TONGDAT_BL.AHC_TONGDATDOITUONG_GETBYFILEID(DONID, TOAANID, BMID, 0, FILEID);
                }
            ...
            else if (IS_TD_NGUYENDON == 1 && IS_TD_DUONGSU != 1)
            {
                if (oBM.MABM == "100-DS")
                {
                    oDTDS = TONGDAT_BL.AHC_TONGDATDOITUONG_ANPHI_GETBY(DONID, TOAANID, BMID, 1, ANPHI_ID);
                }
                else
                {
                    //---------21/11/2025----  vnpt chỉnh lấy thêm lọc checkVNEID
                    oDTDS = TONGDAT_BL.AHC_TONGDATDOITUONG_GETBYFILEID(DONID, TOAANID, BMID, 1, FILEID);
                }
        ...
        {
            BL.GSTP.AHC.AHC_TONGDAT_BL aHC = new BL.GSTP.AHC.AHC_TONGDAT_BL();
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                string TenBM = aHC.AHC_TENBM_GETBYTONGDATID(tongdatID);
                Label lbTenBM = (Label)e.Item.FindControl("lbTenBM");
                lbTenBM.Text = TenBM;
                lbNoiNhan.Text = aHC.AHC_TONGDATDOITUONG_GETTENDUONGSU(id);
                if (duongsu_id != null)
                {
                }
                ...
                BL.GSTP.AHC.AHC_TONGDAT_BL Bl = new BL.GSTP.AHC.AHC_TONGDAT_BL();
                AHC_TONGDAT_GS oND = convertToAHC_TONGDATFromDataTable(Bl.AHC_TONGDAT_GETBYID(tongdatID)).FirstOrDefault();
                ...
                int rowspan = aHC.AHC_TONGDATDOITUONG_GETBYTONGDATID(tongdatID).Rows.Count;
        ...
        protected void dgTructiep_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            BL.GSTP.AHC.AHC_TONGDAT_BL _BL = new BL.GSTP.AHC.AHC_TONGDAT_BL();
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                DataRowView rowView = (DataRowView)e.Item.DataItem;
                ...
                if (ID == null) // nếu là hàng theem 
                {
                    ...
                    dt.AHC_DON_DUONGSU.Where(x => x.DONID == DONID && x.TUCACHTOTUNG_MA == ENUM_DANSU_TUCACHTOTUNG.QUYENNVLQ).ToList()
                        .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.TENDUONGSU, "QUYENNVLQ1" + "," + x.ID)));
                    //if (oT.MAGIAIDOAN == 2)
                    //{
                    //    dt.AHC_DON_THAMGIATOTUNG.Where(x => x.DONID == DONID)
                    //       .ToList()
                    //       .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.HOTEN, x.TUCACHTGTTID + "," + x.ID)));
                    //}
                    //else if (oT.MAGIAIDOAN == 3)
                    //{
                    //    dt.AHC_PHUCTHAM_THAMGIATOTUNG.Where(x => x.DONID == DONID)
                    //       .ToList()
                    //       .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.HOTEN, x.TUCACHTGTTID + "," + x.ID)));
                    //}
                    ...
                    var atddtisSENT = dt.AHC_TONGDAT_DOITUONG.Where(x => x.TONGDATID == tdId).ToList();
                    if (atddtisSENT.Count > 0)
                    {
                        var tddt = atddtisSENT.Select(x => x.DUONGSUID).Where(x => x != null).ToList();
                        lstTGTT = new List<KeyValuePair<string, string>>();
                        dt.AHC_DON_DUONGSU.Where(x => x.DONID == DONID && x.TUCACHTOTUNG_MA != ENUM_HANHCHINH_TUCACHTOTUNG.QUYENNVLQ && !tddt.Contains(x.ID)).ToList()
                        .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.TENDUONGSU, x.TUCACHTOTUNG_MA + "1" + "," + x.ID)));
                    }
                    //---------2/11/2025----  vnpt chỉnh lấy thêm cột checkVNEID
                ...
                {
                    //---------2/12/2025----  vnpt chỉnh lấy thêm cột checkVNEID
        ...
        protected void btn_insert_Click(object sender, EventArgs e)
        {
            if (cmdLammoi.Visible == false)
            {
                ...
                decimal bieu_mau_id = Convert.ToDecimal(ddlBieumau.SelectedValue.Split('_')[0]);
                ANPHI_ID = Convert.ToDecimal(ddlBieumau.SelectedValue.Split('_')[1]);
                //---------2/12/2025----  vnpt - Thêm biến FILEID để lọc riêng từng văn bản trùng
                AHC_FILE oF = new AHC_FILE();
                if (ddlBieumau.SelectedValue.Split('_').Length > 2)
                {
                    FILEID = Convert.ToDecimal(ddlBieumau.SelectedValue.Split('_')[2]);
                ...
                //AHC_FILE oF = dt.AHC_FILE.Where(x => x.DONID == DONID && x.BIEUMAUID == bieu_mau_id).FirstOrDefault();
                //if (oF != null) BMID = oF.BIEUMAUID + "" == "" ? 0 : (decimal)oF.BIEUMAUID;
                ...
                if (IS_TD_DUONGSU == 1)
                {
                    ...
                    {
                        //---------2/12/2025---- Đinh Hoàng Sơn vnpt - Thêm biến FILEID để lọc riêng từng văn bản trùng
                        oDTDS = TONGDAT_BL.AHC_TONGDATDOITUONG_GETBYFILEID(DONID, TOAANID, BMID, 0, FILEID);
                    }
            ...
            else
            {
                ...
                if (oBM.MABM == "100-DS") // 100-DS. Thông báo về việc miễn tạm ứng án phí  
                {
                    nguoiNhans = TONGDATBL.AHC_TONGDATDOITUONG_ANPHI_GETBYTONGDATID(TongDatID);
                }
                else
                {
                    nguoiNhans = TONGDATBL.AHC_TONGDATDOITUONG_GETBYTONGDATID(TongDatID);
                }
            ...
            foreach (DataGridItem item in dgTructiep.Items)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        // vnpt 03/12/2025
                        //AHC_DON_THAMGIATOTUNG tgtt = dt.AHC_DON_THAMGIATOTUNG.FirstOrDefault(x => x.ID == nguoiID);
                        AHC_DON_DUONGSU tgtt = dt.AHC_DON_DUONGSU.FirstOrDefault(x => x.ID == nguoiID);
                        if (tgtt != null)
                        {
                            string diaChi = "";
        ...
        {
            BL.GSTP.AHC.AHC_TONGDAT_BL tONGDAT_BL = new BL.GSTP.AHC.AHC_TONGDAT_BL();
            ...
            foreach (DataGridItem item in dgList.Items)
            {
                ...
                {
                    if (status == 3 && status == 4 && status == 5)//13/01/2025
                    {
                        isPhatHanh = true;
                    }
                    else
                    ...
                    if ((status == 5 || status == 6) && V_NGAYPHATHANH == "")///13/01/2025
                    {
                        isDaGui = true;
                    }
                }
            ...
            try
            {
                ...
                // Call API
                string result = CallApiThuhoi("6", tongDatIds, LyDoThuHoi, NgayThuHoi, NguoiSua);
                JToken jObject = JToken.Parse(result);
                if ((string)jObject["status"] == "SUCCESS")
                {
                    lbThongBaoThuHoi.Text = "Thu hồi thành công !";
                    ...
                    foreach (DataGridItem item in dgList.Items)
                    {
                        ...
                        if (cb_thuhoi.Checked == true)
                        {
                            //AnhPN log
                            LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                            var data = DataExtensions.FindById<AHC_TONGDAT>(v_id);
                            var CONTENT_JSON_OLD = JsonConvert.SerializeObject(data, Formatting.Indented);
                            log.InsertLog("AHC_TONGDAT", v_id.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Thu hồi", LyDoThuHoi, "", CONTENT_JSON_OLD);

                            //AnhPN log
                            List<AHC_TONGDAT_DOITUONG> lst = dt.AHC_TONGDAT_DOITUONG.Where(x => x.TONGDATID == v_id).ToList();
                            CONTENT_JSON_OLD = JsonConvert.SerializeObject(lst.FirstOrDefault(), Formatting.Indented);
                            log.InsertLog("AHC_TONGDAT_DOITUONG", v_id.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Thu hồi", LyDoThuHoi, "", CONTENT_JSON_OLD);

                            tONGDAT_BL.AHC_TONGDAT_THUHOI(v_id, NgayThuHoi, LyDoThuHoi, NguoiSua);
                        }
                    }
                }
                else
        ...
        {
            decimal? TongDatID = null, curTongDatID = null;
            int cntCheckbox = 0;
            ...
            foreach (DataGridItem item in dgList.Items)
            {
                ...
                curTongDatID = Convert.ToDecimal(item.Cells[1].Text);
                if (curTongDatID == TongDatID)
                {
                    ...
                    if ((TrangThai == 5 || TrangThai == 6) && V_NGAYPHATHANH == "")//14/01/2025
                    {
                        canDelete = true;
                    }
                }
        ...
        public List<TongDatDTO> GetAnHanhChinh(List<decimal> ids)
        {
            List<AHC_TONGDAT> ahcTongDats = dt.AHC_TONGDAT.Where(x => ids.Contains(x.ID) && (x.TRANGTHAI >= 0 && x.TRANGTHAI <= 2)).ToList();
            HashSet<decimal?> idBieuMaus = new HashSet<decimal?>(ahcTongDats.Select(x => x.BIEUMAUID).ToList());
            ...
            HashSet<decimal?> idAHC_DON = new HashSet<decimal?>(ahcTongDats.Select(x => x.DONID).ToList());
            List<AHC_DON> allAHCDon = dt.AHC_DON.Where(x => idAHC_DON.Contains(x.ID)).ToList();
            Dictionary<decimal?, AHC_DON> mapAhcDon = new Dictionary<decimal?, AHC_DON>();
            Dictionary<decimal?, List<AHC_DON_DUONGSU>> mapDuongSu = dt.AHC_DON_DUONGSU
                .Where(x => idAHC_DON.Contains(x.DONID))
                .GroupBy(x => x.DONID, x => x)
                .ToDictionary(x => x.Key, x => x.ToList());
            // bo xung thong tin van ban di
            foreach (AHC_DON ahcDon in allAHCDon)
            {
                mapAhcDon.Add(ahcDon.ID, ahcDon);
            }
            ...
            HashSet<string> usernames = new HashSet<string>(ahcTongDats.Where(x => x.NGUOITAO != null).ToList().Select(x => x.NGUOITAO).ToList());
            ...
            string conditions = "(";
            foreach (AHC_TONGDAT tongDat in ahcTongDats)
            {
            ...
            List<TongDatDTO> danhSachTongDat = ahcTongDats.Select(x => ToDTO(x, mapBieuMau, mapUser, mapAhcDon, mapDuongSu)).ToList();
            ...
            List<AHC_TONGDAT_DOITUONG> allDoiTuongs = dt.AHC_TONGDAT_DOITUONG
            ...
            Dictionary<decimal, AHC_DON_DUONGSU> donDuongSu = dt.AHC_DON_DUONGSU
            ...
            foreach (TongDatDTO dto in danhSachTongDat)
            {
                List<AHC_TONGDAT_DOITUONG> subList = allDoiTuongs.Where(x => dto.ID.Equals(x.TONGDATID) && x.TRANGTHAI == 1).ToList();
        ...
        public List<AHC_TONGDAT> GetAnHanhChinhThuHoi(List<decimal> ids)
        {
            List<AHC_TONGDAT> ahcTongDats = dt.AHC_TONGDAT.Where(x => ids.Contains(x.ID)).ToList();
            return ahcTongDats;
        }

        private TongDatDTO ToDTO<T>(T model, Dictionary<decimal?, DM_BIEUMAU> mapBieuMau, Dictionary<string, decimal?> mapUser, Dictionary<decimal?, AHC_DON> mapAHC_DON, Dictionary<decimal?, List<AHC_DON_DUONGSU>> mapDuongSu)
        {
            if (model == null)
                return null;
            ...
            if (model.GetType() == typeof(AHC_TONGDAT))
            {
                ...
                dto.Mavuviec = dto.DonId == null ? null : mapAHC_DON[dto.DonId].MAVUVIEC;
                dto.Tenvuviec = dto.DonId == null ? null : mapAHC_DON[dto.DonId].TENVUVIEC;
                dto.Magiaidoan = dto.DonId == null ? null : mapAHC_DON[dto.DonId].MAGIAIDOAN;
                dto.qhpl = dto.DonId == null ? null : mapAHC_DON[dto.DonId].QUANHEPHAPLUAT_NAME;
                //dto.mapID = dto.mapID;
                //dto.mapTable = dto.mapTable;
                ...
                if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "04-HC")
                {
                    List<AHC_DON_XULY> aHC_DON_XULies = dt.AHC_DON_XULY.Where(x => x.DONID == entity.DONID).ToList();
                    if (aHC_DON_XULies != null && aHC_DON_XULies.Any())
                    {
                        dto.Sothongbao = Convert.ToString(aHC_DON_XULies[0].SOTHONGBAO);
                        dto.Ngaythongbao = Convert.ToDateTime(aHC_DON_XULies[0].NGAYTHONGBAO);
                    }
                    // lay thong tin can bo
                    decimal? canboid = dt.AHC_DON_THAMPHAN.Where(x => x.DONID == entity.DONID && x.MAVAITRO == "VTTP_GIAIQUYETDON").Select(x => x.CANBOID).Take(1).toNumber();
                    dto.Nguoiky = canboid;
                }
                else if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "100-DS") // 100-DS. Thông báo về việc miễn tạm ứng án phí 
                {
                    AHC_ANPHI aDS_ANPHI = dt.AHC_ANPHI.Where(x => x.ID == entity.MAPID).FirstOrDefault();
                    DON_MIENANPHI_BL dON_MIENANPHI = new DON_MIENANPHI_BL();
                    ...
                    if (aDS_ANPHI != null)
                    {
                        ...
                        //dto.mathongbao = dVCQG_THANH_TOAN.DVCQG_THANH_TOAN_SEARCH(2, entity.DONID.Value, aDS_ANPHI.ID, "6");
                    }

                }
                else if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "06-HC")
                {
                    List<AHC_SOTHAM_THULY> aHC_DON_XULies = dt.AHC_SOTHAM_THULY.Where(x => x.DONID == entity.DONID).ToList();
                    if (aHC_DON_XULies != null && aHC_DON_XULies.Any())
                    {
                        dto.Sothongbao = Convert.ToString(aHC_DON_XULies[0].SOTHONGBAO);
                        dto.Ngaythongbao = Convert.ToDateTime(aHC_DON_XULies[0].NGAYTHONGBAO);
                ...
                else if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "35-HC")
                {
                    List<AHC_PHUCTHAM_THULY> aHC_DON_XULies = dt.AHC_PHUCTHAM_THULY.Where(x => x.DONID == entity.DONID).ToList();
                    if (aHC_DON_XULies != null && aHC_DON_XULies.Any())
                    {
                        dto.Sothongbao = Convert.ToString(aHC_DON_XULies[0].SOTHONGBAO);
                        dto.Ngaythongbao = Convert.ToDateTime(aHC_DON_XULies[0].NGAYTHONGBAO);
                        dto.Nguoiky = aHC_DON_XULies[0].NGUOIKYID;
                    }
                }
                else if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "22-HC") // 22-HC. Bản án hành chính sơ thẩm vnpt Lê Bá Thọ 10/12/2025 lấy số cho vbdh
                {
                    AHC_SOTHAM_BANAN ba = dt.AHC_SOTHAM_BANAN.Where(x => x.DONID == entity.DONID).FirstOrDefault();
                    if (ba != null)
                    {
                        ...
                        AHC_SOTHAM_HDXX oHD = dt.AHC_SOTHAM_HDXX.Where(x => x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN && x.DONID == entity.DONID).OrderByDescending(x => x.NGAYPHANCONG).FirstOrDefault();
                        if (oHD != null)
                        {
                ...
                else if (dM_BIEUMAU != null && dM_BIEUMAU.MABM == "46-HC") // 46-HC. Bản án hành chính phúc thẩm vnpt Lê Bá Thọ 10/12/2025 lấy số cho vbdh
                {
                    AHC_PHUCTHAM_BANAN ba = dt.AHC_PHUCTHAM_BANAN.Where(x => x.DONID == entity.DONID).FirstOrDefault();
                    if (ba != null)
                    {
                        dto.Sothongbao = Convert.ToString(ba.SOBANAN);
                        dto.Ngaythongbao = Convert.ToDateTime(ba.NGAYTUYENAN);
                        //load người kí - mặc định chủ tọa
                        AHC_PHUCTHAM_HDXX oHD = dt.AHC_PHUCTHAM_HDXX.Where(x => x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN && x.DONID == entity.DONID).OrderByDescending(x => x.NGAYPHANCONG).FirstOrDefault();
                        if (oHD != null)
                        {
                            DM_CANBO oTPCT = dt.DM_CANBO.Where(x => x.ID == oHD.CANBOID).FirstOrDefault();
                            if (oTPCT != null)
                ...
                {
                    List<AHC_FILE> aHC_FILEs = dt.AHC_FILE.Where(x => x.ID == entity.FILEID).ToList();
                    if (aHC_FILEs != null && aHC_FILEs.Any())
                    {
                        decimal? maGiaiDoan = aHC_FILEs[0].MAGIAIDOAN;
                        // so tham
                        if (maGiaiDoan != null && maGiaiDoan == 2)
                        {
                            List<AHC_SOTHAM_QUYETDINH> aHC_SOTHAM_QUYETDINHs = dt.AHC_SOTHAM_QUYETDINH.Where(x => x.QUYETDINHID == dM_BIEUMAU.QUYETDINHID && x.DONID == entity.DONID).ToList();
                            if (aHC_SOTHAM_QUYETDINHs != null && aHC_SOTHAM_QUYETDINHs.Any())
                            {
                                dto.Sothongbao = Convert.ToString(aHC_SOTHAM_QUYETDINHs[0].SOQD);
                                dto.Ngaythongbao = Convert.ToDateTime(aHC_SOTHAM_QUYETDINHs[0].NGAYQD);
                                dto.Nguoiky = Convert.ToDecimal(aHC_SOTHAM_QUYETDINHs[0].NGUOIKYID);
                            }
                        }
                        // phuc tham
                        else if (maGiaiDoan != null && maGiaiDoan == 3)
                        {
                            List<AHC_PHUCTHAM_QUYETDINH> aHC_PHUCTHAM_QUYETDINHs = dt.AHC_PHUCTHAM_QUYETDINH.Where(x => x.QUYETDINHID == dM_BIEUMAU.QUYETDINHID && x.DONID == entity.DONID).ToList();
                            if (aHC_PHUCTHAM_QUYETDINHs != null && aHC_PHUCTHAM_QUYETDINHs.Any())
                            {
                                dto.Sothongbao = Convert.ToString(aHC_PHUCTHAM_QUYETDINHs[0].SOQD);
                                dto.Ngaythongbao = Convert.ToDateTime(aHC_PHUCTHAM_QUYETDINHs[0].NGAYQD);
                                dto.Nguoiky = Convert.ToDecimal(aHC_PHUCTHAM_QUYETDINHs[0].NGUOIKYID);
                            }
                        }
                    }
                }
            ...
            else
            {
                try
                {
                    ...
                    if (status == "SUCCESS")
                    {
                        ...
                        LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                        //var CONTENT_JSON_OLD = JsonConvert.SerializeObject(data, Formatting.Indented);
                        log.InsertLog("CallApi_AHC", TongDatID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Gửi sang VBĐH", "Gửi thành công", "", "");
                    }
                    else
                    {
                        ...
                        LOG_TONGDAT_QLTA log = new LOG_TONGDAT_QLTA();
                        //var CONTENT_JSON_OLD = JsonConvert.SerializeObject(data, Formatting.Indented);
                        log.InsertLog("CallApi_AHC", TongDatID.ToString(), Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Gửi sang VBĐH", "Gửi thất bại", "", "");
                    }
                }
                catch (Exception)
                {
        ...
        protected void ddlTCTT_SelectedIndexChanged(object sender, EventArgs e)
        {
            foreach (DataGridItem item in dgTructiep.Items)
            {
                ...
                if (oT.MAGIAIDOAN == 2)
                {
                    ...
                    {
                        dt.AHC_DON_THAMGIATOTUNG.Where(x => x.DONID == DONID && x.TUCACHTGTTID == ddlTCTT.SelectedValue)
                                                .ToList()
                                                .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.HOTEN, x.TUCACHTGTTID + "," + x.ID)));
                    }
                }
                else if (oT.MAGIAIDOAN == 3)
                {
                    ...
                    {
                        dt.AHC_PHUCTHAM_THAMGIATOTUNG.Where(x => x.DONID == DONID && x.TUCACHTGTTID == ddlTCTT.SelectedValue)
                                                    .ToList()
                                                    .ForEach(x => lstTGTT.Add(new KeyValuePair<string, string>(x.HOTEN, x.TUCACHTGTTID + "," + x.ID)));
                    }
                }
        ...
        private string GetSoThongBao(decimal bieuMauID, decimal donID, decimal duongsuID)
        {
            ...
            {
                AHC_ANPHI anPhi = dt.AHC_ANPHI.Where(x => x.DONID == donID && x.DUONGSU_ID == duongsuID).FirstOrDefault();
                if (anPhi != null)
                {
            ...
            else if (bieu_mau.MABM == "100-DS") // 100-DS. Thông báo về việc miễn tạm ứng án phí 
            {
                ...
                DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(anPhi.ID, 6);
                if (anPhi != null)
                {
            ...
            {
                AHC_SOTHAM_THULY thuly = dt.AHC_SOTHAM_THULY.Where(x => x.DONID == donID).FirstOrDefault();
                if (thuly != null)
                {
            ...
            else if (bieu_mau.QUYETDINHID != null) // Các biểu mẫu quyết định vụ việc
            {
                ...
                {
                    AHC_SOTHAM_QUYETDINH qdinh = dt.AHC_SOTHAM_QUYETDINH.Where(x => x.DONID == donID).FirstOrDefault();
                    if (qdinh != null)
                    {
                        STB = qdinh.SOQD;
                    }
                ...
                {
                    AHC_PHUCTHAM_QUYETDINH qdinh = dt.AHC_PHUCTHAM_QUYETDINH.Where(x => x.DONID == donID).FirstOrDefault();
                    if (qdinh != null)
                    {
                        STB = qdinh.SOQD;
                    }
