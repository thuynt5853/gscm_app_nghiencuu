...
{
    public partial class p_ToiDanh_cc : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                decimal vBicaoID = Convert.ToDecimal(hddBiCaoID.Value);
                GDTTT_VUAN_DUONGSU obj_ds = dt.GDTTT_VUAN_DUONGSU.Where(x => x.ID == vBicaoID).Single<GDTTT_VUAN_DUONGSU>();
                if (obj_ds != null) {
                    lblTenBicao.InnerText = obj_ds.TENDUONGSU;
                }
        ...
        private void LoadDropToiDanh(DropDownList dropToiDanh)
        {
            ...
            dropToiDanh.Items.Insert(0, new System.Web.UI.WebControls.ListItem("Chọn", "0"));
        }
        ...
        {
            //------------------------------
            GDTTT_VUAN_DUONGSU obj_ds = dt.GDTTT_VUAN_DUONGSU.Where(x => x.ID == DuongSuID).Single<GDTTT_VUAN_DUONGSU>();
            decimal currtoidanhId = Convert.ToDecimal(dropBiCao_ToiDanh.SelectedValue);
            List<GDTTT_VUAN_DUONGSU_TOIDANH> lst = dt.GDTTT_VUAN_DUONGSU_TOIDANH.Where(x => x.DUONGSUID == DuongSuID
                                                                                            && x.TOIDANHID == currtoidanhId).ToList();
            if (lst == null || lst.Count == 0)
            {
                GDTTT_VUAN_DUONGSU_TOIDANH obj = new GDTTT_VUAN_DUONGSU_TOIDANH();
                obj.DUONGSUID = DuongSuID;
                obj.VUANID = obj_ds.VUANID;
        ...
        protected void rptToiDanh_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                //    curr_id = Convert.ToDecimal(e.CommandArgument.ToString());
                //    decimal vBicaoID = Convert.ToDecimal(hddBiCaoID.Value);
                //    // 1.Reset toàn bộ ISMAIN = 0 toi danh cau bi cao
                //    UpdateAllIsMainToZero(vBicaoID);

                //    // 2.Set Toi danh bị cao được chọn = 1
                //    UpdateIsMain(curr_id, vBicaoID);

                //    // 3️.Load lại dữ liệu
                //    LoadDsToiDanh(vBicaoID);
        ...
        protected void chkChon_CheckedChanged(object sender, EventArgs e)
        {
            ...
            HiddenField hdd = (HiddenField)item.FindControl("hddDSToiDanhID");

            if (hdd == null) return;

            decimal toiDanhID = Convert.ToDecimal(hdd.Value);
            decimal biCaoID = Convert.ToDecimal(hddBiCaoID.Value);

            // 1.Reset toàn bộ ISMAIN = 0 toi danh cau bi cao
            UpdateAllIsMainToZero(biCaoID);

            // 2.Set Toi danh bị cao được chọn = 1
            UpdateIsMain(toiDanhID, biCaoID);

            // 3️.Load lại dữ liệu
            LoadDsToiDanh(biCaoID);
        }

        private void UpdateAllIsMainToZero( decimal BiCaoID)
        {
            List<GDTTT_VUAN_DUONGSU_TOIDANH> lst = dt.GDTTT_VUAN_DUONGSU_TOIDANH.Where(x => x.DUONGSUID == BiCaoID
                                                                                      ).ToList();
        ...
        private void UpdateIsMain(decimal id, decimal BiCaoID)
        {
            GDTTT_VUAN_DUONGSU_TOIDANH obToiDanh = dt.GDTTT_VUAN_DUONGSU_TOIDANH.Where(x => x.ID == id && x.DUONGSUID == BiCaoID).SingleOrDefault();
            obToiDanh.ISMAIN = 1;
            dt.SaveChanges();
