...
namespace WEB.GSTP.QLAN.GDTTT.Hoso
{
    public partial class QuanlySoVBVAKN : System.Web.UI.Page
    {
        ...
        private DataTable getDS()
        {
            ...
            DataTable oDT = oBL.GDTTT_QLSOVBVAKN_SEARCH(CurrDonViID, PhongBanID, TYPE_SOVB_DONVI.HCTP, null, LoaiSO, SoCongVan, NgayCongVan, NgayCongVan_den, vloaiAn, page_size, pageindex);
            return oDT;
        }
            ...
            switch (e.CommandName)
            {
                case "Sua":
                    ...
                    else
                    {
                        ...
                        Response.Redirect("Popup/pSuaCongvanVAKN.aspx?ID=" + e.CommandArgument.ToString());
                        Load_Data();
                    }
                    break;
                case "Xoa":
                    ...
                    if (oDT.Rows.Count > 0 && Convert.ToDecimal(oDT.Rows[0]["vcheck"]) > 0)
                    {
                        strMsg = "Vụ án đã được chuyển cho thẩm phán, không được phép xóa!";
                    }
                    else
                    {
                        //----Xoa ở bang So van ban-----------------------
                        oBLDel.DELETE_ALL_SOVANBANVAKN(ID);
                        strMsg = "Xóa thành công Vụ án!";
