...
{
    public partial class SuaToaKhac : System.Web.UI.Page
    {
        ...
        private void SaveData()
        {
            foreach (DataGridItem item in dgList.Items)
            {
                ...
                DropDownList ddlNoiChuyen = (DropDownList)item.FindControl("ddlNoiChuyen");
                ...
                obj.CD_TK_DONVIID = Convert.ToDecimal(ddlNoiChuyen.SelectedValue);
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                try
                {
                    ...
                    DropDownList ddlNoiChuyen = (DropDownList)e.Item.FindControl("ddlNoiChuyen");
                    ddlNoiChuyen.DataSource = dtToaAn;
                    ddlNoiChuyen.DataTextField = "MA_TEN";
                    ddlNoiChuyen.DataValueField = "ID";
                    ddlNoiChuyen.DataBind();
                    ddlNoiChuyen.Items.Insert(0, new ListItem("--- Chọn --- ", "0"));
                    HiddenField hddNoiChuyenID = (HiddenField)e.Item.FindControl("hddNoiChuyenID");
                    ...
                    string strNoiChuyenID = hddNoiChuyenID.Value + "";
                    if (strNoiChuyenID != "" && strNoiChuyenID != "0")
                        ddlNoiChuyen.SelectedValue = strNoiChuyenID;

                    //Kiểm tra, cảnh báo nơi chuyển đến
                    if (toaid != "" && toaid != "0" && strNoiChuyenID != "" && strNoiChuyenID != "0")
                    {
                        decimal ToaXXID = Convert.ToDecimal(toaid);
                        DM_TOAAN objTAXX = dt.DM_TOAAN.Where(x => x.ID == ToaXXID).FirstOrDefault();
                        if (objTAXX.LOAITOA == "CAPTINH" || objTAXX.LOAITOA == "CAPHUYEN")
                        {
                            if(objTAXX.LOAITOA == "CAPHUYEN")
                                objTAXX = dt.DM_TOAAN.Where(x => x.ID == objTAXX.CAPCHAID).FirstOrDefault();
                            decimal ToaCID = Convert.ToDecimal(strNoiChuyenID);
                            DM_TOAAN objTAC = dt.DM_TOAAN.Where(x => x.ID == ToaCID).FirstOrDefault();
                            if (objTAC.LOAITOA == "CAPCAO" && objTAXX.CAPCHAID != ToaCID)
                            {
                                TextBox txtGhichu = (TextBox)e.Item.FindControl("txtGhichu");
                                txtGhichu.BackColor = System.Drawing.Color.Red;
                            }
                        }
                    }
                }
