...
namespace WEB.GSTP.QLAN.GDTTT.Hoso
{
    public partial class Danhsachdon_cc : System.Web.UI.Page
    {
        ...
        private DataTable getDS(Decimal V_GET_LIS_ID, bool isCV, bool isOnPrint, bool isTraigiam, bool isChiDao, bool isTBQuahan)
        {
            ...
            DataTable oDT = oBL.GDTTT_DON_SEARCH(V_GET_LIS_ID, Drop_NDBD.SelectedValue,txt_NDBD.Text,Drop_NOI_NHAN_SEARCH.SelectedValue, Drop_TRANGTHAICHUYEN.SelectedValue, Drop_LOAI_VB_Search.SelectedValue, txt_SODEN_SEARCH.Text.Trim(), txt_SODEN_SEARCH_DEN.Text.Trim()
       ...
       protected void btnGXN_Click(object sender, EventArgs e)
        {
            ...
            if(checkToaAn == 4) //Tòa Hà Nội
            {
                ...
                {
                    string fileName = "";
                    string saveAs = pathTemplateWord + "rptGiayXacNhan_" + Session[ENUM_SESSION.SESSION_USERID] + DateTime.Now.Minute + ".doc";
                    ...
                    foreach (DataRow item in tbl.Rows)
                    {
                        ...
                        string strNgay = "", strThang = "", strNam = "";
                        if (item["SOVANBAN"].ToString().Trim() != "")
                        {
                        ...
                        if (item["LOAIDON"].ToString() == "đơn đề nghị")
                        {
                            if (item["LOAIAN"].ToString() == "HS")
                            {
                                ...
                                {
                                    fileName = pathTemplateWord + "rptGiayXacNhan_CN_HS.doc";
                                }
                                else
                                {
                                    fileName = pathTemplateWord + "rptGiayXacNhan_CN_HS_DKN.doc";
                                }
                            }
                            else
                            {
                                if (String.IsNullOrEmpty(item["NGUOIGUI_DONGKHIEUNAI"].ToString()))
                                {
                                    fileName = pathTemplateWord + "rptGiayXacNhan_CN.doc";
                                }
                                else
                                {
                                    fileName = pathTemplateWord + "rptGiayXacNhan_CN_DKN.doc";
                                }
                            }
                        ...
                        else
                        {
                            if (String.IsNullOrEmpty(item["NGUOIGUI_DONGKHIEUNAI"].ToString()))
                            {
                                fileName = pathTemplateWord + "rptGiayXacNhan_DV.doc";
                            }
                            else
                            {
                                fileName = pathTemplateWord + "rptGiayXacNhan_DV_DKN.doc";
                            }
        ...
        protected void btnTKPhieuchuyen_Click(object sender, EventArgs e)
        {
            ...
            if (oDT != null)
            {
                ...
                foreach (DataRow obj in oDT.Rows)
                {
                    ...
                    //Template new
                    string fileName = pathTemplateWord + "rptPhieuChuyenDon_ToaKhac.doc";
                    string fileNameSave = "rptPhieuChuyenDon_ToaKhac_" + r.NGUOIGUI.ToString().Split(',').First() + ".doc";
                    string saveAs = pathTemplateWord + "rptPhieuChuyenDon_ToaKhac_" + r.NGUOIGUI + Session[ENUM_SESSION.SESSION_USERID] + DateTime.Now.Minute + ".doc";
                    Document baoCao = new Document(fileName);
                    baoCao.MailMerge.Execute(new[] { "TENDONVIHOA" }, new[] { r.TENDONVI.ToUpper() });
