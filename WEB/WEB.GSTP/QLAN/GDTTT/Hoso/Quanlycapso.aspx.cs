...
{
    public partial class Quanlycapso : System.Web.UI.Page
    {
        ...
        private void LoadDropDownList()
        {
            ...
            {
                if (PhongBanID == 1)
                    tblso = soBL.DM_DATAITEM_GETBYGROUPNAME("QLSO_HCTP_TC");
                else
                    tblso = soBL.DM_DATAITEM_GETBYGROUPNAME("QLSO_THAMPHAN");

            }
            else
                tblso = soBL.DM_DATAITEM_GETBYGROUPNAME("QLSO");
            if (tblso.Rows.Count > 0)
            {
                ...
                if (CurrDonViID == 1)
                {
                    int rowCount = tblso.Rows.Count;
                    ddlLoaiso.Items.Insert(rowCount, new ListItem("Sổ Thụ lý mới", "SOTHULY"));
                    rowCount++;
                    ddlLoaiso.Items.Insert(rowCount, new ListItem("Sổ Thụ lý xét xử GDTT", "SOTHULYXX"));
                }
            }
        ...
        private DataTable getDS()
        {
            ...
            DataTable oDT;
            if (oCB.CHUCDANHID != 486)
                //Sổ dành cho đơn vị
                oDT = oBL.GDTTT_QLSOVB_SEARCH(CurrDonViID, PhongBanID, 1, null, LoaiSO, SoCongVan, NgayCongVan, NgayCongVan_den, vloaiAn, page_size, pageindex);
            else
                //So cá nhân cho Thẩm phán tối cao
                oDT = oBL.GDTTT_QLSOVB_SEARCH(CurrDonViID, PhongBanID, 0, CanboID.ToString(), LoaiSO, SoCongVan, NgayCongVan, NgayCongVan_den, vloaiAn, page_size, pageindex);
            return oDT;
        }
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "Sua":
                    ...
                    {
                        ID = Convert.ToDecimal(strSplit[0]);
                        if (strSplit[1] == "SOVUAN") //sổ văn bản của vụ án kháng nghị
                        {
                            oDT = oBLDel.CHECK_GDTTT_SUASOVBVAKN_HCTP(ID);
                            ...
                            else
                            {
                                ...
                                Response.Redirect("Popup/pSuaCongvanVAKN.aspx?ID=" + ID);
                                Load_Data();
                            }
                        }
                        else
                        {
                            oDT = oBLDel.CHECK_GDTTT_SUASOVB(ID);
                            if (oDT.Rows.Count > 0 && Convert.ToDecimal(oDT.Rows[0]["vcheck"]) > 0)
                            {
                            ...
                            {
                                Session[SS_TK.ISHOME] = 1;
                                Response.Redirect("Popup/pSuaCongvan.aspx?ID=" + ID);
                                Load_Data();
                            }
                        }
                    }
                ...
                case "Xoa":
                    if (strSplit.Length == 2)
                    {
                        ...
                        if (strSplit[1] == "SOVUAN") //sổ văn bản của vụ án kháng nghị
                        {
                            oDT = oBLDel.CHECK_GDTTT_SUASOVBVAKN_HCTP(ID);
                            if (oDT.Rows.Count > 0 && Convert.ToDecimal(oDT.Rows[0]["vcheck"]) > 0)
                                strMsg = "Văn bản này thuộc Vụ án có đề xuất ý kiến kháng nghị đã được chuyển cho thẩm phán, nên không được phép xóa!";
                            else
                            {
                        ...
                        else
                        {
                            ...
                            {
                                //----Xoa ở bang So van ban-----------------------
                                oBLDel.DELETE_ALL_SOVANBAN(ID);
                                strMsg = "Xóa thành công Văn bản!";
                                //----Xoa ơ đon đến khi xong các chuc nang in thi bỏ ----------------
