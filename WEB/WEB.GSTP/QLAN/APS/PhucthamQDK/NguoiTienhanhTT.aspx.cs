...
namespace WEB.GSTP.QLAN.APS.PhucthamQDK
{
    public partial class NguoiTienhanhTT : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    //    int countItem = dt.APS_DON_THAMPHAN.Count(s => s.THUKYID == CurrentUserId && s.DONID == DONID && s.MAVAITRO == "VTTP_GIAIQUYETPHUCTHAM");
                    //    if (countItem > 0)
                    //    {
                    //        //được gán
                    //    }
        ...
        private void check_Quyen(decimal DONID)
        {
            ...
            DataTable oDT = oBL.APS_KCKNQDK_PHUCTHAM_QUYETDINH_GETLIST(DONID);
        ...
        private void LoadCombobox()
        {
            ...
            if (loaiQD != null)
            {
                //APS_PHUCTHAM_QUYETDINH qd = dt.APS_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.LOAIQDID == loaiQD.ID).FirstOrDefault<APS_PHUCTHAM_QUYETDINH>();
                APS_KCKNQDK_PHUCTHAM_QUYETDINH qd = DataExtensions.GetAllWithClause<APS_KCKNQDK_PHUCTHAM_QUYETDINH>
                        ("DONID = " + DonID + " and LOAIQDID = " + loaiQD.ID).FirstOrDefault<APS_KCKNQDK_PHUCTHAM_QUYETDINH>();
                if (qd != null)
                {
                    APS_DON_THAMPHAN tpgq = dt.APS_DON_THAMPHAN.Where(x => x.DONID == DonID && x.MAVAITRO == "VTTP_GIAIQUYETPHUCTHAM").OrderByDescending(x => x.NGAYTAO).FirstOrDefault<APS_DON_THAMPHAN>();
                    if (tpgq != null)
                    {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                APS_KCKNQDK_PHUCTHAM_HDXX oND;

                // Lấy count thụ lý và count chủ tọa để cho nhập thêm thẩm phán chủ tọa
                List<APS_DON_THAMPHAN> countTPGQ = dt.APS_DON_THAMPHAN.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM).ToList();
                //List<APS_PHUCTHAM_HDXX> countChuToa = dt.APS_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).ToList();
                List<APS_KCKNQDK_PHUCTHAM_HDXX> countChuToa = DataExtensions.GetAllWithClause<APS_KCKNQDK_PHUCTHAM_HDXX>($"DONID = {DONID} AND MAVAITRO = '{ENUM_NGUOITIENHANHTOTUNG.THAMPHAN}'");
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    if (tucach == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN)
                    {
                        ...
                        //oND = dt.APS_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).FirstOrDefault<APS_PHUCTHAM_HDXX>();
                        oND = DataExtensions.GetAllWithClause<APS_KCKNQDK_PHUCTHAM_HDXX>($"DONID = {DONID} AND MAVAITRO = '{ENUM_NGUOITIENHANHTOTUNG.THAMPHAN}'").FirstOrDefault();
                    ...
                    oND = new APS_KCKNQDK_PHUCTHAM_HDXX();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    //oND = dt.APS_PHUCTHAM_HDXX.Where(x => x.ID == ID).FirstOrDefault();
                    oND = DataExtensions.GetAllWithClause<APS_KCKNQDK_PHUCTHAM_HDXX>("ID = " + ID).FirstOrDefault();
                }
                oND.DONID = DONID;
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    ...
                    //APS_PHUCTHAM_HDXX CheckCanBo = dt.APS_PHUCTHAM_HDXX.Where(x => x.DONID == DONID && x.CANBOID == oND.CANBOID).FirstOrDefault();
                    APS_KCKNQDK_PHUCTHAM_HDXX CheckCanBo = DataExtensions.GetAllWithClause<APS_KCKNQDK_PHUCTHAM_HDXX>
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.APS_KCKNQDK_PHUCTHAM_HDXX_GETLIST(ID);
            if (oDT != null && oDT.Rows.Count > 0)
            {
        ...
        {
            //APS_PHUCTHAM_HDXX oND = dt.APS_PHUCTHAM_HDXX.Where(x => x.ID == id).FirstOrDefault();
            APS_KCKNQDK_PHUCTHAM_HDXX oND = DataExtensions.FindById<APS_KCKNQDK_PHUCTHAM_HDXX>(id);
            //dt.APS_PHUCTHAM_HDXX.Remove(oND);
