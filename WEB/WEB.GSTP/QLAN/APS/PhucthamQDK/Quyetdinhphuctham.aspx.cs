...
namespace WEB.GSTP.QLAN.APS.PhucthamQDK
{
    public partial class Quyetdinhphuctham : System.Web.UI.Page
    {
        ...
        private void check_Quyen(decimal DONID)
        {
            ...
            DataTable oDT = oBL.APS_PHUCTHAMKCKNQDK_BANANQUYETDINH_GETLIST(DONID);
        ...
        private void SetNewSoQD()
        {
            ...
            if (!String.IsNullOrEmpty(txtNgayQD.Text))
                ...
                ngayBD = DateTime.Now;
            txtSoQD.Text = oSTBL.GET_SQD_NEW(DonViID, "APS_PT", ngayBD, LoaiQD).ToString();
        }
        ...
        private void LoadCombobox()
        {
            ...
            decimal DonID = Convert.ToDecimal(hddDonID.Value);
            APS_PHUCTHAM_THULY tl = dt.APS_PHUCTHAM_THULY.Where(x => x.DONID == DonID).OrderByDescending(x => x.NGAYTHULY).FirstOrDefault();
            if (tl != null)
            {
        ...
        private void LoadQD()
        {
            ...
            DataTable oDT = oBL.APS_DM_QUYETDINH_VUAN_PT();
        ...
        private void LoadNguoiKyInfo()
        {
            ...
            APS_KCKNQDK_PHUCTHAM_HDXX oND = DataExtensions.GetAllWithClause<APS_KCKNQDK_PHUCTHAM_HDXX>($"DONID ={DonID} AND MAVAITRO ='{ENUM_NGUOITIENHANHTOTUNG.THAMPHAN}'").OrderByDescending(x => x.ID).FirstOrDefault();
            //  APS_PHUCTHAM_HDXX oND = dt.APS_PHUCTHAM_HDXX.Where(x => x.DONID == DonID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).OrderByDescending(x => x.ID).FirstOrDefault<APS_PHUCTHAM_HDXX>();
            if (oND != null)
            {
        ...
        private bool CheckValid()
        {
            ...
            if (!String.IsNullOrEmpty(txtNgayQD.Text) && !String.IsNullOrEmpty(txtSoQD.Text))
            {
                ...
                ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                Decimal LoaiQD = Convert.ToDecimal(ddlQuyetdinh.SelectedValue);
                Decimal CheckID = oSTBL.CHECK_SQDTheoLoaiAn(DonViID, "APS_PT", so, ngay, LoaiQD);
                if (CheckID > 0)
                {
                    Decimal CurrID = (string.IsNullOrEmpty(hddid.Value)) ? 0 : Convert.ToDecimal(hddid.Value);

                    String strMsg = "";
                    String STTNew = oSTBL.GET_SQD_NEW(DonViID, "APS_PT", ngay, LoaiQD).ToString();
                    if (CheckID != CurrID)
                    {
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.APS_KCKNQDK_PHUCTHAM_QUYETDINH_GETLIST(ID);
        ...
        public void xoa(decimal id)
        {
            ...
            if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.DONID), Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_PHASAN), Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Quyết định Phúc thẩm án Hôn nhân", "Xóa", json) == false)
            {
        ...
        {
            decimal ID = Convert.ToDecimal(hddid.Value);
            APS_KCKNQDK_PHUCTHAM_QUYETDINH oQD = DataExtensions.FindById<APS_KCKNQDK_PHUCTHAM_QUYETDINH>(ID);
            // APS_PHUCTHAM_QUYETDINH oQD = dt.APS_PHUCTHAM_QUYETDINH.Where(x => x.ID == ID).FirstOrDefault();
        ...
        protected void ddlQuyetdinh_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (oT != null)
            {
                ...
                if (ID == 143 || ID == 144 || ID == 145 || ID == 146 || ID == 61)
                {
                    ...
                    ADS_SOTHAM_BL oSTBL = new ADS_SOTHAM_BL();
                    String STTNew = oSTBL.GET_SQD_NEW(DonViID, "APS_PT", ngayQD, ID).ToString();
                    txtSoQD.Text = STTNew;
                }
            }
            LoadLydo();
