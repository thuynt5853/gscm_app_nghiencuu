...
namespace WEB.GSTP.QLAN.APS.PhucthamQDK
{
    public partial class NguoiThamgiaTT : System.Web.UI.Page
    {
        ...
        private void check_Quyen(decimal DONID)
        {
            ...
            //DataTable NTH = oBL.APS_KCKNQDK_PHUCTHAM_HDXX_GETLIST(DONID);
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string current_id = Session[ENUM_LOAIAN.AN_PHASAN] + "";
                decimal DONID = Convert.ToDecimal(current_id);
                APS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG oND;
                if (hddid.Value == "" || hddid.Value == "0")
                    oND = new APS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG();
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    //oND = dt.APS_PHUCTHAM_THAMGIATOTUNG.Where(x => x.ID == ID).FirstOrDefault();
                    oND = DataExtensions.FindById<APS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG>(ID);
                }
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.APS_PHUCTHAM_KCKN_TGTT_GETLIST(ID);
        ...
        public void xoa(decimal id)
        {
            ...
            APS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG oND = DataExtensions.FindById<APS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG>(id);
            if (oND != null)
            {
            ...
            foreach (ListItem oItem in chkListTGTT.Items)
            {
                if (oItem.Selected)
                {
                    ...
                    if (dt.APS_PHUCTHAM_THAMGIATOTUNG.Where(x => x.NTGTT_DONKK_ID == NID).ToList().Count == 0)
                    {
                        ...
                        APS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG oS = new APS_KCKNQDK_PHUCTHAM_THAMGIATOTUNG();
                        oS.DONID = oD.DONID;
                        oS.HOTEN = oD.HOTEN;
