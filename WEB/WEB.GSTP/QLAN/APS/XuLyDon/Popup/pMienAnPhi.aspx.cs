    ...
    public partial class pMienAnPhi : System.Web.UI.Page
    {
        ...
        protected void cmdCapNhatAP_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(APId, 7);
                ...
                try
                {
                    ...
                    if ((dataTable != null && dataTable.Rows.Count > 0 && hddHistoryId.Value != "") || hddHistoryId.Value == "")
                    {
                        ...
                        decimal Mienanphi_ID = dON_MIENANPHI_BL.UPSERT_DON_MIENANPHI(ID_MIENANPHI, APId, DONID, 7, v_lydo, nguoitao, ngaytao, nguoisua, ngaysua, v_sothongbao, d_ngaythongbao, v_stb_phu);
                    ...
                    // lưu ý
                    var DVCQG_THANH_TOAN = DataExtensions.GetAllWithClause<BL.GSTP.BANGSETGET.DVCQG_THANH_TOAN>("ANPHI_ID = " + APId + " AND MALOAIVUVIEC = '7'");
        ...
        private bool CheckValidAP()
        {
            ...
            APS_ANPHI objAnPhi = dt.APS_ANPHI.Where(x => x.ID == A_ANPHI_ID).FirstOrDefault();
            ...
            DON_MIENANPHI_BL dON_MIENANPHI_BL = new DON_MIENANPHI_BL();
            DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(A_ANPHI_ID, 7);
            ...
            if (hddHistoryId.Value != "")
            {
                APS_TONGDAT oTDSua = dt.APS_TONGDAT.Where(x => x.DONID == objAnPhi.DONID).FirstOrDefault();
                APS_TONGDAT_BL dS_TONGDAT_BL = new APS_TONGDAT_BL();
                if (dS_TONGDAT_BL.APS_TONGDATDOITUONG_ANPHI_GETBYANPHIID_TONGDATID(objAnPhi.DONID, objAnPhi.ID).Rows.Count > 0)
                {
                ...
                case "XoaAP":
                    try
                    {
                        ...
                        if (_check_out == 1)
                        {
                            ...
                            APS_TONGDAT oTD = dt.APS_TONGDAT.Where(x => x.DONID == objAP.DONID).FirstOrDefault();
                            APS_TONGDAT_BL dS_TONGDAT_BL = new APS_TONGDAT_BL();
                            if (dS_TONGDAT_BL.APS_TONGDATDOITUONG_ANPHI_GETBYANPHIID_TONGDATID(objAP.DONID, objAP.ID).Rows.Count > 0)
                            {
                            ...
                            else
                            {
                                ...
                                LOG_THONGTINANPHI_QLTA log = new LOG_THONGTINANPHI_QLTA();
                                log.InsertLog(Session[ENUM_LOAIAN.AN_PHASAN].ToString(), CurrID.ToString(), MATHONGBAO, Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Update", CONTENT_JSON, CONTENT_JSON_OLD);
        ...
        protected void rptAP_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                APS_DON_XULY tralaiDonXuly = dt.APS_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 3).FirstOrDefault();

                DON_MIENANPHI_BL dON_MIENANPHI_BL1 = new DON_MIENANPHI_BL();
                DataTable dataTable1 = dON_MIENANPHI_BL1.GET_DON_MIENANPHI_BY_ANPHI_ID(vAPID, 7);
                ...
                {
                    APS_ANPHI objAP = dt.APS_ANPHI.Where(x => x.ID == vAPID).FirstOrDefault();
        ...
        void LoadInfo_AnPhiDaMien(Decimal CurrID)
        {
            ...
            hddHistoryId.Value = CurrID + "";
            APS_ANPHI aDSAnphi = dt.APS_ANPHI.Where(x => x.ID == CurrID).FirstOrDefault();
            if (aDSAnphi != null)
            {
                chkNopAnPhi.Checked = true;
        ...
        private void LoadGrid_AnPhi()
        {
            ...
            decimal A_ANPHI_ID = Convert.ToDecimal(Request.QueryString["ID"].ToString());
            DataTable oDT = oBL.APS_ANPHI_LICHSU_GETBYDONID(DONID, A_ANPHI_ID, 1, pageindex, page_size);
            //if (oDT.Rows.Count > 0)
            //{
