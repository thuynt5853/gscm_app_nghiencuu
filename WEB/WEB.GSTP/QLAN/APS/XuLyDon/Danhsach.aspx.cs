...
namespace WEB.GSTP.QLAN.APS.XuLyDon
{
    public partial class Danhsach : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                sDONID = Session[ENUM_LOAIAN.AN_PHASAN] + "";
                try
                {
                    SetDonGhep();
                    ...
                    APS_DON_XULY oDXuly = dt.APS_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 1).FirstOrDefault();
                    if (oDXuly != null)
                    {
                        APS_CHUYEN_NHAN_AN ca = dt.APS_CHUYEN_NHAN_AN.Where(x => x.VUANID == DONID).FirstOrDefault();
                        if (ca != null)
                        {
        ...
        void SetNewSoTB()
        {
            ...
            APS_DON_BL oSTBL = new APS_DON_BL();
            //Số Thông báo mới
            DateTime ngayTB;
            ...
            String STTNew = oSTBL.GET_STB_XLDon_NEW_V2(DonViID, "APS", ngayTB).ToString();
        ...
        private void CheckQuyen(decimal DONID)
        {
            ...
            List<APS_DON_THAMPHAN> lstCount = dt.APS_DON_THAMPHAN.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETDON).ToList();
            if (lstCount.Count == 0)
            {
            ...
            //List<APS_SOTHAM_THULY> lstTL = dt.APS_SOTHAM_THULY.Where(x => x.DONID == DONID).ToList();
            //if (lstTL.Count > 0)
            //{
        ...
        {
            List<APS_DON_XULY> lstXL = dt.APS_DON_XULY.Where(x => x.DONID == ID && x.LOAIGIAIQUYET == 5).ToList();
            if (lstXL.Count == 0)
            {
                Cls_Comon.SetButton(cmdThemmoiAP, false);
                Cls_Comon.SetButton(cmdCapNhatAP, false);
        ...
        void LoadDropDuongSu(decimal DONID)
        {
            ...
            lbThongbaoAP.Text = "";
            APS_DON_DUONGSU_BL oDSBL = new APS_DON_DUONGSU_BL();
            //Danh mục lý do tra đơn
            DataTable obj = oDSBL.APS_DON_DUONGSU_ANPHI_V2(DONID);
        ...
        {
            APS_DON_XULY_BL oBL = new APS_DON_XULY_BL();
        ...
        protected void rpt_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "Download":
                    ...
                    APS_FILE oND = dt.APS_FILE.Where(x => x.ID == CurrID).FirstOrDefault();
                    if (oND.TENFILE != "")
                    {
                ...
                case "Them":
                    ...
                    if (DonID > 0)
                    {
                        APS_DON_DUONGSU dds = dt.APS_DON_DUONGSU.Where(x => x.DONID == DonID && x.ISDAIDIEN == 1 && x.TUCACHTOTUNG_MA == "NGUOIYEUCAUPS").FirstOrDefault();
                        txtDuongSu.Text = dds.TENDUONGSU;
                    }
                    else
                    {
                        var a = (from d in dt.DON_DUONGSU_CHITIET
                                 join s in dt.APS_DON_DUONGSU on d.DUONGSUID equals s.ID
                                 where d.DONCHITIETID == DonChiTietID && s.TUCACHTOTUNG_MA == "NGUOIYEUCAUPS"
                                 select new
                                 {
                                     s.TENDUONGSU,
                                 }).FirstOrDefault();
                ...
                case "Xoa":
                    ...
                    APS_DON_XULY oT = dt.APS_DON_XULY.Where(x => x.ID == CurrID).FirstOrDefault();
                    ...
                    APS_TONGDAT oTD = dt.APS_TONGDAT.Where(x => x.DONID == oT.DONID).FirstOrDefault();
                    if (oTD != null)
                    {
                    ...
                    string MATHONGBAO = "";
                    var DVCQG_THANH_TOAN = DataExtensions.GetAllWithClause<BL.GSTP.BANGSETGET.DVCQG_THANH_TOAN>("ANPHI_ID = " + CurrID + " AND MALOAIVUVIEC = '7'");
                    ...
                    decimal _VALUE = 0;
                    obj.DVCQG_THANH_TOAN_DELETE_XULY(CurrID, "7", ref _VALUE);

                    if (_VALUE == 1)
                    {
                        DonID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_PHASAN] + "");
                        string StrMsg = "Không được sửa đổi thông tin.";
                        string Result = new APS_CHUYEN_NHAN_AN_BL().Check_NhanAn(DonID, StrMsg, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]));
                        if (Result != "")
                        {
                        ...
                        APS_ANPHI oAnphi = dt.APS_ANPHI.Where(x => x.DONID == oT.DONID).FirstOrDefault();
                        if (oAnphi != null)
                        {
                            ...
                            log.InsertLog(Session[ENUM_LOAIAN.AN_PHASAN].ToString(), CurrID.ToString(), MATHONGBAO, Session[ENUM_SESSION.SESSION_USERNAME].ToString(), "Delete", CONTENT_JSON, CONTENT_JSON_OLD);

                            dt.APS_ANPHI.Remove(oAnphi);
                            //-----
                        }

                        dt.APS_DON_XULY.Remove(oT);
                        ...
                        if (oT.LOAIGIAIQUYET == 4 || oT.LOAIGIAIQUYET == 5)
                        {
                            ...
                            DataTable oYC = oBL.APS_GETALL_DON_YCBS(DonID, CurrID, 1, 5);
                ...
                case "BoSung":
                    ...
                    string link = "/QLAN/APS/XuLyDon/Popup/pBoSungTaiLieu.aspx?ID=" + CurrID;
                ...
                case "TraLaiDon":
                    CurrID = Convert.ToDecimal(e.CommandArgument.ToString());
                    Cls_Comon.CallFunctionJS(this, this.GetType(), "LoadModalDiv();");
                    string linkPop = "/QLAN/APS/XuLyDon/Popup/pTraLaiDon.aspx?ID=" + CurrID;
        ...
        {
            //DONGHEP.Visible = false;
            APS_DON_XULY obj = dt.APS_DON_XULY.Where(x => x.ID == CurrID).Single<APS_DON_XULY>();
            if (obj != null)
            {
                ...
                {
                    APS_FILE objFile = dt.APS_FILE.Where(x => x.ID == obj.FILEID).FirstOrDefault();
        ...
        protected void rpt_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                LinkButton lbtLichSuXuLy = (LinkButton)e.Item.FindControl("lbtLichSuXuLy");
                ...
                else
                {
                    ...
                    if (rowView["ID"] + "" != "")
                    {
                        ...
                        APS_DON_XULY oXL = dt.APS_DON_XULY.Where(x => x.ID == idXuLy).FirstOrDefault();
                        decimal? DONCHITIETID = oXL.DON_CHITIETID;
                        decimal ISDONCHITIET = DON_ID == 0 ? 1 : 0;
                        APS_DON_XULY_BL oBL = new APS_DON_XULY_BL();
                        DataTable oDT = oBL.CHECK_APS_DON_DUONGSU_ANPHI_V2(DONID, DONCHITIETID, ISDONCHITIET);
                        if (oDT.Rows.Count > 0)
                        {
                    ...
                    if (rowView["ID"] + "" != "")
                    {
                        ...
                        decimal idXuLy = Convert.ToDecimal((rowView["ID"] + "").ToString());
                        APS_DON_XULY obj = dt.APS_DON_XULY.Where(x => x.ID == idXuLy).FirstOrDefault();
                        ADS_DON_XULY_BL oBL = new ADS_DON_XULY_BL();
                        DataTable oYC = oBL.APS_GETALL_DON_YCBS(DONID, idXuLy, 1, 5);

                        bool existsYCBS = false;
                        decimal aldThuLy = Convert.ToDecimal(ENUM_ADS_BIENPHAPGQ.ADS_YCBoSungDon);
                        ...
                        //DataTable oDT = oBL.CHECK_LOAIGIAIQUYET_DON_YCBS(idXuLy, 7);
                        //if (oDT.Rows.Count > 0)
                        //{
                    ...
                    if (rowView["ID"] + "" != "")
                    {
                        ...
                        APS_DON_XULY obj = dt.APS_DON_XULY.Where(x => x.ID == idXuLy && x.LOAIGIAIQUYET == adsThuLy).FirstOrDefault();
                    ...
                    APS_SOTHAM_THULY oTLST = dt.APS_SOTHAM_THULY.Where(x => x.DONID == DONID).FirstOrDefault();
        ...
        private decimal UploadFileID(APS_DON oDon, decimal FileID, string strMaBieumau, string STT, string stb_phu)
        {
        ...
        protected void cmdCapNhat_Click(object sender, EventArgs e)
        {
            ...
            string current_id = Session[ENUM_LOAIAN.AN_PHASAN] + "", bienphap = dropBienPhapGQ.SelectedValue;
            decimal DONID = Convert.ToDecimal(current_id);
            ...
            if (hddCurrID.Value != "" && hddCurrID.Value != "0")
            {
                ...
                obj = dt.APS_DON_XULY.Where(x => x.ID == DonXyLyID).FirstOrDefault();
                ...
                else obj = new APS_DON_XULY();
                if (obj.LOAIGIAIQUYET == 4 && bienphap != "4")
                {
                    ADS_DON_XULY_BL oBL = new ADS_DON_XULY_BL();
                    DataTable oYC = oBL.APS_GETALL_DON_YCBS(DONID, obj.ID, 1, 5);
                    if (oYC.Rows.Count > 0) oBL.DEL_DON_YCBS_GETBYDONID(Convert.ToDecimal(oYC.Rows[0]["ID"].ToString()));
                }
            }
            else
            {
                ...
                if (txtSothongbao.Text != "")
                {
                    ...
                    decimal check = oBL.CHECKSTT_APS((decimal)oDon.TOAANID, (decimal)oDon.MAGIAIDOAN, vNam, 0, Convert.ToDecimal(txtSothongbao.Text), ddlStbPhu.SelectedValue, obj.ID);
                    DateTime ngaythongbao = DateTime.Parse(this.txtNgaythongbao.Text.Trim(), cul, DateTimeStyles.NoCurrentDateDefault);
                    String strMsg = "";
                    APS_DON_BL oBL_STB = new APS_DON_BL();
                    String STTNew = oBL_STB.GET_STB_XLDon_NEW_V2((decimal)oDon.TOAANID, "APS", ngaythongbao).ToString();
        ...
        private void ChuyenDonSangToaAnMoi(APS_DON oDon, decimal DonXyLyID, APS_DON_XULY obj, decimal ToaAnCuEdit)
        {
            if (DonXyLyID == 0)
            {
                ...
                GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                GD.GAIDOAN_INSERT_UPDATE("7", oDonMoi.ID, 2, obj.CDTN_TOAANID.Value, 0, 0, 0, 0);
            ...
            else
            {
                ...
                APS_DON oDonNew = dt.APS_DON.FirstOrDefault(s => s.DONID_TOACU == obj.DONID && s.TOAANID == ToaAnCuEdit);
                if (oDonNew != null)
                {
                    if (oDonNew.TOAANID.Value != obj.CDTN_TOAANID)
                    {
                        ...
                        oDonNew.MAVUVIEC = oTA.MA + "." + ENUM_LOAIVUVIEC.AN_PHASAN + "." + oDonNew.TT.ToString();
                        dt.SaveChanges();
        ...
        private void DeleteDonChuyenToaAnKhac(APS_DON_XULY obj)
        {
            APS_DON oDon = dt.APS_DON.FirstOrDefault(s => s.DONID_TOACU == obj.DONID && s.TOAANID == obj.CDTN_TOAANID);
            if (oDon != null)
            {
                if (dt.APS_DON_THAMGIATOTUNG.Count(s => s.DONID == oDon.ID) > 0)
                {
                    List<APS_DON_THAMGIATOTUNG> lstTGTT = dt.APS_DON_THAMGIATOTUNG.Where(s => s.DONID == oDon.ID).ToList();
                    dt.APS_DON_THAMGIATOTUNG.RemoveRange(lstTGTT);
                }

                List<APS_DON_DUONGSU> lstDS = dt.APS_DON_DUONGSU.Where(s => s.DONID == oDon.ID).ToList();

                if (dt.APS_DON_TAILIEU.Count(s => s.DONID == oDon.ID) > 0)
                {
                    List<APS_DON_TAILIEU> lstTGTT = dt.APS_DON_TAILIEU.Where(s => s.DONID == oDon.ID).ToList();
                    dt.APS_DON_TAILIEU.RemoveRange(lstTGTT);
                }

                GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                GD.GIAIDOAN_DELETES("7", oDon.ID, 2);

                dt.APS_DON.Remove(oDon);

                dt.APS_DON_DUONGSU.RemoveRange(lstDS);
                dt.SaveChanges();
            }
        ...
        void UpdateTrangThaiDonKK()
        {
            ...
            string loai_an = ENUM_LOAIAN.AN_PHASAN + "";
            decimal vuviecid = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_PHASAN] + "");
            string yeucau = "";
            switch (bienphap_gd)
            {
                case 1:
        ...
        private void LoadGrid_AnPhi()
        {
            ...
            DataTable oDT = oBL.APS_ANPHI_GETBYDONID_V2(DONID, 1, pageindex, page_size);
            if (oDT.Rows.Count > 0)
            {
            ...
            switch (e.CommandName)
            {
                ...
                case "XoaAP":
                    try
                    {
                        ...
                        obj.CHECK_DELETE_ANPHI(CurrID, "7", ref _check_out);
                        if (_check_out == 1)
                        {
                            APS_ANPHI objAP = dt.APS_ANPHI.Where(x => x.ID == CurrID).FirstOrDefault();

                            APS_TONGDAT oTD = dt.APS_TONGDAT.Where(x => x.DONID == objAP.DONID).FirstOrDefault();
                            DON_MIENANPHI_BL dON = new DON_MIENANPHI_BL();
                            DataTable dataTable = dON.GET_DON_MIENANPHI_BY_ANPHI_ID(objAP.ID, 3);
                            APS_TONGDAT_BL dS_TONGDAT_BL = new APS_TONGDAT_BL();
                            ...
                            if (oTD != null && objAP.TINHTRANG != 1 && dataTable.Rows.Count == 0)
                            {
                                ...
                                && dS_TONGDAT_BL.APS_TONGDATDOITUONG_ANPHI_GETBYANPHIID_TONGDATID(objAP.DONID, objAP.ID).Rows.Count > 0)
                            ...
                            else
                            {
                                ...
                                dt.APS_ANPHI.Remove(objAP);
                                dt.SaveChanges();
                                lbThongbaoAP.Text = "Xóa thành công!";
                                //DELETE DVCQG_THANH_TOAN-------------
                                decimal _VALUE = 0;
                                obj.DVCQG_THANH_TOAN_DELETE_ANPHI(CurrID, "7", ref _VALUE);
                            }                            
                        }
                        else
                        {
                ...
                case "LSTBAnPhi":
                    try
                    {
                        CurrID = Convert.ToDecimal(e.CommandArgument.ToString());
                        Cls_Comon.CallFunctionJS(this, this.GetType(), "LoadModalDiv();");
                        string linkPop = "/QLAN/APS/XuLyDon/Popup/pMienAnPhi.aspx?ID=" + CurrID;
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "onclick", "var Mleft = (screen.width/2)-(950/2); var Mtop = (screen.height/2)-(600/2); javascript:window.open('" + linkPop + "', '_blank', 'height=600px,width=950px,top=\'+Mtop+\', left=\'+Mleft+\',resizable=yes,toolbar=no,scrollbars=0'); ", true);
                    }
                    catch (Exception ex) { lbThongbaoAP.Text = ex.Message; }
                    break;
        ...
        void LoadInfo_AnPhi(Decimal CurrID)
        {
            ...
            List<APS_ANPHI> lst = dt.APS_ANPHI.Where(x => x.ID == CurrID).ToList<APS_ANPHI>();
        ...
        protected void rptAP_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                DAL.GSTP.DVCQG_THANH_TOAN oAP = dt.DVCQG_THANH_TOAN.Where(x => x.ANPHI_ID == vAPID && x.MALOAIVUVIEC == "7").FirstOrDefault();
                APS_SOTHAM_THULY oTLST = dt.APS_SOTHAM_THULY.Where(x => x.DONID == DONID).FirstOrDefault();

                // Kiểm tra xem đơn đã được trả lại (LOAIGIAIQUYET == 3) hay chưa
                bool isDonTraLai = dt.APS_DON_XULY.Any(x => x.DONID == DONID && x.LOAIGIAIQUYET == 3);


                DON_MIENANPHI_BL dON_MIENANPHI_BL = new DON_MIENANPHI_BL();
                DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(vAPID, 7);

                APS_ANPHI aDS_ANPHI = dt.APS_ANPHI.Where(x => x.ID == vAPID).FirstOrDefault();
        ...
        protected void cmdCapNhatAP_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    try
                    {
                        objAP = dt.APS_ANPHI.Where(x => x.ID == APId).FirstOrDefault();
                        if (APId != 0)
                        {
                            objAP.NGAYSUA = DateTime.Now;
                            objAP.NGUOISUA = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                    ...
                    catch (Exception ex) { objAP = new APS_ANPHI(); }
                    objAP.DONID = DONID;
                    objAP.TINHTRANG = chkNopAnPhi.Checked == true ? 1 : 0;
                    objAP.GIATRITRANHCHAP = (String.IsNullOrEmpty(txtGiaTriTranhChap.Text.Trim())) ? 0 : Convert.ToDecimal(txtGiaTriTranhChap.Text.Trim(), cul);
                    objAP.MUCGIAMANPHI = (String.IsNullOrEmpty(txtMucGiamAnPhi.Text.Trim())) ? 0 : Convert.ToDecimal(txtMucGiamAnPhi.Text.Trim(), cul);
                    ...
                    {
                        decimal donID = Convert.ToDecimal(current_id);
                        APS_DON_XULY donXL = dt.APS_DON_XULY.Where(x => (x.DONID == DONID || x.DON_XULYID == DONID) && x.LOAIGIAIQUYET == 5).FirstOrDefault();
                        DVCQG_THANH_TOAN_BL obj_anphi = new DVCQG_THANH_TOAN_BL();
                        obj_anphi.DVCQG_THANH_TOAN_INSERT_ANPHI(2, donID, donXL.ID, objAP.ID, "7",Convert.ToString(objAP.DUONGSU_ID));
                    }
                    else if (APId != 0)
                    {
                        ///UPDATE vào bảng DVCQG_THANH_TOAN
                        DVCQG_THANH_TOAN_BL obj = new DVCQG_THANH_TOAN_BL();
                        obj.DVCQG_THANH_TOAN_UP_ANPHI(objAP.ID, "7");
                    }

                    //anhpn add log án phí 08/01/2025
                    string MATHONGBAO = "";
                    var DVCQG_THANH_TOAN = DataExtensions.GetAllWithClause<BL.GSTP.BANGSETGET.DVCQG_THANH_TOAN>("ANPHI_ID = " + objAP.ID + " AND MALOAIVUVIEC = '7'");
                    if (DVCQG_THANH_TOAN.Count() > 0)
                    {
                        MATHONGBAO = DVCQG_THANH_TOAN.FirstOrDefault().MA_THONGBAO;
                    }
                    ...
                    if (chkNopAnPhi.Checked == false)
                    {
                        ...
                        APS_DON_XULY donXLS = dt.APS_DON_XULY.Where(x => x.DONID == DONID && x.LOAIGIAIQUYET == 5).FirstOrDefault();
                        if (donXLS.LOAIGIAIQUYET == 5)
                        {
                            DataTable oDT = obj_anphiS.CHECK_THANH_TOAN_IN_ANPHI(2, donIDS, donXLS.ID, objAP.ID, "7", objAP.DUONGSU_ID + "");
                            if (oDT.Rows.Count == 0)
                            {
                                obj_anphiS.DVCQG_THANH_TOAN_INSERT_ANPHI(2, donIDS, donXLS.ID, objAP.ID, "7", objAP.DUONGSU_ID + "");
                            }
                        }
                    }
                    else
                    {
                        ...
                        APS_DON aDS_DON = dt.APS_DON.Where(x => x.ID == DONID).FirstOrDefault();
                        DM_BIEUMAU bmMienAnPhi = dt.DM_BIEUMAU.Where(x => x.MABM == "100-DS").FirstOrDefault();
                        APS_FILE objFile = dt.APS_FILE.Where(x => x.DONID == DONID && x.BIEUMAUID == bmMienAnPhi.ID).FirstOrDefault();
                        if (objFile != null)
                        {
                ...
                catch (System.Data.Entity.Validation.DbEntityValidationException dbEx)
{
                    foreach (var validationErrors in dbEx.EntityValidationErrors)
                    {
                        ...
                        {
                            lbThongbaoAP.Text = "property: " + validationError.PropertyName + " Error: " + validationError.ErrorMessage;
                        }
                    }
                }
                #endregion
        ...
        private bool CheckValidAP()
        {
            ...
            else
            {
                ...
                if (hddCurrAPID.Value != "" && hddCurrAPID.Value != "0")
                {
                    ...
                    APS_ANPHI objAP = dt.APS_ANPHI.Where(x => x.ID == APId).FirstOrDefault();
                    APS_TONGDAT oTD = dt.APS_TONGDAT.Where(x => x.DONID == objAP.DONID).FirstOrDefault();
                    DON_MIENANPHI_BL dON = new DON_MIENANPHI_BL();
                    DataTable dataTable = dON.GET_DON_MIENANPHI_BY_ANPHI_ID(objAP.ID, 7);
                    ...
                    {
                        APS_TONGDAT_BL dS_TONGDAT_BL = new APS_TONGDAT_BL();
                        DataTable anphiTable = dS_TONGDAT_BL.APS_TONGDATDOITUONG_ANPHI_GETBYANPHIID_TONGDATID(objAP.DONID, objAP.ID);
                ...
                APS_ANPHI ap = dt.APS_ANPHI.Where(x => x.DUONGSU_ID == dsID && x.ID != APId && APId != 0).FirstOrDefault();
                if (ap != null)
                {
        ...
        void SetNewSoTBAP()
        {
            ...
            String STTNew = oSTBL.GET_STB_ANPHI_NEW(DonViID, "APS", ngayTB).ToString();
