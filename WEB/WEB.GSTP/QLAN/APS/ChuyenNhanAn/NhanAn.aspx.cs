...
namespace WEB.GSTP.QLAN.APS.ChuyenNhanAn
{
    public partial class NhanAn : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    #region Quét các vụ việc được chuyển từ Tòa án cấp cao và lưu vào APS_DON và APS_CHUYEN_NHAN_AN
                    ...
                    //string sql = "select * from HOSO where TOA_AN_XET_XU='" + ToaAnID.ToString() + "' and LOAI_AN='" + ENUM_LOAIAN.AN_PHASAN + "' and TRANG_THAI=1";
                    ...
                    //        #region Update Vụ việc APS_DON
                    //        APS_DON vDON = dt.APS_DON.Where(x => x.MAVUVIEC == MaVuViec && ((x.MAGIAIDOAN == ENUM_GIAIDOANVUAN.SOTHAM && x.TOAANID == ToaAnID) || (x.MAGIAIDOAN == ENUM_GIAIDOANVUAN.PHUCTHAM && x.TOAPHUCTHAMID == ToaAnID))).FirstOrDefault();
                    //        if (vDON == null)
                    //        {
                    //            IsNew = true;
                    //            vDON = new APS_DON();
                    ...
                    //        VuAnID = vDON.ID;
                    //        #endregion
                    //        #region Update Table APS_CHUYEN_NHAN_AN
                    //        IsNew = false;
                    //        APS_CHUYEN_NHAN_AN chuyen_nhan_an = dt.APS_CHUYEN_NHAN_AN.Where(x => x.VUANID == VuAnID && x.TOACHUYENID == Toa_Chuyen_ID && x.TOANHANID == Toa_Nhan_ID).FirstOrDefault();
                    //        if (chuyen_nhan_an == null)
                    //        {
                    //            IsNew = true;
                    //            chuyen_nhan_an = new APS_CHUYEN_NHAN_AN();
                    //        }
                    ...
                    //        if (IsNew)
                    //        {
                    //            dt.APS_CHUYEN_NHAN_AN.Add(chuyen_nhan_an);
                    //        }
                    //        dt.SaveChanges();
                    //        #endregion
                    //    }
        ...
        private void LoadGrid()
        {
            ...
            DataTable oDT = oBL.PS_NHANAN(txt_NG_KC.Text.Trim(), txtSoQD.Text.Trim(), txt_NgayQD.Text.Trim(), vDonViID, txtMaVuViec.Text, txtTenVuViec.Text, txtTenToa.Text, Convert.ToDecimal(ddlTHGN.SelectedValue), dFrom, dTo, Convert.ToDecimal(rdbTrangthai.SelectedValue), Convert.ToDecimal(rdbLoai.SelectedValue));
        ...
        protected void cmdNhanan_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridItem Item in dgList.Items)
            {
                ...
                if (chkChon.Checked)
                {
                    ...
                    input.TruongHopGiaoNhan = Item.Cells[9].Text.Replace("Do có kháng cáo và kháng nghị phúc thẩm", "Do có đề nghị xem xét lại và kháng nghị");
        ...
        private void UpdateDuongSuIdTGTT(APS_DON_THAMGIATOTUNG oTTOLD, APS_DON_THAMGIATOTUNG oTTNEW, List<DuongSuTemp> lstDuongSuTemp)
        {
        ...
        protected void cmdLuu_Click(object sender, EventArgs e)
        {
            ...
            foreach (RepeaterItem item in rptCapNhat.Items)
            {
                ...
                APS_CHUYEN_NHAN_AN oT = dt.APS_CHUYEN_NHAN_AN.Where(x => x.ID == ID).FirstOrDefault();
                if (oT != null)
                {
                    ...
                    if (oIT.MA == ENUM_TRUONGHOP_GIAONHAN.KHONGTHUOC_THAMQUYEN_XETXU)
                    {
                        ...
                        else
                        {
                            if (rdbLoai.SelectedValue == "0")
                            {
                                ...
                                oDonMoi.MAVUVIEC = oTA.MA + "." + ENUM_LOAIVUVIEC.AN_PHASAN + "." + oDonMoi.TT.ToString();
                                ...
                                dt.APS_DON.Add(oDonMoi);
                                dt.SaveChanges();

                                GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                                GD.GAIDOAN_INSERT_UPDATE("7", oDonMoi.ID, 2, oDonMoi.TOAANID.Value, 0, 0, 0, 0);
                                List<DuongSuTemp> lstDuongSuTemp = new List<DuongSuTemp>();

                                //lấy danh sách đương sự
                                IQueryable<APS_DON_DUONGSU> lstoDS = dt.APS_DON_DUONGSU.Where(s => s.DONID == oDon.ID);
                                foreach (var it in lstoDS)
                                {
                                    APS_DON_DUONGSU oDS = new APS_DON_DUONGSU();
                                ...
                                IQueryable<APS_DON_THAMGIATOTUNG> lstoTT = dt.APS_DON_THAMGIATOTUNG.Where(s => s.DONID == oDon.ID);
                                foreach (var it in lstoTT)
                                {
                                    //lưu thông tin người tham gia tố tụng
                                    APS_DON_THAMGIATOTUNG oTT = new APS_DON_THAMGIATOTUNG();
                                    ...
                                    dt.APS_DON_THAMGIATOTUNG.Add(oTT);
                                    dt.SaveChanges();
                                }

                                //
                                IQueryable<APS_DON_TAILIEU> lstTailieu = dt.APS_DON_TAILIEU.Where(s => s.DONID == oDon.ID);
                            ...
                            else
                            {
                                ...
                                */
                                string UserName = Session[ENUM_SESSION.SESSION_USERNAME] + "", LoaiVuViec = ENUM_LOAIVUVIEC.AN_PHASAN, MaToaAn = Session[ENUM_SESSION.SESSION_MADONVI] + "";
                                ...
                                GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                                GD.GAIDOAN_INSERT_UPDATE("7", DonID_New, 2, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), 0, 0, 0, 0);
                            }
                        }
                    }
                    else if (oIT.MA == ENUM_TRUONGHOP_GIAONHAN.XETXULAI_CAPSOTHAM)
                    {
                        ...
                        {
                            APS_CHUYEN_NHAN_AN_BL _caBL = new APS_CHUYEN_NHAN_AN_BL();
                            ...
                            #region toancau - anhnt update lại kết quả đề nghị/kháng nghị
                            List<APS_SOTHAM_KHANGCAO> khangCaos = dt.APS_SOTHAM_KHANGCAO.Where(x => x.DONID == donIdOld && (x.TINHTRANG_GIAIQUYET == null || x.TINHTRANG_GIAIQUYET == 0) && x.LOAIKHANGCAO == 2).ToList();
                            if (khangCaos != null && khangCaos.Count > 0)
                            {
                                foreach (APS_SOTHAM_KHANGCAO kc in khangCaos)
                                {
                                    kc.TINHTRANG_GIAIQUYET = 1;
                                    dt.SaveChanges();
                                }
                            }
                            List<APS_SOTHAM_KHANGNGHI> khangNghis = dt.APS_SOTHAM_KHANGNGHI.Where(x => x.DONID == donIdOld && (x.TINHTRANG_GIAIQUYET == null || x.TINHTRANG_GIAIQUYET == 0) && x.LOAIKN == 2).ToList();
                            if (khangNghis != null && khangNghis.Count > 0)
                            {
                                foreach (APS_SOTHAM_KHANGNGHI kn in khangNghis)
                                {
                                    kn.TINHTRANG_GIAIQUYET = 1;
                                    dt.SaveChanges();
                                }
                            }
                            #endregion toancau - tamnc update lại kết quả đề nghị/kháng nghị
                        }
                        else
                        {
                            ...
                            APS_DON oDON_new = new APS_DON();
                            oDON_new.TOAANID = Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]);

                            oDON_new.MAVUVIEC = Session[ENUM_SESSION.SESSION_MADONVI] + "." + ENUM_LOAIVUVIEC.AN_PHASAN + "." + oDon.TT.ToString();
                            ...
                            dt.APS_DON.Add(oDON_new);
                            dt.SaveChanges();

                            //Tam ung an phi
                            List<APS_ANPHI> lstAP = dt.APS_ANPHI.Where(x => x.DONID == DonID && x.MAGIAIDOAN == 2).ToList();
                            foreach (APS_ANPHI vAP_old in lstAP)
                            {
                                APS_ANPHI vAP_new = new APS_ANPHI();
                                ...
                                dt.APS_ANPHI.Add(vAP_new);
                                dt.SaveChanges();

                                var CNA = new CHUYEN_NHAN_AN();
                                CNA.INSERT_ANPHI_DUONGSUID(7, DonID, oDON_new.ID);

                            }

                            GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                            GD.GAIDOAN_INSERT_UPDATE("7", oDON_new.ID, 2, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), 0, 0, 0, 0);

                            //Câp dương su vụ án
                            List<APS_DON_DUONGSU> lst = dt.APS_DON_DUONGSU.Where(x => x.DONID == DonID).ToList<APS_DON_DUONGSU>();
                            foreach (APS_DON_DUONGSU vDuongsu_old in lst)
                            {
                                APS_DON_DUONGSU vDuongsu_new = new APS_DON_DUONGSU();
                                ...
                                dt.APS_DON_DUONGSU.Add(vDuongsu_new);
                                dt.SaveChanges();
                                //Ban giao tai lieu
                                List<APS_DON_TAILIEU> lstTaiLieu = dt.APS_DON_TAILIEU.Where(x => x.DONID == DonID && x.NGUOIBANGIAO == vDuongsu_old.ID).ToList<APS_DON_TAILIEU>();
                                foreach (APS_DON_TAILIEU vdonFile_old in lstTaiLieu)
                                {
                                    APS_DON_TAILIEU donFile_new = new APS_DON_TAILIEU();
                                    ...
                                    dt.APS_DON_TAILIEU.Add(donFile_new);
                                    dt.SaveChanges();
                                }

                            }
                            // Phan cong Tham phan
                            List<APS_DON_THAMPHAN> lstThamPhan = dt.APS_DON_THAMPHAN.Where(x => x.DONID == DonID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETDON).ToList<APS_DON_THAMPHAN>();
                            foreach (APS_DON_THAMPHAN vThamphan_old in lstThamPhan)
                            {
                                APS_DON_THAMPHAN vThamphan_new = new APS_DON_THAMPHAN();
                                ...
                                dt.APS_DON_THAMPHAN.Add(vThamphan_new);
                                dt.SaveChanges();
                            }
                            //Xu ly don
                            List<APS_DON_XULY> lstXulyDon = dt.APS_DON_XULY.Where(x => x.DONID == DonID).ToList<APS_DON_XULY>();
                            foreach (APS_DON_XULY vXulyDon_old in lstXulyDon)
                            {
                                APS_DON_XULY vXulyDon_new = new APS_DON_XULY();
                                ...
                                dt.APS_DON_XULY.Add(vXulyDon_new);
                                dt.SaveChanges();
                            }
                            //MAP_VUANID_NEW
                    ...
                    else
                    {
                        ...
                        //var soThamKhangCaos = dt.APS_SOTHAM_KHANGCAO.Where(x => x.DONID == oDon.ID && x.LOAIKHANGCAO == 2 && (x.TINHTRANG_GIAIQUYET == null || x.TINHTRANG_GIAIQUYET == 0));
                        //var soThamKhangNghis = dt.APS_SOTHAM_KHANGNGHI.Where(x => x.DONID == oDon.ID && x.LOAIKN == 2 && (x.TINHTRANG_GIAIQUYET == null || x.TINHTRANG_GIAIQUYET == 0));
                        ...
                        if (_chuyenNhanAnBl.CheckCoKcKnTamDinhChi(DonID))
                        {
                            ...
                            dt.SaveChanges();
                            GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                            GD.GAIDOAN_INSERT_UPDATE("7", oDON_new.ID, ENUM_GIAIDOANVUAN.PHUCTHAM_QDK, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), 0, 0, 0);
                            ...
                            //Tam ung an phi
                            List<APS_ANPHI> lstAP = dt.APS_ANPHI.Where(x => x.DONID == DonID).ToList();
                            foreach (APS_ANPHI vAP_old in lstAP)
                            {
                                APS_ANPHI vAP_new = new APS_ANPHI();
                                vAP_new.DONID = oDON_new.ID;
                        ...
                        else
                        {
                            ...
                            //anhvh add 26/06/2020
                            GIAI_DOAN_BL GD = new GIAI_DOAN_BL();
                            GD.GAIDOAN_INSERT_UPDATE("7", DonID, 3, 0, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), 0, 0, 0);
                        }
                    }
        ...
        private void UpdateNguyenDonBiDon(List<NGUOI_THAM_GIA_AK> List_NguyeDon_BiDon, decimal DonID)
        {
            if (List_NguyeDon_BiDon.Count > 0)
            {
                ...
                {
                    IsNew = false;
                    APS_DON_DUONGSU DuongSu = dt.APS_DON_DUONGSU.Where(x => x.DONID == DonID && x.ID_DUONGSU_TACC == item.Guid).FirstOrDefault();
                    if (DuongSu == null)
                    {
                        IsNew = true;
                        DuongSu = new APS_DON_DUONGSU();
                    }
                    ...
                    if (IsNew)
                    {
                        dt.APS_DON_DUONGSU.Add(DuongSu);
                    }
                    dt.SaveChanges();
                }
            }
        ...
        private void UpdateNguoiTGTT(List<NGUOI_THAM_GIA_CHUNG> ListNguoiTGTT, decimal DonID)
        {
            if (ListNguoiTGTT.Count > 0)
            {
                ...
                {
                    APS_DON_THAMGIATOTUNG NguoiTGTT = dt.APS_DON_THAMGIATOTUNG.Where(x => x.DONID == DonID && x.ID_DUONGSU_TACC == item.Guid).FirstOrDefault();
                    if (NguoiTGTT != null)
                    {
                        IsNew = true;
                        NguoiTGTT = new APS_DON_THAMGIATOTUNG();
                    ...
                    if (IsNew)
                    {
                        dt.APS_DON_THAMGIATOTUNG.Add(NguoiTGTT);
                    }
                    dt.SaveChanges();
                }
            }
        ...
        private void UpdateThuLy(THU_LY ThuLy, decimal ToaAnID, decimal DonID, string CapXetXu)
        {
            if (CapXetXu == "SOTHAM")
            {
                ...
                bool IsNew = false;
                APS_SOTHAM_THULY STThuLy = dt.APS_SOTHAM_THULY.Where(x => x.DONID == DonID && x.SOTHULY == ThuLy.SOTHULY.ToString()).FirstOrDefault();
                if (STThuLy != null)
                {
                    IsNew = true;
                    STThuLy = new APS_SOTHAM_THULY();
                ...
                if (IsNew)
                {
                    dt.APS_SOTHAM_THULY.Add(STThuLy);
                }
                dt.SaveChanges();
            }
            else
            {
                ...
                bool IsNew = false;
                APS_PHUCTHAM_THULY PTThuLy = dt.APS_PHUCTHAM_THULY.Where(x => x.DONID == DonID && x.SOTHULY == ThuLy.SOTHULY.ToString()).FirstOrDefault();
                if (PTThuLy != null)
                {
                    IsNew = true;
                    PTThuLy = new APS_PHUCTHAM_THULY();
                ...
                if (IsNew)
                {
                    dt.APS_PHUCTHAM_THULY.Add(PTThuLy);
                }
                dt.SaveChanges();
            }
        }
        private void UpdateNguoiTHTT(List<NGUOI_TIEN_HANH_TO_TUNG> ListNguoiTHTT, decimal DonID, string CapXetXu)
        {
            if (ListNguoiTHTT.Count > 0)
            {
                ...
                foreach (NGUOI_TIEN_HANH_TO_TUNG item in ListNguoiTHTT)
                {
                    ...
                    if (CapXetXu == "SOTHAM")
                    {
                        APS_SOTHAM_HDXX Hdxx = dt.APS_SOTHAM_HDXX.Where(x => x.DONID == DonID && x.CANBOID == CanBoID).FirstOrDefault();
                        if (Hdxx == null)
                        {
                            IsNew = true;
                            Hdxx = new APS_SOTHAM_HDXX();
                        ...
                        if (IsNew)
                        {
                            dt.APS_SOTHAM_HDXX.Add(Hdxx);
                        }
                    }
                    // Phúc thẩm
                    else
                    {
                        APS_PHUCTHAM_HDXX Hdxx = dt.APS_PHUCTHAM_HDXX.Where(x => x.DONID == DonID && x.CANBOID == CanBoID).FirstOrDefault();
                        if (Hdxx == null)
                        {
                            IsNew = true;
                            Hdxx = new APS_PHUCTHAM_HDXX();
                            Hdxx.NGAYTAO = DateTime.Now;
                        }
                        else
                        {
        ...
        private void UpdateQDVuViec(List<QUYET_DINH> ListQD, decimal ToaAnID, decimal DonID, string CapXetXu)
        {
            if (ListQD.Count > 0)
            {
                ...
                foreach (QUYET_DINH item in ListQD)
                {
                    ...
                    if (CapXetXu == "SOTHAM")
                    {
                        APS_SOTHAM_QUYETDINH QdVuAn = dt.APS_SOTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.TOAANID == ToaAnID).FirstOrDefault();
                        if (QdVuAn == null)
                        {
                            IsNew = false;
                            QdVuAn = new APS_SOTHAM_QUYETDINH()
                            {
                        ...
                        if (IsNew)
                        {
                            dt.APS_SOTHAM_QUYETDINH.Add(QdVuAn);
                        }
                        dt.SaveChanges();
                    }
                    // Phúc thẩm
                    ...
                    {
                        APS_PHUCTHAM_QUYETDINH QdVuAn = dt.APS_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.TOAANID == ToaAnID).FirstOrDefault();
                        if (QdVuAn == null)
                        {
                            IsNew = false;
                            QdVuAn = new APS_PHUCTHAM_QUYETDINH()
                            {
                                NGAYTAO = DateTime.Now
                            };
                        }
                        ...
                        if (IsNew)
                        {
                            dt.APS_PHUCTHAM_QUYETDINH.Add(QdVuAn);
                        }
                        dt.SaveChanges();
                    }
                }
        ...
        private void UpdateBanAn(BAN_AN bAn, LICH_XET_XU LichXetXu, decimal ToaAnID, decimal DonID, string CapXetXu)
        {
            ...
            if (CapXetXu == "SOTHAM")
            {
                APS_SOTHAM_BANAN STBanAn = dt.APS_SOTHAM_BANAN.Where(x => x.DONID == DonID && x.TOAANID == ToaAnID).FirstOrDefault();
                if (STBanAn == null)
                {
                    IsNew = true;
                    STBanAn = new APS_SOTHAM_BANAN();
                ...
                if (IsNew)
                {
                    dt.APS_SOTHAM_BANAN.Add(STBanAn);
                }
                dt.SaveChanges();
            }
            // Phúc thẩm
            ...
            {
                APS_PHUCTHAM_BANAN PTBanAn = dt.APS_PHUCTHAM_BANAN.Where(x => x.DONID == DonID && x.TOAANID == ToaAnID).FirstOrDefault();
                if (PTBanAn == null)
                {
                    IsNew = true;
                    PTBanAn = new APS_PHUCTHAM_BANAN();
                ...
                DM_KETQUA_PHUCTHAM KQPT = dt.DM_KETQUA_PHUCTHAM.Where(x => x.ISAPS == 1 && x.MA == bAn.KETQUA).FirstOrDefault();
                if (KQPT != null)
                {
                ...
                if (IsNew)
                {
                    dt.APS_PHUCTHAM_BANAN.Add(PTBanAn);
                }
                dt.SaveChanges();
            }
        }
        private void UpdateKhangCao(List<KHANG_CAO> ListKhangCao, decimal ToaAnID, decimal DonID)
        {
            if (ListKhangCao.Count > 0)
            {
                ...
                {
                    #region Lưu Nội dung đề nghị
                    byte[] Guid_NguoiKC = item.GUID;
                    decimal NguoiKCID = 0;
                    APS_DON_DUONGSU DuongSu = dt.APS_DON_DUONGSU.Where(x => x.DONID == DonID && x.ID_DUONGSU_TACC == Guid_NguoiKC).FirstOrDefault();
                    if (DuongSu != null)
                    {
                        NguoiKCID = DuongSu.ID;
                    }
                    APS_SOTHAM_KHANGCAO kc = dt.APS_SOTHAM_KHANGCAO.Where(x => x.DONID == DonID && x.DUONGSUID == NguoiKCID && x.TOAANRAQDID == ToaAnID).FirstOrDefault();
                    if (kc != null)
                    {
                        IsNew = true;
                        kc = new APS_SOTHAM_KHANGCAO();
        ...
        private decimal Action_ChuyenAn_KhongThuoc_ThamQuyen(decimal DonID, string LoaiVuViec, decimal ToaNhanID, string MaToaNhan, string NguoiTao)
        {
            ...
            DataTable tbl = Cls_Comon.GetTableByProcedurePaging("APS_CVA_KHONG_THAM_QUYEN", parameters);
            decimal Result = 0;
            if (tbl.Rows.Count > 0)
        ...
        {
            APS_DON_BL Bl = new APS_DON_BL();
            //for (int i = 0; i < lstDonID.Count; i++)
            //{
            //    var ChuyenNhanID = lstDonID[i];
            //    decimal DonID_New = 0;
            //    APS_CHUYEN_NHAN_AN ObjChuyenAn = dt.APS_CHUYEN_NHAN_AN.Where(x => x.ID == ChuyenNhanID).FirstOrDefault();
            //    if (ObjChuyenAn != null)
            //    {
            ...
            for (int i = 0; i < lstDonID.Count; i++)
            {
                var ChuyenNhanID = lstDonID[i];
                decimal DonID_New = 0;
                APS_CHUYEN_NHAN_AN ObjNhanAn = dt.APS_CHUYEN_NHAN_AN.Where(x => x.ID == ChuyenNhanID).FirstOrDefault();
                if (ObjNhanAn != null)
                {
                    int LoaiVuViec = Convert.ToInt32(ENUM_LOAIVUVIEC.AN_PHASAN);
                    if (ObjNhanAn.MAP_VUANID_NEW.GetValueOrDefault(0) > 0)
                    {
                        ...
                        if (rdbLoai.SelectedValue == "0")
                        {
                            APS_DON_THAMPHAN thamPhan = dt.APS_DON_THAMPHAN.Where(x => x.DONID == DonID_New && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETDON).FirstOrDefault();
                            if (thamPhan != null)
                            {
                                IsThuLy = true;
                        ...
                        if (!IsThuLy)
                        {
                            ...
                            APS_DON oDonNew = dt.APS_DON.Where(x => x.ID == DonID_New).FirstOrDefault();
                            var json = new JavaScriptSerializer().Serialize(oDonNew);
                            ADS_DON_BL oBL = new ADS_DON_BL();
                            if (oBL.HISTORY_ALLDATA_BY_VUANID(DonID_New, Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_PHASAN), Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Huỷ nhận án hành chính", "Xóa", json) == false)
                            {
                                break;
                            }
                            //Ket thuc 
                            ...
                            APS_DON oVuan = dt.APS_DON.Where(x => x.ID == ObjNhanAn.VUANID).FirstOrDefault();
                    ...
                    else
                    {
                        ...
                        if (oVuan.MAGIAIDOAN == ENUM_GIAIDOANVUAN.PHUCTHAM_QDK)
                        {
                            ...
                            APS_CHUYEN_NHAN_AN chuyenAnSTlenPTTDC = dt.APS_CHUYEN_NHAN_AN.Where(x => x.MAP_VUANID_NEW == oVuan.ID).FirstOrDefault();
                            List<APS_SOTHAM_THULY> oThuLyST = dt.APS_SOTHAM_THULY.Where(x => x.DONID == oldVuAnId && x.NGAYTAO >= chuyenAnSTlenPTTDC.NGAYTAO).ToList();
                            List<APS_SOTHAM_HDXX> oSTNguoiTienHanhToTung = dt.APS_SOTHAM_HDXX.Where(x => x.DONID == oldVuAnId && x.NGAYTAO >= chuyenAnSTlenPTTDC.NGAYTAO).ToList();
                            List<APS_DON_THAMPHAN> oThamPhanST = dt.APS_DON_THAMPHAN.Where(x => x.DONID == oldVuAnId && x.NGAYTAO >= chuyenAnSTlenPTTDC.NGAYTAO).ToList();
                            List<APS_SOTHAM_BANAN> oBanAnST = dt.APS_SOTHAM_BANAN.Where(x => x.DONID == oldVuAnId && x.NGAYTAO >= chuyenAnSTlenPTTDC.NGAYTAO).ToList();
                            List<APS_SOTHAM_QUYETDINH> oQDST = dt.APS_SOTHAM_QUYETDINH.Where(x => x.DONID == oldVuAnId && x.NGAYTAO >= chuyenAnSTlenPTTDC.NGAYTAO).ToList();
                            List<APS_SOTHAM_KHANGCAO> oKCST = dt.APS_SOTHAM_KHANGCAO.Where(x => x.DONID == oldVuAnId && x.NGAYTAO >= chuyenAnSTlenPTTDC.NGAYTAO).ToList();
                            List<APS_SOTHAM_KHANGNGHI> oKNST = dt.APS_SOTHAM_KHANGNGHI.Where(x => x.DONID == oldVuAnId && x.NGAYTAO >= chuyenAnSTlenPTTDC.NGAYTAO).ToList();
                            ...
                            {
                                APS_DON oVuAnOld = dt.APS_DON.Where(x => x.ID == oldVuAnId).FirstOrDefault();
                                APS_CHUYEN_NHAN_AN ObjNhanAnLanCuoi = dt.APS_CHUYEN_NHAN_AN.Where(x => x.VUANID == oldVuAnId && x.TOACHUYENID == oVuAnOld.TOAANID).OrderByDescending(x => x.ID).FirstOrDefault();
                                APS_CHUYEN_NHAN_AN ObjNhanAnLanGanCuoi = dt.APS_CHUYEN_NHAN_AN.Where(x => x.VUANID == oldVuAnId && x.TOACHUYENID == oVuAnOld.TOAANID && x.ID < ObjNhanAnLanCuoi.ID).OrderByDescending(x => x.ID).FirstOrDefault();
                                if (ObjNhanAnLanGanCuoi == null)
                                {
                                    #region toancau - anhnt update lại kết quả đề nghị/kháng nghị
                                    List<APS_SOTHAM_KHANGCAO> khangCaos = dt.APS_SOTHAM_KHANGCAO.Where(x => x.DONID == oldVuAnId && x.TINHTRANG_GIAIQUYET == 1 && x.LOAIKHANGCAO == 2).ToList();
                                    if (khangCaos != null && khangCaos.Count > 0)
                                    {
                                        foreach (APS_SOTHAM_KHANGCAO kc in khangCaos)
                                        {
                                    ...
                                    List<APS_SOTHAM_KHANGNGHI> khangNghis = dt.APS_SOTHAM_KHANGNGHI.Where(x => x.DONID == oldVuAnId && x.TINHTRANG_GIAIQUYET == 1 && x.LOAIKN == 2).ToList();
                                    if (khangNghis != null && khangNghis.Count > 0)
                                    {
                                        foreach (APS_SOTHAM_KHANGNGHI kn in khangNghis)
                                        {
                                    ...
                                    #endregion toancau - anhnt update lại kết quả đề nghị/kháng nghị
                                }
                                else
                                {
                                    #region toancau - anhnt update lại kết quả đề nghị/kháng nghị
                                    List<APS_SOTHAM_KHANGCAO> khangCaos = dt.APS_SOTHAM_KHANGCAO.Where(x => x.DONID == oldVuAnId && x.TINHTRANG_GIAIQUYET == 1 && x.LOAIKHANGCAO == 2 && x.NGAYTAO >= ObjNhanAnLanGanCuoi.NGAYTAO && x.NGAYTAO <= ObjNhanAnLanCuoi.NGAYTAO).ToList();
                                    if (khangCaos != null && khangCaos.Count > 0)
                                    {
                                        foreach (APS_SOTHAM_KHANGCAO kc in khangCaos)
                                        {
                                    ...
                                    List<APS_SOTHAM_KHANGNGHI> khangNghis = dt.APS_SOTHAM_KHANGNGHI.Where(x => x.DONID == oldVuAnId && x.TINHTRANG_GIAIQUYET == 1 && x.LOAIKN == 2 && x.NGAYTAO >= ObjNhanAnLanGanCuoi.NGAYTAO && x.NGAYTAO <= ObjNhanAnLanCuoi.NGAYTAO).ToList();
                                    if (khangNghis != null && khangNghis.Count > 0)
                                    {
                                        foreach (APS_SOTHAM_KHANGNGHI kn in khangNghis)
                                        {
