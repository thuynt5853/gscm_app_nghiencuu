    ...
    public partial class KhangCaoKhangNghi : System.Web.UI.Page
    {
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                //APS_SOTHAM_KHANGCAO oKC = dt.APS_SOTHAM_KHANGCAO.Where(x => x.DONID == DONID && x.ID == idKC).FirstOrDefault();
                //if (oKC != null)
                //{
                ...
                //    //Kiem tra neu Khang cao da chuyen an ròi thi khong duoc sua hoac xoa
                //    APS_CHUYEN_NHAN_AN cn = dt.APS_CHUYEN_NHAN_AN.Where(x => x.VUANID == DONID).FirstOrDefault();
                //    if (cn != null)
                //    {
                //        if (oKC.NGAYKHANGCAO < cn.NGAYGIAO)
                //        {
                //            lblSua.Visible = lbtXoa.Visible = false;
                //        }
                //        else
                //        {   //Tòa cấp trên chưa nhận mới được xóa 
                //            if (cn.TRANGTHAI != 1)
                ...
                Decimal IsKhangCao = Convert.ToDecimal(dv["IsKhangCao"]);
                Decimal idKCKN = Convert.ToDecimal(dv["ID"]);
                if (IsKhangCao == 1)
                {
                    APS_SOTHAM_KHANGCAO oKC = dt.APS_SOTHAM_KHANGCAO.Where(x => x.ID == idKCKN).FirstOrDefault();
                    if (oKC != null)
                    {
                        ...
                        string Result = new APS_CHUYEN_NHAN_AN_BL().Check_ChuyenNhanAn(DONID, StrMsg, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]));
                        if (Result == "")
                        {
                            lblSua.Visible = lbtXoa.Visible = true;
                            lblchitiet.Visible = false;
                ...
                {
                    APS_SOTHAM_KHANGNGHI oKN = dt.APS_SOTHAM_KHANGNGHI.Where(x => x.ID == idKCKN).FirstOrDefault();
                    if (oKN != null)
                    {
        ...
        private void LoadComboboxKhangCao()
        {
            ...
            //Load đương sự
            //Load nguyen don da dong tien va bi don va nguoi có quyen va nghia vu lien quan
            ddlNguoikhangcao.DataSource = oDSBL.APS_DUONGSU_KHANGCAO(DonID);
        ...
        {
            //Load số và ngày bản án để đề nghị PAGE_KHANGCAO_ST
            PAGE_KHANGCAO_ST oBL = new PAGE_KHANGCAO_ST();
            DataTable oDT = oBL.DANHSACH_KHANGCAO_BANAN(ENUM_LOAIVUVIEC_NUMBER.AN_PHASAN, DonID);
            if (oDT != null)
            {
                drop.DataSource = oDT;
        ...
        void LoadDrop_ST_QuyetDinh(DropDownList drop, Decimal DonID, Decimal loaiKC)
        {
            ...
            {
                PAGE_KHANGCAO_ST oBL = new PAGE_KHANGCAO_ST();
                DataTable oDT = oBL.DANHSACH_KHANGCAO_QUYETDINH(ENUM_LOAIVUVIEC_NUMBER.AN_PHASAN, DonID);
                if (oDT != null)
                {
                    drop.DataSource = oDT;
                    drop.DataTextField = "TEN";
            ...
            {
                PAGE_KHANGCAO_ST oBL = new PAGE_KHANGCAO_ST();
                DataTable oDT = oBL.DANHSACH_KHANGCAO_QUYETDINH_KHAC(ENUM_LOAIVUVIEC_NUMBER.AN_PHASAN, DonID);
                if (oDT != null)
                {
                    drop.DataSource = oDT;
                    drop.DataTextField = "TEN";
            ...
            try
            {
                ...
                switch (e.CommandName)
                {
                    case "Download":
                        ...
                        if (IsKhangCao == 1)
                        {
                            APS_SOTHAM_KHANGCAO oKN = dt.APS_SOTHAM_KHANGCAO.Where(x => x.ID == ID).FirstOrDefault();
                            TENFILE = oKN.TENFILE;
                    ...
                    case "Xoa":
                        ...
                        APS_SOTHAM_KHANGCAO oKC = dt.APS_SOTHAM_KHANGCAO.Where(x => x.DONID == DonID && x.ID == ID).FirstOrDefault();
                        if (Result != "")
                        {
                            if (oKC.ISQUAHAN == 1)
                            {
            ...
            try
            {
                ...
                if (IsKhangCao == KHANGCAO)// Kháng cáo
                {
                    ...
                    if (oND != null)
                    {
                        ...
                        APS_ANPHI oAP = dt.APS_ANPHI.Where(x => x.DONID == oND.DONID && x.DUONGSU_ID == oND.DUONGSUID && x.MAGIAIDOAN == 3).FirstOrDefault();
                        if (oAP != null)
                        {
        ...
        public void xoa(decimal ID, decimal IsKhangCao)
        {
            if (IsKhangCao == KHANGCAO)// Kháng cáo
            {
                ...
                APS_ANPHI objAP = dt.APS_ANPHI.Where(x => x.DONID == DonID && x.DUONGSU_ID == oND.DUONGSUID && x.MAGIAIDOAN == 3).FirstOrDefault();
                if (objAP != null)
                {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                decimal ID = 0, IsKhangCao = 0, DonID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_PHASAN] + "");
                APS_DON oDon = dt.APS_DON.Where(x => x.ID == DonID).FirstOrDefault();
                if (rdbPanelKC.SelectedValue == KHANGCAO.ToString() && rdbPanelKN.SelectedValue == KHANGCAO.ToString())// Kháng cáo
                {
                    ...
                    {
                        APS_ANPHI lstAPnew = new APS_ANPHI();
                        ...
                        lstAPnew.DUONGSU_ID = oND.DUONGSUID;

                        dt.APS_ANPHI.Add(lstAPnew);
                        dt.SaveChanges();
                    }
                    #endregion
        ...
        protected void rdbPanelKC_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else // Kháng nghị
            {
                ...
                DON_KHAC oD = dt.DON_KHAC.Where(x => x.DONID == DonID && x.LOAIANID == 7 && x.LOAIKCKN == 2).FirstOrDefault();
        ...
        protected void cmdLoadDonKhac_Click(object sender, EventArgs e)
        {
            ...
            DON_KHAC oD = dt.DON_KHAC.Where(x => x.ID == ID && x.LOAIANID == 7 && x.LOAIKCKN == 1).FirstOrDefault();
            if (oD != null)
            {
                pnKhangCao.Visible = true;
                pnKhangNghi.Visible = false;
        ...
        void CheckNgayKCQuaHan()
        {
            ...
            if (banan_qd_id > 0)
            {
                if (loai_ba_qd == 0)
                {
                    ...
                    APS_SOTHAM_BANAN obj = dt.APS_SOTHAM_BANAN.Where(x => x.ID == banan_qd_id).FirstOrDefault();
                    if (obj != null)
                ...
                else
                {
                    ...
                    APS_SOTHAM_QUYETDINH obj = dt.APS_SOTHAM_QUYETDINH.Where(x => x.ID == banan_qd_id).FirstOrDefault();
                    if (obj != null)
                    {
