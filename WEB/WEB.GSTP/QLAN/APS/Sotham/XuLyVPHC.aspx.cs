...
namespace WEB.GSTP.QLAN.APS.Sotham
{
    public partial class XuLyVPHC : System.Web.UI.Page
    {
        ...
        APS_XULY_VIPHAMHC obj = new APS_XULY_VIPHAMHC();
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                MaGiaiDoan = Convert.ToDecimal(ENUM_GIAIDOANVUAN.SOTHAM);
                VuAnID = (String.IsNullOrEmpty(Session[ENUM_LOAIAN.AN_PHASAN] + "")) ? 0 : Convert.ToDecimal(Session[ENUM_LOAIAN.AN_PHASAN] + "");
                hddMaGD.Value = MaGiaiDoan + "";
                hddVuAnID.Value = VuAnID + "";
                APS_XULY_VIPHAMHC oT = dt.APS_XULY_VIPHAMHC.Where(x => x.VUANID == VuAnID && x.MAGIAIDOAN == MaGiaiDoan).SingleOrDefault();
                if (oT != null)
                {
                    loadinfro();
                }
                APS_DON oDT = dt.APS_DON.Where(x => x.ID == VuAnID).FirstOrDefault();
                ...
                List<APS_SOTHAM_THULY> lstCount = dt.APS_SOTHAM_THULY.Where(x => x.DONID == VuAnID).ToList();
                if (lstCount.Count == 0)
                {
                ...
                List<APS_DON_THAMPHAN> lstTP = dt.APS_DON_THAMPHAN.Where(x => x.DONID == VuAnID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM).ToList();
                if (lstTP.Count == 0)
                {
                ...
                string Result = new APS_CHUYEN_NHAN_AN_BL().Check_NhanAn(VuAnID, StrMsg, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]));
                if (Result != "")
                {
                ...
                //    int countItem = dt.APS_DON_THAMPHAN.Count(s => s.THUKYID == CurrentUserId && s.DONID == VuAnID && s.MAVAITRO == "VTTP_GIAIQUYETSOTHAM");
                //    if (countItem > 0)
                //    {
                //        //được gán 
                //    }
        ...
        void loadinfro()
        {
            ...
            if (oT != null)
            {
                ...
                txtKhacPhuc.Text = (oT.APDUNG_KHACPHUCHAUQUA) + "";
            }
        }
        ...
        void Save()
        {
            ...
            obj.APDUNG_KHACPHUCHAUQUA = Get_Number(txtKhacPhuc.Text);
        ...
        protected void cmdUpdateBottom_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                obj = dt.APS_XULY_VIPHAMHC.Where(x => x.VUANID == mavuan && x.MAGIAIDOAN == MaGiaidoan).SingleOrDefault();
            }
            catch (Exception ex)
            {
            ...
            {
                obj = new APS_XULY_VIPHAMHC();
                Save();
                obj.NGAYTAO = DateTime.Now;
                obj.NGUOITAO = Session[ENUM_SESSION.SESSION_USERNAME] + "";
                dt.APS_XULY_VIPHAMHC.Add(obj);
                dt.SaveChanges();
            }
