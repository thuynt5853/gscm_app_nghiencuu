...
namespace WEB.GSTP.QLAN.APS.AnPhi
{
    public partial class Danhsach : System.Web.UI.Page
    {
        ...
        void CheckQuyen()
        {
            ...
            APS_DON_XULY donxl = dt.APS_DON_XULY.Where(x => x.DONID == ID).FirstOrDefault();
            if (donxl != null)
            {
            ...
            List<APS_DON_XULY> lts_donxl = dt.APS_DON_XULY.Where(x => x.DONID == ID).ToList();
            ...
            //List<APS_SOTHAM_THULY> lstTL = dt.APS_SOTHAM_THULY.Where(x => x.DONID == ID).ToList();
            //if (lstTL.Count > 0)
            //{
        ...
        void LoadDropDuongSu()
        {
            ...
            ddlDuongSu.DataSource = oDSBL.APS_DON_DUONGSU_BIENLAI_V2(DONID); ;
        ...
        private void LoadGrid_AnPhi()
        {
            ...
            DataTable oDT = oBL.APS_ANPHI_GETBYDONID(DONID, 0, pageindex, page_size);
            if (oDT.Rows.Count > 0)
            {
                APS_ANPHI ap = dt.APS_ANPHI.Where(x => x.DONID == DONID && x.SOBIENLAI != null && x.TINHTRANG == 0).FirstOrDefault();
                if (ap != null)
                {
        ...
        protected void rpt_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "ENABLE":
                    ...
                    {
                        oBLs.CHECK_TRANGTHAI_THULY(CurrID, "7");
                ...
                case "Sua":
                    ...
                    DataTable dataTable = dON_MIENANPHI_BL.GET_DON_MIENANPHI_BY_ANPHI_ID(CurrID, 7);
                ...
                case "Xoa":
                    try
                    {
                        ...
                        DataTable dataTableXoa = dON_MIENANPHI_BLs.GET_DON_MIENANPHI_BY_ANPHI_ID(CurrID, 7);
                ...
                case "DownloadAP_THA":
                    ...
                    //--------------------
                    byte[] V_FILE_DATA = oBL.File_Attach_Anphi_Return("7", CurrID, ref V_FILE_NAME);
                    string V_FILE_TYLE = "";
        ...
        {
            Response.Redirect(Cls_Comon.GetRootURL() + "/QLAN/APS/Sotham/ThuLy.aspx");
        }
        ...
        protected void ddlDuongSu_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (idDS != 0)
            {
                string current_id = Session[ENUM_LOAIAN.AN_PHASAN] + "";
                decimal DONID = Convert.ToDecimal(current_id);
                decimal idAP = dt.APS_ANPHI.Where(x => x.DONID == DONID && x.DUONGSU_ID == idDS).FirstOrDefault().ID;
