...
namespace WEB.GSTP.QLAN.APS.Hoso
{
    public partial class Tailieu : System.Web.UI.Page
    {
        ...
        private void LoadDropNguoiBanGiao()
        {
            ...
            {
                List<APS_DON_DUONGSU> lst = dt.APS_DON_DUONGSU.Where(x => x.DONID == DonID).ToList();
            ...
            {
                List<APS_DON_THAMGIATOTUNG> lst = dt.APS_DON_THAMGIATOTUNG.Where(x => x.DONID == DonID).ToList();
                if (lst != null && lst.Count > 0)
                {
                    ddlNguoiBanGiao.DataSource = lst;
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                APS_DON_TAILIEU donFile = new APS_DON_TAILIEU();
                decimal TLID = (String.IsNullOrEmpty(hddID.Value)) ? 0 : Convert.ToDecimal(hddID.Value);
                if (TLID > 0)
                    donFile = dt.APS_DON_TAILIEU.Where(x => x.ID == TLID).FirstOrDefault<APS_DON_TAILIEU>();
                else
                    donFile = new APS_DON_TAILIEU();
                try
                {
                ...
                if (hddID.Value == "" || hddID.Value == "0")
                {
                    ...
                    donFile.DONID = DonID;
                    dt.APS_DON_TAILIEU.Add(donFile);
                }
                else
                {
        ...
        {
            lbthongbao.Text = "";
            decimal DonViID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_PHASAN] + "");
            int Total = 0;
            APS_DON_BL donBL = new APS_DON_BL();
            DataTable tbl = donBL.APS_DON_BanGiaoTaiLieu_GETLIST(DonViID);
            if (tbl != null && tbl.Rows.Count > 0)
            {
        ...
        {
            APS_DON_TAILIEU oT = dt.APS_DON_TAILIEU.Where(x => x.ID == ID).FirstOrDefault();
            dt.APS_DON_TAILIEU.Remove(oT);
            dt.SaveChanges();
            hddPageIndex.Value = "1";
            LoadGridNgayBanGiao();
            ResetControls();
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            try
            {
                ...
                switch (e.CommandName)
                {
                    ...
                    case "Sua":
                        hddFilePath.Value = "";
                        APS_DON_TAILIEU bgtl = dt.APS_DON_TAILIEU.Where(x => x.ID == ID).FirstOrDefault();
                        if (bgtl != null)
                        {
                        ...
                        break;
                    case "Download":
                        APS_DON_TAILIEU oND = dt.APS_DON_TAILIEU.Where(x => x.ID == ID).FirstOrDefault();
                        if (oND.TENFILE != "")
                        {
