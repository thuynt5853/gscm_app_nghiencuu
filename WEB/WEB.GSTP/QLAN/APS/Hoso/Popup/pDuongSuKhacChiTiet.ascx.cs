    ...
    public partial class pDuongSuKhacChiTiet : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                string DONID = Convert.ToDecimal(Session[ENUM_LOAIAN.AN_PHASAN]).ToString();
                if (DONID != "" && DonID == 0)
                    DonID = Convert.ToDecimal(DONID);
                LoadGrid();
                string Result = new APS_CHUYEN_NHAN_AN_BL().Check_NhanAn(DonID, "",Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]));
                //if (Result != "")
                //{
        ...
        protected void dgList_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "Sua":
                    ...
                    string StrMsg = "PopupCenter('/QLAN/APS/Hoso/Popup/pDuongsuChiTiet.aspx?hsID=" + DonID.ToString() + "&bID=" + ND_id + " ','Thêm đương sự',950,450);";
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.APS_DON_DUONGSU_NOTDAIDIEN_CHITIET(IDChitiet);
        ...
        protected void lkThemNguoiTGTT_Click(object sender, EventArgs e)
        {
            ...
            string StrMsg = "PopupCenter('/QLAN/APS/Hoso/Popup/pDuongsuChiTiet.aspx?hsID=" + DonID.ToString() + "&bID=0','Thêm đương sự',950,450);";
            System.Web.UI.ScriptManager.RegisterStartupScript(this, this.GetType(), Guid.NewGuid().ToString(), StrMsg, true);
        }
    }
}