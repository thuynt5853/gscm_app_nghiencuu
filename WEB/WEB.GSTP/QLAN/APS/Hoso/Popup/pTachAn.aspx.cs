...
namespace WEB.GSTP.QLAN.APS.Hoso.Popup
{
    public partial class pTachAn : System.Web.UI.Page
    {
        ...
        {
            APS_DON_BL oBL = new APS_DON_BL(); //KHAI- APS
        ...
        private void Load_Grid_DonChiTiet()
        {
            ...
            DataTable oDT = oBL.GetDonGhep(vDonViID, DonGocID, 7); //KHAI- APS
        ...
        private void Load_Grid_DonKhac()
        {
            ...
            int page_size = 10,
                pageindex = Convert.ToInt32(hddPageIndex.Value);
            DONGHEP_BL oBL = new DONGHEP_BL();
            DataTable oDT = oBL.GetDonGhepDonKC(vDonViID, DonGocID, 7); //KHAI- APS
        ...
        protected void cmdTachAn_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridItem item in dgList.Items)
            {
                ...
                else
                {
                    ...
                    List<DON_DUONGSU_CHITIET> dsCT = dt.DON_DUONGSU_CHITIET
                        .Where(x => x.DONID == DonGocID && x.LOAIAN == 7 && x.DONCHITIETID == donID_DonTach)
                        .ToList(); //KHAI- APS
            ...
            if (!myCheck)
            {
                ...
                try
                {
                    APS_DON objDonGoc = dt.APS_DON.Where(x => x.ID == DonGocID).FirstOrDefault(); //KHAI- APS
                    APS_DON objDonMoi = objDonGoc;
                    ...
                    APS_DON_BL dsBL = new APS_DON_BL(); //KHAI- APS
                    objDonMoi.TT = dsBL.GETNEWTT((decimal)objDonMoi.TOAANID);
                    objDonMoi.MAVUVIEC = Session[ENUM_SESSION.SESSION_MADONVI] + "." + ENUM_LOAIVUVIEC.AN_PHASAN + "." + objDonMoi.TT.ToString(); //KHAI- APS
                    ...
                    List<DON_DUONGSU_CHITIET> duongSuCT = dt.DON_DUONGSU_CHITIET
                        .Where(x => x.DONID == DonGocID && x.LOAIAN == 7 && x.DONCHITIETID == DonDaiDienID)
                        .ToList(); //KHAI- APS

                    APS_DON_DUONGSU nguyenDonDaiDien = new APS_DON_DUONGSU(); //KHAI- APS
                    APS_DON_DUONGSU biDonDaiDien = new APS_DON_DUONGSU();

                    foreach (var item in duongSuCT)
                    {
                        APS_DON_DUONGSU objDS = dt.APS_DON_DUONGSU.Where(x => x.ID == item.DUONGSUID).FirstOrDefault(); //KHAI- APS
                    ...
                    {
                        objDonMoi.TENVUVIEC = nguyenDonDaiDien.TENDUONGSU + " - " + biDonDaiDien.TENDUONGSU + " - Yêu cầu tuyên bố phá sản";

                        dt.APS_DON.Add(objDonMoi); //KHAI- APS
                        dt.SaveChanges();

                        //Thêm thông tin vụ việc vào _DON_GIAIDOAN
                        GIAI_DOAN_BL donGD = new GIAI_DOAN_BL();
                        donGD.GAIDOAN_INSERT_UPDATE("7", objDonMoi.ID, 2, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), 0, 0, 0, 0); //KHAI- APS

                        //Update bảng _DON_XULY
                        APS_DON_XULY donXuLy = dt.APS_DON_XULY.Where(x => x.DON_CHITIETID == DonDaiDienID && x.DON_XULYID == DonGocID).FirstOrDefault(); //KHAI- APS
                        if (donXuLy != null)
                        {
                    ...
                    if (!myCheck)
                    {
                        ...
                        List<APS_DON_DUONGSU> listDS = dt.APS_DON_DUONGSU.Where(x => x.DONID == DonGocID).ToList(); //KHAI- APS
                        ...
                        foreach (DataGridItem item in dgList.Items)
                        {
                            ...
                            if (chonVuTach.Checked)
                            {
                                ...
                                if (donID_DonTach != DonDaiDienID)
                                {
                                    ...
                                    //Update DON_XULYID
                                    APS_DON_XULY donXuLy = dt.APS_DON_XULY.Where(x => x.DON_CHITIETID == donID_DonTach && x.DON_XULYID == DonGocID).FirstOrDefault(); //KHAI- APS
                                    if (donXuLy != null)
                                    {
                                    ...
                                    foreach (var itemDS in dsDuongSu)
                                    {
                                        ...
                                        if (!strDuongSuDonTachID.Contains(tempDuongSuID)) //Nếu đương sự không tồn tại trong danh sách DS của đơn khởi kiện mà không chọn để tách và ds đương dự của vụ gốc
                                        {
                                            ...
                                            APS_DON_DUONGSU donDuongSu = dt.APS_DON_DUONGSU.Where(x => x.ID == itemDS.DUONGSUID).FirstOrDefault(); //KHAI- APS
                                            if (donDuongSu != null)
                                            {
                                            ...
                                            //Cập nhật án phí
                                            APS_ANPHI anPhi = dt.APS_ANPHI.Where(x => x.DONID == DonGocID && x.DUONGSU_ID == itemDS.DUONGSUID).FirstOrDefault(); //KHAI- APS
                                            if (anPhi != null)
                                            {
                                            ...
                                            APS_DON_TAILIEU taiLieu = dt.APS_DON_TAILIEU
                                                .Where(x => x.DONID == DonGocID && x.NGUOIBANGIAO == itemDS.DUONGSUID)
                                                .FirstOrDefault(); //KHAI- APS
                                        ...
                                        else
                                        {
                                            ...
                                            APS_DON_DUONGSU donDuongSu = dt.APS_DON_DUONGSU.Where(x => x.ID == itemDS.DUONGSUID).FirstOrDefault(); //KHAI- APS
                                            APS_DON_DUONGSU objDS = donDuongSu;
                                            ...
                                            if (taiLieu != null)
                                            {
                                                APS_DON_TAILIEU taiLieuCopy = taiLieu; //KHAI- APS
                                                taiLieuCopy.DONID = objDonMoi.ID;
                                                taiLieuCopy.NGUOIBANGIAO = objDS.ID;
                                                dt.APS_DON_TAILIEU.Add(taiLieuCopy);
                                            }
                        ...
                        foreach (DataGridItem itemKhac in dgListKhac.Items)
                        {
                            ...
                            if (chonDonKhac.Checked)
                            {
                                ...
                                {
                                    APS_DON_DUONGSU donKhacDS = dt.APS_DON_DUONGSU
                                ...
                                {
                                    APS_DON_THAMGIATOTUNG donKhacTGTT = dt.APS_DON_THAMGIATOTUNG
                                        .Where(x => x.DONID == DonGocID && x.ID == donKhac.DUONGSUID)
                                        .FirstOrDefault(); //KHAI- APS
                        ...
                        //Copy sang đơn mới
                        List<APS_SOTHAM_HDXX> listHDXX = dt.APS_SOTHAM_HDXX.Where(x => x.DONID == DonGocID).ToList();
                        APS_SOTHAM_HDXX hdxx = new APS_SOTHAM_HDXX(); //KHAI- APS
                        if (listHDXX != null)
                        {
                        ...
                        APS_SOTHAM_THULY thuly = dt.APS_SOTHAM_THULY.Where(x => x.DONID == DonGocID).FirstOrDefault();
                        if (thuly != null)
                        {
                            APS_SOTHAM_THULY thuLyDonMoi = thuly; //KHAI- APS
                            thuLyDonMoi.DONID = objDonMoi.ID;
                            dt.APS_SOTHAM_THULY.Add(thuLyDonMoi);
                            dt.SaveChanges();
                        }

                        List<APS_DON_THAMPHAN> listThamPhan = dt.APS_DON_THAMPHAN.Where(x => x.DONID == DonGocID).ToList();
                        APS_DON_THAMPHAN thamPhan = new APS_DON_THAMPHAN(); //KHAI- APS
                        if (listThamPhan != null)
                        {
                            foreach (var item in listThamPhan)
                            {
                                ...
                                dt.APS_DON_THAMPHAN.Add(thamPhan);
                                dt.SaveChanges();
                            }
                        }

                        List<APS_SOTHAM_QUYETDINH> listQD = dt.APS_SOTHAM_QUYETDINH.Where(x => x.DONID == DonGocID).ToList();
                        APS_SOTHAM_QUYETDINH quyetDinh = new APS_SOTHAM_QUYETDINH(); //KHAI- APS
                        if (listQD != null)
                        {
                            foreach (var item in listQD)
                            {
                                ...
                                dt.APS_SOTHAM_QUYETDINH.Add(quyetDinh);
                                dt.SaveChanges();
                            }
                        }
                        ...
                        List<APS_DON_DUONGSU> dsDuongSuDonNhap = dt.APS_DON_DUONGSU.Where(x => x.DONID == objDonMoi.ID).ToList(); //KHAI- APS
                        string jsonDuongSu = ",";
                        ...
                        APS_DON_BL oBL = new APS_DON_BL(); //KHAI- APS
                        DataTable objVuViec = oBL.APS_DON_SEARCH(Session["CAP_XET_XU"] + "", objDonMoi.TOAANID.ToString(), "", "", objDonMoi.MAVUVIEC, "", objDonMoi.MAGIAIDOAN.ToString(), objDonMoi.TOAANID.ToString(), "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", 0,0, "", 1, 1, 1);
                        ...
                        //Lấy ds thẩm phán
                        List<APS_DON_THAMPHAN> dsThamPhan = dt.APS_DON_THAMPHAN.Where(x => x.DONID == objDonMoi.ID).ToList(); //KHAI- APS
                        string jsonThamPhan = ",";
                        ...
                        //Lấy ds thư kí
                        List<APS_SOTHAM_HDXX> dsSoThamHDXX = dt.APS_SOTHAM_HDXX.Where(x => x.DONID == objDonMoi.ID && x.MAVAITRO == "THUKY").ToList();
                        List<APS_PHUCTHAM_HDXX> dsPhucThamHDXX = dt.APS_PHUCTHAM_HDXX.Where(x => x.DONID == objDonMoi.ID && x.MAVAITRO == "THUKY").ToList();
                        string jsonThuKy = ",";
                        ...
                        APS_SOTHAM_THULY thuLy = dt.APS_SOTHAM_THULY.Where(x => x.DONID == objDonMoi.ID).FirstOrDefault(); //KHAI- APS
                        int ttTL = 0;
                        string ngayTL = "", soTL = "";
                        if (thuLy != null)
                        {
        ...
        protected void chkChon_CheckedChanged(object sender, EventArgs e)
        {
            foreach (DataGridItem item in dgList.Items)
            {
                ...
                if (chonVuTach.Checked)
                {
                    ...
                    {
                        APS_DON apsdon = dt.APS_DON.Where(x => x.ID == idDCT).FirstOrDefault();
                        if (apsdon.LOAIDON == 1)
                        {
                            chonDaiDien.Visible = true;
                        }
