...
namespace WEB.GSTP.QLAN.APS.Thamphan
{
    public partial class GiaiquyetSotham : System.Web.UI.Page
    {
        ...
        private void CheckQuyen(decimal DONID)
        {
            ...
            List<APS_SOTHAM_THULY> lstCount = dt.APS_SOTHAM_THULY.Where(x => x.DONID == DONID).ToList();
            if (lstCount.Count == 0)
            {
            ...
            APS_SOTHAM_BANAN oBA = dt.APS_SOTHAM_BANAN.Where(x => x.DONID == DONID).FirstOrDefault();
            if (oBA != null)
            {
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                APS_DON_THAMPHAN obj = dt.APS_DON_THAMPHAN.Where(x => x.ID == ID).FirstOrDefault();
                if (obj != null)
                {
                    ...
                    if (chuyenan != null)
                    {
                        ...
                        {
                            List<APS_SOTHAM_HDXX> NTHTT = dt.APS_SOTHAM_HDXX.Where(x => x.DONID == DONID).ToList();
                        ...
                        {
                            List<APS_SOTHAM_HDXX> NTHTT = dt.APS_SOTHAM_HDXX.Where(x => x.DONID == DONID && x.NGAYTAO > chuyenan.NGAYTAO).ToList();
                ...
                //APS_SOTHAM_HDXX hdxx = dt.APS_SOTHAM_HDXX.Where(x => x.DONID == DONID).FirstOrDefault();
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string current_id = Session[ENUM_LOAIAN.AN_PHASAN] + "", MaVaiTro = ddlVaitro.SelectedValue;
                decimal DONID = Convert.ToDecimal(current_id), CanBoID = Convert.ToDecimal(ddlThamphan.SelectedValue);

                APS_DON_THAMPHAN oND;
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    // Kiểm tra nếu thẩm phán đã được phân công rồi thì không phân lại nữa. Chọn thẩm phán khác
                    oND = dt.APS_DON_THAMPHAN.Where(x => x.DONID == DONID && x.CANBOID == CanBoID && x.MAVAITRO == MaVaiTro).FirstOrDefault();
                    if (oND != null)
                    {
                    ...
                    oND = new APS_DON_THAMPHAN();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.APS_DON_THAMPHAN.Where(x => x.ID == ID).FirstOrDefault();
                }
                oND.DONID = DONID;
                ...
                APS_SOTHAM_HDXX oTHTT = dt.APS_SOTHAM_HDXX.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_NGUOITIENHANHTOTUNG.THAMPHAN).FirstOrDefault<APS_SOTHAM_HDXX>();
                if (oTHTT == null)
                {
                    oTHTT = new APS_SOTHAM_HDXX();
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.APS_DON_THAMPHAN_GETBY(ID, ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETSOTHAM);
        ...
        {
            APS_DON_THAMPHAN oND = dt.APS_DON_THAMPHAN.Where(x => x.ID == id).FirstOrDefault();

            dt.APS_DON_THAMPHAN.Remove(oND);
            dt.SaveChanges();
            dgList.CurrentPageIndex = 0;
            LoadGrid();
            ResetControls();
        ...
        private bool CheckQDofThamPhan(decimal DonID, decimal TPID)
        {
            ...
            APS_SOTHAM_QUYETDINH obj_QD = dt.APS_SOTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.NGUOIKYID == TPID).FirstOrDefault<APS_SOTHAM_QUYETDINH>();
            if (obj_QD != null)
            {
