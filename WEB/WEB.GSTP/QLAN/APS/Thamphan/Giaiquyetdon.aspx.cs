    ...
    public partial class Giaiquyetdon : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                APS_CHUYEN_NHAN_AN objCN = dt.APS_CHUYEN_NHAN_AN.Where(x => x.MAP_VUANID_NEW == ID && x.TRUONGHOPGIAONHANID == 266).FirstOrDefault();
                if (objCN != null)
                {
                    APS_DON_XULY oldDonXuLy = dt.APS_DON_XULY.Where(x => x.DONID == objCN.VUANID && x.LOAIGIAIQUYET == 1).FirstOrDefault();
                    if (oldDonXuLy == null)
                    {
        ...
        public void LoadGrid()
        {
            ...
            string current_id = Session[ENUM_LOAIAN.AN_PHASAN] + "";
            decimal ID = Convert.ToDecimal(current_id);
            DataTable oDT = oBL.APS_DON_THAMPHAN_GETBY(ID, ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETDON);
        ...
        public void xoa(decimal id)
        {
            ...
            APS_DON_XULY xld = dt.APS_DON_XULY.Where(x => x.DONID == oND.DONID).FirstOrDefault();
            ...
            if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.DONID), 7, Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Thẩm phán giải quyết đơn Phá sản", "Xóa", json) == false)
            {
        ...
        protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                APS_DON_XULY oXL = dt.APS_DON_XULY.Where(x => x.DONID == DONID || x.DON_XULYID == DONID).FirstOrDefault();
                if (oXL != null)
                {
                    lbtXoa.Visible = false;
                }
