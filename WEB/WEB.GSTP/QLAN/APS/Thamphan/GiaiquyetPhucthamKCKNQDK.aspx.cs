...
namespace WEB.GSTP.QLAN.APS.Thamphan
{
    public partial class GiaiquyetPhucthamKCKNQDK : System.Web.UI.Page
    {
        ...
        void check_Quyen(decimal DONID)
        {
            
            APS_KCKN_PHUCTHAM_BL oBL = new APS_KCKN_PHUCTHAM_BL();
            DataTable oDT = oBL.APS_PHUCTHAM_KCKN_TGTT_GETLIST(DONID);
            ...
            APS_KCKN_PHUCTHAM_BL BL = new APS_KCKN_PHUCTHAM_BL();
            DataTable DT = BL.APS_KCKNQDK_PHUCTHAM_HDXX_GETLIST(DONID);
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //toancau-Tamnc-insert phân công thẩm phán vào người tiến hành tố tụng
                APS_KCKNQDK_PHUCTHAM_HDXX oTHTT = DataExtensions.GetAllWithClause<APS_KCKNQDK_PHUCTHAM_HDXX>($"DONID = {DONID} AND MAVAITRO = '{ENUM_NGUOITIENHANHTOTUNG.THAMPHAN}'").FirstOrDefault();
                if (oTHTT == null)
                {
                    oTHTT = new APS_KCKNQDK_PHUCTHAM_HDXX();
                    oTHTT.DONID = DONID;
        ...
        public void LoadGrid()
        {
            ...
            DataTable oDT = oBL.APS_DON_THAMPHAN_GETBY(ID, ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM);
        ...
        //    if (oBL.HISTORY_ALLDATA_BY_VUANID(Convert.ToDecimal(oND.DONID), Convert.ToDecimal(ENUM_LOAIVUVIEC_NUMBER.AN_PHASAN), Session[ENUM_SESSION.SESSION_USERID] + "", strUserName, "Thẩm phán giải quyết Phúc thẩm án Hôn nhân", "Xóa", json) == false)
        //    {
        //        lbthongbao.Text = "Xóa không thành công!";
        //        return;
        //    }//Ket thuc
        //    //Xoa Thẩm phán giải quyết Phúc thẩm
        //    dt.APS_DON_THAMPHAN.Remove(oND);
        //    dt.SaveChanges();
        //    dgList.CurrentPageIndex = 0;
        //    LoadGrid();
        ...
        private bool CheckQDofThamPhan(decimal DonID,decimal TPID)
        {
            ...
            APS_PHUCTHAM_QUYETDINH obj_QD = dt.APS_PHUCTHAM_QUYETDINH.Where(x => x.DONID == DonID && x.NGUOIKYID == TPID).FirstOrDefault<APS_PHUCTHAM_QUYETDINH>();
            if (obj_QD != null)
            {
