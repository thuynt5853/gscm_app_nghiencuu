...
namespace WEB.GSTP.QLAN.APS.Phuctham
{
    public partial class ThuLy : System.Web.UI.Page
    {
        ...
        private decimal SetNewSoThuLy()
        {
            ...
            string sothulymoi = oSTBL.GET_STL_NEW(DonViID, "APS_PT", ngaythuly).ToString();
        ...
        private void SetNewSoThongbao()
        {
            ...
            txtSothongbao.Text = oSTBL.GET_STBTL_NEW(DonViID, "APS_PT", ngaythongbao).ToString();
        ...
        private void CheckShowCommand(decimal DonID)
        {
            ...
            if (oT != null)
            {
                //Kiểm tra có đề nghị, kháng nghị hay không?
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                APS_PHUCTHAM_THULY oND;
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    oND = new APS_PHUCTHAM_THULY();
                }
                else
                {
                    decimal ID = Convert.ToDecimal(hddid.Value);
                    oND = dt.APS_PHUCTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
                ...
                oQLSTL.update_STPT_QUANLY_SOTHULY(3, 7, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), ((DateTime)oND.NGAYTHULY).ToString("dd/MM/yyyy", cul), Regex.Match(oND.SOTHULY, @"\d+").Value);
                ...
                if (hddid.Value == "" || hddid.Value == "0")
                {
                    APS_PHUCTHAM_BL oPTBL = new APS_PHUCTHAM_BL();
        ...
        {
            APS_PHUCTHAM_THULY oND = dt.APS_PHUCTHAM_THULY.Where(x => x.ID == ID).FirstOrDefault();
            if (oND != null)
            {
        ...
        protected void txtNgayThuLy_TextChanged(object sender, EventArgs e)
        {
            ...
            //    STPT_QUANLY_SOTHULY oQLSTL = new STPT_QUANLY_SOTHULY();
            //    DataTable dtStl = oQLSTL.get_STPT_QUANLY_SOTHULY(3, 7, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtNgaythuly.Text);
            //    if (dtStl != null)
            //    {
            ...
            //    string check_ngaythulycuoi_trongnam = oQLSTL.get_LATEST_DATE_IN_SOTHULY(3, 7, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_DONVIID]), txtNgaythuly.Text);
        ...
        protected void btnSaveLydoXoa_Insert(object sender, EventArgs e)
        {
            ...
            MenuPermission oPer = Cls_Comon.GetMenuPer(Request.FilePath, Convert.ToDecimal(Session[ENUM_SESSION.SESSION_USERID]));
            APS_PHUCTHAM_THULY oND = dt.APS_PHUCTHAM_THULY.Where(x => x.ID == ND_id).FirstOrDefault();
            ...
            if (oND != null)
            {
                ...
                APS_DON_THAMPHAN APS_DON_THAMPHAN = dt.APS_DON_THAMPHAN.Where(x => x.DONID == DONID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM).FirstOrDefault<APS_DON_THAMPHAN>();
                if (APS_DON_THAMPHAN != null)
                {
                    lbthongbao.Text = "Xóa không thành công! Do đang tồn tại thông tin phân công thẩm phán giải quyết.";
                    return;
                }
                APS_PHUCTHAM_THAMGIATOTUNG APS_PHUCTHAM_THAMGIATOTUNG = dt.APS_PHUCTHAM_THAMGIATOTUNG.Where(x => x.DONID == DONID).FirstOrDefault<APS_PHUCTHAM_THAMGIATOTUNG>();
                if (APS_PHUCTHAM_THAMGIATOTUNG != null)
                {
                    lbthongbao.Text = "Xóa không thành công! Do đang tồn tại người tham gia tố tụng.";
                    return;
                }
                APS_PHUCTHAM_HDXX APS_PHUCTHAM_HDXX = dt.APS_PHUCTHAM_HDXX.Where(x => x.DONID == DONID).FirstOrDefault<APS_PHUCTHAM_HDXX>();
