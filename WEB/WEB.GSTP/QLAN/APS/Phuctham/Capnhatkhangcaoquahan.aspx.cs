...
namespace WEB.GSTP.QLAN.APS.Phuctham
{
    public partial class Capnhatkhangcaoquahan : System.Web.UI.Page
    {
        ...
        {
            APS_SOTHAM_KHANGCAO oND = dt.APS_SOTHAM_KHANGCAO.Where(x => x.DONID == id).FirstOrDefault();
            if (oND != null)
            {
                oND.GQ_NGAY = null;
        ...
        protected void cmdTimkiem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    lbthongbao.Text = "Bạn phải nhập đề nghị từ ngày theo định dạng (dd/MM/yyyy)!";
                    txtTuNgay.Focus();
                    return;
                }
                if (txtDenNgay.Text != "" && Cls_Comon.IsValidDate(txtDenNgay.Text) == false)
                {
                    lbthongbao.Text = "Bạn phải nhập đề nghị đến ngày theo định dạng (dd/MM/yyyy)!";
                    txtDenNgay.Focus();
                    return;
                }
                if (txtTuNgay.Text != "" && txtDenNgay.Text != "")
                {
                    ...
                    {
                        lbthongbao.Text = "Bạn phải nhập đề nghị từ ngày phải nhỏ hơn đến ngày!";
                        txtDenNgay.Focus();
                        return;
                    }
                }
        ...
        protected void cmdGiaiQuyet_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                APS_DON don = dt.APS_DON.Where(x => x.ID == ID).FirstOrDefault<APS_DON>();
                if (don != null)
                {
        ...
        protected void cmdCapnhat_Click(object sender, EventArgs e)
        {
            ...
            APS_SOTHAM_KHANGCAO oT = dt.APS_SOTHAM_KHANGCAO.Where(x => x.DONID == VuViecID && x.GQ_TOAANID == DonViID).FirstOrDefault();
            if (oT != null)
            {
                //Cập nhật lại thông tin giải quyết đề nghị quá hạn
            ...
            {
                lbthongBaoUpdate.Text = "Không tìm thấy đề nghị cần giải quyết!";
            }
        }
    }
}