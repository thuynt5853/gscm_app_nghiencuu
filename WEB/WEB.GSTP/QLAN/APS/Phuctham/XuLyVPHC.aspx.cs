    ...
    public partial class XuLyVPHC : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                APS_DON oVAT = dt.APS_DON.Where(x => x.ID == VuAnID).FirstOrDefault();
                List<APS_PHUCTHAM_THULY> lstCount = dt.APS_PHUCTHAM_THULY.Where(x => x.DONID == VuAnID).ToList();
                ...
                List<APS_DON_THAMPHAN> lstTP = dt.APS_DON_THAMPHAN.Where(x => x.DONID == VuAnID && x.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM).ToList();
                if (lstTP.Count == 0)
                {
                ...
                //    int countItem = dt.APS_DON_THAMPHAN.Count(s => s.THUKYID == CurrentUserId && s.DONID == VuAnID && s.MAVAITRO == ENUM_VAITROTHAMPHAN.VTTP_GIAIQUYETPHUCTHAM);
                //    if (countItem > 0)
                //    {
                //        //được gán 
                //    }
