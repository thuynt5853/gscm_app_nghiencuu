...
{
    public partial class CauHinhThamPhan : System.Web.UI.Page
    {
        ...
        private void Load_Data()
        {
            ...
            tbl = pca.PCANN_DS_THAMPHAN_CAUHINH(toa_an_id, ten_thamphan);
            if (tbl.Rows.Count > 0)
            {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            decimal pcdon = 0;
            decimal pcan = 0;
            pca.PCANN_XOA_CAUHINH_TP(toa_an_id);
            foreach (DataGridItem Item in dgList.Items)
            {

                CheckBox chkDon = (CheckBox)Item.FindControl("chkDon");
                CheckBox chkAn = (CheckBox)Item.FindControl("chkAn");
                if (!chkDon.Checked || !chkAn.Checked)
                {
                    pcdon = chkDon.Checked == true ? 1 : 0;
                    pcan = chkAn.Checked == true ? 1 : 0;
                    pca.PCANN_CAUHINH_TP(Convert.ToDecimal(Item.Cells[0].Text), toa_an_id, pcdon, pcan);
                }
            }          
