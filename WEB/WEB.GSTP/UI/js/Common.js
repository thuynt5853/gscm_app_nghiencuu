...
function CheckChangeRadioButtonList_03_value(control_clientID, message_no_change) {
    var giatri = GetStatusRadioButtonList(control_clientID);
    if ((giatri != 0) && (giatri != 1) && (giatri != 2)) {
        alert(message_no_change);
        control_clientID.focus();
        return false;
    }
...
function PopupCenter(pageURL, title, w, h) {
    ...
    var targetWin = window.open(pageURL, "", 'titlebar=yes,toolbar=yes,scrollbars=yes,resizable=yes,width=' + w + ', height=' + h + ', top=' + top + ', left=' + left);
