...
namespace WEB.TP.Ajax
{
        public partial class SearchDanhmuc : System.Web.UI.Page
        {
            ...
            [WebMethod]
            public static string[] GetDMThiHanhAn(string q)
            {
                GSTPContext dt = new GSTPContext();
                List<string> arr = new List<string>();
                q = q.ToLower();
                List<DM_DONVITHIHANHAN> lst;
                if (q == " ")
                    lst = dt.DM_DONVITHIHANHAN.Where(x => x.HIEULUC == 1 && (x.SOCAP == 3 || x.SOCAP == 4)).OrderBy(y => y.ARRTHUTU).Take(20).ToList();
                else
                    lst = dt.DM_DONVITHIHANHAN.Where(x => x.HIEULUC == 1 && (x.SOCAP==3 || x.SOCAP == 4) && (x.MA_TEN.ToLower().Contains(q) || x.MA == q)).OrderBy(y => y.ARRTHUTU).Take(20).ToList();

                foreach (DM_DONVITHIHANHAN it in lst)
                {
                    arr.Add(it.ID.ToString() + "_" + it.MA_TEN);
                }
                return arr.ToArray();
