...
{
    public class AHN_CHUYEN_NHAN_AN_BL
    {
        GSTPContext dt = new GSTPContext();
        public DataTable AHN_CHUYEN_NHAN_AN_GETCHUYEN()
        {
            OracleParameter[] prm = new OracleParameter[]
            {
                new OracleParameter("CurReturn",OracleDbType.RefCursor, ParameterDirection.Output )
            };
            return Cls_Comon.GetTableByProcedurePaging("AHN_CHUYEN_NHAN_AN_GETCHUYEN", prm);
        }
        //public string Check_NhanAn(decimal DonID, string Message_Ex)
        //{
        //    string Result = "";
        //    AHN_CHUYEN_NHAN_AN ObjNhan = dt.AHN_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonID).FirstOrDefault();
        //    if (ObjNhan != null)
        //    {
        ...
        {
            string Result = "";
            AHN_CHUYEN_NHAN_AN ObjNhan = dt.AHN_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonID && x.TRANGTHAI == 1).OrderByDescending(x => x.ID).FirstOrDefault();
            if (ObjNhan != null)
            {
                if (toaAnId != null)
                {
                    ...
                    {
                        AHN_CHUYEN_NHAN_AN ObjNhanPTTDC = dt.AHN_CHUYEN_NHAN_AN.Where(x => x.VUANID == ObjNhan.MAP_VUANID_NEW && x.TRANGTHAI == 1 && x.ID > ObjNhan.ID).FirstOrDefault();
                        if (ObjNhanPTTDC != null)
                        {
                            AHN_DON donPT = dt.AHN_DON.Where(x => x.ID == ObjNhan.MAP_VUANID_NEW).FirstOrDefault();
        ...
        {
            string Result = "";
            AHN_CHUYEN_NHAN_AN ObjNhan = dt.AHN_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonID && (x.TRANGTHAI == 1 || x.TRANGTHAI == 0)).OrderByDescending(x => x.ID).FirstOrDefault();
            if (ObjNhan != null)
            {
                if (toaAnId != null)
                {
        ...
        {
            AHN_CHUYEN_NHAN_AN AHN_CHUYEN_NHAN_AN = dt.AHN_CHUYEN_NHAN_AN.Where(x => x.MAP_VUANID_NEW == donId).FirstOrDefault();
            if (AHN_CHUYEN_NHAN_AN != null)
                return AHN_CHUYEN_NHAN_AN.VUANID.Value;
            return donId;
        ...
        public bool UPDATE_NOIDUNG_CHUYENNHANAN(decimal ChuyenNhanId, string NoiDung, decimal DonId)
        {
            ...
            OracleCommand comm = new OracleCommand("PKG_STPT_AHN_GS.UPDATE_NOIDUNG_CHUYENNHANAN", conn);
        ...
        {
            OracleConnection conn = Cls_Comon.OpenConnection();
            OracleCommand comm = new OracleCommand("PKG_STPT_AHN_GS.COUNT_KCKN_TDC", conn);
            comm.CommandType = CommandType.StoredProcedure;
            comm.CommandTimeout = 60; // timeout 60 giây
            OracleCommandBuilder.DeriveParameters(comm);
            OracleTransaction tran = conn.BeginTransaction();
        ...
        {
            AHN_SOTHAM_THULY oThuLyST = dt.AHN_SOTHAM_THULY.Where(x => x.ID == ThuLyId).FirstOrDefault();
            AHN_CHUYEN_NHAN_AN oChuyenAn = dt.AHN_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonId && x.TOACHUYENID == ToaAnId).OrderByDescending(x => x.ID).FirstOrDefault();
        ...
        {
            AHN_SOTHAM_HDXX oB = dt.AHN_SOTHAM_HDXX.Where(x => x.ID == Id).FirstOrDefault();
            AHN_CHUYEN_NHAN_AN oChuyenAn = dt.AHN_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonId && x.TOACHUYENID == ToaAnId).OrderByDescending(x => x.ID).FirstOrDefault();
        ...
        {
            AHN_DON_THAMPHAN oB = dt.AHN_DON_THAMPHAN.Where(x => x.ID == Id).FirstOrDefault();
            AHN_CHUYEN_NHAN_AN oChuyenAn = dt.AHN_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonId && x.TOACHUYENID == ToaAnId).OrderByDescending(x => x.ID).FirstOrDefault();
            if (oChuyenAn != null && oChuyenAn.NGAYTAO >= oB.NGAYTAO)
                return true;
            return false;
