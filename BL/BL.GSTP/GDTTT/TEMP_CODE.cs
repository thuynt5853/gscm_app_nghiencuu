...
namespace BL.GSTP.GDTTT
{
    ...
    { GSTPContext dt = new GSTPContext();
        public DataTable GDTTT_DON_SEARCH(String V_NDBD_VALUE, String V_NDBD_TEXT, String V_DONVI_CHUYEN_ID, String V_TRANGTHAICHUYEN, String V_LOAI_VB, String V_SODEN_TU, String V_SODEN_DEN, String V_NGAY_FROM, String V_NGAY_TO, String V_NGUOI_GUI_BT,
          ...
          decimal vThamtravienID, decimal vLoaiCVID, DateTime? vNgayNhapTu, DateTime? vNgayNhapDen, decimal vIsDonGoc, decimal vIsTuHinh, decimal vLoaiAn, string vCVPC_So, string vCVPC_Ngay, string vCVPC_TenCQ, decimal vGuitoiCA_TA, decimal vLOAI_GDTTT, decimal PageIndex, decimal PageSize)
        {
            try
            {
                ...
                SQL += ",d.MADON,d.LOAIDON,NULL MADON_CC,d.SOHIEUDON,d.NGUOIGUI_HOTEN,d.SOTHUTUDON" +
                    ...
                    ",d.CV_SO,d.NGAYGHITRENDON,d.CV_NGAY,d.NGAY_HSKN, null NGAYGHITRENDON_CC" +
                    ...
                    ",TRIM(d.NOIDUNGTOMTAT) NOIDUNGTOMTAT" +
                    ",d.CD_TRALAI_LYDOKHAC,TB1_SO,TB1_NGAY" +
                    ",TB2_SO,TB2_NGAY,nsd.GHICHU BIDANH,d.CD_SOTOTRINH,d.CD_NGAYTOTRINH,c.HOTEN TENTHAMPHAN" +
                    ",QLS.SOVB,QLS.NGAYVB,NULL THAMPHAN_SONGAY,NULL TOTRINH_SONGAY,d.THAMPHANID" +
                    ",1 SODON,NULL TONG_SODON,NULL ARR_DON_IDS,d.CD_TA_TRANGTHAI,va.SOTHULYXXGDT,va.NGAYTHULYXXGDT,va.IsVienTruongKN" +
                    ",null IsShowNB,null IsShowTK,null GIAIQUYET" +
                    ",d.DONTRUNGID,null IsShowDDK,null IsShowCDDK,null IsShowTLMOI,null IsShowDATL,null IsThulyXX,NULL arrCongvan, null arrDonID" +
               ...
               "LEFT JOIN ( select sd.donid,so.* from QUANLY_SOPHATHANH so left join SOPHATHANH_DON sd on so.id = sd.SOPHATHANH_ID where so.maso = 'SoGXN')sph on sph.donid = d.id " +
               ...
               "left join (SELECT v.DONID,v.id,v.GHICHU FROM GDTTT_DON_CHUYEN_HISTORY v inner join ( SELECT TT.DONID,TT.ID FROM (  SELECT DONID,FIRST_VALUE(ID) OVER (PARTITION BY DONID ORDER BY NGAYTRA DESC) ID FROM  GDTTT_DON_CHUYEN_HISTORY)TT GROUP BY TT.DONID,TT.ID)t on t.id=v.id)tralai on d.id = tralai.donid " +
               ...
               " AND NVL(d.CD_TA_TRANGTHAI,0) IN (0,1)";//--19/03/2024 là một trường hợp khác để group những đơn không đủ điều kiện lại
                if (vIsThuLy != -1)
                {
                    ...
                    {
                        SQL += " AND (d.ISTHULY=1 and and d.ARR_DON_ID>0)";
                    }
                    if (vIsThuLy == 4)
                    {
                ...
                {
                    SQL += " AND (" +
                                 "(LOWER(D.BAQD_SO) LIKE LOWER('" + vSoBAQD + "')||'%'" + " AND TO_CHAR(D.BAQD_NGAYBA,'dd/MM/yyyy')='" + vNgayBAQD + "'" + ")" +
                                 " OR (LOWER(D.BAQD_SO_PT) LIKE LOWER('" + vSoBAQD + "')|| '%'" + " AND TO_CHAR(D.BAQD_NGAYBA_PT,'dd/MM/yyyy')='" + vNgayBAQD + "'" + ")" +
                                 " OR (LOWER(D.BAQD_SO_ST) LIKE LOWER('" + vSoBAQD + "')|| '%'" + " AND TO_CHAR(D.BAQD_NGAYBA_ST,'dd/MM/yyyy')='" + vNgayBAQD + "'" + ")" +
                                 " OR (LOWER(D.KN_SOQD) LIKE LOWER('" + vSoBAQD + "')|| '%'" + " AND TO_CHAR(D.KN_NGAY,'dd/MM/yyyy')='" + vNgayBAQD + "'" + ")" +
                                 ")";
                }
                else if (vSoBAQD != "" && vNgayBAQD == "" && vToaRaBAQD == 0)
                {
                    SQL += " AND (LOWER(D.BAQD_SO) LIKE LOWER('" + vSoBAQD + "')||'%'" +
                ...
                {
                    SQL += " AND (" +
                                 "(d.BAQD_TOAANID = " + vToaRaBAQD + "AND LOWER(D.BAQD_SO) LIKE LOWER('" + vSoBAQD + "')||'%'" + " AND TO_CHAR(D.BAQD_NGAYBA,'dd/MM/yyyy')='" + vNgayBAQD + "'" + ")" +
                                 " OR (d.BAQD_TOAANID_PT = " + vToaRaBAQD + "AND LOWER(D.BAQD_SO_PT) LIKE LOWER('" + vSoBAQD + "')|| '%'" + " AND TO_CHAR(D.BAQD_NGAYBA_PT,'dd/MM/yyyy')='" + vNgayBAQD + "'" + ")" +
                                 " OR (d.BAQD_TOAANID_ST = " + vToaRaBAQD + "AND LOWER(D.BAQD_SO_ST) LIKE LOWER('" + vSoBAQD + "')|| '%'" + " AND TO_CHAR(D.BAQD_NGAYBA_ST,'dd/MM/yyyy')='" + vNgayBAQD + "'" + ")" +
                                 " OR (LOWER(D.KN_SOQD) LIKE LOWER('" + vSoBAQD + "')|| '%'" + " AND TO_CHAR(D.KN_NGAY,'dd/MM/yyyy')='" + vNgayBAQD + "'" + ")" +
                                 ")";
                }
                else if (vSoBAQD != "" && vNgayBAQD == "" && vToaRaBAQD != 0)
                {
                    SQL += " AND (" +
                                 "(d.BAQD_TOAANID = " + vToaRaBAQD + "AND LOWER(D.BAQD_SO) LIKE LOWER('" + vSoBAQD + "')||'%')" +
                                 " OR (d.BAQD_TOAANID_PT = " + vToaRaBAQD + "AND LOWER(D.BAQD_SO_PT) LIKE LOWER('" + vSoBAQD + "')|| '%')" +
                                 " OR (d.BAQD_TOAANID_ST = " + vToaRaBAQD + "AND LOWER(D.BAQD_SO_ST) LIKE LOWER('" + vSoBAQD + "')|| '%')" +
                                 " OR (LOWER(D.KN_SOQD) LIKE LOWER('" + vSoBAQD + "')|| '%')" +
                                 ")";
                ...
                if (vSoCongVan != "")
                {
                    if (vNoiChuyen == 2)
                    {
                        SQL += " AND (LOWER(D.CD_SOCV) =LOWER('" + vSoCongVan + "') )";
                    }
                    if (vCD_TENDONVI == "CVPC")
                    {
                        SQL += " AND (LOWER(D.CD_SOCV) =LOWER('" + vSoCongVan + "') )";
                    }
                    if (vCD_TENDONVI == "TTR")
                    {
                        SQL += " AND (LOWER(D.CD_SOTOTRINH) =LOWER('" + vSoCongVan + "') )";
                    }
                }
                if (vNgayCongVan != "")
                {
                    if (vNoiChuyen == 2)
                    {
                        SQL += " AND (TO_CHAR(D.CD_NGAYCV,'dd/MM/yyyy')='" + vNgayCongVan + "' )";
                    }
                    if (vCD_TENDONVI == "CVPC")
                    {
                        SQL += " AND (TO_CHAR(D.CD_NGAYCV,'dd/MM/yyyy')='" + vNgayCongVan + "' )";
                    ...
                    if (vCD_TENDONVI == "TTR")
                    {
                        SQL += " AND (TO_CHAR(D.CD_NGAYTOTRINH,'dd/MM/yyyy')='" + vNgayCongVan + "' )";
                    }
                ...
                if (vTrangthai != -1)
                {
                    if (vTrangthai == 1)
                    {
                        SQL += " AND ( d.CD_TRANGTHAI in (1,2) )";
                    }
                    else
                    {
                        SQL += " AND ( d.CD_TRANGTHAI=" + vTrangthai + " )";
                    }
                }
                if (vNoiChuyen == 0)
                {
                ...
                if (vSoThuly != "")
                {
                    SQL += " AND (lower(d.TL_SO) like '%' || LOWER('" + vSoThuly + "') || '%')";
                }
                if (vArrSelectID != "")
                {
