...
{
    public class THA_TONGHOPHINHPHAT
    {
        ...
        public string Tonghophinhphat_ST(Decimal VVUANID, Decimal VBICAOID)
        {
            ...
            DataTable tbl = Cls_Comon.GetTableByProcedurePaging("PKG_THA_DIEULUAT.THA_TONGHOPHINHPHAT_ST", parameters);
        ...
        public string Tonghophinhphat_PT(Decimal VUANID, Decimal VBICAOID)
        {
            ...
            string HinhPhaTongHop = "";
            DataTable tbl = Cls_Comon.GetTableByProcedurePaging("PKG_THA_DIEULUAT.THA_TONGHOPHINHPHAT_PT", parameters);
            if (tbl != null && tbl.Rows.Count > 0)
            {
                HinhPhaTongHop = tbl.Rows[0][0].ToString();
            }
        ...
        public string Tonghophinhphat_Sosanh(Decimal VUANID, Decimal VBICAOID)
        {
            ...
            string HinhPhaTongHop = "";
            DataTable tbl = Cls_Comon.GetTableByProcedurePaging("PKG_THA_DIEULUAT.THA_TONGHOPHINHPHAT_SOSANH", parameters);
            if (tbl != null && tbl.Rows.Count > 0)
            {
                HinhPhaTongHop = tbl.Rows[0][0].ToString();
            }
        ...
        // MAGIAIDOAN dựa theo mã giai đoạn của bảng AHS_VUAN để xem bản án được ra ở giai đoạn nào
        public bool Capnhat_Tonghophinhphat_byVuanAndBicanid(Decimal MAGIAIDOAN, Decimal VUANID, Decimal BICANID, decimal DONVIID, String USERNAME)
        {
            try
            {
                if (MAGIAIDOAN == 2)
                {
                    ...
                    THA_TONGHOPHINHPHAT saveTHHP = DataExtensions.GetAllWithClause<THA_TONGHOPHINHPHAT>($"VUANID= {VUANID} AND BICAOID = {BICANID}").FirstOrDefault();

                    if (saveTHHP == null)
                    {
                        saveTHHP = new THA_TONGHOPHINHPHAT();
                        checkValidUpdateorinsert = 1;
                    }

                    saveTHHP.HINHPHAT_ST = saveTHHP.Tonghophinhphat_ST(VUANID, BICANID);
                
                    THA_SOTHAM_BANAN_DIEU_CHITIET toidanhchinh = DataExtensions.GetAllWithClause<THA_SOTHAM_BANAN_DIEU_CHITIET>($"BICANID= {BICANID} AND VUANID = {VUANID} AND ISMAIN = 1").FirstOrDefault();
                    string toidanh = "";
                ...
                if (MAGIAIDOAN == 3)
                {
                    ...
                    string toidanh = "";
                    THA_PHUCTHAM_BANAN_DIEU_CT toidanhchinh = DataExtensions.GetAllWithClause<THA_PHUCTHAM_BANAN_DIEU_CT>($"BICANID= {BICANID} AND ISMAIN = 1").FirstOrDefault();
