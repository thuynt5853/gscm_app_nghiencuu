...
namespace BL.GSTP.AHC
{
    public class AHC_DON_XULY_BL
    {
        public DataTable GetByDonID(decimal donID, int PageIndex, int PageSize)
        {
            ...
            DataTable tbl = Cls_Comon.GetTableByProcedurePaging("AHC_DON_XULY_GETBYDONID", parameters);
            return tbl;
        }
        public DataTable GetTinhTrangVuViec(decimal vu_viec_id)
        {
            string ma_loai = ENUM_LOAIAN.AN_HANHCHINH;
            OracleParameter[] parameters = new OracleParameter[] {
                                                                        new OracleParameter("vu_viec_id",vu_viec_id),
                                                                        new OracleParameter("ma_loai_vu_viec",ma_loai),
                                                                        new OracleParameter("curReturn",OracleDbType.RefCursor, ParameterDirection.Output )
                                                                      };
            DataTable tbl = Cls_Comon.GetTableByProcedurePaging("AHC_Don_GetTinhTrang", parameters);
            return tbl;
        }
        public DataTable CHECK_AHC_DON_DUONGSU_ANPHI(decimal? DONID, decimal? DONCHITIETID, decimal ISDONCHITIET)
        {
            ...
            return Cls_Comon.GetTableByProcedurePaging("PKG_DUONGSU_ANPHI.CHECK_AHC_DON_DUONGSU_ANPHI", prm);
        }

        public DataTable CHECK_AHC_DON_DUONGSU_ANPHI_V2(decimal? DONID, decimal? DONCHITIETID, decimal ISDONCHITIET)
        {
            OracleParameter[] prm = new OracleParameter[]
            {
                new OracleParameter("vDONID", DONID),
            ...
            return Cls_Comon.GetTableByProcedurePaging("PKG_DUONGSU_ANPHI.CHECK_AHC_DON_DUONGSU_ANPHI_V2", prm);
        }
