...
{
    public class AHC_CHUYEN_NHAN_AN_BL
    {
        GSTPContext dt = new GSTPContext();
        public DataTable AHC_CHUYEN_NHAN_AN_GETCHUYEN()
        {
            OracleParameter[] prm = new OracleParameter[]
            {
                new OracleParameter("CurReturn",OracleDbType.RefCursor, ParameterDirection.Output )
            };
            return Cls_Comon.GetTableByProcedurePaging("AHC_CHUYEN_NHAN_AN_GETCHUYEN", prm);
        }
        //public string Check_NhanAn(decimal DonID, string Message_Ex)
        //{
        //    string Result = "";
        //    AHC_CHUYEN_NHAN_AN ObjNhan = dt.AHC_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonID).FirstOrDefault();
        //    if (ObjNhan != null)
        //    {
        //        if (ObjNhan.TRANGTHAI == 1) // Đã nhận
        //        {
        //            Result = "Án đã được đã được ";
        ...
        {
            string Result = "";
            AHC_CHUYEN_NHAN_AN ObjNhan = dt.AHC_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonID && x.TRANGTHAI == 1).OrderByDescending(x => x.ID).FirstOrDefault();
            if (ObjNhan != null)
            {
                if (toaAnId != null)
                {
                    ...
                    {
                        AHC_CHUYEN_NHAN_AN ObjNhanPTTDC = dt.AHC_CHUYEN_NHAN_AN.Where(x => x.VUANID == ObjNhan.MAP_VUANID_NEW && x.TRANGTHAI == 1 && x.ID > ObjNhan.ID).FirstOrDefault();
                        if (ObjNhanPTTDC != null)
                        {
                            AHC_DON donPT = dt.AHC_DON.Where(x => x.ID == ObjNhan.MAP_VUANID_NEW).FirstOrDefault();
        ...
        {
            string Result = "";
            AHC_CHUYEN_NHAN_AN ObjNhan = dt.AHC_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonID && (x.TRANGTHAI == 1 || x.TRANGTHAI == 0)).OrderByDescending(x => x.ID).FirstOrDefault();
            if (ObjNhan != null)
            {
                if (toaAnId != null)
                {
        ...
        {
            AHC_CHUYEN_NHAN_AN chuyenNhanAn = dt.AHC_CHUYEN_NHAN_AN.Where(x => x.MAP_VUANID_NEW == donId).FirstOrDefault();
            if (chuyenNhanAn != null)
                return chuyenNhanAn.VUANID.Value;
            return donId;
        ...
        public bool UPDATE_NOIDUNG_CHUYENNHANAN(decimal ChuyenNhanId, string NoiDung, decimal DonId)
        {
            ...
            OracleCommand comm = new OracleCommand("PKG_STPT_AHC_GS.UPDATE_NOIDUNG_CHUYENNHANAN", conn);
        ...
        {
            OracleConnection conn = Cls_Comon.OpenConnection();
            OracleCommand comm = new OracleCommand("PKG_STPT_AHC_GS.COUNT_KCKN_TDC", conn);
            comm.CommandType = CommandType.StoredProcedure;
            comm.CommandTimeout = 60; // timeout 60 giây
            OracleCommandBuilder.DeriveParameters(comm);
            OracleTransaction tran = conn.BeginTransaction();
        ...
        {
            AHC_SOTHAM_THULY oThuLyST = dt.AHC_SOTHAM_THULY.Where(x => x.ID == ThuLyId).FirstOrDefault();
            AHC_CHUYEN_NHAN_AN oChuyenAn = dt.AHC_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonId && x.TOACHUYENID == ToaAnId).OrderByDescending(x => x.ID).FirstOrDefault();
        ...
        {
            AHC_SOTHAM_HDXX oB = dt.AHC_SOTHAM_HDXX.Where(x => x.ID == Id).FirstOrDefault();
            AHC_CHUYEN_NHAN_AN oChuyenAn = dt.AHC_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonId && x.TOACHUYENID == ToaAnId).OrderByDescending(x => x.ID).FirstOrDefault();
        ...
        {
            AHC_DON_THAMPHAN oB = dt.AHC_DON_THAMPHAN.Where(x => x.ID == Id).FirstOrDefault();
            AHC_CHUYEN_NHAN_AN oChuyenAn = dt.AHC_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonId && x.TOACHUYENID == ToaAnId).OrderByDescending(x => x.ID).FirstOrDefault();
            if (oChuyenAn != null && oChuyenAn.NGAYTAO >= oB.NGAYTAO)
                return true;
            return false;
