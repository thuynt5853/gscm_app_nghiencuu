...
{
    public class ALD_CHUYEN_NHAN_AN_BL
    {
        GSTPContext dt = new GSTPContext();
        public DataTable ALD_CHUYEN_NHAN_AN_GETCHUYEN()
        {
            OracleParameter[] prm = new OracleParameter[]
            {
                new OracleParameter("CurReturn",OracleDbType.RefCursor, ParameterDirection.Output )
            };
            return Cls_Comon.GetTableByProcedurePaging("ALD_CHUYEN_NHAN_AN_GETCHUYEN", prm);
        }
        //public string Check_NhanAn(decimal DonID, string Message_Ex)
        //{
        //    string Result = "";
        //    ALD_CHUYEN_NHAN_AN ObjNhan = dt.ALD_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonID).FirstOrDefault();
        //    if (ObjNhan != null)
        //    {
        ...
        {
            string Result = "";
            ALD_CHUYEN_NHAN_AN ObjNhan = dt.ALD_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonID && x.TRANGTHAI == 1).OrderByDescending(x => x.ID).FirstOrDefault();
            if (ObjNhan != null)
            {
                if (toaAnId != null)
                {
                    ...
                    {
                        ALD_CHUYEN_NHAN_AN ObjNhanPTTDC = dt.ALD_CHUYEN_NHAN_AN.Where(x => x.VUANID == ObjNhan.MAP_VUANID_NEW && x.TRANGTHAI == 1 && x.ID > ObjNhan.ID).FirstOrDefault();
                        if (ObjNhanPTTDC != null)
                        {
                            ALD_DON donPT = dt.ALD_DON.Where(x => x.ID == ObjNhan.MAP_VUANID_NEW).FirstOrDefault();
        ...
        {
            string Result = "";
            ALD_CHUYEN_NHAN_AN ObjNhan = dt.ALD_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonID && (x.TRANGTHAI == 1 || x.TRANGTHAI == 0)).OrderByDescending(x => x.ID).FirstOrDefault();
            if (ObjNhan != null)
            {
                if (toaAnId != null)
                {
        ...
        {
            ALD_CHUYEN_NHAN_AN chuyenNhanAn = dt.ALD_CHUYEN_NHAN_AN.Where(x => x.MAP_VUANID_NEW == donId).FirstOrDefault();
        ...
        public bool UPDATE_NOIDUNG_CHUYENNHANAN(decimal ChuyenNhanId, string NoiDung, decimal DonId)
        {
            ...
            OracleCommand comm = new OracleCommand("PKG_STPT_ALD_GS.UPDATE_NOIDUNG_CHUYENNHANAN", conn);
        ...
        {
            OracleConnection conn = Cls_Comon.OpenConnection();
            OracleCommand comm = new OracleCommand("PKG_STPT_ALD_GS.COUNT_KCKN_TDC", conn);
            comm.CommandType = CommandType.StoredProcedure;
            comm.CommandTimeout = 60; // timeout 60 giây
            OracleCommandBuilder.DeriveParameters(comm);
            OracleTransaction tran = conn.BeginTransaction();
        ...
        {
            ALD_SOTHAM_THULY oThuLyST = dt.ALD_SOTHAM_THULY.Where(x => x.ID == ThuLyId).FirstOrDefault();
            ALD_CHUYEN_NHAN_AN oChuyenAn = dt.ALD_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonId && x.TOACHUYENID == ToaAnId).OrderByDescending(x => x.ID).FirstOrDefault();
        ...
        {
            ALD_SOTHAM_HDXX oB = dt.ALD_SOTHAM_HDXX.Where(x => x.ID == Id).FirstOrDefault();
            ALD_CHUYEN_NHAN_AN oChuyenAn = dt.ALD_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonId && x.TOACHUYENID == ToaAnId).OrderByDescending(x => x.ID).FirstOrDefault();
        ...
        {
            ALD_DON_THAMPHAN oB = dt.ALD_DON_THAMPHAN.Where(x => x.ID == Id).FirstOrDefault();
            ALD_CHUYEN_NHAN_AN oChuyenAn = dt.ALD_CHUYEN_NHAN_AN.Where(x => x.VUANID == DonId && x.TOACHUYENID == ToaAnId).OrderByDescending(x => x.ID).FirstOrDefault();
            if (oChuyenAn != null && oChuyenAn.NGAYTAO >= oB.NGAYTAO)
                return true;
            return false;
